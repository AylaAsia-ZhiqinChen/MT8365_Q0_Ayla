...
#define MTK_DEFAULT_STARTUP_TIMEOUT_MS 12000
...
static const uint32_t COMMAND_TIMEOUT_RESTART_MS = 20000;
...
static void command_timed_out(void* original_wait_entry) {
  ...
  osi_free(bt_hdr);
  LOG_ERROR(LOG_TAG, "%s: Setting a %ds timer to restart.", __func__, COMMAND_TIMEOUT_RESTART_MS / 1000);
