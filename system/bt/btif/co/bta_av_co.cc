...
class BtaAvCoPeer {
 public:
  ...
  BtaAvCoSep sinks[BTAV_A2DP_CODEC_INDEX_MAX];    // Peer supported sinks
  BtaAvCoSep sources[BTAV_A2DP_CODEC_INDEX_MAX];  // Peer supported sources
...
class BtaAvCo {
 public:
  ...
  /** M: Used to check if the codec is disable. @{ */
  /**
   * Get the peer by index.
   *
   * @param index for the peer in BtaAvCo.
  */
  BtaAvCoPeer* GetPeerByIndex(uint8_t index);
...
/** M: Used to check if the codec is disable. @{ */
   ...
   * @param index for the peer in BtaAvCo.
  */
BtaAvCoPeer* BtaAvCo::GetPeerByIndex(uint8_t index) {
  return FindPeer(BTA_AV_CO_AUDIO_INDEX_TO_HANDLE(index));
}
/** @} */
...
bool bta_av_co_audio_init(btav_a2dp_codec_index_t codec_index,
  ...
  /** M: Remove the disable codecs @{ */
  // We assumed that all the peers have the same local codecs,
  // so just check the first peers_[0].
  BtaAvCoPeer* peer = bta_av_co_cb.GetPeerByIndex(0);
  if (peer != nullptr) {
      ...
      {
        list_codec = peer->GetCodecs()->orderedSourceCodecs();
      }
      else
      {
        list_codec = peer->GetCodecs()->orderedSinkCodecs();
      }
      bool found = false;
      ...
      if (!found) {
        APPL_TRACE_DEBUG("%s: codec_index=%d not found", __func__, codec_index);
        return false;
      }
  }
  /** @} */
...
#if defined(MTK_PTS_AV_TEST) && (MTK_PTS_AV_TEST == TRUE)  //pts test
/*******************************************************************************
 **
...
void bta_av_co_pts_force_reconfig(void)
{
  ...
  {
    p_peer = bta_av_co_cb.GetPeerByIndex(index);
    //BtaAvCoSep* p_source;
    if (p_peer->opened)
    {
      if (kContentProtectEnabled && p_peer->ContentProtectActive()) {
        num_protect = AVDT_CP_INFO_LEN;
      }
      for (snk_index = 0; snk_index < BTAV_A2DP_CODEC_INDEX_MAX; snk_index++) {
        BtaAvCoSep* p_source = &p_peer->sources[snk_index];
        if (p_source) {
          BTA_AvReconfig(p_peer->BtaAvHandle(), true, p_source->sep_info_idx,
                         p_peer->codec_config, num_protect, bta_av_co_cp_scmst);
          break;
        }
      }
      ...
      APPL_TRACE_DEBUG("[pts] %s: peer %s not open, index: %d ",
                       __func__, p_peer->addr.ToString().c_str(), index);
    }
  }
}
