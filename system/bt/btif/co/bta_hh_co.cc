...
static int uhid_read_event(btif_hh_device_t* p_dev) {
  ...
  switch (ev.type) {
    ...
    case UHID_OUTPUT:
      ...
      /* The following patch in "#else" is only for some special devies,not for every device.
           Some output report exchanges will be stopped unexpected.*/
#else
  ...
  for (i = 0; i < BTIF_HH_MAX_HID; i++) {
    ...
    if (p_dev->dev_status != BTHH_CONN_STATE_UNKNOWN &&
...
#if defined(MTK_COMMON) && (MTK_COMMON == TRUE) //reconnect too quick
        if (BTHH_CONN_STATE_DISCONNECTING == p_dev->dev_status)
        {
            APPL_TRACE_DEBUG("%s: semaphore_wait_timeout hh status 500ms fd= %d",
        ...
        else
        {
            ...
            APPL_TRACE_DEBUG("%s: semaphore_wait_timeout hh status 1ms fd= %d", __func__, (p_dev->status_sema)->fd);
            if (p_dev->sema_inited == TRUE)
            {
...
#if defined(MTK_COMMON) && (MTK_COMMON == TRUE) //reconnect too quick
      APPL_TRACE_WARNING("%s: set disconnecting dev_handle = %d", __func__, dev_handle);
