...
extern "C"
{
  const btmesh_interface_t *btif_mesh_get_interface();
}
#endif
...
static int init(bt_callbacks_t* callbacks, bool start_restricted,
  ...
  LOG_INFO(LOG_TAG, "MTK_INFO: %s", MTK_INFO);
#endif
...
#if defined(MTK_ANDROID_MESH) && (MTK_ANDROID_MESH == TRUE)
  if (is_profile(profile_id, BT_PROFILE_MESH_ID))
    return btif_mesh_get_interface();
#endif
