...
EXPORT_SYMBOL void btif_do_in_jni_thread(evt_handler handler, void *evt)
{
    do_in_jni_thread(base::Bind(handler, evt));
}
#endif
...
static void execute_storage_request(uint16_t event, char* p_param) {
  ...
  switch (event) {
    case BTIF_CORE_STORAGE_ADAPTER_WRITE: {
      ...
      BTIF_TRACE_EVENT("type: %d, len %d, %p", p_prop->type, p_prop->len,
                       p_prop->val);
...
bt_status_t btif_set_adapter_property(const bt_property_t* property) {
  ...
  BTIF_TRACE_EVENT("btif_set_adapter_property type: %d, len %d, %p",
