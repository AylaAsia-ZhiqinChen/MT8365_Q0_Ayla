...
typedef struct {
  ...
  bool launch_cmd_pending; /* true: getcap/regvolume */
#endif
} btif_rc_device_cb_t;
...
void handle_rc_ctrl_features_all(btif_rc_device_cb_t* p_dev) {
  ...
  BTIF_TRACE_DEBUG("%s: peer_tg_features: 0x%x, rc_features_processed=%d, connected=%d, peer_is_src:%d",
  ...
  if (btif_av_is_connected(p_dev->rc_addr)) {
    ...
    BTIF_TRACE_DEBUG("%s: %s is not connected, pending",
      __func__, p_dev->rc_addr.ToString().c_str());
...
void btif_rc_check_pending_cmd (const RawAddress& peer_address) {
    ...
    if (p_dev->launch_cmd_pending && p_dev->rc_connected) {
      if (btif_av_peer_is_sink(p_dev->rc_addr)) {
...
void handle_rc_ctrl_features(btif_rc_device_cb_t* p_dev) {
  ...
  {
    return handle_rc_ctrl_features_all(p_dev);
  }
#endif
...
void handle_rc_connect(tBTA_AV_RC_OPEN* p_rc_open) {
  ...
  BTIF_TRACE_DEBUG("%s ct_feature=0x%x, tg_feature=0x%x ",
      __func__, p_dev->peer_ct_features, p_dev->peer_tg_features);
#endif
...
static void btif_rc_pts_test_thread(UNUSED_ATTR void *context) {
  ...
  */
  ...
  while(1) {
    BTIF_TRACE_VERBOSE("[pts] *********enter %s()", __func__);
    if ((osi_property_get("bluetooth.pts.avrcp.auto", value, "off"))
      ...
      const char * const pCmdTableStr[] = {"PLAY",
                              "PAUSE",
                              "STOP",
                              "REWIND",
                              "FAST_FOR",
      ...
      uint8_t i = 0, cmdSize = sizeof(cmdTable);
      for(i = 0; i < cmdSize; i++) {
        BTIF_TRACE_DEBUG("[pts] run avrcp cmd 0x%x(%s)",cmdTable[i], pCmdTableStr[i]);
        send_passthrough_cmd(rc_addr, cmdTable[i], AVRC_STATE_PRESS);
        send_passthrough_cmd(rc_addr, cmdTable[i], AVRC_STATE_RELEASE);
      ...
      && (strcmp(value, "00"))) {
      uint8_t* key_code = (uint8_t*)osi_calloc(2);
      ...
      send_passthrough_cmd(rc_addr, *key_code, AVRC_STATE_PRESS);
      send_passthrough_cmd(rc_addr, *key_code, AVRC_STATE_RELEASE);
      ...
      /* currently only one avrcp connection to be concerned, for future multi-link case need modify this case */
      if (BT_STATUS_SUCCESS == tran_status && NULL != p_transaction){
          AVRC_UnitCmd(btif_rc_cb.rc_multi_cb[0].rc_handle, p_transaction->lbl);
      }
      osi_property_set("bluetooth.pts.avrcp.uni_cmd", "false");
    } else if ((osi_property_get("bluetooth.pts.avrcp.subuni_cmd", value, "9"))
        && (strcmp(value, "9"))) {
      // page 0-7
      BTIF_TRACE_DEBUG("[pts] send SUB_UNIT INFO cmd");
      int page;
      sscanf(value, "%x", &page);
      rc_transaction_t *p_transaction = NULL;
      bt_status_t tran_status = get_transaction(&p_transaction);
      ...
      /* currently only one avrcp connection to be concerned, for future multi-link case need modify this case */
      if (BT_STATUS_SUCCESS == tran_status && NULL != p_transaction){
          AVRC_SubCmd(btif_rc_cb.rc_multi_cb[0].rc_handle, p_transaction->lbl, page);
      }
      osi_property_set("bluetooth.pts.avrcp.subuni_cmd", "9");
      ...
      get_element_attr_rsp(rc_addr, num_attr, attrs);
      osi_property_set("bluetooth.pts.avrcp.attr_rsp", "false");
    } else if ((osi_property_get("bluetooth.pts.avrcp.bd_addr", value, "0"))
        && (strcmp(value, "0"))) {
      int addr[6], i = 0;
      ...
      BTIF_TRACE_DEBUG("%s: rc_addr: %s", __func__,rc_addr.ToString().c_str());
      osi_property_set("bluetooth.pts.avrcp.bd_addr", "0");
    } else if ((osi_property_get("bluetooth.pts.avrcp.setvolume", value, "0"))
        && (strcmp(value, "0"))) {
      int volume;
      sscanf(value, "%x", &volume);
      BTIF_TRACE_DEBUG("[pts] set absolute volume");
      set_volume(volume);
      osi_property_set("bluetooth.pts.avrcp.setvolume", "0");
    } else if ((osi_property_get("bluetooth.pts.avrcp.get_folder_item", value, "0"))
        && (strcmp(value, "0"))) {
      int items[2];
      sscanf(value, "%x:%x", &items[0], &items[1]);
      BTIF_TRACE_DEBUG("[pts] get_folder_item");
      get_player_list_cmd(rc_addr, items[0], items[1]);
      osi_property_set("bluetooth.pts.avrcp.get_folder_item", "0");
    } else if ((osi_property_get("bluetooth.pts.avrcp.set_addressed_player", value, "0"))
        && (strcmp(value, "0"))) {
      int id;
      sscanf(value, "%x", &id);
      BTIF_TRACE_DEBUG("[pts] set_addressed_player");
      set_addressed_player_cmd(rc_addr, id);
      osi_property_set("bluetooth.pts.avrcp.set_addressed_player", "0");
    } else {
      //unknow
      BTIF_TRACE_VERBOSE("[pts] #####enter %s()", __func__);
    }
...
static bt_status_t init(btrc_callbacks_t* callbacks) {
  ...
  MTK_PTS_TEST_IS_ENABLE {
    BTIF_TRACE_DEBUG("[pts] bluetooth.pts.thread_enable");
...
#if defined(MTK_PTS_AV_TEST) && (MTK_PTS_AV_TEST == TRUE)
  MTK_PTS_TEST_IS_ENABLE {
    BTIF_TRACE_DEBUG("[pts] bluetooth.pts.rc_thread_enable");
    btif_rc_pts_test_init();
  }
#endif
...
static void btif_rc_play_status_timeout_handler(UNUSED_ATTR uint16_t event,
  ...
  if (p_dev == NULL) {
    BTIF_TRACE_ERROR("%s : no device found for handle %d",
...
static void btif_rc_play_status_timer_timeout(void* data) {
  ...
  BTIF_TRACE_ERROR("%s : called with handle: %d", __func__, rc_handle.handle);
...
#if defined(MTK_A2DP_SRC_SINK_BOTH) && (MTK_A2DP_SRC_SINK_BOTH == TRUE)
  if (p_rsp->event_id == AVRC_EVT_VOLUME_CHANGE) {
    BTIF_TRACE_ERROR("%s: legacy TG dont handle absolute volume change. leave it to new avrcp", __func__);
    return;
  }
#endif
  ...
  if ((AVRC_OP_VENDOR == pmeta_msg->p_msg->hdr.opcode) &&
    ...
    if (status != AVRC_STS_NO_ERROR) {
      ...
      if ( AVRC_PDU_GET_CAPABILITIES == avrc_cmd.pdu ||
        AVRC_PDU_LIST_PLAYER_APP_ATTR == avrc_cmd.pdu ||
        AVRC_PDU_LIST_PLAYER_APP_VALUES == avrc_cmd.pdu ||
        AVRC_PDU_GET_CUR_PLAYER_APP_VALUE == avrc_cmd.pdu ||
        AVRC_PDU_GET_PLAYER_APP_ATTR_TEXT == avrc_cmd.pdu ||
        AVRC_PDU_GET_PLAYER_APP_VALUE_TEXT == avrc_cmd.pdu ||
