...
void btif_gattc_open_impl(int client_if, RawAddress address, bool is_direct,
  ...
  if ( !btif_get_device_type(address, &device_type) &&
      device_type == 0 && transport_p == GATT_TRANSPORT_LE ) {
      // unknown device_type but want to connect LE transport. Add it as a BLE device first
    device_type = BT_DEVICE_TYPE_BLE;
    addr_type = BLE_ADDR_RANDOM;
    BTA_DmAddBleDevice(address, addr_type, device_type);
    BTIF_TRACE_DEBUG("AddBleDevice with random addr_type");
  }
...
bt_status_t btif_gattc_open(int client_if, const RawAddress& bd_addr,
  ...
  CHECK_BTGATT_INIT();
#ifdef MTK_MESH_SUPPORT
  meshif_ble_addr_t addr;
  memcpy(addr.addr, bd_addr.address, sizeof(bd_addr.address));

  if (meshif_gatt_is_connected(&addr) == true) {
    BTIF_TRACE_WARNING("%s: Mesh device GATT already connected, should not connect!", __func__);
