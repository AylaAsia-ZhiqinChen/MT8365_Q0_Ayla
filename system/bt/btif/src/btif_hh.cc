...
btif_hh_device_t* btif_hh_find_disconnecting_dev_by_handle(uint8_t handle) {
  uint32_t i;
  for (i = 0; i < BTIF_HH_MAX_HID; i++) {
      if (btif_hh_cb.devices[i].dev_status == BTHH_CONN_STATE_DISCONNECTING &&
      btif_hh_cb.devices[i].dev_handle == handle) {
      return &btif_hh_cb.devices[i];
    }
  }
...
bt_status_t btif_hh_virtual_unplug(const RawAddress* bd_addr) {
  ...
  if ((p_dev != NULL) && (p_dev->dev_status == BTHH_CONN_STATE_CONNECTED) &&
    ...
    BTIF_TRACE_ERROR("%s: Virtual unplug not suported, disconnecting device", __func__);
    /* start the timer */
    btif_hh_start_vup_timer(bd_addr);
    p_dev->local_vup = true;
...
#if defined(MTK_COMMON) && (MTK_COMMON == TRUE) //reconnect too quick added
      if (NULL == p_dev)
      {
          BTIF_TRACE_DEBUG("dev is disconnecting, need find_disconnecting_dev_by_handle:%d",
                           p_data->dev_status.handle);
          p_dev = btif_hh_find_disconnecting_dev_by_handle(p_data->dev_status.handle);
          if (p_dev != NULL)
          {
              BTIF_TRACE_DEBUG("BTA_HH_CLOSE_EVT: p_dev->dev_status = %d",
                               p_dev->dev_status);
          }
      }
#endif
...
#endif
/** M: side effect: a2dp no snoomth when hid connect @{*/
#if defined(MTK_COMMON) && (MTK_COMMON == TRUE)
...
/** @} */
...
#if defined(MTK_COMMON) && (MTK_COMMON == TRUE)
      /* Fix two or more hid or hogp device reconnect concurrently*/
      BTIF_TRACE_DEBUG("BTA_HH_GET_DSCP_EVT: dev_handle=%d", p_data->dscp_info.hid_handle);
    ...
    case BTA_HH_VC_UNPLUG_EVT:
      ...
      if (NULL == p_dev)
      {
          ...
          if (p_dev != NULL)
          {
              BTIF_TRACE_DEBUG("BTA_HH_VC_UNPLUG_EVT: p_dev->dev_status = %d", p_dev->dev_status);
          }
      }
#endif
      if (p_dev != NULL) {
