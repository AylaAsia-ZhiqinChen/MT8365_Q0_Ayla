...
///M : add for AAC dynamic adjust casued audiotrack crash ALPS04926444 @{
...
#if defined(MTK_A2DP_SRC_SINK_BOTH) && (MTK_A2DP_SRC_SINK_BOTH == TRUE)
  BtifAvPeer* PeakPeer(const RawAddress& peer_address);
...
static void btif_av_pts_test_thread(void *context) {
  char value[PROPERTY_VALUE_MAX] = {0};
  const RawAddress* peer_address = (const RawAddress*)context;
  LOG_INFO(LOG_TAG, "%s: peer_address=%s", __PRETTY_FUNCTION__, peer_address->ToString().c_str());
  /* properties
  ...
  while(1){
      if ((osi_property_get("bluetooth.pts.avdt.recfg", value, "off"))
          ...
          osi_property_set("bluetooth.pts.avdt.recfg","off");
                  ...
                  && (strcmp(value, "00"))){
          uint8_t* option = (uint8_t*)osi_calloc(2);
          BtifAvPeer* peer = btif_av_sink.FindPeer(btif_av_sink.ActivePeer());
          ascii_2_hex(value, 2, option);

          BTIF_TRACE_EVENT("[pts] audio opetion %d", *option);

          if (*option == 1) {
              BTA_AvStart(peer->BtaHandle());
          } else if(*option == 2) {
              //btif_dispatch_sm_event(BTIF_AV_STOP_STREAM_REQ_EVT, NULL, 0);
              //BTA_AvStop(FALSE);
              //BTA_AvClose(btif_av_cb.bta_handle);
              BTA_AvClose(peer->BtaHandle());
          }
          else if (*option == 3) {
             /*send suspend*/
              BTA_AvStop(peer->BtaHandle(), true);
          }
...
static void btif_av_pts_test_init(const RawAddress* peer_address)
{
  //1. create a thread to check property
  // 2. check the property and do sth.
  if (NULL != pts_av_test_thread)
  {
      return;
  }
  ...
  thread_post(pts_av_test_thread, btif_av_pts_test_thread, (void*)peer_address);
}
  ...
  for (peer_id = kPeerIdMin; peer_id < kPeerIdMax; peer_id++) {
    ...
    if (btif_av_both_enable()) {
      if (FindPeerByPeerId(peer_id) == nullptr
        && btif_av_sink.FindPeerByPeerId(peer_id) == nullptr) break;
...
bool BtifAvSource::AllowedToConnect(const RawAddress& peer_address) const {
  ...
  {
    LOG_INFO(LOG_TAG,
           "%s: connected=%d, max_connected_peers_=%d, sink_peers=%d",
...
#if defined(MTK_A2DP_SRC_SINK_BOTH) && (MTK_A2DP_SRC_SINK_BOTH == TRUE)
BtifAvPeer* BtifAvSource::PeakPeer(const RawAddress& peer_address) {
  auto it = peers_.find(peer_address);
  if (it == peers_.end()) return nullptr;
  BtifAvPeer* peer = it->second;
  peers_.erase(it);
  LOG_INFO(LOG_TAG, "%s: peer_address=%s, state=%d", __PRETTY_FUNCTION__,
    peer->PeerAddress().ToString().c_str(), peer->StateMachine().StateId());
  return peer;
}
...
bt_status_t BtifAvSink::Init(btav_sink_callbacks_t* callbacks) {
  ...
  MTK_PTS_TEST_IS_ENABLE {
    btif_av_pts_test_init(&active_peer_);
  }
#endif
...
BtifAvPeer* BtifAvSink::FindOrCreatePeer(const RawAddress& peer_address,
  ...
  for (peer_id = kPeerIdMin; peer_id < kPeerIdMax; peer_id++) {
    ...
    if (btif_av_both_enable()) {
      if (FindPeerByPeerId(peer_id) == nullptr
        && btif_av_source.FindPeerByPeerId(peer_id) == nullptr) break;
...
bool BtifAvSink::AllowedToConnect(const RawAddress& peer_address) const {
  ...
  if (btif_av_both_enable()) {
    LOG_INFO(LOG_TAG,
           "%s: connected=%d, max_connected_peers_=%d, source_peers=%d",
           __PRETTY_FUNCTION__, connected, max_connected_peers_,
           (int)btif_av_source.Peers().size());
    /* if source device connected, don't connect sink device */
    if (connected >= max_connected_peers_ || !btif_av_source.Peers().empty()) {
      return false;
    } else {
      return true;
    }
...
#if defined(MTK_A2DP_SRC_SINK_BOTH) && (MTK_A2DP_SRC_SINK_BOTH == TRUE)
BtifAvPeer* BtifAvSink::PeakPeer(const RawAddress& peer_address) {
  auto it = peers_.find(peer_address);
  if (it == peers_.end()) return nullptr;
  BtifAvPeer* peer = it->second;
  peers_.erase(it);
  ...
  switch (event) {
    ...
    case BTA_AV_RC_OPEN_EVT: {
      ...
      if (peer_.IsSink()) {
        can_connect = btif_av_source.AllowedToConnect(peer_.PeerAddress());
#if defined(MTK_A2DP_SRC_SINK_BOTH) && (MTK_A2DP_SRC_SINK_BOTH == TRUE)
        if (!can_connect) BTA_AvCloseRc(((tBTA_AV*)p_data)->rc_open.rc_handle);
#else
        if (!can_connect) src_disconnect_sink(peer_.PeerAddress());
#endif
      } else if (peer_.IsSource()) {
    ...
    case BTA_AV_OPEN_EVT: {
      ...
      if (p_bta_data->open.status == BTA_AV_SUCCESS) {
        ...
        if (peer_.PeerSep() != p_bta_data->open.sep) {
          ...
          if (peer_.PeerSep() == AVDT_TSEP_SNK) {
            tmp_peer = btif_av_source.PeakPeer(peer_.PeerAddress());
            ...
            tmp_peer = btif_av_sink.PeakPeer(peer_.PeerAddress());
      ...
      /** M: Fix hfp Fatal NE when a2dp call twice btif_queue_advance(). @{ */
      if (p_bta_data->open.status == BTA_AV_SUCCESS) {
        btif_queue_advance();
      }
      /** @} */
  ...
  switch (event) {
    ...
    case BTA_AV_OPEN_EVT: {
      ...
      if (p_bta_data->open.status == BTA_AV_SUCCESS) {
         ...
         * 1 speacker connected to DUT and phone connect DUT, because default
         *    connect req is as SINK peer. only at here, we can know which role
         *    it is.
         */
        bool can_connect = true;
        if (peer_.IsSink()) {
...
/** @} */
      ...
      if (peer_.IsSink()) {
        ...
        // Bring up AVRCP connection as well
#if defined(MTK_A2DP_SRC_SINK_BOTH) && (MTK_A2DP_SRC_SINK_BOTH == TRUE)
        if (btif_av_sink.AllowedToConnect(peer_.PeerAddress()))
#endif
        BTA_AvOpenRc(peer_.BtaHandle());
      }
#if defined(MTK_PTS_AV_TEST) && (MTK_PTS_AV_TEST == TRUE) \
          && defined(MTK_PTS_AV_SEC_CTRL) && (MTK_PTS_AV_SEC_CTRL == TRUE) //pts test -- to send Security Control.
      //for AVDTP/SNK/INT/SIG/SEC/BV-01-C
      //    AVDTP/SRC/INT/SIG/SEC/BV-01-C
      MTK_PTS_TEST_IS_ENABLE {
      BTIF_TRACE_DEBUG("%s: Peer %s : state: %d, sep:%d[ProctectReq]",
        __PRETTY_FUNCTION__, peer_.PeerAddress().ToString().c_str(),
        peer_.StateMachine().StateId(), peer_.PeerSep());
        BTA_AvProtectReq(peer_.BtaHandle(), NULL, 0);
      }
#endif
      /** M: Bug fix for a2dp connect fail @{ */
...
#if defined(MTK_PTS_AV_TEST) && (MTK_PTS_AV_TEST == TRUE) \
   && defined(MTK_PTS_AV_SEC_CTRL) && (MTK_PTS_AV_SEC_CTRL == TRUE)  // pts test --  for AVDTP/SNK/ACP/SIG/SEC/BV-02-C to do Security Conrol response.
    ...
    MTK_PTS_TEST_IS_ENABLE {
      ...
      BTIF_TRACE_DEBUG("do BTA_AvProtectRsp(handle = %d)",peer_.BtaHandle());
      //BTA_AvProtectRsp(btif_av_cb.bta_handle, error_code, p_data,data_len);
      BTA_AvProtectRsp(peer_.BtaHandle(), error_code, NULL, 0);
    }
    else {
...
#ifdef MTK_MESH_SUPPORT
  meshif_a2dp_streaming_change(true);
#endif
...
void BtifAvStateMachine::StateStarted::OnExit() {
  BTIF_TRACE_DEBUG("%s: Peer %s", __PRETTY_FUNCTION__,
                   peer_.PeerAddress().ToString().c_str());
#ifdef MTK_MESH_SUPPORT
  meshif_a2dp_streaming_change(false);
#endif
}
...
static void btif_av_handle_event(uint8_t peer_sep,
  ...
  BTIF_TRACE_EVENT("%s: peer_sep=%s (%d) peer_address=%s handle=0x%x event=%s",
                   __func__, (peer_sep == AVDT_TSEP_SRC) ? "Source" :
                   ((peer_sep == AVDT_TSEP_SNK) ? "Sink" : "Invalid"),
    ...
    if (btif_av_both_enable()) {
      ...
      if (peer == nullptr) {
        if (peer_sep == AVDT_TSEP_SRC) {
          BTIF_TRACE_DEBUG("%s: peer_sep(%d), create a new source peer", __func__, peer_sep);
          peer = btif_av_sink.FindOrCreatePeer(peer_address, bta_handle);
        } else {
          BTIF_TRACE_DEBUG("%s: peer_sep(%d), create a new sink peer", __func__, peer_sep);
...
static void btif_av_handle_bta_av_event(uint8_t peer_sep,
  ...
  BTIF_TRACE_DEBUG("%s: peer_sep=%s (%d) event=%s", __func__,
                   (peer_sep == AVDT_TSEP_SRC) ? "Source" :
                   ((peer_sep == AVDT_TSEP_SNK) ? "Sink" : "Invalid"), peer_sep,
                   btif_av_event.ToString().c_str());
...
#if defined(MTK_A2DP_SRC_SINK_BOTH) && (MTK_A2DP_SRC_SINK_BOTH == TRUE)
      if (peer_sep == AVDT_TSEP_INVALID) {
        if (registr.peer_sep == AVDT_TSEP_SNK)
...
// Mediatek Android Patch Begin
      ...
      if (src_outputPcmSampleFile != NULL) {
        BTIF_TRACE_DEBUG("%s() open file %s success.",__PRETTY_FUNCTION__, src_outputFilename);
      } else {
        BTIF_TRACE_ERROR("%s() open file %s fail!.",__PRETTY_FUNCTION__, src_outputFilename);
      }
#endif
// Mediatek Android Patch End
      break;
...
// Mediatek Android Patch Begin
      ...
      if (src_outputPcmSampleFile) {
        ...
        BTIF_TRACE_DEBUG("%s() close file %s success.", __PRETTY_FUNCTION__, src_outputFilename);
      } else {
        BTIF_TRACE_DEBUG("%s() file %s may already closed.", __PRETTY_FUNCTION__, src_outputFilename);
      }
...
bool btif_av_both_enable(void) {
    return (btif_av_is_sink_enabled() && btif_av_source.Enabled());
}
#endif
...
static void bta_av_event_callback(tBTA_AV_EVT event, tBTA_AV* p_data) {
  ...
  if (true == btif_av_is_sink_enabled()) {
...
static bt_status_t init_src(
  ...
  for (auto &config : codec_priorities) {
    if (config.codec_type != BTAV_A2DP_CODEC_INDEX_SOURCE_SBC) {
      config.codec_priority = BTAV_A2DP_CODEC_PRIORITY_DISABLED;
    }
  }
#endif
...
#if defined(BT_TUNNEL_SUPPORT) && (BT_TUNNEL_SUPPORT == TRUE)
  for (auto &config : codec_preferences) {
    if (config.codec_type != BTAV_A2DP_CODEC_INDEX_SOURCE_SBC) {
      config.codec_priority = BTAV_A2DP_CODEC_PRIORITY_DISABLED;
    }
  }
...
#endif
#if defined(MTK_PTS_AV_TEST) && (MTK_PTS_AV_TEST == TRUE)
    features |= BTA_AV_FEAT_PROTECT;  //add to test PTS     -- AVDTP/SRC/INT/SIG/SEC/BV-01-C
#endif
#if defined(MTK_A2DP_SRC_SINK_BOTH) && (MTK_A2DP_SRC_SINK_BOTH == TRUE)
...
bt_status_t btif_av_sink_execute_service(bool enable) {
  ...
  if (enable) {
    ...
    tBTA_AV_FEAT features = BTA_AV_FEAT_NO_SCO_SSPD | BTA_AV_FEAT_RCCT |
                            ...
                            BTA_AV_FEAT_BROWSE;
#if defined(MTK_PTS_AV_TEST) && (MTK_PTS_AV_TEST == TRUE)
    features |= BTA_AV_FEAT_PROTECT;   //add to test PTS     -- AVDTP/SNK/ACP/SIG/SEC/BV-02-C
#endif
#if defined(MTK_A2DP_SRC_SINK_BOTH) && (MTK_A2DP_SRC_SINK_BOTH == TRUE)
    BTA_AvEnable(BTA_SEC_AUTHENTICATE, features, bta_av_event_callback);
#else
