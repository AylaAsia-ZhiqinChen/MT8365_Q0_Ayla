...
#if defined(FILE_FLUSH_FOR_DEVICE_INFO_LOST) && (FILE_FLUSH_FOR_DEVICE_INFO_LOST == TRUE)
  //bugfix for when profile just disconnected, AC off/on suddently, then hid info will not save correctly
  btif_config_flush();
  BTIF_TRACE_DEBUG("btif_storage_remove_hid_info: sync after config flush");
#else
  btif_config_save();
#endif
  return BT_STATUS_SUCCESS;
