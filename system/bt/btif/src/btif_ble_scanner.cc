...
void bta_scan_results_cb_impl(RawAddress bd_addr, tBT_DEVICE_TYPE device_type,
  ...
  /*modified for no name issue(got directed packet first), change "||" to "&&" */
...
#if defined(MTK_COMMON) && (MTK_COMMON == TRUE)
//[WCNCR00174737]RC reconnect failed because of COD error
  {
    ...
    if (cod == 0) {
      ...
      ASSERTC(status == BT_STATUS_SUCCESS, "failed to get remote device class", status);
