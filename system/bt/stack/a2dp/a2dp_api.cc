...
/** M: Divide A2dp Source and Sink into two version@{ */
#if defined(MTK_COMMON) && (MTK_COMMON == TRUE)
  if (service_uuid == UUID_SERVCLASS_AUDIO_SINK)
    result &= SDP_AddProfileDescriptorList(
      sdp_handle, UUID_SERVCLASS_ADV_AUDIO_DISTRIBUTION, A2DP_SINK_VERSION);
  else
#endif
/** }@ */
  result &= SDP_AddProfileDescriptorList(
