...
typedef void (tBTM_BLE_MULTI_ADV_CBACK)(uint8_t inst_id, uint8_t status);
#endif
...
#ifdef MTK_MESH_SUPPORT
extern "C" void BTM_MeshRegisterAdv(void);
extern "C" void BTM_MeshUnregisterAdv(void);
extern "C" tBTM_STATUS BTM_MeshEnableAdv(tBTM_BLE_ADV_PARAMS *p_params, tBTM_BLE_MULTI_ADV_CBACK *p_cback);
extern "C" tBTM_STATUS BTM_MeshDisableAdv(uint8_t inst_id);
extern "C" tBTM_STATUS BTM_MeshSaveAdvData(uint8_t *p_data, uint8_t len);
extern "C" tBTM_STATUS BTM_MeshSetUuid(uint8_t *p_data, uint8_t len);
extern "C" tBTM_STATUS BTM_MeshGetPubAddr(uint8_t *pub_addr);
extern "C" const uint8_t* BTM_MeshGetFieldByType(const uint8_t* ad, size_t ad_len, uint8_t type, uint8_t* p_length);
extern "C" tBTM_STATUS BTM_MeshScan(bool start, tBTM_INQ_RESULTS_CB *p_results_cb);
extern "C" void BTM_MeshSetScanParams(uint32_t scan_interval, uint32_t scan_window, tBLE_SCAN_MODE scan_mode);
extern "C" void smp_generate_key(uint8_t pub_key[BT_OCTET32_LEN * 2], uint8_t pri_key[BT_OCTET32_LEN]);
extern "C" void smp_calculate_dhkey(const uint8_t pub_key[BT_OCTET32_LEN * 2],
    const uint8_t pri_key[BT_OCTET32_LEN], uint8_t secret[BT_OCTET32_LEN]);
#endif
#endif
