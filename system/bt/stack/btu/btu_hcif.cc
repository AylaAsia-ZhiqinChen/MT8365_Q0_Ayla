...
#define HCI_DISC_EVT_LEN (5)
  uint8_t evt_data[HCI_DISC_EVT_LEN] = {0};
  evt_data[0] = HCI_DISCONNECTION_COMP_EVT;
  memcpy(evt_data + 1, p, HCI_DISC_EVT_LEN - 1);
  if (btm_cb.ble_ctr_cb.mesh_enabled && (btm_cb.ble_ctr_cb.hci_evt_cb))
  {
    (btm_cb.ble_ctr_cb.hci_evt_cb)(evt_data, HCI_DISC_EVT_LEN);
  }
#endif
  ...
  if (reason == 8) {
    osi_property_set("bluetooth.autoreconnect", "1");
    HCI_TRACE_ERROR("Ready to reconnet for CONN TIME-OUT");
  }
