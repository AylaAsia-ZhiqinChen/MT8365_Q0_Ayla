...
EXPORT_SYMBOL uint16_t GATTS_AddService(tGATT_IF gatt_if, btgatt_db_element_t* service,
                          int count) {
#else
 ...
 ******************************************************************************/
#ifdef MTK_MESH_SUPPORT
EXPORT_SYMBOL tGATT_STATUS GATTS_SendRsp(uint16_t conn_id, uint32_t trans_id,
 ...
 ******************************************************************************/
#ifdef MTK_MESH_SUPPORT
EXPORT_SYMBOL tGATT_STATUS GATTC_ConfigureMTU(uint16_t conn_id, uint16_t mtu)
#else
 ...
 ******************************************************************************/
#ifdef MTK_MESH_SUPPORT
EXPORT_SYMBOL tGATT_STATUS GATTC_Discover(uint16_t conn_id, tGATT_DISC_TYPE disc_type,
 ...
 ******************************************************************************/
#ifdef MTK_MESH_SUPPORT
EXPORT_SYMBOL tGATT_STATUS GATTC_Write(uint16_t conn_id, tGATT_WRITE_TYPE type,
                         tGATT_VALUE* p_write)
#else
 ...
 ******************************************************************************/
#ifdef MTK_MESH_SUPPORT
EXPORT_SYMBOL tGATT_IF GATT_Register(const Uuid& app_uuid128, tGATT_CBACK* p_cb_info)
#else
 ...
 ******************************************************************************/
#ifdef MTK_MESH_SUPPORT
EXPORT_SYMBOL void GATT_Deregister(tGATT_IF gatt_if)
#else
 ...
 ******************************************************************************/
#ifdef MTK_MESH_SUPPORT
EXPORT_SYMBOL bool GATT_Connect(tGATT_IF gatt_if, const RawAddress& bd_addr, bool is_direct,
 ...
 ******************************************************************************/
#ifdef MTK_MESH_SUPPORT
EXPORT_SYMBOL tGATT_STATUS GATT_Disconnect(uint16_t conn_id)
#else
...
EXPORT_SYMBOL bluetooth::Uuid Mesh_UuidEmpty(void)
{
  return Uuid::kEmpty;
}

EXPORT_SYMBOL bluetooth::Uuid Mesh_UuidFrom16Bit(uint16_t uuid16)
{
  Uuid u;
  return u.From16Bit(uuid16);
}

EXPORT_SYMBOL bluetooth::Uuid Mesh_UuidFrom128BitLE(const uint8_t* uuid)
{
  Uuid u;
  return u.From128BitLE(uuid);
}

EXPORT_SYMBOL void Mesh_UuidTo128BitLE(const bluetooth::Uuid& uuid, uint8_t uuid_dst[16])
{
  LOG(INFO) << __func__ << " uuid=" << +uuid.ToString().c_str();
  bluetooth::Uuid::UUID128Bit uuid128Bit = uuid.To128BitLE();
  int i = 0;
  for (auto it = std::begin(uuid128Bit); it != std::end(uuid128Bit); ++it) {
    uuid_dst[i++] = (uint8_t) *it;
  }
}

EXPORT_SYMBOL void Mesh_UuidAs16Bit(const bluetooth::Uuid& uuid, uint8_t uuid_dst[16])
{
  LOG(INFO) << __func__ << " uuid=" << +uuid.ToString().c_str();
  uint16_t uuid16 = uuid.As16Bit();
  uuid_dst[1] = (0xFF00 & uuid16) >> 8;
  uuid_dst[0] = 0x00FF & uuid16;
}

EXPORT_SYMBOL size_t Mesh_UuidGetShortestRepresentationSize(const bluetooth::Uuid& uuid)
{
  LOG(INFO) << __func__ << " uuid=" << +uuid.ToString().c_str();
  return uuid.GetShortestRepresentationSize();
}
#endif
