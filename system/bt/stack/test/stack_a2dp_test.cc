...
const uint8_t codec_info_sbc[AVDT_CODEC_SIZE] = {
    ...
    0x10 |               // Sample Frequency: A2DP_SBC_IE_SAMP_FREQ_48 |
#else
    0x20 |               // Sample Frequency: A2DP_SBC_IE_SAMP_FREQ_44 |
#endif
        0x01,            // Channel Mode: A2DP_SBC_IE_CH_MD_JOINT
...
#if defined(BT_TUNNEL_SUPPORT) && (BT_TUNNEL_SUPPORT == TRUE)
  EXPECT_EQ(A2DP_GetTrackSampleRate(codec_info_sbc), 48000);
#else
...
#if defined(BT_TUNNEL_SUPPORT) && (BT_TUNNEL_SUPPORT == TRUE)
  EXPECT_EQ(A2DP_GetSamplingFrequencyCodeSbc(codec_info_sbc), 3);
#else
