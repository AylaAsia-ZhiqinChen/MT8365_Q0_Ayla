...
static void avrc_ctrl_cback(uint8_t handle, uint8_t event, uint16_t result,
  ...
  if (event <= AVRC_MAX_RCV_CTRL_EVT && avrc_cb.ccb[handle].ctrl_cback) {
    ...
    /* only ACP scb will use two callback for legacy and new avrcp */
    if (avrc_cb.ccb_acp[handle].company_id != 0) {
      AVRC_TRACE_DEBUG("AVRC: ctrl handle=0x%02x, new_avrcp=%d",
        handle, avrc_cb.ccb_acp[handle].company_id == AVRC_CO_GOOGLE);
      avrc_cb.ccb_acp[handle].ctrl_cback.Run(handle, avrc_event, result, peer_addr);
    }
#endif
  }
...
#if defined(MTK_A2DP_SRC_SINK_BOTH) && (MTK_A2DP_SRC_SINK_BOTH == TRUE)
    if (avrc_cb.ccb_acp[param->handle].company_id != 0) {
      /* only ACP scb will use two callback for legacy and new avrcp */
      avrc_cb.ccb_acp[param->handle].ctrl_cback.Run(
        param->handle, AVRC_CMD_TIMEOUT_EVT, param->label, NULL);
    }
#endif
  }
...
#if defined(MTK_A2DP_SRC_SINK_BOTH) && (MTK_A2DP_SRC_SINK_BOTH == TRUE)
    if (avrc_cb.ccb_acp[handle].company_id != 0) {
      AVRC_TRACE_DEBUG("AVRC: %s msg acp handle=0x%02x, label=%d, opcode=0x%x new_avrcp=%d",
        __func__, handle, label, opcode,
        avrc_cb.ccb_acp[handle].company_id==AVRC_CO_GOOGLE);
      avrc_cb.ccb_acp[handle].msg_cback.Run(handle, label, opcode, &msg);
    }
#endif
  } else {
...
uint16_t AVRC_Open(uint8_t* p_handle, tAVRC_CONN_CB* p_ccb,
...
#if defined(MTK_A2DP_SRC_SINK_BOTH) && (MTK_A2DP_SRC_SINK_BOTH == TRUE)
    /* need clear it for another ACP register */
    memset(&avrc_cb.ccb_acp[*p_handle], 0, sizeof(tAVRC_CONN_CB));
#endif
  ...
  AVRC_TRACE_DEBUG("%s device %s role: %d, control:%d status:%d, handle:%d, company_id:%d", __func__,
    peer_addr.ToString().c_str(), cc.role, cc.control, status, *p_handle, p_ccb->company_id);

#if defined(MTK_A2DP_SRC_SINK_BOTH) && (MTK_A2DP_SRC_SINK_BOTH == TRUE)
  /* if ct and new avrcp register acp, there has been one registered */
  if (status == AVCT_ACP_EXIST) {
    AVRC_TRACE_DEBUG("%p acp has exist, handle:%d, company_id=%d, acp_company_id=%d",
       __func__, *p_handle, avrc_cb.ccb[*p_handle].company_id, p_ccb->company_id);
    if (avrc_cb.ccb[*p_handle].company_id != p_ccb->company_id)
    {
      avrc_cb.ccb_acp[*p_handle] = *p_ccb;
    }
    status = AVCT_SUCCESS;
  }
#endif
...
uint16_t AVRC_PassCmd(uint8_t handle, uint8_t label, tAVRC_MSG_PASS* p_msg) {
  ...
  if (p_buf) {
    ...
    /*fix AVRCP/CT/PTH/BV-01-C  AVRCP /CT/PTT/BV-01-I*/
      if (avrc_cb.ccb_int[handle].tle == NULL) {
        BTIF_TRACE_WARNING("%s: tle is NULL", __func__);
        return (status);
      }
#endif
