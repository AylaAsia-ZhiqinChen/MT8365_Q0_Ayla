...
/* Interval(scan_int) = 90 ms= 0x0090 * 0.625 ms */
#define BTM_BLE_GAP_DISC_SCAN_INT 144
#else
...
typedef struct {
  ...
  int8_t tx_power;
#ifdef MTK_MESH_SUPPORT
  uint32_t mesh_scan_window;   /* for mesh sacn */
  uint32_t mesh_scan_interval; /* for mesh sacn */
  uint8_t mesh_scan_type;      /* current scan type: active or passive */
#endif
} tBTM_BLE_INQ_CB;
...
typedef void (*bt_hci_evt_callback_func_t) (const uint8_t *data, uint8_t size);
#endif
