...
#ifdef MTK_MESH_SUPPORT
#define BTM_MESH_ADV_DATA_LEN_MAX       (31)

typedef struct
{
  uint8_t len;
  uint8_t data[BTM_MESH_ADV_DATA_LEN_MAX];
} MESH_ADV_DATA;

static MESH_ADV_DATA g_mesh_adv_data;
static uint8_t g_mesh_inst_id = 0;
static uint8_t g_mesh_scan_rsp_data[] =
{
  0x0a, 0x09, 'M', 'e', 's', 'h', ' ', 'D', 'e', 'm', 'o',
  0x11, 0x07, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00
};

static void MeshAdvRegisterCb(uint8_t inst_id, uint8_t status)
{
  if (0 == status)
  {
    g_mesh_inst_id = inst_id;
    BTM_TRACE_DEBUG("%s : g_mesh_inst_id:%d", __func__, g_mesh_inst_id);
  }
  else
  {
    BTM_TRACE_ERROR("%s : RegisterAdvertiser for mesh failed, status:%d", __func__, status);
  }
}

static void MeshAdvStartCb(uint8_t status)
{
  BTM_TRACE_API("%s : status:%d", __func__, status);
}

EXPORT_SYMBOL void BTM_MeshRegisterAdv(void)
{
  BTM_TRACE_API("%s", __func__);
  g_mesh_inst_id = 0;
  memset(&g_mesh_adv_data, 0x00, sizeof(g_mesh_adv_data));
  BleAdvertisingManager::Get()->RegisterAdvertiser(base::Bind(&MeshAdvRegisterCb));
}

EXPORT_SYMBOL void BTM_MeshUnregisterAdv(void)
{
  BTM_TRACE_API("%s", __func__);
  BleAdvertisingManager::Get()->Unregister(g_mesh_inst_id);
}

EXPORT_SYMBOL tBTM_STATUS BTM_MeshEnableAdv(tBTM_BLE_ADV_PARAMS *p_params, tBTM_BLE_MULTI_ADV_CBACK *p_cback)
{
  std::vector<uint8_t> adv_data(g_mesh_adv_data.data, g_mesh_adv_data.data + g_mesh_adv_data.len);
  std::vector<uint8_t> scan_rsp_data;
  //std::vector<uint8_t> scan_rsp_data(g_mesh_scan_rsp_data, g_mesh_scan_rsp_data + sizeof(g_mesh_scan_rsp_data));

  BTM_TRACE_API("%s enable!", __func__);
  BleAdvertisingManager::Get()->StartAdvertising(g_mesh_inst_id, base::Bind(&MeshAdvStartCb), p_params,
    adv_data, scan_rsp_data, 0, base::Bind(&MeshAdvStartCb));

  if (p_cback)
  {
    (*p_cback)(g_mesh_inst_id, 0);
  }
  return BTM_SUCCESS;
}

EXPORT_SYMBOL tBTM_STATUS BTM_MeshDisableAdv(uint8_t inst_id)
{
  BTM_TRACE_API("%s : inst_id:%d", __func__, inst_id);
  BleAdvertisingManager::Get()->Enable(g_mesh_inst_id, false, base::DoNothing(), 0, 0, base::DoNothing());
  return BTM_SUCCESS;
}

EXPORT_SYMBOL tBTM_STATUS BTM_MeshSaveAdvData(uint8_t *p_data, uint8_t len)
{
  BTM_TRACE_API("%s : len:%d", __func__, len);
  g_mesh_adv_data.len = len;
  memcpy(g_mesh_adv_data.data, p_data, len);
  return BTM_SUCCESS;
}

EXPORT_SYMBOL tBTM_STATUS BTM_MeshSetUuid(uint8_t *p_data, uint8_t len)
{
  BTM_TRACE_API("%s : len:%d", __func__, len);
  if (len >= 16)
  {
    memcpy(g_mesh_scan_rsp_data + 13, p_data, 16);
  }
  return BTM_SUCCESS;
}

EXPORT_SYMBOL tBTM_STATUS BTM_MeshGetPubAddr(uint8_t *pub_addr)
{
  BTM_TRACE_API("%s", __func__);
...
EXPORT_SYMBOL const uint8_t* BTM_MeshGetFieldByType(const uint8_t* ad, size_t ad_len, uint8_t type, uint8_t* p_length)
{
  return AdvertiseDataParser::GetFieldByType(ad, ad_len, type, p_length);
}
#endif
