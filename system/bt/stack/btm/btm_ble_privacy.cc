...
/* Add suspend/resume functions for all RPA-related commands */
...
#if defined(MTK_COMMON) && (MTK_COMMON == TRUE)
  if (!btm_ble_suspend_resolving_list_activity()) return BTM_WRONG_MODE;
#endif
...
#if defined(MTK_COMMON) && (MTK_COMMON == TRUE)
  /* Suspend should support for all advertising types */
...
#if defined(MTK_COMMON) && (MTK_COMMON == TRUE)
  /* Resume should support for all advertising types */
  if (BleAdvertisingManager::IsInitialized()) {
...
#if defined(MTK_COMMON) && (MTK_COMMON == TRUE)
  /* The corresponding resume function will be called at btm_ble_enable_resolving_list() */
