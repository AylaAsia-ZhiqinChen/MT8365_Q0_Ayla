...
#ifdef MTK_MESH_SUPPORT
//for mesh scan
EXPORT_SYMBOL tBTM_STATUS BTM_MeshScan(bool start, tBTM_INQ_RESULTS_CB *p_results_cb)
{
    tBTM_BLE_INQ_CB *p_inq = &btm_cb.ble_ctr_cb.inq_var;
    tBTM_STATUS status = BTM_WRONG_MODE;

    uint32_t scan_interval = !p_inq->mesh_scan_interval ? BTM_BLE_GAP_DISC_SCAN_INT : p_inq->mesh_scan_interval;
    uint32_t scan_window = !p_inq->mesh_scan_window ? BTM_BLE_GAP_DISC_SCAN_WIN : p_inq->mesh_scan_window;
    ...
    if (start)
    {
        ...
        btm_send_hci_set_scan_params(p_inq->mesh_scan_type, (uint16_t)scan_interval,
...
EXPORT_SYMBOL void BTM_MeshSetScanParams(uint32_t scan_interval, uint32_t scan_window, tBLE_SCAN_MODE scan_mode)
{
    ...
    BTM_TRACE_EVENT ("%s, scan_interval = 0x%x, scan_window = 0x%x, scan_mode: %d", __func__, scan_interval, scan_window, scan_mode);
    if (!controller_get_interface()->supports_ble())
        return;
...
extern "C" void BTM_MeshRegisterCb(bt_hci_evt_callback_func_t callback);
...
static void btm_ble_process_adv_pkt_cont(
  ...
  if (btm_inq_find_bdaddr(bda)) {
    ...
    /* if mesh scan enable, no need to discarding advertising packet pkt */
      ...
      //BTM_TRACE_DEBUG("Mesh need handle this ADV!");
    }
#endif
    else {
      /* if yes, skip it */
...
#ifdef MTK_MESH_SUPPORT
  /* if mesh scan enable, no need to discarding advertising packet pkt */
  if ((result == 0) && !btm_cb.ble_ctr_cb.mesh_enabled)
#else
  if (result == 0)
#endif
  {
  ...
  {
    //do_in_jni_thread(base::Bind(btm_cb.ble_ctr_cb.p_mesh_results_cb, (tBTM_INQ_RESULTS*)&p_i->inq_info.results,
                       //const_cast<uint8_t*>(adv_data.data()), adv_data.size()));
    (btm_cb.ble_ctr_cb.p_mesh_results_cb)((tBTM_INQ_RESULTS*)&p_i->inq_info.results,
                       const_cast<uint8_t*>(adv_data.data()), adv_data.size());
  }
#endif
...
void btm_ble_update_mode_operation(uint8_t link_role, const RawAddress* bd_addr,
  ...
  if (btm_ble_get_conn_st() == BLE_CONN_IDLE &&
      ...
      // Resolve issue: already create_le_connection but if trigger again,
      // fw will return HCI_ERR_COMMAND_DISALLOWED, then into dead loop.
