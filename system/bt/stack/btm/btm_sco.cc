...
bool BTM_IsSupportESO2MPS_ForPeer(const RawAddress* bd_bda) {
  tACL_CONN* p_acl = NULL;
  p_acl = btm_bda_to_acl(*bd_bda, BT_TRANSPORT_BR_EDR);
  if (NULL != p_acl && HCI_EDR_ESCO_2MPS_SUPPORTED(p_acl->peer_lmp_feature_pages[0]))
    return true;
  return false;
}
