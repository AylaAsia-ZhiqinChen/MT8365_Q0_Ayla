  ...
  if ((pm_req != BTA_DM_PM_EXECUTE) && (timeout_ms > 0)) {
    for (i = 0; i < BTA_DM_NUM_PM_TIMER; i++) {
      if (bta_dm_cb.pm_timer[i].in_use &&
        ...
        APPL_TRACE_DEBUG("%s dm_pm_timer:%d, %llu ms", __func__, i, timeout_ms);
...
#if defined(MTK_INTEROP_EXTENSION) && (MTK_INTEROP_EXTENSION == TRUE)
  if (interop_mtk_match_addr_name(INTEROP_MTK_HID_NOT_DO_SNIFF_SUBRATING, &p_peer_dev->peer_bdaddr))
    return true;
#endif
...
/*modify sniff mode interval for WCNCR00186093, FW provide Dynamic sniff interval setting subsequencetly*/
    pwr_md.min = 125;
    pwr_md.max = 125;
    APPL_TRACE_ERROR("bta_dm_pm_sniff sniff min :%d, max:%d ", pwr_md.min, pwr_md.max);
#endif
