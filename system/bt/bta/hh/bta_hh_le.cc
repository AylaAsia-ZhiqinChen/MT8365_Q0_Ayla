...
void bta_hh_security_cmpl(tBTA_HH_DEV_CB* p_cb,
...
#if defined(MTK_COMMON) && (MTK_COMMON == TRUE)
  /*  add for HANDLE_KEY_MISSING:if encryption fail and reason is BTM_ERR_KEY_MISSING, do unapir */
  else if(p_cb->reason == BTM_ERR_KEY_MISSING)
  {
    ...
    tBTA_DM_SEC param;
    param.delete_key_RC_to_unpair.bd_addr = p_cb->addr;
...
#if defined(MTK_COMMON) && (MTK_COMMON == TRUE)
  /* Add judgement for gatt null pointer.*/
  if(p_char == NULL) {
    APPL_TRACE_ERROR("%s, No such characteristic! conn_id = %d,handle = %d,status = %d",
