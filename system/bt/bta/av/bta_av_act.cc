...
/// M: add these code to check whether A2dp SRC is on @{
...
uint8_t bta_av_rc_create(tBTA_AV_CB* p_cb, uint8_t role, uint8_t shdl,
  ...
  if (role == AVCT_INT) {
    ...
    if (p_rcb != NULL) {
      APPL_TRACE_ERROR("%s: ACP handle exist for shdl:%d, rcb_handle:%d", __func__, shdl, p_rcb->handle);
      p_rcb->lidx = lidx;
      return p_rcb->handle;
    }
  }
...
void bta_av_rc_opened(tBTA_AV_CB* p_cb, tBTA_AV_DATA* p_data) {
  ...
  APPL_TRACE_DEBUG("%s: local features 0x%X peer features 0x%X", __func__,
  ...
  {
      if (rc_open.peer_addr == p_cb->rc_feat.peer_addr) {
          rc_open.peer_features = p_cb->rc_feat.peer_features;
          rc_open.peer_ct_features = p_cb->rc_feat.peer_ct_features;
          rc_open.peer_tg_features = p_cb->rc_feat.peer_tg_features;
      } else {
...
//#if defined(MTK_A2DP_SRC_SINK_BOTH) && (MTK_A2DP_SRC_SINK_BOTH == TRUE)
        ...
        AVRC_MsgReq(p_rcb->handle, p_data->api_meta_rsp.label,
                    p_data->api_meta_rsp.rsp_code, p_data->api_meta_rsp.p_pkt, false);
#else
        AVRC_MsgReq(p_rcb->handle, p_data->api_meta_rsp.label,
            ...
            *p_ctype = AVRC_RSP_IMPL_STBL;
            ...
            if (btif_av_both_enable()) {
              ...
              memcpy(p_rc_rsp->get_caps.param.event_id,
                    p_src_cfg->p_meta_evt_ids, p_src_cfg->num_evt_ids);
              ...
              if (rc_ver != 0x103) {
                ...
                p_rc_rsp->get_caps.count += bta_avk_cfg.num_evt_ids;
...
void bta_av_rc_msg(tBTA_AV_CB* p_cb, tBTA_AV_DATA* p_data) {
  ...
  if (p_data->rc_msg.opcode == AVRC_OP_PASS_THRU) {
    ...
    if ((p_data->rc_msg.msg.hdr.ctype == AVRC_CMD_CTRL) ||
      ...
      bool newarcp_rsp = is_new_avrcp && btif_av_peer_is_connected_sink(p_cb->rc_feat.peer_addr);
#endif
...
#if defined(MTK_A2DP_SRC_SINK_BOTH) && (MTK_A2DP_SRC_SINK_BOTH == TRUE)
      bool is_new_avrcp = osi_property_get_bool("persist.bluetooth.enablenewavrcp", true);
      bool newarcp_rsp = (AVRC_PDU_GET_CAPABILITIES == rc_rsp.pdu) && is_new_avrcp
        && btif_av_peer_is_connected_sink(p_cb->rc_feat.peer_addr);
      if (!newarcp_rsp) {
#endif
...
#if defined(MTK_COMMON) && (MTK_COMMON == TRUE)
uint16_t bta_av_get_peer_ver (uint16_t service_uuid)
{
    tBTA_AV_CB   *p_cb = &bta_av_cb;
    tSDP_DISC_REC       *p_rec = NULL;
    uint16_t            peer_rc_version=0;
...
void bta_av_rc_disc_done_all(UNUSED_ATTR tBTA_AV_DATA* p_data) {
  ...
  if ((p_cb->disc & BTA_AV_CHNL_MSK) == BTA_AV_CHNL_MSK) {
    ...
    APPL_TRACE_ERROR("%s: WRONG MASK A2dp not connect", __func__);
  } else {
    /* Validate array index*/
  ...
  p_cb->disc = 0;
  osi_free_and_reset((void**)&p_cb->p_disc_db);
  p_cb->rc_feat.peer_ct_features = peer_ct_features;
  p_cb->rc_feat.peer_tg_features = peer_tg_features;
  p_cb->rc_feat.rc_handle = rc_handle;
  if (p_scb == NULL) {
    APPL_TRACE_ERROR("%s:NULL p_scb for a2dp not being connected", __func__);
    return;
  }
  p_cb->rc_feat.peer_addr = p_scb->PeerAddress();
  ...
  if (rc_handle == BTA_AV_RC_HANDLE_NONE) {
    if (p_scb) {
      ...
      if ((((p_cb->features & BTA_AV_FEAT_RCCT) &&
        ...
        rc_open.peer_ct_features = peer_ct_features;
        rc_open.peer_tg_features = peer_tg_features;
...
void bta_av_rc_closed(tBTA_AV_DATA* p_data) {
  ...
  for (i = 0; i < BTA_AV_NUM_RCB; i++) {
    ...
    if (p_rcb->handle == p_msg->handle) {
      ...
      p_rcb->peer_ct_features = 0;
      p_rcb->peer_tg_features = 0;
      p_cb->rc_feat.peer_features = 0;
      p_cb->rc_feat.peer_ct_features = 0;
      p_cb->rc_feat.peer_tg_features = 0;
      p_cb->rc_feat.peer_ct_features = 0;
      p_cb->rc_feat.peer_addr = RawAddress::kEmpty;
#endif
