...
/// M : add code to handle AVDTP connection collision-ALPS04725350 @{
    ...
    p_ret->avdt_conn_timer = alarm_new("bta_av.avdt_conn_timer");
#endif
/// @}
...
/// M : add code to handle AVDTP connection collision-ALPS04725350 @{
#if defined(MTK_COMMON) && (MTK_COMMON == TRUE)
  alarm_free(p_scb->avdt_conn_timer);
#endif
/// @}
  // TODO: After tBTA_AV_SCB is changed to a proper class, the entry
...
static void bta_av_api_register(tBTA_AV_DATA* p_data) {
  ...
  APPL_TRACE_DEBUG("%s: profile: 0x%x, app_id: %d, chnl: %d", __func__,
      profile_initialized, registr.app_id, registr.chnl);
...
#if defined(MTK_PTS_AV_TEST) && (MTK_PTS_AV_TEST == TRUE) //PTS test -- AVDTP/SNK/ACP/SIG/SMG/BV-14-C
...
#if defined(MTK_A2DP_SRC_SINK_BOTH ) && (MTK_A2DP_SRC_SINK_BOTH == TRUE)
    if ((profile_initialized == UUID_SERVCLASS_AUDIO_SOURCE)
      && (bta_av_cb.features & BTA_AV_FEAT_DELAY_RPT)) {
      avdtp_stream_config.cfg.psc_mask |= AVDT_PSC_DELAY_RPT;
    }
/** M: Make a2dp sink got no delay report but src does@{ */
#if defined(MTK_COMMON) && (MTK_COMMON == TRUE)
    if ((profile_initialized == UUID_SERVCLASS_AUDIO_SINK)
      && (bta_av_cb.sink_features & BTA_AV_FEAT_DELAY_RPT)) {
      avdtp_stream_config.cfg.psc_mask &= ~AVDT_PSC_DELAY_RPT;
    }
#endif
/** }@*/
...
/** M: Make a2dp sink got no delay report but src does@{ */
#if defined(MTK_COMMON) && (MTK_COMMON == TRUE)
    uint16_t msk = p_scb->cfg.psc_mask;
    p_scb->cfg = avdtp_stream_config.cfg;
    p_scb->cfg.psc_mask |= msk;
#else
    p_scb->cfg = avdtp_stream_config.cfg;
#endif
/** }@*/
...
#if (BTA_AR_INCLUDED == TRUE)
        /* create an SDP record as AVRC CT. We create 1.5 for SOURCE
         * because we rely on feature bits being scanned by external
         * devices more than the profile version itself.
         *
         * We create 1.5 for SINK since we support browsing.
         */
        if (profile_initialized == UUID_SERVCLASS_AUDIO_SOURCE) {
