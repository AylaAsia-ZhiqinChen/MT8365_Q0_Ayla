...
/// M : add code to handle AVDTP connection collision-ALPS04725350 @{
#if defined(MTK_COMMON) && (MTK_COMMON == TRUE)
    else if (event == AVDT_DISCONNECT_IND_EVT && p_data->hdr.err_code == L2CAP_CONN_NO_RESOURCES){
      APPL_TRACE_ERROR("%s: try to re-init connection fro Resource Collision", __func__);
      int random_interval = (1 + rand()%10)*100;
      bta_sys_start_timer(p_scb->avdt_conn_timer, (uint64_t)random_interval,
                          BTA_AV_SDP_DISC_OK_EVT, p_scb->hndl);
      return;
    } else if (event == AVDT_CONNECT_IND_EVT)
      alarm_cancel(p_scb->avdt_conn_timer);
#endif
/// @}
  }
...
void bta_av_do_disc_a2dp(tBTA_AV_SCB* p_scb, tBTA_AV_DATA* p_data) {
  ...
  if (p_scb->skip_sdp == true) {
    ...
    /* Normally the sink peer in this situation defined as 1.2 and for compatibility*/
    a2dp_ser.avdt_version = 0x0102;
...
void bta_av_str_opened(tBTA_AV_SCB* p_scb, tBTA_AV_DATA* p_data) {
  ...
  APPL_TRACE_ERROR("%s:sep_idx: %x, tsep: %d, rc_handle:%x", __func__, p_scb->sep_idx,
                     p_scb->seps[p_scb->sep_idx].tsep, p_scb->rc_handle);
...
void bta_av_suspend_cfm(tBTA_AV_SCB* p_scb, tBTA_AV_DATA* p_data) {
...
#if defined(MTK_PTS_AV_TEST) && (MTK_PTS_AV_TEST == TRUE)  //  PTS test-- AVDTP/SNK/INT/SIG/SMG/BV-13-C
  ...
  MTK_PTS_TEST_IS_ENABLE {
    ...
    if(!err_code) {
      ...
      /* reconfig the stream */
      //APPL_TRACE_DEBUG("[pts]p_scb->avdt_handle=%d, p_cap=%p", p_scb->avdt_handle, p_scb->cfg);
      AVDT_ReconfigReq(p_scb->avdt_handle, &p_scb->cfg);
      p_scb->cfg.psc_mask = p_scb->cur_psc_mask;
    }
  }
...
void bta_av_suspend_cont(tBTA_AV_SCB* p_scb, tBTA_AV_DATA* p_data) {
  ...
  if (err_code) {
    if (AVDT_ERR_CONNECT == err_code) {
      /* report failure */
      tBTA_AV_RECONFIG reconfig;
      reconfig.chnl = BTA_AV_CHNL_MSK;
      reconfig.hndl = BTA_AV_HNDL_MSK;
...
#if defined(MTK_PTS_AV_TEST) && (MTK_PTS_AV_TEST == TRUE)  //  PTS test-- AVDTP/SNK/INT/SIG/SMG/BV-13-C
    MTK_PTS_TEST_IS_ENABLE {
    APPL_TRACE_DEBUG("[pts]p_scb->avdt_handle=%d, SepConfig=%p", p_scb->avdt_handle, p_scb->cfg);
      AVDT_ReconfigReq(p_scb->avdt_handle, &p_scb->cfg);
      p_scb->cfg.psc_mask = p_scb->cur_psc_mask;
    }
    else {
    ...
    if (bta_av_cb.disc) {
      ...
      if (btif_av_both_enable()) {
        ...
        if ((p_scb->seps[p_scb->sep_idx].tsep == AVDT_TSEP_SRC)
          ...
          APPL_TRACE_WARNING("%s: local src Using the new AVRCP Profile", __func__);
        ...
        APPL_TRACE_WARNING("%s: local sink Using the legacy AVRCP Profile", __func__);
...
void bta_av_open_at_inc(tBTA_AV_SCB* p_scb, tBTA_AV_DATA* p_data) {
  ...
  if (p_scb->coll_mask & BTA_AV_COLL_INC_TMR) {
    ...
    p_scb->skip_sdp = true;
#if defined(MTK_PTS_AV_TEST) && (MTK_PTS_AV_TEST == TRUE)  //  PTS test-- AVDTP/SRC/INT/SIG/SMG/BV-25-C
    MTK_PTS_TEST_IS_ENABLE {
      p_scb->skip_sdp = false;
    }
#endif
