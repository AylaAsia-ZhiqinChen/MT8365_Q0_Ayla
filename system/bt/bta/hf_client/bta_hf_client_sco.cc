...
static bool hf_retry_with_sco_only = false; /* indicator to try with SCO only when eSCO fails */
/** @} */
  ...
  if (client_cb->sco_state == BTA_HF_CLIENT_SCO_LISTEN_ST) {
    if (p_data->link_type == BTM_LINK_TYPE_SCO) {
      ...
      if (client_cb->negotiated_codec == BTA_AG_CODEC_MSBC) {
        /** M: Change for bug fix: Use 2-EV3 package type. @{ */
        if (BTM_IsSupportESO2MPS_ForPeer(&p_data->bd_addr)) {
          resp = esco_parameters_for_codec(ESCO_CODEC_MSBC_T2);
        /** @} */
#if defined(MTK_PTS_HF_TEST) && (MTK_PTS_HF_TEST == TRUE)
          /** M: Change for HF PTS: AAC/BV-06-I @{ */
          MTK_PTS_TEST_IS_ENABLE {
            resp.max_latency_ms = 8;
          /** @} */
          }
#endif
        } else {
...
#if defined(MTK_PTS_HF_TEST) && (MTK_PTS_HF_TEST == TRUE)
        /** M: Change for HF PTS: AAC/BV-01-I, AAC/BV-03-I and so on. @{ */
        MTK_PTS_TEST_IS_ENABLE {
          APPL_TRACE_DEBUG("%s: persist.bluetooth.pts.enable on", __func__ );
          resp.retransmission_effort = ESCO_RETRANSMISSION_POWER;
          resp.max_latency_ms = 10;
        /** @} */
        }
#endif
      }
...
static void bta_hf_client_sco_create(tBTA_HF_CLIENT_CB* client_cb,
  ...
  if (is_orig) {
    ...
    if (client_cb->peer_version >= HFP_VERSION_1_5 && !hf_retry_with_sco_only) {
