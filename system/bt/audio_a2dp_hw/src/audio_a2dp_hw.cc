...
static void ts_log(UNUSED_ATTR const char* tag, UNUSED_ATTR int val,
  ...
  now_us = ((uint64_t)now.tv_sec * USEC_PER_SEC) + ((uint64_t)now.tv_nsec / 1000);

  if (pprev_opt) {
    diff_us = ((uint64_t)(now.tv_sec - prev.tv_sec) * USEC_PER_SEC) +
              ((uint64_t)(now.tv_nsec - prev.tv_nsec) / 1000);
    *pprev_opt = now;
...
static int calc_audiotime_usec(struct a2dp_config cfg, int bytes) {
  ...
  if (0 != chan_count) {
      ...
      ASSERTC(false, "unexpect channel count", chan_count);
      return 0;
  }
}
