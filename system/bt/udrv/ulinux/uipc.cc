...
static inline int create_server_socket(const char* name) {
  ...
  if (listen(s, 5) < 0) {
    BTIF_TRACE_EVENT("listen failed err %s", strerror(errno));
    close(s);
    return -1;
  }
...
static int uipc_setup_server_locked(tUIPC_STATE& uipc, tUIPC_CH_ID ch_id,
  ...
  if (fd < 0) {
    BTIF_TRACE_ERROR("failed to setup %s err %s", name, strerror(errno));
    return -1;
  }
...
bool UIPC_Open(tUIPC_STATE& uipc, tUIPC_CH_ID ch_id, tUIPC_RCV_CBACK* p_cback,
  ...
  BTIF_TRACE_DEBUG("UIPC_Open : ch_id %d, p_cback %p", ch_id, p_cback);
