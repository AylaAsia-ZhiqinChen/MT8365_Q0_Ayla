...
bool fscrypt_initialize_global_de() {
    ...
    android::vold::write_file_bootprof("vold:retrieveAndInstallKey:START");
    bool rtn = android::vold::retrieveAndInstallKey(true, kEmptyAuthentication, device_key_path,
                                              device_key_temp, &device_ref.key_raw_ref);
    android::vold::write_file_bootprof(StringPrintf("vold:retrieveAndInstallKey:END, rtn =%d", rtn).c_str());
    if (!rtn)
        return false;
