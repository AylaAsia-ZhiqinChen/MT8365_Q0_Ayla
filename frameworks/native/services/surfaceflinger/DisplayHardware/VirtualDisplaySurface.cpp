...
#ifdef MTK_DISPLAY_DECOMPRESSION
...
#ifdef MTK_DISPLAY_DECOMPRESSION
    mConsumer->setConsumerUsageBits(GRALLOC_USAGE_HW_COMPOSER | GRALLOC_USAGE_NO_COMPRESS);
#else
...
#ifdef MTK_SF_DEBUG_SUPPORT
    if (!mDisplayId) {
        status_t result = mSource[SOURCE_SINK]->dequeueBuffer(
                pslot, fence, w, h, format, usage, outBufferAge, outTimestamps);
