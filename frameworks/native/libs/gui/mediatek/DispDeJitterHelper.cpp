...
, mFnMarkTimestamp(nullptr) {
    ...
    typedef bool (*ShouldDelayPresent)(DispDeJitter*, const String8&, const sp<GraphicBuffer>&, const nsecs_t&);
...
bool DispDeJitterHelper::shouldPresentNow(
    ...
    if (mFnShouldDelayPresent) {
        needDelay = mFnShouldDelayPresent(dispDeJitter, name, gb, expectedPresent);
    }
