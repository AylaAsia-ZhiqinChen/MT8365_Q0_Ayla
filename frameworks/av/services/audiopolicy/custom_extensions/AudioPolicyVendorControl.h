...
*/
    ...
    public:
        AudioPolicyVendorControl():mVoiceCurveReplaceDTMFCurve(false),mUseCustomVolume(false),mFMTxEnable(false),
                                    mNumHSPole(4), mHDMI_ChannelCount(8), mUsbSuspended(false), mPrevMode(AUDIO_MODE_NORMAL), mCurMode(AUDIO_MODE_NORMAL), mBTSupportVGS(true), mStillInCallWithoutEnteringNormal(false) {};
        ...
        audio_mode_t getCurMode() const {return mCurMode;}
        void setCurMode(audio_mode_t mode)
        {
            if (mode != mCurMode) {
                mPrevMode = mCurMode;
                mCurMode = mode;
                if (mode == AUDIO_MODE_IN_CALL) {
                    mStillInCallWithoutEnteringNormal = true;
                } else if (mStillInCallWithoutEnteringNormal && mode == AUDIO_MODE_NORMAL) {
                    mStillInCallWithoutEnteringNormal = false;
                }
            }
        }
        bool getBTSupportVGS() const {return mBTSupportVGS;}
        void setBTSupportVGS(bool enable) {mBTSupportVGS = enable;}
        bool getStillInCallWithoutEnteringNormal() const {return mStillInCallWithoutEnteringNormal;};
    private:
        ...
        audio_mode_t mCurMode;
        bool mBTSupportVGS;
        bool mStillInCallWithoutEnteringNormal;
    };
}
