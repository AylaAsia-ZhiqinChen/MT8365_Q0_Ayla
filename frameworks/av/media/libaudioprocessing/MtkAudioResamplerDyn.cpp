...
#define LOG_TAG "MtkAudioResamplerDyn"
//#define LOG_NDEBUG 0
...
MtkAudioResamplerDyn<TC, TI, TO>::InBuffer::InBuffer()
...
template<typename TC, typename TI, typename TO>
void MtkAudioResamplerDyn<TC, TI, TO>::InBuffer::init()
{
    free(mState);
    mState = NULL;
    mImpulse = NULL;
...
template<typename TC, typename TI, typename TO>
void MtkAudioResamplerDyn<TC, TI, TO>::InBuffer::resize(int CHANNELS, int halfNumCoefs)
{
...
void MtkAudioResamplerDyn<TC, TI, TO>::InBuffer::readAgain(TI*& impulse, const int halfNumCoefs,
...
template<typename TC, typename TI, typename TO>
template<int CHANNELS>
void MtkAudioResamplerDyn<TC, TI, TO>::InBuffer::readAdvance(TI*& impulse, const int halfNumCoefs,
        const TI* const in, const size_t inputIndex)
{
    impulse += CHANNELS;
...
template<typename TC, typename TI, typename TO>
void MtkAudioResamplerDyn<TC, TI, TO>::InBuffer::reset()
{
    // clear resampler state
    if (mState != nullptr) {
        ALOGD("MtkAudioResamplerDyn InBuffer::reset!!");
        memset(mState, 0, mStateCount * sizeof(TI));
    }
}

template<typename TC, typename TI, typename TO>
void MtkAudioResamplerDyn<TC, TI, TO>::Constants::set(
...
template<typename TC, typename TI, typename TO>
MtkAudioResamplerDyn<TC, TI, TO>::MtkAudioResamplerDyn(
...
// <MTK_AUDIO
    ...
    mOutFormat = AUDIO_FORMAT_DEFAULT;
#if defined(MTK_AUDIO_DEBUG)
    TI test = (TI)0.5;
    if (test) { // float
...
template<typename TC, typename TI, typename TO>
void MtkAudioResamplerDyn<TC, TI, TO>::init()
{
    mFilterSampleRate = 0; // always trigger new filter generation
    mInBuffer.init();
}

//ALPS03916900: SRC leak sound
template<typename TC, typename TI, typename TO>
void MtkAudioResamplerDyn<TC, TI, TO>::reset()
{
...
template<typename TC, typename TI, typename TO>
void MtkAudioResamplerDyn<TC, TI, TO>::setVolume(float left, float right)
{
...
template<typename TC, typename TI, typename TO>
void MtkAudioResamplerDyn<TC, TI, TO>::createKaiserFir(Constants &c,
...
template<typename TC, typename TI, typename TO>
void MtkAudioResamplerDyn<TC, TI, TO>::setSampleRate(int32_t inSampleRate)
{
    ...
    if (mFilterQuality != getQuality() ||
        ...
        if (mFilterQuality == MTK_DYN_HIGH_QUALITY) {
        ...
        } else if (mFilterQuality == DYN_HIGH_QUALITY) {
            // 32b coefficients, 64 length
            useS32 = true;
            stopBandAtten = 98.;
            if (inSampleRate >= mSampleRate * 4) {
    ...
    if (locked) {
        ...
        case 1:
            mResampleFunc = &MtkAudioResamplerDyn<TC, TI, TO>::resample<1, true, 16>;
            break;
        case 2:
            mResampleFunc = &MtkAudioResamplerDyn<TC, TI, TO>::resample<2, true, 16>;
            break;
        case 3:
            mResampleFunc = &MtkAudioResamplerDyn<TC, TI, TO>::resample<3, true, 16>;
            break;
        case 4:
            mResampleFunc = &MtkAudioResamplerDyn<TC, TI, TO>::resample<4, true, 16>;
            break;
        case 5:
            mResampleFunc = &MtkAudioResamplerDyn<TC, TI, TO>::resample<5, true, 16>;
            break;
        case 6:
            mResampleFunc = &MtkAudioResamplerDyn<TC, TI, TO>::resample<6, true, 16>;
            break;
        case 7:
            mResampleFunc = &MtkAudioResamplerDyn<TC, TI, TO>::resample<7, true, 16>;
            break;
        case 8:
            mResampleFunc = &MtkAudioResamplerDyn<TC, TI, TO>::resample<8, true, 16>;
            break;
        }
    } else {
        switch (mChannelCount) {
        case 1:
            mResampleFunc = &MtkAudioResamplerDyn<TC, TI, TO>::resample<1, false, 16>;
            break;
        case 2:
            mResampleFunc = &MtkAudioResamplerDyn<TC, TI, TO>::resample<2, false, 16>;
            break;
        case 3:
            mResampleFunc = &MtkAudioResamplerDyn<TC, TI, TO>::resample<3, false, 16>;
            break;
        case 4:
            mResampleFunc = &MtkAudioResamplerDyn<TC, TI, TO>::resample<4, false, 16>;
            break;
        case 5:
            mResampleFunc = &MtkAudioResamplerDyn<TC, TI, TO>::resample<5, false, 16>;
            break;
        case 6:
            mResampleFunc = &MtkAudioResamplerDyn<TC, TI, TO>::resample<6, false, 16>;
            break;
        case 7:
            mResampleFunc = &MtkAudioResamplerDyn<TC, TI, TO>::resample<7, false, 16>;
            break;
        case 8:
            mResampleFunc = &MtkAudioResamplerDyn<TC, TI, TO>::resample<8, false, 16>;
            break;
        }
    }
...
template<typename TC, typename TI, typename TO>
size_t MtkAudioResamplerDyn<TC, TI, TO>::resample(int32_t* out, size_t outFrameCount,
...
size_t MtkAudioResamplerDyn<TC, TI, TO>::resample(TO* out, size_t outFrameCount,
        AudioBufferProvider* provider)
{
    ...
    while (outputIndex < outputSampleCount) {
        ...
        while (mBuffer.frameCount == 0 && inFrameCount > 0) {
            ...
            if (mBuffer.raw == NULL) {
                ...
                mInBuffer.reset();
#if defined(MTK_AUDIO_DEBUG)
                if (AudioDump::getProperty(AudioDump::PROP_AUDIO_DUMP_RESAMPLER)) {
                    String8 fileName;
                    fileName = String8::format("%s_%p.pcm", AudioDump::af_resampler_in_pcm, provider);
                    AudioDump::threadDump(fileName, out, outFrameCount * mChannelCount * sizeof(TI),
                        mInFormat, mInSampleRate, mChannelCount);
                }
#endif // MTK_AUDIO_DEBUG
                goto resample_exit;
            }
        ...
        if (inputIndex > 0) {  // we've acquired a buffer (alternatively could check frameCount)
            ...
            if (AudioDump::getProperty(AudioDump::PROP_AUDIO_DUMP_RESAMPLER)) {
                if(outputIndex == outputSampleCount) {
                    String8 fileName;
                    fileName = String8::format("%s_%p.pcm", AudioDump::af_resampler_out_pcm, provider);
                    AudioDump::threadDump(fileName, out, outputIndex * sizeof(TO),
...
template class MtkAudioResamplerDyn<float, float, float>;
template class MtkAudioResamplerDyn<int16_t, int16_t, int32_t>;
template class MtkAudioResamplerDyn<int32_t, int16_t, int32_t>;
