...
bool NuPlayer::Renderer::onDrainAudioQueue() {
    ...
    while (!mAudioQueue.empty()) {
        ...
        sp<AMessage> meta = entry->mMeta;
        int32_t queueGeneration;
        CHECK(meta->findInt32("queueGeneration", &queueGeneration));
        if (queueGeneration != getQueueGeneration(true)) {
            ALOGD("flushing....ingore this ouput buffer");
            return false;
        }
