...
SVP_COPY2SEC_CB_T gSVPCopy2SecCallBack=NULL;

void SVP_reg_callback(SVP_COPY2SEC_CB_T pFnCopy2Sec)
{
    gSVPCopy2SecCallBack = pFnCopy2Sec;
    ALOGD("SVP_reg_callback");
}
#endif
...
sp<MediaCodec> MediaCodec::CreateByType(
...
#endif // ifdef MTK_SEC_VIDEO_PATH_SUPPORT
    ...
    ALOGD("[SVP] onAllocateComponent, match size=%zu", matchingCodecs.size());
#endif // ifdef MTK_SEC_VIDEO_PATH_SUPPORT
    if (err != NULL) {
        *err = NAME_NOT_FOUND;
    }
...
#ifdef MTK_SEC_VIDEO_PATH_SUPPORT
    if (!strncasecmp(mInitName.c_str(), "video/", 6)) {
        ALOGD("queueCSDInputBuffer, srcBuffer=%p srcBuffer->data()=%p size=%zu", csd.get(), csd->data(), csd->size());
        ALOGD("queueCSDInputBuffer, dstBuffer=%p dstBuffer->data()=%p ", codecInputData.get(), codecInputData->data());
        if (gSVPCopy2SecCallBack != NULL) {
            gSVPCopy2SecCallBack(csd->data(), csd->size(), codecInputData->data());
        } else {
