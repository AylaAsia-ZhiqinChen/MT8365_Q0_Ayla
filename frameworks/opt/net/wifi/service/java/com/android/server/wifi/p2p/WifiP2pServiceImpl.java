    ...
    private class P2pStateMachine extends StateMachine {
        ...
        class GroupCreatingState extends State {
            ...
            public boolean processMessage(Message message) {
                ...
                switch (message.what) {
					...
					//ALPS01497387
					case BLOCK_DISCOVERY:
						if (mVerboseLoggingEnabled) logd("defer BLOCK_DISCOVERY@GroupCreatingState");
						deferMessage(message);
						break;
					//ALPS02191437: p2pStopFind() would make group nego. failed
					case WifiP2pMonitor.P2P_FIND_STOPPED_EVENT:
						if (mVerboseLoggingEnabled) logd("defer P2P_FIND_STOPPED_EVENT@GroupCreatingState");
						deferMessage(message);
						break;
					//ALPS02191437: STOP_DISCOVERY procedure would trigger p2pStopFind(),
					// it would make group nego. failed.
					case WifiP2pManager.STOP_DISCOVERY:
						if (mVerboseLoggingEnabled) logd("defer STOP_DISCOVERY@GroupCreatingState");
						deferMessage(message);
						break;
                    default:
                        ret = NOT_HANDLED;
            ...
            public boolean processMessage(Message message) {
                ...
                switch (message.what) {
					...
					case WifiP2pManager.PEER_CONNECTION_USER_ACCEPT_FROM_OUTER:
						...
						if (mVerboseLoggingEnabled) logd("isWfdSinkEnabled()=" + isWfdSinkEnabled());
						if (isWfdSinkEnabled()) {
							WifiInfo wifiInfo = getWifiConnectionInfo();
							if (wifiInfo != null) {
								if (mVerboseLoggingEnabled) logd("wifiInfo=" + wifiInfo);
            ...
            public void enter() {
                ...
                if (mMccSupport == true) {
                    if (mConnectToPeer == true) {
                        if (mVerboseLoggingEnabled) logd(getName() + " SCC->MCC, mConnectToPeer=" + mConnectToPeer +
            ...
            public void enter() {
				...
				///M:@{
					...
					if (isWfdSinkConnected()) {
						if (mVerboseLoggingEnabled) logd(getName() + " [wfd sink] stop scan@GC, to avoid packet lost");
						mWifiNative.p2pStopFind();
					}
					///@}
                }
            ...
            public boolean processMessage(Message message) {
                ...
                switch (message.what) {
					...
					case WifiP2pManager.REMOVE_CLIENT:
						...
						break;
					//case WifiP2pMonitor.P2P_PEER_DISCONNECT_EVENT:
					...
					case WifiP2pMonitor.P2P_DEVICE_FOUND_EVENT:
						WifiP2pDevice peerDevice = (WifiP2pDevice) message.obj;
						if (mThisDevice.deviceAddress.equals(peerDevice.deviceAddress)) {
							break;
						}
						...
						break;
					case WifiP2pMonitor.SUP_DISCONNECTION_EVENT:
						if (mVerboseLoggingEnabled) loge("Supplicant close unexpected, send fake Group Remove event");
						sendMessage(WifiP2pMonitor.P2P_GROUP_REMOVED_EVENT);
						deferMessage(message);
						break;
					case WifiP2pManager.DISCOVER_PEERS:
						// do not send service discovery request while normal find operation.
        ...
        private WifiP2pDevice eraseOwnDeviceAddress(WifiP2pDevice device) {
            ...
            if (device.deviceAddress != null
                ...
                result.deviceAddress = mThisDevice.deviceAddress;
            }
            return result;
        }
        ...
        */
		...
		private void sendP2pConnectionChangedBroadcast(P2pStatus reason) {
			if (mVerboseLoggingEnabled) logd("sending p2p connection changed broadcast, reason = " + reason +
				", mGroup: " + mGroup +
				", mP2pOperFreq: " + mP2pOperFreq);
			Intent intent = new Intent(WifiP2pManager.WIFI_P2P_CONNECTION_CHANGED_ACTION);
			intent.addFlags(Intent.FLAG_RECEIVER_REGISTERED_ONLY_BEFORE_BOOT
			...
			if (reason == P2pStatus.NO_COMMON_CHANNEL) {
				...
				if (mVerboseLoggingEnabled) logd("channel conflict, user decline, broadcast with reason=-3");
				intent.putExtra("reason=", -3);
			} else if (reason == P2pStatus.MTK_EXPAND_01) {
				if (mVerboseLoggingEnabled) logd("[wfd sink/source] broadcast with reason=-2");
		...
		private void sendP2pGOandGCRequestConnectBroadcast() {
			if (mVerboseLoggingEnabled) logd("sendP2pGOandGCRequestConnectBroadcast");
		...
		private void sendP2pOPChannelBroadcast() {
			if (mVerboseLoggingEnabled) logd("sendP2pOPChannelBroadcast: OperFreq = " + mP2pOperFreq);
		...
		private void sendP2pFreqConflictBroadcast() {
			if (mVerboseLoggingEnabled) logd("sendP2pFreqConflictBroadcast");
		...
		private String getInterfaceAddress(String deviceAddress) {
			if (mVerboseLoggingEnabled) logd("getInterfaceAddress(): deviceAddress=" + deviceAddress);
			WifiP2pDevice d = mPeers.get(deviceAddress);
			if (d == null) {
				...
				if (mVerboseLoggingEnabled) logd("getInterfaceAddress(): interfaceAddress=" + d.interfaceAddress);
		...
		private void p2pConfigWfdSink() {
			...
			mWifiNative.setP2pDeviceType("8-0050F204-2");
			//ALPS01511867: disable DEVICE_CAPAB_INVITATION_PROCEDURE
			if (mVerboseLoggingEnabled) logd("[wfd sink] p2pConfigWfdSink() ori deviceCapa = " + mDeviceCapa);
		}
		...
		private boolean isWfdSinkEnabled() {
			if (!SystemProperties.get("ro.vendor.mtk_wfd_sink_support").equals("1")) {
				if (mVerboseLoggingEnabled) logd("[wfd sink] isWfdSinkEnabled, property unset");
			} else if (mThisDevice.wfdInfo == null) {
				if (mVerboseLoggingEnabled) logd("[wfd sink] isWfdSinkEnabled, device wfdInfo unset");
		...
		/*
			int netId = mWifiNative.addP2pNetwork();
