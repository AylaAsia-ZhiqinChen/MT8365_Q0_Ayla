...
public class WifiConfigurationUtil {
    ...
    public static boolean hasCredentialChanged(WifiConfiguration existingConfig,
        ...
        if (TelephonyUtil.getSimSlot(existingConfig)
                != TelephonyUtil.getSimSlot(newConfig)) {
            return true;
        }
        /// M:[WAPI]
        if (com.mediatek.server.wifi.MtkWapi.hasWapiConfigChanged(existingConfig, newConfig)) {
            return true;
        }
        return false;
    }
