    ...
    private class WiredAccessoryExtconObserver extends ExtconStateObserver<Pair<Integer, Integer>> {
        ...
        public void updateState(ExtconInfo extconInfo, String name,
            ...
            synchronized (mLock) {
                ...
                // fix connected state logic for Extcon
                //
                // step 1: state 1, add new mask to mHeadsetState
                // step 2: state 0, remove mask from mHeadsetState
                int headset = mHeadsetState & ~mask | (mask & state);
                Log.i(TAG,"mConHeadsets= "+mConHeadsets+",mHeadsetState="+mHeadsetState+ ",headset="+headset);
                // Use the condition mConHeadsets to avoid updating state 0 on power-on
                if((mConHeadsets & mHeadsetState) != 0 || headset != 0){
                    updateLocked(name, headset);
                    mConHeadsets = headset;
                }
                return;
            }
        }
