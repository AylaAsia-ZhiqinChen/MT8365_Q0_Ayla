...
//ifdef MTK_WFD_SINK_SUPPORT
        ,mWifiP2pManager,mWifiP2pChannel
//endif
        );
    }
//ifdef MTK_WFD_SINK_SUPPORT
    ...
    public boolean getCertMode(){
        return mWifiDisplayCertMode;
    }

    public MtkWifiDisplayController getMtkController(){
        return mMtkController;
    }
...
//ifdef MTK_WFD_SINK_SUPPORT
        //mMtkController.enableWifiDisplay();
//endif
    }
...
//ifdef MTK_WFD_SINK_SUPPORT
        ///M:@{ WFD Sink Support
        if (SystemProperties.get("ro.vendor.mtk_wfd_sink_support").equals("1")  && mMtkController != null && mMtkController.getIfSinkEnabled()) {
            return;
        }
//endif
    ...
    public void disconnect() {
        ...
        ///M:@{ WFD Sink Support
        if (SystemProperties.get("ro.vendor.mtk_wfd_sink_support").equals("1") && mMtkController != null && mMtkController.getIfSinkEnabled()) {
            mMtkController.disconnectWfdSink();
            return;
        }
        updateConnection();
    }
        ...
        if (mConnectedDevice != null && mRemoteDisplay == null) {
            ...
            mRemoteDisplay = RemoteDisplay.listen(iface, new RemoteDisplay.Listener() {
                ...
                public void onDisplayKeyEvent(int uniCode, int flags) {
                    Slog.d(TAG, "onDisplayKeyEvent:uniCode=" + uniCode + "flags" + flags);
                }
    ...
    private void handleConnectionChanged(NetworkInfo networkInfo) {
        ...
        if (null == mDesiredDevice) {
            boolean mIsConnected_OtherP2p = networkInfo.isConnected();
            if (true == mIsConnected_OtherP2p) {
                Slog.i(TAG, "Wifi P2p connection is connected but it does not wifidisplay trigger");
                mMtkController.resetReconnectVariable();
            }
        }else{
            Slog.i(TAG, "mDesiredDevice!=null");
        }
        ///@}
...
///M: ALPS00677009: receive p2p broadcast to know the group removed reason @{
                ...
                ///M:@{ WFD Sink Support
                if (SystemProperties.get("ro.vendor.mtk_wfd_sink_support").equals("1")  && mMtkController.getIfSinkEnabled()) {
                    if (reason != -2) {
                        mMtkController.handleSinkP2PConnection(networkInfo);
                    }
                    return;
                }
//endif
...
//ifdef MTK_WFD_SINK_SUPPORT
                ...
                mMtkController.updateDeviceName(mSinkDeviceName);
                Slog.i(TAG, "Received WFD_SINK_GC_REQUEST_CONNECT, mSinkDeviceName:" + mSinkDeviceName);

                if (!mMtkController.isSinkWaitingP2pConnectionState()) {
                ...
                if (mSinkDeviceName != null) {
                    ...
                    mMtkController.showDialog(mMtkController.WFD_CONFIRM_CONNECT_DIALOG);
                }
            }
//endif
        }
