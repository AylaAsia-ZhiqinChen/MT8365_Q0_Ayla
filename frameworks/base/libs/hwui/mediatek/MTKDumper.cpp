...
public:
    ...
    DumpTask(const SkBitmap* b, const char* f, bool c):
        width(b->width()), height(b->height()), size(b->computeByteSize()), compress(c), flip(false) {
        memcpy(filename, f, 512);
...
bool Dumper::addTask(DumpTask *task) {
    task->preProcess();
    if (mThreads.size() > 0) {
        size_t minQueueSize = MAX_BUFFER_SIZE / mThreadCount / task->bitmap.computeByteSize();
