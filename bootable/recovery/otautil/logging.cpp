...
void copy_logs(bool save_current_log, bool has_cache, const selabel_handle* sehandle) {
  ...
  //adupsfota start  
  std::string is_adupsfota = android::base::GetProperty("ro.fota.version", "");
  if (!is_adupsfota.empty()) {
    const char *LAST_ERROR_LOG_FILE = "/cache/recovery/last_error_log";
    PLOG(ERROR) << "Fota installation aborted.\n";
    copy_log_file_to_pmsg(Paths::Get().temporary_log_file(), LAST_ERROR_LOG_FILE);
    ensure_path_mounted(LAST_ERROR_LOG_FILE);
    copy_log_file(Paths::Get().temporary_log_file(), LAST_ERROR_LOG_FILE, false, sehandle);
    chmod(LAST_ERROR_LOG_FILE, 0644);
  }
  //adupsfota end
