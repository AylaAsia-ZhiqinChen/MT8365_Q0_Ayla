...
#define MTK_FEATURE_COMBINATION_AIHDR          (MTK_FEATURE_AIHDR   | MTK_FEATURE_NR| MTK_FEATURE_ABF| MTK_FEATURE_CZ| MTK_FEATURE_DRE| MTK_FEATURE_HFG| MTK_FEATURE_DCE | MTK_FEATURE_FB)
...
#define MTK_FEATURE_COMBINATION_ISP_HIDL_SOURCE_COMMON  (NO_FEATURE_NORMAL | MTK_FEATURE_NR| MTK_FEATURE_CZ| MTK_FEATURE_DRE| MTK_FEATURE_HFG| MTK_FEATURE_DCE| MTK_FEATURE_DSDN)
...
// add scenario/feature set for camera scenario
    ...
    if (scenarioHint.captureScenarioIndex > 0) {  // force by vendor tag (ex:Pro mode))
        ...
        if (scenarioHint.dualDevicePath == static_cast<int32_t>(DualDevicePath::MultiCamControl)) {
            if (scenarioHint.multiCamFeatureMode == MTK_MULTI_CAM_FEATURE_MODE_VSDOF) {
                ...
                MY_LOGD("Multicam scenario! (normal set)");
...
bool mtk_update_capture_feature_combination(
...
) -> bool
{
    ...
    auto scenarioFeaturesMap = gMtkScenarioFeaturesMaps[0];
    ...
    switch (scenario) {
        ...
        default:
            ...
            if (gIsLowMemoryDevice) {
                ...
                captureConfig.maxAppJpegStreamNum = 3;
            }
            else {
                //for best CShot performance
...
)
{
    return mtk_get_capture_scenario(scenario, scenarioHint, pAppMetadata) &&
           mtk_get_capture_config_by_scenario(scenario, scenarioHint, captureConfig) &&
           mtk_get_features_table_by_scenario(scenario, scenarioFeatures);
}
...
)
{
    return mtk_get_streaming_scenario(scenario, scenarioHint, pAppMetadata) &&
           mtk_get_features_table_by_scenario(scenario, scenarioFeatures) &&
           mtk_get_streaming_config(scenario, scenarioFeatures, scenarioConfig, scenarioHint, pAppMetadata);
}
