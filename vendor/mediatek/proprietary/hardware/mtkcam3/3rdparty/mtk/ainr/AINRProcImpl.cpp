...
public:
    ...
    virtual MERROR negotiate(Selection& sel)
    {
        ...
        if(CC_UNLIKELY(mEnable == 1)) {
            ainrLogD("Force on AINR");
        }
        ...
        if (sel.mRequestIndex == 0) {
            ...
            if (CC_UNLIKELY( mFlashOn && mZSDMode )) {
                ainrLogD("do not use ZSD buffers due to Flash MFNR");
                mZSDMode = MFALSE;
            }

            //auto memoryFree = sel.mState.mFreeMemoryMBytes;

            //ainrLogD("Free memory in system(%d MB)", memoryFree);

            if(mEnable == 0 || mShotCount > 1/* || memoryFree < MEM_THRES*/) {
                ainrLogE("No need to do AINR. iso(%d), shotCount(%d)"
                    , mRealIso, mShotCount.load(std::memory_order_relaxed));
                return BAD_VALUE;
            }
    ...
    void updatePerFrameMetadata(const AinrCtrler* ctrler, IMetadata* pAppMeta, IMetadata* pHalMeta, bool bLastFrame)
    {
        ...
        // update_app_setting(pAppMeta, pShotInfo);
        {
            bool bNeedManualAe = [&](){
                ...
                if (ainr::isZhdrMode(ctrler->getMfbMode()))
                    return false;
                ...
                if (ainr::isAinrMode(ctrler->getMfbMode())) {
                    if (ainr::isZsdMode(ctrler->getMfbMode()))
                        return false;
                    else
                        return true;
                }
            ...
            if (bNeedManualAe) {
                ainrLogD("We set manual AE (iso,exp)=(%d,%d ns)", ctrler->getFinalIso(), ctrler->getFinalShutterTime());
...

        {
            ...
            if (mfll::isZhdrMode(pShotInfo->getMfbMode())) {
                ainrLogE("Zhdr is not support in MFNR plugin");
                ...
                if (!helper.getSensorSize(sensorMode, sensorSize)) {
                    ainrLogW("cannot get sensor size");
                }
                else {
                    ...
                    {
                        ainrLogD("ISP profile is set(%u)", profile);
                    ...
                    {
                        ainrLogW("ISP profile is not set(%u)", profile);
                    }
                }
            }
        #endif
            ...
            if ( !IMetadata::getEntry<MINT32>(pHalMeta, MTK_PLUGIN_CUSTOM_HINT, customHintInHal) ) {
                ainrLogW("%s: cannot retrieve MTK_PLUGIN_CUSTOM_HINT from HAL metadata, assume "\
                ...
                ainrLogW("%s: MTK_PLUGIN_CUSTOM_HINT in MFNR(%d) and Hal(%d) setting are different"
    ...
    virtual void abort(vector<RequestPtr>& pRequests)
    {
    ...
    #if 0
        bool babort                  = false;
        RequestCallbackPtr     cbPtr = nullptr;
...

        {
            ...
            for (auto req : pRequests){
                ...
                if (!babort){
                    ainrLogW("Desire abort request[%d] is not found", req->mRequestIndex);
                }
            }

            if(mvRequests.empty()) {
                ainrLogD("abort() cleans all the requests");
            } else {
                ainrLogW("abort() does not clean all the requests");
            }
        }
    #endif
    };
