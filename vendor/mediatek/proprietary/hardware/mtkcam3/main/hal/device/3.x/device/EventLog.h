...
#ifndef _MTK_HARDWARE_MTKCAM_MAIN_HAL_DEVICE_3_X_DEVICE_EVENTLOG_H_
#define _MTK_HARDWARE_MTKCAM_MAIN_HAL_DEVICE_3_X_DEVICE_EVENTLOG_H_
//
    ...
    auto            print(android::Printer& printer) const
                    {
                        ...
                        for (const auto& item : mLogBuffer1) {
                            auto timeStr = mLogTool->convertToFormattedLogTime(&item.timestamp);
                            auto eventStr = item.event;

                            if(timeStr.length() > 0 && eventStr.length() > 0) {
...
#endif  //_MTK_HARDWARE_MTKCAM_MAIN_HAL_DEVICE_3_X_DEVICE_EVENTLOG_H_
