...
)   const -> int
{
    ...
    if ( it == mBufferHandleMap.end() ) {
        ...
        appBufferHandle = it->second.appBufferHandle;
    }
    //
    if ( importedBufferHandle == nullptr ) {
...
)   -> int
{
    ...
    if ( it == mBufferHandleMap.end() ) {
        mBufferHandleMap[bufferId] = {
            ...
            .appBufferHandle = appBufferHandle,
    ...
    else if ((it->second.appBufferHandle)->bufferHandle != appBufferHandle->bufferHandle){
         MY_LOGE("bufferId %" PRIu64 "has different AppStreamBufferHandles:%p != %p",
            bufferId, (it->second.appBufferHandle)->bufferHandle, appBufferHandle->bufferHandle);
         return -ENODEV;
    }
    // MY_LOGD("bufferId %-3" PRIu64 " -> %p %3d", bufferId, importedBufferHandle, importedBufferHandle->data[0]);
    //
...
clear() -> void
{
    ...
    for (auto& pair : mBufferHandleMap) {
        //check if the buffer would be freed by others, if yes, skip freeing the buffer here
        if ( !mBuffersFreedByOthers.empty() ){
            std::unordered_set<uint64_t>::const_iterator iter = mBuffersFreedByOthers.find(pair.first);
            if ( iter != mBuffersFreedByOthers.end() ){
                MY_LOGI("skip clearing bufferId %-3" PRIu64 " -> fd %d, which must be a pre-released buffer", pair.first, pair.second.ionFd);
                continue;
            }
        }
        if (pair.second.appBufferHandle.use_count() > 1){
            MY_LOGW("Buffer won't be freed after clearing! bufferId: %" PRIu64 ", buffer: %p, fd: %d, count: %ld", pair.first, pair.second.bufferHandle, pair.second.ionFd, pair.second.appBufferHandle.use_count());
        }
...
auto
ThisNamespace::
markBufferFreeByOthers(uint64_t bufferId) -> void
{
    FUNC_START;
    mBuffersFreedByOthers.insert(bufferId);
    MY_LOGD("bufferId:%" PRIu64 " would be freed by others", bufferId);
    FUNC_END;
}
