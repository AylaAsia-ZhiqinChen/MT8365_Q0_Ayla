...
public:
     ...
     *  @param commonDump       - Save file using common utility
     *  @return                 - Returns MfllErr_Ok if ok.
     */
    virtual enum MfllErr saveFile(const char *name, bool commonDump = false) = 0;
    ...
    /**
     *  helper function to translate between mfll::ImageFormat and NSCam::eImgFmt
     *
     *  @param fmt              Format.
     */
    virtual unsigned int tranImageFormat(const enum ImageFormat &fmt) = 0;
    virtual enum ImageFormat tranImageFormat(const unsigned int &fmt) = 0;
