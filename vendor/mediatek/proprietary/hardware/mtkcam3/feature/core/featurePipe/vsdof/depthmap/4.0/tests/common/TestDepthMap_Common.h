...
{\
    ::property_set("depthmap.pipe.dump.size", size);\
    ::property_set("depthmap.pipe.dump.start", start);\
}
...
private:
    IHalSensor *mpHalSensor_Main1 = nullptr;
    IHalSensor *mpHalSensor_Main2 = nullptr;
    int mSensorCount;
    MUINT mvSensorIndex[2];
    bool mIsReadyToUT;
    char *mUsername;
    Profile mProfile;
};
...
MBOOL prepareReqOutputBuffer(DepthMapPipeOpState opType, sp<IDepthMapEffectRequest> pRequest);
// capture
MBOOL prepareReqInputBuffer_Capture(sp<IDepthMapEffectRequest> pRequest);
MBOOL prepareReqOutputBuffer_Capture(DepthMapPipeOpState opType, sp<IDepthMapEffectRequest> pRequest);
