...
: miFrameID(frameID)
{
    ...
    mPass2EnqueFrame.UniqueKey = streamTag;
}
...
)
{
    ...
    MY_LOGE("wrong LMV region count %zu", entry.count());
    return MFALSE;
}
        ...
        for(size_t i=0;i<frameParam.mvExtraParam.size(); i++)
        {
            ...
            else if(extraParam.CmdIdx == EPIPE_MDP_PQPARAM_CMD)
            {
                ...
                if(param->WDMAPQParam != nullptr)
                {
                    ...
                    MY_LOGD("WDMAPQParam defaultDownTable = %d, IBSEGain=%d switchRatio6Tap6nTap=%d switchRatio6nTapAcc=%d", vsdofParam.defaultDownTable, vsdofParam.IBSEGain, vsdofParam.switchRatio6Tap6nTap, vsdofParam.switchRatio6nTapAcc);
                }
                if(param->WROTPQParam != nullptr)
                {
                    ...
                    MY_LOGD("WROTPQParam lensId = %d, isRefocus=%d defaultUpTable=%d",
                            ispParam.lensId , vsdofParam.isRefocus, vsdofParam.defaultUpTable);
                    MY_LOGD("WROTPQParam defaultDownTable = %d, IBSEGain=%d switchRatio6Tap6nTap=%d switchRatio6nTapAcc=%d", vsdofParam.defaultDownTable, vsdofParam.IBSEGain, vsdofParam.switchRatio6Tap6nTap, vsdofParam.switchRatio6nTapAcc);
                }
...
)
{
    ...
    else
    {
        ...
        // load tuning
        StereoTuningProvider::getMDPPQParam(pDpPqParam->u.isp.vsdofParam);
    }
    //
...
)
{
    ...
    else
    {
        ...
        // load tuning
        StereoTuningProvider::getMDPPQParam(rDpPqParam.u.isp.vsdofParam);
    }
    //
