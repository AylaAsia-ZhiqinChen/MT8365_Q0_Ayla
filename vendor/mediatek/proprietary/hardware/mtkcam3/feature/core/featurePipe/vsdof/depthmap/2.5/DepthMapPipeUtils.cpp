...
MVOID debugQParams(const QParams& rInputQParam)
{
    ...
    for(size_t index = 0; index < rInputQParam.mvFrameParams.size(); ++index)
    {
        ...
        MY_LOGD("Frame Expected End Time (tv_sec) = %d", frameParam.ExpectedEndTime.tv_sec);
        MY_LOGD("Frame Expected End Time (tv_usec) = %d", frameParam.ExpectedEndTime.tv_usec);
...
)
{
    ...
    {
        MY_LOGE("Failed to get ISO value. Meta=%x", inApp);
        return MFALSE;
    }
