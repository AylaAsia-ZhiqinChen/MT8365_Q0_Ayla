...
void CookieStore<T_User>::enque(T_User *user, T_Stream *stream, T_Param param, const T_Data &data)
{
    ...
    {
        CAM_ULOGE(NSCam::Utils::ULog::MOD_FPIPE_STREAMING, "onCookieEnque failed, token=%p", token);
...
void CookieStore<T_User>::staticProcessCB(const T_Msg &msg, T_Param param, T_Token token)
{
    ...
    if( !T_Store::fetch(token, store, user, cookie, data) )
    {
        CAM_ULOGE(NSCam::Utils::ULog::MOD_FPIPE_STREAMING, "Invalid data from token=%p, store=%p, user=%p", token, store, user);
    }
    else if( !user )
    {
        CAM_ULOGE(NSCam::Utils::ULog::MOD_FPIPE_STREAMING, "Can not find user from token. Token release. store=%p", store);
    }
    else
    {
        cookie.restore(param);
...
bool CookieStore<T_User>::fetch(T_Token token, T_Store* &store, T_User* &user, T_Cookie &cookie, T_Data &data)
{
    ...
    if( backup )
    {
        ...
        {
            CAM_ULOGE(NSCam::Utils::ULog::MOD_FPIPE_STREAMING, "Backup data is corrupted: token=%p backup=%p magic=0x%x", token, backup, backup->mMagic);
        }
        else
        {
