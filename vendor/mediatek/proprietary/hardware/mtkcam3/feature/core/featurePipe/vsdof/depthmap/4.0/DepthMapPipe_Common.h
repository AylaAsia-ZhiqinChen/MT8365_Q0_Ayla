...
// ======== public id section start ==========
    ...
    // P1YUV is rect_in1
    BID_P2A_IN_RECT_IN1 = PBID_IN_P1YUV1,
    // output buffer
    ...
    BID_DLDEPH_DPE_OUT_DEPTH = PBID_OUT_DEPTHMAP,
    BID_DPE_OUT_UNPROCESS_DEPTH = PBID_OUT_DEPTHMAP,
...
enum DepthMapDataID {
    ...
    N3D_TO_P2ABYER_P1YUV_USED,
    // WPENode
    WPE_TO_DPE_WARP_IMG,
    WPE_TO_DLDEPTH_MV_SV,
    // DPE
    DPE_TO_GF_DMW_N_DEPTH,
    DPE_OUT_INTERNAL_DEPTH,
    // DLDEPTH
...
#define VSDOF_LOGD(fmt, arg...) \
    do { if(DepthPipeLoggingSetup::mbDebugLog >= 1) {MY_LOGD("%d: " fmt, __LINE__, ##arg);} } while(0)
#define VSDOF_LOGD2(fmt, arg...) \
    do { if(DepthPipeLoggingSetup::mbDebugLog >= 2) {MY_LOGD("%d: " fmt, __LINE__, ##arg);} } while(0)
