...
CamGraph<Node_T>::~CamGraph()
{
  ...
  {
    CAM_ULOGE(NSCam::Utils::ULog::MOD_FPIPE_COMMON, "Error: CamGraph need to be disconnected before destroy");
  }
}
...
template <typename Node_T>
MBOOL CamGraph<Node_T>::connectData(DataID_T srcID, DataID_T dstID, Node_T &srcNode, Node_T &dstNode, ConnectionType type)
{
  MBOOL ret = MFALSE;
  android::Mutex::Autolock lock(mMutex);
  if( mStage == STAGE_IDLE )
  {
    ...
    ret = srcNode.connectData(srcID, dstID, &dstNode, type);
    if( ret )
    {
...
template <typename Node_T>
MBOOL CamGraph<Node_T>::connectData(DataID_T src, DataID_T dst, Node_T &node, Handler_T *handler, ConnectionType type)
{
  MBOOL ret = MFALSE;
  android::Mutex::Autolock lock(mMutex);
  if( mStage == STAGE_IDLE )
...
MBOOL CamGraph<Node_T>::disconnect()
{
  ...
  if( mStage == STAGE_IDLE )
  {
    ...
    for( it = mNodes.begin(), end = mNodes.end(); it != end; ++it )
    {
      if( !(*it)->disconnect() )
      {
        CAM_ULOGE(NSCam::Utils::ULog::MOD_FPIPE_COMMON, "disconnect failed");
      }
    }
    mRootNode = NULL;
    mNodes.clear();
...
MBOOL CamGraph<Node_T>::init()
{
  ...
  if( mStage != STAGE_IDLE )
  {
    CAM_ULOGE(NSCam::Utils::ULog::MOD_FPIPE_COMMON, "invalid stage(%d)", mStage);
  }
  else if( !mRootNode )
  {
    CAM_ULOGE(NSCam::Utils::ULog::MOD_FPIPE_COMMON, "mRootNode not set");
  }
  for( it = mNodes.begin(), end = mNodes.end(); it != end; ++it )
  {
    if( !(*it)->init() )
...
MBOOL CamGraph<Node_T>::uninit()
{
  ...
  if( mStage == STAGE_READY )
  {
    ...
    for( it = mNodes.rbegin(), end = mNodes.rend(); it != end; ++it )
    {
      if( !(*it)->uninit() )
      {
        CAM_ULOGE(NSCam::Utils::ULog::MOD_FPIPE_COMMON, "%s uninit failed", (*it)->getName());
      }
    }
    mStage = STAGE_IDLE;
    ret = MTRUE;
...
MBOOL CamGraph<Node_T>::stop()
{
  ...
  if( mStage == STAGE_RUNNING )
  {
    ...
    if( mFlushOnStop )
    {
      CAM_ULOGD(NSCam::Utils::ULog::MOD_FPIPE_COMMON, "flush on stop");
      this->setFlow(MFALSE);
      this->waitFlush();
      this->waitSync();
    }
    ...
    {
      CAM_ULOGD(NSCam::Utils::ULog::MOD_FPIPE_COMMON, "sync on stop");
      this->waitSync();
      this->setFlow(MFALSE);
    }
    for( it = mNodes.begin(), end = mNodes.end(); it != end; ++it )
    {
      if( !(*it)->stop() )
      {
        CAM_ULOGE(NSCam::Utils::ULog::MOD_FPIPE_COMMON, "%s stop failed", (*it)->getName());
      }
    }
    mStage = STAGE_READY;
    ret = MTRUE;
...
MBOOL CamGraph<Node_T>::enque(DataID_T id, BUFFER_T &buffer)
{
  ...
  if( mStage == STAGE_RUNNING )
  {
    CAM_NODE_ULOGT_ENTER("CamGraph", mRootNode, buffer, NSFeaturePipe::getULogRequestSerial(buffer));
...
MBOOL CamGraph<Node_T>::broadcast(DataID_T id, MSG_T &msg)
{
  ...
  if( mStage == STAGE_RUNNING )
  {
    ...
    for( it = mNodes.begin(), end = mNodes.end(); it != end; ++it )
    {
      CAM_NODE_ULOGT_ENTER("CamGraph", (*it), msg, NSFeaturePipe::getULogRequestSerial(msg));
      (*it)->onData(id, msg);
    }
    ret = MTRUE;
  }
...
template <typename Node_T>
CamGraph<Node_T>::Watchdog::Watchdog(CamGraph<Node_T> *parent)
  : mParent(parent)
{
  TRACE_FUNC_ENTER();
  TRACE_FUNC_EXIT();
...
MBOOL CamGraph<Node_T>::Watchdog::onNotify()
{
  ...
  if( mParent != NULL )
  {
    ULogPrinter printer(NSCam::Utils::ULog::MOD_FPIPE_COMMON, "MtkCam/FeaturePipe", DetailsType::DETAILS_WARNING);
    mParent->onDumpStatus(printer);
  }
  TRACE_FUNC_EXIT();
  return MTRUE;
}
...
MVOID CamGraph<Node_T>::waitFlush(unsigned watchdog_ms)
{
  ...
  if( watchdog_ms > 0 )
  {
    android::sp<TimeoutCB> timeoutCB = new TimeoutCB(watchdog_ms);
    timeoutCB->insertCB(new BacktraceNotifyCB());
    timeoutCB->insertCB(new Watchdog(this));
...
MVOID CamGraph<Node_T>::waitSync(unsigned watchdog_ms)
{
  ...
  for( it = mNodes.begin(), end = mNodes.end(); it != end; ++it )
  {
    (*it)->registerSyncCB(NULL);
  }
  TRACE_FUNC_EXIT();
}

template <typename Node_T>
MVOID CamGraph<Node_T>::onDumpStatus(android::Printer &printer) const
{
  TRACE_FUNC_ENTER();
  NODE_SET_ITERATOR it, end;
  for( it = mNodes.begin(), end = mNodes.end(); it != end; ++it )
  {
    (*it)->dumpWaitQueueInfo(printer);
    (*it)->dumpCamThreadInfo(printer);
  }
  TRACE_FUNC_EXIT();
}
