...
#define MAKE_NAME_CASE(name) \
  ...
  switch(id)
  {
    ...
    MAKE_NAME_CASE(P2A_TO_DPE_MY_S);
    MAKE_NAME_CASE(N3D_TO_WPE_RECT2_WARPMTX);
    MAKE_NAME_CASE(N3D_TO_P2ABYER_P1YUV_USED);
    MAKE_NAME_CASE(WPE_TO_DPE_WARP_IMG);
    MAKE_NAME_CASE(WPE_TO_DLDEPTH_MV_SV);
    MAKE_NAME_CASE(DPE_TO_GF_DMW_N_DEPTH);
    ...
    MAKE_NAME_CASE(DPE_OUT_INTERNAL_DEPTH);
...
, mNodeId(nodeID)
{
    ...
    MY_LOGD("mbDebugLog=%d mbProfileLog=%d, miDumpBufSize=%d miDumpStartIdx=%d mbDumpCapture=%d",
            DepthPipeLoggingSetup::mbDebugLog, DepthPipeLoggingSetup::mbProfileLog,
            miDumpBufSize, miDumpStartIdx, mbDumpCapture);
}
...
#define MAKE_NAME_CASE(name) \
    ...
    switch(BID)
    {
        ...
        MAKE_NAME_CASE(BID_P2A_IN_RECT_IN1); //P1YUV
        ...
        MAKE_NAME_CASE(BID_DLDEPH_DPE_OUT_DEPTH);
