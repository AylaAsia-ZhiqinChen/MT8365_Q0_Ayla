...
sb<T> BufferPool<T>::request()
{
  ...
  do
  {
            ...
            ((mPool.size() + mAllocatingCount) < (MUINT32)mAutoAllocate) )
    {
      ...
      if( !this->allocate() )
      {
        CAM_ULOGE(NSCam::Utils::ULog::MOD_FPIPE_COMMON, "%s: Auto allocate attemp failed", mName);
      }
      mMutex.lock();
      --mAllocatingCount;
    }
...
template <typename T>
sp<IIBuffer> BufferPool<T>::requestIIBuffer()
{
  TRACE_FUNC_ENTER();
  TRACE_FUNC_EXIT();
  return NULL;
...
MVOID BufferPool<T>::releaseAll()
{
  ...
  {
    CAM_ULOGE(NSCam::Utils::ULog::MOD_FPIPE_COMMON, "%s: Some buffers are not released before pool released",
            mName ? mName : "unknown pool");
  }
