...
{
    TRACE_FUNC_ENTER();
    MY_LOGD_IF(getLogLevel(), "Acquire buffer %s", mName.c_str());
    if( mBuffer != NULL )
    {
...
{
    TRACE_FUNC_ENTER();
    MY_LOGD_IF(getLogLevel(), "Release buffer %s", mName.c_str());
    if( mBuffer != NULL )
    {
...
MBOOL NativeBufferWrapper::allocate(MUINT32 width, MUINT32 height, android_pixel_format_t format, MUINT64 usage)
{
    ...
    int ret = 0;
    MY_LOGD_IF(getLogLevel(), "Allocate Buffer = %s, w(%d), h(%d), format(%d), usage(%" PRIi64 ")",
...
MUINT32 NativeBufferWrapper::getLogLevel()
{
    static MUINT32 level = ::property_get_int32("vendor.debug.fpipe.nativebuffer", 0);
    return level;
}
