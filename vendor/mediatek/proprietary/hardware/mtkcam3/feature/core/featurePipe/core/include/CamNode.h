...
MBOOL CamNode<Handler_T>::init()
{
  ...
  if( mStage == STAGE_IDLE )
  {
    ...
    if( !this->onInit() )
    {
      CAM_ULOGE(NSCam::Utils::ULog::MOD_FPIPE_COMMON, "%s onInit() failed", this->getName());
    }
    else
    {
      mStage = STAGE_READY;
...
MBOOL CamNode<Handler_T>::start()
{
  ...
  if( mStage == STAGE_READY )
  {
    if( !this->onStart() )
    {
      CAM_ULOGE(NSCam::Utils::ULog::MOD_FPIPE_COMMON, "%s onStart() failed", this->getName());
    }
    else
    {
...
MBOOL CamNode<Handler_T>::stop()
{
  ...
  if( mStage == STAGE_RUNNING )
  {
    if( !this->onStop() )
    {
      CAM_ULOGE(NSCam::Utils::ULog::MOD_FPIPE_COMMON, "%s onStop() failed", this->getName());
    }
    mStage = STAGE_READY;
    ret = MTRUE;
  }
...
MBOOL CamNode<Handler_T>::handleData(DataID_T id, const BUFFER_T &buffer)
{
  TRACE_FUNC_ENTER();
  TRACE_FUNC("%s", getName());
  CAM_NODE_ULOGT_EXIT("CamNode", this, buffer, NSFeaturePipe::getULogRequestSerial(buffer));
  MBOOL ret = MFALSE;
  android::Mutex::Autolock _l(mNodeLock);
  if( mStage == STAGE_RUNNING && mAllowDataFlow )
  {
    ...
    if( it != mHandlerMap.end() )
    {
      ...
      if( handler )
      {
        ...
        {
          CAM_NODE_ULOGT_ENTER("CamNode", handler, buffer, NSFeaturePipe::getULogRequestSerial(buffer));
        ...
        {
          CAM_ULOGE(NSCam::Utils::ULog::MOD_FPIPE_COMMON, "%s: (%d:%s) use unsupported connection type(%d)", getName(), id, Handler_T::ID2Name(id), type);
        }
      }
    }
    ...
    if( !ret )
    {
      CAM_ULOGE(NSCam::Utils::ULog::MOD_FPIPE_COMMON, "%s: handleData(%d:%s) failed", getName(), id, Handler_T::ID2Name(id));
    }
  }
  TRACE_FUNC_EXIT();
  return ret;
