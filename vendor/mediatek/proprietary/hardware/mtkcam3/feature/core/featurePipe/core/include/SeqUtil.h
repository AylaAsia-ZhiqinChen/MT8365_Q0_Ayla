...
void SequentialQueue<T, SeqConverter>::enque(const T &val)
{
  ...
  {
    CAM_ULOGD(NSCam::Utils::ULog::MOD_FPIPE_COMMON, "seq(%d) != enque(%d)", mSeq, mSeqConverter(val));
  }
  TRACE_FUNC_EXIT();
...
{
  TRACE_FUNC_ENTER();
  CAM_ULOGE(NSCam::Utils::ULog::MOD_FPIPE_COMMON, "Please define Handler_T::getSeq() before using SequentialHandler");
  TRACE_FUNC_EXIT();
...
MBOOL SequentialHandler<Handler_T, typename std::enable_if< Handler_T::supportSeq >::type>::onData(DataID_T id, const DATA_T &data, Handler_T *handler)
{
  ...
  {
    CAM_NODE_ULOGT_ENTER("SeqUtil", handler, data, NSFeaturePipe::getULogRequestSerial(data));
    handler->onData(id, d);
  }
