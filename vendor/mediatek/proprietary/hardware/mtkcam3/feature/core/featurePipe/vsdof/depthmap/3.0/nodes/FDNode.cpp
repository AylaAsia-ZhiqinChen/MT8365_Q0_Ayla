...
onThreadLoop()
{
    ...
    //Do FD
    {
        VSDOF_PRFLOG("halFDDo start, reqId=%d   %p  %p   %d  %p", pRequest->getRequestNo(), mpDDPBuffer,  mpExtractYBuffer, jpegOrientation, pFDBuf->getBufPA(0));
    	// timer
...
mergeExtraData(DepthMapRequestPtr& pRequest)
{
    ...
    if(pExtraDataImgBuf != nullptr)
    {
        ...
        {
            MY_LOGE("Extra data length is larger than the output buffer size, ExtraData size=%d , output buffer size=%zu", strlen(sExtraData), pExtraDataImgBuf->getBufSizeInBytes(0));
        }
        else
        {
