...
#include <mtkcam3/feature/effectHalBase/BasicParameters.h>
...
BasicParameters() : mMap()
{
    ...
    // if ( 0 == mLogLevel ) {
    //     ::property_get("vendor.debug.camera.log.basicParameter", cLogLevel, "0");
    //     mLogLevel = ::atoi(cLogLevel);
    // }
...
set(const char *key, int value)
{
    FUNCTION_LOG_START;
    char str[16]={0};
    snprintf(str, sizeof(str)-1, "%d", value);
    set(key, str);
    FUNCTION_LOG_END;
}
...
setInt64(const char *key, int64_t value)
{
    FUNCTION_LOG_START;
    char str[32]={0};
    snprintf(str, sizeof(str)-1, "%" PRId64 "", value);
...
setPtr(const char *key, void *value)
{
    ...
    snprintf(str, sizeof(str)-1, "%#" PRIxPTR "", val);
...
{
    FUNCTION_LOG_START;
    char str[16]={0};  // 14 should be enough. We overestimate to be safe.
    snprintf(str, sizeof(str)-1, "%g", value);
    set(key, str);
    FUNCTION_LOG_END;
}
