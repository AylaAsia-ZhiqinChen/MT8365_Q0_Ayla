...
static void* thread_allocate_memory(void *arg)
{
    ...
    if (bIsMultiFrame) {
        /* mfllcore1.4 dont need MC working buffer */
...
enum MfllErr MfllCore::init(const MfllConfig_t &cfg)
{
    ...
    /* update nvram debug info */
    {
        size_t chunkSize = 0;
        const char *pChunk = m_spNvramProvider->getChunk(&chunkSize);
        if (pChunk == NULL) {
            err = MfllErr_UnexpectedError;
        ...
        updateExifInfo((unsigned int)MF_TAG_ISO_HI_TH,(unsigned int)pNvram->mfll_iso_th);
        updateExifInfo((unsigned int)MF_TAG_ISO_LOW_TH,(unsigned int)pNvram->mfll_iso_th);
        updateExifInfo((unsigned int)MF_TAG_AIS_ADVANCED_MAX_ISO,(unsigned int)pNvram->ais_advanced_max_iso);
        updateExifInfo((unsigned int)MF_TAG_AIS_ADVANCED_MAX_EXPOSURE,(unsigned int)pNvram->ais_advanced_max_exposure);
        updateExifInfo((unsigned int)MF_TAG_AIS_ADVANCED_ENABLE,(unsigned int)pNvram->ais_advanced_tuning_en);
        updateExifInfo((unsigned int)MF_TAG_AIS_EXPOSURE_TH1,(unsigned int)pNvram->ais_exp_th);
        updateExifInfo((unsigned int)MF_TAG_AIS_EXPOSURE_TH0,(unsigned int)pNvram->reserved[0]);
        updateExifInfo((unsigned int)MF_TAG_AIS_ISO_TH0,(unsigned int)pNvram->reserved[1]);
    }
    ...
    MfllMotionVector        mv)
{
    ...
    if (m_bypass.bypassEncodeQYuv[requestNo]) {
        ...
        /* Format yv16 for MEMC */
        yuy2_to_yv16((MUINT8*)quarterImg->getVa(), (MUINT8*)quarterImg->getVa(), quarterImg->getAlignedWidth(), quarterImg->getAlignedHeight());
...
enum MfllErr MfllCore::setQueueYuvFramesCounts(uint32_t nums)
{
    ...
    /* if Queue frame number is not equal to capture frame number,using single capture */
    if (getCaptureFrameNum() > nums) {
        mfllLogW("Queue frame number(%d) is not equal to capture frame number(%d),using single capture ",nums,getCaptureFrameNum());
        ...
        )
{
    ...
    if (type == MixYuvType_Base || buffer == nullptr) {
        m_imgYuvMixedWorking = nullptr;
        m_ptrImgYuvMixedWorking = nullptr;
    }
    else {
        m_imgYuvMixedWorking = buffer;
...
MixYuvBufferType MfllCore::getMixingBufferType()
{
    ...
    if (m_mixYuvBufferType == MixYuvType_Working) {
        if (CC_UNLIKELY(m_imgYuvMixedWorking == nullptr)) {
...
enum MfllErr MfllCore::do_CaptureSingle(void *void_index)
{
    ...
    if (CC_LIKELY(index >= 0 && index < getCaptureFrameNum()))
    {
        ...
        if (index == 0) {
            ...
            m_dbgInfoCore.frameCapture = m_frameCapture;
            //m_caputredCount = m_frameCapture;
        ...
        /* prepare output buffer */
        {
            /* prepare raws for isp3.0 */
            raws    = &m_imgRaws[index];
            qyuvs   = &m_imgQYuvs[index];
        ...
        err = m_spCapturer->captureFrames(index, raws, qyuvs, gmvs, rStatus);
        if (CC_UNLIKELY(err != MfllErr_Ok)) {
            mfllLogE("%s: MfllCapture::captureFrames failed with code %d", __FUNCTION__, err);
            goto lbExit;
        }
...
lbExit:
    if (status.ignore || err != MfllErr_Ok) {
        ...
        if (getMixingBufferType() == MixYuvType_Working) {
            ...
            IMfllImageBuffer* pImgSrc = (*m_ptrImgYuvGolden).get();
            IMfllImageBuffer* pImgDst = (*m_ptrImgYuvMixed).get();
...
enum MfllErr MfllCore::doAllocYuvBase(JOB_VOID)
{
    ...
    if (m_bypass.bypassAllocYuvBase || status.ignore) {
        ...
        if (!pImg->isInited()) {
            ...
            pImg->setResolution(m_width, m_height);
            pImg->setAligned(2, 2); // always uses 2 pixels aligned
            err = pImg->initBuffer();
