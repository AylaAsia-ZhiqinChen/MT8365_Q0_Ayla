...
void MfllFeatureMrp::releaseStageBlending(IMfllCore *pCore, unsigned int index)
{
    ...
    if (MfllProperty::getMfbP2DirectLink()) {
        mfllLogD("don't relase any buffer after MFB blending due to direct link");
        mfllFunctionOut();
        return;
    }
...
void MfllFeatureMrp::releaseStageMixing(IMfllCore *pCore)
{
    ...
    pCore->releaseBuffer(MfllBuffer_DCESOWorking);
    for (auto i = 0 ; i < pCore->getBlendFrameNum() ; i++) {
        pCore->releaseBuffer(MfllBuffer_ConfidenceMap);
        pCore->releaseBuffer(MfllBuffer_MationCompensationMv);
    }
