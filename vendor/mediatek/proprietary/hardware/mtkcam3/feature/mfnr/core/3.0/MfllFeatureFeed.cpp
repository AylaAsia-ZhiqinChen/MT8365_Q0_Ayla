...
)
{
    ...
    case EventType_Blending:
        if ( m_feedFlag.mfb && (!status.ignore) ) {
            ...
            // [in] mcmv
            pImg = c->retrieveBuffer(MfllBuffer_MationCompensationMv, index).get();
            _read_image(pImg, MFLL_FEED_MFB_IN_MCMV, index);
    ...
    case EventType_Mixing:
        if ( m_feedFlag.mix && (!status.ignore) ) {
            ...
            // [in] dceso
            pImg = c->retrieveBuffer(MfllBuffer_DCESOWorking).get();
            _read_image(pImg, MFLL_FEED_MIX_IN_DCESO, index);
        }
        break;
    // }}}
