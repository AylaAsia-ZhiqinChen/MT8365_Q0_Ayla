        ...
        MfllStrategyConfig_t *out)
{
    ...
    /* checks ISO only */
    {
        ...
        if (finalCfg.isAis != 0) { // AIS (MFNR with changing pline mechanism)
            ...
            mfllLogD("%s:[AIS] Set enableMfb to %d due to Pline(%d)/MFNR(%d) will be triggered",
        ...
        mfllLogD("%s: iso:%u, origin_iso:%u, pline_iso_th:%d, mfll_iso_th:%d, finalCfg(enableMfb:%d, frameCapture:%d)",
                __FUNCTION__, cfg.iso, cfg.original_iso, pNvram->pline_iso_th, pNvram->mfll_iso_th, finalCfg.enableMfb, finalCfg.frameCapture);
    }
