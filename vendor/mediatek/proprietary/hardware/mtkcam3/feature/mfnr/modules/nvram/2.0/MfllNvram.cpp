...
//-----------------------------------------------------------------------------
static std::shared_ptr<char> get_nvram_chunk(IMfllNvram::ConfigParams& params)
{
    ...
    params.iQueryIndex_Bss = params.iQueryIndex_Mfnr = params.iQueryIndex_MfnrTh = doQueryIndex(params);

    if (CC_UNLIKELY(params.iQueryIndex_Mfnr < 0 || params.iQueryIndex_Mfnr >= NVRAM_MFNR_TBL_NUM_2)) {
        mfllLogE("%s: nvramIndex invalid(%d)", __FUNCTION__, params.iQueryIndex_Mfnr);
        params.iQueryIndex_Bss = params.iQueryIndex_Mfnr = params.iQueryIndex_MfnrTh = -1;
        return nullptr;
    }

    int key = NVRAM_ENCODE_KEY(params.iSensorId, params.iQueryIndex_Mfnr);
