...
private:
    ...
    IMfllImageBuffer_Flag_t m_flag; // flag
...
public:
    ...
    enum MfllErr saveFile(const char *name, bool commonDump = false);
    enum MfllErr loadFile(const char *name);
    unsigned int tranImageFormat(const enum ImageFormat &fmt);
    enum ImageFormat tranImageFormat(const unsigned int &fmt);
