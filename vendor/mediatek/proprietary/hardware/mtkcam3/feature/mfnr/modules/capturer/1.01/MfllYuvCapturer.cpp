...
)
{
    ...
    for (size_t i = 0; i < captureFrameNum; i++) {
        ...
        if (CC_UNLIKELY(eventStatusYuv.ignore != 0)) { // Ignored
            ...
            if (bFullYuv) {
                // full size MC, wait both source image and Qyuv
                auto _imgsrc = dequeSourceImg();
                auto _imgqyuv = dequeQyuv();
                (*yuvs[i])  = _imgsrc;
                (*qyuvs[i]) = _imgqyuv;
            }
            else {
...
)
{
    ...
    /* is need full raw */
    bool bFullRaw = raws != nullptr ? true : false;
    ...
    /* handle RAW to YUV frames first (and then, GMVs) */
    {
        ...
        /* no raw or ignored */
        if (CC_UNLIKELY(eventStatusRaw.ignore != 0)) { // Ignored
            eventStatusRaw.err = MfllErr_NullPointer;
            eventStatusRaw.ignore = 1;
            status = 1;
        }
        else {
            if (bFullRaw) {
...
{
    {
        std::lock_guard<std::mutex> __l(m_mutexQyuv);
        m_vQyuv.push_back(img);
        m_condQyuv.notify_one();
    }
    return MfllErr_Ok;
}
...
sp<IMfllImageBuffer> MfllYuvCapturer::dequeQyuv()
{
    sp<IMfllImageBuffer> img;
    {
        std::unique_lock<std::mutex> __l(m_mutexQyuv);
        while (m_vQyuv.size() <= 0)
            m_condQyuv.wait(__l);

        img = m_vQyuv.front();
        m_vQyuv.erase(m_vQyuv.begin());
    }
    return img;
}
