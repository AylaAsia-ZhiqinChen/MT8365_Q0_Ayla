...
#define EISHAL_WaitGyroQueue(id, last, target) \
    ...
    {\
        ...
        {\
            ...
            {\
                MY_LOGD("video (%" PRIi64 ") > gyro.ts (%" PRIi64 ") => wait",target,last.ts);\
            }\
        ...
        {\
            if (UNLIKELY(mDebugDump >= 1))\
            {\
                MY_LOGD("video (%" PRIi64 ") < gyro.ts (%" PRIi64 ") => go GIS",target, last.ts);\
            }\
        }\
    }else\
    {\
...
#define EISHAL_PopGyroQueue(id, t_array, xyz_array, gyro_num) \
    ...
    {\
        ...
        if (UNLIKELY(mDebugDump >= 1))\
        {\
            MY_LOGD("Gyro(%f,%f,%f,(%" PRIi64 ")",tmp.x, tmp.y, tmp.z, tmp.ts);\
        }\
        EISHAL_GyroDataQueue_Pop(id)\
        gyro_num++;\
    }
