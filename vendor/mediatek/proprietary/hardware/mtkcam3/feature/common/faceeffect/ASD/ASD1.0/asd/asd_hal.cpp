...
CAM_ULOG_DECLARE_MODULE_ID(MOD_ASD_HAL);
...
getInstance()
{
    ...
    MY_LOGD("[Create] ASD Count: %d", gASDCount);
    if (gASDCount > 0)
    {
        MY_LOGW("[Create] Currently, not support multi-thread ASD");
        return NULL;
    }
    ::android_atomic_inc(&gASDCount);
    halASDBase *pHalASD = NULL;

    pHalASD = new halASD();
    MY_LOGD("[Create] HalASD : %p", pHalASD);
...
{
    Mutex::Autolock _l(gsLock);
    ::android_atomic_dec(&gASDCount);
    MY_LOGD("[Delete] ASD count: %d, this : %p", gASDCount, this);
...
{
#if (MTKCAM_BASIC_PACKAGE != 1) && (AUTO_SCENE_DETECT_SUPPORT == 1)
    mCount++;
    ...
    if (AFtable == NULL)
    {
        MY_LOGW("AFtable == NULL, return error");
        return E_ASD_ERR;
    }
...
void halASD::dumpASDInputImage(void *src, MUINT16 width, MUINT16 height, MUINT32 count)
{
    ...
    fwrite((void *)src, 2, ( width * height * 1), pRawFp);
    fflush(pRawFp);
    fclose(pRawFp);*/
#endif
}
