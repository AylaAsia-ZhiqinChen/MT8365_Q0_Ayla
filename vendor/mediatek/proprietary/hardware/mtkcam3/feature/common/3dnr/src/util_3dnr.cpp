...
MBOOL Util3dnr::canEnable3dnr(MBOOL isUIEnable, MINT32 iso, MINT32 isoThreshold)
{
    ...
    if (isUIEnable)
    {
        if (iso < isoThreshold)
        {
            ...
            if (mforce3dnr && ::property_get_int32("vendor.debug.camera.3dnr.fix_latency", 0))
            {
                MY_LOGD_IF(mLogLevel, "fix 3dnr latency");
                canEnable3dnr = MTRUE;
            }
        }
        else
...
void Util3dnr::prepareFeatureData(MBOOL canEnable3dnr, const NR3D::NR3DMVInfo &mvInfo,
...
//        featureEnqueParams.setFeatureMask(MASK_RSC, 0);
        return;
    }

    if (isCRZMode)
    {
        featureEnqueParams.setVar<MBOOL>(SFP_VAR::NR3D_EIS_IS_CRZ_MODE, isCRZMode);
    }

    featureEnqueParams.setVar<MBOOL>(SFP_VAR::NR3D_P2A_CAN_ENABLE_ON_FRAME, canEnable3dnr);
    featureEnqueParams.setVar<NR3D::NR3DMVInfo>(SFP_VAR::NR3D_MV_INFO, mvInfo);

    MY_LOGD_IF(canEnable3dnr && mLogLevel,
        "ID(%d) 3dnr.eis: x,y=%d,%d, gmv=%d,%d, conf=%d,%d, iso(%d) CRZ(%d)",
        mOpenId,
        mvInfo.x_int,
        mvInfo.y_int,
        mvInfo.gmvX,
    ...
    {
        featureEnqueParams.setVar<MUINT32>(SFP_VAR::NR3D_ISO, iso);
    }
    featureEnqueParams.setVar<MUINT32>(SFP_VAR::NR3D_P2A_ISO_THRESHOLD, isoThreshold);
}
...
#define INVALID_ISO_VALUE (-1)
    ...
    // === common ===
    featureEnqueParams.setVar<MBOOL>(SFP_VAR::NR3D_EIS_IS_CRZ_MODE, in.isCRZ_p2a);
    ...
    featureEnqueParams.setVar<NR3D::NR3DMVInfo>(SFP_VAR::NR3D_MV_INFO, out.mvInfo);
    ...
    // curr iso
    if (in.iso != INVALID_ISO_VALUE)
    {
        featureEnqueParams.setVar<MUINT32>(SFP_VAR::NR3D_ISO, in.iso);
    }

    // gyro
    prepareGyro(out.pGyroData, &featureEnqueParams);

    // === p2a ===
    featureEnqueParams.setVar<MBOOL>(SFP_VAR::NR3D_P2A_CAN_ENABLE_ON_FRAME, in.is3dnrOn_p2a);
    if (in.is3dnrOn_p2a)
    {
        featureEnqueParams.setVar<MUINT32>(SFP_VAR::NR3D_P2A_ISO_THRESHOLD, in.isoThr_p2a);
    }
    // === dsdn ===
    featureEnqueParams.setVar<MBOOL>(SFP_VAR::NR3D_DSDN_CAN_ENABLE_ON_FRAME, in.is3dnrOn_dsdn);
    if (in.is3dnrOn_dsdn)
    {
        featureEnqueParams.setVar<MUINT32>(SFP_VAR::NR3D_DSDN_ISO_THRESHOLD, in.isoThr_dsdn);
    }
}
...
void Util3dnr::prepareNR3DTuningInfo(const PrepareNR3DTuningParam &prepareNR3DTuningParam, NR3DTuningInfo &tuningInfo)
{
    MUINT8 ispProfile = prepareNR3DTuningParam.ispProfile;
    const NSCam::IMetadata *pMeta_InApp= prepareNR3DTuningParam.pMeta_InApp;
    const MSize &inputSize= prepareNR3DTuningParam.inputSize;
    const MRect &inputCrop= prepareNR3DTuningParam.inputCrop;
    const NR3DHwParam &Nr3dParam= prepareNR3DTuningParam.Nr3dParam;
    const NR3DMVInfo &Nr3dMvInfo= prepareNR3DTuningParam.Nr3dMvInfo;

    MINT32 isoThreshold = NR3DCustom::get_3dnr_off_iso_threshold(ispProfile, mforce3dnr);
    MINT32 iso = extractIso(pMeta_InApp);
    tuningInfo.canEnable3dnrOnFrame = canEnable3dnr(MTRUE, iso, isoThreshold);
    tuningInfo.isoThreshold = isoThreshold;
    tuningInfo.mvInfo = Nr3dMvInfo;
    tuningInfo.inputSize = inputSize;
    tuningInfo.inputCrop = inputCrop;
    prepareGyro(&tuningInfo.gyroData, NULL);
    tuningInfo.nr3dHwParam = Nr3dParam;
}
    ...
    NSCam::NSCamFeature::NSFeaturePipe::FeaturePipeParam *featureEnqueParams)
{
    ...
    if (isGyroValid)
    {
        ...
        {
            featureEnqueParams->setVar<NSCam::NR3D::GyroData>(SFP_VAR::NR3D_GYRO, gyroData);
        }
    }
    else
    {
