...
N3D_HAL_KERNEL::__logInitInfo(const STEREO_KERNEL_SET_ENV_INFO_STRUCT &initInfo)
{
    ...
    const size_t PREFIX_SIZE = 32;
    char logPrefix[PREFIX_SIZE];
    int i = 0;
    for(i = 0; i < MAX_GEO_LEVEL; i++) {
        ...
        snprintf(logPrefix, PREFIX_SIZE, "[geo_img][%d][main]", i);
        __logImgInfo(logPrefix, initInfo.geo_img[i].img_main);

        snprintf(logPrefix, PREFIX_SIZE, "[geo_img][%d][auxi]", i);
    ...
    FAST_LOGD("[dpe_convergence_offset] %d", initInfo.dpe_convergence_offset);
...
N3D_HAL_KERNEL::__logSetProcInfo(const char *prefix, const STEREO_KERNEL_SET_PROC_INFO_STRUCT &setprocInfo)
{
    ...
    if(!__DISABLE_GPU) {
        if(__useWPE[1]) {
            FAST_LOGD("%s[OutputGB_AuxiWarpMapX] %p", prefix, setprocInfo.OutputGB_AuxiWarpMapX);
            FAST_LOGD("%s[OutputGB_AuxiWarpMapY] %p", prefix, setprocInfo.OutputGB_AuxiWarpMapY);

            FAST_LOGD("%s[OutputGB_MainWarpMapX] %p", prefix, setprocInfo.OutputGB_MainWarpMapX);
            FAST_LOGD("%s[OutputGB_MainWarpMapY] %p", prefix, setprocInfo.OutputGB_MainWarpMapY);
        } else {
