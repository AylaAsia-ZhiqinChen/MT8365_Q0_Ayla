...
OCC_HAL_IMP::OCC_HAL_IMP()
    : DUMP_BUFFER(checkStereoProperty("vendor.OCCNode"))
...
OCC_HAL_IMP::OCCHALRun(OCC_HAL_PARAMS &occHalParam, OCC_HAL_OUTPUT &occHalOutput)
{
    ...
    __setOCCParams(occHalParam);
    __runOCC(occHalOutput);

    return true;
}

void
OCC_HAL_IMP::__setOCCParams(OCC_HAL_PARAMS &occHalParam)
{
    AutoProfileUtil profile(LOG_TAG, "OCC Set Proc");

    MY_LOGD_IF(LOG_ENABLED, "+");
    __dumpHint = occHalParam.dumpHint;
    ...
    __splitDisparityMapAndDump(__curProcInfo.dispL, __curProcInfo.dispR);
...
void
OCC_HAL_IMP::__runOCC(OCC_HAL_OUTPUT &occHalOutput)
{
    MY_LOGD_IF(LOG_ENABLED, "+");
    //================================
    //  Run OCC
    ...
    for(MUINT32 i = 0; i < __curResultInfo.numOfBuffer; i++) {
        ...
        if(OCC_BUFFER_TYPE_DS == __curResultInfo.bufferInfo[i].type) {
            ...
            if(StereoSettingProvider::isMTKDepthmapMode() &&
               StereoSettingProvider::getDepthmapRefineLevel() == E_DEPTHMAP_REFINE_NONE)
            {
                MY_LOGD("Use NOC as depthmap, ignore OCC_BUFFER_TYPE_DEPTH");
            } else {
            ...
            {
                if(occHalOutput.depthMap) {
                    MY_LOGD_IF(LOG_ENABLED, "Copy OCC_BUFFER_TYPE_NOC as depthmap");
                    bufferSize = MSize(occHalOutput.depthMap->getImgSize().w, occHalOutput.depthMap->getImgSize().h);
                    if(bufferSize.w*bufferSize.h == resultSize) {
                        ::memcpy((void *)occHalOutput.depthMap->getBufVA(0), __curResultInfo.bufferInfo[i].planeAddr0, resultSize);
                    } else {
...
void
OCC_HAL_IMP::__splitDisparityMapAndDump(const short *dispL, const short *dispR)
{
    if(NULL == __dumpHint) {
        return;
    }
    ...
    sprintf(fileName, "DPE_DISP_L_X_%dx%d.y", szDMP.w, szDMP.h);
    ...
    sprintf(fileName, "DPE_DISP_L_Y_%dx%d.y", szDMP.w, szDMP.h);
    genFileName_VSDOF_BUFFER(dumpPath, DUMP_PATH_SIZE, __dumpHint, fileName);
    ...
    sprintf(fileName, "DPE_DISP_R_X_%dx%d.y", szDMP.w, szDMP.h);
    genFileName_VSDOF_BUFFER(dumpPath, DUMP_PATH_SIZE, __dumpHint, fileName);
    fp = fopen(dumpPath, "wb");
    if(fp) {
    ...
    sprintf(fileName, "DPE_DISP_R_Y_%dx%d.y", szDMP.w, szDMP.h);
    genFileName_VSDOF_BUFFER(dumpPath, DUMP_PATH_SIZE, __dumpHint, fileName);
    fp = fopen(dumpPath, "wb");
    if(fp) {
...
OCC_HAL_IMP::__logSetProcData()
{
    ...
    for(MUINT32 i = 0; i < __curProcInfo.numOfBuffer; i++) {
        __logOCCBufferInfo(&__curProcInfo.bufferInfo[i], i);
    }
...
OCC_HAL_IMP::__logOCCResult()
{
    ...
    for(MUINT32 i = 0; i < __curProcInfo.numOfBuffer; i++) {
        __logOCCBufferInfo(&__curResultInfo.bufferInfo[i], i);
    }
...
void
OCC_HAL_IMP::__logOCCBufferInfo(OccBufferInfo *buf, int index)
{
    if(NULL == buf ||
       !LOG_ENABLED)
    {
