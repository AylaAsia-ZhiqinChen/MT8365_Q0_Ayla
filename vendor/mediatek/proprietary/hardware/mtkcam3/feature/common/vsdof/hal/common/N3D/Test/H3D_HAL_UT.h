...
protected:
    ...
    virtual void loadProcInfo()
    {
        ...
        int eis[4] = {0};
        if(LogicalCamJSONUtil::HasMember(document, "eis")) {
            json& eisValues = document["eis"];
            for(int i = 0; i < eisValues.size(); i++) {
                const json &eisValue = eisValues[i];
                eis[i] = eisValues.get<int>();
            }
    ...
    virtual void loadBuffers()
    {
        ...
        sprintf(filePath, UT_CASE_IN_FOLDER"/BID_P2A_OUT_RECT_IN1_%dx%d.yuv", _n3dInitInfo.iio_main.src_w, _n3dInitInfo.iio_main.src_h);
        readImage(filePath, _inputImageMain1.get());

        sprintf(filePath, UT_CASE_IN_FOLDER"/BID_P2A_OUT_RECT_IN2_%dx%d.yuv", _n3dInitInfo.iio_auxi.src_w, _n3dInitInfo.iio_auxi.src_h);
