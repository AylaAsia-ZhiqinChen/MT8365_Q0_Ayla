...
BOKEH_HAL_IMP::Run(BOKEH_HAL_PARAMS &param, BOKEH_HAL_OUTPUT &output)
{
    ...
    bool needInit = (__captureSize != param.cleanImage->getImgSize() ||
                     param.logicalDeviceID != __logicalDeviceID);
    __captureSize = param.cleanImage->getImgSize();
    __logicalDeviceID = param.logicalDeviceID;
...
BOKEH_HAL_IMP::__initBokeh(BOKEH_HAL_INIT_PARAMS *initParams)
{
    ...
    __moduleRotation = StereoSettingProvider::getModuleRotation(__logicalDeviceID);
    ...
    __imgInfo.DepthWidth  = DEPTH_SIZE.w;
    __imgInfo.DepthHeight = DEPTH_SIZE.h;
    __imgInfo.MainCamPos = static_cast<REFOCUS_MAINCAM_POS_ENUM>(StereoSettingProvider::getSensorRelativePosition(__logicalDeviceID));
...
BOKEH_HAL_IMP::__initTuningInfo(BOKEH_HAL_INIT_PARAMS *initParams)
{
    ...
    StereoSettingProvider::getStereoSensorIndex(main1Idx, main2Idx, __logicalDeviceID);
    if(__abfEnabled &&
       NULL == __abfAdapter)
    {
    ...
    __tuningInfo.Baseline            = StereoSettingProvider::getStereoBaseline(__logicalDeviceID);
...
BOKEH_HAL_IMP::__parseExtraData(char *extraData)
{
    ...
    {
        MY_LOGW("Fail to parse extra data: %s", e.what());
        result = false;
    }

    if(!result ||
       extraDataJson.is_discarded())
    {
        return false;
    }

    __dumpExtraData(extraDataJson);

    //Dof level
    if(LogicalCamJSONUtil::HasMember(extraDataJson, EXTRA_DATA_DOF_LEVEL)) {
        __imgInfo.DepthOfField = StereoTuningProvider::getSWBokehDoFValue(extraDataJson["dof_level"].get<int>());
    } else {
    ...
    //Capture orientation
    if(LogicalCamJSONUtil::HasMember(extraDataJson, EXTRA_DATA_CAPTURE_ORIENTATION)) {
        int rotation = extraDataJson[EXTRA_DATA_CAPTURE_ORIENTATION][EXTRA_DATA_ORIENTATION].get<int>();
        switch(rotation){
            case 4:
                rotation = 90;
                break;
    ...
    //Focus point
    if(LogicalCamJSONUtil::HasMember(extraDataJson, EXTRA_DATA_FOCUS_ROI)) {
        auto toTarget = [](auto p, auto range) {
            return (int)((p + 1000.0f)/2000.0f * range);
        };

        __imgInfo.afInfo.x1 = toTarget(extraDataJson[EXTRA_DATA_FOCUS_ROI][EXTRA_DATA_LEFT].get<int>(),   __imgInfo.TargetWidth);
        __imgInfo.afInfo.y1 = toTarget(extraDataJson[EXTRA_DATA_FOCUS_ROI][EXTRA_DATA_TOP].get<int>(),    __imgInfo.TargetHeight);
        __imgInfo.afInfo.x2 = toTarget(extraDataJson[EXTRA_DATA_FOCUS_ROI][EXTRA_DATA_RIGHT].get<int>(),  __imgInfo.TargetWidth);
        __imgInfo.afInfo.y2 = toTarget(extraDataJson[EXTRA_DATA_FOCUS_ROI][EXTRA_DATA_BOTTOM].get<int>(), __imgInfo.TargetHeight);
    ...
    //DAC info
    if(LogicalCamJSONUtil::HasMember(extraDataJson, EXTRA_DATA_FOCUS_INFO)) {
        __imgInfo.dacInfo.min    = extraDataJson[EXTRA_DATA_FOCUS_INFO][EXTRA_DATA_DAC_MIN].get<int>();
        __imgInfo.dacInfo.max    = extraDataJson[EXTRA_DATA_FOCUS_INFO][EXTRA_DATA_DAC_MAX].get<int>();
        __imgInfo.dacInfo.cur    = extraDataJson[EXTRA_DATA_FOCUS_INFO][EXTRA_DATA_DAC_CUR].get<int>();
        __imgInfo.faceInfo.isFd  = extraDataJson[EXTRA_DATA_FOCUS_INFO][EXTRA_DATA_IS_FACE].get<bool>();
        __imgInfo.faceInfo.ratio = extraDataJson[EXTRA_DATA_FOCUS_INFO][EXTRA_DATA_FACE_RATIO].get<float>();
        __imgInfo.afInfo.afType  = static_cast<REFOCUS_AF_TYPE_ENUM>(
                                        extraDataJson[EXTRA_DATA_FOCUS_INFO][EXTRA_DATA_FOCUS_TYPE].get<int>()
                                   );
    } else {
