...
typedef const std::map<int, const char *> FEATURE_STRING_MAP_T;
...
struct StereoSensorSetting_T
{
    ...
    std::map<int, float>    frzRatioSetting;    //feature mode -> FRZ Ratio
    ...
    MINT32 getSensorScenario(MINT32 featureMode, MINT32 pipelineMode=PipelineMode_ZSD)
    {
        ...
        ALOGW("Cannot find sensor scenario for sensor %d, featureMode 0x%X, pipeline mode %d",
...
struct StereoSensorConbinationSetting_T
{
    ...
    std::map<ENUM_STEREO_RATIO, NSCam::MSize> depthmapSizeCapture;
