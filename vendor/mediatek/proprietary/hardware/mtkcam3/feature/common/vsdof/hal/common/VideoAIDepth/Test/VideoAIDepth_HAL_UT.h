 ...
 ************************************************************************************************/
#ifndef VIDEOAIDEPTH_HAL_UT_H_
#define VIDEOAIDEPTH_HAL_UT_H_

#define LOG_TAG "VIDEOAIDEPTH_HAL_UT"
...
#define UT_CASE_PATH            "/sdcard/VIDEOAIDEPTH_HAL_UT"
...
class VideoAIDepthHALUTBase: public ::testing::Test
{
public:
    VideoAIDepthHALUTBase() {}
    virtual ~VideoAIDepthHALUTBase() {}

protected:
    virtual const char *getUTCaseName() { return "VIDEO_AIDEPTH_UT"; }
    ...
    virtual void SetUp() {
        ...
        MY_LOGD("[Create VIDEOAIDEPTH_HAL...]");
        _pAIDepthHAL = VIDEO_AIDEPTH_HAL::createInstance();
        MY_LOGD("VIDEOAIDepthHAL %p", _pAIDepthHAL);
    ...
    virtual void TearDown() {
        ...
        if(_disparityMap.get()) {
            StereoDpUtil::freeImageBuffer(getUTCaseName(), _disparityMap);
        }

        if(_confidenceMap.get()) {
            StereoDpUtil::freeImageBuffer(getUTCaseName(), _confidenceMap);
        }
    ...
    virtual void loadParam()
    {
        //Load Init Info
        json document;
        std::ifstream fin(UT_CASE_IN_FOLDER"/VIDEO_AIDEPTH_INIT_INFO.json");
        ...
        StereoDpUtil::allocImageBuffer(LOG_TAG, eImgFmt_Y8, _imgSize, !IS_ALLOC_GB, _disparityMap);
        StereoDpUtil::allocImageBuffer(LOG_TAG, eImgFmt_Y8, _imgSize, !IS_ALLOC_GB, _confidenceMap);
        ...
        _input.disparityMap = _disparityMap.get();
        _input.confidenceMap = _confidenceMap.get();
        ...
        //Load Params
        fin.open(UT_CASE_IN_FOLDER"/VIDEO_AIDEPTH_PROC_INFO.json");
        parseSuccess = false;
        if(fin) {
            try
            {
        ...
        _input.dumpHintL1   = &_dumphintL1;
        _input.dumpHintL2   = &_dumphintL2;
    ...
    void loadImages()
    {
        ...
        sprintf(filePath, UT_CASE_IN_FOLDER"/BID_OCC_OUT_NOC_M_%dx%d.yuv", _tuningInfo.imgWidth, _tuningInfo.imgHeight);
        readImage(filePath, _disparityMap.get());
        sprintf(filePath, UT_CASE_IN_FOLDER"/BID_OCC_OUT_CFM_M_%dx%d.yuv", _tuningInfo.imgWidth, _tuningInfo.imgHeight);
        readImage(filePath, _confidenceMap.get());
...
protected:
    //AIDEPTH parameters
    VIDEO_AIDEPTH_HAL *_pAIDepthHAL = NULL;
    VIDEO_AIDEPTH_HAL_PARAMS        _input;
    VIDEO_AIDEPTH_HAL_OUTPUT        _output;
    TuningUtils::FILE_DUMP_NAMING_HINT _dumphintL1;
    TuningUtils::FILE_DUMP_NAMING_HINT _dumphintL2;

    //data
    MSize                     _imgSize;
    MSize                     _finalOutputSize;
    VideoAIDepthTuningInfo    _tuningInfo;
    VideoAIDepthInitInfo      _initInfo;
    VideoAIDepthParam         _paramInfo;

    sp<IImageBuffer> _imgMain1;
    sp<IImageBuffer> _imgMain2;
    sp<IImageBuffer> _disparityMap;
    sp<IImageBuffer> _confidenceMap;
    sp<IImageBuffer> _depthmap;
    sp<IImageBuffer> _depthmapGolden;

    bool _isReadyToUT = true;
    bool LOG_ENABLED = StereoSettingProvider::isLogEnabled("vendor.STEREO.log.hal.vaidepth.ut");
};
