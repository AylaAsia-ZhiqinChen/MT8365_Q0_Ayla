...
StereoSettingProviderKernel::getSensorCombinationSetting(MUINT32 logicalDeviceID)
{
    ...
    if(__sensorCombinationSettings.find(logicalDeviceID) != __sensorCombinationSettings.end()) {
        return &(__sensorCombinationSettings.find(logicalDeviceID)->second);
    }
...
StereoSettingProviderKernel::__parseDocument(json &jsonObj)
{
    ...
    int logicalDeviceID = __sensorNameMap.size();
