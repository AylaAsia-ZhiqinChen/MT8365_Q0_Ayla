...
MINT32 FSCHalImp::Uninit(IHal3A* p3A)
{
    ...
    if (mpAFUpdateFp != NULL)
    {
        ::fclose(mpAFUpdateFp);
        mpAFUpdateFp = NULL;
    }
    if (mpFSCUpdateFp != NULL)
    {
        ::fclose(mpFSCUpdateFp);
        mpFSCUpdateFp = NULL;
    }
...
void FSCHalImp::createDumpFile()
{
    ...
    char path[256];
    sprintf(path, FSC_DUMP_FOLDER_PATH);
    ...
    //open file
    sprintf(path, FSC_DUMP_FOLDER_PATH"/%" PRId64 "_FSC_Init.csv", timewithoutSleep);
    mpFSCInitFp = ::fopen(path, "w+t");
    if (mpFSCInitFp == NULL)
    {
    ...
    {
        ::fprintf(mpFSCInitFp, "macro_to_inf_ratio,max_cropped_ratio,dac_Inf,dac_Macro,"
            "af_table_start,af_table_end,readout_time_mus,af_time_mus\n");
    }
    sprintf(path, FSC_DUMP_FOLDER_PATH"/%" PRId64 "_AF_Update.csv", timewithoutSleep);
    mpAFUpdateFp = ::fopen(path, "w+t");
    if (mpAFUpdateFp == NULL)
    {
        MY_LOGW("Can't open file %s", path);
    }
    ...
    {
        ::fprintf(mpAFUpdateFp, "request_frame_num,rolling_shutter_percentage,target_DAC\n");
    }
    sprintf(path, FSC_DUMP_FOLDER_PATH"/%" PRId64 "_FSC_Update.csv", timewithoutSleep);
    mpFSCUpdateFp = ::fopen(path, "w+t");
    if (mpFSCUpdateFp == NULL)
    {
        MY_LOGW("Can't open file %s", path);
    }
    ...
    {
        ::fprintf(mpFSCUpdateFp, "request_frame_num,exposureTime_mus,w_sensor_out,h_sensor_out,"
