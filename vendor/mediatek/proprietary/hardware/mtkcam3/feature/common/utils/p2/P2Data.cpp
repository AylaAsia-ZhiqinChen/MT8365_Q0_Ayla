...
P2ConfigInfo::P2ConfigInfo(const ILog &log)
    : mLog(log)
{
}
...
P2SensorInfo::P2SensorInfo(const ILog &log, const MUINT32 &id)
    : mLog(log)
    , mSensorID(id)
    , mPlatInfo(P2PlatInfo::getInstance(id))
{
    if( mPlatInfo )
    {
...
P2FrameData::P2FrameData(const ILog &log)
    : mLog(log)
{
}
...
P2SensorData::P2SensorData(const ILog &log)
    : mLog(log)
{
}
