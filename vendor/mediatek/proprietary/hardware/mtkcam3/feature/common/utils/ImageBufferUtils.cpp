...
MBOOL ImageBufferUtils::allocBuffer(
        ...
        MUINT32 w, MUINT32 h, MUINT32 format, MBOOL isContinuous, MUINT32 aligned)
{
    MBOOL ret = MTRUE;
    ...
    for (size_t i = 0; i < planeCount; i++)
    {
        ...
        strideInBytes[i] = ALIGN(strideInBytes[i], aligned);
    ...
    if (isContinuous)
    {
        ...
        for (size_t i = 0; i < planeCount; i++)
        {
            MUINT32 planeWidth = (queryPlaneWidthInPixels(format, i, w) *
                  queryPlaneBitsPerPixel(format, i) + 7) / 8;
            planeWidth = ALIGN(planeWidth, aligned);
            allPlaneSize += planeWidth * queryPlaneHeightInPixels(format, i, h);
        }
