...
public:     ////                            Definitions.
    struct NodeWithAdj_T
    {
        ...
        MUINT32                             mInAdjReqCnt;  //In adjacent node send request count. After receive all request and then queue to nex node
...
public:     ////                            Attributes.
    ...
    virtual Vector<NodeObj_T>               getRootNode() const;

    virtual NodeSet                         getRootNodeSet() const;
    ...
    virtual MERROR                          getInAdjacentNodesReqCnt(
                                                NodeId_T id,
                                                MUINT32& count
                                            ) const;

    virtual MERROR                          addInAdjacentNodesReqCnt(
                                                NodeId_T id
                                            );
        ...
        for (size_t i = 0; i < mNodesVec.size(); i++)
        {
            ...
            if  ( 0 > index ) {
                ...
                for(size_t i=0; i<mRootIds.size(); i++)
                {
                    ...
                    {
                        MY_LOGE("RootId:%" PRIxPTR " is not specified within the given set", mRootIds[i]);
                        return NULL;
                    }
                }
            }
...
removeNode(NodeId_T id)
{
    ...
    while(it != mRootIds.end())
    {
        if(id == (*it) )
        {
            MY_LOGD("erase root node: id = %#" PRIxPTR " ", id);
...
MERROR
PipelineDAGImp::
setRootNode(NodeSet roots)
{
    RWLock::AutoWLock _(mMutex);
    if(roots.size()==0)
    {
        MY_LOGE("Input error, roots.size() == %zu", roots.size());
        return BAD_VALUE;
    }
    for(size_t i=0;i<roots.size();i++)
    {
        ssize_t err = mNodesVec.indexOfKey(roots[i]);
        if (0 > err)
        {
            MY_LOGE("Node does not exist, RootNode[%zu] ID:%#" PRIxPTR "", i, roots[i]);
            return err;
        }
    }
    mRootIds = roots;
 ...
 ******************************************************************************/
Vector<PipelineDAGImp::NodeObj_T>
PipelineDAGImp::
getRootNode() const
{
    RWLock::AutoRLock _(mMutex);
    android::Vector<NodeObj_T> node_set;
    //Check if mRootIds has been set or not
    if(mRootIds.size() == 0)
    {
        MY_LOGW("There is no root node (mRootIds.size() = %zu)", mRootIds.size());
        return node_set;
    }
    ...
    for(size_t i=0;i<mRootIds.size();i++)
    {
        idx = mNodesVec.indexOfKey(mRootIds[i]);
        if (0 > idx)
        {
            MY_LOGE("Node does not exist (ID[%zu]:%#" PRIxPTR ")", i, mRootIds[i]);
            return node_set;
        }
        node_set.push_back(mNodesVec.valueAt(idx).mNode);
    }
    return node_set;
}
...
/******************************************************************************
 *get root node of a DAG
 ******************************************************************************/
NodeSet
PipelineDAGImp::
getRootNodeSet() const
{
    RWLock::AutoRLock _(mMutex);
    return mRootIds;
}
...
MERROR
PipelineDAGImp::
getInAdjacentNodesReqCnt(NodeId_T id, MUINT32& count) const
{
    RWLock::AutoRLock _(mMutex);
    ssize_t idx = mNodesVec.indexOfKey(id);
    if (0 > idx)
...
MERROR
PipelineDAGImp::
addInAdjacentNodesReqCnt(NodeId_T id)
{
    RWLock::AutoWLock _(mMutex);
    ssize_t idx = mNodesVec.indexOfKey(id);
    if (0 > idx)
    ...
    mNodesVec.editValueAt(idx).mInAdjReqCnt += 1;
    return OK;
}
...
) const
{
    ...
    for(size_t i=0; i<mRootIds.size();i++)
    {
        if (0 > checkListDFS(mRootIds[i], checkList.editArray()))
        {
            orphanNodes.clear();
...
checkListDFS(NodeId_T id, bool *checkList) const
{
    ...
    if (0 > idx)
    {
        MY_LOGE("Node ID=%zd does not exist", idx);
        return -1L;
    }
    checkList[idx] = true;
...
dump(android::Vector<android::String8>& rLogs) const
{
    ...
    //
    for(size_t i = 0; i < mRootIds.size(); i++) {
        *it++ = String8::format("RootId:%#" PRIxPTR " Nodes:#%zu", mRootIds[i], mNodesVec.size());
    }
    for (size_t i = 0; i < mNodesVec.size(); i++) {
        NodeId_T const nodeId = mNodesVec.keyAt(i);
