...
auto ResultUpdateHelper(
    android::wp<IPipelineModelCallback>& rpCallback,
    uint32_t requestNo,
    android::sp<IMetaStreamBuffer> resultMeta,
    bool isLastPartial,
    bool isRealTime
) -> void
{
    ...
    // we don't know the actual number. device check this value >=0 to identify last partial result.
    params.nOutMetaLeft = (isLastPartial)? 0 : 1;
    params.vOutMeta.push_back(resultMeta);
    params.isRealTimeUpdate = isRealTime;

    MY_LOGD_IF( 0, "requestNo %d, user %#" PRIxPTR " AppResult(%p) left:%zd",
                requestNo, userId, resultMeta.get(), params.nOutMetaLeft );
