...
typedef enum
{
  ...
  eTEST_CASE_P1MAIN1_P1SUB_DUMMYNODEX2,
  eTEST_CASE_MAX,
} eTestCase ;
...
namespace {
    ...
    enum NODE_ID{
        ...
        eNODEID_DualDummyNode_0 = 0xf2,
        eNODEID_DualDummyNode_1 = 0xf3,
    };
    //
    ...
    P2StreamingNode::ConfigParams        gP2ConfigParam;
    //
    MSize                       gRrzoSize;
    MSize                       gRrzoSize1;
...
static void help(char *pProg)
{
    ...
    printf("\t\t\t 11: multi-root & previous node test (P1_Main1+P2_Sub+dummy+dummy)\n");
    printf("\t\t RequestCnt:\n");
...
void setupImageStreamInfo()
{
    ...
    if(testCase==eTEST_CASE_P1MAIN1_P1SUB_DUMMYNODEX2)
    {// Resized Raw (sub)
...
void prepareSensor(MUINT32 SensorId)
{
    ...
    //
    MINT ret = mpSensorHalObj->sendCommand(
    ...
    if(ret != 0)
    {
        MY_LOGE("sensor hal sendCommand fail(%x)", ret);
    }
    //
...
bool setupP1Node(MUINT32 SensorId)
{
    ...
    NodeT::ConfigParams cfgParam;
    {
        ...
        cfgParam.pOutImage_full_HAL= Image_ImgoRaw; //N/A
        cfgParam.sensorParams = SensorParam;
        // test the NULL pool parameter, right now there's no access to ppl pool.
...
bool setupDummyNode(MUINT32 NodeId)
{
    typedef DualDummyNode           NodeT;
    //
    MY_LOGD("Nodebuilder DualDummyNode (%d) +\n", NodeId);
    NodeT::InitParams initParam;
    {
        initParam.openId   = NodeId;
        initParam.nodeId   = (NodeId == 0)? eNODEID_DualDummyNode_0: eNODEID_DualDummyNode_1;
        initParam.nodeName = (NodeId == 0)? "DualDummyNode0" : "DualDummyNode1";
    }
    NodeT::ConfigParams cfgParam;
    {
        cfgParam.pInAppMeta    = gControlMeta_App;
        ...
        cfgParam.pInAppRetMeta1= gResultMeta_P1_sub_App;
        cfgParam.pInHalMeta    = gResultMeta_P1_Hal;
        cfgParam.pInHalMeta1   = gResultMeta_P1_sub_Hal;

        if (gImage_ImgoRaw != 0)
            cfgParam.pvInFullRaw.push_back(gImage_ImgoRaw);
        if (gImage_ImgoRaw_sub!= 0)
            cfgParam.pvInFullRaw_sub.push_back(gImage_ImgoRaw_sub);

        cfgParam.pInResizedRaw = gImage_RrzoRaw;
        cfgParam.pInResizedRaw_sub = gImage_RrzoRaw_sub;
    }
    //
    android::sp<DualDummyNode> spDualDummyNode = new DualDummyNode();
    auto pNode = makeINodeActor(spDualDummyNode);
    pNode->setInitParam(initParam);
    pNode->setConfigParam(&cfgParam);
    //
    NodeBuilder aNodeBuilder(initParam.nodeId, pNode);
    ...
    if( ret != OK ) {
        MY_LOGE("build DualDummyNode (%d) error\n",NodeId);
        return 0;
    }
...
void setupPipelineContext()
{
    ...
    if(testCase == eTEST_CASE_P1MAIN1_P1SUB_DUMMYNODEX2)
    {
        if(!setupP1Node(gSensorId1))
            return;
        if(!setupDummyNode(gSensorId))
            return;
        if(!setupDummyNode(gSensorId1))
            return;
    }
    ...
    if(testCase != eTEST_CASE_P1MAIN1_P1SUB_DUMMYNODEX2)
    {
        ...
        NodeT::InitParams initParam;
        {
            ...
            initParam.nodeName = "P2StreamingNode";
        }
        NodeT::ConfigParams cfgParam;
        {
            cfgParam.pInAppMeta    = gControlMeta_App;
        ...
        P2Common::UsageHint p2Usage;
        {
            ...
            p2Usage.mStreamingSize = gImage_RrzoRaw->getImgSize();
            ...
            for(auto&& out : cfgParam.vOutImage)
            {
                ...
                //TODO if physical , set mHalPhysical = true
            }
        }
        cfgParam.mUsageHint = p2Usage;
        //
        auto pNode = makeINodeActor( NodeT::createInstance(P2StreamingNode::ePass2Type::PASS2_STREAM, p2Usage) );
        pNode->setInitParam(initParam);
        pNode->setConfigParam(&cfgParam);
        //
    ...
    if(testCase != eTEST_CASE_P1MAIN1_P1SUB_DUMMYNODEX2)
    {
        ...
        {
            cfgParam.pInAppMeta       = gControlMeta_App;
            cfgParam.pInHalMeta_streaming       = gResultMeta_P2_Hal;
    ...
    {
        NodeEdgeSet edges;
        edges.addEdge(eNODEID_P1Node, eNODEID_DualDummyNode_0);
        edges.addEdge(eNODEID_P1Node, eNODEID_DualDummyNode_1);
        edges.addEdge(eNODEID_P1Node_sub, eNODEID_DualDummyNode_0);
        edges.addEdge(eNODEID_P1Node_sub, eNODEID_DualDummyNode_1);
...
void setupRequestBuilderDualP1()
{
    ...
    metaIOMapSet.add(IOMap()
                  ...
                  .addIn(gResultMeta_P1_sub_Hal->getStreamId()));
    gRequestBuilderDualP1->setIOMap(
                            eNODEID_DualDummyNode_0,
                            imgIOMapSet,
                            metaIOMapSet);
    gRequestBuilderDualP1->setIOMap(
                            eNODEID_DualDummyNode_1,
                            imgIOMapSet,
                            metaIOMapSet);
    NodeSet root_nodes;
    root_nodes.add(eNODEID_P1Node);
...
void processRequest()
{
    ...
    }
    {
        ...
        switch(testCase)
        {
            case eTEST_CASE_P1MAIN1_P1SUB_DUMMYNODEX2:
        ...
        {
                pRequestBuilder->setAppMetaStreamBuffer(
                                      gControlMeta_App->getStreamId(),
                                      pAppMetaControlSB
                                  );
                pRequestBuilder->setHalMetaStreamBuffer(
...
static void saveJpegtoFile()
{
    ...
    //acquireFromPool
    // auto pPool_HalImageJpeg = nullptr;
