...
void createFullImageStream(MSize size)
{
    ...
    bufPlanes.count = 1;
    bufPlanes.planes[0].rowStrideInBytes = queryRst.stride_byte;
    bufPlanes.planes[0].sizeInBytes = bufPlane.rowStrideInBytes * size.h ;
    MY_LOGI("Stream Full Size query (%dx%d) stride[%d] size[%d]", size.w, size.h, bufPlanes.planes[0].rowStrideInBytes, bufPlanes.planes[0].sizeInBytes);
    ...
    for (size_t i = 0; i < bufPlanes.count; i++) {
        bufStridesInBytes[i] = bufPlanes.plane[i].rowStrideInBytes;
    }
...
void createResizerImageStream(MSize size)
{
    ...
    bufPlanes.planes[0].rowStrideInBytes = queryRst.stride_byte;
    bufPlanes.planes[0].sizeInBytes = bufPlane.rowStrideInBytes * size.h ;
    MY_LOGI("Stream Resizer Size query (%dx%d) stride[%d] size[%d]", size.w, size.h, bufPlanes.planes[0].rowStrideInBytes, bufPlanes.planes[0].sizeInBytes);
    //
    sp<IImageStreamInfo>
    pStreamInfo = new ImageStreamInfo(
