    ...
    , mbIspWBeffect(0)
{
    ...
    MINT32 p1_log = ::property_get_int32("vendor.debug.camera.log.p1node", 0);
...
check_config(ConfigParams const& rParams)
{
    ...
    #if (IS_P1_LOGI)
    {
        ...
        strInfo += String8::format("Param["
            ...
            "B:p%d,b%d,t%d,h%d,u%d,e%d,l%d,r%d,f%d,d%d,q%d,c%d,f%d,v%d,s%d,LL%d,SSC%d,SE%d,ST%d,SB%p] ",
            // Param-iNt/eNum
    ...
    if (!bTargetModeReady) { // TargetMode is UNKNOWN
        ...
        { // decide AETargetMode
            ...
            if (mpVhdr != NULL) {
                ...
                if( pVhdr->Init(mSensorParams.vhdrMode) != VHDR_RETURN_NO_ERROR)
                {
                    pVhdr = NULL;
                }
                if(pVhdr != NULL) {
