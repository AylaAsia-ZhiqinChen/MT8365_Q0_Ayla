...
#define LOG_TAG "mtkcam-ConfigPipelineNodesNeedPolicy-Security"
...
/**
 * Make a function target as a policy for Non-P1 - Security version
 */
FunctionType_Configuration_PipelineNodesNeedPolicy makePolicy_Configuration_PipelineNodesNeed_Security()
{
    return [](Configuration_PipelineNodesNeed_Params const& params) -> int {
        ...
        pOut->needJpegNode      = false;
        pOut->needRaw16Node     = false;
        pOut->needRaw16Node     = false;
        pOut->needPDENode       = false;
        ...
        int disableFD = ::property_get_int32("vendor.debug.camera.fd.disable", 0);
        if ( disableFD == 0 && MTKCAM_HAVE_SECURE_FD_SUPPORT !=0 ) //if (MTKCAM_HAVE_SECURE_FD_SUPPORT==0) => disable FD
        {
        ...
        MY_LOGD("disableFD(%d) MTKCAM_HAVE_SECURE_FD_SUPPORT(%d) needFDNode(%d)",disableFD, MTKCAM_HAVE_SECURE_FD_SUPPORT, pOut->needFDNode);
#else
        MY_LOGD("MTKCAM_HAVE_SECURE_FD_SUPPORT not define. needFDNode(%d)",pOut->needFDNode);
#endif
        return OK;
    };
}
