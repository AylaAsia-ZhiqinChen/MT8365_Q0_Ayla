...
public:
    ...
    virtual bool push(const int& in) {
        ...
        ALOGD("    count:%u head:%d", mRingBuffer.getCount(), head);
...
public:
    ...
    virtual bool push(const float& in) {
        ...
        ALOGD("    count:%u head:%f", mRingBuffer.getCount(), head);
...
public:
    ...
    virtual bool push(const sp<CustomStruct>& in) {
        ...
        ALOGD("    count:%u head:%f", mRingBuffer.getCount(), head->mFloat);
...
public:
    ...
    virtual void dump() {
        for (uint32_t i = 0; i < mRingBuffer.getValidSize(); i++) {
            ...
            ALOGD("    [%d] %s", i, item->mString.string());
        }
    }
};
