...
*/
MVOID getAWBStat_ext(AWB_INPUT_T &a_rAWBInput, 
    MINT32 i4IsHdrSupport,
    MINT32 i4IsLinearMode, 
    MUINT32 i4WinPixCnt, 
    MINT32 i4WindowNumX, MINT32 i4WindowNumY, 
    MVOID *pAWBStatBufOut);
...
#ifdef PC_SIMU
	//====================== AWB Simulator Debug ======================//
	virtual void get_dbg_image_buff(MUINT8 **pin) = 0;
	virtual int awb_dbg_init(unsigned char **dbpimg,unsigned char** awb_tag_info, int raw_w, int raw_h) = 0;
	virtual int getAWB_TAG_NUM() = 0;
	virtual void set_strobe_light_win(LIGHT_AREA_T *plight_area) = 0;
	virtual void get_awb_exif_info(MUINT8 **pinfo) = 0;
	virtual void print_awb_exif_info(char *filename) = 0;
	virtual void drawAWBDebugImage(MUINT8 *pimg) = 0;

#endif

    virtual void Flash_Awb_calResultGain(FLASH_AWB_CAL_GAIN_INPUT_T& calGainInput, FLASH_AWB_OUTPUT_T& flashAwbOuput) = 0;
