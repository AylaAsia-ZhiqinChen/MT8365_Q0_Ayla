...
public:
    ...
    virtual int ccuControl(uint32_t msgId, void *inDataPtr, void *outDataPtr) = 0;
};

};  //namespace NSCcuIf
#endif

/** \fn virtual int NSCcuIf::ICcuCtrl::ccuControl(uint32_t msgId, void *inDataPtr, void *outDataPtr) = 0;
 *  \brief Send command to CCU and get result if need
 *  \param[in] msgId message id to tell CCU which command to execute, see \ref uint32_t for available commands
