...
JPEG_DEC_STATUS_ENUM jpegDecLockDecoder(int* drvID)
{
    ...
    if (currentID == JPEG_HAL_ID_UNLOCK)
    {
        *drvID = open(JPEG_DRV_NAME, O_RDONLY, 0);
        if( *drvID == -1 )
        {
            ALOGE("::[DRIVER] Open JPEG Driver Error (%s)", strerror(errno));
            return JPEG_DEC_STATUS_ERROR_CODEC_UNAVAILABLE;
        }
        
        if(ioctl(*drvID, JPEG_DEC_IOCTL_INIT))
        {
            ALOGE("::[DRIVER] JPEG_DEC_IOCTL_INIT Error (%s)", strerror(errno));
            close(*drvID);
            return JPEG_DEC_STATUS_ERROR_CODEC_UNAVAILABLE;
        }
        currentID = *drvID;
                                      ...
                                      long timeout, JPEG_DEC_RESULT_ENUM *result)
{
    ...
    unsigned int pause_row_cnt = 0 ;//1;    
    ...
    ATRACE_CALL();
      ...
      //ALOGD("::[RESUME_ROW_SET] pause_row_cnt %d/%d, row %d, out_base %x %x %x, size %x %x %x, L:%d!!\n", 
...
#else
{
      ...
      //ALOGD("::[RESUME_ROW] pause_row_cnt %d, ret %x, MCUinRow %d, p_idx %d, %x %x %x!!\n", 
