...
#ifdef BACK_GROUND_LOW
{
    ...
    xlog("mHalJpgDec:: parser lock success,fg(%d),SWS 0x%x, L:%d!!\n",fg, SW_PATH_SIZE,__LINE__);
    ...
    //xlog("mHalJpgDec:: parse success, fg(%d), L:%d!!\n",fg ,__LINE__);
    return 0;
}

int mHalJpgDecGetInfo(MHAL_JPEG_DEC_INFO_OUT *decOutInfo)
{
    xlog("mHalJpgDec:: getInfo L:%d!!\n", __LINE__);
...
{
    xlog("mHalJpgDec:: enter start L:%d!!\n", __LINE__);
    if(jpgDecoder == NULL) { return return_false("null decoder"); }

    if(decInParams == NULL)
    {
        xlog("mHalJpgDec:: cancel Decoder L:%d!!\n", __LINE__);
        jpgDecoder->unlock();
        delete jpgDecoder;
        jpgDecoder = NULL;
    ...
    xlog("mhalJpegDecoder: src %x, srcSize %x, dst %x, size %x, w %d, h %d, fmt %d, PQ %d, IDCT %d!!\n",decInParams->srcBuffer
    , decInParams->srcLength ,decInParams->dstVirAddr,dst_size,decInParams->dstWidth
    ...
    xlog("mHalJpgDec:: trigger Decoder L:%d!!\n", __LINE__);    
    if(!jpgDecoder->start())
    {
        xlog("mHalJpgDec:: trigger Decoder Fail L:%d!!\n", __LINE__);    
        jpgDecoder->unlock();
        delete jpgDecoder;
        jpgDecoder = NULL;
    ...
    xlog("mHalJpgDec:: trigger Decoder Success Done L:%d!!\n", __LINE__);    
   ...
   JpgDecHal* jHalDecoder = (JpgDecHal*) *jDecHandle;
    ...
    switch (decInParams->dstFormat)
    {
        ...
        case JPEG_OUT_FORMAT_YV12:
            ...
            //dst_csize = TO_CEIL( decInParams->dstWidth>>1, 16 ) * (decInParams->dstHeight>>1) ;
    ...
    xlog("mhalJpegDecoder: src %x, srcSize %x, dst %x, size %x %x, w %d, h %d, fmt %d, PQ %d, IDCT %d!!\n",decInParams->srcBuffer
    , decInParams->srcLength ,decInParams->dstVirAddr,dst_size, dst_csize,decInParams->dstWidth
    , decInParams->dstHeight,decInParams->dstFormat, decInParams->doPostProcessing, decInParams->PreferQualityOverSpeed);
    ...
    if(decInParams->dstFormat == JPEG_OUT_FORMAT_YV12){
