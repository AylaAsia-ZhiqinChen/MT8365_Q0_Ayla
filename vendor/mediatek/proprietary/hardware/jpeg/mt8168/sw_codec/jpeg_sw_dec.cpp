...
bool JpgSwDec::parse()
{
    ...
    //swjpeg_source_mgr       sk_stream(srcStream, NULL, false);
    srcStream = new SwBufStream(fSrcAddr, fSrcBufferSize);   
    jpegStream = new swjpeg_source_mgr(srcStream, false);    
    
    JPG_DBG("decoder parse src %p, size %x, stream %p, jpegStream %p!!\n", fSrcAddr, (unsigned int)fSrcBufferSize, srcStream, jpegStream);
    ...
    //JPG_DBG("JpgSwDec::start -> config jpeg path");
    if(cinfo->comps_in_scan > 1 ){
