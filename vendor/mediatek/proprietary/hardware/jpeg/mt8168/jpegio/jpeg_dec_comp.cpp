...
JpgDecComp::JpgDecComp()
{
    ...
    fm4uJpegDecInputID  = M4U_PORT_JPGDEC_BSDMA ;
    fm4uJpegDecOutputID = M4U_PORT_JPGDEC_WDMA  ;
...
#ifdef USE_PMEM //bobule workaround pdk build error needing review
...
bool JpgDecComp::alloc_m4u()
{
    ...
    if(!imgMmu_create(&pM4uDrv, fm4uJpegDecInputID))
      return false;

    if(!imgMmu_reset(&pM4uDrv, fm4uJpegDecInputID))
      return false;

    //src bitstream
    if(en_mci){
      if(!imgMmu_alloc_pa_mci(&pM4uDrv, fm4uJpegDecInputID, (void*) fSrcAddr, fSrcBufferSize, &fDecSrcM4UAddrMva))
        return false;
    }else{
      if(!imgMmu_alloc_pa(&pM4uDrv, fm4uJpegDecInputID, (void*) fSrcAddr, fSrcBufferSize, &fDecSrcM4UAddrMva))
        return false;
    }

#if 1
    //dst bank 0 Luma, Cb, Cr
    if(!imgMmu_alloc_pa(&pM4uDrv, fm4uJpegDecOutputID, (void*) fDecOutBufAddr0[0], fDecDstBufSize[0], &fDecDstM4UAddrMva0[0]))
      return false;
    if(!imgMmu_alloc_pa(&pM4uDrv, fm4uJpegDecOutputID, (void*) fDecOutBufAddr0[1], fDecDstBufSize[1], &fDecDstM4UAddrMva0[1]))
      return false;
    if(!imgMmu_alloc_pa(&pM4uDrv, fm4uJpegDecOutputID, (void*) fDecOutBufAddr0[2], fDecDstBufSize[2], &fDecDstM4UAddrMva0[2]))
      return false;

    if(fDecType == IMG_DEC_MODE_LINK){

      //dst bank 1 Luma, Cb, Cr
      if(!imgMmu_alloc_pa(&pM4uDrv, fm4uJpegDecOutputID, (void*) fDecOutBufAddr1[0], fDecDstBufSize[0], &fDecDstM4UAddrMva1[0]))
        return false;
      if(!imgMmu_alloc_pa(&pM4uDrv, fm4uJpegDecOutputID, (void*) fDecOutBufAddr1[1], fDecDstBufSize[1], &fDecDstM4UAddrMva1[1]))
        return false;
      if(!imgMmu_alloc_pa(&pM4uDrv, fm4uJpegDecOutputID, (void*) fDecOutBufAddr1[2], fDecDstBufSize[2], &fDecDstM4UAddrMva1[2]))
        return false;
    ...
    //config module port
    if(en_mci)
      imgMmu_cfg_port_mci(pM4uDrv, fm4uJpegDecInputID, fm4uJpegDecInputID);
    else
      imgMmu_cfg_port(pM4uDrv, fm4uJpegDecInputID, fm4uJpegDecInputID);

    imgMmu_cfg_port(pM4uDrv, fm4uJpegDecOutputID, fm4uJpegDecOutputID);

    //flush cache
    if(!en_mci){
      if (!imgMmu_sync(pM4uDrv, fm4uJpegDecInputID, (void*) fSrcAddr, fDecSrcM4UAddrMva, fSrcBufferSize, SYNC_HW_READ))
      {
        JPG_DBG("srcBuf m4u cache sync fail mva %x, va %p, size %x", fDecSrcM4UAddrMva, fSrcAddr, fSrcBufferSize);
      }
    }
#if 1
    //if(!en_mci)
    {
      if (!imgMmu_sync(pM4uDrv, fm4uJpegDecOutputID, (void*) fDecOutBufAddr0[0], fDecDstM4UAddrMva0[0], fDecDstBufSize[0], SYNC_HW_WRITE))
      {
        JPG_DBG("dstBuf[0] m4u cache sync fail mva %x, va %x, size %x", fDecDstM4UAddrMva0[0], fDecOutBufAddr0[0], fDecDstBufSize[0]);
      }
      if (!imgMmu_sync(pM4uDrv, fm4uJpegDecOutputID, (void*) fDecOutBufAddr0[1], fDecDstM4UAddrMva0[1], fDecDstBufSize[1], SYNC_HW_WRITE))
      {
        JPG_DBG("dstBuf[1] m4u cache sync fail mva %x, va %x, size %x", fDecDstM4UAddrMva0[1], fDecOutBufAddr0[1], fDecDstBufSize[1]);
      }
      if (!imgMmu_sync(pM4uDrv, fm4uJpegDecOutputID, (void*) fDecOutBufAddr0[2], fDecDstM4UAddrMva0[2], fDecDstBufSize[2], SYNC_HW_WRITE))
      {
        JPG_DBG("dstBuf[2] m4u cache sync fail mva %x, va %x, size %x", fDecDstM4UAddrMva0[2], fDecOutBufAddr0[2], fDecDstBufSize[2]);
      }

      if(fDecType == IMG_DEC_MODE_LINK){
        if (!imgMmu_sync(pM4uDrv, fm4uJpegDecOutputID, (void*) fDecOutBufAddr1[0], fDecDstM4UAddrMva1[0], fDecDstBufSize[0], SYNC_HW_WRITE))
        {
          JPG_DBG("dstBuf[0] m4u cache sync fail mva %x, va %x, size %x", fDecDstM4UAddrMva1[0], fDecOutBufAddr1[0], fDecDstBufSize[0]);
        }
        if (!imgMmu_sync(pM4uDrv, fm4uJpegDecOutputID, (void*) fDecOutBufAddr1[1], fDecDstM4UAddrMva1[1], fDecDstBufSize[1], SYNC_HW_WRITE))
        {
          JPG_DBG("dstBuf[1] m4u cache sync fail mva %x, va %x, size %x", fDecDstM4UAddrMva1[1], fDecOutBufAddr1[1], fDecDstBufSize[1]);
        }
        if (!imgMmu_sync(pM4uDrv, fm4uJpegDecOutputID, (void*) fDecOutBufAddr1[2], fDecDstM4UAddrMva1[2], fDecDstBufSize[2], SYNC_HW_WRITE))
        {
          JPG_DBG("dstBuf[2] m4u cache sync fail mva %x, va %x, size %x", fDecDstM4UAddrMva1[2], fDecOutBufAddr1[2], fDecDstBufSize[2]);
        }
      }
    }
#endif
...
bool JpgDecComp::free_m4u()
{
   ...
   {
      JPG_DBG("free_m4u!!\n");
      imgMmu_dealloc_pa(pM4uDrv, fm4uJpegDecInputID, (void*) fSrcAddr           , fSrcBufferSize   , &fDecSrcM4UAddrMva       );
#if 1
      imgMmu_dealloc_pa(pM4uDrv, fm4uJpegDecOutputID, (void*) fDecOutBufAddr0[0] , fDecDstBufSize[0], &fDecDstM4UAddrMva0[0]   );
      imgMmu_dealloc_pa(pM4uDrv, fm4uJpegDecOutputID, (void*) fDecOutBufAddr0[1] , fDecDstBufSize[1], &fDecDstM4UAddrMva0[1]   );
      imgMmu_dealloc_pa(pM4uDrv, fm4uJpegDecOutputID, (void*) fDecOutBufAddr0[2] , fDecDstBufSize[2], &fDecDstM4UAddrMva0[2]   );
      if( fDecType == IMG_DEC_MODE_LINK ){
        imgMmu_dealloc_pa(pM4uDrv, fm4uJpegDecOutputID, (void*) fDecOutBufAddr1[0] , fDecDstBufSize[0], &fDecDstM4UAddrMva1[0]   );
        imgMmu_dealloc_pa(pM4uDrv, fm4uJpegDecOutputID, (void*) fDecOutBufAddr1[1] , fDecDstBufSize[1], &fDecDstM4UAddrMva1[1]   );
        imgMmu_dealloc_pa(pM4uDrv, fm4uJpegDecOutputID, (void*) fDecOutBufAddr1[2] , fDecDstBufSize[2], &fDecDstM4UAddrMva1[2]   );
      }
#endif
      delete pM4uDrv;
   }
