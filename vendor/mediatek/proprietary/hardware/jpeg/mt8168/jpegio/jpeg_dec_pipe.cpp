...
//disable default debug log, if need debug, open that
...
static unsigned int pixel_byte[9] = {2,3,3,4,4,4,4,4,1};
...
bool JpgDecHal::checkParam()
{
    ...
    {
        JPG_LOG("Invalid width/height %u/%u [Range 3~8191]", fOutWidth, fOutHeight);
        return false;
    }
...
bool JpgDecHal::onConfigCompTile()
{
    ...
    if( fScalerBufferType == IMG_MEM_DEC_BUF_SCALER ){
      ...
      if (fDecDstBufAddrVA0[0])
        memset((void *)fDecDstBufAddrVA0[0], 0xAA, fDecDstBufSize[0]);

      if (fDecDstBufAddrVA0[1])
        memset((void *)fDecDstBufAddrVA0[1], 0xAA, fDecDstBufSize[1]);

      if (fDecDstBufAddrVA0[2])
        memset((void *)fDecDstBufAddrVA0[2], 0xAA, fDecDstBufSize[2]);
    }else{
...
bool JpgDecHal::onStartTile()
{
    ...
    *===========================================================*/
    ...
    for( ; !pImgCdp->isScalingFinish(); )
    {
      ...
      //android::Tracer::traceBegin(ATRACE_TAG_GRAPHICS,"DECODE_ROW");
...

      {
        ...
        if(dump2file(DUMP_DEC_LVL_STEP,"dumpDecPipe_decStep", &fileNameDecIdx, fDecDstBufAddrVA0[0], fDecDstBufSize[0]))
          fileNameDecIdx++;
      }
      {
      #if 0
        ...
        if(dump2file(DUMP_DEC_LVL_STEP,"dumpDecPipe_U_decStep", &fileNameDecIdxU, fDecDstBufAddrVA0[1], fDecDstBufSize[1]))
          fileNameDecIdxU++;

        if(dump2file(DUMP_DEC_LVL_STEP,"dumpDecPipe_V_decStep", &fileNameDecIdxV, fDecDstBufAddrVA0[2], fDecDstBufSize[1]))
          fileNameDecIdxV++;
      #endif
      }

      //android::Tracer::traceBegin(ATRACE_TAG_GRAPHICS,"MDP_SCALE");
      if(!pImgCdp->Start()){
      ...
      {
        static unsigned int fileNameDecIdx = 0;
        if(dump2file(DUMP_DEC_LVL_SCALE,"dumpDecPipe_scaleStep", &fileNameDecIdx, (unsigned char *)fCdpDstBufAddr, fOutBufSize))
          fileNameDecIdx++;
      }
...

    {
      ...
      if(dump2file(DUMP_DEC_LVL_RING,"dumpDecPipe_decRing", &fileNameDecRingIdx, fDecDstBufAddrVA0[0], fDecDstBufSize[0]))
        fileNameDecRingIdx++;
    }
...
bool JpgDecHal::allocIOMEM()
{
    if(fMemType == IMG_MEM_TYPE_PMEM){
      ...
      if(fOutPmemVA) //if use PMEM
        fCdpDstBufAddr = (void*)fOutPmemVA ; //(unsigned int)fDstAddr ;
...
bool JpgDecHal::start()
{
    ...
    // dump source image
    {
      ...
      if(dump2file(DUMP_DEC_LVL_SRCBS,"dumpDecPipe_decSrc", &fileNameSrcIdx, fSrcAddr, fSrcBsSize))
        fileNameSrcIdx++;
    }
    ...
    if( fScaleType == IMG_RESCALE_TYPE_LINK ){
      ...
      // dump output buffer
      {
        ...
        if(dump2file(DUMP_DEC_LVL_OUT,"dumpDecPipe_out", &index, fDstAddr, fOutBufSize))
          index++;
      }
