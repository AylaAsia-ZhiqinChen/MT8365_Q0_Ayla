...
int main()
{
    ...
    unsigned int test_type = MEM_TYPE_PMEM;//MEM_TYPE_M4U ;//MEM_TYPE_ION;//
    int ionFD ;    
    ...
    if(test_type == MEM_TYPE_PMEM){
      JPEG_PMEM_ALLOC(BUFFER_SIZE, src_va, src_pa, src_fd);
    }else if(test_type == MEM_TYPE_M4U){ 
      IMGDEC_ALLOC(alloc_src_va, BUFFER_SIZE, 64, src_va)        
      ...
      alloc_ion (ionFD, &src_ion_alloc_data, &src_fd_data, BUFFER_SIZE, &src_va);
#endif            
    }
    ...
    jpgDec->setSrcAddr(src_va+HeaderSize);

    if(test_type == MEM_TYPE_PMEM){
      jpgDec->setSrcAddrPA(src_pa+HeaderSize);
    }

    jpgDec->setSrcSize(BUFFER_SIZE, BUFFER_SIZE);
    jpgDec->parse();
    printf("go %d !!\n", __LINE__);   
    ...
    jpgDec->setTileBufNum(tileRowNum);
    jpgDec->setFirstDecRow(1);
    
    
    dst_size    = jpgDec->getMinLumaBufSize()* tileRowNum ; 
    dst_size1   = jpgDec->getMinCbCrBufSize()* tileRowNum ;
    ...
    if(test_type == MEM_TYPE_PMEM){    
      ...
      IMGDEC_ALLOC(alloc_dst_buffer , dst_size , 64, dst_buffer )
      IMGDEC_ALLOC(alloc_dst_buffer1, dst_size1, 64, dst_buffer1)
      IMGDEC_ALLOC(alloc_dst_buffer2, dst_size1, 64, dst_buffer2)    
    ...
    {     
      ...
      //int decodeRow = 1;
      int RowPerGo = 12 ;
      for(i =1 ; (i+RowPerGo)<tileRowNum ;i+=RowPerGo){
         
        jpgDec->ResumeDecRow(RowPerGo);
        //decodeRow += RowPerGo ;
      }
      jpgDec->ResumeDecRow(tileRowNum - i);
       printf("go %d !!\n", __LINE__);  
...
#if 0
{
    ...
    if(test_type == MEM_TYPE_PMEM){
      JPEG_PMEM_FREE(src_va, BUFFER_SIZE, src_fd);
...
#ifdef TEST_ION      
      free_ion (ionFD, &src_ion_alloc_data , &src_fd_data, BUFFER_SIZE, &src_va);
