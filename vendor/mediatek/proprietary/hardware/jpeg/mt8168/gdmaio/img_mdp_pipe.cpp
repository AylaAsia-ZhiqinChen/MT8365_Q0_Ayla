...
#define IMGRSZ_W(fmt, arg...)    ALOGW(fmt, ##arg)
...
int ImgCdpPipe::onHWScalerDeInit()
{
   ...
   if(pStream != NULL){
     ...
     //android::Tracer::traceBegin(ATRACE_TAG_GRAPHICS,"FragSTOP");
...
bool ImgCdpPipe::getSrcBuffer(void **pSrcBuf, JUINT32 *pSrcBufStride, JUINT32 *tBufRowNum )
{
    ATRACE_CALL();
    ...
    if(fDstFormat == SCALER_OUT_3P_YVU420){
   ...
   if(pStream == NULL){
    ...
    IMGRSZ_D("ScalerInit: CONFIG_DST:fmt %d, w/h(%d %d), stride %d, addr0 %x, addr1 %x, addr2 %x, size0 %x, size1 %x, size2 %x!!\n", fOutputFormat, fOutWidth, fOutHeight, fOutWidth,
...

    {
        ...
        status = pStream->setDstBuffer(dstBuf, dstBufSize, planeNumber);
    }
    ...
    //android::Tracer::traceBegin(ATRACE_TAG_GRAPHICS,"FragStart");
...
//    decodeRowNum = dqBufHeight/fRowHeight[0] ;
    ...
    for(int i = 0;i <3;i++){
      ...
      cfgCdpData.tSrcBufVA[i] = fSrcBufAddr0[i] ;
      cfgCdpData.tSrcBufPA[i] = fSrcBufAddrPA0[i] ;
      IMGRSZ_D("ScalerInit: [%d] w %d, h %d,s %d, tSize %x, rSize %x, va %p, pa %x!! \n", i,fSrcImgWidth[i], fSrcImgHeight[i], fSrcBufStride[i]
      ,cfgCdpData.tSrcBufTotalSize[i], cfgCdpData.tSrcBufRingSize[i], fSrcBufAddr0[i], fSrcBufAddrPA0[i]);
    }
...
bool ImgCdpPipe::Start()
{
    ...
    if( fScalerMode != SCALER_MODE_TILE || fTileRunCnt == 0){
      IMGRSZ_D("ReSizer Src Addr bank[0]:%p %p %p, [1]:%p %p %p !! ", fSrcBufAddr0[0], fSrcBufAddr0[1], fSrcBufAddr0[2], fSrcBufAddr1[0], fSrcBufAddr1[1], fSrcBufAddr1[2]);
