...
typedef struct
{
    ...
    unsigned int srcChromaSize;

    unsigned int dstDatasize;
    unsigned int dstSecHandle;
    unsigned int srcLumaSecHandle;
    unsigned int srcChromaSecHandle;
    unsigned int bIssecure;
    unsigned char bIsSrc2p;

    unsigned char enableEXIF;                        ///< the flag to encode SOI header or not
    unsigned char isPhyAddr;                         ///< the flag to record dstBufferAddr is physical or not
    unsigned char enableSyncReset;                   ///< the flag to enable sync reset bit

    JPEG_ENCODE_QUALITY_ENUM dstQuality;    ///< the encoded quality
    JPEG_SAMPLING_FORMAT dstFormat;         ///< the encoded format
    JPEG_ENC_RESULT_ENUM encresult;
}JPEG_ENC_HAL_IN;
