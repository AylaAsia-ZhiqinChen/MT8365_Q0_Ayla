...
int jpeg_drv_parse_file(UINT8 *f_ptr, UINT32 f_size, JPEG_FILE_INFO *info)
{
    ...
    ALOGW(" enter Driver parse !!!"); 
    if ((byte1 != JPEG_MARKER_START_CODE) || (byte2 != JPEG_MARKER_SOI))
    { 
        ALOGW("HW parse can't find Start Code (%x %x) from %p !!!",(unsigned int)byte1, (unsigned int)byte2, f_ptr); 
        return -1;
    }
        ...
        switch (byte2)
        {
            ...
            case JPEG_MARKER_SOF2:
                ...
                if(info->componentNum != 1 && info->componentNum != 3){ 
                  ALOGW("HW decoder unsupport componentNum (%d) !!!", info->componentNum); 
                  return -1;
                }
            ...
            case JPEG_MARKER_DHT:
                ...
                {
                    ALOGW("Marker:%x length:%d to larger", byte2, index_offset);
                    return -1;
                }
                ...
                if(byte2 == JPEG_MARKER_DHT)
                {
                    ...
                    {
                        ALOGW("Marker:%x table number error : 0x%x", byte2, byte);
                        return -1;
                    }
                    count = 1;
                ...
                {
                    if((byte >> 4) != 0){
                        ALOGW("Marker DQT:invalid number error : 0x%x", byte);
                        return -1;
                    }
                    if(index_offset > 65)
                    {
                        ...
                        if((byte >> 4) != 0)
                        {
                            ALOGW("Marker:%x Pq number error : 0x%x", byte2, byte);
                            return -1;                        
                        }
                    }
            ...
            //     ALOGW("Marker:%x unsupported : 0x%x", byte2, byte);
            //     return -1;                
            //   break;    
            default:
...
int jpeg_drv_calculate_info(JPEG_FILE_INFO *info)
{
    ...
    switch (info->componentNum)
    {
        ...
        default:
            ALOGW("Unsupport color component number %d!!!", info->componentNum);
            info->samplingFormat = 0;
            return -1;
    }
    ...
    {
        ALOGW("SamplingFactorMax = 0 (%d %d) ", info->hSamplingFactorMax, info->vSamplingFactorMax);
        return -1;
    }
    ...
    for(i = 0 ; i < info->componentNum ; i++)
    {
        ...
        {
            ALOGW("SamplingFactor[%d] = 0 (%d %d) ", i, info->hSamplingFactor[i], info->vSamplingFactor[i]);
            return -1;
        }
