...
JPEG_ENC_STATUS_ENUM jpegEncConfigEncoder(int drvID, JPEG_ENC_HAL_IN *inParams)
{
    ...
    drvIn.bSecure               = inParams->bIssecure;
    ...
    xlog("config: dst_base %x, dst_of %x, dst_ms %x, m4u %x, secure %d!!\n",
    drvIn.dstBufferAddr, drvIn.dstBufAddrOffset, drvIn.dstBufAddrOffsetMask, m4u_flag, drvIn.bSecure);

    //xlog("JpegEncHal: 420 %x, Src_Y %x, Src_C %x, imgS %x, memS %x, DU %x, %x, %x, %x, \n");
    xlog("JPEG_ENC_IOCTL_CONFIG:%d,sizeof(%d)",JPEG_ENC_IOCTL_CONFIG,sizeof(JPEG_ENC_DRV_IN));
...
//
//------------------------------------------------------------------------
JPEG_ENC_STATUS_ENUM jpegEncWaitIRQ(int drvID, long timeout_msec, JPEG_ENC_HAL_IN *JpgEncParam)
{
    ...
    drvOut.bSecure = JpgEncParam->bIssecure;
    ...
    JpgEncParam->dstDatasize = fileSize;

    switch(encodeResult)
    {
        case 0:
            JpgEncParam->encresult = JPEG_ENC_RESULT_DONE;
            break;
        case 1:
            JpgEncParam->encresult = JPEG_ENC_RESULT_PAUSE;
            break;
        case 2:
            JpgEncParam->encresult = JPEG_ENC_RESULT_HALT;
            break;
    }
