...
#if 1
#define IMGMMU_W(fmt, arg...)    ALOGD(LOG_TAG fmt, ##arg)
...
bool imgMmu_create(MTKM4UDrv **ppM4uDrv, M4U_MODULE_ID_ENUM mID)
{
   ...
   IMGMMU_D("::[CREATE] enable m4u %p, ID %d, L:%d,  JD(9),JE(10),GD(14),IMG(15),VD(1)!! \n", *ppM4uDrv, mID, __LINE__);
...
bool imgMmu_reset(MTKM4UDrv **ppM4uDrv, M4U_MODULE_ID_ENUM mID)
{
    ...
    if(*ppM4uDrv == NULL){
       IMGMMU_W("::[RESET] m4u is %p, mID %d, L:%d!! \n", *ppM4uDrv, mID, __LINE__);
       return false;
    }
#if 0
...
bool imgMmu_alloc_pa_mci(MTKM4UDrv **ppM4uDrv, M4U_MODULE_ID_ENUM mID, void* va, JUINT32 size, JUINT32 *pa)
{
   unsigned int prot = M4U_PROT_READ | M4U_PROT_WRITE;
    ...
    if(*ppM4uDrv == NULL || size == 0 || va == 0){
       IMGMMU_W("::[ALLOC] m4u is %p, mID %d, size %x, va %x, L:%d!! \n", *ppM4uDrv, mID, size, va,__LINE__);
       return false;
    }
    {
...
{
    if(pM4uDrv == NULL){
       IMGMMU_W("::[CONFIG] m4u is %p, mID %d, L:%d!! \n", pM4uDrv, mID, __LINE__);
       return false;
    }
#if 0
    M4U_PORT_STRUCT m4uPort;
...
{
  if(pM4uDrv == NULL){
     IMGMMU_W("::[SYNC] m4u is %p, mID %d, L:%d!! \n", pM4uDrv, mID, __LINE__);
     return false;
  }
...
void imgMmu_dealloc_pa(MTKM4UDrv *pM4uDrv, M4U_MODULE_ID_ENUM mID, void* va, JUINT32 size, JUINT32 *pa)
{
      ...
      if(pM4uDrv == NULL){
         IMGMMU_W("::[DEALLOC] m4u is %p, mID %d, L:%d!! \n", pM4uDrv, mID, __LINE__);
         return ;
      }
...
bool imgMmu_pa_map_tlb(MTKM4UDrv **ppM4uDrv, M4U_MODULE_ID_ENUM mID, JUINT32 size, JUINT32 *pa)
{
    ...
    if(*ppM4uDrv == NULL || size == 0 ){
       IMGMMU_W("::[MAP] m4u is %p, mID %d, size %x, L:%d!! \n", *ppM4uDrv, mID, size,__LINE__);
       return false;
    }
...
{
    if( pM4uDrv == NULL){
       IMGMMU_W("::[UNMAP] m4u is %p, mID %d, L:%d!! \n", pM4uDrv, mID, __LINE__);
       return ;
    }
...
bool imgIon_sync(JINT32 ionFD, JINT32 bufHandle)
{
    ...
    if(err != 0)
    {
      IMGMMU_D("[ION_SYNC] imgIon_sync fail (err_no = %x)!!\n", err);
      return false;
    }
    return true;
}
