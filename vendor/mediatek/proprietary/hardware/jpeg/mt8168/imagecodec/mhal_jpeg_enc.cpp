...
int mHalJpgEncStart(MHAL_JPEG_ENC_START_IN *encInParams)
{
    ...
    jHalEncoder->setEncSize(encInParams->imgWidth, encInParams->imgHeight,
            (JpgEncHal::EncFormat)encInParams->encFormat, encInParams->bOnePlane);
    ...
    jHalEncoder->SetSrcBufHeight(encInParams->AlignedH);

#ifdef JPEG_SUPPORT_SECURE_PATH
    if (encInParams->bSecure)
    {
        jHalEncoder->setSecureMode(encInParams->bSecure);
        jHalEncoder->setSrcSecHandle(encInParams->SrcLumaSecHandle, encInParams->SrcChromaSecHandle);
        jHalEncoder->setDstSecHandle(encInParams->DstSecHandle);
    }
#endif
