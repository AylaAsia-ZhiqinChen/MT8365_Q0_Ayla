...
status_t AudioALSAHardwareResourceManager::setRoutePath(const uint32_t scenario, const audio_devices_t devices)
{
    ...
    case ROUTE_NORMAL_PLAYBACK:
        ...
        if (AudioHfpController::getInstance()->getHfpEnable()) {
            ...
            applyRoutePath(ROUTE_HFP_CLIENT, true);
#endif
            return NO_ERROR;
        }
#endif
...
#ifdef MTK_BT_PROFILE_HFP_CLIENT_AUDIO
#if !defined(MTK_CONSYS_MT8167)
        applyRoutePath(ROUTE_HFP_CLIENT, false);
#endif
        return NO_ERROR;
#endif
        if (devices == (AUDIO_DEVICE_OUT_SPEAKER | AUDIO_DEVICE_OUT_WIRED_HEADSET) ||
...
void AudioALSAHardwareResourceManager::DeviceDoDcCalibrate(void)
{
    ...
    ctl = mixer_get_ctl_by_name(mMixer, "HP DC Offsets");
    if (!ctl) {
        ALOGW("%s() missing mix ctrl for headphone dc offsets", __FUNCTION__);
        return;
    }

    count = mixer_ctl_get_num_values(ctl);

    memset(&mAudioBufferDcCalibrate, 0, sizeof(mAudioBufferDcCalibrate));

    // read calibration result from NVRAM
    AudioCustParamClient::GetInstance()->GetDcCalibrationParamFromNV(&mAudioBufferDcCalibrate);

    if (mAudioBufferDcCalibrate.cali_flag != true) {
        // query calibration result
        mixer_ctl_get_array(ctl, (void *) mAudioBufferDcCalibrate.cali_vals_hp, count);

        // one time calibration for first boot
        mAudioBufferDcCalibrate.cali_flag = true;

        // update calibration result to NVRAM
        ...
        // update setting to device mixer ctrl
        mixer_ctl_set_array(ctl, mAudioBufferDcCalibrate.cali_vals_hp, count);
    }
	#endif
}
