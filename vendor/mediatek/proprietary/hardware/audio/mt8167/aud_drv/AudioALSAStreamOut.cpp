    ...
    uint32_t flags)
{
    ...
    *status = NO_ERROR;
...
#if !defined(DOWNLINK_LOW_LATENCY)
    flags &= ~AUDIO_OUTPUT_FLAG_FAST;
#endif

    if (devices == AUDIO_DEVICE_OUT_AUX_DIGITAL)
    {
        if ((flags & AUDIO_OUTPUT_FLAG_DIRECT) || (flags & AUDIO_OUTPUT_FLAG_IEC958_NONAUDIO)) {
    ...
    {
        mStreamOutType = STREAM_OUT_LOW_LATENCY;
    }
    else
    {
        mStreamOutType = STREAM_OUT_PRIMARY;
    ...
    TYPE_LOGD("%s(), devices = 0x%x, format = 0x%x, channels = 0x%x, sampleRate = %d, flags = 0x%x",
              __FUNCTION__, devices, *format, *channels, *sampleRate, flags);

    if (mStreamManager->isStreamOutExist(mStreamOutType)) {
        TYPE_LOGW("%s() stream out(%u) existed already", __FUNCTION__, mStreamOutType);
        *status = BAD_VALUE;
        return *status;
    }

    bool specifiedFormat = true;

    // handle default format
    if (*format == 0) {
        *format = kDefaultOutputSourceFormat;
        specifiedFormat = false;
    } else {
    ...
    if (*channels == 0)
        *channels = (mStreamOutType == STREAM_OUT_HDMI_MULTI_CHANNEL) ?
                     AUDIO_CHANNEL_OUT_5POINT1 : AUDIO_CHANNEL_OUT_STEREO;

    uint32_t channelCount = popcount(*channels);

    // check format, sample rate & channel count
    if (mStreamOutType == STREAM_OUT_HDMI_STEREO ||
        mStreamOutType == STREAM_OUT_HDMI_MULTI_CHANNEL) {
        if (!mStreamManager->isHdmiStreamOutExist()) {
            // query edid info when first hdmi stream triggers
            HDMITxController::queryEdidInfo();
        }

        if (!HDMITxController::isSinkSupportedFormat(*format, *sampleRate, channelCount)) {
            if (mStreamOutType == STREAM_OUT_HDMI_MULTI_CHANNEL && !specifiedFormat) {
                // check if support raw output in case multi-ch PCM is not supported
                if (!HDMITxController::isSinkSupportedFormat(AUDIO_FORMAT_AC3) &&
                    !HDMITxController::isSinkSupportedFormat(AUDIO_FORMAT_E_AC3)) {
                    TYPE_LOGW("%s() unsupported format 0x%x rate %u ch_mask %u by sink",
                          __FUNCTION__, *format, *sampleRate, *channels);
                    *status = BAD_VALUE;
                }
            } else {
                TYPE_LOGW("%s() unsupported format 0x%x rate %u ch_mask %u by sink",
    ...
    else
    {
        ...
        {
            TYPE_LOGE("%s(), wrong format 0x%x, use 0x%x instead.",
                      __FUNCTION__, *format, kDefaultOutputSourceFormat);
            *format = kDefaultOutputSourceFormat;
            *status = BAD_VALUE;
            return *status;
        }
        ...
        if (*channels != kDefaultOutputSourceChannelMask) {
            TYPE_LOGE("%s(), wrong channels 0x%x, use 0x%x instead.",
                      __FUNCTION__, *channels, kDefaultOutputSourceChannelMask);
            *channels = kDefaultOutputSourceChannelMask;
            *status = BAD_VALUE;
            return *status;
        }

        if (!SampleRateSupport(*sampleRate)) {
            TYPE_LOGE("%s(), wrong sampleRate %d, use %d instead.",
                      __FUNCTION__, *sampleRate, kDefaultOutputSourceSampleRate);
            *sampleRate = kDefaultOutputSourceSampleRate;
            *status = BAD_VALUE;
            return *status;
        }
    ...
    mStreamAttributeSource.num_channels = channelCount;
    mStreamAttributeSource.sample_rate = *sampleRate;

    // setup buffer size and latency
    setBufferSize();

    if (mStreamOutType == STREAM_OUT_HDMI_MULTI_CHANNEL)
    {
        // basic audio capability
        checkRedundantAndAdd(mSupportedSampleRates, String8("32000"));
        checkRedundantAndAdd(mSupportedSampleRates, String8("44100"));
        checkRedundantAndAdd(mSupportedSampleRates, String8("48000"));
        checkRedundantAndAdd(mSupportedFormats, String8(ENUM_TO_STRING(AUDIO_FORMAT_PCM_16_BIT)));
        checkRedundantAndAdd(mSupportedChannelMasks, String8(ENUM_TO_STRING(AUDIO_CHANNEL_OUT_STEREO)));

        // check multi-ch capability
        if (HDMITxController::isSinkSupportedFormat(AUDIO_FORMAT_PCM_16_BIT, 48000, 6))
            checkRedundantAndAdd(mSupportedChannelMasks, String8(ENUM_TO_STRING(AUDIO_CHANNEL_OUT_5POINT1)));

        if (HDMITxController::isSinkSupportedFormat(AUDIO_FORMAT_PCM_16_BIT, 48000, 8))
            checkRedundantAndAdd(mSupportedChannelMasks, String8(ENUM_TO_STRING(AUDIO_CHANNEL_OUT_7POINT1)));

        // check raw capbility
        if (HDMITxController::isSinkSupportedFormat(AUDIO_FORMAT_AC3))
            checkRedundantAndAdd(mSupportedFormats, String8(ENUM_TO_STRING(AUDIO_FORMAT_AC3)));

        if (HDMITxController::isSinkSupportedFormat(AUDIO_FORMAT_E_AC3))
            checkRedundantAndAdd(mSupportedFormats, String8(ENUM_TO_STRING(AUDIO_FORMAT_E_AC3)));

        // add extra capability if sink can support raw bitstream
        if (HDMITxController::isSinkSupportedFormat(AUDIO_FORMAT_AC3) ||
            HDMITxController::isSinkSupportedFormat(AUDIO_FORMAT_E_AC3))
        {
            checkRedundantAndAdd(mSupportedSampleRates, String8("176400"));
            checkRedundantAndAdd(mSupportedSampleRates, String8("192000"));
            checkRedundantAndAdd(mSupportedFormats, String8(ENUM_TO_STRING(AUDIO_FORMAT_IEC61937)));
        }
    }
    ...
    else if (mStreamOutType == STREAM_OUT_PRIMARY)
    {
