...
int AudioFtm::RecieverTest(char receiver_test)
{
    ...
    if (receiver_test) // enable
    {
        ...
        mHardwareResourceManager->setRoutePath(ROUTE_NORMAL_PLAYBACK, AUDIO_DEVICE_OUT_EARPIECE);
    ...
    else // disable
    {
        ...
        mHardwareResourceManager->resetRoutePath(ROUTE_NORMAL_PLAYBACK, AUDIO_DEVICE_OUT_EARPIECE);
...
int AudioFtm::EarphoneTest(char bEnable)
{
    ...
    if (bEnable) // enable
    {
        ...
        mHardwareResourceManager->setRoutePath(ROUTE_NORMAL_PLAYBACK, AUDIO_DEVICE_OUT_WIRED_HEADPHONE);
        mHardwareResourceManager->openAddaOutput(32000);
        mHardwareResourceManager->setSgenMode(SGEN_MODE_O03_O04);
        mHardwareResourceManager->setSgenSampleRate(SGEN_MODE_SAMPLERATE_32000HZ);
    }
    else // disable
    {
        mHardwareResourceManager->setSgenMode(SGEN_MODE_DISABLE);
        mHardwareResourceManager->closeAddaOutput();
        mHardwareResourceManager->resetRoutePath(ROUTE_NORMAL_PLAYBACK, AUDIO_DEVICE_OUT_WIRED_HEADPHONE);
        mHardwareResourceManager->stopOutputDevice();
    }
    return true;
}
...
/// Input device test
int AudioFtm::SpecificBuildInMicTest(char type __unused)
{
    //TODO
    return true;
}
...
int AudioFtm::HDMI_SineGenPlayback(bool Enable, int Freq)
{
    ...
    if (Enable)
    {
        ...
        if (mixer_ctl_set_value(mixer_get_ctl_by_name(mMixer, "TDM_Out_Sgen_Switch"), 0, 1) < 0)
            ALOGW("%s mixer_ctl_set_enum_by_string TDM_Out_Sgen_Switch On failed", __FUNCTION__);
    }
    else
    {
        if (mixer_ctl_set_value(mixer_get_ctl_by_name(mMixer, "TDM_Out_Sgen_Switch"), 0, 0) < 0)
            ALOGW("%s mixer_ctl_set_enum_by_string TDM_Out_Sgen_Switch On failed", __FUNCTION__);
