...
class AudioHfpController
{
    public:
	 ...
	 virtual status_t setHfpMute(bool mute);
    ...
    protected:
		...
		status_t boostEchoRefData(char *buffer, uint32_t size);
        ...
        FILE *mAudioHfpULEchoPcmMergeInputDumpFile;
        ...
        bool mMicMute;
	 bool mHfpMute;
	 bool mHfpDLinputClearBTMuteSignal;//signal sample value 00 00 55 55 55 55
		...
		char *mAudioULEchoPcmMergeInputBuffer;

		status_t setULEchoPcmMergeAttribute();

        status_t ULEchoPcmMergeSourceAllocate();
        status_t ULEchoPcmMergeSourceStart();
        int ULEchoPcmMergeGetDataSource();
		status_t ULEchoPcmMergeSourceFree();
        status_t ULEchoPcmMergeSourceStop();
};
