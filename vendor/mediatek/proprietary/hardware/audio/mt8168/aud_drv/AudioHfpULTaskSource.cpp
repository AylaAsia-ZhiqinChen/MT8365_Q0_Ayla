...
status_t AudioHfpULTaskSource::start()
{
    ...
    if (!mStarted)
    {
        ...
        else
        {
	     ...
	     memset(mBitConverterReadBuffer, 0, mMaxPcmDriverBufferSize);
        }
#else
        ...
        {
            mAudioALSAVolumeController->ApplyMicGain(VOIP_Normal_Mic, AUDIO_MODE_IN_COMMUNICATION);
        }
        else if (mOutputDevice == AUDIO_DEVICE_OUT_SPEAKER)
        {
            mAudioALSAVolumeController->ApplyMicGain(VOIP_Handfree_Mic, AUDIO_MODE_IN_COMMUNICATION);
        }
        else
        {
            mAudioALSAVolumeController->ApplyMicGain(VOIP_Headset_Mic, AUDIO_MODE_IN_COMMUNICATION);
        }
...
ssize_t AudioHfpULTaskSource::read(void *buffer, ssize_t bytes)
{
    ...
    if(mFormat == AUDIO_FORMAT_PCM_16_BIT)
    {
        unsigned int pcmReadSize = bytes * 2;
