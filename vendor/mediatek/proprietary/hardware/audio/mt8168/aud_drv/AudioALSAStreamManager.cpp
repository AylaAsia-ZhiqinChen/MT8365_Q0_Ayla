...
#ifdef MTK_ADSP_VA_SUPPORT
    mAudioALSAAdspVaController(AudioALSAAdspVaController::getInstance()),
    mAdspVaNeedOn(false),
    mForceDisableAdspVaForSetMode(false),
#endif
...
#ifdef MTK_ADSP_VA_SUPPORT
    // make sure adsp voice detect is resume when all capture stream stop if need
    if (mAdspVaNeedOn == true &&
        mForceDisableAdspVaForSetMode == false) {
        ALOGD("%s(), resume adsp voice detect", __FUNCTION__);
        if (mAudioALSAAdspVaController->getVoiceDetectEnable() == false) {
            mAudioALSAAdspVaController->setVoiceDetectEnable(true);
        }
    }
#endif
...
AudioALSACaptureHandlerBase *AudioALSAStreamManager::createCaptureHandler(
...
#endif
    {
        if (stream_attribute_target->input_source == AUDIO_SOURCE_FM_TUNER) {
            ...
            pCaptureHandler = new AudioALSACaptureHandlerHotword(stream_attribute_target);
        }
#endif
            ...
            if ((stream_attribute_target->input_source != AUDIO_SOURCE_VOICE_UNLOCK) &&
                ...
                } else  if (stream_attribute_target->input_source != AUDIO_SOURCE_UNPROCESSED)
                {
                    stream_attribute_target->BesRecord_Info.besrecord_enable = EnableBesRecord();
                }
            }
...
#if defined(MTK_ADSP_VA_SUPPORT)
    //TODO for AUDIO_SOURCE_VOICE_RECOGNITION may not support here
    if (stream_attribute_target->input_source != AUDIO_SOURCE_HOTWORD) {
        if (mAudioALSAAdspVaController->getVoiceDetectEnable() == true) {
            ALOGI("Not Hotword Record,Actually Force Close VOW");
            mAudioALSAAdspVaController->setVoiceDetectEnable(false);
        }
    }
#endif
...
#ifdef MTK_ADSP_VA_SUPPORT
    // make sure adsp voice detect is closed before leaving normal mode
    if (new_mode != AUDIO_MODE_NORMAL) {
        mForceDisableAdspVaForSetMode = true;
        if (mAudioALSAAdspVaController->getVoiceDetectEnable() == true) {
            ALOGD("%s(), force close adsp voice detect", __FUNCTION__);
            mAudioALSAAdspVaController->setVoiceDetectEnable(false);
        }
    }
#endif
...
status_t AudioALSAStreamManager::DeviceNoneUpdate() {
    ...
    mAudioALSAAdspVaController->updateDeviceInfoForVoiceWakeUp();
#endif
...
#ifdef MTK_ADSP_VA_SUPPORT
status_t AudioALSAStreamManager::setAdspVaDetectNeedOn(const bool enable) {
    status_t retVal = NO_ERROR;

    ALOGD("+%s(), mAdspVaNeedOn: %d => %d ", __FUNCTION__, mAdspVaNeedOn, enable);
    AL_AUTOLOCK(mLock);

    if (enable == mAdspVaNeedOn) {
        ALOGW("-%s(), enable(%d) == mAdspVaNeedOn(%d), return", __FUNCTION__, enable, mAdspVaNeedOn);
        return INVALID_OPERATION;
    }

    if (enable == true) {
        if (mStreamInVector.size() != 0 || mForceDisableAdspVaForSetMode == true) {
            ALOGD("-%s(), mStreamInVector.size() = %zu, mForceDisableVoiceWakeUpForSetMode = %d, return",
                  __FUNCTION__, mStreamInVector.size(), mForceDisableAdspVaForSetMode);
        } else {
            retVal= mAudioALSAAdspVaController->setVoiceDetectEnable(true);
        }
    } else {
        retVal = mAudioALSAAdspVaController->setVoiceDetectEnable(false);
    }

    mAdspVaNeedOn = enable;

    ALOGD("-%s(), mAdspVaNeedOn: %d, retVal:%d", __FUNCTION__, mAdspVaNeedOn, retVal);
    return retVal;
}
#endif
...
void AudioALSAStreamManager::handleHfpClientState(bool enable)
{
    ...
    if(enable)
    {
        hfpController->setHfpMute(false);
    }
    else
    {
        hfpController->setHfpMute(true);
    }
...

    {
        ...
        if (enable)
        {
            ...
            if (mixer_ctl_set_value(mixer_get_ctl_by_name(mixer, "BTCVSD Bypass Switch"), 0, 0)) {
        ...
        else
        {
            ...
            if (mixer_ctl_set_value(mixer_get_ctl_by_name(mixer, "BTCVSD Bypass Switch"), 0, 1)) {
               ALOGE("Error: BTCVSD Bypass Switch invalid value");
            }
        }
