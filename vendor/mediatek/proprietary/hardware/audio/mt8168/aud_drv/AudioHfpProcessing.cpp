...
#define EnhanceModeParasExtNum1 32
#define EnhanceParasNum1  (SPEECH_PARA_NUM + SPEECH_COMMON_NUM + EnhanceModeParasExtNum1)  // (16+12+32)
...
status_t AudioHfpProcessing::UpdateParameters()
{
   if (mAudioCustParamClient->GetAudioHFPParamFromNV(&mHfpProcessingParam) == 0) {
    ...
    for (i = 0; i < SPEECH_COMMON_NUM; i+=6) {
        ALOGV("%s speech_common_para[%d:%d] = %u-%u-%u-%u-%u-%u", __FUNCTION__, i, i+5,
    ...
    for (i = 0; i < NUM_AUDIO_HFP_DEVICE; i++) {
        for (j = 0; j < SPEECH_PARA_NUM; j+=8) {
            ALOGV("%s speech_mode_para[%d][%d:%d] = %u-%u-%u-%u-%u-%u-%u-%u", __FUNCTION__, i, j, j+7,
    ...
    for (i = 0; i < NUM_AUDIO_HFP_DEVICE; i++) {
        for (j = 0; j < WB_FIR_NUM; j+=10) {
            ALOGV("%s in_fir[%d][%d:%d] = %u-%u-%u-%u-%u-%u-%u-%u-%u-%u", __FUNCTION__, i, j, j+9,
        ...
        for (j = 0; j < WB_FIR_NUM; j+=10) {
            ALOGV("%s out_fir[%d][%d:%d] = %u-%u-%u-%u-%u-%u-%u-%u-%u-%u", __FUNCTION__, i, j, j+9,
...
{
    (void)mode;
    memcpy(&mSph_Enh_ctrl.enhance_pars, pEnhance_pars, EnhanceParasNum1 * sizeof(unsigned long));
    return true;
}
...
{
    ALOGD("%s mode = %d gain = %ld", __FUNCTION__, mode, gain);
    mMIC_DG = gain;
    return true;
}
...
bool AudioHfpProcessing::Start()
{
    ...
    int index = 0;
    unsigned long HDRecordEnhanceParas[EnhanceParasNum1] = {0};
    ...
    for (index = SPEECH_PARA_NUM; index < SPEECH_PARA_NUM + EnhanceModeParasExtNum1; index++)
    {
        // Fill with 0 for parameter extension
        HDRecordEnhanceParas[index] = 0;
    }
    for (index = SPEECH_PARA_NUM + EnhanceModeParasExtNum1; index < EnhanceParasNum1; index++)
    {
        // Common parameters
        HDRecordEnhanceParas[index] = mHfpProcessingParam.speech_common_para[index - (SPEECH_PARA_NUM + EnhanceModeParasExtNum1)];
    }
