...
{
    //
    android::sp<IStreamBufferProvider> pProvider = mpProvider.promote();
    //
    if ( pProvider == 0 ) {
        MY_LOGW(
            "[%s:%p] NULL promote of buffer pool:%p",
            ParentT::getName(), this, mpProvider.unsafe_get()
        );
        return;
    }
    //
    ...
    resetBuffer();
    //  Release to Provider
    pProvider->enqueStreamBuffer(mStreamInfo, this, bufstatus);
}
