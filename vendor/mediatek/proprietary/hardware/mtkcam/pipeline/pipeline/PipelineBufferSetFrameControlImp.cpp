...
)   const
{
    ...
    if  ( ! pItem->mBitStatus.hasBit(eBUF_STATUS_ACQUIRE) ) {
        ...
        MY_LOGF_IF(pItem->mBuffer!=0, "[requestNo:%u frameNo:%u streamId:%#" PRIxPTR "] Non-null buffer but non-acquired status:%#x", getRequestNo(), getFrameNo(), streamId, pItem->mBitStatus.value);
