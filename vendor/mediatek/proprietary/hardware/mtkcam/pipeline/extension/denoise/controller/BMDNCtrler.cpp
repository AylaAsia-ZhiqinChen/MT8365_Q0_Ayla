...
#define LOG_TAG "BMDNCtrler"
static const char* __CALLERNAME__ = LOG_TAG;

#include "BMDNCtrler.h"
// log
...
{
    FUNCTION_SCOPE;
    setCaptureNum(getBMDNCaptureCnt());
    setDelayFrameNum(getBMDNDelayFrameCnt());
    setHWSyncStableNum(getHWSyncStableCnt());
    mpCB = new BMDNListenerImp(this);
...
{
    FUNCTION_SCOPE;
    MY_LOGD("CKH: request(%p) main1Frame(%p) main2Frame(%p)",
                    request.get(),
                    main1Frame.get(),
                    main2Frame.get());
