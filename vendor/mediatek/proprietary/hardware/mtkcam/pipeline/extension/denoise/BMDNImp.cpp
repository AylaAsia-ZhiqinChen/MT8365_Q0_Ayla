 ...
 */
#define __DEBUG
#define LOG_TAG "BMDNVendorImp"
static const char* __CALLERNAME__ = LOG_TAG;
#include "BMDNImp.h"
#include "controller/BMDNCtrler.h"
#include "controller/MFHRCtrler.h"
// mtkcam
...
using NS3Av3::IHal3A;
//
wp<BMDNVendor> BMDNVendorImp::mThis = nullptr;
sp<IBMDeNoisePipe> BMDNVendorImp::gspFeaturePipe = nullptr;
MINT32 BMDNVendorImp::gDenoisePipeMode = -1;
MINT32 BMDNVendorImp::gReferenceCount = 0;
bool BMDNVendorImp::gbAlreadyGet = false;
bool BMDNVendorImp::gbFirstSet = true;
ControllerContainer<BMDNVendorImp::CtrlerType> BMDNVendorImp::sUnusedCtrlers;
static Mutex BMDNVendorCreateLock;
    //
static std::mutex mCtrlerLock;
//
std::future<void> BMDNVendorImp::gPipeSyncThread;
// ----------------------------------------------------------------------------
// helper function
// ----------------------------------------------------------------------------
static
inline
std::shared_ptr<BaseDNCtrler>
create_denoise_controller(
    MINT32 mode
)
{
    std::shared_ptr<BaseDNCtrler> p = nullptr;
    if(BMDeNoiseFeatureType::TYPE_BMDN == mode)
    {
        MY_LOGD("create bmdn ctrler");
        p = std::shared_ptr<BaseDNCtrler>(
                            new BMDNCtrler(mode));
    }
    else if(BMDeNoiseFeatureType::TYPE_MFHR == mode)
    {
        MY_LOGD("create mfhr ctrler");
        p = std::shared_ptr<BaseDNCtrler>(
                            new MFHRCtrler(mode));
    }
    else
    {
        MY_LOGD("mode(%d) not support", mode);
    }
    return p;
}
// ----------------------------------------------------------------------------
//
// ----------------------------------------------------------------------------
sp<BMDNVendor> BMDNVendor::createInstance(
        char const*  pcszName,
        ...
        )
{
    ...
    Mutex::Autolock _l(BMDNVendorCreateLock);
    sp<BMDNVendor> pInstance = BMDNVendorImp::mThis.promote();
    if(pInstance != nullptr)
    {
    }
    else
    {
        BMDNVendorImp::mThis = new BMDNVendorImp(pcszName, i4OpenId, vendorMode);
        pInstance = BMDNVendorImp::mThis.promote();
        MY_LOGD("[create by %d]createInstance() new %p", i4OpenId, pInstance.get());
    }
    BMDNVendorImp::gReferenceCount++;
    MY_LOGD("[%d] Count(%d): caller(%s)", i4OpenId, BMDNVendorImp::gReferenceCount, pcszName);
    return pInstance;
}
// ----------------------------------------------------------------------------
//
...
BMDNVendorImp(
    ...
    MINT32 const i4OpenId,
    MINT64 const vendorMode
) : BaseVendor(i4OpenId, vendorMode, String8::format("BMDN vendor"))
{
    FUNCTION_SCOPE(i4OpenId);
...
)
{
    ...
    auto checkDenoiseMode = [](const InputInfo& in, MINT32& mode)
    {
        for(auto itr : in.combination)
        {
            if(itr == MTK_PLUGIN_MODE_BMDN)
            {
                mode = BMDeNoiseFeatureType::TYPE_BMDN;
                return true;
            }
            else if(itr == MTK_PLUGIN_MODE_MFHR)
            {
                mode = BMDeNoiseFeatureType::TYPE_MFHR;
                return true;
            }
            /*else if(itr == MTK_PLUGIN_MODE_BMDN_3rdParty)
            {
                mode = BMDeNoiseFeatureType::TYPE_BMDN_3rdParty;
                return true;
            }
            else if(itr == MTK_PLUGIN_MODE_MFHR_3rdParty)
            {
                mode = BMDeNoiseFeatureType::TYPE_MFHR_3rdParty;
                return true;
            }*/
    }
        return false;
    ...
    std::shared_ptr<CtrlerType> ctrler = nullptr;
    {
        ...
        if(!checkDenoiseMode(in, mFeatureMode))
        {
            MY_LOGE("[%d] cannot find suitable mode for denoise vendor", openId);
            return UNKNOWN_ERROR;
        }
        // check need to create denoise pipe
        if(BMDNVendorImp::gPipeSyncThread.valid())
        {
            SCOPE_TIMER(__t0, "wait sync");
            MY_LOGD("have share state");
            BMDNVendorImp::gPipeSyncThread.get();
        }
        if(    mFeatureMode != BMDNVendorImp::gDenoisePipeMode
            || isJpegSizeChanged(openId, in)
            ...
            if(CC_LIKELY(gspFeaturePipe.get()))
            {
                ...
                gspFeaturePipe = nullptr;
                //clean ctler
                BMDNVendorImp::sUnusedCtrlers.clear();
                mUsedCtrlers.clear();
            }
            BMDNVendorImp::gPipeSyncThread =
                std::async(std::launch::async,
                            [this, &openId]()
                            {
                                SCOPE_TIMER(__t1, "denoise pipe create");
                                MY_LOGD("[%d] create new pipe(%d)", openId, mFeatureMode);
                                gspFeaturePipe = IBMDeNoisePipe::createInstance(
                                                miMain1OpenId,
                                                miMain2OpenId,
                                                mFeatureMode);
            ...
            BMDNVendorImp::gDenoisePipeMode = mFeatureMode;
        }
    ...
    MUINT32 finalCaptureCnt = frameCount;
#ifdef __DEBUG
    // {{{ show info
    MY_LOGD("[%d] %s: cap  = %u", openId, __FUNCTION__, frameCount);
    MY_LOGD("[%d] %s: dely = %u", openId, __FUNCTION__, frameDelayCount);
    // }}}
#endif
    ...
    for(MUINT32 i = 0 ; i<finalCaptureCnt ; i++)
    {
        bool bLastFrame = ((i + 1) == finalCaptureCnt);
        ...
        updateAppSettings(&appSetting, openId, mFeatureMode,
                          expParam, capParam,
                          (i==0) ? MTRUE : MFALSE);
...
)
{
    ...
    std::shared_ptr<BMDNVendorImp::CtrlerType> ctrler = nullptr;
    if(gbFirstSet)
    {
        // for first set, just can set requestNo and do not remove
        // from container.
        MY_LOGD("set request only");
        ctrler = getCtrler(CTRLER_UNUSED, BMDNVendorImp::gDenoisePipeMode);
    }
    else
    {
    ...
    }
    {
        ...
        if(!gbFirstSet)
        {
            ...
            asyncRun([this, ctrler]() mutable {
                    ::prctl(PR_SET_NAME, "doDenoise", 0, 0, 0);
...
)
{
    ...
    MY_LOGD("CKH: spCtrl(%p)", spCtrl.get());
...
)
{
    ...
    BMDNVendorImp::mThis = nullptr;
    BMDNVendorImp::gspFeaturePipe = nullptr;
    BMDNVendorImp::gDenoisePipeMode = -1;
    BMDNVendorImp::gReferenceCount = 0;
    BMDNVendorImp::gbAlreadyGet = false;
    BMDNVendorImp::gbFirstSet = true;
}
// ----------------------------------------------------------------------------
//
// ----------------------------------------------------------------------------
...
//
// ----------------------------------------------------------------------------
ControllerContainer<NSCam::plugin::BMDNVendorImp::CtrlerType>*
BMDNVendorImp::
...
//
// ----------------------------------------------------------------------------
std::shared_ptr<NSCam::plugin::BMDNVendorImp::CtrlerType>
BMDNVendorImp::
getCtrler(CTRLER u, MINT32 mode)
{
    ...
    (
        ...
        (Data_T& data)
        {
            if(data.size() <= 0)
            {
                ctrler = create_denoise_controller(mode);
...
void
BMDNVendorImp::
store_app_setting(IMetadata* pAppMeta, BaseDNCtrler* ctrler)
{
    // {{{
    // get the data and saves to BaseDNCtrler,
    MUINT8 __aemode = 0;
    MUINT8 __awblock = 0;
    MUINT8 __eis = 0;
    MINT32 __iso = 0;
...
{
    FUNCTION_SCOPE(openId);
    MINT32 enable = ::property_get_int32("debug.bmdn.updateAE", 1);
    if(!enable){
    ...
    switch(featureMode){
        case TYPE_BMDN:
        case TYPE_BMDN_3rdParty:
            isPureZSD = getIsPureZSD_BMDN();
            break;
        case TYPE_MFHR:
        case TYPE_MFHR_3rdParty:
            isPureZSD = getIsPureZSD_MFHR();
            break;
        default:
    ...
    if(needUpdate3A)
    {
        ...
        MUINT32 AEManualMode = E3ACtrl_EnableBMDN;
        switch(featureMode){
            case TYPE_BMDN:
            case TYPE_BMDN_3rdParty:
                AEManualMode = E3ACtrl_EnableBMDN;
                break;
            case TYPE_MFHR:
            case TYPE_MFHR_3rdParty:
                AEManualMode = E3ACtrl_EnableMFHR;
                break;
            default:
                MY_LOGW("unknown feature type:%d", featureMode);
        }
        hal3a->send3ACtrl((E3ACtrl_T)AEManualMode, 1, 0);
        hal3a->send3ACtrl(E3ACtrl_GetExposureInfo,  (MINTPTR)&expParam, 0);
        hal3a->send3ACtrl(E3ACtrl_GetExposureParam, (MINTPTR)&capParam, 0);
        hal3a->send3ACtrl((E3ACtrl_T)AEManualMode, 0, 0);
    }
    MINT32 newISO = capParam.u4RealISO;
    MINT32 newExp = capParam.u4Eposuretime;
...
)
{
    FUNCTION_SCOPE(openId);
    bool ret = false;
    if(in.jpegYuv->getImgSize().w == 0 || in.jpegYuv->getImgSize().h == 0){
        MY_LOGD("jpegYuv size is 0, ignored");
        return ret;
    }
    if(in.jpegYuv->getImgSize().w == 1 && in.jpegYuv->getImgSize().h == 1){
        MY_LOGD("jpegYuv is dummy, ignored");
        return ret;
    }
    if(mPrvJpegSize.w != 0 && mPrvJpegSize.h != 0){
        if( in.jpegYuv->getImgSize().w != mPrvJpegSize.w ||
            in.jpegYuv->getImgSize().h != mPrvJpegSize.h
        ){
            ret = true;
        }
    }
    MY_LOGD("prv(%dx%d)->cur(%dx%d), ret:%d",
        mPrvJpegSize.w, mPrvJpegSize.h,
        in.jpegYuv->getImgSize().w, in.jpegYuv->getImgSize().h,
        ret
    );
    mPrvJpegSize = in.jpegYuv->getImgSize();
    return ret;
}