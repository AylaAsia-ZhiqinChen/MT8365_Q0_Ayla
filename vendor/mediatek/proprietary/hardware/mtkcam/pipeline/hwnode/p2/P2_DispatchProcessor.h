...
class PartialP2Request : virtual public P2Request
{
public:
    PartialP2Request(const sp<P2Request> &request);
    virtual ~PartialP2Request();
    virtual MBOOL isResized() const;
    virtual MBOOL isReprocess() const;

private:
    sp<P2Request> mFullRequest;
    MBOOL mIsResized;
    MBOOL mIsReprocess;
};
...
private:
    sp<P2Request> parseCaptureRequest(const sp<P2Request> &request);
    sp<P2Request> parseStreamingRequest(const sp<P2Request> &request);
    MBOOL needBasicProcess(const sp<P2Request> &request);
    MBOOL needDebugDump(const sp<P2FrameRequest> &request);
...
private:
    ...
    MUINT32 mForceProcessor;
    MUINT32 mDebugDump;
    MUINT32 mDebugDumpCount;
    MBOOL mEnableScanline;
};
