...
{
    CAM_TRACE_NAME("P2_MDP:threadStart()");
}

MVOID MDPProcessor::onThreadStop()
{
    CAM_TRACE_NAME("P2_MDP:threadStop()");
}
...
{
    CAM_TRACE_NAME("P2_MDP:enque()");
    MBOOL ret = MTRUE;
    if( param.mDst.size() && isValid(param.mSrc) )
    {
        Logger logger = param.mRequest->getLogger();
        ...
        while( i < n )
        {
            ...
            if( out == 1 )
            {
                mSImager->execute(src, dst[0], dst[1], crop[0], tran[0], 0xFFFFFFFF);
            }
            else if (out == 2 )
            {
                mSImager->execute(src, dst[0], dst[1], crop[0], crop[1], tran[0], tran[1], 0xFFFFFFFF);
            }
        }
        for( i = 0; i < n; ++i)
        {
...
{
    CAM_TRACE_NAME("P2_MDP:notifyFlush()");
}

MVOID MDPProcessor::onWaitFlush()
{
    CAM_TRACE_NAME("P2_MDP:waitFlush()");
}
