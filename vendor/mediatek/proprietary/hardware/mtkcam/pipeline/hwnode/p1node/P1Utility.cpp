...
releaseBuffer(sp<IImageBuffer> & imageBuffer)
{
    ...
    //
    MY_LOGD2("StuffBuffer-Release (%s) index(%zu) (%zu/%d)",
...
createBuffer(sp<IImageBuffer> & imageBuffer)
{
    ...
    imageBuffer = pImgBuf;
    //
    MY_LOGD2("StuffBuffer-Create (%s) (%zu/%d) "
    ...
    MUINT8 multiple, MBOOL writable, MBOOL isSecure, SecType secType)
{
    ...
    if (bufPool == NULL) {
        ...
        it = mvPoolSet.end();
        bufPool = *(it - 1);
        MY_LOGD1("PoolSet.size(%zu)", mvPoolSet.size());
    }
    //
    U_if (bufPool == NULL) {
...
TimingCheckerMgr::setEnable(MBOOL en)
{
    ...
    MY_LOGD0("TimingChecker enable(%d)", en);
    {
        Mutex::Autolock _l(mLock);
        mIsEn = en;
        if (mIsEn) {
...
ProcedureStageControl::done(MUINT32 eStage, MBOOL bSuccess)
{
    ...
    MY_LOGI0("StageCtrl done(%d) OK", eStage);
    return MTRUE;
};
