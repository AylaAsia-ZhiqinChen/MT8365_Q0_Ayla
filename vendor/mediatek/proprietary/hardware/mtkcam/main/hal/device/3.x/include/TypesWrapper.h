...
public:
    ...
    {
        __v34Instance.streams = ::android::hardware::hidl_vec<V3_4::Stream>(p.streams.size());
        int i = 0;
        for(auto &s : p.streams) {
            (V3_4::Stream)__v34Instance.streams[i++] = (V3_4::Stream&)WrappedStream(s);
        }
        __v34Instance.operationMode = p.operationMode;

        // No value:
        // __v34Instance.sessionParams is empty
    }
    ...
    operator V3_2::StreamConfiguration() const
    {
        ...
        v32Instance.streams = ::android::hardware::hidl_vec<V3_2::Stream>(__v34Instance.streams.size());
        int i = 0;
        for(auto &s : __v34Instance.streams) {
            (V3_2::Stream)v32Instance.streams[i++] = (V3_2::Stream&)WrappedStream(s);
        }
        v32Instance.operationMode = __v34Instance.operationMode;
        return v32Instance;
    }

    operator V3_2::StreamConfiguration&()
    {
        __v32Instance.streams = ::android::hardware::hidl_vec<V3_2::Stream>(__v34Instance.streams.size());
        int i = 0;
        for(auto &s : __v34Instance.streams) {
            (V3_2::Stream)__v32Instance.streams[i++] = (V3_2::Stream&)WrappedStream(s);
        }
        __v32Instance.operationMode = __v34Instance.operationMode;
        return __v32Instance;
    }

    operator V3_4::StreamConfiguration() const { return __v34Instance; }
    operator V3_4::StreamConfiguration&() { return __v34Instance; }

private:
    V3_4::StreamConfiguration __v34Instance;
