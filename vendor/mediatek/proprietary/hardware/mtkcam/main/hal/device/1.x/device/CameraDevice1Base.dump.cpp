...
//
#define DUMPSYS_COMMAND "vendor.debug.camera.hal.dumpsys"
...
dumpState(const hidl_handle& fd)
{
    ...
    Vector<String8> vCommand = queryCommandVector(vFd);
    if  ( vCommand.empty() )
    {
        return  Status::OK;
    }
    ...
    if  ( *vCommand.begin() == "DisplayClient" )
    {
        if  ( mpDisplayClient != 0 ) {
            vCommand.erase(vCommand.begin());
            mpDisplayClient->dump(vFd, vCommand);
        }
        return  Status::OK;
    }
    //
    ...
    if  ( *vCommand.begin() == "CamAdapter" )
    {
        if  ( mpCamAdapter != 0 ) {
            vCommand.erase(vCommand.begin());
            mpCamAdapter->dump(vFd, vCommand);
        }
        return  Status::OK;
    }
    //
