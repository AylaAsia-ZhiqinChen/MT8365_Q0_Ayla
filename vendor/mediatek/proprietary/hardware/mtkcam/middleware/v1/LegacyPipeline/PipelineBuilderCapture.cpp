...
#if (MTK_CAM_STEREO_DENOISE_SUPPORT == 1) || (MTKCAM_HAVE_DUAL_ZOOM_FUSION_SUPPORT == 1)
#define DUALCAM (1)
...
MBOOL updateYuv(Param& rParam) {
    ...
    sp<IImageStreamInfo>& pYuv_01 = rParam.pCfg->pImage_Yuv01;
    //
    if( pYuv == 0 && pYuv_01 == 0 && pYuvJpeg == 0 && pYuvThumbnail == 0) return MTRUE;
    ASSERT_NOT_NULL(pImgoRaw, "should config imgo");
    //
    if(!rParam.bMemoryIn)
    {
    ...
    // Most single camera feature not support submit full raw & resized raw to P2Node
    // at the same time, so the default iomap without rrzo in P2 IOMap.
    // If the feature must to query resized raw, it should create dedicated streams and use
    // submitRequest to get the desired rrzo image stream.
#if DUALCAM
    if ( pRrzoRaw != NULL && rParam.dualcamMode == NSCam::v1::Stereo::E_STEREO_FEATURE_DENOISE)
       rParam.pMapP2->addIn( pRrzoRaw->getStreamId() );
#endif

    if ( pYuv != 0 ) rParam.pMapP2->addOut ( pYuv->getStreamId() );
    if ( pYuv_01 != 0 ) rParam.pMapP2->addOut ( pYuv_01->getStreamId() );
...
)
{
    ...
    // Node
    {
        ...
        NodeT::ConfigParams cfgParam;
        {
            ...
            cfgParam.enableQuadCode = (!mConfigParams.enableRrzoCapture &&
                                       mConfigParams.support4cellsensor &&
                                       mConfigParams.enableQuadCode) ? MTRUE : MFALSE;
...
)
{
    ...
    if(bMemoryIn)
    {
        for(size_t i = 0 ; i < srcParams.size() ; i++)
        {
            ...
            if(srcParams[i].pInfo->getStreamId() == eSTREAMID_IMAGE_PIPE_RAW_RESIZER)
                mpConfig->pImage_RrzoRaw = srcParams[i].pInfo;
        }
    ...
    sp<IImageStreamInfo>& pImage_Yuv01        = mpConfig->pImage_Yuv01;
    ...
    // Node
    {
        ...
        MINT32 supportDS = ::property_get_int32("camera.mdp.ds.enable", 1);
        ...
        NodeT::ConfigParams cfgParam;
        {
            ...
            if ( pImage_Yuv01 != 0 )
                cfgParam.vOutImage.push_back(pImage_Yuv01);
            if ( pImage_YuvJpeg != 0 ) {
                cfgParam.vOutImage.push_back(pImage_YuvJpeg);
                if(supportDRE || supportCZ)
                    cfgParam.pOutCaptureImage = pImage_YuvJpeg;
            }

#if (MTKCAM_HAVE_DUAL_ZOOM_FUSION_SUPPORT == 1)
            // set Thubmnail to iVendor
            if (StereoSettingProvider::getStereoShotMode() == eShotMode_FusionShot)
            {
                cfgParam.vOutImage.push_back(pImage_YuvThumbnail);
            }
            else
#endif
            {
            ...
            // if vendorManagerId has been specified, use the specified one
            if (mConfigParams.vendorManagerId <= 0 )
                cfgParam.pVendor = plugin::NSVendorManager::get(getOpenId());
            else
                cfgParam.pVendor = plugin::NSVendorManager::get(mConfigParams.vendorManagerId);

            cfgParam.uUserId = mConfigParams.pluginUser;
            cfgParam.customOption = (mConfigParams.enableRrzoCapture)? 0 : mConfigParams.support4cellsensor;
            if(supportCZ)
                cfgParam.customOption |= v3::P2Node::CUSTOM_OPTION_CLEAR_ZOOM_SUPPORT;
            if(supportDRE && (getMode()!=LegacyPipelineMode_T::PipelineMode_MShotCapture))
                cfgParam.customOption |= v3::P2Node::CUSTOM_OPTION_DRE_SUPPORT;
            if(supportDS && (getMode()!=LegacyPipelineMode_T::PipelineMode_MShotCapture) )
                cfgParam.customOption |= v3::P2Node::CUSTOM_OPTION_DOWNSCALE_DN;
        }
        //
        ...
        add_stream_to_set(vOut, pImage_Yuv01);
...
create()
{
    ...
    IOMap jpeg_Image_IOMap;
    {
        ...
        params.pMapP2_rrzo = &p2_Rrzo_IOMap;
    ...
    {
        p2_Image_IOMap_Set.add(p2_Image_IOMap);
        if (p2_Rrzo_IOMap.isEmpty() == MFALSE && mConfigParams.needAddRrzoIntoP2IoMap) {
            MY_LOGD("add p2_Rrzo_IOMap to p2_Image_IOMap_Set");
            p2_Image_IOMap_Set.add(p2_Rrzo_IOMap);
        }
        pRequestBuilder->setIOMap(
                eNODEID_P2Node,
                p2_Image_IOMap_Set,
