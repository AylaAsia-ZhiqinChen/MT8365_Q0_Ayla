    ...
    PipelineContext* pipeContext)
{
    ...
    {
        MY_LOGD("imageManager+");
        addStreamBegin(MFALSE);
        if(initParam.nodeId == eNODEID_HWBokehNode ||
           initParam.nodeId == eNODEID_BokehNode ||
           initParam.nodeId == eNODEID_VendorBokehNode)
        {
            ...
            //
            cfgParam.pInHalImageP2Tuning = imageManager->getStreamInfoByStreamId(eSTREAMID_IMAGE_PIPE_STEREO_P2_TUNING);
            addStream(  cfgParam.pInHalImageP2Tuning, MTRUE);
            //
            cfgParam.pInHalImageDepth = imageManager->getStreamInfoByStreamId(eSTREAMID_IMAGE_PIPE_DEPTHMAPNODE_DEPTHMAPYUV);
            addStream(  cfgParam.pInHalImageDepth, MTRUE);
            //
        ...
        //
        if(initParam.nodeId == eNODEID_SWBokehNode ||
           initParam.nodeId == eNODEID_BokehNode)
        {
            ...
            // sw bokeh
            cfgParam.pInHalDepthWrapper = imageManager->getStreamInfoByStreamId(eSTREAMID_IMAGE_PIPE_DEPTHMAPNODE_HAL_DEPTHWAPPER);
            addStream(  cfgParam.pInHalDepthWrapper, MTRUE);
            // sw bokeh
            cfgParam.pInHalExtraData = imageManager->getStreamInfoByStreamId(eSTREAMID_IMAGE_PIPE_STEREO_DBG);
            addStream(  cfgParam.pInHalExtraData, MTRUE);
            //
            ...
            // sw bokeh
            cfgParam.pOutAppDepthMapWrapper = imageManager->getStreamInfoByStreamId(eSTREAMID_IMAGE_PIPE_STEREO_DEPTHWRAPPER);
            addStream(  cfgParam.pOutAppDepthMapWrapper, MFALSE);
            //
            cfgParam.pOutAppExtraData = imageManager->getStreamInfoByStreamId(eSTREAMID_IMAGE_PIPE_STEREO_APP_EXTRA_DATA);
            addStream(  cfgParam.pOutAppExtraData, MFALSE);
        }
        //
    ...
    {
        MY_LOGD("SW");
        cfgParam.bokehMode = BokehNode::SW_BOKEH_NODE;
    }
    else if(initParam.nodeId == eNODEID_HWBokehNode)
    {
        MY_LOGD("HW");
        cfgParam.bokehMode = BokehNode::HW_BOKEH_NODE;
    }
    else if(initParam.nodeId == eNODEID_VendorBokehNode)
    {
        MY_LOGD("Vendor");
        cfgParam.bokehMode = BokehNode::VENDOR_BOKEH_NODE;
    }
    else
    {
        MY_LOGD("ALL");
        cfgParam.bokehMode = BokehNode::ALL_HW;
    }
    MY_LOGD("-");
}
//
...
dump()
{
    MY_LOGD("Dump bokeh node config data");
    MY_LOGD("mvImageIOSet.size(%d)", mvImageIOSet.size());
    for(MUINT32 i=0;i<mvImageIOSet.size();++i)
    {
        MY_LOGD("mvImageIOSet[%d].vIn.size(%d)", i, mvImageIOSet[i].vIn.size());
        for(MUINT32 j=0;j<mvImageIOSet[i].vIn.size();++j)
        {
            MY_LOGD("InImageIOMap StreamId(%d)", mvImageIOSet[i].vIn[j]);
        }
        MY_LOGD("mvImageIOSet[%d].vOut.size(%d)", i, mvImageIOSet[i].vOut.size());
    ...
    MY_LOGD("mvMetaIOSet");
    MY_LOGD("mvMetaIOSet.size(%d)", mvMetaIOSet.size());
    for(MUINT32 i=0;i<mvMetaIOSet.size();++i)
    {
        MY_LOGD("mvMetaIOSet[%d].vIn.size(%d)", i, mvMetaIOSet[i].vIn.size());
        for(MUINT32 j=0;j<mvMetaIOSet[i].vIn.size();++j)
        {
            MY_LOGD("InMetaIOMap StreamId(%d)", mvMetaIOSet[i].vIn[j]);
        }
        MY_LOGD("mvMetaIOSet[%d].vOut.size(%d)", i, mvMetaIOSet[i].vOut.size());
