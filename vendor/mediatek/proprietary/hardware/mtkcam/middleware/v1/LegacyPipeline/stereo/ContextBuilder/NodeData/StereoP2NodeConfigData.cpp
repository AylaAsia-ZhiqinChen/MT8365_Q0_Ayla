 ...
 */
#define LOG_TAG "ContextBuilder/P2NodeData"
//
...
//
#include "StereoP2NodeConfigData.h"
...
#define DOUT    MFALSE
    ...
    if(metaManager != NULL)
    {
        ...
        //
        cfgParam.pOutAppMeta = metaManager->getStreamInfoByStreamId(eSTREAMID_META_APP_DYNAMIC_P2);
        addStream(cfgParam.pOutAppMeta, DOUT);
        //
        cfgParam.pOutHalMeta = metaManager->getStreamInfoByStreamId(eSTREAMID_META_HAL_DYNAMIC_P2);
    ...
    if(imageManager != NULL)
    {
        ...
        // resize yuv
        spStrmInfo = imageManager->getStreamInfoByStreamId(eSTREAMID_IMAGE_PIPE_THIRDPARTY_RESIZE_YUV_0);
        if(spStrmInfo != NULL)
        {
            cfgParam.vOutImage.push_back(spStrmInfo);
            addStream(spStrmInfo, DOUT);
        }
        // rrzo postview (only main1)
        spStrmInfo = imageManager->getStreamInfoByStreamId(eSTREAMID_IMAGE_PIPE_DUALYUVNODE_THUMBNAIL);
        if(spStrmInfo != NULL)
        {
            cfgParam.vOutImage.push_back(spStrmInfo);
            addStream(spStrmInfo, DOUT);
        ...
        addStreamBegin_keepIOSet(MFALSE);
        // imgo
        ...
        // full yuv
        spStrmInfo = imageManager->getStreamInfoByStreamId(eSTREAMID_IMAGE_PIPE_THIRDPARTY_FULLSIZE_YUV_0);
        if(spStrmInfo != NULL)
        {
            cfgParam.vOutImage.push_back(spStrmInfo);
            addStream(spStrmInfo, DOUT);
