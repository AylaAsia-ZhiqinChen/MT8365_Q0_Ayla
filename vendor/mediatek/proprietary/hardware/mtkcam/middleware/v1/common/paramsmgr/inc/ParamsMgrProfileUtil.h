 ...
 ************************************************************************************************/
#ifndef PARAMS_MGR_PROFILE_UTIL_H_
#define PARAMS_MGR_PROFILE_UTIL_H_
 ...
 *          profile.endProfile();
 */
class ParamsMgrProfileUtil : public android::RefBase
{
public:
     ...
     */
    ParamsMgrProfileUtil(const char *TAG, const char *MESSAGE)
        : PROFILE_ENABLED(property_get_int32("vendor.paramsmgr.profile", 0))
    {
        mTag = (TAG) ? std::string(TAG) : "";
        setMessage(MESSAGE);

#ifdef MET_USER_EVENT_SUPPORT
        if(PROFILE_ENABLED) {
            mMetTag = _getFullLogTag();
            met_tag_init();
        }
#endif
    }
...
// #endif
    ...
    {
        mMessage = (MESSAGE) ? std::string(MESSAGE) : "";
    }
 ...
 *          AutoParamsMgrProfileUtil profile(LOG_TAG, "xxxx");
 *          or
 *          AutoParamsMgrProfileUtil profile(LOG_TAG, __func__);
 *
 *          The log will be in format: [Benchmark]<TAG>::<MESSAGE>: ooo.xxx sec
 *          To enable/disable log: setprop debug.STEREO.log.Profile [1|0], default is 0
 * \see ProfileUtil
 */
class AutoParamsMgrProfileUtil : public ParamsMgrProfileUtil
{
public:
    /**
     ...
     * \param MESSAGE The message in prefix
     */
    AutoParamsMgrProfileUtil(const char *TAG, const char *MESSAGE) : ParamsMgrProfileUtil(TAG, MESSAGE)
    {
        beginProfile();
    }
