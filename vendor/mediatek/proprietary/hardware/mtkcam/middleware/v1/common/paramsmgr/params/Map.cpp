...
_PARAM_MAP_BEG_(eMapAppMode)
    ...
    _ADD_STRING_VALUE_MAP_(MtkCameraParameters::APP_MODE_NAME_MTK_DUALCAM,  eAppMode_DualCam);
    }
...
*   EXPOSURE TIME
*******************************************************************************/
_PARAM_MAP_BEG_(eMapExpTime)
    _DEFAULT_STRING_VALUE_(MtkCameraParameters::EXPOSURE_TIME_AUTO,     0) // Default
    {
    }
    //
    ...
    {
        VAL_T value = 0;
        if  ( str == MtkCameraParameters::EXPOSURE_TIME_AUTO ) {
            value = 0;
        }
        else {
        ...
        //
        CAM_LOGD_IF(1, "exposure time value=%d(%s)", value, str.string());
        return  value;
    }
_PARAM_MAP_END_(eMapExpTime)
