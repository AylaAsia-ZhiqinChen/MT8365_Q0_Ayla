...
#include <inc/ParamsMgrProfileUtil.h>
#include <map>
#include <vector>
#include <atomic>
...
enum PARAMS_MGR_LOG_LEVEL
{
    PARAMS_MGR_NO_LOG,
    PARAMS_MGR_SHOW_PARAMETER,
    PARAMS_MGR_SHOW_DEBUG,
};

#define IS_SHOW_PARAMS(level) (level >= PARAMS_MGR_SHOW_PARAMETER)
#define IS_SHOW_DEBUG(level)  (level >= PARAMS_MGR_SHOW_DEBUG)
...
public:     ////                        Operations.
    ...
    virtual status_t                    setParameters(String8 const& params);
    virtual String8                     getPreviewFormat() const;
    virtual void                        getPreviewSize(int *width, int *height) const;
    virtual void                        getIsoSpeed(int *iso) const;
    virtual void                        getExposureTime(int *exptime) const;
    virtual bool                        getManualCap() const;
    virtual bool                        getDynamicSwitchSensorMode() const;
    virtual bool                        getHighFpsCapture() const;
    virtual bool                        getNonSlowmotionPreviewLimitMaxFps(int *fps) const;
    virtual bool                        getLowLightCaptureUsePreviewMode() const;
    virtual void                        getVideoSize(int *width, int *height) const;
    virtual void                        getPictureSize(int *width, int *height) const;
    virtual void                        getDualZoomInfo(int *Main2ID, int *Supported) const;
    virtual uint32_t                    getZoomRatio(int id = -1) const;
    ...
    virtual bool                        getRecordLikely() const;
    virtual void                        setRecordLikely(bool Recording);
    ...
    virtual String8                     getStaticVhdrMode() const;
    ...
    virtual bool                        getDualZoomSupported() const;
    virtual uint32_t                    getCustomHint() const;
...
public:     ////                        Utility.
    ...
    virtual bool                        isSetParametersChangeNothing();
...
protected:  ////                        Update Params based on FeatureInfo
    ...
    bool                                updateSupCamZoomParams(NSCameraFeature::FeatureInfo const& rFeatureInfo);
...
public:
    virtual void                        getBokehPictureSize(int &width, int &height) const;

protected:
    virtual void                        updateLogStatus();

protected:
    virtual void                        getDiffParameters(const String8 &stringParams, DefaultKeyedVector<string,string> &diffParams, vector<string> &removedKeys);
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
//  Data Members.
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
protected:  ////
...
//#endif
    ...
    // for dual zoom
    Vector<uint32_t>                    mvZoomRatios_Sup;
    bool                                mIsDualZoomMode;
    int                                 mMain2CamID;
    bool                                mRecordLikely;
    int                                 mSwitchZoomRatio;

    bool                                mSetParametersByString = true;
    map<string, bool>                   mExistKeys;
    String8                             mStaticVhdrMode;
    //
    std::atomic<bool>                   mbSetParametersChangeNothing;
};
