...
updateParams(int32_t const i4FKeyIndex)
{
    ...
    if  ( rSceneKeyedMap.getType() == "default-values" )
    {
        ...
        //2017/10/04 SeanC, Don't update the flash-mode when torch mode
        char const* p = mParameters.get(s8KeyName.string());
        String8 value = (0 != p) ? String8(p) : String8::empty();
        if (value != CameraParameters::FLASH_MODE_TORCH) {
            mParameters.set(s8KeyName.string(), s8ItemDefault.string()); //ex: "scene-mode=auto"                   //ex: "scene-mode=auto"
        }
        //
    ...
    //
    //Don't update the EV-Mode to default when snow or beach scene
    char const* sceneMode = mParameters.get(CameraParameters::KEY_SCENE_MODE);
    if((s8KeyName == CameraParameters::KEY_EXPOSURE_COMPENSATION)
            && (0 != sceneMode)
            && ((0 == ::strcmp(sceneMode, CameraParameters::SCENE_MODE_SNOW))
               ||(0 == ::strcmp(sceneMode, CameraParameters::SCENE_MODE_BEACH)))) {
        MY_LOGD_IF(mEnableDebugLog, "Don't update the Exposure Compensation to default when snow or beach scene");
        return true;
    }
    //
...
updateUserTypeParams(String8 const& s8KeyName, NSCameraFeature::FeatureInfo const& rFeatureInfo)
{
    ...
    //
#if DUAL_CAM
    if  ( s8KeyName == MtkCameraParameters::KEY_SUPCAM_ZOOM_RATIO )
    {
        return  updateSupCamZoomParams(rFeatureInfo);
    }
#endif // DUAL_CAM
    //
    return  false;
...
bool
ParamsManager::
updateSupCamZoomParams(NSCameraFeature::FeatureInfo const& rFeatureInfo)
{
    MY_LOGD("+");
    //
    String8 const   s8SupCamId   = rFeatureInfo.getDefaultItem();
    String8 const   s8ZoomRatios = rFeatureInfo.getSupportedList();
    //
    mvZoomRatios_Sup.clear();
    for (size_t i = 0; i < rFeatureInfo.getSupportedSize(); i++)
    {
        mvZoomRatios_Sup.push_back(::atoi(rFeatureInfo.getSupportedItem(i).string()));
    }
    //
    MY_LOGD_IF(mEnableDebugLog, " %s=%s", MtkCameraParameters::KEY_SUPCAM_ZOOM_RATIO, s8ZoomRatios.string());
    //
    mMain2CamID = ::atoi(s8SupCamId.string());
    if (ms8ClientAppMode == MtkCameraParameters::APP_MODE_NAME_MTK_DUALCAM) {
        mIsDualZoomMode = 1;
    }
    //
#if (MTKCAM_HAVE_DUAL_ZOOM_SUPPORT == 1)
    mSwitchZoomRatio = DUALZOOM_SWICH_CAM_ZOOM_RATIO;
    mParameters.set(MtkCameraParameters::KEY_DUALCAM_SWITCH_ZOOM_RATIO, mSwitchZoomRatio);
    MY_LOGD_IF(mEnableDebugLog, "%s=%d", MtkCameraParameters::KEY_DUALCAM_SWITCH_ZOOM_RATIO, mSwitchZoomRatio);
...
updateSingleFrameCaptureHDRParams(NSCameraFeature::FeatureInfo const& rFeatureInfo)
{
    ...
    //
    MY_LOGD_IF(mEnableDebugLog, "%s=%s",
...
updateSensorSupportMZAF(NSCameraFeature::FeatureInfo const& rFeatureInfo)
{
    String8 const& rs8Supported = rFeatureInfo.getDefaultItem();
    //
    MY_LOGD_IF(mEnableDebugLog, "MZAF %s=%s", MtkCameraParameters::KEY_SUPPORT_MZAF_FEATURE, rs8Supported.string());
    //
...
updateHDRDetectionParams(NSCameraFeature::FeatureInfo const& rFeatureInfo)
{
    String8 const& rs8Supported = rFeatureInfo.getDefaultItem();
    //
    MY_LOGD_IF(mEnableDebugLog, "%s=%s", MtkCameraParameters::KEY_HDR_DETECTION_SUPPORTED, rs8Supported.string());
    //
...
updateDefaultParams1_ByDefault()
{
    ...
    //  exposure-time
    mParameters.set(MtkCameraParameters::KEY_EXPOSURE_TIME, MtkCameraParameters::EXPOSURE_TIME_AUTO);
    mParameters.set(getValuesKeyName(MtkCameraParameters::KEY_EXPOSURE_TIME), MtkCameraParameters::EXPOSURE_TIME_AUTO);
    //  manual-cap
    mParameters.set(MtkCameraParameters::KEY_MANUAL_CAP, MtkCameraParameters::OFF);
    mParameters.set(getValuesKeyName(MtkCameraParameters::KEY_MANUAL_CAP), MtkCameraParameters::OFF);
    //  iso-speed
    ...
    //  dynamic-switch-sensor-mode
    mParameters.set(MtkCameraParameters::KEY_DYNAMIC_SWITCH_SENSOR_MODE, MtkCameraParameters::OFF);
    mParameters.set(getValuesKeyName(MtkCameraParameters::KEY_DYNAMIC_SWITCH_SENSOR_MODE), MtkCameraParameters::OFF);
    //  high-fps-capture
    mParameters.set(MtkCameraParameters::KEY_HIGH_FPS_CAPTURE, MtkCameraParameters::OFF);
    mParameters.set(getValuesKeyName(MtkCameraParameters::KEY_HIGH_FPS_CAPTURE), MtkCameraParameters::OFF);
    //  KEY_NON_SLOWMOTION_PREVIEW_LIMIT_MAX_FPS
    mParameters.set(MtkCameraParameters::KEY_NON_SLOWMOTION_PREVIEW_LIMIT_MAX_FPS, 30);
    //  low-light-capture-use-preview-mode
    mParameters.set(MtkCameraParameters::KEY_LOW_LIGHT_CAPTURE_USE_PREVIEW_MODE, MtkCameraParameters::OFF);
    mParameters.set(getValuesKeyName(MtkCameraParameters::KEY_LOW_LIGHT_CAPTURE_USE_PREVIEW_MODE), MtkCameraParameters::OFF);
    //  Capture Mode
...
updateDefaultParams0()
{
    ...
    mParameters.set(CameraParameters::KEY_SUPPORTED_JPEG_THUMBNAIL_SIZES, "0x0,160x128,160x90");
...
//    mParameters.set(MtkCameraParameters::KEY_ISP_MODE, 0);     //0: normal
    ...
    //  parameters for customized hint.
    mParameters.set(MtkCameraParameters::KEY_CUSTOM_HINT, MtkCameraParameters::KEY_CUSTOM_HINT_0);
    //----------------------------------------------------------------------------------------------
    //  MTK proprietary parameters
    //----------------------------------------------------------------------------------------------
    MY_LOGD_IF(mEnableDebugLog, "-");
...
{
    // AutoParamsMgrProfileUtil profile(LOG_TAG, "updateSceneAndParams");
    return
            //  (1) Set Scene Mode.
            updateScene()
...
*
*******************************************************************************/
#define PROPERTY_KEY_HAL_FORCEAPPMODE    "vendor.hal.forceappmode"
bool
ParamsManager::
...
evaluateHalAppModeUtility(String8& mode) const
{
    ...
    property_get("vendor.hal.appmode", halForceMode, "None");
    ...
    //
    //  If Client App Mode != <Default> <MtkDualCam> App, then.
    //
    ...
    if  ( i4CamMode == MtkCameraParameters::CAMERA_MODE_NORMAL )
    {
        ...
        {
            int32_t i4ZsdMode = 0;
            i4ZsdMode = property_get_int32("vendor.camera.zsdmode", 0);
            MY_LOGD_IF(mEnableDebugLog, "CAMERA_MODE_NORMAL: zsd mode %d", i4ZsdMode);
...
*
*******************************************************************************/
#define PROPERTY_KEY_HAL_APPMODE    "vendor.hal.appmode"
bool
ParamsManager::
updateHalAppMode()
{
