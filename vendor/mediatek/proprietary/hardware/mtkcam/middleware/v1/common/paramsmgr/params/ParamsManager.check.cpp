...
{
    AutoParamsMgrProfileUtil profile(LOG_TAG, "checkParams");
    status_t status = OK;
...
checkFocusMode(CameraParameters const& params) const
{
    ...
    if(NULL != pszMode &&
       NULL == pszModeSupported)
    {
        pszModeSupported = mParameters.get(CameraParameters::KEY_SUPPORTED_FOCUS_MODES);
    }
...
{
    // Support diff parameters in setParameters()
    ...
    if ( NULL == strstr(supportedParams, param) )
    {
...
checkFocusAreas(CameraParameters const& params) const
{
    ...
    {
        int numArea = 0;
        if(NULL == params.get(CameraParameters::KEY_MAX_NUM_FOCUS_AREAS)) {
            numArea = mParameters.getInt(CameraParameters::KEY_MAX_NUM_FOCUS_AREAS);
        } else {
            numArea = params.getInt(CameraParameters::KEY_MAX_NUM_FOCUS_AREAS);
        }
...
checkMeteringAreas(CameraParameters const& params) const
{
    ...
    {
        int numArea = 0;
        if(NULL == params.get(CameraParameters::KEY_MAX_NUM_METERING_AREAS)) {
            numArea = mParameters.getInt(CameraParameters::KEY_MAX_NUM_METERING_AREAS);
        } else {
            numArea = params.getInt(CameraParameters::KEY_MAX_NUM_METERING_AREAS);
        }
...
{
    if(params.get(CameraParameters::KEY_PREVIEW_SIZE)) {
...
{
    if(params.get(CameraParameters::KEY_PREVIEW_FPS_RANGE)) {
...
checkZoomValue(CameraParameters const& params) const
{
    ...
    if(maxZoom < 0) {
        maxZoom = mParameters.getInt(CameraParameters::KEY_MAX_ZOOM);
    }

    if(zoomValue < 0) {
        zoomValue = mParameters.getInt(CameraParameters::KEY_ZOOM);
    }
