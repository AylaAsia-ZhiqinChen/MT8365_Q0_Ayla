...
public:     ////    Operations.
    virtual MBOOL   start(SensorParam const & /* rSensorParam */, MUINT32 u4ShotCount __attribute__((unused)) = 0xFFFFFFFF) {
                        return MFALSE;
                    };
    virtual MBOOL   startAsync(SensorParam const & rSensorParam);
    virtual MBOOL   startOne(SensorParam const & rSensorParam);
    virtual MBOOL   startOne(SensorParam const & /*rSensorParam*/, const IImageBuffer* /*pImgBuffer*/) {
                        return MFALSE;
                    }
...
{
    Mutex::Autolock _lock(mLock);
    return  getImp()->getRawSize(rSensorParam, size);
}
