...
setShotMode(uint32_t const u4ShotMode, const IState::ENState nextState, String8 const s8AppMode)
{
    ...
    mIsExtCallback = !(
            ((mu4ShotMode == eShotMode_NormalShot || mu4ShotMode == eShotMode_4CellRemosaicShot) && mCameraMode == MtkCameraParameters::CAMERA_MODE_NORMAL) ||
            (mu4ShotMode == eShotMode_VideoSnapShot) ||
            ((mu4ShotMode == eShotMode_ZsdShot) && mIsThirdPartyZsd == MTRUE)
            );
    return;
}
...
threadLoop()
{
    ...
    if(one.type == callback_type_exit)
    {
        ...
        {
            Mutex::Autolock _l(mLock);
            size_t sz = mqTodoCallback.size();
            if ( sz != 0)
            {
                for(size_t i=0; i<sz; i++)
                {
                    callback_data data = mqTodoCallback.front();
                    mqTodoCallback.pop();
                    MY_LOGD("mqTodoCallback[%d] type(%d) need be processed before exit!", (int)i, data.type);
...
processCallback(callback_data& one)
{
    ...
    switch(one.type)
    {
        ...
        {
            return handleRawNotifyCallback(one);
        }
...
handleShutter(callback_data const& data)
{
    ...
    //
    MY_LOGD("data.i4PlayShutterSound(%d) mu4ShotMode(%d) data.u4ShotMode(%d)", data.i4PlayShutterSound,mu4ShotMode,data.u4ShotMode);
    if(data.u4ShotMode != mu4ShotMode)
    {
        MY_LOGW("data.u4ShotMode(%d) != mu4ShotMode(%d), don't handle shutter callback!", data.u4ShotMode, mu4ShotMode);
        return false;
    }
    //
    if(data.u4ShotMode == eShotMode_Undefined)
    {
        MY_LOGW("data.u4ShotMode is eShotMode_Undefined!");
    }
    //
    MY_LOGD("(%d)[ShutterThread] +", ::gettid());
    if(mIsExtCallback)
...
handleP2doneNotify(callback_data const& data)
{
          ...
          mu4ShotMode == eShotMode_FaceBeautyShotCc )
    {
        ...
        if( !mbForceExit )
        {
            ...
            constexpr const int64_t TIMEDOUT_IN_NS = 250000000LL; // 250 ms
            for (size_t times = 0; true; ++times)
            {
                // timed out in TIMEDOUT_IN_NS for waiting state.
                MY_LOGD("wait state change [+]");
                status_t _result = stateWaiter.waitState(mNextState, TIMEDOUT_IN_NS);
                MY_LOGD("wait state change [-]");
                ...
                // if it's not timed out, may happen something wrong...
                ...
                // timed out, check the currest state, if it's NOT ZSLCapture,
                // no need to wait anymore.
                auto state = mpStateMgr->getCurrentStateEnum();
                if (state != IState::eState_ZSLCapture)
                {
                    break;
                }
            }
...
bool
ZipImageCallbackThread::
handleRawNotifyCallback(callback_data const& data)
{
    CAM_TRACE_CALL();
    if( mbForceExit )
    {
...
handleImageCallback(callback_data const& data)
{
    CAM_TRACE_CALL();
    //
    MY_LOGD("mNextState(%d) mu4ShotMode(%d) data.u4ShotMode(%d)", mNextState,mu4ShotMode,data.u4ShotMode);
    if(data.u4ShotMode != mu4ShotMode)
    {
        #if 1
        MY_LOGW("data.u4ShotMode(%d) != mu4ShotMode(%d), but still handle image callback", data.u4ShotMode, mu4ShotMode);
        #else
        MY_LOGW("data.u4ShotMode(%d) != mu4ShotMode(%d), don't handle image callback!", data.u4ShotMode, mu4ShotMode);
        data.pImage->release(data.pImage);
        return false;
        #endif
    }
    ...
    if  ( data.isFinalImage &&
          data.u4ShotMode != eShotMode_ZsdHdrShot &&
          data.u4ShotMode != eShotMode_ZsdMfllShot &&
          data.u4ShotMode != eShotMode_FaceBeautyShotCc &&
          data.u4ShotMode != eShotMode_ZsdShot &&
          data.u4ShotMode != eShotMode_BMDNShot &&
          data.u4ShotMode != eShotMode_MFHRShot &&
          data.u4ShotMode != eShotMode_DCMFShot &&
          data.u4ShotMode != eShotMode_DCMFHdrShot
        )
    {
        Mutex::Autolock _l(mExitLock);
        MY_LOGD("mbForceExit(%d)",mbForceExit);
        ...
        if( !mbForceExit )
        {
            ...
            if  ( OK != stateWaiter.waitState(mNextState) )
            {
                MY_LOGW(
                        ...
                        data.u4CallbackIndex, data.u4ShotMode
                        );
                return  false;
            }
        }
    ...
    bool bNeedExtCallback = false;
    if(data.u4ShotMode != mu4ShotMode)
    {
        bNeedExtCallback = !(
            ((data.u4ShotMode == eShotMode_NormalShot) && mCameraMode == MtkCameraParameters::CAMERA_MODE_NORMAL) ||
            (data.u4ShotMode == eShotMode_VideoSnapShot) ||
            ((data.u4ShotMode == eShotMode_ZsdShot) && mIsThirdPartyZsd == MTRUE));
        MY_LOGD("bNeedExtCallback(%d) data.u4ShotMode(%d) mIsThirdPartyZsd(%d)", bNeedExtCallback, data.u4ShotMode);
    }

    if( mIsExtCallback || bNeedExtCallback )
    {
#if DUAL_CAM
        // if background service is enable in VSDOF mode, just callback msg.
        // because background service, will using mmsdk to callback image.
        MBOOL bBackgroundServiceEnable =
                            ICallbackClientMgr::getInstance()->canSupportBGService();
        if( bBackgroundServiceEnable &&
            (data.u4ShotMode == eShotMode_DCMFShot || data.u4ShotMode == eShotMode_DCMFHdrShot)
          )
        {
            // release dummy buffer directlly.
            data.pImage->release(data.pImage);
            // callback notify to device.
        ...
        if  ( data.isFinalImage && !mbForceExit )
        {
            MY_LOGD("MTK_CAMERA_MSG_EXT_NOTIFY_CAPTURE_DONE - Index:%d ShotMode:%d",data.u4CallbackIndex, data.u4ShotMode);
            mpCamMsgCbInfo->mNotifyCb(
                    MTK_CAMERA_MSG_EXT_NOTIFY,
    ...
    if  (   data.isFinalImage &&
            !mbForceExit &&
            (   eShotMode_ContinuousShot == data.u4ShotMode ||
                eShotMode_ContinuousShotCc == data.u4ShotMode) )
     {
