...
protected:  //// handle data
    ...
    //  @param[in]  pHalMeta        The HAL metadata.
    //  @param[out] pAppMeta        The APP metadata.
    //  @note                       Image format can be also converted during
    //                              executing post EE/ANR.
    MERROR          postEeAnr(
                        sp<IImageBuffer>    src,
                        sp<IImageBuffer>    out,
                        IMetadata*          pHalMeta,
...
protected:  //// handle setting
    ...
    MBOOL           applyRawBufferSettings(
                        Vector< SettingSet > vSettings,
                        MINT32 shotCount,
                        MINT32 frontDummyCount = 0,
                        MBOOL  bNeedStoreSelector = MTRUE
                    );
...
protected:
    ...
    MVOID           endCapture();
    MVOID           endFailedCapture();
    MBOOL           constructCapturePipeline(Vector<BufferItemSet>& rBufSet);
...
protected:
    ...
    // get mutliple RAW buffers at the same time (from ZSD buffer)
    //  @note All Vector must be resized as the same of frameNo
    status_t        getSelectorDataMultiple(
    ...
    // the original flow
    status_t        runSubmitRequestAndGetSelectorDataFlow(
                        MINT32 shotCount,
                        Vector< SettingSet > &vSettings,
                        MINT32 Cap_type
                    );

    // new flow, taking ZSD buffers
    status_t        runGetSelectorDataMultipleFlow(
                        MINT32 shotCount,
                        MINT32 Cap_type
                    );


    status_t        returnSelectorBuffer(Vector<BufferItemSet>& rBufferSet);
...
protected:
    std::future<void>                                   mBoostThread;

    size_t          getFlashCaptureCount(
