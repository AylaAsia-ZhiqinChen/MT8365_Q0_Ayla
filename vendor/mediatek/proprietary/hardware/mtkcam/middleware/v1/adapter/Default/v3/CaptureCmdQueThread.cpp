...
struct Command
{
    ...
    //
    //  Operations.
    Command(EID const _eId = eID_EXIT, uint32_t shotMode = 0)
        : eId(_eId), u4ShotMode(shotMode)
    {}
    //
...
//~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
public:     ////        Interfaces.
    virtual status_t    onCapture(uint32_t shotMode);
...
threadLoop()
{
    ...
    if  ( getCommand(cmd) )
    {
        ...
        case Command::eID_CAPTURE:
            ...
            {
                mpCmdQueThreadHandler->onCaptureThreadLoop(cmd.u4ShotMode);
            }
            else
            {
...
status_t
CaptureCmdQueThread::
onCapture(uint32_t shotMode)
{
    postCommand(Command(Command::eID_CAPTURE, shotMode));
    return  OK;
}
