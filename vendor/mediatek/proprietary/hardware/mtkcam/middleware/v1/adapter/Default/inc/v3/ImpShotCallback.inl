...
public:     ////
    ...
    //
    //      u4BufOffset4Ts
    //          [I] Buffer offset for timestamp, if this value > 0,
    //              the bitstream data will be inserted u4BufOffset4Ts bytes at the beginning for
    //              saving timestamp (int64_t). Thus this value is either 0 or greater than 8.
    virtual bool                    onCB_PostviewClient(
                                        int64_t const   i8Timestamp,
                                        NSCam::IImageBuffer const *pPostviewBuf,
                                        uint32_t const  u4BufOffset4Ts = 0,
                                        uint32_t const  bBgServ = 0,
                                        uint32_t const  u4CbId = NSCam::MTK_CAMERA_CALLBACK_CLIENT_ID_SINGLE
                                    );
