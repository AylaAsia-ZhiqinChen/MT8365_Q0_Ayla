...
#define LOG_TAG "DepthResultCapture"

#include "DepthResultCaptureRequstBuilder.h"
// mtkcam
 ...
 *
 ******************************************************************************/
DepthResultCaptureRequstBuilder::
~DepthResultCaptureRequstBuilder()
{
...
onPrepareImageBufferProvider()
{
    ...
    sp<IImageStreamInfo> pAppDepthWrapperStreamInfo = nullptr;
    ...
    // pMainImage_Cap
    {
        pMainImage_Cap = createImageStreamInfo(
                                        (std::string("App:Image:Main_Cap")+std::to_string(miCaptureNo)).c_str(),
    ...
    // pAppDepthWrapperStreamInfo
    {
        pAppDepthWrapperStreamInfo = createImageStreamInfo(
                                        (std::string("App:Image:DepthWrapper")+std::to_string(miCaptureNo)).c_str(),
    ...
    // pLDCDataStreamInfo
    {
        pLDCDataStreamInfo = createImageStreamInfo(
                                        (std::string("App:Buf:LDC")+std::to_string(miCaptureNo)).c_str(),
    ...
    // pN3DDebugStreamInfo
    {
        pN3DDebugStreamInfo = createImageStreamInfo(
                                        (std::string("App:Buf:N3DDebug")+std::to_string(miCaptureNo)).c_str(),
    ...
    // allocate buffer
    ret = mpCallbackImageBufferManager->allocBuffer(pMainImage_Cap,  mpCallbackBufferHandler);
    ret |= mpCallbackImageBufferManager->allocBuffer(pAppDepthWrapperStreamInfo,  mpCallbackBufferHandler);
    ret |= mpCallbackImageBufferManager->allocBuffer(pLDCDataStreamInfo,    mpCallbackBufferHandler);
    ret |= mpCallbackImageBufferManager->allocBuffer(pExtraDataStreamInfo,  mpCallbackBufferHandler);
...
onSetImageCallback()
{
    ...
    // create clean image callback
    {
        params.streamId = eSTREAMID_IMAGE_PIPE_DEPTHMAPNODE_MAINIMAGE_CAPYUV;
        params.streamName = "MainImage";
    ...
    // create depth
    {
        params.streamId = eSTREAMID_IMAGE_PIPE_DEPTHMAPNODE_HAL_DEPTHWAPPER;
        params.streamName = "DepthWapper";
        params.fileExtension = ".yuv";
