...
)
{
    ...
    //
    camera_memory = requestMemory(pMemHeapBase->getHeapID(), rpImgInfo->mImgBufSize, 1, user);
    if  ( ! camera_memory )
    {
...
~PrvCbImgBuf()
{
    ...
    if(mIonDevFd >= 0)
    {
        if(mIonBufFd >= 0)
        {
            //add ion_share_close, ion_free, ion_close, mCamMem.release log
            //to debug buf release time too long issue
            CAM_LOGD("[PrvCbImgBuf::~PrvCbImgBuf]ion_share_close");
            ion_share_close(
                mIonDevFd,
                mIonBufFd);
        }
        //
        CAM_LOGD("[PrvCbImgBuf::~PrvCbImgBuf]ion_free");
        ion_free(
            mIonDevFd,
            mpIonHandle);
        //
        CAM_LOGD("[PrvCbImgBuf::~PrvCbImgBuf]ion_close");
        ion_close(mIonDevFd);
    }
#endif
    if  ( mCamMem.release )
    {
        CAM_LOGD("[PrvCbImgBuf::~PrvCbImgBuf]mCamMem.release+");
