...
handleReturnBuffers(Vector<ImgBufQueNode>const& rvQueNode)
{
    ...
    for (int32_t i = 0; i < queSize; i++)
    {
        ...
        //
        //  (.4)  Perform callback.
        if  ( (!mbApplyFrameControl && i == idxToCallback) || (mbApplyFrameControl && rvQueNode[i].isDONE())) {
            //------------------------------------------------------------
            if(mbApplyFrameControl)
            {
            ...
            {
                mpController->bufferControl(rvQueNode[i].getImgBuf()->getTimestamp());
            }
            //
            MY_LOG_ID(
                (2<=miLogLevel),
                "callback:(%d/%d)(%d) [%p/%d %#x] CookieDE:%#x",
                i, queSize, rQueNode.getStatus(), pListImgBuf->getVirAddr(),
                pListImgBuf->getBufSize(), pListImgBuf->getTimestamp(),
                rQueNode.getCookieDE()
            );
            MY_LOGD_IF((2<=miLogLevel),"PrvCB Enque+");
