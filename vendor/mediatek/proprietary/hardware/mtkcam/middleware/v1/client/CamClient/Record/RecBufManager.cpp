...
)
{
    ...
    //
    int32_t logLevel = ::property_get_int32("debug.camera.log.RecImgBuf", 0);
    //
    logLevel = ::property_get_int32("debug.camera.log.RecImgBuf", 0);
    //
    if(rpImgInfo->mbMetaMode)
    {
        //(1) create gralloc buffer
        if(logLevel>=1) CAM_LOGD("[RecImgBuf::alloc][GraphicBuffer] Size(%d x %d), F(0x%X), BU(0x%X)",
    ...
    #if REC_BUF_ION
    //
    if(logLevel>=1) CAM_LOGD("[RecImgBuf::alloc][mt_ion_open]");
    ...
    //
    if(logLevel>=1) CAM_LOGD("[RecImgBuf::alloc][ion_alloc_mm]IonDevFd(%d), ImgBufSize(%d)",
            IonDevFd,
            ImgBufSize);
    if(ion_alloc_mm(
        IonDevFd,
    ...
    //
    if(logLevel>=1) CAM_LOGD("[RecImgBuf::alloc][ion_share]");
    if(ion_share(
        IonDevFd,
        IonHandle,
        &IonBufFd))
    ...
    //
    if(logLevel>=1) CAM_LOGD("[RecImgBuf::alloc][requestMemory] fd(%d), S(%d)", IonBufFd, ImgBufSize);
    ...
    #else
    //
    if(logLevel>=1) CAM_LOGD("[RecImgBuf::alloc][MemoryHeapBase]");
    ...
    //
    if(logLevel>=1) CAM_LOGD("[RecImgBuf::alloc][requestMemory]");
    camera_memory = requestMemory(pMemHeapBase->getHeapID(), ImgBufSize, 1, user);
    if  ( ! camera_memory )
    {
        ret = false;
...
lbExit:
        //
    CAM_LOGD("[RecImgBuf::alloc]Size(%d x %d), F(0x%X), BU(0x%X), ImgBufSize(%d), IonDevFd(%d), IonBufFd(%d)",
            rpImgInfo->mu4ImgWidth,
            rpImgInfo->mu4ImgHeight,
            rpImgInfo->mu4ImgFormat,
            rpImgInfo->mu4BufUsage,
