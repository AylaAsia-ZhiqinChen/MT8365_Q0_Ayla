...
prepareAllTodoBuffers(sp<IImgBufQueue>const& rpBufQueue)
{
    bool ret = false;
    //
    MY_LOGD_IF((2<=miLogLevel), "+ mStreamBufList.size(%zu)", mStreamBufList.size());
    //
    while   ( mStreamBufList.size() < (size_t)mi4MaxImgBufCount )
    {
        if  ( ! prepareOneTodoBuffer(rpBufQueue) )
        {
            MY_LOGW("mStreamBufList.size(%zu) < mi4MaxImgBufCount(%d)", mStreamBufList.size(), mi4MaxImgBufCount);
            break;
        }
    }
    //
    ...
    //
    MY_LOGD_IF((2<=miLogLevel), "- ret(%d) mStreamBufList.size(%zu)", ret, mStreamBufList.size());
    return ret;
}
...
cancelAllUnreturnBuffers()
{
    ...
    Mutex::Autolock _l(mModuleMtx);
    //
    if ( ! mOutOrderStreamBufList.empty() )
        mOutOrderStreamBufList.clear();
    ...
    while   ( ! mStreamBufList.empty() )
    {
        ...
        //
        MY_LOGD(
            "Cancel buffer:[ion:%d %p/%zu %" PRIi64 " %dx%d %s] StrongCount:%d",
...
{
    CAM_TRACE_NAME("waitAndHandleReturnBuffers");
    bool ret = false;
    ...
    {
        CAM_TRACE_NAME("waitAndHandleReturnBuffers:deque");
...
handleReturnBuffers(Vector<ImgBufQueNode>const& rvQueNode)
{
    ...
    int32_t idxToDisp = 0;
    CAM_TRACE_NAME("handleReturnBuffers");
    if(!mbApplyFrameControl)
    {
        ...
        if  ( 0 <= idxToDisp )
        {
            ...
            if(mFirstReturnFrame)
            {
                _msDuration_buffer_timestamp = 1;
            }
    ...
    for (int32_t i = 0; i < queSize; i++)
    {
        ...
        if  ( rpQueImgBuf->getVirAddr() != pStreamImgBuf->getVirAddr() )
        {
            ...
            // (..1) Keep the out of order image buffer in another List to make image buffers being returned in sequence
            for (StreamBufList_t::iterator it = mStreamBufList.begin(); it != mStreamBufList.end(); ++it)
            {
                if ( rpQueImgBuf->getVirAddr() == (*it)->getVirAddr() ) {
                    mOutOrderStreamBufList.push_back(*it);
                    break;
                }
            }
        ...
        if  ( (!mbApplyFrameControl && i == idxToDisp) || (mbApplyFrameControl && rvQueNode[i].isDONE())) {
            //------------------------------------------------------------
            if(mbApplyFrameControl)
            {
                nsecs_t const _timestamp1 = rvQueNode[i].getImgBuf()->getTimestamp();
                mProfile_buffer_timestamp.pulse(_timestamp1);
                _msDuration_buffer_timestamp = ::ns2ms(mProfile_buffer_timestamp.getDuration());
                if(mFirstReturnFrame)
                {
            ...
            if(mpDebugScanLine != NULL)
            {
                mpDebugScanLine->drawScanLine(
                                    ...
                                    (void*)(pStreamImgBuf->getVirAddr()),
                                    pStreamImgBuf->getBufSize(),
                                    NSCam::Utils::Format::queryPlaneCount(MtkCameraParameters::queryImageFormat(pStreamImgBuf->getImgFormat().string())) == 1 ? ((pStreamImgBuf->getImgWidthStride(0) * pStreamImgBuf->getBitsPerPixel())>>3) : pStreamImgBuf->getImgWidthStride(0));
            }
            ...
            if(mFirstReturnFrame)
            {
                mFirstReturnFrame = false;
                MY_LOGI("First Displaybuffer Return");
            }
            //
            if(_msDuration_buffer_timestamp > 0)
            {
                MY_LOG_ID(
                    (2<=miLogLevel),
                    "(%lld) %s(%lld), Show frame:(%d/%d) %d [ion:%d %p/%d %lld]",
                    _msDuration_buffer_timestamp,
                    (_msDuration_dequeProcessor > 34) ? "34ms < Duration" : "", _msDuration_dequeProcessor,
                    i,queSize, rvQueNode[i].getStatus(), pStreamImgBuf->getIonFd(),
                    pStreamImgBuf->getVirAddr(), pStreamImgBuf->getBufSize(), pStreamImgBuf->getTimestamp()
                );
                {
            ...
            {
                MY_LOGW(
                    "time inversion!(%lld)", (long long)_msDuration_buffer_timestamp
                );
                MY_LOGW(
    ...
    // (4) Drop out of order image buffers from list and cancelPrvOps in sequence
    while ( !mOutOrderStreamBufList.empty() )
    {
        bool found = false;
        sp<StreamImgBuf>const pStreamImgBuf = *mStreamBufList.begin();  //  ImgBuf in List.
        for (StreamBufList_t::iterator it = mOutOrderStreamBufList.begin(); it != mOutOrderStreamBufList.end(); ++it)
        {
            if ( pStreamImgBuf->getVirAddr() == (*it)->getVirAddr() ) {
                MY_LOGW(
                    "Drop frame:[ion:%d %p/%d %lld]",
                    pStreamImgBuf->getIonFd(), pStreamImgBuf->getVirAddr(),
                    pStreamImgBuf->getBufSize(), pStreamImgBuf->getTimestamp());
                mOutOrderStreamBufList.erase(it);
                mStreamBufList.erase(mStreamBufList.begin());
