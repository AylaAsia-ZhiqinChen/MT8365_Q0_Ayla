...
#define PRVCB_WAIT_BUF_TIMEOUT      30
/******************************************************************************
*
*******************************************************************************/
...
PreviewClient(sp<IParamsManager> pParamsMgr)
    ...
    , mpController(new FrameController(String8("prvcb")))
{
    MY_LOGD("+ this(%p)", this);
    char cLogLevel[PROPERTY_VALUE_MAX] = {'\0'};
    ::property_get("vendor.debug.camera.prvcb.log", cLogLevel, "1");
    miLogLevel = ::atoi(cLogLevel);
    #if MTK_CAM_DISPAY_FRAME_CONTROL_ON
    mbApplyFrameControl=true;
    #endif
    int iFrameCtrol;
    iFrameCtrol = property_get_int32("debug.camera.prvcb.framecontrol", -1);
    if(iFrameCtrol != -1 )
    {
...
uninit()
{
    ...
    if  ( 0 != mi4CallbackRefCount )
    {
        ...
        MY_LOGW(
            "Preview Callback: ref count(%d)!=0, the last callback before %" PRId64 " ms, timestamp:(the last, current)=(%" PRId64 " ms, %" PRId64 " ms)",
...
{
    {
        ...
        {
            int32_t sensor_orientation = 0, rotation = 0;
            //
...
PreviewClient::
sendCommand(
    int32_t cmd __attribute__((__unused__)),
    int32_t arg1 __attribute__((__unused__)),
    int32_t arg2 __attribute__((__unused__))
)
{
