...
)
{
    ...
    //
    int dispFmtProp = property_get_int32("debug.camera.disp.fmt", -1);
    bool isForGpuUsageflow = false;
    //
    //  (2) Check
...
//            "and then call cleanupQueue()"
    ...
    //
    MY_LOGD("NATIVE_WINDOW_CONSUMER_USAGE_BITS:(%d)", mNativeWindowConsumerUsage);
    if(mNativeWindowConsumerUsage == GRALLOC_USAGE_HW_TEXTURE)
    {
        MY_LOGD("the window cunsumer is GRALLOC_USAGE_HW_TEXTURE");
        isForGpuUsageflow = true;
        mi4Orientation = 0;
    }
    else
    {
        MY_LOGD("the window cunsumer is not GRALLOC_USAGE_HW_TEXTURE");
        isForGpuUsageflow = false;
    }
    //
    // Decide to run default buffer format flow or auto decide buffer format flow
    // Decide condition piority:
    //     pior(1) adb command (debug.camera.disp.fmt > -1)
    //     pior(2) mbEnableAutoDecideBufferFormat
    MY_LOGD("debug.camera.disp.fmt = %d, isForGpuUsageflow(%d), mbEnableAutoDecideBufferFormat(%d)",dispFmtProp,isForGpuUsageflow,mbEnableAutoDecideBufferFormat);
    if(mbEnableAutoDecideBufferFormat || dispFmtProp > -1)
    {
        if(dispFmtProp== -1) //default
        {
            if(isForGpuUsageflow)
            {
                ...
                {
                    mpStreamImgInfo     = new ImgInfo(wndWidth, wndHeight, MtkCameraParameters::PIXEL_FORMAT_RGB_888, HAL_PIXEL_FORMAT_RGB_888, "Camera@Display");
                }
            }
            else
            {
        ...
        else
        {
            if(dispFmtProp==1)  //RGBA888
            {
                ...
                {
                    mpStreamImgInfo     = new ImgInfo(wndWidth, wndHeight, CameraParameters::PIXEL_FORMAT_RGBA8888, HAL_PIXEL_FORMAT_RGBA_8888, "Camera@Display");
                }
            }
            else if(dispFmtProp==2) //
            {
                //static const char PIXEL_FORMAT_RGB_888[] = "rgb-888";
                if(mi4Orientation & HAL_TRANSFORM_ROT_90)
                {
                    // For Display Rotation
                    MY_LOGD("mi4Orientation = %d", mi4Orientation);
            ...
            else if(dispFmtProp==3)
            {
                //static const char PIXEL_FORMAT_YCrCb_420_SP[] = "ycrcb-420-sp";
                if(mi4Orientation & HAL_TRANSFORM_ROT_90)
                {
                    // For Display Rotation
                    MY_LOGD("mi4Orientation = %d", mi4Orientation);
                ...
                {
                    mpStreamImgInfo     = new ImgInfo(wndWidth, wndHeight, MtkCameraParameters::PIXEL_FORMAT_YCrCb_420_SP, HAL_PIXEL_FORMAT_YCrCb_420_SP, "Camera@Display");
                }
            }
            else
            {
    ...
    {
        MY_LOGD("mbEnableAutoDecideBufferFormat == false");
        if(mi4Orientation & HAL_TRANSFORM_ROT_90)
        {
            // For Display Rotation
            MY_LOGD("mi4Orientation = %d", mi4Orientation);
    ...
    //
    if(min_undequeued_buf_count < 2)
    {
            min_undequeued_buf_count = 2;
    }
...
enquePrvOps(sp<StreamImgBuf>const& rpImgBuf)
{
...
#if ('1'==MTKCAM_HAVE_GRALLOC_EXTRA)
    {
        CAM_TRACE_NAME("enquePrvOps:MTKCAM_HAVE_GRALLOC_EXTRA");
        ...
        // send buffer time stamp to gralloc extra
        uint64_t Buffertimestamp = rpImgBuf->getTimestamp();
        gralloc_extra_perform(rpImgBuf->getBufHndl(), GRALLOC_EXTRA_SET_TIMESTAMP, &Buffertimestamp);
    }
#endif
    //
    // post buffer timing control
    if( mbApplyFrameControl && mpController.get() != NULL )
    {
        mpController->bufferControl(rpImgBuf->getTimestamp());
    }
    // min buffer time diff control
    if (mbApplyMinBufferDiffTimeControl)
    {
        if(miLastEnqueTime!=0)
        {
            miCurrentEnqueTime = systemTime();
            int64_t timediff = miCurrentEnqueTime - miLastEnqueTime;
            MY_LOGD_IF((2<=miLogLevel),"last(%lld),current(%lld),diff(%lld)",miLastEnqueTime,miCurrentEnqueTime,timediff);
            if(timediff < miBufferMinDiffTime)
            {
                int64_t sleepTime= (miBufferMinDiffTime - timediff)/1000;
                if(sleepTime >0)
                {
                    MY_LOGI("last(%lld),current(%lld),diff(%lld),sleep(%lld)",miLastEnqueTime,miCurrentEnqueTime,timediff,sleepTime);
                    usleep(sleepTime);
                }
            }
            miLastEnqueTime = systemTime();
        }
        else
        {
            miLastEnqueTime = systemTime();
    ...
    //
    profile.print_overtime(10000, "Buffer control sleep > 10 second");
