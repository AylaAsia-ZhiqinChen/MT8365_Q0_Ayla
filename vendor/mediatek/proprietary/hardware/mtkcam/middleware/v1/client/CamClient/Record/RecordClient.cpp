...
uninit()
{
    ...
    //
    {
        ...
        #else
        {
            ...
            #define WAIT_THREAD_EXIT_CNT    (30)
            //
...
startPreview()
{
    ...
    {
        mRecBufNum = (mpParamsMgr->getInt(CameraParameters::KEY_PREVIEW_FRAME_RATE)/30)*2 + 10;
    }
    else
    {
...
startRecording()
{
    ...
    mLastTimeStamp = 0;
    //
    mpParamsMgr->set(MtkCameraParameters::KEY_VR_BUFFER_COUNT, mVrBufCnt);
    mExtraRecBufNum = mVrBufCnt - 1;
    mRecBufNum = 1;
    //
    if(mpParamsMgr->getVideoStabilization() && (mpParamsMgr->getInt(MtkCameraParameters::KEY_EIS25_MODE) == 1))
    {
        mExtraRecBufNum += (mpParamsMgr->getInt(MtkCameraParameters::KEY_EIS_SUPPORTED_FRAMES));
        MY_LOGD("+ record: WxH=%dx%d, EIS VR: %d, %d", mi4RecWidth, mi4RecHeight, mRecBufNum, mExtraRecBufNum);
    }
    //
    mAllocatedExtraRecBufNum = 0;
    //
    ...
    if(mbMetaMode)
    {
        MY_LOGD("- record: WxH=%dx%d, format(%s), num(%d + %d = %d)",
                mi4RecWidth,
                mi4RecHeight,
...
stopRecording()
{
    ...
    {
        if(count >= (int)(mRecBufNum + mAllocatedExtraRecBufNum))
        {
