...
#define LOG_TAG "MtkCam/HwPipeline/VendorHandler"
//
 ...
 *
 ******************************************************************************/
PipelineDefaultImp::VendorHandler::
VendorHandler(
    ...
    , mParams(pParams)
{
    ...
    mpVendorMgr = plugin::IVendorManager::createInstance(mCommonInfo->mName.string(), mCommonInfo->mOpenId);
    plugin::NSVendorManager::add(mCommonInfo->mOpenId, mpVendorMgr.get());
    //
    mUserId = plugin::CALLER_PIPELINEMODEL_DEFAULT;
#endif
    FUNC_END;
}
...
)
{
    ...
    {
        // set plugin info
        input.fullRaw      = mParams->mStreamSet.mpHalImage_P1_Raw;
        input.resizedRaw   = mParams->mStreamSet.mpHalImage_P1_ResizerRaw;
        input.jpegYuv      = mParams->mStreamSet.mpHalImage_Jpeg_YUV;
        input.thumbnailYuv = mParams->mStreamSet.mpHalImage_Thumbnail_YUV;
        input.jpeg         = mParams->mStreamSet.mpAppImage_Jpeg;
        for ( size_t i = 0; i < mParams->mStreamSet.mvAppYuvImage.size(); ++i ) {
            // app yuv image
            input.vYuv.push_back(mParams->mStreamSet.mvAppYuvImage.editValueAt(i));
        }
        if (mParams->mStreamSet.mpHalImage_FD_YUV.get()) input.vYuv.push_back(mParams->mStreamSet.mpHalImage_FD_YUV);
    }
    //
    // plugin result
    {
        sp<IMetaStreamBuffer> pAppStreamBuffer = result.vAppMetaBuffers.valueFor(mParams->mStreamSet.mpAppMeta_Control->getStreamId());
        sp<IMetaStreamBuffer> pHalStreamBuffer = result.vHalMetaBuffers.valueFor(mParams->mStreamSet.mpHalMeta_Control->getStreamId());
        ...
        // update p2 control
        {
            ...
            if (out.inCategory == plugin::FORMAT_RAW && out.outCategory == plugin::FORMAT_YUV) {
                ...
                if ( mParams->mStreamSet.mpHalImage_P1_Raw.get() ) p2_setting |= MTK_P2_CAPTURE_REQUEST;
                if ( mParams->mStreamSet.mpHalImage_P1_ResizerRaw.get() ) p2_setting |= MTK_P2_PREVIEW_REQUEST;
            }
        ...
        for ( int i = 1; i < out.frameCount; ++i ) {
            ...
            sp<HalMetaStreamBuffer> pAppBuffer =
                HalMetaStreamBufferAllocatorT(mParams->mStreamSet.mpAppMeta_Control.get())(appMeta);
            sub.vAppMetaBuffers.add(mParams->mStreamSet.mpAppMeta_Control->getStreamId(), pAppBuffer);
            //
            sub.vHalMetaBuffers.setCapacity(1);
            sp<HalMetaStreamBuffer> pHalBuffer =
                HalMetaStreamBufferAllocatorT(mParams->mStreamSet.mpHalMeta_Control.get())(halMeta);
            sub.vHalMetaBuffers.add(mParams->mStreamSet.mpHalMeta_Control->getStreamId(), pHalBuffer);
            ...
            // iomap
            {
                ...
                if ( mParams->mStreamSet.mpHalImage_P1_Raw.get() ) {
                    p1_Image_IOMap.addOut(mParams->mStreamSet.mpHalImage_P1_Raw->getStreamId());
                    p2_Image_IOMap_Set.add(
                        IOMap().addIn(mParams->mStreamSet.mpHalImage_P1_Raw->getStreamId())
                    );

                    //
                    sub.vHalImageBuffers.add(
                            mParams->mStreamSet.mpHalImage_P1_Raw->getStreamId(),
                            result.vHalImageBuffers.valueFor(mParams->mStreamSet.mpHalImage_P1_Raw->getStreamId())
                        );
                }
                if ( mParams->mStreamSet.mpHalImage_P1_ResizerRaw.get() ) {
                    p1_Image_IOMap.addOut(mParams->mStreamSet.mpHalImage_P1_ResizerRaw->getStreamId());
                    p2_Image_IOMap_Set.add(
                        IOMap().addIn(mParams->mStreamSet.mpHalImage_P1_ResizerRaw->getStreamId())
                    );
                    //
                    sub.vHalImageBuffers.add(
                            mParams->mStreamSet.mpHalImage_P1_ResizerRaw->getStreamId(),
                            result.vHalImageBuffers.valueFor(mParams->mStreamSet.mpHalImage_P1_ResizerRaw->getStreamId())
                        );
                }
                //
                ...
                sub.nodeIOMapMeta.add(
                    eNODEID_P1Node,
                    IOMapSet().add(IOMap()
                        .addIn(mParams->mStreamSet.mpAppMeta_Control->getStreamId())
                        .addIn(mParams->mStreamSet.mpHalMeta_Control->getStreamId())
                        .addOut(mParams->mStreamSet.mpHalMeta_DynamicP1->getStreamId())
                        .addOut(mParams->mStreamSet.mpAppMeta_DynamicP1->getStreamId())
                    )
                );
                sub.nodeIOMapMeta.add(
                    eNODEID_P2Node,
                    IOMapSet().add(IOMap()
                        .addIn(mParams->mStreamSet.mpAppMeta_Control->getStreamId())
                        .addIn(mParams->mStreamSet.mpHalMeta_DynamicP1->getStreamId())
                        .addIn(mParams->mStreamSet.mpAppMeta_DynamicP1->getStreamId())
                        .addOut(mParams->mStreamSet.mpHalMeta_DynamicP2->getStreamId())
                        .addOut(mParams->mStreamSet.mpAppMeta_DynamicP2->getStreamId())
                    )
                );
            }
            //
...
#if MTKCAM_HAVE_IVENDOR_SUPPORT
    {
               ...
               eCaptureIntent.itemAt(0, Type2Type<MUINT8>()) == MTK_CONTROL_CAPTURE_INTENT_STILL_CAPTURE )
        {
            #define HANDLE_EXTRA_YUV(_MODE_) {                                          \
                ...
                for ( size_t i = 0; i < mParams->mStreamSet.mvAppYuvImage.size(); ++i )                     \
                    info.appOut.push_back(mParams->mStreamSet.mvAppYuvImage.editValueAt(i)->getStreamId()); \
                if (mParams->mStreamSet.mpHalImage_FD_YUV.get())                                            \
                    info.appOut.push_back(mParams->mStreamSet.mpHalImage_FD_YUV->getStreamId());            \
            ...
            do {
                ...
                if (mParams->mPipelineConfigParams.mbHasRaw) {
                    MY_LOGD("Disable MFNR due to enable DNG");
                    break;
                }
                // Check flash LED status
                int isFlashOn = 0;
                IHal3A* pHal3a = MAKE_Hal3A(mCommonInfo->mOpenId, LOG_TAG);
                if (pHal3a)
                {
...
#define setFrameInfo( _frameNo_, _data_ )                                             \
    do {                                                                              \
        sp<IMetaStreamBuffer> pAppStreamBuffer =                                      \
            _data_.vAppMetaBuffers.valueFor(mParams->mStreamSet.mpAppMeta_Control->getStreamId());        \
        sp<IMetaStreamBuffer> pHalStreamBuffer =                                      \
            _data_.vHalMetaBuffers.valueFor(mParams->mStreamSet.mpHalMeta_Control->getStreamId());        \
        IMetadata* pAppMetadata = pAppStreamBuffer->tryReadLock(LOG_TAG);             \
        IMetadata* pHalMetadata = pHalStreamBuffer->tryReadLock(LOG_TAG);             \
                                                                                      \
