...
#undef LOG_ERR
#undef LOG_AST
#define LOG_VRB(fmt, arg...)        do { if (mfb_drv_DbgLogEnable_VERBOSE) { BASE_LOG_VRB(fmt, ##arg); } } while(0)
#define LOG_DBG(fmt, arg...)        do { if (mfb_drv_DbgLogEnable_DEBUG  ) { BASE_LOG_DBG(fmt, ##arg); } } while(0)
#define LOG_INF(fmt, arg...)        do { if (mfb_drv_DbgLogEnable_INFO   ) { BASE_LOG_INF(fmt, ##arg); } } while(0)
#define LOG_WRN(fmt, arg...)        do { if (mfb_drv_DbgLogEnable_WARN   ) { BASE_LOG_WRN(fmt, ##arg); } } while(0)
#define LOG_ERR(fmt, arg...)        do { if (mfb_drv_DbgLogEnable_ERROR  ) { BASE_LOG_ERR(fmt, ##arg); } } while(0)
#define LOG_AST(cond, fmt, arg...)  do { if (mfb_drv_DbgLogEnable_ASSERT ) { BASE_LOG_AST(cond, fmt, ##arg); } } while(0)
...
MINT32              MfbDrv::m_Fd = -1;
volatile MINT32     MfbDrvImp::m_UserCnt = 0;
MFB_DRV_RW_MODE     MfbDrv::m_regRWMode = MFB_DRV_R_ONLY;
...
{
    (void)pWaitIrq;
    LOG_ERR("waitIrq Error!!, This is MFB Dummy Driver!!");
...
{
    (void)pClearIrq;
    LOG_ERR("clearIrq Error!!, This is MFB Dummy Driver!!");
...
MBOOL MfbDrvImp::waitMFBFrameDone(unsigned int Status, MINT32 timeoutMs)
{
    (void)Status;
    (void)timeoutMs;
    LOG_ERR("waitFrameDone Error!!, This is MFB Dummy Driver!!");

    return MTRUE;
}

MBOOL MfbDrvImp::enqueMFB(vector<NSCam::NSIoPipe::MFBConfig>& MfbConfigVec)
{
    (void) MfbConfigVec;
    LOG_ERR("enque Error!!, This is MFB Dummy Driver!!");

    return MTRUE;
}

MBOOL MfbDrvImp::dequeMFB(vector<NSCam::NSIoPipe::MFBConfig>& MfbConfigVec)
{
    (void) MfbConfigVec;
    LOG_ERR("deque Error!!, This is MFB Dummy Driver!!");
...
MUINT32 MfbDrvImp::readReg(MUINT32 Addr,MINT32 caller)
{
    (void)Addr;
    (void)caller;
    LOG_ERR("readReg Error!!, This is MFB Dummy Driver!!");
...
MBOOL MfbDrvImp::readRegs(MFB_DRV_REG_IO_STRUCT* pRegIo,MUINT32 Count,MINT32 caller)
{
    ...
    (void)Count;
    (void)caller;
    LOG_ERR("readRegs Error!!, This is MFB Dummy Driver!!");
    return MTRUE;
}
