...
#include "cam_fdvt.h"
//#include "fdvt_learning_data.h"
#include "fdvt_learning_data1.h"
...
void FDVT_AllocMem_LearningData()
{
    if(g_FdDrv_Para->Learning_Type == 1)
    {
        ...
        {
            if(i==0)          {Imem_Size=sizeof(learning1_data0);}
            else if(i==1)     {Imem_Size=sizeof(learning1_data1);}
            else if(i==2)     {Imem_Size=sizeof(learning1_data2);}
            else if(i==3)     {Imem_Size=sizeof(learning1_data3);}
            else if(i==4)     {Imem_Size=sizeof(learning1_data4);}
            else if(i==5)     {Imem_Size=sizeof(learning1_data5);}
            else if(i==6)     {Imem_Size=sizeof(learning1_data6);}
            else if(i==7)     {Imem_Size=sizeof(learning1_data7);}
            ...
            if(i==0)          {memcpy( (MUINT8*)(Imem_Learning_Data[i].virtAddr), (MUINT8*)(learning1_data0), Imem_Size);}
            else if(i==1)     {memcpy( (MUINT8*)(Imem_Learning_Data[i].virtAddr), (MUINT8*)(learning1_data1), Imem_Size);}
            else if(i==2)     {memcpy( (MUINT8*)(Imem_Learning_Data[i].virtAddr), (MUINT8*)(learning1_data2), Imem_Size);}
            else if(i==3)     {memcpy( (MUINT8*)(Imem_Learning_Data[i].virtAddr), (MUINT8*)(learning1_data3), Imem_Size);}
            else if(i==4)     {memcpy( (MUINT8*)(Imem_Learning_Data[i].virtAddr), (MUINT8*)(learning1_data4), Imem_Size);}
            else if(i==5)     {memcpy( (MUINT8*)(Imem_Learning_Data[i].virtAddr), (MUINT8*)(learning1_data5), Imem_Size);}
            else if(i==6)     {memcpy( (MUINT8*)(Imem_Learning_Data[i].virtAddr), (MUINT8*)(learning1_data6), Imem_Size);}
            else if(i==7)     {memcpy( (MUINT8*)(Imem_Learning_Data[i].virtAddr), (MUINT8*)(learning1_data7), Imem_Size);}
            MY_LOGD("learning data (YUV)[%d] PA/VA (0x%lx)/(0x%lx) \n",i,(unsigned long)Imem_Learning_Data[i].phyAddr,(unsigned long)Imem_Learning_Data[i].virtAddr);
        }
    }
    else
    {
...
void FDVT_InitialDRAM(MUINT32 learning_type)
{
    ...
    g_FdDrv_Para->GFD_Info.pose = 0x803;
}
...
void FDVT_SetHWRIPPose(FdDrv_input_struct *FdDrv_input)
{
    ...
    if (0) // FB Mode
    {
        if(current_feature==1)
            g_FdDrv_Para->GFD_Info.pose = 0x001;
        else if(current_feature==2)
            g_FdDrv_Para->GFD_Info.pose = 0x002;
        else if(current_feature==3)
            g_FdDrv_Para->GFD_Info.pose = 0x800;
        else if(current_feature==4)
            g_FdDrv_Para->GFD_Info.pose = 0x004;
        else if(current_feature==5)
            g_FdDrv_Para->GFD_Info.pose = 0x400;
        else if(current_feature==6)
            g_FdDrv_Para->GFD_Info.pose = 0x008;
        else if(current_feature==7)
            g_FdDrv_Para->GFD_Info.pose = 0x200;
        else if(current_feature==8)
            g_FdDrv_Para->GFD_Info.pose = 0x010;
        else if(current_feature==9)
            g_FdDrv_Para->GFD_Info.pose = 0x100;
        else if(current_feature==10)
            g_FdDrv_Para->GFD_Info.pose = 0x020;
        else if(current_feature==11)
            g_FdDrv_Para->GFD_Info.pose = 0x080;
        else if(current_feature==12)
            g_FdDrv_Para->GFD_Info.pose = 0x040;
        else
        {
            g_FdDrv_Para->GFD_Info.pose = 0x001;
    ...
    else
    {
        if(current_feature==1)
            ...
            g_FdDrv_Para->GFD_Info.pose = 0x01C;
        else if(current_feature==5)
            g_FdDrv_Para->GFD_Info.pose = 0x700;
        else if(current_feature==6)
            g_FdDrv_Para->GFD_Info.pose = 0x01C;
        else if(current_feature==7)
            g_FdDrv_Para->GFD_Info.pose = 0x700;
            ...
            g_FdDrv_Para->GFD_Info.pose = 0x0E0;
        else if(current_feature==11)
            g_FdDrv_Para->GFD_Info.pose = 0x0E0;
        else if(current_feature==12)
            g_FdDrv_Para->GFD_Info.pose = 0x0E0;
...
void FDVT_DramConfig(FdDrv_input_struct *FdDrv_input)
{
    ...
    MUINT32  srcbuf = 0x0;
    MUINT64  srcbuf_64 = 0x0;
    MUINT32  src_w = 0x0, src_h = 0x0, des_w = 0x0, des_h = 0x0;
    MUINT32  rFactor = 0x0, sFactor = 0x0, iFactor = 0x0;
    //MUINT16  src_h, src_w;
    MUINT8    rsLoop;
    MUINT16   XMAG_YMAG; //rfactor
    //MUINT8    SRCFORMAT;
    MUINT32   SCALEFORMAT_OtherSCALE = 0x10000000;
    //**************************************//
    ...
    for (rsLoop = 0; rsLoop < g_FdDrv_Para->RS_Num; rsLoop++)
    {
        ...
        if (rsLoop==0)
        {
            ...
            *((MUINT32 *)rs_cfg) = srcbuf;
            // IGMA[31:31] SRC_FORMAT[30:28], always enable inverse gamma in first scale
            *((MUINT32 *)rs_cfg+3) = (*((MUINT32 *)rs_cfg+3) & 0x0FFFFFFF) | ((FdDrv_input->source_img_fmt << 28) & 0x70000000) | 0x80000000;
        }
        else
        {
            // Nth rs src addr = (N -1)th rs dst addr
            *((MUINT32 *)rs_cfg+8*rsLoop) = *((MUINT32 *)rs_cfg+8*(rsLoop-1)+1);
            // Always set format Y_ONLY in other scales
            *((MUINT32 *)rs_cfg+8*rsLoop+3) = (*((MUINT32 *)rs_cfg+8*rsLoop+3) & 0x0FFFFFFF) | SCALEFORMAT_OtherSCALE;
        }
        ...
        *((MUINT32 *)fd_cfg+4*(g_FdDrv_Para->RS_Num-rsLoop-1)) = rs_dst_addr;
        ...
        else
        {
            ...
            *((MUINT32 *)rs_cfg+8*rsLoop+2) = (src_w & 0x000007FF) | ((src_h << 12) & 0x007FF000) | (((des_w & 0x0FF) << 24) & 0xFF000000);
            *((MUINT32 *)rs_cfg+8*rsLoop+3) = (*((MUINT32 *)rs_cfg+8*rsLoop+3) & 0xF0000000) | ((des_w >> 8) & 0x007) | ((des_h << 4) & 0x00007FF0) | ((rFactor << 16) & 0x0FFF0000);
            *((MUINT32 *)rs_cfg+8*rsLoop+5) = src_w & 0x0000FFFF; //stride = src_w
            *((MUINT32 *)fd_cfg+4*(g_FdDrv_Para->RS_Num-rsLoop-1)+1) = des_w | (des_h << 10) | (0x3 << 20);
            *((MUINT32 *)fd_cfg+4*(g_FdDrv_Para->RS_Num-rsLoop-1)+2) = sFactor | (iFactor << 15);
            *((MUINT32 *)fd_cfg+4*(g_FdDrv_Para->RS_Num-rsLoop-1)+3) = des_w << 16;
        }
...
static int FDVT_SetFrontFeatureNum(long long *poseBuf, int aryStartIdx,
    unsigned int *regArray, unsigned int *dataArray)
{
    int i = 0, typeNum = 0;
    unsigned int ffNum[8] = {0, 0, 0, 0, 0, 0, 0, 0};
    static long long lastPose = 0;
    unsigned int lsbPose = 0;
    unsigned int msbPose = 0;

    if ((poseBuf==NULL) || (aryStartIdx > FDVT_PARA_NUM-4) || regArray == NULL
        || dataArray == NULL) {
        MY_LOGE("*buf==NULL or *aryStartIdx Error!");
        return 0;
    }

    if (lastPose == *poseBuf) {
        return 0;/*Same Pose, No Need to config again*/
    }

    lsbPose = (unsigned int)*poseBuf;
    msbPose = (unsigned int)((*poseBuf)>>32);

    /*pose 60 bit (0-11: ROP00, 12-23: ROP+50, 24-35: ROP-50,
    36-47: ROP+90, 48-59: ROP-90)*/
    /*ROP0*/
    ffNum[0] = (lsbPose & 0x249);/*1001001001*/
    ffNum[1] = (lsbPose & 0xDB6);/*110110110110*/

    /*ROP50*/
    ffNum[2] = (lsbPose & 0x49249000);/*1001001001001001001000000000000*/
    ffNum[2] = (ffNum[2] | (msbPose & 0x002));/*MSB,10*/
    ffNum[3] = (lsbPose & 0xDB6000);/*110110110110000000000000*/

    /*ROP90*/
    ffNum[4] = (msbPose & 0x2492490);/*10010010010010010010010000*/
    ffNum[5] = (msbPose & 0xDB60);/*1101101101100000*/

    /*ROP50*/
    ffNum[6] = (lsbPose & 0xB6000000);/*10110110000000000000000000000000*/
    ffNum[6] = (ffNum[6] | (msbPose & 0x0D));/*MSB,1101*/

    /*ROP90N*/
    ffNum[7] = (msbPose & 0xDB60000);/*1101101101100000000000000000*/
    ...
    for (i=0; i<8; i++) {
        if (typeNum != 0) {
            if (ffNum[i] != 0) {
                ffNum[i] = (unsigned int)(FD35_FFNUM_MAX/typeNum);
            }
        }
        else
            ffNum[i] = 0x00180018;/*Use Default Num*/

    }

    regArray[aryStartIdx] = 0x0044;
    dataArray[aryStartIdx++] = ((ffNum[1]<<16) | ffNum[0]);

    regArray[aryStartIdx] = 0x0048;
    dataArray[aryStartIdx++] = ((ffNum[3]<<16) | ffNum[2]);

    regArray[aryStartIdx] = 0x004C;
    dataArray[aryStartIdx++] = ((ffNum[5]<<16) | ffNum[4]);

    regArray[aryStartIdx] = 0x0050;
    dataArray[aryStartIdx] = ((ffNum[7]<<16) | ffNum[6]);
/*
    MY_LOGD("fdvtreg_adr[%d]=%x, fdvtreg_value[%d]=%x",
        aryStartIdx, fdvtreg_adr[aryStartIdx], aryStartIdx, fdvtreg_value[aryStartIdx]);
*/
    lastPose = *poseBuf;
    return 4;/*increment*/
}
...
{
    MUINT8 GFD_skip = FdDrv_input->GFD_skip;

    g_FdDrv_Para->GFD_Skip_Num = GFD_skip;
    if(g_GFD_PatternType>g_FdDrv_Para->GFD_Skip_Num)
    {
        g_GFD_PatternType = 0;
    }

    MY_LOGD("FDRegisterConfig In, (HW)skip:%d, \n", g_FdDrv_Para->GFD_Skip_Num);
    ...
    FdReg_Addr[i+4]=0x0160;
    FdReg_Value[i+4]=(MINT32)((g_FdDrv_Para->HW_Used_Width&0x03ff)<< 16   )+
                       (MINT32)((g_FdDrv_Para->HW_Used_Height&0x03ff)<<  0   );

    /* Indicate the address of RS config data in DRAM */
    FdReg_Addr[i+5]=0x000C;
    FdReg_Value[i+5]=(MUINT32)((MUINTPTR)g_FdDrv_Para->RS_Config_PA & 0x00000000ffffffff);

    /* Indicate the address of FD config data in DRAM */
    FdReg_Addr[i+6]=0x0080;
    FdReg_Value[i+6]=(MUINT32)((MUINTPTR)g_FdDrv_Para->FD_Config_PA & 0x00000000ffffffff);

    /* Indicate the address of where to store the FD result*/
    FdReg_Addr[i+7]=0x009C;
    FdReg_Value[i+7]=(MUINT32)((MUINTPTR)g_FdDrv_Para->FD_Result_PA & 0x00000000ffffffff);
    ...
    FdReg_Addr[i+9]=0x0004;
    FdReg_Value[i+9]=0x111;

    /* Indicate the address of learning data */
    FdReg_Addr[i+10]=0x0054;
    FdReg_Value[i+10]=(MUINT32)((MUINTPTR)g_FdDrv_Para->Learning_Data0_PA);
    FdReg_Addr[i+11]=0x0058;
    FdReg_Value[i+11]=(MUINT32)((MUINTPTR)g_FdDrv_Para->Learning_Data1_PA);
    FdReg_Addr[i+12]=0x005C;
    FdReg_Value[i+12]=(MUINT32)((MUINTPTR)g_FdDrv_Para->Learning_Data2_PA);
    FdReg_Addr[i+13]=0x0060;
    FdReg_Value[i+13]=(MUINT32)((MUINTPTR)g_FdDrv_Para->Learning_Data3_PA);
    FdReg_Addr[i+14]=0x0064;
    FdReg_Value[i+14]=(MUINT32)((MUINTPTR)g_FdDrv_Para->Learning_Data4_PA);
    FdReg_Addr[i+15]=0x0068;
    FdReg_Value[i+15]=(MUINT32)((MUINTPTR)g_FdDrv_Para->Learning_Data5_PA);
    FdReg_Addr[i+16]=0x006C;
    FdReg_Value[i+16]=(MUINT32)((MUINTPTR)g_FdDrv_Para->Learning_Data6_PA);
    FdReg_Addr[i+17]=0x0070;
    FdReg_Value[i+17]=(MUINT32)((MUINTPTR)g_FdDrv_Para->Learning_Data7_PA);

    MUINT8 feature_TH = FdDrv_input->feature_threshold;
    FdReg_Addr[i+18]=0x0074;
    FdReg_Value[i+18]= 0x3F000000;
    ...
    FdReg_Addr[i+19]=0x0084;
    FdReg_Value[i+19]=((g_GFD_PatternType<<4)&0x70)|(g_FdDrv_Para->GFD_Skip_Num & 0x07);
    ...
    /* Set FD Loop Num */
    FdReg_Addr[i+20]=0x007C;
    FdReg_Value[i+20]=0x01FF0000 | g_FdDrv_Para->RS_Num;

    /* Set RS Loop Num */
    FdReg_Addr[i+21]=0x0008;
    FdReg_Value[i+21]=0x00000400 | g_FdDrv_Para->RS_Num;

    /* Control the Learning Data Number included */
    FdReg_Addr[i+22]=0x0024;
    if(g_FdDrv_Para->Learning_Type == 1) //1:Black human
    {
        FdReg_Value[i+22]=0x02310231;
    }
    else                                 //0:Non-Black human
    {
        FdReg_Value[i+22]=0x01D10203;
    }

    /* Set register value to kernel space */
    FDVT_IOCTL_ParaSetting(FdReg_Addr,FdReg_Value,i+23,FDVT_MODE_GFD);
...
MINT32 FDVT_FDGetHWResult()
{
    ...
    for(i = 0; i < g_FdDrv_Para->GFD_Total_Num; i++)
    {
        ...
        FD_Result[i].x1 = ((reg_val>>30)&0x003);
        FD_Result[i].y0 = ((reg_val>>20)&0x3ff);
        FD_Result[i].x0 = ((reg_val>>10)&0x3ff);
        FD_Result[i].type = ((reg_val>>9)&0x001);
        FD_Result[i].face_index = ((reg_val>>0)&0x1ff);

        //reg_val = g_FdDrv_Para->FD_Result_VA[4*i+1];
        reg_val = (g_FdDrv_Para->FD_Result_VA[2*i] & 0xffffffff00000000) >> 32;
        //MY_LOGD("reg_val_32 = %x\n", reg_val);
        FD_Result[i].rop_dir = ((reg_val>>30)&0x007);
        FD_Result[i].rip_dir = ((reg_val>>26)&0x00f);
#ifndef FDVT_VERSION_35P
        FD_Result[i].fcv = ((reg_val>>18)&0x0ff);
        //MY_LOGD("VERSION 35, fcv = %d\n", FD_Result[i].fcv);
#endif
        FD_Result[i].y1 = ((reg_val>>8)&0x3ff);
        FD_Result[i].x1 |= (((reg_val>>0)&0xff)<<2);

        //reg_val = g_FdDrv_Para->FD_Result_VA[4*i+2];
        reg_val = (g_FdDrv_Para->FD_Result_VA[2*i+1] & 0x00000000ffffffff);
        //MY_LOGD("reg_val_32 = %x\n", reg_val);
        FD_Result[i].size_index = ((reg_val>>1)&0x01f);
        FD_Result[i].rop_dir = (((reg_val>>0)&0x001)<<2);
#ifdef FDVT_VERSION_35P
        FD_Result[i].fcv = ((reg_val>>6)&0x3ff);
        //MY_LOGD("VERSION 35P, fcv = %d\n", FD_Result[i].fcv);
#endif
...
*/
    ...
    //{
       char szFileName[100];
       sprintf(szFileName, "/sdcard/FDVT_DUMP_Y_640x480_%d.raw", Count);
       MY_LOGI("/sdcard/FDVT_DUMP_Y_640x480_%d.raw", Count);
       ...
       MINT32 i4WriteCnt = fwrite((void *)g_FdDrv_Para->RS_Result_VA, 2, (640*480),pRawFp);
       fflush(pRawFp);
       fclose(pRawFp);
    //}
#endif
...
void FDVT_Enque(FdDrv_input_struct *FdDrv_input)
{
    ...
    for (MUINT32 u4Index = 0; u4Index <= 0x178; u4Index += 0x4)
    {
        FdReg_Addr[u4Index/0x4] = u4Index;
    }
    MUINT32 result = 0;
    FDVT_IOCTL_GetHWResult(FdReg_Addr,FdReg_Value,(0x178/0x4)+1,result);
    for (MUINT32 loop = 0; loop <= (0x178/0x4); loop ++)
    {
