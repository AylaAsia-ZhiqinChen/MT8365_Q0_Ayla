...
void show_Sensors()
{
    ...
    if(pHalSensorList) {
        ...
        for( unsigned int i = 0; i < sensorNum; i++) {
            ...
            int sensorIndex = i;//IMGSENSOR_SENSOR_DUAL2IDX(sensorIndexDual);
...
#ifdef CONFIG_MTK_CAM_SECURE
if(isSecure == 1)
    pHalSensorList->enableSecure(szCallerName);
    else
    pHalSensorList->disableSecure(szCallerName);
#endif
    ...
    for(i = 0; i < sensorNum; i++) {
        ...
        if(sendev & sensorIndexDual) {
...
#ifdef CONFIG_MTK_CAM_SECURE
            PRINT_MSG("Sensor index:%d, scenarioId:%d secureType %d\n", sensorIndex, scenario, sensorStaticInfo.sensorSecureType);
#else
            ...
            configParam =
            {
              ...
              .isContinuous        = 1,
//              .isp_tg_num          = 1,
//              .isp_grab_alignment  = 64,
//              .isp_pixel_mode      = FOUR_PIXEL_MODE,
            };
            if(input_fps != 0) {
            ...
            {
                std::vector<SeninfConfigInfo>::iterator it = sensorDynamicInfo.config_infos.begin();
                for (; it != sensorDynamicInfo.config_infos.end(); ++it) {
                    pHalSensor->sendCommand(sensorIndexDual, SENSOR_CMD_SET_SENINF_CAM_TG_MUX, (MUINTPTR)&it->tg_info, (MUINTPTR)&it->tg_info, 0);
                }
    ...
    while(1) {
        unsigned int meter[5]= {0};
        double d_meter[5]= {0};
        double d_width = width;
        double d_fps = fps;
        if(pHalSensor != NULL)
            pHalSensor->sendCommand(DUAL_CAMERA_MAIN_SENSOR, SENSOR_CMD_DEBUG_GET_SENINF_METER, (MUINTPTR)meter, 0, 0);
        PRINT_MSG("---------------------------------------------------\n");
