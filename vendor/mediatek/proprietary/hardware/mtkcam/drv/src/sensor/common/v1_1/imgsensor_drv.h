...
typedef enum {
    ...
    CMD_SENSOR_SET_HDR_ATR                              = SENSOR_CMD_SET_SENSOR_HDR_ATR,
    CMD_SENSOR_SET_HDR_TRI_GAIN                         = SENSOR_CMD_SET_SENSOR_HDR_TRI_GAIN,
    CMD_SENSOR_SET_HDR_TRI_SHUTTER                      = SENSOR_CMD_SET_SENSOR_HDR_TRI_SHUTTER,
    CMD_SENSOR_SET_SENSOR_LSC_TBL                       = SENSOR_CMD_SET_SENSOR_LSC_TBL,
    ...
    CMD_SENSOR_GET_SENSOR_VC_INFO2                      = SENSOR_CMD_GET_SENSOR_VC_INFO2,
    ...
    CMD_SENSOR_GET_PIXEL_CLOCK_FREQ_BY_SCENARIO         = SENSOR_CMD_GET_PIXEL_CLOCK_FREQ_BY_SCENARIO,
    CMD_SENSOR_GET_FRAME_SYNC_PIXEL_LINE_NUM_BY_SCENARIO= SENSOR_CMD_GET_FRAME_SYNC_PIXEL_LINE_NUM_BY_SCENARIO,
    CMD_SENSOR_GET_BINNING_TYPE                         = SENSOR_CMD_GET_BINNING_TYPE,
...
class ImgSensorDrv {
public:
    static ImgSensorDrv* getInstance(IMGSENSOR_SENSOR_IDX sensorIdx);
    MINT32 init(IMGSENSOR_SENSOR_IDX sensorIdx);
    MINT32 uninit();
    MINT32 open();
    MINT32 close();
    MINT32 setScenario(MSDK_SCENARIO_ID_ENUM scenarioID, MUINT32 framerate, MUINT32 hdrMode, MUINT32 pdafMode);
    SENSORDRV_INFO_STRUCT* getDrvInfo();
    MINT32 sendCommand(MUINT32 cmd, MUINTPTR parg1 = 0, MUINTPTR parg2 = 0, MUINTPTR parg3 = 0);
    MUINT32 getID();
    MUINT8* getName();
    IMAGE_SENSOR_TYPE getType();
    MINT32 searchSensor();

private:
    ImgSensorDrv();
    ~ImgSensorDrv();
    MINT32 featureControl(ACDK_SENSOR_FEATURE_ENUM FeatureId, MUINT8 *pFeaturePara, MUINT32 featureParaLen);
    MINT32 getInfo();

private:
    IMGSENSOR_SENSOR_LIST m_list;
    SENSORDRV_INFO_STRUCT m_drvInfo;
    CUSTOM_CFG           *m_pCustomCfg;
    IMGSENSOR_SENSOR_IDX  m_sensorIdx;
    MUINT32               m_LineTimeInus;
    MUINT32               m_MinFrameLength;
    int                   m_fd;
    volatile int          m_user;
    MUINT32               m_shutter[3];
    mutable Mutex         mLock;
