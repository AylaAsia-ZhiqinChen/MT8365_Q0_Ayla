...
class OweDrv
{
    protected:
        virtual         ~OweDrv(){}
    public:
        static OweDrv*  createInstance();
        virtual void    destroyInstance(void) = 0;
        virtual MBOOL   init(const char* userName="") = 0;
        virtual MBOOL   uninit(const char* userName="") = 0;

        virtual MBOOL   waitIrq(OWE_WAIT_IRQ_STRUCT* pWaitIrq) = 0;
        virtual MBOOL   clearIrq(OWE_CLEAR_IRQ_STRUCT* pClearIrq) = 0;

        virtual MBOOL   readRegs(OWE_DRV_REG_IO_STRUCT*  pRegIo,MUINT32 Count,MINT32 caller=0) = 0;
        virtual MUINT32 readReg(MUINT32 Addr,MINT32 caller=0) = 0;
        virtual MBOOL   writeRegs(OWE_DRV_REG_IO_STRUCT*  pRegIo,MUINT32 Count,MINT32 caller=0) = 0;
        virtual MBOOL   writeReg(MUINT32 Addr,unsigned long Data,MINT32 caller=0) = 0;

        virtual MBOOL   waitOWEFrameDone(unsigned int Status, MINT32 timeoutMs) = 0;
        virtual MBOOL   enqueOCC(vector<NSCam::NSIoPipe::OCCConfig>& OccConfigVec) = 0;
        virtual MBOOL   enqueWMFE(vector<NSCam::NSIoPipe::OWMFEConfig>& WmfeConfigVec) = 0;

        virtual MBOOL   dequeOCC(vector<NSCam::NSIoPipe::OCCConfig>& OccConfigVec) = 0;
        virtual MBOOL   dequeWMFE(vector<NSCam::NSIoPipe::OWMFEConfig>& WmfeConfigVec) = 0;
...
class OweDrvImp : public OweDrv
{
    ...
    public:
        ...
        virtual MBOOL   checkWMFESetting(NSCam::NSIoPipe::OWMFECtrl* pWmfeCtrl);

        virtual MBOOL   waitOWEFrameDone(unsigned int Status, MINT32 timeoutMs);
        virtual MBOOL   enqueOCC(vector<NSCam::NSIoPipe::OCCConfig>& OccConfigVec);
        virtual MBOOL   enqueWMFE(vector<NSCam::NSIoPipe::OWMFEConfig>& WmfeConfigVec);

        virtual MBOOL   dequeOCC(vector<NSCam::NSIoPipe::OCCConfig>& OccConfigVec);
        virtual MBOOL   dequeWMFE(vector<NSCam::NSIoPipe::OWMFEConfig>& WmfeConfigVec);
    ...
    private:
        ...
        static MUINT32*     m_pOweHwRegAddr;

        static android::Mutex   m_OweInitMutex;
        mutable android::Mutex  OweRegMutex;
