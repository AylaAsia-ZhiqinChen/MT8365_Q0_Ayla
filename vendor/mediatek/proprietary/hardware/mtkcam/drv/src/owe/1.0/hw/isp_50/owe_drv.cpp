...
MBOOL OweDrvImp::init(const char* userName)
{
    ...
    else    // 1st time open success.   // Sometimes GDMA will go this path, too. e.g. File Manager -> Phone Storage -> Photo.
    {
            ...
            m_pOweHwRegAddr = (MUINT32 *) mmap(0, OWE_REG_RANGE, (PROT_READ | PROT_WRITE | PROT_NOCACHE), MAP_SHARED, this->m_Fd, OWE_BASE_HW);
...
MBOOL OweDrvImp::checkOCCSetting(NSCam::NSIoPipe::OCCConfig* pOccConfig)
{
#ifndef BYPASS_RULES
    int x;
    ...
    /* REF_PXL */
    if (pOccConfig->occ_imgi_ref_fmt != 0)
    ...
    /* MAJ_PXL */
    if (pOccConfig->occ_imgi_maj_fmt != 0)
...
MUINT32 OweDrvImp::readReg(MUINT32 Addr,MINT32 caller)
{
    ...
    android::Mutex::Autolock lock(this->OweRegMutex);
    //(void)caller;
    if(-1 == FD_CHK()){
        return 1;
    }
    ...
    if(this->m_regRWMode==OWE_DRV_RW_MMAP){
        ...
        value = this->m_pOweHwRegAddr[(Addr>>2)];
    }
    else{
...
MBOOL OweDrvImp::readRegs(OWE_DRV_REG_IO_STRUCT*  pRegIo,MUINT32 Count,MINT32 caller)
{
    ...
    if(this->m_regRWMode == OWE_DRV_RW_MMAP){
        ...
        for (i=0; i<Count; i++)
        {
            ...
            {
                pRegIo[i].Data = this->m_pOweHwRegAddr[(pRegIo[i].Addr>>2)];
            }
        }
    }
    else{
...
MBOOL OweDrvImp::writeReg(MUINT32 Addr,unsigned long Data,MINT32 caller)
{
    ...
    switch(this->m_regRWMode){
        case OWE_DRV_RW_MMAP:
            ...
            this->m_pOweHwRegAddr[(Addr>>2)] = Data;
            break;
        ...
        case OWE_DRV_RW_MMAP:
            ...
            do{
                if(pRegIo[i].Addr >= legal_range){
                    ...
                    this->m_pOweHwRegAddr[(pRegIo[i].Addr>>2)] = pRegIo[i].Data;
            }while(++i<Count);
            break;
        case OWE_DRV_R_ONLY:
            LOG_ERR("OWE Read Only");
