...
MBOOL TsfDrvImp::init(const char* userName)
{
    ...
    LOG_ERR("Init Error!!, This is TSF Dummy Driver!!");
    //
    ...
    LOG_DBG("-,ret: %d. mInitCount:(%d),m_pTsfHwRegAddr(0x%lx)", Result, m_UserCnt, (unsigned long)m_pTsfHwRegAddr);
    return Result;
}
...
MBOOL TsfDrvImp::uninit(const char* userName)
{
    ...
    //
    LOG_INF("-,m_UserCnt(%d),curUser(%s)", m_UserCnt,userName);
    LOG_ERR("Uninit Error!!, This is TSF Dummy Driver!!");
    //
    if (!((userName !=NULL) && (strlen(userName) <= (MAX_USER_NAME_SIZE))))
    {
...
MBOOL TsfDrvImp::waitTSFFrameDone(unsigned int Status, MINT32 timeoutMs)
{
    (void)Status;
    (void)timeoutMs;
    LOG_ERR("waitTSFFrameDone Error!!, This is TSF Dummy Driver!!");
    return MTRUE;
}
...
MUINT32 TsfDrvImp::readReg(MUINT32 Addr,MINT32 caller)
{
    (void)Addr;
    (void)caller;
    LOG_ERR("readReg Error!!, This is TSF Dummy Driver!!");
    return 0;
}
...
MBOOL TsfDrvImp::readRegs(TSF_DRV_REG_IO_STRUCT*  pRegIo,MUINT32 Count,MINT32 caller)
{
    ...
    (void)Count;
    (void)caller;
    LOG_ERR("readRegs Error!!, This is TSF Dummy Driver!!");
    return MTRUE;
}
...
MBOOL TsfDrvImp::writeReg(MUINT32 Addr,unsigned long Data,MINT32 caller)
{
    ...
    (void)Data;
    (void)caller;
    LOG_ERR("writeReg Error!!, This is TSF Dummy Driver!!");
    return MTRUE;
}
...
MBOOL TsfDrvImp::writeRegs(TSF_DRV_REG_IO_STRUCT*  pRegIo,MUINT32 Count,MINT32 caller)
{
    ...
    (void)Count;
    (void)caller;
    LOG_ERR("writeRegs Error!!, This is TSF Dummy Driver!!");
    return MTRUE;
}
...
{
    (void)rTSFConfig;
    LOG_ERR("enqueTSF Error!!, This is TSF Dummy Driver!!");
    return MTRUE;
}
