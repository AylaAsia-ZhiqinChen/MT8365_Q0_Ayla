...
//                    pStream2= NSCam::NSIoPipe::NSPostProc::INormalStream::createInstance("onThreadLoop", NSCam::NSIoPipe::NSPostProc::ENormalStreamTag_Cap,0xFFFF, isV3);
                    ...
                    for(int i=0;i<10;i++)
                    {
                        ...
                        if(isV3)
                        {
                            pIPStream2->deTuningQue(size, pTuningQueBuf);
                            pIspPhyReg = (isp_reg_t *)pTuningQueBuf;
                            applyTuningSetting(0, &pIspPhyReg);
                            enqueParams.mvTuningData.push_back(pTuningQueBuf);
                        }
                        ...
                        {
                            // return tuningque buffer
                            pIPStream2->enTuningQue(pTuningQueBuf);
                        }
...
//                ISP_WRITE_BITS(mpIspDrv,CAM_CTL_SPARE3,INT_MRG, 1, ISP_DRV_USER_ISPF);
                ...
                for(int i=0;i<10;i++)
                {
                    ...
                    if(isV3)
                    {
                        // test detuningQue and enTuningQue
                        for(MUINT32 i=0;i<P2_TUNINGQUE_NUM;i++){
                            pCCStream->deTuningQue(size, pTuningQueBuf[i]);
                            LOG_INF("i(%d),size(0x%x),pTuningQueBuf(0x%x)\n",i,size,pTuningQueBuf[i]);
                        }
                        pIspPhyReg[0] = (isp_reg_t *)pTuningQueBuf[0];
                        applyTuningSetting(0, &pIspPhyReg[0]);
                    ...
                    if(isV3)
                    {
                        // return tuningque buffer
                         for(MUINT32 i=0;i<P2_TUNINGQUE_NUM;i++){
                            pCCStream->enTuningQue(pTuningQueBuf[i]);
                            LOG_INF("i(%d),size(0x%x),pTuningQueBuf(0x%x)\n",i,size,pTuningQueBuf[i]);
                        }
                    }
                    ...
                    char filename[256];
                        ...
                        ret = mpImemDrv->freeVirtBuf(&img2oBuf);
                }
                ...
                for(int i=0;i<10;i++)
                {
                    ...
                    char filename[256];
                        ...
                        outBuffer3->unlockBuf("pass2_loopTest");
                         ret = mpImemDrv->freeVirtBuf(&wdmaoBuf);
                         ret = mpImemDrv->freeVirtBuf(&wrotoBuf);
                }
                    LOG_INF("pass2_loopTest loop end");
                //destroy
                    pCCStream->uninit();
