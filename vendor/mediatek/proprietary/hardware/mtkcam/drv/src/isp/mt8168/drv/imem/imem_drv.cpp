...
MINT32 IMemDrvImp::cacheSyncbyRange(IMEM_CACHECTRL_ENUM ctrl,IMEM_BUF_INFO* pInfo)
{
    ...
    else
    {
        if((ctrl == IMEM_CACHECTRL_ENUM_FLUSH) && (pInfo->size >= 0x3200000))
        {
            ...
            IMEM_DBG("+ Flush all, c/m/va/sz/pa(%d/0x%x/0x%lu/0x%x/0x%lu)",ctrl,pInfo->memID,(unsigned long)(pInfo->virtAddr),pInfo->size,(unsigned long)(pInfo->phyAddr));
            return 0;
        }
...
MINT32 IMemDrvImp::allocSecBuf(IMEM_BUF_INFO* pInfo)
{
    struct ion_sys_data sys_data;
...
MINT32 IMemDrvImp::freeSecBuf(IMEM_BUF_INFO* pInfo)
{
    IMEM_INF("memID[0x%x]/size[0x%x]/VA[0x%x]/PA[0x%x]",
                pInfo->memID, pInfo->size, pInfo->virtAddr, pInfo->phyAddr);

    ion_user_handle_t pIonHandle;

    //a. get handle
    pIonHandle = (ion_user_handle_t)pInfo->memID;
