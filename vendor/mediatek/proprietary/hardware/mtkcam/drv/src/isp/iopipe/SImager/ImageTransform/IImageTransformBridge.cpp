       ...
       )
{
    ...
    return bBypassISP ? getImp()->executeByPassISP(pSrcBuf, pDstBuf_0, pDstBuf_1, rROI, u4Transform_0, u4TimeOutInMs) :
                        getImp()->execute(pSrcBuf, pDstBuf_0, pDstBuf_1, rROI, u4Transform_0, u4TimeOutInMs);
}
        ...
        )
{
    ...
    MINT srcFormat = pSrcBuf ? pSrcBuf->getImgFormat() : eImgFmt_UNKNOWN;
    MBOOL bBypassISP = this->check10BitsFmt(srcFormat);
    if(DpIspStream::queryMultiPortSupport(DpIspStream::ISP_ZSD_STREAM) == 1 && !bBypassISP) {
        MBOOL r = MTRUE;
        r &= getImp()->execute(pSrcBuf, pDstBuf_0, nullptr, rROI_0, u4Transform_0, u4TimeOutInMs, pDstBuf_1 == nullptr);
        if(pDstBuf_1 != nullptr)
            r &= getImp()->execute(pSrcBuf, nullptr, pDstBuf_1, rROI_1, u4Transform_1, u4TimeOutInMs);
        return r;
    }
    else
        return bBypassISP ? getImp()->executeByPassISP(pSrcBuf, pDstBuf_0, pDstBuf_1, rROI_0, rROI_1, u4Transform_0, u4Transform_1, u4TimeOutInMs)
                          : getImp()->execute(pSrcBuf, pDstBuf_0, pDstBuf_1, rROI_0, rROI_1, u4Transform_0, u4Transform_1, u4TimeOutInMs);
}
