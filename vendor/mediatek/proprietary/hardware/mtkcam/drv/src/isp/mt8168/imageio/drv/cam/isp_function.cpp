...
void* glsciSwWorkaroundBufVa;
MUINT32 glsciSwWorkaroundBufSize = 0;
#endif
...
class IspDrvShellImp:public IspDrvShell
{
    ...
    public://commandQ operation
        ...
        // secure method
        virtual MBOOL getSecureCQDescBufInfo(MINT32 p2cq, MUINT32 bq, MUINT32 dup, MUINT32& CQDescHdl, MUINT32& CQDescSize);
        virtual MBOOL getSecureCQVirtBufInfo(MINT32 p2cq, MUINT32 bq, MUINT32 dup, MUINT32& CQVirtHdl, MUINT32& CQVirtSize);
        virtual MBOOL getSecureTpipeBufInfo(MINT32 p2cq, MUINT32 bq, MUINT32 dup, MUINT32& tpipeHdl, MUINT32& tpipeSize);
...
public:
    ...
    IMEM_BUF_INFO  CQDescSecBufInfo[ISP_DVR_MAX_BURST_QUEUE_NUM][ISP_DRV_P2_CQ_NUM][ISP_DRV_P2_CQ_DUPLICATION_NUM];
    IMEM_BUF_INFO  CQVirtSecBufInfo[ISP_DVR_MAX_BURST_QUEUE_NUM][ISP_DRV_P2_CQ_NUM][ISP_DRV_P2_CQ_DUPLICATION_NUM];
    IMEM_BUF_INFO  tpipeSecBufInfo[ISP_DVR_MAX_BURST_QUEUE_NUM][ISP_DRV_P2_CQ_NUM][ISP_DRV_P2_CQ_DUPLICATION_NUM];
};
//
...
int ret = 0;
    ...
    ISP_FUNC_INF("IspDrvShellImp:mInitCount(%d), secTag(0x%x)", mInitCount, secTag);
    //
    if(mInitCount > 0)
    {
    ...
    m_secTag = secTag;
    ISP_FUNC_DBG("m_secTag(%d)", m_secTag);
    ...
    =============================================*/
    {
       ...
       glsciSwWorkaroundBufVa =  lsciSwWorkaroundBufVa;
       glsciSwWorkaroundBufSize = lsciSwWorkaroundBufInfo.size;
#endif
#endif  // __PMEM_ONLY__
    }
    ...
    //for security
    if (pSec) {
        if (pSec->SecMgr_GetSecureCamStatus(CAM_A) > 0) {
            for (i = 0; i < ISP_DRV_P1_PER_CQ_SET_NUM; i++) {
                for (j = 0; j < ISP_DRV_P1_CQ_DUPLICATION_NUM; j++) {
                    SecMgr_SecInfo secinfo;

                    secinfo.module = CAM_A;
                    secinfo.cq = i;
                    secinfo.dupq = j;
                    secinfo.dummy = 0;
                    secinfo.port = 0;
                    ...
                    secinfo.buff_size = sizeof(ISP_DRV_CQ_CMD_DESC_STRUCT)*CAM_MODULE_MAX;
                    secinfo.buff_va = (uint64_t)m_pPhyIspDrv->getCQDescBufVirAddr(i, 0, j);
                    secinfo.memID = m_pPhyIspDrv->mIspCQDescFd;

                    secinfo.sub.type = SECMEM_VIRTUAL_REG_TABLE;
                    secinfo.sub.buff_size = ISP_BASE_RANGE;
                    secinfo.sub.buff_va = (uint64_t)m_pPhyIspDrv->getCQVirBufVirAddr(i, 0, j);
                    secinfo.sub.memID = m_pPhyIspDrv->mIspVirRegFd;

                    ISP_FUNC_DBG("type:desc module:%d cq:%d dupq:%d buf_size:0x%x buf_va:0x%llx memID:0x%x",
                                 secinfo.module,secinfo.cq,secinfo.dupq,secinfo.buff_size,secinfo.buff_va, secinfo.memID);
                    ISP_FUNC_DBG("type:vreg module:%d cq:%d dupq:%d buf_size:0x%x buf_va:0x%llx memID:0x%x",
                    ...
                    if(pSec->SecMgr_P1RegisterShareMem(&secinfo) == MFALSE){
                        ISP_FUNC_ERR("P1RegisterShareMem failed\n");
                        return -1;
                    }
                }
            }
...
//js_test move to _disable.
    ...
    /*============================================
     isp driver
    =============================================*/
    ISP_FUNC_DBG("m_pP1IspDrv(%p),m_pP2IspDrv(%p),m_pPhyIspDrv_bak(%p) ",(unsigned int*)m_pP1IspDrv,(unsigned int*)m_pP2IspDrv,(unsigned int*)m_pPhyIspDrv_bak);
    if (m_pPhyIspDrv_bak) {
        ...
        for(k=0;k<burstQueueNum;k++){
            for(i=0;i<ISP_DRV_P2_CQ_NUM;i++){
                for(j=0;j<ISP_DRV_P2_CQ_DUPLICATION_NUM;j++){
                    ...
                    ISP_FUNC_INF("[Tpipe]i(%d),j(%d)va(0x%08lx),pa(0x%08lx), m_secTag(0x%x)",i,j,(unsigned long)(gpMdpMgrCfgData[k][i][j].ispTpipeCfgInfo.drvinfo.tpipeTableVa),(unsigned long)(gpMdpMgrCfgData[k][i][j].ispTpipeCfgInfo.drvinfo.tpipeTablePa), m_secTag);

                    if (m_secTag)
                    {
                        tpipeSecBufInfo[k][i][j].size = tpipeBufInfo[k][i][j].size;
                        tpipeSecBufInfo[k][i][j].useNoncache = 1; //alloc non-cacheable mem.
                        //
                        if ( m_pIMemDrv->allocSecBuf(&tpipeSecBufInfo[k][i][j]) ) {
                            ISP_FUNC_ERR("ERROR:m_pIMemDrv->allocSecBuf(tpipeBufInfo)");
                        }
                        tpipeSecBufInfo[k][i][j].secHandle = tpipeSecBufInfo[k][i][j].phyAddr;
                        ISP_FUNC_INF("tpipeSecBufInfo[cq(%d)][bq(%d)][dup(%d)].secHandle(0x%08lx), size(0x%08lx)",
											i, k, j, tpipeSecBufInfo[k][i][j].secHandle, tpipeSecBufInfo[k][i][j].size);
                    }
                    #endif
                }
            }
			...
			for(k=0; k < burstQueueNum; k++) {
				for(i=0; i < ISP_DRV_P2_CQ_NUM; i++) {
					for(j=0; j < ISP_DRV_P2_CQ_DUPLICATION_NUM; j++) {
						...
						// CQ Descriptor secure buffer
						CQDescSecBufInfo[k][i][j].size = m_pVirtIspDrv[ISP_DRV_CQ01 + i]->mIspCQDescSize;
						CQDescSecBufInfo[k][i][j].useNoncache = 1;

						if (m_pIMemDrv->allocSecBuf(&CQDescSecBufInfo[k][i][j])) {
							ISP_FUNC_ERR("ERROR:m_pIMemDrv->allocSecBuf(CQDescSecBufInfo)");
						}
						CQDescSecBufInfo[k][i][j].secHandle = CQDescSecBufInfo[k][i][j].phyAddr;
						ISP_FUNC_INF("[CQDescSecBufInfo[cq(%d)][bq(%d)][dip(%d)].secHandle = 0x%x, size = 0x%x", i, k, j,
										CQDescSecBufInfo[k][i][j].secHandle, CQDescSecBufInfo[k][i][j].size);

						// CQ Virtual secure buffer
						CQVirtSecBufInfo[k][i][j].size = ISP_BASE_RANGE;
						CQVirtSecBufInfo[k][i][j].useNoncache = 1;

						if (m_pIMemDrv->allocSecBuf(&CQVirtSecBufInfo[k][i][j])) {
							ISP_FUNC_ERR("ERROR:m_pIMemDrv->allocSecBuf(CQVirtSecBufInfo)");
						}
						CQVirtSecBufInfo[k][i][j].secHandle = CQVirtSecBufInfo[k][i][j].phyAddr;
						ISP_FUNC_INF("[CQVirtSecBufInfo[cq(%d)][bq(%d)][dip(%d)].secHandle = 0x%x, size = 0x%x", i, k, j,
										CQVirtSecBufInfo[k][i][j].secHandle, CQVirtSecBufInfo[k][i][j].size);
					}
				}
			}
		}
        ...
        for(k=0;k<burstQueueNum;k++){
            for(i=0;i<ISP_DRV_P2_CQ_NUM;i++){
                for(j=0;j<ISP_DRV_P2_CQ_DUPLICATION_NUM;j++){
                    ...
                    if (m_secTag)
                    {
                        if ( m_pIMemDrv->freeSecBuf(&CQDescSecBufInfo[k][i][j]) ) {
                            ISP_FUNC_ERR("ERROR:m_pIMemDrv->freeSecBuf(CQDescSecBufInfo)");
                        }
                        CQDescSecBufInfo[k][i][j].size = 0;

                        if ( m_pIMemDrv->freeSecBuf(&CQVirtSecBufInfo[k][i][j]) ) {
                            ISP_FUNC_ERR("ERROR:m_pIMemDrv->freeSecBuf(CQVirtSecBufInfo)");
                        }
                        CQVirtSecBufInfo[k][i][j].size = 0;

                        if ( m_pIMemDrv->freeSecBuf(&tpipeSecBufInfo[k][i][j]) ) {
                            ISP_FUNC_ERR("ERROR:m_pIMemDrv->freeSecBuf(tpipeSecBufInfo)");
                        }
                        tpipeSecBufInfo[k][i][j].size = 0;
                    }
                    #endif  // __PMEM_ONLY__
                }
            }
...
MBOOL
IspDrvShellImp::getSecureCQDescBufInfo(MINT32 p2cq, MUINT32 bq, MUINT32 dup,
                                        MUINT32& CQSecDescHdl, MUINT32& CQSecDescSize)
{
    CQSecDescHdl = CQDescSecBufInfo[bq][p2cq][dup].secHandle;
    CQSecDescSize =CQDescSecBufInfo[bq][p2cq][dup].size;
    ISP_FUNC_DBG("[cq(%d)][bq(%d)][dup(%d)]: CQSecDescHdl(0x%lx): CQSecDescSize(0x%lx)", p2cq, bq, dup,
					(unsigned long)CQSecDescHdl, (unsigned long)CQSecDescSize);
    return true;
}
MBOOL
IspDrvShellImp::getSecureCQVirtBufInfo(MINT32 p2cq, MUINT32 bq, MUINT32 dup,
                                        MUINT32& CQSecVirtHdl, MUINT32& CQSecVirtSize)
{
    CQSecVirtHdl = CQVirtSecBufInfo[bq][p2cq][dup].secHandle;
    CQSecVirtSize = CQVirtSecBufInfo[bq][p2cq][dup].size;
    ISP_FUNC_DBG("[cq(%d)][bq(%d)][dup(%d)]: CQSecVirtHdl(0x%lx): CQSecVirtSize(0x%lx)", p2cq, bq, dup,
					(unsigned long)CQSecVirtHdl, (unsigned long)CQSecVirtSize);
    return true;
}
MBOOL
IspDrvShellImp::getSecureTpipeBufInfo(MINT32 p2cq, MUINT32 bq, MUINT32 dup,
                                        MUINT32& tpipeHdl, MUINT32& tpipeSize)
{
	tpipeHdl = tpipeSecBufInfo[bq][p2cq][dup].secHandle;
	tpipeSize = tpipeSecBufInfo[bq][p2cq][dup].size;
	ISP_FUNC_DBG("[cq(%d)][bq(%d)][dup(%d)]: tpipeHdl(0x%lx): tpipeSize(0x%lx)", p2cq, bq, dup,
					(unsigned long)tpipeHdl, (unsigned long)tpipeSize);
	return true;
}
...
int DMAI_B::_config( void )
{
    ...
    switch(this->id())
    {
        ...
        case ISP_DMA_IMGI:
            ...
            gpMdpMgrCfgData[this->burstQueIdx][p2Cq][this->dupCqIdx].srcSecureTag = this->dma_cfg.secureTag;
...
unsigned int cq_module_id = 0xFF;
    ...
    unsigned int myCq = CAM_ISP_CQ_NONE;
     ...
     {
        case ISP_DMA_IMGI:
            if (this->dma_cfg.secureTag != 0)
				cq_module_id = CAM_DMA_IMGI_SECURE;
            else
    ...
    if (0xFF != cq_module_id) {
        if ((CAM_DMA_IMGI == cq_module_id) || (CAM_DMA_IMGI_SECURE == cq_module_id))
        {
...
int DMAO_B::_write2CQ(void)
{
    ...
    ISP_FUNC_DBG("[id:0X%08lu],CQ(%d)",this->id(),this->CQ);
     ...
     switch(this->id())
     {
        ...
        case ISP_DMA_IMG2O:
            if (this->dma_cfg.secureTag != 0)
				cq_module_id = CAM_DMA_IMG2O_SECURE;
            else
    ...
    if ( CAM_ISP_PASS1_START == (*start) ) {
        ...
        if(pSec){
            if(pSec->SecMgr_GetSecureCamStatus(CAM_A) > 0){
                ...
                secinfo.sub.type = SECMEM_VIRTUAL_REG_TABLE;
                secinfo.sub.buff_size = ISP_BASE_RANGE;
                secinfo.buff_va = (uint64_t)this->m_pIspDrvShell->getPhyIspDrv()->getCQDescBufVirAddr(CAM_ISP_CQ0, 0, secinfo.dupq);;

                ISP_FUNC_DBG("CAM_ISP_CQ0 module:%d cq:%d dupq:%d",
                             secinfo.module,secinfo.cq,secinfo.dupq);
                if(pSec->SecMgr_P1MigrateTable(&secinfo) == MFALSE){
                    ISP_FUNC_ERR("P1Migrate Table failed\n");
                    return -1;
                }
            }
        }
        ...
        if(pSec){
            if(pSec->SecMgr_GetSecureCamStatus(CAM_A) > 0){
                ...
                secinfo.type = SECMEM_CQ_DESCRIPTOR_TABLE;
                secinfo.module = CAM_A;
                secinfo.cq = CAM_ISP_CQ0B;
                secinfo.dummy = 0;
                secinfo.dupq = 0;
                secinfo.type = SECMEM_CQ_DESCRIPTOR_TABLE;
                secinfo.buff_size = sizeof(CQ0B_RTBC_RING_ST);
                secinfo.buff_va = (uint64_t)this->m_pIspDrvShell->getPhyIspDrv()->getCQDescBufVirAddr(CAM_ISP_CQ0B, 0, secinfo.dupq);;
                        ...
                        if (lsc_en) {
                            ...
                            if(pSec){
                                if(pSec->SecMgr_GetSecureCamStatus(CAM_A) > 0){
                                    ...
                                    secinfo.buff_size = glsciSwWorkaroundBufSize;
                                    secinfo.buff_va = (uint64_t)glsciSwWorkaroundBufVa;
                                    if(pSec->SecMgr_P1RegisterShareMem(&secinfo) == MFALSE){
                                        ISP_FUNC_ERR("SECMEM_LSC P1RegisterShareMem failed\n");
                                        return -1;
                                    }
                                    lsci.dma_cfg.memBuf.base_pAddr = secinfo.buff_sec_mva;
                                    if(pSec->SecMgr_P1MigrateTable(&secinfo) == MFALSE){
                                        ISP_FUNC_ERR("SECMEM_LSC P1Migrate Table failed\n");
                                        return -1;
                                    }
                                    ISP_FUNC_DBG("SECMEM_LSC size:0x%x buff sec_mva/va:0x%x_0x%llx memID:0x%x",
                            ...
                            if(this->isTwinMode){
                                ...
                                if(pSec){
                                    if(pSec->SecMgr_GetSecureCamStatus(CAM_A) > 0){
                                        lsci_d.dma_cfg.memBuf.base_pAddr = secinfo.buff_sec_mva;
                                    }
                                }
                                lsci_d.config();
                                //special case for twin_drv, special case: bpci|leci dma en && baseaddr
...
//        this->src_crop.h);
    ...
    if(this->CQ!=ISP_DRV_CQ01 && this->CQ!=ISP_DRV_CQ02 && this->CQ!=ISP_DRV_CQ03){
        ...
        gpMdpMgrCfgData[this->burstQueIdx][p2Cq][this->dupCqIdx].isSecureFra = this->isSecureFra;
        gpMdpMgrCfgData[this->burstQueIdx][p2Cq][this->dupCqIdx].srcSecureTag = this->srcSecureTag;
    }
    ...
    TdriDrvCfg* pSrcImageio)
{
    ...
    MUINT32 cqDescSecHdl = 0, cqVirtSecHdl = 0, tpipeSecHdl = 0;
    MUINT32 cqDescSecSize = 0, cqVirtSecSize = 0, tpipeSecSize = 0;
...
//            }
    ...
    pDst->drvinfo.DesCqPa = (uint64_t)(this->m_pIspDrvShell->getPhyIspDrv()->getCQDescBufPhyAddr(this->CQ,this->burstQueIdx,this->dupCqIdx));
    pDst->drvinfo.DesCqVa = (uint64_t)(this->m_pIspDrvShell->getPhyIspDrv()->getCQDescBufVirAddr(this->CQ,this->burstQueIdx,this->dupCqIdx));
    pDst->drvinfo.VirtRegPa = (uint64_t)(this->m_pIspDrvShell->getPhyIspDrv()->getCQVirBufPhyAddr(this->CQ,this->burstQueIdx,this->dupCqIdx));
    pDst->drvinfo.VirtRegVa = (uint64_t)(this->m_pIspDrvShell->getPhyIspDrv()->getCQVirBufVirAddr(this->CQ,this->burstQueIdx,this->dupCqIdx));
    pDst->drvinfo.dupCqIdx = this->dupCqIdx;
    pDst->drvinfo.p2Cq = (ISP_TPIPE_P2_CQ_ENUM)p2Cq;


    ISP_FUNC_DBG("VirCqPa(0x%llx),dupCqIdx(%d)",pDst->drvinfo.DesCqPa,pDst->drvinfo.dupCqIdx);

    if (this->isSecureFra == 1)
    {
        if (pSrcImageio->img2o.secureTag != 0)
        {
            pDst->drvinfo.dip_img2o_base_addr = pSrcImageio->img2o.base_addr;
        }
        else
        {
            pDst->drvinfo.dip_img2o_base_addr = 0;
        }
        pDst->drvinfo.dip_img2o_size[0] = pSrcImageio->img2o.size;
        pDst->drvinfo.dip_img2o_size[1] = 0;
        pDst->drvinfo.dip_img2o_size[2] = 0;

        if (pSrcImageio->imgi.secureTag != 0)
        {
            pDst->drvinfo.dip_imgi_base_addr = pSrcImageio->imgi.base_addr;
        }
        else
        {
            pDst->drvinfo.dip_imgi_base_addr = 0;
        }
        pDst->drvinfo.dip_imgi_size[0] = pSrcImageio->imgi.size;
        pDst->drvinfo.dip_imgi_size[1] = 0;
        pDst->drvinfo.dip_imgi_size[2] = 0;

        pDst->drvinfo.dip_secure_tag = this->isSecureFra;
        pDst->drvinfo.dip_img2o_secure_tag = pSrcImageio->img2o.secureTag;
        pDst->drvinfo.dip_imgi_secure_tag = pSrcImageio->imgi.secureTag;
        m_pIspDrvShell->getSecureCQDescBufInfo(p2Cq, this->burstQueIdx, this->dupCqIdx, cqDescSecHdl, cqDescSecSize);
        m_pIspDrvShell->getSecureCQVirtBufInfo(p2Cq, this->burstQueIdx, this->dupCqIdx, cqVirtSecHdl, cqVirtSecSize);
        m_pIspDrvShell->getSecureTpipeBufInfo(p2Cq, this->burstQueIdx, this->dupCqIdx, tpipeSecHdl, tpipeSecSize);
        pDst->drvinfo.CQDescSecBufHdl = cqDescSecHdl;
        pDst->drvinfo.CQDescSecBufSize = cqDescSecSize;
        pDst->drvinfo.CQVirtSecBufHdl = cqVirtSecHdl;
        pDst->drvinfo.CQVirtSecBufSize = cqVirtSecSize;
        pDst->drvinfo.tpipeTableSecHdl = tpipeSecHdl;
        pDst->drvinfo.tpipeTableSecSize = tpipeSecSize;
    }
    else
    {
        pDst->drvinfo.dip_secure_tag = 0;
        pDst->drvinfo.CQDescSecBufHdl = 0;
        pDst->drvinfo.CQDescSecBufSize = 0;
        pDst->drvinfo.CQVirtSecBufHdl = 0;
        pDst->drvinfo.CQVirtSecBufSize = 0;
        pDst->drvinfo.tpipeTableSecHdl = 0;
        pDst->drvinfo.tpipeTableSecSize = 0;
    }
    ...
    if ( ISP_PASS1   == this->path || \
        ...
        if ( _imgo_ == rt_dma || \
            ...
            if( (_imgo_ == rt_dma) || (_rrzo_ == rt_dma) ){
                ...
                fbc[0] = readReg(0x0005001C);
                fbc[1] = 0x0;
                vf = readReg(0x00050414);
            }
            else if (_camsv2_imgo_ == rt_dma){ //( (_camsv_imgo_ == rt_dma) || (_camsv2_imgo_))
                fbc[0] = 0x0;
                fbc[1] = readReg(0x0005081C);
                vf = readReg(0x00050C14);
            }
        ...
        if(pSec && ISP_PASS1 == this->path){
            if(pSec->SecMgr_GetSecureCamStatus(CAM_A) > 0){
                SecMgr_SecInfo secinfo;
                if(this->burstQueIdx != 0){
                    ISP_FUNC_ERR("secure cam not support burst");
                    return -1;
                }
                secinfo.type = SECMEM_CQ_DESCRIPTOR_TABLE;
                secinfo.module = CAM_A;
                secinfo.cq = CAM_ISP_CQ0C;
                secinfo.dummy = 0;
                secinfo.dupq = this->dupCqIdx;
                secinfo.buff_size = sizeof(CQ_RTBC_RING_ST);
                secinfo.buff_va = (uint64_t)m_pPhyIspDrv->getCQDescBufVirAddr(CAM_ISP_CQ0C, 0, 0);
                ISP_FUNC_DBG("migrate CQ0C module:%d cq:%d dupq:%d",
                             secinfo.module,secinfo.cq,secinfo.dupq);
                if(pSec->SecMgr_P1MigrateTable(&secinfo) == MFALSE){
                    ISP_FUNC_ERR("P1Migrate Table failed(0x%x)\n",this->dupCqIdx);
                    return -1;
                }
            }
        }
