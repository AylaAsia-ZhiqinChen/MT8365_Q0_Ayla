...
init(char const* szCallerName, MUINT32 secTag)
{
    ...
    {
        LOG_INF("sTag/sensorDev/swScen (%d)/(0x%x)/(%d), secTag(0x%x)",mStreamTag,mOpenedSensor,mSWScen, secTag);
        mpHalPipeWrapper->init(mSWScen,(unsigned long &)(this->mpPostProcPipe),misV3,pixIdP2, secTag);
    }
