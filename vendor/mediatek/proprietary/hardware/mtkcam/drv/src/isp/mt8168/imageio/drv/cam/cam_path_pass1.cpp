...
MINT32 CamPathPass1::updateCQI(void){
    ...
    if(this->CQ == CAM_ISP_CQ0){
        ...
        if (pSec && pSec->SecMgr_GetSecureCamStatus(CAM_A) > 0) {
            ...
            secinfo.dupq = this->DupQIdx;
            secinfo.buff_va = (uint64_t)this->ispTopCtrl.m_pIspDrvShell->getPhyIspDrv()->getCQDescBufVirAddr(CAM_ISP_CQ0, 0, secinfo.dupq);;

            ISP_PATH_DBG("CAM_ISP_CQ0 module:%d cq:%d dupq:%d",
                        secinfo.module,secinfo.cq,secinfo.dupq);
            if(pSec->SecMgr_P1MigrateTable(&secinfo) == MFALSE){
                ISP_PATH_ERR("P1Migrate Table failed(0x%x)\n",this->DupQIdx);
                return -1;
            }
            return 0;
        }
