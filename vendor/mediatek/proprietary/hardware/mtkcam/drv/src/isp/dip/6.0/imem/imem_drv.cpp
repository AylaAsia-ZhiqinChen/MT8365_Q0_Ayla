    ...
    IMEM_BUF_INFO* pInfo)
{
...
#endif
    {
        IMEM_ERR("ion_alloc fail, mIonDrv fd(0x%x); size(0x%x)", mIonDrv, pInfo->size);
        return -1;
    }
	IMEM_DBG("ion_share \n");
    ...
    if (!pInfo->virtAddr)
    {
        IMEM_ERR("Cannot map ion buffer. mIonDrv fd(0x%x)/memID(0x%x)/size(0x%x)/va(0x%lx)",
            mIonDrv, pInfo->memID, pInfo->size, (unsigned long)pInfo->virtAddr);
        return -1;
    }
    ...
    IMEM_BUF_INFO* pInfo)
{
    ...
    //
        ...
        if(ion_import(
            ...
            &pIonHandle))
        {
            IMEM_ERR("ion_import fail, mIonDrv fd(0x%x); memID(0x%x)", mIonDrv, IonBufFd);
            return -1;
        }
        ...
        if(ion_free(
            ...
            pIonHandle))
        {
            IMEM_ERR("ion_free fail. mIonDrv fd(0x%x)", mIonDrv);
            return -1;
        }
    ...
    IMEM_BUF_INFO* pInfo)
{
    ...
    if ( 0 ==  buf_map.count(pInfo->virtAddr) ) {
        ...
        if ( IMEM_MIN_ION_FD > pInfo->memID ) {
            ...
            mm_data.config_buffer_param.eModuleID = M4U_PORT_IMGI_D1;    // 1;
