...
//    cdpPipe.conf_cdrz = 0;
    ...
    DMAImgi.dma_cfg.secureTag = p_parameter->imgi.secureTag;
    ...
    ISP_PATH_DBG("[Secure]DMAImgi.dma_cfg.secureTag(0x%x), p_parameter->imgi.secureTag(0x%x)", DMAImgi.dma_cfg.secureTag, p_parameter->imgi.secureTag);
    //mdpi
    ...
    if ( CAM_CTL_EN_P2_DMA_IMG2O_EN & ispTopCtrl.isp_top_ctl.CAM_CTL_EN_P2_DMA.Raw){
        ...
        DMAImg2o.dma_cfg.secureTag = p_parameter->img2o.secureTag;
        ISP_PATH_DBG("[Secure]DMAImg2o.dma_cfg.secureTag(0x%x), p_parameter->img2o.secureTag(0x%x)", DMAImg2o.dma_cfg.secureTag, p_parameter->img2o.secureTag);
    } else {
        DMAImg2o.bypass = 1;
    }
    ...
    ispMdpPipe.isSecureFra = p_parameter->isSecureFra;
    ispMdpPipe.srcSecureTag = p_parameter->imgi.secureTag;
    ...
    tdriPipe.isSecureFra = p_parameter->isSecureFra;
...
int CamPathPass2::configTpipeData( struct CamPathPass2Parameter* p_parameter )
{
   ...
   // tdriPipe.tdri.top.img3bo_en = pP2DMA->Bits.IMG3BO_EN;
    ...
    /* config dma */
    if (p_parameter->isSecureFra)
    {
        tdriPipe.tdri.imgi.base_addr = p_parameter->imgi.memBuf.base_pAddr;
        tdriPipe.tdri.imgi.size = p_parameter->imgi.memBuf.size;
        tdriPipe.tdri.imgi.secureTag = p_parameter->imgi.secureTag;
    }
...
//            tdriPipe.tdri.tuningFunc.sl2b.sl2b_hrz_comp = p_parameter->pTuningIspReg->CAM_SL2_HRZ.Bits.SL2_HRZ_COMP; // sl2b hrz comp get from sl2 hrz comp
    ...
    if (p_parameter->isSecureFra)
    {
        tdriPipe.tdri.img2o.base_addr = p_parameter->img2o.memBuf.base_pAddr;
        tdriPipe.tdri.img2o.size = p_parameter->img2o.memBuf.size;
        tdriPipe.tdri.img2o.secureTag = p_parameter->img2o.secureTag;
    }
