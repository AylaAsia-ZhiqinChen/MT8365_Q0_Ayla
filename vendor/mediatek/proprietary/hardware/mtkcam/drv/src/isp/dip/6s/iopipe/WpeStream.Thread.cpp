    ...
    MUINT32 callerID)
{
    ...
    {  //erase frame package from list
        ...
        LOG_DBG("updateFUList(%zu)",this->mLWPEwaitDQFrameUnitList.size());
...
dequeueJudgement()
{
    ...
    mdp_err = this->mwarpengine->dequeue(wpeDupIdx, wpeBurstIdx, DupCmdIdx);
    ...
    if(mdp_err == 0)
    {
        ...
        {
            LOG_DBG("WPE(0x%x)_FmvOut(0x%" PRIxPTR "),idx(%d)",frameunit.callerID, this->mLWPEDQFramePackList[index].rParams.mvFrameParams[wpeBurstIdx].mvOut[0].mBuffer->getBufPA(0),index);
        }
        else
        {
