...
//            cfgData.srcCropX,cfgData.srcCropFloatX,cfgData.srcCropY,cfgData.srcCropFloatY,cfgData.srcCropW,cfgData.srcCropH,cfgData.mdpSrcPlaneNum);
    ...
    uintptr_t dstPhyList[PLANE_NUM];
    DpRect pROI;
...
//         if(cfgData.dstDma[index].withCropM && cfgData.dstDma[index].enSrcCrop)
        {
            ...
            LOG_INF("dstSizeList=(0x%8x,0x%8x,0x%8x),dstVirList=(0x%lx,0x%lx,0x%lx),dstPhyList=(0x%8x,0x%8x,0x%8x),plane num(%d)",
            ...
            {
				// for 32/64-bit compatible and also keep dpframework API consistence,
				// here truncate the pointer(32/64bit) to uint32_t
				uint32_t dstPhyList32[PLANE_NUM] = {(uint32_t)dstPhyList[0], (uint32_t)dstPhyList[1], (uint32_t)dstPhyList[2]};
                err2 = pDpStream->queueDstBuffer(index, &dstVirList[0], dstPhyList32, dstSizeList,(cfgData.dstDma[index].Plane + 1 - MDPMGR_PLANE_1));
            }
            if(err2 != DP_STATUS_RETURN_SUCCESS)
            {
    ...
    {
        // Normal world info
        isp2mdpcfg.MET_String = NULL;
        isp2mdpcfg.MET_String_length = 0;

        isp2mdpcfg.secIspData.ispBufs[0].va = cfgData.ispTpipeCfgInfo.drvinfo.DesCqVa;
        isp2mdpcfg.secIspData.ispBufs[0].size = cfgData.ispTpipeCfgInfo.drvinfo.CQDescSecBufSize;
        isp2mdpcfg.secIspData.ispBufs[1].va = cfgData.ispTpipeCfgInfo.drvinfo.VirtRegVa;
        isp2mdpcfg.secIspData.ispBufs[1].size = ISP_BASE_RANGE; //DIP_REG_RANGE;
        ...
        // Secure world info
        isp2mdpcfg.secIspData.CqSecHandle = cfgData.ispTpipeCfgInfo.drvinfo.CQDescSecBufHdl;
        isp2mdpcfg.secIspData.CqSecSize = cfgData.ispTpipeCfgInfo.drvinfo.CQDescSecBufSize;

        /*
        *   Need refactor struct cmdqSecIspMeta, since HW is different version
        */
//        isp2mdpcfg.secIspData.CqDesOft = cfgData.ispTpipeCfgInfo.drvinfo.CQVirtSecBufHdl;
//        isp2mdpcfg.secIspData.CqVirtOft = cfgData.ispTpipeCfgInfo.drvinfo.CQVirtSecBufSize;
        isp2mdpcfg.secIspData.CQVirtSecBufHdl = cfgData.ispTpipeCfgInfo.drvinfo.CQVirtSecBufHdl;
        isp2mdpcfg.secIspData.CQVirtSecBufSize = cfgData.ispTpipeCfgInfo.drvinfo.CQVirtSecBufSize;
        ...
        isp2mdpcfg.secIspData.TpipeOft = 0;//cfgData.ispTpipeCfgInfo.drvinfo.tpipeTableOft;
        ...
        LOG_INF("isp2mdpcfg(secIspData): CqDesVA(0x%lx), CqDesSize(0x%lx), CqVirtVA(0x%lx), CqVirtSize(0x%lx), TpipeVA(0x%lx), TpipeSize(0x%lx),\n\
            CqSecHandle(0x%lx), CqSecSize(0x%x), CQVirtSecBufHdl(0x%x), CQVirtSecBufSize(0x%x), TpipeSecHandle(0x%lx), TpipeSecSize(0x%x), TpipeOft(0x%x)",\
            (unsigned long)isp2mdpcfg.secIspData.ispBufs[0].va, (unsigned long)isp2mdpcfg.secIspData.ispBufs[0].size,
            (unsigned long)isp2mdpcfg.secIspData.ispBufs[1].va, (unsigned long)isp2mdpcfg.secIspData.ispBufs[1].size,
            (unsigned long)isp2mdpcfg.secIspData.ispBufs[2].va, (unsigned long)isp2mdpcfg.secIspData.ispBufs[2].size,
            (unsigned long)isp2mdpcfg.secIspData.CqSecHandle, isp2mdpcfg.secIspData.CqSecSize,
            //isp2mdpcfg.secIspData.CqDesOft, isp2mdpcfg.secIspData.CqVirtOft,
            isp2mdpcfg.secIspData.CQVirtSecBufHdl, isp2mdpcfg.secIspData.CQVirtSecBufSize,
            (unsigned long)isp2mdpcfg.secIspData.TpipeSecHandle, isp2mdpcfg.secIspData.TpipeSecSize, isp2mdpcfg.secIspData.TpipeOft);
#if 0
        LOG_INF("isp2mdpcfg(secIspData): CqDesVA(0x%lx), CqDesSize(0x%lx), CqVirtVA(0x%lx), CqVirtSize(0x%lx), TpipeVA(0x%lx), TpipeSize(0x%lx),\n\
