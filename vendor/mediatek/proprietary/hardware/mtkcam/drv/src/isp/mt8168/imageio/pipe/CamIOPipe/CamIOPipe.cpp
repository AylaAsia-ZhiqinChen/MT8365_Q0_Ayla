...
PIPE_DBG("+, CQ:%d/%d",m_pass1_Raw_CQ, m_pass1_RawD_CQ);
    ...
    for (MUINT32 i = 0 ; i < vOutPorts.size() ; i++ ) {
        ...
        if ( NSImageio::NSIspio::EPortIndex_IMGO == vOutPorts[i]->index ) {
            ...
            if(CAMSV_FMT_SEL_TG_FMT_RAW8 == camsv_fmt || CAMSV_FMT_SEL_TG_FMT_RAW10 == camsv_fmt ||
                CAMSV_FMT_SEL_TG_FMT_RAW12 == camsv_fmt || CAMSV_FMT_SEL_TG_FMT_RAW14 == camsv_fmt ||
                CAMSV_FMT_SEL_TG_FMT_YUV422){
                bCamsv_2pix = MFALSE;
            }
            else{
                bCamsv_2pix = MTRUE;
            ...
            if(CAMSV_FMT_SEL_TG_FMT_RAW8 == camsv2_fmt || CAMSV_FMT_SEL_TG_FMT_RAW10 == camsv2_fmt ||
                CAMSV_FMT_SEL_TG_FMT_RAW12 == camsv2_fmt || CAMSV_FMT_SEL_TG_FMT_RAW14 == camsv2_fmt ||
                CAMSV_FMT_SEL_TG_FMT_YUV422){
                bCamsv2_2pix = MFALSE;
            }
            else{
        ...
        case EPIPECmd_AE_SMOOTH:
        {
            ...
            if(pSec){
                if(pSec->SecMgr_GetSecureCamStatus(CAM_A) > 0){
                    ...
                    secinfo.dupq = mDupCmdQMgr.CmdQMgr_GetDuqQIdx();
                    secinfo.dummy = 0;
                    ...
                    if(mDupCmdQMgr.CmdQMgr_GetBurstQ() != 0){
                        PIPE_ERR("not support burst");
                        return MFALSE;
                    }
                    secinfo.type = SECMEM_CQ_DESCRIPTOR_TABLE;
                    PIPE_DBG("type:CQ0B module:%d cq:%d dupq:%d",
                            secinfo.module,secinfo.cq,secinfo.dupq);
                    if(pSec->SecMgr_P1MigrateTable(&secinfo) == MFALSE){
                        PIPE_ERR("P1Migrate Table failed(0x%x)\n",mDupCmdQMgr.CmdQMgr_GetDuqQIdx());
                        return MFALSE;
                    }
                }
            }
