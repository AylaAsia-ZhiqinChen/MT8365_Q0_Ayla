...
#define ISP_VIR_REG_RANGE  0xA000
...
MUINT32*    IspDrvImp::mpIspHwRegAddr_seninf = NULL;
MUINT32*    IspDrvImp::mpIspHwRegAddr_camsv = NULL;
...
vector<MUINT32>     IspDrv::m_DapcReg;
MBOOL               IspDrv::m_SecOn = MFALSE;
...
//
    ...
    =   {
            ...
            {CAM_DMA_IMGI_SECURE,    0x7234,  6    }, 	// Baseaddress is updated by secure cmdq
            ...
            {CAM_DMA_IMG2O_SECURE,   0x7444,  7    }, // Baseaddress is updated by secure cmdq
            ...
            {CAM_DMA_IMGO_SV,        0x5020C,  7    },
            {CAM_DMA_IMGO_SV2,       0x5022C,  7    },
...
//-----------------------------------------------------------------------------
MUINT32* IspDrv::getCQVirBufPhyAddr(MINT32 cq,MINT32 burstQIdx,MINT32 dupCqIdx)
{
    Mutex::Autolock lock(cqVirDesLock);
    int realCQIdx=getRealCQIndex(cq,burstQIdx,dupCqIdx);
    LOG_DBG("cq(%d),dupCqIdx(%d),realCQIdx(%d)",cq,dupCqIdx,realCQIdx);
    return (MUINT32*)mpIspVirRegAddrPA[realCQIdx];
}
...
#define ISP_TUNINGQUE_SIZE 0xA000
    ...
    {
        mpIspHwRegAddr_camsv = (MUINT32 *) mmap(0, ISP_CAMSV_BASE_RANGE, (PROT_READ | PROT_WRITE | PROT_NOCACHE), MAP_SHARED, mFd, ISP_CAMSV_BASE_HW);            
...
//            mpIspHwRegAddr_seninf = (MUINT32 *) mmap(0, SENINF_BASE_RANGE, (PROT_READ | PROT_WRITE | PROT_NOCACHE), MAP_SHARED, mFd, SENINF_BASE_HW);            
        }
        ...
        {
            LOG_ERR("mpIspHwRegAddr_camsv ISP mmap fail, errno(%d):%s", errno, strerror(errno));
            Result = MFALSE;
            goto EXIT;
        }        
...
MBOOL IspDrvImp::uninit(const char* userName)
{
    ...
    munmap(mpIspHwRegAddr_camsv, ISP_CAMSV_BASE_RANGE); 
...
MBOOL IspDrvImp::setDapcRegVal(ISP_DRV_DAPC_REG_STRUCT* _DapcReg)
{

    ISP_DAPC_REG_STRUCT DAPCReg;
    DAPCReg.CAM_CTL_EN_P1       = _DapcReg->CAM_CTL_EN_P1;
    DAPCReg.CAM_CTL_EN_P1_DMA   = _DapcReg->CAM_CTL_EN_P1_DMA;
    DAPCReg.CAM_CTL_EN_P1_D     = _DapcReg->CAM_CTL_EN_P1_D;
    DAPCReg.CAM_CTL_EN_P1_DMA_D = _DapcReg->CAM_CTL_EN_P1_DMA_D;
    DAPCReg.CAM_CTL_FMT_SEL_P1  = _DapcReg->CAM_CTL_FMT_SEL_P1;
    DAPCReg.CAM_CTL_FMT_SEL_P1_D = _DapcReg->CAM_CTL_FMT_SEL_P1_D;
    DAPCReg.CAM_CTL_SEL_P1      = _DapcReg->CAM_CTL_SEL_P1;
    DAPCReg.CAM_CTL_SEL_P1_D    = _DapcReg->CAM_CTL_SEL_P1_D;
    DAPCReg.CAM_CTL_SEL_GLOBAL  = _DapcReg->CAM_CTL_SEL_GLOBAL;

    if(ioctl(mFd, ISP_SET_DAPC_REG, &DAPCReg) < 0)
    {
        LOG_ERR("ISP_SET_DAPC_REG error\n");
        return MFALSE;
    }
...
MUINT32 IspDrvImp::readReg(MUINT32 Addr,MINT32 caller)
{
    ...
    if(this->m_SecOn){
        for(MUINT32 i=0;i<m_DapcReg.size();i++){
            if(Addr == m_DapcReg.at(i)){
                LOG_WRN("[SecCam ON]Reg Addr:0x%x is not allowed to read\n",Addr);
                return 1;
            }
        }
    }
    if(Addr >= 0x00050000)
    {
        LOG_DBG("map to camsv ba [0x%x]", Addr);
        pISPmmapAddr = mpIspHwRegAddr_camsv;
        Addr -= 0x00050000;
    }    
