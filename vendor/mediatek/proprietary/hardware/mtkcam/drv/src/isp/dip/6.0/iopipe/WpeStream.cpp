...
wpeGlobalListCtrl(WpePackage wpeframepackage, MUINT32 framenumber, MUINT32 IsFirstFrame, MINT32& GlobalListIndex)
{
    ...
    if(wpeframepackage.wpeBurstIdx == 0 && IsFirstFrame == 0)
    {
        ...
        LOG_INF("EQ GlobalListCtrl,WPE WPE Caller(0x%x), GlobalListIndex(%d), idx_DQFrmPackList(%d), Engine ID(%d), Burstidx_Dupidx(%d/%d), FrameNumber(%d)", bufNode.callerID, GlobalListIndex, bufPackage.idxofwaitDQFrmPackList, bufNode.wpeModeIdx, bufNode.wpeBurstIdx, bufNode.wpeDupIdx, framenumber);
    ...
    else
    {
        ...
        LOG_INF("EQ GlobalListCtrl,WPE WPE Caller(0x%x), WPE Engine ID(%d), Burstidx_Dupidx(%d/%d), GlobalListIndex (%d), idxofwaitDQFrmPackList(%d)",
