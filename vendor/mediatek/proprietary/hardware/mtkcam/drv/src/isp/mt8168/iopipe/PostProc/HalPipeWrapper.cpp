...
)
{
    ...
    LOG_INF("mUCnt(%d),mUser(%s), secTag(0x%x)",mInitCount,mSwHwPathMapping[swScenario].iHalPipeUser, secTag);
    ...
    if (0 == callerPostProc || ! ((NSImageioIsp3::NSIspio::IPostProcPipe *)callerPostProc)->init(secTag))
    {
    ...
    // create imem driver
        ...
        if (NULL == mpPostProcPipe || !mpPostProcPipe->init(secTag))
        {
    ...
    {   //enque by vector, v3 and v1 [v3 only support vector enque]
        ...
        for(int q=0;q<bufPackageNum;q++)
        {
            ...
            for(MUINT32 i=0;i< rParams.mvFrameParams[q].mvIn.size() ;i++)
            {
                ...
                ports[cnt].pipePass    = mSwHwPathMapping[swScenario].ePipePass;
                ports[cnt].SecureTag   = rParams.mvFrameParams[q].mvIn[i].mSecureTag;
                ports[cnt].SecHandle   = rParams.mvFrameParams[q].mvIn[i].mSecHandle;
                if(mSwHwPathMapping[swScenario].ePathCQ == EPathCQ_CQ2)     // debug
                    LOG_WRN("using EPathCQ_CQ2, swScenario: %d\n", swScenario);
...
#endif
                LOG_INF("[mvIn (%d)] fmt(%d),w/h(%d/%d),oft(%lu/%d/%d),stride(%d/%d/%d),cap(%d),mem( va(0x%lu,0x%lu,0x%lu)/pa(0x%lu,0x%lu,0x%lu)/size(0x%x,0x%x,0x%x), SecureTag(0x%x), SecHandle(0x%x) )",\
                    ...
                    ports[cnt].u4BufSize[0],ports[cnt].u4BufSize[1],ports[cnt].u4BufSize[2], ports[cnt].SecureTag, ports[cnt].SecHandle);
                vPostProcInPorts.at(inCnt)=&(ports[cnt]);
                cnt++;
                inCnt++;
            }
            ...
            for(MUINT32 i=0;i< rParams.mvFrameParams[q].mvOut.size() ;i++)
            {
                ...
                ports[cnt].SecureTag   = rParams.mvFrameParams[q].mvOut[i].mSecureTag;
...
#endif
                LOG_INF("[mvOut (%d)] fmt(%d),w/h(%d/%d),oft(%lu/%d/%d),stride(%d/%d/%d),cap(%d),mem( va(0x%lu,0x%lu,0x%lu)/pa(0x%lu,0x%lu,0x%lu)/size(0x%x,0x%x,0x%x), SecureTag(0x%x) )",\
                    ...
                    ports[cnt].capbility,(unsigned long)(ports[cnt].u4BufVA[0]),(unsigned long)(ports[cnt].u4BufVA[1]),(unsigned long)(ports[cnt].u4BufVA[2]),\
                    (unsigned long)(ports[cnt].u4BufPA[0]),(unsigned long)(ports[cnt].u4BufPA[1]),(unsigned long)(ports[cnt].u4BufPA[2]),\
                    ports[cnt].u4BufSize[0],ports[cnt].u4BufSize[1],ports[cnt].u4BufSize[2], ports[cnt].SecureTag);
            ...
            pipePackageInfo.SecureFra = rParams.mvFrameParams[q].mSecureFra;
            ...
            if(q==0)
            {
                ...
                mp2CQUserNum[pathCQ]++;  //update current user number
                releaseLock(ELockEnum_CQUser);
                LOG_INF("Enque bufNode[0]: %d", bufNode.idxInBufBatchList);
            }
            else
            {
                BufParamNode bufNode;
                ...
                releaseLock(ELockEnum_BufNodeEnQ);
                releaseLock(ELockEnum_BufBatchEnQ);
                LOG_INF("Enque bufNode[%d]: %d", q, bufNode.idxInBufBatchList);
            }
            if(!ret)
            {
