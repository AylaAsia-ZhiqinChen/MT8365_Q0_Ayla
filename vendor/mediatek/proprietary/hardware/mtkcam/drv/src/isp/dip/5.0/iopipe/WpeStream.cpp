...
init(MUINT32 secTag)
{
    ...
    if(mInitCount==1)
    {
		...
		for(int i=0;i<2;i++)
		{
			ret = this->mwarpengine[i] = NSImageio::NSIspio::IWarpEnginePipe::createInstance();
			if (NULL ==this->mwarpengine[i] || ret == MFALSE)
			{
		...
		this->mwarpengine[0]->init(WPE_MODULE_IDX_WPE_A, secTag);
		if(MFALSE == ret) {
			LOG_ERR("[Error]mwarpengine[0]->init fail(%d)",ret);
			ret = MFALSE;
		}
	    this->mwarpengine[1]->init(WPE_MODULE_IDX_WPE_B, secTag);
		if(MFALSE == ret) {
			LOG_ERR("[Error]mwarpengine[1]->init fail(%d)",ret);
			ret = MFALSE;
		}
...
uninit()
{
    ...
    if(mInitCount==0)
    {
        ...
        subRet = this->mwarpengine[0]->uninit(WPE_MODULE_IDX_WPE_A);
		if(MFALSE == subRet) {
			LOG_ERR("[Error]mwarpengine[0]->uninit fail(%d)",subRet);
			ret = MFALSE;
		}

		subRet = this->mwarpengine[1]->uninit(WPE_MODULE_IDX_WPE_B);
		if(MFALSE == subRet) {
			LOG_ERR("[Error]mwarpengine[1]->uninit fail(%d)",subRet);
			ret = MFALSE;
		}

		for(int i=0;i<2;i++)
		{
			this->mwarpengine[i]->destroyInstance();
		}
...
wpeGlobalListCtrl(WpePackage wpeframepackage, MUINT32 framenumber, MUINT32 IsFirstFrame, MINT32& GlobalListIndex)
{
	...
	if(wpeframepackage.wpeBurstIdx == 0 && IsFirstFrame == 0)
	{
		...
		bufNode.wpeModeIdx = wpeframepackage.wpeModuleIdx;
	...
	else
	{
		...
		if(GlobalListIndex >= mLWPEDQFramePackList.size())
		{
		...
		{
			LOG_ERR("ERR shift(%d/%d/%d)",tempshift, GlobalListIndex, mLWPEDQFramePackList.size());
			releaseWPELock(EWPELockEnum_wDQFramePackList);
			return MFALSE;
		}
...
WPEenque(QParams const& rParams, MUINT32 callerID)
{
    ...
    vector<WPEConfig>::const_iterator iter;
	...
	ISP_WPE_MODULE_IDX hwModule = WPE_MODULE_IDX_WPE_A;
	android::Vector<NSCam::NSIoPipe::ExtraParam>::const_iterator rParams_iter;
	...
	for(int q = 0 ; q < rParams.mvFrameParams.size(); q++)
    {
		NSCam::NSIoPipe::NSWpe::WPEQParams *wpeqpararms;
		NSCam::NSIoPipe::ExtraParam CmdInfo;


		for(rParams_iter = rParams.mvFrameParams[q].mvExtraParam.begin(); rParams_iter < rParams.mvFrameParams[q].mvExtraParam.end() ; rParams_iter++)
   	 	{
        	CmdInfo = (*rParams_iter);
        	switch (CmdInfo.CmdIdx)
        	{
            	case NSCam::NSIoPipe::EPIPE_WPE_INFO_CMD:
					...
					if(wpeqpararms->wpe_mode == 0) //WPE Stans alone mode
					{
            			//EImgFmt_t   eImgFmt = (EImageFormat)(rParams.mvFrameParams[q].mvIn[0].mBuffer->getImgFormat());
						if((EImageFormat)(rParams.mvFrameParams[q].mvIn[0].mBuffer->getImgFormat()) == eImgFmt_NV21 ||
							(EImageFormat)(rParams.mvFrameParams[q].mvIn[0].mBuffer->getImgFormat()) == eImgFmt_NV12)
							framenumber++;
					}
                	break;
    ...
    {
		MBOOL configUVplane;
		//hwModule = WPE_MODULE_IDX_WPE_A;

		if((EImageFormat)(rParams.mvFrameParams[q].mvIn[0].mBuffer->getImgFormat()) == eImgFmt_NV21 ||
									(EImageFormat)(rParams.mvFrameParams[q].mvIn[0].mBuffer->getImgFormat()) == eImgFmt_NV12)
		{
			configUVplane = MTRUE;
			//hwModule = WPE_MODULE_IDX_WPE_B;
			wpepackage.wpeBurstIdx = q;
			wpepackage.m_wpeDupCmdIdx = 1;
			wpepackage.wpeModuleIdx = hwModule;
			ret = this->mwarpengine[hwModule]->configwpe(wpepackage, configUVplane);
			if(ret == MTRUE)
			{
				LOG_INF("m_curEQUserundex(%d), q(%d), m_wpeDupCmdIdx(%d), WAPR Engine ID (%d), GlobalListIndex(%d)", m_curEQUserundex, q, wpepackage.m_wpeDupCmdIdx, hwModule, GlobalListIndex);

				err = this->mwarpengine[hwModule]->start(wpepackage.wpeDupIdx, wpepackage.wpeBurstIdx, wpepackage.m_wpeDupCmdIdx);
				if(err == 0)
				{
...
WPEdeque(MUINT32 callerID, MINT64 i8TimeoutNs)
{
    ...
    LOG_DBG("WPE i8TimeoutNs(%lld)", i8TimeoutNs);
