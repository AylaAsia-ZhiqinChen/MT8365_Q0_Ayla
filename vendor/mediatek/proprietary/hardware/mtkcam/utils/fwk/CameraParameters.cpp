...
#include <string.h>
#include <stdlib.h>
#include <mtkcam/utils/fwk/CameraParameters.h>
...
void CameraParameters::set(const char *key, const char *value)
{
    ...
    if (strchr(key, '=') || strchr(key, ';')) {
        //XXX CAM_ULOGME("Key \"%s\"contains invalid character (= or ;)", key);
        return;
    }

    if (strchr(value, '=') || strchr(value, ';')) {
        //XXX CAM_ULOGME("Value \"%s\"contains invalid character (= or ;)", value);
        return;
    }
                      ...
                      char **endptr = NULL)
{
    ...
    if (*end != delim) {
        CAM_ULOGME("Cannot find delimeter (%c) in str=%s", delim, str);
        return -1;
    }
...
static void parseSizesList(const char *sizesStr, Vector<Size> &sizes)
{
    ...
    while (true) {
        ...
        if (success == -1 || (*sizeStartPtr != ',' && *sizeStartPtr != '\0')) {
            CAM_ULOGME("Picture sizes string \"%s\" contains invalid character.", sizesStr);
            return;
        }
...
{
    CAM_ULOGMD("dump: mMap.size = %zu", mMap.size());
...
void CameraParameters::getSupportedPreviewFormats(Vector<int>& formats) const {
    ...
    if (supportedPreviewFormats == NULL) {
        CAM_ULOGMW("%s: No supported preview formats.", __FUNCTION__);
        return;
    }
