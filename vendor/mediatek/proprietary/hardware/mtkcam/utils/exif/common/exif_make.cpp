...
bool
ExifUtils::
init(unsigned int const gpsEnFlag)
{
...
)
{
    ...
    *(unsigned int*)buf = pexifImgInfo->thumbSize;
    ...
    err = ifdListNodeModify(IFD_TYPE_EXIFIFD, EXIF_TAG_SUBSECTIME, pexifAPP1Info->strSubSecTime);
    if (err != EXIF_NO_ERROR) {
        return err;
    }

    err = ifdListNodeModify(IFD_TYPE_EXIFIFD, EXIF_TAG_SUBSECTIMEORIG, pexifAPP1Info->strSubSecTime);
    if (err != EXIF_NO_ERROR) {
        return err;
    }

    err = ifdListNodeModify(IFD_TYPE_EXIFIFD, EXIF_TAG_SUBSECTIMEDIGI, pexifAPP1Info->strSubSecTime);
    if (err != EXIF_NO_ERROR) {
        return err;
    }
    ...
    err = ifdListNodeModify(IFD_TYPE_EXIFIFD, EXIF_TAG_FOCALLEN35MM, &pexifAPP1Info->focalLength35mm);
    if (err != EXIF_NO_ERROR) {
        return err;
    }
...
)
{
    ...
    // appnSize = data size + 2byte(data size)
