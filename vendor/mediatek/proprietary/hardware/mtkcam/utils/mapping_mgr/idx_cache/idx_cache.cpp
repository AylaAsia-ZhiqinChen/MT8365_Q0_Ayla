...
{
    char *pScenario;
    getPropInt("vendor.debug.idxcache.log", &m_bLogEnable, 0);
...
#define IDXCACHE_LOG(bHit, module, index, info, result) \
    CAM_LOGD_IF(m_bLogEnable, \
        "[IdxCache::From:%s] [Caller: %s] [Dev:%d-Mod:%s(%d)] (Idx %d) (%s) (PF %s, SM %s, Bin %d, P2 %d, FLASH %d, APP %s, FD %d, ZOOM %d, LV %d, CT %d, ISO %d, ISO_GROUP %d)  (Base_Idx %d)",\
        bHit ? "IdxCache":"IdxMgr", caller, m_eSensorDev, strEModule[module], module, index, (*m_pScenarios)[u2ScnIdx], strEIspProfile[info.eIspProfile], strESensorMode[info.eSensorMode], info.eFrontBin, info.eSize, info.eFlash, strEApp[info.eApp], info.eFaceDetection, info.eZoom_Idx, info.eLV_Idx, info.eCT_Idx, info.eISO_Idx[u2IsoGrpIdx], u2IsoGrpIdx, u2IdxBase);

MUINT16 IdxCache::query(EModule_T eMod, IDXCACHE_STAGE eStage, IDXCACHE_VALTYPE eType, CAM_IDX_QRY_COMB& sQry, const char *caller, IDXCACHE_QRYMODE eMode)
{
    MUINT16 u2Idx = 0, u2ScnIdx = 0, u2IsoGrpIdx = 0, u2IdxBase = 0;
    IDXMGR_QUERY_RESULT result = {0, 0, NULL, 0, 0};
    ...
    if (bUseCache)
    {
        if (m_cacheSameKey[eStage] && (m_cacheValid[eStage][eMod] & ((MUINT8)0x1 << idxType)))
        {
            ...
            u2IdxBase = m_cacheIdxBase[eStage][eMod].val[idxType];
            bCacheHit = MTRUE;
    ...
    u2Idx = IdxMgr::createInstance(m_eSensorDev)->query(m_eSensorDev, eMod, sQry, result, caller);
    ...
    //if (bUseCache)
    {
        ...
        m_cacheIdxBase[eStage][eMod].val[idxType] = u2IdxBase = result.base;
    }
...
{
    MUINT16 u2Idx = 0;
    u2Idx = query(eMod, eStage, eType, sQry,__FUNCTION__ ,eMode);
...
MVOID IdxCache::setCacheValid(EModule_T eMod, IDXCACHE_STAGE eStage, MBOOL enabled, IDXCACHE_VALTYPE eType)
{
    if (enabled){
        MUINT8 idxType = s_idxReMapping[eType];
        m_cacheValid[eStage][eMod]  |= ((MUINT8)0x1 << idxType);
    } else {
        m_cacheValid[eStage][eMod] = 0;
    }
}
