...
//
#include <PlugProcessing/PlugProcessing.h>
...
createInstance(MUINT32 const plugId, DEV_ID const devId)
{
    ...
    #define PLUGPROC_INST_DEV(PLUG, DEV, INSTANCE)\
        ...
        {\
            ...
            if (returnPtr == NULL) {\
                CAM_ULOGMI("PlugID:%d DevID:%d create new instance", PLUG, DEV);\
                returnPtr = new INSTANCE((MUINT32)((PLUG << 8) | DEV));\
                staticPtr = returnPtr;\
            }/* else CAM_ULOGMD("use previous instance");*/\
            return returnPtr;\
        }
    //
    ...
    #define PLUGPROC_INST_CASE(PLUG, INSTANCE)\
        ...
        {\
            switch (devId) {\
                ...
                default:\
                    CAM_ULOGMI("Dev-ID(%d) is not defined", devId);\
                    break;\
            };\
        };\
        break;
    ...
    switch ((PLUG_ID)plugId) {
        ...
        default:
            CAM_ULOGMI("Plug-ID(%d) is not defined", plugId);
            break;
    };
    #undef PLUGPROC_INST_DEV
    #undef PLUGPROC_INST_CASE
