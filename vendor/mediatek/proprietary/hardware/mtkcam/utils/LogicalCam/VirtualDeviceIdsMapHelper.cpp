...
VirtualDeviceIdsMapHelper()
{
    ...
    else
    {
        ...
        {
            auto vid_maps_jason = loadedJson[VID_MAPS_TAG];
            for(auto& item : vid_maps_jason.items())
            {
                mVIDsList.insert({item.key(), (uint32_t)item.value().get<int>()});
            }
            // dump
            for(auto&& item : mVIDsList)
            {
                MY_LOGD("VID_MAP: name(%s) vid(%u)", item.first.c_str(), item.second);
            }
            supportVIDList = true;
        }
        else
...
{
    bool ret = false;
    auto iter = mVIDsList.find(deviceName);
    if(iter != mVIDsList.end())
    {
        vid = iter->second;
        ret = true;
    }
