    ...
    virtual MBOOL                   doAllocIon(MyIonInfo& rIonInfo, size_t /*boundaryInBytesToAlloc*/) override
                                    {
                                        ...
                                        if  (CC_UNLIKELY(rIonInfo.sizeInBytes > rBlobIonInfo.sizeInBytes)) {
                                            MY_LOGE("[%s] BLOB heap size:%zu < requested heap size:%zu", __FUNCTION__, rBlobIonInfo.sizeInBytes, rIonInfo.sizeInBytes);
                                            return MFALSE;
                                        }
...
)
{
    ...
    //
    auto ret = pHeap->onCreate(IonImageBufferHeap::OnCreate{
                        .imgSize = rImgParam.imgSize,
                        .imgFormat = rImgParam.imgFormat,
                        .bitstreamSize = rImgParam.bufSize,
...
)
{
    ...
    //  rIonInfo.va
    auto mapVA = [this,prot_flag](char const* szCallerName, MyIonInfo& rIonInfo, int const ionDeviceFd) {
...
public:     ////    IIonImageBufferHeapAllocator Interface.
    ...
    virtual auto    reserveWithoutUndo(
                    ...
                    {
                        CAM_ULOGME("[%s] Not Supported!!! %s: try to reserve size:%zux%zu",
    ...
    virtual auto    create(
                    ...
                    ) -> IIonImageBufferHeap*
                    {
                        ...
                        //MY_LOGD_IF(0, "[%s] %s@ state:(%u) - alloc normally", __FUNCTION__, szUserName, state);
        ...
        virtual auto    threadLoop() -> bool override
                        {
                            ...
                            auto waitForPendingCommand = [this]()->bool{
                                ...
                                while ( ! exitPending() && ! mCmd1 ) {
                                    ...
                                    if(OK != err) {
                                        CAM_ULOGMW("[%s] exitPending:%d err:%d(%s)",
        ...
        auto            add(CommandT cmd) -> int
                        {
                            if  (CC_UNLIKELY(nullptr==cmd)) {
                                CAM_ULOGME("[%s] Bad command", __FUNCTION__);
                                return -1;
                            }
        ...
        auto            waitDone(int cmdNo, nsecs_t const timeout) -> int
                        {
                            ...
                            if  (CC_UNLIKELY(cmdNo<0)) {
                                CAM_ULOGME("Bad commandNo:%d<0", cmdNo);
                                return NO_INIT;
                            }
...
public:
                    ...
                    IonDriverAllocatorImpl()
                    {
                        ...
                        if  ( mReserveThread == nullptr ) {
                            CAM_ULOGME("[%s] Bad mReserveThread", __FUNCTION__);
                        }
                        else {
                            int status = mReserveThread->run("Ion-Reserving");
                            if  ( OK != status ) {
                                CAM_ULOGME("[%s] Fail to run the reserve thread - status:%d(%s)", __FUNCTION__, status, ::strerror(-status));
    ...
    static auto     doQuerySizeByDriver() -> unsigned int
                    {
                        ...
                        if  (CC_UNLIKELY(pIonDevice == nullptr)) {
                            CAM_ULOGME("[%s] fail to makeIonDevice", __FUNCTION__);
                            return false;
                        }
                        ...
                        if  (CC_UNLIKELY(err!=0)) {
                            CAM_ULOGME("[%s] ION_MM_QRY_CACHE_POOL - err:%d(%s)", __FUNCTION__, err, ::strerror(-err));
                            return 0;
                        }
                        return sizeInBytes;
                    }
    ...
    static auto     doReserveByDriver(size_t len, size_t align, unsigned int flags) -> bool
                    {
                        ...
                        if  (CC_UNLIKELY(err!=0)) {
                            CAM_ULOGME("[%s] ION_MM_ACQ_CACHE_POOL - err:%d(%s)", __FUNCTION__, err, ::strerror(-err));
                            return false;
                        }
                        else {
                            if (duration>=1000000) {
                                CAM_ULOGMI("[%s] ion_alloc_camera_pool - duration(ns):%" PRId64 " len:%zu align:%zu flags:%x", __FUNCTION__, duration, len, align, flags);
                            }
                        }
                        return true;
                    }
    ...
    auto            doReserve(bool waitDone, size_t len, size_t align, unsigned int flags) -> MBOOL
                    {
                        ...
                        if  ( ! waitDone ) {
                            ...
                            if  (CC_UNLIKELY(OK!=err)) {
                                CAM_ULOGME("[%s] waitDone fail - err:%d(%s)", __FUNCTION__, err, ::strerror(-err));
                                ret = MFALSE;
                            }
                        }
                        nsecs_t const duration = (::systemTime() - startTime);

                        CAM_ULOGMI("[%s] duration(ns):%" PRId64 " cmdNo:%d waitDone:%d ret:%d len:%zu align:%zu flags:%x",
...
public:     ////    IIonImageBufferHeapAllocator Interface.
    ...
    virtual auto    unreserve(char const* szUserName) -> MBOOL
                    {
                        ...
                        CAM_ULOGMI("[%s] %s: try to unreserve %zux%zu",
                            __FUNCTION__, szUserName,
                            mReserveParam.numberOfBuffers, mReserveParam.bufferSizeInBytes);
                        {
                            Mutex::Autolock _l(mLock);
    ...
    virtual auto    reserve(
                    ...
                    ) -> MBOOL
                    {
                        ...
                        CAM_ULOGMI("[%s] %s: try to reserve %zux%zu -> %zux%zu",
                            __FUNCTION__, szUserName,
                            mReserveParam.numberOfBuffers, mReserveParam.bufferSizeInBytes,
                            param.numberOfBuffers, param.bufferSizeInBytes);
                        {
    ...
    virtual auto    reserveWithoutUndo(
                    ...
                    ) -> MBOOL
                    {
                        ...
                        nsecs_t const duration = (::systemTime() - startTime);
                        CAM_ULOGMI("[%s] %s: try to reserve %zux%zu align:%zu flags:%x - duration(ns):%" PRId64 " ret:%d",
                            __FUNCTION__, szUserName,
                            param.numberOfBuffers, param.bufferSizeInBytes,
    ...
    virtual auto    create(
                    ...
                    ) -> IIonImageBufferHeap*
                    {
                        ...
                        nsecs_t const duration = (::systemTime() - startTime);
                        //MY_LOGD_IF(0, "[%s] %s: IIonImageBufferHeap::create - duration(ns):%" PRId64 "", __FUNCTION__, szUserName, duration);
                        return p;
                    }
