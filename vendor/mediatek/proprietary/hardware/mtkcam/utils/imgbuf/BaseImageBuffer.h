...
public:     ////                    Image Attributes.
    ...
    virtual MINT32                  getColorArrangement()               const   { return mspImgBufHeap->getColorArrangement(); };
    ...
    virtual MVOID                   setColorProfile(eColorProfile profile) { mColorProfile = profile; };
...
public:     ////                    Buffer Attributes.
    ...
    virtual SecType                 getSecType() const { return mSecType; }
...
private:    ////                    Called inside lock.
    ...
    std::unique_ptr<char[], std::function<void(char[])>> toWorldSharedMemory(
            uint32_t secureHandle, size_t size);
    MBOOL openFile(int &fd, sp<IFileCache> &pCache, MBOOL useCache, char const* filepath);
    MBOOL writeFile(int &fd, sp<IFileCache> &pCache, MBOOL useCache, void *pBuf, size_t size, char const* filepath, size_t index);
