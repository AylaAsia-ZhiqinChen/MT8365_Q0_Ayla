 ...
 ******************************************************************************/
namespace {
struct  ManagerImpl : public IManager
                    , public IDebuggee
{
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
//  Implementations.
...
protected:
    ...
    struct Cookie : public IRegistrationCookie
    {
        ...
        android::wp<BaseImageBufferHeap> mHeap;
    ...
    virtual auto    debug(
                    ...
                    ) -> void
                    {
                        ...
                        for (auto const& v : list) {
                            auto s = v.lock();
                            if ( s == nullptr || s->mHeap == nullptr ) {
                                printer.printLine("dead image buffer heap (outside locking)");
                            }
                            else {
                                android::sp<BaseImageBufferHeap> heap = s->mHeap.promote();
                                if  ( heap != nullptr ) {
...
const std::string ManagerImpl::mDebuggeeName{"NSCam::IImageBufferHeapManager"};
};//namespace
...
~BaseImageBufferHeap()
{
    if  ( mRegCookie ) {
        ...
        mRegCookie = nullptr;
        MY_LOGW("[%s] - During dtor, virtual functions shouldn't be called in print()", __FUNCTION__);
    }
    if (mCreator != NULL) {
        delete mCreator;
    }
...
onFirstRef()
{
    mRegCookie = ManagerImpl::get()->attach(this);
...
onCreate(OnCreate const& arg)
{
    ...
    //
    MY_LOGD_IF(0, "[%s] this:%p %dx%d format:%#x secType:%u init:%d cost(ns):%" PRId64 "",
...
setImgDesc(ImageDescId id, MINT64 value, MBOOL overwrite)
{
    static_assert(static_cast<size_t>(eIMAGE_DESC_ID_MAX) < 20U,
        "Too many IDs, we had better review or use a more economical data structure");
...
MBOOL
BaseImageBufferHeap::
initLocked(std::shared_ptr<IIonDevice> const& pDevice)
{
    MBOOL ret = MFALSE;
    //
    mIonDevice = (pDevice != nullptr) ? pDevice : IIonDeviceProvider::get()->makeIonDevice((String8(getMagicName()) + "-" + mCallerName).c_str(), mUseSharedDeviceFd);
...
createImageBuffer(ImgBufCreator* pCreator)
{
    ...
    pImgBuffer = new NSCam::NSImageBuffer::BaseImageBuffer(this, getImgSize(), format, getBitstreamSize(), bufStridesInBytes, 0, getSecType());
...
)
{
    ...
    NSCam::NSImageBuffer::BaseImageBuffer* pImgBuffer = NULL;
    pImgBuffer = new NSCam::NSImageBuffer::BaseImageBuffer(this, imgSize, getImgFormat(), getBitstreamSize(), bufStridesInBytes, offsetInBytes, getSecType());
    //
    if  ( ! pImgBuffer )
    {
        MY_LOGE("Fail to new");
...
)
{
    ...
    //
    NSCam::NSImageBuffer::BaseImageBuffer* pImgBuffer = NULL;
    pImgBuffer = new NSCam::NSImageBuffer::BaseImageBuffer(this, imgSize, imgFormat, getBitstreamSize(), bufStridesInBytes, offsetInBytes, getSecType());
    //
    if  ( ! pImgBuffer )
    {
        MY_LOGE("Fail to new");
...
)
{
    ...
    for (MINT32 i = 0; i < bufCount; i++) {
        vpImageBuffer[i] = createImageBuffer_FromBlobHeap(
            info.startOffset + (info.bufStep * i),
            info.imgFormat,
            MSize(info.imgWidth, info.imgHeight),
            bufStridesInBytes
        );
...
createImageBuffer_SideBySide(MBOOL isRightSide)
{
    ...
    //
    NSCam::NSImageBuffer::BaseImageBuffer* pImgBuffer = NULL;
    pImgBuffer = new NSCam::NSImageBuffer::BaseImageBuffer(this, SBSImgSize, getImgFormat(), getBitstreamSize(), bufStridesInBytes, offset, getSecType());
    if  ( ! pImgBuffer )
    {
        MY_LOGE("Fail to new");
        return NULL;
...
helpMapPhyAddr(char const* szCallerName, HelperParamMapPA& rParam)
{
    ...
    if ( 0 > mBufMap.indexOfKey(memID) )
    {
        ...
        else
        {
            ...
            mm_data.config_buffer_param.eModuleID   = M4U_PORT_CAM_MDP;
