...
#define FUNCTION_LOG_START      META_LOGD("[%s] - E.", __FUNCTION__)
#define FUNCTION_LOG_END        do {if(!ret) META_LOGE("[%s] fail", __FUNCTION__); META_LOGD("[%s] - X. ret=%d", __FUNCTION__, ret);} while(0)
#define FUNCTION_LOG_END_MUM    META_LOGD("[%s] - X.", __FUNCTION__)
#define ASSERT_IF_NOT_EQAL(_In1, _In2) \
        META_ASSERT(_In1 == _In2, "should be %f, but is %f", (float)_In2, (float)_In1)
