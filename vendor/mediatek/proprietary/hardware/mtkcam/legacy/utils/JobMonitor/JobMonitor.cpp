...
registerJob(struct Job job)
{
    ...
    MY_LOGV("size:%d, id:%" PRId64 ", job=( %s, %d, %p, 0x%x, %d)"
...
unregisterJob(MUINT64 jobId, MBOOL printResult)
{
    ...
    if( mvJob.indexOfKey(jobId) >= 0 )
    {
        ...
        if( printResult )
        {
            MY_LOGI("size:%d, id:%" PRId64 ", job=( %s, %d, %p, 0x%x, %d)"
...
threadLoop()
{
    ...
    MY_LOGI("exit monitor thread, remain %d job", jobSize);
    return false;
}
