...
using namespace NSCam::legacy::NSImageBuffer;
...
void
BaseImageBuffer::
onLastStrongRef(__attribute__((unused)) const void* id)
{
...
onCreate()
{
    ...
    for (size_t i = 0; i < mvImgBufInfo.size(); ++i)
    {
        ...
        #if MTKCAM_TEMP_IMAGEBUFFER_FORCE_IGNORE_ERR
        #warning "MTKCAM_TEMP_IMAGEBUFFER_FORCE_IGNORE_ERR is 1"
            //return  MFALSE;
        #else
        //#warning "MTKCAM_TEMP_IMAGEBUFFER_FORCE_IGNORE_ERR is 0"
            return  MFALSE;
        #endif
        }
        switch ( getImgFormat() )
...
lockBuf(char const* szCallerName, MINT usage)
{
    ...
    Mutex::Autolock _l(mLockMtx);
    //
    ret = lockBufLocked(szCallerName, usage);
    //
...
unlockBuf(char const* szCallerName)
{
    ...
    Mutex::Autolock _l(mLockMtx);
    //
    ret = unlockBufLocked(szCallerName);
    //
    if ( getPlaneCount() == mspImgBufHeap->getPlaneCount() )
    {
        for (size_t i = 0; i < mvImgBufInfo.size(); ++i)
...
unlockBufLocked(char const* szCallerName)
{
    ...
    {
        MY_LOGE("%s@ impUnlockBuf() usage:%d", szCallerName, mLockUsage);
        return  MFALSE;
    }
    for (size_t i = 0; i < mvBufHeapInfo.size(); i++)
    {
