...
{
    mEnableDebugLog = (1 == property_get_int32("vendor.paramsmgr.log", 0));
}
...
status_t
ParamsManager::
setParameters(String8 const& s8Params)
{
    updateLogStatus();
    MY_LOGD_IF(mEnableDebugLog, "%d: + AutoWLock", getOpenId());
    MyRWLock::AutoWLock _lock(mRWLock, __FUNCTION__);
    //
    IParamsManager::showParameters(s8Params);
    CameraParameters params(s8Params);
    //
    status_t    status = OK;
...
getMultFrameBlending() const
{
    ...
    if (0 == ::strcmp(pMfb, MtkCameraParameters::KEY_MFB_MODE_AUTO))
    {
        MY_LOGD_IF(mEnableDebugLog, " MFB enable for AUTO");
        mfbMode = 1;
    }
...
isEnabled(char const* key) const
{
    ...
    //
    if ( ! p ) {
        MY_LOGD_IF(mEnableDebugLog, "Key not found: %s", key);
        return false;
    }
    //
