...
enqueProvider(ImgBufQueNode const& rNode)
{
    ...
    MY_LOGV_IF(
        ...
        "+ Que.size(%zu); (CookieED/CookieDE)=(%#x,%#x) %s; Buffer[%s@%p@%zu@%s@(%d)%dx%d@%d@Timestamp(%lld)]",
        mDoneImgBufQue.size(),
        ...
        rNode->getImgWidthStride(), rNode->getImgWidth(), rNode->getImgHeight(), rNode->getBitsPerPixel(), (long long)rNode->getTimestamp()
    );
    //
...
stopProcessor()
{
    ...
    if  ( ! mDoneImgBufQue.empty() ) {
        MY_LOGW("intent to clear Done Que: size(%zu)!=0", mDoneImgBufQue.size());
...
flushProcessor()
{
    ...
    //
    MY_LOGD("TODO Que.size(%zu)", mTodoImgBufQue.size());
    for ( List<ImgBufQueNode>::iterator it = mTodoImgBufQue.begin(); it != mTodoImgBufQue.end(); it++ )
    {
        ...
        MY_LOGD_IF(
            ENABLE_LOG_PER_FRAME,
            "%s: (CookieED/CookieDE)=(%#x/%#x); Buffer[%s@%p@%zu@%s@(%d)%dx%d-%dBit@Timestamp(%lld)]",
            ...
            (*it)->getBitsPerPixel(), (long long)(*it)->getTimestamp()
        );
...
enqueProcessor(ImgBufQueNode const& rNode)
{
    ...
    MY_LOGV_IF(
        ENABLE_LOG_PER_FRAME,
        "+ Que.size(%zu); %s: (CookieED/CookieDE)=(%#x/%#x); Buffer[%s@%p@%zu@%s@(%d)%dx%d-%dBit@Timestamp(%lld)]",
        ...
        rNode->getBitsPerPixel(), (long long)rNode->getTimestamp()
    );
    //
...
dequeProcessor(Vector<ImgBufQueNode>& rvNode)
{
    ...
    while   ( mDoneImgBufQue.empty() && mbIsProcessorRunning )
    {
        ...
        if  ( NO_ERROR != status )
        {
            MY_LOGW("wait status(%d), Que.size(%zu), IsProcessorRunning(%d)", status, mDoneImgBufQue.size(), mbIsProcessorRunning);
        }
    }
    //
    if  ( ! mDoneImgBufQue.empty() )
...
bool
ImgBufQueue::
isProcessorQueueEmpty()
{
    bool ret = false;
    //
    Mutex::Autolock _lock(mDoneImgBufQueMtx);
    //
    ret = mDoneImgBufQue.empty();
    //
    return ret;
}
