...
getParameters()
{
    ...
    if  ( params_string )
    {
        ::strncpy(params_string, params_str8.string(), params_len);
    }
    //
...
sendCommand(int32_t cmd, int32_t arg1, int32_t arg2)
{
    ...
    case CAMERA_CMD_START_SMOOTH_ZOOM:
        status = OK;
        MY_LOGW("CAMERA_CMD_START_SMOOTH_ZOOM"); //on purpose without break
        //on purpose without break
    default:
...
addCamClient(sp<ICamClient> const& client, const char* name)
{
    ...
    for (it = vmpCamClient.begin(); it != vmpCamClient.end(); ++it) {
        if ((*it) == pClient) {
            MY_LOGE("client(%s) was already subscribed", name);
