...
postCommand(Command const& rCmd)
{
    ...
    if  ( ! mCmdQue.empty() )
    {
        ...
        MY_LOGW("que size:%d > 0 with begin cmd::%s", (int)mCmdQue.size(), rBegCmd.name());
    }
    //
...
getCommand(Command& rCmd)
{
    ...
    Mutex::Autolock _lock(mCmdQueMtx);
    //
    MY_LOGD_IF(ENABLE_LOG_PER_FRAME, "+ que size(%d)", (int)mCmdQue.size());
    //
    ...
    while   ( mCmdQue.empty() && ! exitPending() )
    {
        ...
        if  ( NO_ERROR != status )
        {
            MY_LOGW("wait status(%d), que size(%d), exitPending(%d)", status, (int)mCmdQue.size(), exitPending());
        }
    }
    //
    if  ( ! mCmdQue.empty() )
    ...
    //
    MY_LOGD_IF(ENABLE_LOG_PER_FRAME, "- que size(%d), ret(%d)", (int)mCmdQue.size(), ret);
    return  ret;
}
    ...
    while   (1)
    {
        ...
        if  ( ! isEnabledState() || ! pBufQueue->isProcessorRunning())
        {
            ...
            while(1)
            {
                ...
                {
                    MY_LOGI("Still buffer not return %d", waitFrameCnt);
                    waitFrameCnt++;
                    if(waitFrameCnt > (mRecBufNum + mAllocatedExtraRecBufNum))
                    {
                        MY_LOGI("force stop wait buffer return (%d/%d)",
                                waitFrameCnt,
                                mRecBufNum + mAllocatedExtraRecBufNum);
                        break;
                    }
    ...
    {
        {
            ...
            for(i=0; i<size; i++)
            {
                ...
                {
                    MY_LOGW("CNT(%d):Wait STA(%d)/FD(%d)/ADDR(%lu/%lu) to be released",
                            releaseFrameCnt,
                            mvRecBufInfo[i].Sta,
                            (uint32_t)(mvRecBufInfo[i].BufIonFd),
                            (unsigned long)(MUINTPTR)(mvRecBufInfo[i].CbVirAddr),
                            (unsigned long)mvRecBufInfo[i].PhyAddr);
                    releaseFrameCnt++;
                    break;
                }
            }
        ...
        if(releaseFrameCnt > (mRecBufNum + mAllocatedExtraRecBufNum))
        {
