...
void
RecordClient::
initBufferInfo(int32_t index)
{
    Mutex::Autolock _l(mModuleMtx);
    REC_BUF_INFO_STRUCT RecBufInfo;
    RecBufInfo.Sta = REC_BUF_STA_EMPTY;
    RecBufInfo.BufIonFd = mpImgBufMgr->getBuf(index)->getIonFd();
    RecBufInfo.CbVirAddr = (void*)(mpImgBufMgr->getBuf(index)->get_camera_memory_virAddr());
...
prepareAllTodoBuffers(sp<IImgBufQueue>const& rpBufQueue, sp<RecBufManager>const& rpBufMgr)
{
    ...
    for(i = 0; i < mvRecBufInfo.size(); i++)
    {
        if(mvRecBufInfo[i].Sta == REC_BUF_STA_EMPTY)
        {
            ...
            MY_LOGD("EQ:Idx(%d),Addr(%lu/%lu),CbVA(%lu)",
                    i,
                    (unsigned long)(MUINTPTR)(pCameraImgBuf->getVirAddr()),
                    (unsigned long)(MUINTPTR)(pCameraImgBuf->getPhyAddr()),
                    (unsigned long)(MUINTPTR)(rpBufMgr->getBuf(i)->get_camera_memory_virAddr()));
            //
...
bool
RecordClient::
waitAndHandleReturnBuffers(sp<IImgBufQueue>const& rpBufQueue, bool blocking)
{
    bool ret = false;
    uint32_t i,size;
    ...
    if( !blocking &&
        rpBufQueue->isProcessorQueueEmpty())
    {
...
handleReturnBuffers(Vector<ImgBufQueNode>const& rvQueNode)
{
    ...
    for (i = 0; i < (uint32_t)queSize; i++)
    {
        ...
        {
            MY_LOGE("Can't find FD(%d)/VA(%lu)",
                    rpQueImgBuf->getIonFd(),
                    (unsigned long)(MUINTPTR)(rpQueImgBuf->getVirAddr()));
            return  false;
        }
        //
        MY_LOGD_IF(
            0,
            "CB:i(%d/%d),Idx(%d),Sta(%d),Info(%lu/%d.%06d)",
            i,
            queSize-1,
            j,
            rQueNode.getStatus(),
...
performRecordCallback(int32_t bufIdx, sp<RecImgBuf>const& pCameraImgBuf, int32_t const /*msgType*/)
{
    ...
    if  ( pCameraImgBuf != 0 )
    {
        ...
        //
        MY_LOGD("CB:FC(%03d),Idx(%03d),Addr(%p/%p),CbVA(%p),GH(%p),TS(%d.%06d)",
                mFrameCount,
                bufIdx,
...
releaseRecordingFrame(const void *opaque)
{
        ...
        if(mbMetaMode)
        {
                ...
                mvRecBufInfo[i].Sta  == REC_BUF_STA_CB)
            {
                if(mvRecBufInfo[i].CbVirAddr == opaque)
                {
                    ...
                    bFindTarget = true;
                    //
                    MY_LOGD("Same:Idx(%d),Addr(%p/%ld)",
                            i,
                            mvRecBufInfo[i].CbVirAddr,
                            (long)mvRecBufInfo[i].PhyAddr);
                }
                else
                {
                    for(j=0; j<size; j++)
                    {
                            ...
                            mvRecBufInfo[j].Sta  == REC_BUF_STA_CB)
                        {
                            ...
                            bFindTarget = true;
                            //
                            MY_LOGD("Replace:Idx(%d -> %d),VA(0x%p),PA(0x%ld -> 0x%ld)",
                                    i,
                                    j,
                                    mvRecBufInfo[j].CbVirAddr,
                                    (long)mvRecBufInfo[i].PhyAddr,
                                    (long)mvRecBufInfo[j].PhyAddr);
                            //
                            break;
                        }
                    }
    ...
    if(!bFindTarget)
    {
        if(mbMetaMode)
        {
            if(pMetadata != NULL)
            {
                MY_LOGE("Can't find PA(%u)",sys_data.get_phys_param.phy_addr);
                for(i=0; i<size; i++)
                {
                    MY_LOGE("Idx(%d),Addr(%p/0x%x)",
                            i,
                            mvRecBufInfo[i].CbVirAddr,
                            (unsigned int)mvRecBufInfo[i].PhyAddr);
                }
            }
        }
        else
