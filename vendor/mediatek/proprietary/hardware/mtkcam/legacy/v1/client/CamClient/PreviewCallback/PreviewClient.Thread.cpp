...
postCommand(Command const& rCmd)
{
    ...
    if  ( ! mCmdQue.empty() )
    {
        ...
        MY_LOGW("que size:%zu > 0 with begin cmd::%s", mCmdQue.size(), rBegCmd.name());
    }
    //
    if  ( ! mCmdQue.empty() )
    {
        if( mLastCmd == Command::eID_WAKEUP &&
            rCmd.eId == Command::eID_WAKEUP)
        {
            MY_LOGD("The same cmd %d", rCmd.eId);
            return;
        }
    }
    //
    ...
    //
    MY_LOGD("- que size(%zu), new command::%s", mCmdQue.size(), rCmd.name());
}
...
getCommand(Command& rCmd)
{
    ...
    Mutex::Autolock _lock(mCmdQueMtx);
    //
    MY_LOGD_IF((2<=miLogLevel), "+ que size(%zu)", mCmdQue.size());
    //
    ...
    if  ( ! mCmdQue.empty() )
    {
        ...
        {
            MY_LOGD("Preview has stopped, que size(%zu)", mCmdQue.size());
            ret = false;
        }
    }
    //
    MY_LOGD_IF((2<=miLogLevel), "- que size(%zu), ret(%d)", mCmdQue.size(), ret);
    return  ret;
}
...
void
PreviewClient::
onClientThreadLoop(__attribute__((unused)) Command const& rCmd)
{
    ...
    while   (1)
    {
        ...
        //
        while(  0 == ::android_atomic_release_load(&mIsMsgEnabled) &&
                1 == ::android_atomic_release_load(&mIsPrvStarted) &&
                mOneShotSleepCount <= 10)
        {
            MY_LOGI("mOneShotSleepCount %d", mOneShotSleepCount);
            mOneShotSleepCount++;
            usleep(5*1000);
        }
        mOneShotSleepCount = 0;
        if(isEnabledState())
        {
            if  ( ! pBufQueue->startProcessor() )
            {
                MY_LOGE("startProcessor fail");
            }
        }
        //  (.2) break if disabled.
        ...
        if  ( ! isEnabledState() || ! pBufQueue->isProcessorRunning() )
        {
            ...
            {
                MY_LOGI("mImgBufList.size(%zu)",mImgBufList.size());
                usleep(30*1000);
