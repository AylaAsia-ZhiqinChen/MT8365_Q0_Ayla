...
{
    CAM_LOGD(
        "[ImgInfo::ImgInfo] [%s](%s:%x@%dx%d@%d-bit@%zu)",
...
)
{
    ...
    ion_user_handle_t pIonHandle = (ion_user_handle_t)NULL;
    //
    //
    ...
    if(ion_alloc_mm(
        ...
        32,
        ION_FLAG_CACHED | ION_FLAG_CACHED_NEEDS_SYNC, // 0, change to cached for better performance
        &pIonHandle))
    {
        CAM_LOGE("ion_alloc_mm fail:IonDevFd(%d),BufSize(%zu)",
                IonDevFd,
                rpImgInfo->mImgBufSize);
        goto lbExit;
    }
    ...
    if(ion_share(
        ...
        &IonBufFd))
    {
        CAM_LOGE("ion_share fail:IonDevFd(%d),BufSize(%zu),pIonHandle(%d)",
                IonDevFd,
                rpImgInfo->mImgBufSize,
                pIonHandle);
        goto lbExit;
    ...
    //
    camera_memory = requestMemory(IonBufFd, rpImgInfo->mImgBufSize, 1, user);
    if  ( ! camera_memory )
    {
        CAM_LOGE("[requestMemory] fd:%d, id:%d, size:%zu",
                IonDevFd,
                IonBufFd,
                rpImgInfo->mImgBufSize);
        goto lbExit;
...
~PrvCbImgBuf()
{
    ...
    CAM_LOGD(
        ...
        "Name(%s),FD(%d),ION(%d),VA(0x%p),Size(%zu),Fmt(%s),Str(%d),W(%d),H(%d),BPP(%d),TS(%lld)",
        getBufName(),
        mIonDevFd,
        getIonFd(),
        getVirAddr(),
...
dump() const
{
    CAM_LOGD(
        "[PrvCbImgBuf::dump]"
        "Name(%s),FD(%d),ION(%d),VA(0x%p),Size(%zu),Fmt(%s),Str(%d),W(%d),H(%d),BPP(%d),TS(%lld),S/C(%d/%d)",
        getBufName(),
        mIonDevFd,
        getIonFd(),
        getVirAddr(),
...
init(void* user)
{
    ...
    for (size_t i = 0; i < mu4BufCount; i++)
    {
        ...
        if  ( pPrvCbImgBuf == 0 )
        {
            MY_LOGE("cannot allocate pPrvCbImgBuf [%zu]", i);
            goto lbExit;
        }
        //
        pPrvCbImgBuf->dump();
        ...
        if  ( mvImgBuf[i] == 0 )
        {
            MY_LOGE("cannot allocate mvImgBuf[%zu]", i);
            goto lbExit;
        }
    }
    //
...
uninit()
{
    MY_LOGD("+ mvImgBuf.size(%zu), mu4BufCount(%d)", mvImgBuf.size(), mu4BufCount);
    //
