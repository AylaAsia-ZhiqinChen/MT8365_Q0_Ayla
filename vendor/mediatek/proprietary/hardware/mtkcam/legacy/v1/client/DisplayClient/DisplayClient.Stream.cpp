...
{
    MY_LOGD("[%s] + StreamBufList_t.size(%d)", pszDbgText, (int)rQue.size());
    if  ( ! rQue.empty() )
    {
        MY_LOGD("NOT EMPTY StreamBufList_t.size(%d)", (int)rQue.size());
...
//            "and then call cleanupQueue()"
    ...
    MY_LOGD("mNativeWindowConsumerUsage(%d), mi4Orientation(%d)",
            mNativeWindowConsumerUsage,
            mi4Orientation);
    if(mi4Orientation != 0)
    {
        // For Display Rotation
        mpStreamImgInfo = new ImgInfo(
                                wndWidth,
                                wndHeight,
                                CAMERA_DISPLAY_FORMAT_ROTATION,
                                CAMERA_DISPLAY_FORMAT_HAL_ROTATION,
                                "Camera@Display",
                                mi4Orientation);
    }
    else
    {
        mpStreamImgInfo = new ImgInfo(
                                wndWidth,
                                wndHeight,
                                CAMERA_DISPLAY_FORMAT,
                                CAMERA_DISPLAY_FORMAT_HAL,
                                "Camera@Display");
    }
    //
    mpStreamOps         = window;
...
dequePrvOps(sp<StreamImgBuf>& rpImgBuf)
{
    ...
    if  ( err || NULL == phBuffer )
    {
        MY_LOGW(
            ...
            phBuffer, ::strerror(-err), -err, (int)mStreamBufList.size()
        );
        goto lbExit;
    }
    //
...
enquePrvOps(sp<StreamImgBuf>const& rpImgBuf)
{
    ...
    if  ( mProfile_enquePrvOps.getDuration() >= ::s2ns(1) ) {
    ...
    if  ( err )
    {
        MY_LOGE(
            ...
            ::strerror(-err), -err, rpImgBuf.get(), (long long)rpImgBuf->getTimestamp()
        );
    }
    //
