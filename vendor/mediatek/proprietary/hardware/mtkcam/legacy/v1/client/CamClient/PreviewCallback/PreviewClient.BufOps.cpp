...
initBuffers()
{
    ...
    //
    if(::property_get_int32("debug.camera.scanline.prvcb", 0))
    {
...
cancelAllUnreturnBuffers()
{
    ...
    {
        MY_LOGD("mImgBufList.size(%zu)", mImgBufList.size());
        mImgBufList.clear();
    }
    //
    MY_LOGD_IF((1<=miLogLevel), "-");
...
handleReturnBuffers(Vector<ImgBufQueNode>const& rvQueNode)
{
    ...
    if  ( rvQueNode.size() > 1 )
    {
        MY_LOGW("(%zu) preview callback frame count > 1 --> select %d to callback", rvQueNode.size(), idxToCallback);
    }
    //
    //  Show Time duration.
    if  ( 0 <= idxToCallback )
    {
        ...
        MY_LOGD_IF(
            ...
            (_msDuration_buffer_timestamp > 34 ) ? "34ms<Duration" : "", (long long)_msDuration_buffer_timestamp,
        ...
        if  ( i == idxToCallback ) {
            ...
            if(mpDebugScanLine != NULL)
            {
                mpDebugScanLine->drawScanLine(
                                    rpQueImgBuf->getImgWidth(),
                                    rpQueImgBuf->getImgHeight(),
                                    (void*)(rpQueImgBuf->getVirAddr()),
                                    rpQueImgBuf->getBufSize(),
                                    NSCam::Utils::Format::queryPlaneCount(MtkCameraParameters::queryImageFormat(rpQueImgBuf->getImgFormat().string())) == 1 ? ((rpQueImgBuf->getImgWidthStride(0) * rpQueImgBuf->getBitsPerPixel())>>3) : rpQueImgBuf->getImgWidthStride(0));
            }
            //
            MY_LOGD_IF(
                (1<=miLogLevel),
                "callback:%d(%d) [%p/%zu %lld] CookieDE:%#x",
                i, rQueNode.getStatus(), pListImgBuf->getVirAddr(),
                pListImgBuf->getBufSize(), (long long)pListImgBuf->getTimestamp(),
...
performPreviewCallback(sp<ICameraImgBuf>const& pCameraImgBuf, int32_t const msgType)
{
    if  ( pCameraImgBuf != 0 )
    {
        ...
        //
        if( 1 == ::android_atomic_release_load(&mIsMsgEnabled) &&
            1 == ::android_atomic_release_load(&mIsPrvStarted))
        {
