...
{
    MY_LOGI("+ window(%p), WxH=%dx%d, count(%d), iNativeWindowConsumerUsage(%d)", window, wndWidth, wndHeight, i4MaxImgBufCount, iNativeWindowConsumerUsage);
    //
    if  ( ! window )
    {
...
waitUntilDrained()
{
    ...
    {
        MY_LOGD("Wait %lld ns", (long long)timeout);
