...
showFps() const
{
    CAM_LOGD_IF(
        1,
        "[%s] fps: %3f = %d / %lld ns",
        ms8SubjectName.string(), mfFps, mi4Count, (long long)mnsDuration
    );
}
...
dump(int fd, Vector<String8>& args)
{
    ...
    //
    MY_LOGD("args(%zu)=%s", args.size(), (*args.begin()).string());
    //
