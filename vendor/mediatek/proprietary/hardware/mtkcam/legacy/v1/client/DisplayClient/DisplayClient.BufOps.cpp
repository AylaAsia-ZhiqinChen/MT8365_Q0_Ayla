...
#else
        uint32_t rotation = mi4Orientation == 0 ? 0
                          : mi4Orientation == eTransform_ROT_90 ? 90
                          : mi4Orientation == eTransform_ROT_180 ? 180 : 270;
        //
        ret = rpBufQueue->enqueProcessor(
            ImgBufQueNode(pStreamImgBuf, ImgBufQueNode::eSTATUS_TODO, 0, 0, rotation , (uint32_t)mi4Orientation)
        );
#endif
    }
    else
...
prepareAllTodoBuffers(sp<IImgBufQueue>const& rpBufQueue)
{
    bool ret = false;
    //
    MY_LOGD_IF((2<=miLogLevel), "+ mStreamBufList.size(%d)", (int)mStreamBufList.size());
    //
    while   ( mStreamBufList.size() < (size_t)mi4MaxImgBufCount )
    {
        if  ( ! prepareOneTodoBuffer(rpBufQueue) )
        {
            MY_LOGW("mStreamBufList.size(%d) < mi4MaxImgBufCount(%d)", (int)mStreamBufList.size(), mi4MaxImgBufCount);
            break;
        }
    }
    //
    ...
    //
    MY_LOGD_IF((2<=miLogLevel), "- ret(%d) mStreamBufList.size(%d)", ret, (int)mStreamBufList.size());
    return ret;
}
...
cancelAllUnreturnBuffers()
{
    ...
    while   ( ! mStreamBufList.empty() )
    {
        ...
        MY_LOGD(
            ...
            pStreamImgBuf->getVirAddr(), (int)pStreamImgBuf->getBufSize(), (long long)pStreamImgBuf->getTimestamp(),
...
handleReturnBuffers(Vector<ImgBufQueNode>const& rvQueNode)
{
    ...
    if  ( rvQueNode.size() > 1 )
    {
        MY_LOGW("(%d) display frame count > 1 --> select %d to display", (int)rvQueNode.size(), idxToDisp);
    }
    //
    //  Show Time duration.
    if  ( 0 <= idxToDisp )
    {
        ...
        MY_LOGD_IF(
            0, "+ %s(%lld) %s(%lld)",
            (_msDuration_buffer_timestamp < 0 ) ? "time inversion!" : "", (long long)_msDuration_buffer_timestamp,
            (_msDuration_dequeProcessor > 34) ? "34ms < Duration" : "", (long long)_msDuration_dequeProcessor
        );
    }
    //
    //  (2) Lock
    ...
    for (int32_t i = 0; i < queSize; i++)
    {
        ...
        if  ( i == idxToDisp ) {
            MY_LOGD_IF(
                (1<=miLogLevel),
                "+ %s(%lld) %s(%lld), Show frame:%d %d [ion:%d %p/%zu %lld]",
                (_msDuration_buffer_timestamp < 0 ) ? "time inversion!" : "", (long long)_msDuration_buffer_timestamp,
                (_msDuration_dequeProcessor > 34) ? "34ms < Duration" : "", (long long)_msDuration_dequeProcessor,
                i, rvQueNode[i].getStatus(), pStreamImgBuf->getIonFd(),
                pStreamImgBuf->getVirAddr(), pStreamImgBuf->getBufSize(), (long long)pStreamImgBuf->getTimestamp()
            );
            MY_LOGW_IF(
                (_msDuration_buffer_timestamp <= 0), "time inversion!(%lld)", (long long)_msDuration_buffer_timestamp
            );
            //
