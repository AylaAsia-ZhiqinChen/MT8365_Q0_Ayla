...
class DbgInfoContainer : public IDbgInfoContainer
{
    ...
    public: //operations
        ...
        virtual MVOID*  queryMemory(MUINT32 const key, size_t const size, MBOOL clean = MTRUE);

        virtual MVOID*  queryMemoryIfExist(MUINT32 const key, size_t& size);
...
MVOID*
DbgInfoContainer::
queryMemory(MUINT32 const key, size_t const size, MBOOL clean)
{
...
MVOID*
DbgInfoContainer::
queryMemoryIfExist(MUINT32 const key, size_t& size)
{
    MVOID* addr = nullptr;
    for (auto&& itr : mlBlocks) {
        if (itr.mKey == key) {
            addr = itr.mPtr;
            size = itr.mSize;
            break;
