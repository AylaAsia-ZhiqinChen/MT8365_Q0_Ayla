...
)
{
    ...
    MUINT32 const size = u4Size ;    // Data size contains 'Data size' descriptor (plus 2 bytes).
...
)
{
    ...
    MUINT8* pDst = puAppnBuf;
    MUINT32 appnReturnSize = 0;
    MUINT8* pReserveBuf = new MUINT8 [u4ReserveSize];
    //
...
queryExifApp1Info(exifAPP1Info_s*const pexifApp1Info)
{
    ...
    **********************************************************************************/
    if  (mCamExifParam.u4GpsIsOn == 1)
    {
        ...
        // Set GPS Info
        if (latitude >= 0) {
            strncpy((char *) pexifApp1Info->gpsLatitudeRef, "N", sizeof(pexifApp1Info->gpsLatitudeRef)/sizeof(char));
        }
        else {
            strncpy((char *) pexifApp1Info->gpsLatitudeRef, "S", sizeof(pexifApp1Info->gpsLatitudeRef)/sizeof(char));
            latitude *= -1;     // make it positive
        }
        if (longitude >= 0) {
            strncpy((char *) pexifApp1Info->gpsLongitudeRef, "E", sizeof(pexifApp1Info->gpsLongitudeRef)/sizeof(char));
        }
        else {
            strncpy((char *) pexifApp1Info->gpsLongitudeRef, "W", sizeof(pexifApp1Info->gpsLongitudeRef)/sizeof(char));
    ...
    **********************************************************************************/
    ...
    strncpy((char *)pexifApp1Info->strSoftware, "MediaTek Camera Application", sizeof(_strSoftware));
    //get datetime
    ...
    **********************************************************************************/
    ...
    strncpy((char *)pexifApp1Info->strJpsFileName, (char*)mCamExifParam.uJpsFileName, 32);
