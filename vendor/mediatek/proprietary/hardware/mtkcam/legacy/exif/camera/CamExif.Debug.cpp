...
)
{
    ...
    if (mi4DbgModuleType & dbgModuleID) {
        ...
        //
        MY_LOGI("Get: ID_%s(0x%04x), Size(%d), Addr(0x%lu)",
            mpDebugIdMap->stringFor(dbgModuleID).string(), dbgModuleID,
            mDbgInfo[moduleIndex].u4BufSize, (unsigned long)&mDbgInfo[moduleIndex].puDbgBuf);
        //
...
)
{
    ...
    if (mi4DbgModuleType & dbgModuleID)
    {
        ...
        //
        MY_LOGI("Get: ID_%s(0x%04x), Index(%d), Size(%d), Addr(0x%lu), APP%d",
            mpDebugIdMap->stringFor(dbgModuleID).string(), dbgModuleID,
            dbgModuleIndex, mDbgInfo[dbgModuleIndex].u4BufSize, (unsigned long)mDbgInfo[dbgModuleIndex].puDbgBuf, dbgAppn);
        //
        pDbgModuleInfo = mDbgInfo[dbgModuleIndex].puDbgBuf;
        dbgModuleSize = mDbgInfo[dbgModuleIndex].u4BufSize ; // Data(n bytes) + Data size(2 bytes)
        //
        if ( (dbgModuleSize > 0) && (dbgModuleSize < 64*1024) )
        {
...
)
{
    ...
    //// CAM debug info: Common, MF, N3D, Sensor, Shading
    {
        ...
        dbgCamSize  = sizeof(DEBUG_CAM_INFO_T) ; // Data(n bytes) + Data size(2 bytes)

        if ( pDbgCamInfo && dbgCamSize > 0)
        {
...
)
{
    ...
    MBOOL enableDbgInfo = MTRUE;
    int dbgProperty = property_get_int32("debug.camera.dbg.enable", -1);
    if( dbgProperty >= 0 )
    {
        enableDbgInfo = (dbgProperty == 0) ? MFALSE : MTRUE;
    }
    MY_LOGI("enableDbgInfo(%d), dbgProperty(%d)", enableDbgInfo, dbgProperty);
    if( !enableDbgInfo )
    {
        return ret;
    }
        ...
        case CMD_SET_DBG_EXIF:
        {
            ...
            else
            {
                if (u4ID != ID_ERROR) {
                    ...
                    //
                    MY_LOGI("Set: ID_%s(0x%04x), Size(%d), Addr(0x%lu)",
                        mpDebugIdMap->stringFor(u4ID).string(), u4ID, tmpDbgInfo.u4BufSize, (unsigned long)&tmpDbgInfo.puDbgBuf);
                }
                else {
                    MY_LOGE("ID_ERROR");
                    ret = MFALSE;
