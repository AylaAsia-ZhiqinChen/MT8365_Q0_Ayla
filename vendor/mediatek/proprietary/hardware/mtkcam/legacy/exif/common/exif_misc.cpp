...
********************************************************************************/
size_t
ExifUtils::exifApp1SizeGet() const
{
    size_t size = (size_t)exifApp1Sizeof() + 2; // 0xFFD8
    MEXIF_LOGI("app1 size(%d)", (int)size);
    return size;
}
...
********************************************************************************/
unsigned int
ExifUtils::exifApp1Sizeof () const
{
    unsigned int size = 0x0a + sizeof(TiffHeader_t) + ifdListSizeof()+ IFD_TYPE_ITOPIFD * 6 ;
