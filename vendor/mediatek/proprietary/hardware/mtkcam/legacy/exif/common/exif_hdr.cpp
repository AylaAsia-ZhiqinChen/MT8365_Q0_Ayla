...
)
{
    ...
    *pretSize += 2;
    ...
    strncpy((char*) pdata, "Exif", strlen("Exif")); /* "Exif"00 */
    pdata += 5;
    *pdata++ = 0x00; /* pad */
    *pretSize += 6;
    ...
    for (tagIdx = IFD_TYPE_ZEROIFD; tagIdx <= IFD_TYPE_ITOPIFD; tagIdx++) {
        ...
        *pretSize += 4;
        ...
        if (tagIdx == IFD_TYPE_EXIFIFD) {
            ...
            err = ifdListNodeModify(IFD_TYPE_ZEROIFD, IFD0_TAG_EXIFPTR, buf);
            if (err != EXIF_NO_ERROR) return err;
        }

        if (tagIdx == IFD_TYPE_GPSIFD) {
            write32(buf, exifNextPtr);
            err = ifdListNodeModify(IFD_TYPE_ZEROIFD, IFD0_TAG_GPSINFO, buf);
            if (err != EXIF_NO_ERROR) return err;
        }

        if (tagIdx == IFD_TYPE_ITOPIFD) { /* update */
            write32(buf, exifNextPtr);
            err = ifdListNodeModify(IFD_TYPE_EXIFIFD, EXIF_TAG_ITOPIFDPTR, buf);
            if (err != EXIF_NO_ERROR) return err;
        }
    }
