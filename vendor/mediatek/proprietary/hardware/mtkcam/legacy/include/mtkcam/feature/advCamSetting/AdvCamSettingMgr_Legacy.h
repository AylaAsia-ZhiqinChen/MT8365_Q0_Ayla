 ...
 *
 * Manage advanced camera setting from app. (For Legacy Platform use)
 *
 */
...
public:
    ...
    MBOOL isCustomConfig; // True only if turnkey App do MMSDK setConfigParam() before pipeline config
...
public:
    ...
    struct AdvCamInputParam
    {
        ...
        AdvCamInputParam(MBOOL _isRec, MSize _vSize, MBOOL _is4K, android::sp<AdvCamSetting> _lastAdvCamSetting)
            : isRecordPipe(_isRec)
            , videoSize(_vSize)
            , is4K2K(_is4K)
     ...
     */
    virtual MBOOL generateAdvSetting(const AdvCamInputParam &inputParam);
     ...
     * @param[in] inputPara : some advcam setting need to reference pipeline param, such as isRecording or is4K for EIS.
     */
    virtual MBOOL regenAdvSetting(IMetadata*  appMetadata, const AdvCamInputParam &inputParam);

    /**
     * This function should be called after generateAdvSetting().
     * You can use this function to know app has set adv setting or not.
     */
    virtual MBOOL isAdvSettingExist();
     ...
     * This function should be called after generateAdvSetting().
     */
    virtual android::sp<AdvCamSetting> getAdvSetting();
