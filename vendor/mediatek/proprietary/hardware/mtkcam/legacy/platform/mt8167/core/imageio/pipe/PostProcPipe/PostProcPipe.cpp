...
//m_pIspDrvShell->getPhyIspDrv()->reset();
    ...
    //
    PIPE_INF("m_pIspDrvShell(0x%lx) pTdriVir(0x%lx) tdriPhy(0x%x) configVa(0x%lx)",
        (unsigned long)m_pIspDrvShell,(unsigned long)pTdriVir,tdriPhy,(unsigned long)pTpipeConfigVa);
...
enqueOutBuf(PortID const portID, QBufInfo const& rQBufInfo)
{
    ...
    PIPE_DBG("[%d]:0x%08d,0x%lx,0x%lx ",portID.index,
                                        rQBufInfo.vBufInfo[0].u4BufSize,
                                        (unsigned long)rQBufInfo.vBufInfo[0].u4BufVA,
                                        (unsigned long)rQBufInfo.vBufInfo[0].u4BufPA);
...
MBOOL PostProcPipe::configPipe(vector<PortInfo const*>const& vInPorts, vector<PortInfo const*>const& vOutPorts, MUINT32 isUseDefaultTuning)
{
    ...
    PIPE_DBG("settingStage(%d) in[%lu]/out[%lu]", settingStage, (unsigned long)vInPorts.size(), (unsigned long)vOutPorts.size());
    ...
    for (MUINT32 i = 0 ; i < vInPorts.size() ; i++ )
    {
        ...
        PIPE_INF("vInPorts:[%d]:(0x%x),w(%d),h(%d),stride(%d,%d,%d),type(%d),idx(%d),dir(%d),size/va/pa/md(0x%8x/0x%lx/0x%lx/0x%8x)",
                                                        i,
                                                        vInPorts[i]->eImgFmt,
                                                        ...
                                                        (unsigned long)vInPorts[i]->u4BufVA,
                                                        (unsigned long)vInPorts[i]->u4BufPA,
                                                        vInPorts[i]->memID);
        //
    ...
    for (MUINT32 i = 0 ; i < vOutPorts.size() ; i++ )
    {
        ...
        //
        PIPE_INF("vOutPorts:[%d]:(0x%x),w(%d),h(%d),stride(%d,%d,%d),type(%d),idx(%d),dir(%d),size/va/pa/md(0x%8x/0x%lx/0x%lx/0x%8x)",i,
                                                        vOutPorts[i]->eImgFmt,
                                                        ...
                                                        (unsigned long)vOutPorts[i]->u4BufVA,
                                                        (unsigned long)vOutPorts[i]->u4BufPA,
                                                        vOutPorts[i]->memID);
...
//SL TEST_RAW_YUV            tcm_en = 0x8c100003;//SL TEST_MDP         tcm_en = 0x0C100003; //SL TEST_MDP // from IspStreamCase0() 0x15004054 8C100003
    ...
    PIPE_INF("Pass(%d),sceFmt(%d),imgi.fmt(%d),isByImgi(%d),sceID(%d),en1/en2/dma(0x%x/0x%x/0x%x) ,mux sel/sel2/sram(0x%x/0x%x/0x%x),isTurn(%d),tdri VA=0x%lx tdr PA=0x%lx\n",
        this->m_pipePass, \
        meScenarioFmt, \
        ...
        isApplyTurn, \
        (unsigned long)m_camPass2Param.tdri.memBuf.base_vAddr, \
        (unsigned long)m_camPass2Param.tdri.memBuf.base_pAddr);
    //
