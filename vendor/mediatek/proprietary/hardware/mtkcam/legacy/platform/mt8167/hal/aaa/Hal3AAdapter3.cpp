...
MINT32 _convertResultToMeta(const Result_T& rResult, const IMetadata& staticMeta, MetaSet_T& rMetaResult)
{
    ...
    MINT32 rRgn[5];
    MUINT8 u1Mode;
    memset(&rRgn[0], 0, 5*sizeof(MINT32));
    ...
    if (QUERY_ENTRY_SINGLE(rMetaResult.appMeta, MTK_NOISE_REDUCTION_MODE, u1Mode))
    {
        ...
        UPDATE_ENTRY_SINGLE(rMetaResult.appMeta, MTK_NOISE_REDUCTION_MODE, (MUINT8)u1Mode);
    }
    else
    {
        UPDATE_ENTRY_SINGLE(rMetaResult.appMeta, MTK_NOISE_REDUCTION_MODE, (MUINT8)0);
    }
...
start(MINT32 i4StartNum)
{
    ...
    if(mpHal3aObj->getYuvSensor() == MTRUE)
    {
        MY_LOG_IF(1, "[%s]start yuv sensor", __FUNCTION__);
        mu1Start = 1;
        return 0;
    }
            ...
            case MTK_NOISE_REDUCTION_MODE:
                {
                    ...
                    MY_LOG_IF(fgLogEn1, "[%s] MTK_NOISE_REDUCTION_MODE(%d) tag=0x%x", __FUNCTION__, u1NRMode,MTK_NOISE_REDUCTION_MODE);
                }
                break;
...
setIsp(MINT32 flowType, const MetaSet_T& control, void* pRegBuf, MetaSet_T* pResult)
{
    ...
    if(mpHal3aObj->getYuvSensor() == MTRUE)
    {
        MY_LOG_IF(1, "[%s]yue sensor", __FUNCTION__);
        return 0;
    }
