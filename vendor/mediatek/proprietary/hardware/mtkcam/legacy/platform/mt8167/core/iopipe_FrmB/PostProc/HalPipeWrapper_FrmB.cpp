...
)
{
    ...
    if (0x0 == callerPostProc || ! ((NSImageio_FrmB::NSIspio_FrmB::IPostProcPipe *)callerPostProc)->init())
    {
    ...
    m_pIMemDrv->init();
        ...
        if(!gTuningQueNode.empty() || !gTuningQueData.empty()) {
            ...
            for(int i=0;i<DEFAULT_P2_TUNING_QUE_BUFFER_NUM;i++) {
                ...
                #endif
                LOG_INF("[TuningQue]i(%d),VA(0x%lx),size(0x%lx)\n",i,(unsigned long)tuningQueDataNew[i].bufferInfo.virtAddr,(unsigned long)mP2TuningQueBufferSize);
    ...
    NSImageio_FrmB::NSIspio_FrmB::IPostProcPipe * callerPostProc)
{
    ...
    if(mInitCount==0)
    {
        ...
        for(MUINT32 i=0;i<mTuningQueNum;i++) {
            if((gTuningQueData[i].bufferInfo.memID>=0) && (gTuningQueData[i].bufferInfo.virtAddr!=0)){
                    ...
                    gTuningQueData[i].bufferInfo.virtAddr = 0x0;
                ...
                LOG_ERR("[Error]free TuningQue error i(%d),fd(%d),va(0x%lx)",i,gTuningQueData[i].bufferInfo.memID,(unsigned long)gTuningQueData[i].bufferInfo.virtAddr);
            }
        }
    ...
    EBufferTag bufferTag)
{
    ...
    LOG_DBG("bufPackageNum(%d),misV3(%d),magic(%lu),tuningData(%lu)",bufPackageNum,misV3,(unsigned long)rParams.mvMagicNo.size(),(unsigned long)rParams.mpTuningData.size());
    ...
    else
    {
            ...
            for(int q=0;q<bufPackageNum;q++)
        {
            //mvin
            tmpC=0;
            for(MUINT32 i=inStartIdx;i< rParams.mvIn.size() ;i++)
                {
            ...
            //mvout
            tmpC=0;
            for(MUINT32 i=outStartIdx;i<rParams.mvOut.size();i++)
                {
                //check cur element is next buffer group or not
                    if(tmpC==0)
                    {    //first element in this buffer group
        ...
        for(MUINT32 i=0;i<rParams.mvModuleData.size();i++)
        {
        ...
        for(int q=0;q<bufPackageNum;q++)
        {
            ...
            for(int i=inStartIdx;i< inEndIdx ;i++)
                ...
                switch(p2PortIdx)
                        ...
                        case EPostProcPortIdx_IMGI:
                            if(rParams.mvCropRsInfo.size()>0)
                            {
                                ports[cnt].crop1.x      = 0;
                                    ...
                                    for(MUINT32 k=crop1StartIdx;k<rParams.mvCropRsInfo.size();k++)
                                    {
                    ...
                    LOG_DBG("[mvIn (%d)] fmt(%d),w/h(%lu/%lu),oft(%lu/%lu/%lu),stride(%lu/%lu/%lu),cap(%d),mem( va(0x%lx,0x%lx,0x%lx)/pa(0x%lx,0x%lx,0x%lx)/size(0x%lx,0x%lx,0x%lx) )",\
                        ports[cnt].index,ports[cnt].eImgFmt,(unsigned long)ports[cnt].u4ImgWidth,(unsigned long)ports[cnt].u4ImgHeight,(unsigned long)rParams.mvIn[i].mBuffer->getExtOffsetInBytes(0),\
                        (unsigned long)ports[cnt].xoffset,(unsigned long)ports[cnt].yoffset,(unsigned long)ports[cnt].u4Stride[0],(unsigned long)ports[cnt].u4Stride[1],(unsigned long)ports[cnt].u4Stride[2],\
                        ...
                        (unsigned long)ports[cnt].u4BufSize[0],(unsigned long)ports[cnt].u4BufSize[1],(unsigned long)ports[cnt].u4BufSize[2]);
            ...
            //mvout
                ...
                //
                //ports[cnt].u4MagicNum  = rParams.mvMagicNo[q];
                LOG_DBG("[mvOut (%d)] fmt(%d),w/h(%lu/%lu),oft(%lu/%lu/%lu),stride(%lu/%lu/%lu),cap(%d),mem( va(0x%lx,0x%lx,0x%lx)/pa(0x%lx,0x%lx,0x%lx)/size(0x%lx,0x%lx,0x%lx) )",\
                    ports[cnt].index,ports[cnt].eImgFmt,(unsigned long)ports[cnt].u4ImgWidth,(unsigned long)ports[cnt].u4ImgHeight,(unsigned long)rParams.mvOut[i].mBuffer->getExtOffsetInBytes(0),\
                    (unsigned long)ports[cnt].xoffset,(unsigned long)ports[cnt].yoffset,(unsigned long)ports[cnt].u4Stride[0],(unsigned long)ports[cnt].u4Stride[1],(unsigned long)ports[cnt].u4Stride[2],\
                    ports[cnt].capbility,(unsigned long)ports[cnt].u4BufVA[0],(unsigned long)ports[cnt].u4BufVA[1],(unsigned long)ports[cnt].u4BufVA[2],(unsigned long)ports[cnt].u4BufPA[0],(unsigned long)ports[cnt].u4BufPA[1],(unsigned long)ports[cnt].u4BufPA[2],\
                    (unsigned long)ports[cnt].u4BufSize[0],(unsigned long)ports[cnt].u4BufSize[1],(unsigned long)ports[cnt].u4BufSize[2]);
            ...
            for(MUINT32 i=0;i<rParams.mvModuleData.size();i++)
            {
                ...
                else
                {
                    ...
                    {
                        case NSImageio_FrmB::NSIspio_FrmB::EP2Module_STA_FEO:
                            LOG_DBG("[mvOut (%d)] fmt(%d),w(%lu),h(%lu),stride(%lu/%lu/%lu),cap(%d),mem( va(0x%lx,0x%lx,0x%lx)/pa(0x%lx,0x%lx,0x%lx)/size(0x%lx,0x%lx,0x%lx) )",\
                                ports[cnt].index,ports[cnt].eImgFmt,(unsigned long)ports[cnt].u4ImgWidth,(unsigned long)ports[cnt].u4ImgHeight,(unsigned long)ports[cnt].u4Stride[0],(unsigned long)ports[cnt].u4Stride[1],(unsigned long)ports[cnt].u4Stride[2],\
                                ports[cnt].capbility,(unsigned long)ports[cnt].u4BufVA[0],(unsigned long)ports[cnt].u4BufVA[1],(unsigned long)ports[cnt].u4BufVA[2],(unsigned long)ports[cnt].u4BufPA[0],(unsigned long)ports[cnt].u4BufPA[1],(unsigned long)ports[cnt].u4BufPA[2],\
                                (unsigned long)ports[cnt].u4BufSize[0],(unsigned long)ports[cnt].u4BufSize[1],(unsigned long)ports[cnt].u4BufSize[2]);
                                vPostProcOutPorts.at(outCnt)=&(ports[cnt]);
                                cnt++;
                                outCnt++;
                                            break;
                        case NSImageio_FrmB::NSIspio_FrmB::EP2Module_STA_LCEI:
                            LOG_DBG("[mvIn (%d)] fmt(%d),w(%lu),h(%lu),stride(%lu/%lu/%lu),cap(%d),mem( va(0x%lx,0x%lx,0x%lx)/pa(0x%lx,0x%lx,0x%lx)/size(0x%lx,0x%lx,0x%lx) )",\
                                ports[cnt].index,ports[cnt].eImgFmt,(unsigned long)ports[cnt].u4ImgWidth,(unsigned long)ports[cnt].u4ImgHeight,(unsigned long)ports[cnt].u4Stride[0],(unsigned long)ports[cnt].u4Stride[1],(unsigned long)ports[cnt].u4Stride[2],\
                                ports[cnt].capbility,(unsigned long)ports[cnt].u4BufVA[0],(unsigned long)ports[cnt].u4BufVA[1],(unsigned long)ports[cnt].u4BufVA[2],(unsigned long)ports[cnt].u4BufPA[0],(unsigned long)ports[cnt].u4BufPA[1],(unsigned long)ports[cnt].u4BufPA[2],\
                                (unsigned long)ports[cnt].u4BufSize[0],(unsigned long)ports[cnt].u4BufSize[1],(unsigned long)ports[cnt].u4BufSize[2]);
                                vPostProcInPorts.at(inCnt)=&(ports[cnt]);
                ...
                if(rParams.mvPrivaData[q])
                    {
                    LOG_INF("dmaEnPort(0x%08x),featureEntag (0x%x),q(%d),pvd(0x%lx)",dmaEnPort,featureEntag,q,(unsigned long)rParams.mvPrivaData[q]);
                    }
                   else
                    {
            ...
            else
            {
                ...
                bufNode.drvSce=bufPackage.drvSce;
                bufNode.callerID=bufPackage.callerID;
                if((unsigned long)(bufPackage.idxINbatchLforBnode) == mvEnqueuedBufBatch.size())    //update real index for waited added buffer node in batchA
    ...
    MINT64 i8TimeoutNs)
{
    ...
    LOG_DBG("timeout(%d),i8TimeoutNs(%lld),callerID(0x%x),de_frameNo(%d)",timeout,(long long)i8TimeoutNs,callerID,rParams.mFrameNo);
    ...
    MINT32 callerID)
{
    ...
    for (list<BufParamPackage>::iterator it = mvDequeuedBufBatch.begin(); it != mvDequeuedBufBatch.end();)
    {
        if((*it).callerID == (MUINT32)callerID)
        {   //get matched buffer
            params=(*it).rParams;
    ...
    MINTPTR arg3)
{
    ...
    LOG_DBG("+,cmd(0x%x),arg1(0x%lx),arg2(0x%lx),arg3(0x%lx)",cmd,(unsigned long)arg1,(unsigned long)arg2,(unsigned long)arg3);
    switch (cmd){
