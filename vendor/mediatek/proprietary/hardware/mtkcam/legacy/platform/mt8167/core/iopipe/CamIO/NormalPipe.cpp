...
namespace
{
    ...
    //
    NSCam::IMetadata                gPipeCap;

    //  OUT PORT
    NSCam::IMetadata                gPortCap_IMGO;
};
...
do {\
   ...
   struct stNPipe_BUF_INFO
   {
    ...
    , header_size(0)
    {                        
        memset(&rrz_info, 0, sizeof(stNPipe_RRZ_INFO));
    }
   };
...
INormalPipe::queryCapability(MUINT32 iOpenId) //iOpenId is sensorID
{
    ...
    //  Pipe
    {
        NSCam::IMetadata& cap = gPipeCap;
        {   // out port
        NSCam::IMetadata::IEntry entry(MTK_IOPIPE_INFO_AVAILABLE_OUT_PORT_INFO);
        entry.push_back(gPortCap_IMGO, Type2Type<NSCam::IMetadata>());
...
NormalPipe::NormalPipe(char const* szCallerName)
    ...
    , m_bEISChkFlg(MFALSE)
    , mpResMgrDrv(NULL)
    , mScenarioID(eScenarioID_VSS)
    , mUserCnt(0)
    , mScenarioFmt(eScenarioFmt_RAW)
    , mSensorOpenId(0)
    , mu4DeviceID(0)
    , mu4SkipFrame(0)
    , mSemDeQue()
...
NormalPipe::start()
{
    ...
    *   Frame based (end)
    ...
    //
    #if defined(PASS1_CQ_CONTINUOUS_MODE__)
...
//                return MFALSE;
            ...
            this->mpDeQueue[_PortMap(portID.index)]->sendCmd(NormalPipe::QueueMgr::eCmd_front,(MUINTPTR)&_buf,0);
...
//        CAM_LOGD("rQBuf.mvOut.push_back loop, rQTSBufInfo size(%d)", rQTSBufInfo.vBufInfo.size());
        for (MUINT32 i = 0; i < rQTSBufInfo.vBufInfo.size(); i++)
        {
            ...
            *   Frame based
            ---------------------------------------------------*/
            this->mpDeQueue[_PortMap(portID.index)]->sendCmd(NormalPipe::QueueMgr::eCmd_push,(MUINTPTR)&buff,0);
    #if 0// for debug
            int nDqSize;
...
//    IHalSensor::ConfigParam    sensorConfigParam[4];
    ...
    *********************************************************/
    ...
    pSensorPort->u4DeviceID = pHalSensorList->querySensorDevIdx(vInPorts.mSensorCfg.at(0).index);
...
//        if(pHalSensorList != NULL)
        {
            ...
            for(i=0; i<sensorNum; i++)
            {
                ...
                {
                    mSensorOpenId = i;
                    CAM_LOGD("sensorOpenId(%d)",mSensorOpenId);
                    break;
                }
            }
...
//        }
    ...
    *********************************************************/
    ...
    for (int i=0; i<vInPorts.mPortInfo.size();i++) {
        //Port info convert
        CAM_LOGD("%d ?= %d, %d", (MUINT32)PORT_IMGO, (MUINT32)vInPorts.mPortInfo.at(i).mPortID,
            PORT_IMGO == vInPorts.mPortInfo.at(i).mPortID);
...
{
    //CAM_LOGD("+(id,scen,bits) = (%d,%d,%d)", u4DeviceID, u4Scenario, u4BitDepth);
...
#define scenario_case(scenario, KEY)       \
                ...
                switch(u4Scenario)
                {
                    ...
                    default:
                        CAM_LOGE("not support sensor scenario(0x%x)", u4Scenario);
                        return MFALSE;
                        break;
                }
#undef scenario_case
...
#undef scenario_case
    ...
    mScenarioFmt = _sensorTypeToPortFormat(mSensorStaticInfo.sensorType);
    ...
    //CAM_LOGD("-(fmt,w,h,pixID) = (0x%x,%d,%d,%d)", eFmt, u4Width, u4Height, u4RawPixelID);
...
MBOOL
NormalPipe:: QueueMgr:: sendCmd(QueueMgr_cmd cmd,MUINTPTR arg1,MUINTPTR arg2)
{
    MBOOL ret = MTRUE;
