...
#define DUMP_PATH  "/sdcard/DCIM/Camera/"
...
protected:  ////      Data Members. (Request Queue)
    ...
    mutable Mutex     mHDRProcCompleteLock3;
...
MVOID HDRNodeImp::onProcessFrame(android::sp<IPipelineFrame> const& pFrame)
{
    ...
    MY_LOGD("cropInfos.dstsize_resizer (%dx%d)",dstsize_resizer.w,dstsize_resizer.h);
    MY_LOGD("cropInfos.crop_dma (%dx%d)",crop_dma.s.w,crop_dma.s.h);
    // for crop
    ...
    if ((mNodeEnqueIndex + 1) == mHDRFrameNum)
    {
        ...
        MY_LOGD(" HDR call onDispatchFrame()");
...
{
    MY_LOGD("HDRProcThread::threadLoop + ");
