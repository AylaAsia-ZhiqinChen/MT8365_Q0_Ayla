...
#include "MyUtils.h"
//
#if '1'==MTKCAM_HAVE_HAL_3A_ADAPTER
 ...
 *
 ******************************************************************************/
#define MY_LOGV(fmt, arg...)        CAM_LOGV("%d[Hal3AAdapter::%s] " fmt, mId, __FUNCTION__, ##arg)
#define MY_LOGD(fmt, arg...)        CAM_LOGD("%d[Hal3AAdapter::%s] " fmt, mId, __FUNCTION__, ##arg)
#define MY_LOGI(fmt, arg...)        CAM_LOGI("%d[Hal3AAdapter::%s] " fmt, mId, __FUNCTION__, ##arg)
#define MY_LOGW(fmt, arg...)        CAM_LOGW("%d[Hal3AAdapter::%s] " fmt, mId, __FUNCTION__, ##arg)
#define MY_LOGE(fmt, arg...)        CAM_LOGE("%d[Hal3AAdapter::%s] " fmt, mId, __FUNCTION__, ##arg)
...
public:     ////    Operations.
                    ...
                    Hal3AAdapter(int32_t id, std::string const& name)
                        ...
                        , mName(name)
                    {
                    #if '0'==MTKCAM_HAVE_HAL_3A_ADAPTER
                        MY_LOGW("'0'==MTKCAM_HAVE_HAL_3A_ADAPTER");
                    #endif
                        MY_LOGD("%p", this);
                    }
    ...
    virtual auto    init() -> bool
                    ...
                    #if '1'==MTKCAM_HAVE_HAL_3A_ADAPTER
                        mHal3a = NS3Av3::IHal3A::createInstance(NS3Av3::IHal3A::E_Camera_3, mId, mName.c_str());
    ...
    virtual auto    notifyPowerOn() -> bool
                    {
                    #if '1'==MTKCAM_HAVE_HAL_3A_ADAPTER_POWER_NOTIFICATION
                        if  ( mHal3a ) {
    ...
    ) -> android::sp<IHal3AAdapter>
{
    ...
    if  ( p == nullptr ) {
        CAM_LOGE("Fail to new Hal3AAdapter");
        return nullptr;
    }
