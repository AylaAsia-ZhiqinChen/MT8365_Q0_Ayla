...
enqueBuf(PortID const ePortID, QBufInfo const& rQBufInfo)
{
    ...
    {
        MY_LOGD("+ tid(%d) type(%d,%d,%d) (VA,PA,Size,ID)(0x%lx,0x%x,%d,%d)",
            gettid(),
            ePortID.type, ePortID.index, ePortID.inout,
            (unsigned long)rQBufInfo.vBufInfo.at(i).u4BufVA, rQBufInfo.vBufInfo.at(i).u4BufPA,
...
dequeBuf(PortID const ePortID, QTimeStampBufInfo& rQBufInfo, MUINT32 const u4TimeoutMs /*= 0xFFFFFFFF*/)
{
    ...
    for (MUINT32 i = 0; i < rQBufInfo.vBufInfo.size(); i++)
    {
        MY_LOGD("(VA,PA,Size,ID)(0x%lx,0x%x,%d,%d)",
            (unsigned long)rQBufInfo.vBufInfo.at(i).u4BufVA, rQBufInfo.vBufInfo.at(i).u4BufPA,
            rQBufInfo.vBufInfo.at(i).u4BufSize, rQBufInfo.vBufInfo.at(i).i4MemID);
    }
...
{
    //FUNCTION_LOG_START;
    MY_LOGD("+ %zu in / %zu out, concurr(%d)", \
...
{
    FUNCTION_LOG_START;
    MY_LOGD("+ %zu in / %zu out", vInPorts.size(), vOutPorts.size());
    FUNCTION_LOG_END;
    return  MTRUE;
}
