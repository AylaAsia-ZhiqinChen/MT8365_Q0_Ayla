...
bool doRGB565Buffer_SW(MUINT8 *a_dstAddr, MUINT8 *a_srcAddr)
{
    ...
    for(i=0;i<(unsigned int)dImgH;i=i+2)
    {
    for(j=0;j<(unsigned int)dImgW;j=j+2)
    {
...
)
{
    ...
    //MY_LOGD("SD Status: SDEnable:%d, img_w:%d, img_h:%d, Src_Phy_Addr:0x%p,", g_CurrentStatus, IMAGE_MAX_WIDTH, IMAGE_MAX_HEIGHT, ImageBufferPhy);
