...
//#include <core/featureio/pipe/aaa/state_mgr/aaa_state.h>
...
Hal3ARaw::AESensorThreadLoop(MVOID *arg)
{
    ...
    while(1)
    {
        ...
        MY_LOG_IF(DEBUG_LOG_EN, "[AESensorThreadLoop] updateSensorbyI2C\n");
        ...
        {
            int Val;
            MY_LOG_IF(DEBUG_LOG_EN, "[Hal3A::AESensorThreadLoop] start waiting mAESenMtx lock (3)\n");
            Mutex::Autolock autoLock(_this->mAESenMtx);
            MY_LOG_IF(DEBUG_LOG_EN, "[Hal3A::AESensorThreadLoop] get mAESenMtx lock (3)\n");

            ::sem_getvalue(&_this->semAESenThdEnd, &Val);
            MY_LOG_IF(DEBUG_LOG_EN, "[AESensorThreadLoop] semAESenThdEnd before post = %d\n", Val);
...
Hal3ARaw::postToAESenThread()
{
    ...
    ::sem_post(&semAESen);
    MY_LOG_IF(DEBUG_LOG_EN, "[postToAESenThread] sem_post semAESen, semAESenThdEnd before wait = %d\n", Val);
}
