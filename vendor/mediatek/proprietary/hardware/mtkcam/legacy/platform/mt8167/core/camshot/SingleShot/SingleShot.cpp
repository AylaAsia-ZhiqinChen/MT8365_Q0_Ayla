...
registerImgBufInfo(ECamShotImgBufType const eBufType, ImgBufInfo const &rImgBuf)
{
     ...
     MY_LOGD("[registerImgBufInfo] (VA, PA, Size, ID) = (%u, %d, %d, %d)", (unsigned int)rImgBuf.u4BufVA, rImgBuf.u4BufPA, rImgBuf.u4BufSize, rImgBuf.i4MemID);
...
allocImgMem(char const* const pszName, EImageFormat const eFmt, MUINT32 const u4Width, MUINT32 const u4Height, IMEM_BUF_INFO & rMem)
{
    ...
    if (0 == rMem.size)
    {
        ...
        allocMem(rMem);
        MY_LOGD("[allocImgMem] (va, pa, size) = (%lu, %u, %d)",  (unsigned long)rMem.virtAddr, (unsigned int)rMem.phyAddr, rMem.size);
    }
    else
    {
        if (rMem.size < u4BufSize)
        {
            ...
            MY_LOGD("[allocImgMem] re-allocate (va, pa, size) = (%lu, %u, %d)", (unsigned long)rMem.virtAddr, (unsigned int)rMem.phyAddr, rMem.size);
        }
    }
    profile.print();
    return MTRUE;
