...
MINT32 _startMdp(MDPMGR_DPSTREAM_TYPE dpStreamtype,MDPMGR_CFG_STRUCT_FRMB cfgData,T* pDpStream)
{
    ...
    for(MINT32 index = 0; index < ISP_MDP_DL_NUM; index++)
    {
        ...
        if(cfgData.dstPortCfg[index] == 1)
        {
            ...
            LOG_INF("index(%d),capbility(%d),fmt(%d),rot(%d),flip(%d),(w,h,s)=(%lu,%lu,%lu),C(w,h,s)=(%lu,%lu,%lu),V(w,h,s)=(%lu,%lu,%lu),Wmdpindx(%d)",
                    index, cfgData.dstDma[index].capbility, dstFmt, cfgData.dstDma[index].Rotation, cfgData.dstDma[index].Flip,
                    (unsigned long)cfgData.dstDma[index].size.w,(unsigned long)cfgData.dstDma[index].size.h,(unsigned long)cfgData.dstDma[index].size.stride,
                    (unsigned long)cfgData.dstDma[index].size_c.w,(unsigned long)cfgData.dstDma[index].size_c.h,(unsigned long)cfgData.dstDma[index].size_c.stride,
                    (unsigned long)cfgData.dstDma[index].size_v.w,(unsigned long)cfgData.dstDma[index].size_v.h,(unsigned long)cfgData.dstDma[index].size_v.stride,
            ...
            if(cfgData.dstDma[index].memBuf.base_vAddr == 0 || cfgData.dstDma[index].memBuf.base_pAddr == 0) {
                ...
                LOG_ERR("stride(%lu),stride_c(%lu),stride_v(%lu)", (unsigned long)cfgData.dstDma[index].size.stride,
                                                                   (unsigned long)cfgData.dstDma[index].size_c.stride,
                                                                   (unsigned long)cfgData.dstDma[index].size_v.stride);
                return MDPMGR_NULL_OBJECT;
            }
...
MINT32 MdpMgrImp_FrmB::startMdp(MDPMGR_CFG_STRUCT_FRMB cfgData)
{
    ...
    LOG_DBG("+,cqIndx(%u),sceID(%u),cqPA(0x%x),tpipVa(0x%lx),tpipPa(0x%x),isWaitBuf(%d),nbc_Scal(%d)", \
