    ...
    ECmd const &cmd)
{
    ...
    LOG_DBG("mCmdList, size(%lu) + cmd(%d)", (unsigned long)mCmdList.size(), cmd);
    mTotalCmdNum++;
    ...
    ECmd &cmd)
{
    ...
    if(mCmdList.size()>0)
    {
        LOG_DBG("mCmdList curSize(%lu), todo(%d)", (unsigned long)mCmdList.size(), *mCmdList.begin());
        cmd = *mCmdList.begin();
    ...
    MUINT32 callerID)
{
    ...
    if(bufferListTag==EBufferListTag_Batch)
    {
        ...
        LOG_DBG("Lsize(%lu)",(unsigned long)mvEnqueuedBufBatch.size());
        ...
        LOG_DBG("updateBNList(%lu)",(unsigned long)mvEnqueueBufNode.size());
        ...
        LOG_DBG("updateBList(%lu)",(unsigned long)mvEnqueuedBufBatch.size());
...
)
{
        ...
        if( (int)((bufNode.portEn & (1 << i)) >> i))
        {
            ...
            switch(deQret)
            {
                case eIspRetStatus_Failed:
                    ...
                    {
                        LOG_INF("Fst mvOut mva(0x%lx)",(unsigned long)mvEnqueuedBufBatch[bufNode.idxInBufBatchList].rParams.mvOut[0].mBuffer->getBufPA(0));
                    }
                    else
                    {
    ...
    if(retFrame==true)
    {
        ...
        if(mvEnqueuedBufBatch[bufNode.idxInBufBatchList].rParams.mvOut.size()>0)
        {
            LOG_INF("FmvOut(0x%lx),dCq(%d/%d)",(unsigned long)mvEnqueuedBufBatch[bufNode.idxInBufBatchList].rParams.mvOut[0].mBuffer->getBufPA(0),bufNode.p2cqIdx,bufNode.p2cqDupIdx);
        }
        else
        {
            LOG_ERR("no output dma");
