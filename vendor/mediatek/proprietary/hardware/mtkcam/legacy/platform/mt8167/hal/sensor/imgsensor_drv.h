...
private:
	...
	// debug sensor info dynamiclly
	virtual void customSetSensorInfo(SENSOR_DEV_ENUM sensorDevId, SENSORDRV_INFO_STRUCT *pSensorInfo);
	virtual void addMapOfSensorInfoAlias();
...
private:
	...
	map <string, string> sensorInfoAliases;
};

// alias: eg: MIPIDataLowPwr2HighSpeedSettleDelayCount < -->  MIPIDataLP2HSSettleDelay
// it's for setprop can set correctly.  setprop key max length  is 32 now.
// I don't use map.
typedef struct
{
	const char * name;
