            ...
            socketPos = this->getSocketPosition((CAMERA_DUAL_CAMERA_SENSOR_ENUM)SensorEnum);
                ...
                if (err < 0 || err2 < 0) {
                    ...
                    if (SensorEnum == DUAL_CAMERA_MAIN_SENSOR) {
					    ...
					    property_get("vendor.debug.cam.ignore_main", value, "-1");
					    int ignoreMain = atoi(value);
						if(ignoreMain == 1) {
							ALOGI("ignoreMain:%d\n", ignoreMain);
							continue;
						}
...
)
{
    ...
    if (mUsers == 0) {
        if (m_fdSensor == -1) {
            ...
            // add map info of sensor info & alias
            addMapOfSensorInfoAlias();
        }
    }
...
#define SENDCMD_LOG // Open sendcommand log, for test only
    ...
    case CMD_SENSOR_GET_SENSOR_FEATURE_INFO:
     ...
     case CMD_SENSOR_GET_DEFAULT_FRAME_RATE_BY_SCENARIO:
        ...
        FeaturePara[0] = *parg1;
        FeaturePara[1] = 0;//(MUINTPTR)(*parg2) // Pass the address of parg2
        FeatureParaLen = sizeof(MUINT64) * 2;
...
#ifdef SENDCMD_LOG
        LOG_MSG("E--[CMD_SENSOR_GET_DEFAULT_FRAME_RATE_BY_SCENARIO] FeaturePara[0] = %llu, FeaturePara[1] = %llx, FeaturePara[2] = %llu, FeaturePara[3] = %llu\n",
			FeaturePara[0], FeaturePara[1], FeaturePara[2], FeaturePara[3]);
        ...
        switch(sensorDevId)
        {
            ...
            case SENSOR_MAIN:
                *parg1 = getMipiSensorPort(eDevId_ImgSensor0);
			    ...
			    property_get("vendor.debug.mipi_pad", value, "-1");
			    pad = atoi(value);
				if(pad != -1 && sensorDevId == SENSOR_MAIN) {
					*parg1 = pad;
					ALOGI("new main mipi pad:%d\n", pad);
				}
                err = 0;
                break;
            case SENSOR_SUB:
    ...
    if(FeatureId == SENSOR_FEATURE_GET_DEFAULT_FRAME_RATE_BY_SCENARIO)
    {
#ifdef SENDCMD_LOG
        LOG_MSG("X--[CMD_SENSOR_GET_DEFAULT_FRAME_RATE_BY_SCENARIO] FeaturePara[0] = %llu, FeaturePara[1] = %lld, FeaturePara[2] = %llu, FeaturePara[3] = %llu\n",
            FeaturePara[0], FeaturePara[1], FeaturePara[2], FeaturePara[3]);
#endif
    }
     ...
     switch (cmd) {
        ...
        case CMD_SENSOR_GET_DEFAULT_FRAME_RATE_BY_SCENARIO:
            *((MUINT32*)parg2)=FeaturePara[1];
#ifdef SENDCMD_LOG
            LOG_MSG("X--[CMD_SENSOR_GET_DEFAULT_FRAME_RATE_BY_SCENARIO] FeaturePara[0] = %llu, FeaturePara[1] = %llx, FeaturePara[2] = %llu, FeaturePara[3] = %llu\n", 		  	FeaturePara[0], FeaturePara[1], FeaturePara[2], FeaturePara[3]);
            LOG_MSG("FeaturePara %d\n",(FeaturePara[1]));
...
void ImgSensorDrv::addMapOfSensorInfoAlias()
{
	sensorInfoAliases.clear();

	sensorInfoAliases.insert(pair<string, string>("SensorPreviewResolutionX_ZSD", "SensorPrvX_ZSD"));
	sensorInfoAliases.insert(pair<string, string>("SensorPreviewResolutionY_ZSD", "SensorPrvY_ZSD"));
	sensorInfoAliases.insert(pair<string, string>("SensorCameraPreviewFrameRate", "SensorPrvFps"));
	sensorInfoAliases.insert(pair<string, string>("SensorWebCamCaptureFrameRate", "SensorWebCamCapFps"));
	sensorInfoAliases.insert(pair<string, string>("MIPIDataLowPwr2HighSpeedTermDelayCount", "MIPIDataLP2HSTermDelay"));
	sensorInfoAliases.insert(pair<string, string>("MIPIDataLowPwr2HSSettleDelayM0", "MIPIDataLP2HSSettleDelayM0"));
	sensorInfoAliases.insert(pair<string, string>("MIPIDataLowPwr2HSSettleDelayM1", "MIPIDataLP2HSSettleDelayM1"));
	sensorInfoAliases.insert(pair<string, string>("MIPIDataLowPwr2HSSettleDelayM2", "MIPIDataLP2HSSettleDelayM2"));
	sensorInfoAliases.insert(pair<string, string>("MIPIDataLowPwr2HSSettleDelayM3", "MIPIDataLP2HSSettleDelayM3"));
	sensorInfoAliases.insert(pair<string, string>("MIPIDataLowPwr2HSSettleDelayM3", "MIPIDataLP2HSSettleDelayM4"));
	sensorInfoAliases.insert(pair<string, string>("MIPIDataLowPwr2HighSpeedSettleDelayCount", "MIPIDataLP2HSSettleDelay"));
	sensorInfoAliases.insert(pair<string, string>("MIPICLKLowPwr2HighSpeedTermDelayCount", "MIPICLKLP2HSTermDelay"));
	// add other aliases below
	// ......
}


//  set sensor info macro
// note: strlen(#sensorInfoItem) > 27:  27 = (PROPERTY_KEY_MAX - 1) - 4 = 31 - 4  (d.%d.), we assume max length of sensorDevId  is 1.
#define SET_SENSOR_INFO(sensorDevId, pSensorInfo, sensorInfoItem, dataType) \
{															\
	char value[PROPERTY_VALUE_MAX] = {'\0'};				\
	char prop[PROPERTY_KEY_MAX];	\
	int found = 0;			\
	if(strlen(#sensorInfoItem) >= PROPERTY_KEY_MAX - 4) { \
		map <string, string>::iterator iter;		\
		if((iter = sensorInfoAliases.find(#sensorInfoItem)) != sensorInfoAliases.end()) {		\
			if(iter->second.length() < PROPERTY_KEY_MAX - 4) {			\
				snprintf(prop, PROPERTY_KEY_MAX, "vendor.d.%d.%s", sensorDevId, iter->second.c_str());	\
				found = 1;				\
			}			\
		}			\
	} else  {	\
		snprintf(prop, PROPERTY_KEY_MAX, "vendor.d.%d.%s", sensorDevId, #sensorInfoItem);	\
		found = 1;			\
	}				\
						\
	if(found == 1) {			\
		property_get(prop, value, "-1");							\
		int int_value = atoi(value);							\
		if(int_value != -1) {									\
			pSensorInfo->sensorInfoItem = (dataType)int_value;			\
			LOG_WRN("[Debug] set new prop:%s, value:%d", #sensorInfoItem, int_value);	\
		}	\
		else {		\
			property_get("vendor.debug.cam.output_dbg_seninfo", value, "-1");		\
			int int_value = atoi(value);		\
			if(int_value == 1)					\
				LOG_WRN("%s = %d", prop, value);		\
		}			\
	} 				\
}

// not used now
static SENSOR_INFO_ALIAS sensorInfoAlias[] =
{
		{"SensorPreviewResolutionX_ZSD", 				"SensorPrvX_ZSD"},
		{"SensorPreviewResolutionY_ZSD", 				"SensorPrvY_ZSD"},
		{"SensorCameraPreviewFrameRate", 				"SensorPrvFps"},
		{"SensorWebCamCaptureFrameRate", 				"SensorWebCamCapFps"},
		{"MIPIDataLowPwr2HighSpeedTermDelayCount", 		"MIPIDataLP2HSTermDelay"},
		{"MIPIDataLowPwr2HSSettleDelayM0", 				"MIPIDataLP2HSSettleDelayM0"},
		{"MIPIDataLowPwr2HSSettleDelayM1", 				"MIPIDataLP2HSSettleDelayM1"},
		{"MIPIDataLowPwr2HSSettleDelayM2", 				"MIPIDataLP2HSSettleDelayM2"},
		{"MIPIDataLowPwr2HSSettleDelayM3", 				"MIPIDataLP2HSSettleDelayM3"},
		{"MIPIDataLowPwr2HSSettleDelayM4", 				"MIPIDataLP2HSSettleDelayM4"},
		{"MIPIDataLowPwr2HighSpeedSettleDelayCount", 	"MIPIDataLP2HSSettleDelay"},
		{"MIPICLKLowPwr2HighSpeedTermDelayCount", 		"MIPICLKLP2HSTermDelay"},
};

void ImgSensorDrv::customSetSensorInfo(SENSOR_DEV_ENUM sensorDevId, SENSORDRV_INFO_STRUCT *pSensorInfo)
{
	SET_SENSOR_INFO(sensorDevId, pSensorInfo, SensorPreviewResolutionX, MUINT16)
	SET_SENSOR_INFO(sensorDevId, pSensorInfo, SensorPreviewResolutionY, MUINT16)
	SET_SENSOR_INFO(sensorDevId, pSensorInfo, SensorPreviewResolutionX_ZSD, MUINT16)
	SET_SENSOR_INFO(sensorDevId, pSensorInfo, SensorPreviewResolutionY_ZSD, MUINT16)
	SET_SENSOR_INFO(sensorDevId, pSensorInfo, SensorFullResolutionX, MUINT16)
	SET_SENSOR_INFO(sensorDevId, pSensorInfo, SensorFullResolutionY, MUINT16)
	SET_SENSOR_INFO(sensorDevId, pSensorInfo, SensorClockFreq, MUINT8)
	SET_SENSOR_INFO(sensorDevId, pSensorInfo, SensorCameraPreviewFrameRate, MUINT8)
	SET_SENSOR_INFO(sensorDevId, pSensorInfo, SensorVideoFrameRate, MUINT8)
	SET_SENSOR_INFO(sensorDevId, pSensorInfo, SensorStillCaptureFrameRate, MUINT8)
	SET_SENSOR_INFO(sensorDevId, pSensorInfo, SensorWebCamCaptureFrameRate, MUINT8)
	SET_SENSOR_INFO(sensorDevId, pSensorInfo, SensorClockPolarity, MUINT8)
	SET_SENSOR_INFO(sensorDevId, pSensorInfo, SensorClockFallingPolarity, MUINT8)
	SET_SENSOR_INFO(sensorDevId, pSensorInfo, SensorClockRisingCount, MUINT8)
	SET_SENSOR_INFO(sensorDevId, pSensorInfo, SensorClockFallingCount, MUINT8)
	SET_SENSOR_INFO(sensorDevId, pSensorInfo, SensorClockDividCount, MUINT8)
	SET_SENSOR_INFO(sensorDevId, pSensorInfo, SensorPixelClockCount, MUINT8)
	SET_SENSOR_INFO(sensorDevId, pSensorInfo, SensorDataLatchCount, MUINT8)
	SET_SENSOR_INFO(sensorDevId, pSensorInfo, SensorHsyncPolarity, MUINT8)
	SET_SENSOR_INFO(sensorDevId, pSensorInfo, SensorVsyncPolarity, MUINT8)
	SET_SENSOR_INFO(sensorDevId, pSensorInfo, SensorInterruptDelayLines, MUINT8)
	SET_SENSOR_INFO(sensorDevId, pSensorInfo, SensorResetActiveHigh, MINT32)
	SET_SENSOR_INFO(sensorDevId, pSensorInfo, SensorResetDelayCount, MUINT32)
	SET_SENSOR_INFO(sensorDevId, pSensorInfo, SensroInterfaceType, ACDK_SENSOR_INTERFACE_TYPE_ENUM)
	SET_SENSOR_INFO(sensorDevId, pSensorInfo, SensorOutputDataFormat, ACDK_SENSOR_OUTPUT_DATA_FORMAT_ENUM)
	SET_SENSOR_INFO(sensorDevId, pSensorInfo, SensorMIPILaneNumber, ACDK_SENSOR_MIPI_LANE_NUMBER_ENUM)
	SET_SENSOR_INFO(sensorDevId, pSensorInfo, CaptureDelayFrame, MUINT32)
	SET_SENSOR_INFO(sensorDevId, pSensorInfo, PreviewDelayFrame, MUINT32)
	SET_SENSOR_INFO(sensorDevId, pSensorInfo, VideoDelayFrame, MUINT32)
	SET_SENSOR_INFO(sensorDevId, pSensorInfo, HighSpeedVideoDelayFrame, MUINT32)
	SET_SENSOR_INFO(sensorDevId, pSensorInfo, SlimVideoDelayFrame, MUINT32)
	SET_SENSOR_INFO(sensorDevId, pSensorInfo, YUVAwbDelayFrame, MUINT32)
	SET_SENSOR_INFO(sensorDevId, pSensorInfo, YUVEffectDelayFrame, MUINT32)
	SET_SENSOR_INFO(sensorDevId, pSensorInfo, Custom1DelayFrame, MUINT32)
	SET_SENSOR_INFO(sensorDevId, pSensorInfo, Custom2DelayFrame, MUINT32)
	SET_SENSOR_INFO(sensorDevId, pSensorInfo, Custom3DelayFrame, MUINT32)
	SET_SENSOR_INFO(sensorDevId, pSensorInfo, Custom4DelayFrame, MUINT32)
	SET_SENSOR_INFO(sensorDevId, pSensorInfo, Custom5DelayFrame, MUINT32)
	SET_SENSOR_INFO(sensorDevId, pSensorInfo, SensorGrabStartX_PRV, MUINT16)
	SET_SENSOR_INFO(sensorDevId, pSensorInfo, SensorGrabStartY_PRV, MUINT16)
	SET_SENSOR_INFO(sensorDevId, pSensorInfo, SensorGrabStartX_CAP, MUINT16)
	SET_SENSOR_INFO(sensorDevId, pSensorInfo, SensorGrabStartY_CAP, MUINT16)
	SET_SENSOR_INFO(sensorDevId, pSensorInfo, SensorGrabStartX_VD, MUINT16)
	SET_SENSOR_INFO(sensorDevId, pSensorInfo, SensorGrabStartY_VD, MUINT16)
	SET_SENSOR_INFO(sensorDevId, pSensorInfo, SensorGrabStartX_VD1, MUINT16)
	SET_SENSOR_INFO(sensorDevId, pSensorInfo, SensorGrabStartY_VD1, MUINT16)
	SET_SENSOR_INFO(sensorDevId, pSensorInfo, SensorGrabStartX_VD2, MUINT16)
	SET_SENSOR_INFO(sensorDevId, pSensorInfo, SensorGrabStartY_VD2, MUINT16)
	SET_SENSOR_INFO(sensorDevId, pSensorInfo, SensorGrabStartX_CST1, MUINT16)
	SET_SENSOR_INFO(sensorDevId, pSensorInfo, SensorGrabStartY_CST1, MUINT16)
	SET_SENSOR_INFO(sensorDevId, pSensorInfo, SensorGrabStartX_CST2, MUINT16)
	SET_SENSOR_INFO(sensorDevId, pSensorInfo, SensorGrabStartY_CST2, MUINT16)
	SET_SENSOR_INFO(sensorDevId, pSensorInfo, SensorGrabStartX_CST3, MUINT16)
	SET_SENSOR_INFO(sensorDevId, pSensorInfo, SensorGrabStartY_CST3, MUINT16)
	SET_SENSOR_INFO(sensorDevId, pSensorInfo, SensorGrabStartX_CST4, MUINT16)
	SET_SENSOR_INFO(sensorDevId, pSensorInfo, SensorGrabStartY_CST4, MUINT16)
	SET_SENSOR_INFO(sensorDevId, pSensorInfo, SensorGrabStartX_CST5, MUINT16)
	SET_SENSOR_INFO(sensorDevId, pSensorInfo, SensorGrabStartY_CST5, MUINT16)
	SET_SENSOR_INFO(sensorDevId, pSensorInfo, SensorDrivingCurrent, MUINT16)
	SET_SENSOR_INFO(sensorDevId, pSensorInfo, SensorMasterClockSwitch, MUINT8)
	SET_SENSOR_INFO(sensorDevId, pSensorInfo, AEShutDelayFrame, MUINT8)
	SET_SENSOR_INFO(sensorDevId, pSensorInfo, AESensorGainDelayFrame, MUINT8)
	SET_SENSOR_INFO(sensorDevId, pSensorInfo, AEISPGainDelayFrame, MUINT8)
	SET_SENSOR_INFO(sensorDevId, pSensorInfo, MIPIDataLowPwr2HighSpeedTermDelayCount, MUINT8)
	SET_SENSOR_INFO(sensorDevId, pSensorInfo, MIPIDataLowPwr2HSSettleDelayM0, MUINT8)
	SET_SENSOR_INFO(sensorDevId, pSensorInfo, MIPIDataLowPwr2HSSettleDelayM1, MUINT8)
	SET_SENSOR_INFO(sensorDevId, pSensorInfo, MIPIDataLowPwr2HSSettleDelayM2, MUINT8)
	SET_SENSOR_INFO(sensorDevId, pSensorInfo, MIPIDataLowPwr2HSSettleDelayM3, MUINT8)
	SET_SENSOR_INFO(sensorDevId, pSensorInfo, MIPIDataLowPwr2HSSettleDelayM4, MUINT8)
	SET_SENSOR_INFO(sensorDevId, pSensorInfo, MIPIDataLowPwr2HighSpeedSettleDelayCount, MUINT8)
	SET_SENSOR_INFO(sensorDevId, pSensorInfo, MIPICLKLowPwr2HighSpeedTermDelayCount, MUINT8)
	SET_SENSOR_INFO(sensorDevId, pSensorInfo, SensorWidthSampling, MUINT8)
	SET_SENSOR_INFO(sensorDevId, pSensorInfo, SensorHightSampling, MUINT8)
	SET_SENSOR_INFO(sensorDevId, pSensorInfo, SensorPacketECCOrder, MUINT8)
	SET_SENSOR_INFO(sensorDevId, pSensorInfo, iHDR_First_IS_LE, MUINT8)
	SET_SENSOR_INFO(sensorDevId, pSensorInfo, iHDRSupport, MBOOL)
	SET_SENSOR_INFO(sensorDevId, pSensorInfo, PDAF_Support, MUINT8)
	SET_SENSOR_INFO(sensorDevId, pSensorInfo, SensorModeNum, MUINT8)
	SET_SENSOR_INFO(sensorDevId, pSensorInfo, virtualChannelSupport, MBOOL)
	SET_SENSOR_INFO(sensorDevId, pSensorInfo, MIPIsensorType, SENSOR_MIPI_TYPE_ENUM)
	SET_SENSOR_INFO(sensorDevId, pSensorInfo, SettleDelayMode, SENSOR_SETTLEDELAY_MODE_ENUM)
	SET_SENSOR_INFO(sensorDevId, pSensorInfo, IMGSENSOR_DPCM_TYPE_PRE, MUINT8)
	SET_SENSOR_INFO(sensorDevId, pSensorInfo, IMGSENSOR_DPCM_TYPE_CAP, MUINT8)
	SET_SENSOR_INFO(sensorDevId, pSensorInfo, IMGSENSOR_DPCM_TYPE_VD, MUINT8)
	SET_SENSOR_INFO(sensorDevId, pSensorInfo, IMGSENSOR_DPCM_TYPE_VD1, MUINT8)
	SET_SENSOR_INFO(sensorDevId, pSensorInfo, IMGSENSOR_DPCM_TYPE_VD2, MUINT8)
	SET_SENSOR_INFO(sensorDevId, pSensorInfo, PerFrameCTL_Support, MUINT8)
	SET_SENSOR_INFO(sensorDevId, pSensorInfo, SCAM_DataNumber, SENSOR_SCAM_DATA_CHANNEL_ENUM)
	SET_SENSOR_INFO(sensorDevId, pSensorInfo, SCAM_DDR_En, MUINT8)
	SET_SENSOR_INFO(sensorDevId, pSensorInfo, SCAM_CLK_INV, MUINT8)
	SET_SENSOR_INFO(sensorDevId, pSensorInfo, SensorImageMirror, ACDK_SENSOR_IMAGE_MIRROR_ENUM)
	SET_SENSOR_INFO(sensorDevId, pSensorInfo, EnableShutterTansfer, MINT32)
	SET_SENSOR_INFO(sensorDevId, pSensorInfo, EnableFlashlightTansfer, MINT32)
	SET_SENSOR_INFO(sensorDevId, pSensorInfo, SensorOperationMode, ACDK_SENSOR_OPERATION_MODE_ENUM)
	SET_SENSOR_INFO(sensorDevId, pSensorInfo, ImageTargetWidth, MUINT16)
	SET_SENSOR_INFO(sensorDevId, pSensorInfo, ImageTargetHeight, MUINT16)
	SET_SENSOR_INFO(sensorDevId, pSensorInfo, CaptureShutter, MUINT16)
	SET_SENSOR_INFO(sensorDevId, pSensorInfo, FlashlightDuty, MUINT16)
	SET_SENSOR_INFO(sensorDevId, pSensorInfo, FlashlightOffset, MUINT16)
	SET_SENSOR_INFO(sensorDevId, pSensorInfo, FlashlightShutFactor, MUINT16)
	SET_SENSOR_INFO(sensorDevId, pSensorInfo, FlashlightMinShutter, MUINT16)
	SET_SENSOR_INFO(sensorDevId, pSensorInfo, MetaMode, ACDK_CAMERA_OPERATION_MODE_ENUM)
	SET_SENSOR_INFO(sensorDevId, pSensorInfo, DefaultPclk, MUINT32)
	SET_SENSOR_INFO(sensorDevId, pSensorInfo, Pixels, MUINT32)
	SET_SENSOR_INFO(sensorDevId, pSensorInfo, Lines, MUINT32)
	SET_SENSOR_INFO(sensorDevId, pSensorInfo, Shutter, MUINT32)
	SET_SENSOR_INFO(sensorDevId, pSensorInfo, FrameLines, MUINT32)
	SET_SENSOR_INFO(sensorDevId, pSensorInfo, SensorPreviewWidth, MUINT16)
	SET_SENSOR_INFO(sensorDevId, pSensorInfo, SensorPreviewHeight, MUINT16)
	SET_SENSOR_INFO(sensorDevId, pSensorInfo, SensorCapWidth, MUINT16)
	SET_SENSOR_INFO(sensorDevId, pSensorInfo, SensorCapHeight, MUINT16)
	SET_SENSOR_INFO(sensorDevId, pSensorInfo, SensorVideoWidth, MUINT16)
	SET_SENSOR_INFO(sensorDevId, pSensorInfo, SensorVideoHeight, MUINT16)
	SET_SENSOR_INFO(sensorDevId, pSensorInfo, SensorVideo1Width, MUINT16)
	SET_SENSOR_INFO(sensorDevId, pSensorInfo, SensorVideo1Height, MUINT16)
	SET_SENSOR_INFO(sensorDevId, pSensorInfo, SensorVideo2Width, MUINT16)
	SET_SENSOR_INFO(sensorDevId, pSensorInfo, SensorVideo2Height, MUINT16)
	SET_SENSOR_INFO(sensorDevId, pSensorInfo, SensorCustom1Width, MUINT16)
	SET_SENSOR_INFO(sensorDevId, pSensorInfo, SensorCustom1Height, MUINT16)
	SET_SENSOR_INFO(sensorDevId, pSensorInfo, SensorCustom2Width, MUINT16)
	SET_SENSOR_INFO(sensorDevId, pSensorInfo, SensorCustom2Height, MUINT16)
	SET_SENSOR_INFO(sensorDevId, pSensorInfo, SensorCustom3Width, MUINT16)
	SET_SENSOR_INFO(sensorDevId, pSensorInfo, SensorCustom3Height, MUINT16)
	SET_SENSOR_INFO(sensorDevId, pSensorInfo, SensorCustom4Width, MUINT16)
	SET_SENSOR_INFO(sensorDevId, pSensorInfo, SensorCustom4Height, MUINT16)
	SET_SENSOR_INFO(sensorDevId, pSensorInfo, SensorCustom5Width, MUINT16)
	SET_SENSOR_INFO(sensorDevId, pSensorInfo, SensorCustom5Height, MUINT16)
	SET_SENSOR_INFO(sensorDevId, pSensorInfo, SensorEffectivePreviewWidth, MUINT16)
	SET_SENSOR_INFO(sensorDevId, pSensorInfo, SensorEffectivePreviewHeight, MUINT16)
	SET_SENSOR_INFO(sensorDevId, pSensorInfo, SensorEffectiveCapWidth, MUINT16)
	SET_SENSOR_INFO(sensorDevId, pSensorInfo, SensorEffectiveCapHeight, MUINT16)
	SET_SENSOR_INFO(sensorDevId, pSensorInfo, SensorEffectiveVideoWidth, MUINT16)
	SET_SENSOR_INFO(sensorDevId, pSensorInfo, SensorEffectiveVideoHeight, MUINT16)
	SET_SENSOR_INFO(sensorDevId, pSensorInfo, SensorPreviewWidthOffset, MUINT16)
	SET_SENSOR_INFO(sensorDevId, pSensorInfo, SensorPreviewHeightOffset, MUINT16)
	SET_SENSOR_INFO(sensorDevId, pSensorInfo, SensorCapWidthOffset, MUINT16)
	SET_SENSOR_INFO(sensorDevId, pSensorInfo, SensorCapHeightOffset, MUINT16)
	SET_SENSOR_INFO(sensorDevId, pSensorInfo, SensorVideoWidthOffset, MUINT16)
	SET_SENSOR_INFO(sensorDevId, pSensorInfo, SensorVideoHeightOffset, MUINT16)
	//SET_SENSOR_INFO(sensorDevId, pSensorInfo, SensorVerFOV, MUINT16)
	//SET_SENSOR_INFO(sensorDevId, pSensorInfo, SensorHorFOV, MUINT16)
...
)
{
	...
	// customarize the sensor info
	customSetSensorInfo(sensorDevId, pSensorInfo);

	char value[PROPERTY_VALUE_MAX] = {'\0'};
    property_get("vendor.debug.settle_delay", value, "-1");
    int settleDelay = atoi(value);
	if(settleDelay != -1) {
		pSensorInfo->MIPIDataLowPwr2HSSettleDelayM0 = settleDelay;
		ALOGI("new settle delay:%d\n", settleDelay);
	}
