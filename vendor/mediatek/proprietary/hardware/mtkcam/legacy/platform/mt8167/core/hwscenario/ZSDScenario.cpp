            ...
            , mpExtImgProcHw(NULL)
{
    ...
    MY_LOGD("this=%p, sizeof:%d", this, (int)sizeof(ZSDScenario));
}
...
deque(EHwBufIdx port, vector<PortQTBufInfo> *pBufIn)
{
    ...
    if (port & eID_Pass1DispOut || port & eID_Pass1RawOut)
    {
        ...
        if (port & eID_Pass1RawOut) {
            ...
            {
                MY_LOGE("[eID_Pass1RawOut]mpCamIOPipe->dequeOutBuf fail");
                return false;
            }
            ...
            {
                MY_LOGD_IF(ENABLE_LOG_PER_FRAME, "(va:0x%x) - ", (unsigned int)one.bufInfo.vBufInfo.at(i).u4BufVA);
            }
        }
        if (port & eID_Pass1DispOut) {
            ...
            if( !mpCamIOPipe->dequeOutBuf(rPortID, one.bufInfo) )
            {
                MY_LOGE("[eID_Pass1DispOut]mpCamIOPipe->dequeOutBuf fail");
                return false;
            }
            pBufIn->push_back(one);
            MY_LOGE_IF(one.bufInfo.vBufInfo.size()==0, "Pass 1 deque without buffer");
    ...
    if ((port & eID_Pass2DISPO) || (port & eID_Pass2VIDO))
    {
        ...
        if (port & eID_Pass2DISPO) {
            ...
            {
                MY_LOGE( "dispo: va(0x%08x) != mQ(0x%08x)", (unsigned int)va[0], (unsigned int)it->u4BufVA );
            }
        }
        ...
        if (port & eID_Pass2VIDO){
            ...
            {
                MY_LOGE( "vido: va(0x%08x) != mQ(0x%lu)", (unsigned int)va[0], (unsigned long)it->u4BufVA );
            }
        }
...
replaceQue(vector<PortBufInfo> *pBufOld, vector<PortBufInfo> *pBufNew)
{
    ...
    {
        MY_LOGE("Buffer size not same D=%d E=%d", (int)pBufOld->size(), (int)pBufNew->size());
        return MFALSE;
    }

    for ( int i=0; i < (int)pBufOld->size(); i++) {
...
#undef  FMTCASE
        ...
        case eImgFmt_NV12_BLK: // 420, 2 plane (Y), (VU)
            ...
            break;
        default:
            MY_LOGW(" fmt (0x%x) is not supported", fmt);
            return false;
            break;
    }
