...
FeatureStream(
    ...
    MUINT32 openedSensorIndex,
    NSCam::NSIoPipe::EScenarioFormat scenFmt)    //change to use sensor index, 0xFFFF means pure pass2
            : mpHalPipeWrapper(NULL)
            , mpPostProcPipe(NULL)
            ...
            , mScenFmt(NSCam::NSIoPipe::eScenarioFormat_RAW)
{
   ...
   mScenFmt=scenFmt;
   #endif
   LOG_INF("tag/sidx/sdev(%d/0x%x/0x%x),fmt(%d)",mStreamTag,openedSensorIndex,mOpenedSensor,mScenFmt);
}
    ...
    )
{
    ...
    ret=mpHalPipeWrapper->deque((rParams),this->mpPostProcPipe,0/*(MUINT32)(&mDequeuedBufList)*/,i8TimeoutNs,rFeatureData);
    FUNCTION_LOG_END;
    return ret;
}
