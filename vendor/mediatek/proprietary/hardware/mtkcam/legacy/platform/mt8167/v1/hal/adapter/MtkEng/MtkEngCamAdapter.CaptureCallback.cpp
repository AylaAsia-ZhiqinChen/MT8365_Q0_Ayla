...
{
    MY_LOGD("timestamp(%#" PRIx64 "), size/buf=%u%p", i8Timestamp, u4PostviewSize, puPostviewBuf);
#if 1
    //
    if  ( ! u4PostviewSize || ! puPostviewBuf )
    {
        MY_LOGW("Bad callback: size/buf=%u/%p", u4PostviewSize, puPostviewBuf);
        return  false;
    }
    //
    ...
    {
        MY_LOGW(
            "callback size(%d) != display:[%zu %s %dx%d]",
...
{
    MY_LOGD("timestamp(%#" PRIx64 "), size/buf=%d/%p", i8Timestamp, u4PostviewSize, puPostviewBuf);
...
{
    CAM_LOGD("Raw image size, buf = %d, %p", u4RawImgSize, puRawImgBuf);
...
{
    MY_LOGD(
        "timestamp(%" PRIx64 "), bitstream:size/buf=%d/%p, header:size/buf=%d/%p, index(%d), IsFinalImage(%d)",
