...
createJpegImgWithThumbnail(NSCamHW::ImgBufInfo const &rYuvImgBufInfo, NSCamHW::ImgBufInfo const &rPostViewBufInfo, MUINT32 const Mode)
{
    ...
    if(mpIMemDrv->allocVirtBuf(&jpegBuf))
    {
        MY_LOGE("g_pIMemDrv->allocVirtBuf(&jpegBuf) error \n");
        ret = false;
    }
    ...
    if(mpIMemDrv->allocVirtBuf(&thumbBuf))
    {
        MY_LOGE("g_pIMemDrv->allocVirtBuf(&thumbBuf) error \n");
        ret = false;
    }
    ...
    if(mpIMemDrv->freeVirtBuf(&jpegBuf))
    {
        MY_LOGE("m_pIMemDrv->freeVirtBuf(&jpegBuf) error");
        ret = false;
    }
    if(mpIMemDrv->freeVirtBuf(&thumbBuf))
    {
        MY_LOGE("m_pIMemDrv->freeVirtBuf(&thumbBuf) error");
        ret = false;
    }
