    ...
    //
{
    MY_LOGD(
        ...
        (int)sizeof(BaseCamAdapter), this, &mpCamMsgCbInfo, &mpParamsMgr, &mpImgBufProvidersMgr
    );
}
...
onImgBufProviderCreated(sp<IImgBufProvider>const& rpProvider)
{
    ...
    {
        MY_LOGE("bad ProviderId=%x >= %d", i4ProviderId, (int)mpImgBufProvidersMgr->getProvidersSize());
        return  false;
    }
    //
