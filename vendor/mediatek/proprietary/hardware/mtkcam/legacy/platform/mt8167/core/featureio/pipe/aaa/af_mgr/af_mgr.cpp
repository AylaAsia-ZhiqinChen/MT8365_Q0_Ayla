...
MINT32 AfMgr::CCTOPAFGetNVRAMParam(MVOID *a_pAFNVRAM, MUINT32 *a_pOutLen)
{
    ...
    memcpy( &m_NVRAM_LENS, g_pNVRAM_LENS, sizeof(NVRAM_LENS_PARA_STRUCT));
    if(a_pAFNVRAM)
    {
        memcpy( a_pAFNVRAM, g_pNVRAM_LENS, sizeof(NVRAM_LENS_PARA_STRUCT));
    }
...
MINT32 AfMgr::CCTOPAFApplyNVRAMParam(MVOID *a_pAFNVRAM)
{
    ...
    memcpy(&m_NVRAM_LENS, pAFNVRAM, sizeof(NVRAM_LENS_PARA_STRUCT));
...
MINT32 AfMgr::CCTOPAFSaveNVRAMParam()
{
    ...
    if(g_pNVRAM_LENS)
    {
        memcpy( g_pNVRAM_LENS, &m_NVRAM_LENS, sizeof(NVRAM_LENS_PARA_STRUCT));
    }
...
MVOID AfMgr::TimeOutHandle()
{
    ...
    if(m_i4EnableAF != 1 && m_pIAfAlgo != NULL)
    m_pIAfAlgo->cancelAF();
}
