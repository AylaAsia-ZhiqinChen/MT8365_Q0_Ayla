...
create(MBOOL const enableLog, InitParams const& params)
{
    ...
    //
    if(IHalSensorList::get()->queryType(params.openId) == NSSensorType::eRAW) {
        pPipe = NSPostProc_FrmB::INormalStream::createInstance(LOG_TAG, streamtag, params.openId, true);
    } else if(IHalSensorList::get()->queryType(params.openId) == NSSensorType::eYUV) {
        pPipe = NSPostProc::INormalStream::createInstance(LOG_TAG, streamtag, params.openId, eScenarioFormat_YUV);
    }
    if( pPipe == NULL ) {
...
)
{
    ...
    MERROR ret = OK;
    //
    MBOOL isYuv = IHalSensorList::get()->queryType(mInitParams.openId) == NSSensorType::eYUV;
...

    {
        ...
        crop1.mGroupID    = 1;
        if(isYuv) {
            crop1.mCropRect.s = cropInfos.crop_a.s;
            crop1.mCropRect.p_integral = cropInfos.crop_a.p;
        } else
    ...
    enqueParams.mpPrivaData = (capture_intent);
    //
    ...
    //
    {
        ...
        if(isYuv) {
            if(!mpPipe->deque(enqueParams)) {
                MY_LOGE("deque pass2 failed");
            }
            pass2CbFunc(enqueParams);
        }
    }
    //
    return OK;
 ...
 *
 ******************************************************************************/
#define P2_DEBUG_DUMP_OUTPUT_FOLDER		"/storage/sdcard0/DCIM/Camera/"	
...
pass2CbFunc(QParams& rParams)
{
    if (enableP2RawDump() > 0) {
        if (enableP2RawDump() == DUMP_P2_BUF_ALL
                || enableP2RawDump() == (rParams.mpPrivaData)) {
            for(int i = 0; i < rParams.mvOut.size(); ++i) {
                char filename[256];
                sprintf(filename, P2_DEBUG_DUMP_OUTPUT_FOLDER"P2_%d_%dx%d.yuv", 
                        rParams.mFrameNo,
...
#endif
    //MY_LOGD(" ----P2Node: eisMode: [%d] ----", eisMode);
    return eisMode;
