...
createJpegImgSW(ImgBufInfo const & rSrcImgBufInfo, ImgBufInfo const & rDstImgBufInfo, MUINT32 & u4JpegSize)
{
	...
	ISImager *pISImager = ISImager::createInstance(mThumbImgBufInfoYuv);
	CHECK_OBJECT(pISImager);
	
	pISImager->setTargetBufInfo(rDstImgBufInfo);
	pISImager->setStrideAlign(mThumbImgBufInfoYuv.u4Stride);
	pISImager->setFormat(eImgFmt_JPEG);
	pISImager->setRotation(mShotParam.u4PictureRotation);
	pISImager->setFlip(mShotParam.u4PictureFlip);
	pISImager->setResize(rDstImgBufInfo.u4ImgWidth, rDstImgBufInfo.u4ImgHeight);
	pISImager->setEncodeParam(mJpegParam.fgIsSOI, mJpegParam.u4Quality, JPEGENC_SW);
	pISImager->setROI(Rect(0, 0, mThumbImgBufInfoYuv.u4ImgWidth, mThumbImgBufInfoYuv.u4ImgHeight));
	pISImager->execute();
	u4JpegSize = pISImager->getJpegSize();
	pISImager->destroyInstance();
