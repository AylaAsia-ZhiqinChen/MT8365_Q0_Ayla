...
int CamPathPass2::config( struct CamPathPass2Parameter* p_parameter )
{
    ...
    {
        ISP_PATH_DBG("[imgi] in(%lu,%lu),out(%lu,%lu),imgiCrop(%d,%d,%lu,%lu),CropFloat(0x%x,0x%x)\n", (unsigned long)p_parameter->croppara.cdrz_in.w,
                                                                                       (unsigned long)p_parameter->croppara.cdrz_in.h,
                                                                                       (unsigned long)p_parameter->croppara.cdrz_out.w,
                                                                                       (unsigned long)p_parameter->croppara.cdrz_out.h,
                                                                                       p_parameter->croppara.cdrz_crop.x,
                                                                                       p_parameter->croppara.cdrz_crop.y,
                                                                                       (unsigned long)p_parameter->croppara.cdrz_crop.w,
                                                                                       (unsigned long)p_parameter->croppara.cdrz_crop.h,
    ...
    ISP_PATH_DBG("[imgi](%lu, %lu, %lu) vidoRotate(%d)\n", (unsigned long)p_parameter->imgi.size.w, (unsigned long)p_parameter->imgi.size.h, (unsigned long)p_parameter->imgi.size.stride,ispMdpPipe.wroto_out.Rotation);
    ...
    ISP_PATH_DBG("[imgi_mdp]p2MdpSrcFmt(0x%x),[Pa]dispo(0x%lx) vido(0x%lx) imgi(0x%lx) \n", \
        DMAImgi.p2MdpSrcFmt,(unsigned long)p_parameter->wdmao.memBuf.base_pAddr,(unsigned long)p_parameter->wroto.memBuf.base_pAddr,(unsigned long)p_parameter->imgi.memBuf.base_pAddr);
    ...
    if ( p_parameter->tpipe == CAM_MODE_TPIPE ) {
        ...
        ISP_PATH_DBG("[mdp_src](x,y,w,h)=(%d,%d,%lu,%lu)",ispMdpPipe.src_crop.x,ispMdpPipe.src_crop.y,(unsigned long)ispMdpPipe.src_crop.w,(unsigned long)ispMdpPipe.src_crop.h);
    }
    ...
    if(enVido && endispo) { // vido:(1), dispo(1)
        ...
        if(BASIC_SCALE_RATIO <= wdmaoXCropRatio && BASIC_SCALE_RATIO <= wdmaoYCropRatio) { // scale up
            ...
            ISP_PATH_WRN("[warning]x,y direction is not the same[Imgi]=[%lu, %lu], [dispo]=[%lu, %lu]\n",
                    (unsigned long)pass2Parameter.imgi.size.w,(unsigned long)pass2Parameter.imgi.size.h,(unsigned long)pass2Parameter.wdmao.size.w,(unsigned long)pass2Parameter.wdmao.size.h);
        ...
        ISP_PATH_DBG("[Imgi]=[%lu,%lu],[Imgi_crop]=[%d,%d,%lu,%lu],[dispo]=[%lu,%lu]\n",(unsigned long)pass2Parameter.imgi.size.w,(unsigned long)pass2Parameter.imgi.size.h,
            ...
            (unsigned long)pass2Parameter.wdmao.size.w,(unsigned long)pass2Parameter.wdmao.size.h);
        ...
        if(BASIC_SCALE_RATIO <= wrotoXCropRatio && BASIC_SCALE_RATIO <= wrotoYCropRatio) { // scale up
            ...
            ISP_PATH_WRN("[warning]x,y direction is not the same[ImgiCrop]=[%lu, %lu],[vido]=[%lu, %lu]\n",
                    (unsigned long)pass2Parameter.imgi.crop.w,(unsigned long)pass2Parameter.imgi.crop.h,(unsigned long)pass2Parameter.wroto.size.w,(unsigned long)pass2Parameter.wroto.size.h);
        ...
        ISP_PATH_DBG("[Imgi]=[%lu,%lu],[Imgi_crop]=[%d,%d,%lu,%lu],[vido]=[%lu,%lu]\n",(unsigned long)pass2Parameter.imgi.size.w,(unsigned long)pass2Parameter.imgi.size.h,
            pass2Parameter.imgi.crop.x,pass2Parameter.imgi.crop.y,(unsigned long)pass2Parameter.imgi.crop.w,(unsigned long)pass2Parameter.imgi.crop.h,
            (unsigned long)pass2Parameter.wroto.size.w,(unsigned long)pass2Parameter.wroto.size.h);
