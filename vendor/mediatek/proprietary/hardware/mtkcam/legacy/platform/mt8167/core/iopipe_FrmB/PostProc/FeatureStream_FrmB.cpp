            ...
            , misV3(false)
{
   ...
   LOG_INF("v3(%d) tag/sidx/sdev(%d/0x%x/0x%x),swPipe cID(0x%lx)",misV3,mStreamTag,openedSensorIndex,mOpenedSensor,(unsigned long)(&mDequeuedBufList));
}
...
init()
{
    ...
    if(this->mpPostProcPipe)
    {
        ...
        for(MUINT32 i=0;i<cropPathNum;i++)
        {
            CropPathInfo crop;
    ...
    MINT64 i8TimeoutNs)
{
    ...
    LOG_DBG("sTag(%d),i8TimeoutNs(%lld)",mStreamTag,(long long)i8TimeoutNs);
...
{
    for(MUINT32 i=0;i<mCropPaths.size();i++)
    {

        MCropPathInfo crop;
        crop.mGroupIdx=mCropPaths[i].u4CropGroup;
        for(MUINT32 j=0;j<mCropPaths[i].PortIdxVec.size();j++)
        {
    ...
    android::Vector<MINT32>& vGroupPortIdx)
{
    ...
    LOG_INF("return bcrz(%d)/size(%lu)",beforeCRZ,(unsigned long)vGroupPortIdx.size());
    FUNCTION_LOG_END;
    return true;
}
