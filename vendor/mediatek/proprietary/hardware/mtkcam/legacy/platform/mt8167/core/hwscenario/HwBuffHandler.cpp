...
createBuffer()
{
    ...
    MY_LOGW_IF( mbufInfo.virtAddr & (L1_CACHE_BYTES-1), "bufAddr(%lu) not aligned!", (unsigned long)mbufInfo.virtAddr);
    //
    FUNCTION_OUT;
}
...
removeBuf()
{
    ...
    MY_LOGW_IF(mvHwBufQ.size() < (uint32_t)mbufCnt,
               "the number of to-be-de-allocated buffer is not enough: %d ", (int)mvHwBufQ.size());

    MY_LOGD("clear Hwbuff with size: %d, %d", (int)mvHwBufQ.size(), (int)mvDequedBuf.size());
