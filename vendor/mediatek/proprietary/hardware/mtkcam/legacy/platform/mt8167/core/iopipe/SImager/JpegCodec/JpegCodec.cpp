...
)
{
    ...
    if (checkIfNeedImgTransform(pSrcBufInfo, pDstBufInfo, rROI, u4Transform))
    {
     ...
     /*   do not need this anymore, jpeg encoder hw already can handle this
        {
            ...
            u4AlignedHeight = (~(h_align-1)) & ((h_align-1) + u4AlignedHeight);
                    // need to support 1920x1080 without aligh to 16
            if (pDstBufInfo->getImgSize().w == 1920 && pDstBufInfo->getImgSize().h == 1080)
            {
                u4AlignedWidth = 1920;
                u4AlignedHeight = 1080;
            }
            if (pDstBufInfo->getImgSize().w == 1080 && pDstBufInfo->getImgSize().h == 1920)
            {
                u4AlignedWidth = 1080;
                u4AlignedHeight = 1920;
            }   
