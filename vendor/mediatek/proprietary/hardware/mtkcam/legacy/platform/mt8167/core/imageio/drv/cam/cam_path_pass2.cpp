...
else    // GDMA Mode.
{
    ...
    ISP_PATH_DBG("[imgi] in[%lu, %lu] imgiCrop[%d, %d, %lu, %lu]_f(0x%x, 0x%x)\n",(unsigned long)p_parameter->imgi.size.w,(unsigned long)p_parameter->imgi.size.h, \
                p_parameter->imgi.crop.x,p_parameter->imgi.crop.y,(unsigned long)p_parameter->imgi.crop.w,(unsigned long)p_parameter->imgi.crop.h, \
                p_parameter->imgi.crop.floatX,p_parameter->imgi.crop.floatY);


    if(CAM_CTL_EN2_CDRZ_EN & p_parameter->en_Top.enable2 )
    {
        ISP_PATH_DBG("[imgi] in(%lu,%lu),imgiCrop(%d,%d,%lu,%lu),CropFloat(0x%x,0x%x)\n", (unsigned long)p_parameter->imgi.size.w,
                                                                                      (unsigned long)p_parameter->imgi.size.h,
                                                                                       p_parameter->imgi.crop.x,
                                                                                       p_parameter->imgi.crop.y,
                                                                                       (unsigned long)p_parameter->imgi.crop.w,
                                                                                       (unsigned long)p_parameter->imgi.crop.h,
    ...
    ISP_PATH_DBG("[imgi](%lu , %lu , %lu) vidoRotate(%d)\n", (unsigned long)p_parameter->imgi.size.w, (unsigned long)p_parameter->imgi.size.h, (unsigned long)p_parameter->imgi.size.stride,cdpPipe.vido_out.Rotation);

    ISP_PATH_DBG("isRunSegment(%d) [Pa]dispo(0x%lx) vido(0x%lx) imgi(0x%lx) \n", \
        p_parameter->capTdriCfg.isRunSegment, \
        (unsigned long)p_parameter->dispo.memBuf.base_pAddr,(unsigned long)p_parameter->vido.memBuf.base_pAddr,(unsigned long)p_parameter->imgi.memBuf.base_pAddr);
...
int CamPathPass2::configRingTpipe( struct CamPathPass2Parameter* p_parameter )
{
    ...
    */
    ...
    if ( CAM_CTL_EN2_CURZ_EN & p_parameter->en_Top.enable2 &&
        ...
        //
        ISP_PATH_DBG("[CURZ_in] w(%lu) h(%lu) stride(%lu)\n",(unsigned long)cdpPipe.curz_in.w,(unsigned long)cdpPipe.curz_in.h,(unsigned long)cdpPipe.curz_in.stride);
        ISP_PATH_DBG("[CURZ_out] w(%lu) h(%lu) stride(%lu)\n",(unsigned long)cdpPipe.curz_out.w,(unsigned long)cdpPipe.curz_out.h,(unsigned long)cdpPipe.curz_out.stride);
        //
        ...
        ISP_PATH_DBG("CURZ:in[%lu, %lu] out[%lu, %lu] hCoeffStep(%d) vCoeffStep(%d)\n",
            (unsigned long)cdpPipe.curz_in.w,(unsigned long)cdpPipe.curz_in.h,(unsigned long)cdpPipe.curz_out.w,(unsigned long)cdpPipe.curz_out.h,
        ...
        ISP_PATH_DBG("PRZ:in[%lu, %lu] out[%lu, %lu] hAlgo(%d) vAlgo(%d) hCoeffStep(%d) vCoeffStep(%d)\n",
            (unsigned long)cdpPipe.prz_in.w,(unsigned long)cdpPipe.prz_in.h,(unsigned long)cdpPipe.prz_out.w,(unsigned long)cdpPipe.prz_out.h,
    ...
    ISP_PATH_DBG("[imgi](%lu , %lu , %lu)\n", (unsigned long)p_parameter->imgi.size.w, (unsigned long)p_parameter->imgi.size.h, (unsigned long)p_parameter->imgi.size.stride);
    ...
    ISP_PATH_DBG("Imgi:[%lu, %lu, %lu]",(unsigned long)DMAImgi.dma_cfg.size.w,(unsigned long)DMAImgi.dma_cfg.size.h,(unsigned long)DMAImgi.dma_cfg.size.stride);
    ISP_PATH_DBG("Vipi:[%lu, %lu, %lu]",(unsigned long)DMAVipi.dma_cfg.size.w,(unsigned long)DMAVipi.dma_cfg.size.h,(unsigned long)DMAVipi.dma_cfg.size.stride);
    ISP_PATH_DBG("Vip2i:[%lu, %lu, %lu]",(unsigned long)DMAVip2i.dma_cfg.size.w,(unsigned long)DMAVip2i.dma_cfg.size.h,(unsigned long)DMAVip2i.dma_cfg.size.stride);
    ...
    *pDispVidSel = -1; // set wrong initial value for debug
    ...
    if(enVido && endispo) { // vido:(1), dispo(1)
        ...
        if(BASIC_SCALE_RATIO <= dispoXCropRatio && BASIC_SCALE_RATIO <= dispoYCropRatio) { // scale up
            ...
            //
            ISP_PATH_WRN("[warning]x,y direction is not the same[Imgi]=[%lu, %lu], [dispo]=[%lu, %lu]\n",
                    (unsigned long)pass2Parameter.imgi.size.w,(unsigned long)pass2Parameter.imgi.size.h,(unsigned long)pass2Parameter.dispo.size.w,(unsigned long)pass2Parameter.dispo.size.h);

            ISP_PATH_DBG("[dispo]xAbsRatio(%d) yAbsRatio(%d) AbsRatio(%d) CropRatio(%d)\n",xAbsRatio,yAbsRatio,dispoAbsRatio,dispoCropRatio);
        }

        ISP_PATH_DBG("[Imgi]=[%lu,%lu],[Imgi_crop]=[%d,%d,%lu,%lu],[dispo]=[%lu,%lu]\n",(unsigned long)pass2Parameter.imgi.size.w,(unsigned long)pass2Parameter.imgi.size.h,
            pass2Parameter.imgi.crop.x,pass2Parameter.imgi.crop.y,(unsigned long)pass2Parameter.imgi.crop.w,(unsigned long)pass2Parameter.imgi.crop.h,
            (unsigned long)pass2Parameter.dispo.size.w,(unsigned long)pass2Parameter.dispo.size.h);
        ...
        if(BASIC_SCALE_RATIO <= vidoXCropRatio && BASIC_SCALE_RATIO <= vidoYCropRatio) { // scale up
            ...
            ISP_PATH_WRN("[warning]x,y direction is not the same[ImgiCrop]=[%lu, %lu],[vido]=[%lu, %lu]\n",
                    (unsigned long)pass2Parameter.imgi.crop.w,(unsigned long)pass2Parameter.imgi.crop.h,(unsigned long)pass2Parameter.vido.size.w,(unsigned long)pass2Parameter.vido.size.h);
            ISP_PATH_DBG("[vido]xAbsRatio(%d) yAbsRatio(%d) AbsRatio(%d) CropRatio(%d)\n",xAbsRatio,yAbsRatio,vidoAbsRatio,vidoCropRatio);
        }

        ISP_PATH_DBG("[Imgi]=[%lu,%lu],[Imgi_crop]=[%d,%d,%lu,%lu],[vido]=[%lu,%lu]\n",(unsigned long)pass2Parameter.imgi.size.w,(unsigned long)pass2Parameter.imgi.size.h,
            pass2Parameter.imgi.crop.x,pass2Parameter.imgi.crop.y,(unsigned long)pass2Parameter.imgi.crop.w,(unsigned long)pass2Parameter.imgi.crop.h,
            (unsigned long)pass2Parameter.vido.size.w,(unsigned long)pass2Parameter.vido.size.h);
...
int CamPathPass2::configTpipeData( struct CamPathPass2Parameter* p_parameter )
{
   ...
   // tdriPipe.tdri.top.tcm_load_en =p_parameter->fmt_sel.bit_field.cam_in_fmt;
    ISP_PATH_INF("[Top]scenario(%d) mode(%d) debug_sel(%d) pixel_id(%d) cam_in_fmt(%d),mdp_crop_en(%d)",
