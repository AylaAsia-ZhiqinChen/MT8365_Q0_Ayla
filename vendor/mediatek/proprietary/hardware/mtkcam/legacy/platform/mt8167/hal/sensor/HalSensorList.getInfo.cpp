...
#include "debug/ImgsensorProfiles.h"
...
MVOID HalSensorList::querySensorDrvInfo()
{
    ...
    property_get("vendor.debug.imgsensor.configs", value, "0");
    if(::atoi(value) == 1) {
        PSENSORDRV_INFO_STRUCT infos[3] = {&sensorDrvInfo[0], &sensorDrvInfo[1], &sensorDrvInfo[2]};
        ImgsensorProfiles *profiles = ImgsensorProfiles::getInstance();
        if(profiles) {
            profiles->getSensorDrvInfoFromXmlFile(infos);
        }
    }
    ...
    for(idx=0; idx<1; idx++) {
        ...
        switch (idx) {
            case 0:
                ...
                MY_LOGD("sensor[%d]: previewFrameRate= %d, captureFrameRate= %d,videoFrameRate= %d \n", idx, 
                        sensorStaticInfo[idx].previewFrameRate,sensorStaticInfo[idx].captureFrameRate,sensorStaticInfo[idx].videoFrameRate );
