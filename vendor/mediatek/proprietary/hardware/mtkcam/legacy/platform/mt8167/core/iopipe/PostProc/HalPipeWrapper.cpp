...
PortIdxMappingAlltoP2 mPortIdxMappingAlltoP2[NSImageio::NSIspio::EPortIndex_FDO+1]=
{
    ...
    {NSImageio::NSIspio::EPortIndex_WDMAO,  EPostProcPortIdx_DISPO},
    {NSImageio::NSIspio::EPortIndex_WROTO, EPostProcPortIdx_VIDO},
...
)
{
    ...
    callerPostProc=(MUINTPTR)NSImageio::NSIspio::IPostProcPipe::createInstance(mSwHwPathMapping[swScenario].hwPath,fmt);
    ...
    QFeatureData& rFeatureData)
{
    ...
    {    //original version
        ...
        for(int i=0;i<rParams.mvIn.size();i++)
        {
            ...
            for(int k=0; k < sizeof(mPortIdxMappingAlltoP2)/sizeof(PortIdxMappingAlltoP2); k++) {
                if(rParams.mvIn[i].mPortID.index == mPortIdxMappingAlltoP2[k].ePortIndex) {
                    p2PortIdx = mPortIdxMappingAlltoP2[k].eP2PortIdx;
                    break;
                }
            }
        ...
        for(int i=0;i<rParams.mvOut.size();i++)
        {
            ...
            for(int k=0; k < sizeof(mPortIdxMappingAlltoP2)/sizeof(PortIdxMappingAlltoP2); k++) {
                if(rParams.mvOut[i].mPortID.index == mPortIdxMappingAlltoP2[k].ePortIndex) {
                    p2PortIdx = mPortIdxMappingAlltoP2[k].eP2PortIdx;
                    break;
                }
            }
        ...
        ret = callerPostProc->sendCommand((MINT32)NSImageio::NSIspio::EPIPECmd_SET_CONFIG_STAGE,
                                      (MINTPTR)NSImageio::NSIspio::eConfigSettingStage_UpdateTrigger, 0,0);
        ...
        for(int i=0;i<vPostProcInPorts.size();i++)
        {
            callerPostProc->sendCommand((MINT32)NSImageio::NSIspio::EPIPECmd_SET_CURRENT_BUFFER,
                                  (MINTPTR)(vPostProcInPorts[i]->index),0,0);
        }
        //set pass2 OUT DMA register before pass2 start
        for(int i=0;i<vPostProcOutPorts.size();i++)
        {
            callerPostProc->sendCommand((MINT32)NSImageio::NSIspio::EPIPECmd_SET_CURRENT_BUFFER,
                                  (MINTPTR)(vPostProcOutPorts[i]->index),0,0);
        }
            //start
