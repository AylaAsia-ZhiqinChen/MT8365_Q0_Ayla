...
postCommand(MUINTPTR buf_addr, MUINT32 buf_size)
{
    ...
    //
    {
        ...
        MY_LOGD("+ tid(%d), frame_count (%d), buf_addr(%#" PRIxPTR "), buf_size(%u)", ::gettid(), mFrameCnt, buf_addr, buf_size);


        if (!mCmdQ.empty())
        {
            MY_LOGD("queue is not empty, (%d) is in the head of queue, Q size (%zu)", ((*mCmdQ.begin())->getSlotIndex()), mCmdQ.size());
        }
        ...
        {
            MY_LOGD("- frame dropped:  tid(%d), frame_count(%d), Q size(%zu)", ::gettid(), mFrameCnt, mCmdQ.size());
        ...
        else
        {
            ...
            mCmdQ.push_back(cmdCookie);
            mCmdCond.broadcast();
            MY_LOGD("- frame added:  tid(%d), slot_index(%d), frame_count(%d), que size(%zu)", ::gettid(),i, mFrameCnt, mCmdQ.size());
        }
    }
    //
    FUNCTION_OUT;
...
RawDumpCmdQueThread::threadLoop()
{
    ...
    if (getCommand(pCmdCookie))
    {
        ...
        if (ret==false)
        {
            ...
            //mpCamMsgCbInfo->mNotifyCb(MTK_CAMERA_MSG_EXT_NOTIFY, MTK_CAMERA_MSG_EXT_NOTIFY_RAW_DUMP_STOPPED, 0, 0);
        #endif
        }
