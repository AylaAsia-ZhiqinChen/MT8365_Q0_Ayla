...
{
    MINT32 ret = m_pGgmMgr->setTonemapCurve_Red(p_in_red, p_out_red, pCurvePointNum);

    return  ((ret == GGM_ERROR) ? MERR_BAD_PARAM : MERR_OK);
}
...
{
    MINT32 ret = m_pGgmMgr->setTonemapCurve_Green(p_in_green, p_out_green, pCurvePointNum);
...
{
    MINT32 ret = m_pGgmMgr->setTonemapCurve_Blue(p_in_blue, p_out_blue, pCurvePointNum);
