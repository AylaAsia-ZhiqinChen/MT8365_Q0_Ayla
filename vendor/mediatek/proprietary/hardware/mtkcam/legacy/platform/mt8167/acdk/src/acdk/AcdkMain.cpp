...
MINT32 AcdkMain::init()
{
    ...
    if(mOperationMode == ACDK_CCT_MODE && m_pSensorHalObj != NULL)
                ...
                MINT32  const isSaveImg)
{
    ...
    if(capCb != NULL)
    {
        ...
        if(mCapType & 0x3C)
        {
            ...
            {
                sprintf(szFileName, "%s/pure__%dx%d_10_%d", mCctOn? MEDIA_PATH_CCT:MEDIA_PATH,
                    acdkCapInfo.RAWImgBufInfo.imgWidth,acdkCapInfo.RAWImgBufInfo.imgHeight,sensorStaticInfo[0].sensorFormatOrder);
            }
            else
            {
                sprintf(szFileName, "%s/proc__%dx%d_10_%d", mCctOn? MEDIA_PATH_CCT:MEDIA_PATH,
                    acdkCapInfo.RAWImgBufInfo.imgWidth,acdkCapInfo.RAWImgBufInfo.imgHeight,sensorStaticInfo[0].sensorFormatOrder);
            }
            ...
            if(NULL == pFp )
            {
                ACDK_LOGE("Can't open file to read image:%s",szFileName);
                //fclose(pFp);
                return ACDK_RETURN_NULL_OBJ;
            }
            ...
            if(mUnPack == MTRUE) //for specail mode use : unpack raw image to normal package
            {
                ...
                {
                case 8 : 
                    tempFmt  = eImgFmt_BAYER8;
                    break;
                case 10 : 
                    tempFmt = eImgFmt_BAYER10;
                    break;
                case 12 : 
                    tempFmt = eImgFmt_BAYER12;
                    break;
                default : 
                    tempFmt = eImgFmt_UNKNOWN;
                    ACDK_LOGE("unknown raw image bit depth(%u)",mSensorFormatInfo.u4BitDepth);
                    break;
                }
...
MINT32 AcdkMain::handlePreviewCB(MVOID *param)
{
    ...
    ACDK_LOGD_DYN(g_acdkMainDebug,"index(%u),VA(%p),PA(0x%x),Size(0x%x)",mSurfaceIndex,(int)mSurfaceIMemInfo[mSurfaceIndex].virtAddr,(int)mSurfaceIMemInfo[mSurfaceIndex].phyAddr,mSurfaceIMemInfo[mSurfaceIndex].size);
