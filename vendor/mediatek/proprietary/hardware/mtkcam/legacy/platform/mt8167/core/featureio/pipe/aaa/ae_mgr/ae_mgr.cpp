        ...
        if(pAEStatisticBuf != NULL)  {
          ...
          localTimer.End();
            ...
            if((m_bIsAutoFlare == MTRUE) && ((bAAASchedule == 0)||(i4ActiveAEItem & E_AE_FLARE))){
                ...
                mPreviewMode.i2FlareOffset = i2FlareOffset;
                mPreviewMode.i2FlareGain = i2FlareGain;
...
#endif
                MY_LOG_IF(m_3ALogEnable, "[%s:e] i4SensorDev:%d VDNum %d, Delay %d/%d/%d, Prev %d/%d/%d, Cur %d/%d/%d, Output %d/%d/%d State:%d FrameRate:%d %d %d Flare Offset/Gain:%d %d", __FUNCTION__, m_eSensorDev, m_i4WaitVDNum,
