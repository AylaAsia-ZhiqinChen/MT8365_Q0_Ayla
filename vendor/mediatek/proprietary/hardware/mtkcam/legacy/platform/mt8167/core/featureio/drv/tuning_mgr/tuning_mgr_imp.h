...
#if MTKCAM_LOGENABLE_DEFAULT
#warning [----userdebug above]
#define  DBG_LOG_LEVEL      4               // 2(VERBOSE)/3(DEBUG)/4(INFO)/5(WARN)/6(ERROR)/7(ASSERT)/8(SILENT).
#else
#warning [----user load]
#define  DBG_LOG_LEVEL      8               // 2(VERBOSE)/3(DEBUG)/4(INFO)/5(WARN)/6(ERROR)/7(ASSERT)/8(SILENT).
#endif
...
//    #include <cutils/log.h>
    #define NEW_LINE_CHAR           ""      // ALOG?() already includes a new line char at the end of line, so don't have to add one.
        ...
        #undef LOG_ERR
        #define LOG_ERR(fmt, arg...)        CAM_LOGE(DBG_LOG_TAG "[%s, %s, line%04d] ERROR: " fmt NEW_LINE_CHAR, __FILE__, __func__, __LINE__, ##arg)    // When MP, will only show log of this level. // <Fatal>: Serious error that cause program can not execute. <Error>: Some error that causes some part of the functionality can not operate normally.
    #endif

    #if (DBG_LOG_LEVEL <= DBG_LOG_LEVEL_WARN)        // 5
        #undef LOG_WRN
        #define LOG_WRN(fmt, arg...)        CAM_LOGW(DBG_LOG_TAG "[%s] " fmt NEW_LINE_CHAR, __func__, ##arg)    // <Warning>: Some errors are encountered, but after exception handling, user won't notice there were errors happened.
    #endif

    #if (DBG_LOG_LEVEL <= DBG_LOG_LEVEL_INFO)        // 4
        #undef LOG_INF
        #define LOG_INF(fmt, arg...)        CAM_LOGI(DBG_LOG_TAG "[%s] " fmt NEW_LINE_CHAR, __func__, ##arg)    // <Info>: Show general system information. Like OS version, start/end of Service...
    #endif

    #if (DBG_LOG_LEVEL <= DBG_LOG_LEVEL_DEBUG)        // 3
        #undef LOG_DBG
        #define LOG_DBG(fmt, arg...)        CAM_LOGD(DBG_LOG_TAG "[%s] " fmt NEW_LINE_CHAR, __func__, ##arg)    // <Debug>: Show general debug information. E.g. Change of state machine; entry point or parameters of Public function or OS callback; Start/end of process thread...
    #endif

    #if (DBG_LOG_LEVEL <= DBG_LOG_LEVEL_VERBOSE)    // 2
        #undef LOG_VRB
      #define LOG_VRB(fmt, arg...)        CAM_LOGV(DBG_LOG_TAG "[%s] " fmt NEW_LINE_CHAR, __func__, ##arg)    // <Verbose>: Show more detail debug information. E.g. Entry/exit of private function; contain of local variable in function or code block; return value of system function/API...
     #endif
#else   // Using LDVT.
    #include "uvvf.h"
