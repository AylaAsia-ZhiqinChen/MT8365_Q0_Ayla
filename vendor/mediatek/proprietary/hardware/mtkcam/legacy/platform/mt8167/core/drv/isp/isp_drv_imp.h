...
class IspDrvVirImp : public IspDrv
{
    public:
        ...
        virtual MBOOL  waitIrq(ISP_DRV_WAIT_IRQ_STRUCT WaitIrq){(void)WaitIrq;/*LOG_ERR("Not support");*/return MTRUE;}
        virtual MBOOL  readIrq(ISP_DRV_READ_IRQ_STRUCT *pReadIrq){(void)pReadIrq;/*LOG_ERR("Not support");*/return MTRUE;}
        virtual MBOOL  checkIrq(ISP_DRV_CHECK_IRQ_STRUCT CheckIrq){(void)CheckIrq;/*LOG_ERR("Not support");*/return MTRUE;}
        virtual MBOOL  clearIrq(ISP_DRV_CLEAR_IRQ_STRUCT ClearIrq){(void)ClearIrq;/*LOG_ERR("Not support");*/return MTRUE;}
        virtual MBOOL  reset();
        ...
        virtual MBOOL   writeRegs(ISP_DRV_REG_IO_STRUCT *pRegIo, MUINT32 Count);
        virtual MBOOL   writeReg(MUINT32 Addr, MUINT32 Data);
        virtual MBOOL   holdReg(MBOOL En){(void)(En);/*LOG_ERR("Not support");*/return MTRUE;}
        virtual MBOOL   dumpReg(){/*LOG_ERR("Not support");*/return MTRUE;}
        virtual MBOOL   IsReadOnlyMode(){return MFALSE;}
        virtual MBOOL   ISPWakeLockCtrl(MBOOL WakeLockEn) {if(WakeLockEn){} return MTRUE;};
//      virtual MBOOL   IsGdmaMode(){return MFALSE;}
        virtual MBOOL   rtBufCtrl(MVOID *pBuf_ctrl){(void)(pBuf_ctrl);return MTRUE;}
