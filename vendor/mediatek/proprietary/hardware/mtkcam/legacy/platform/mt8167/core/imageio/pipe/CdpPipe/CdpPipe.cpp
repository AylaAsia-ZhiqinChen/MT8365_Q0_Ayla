...
init()
{
    ...
    if (m_pIspDrvShell->m_pIMemDrv)    // Only do following steps when m_pIspDrvShell->m_pIMemDrv is not NULL (which implicitly imply it's non-GDMA mode.).
    {
        ...
        PIPE_DBG("ALLOC pTdriVir(0x%lx) tdriPhy(0x%x)\n",(unsigned long)pTdriVir,tdriPhy);
        //
        ...
        PIPE_DBG("ALLOC pTpipeConfigVa(0x%lx)\n",(unsigned long)pTpipeConfigVa);
        //
        tdriRingPhy = 0;
        pTdriRingVir = NULL;
        //
        ...
        //
        PIPE_DBG("m_pIspDrvShell(0x%lx) m_resMgr(0x%lx) pTdriVir(0x%lx) tdriPhy(0x%x) configVa(0x%lx)",
            (unsigned long)m_pIspDrvShell,(unsigned long)m_resMgr,(unsigned long)pTdriVir,tdriPhy,(unsigned long)pTpipeConfigVa);
        //
    }
...
uninit()
{
    ...
    if (m_pIspDrvShell->m_pIMemDrv)    // Only do following steps when m_pIspDrvShell->m_pIMemDrv is not NULL (which implicitly imply it's non-GDMA mode.).
    {
        ...
        PIPE_DBG("tdriRingPhy(0x%x) pTdriRingVir(0x%lx)\n",tdriRingPhy,(unsigned long)pTdriRingVir);
    ...
    if (-1 != idx_imgi ) {
        ...
        if(m_camPass2Param.imgi.ring_en) {
            ...
            if(pTdriRingVir==NULL && tdriRingPhy==0) {
                ...
                PIPE_DBG("ALLOC pTdriRingVir(0x%lx) tdriRingPhy(0x%x)\n",(unsigned long)pTdriRingVir,tdriRingPhy);
            }
