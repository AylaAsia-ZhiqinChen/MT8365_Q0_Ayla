...
MBOOL TpipeDrvImp::init(void)
{
    ...
    if(mInitCount == 0) {
        ...
        if ( NULL == m_pImemDrv ) {
            ...
            LOG_ERR("[Error] m_pImemDrv(0x%lx) not equal to null\n",(unsigned long)m_pImemDrv);
        }
        //
        ...
        if(pConfigTpipeStruct == 0) {
            ...
            fdConfigTpipeStruct = -1;
            Result = MFALSE;
            LOG_ERR("pConfigTpipeStruct not 4 alignment pConfigTpipeStruct(0x%lx) tpipeAlgoStructSize(%d)\n",(unsigned long)pConfigTpipeStruct,tpipeAlgoStructSize);
            goto EXIT;
        }
        ...
        for(i =TPIPE_DRV_CQ01; i < TPIPE_DRV_CQ_NUM;i++) {
            ...
            if(pKeepTdriInfo[i] == 0) {
                ...
                fdKeepTpipeInfo = -1;
                Result = MFALSE;
                LOG_ERR("pConfigTpipeStruct not 4 alignment pKeepTdriInfo[%d](0x%lx) tpipeAlgoStructSize(%d)\n",i,(unsigned long)(pKeepTdriInfo[i]),tpipeAlgoStructSize);
                goto EXIT;
            }
...
MBOOL TpipeDrvImp::uninit(void)
{
    ...
    if(mInitCount == 0) {
        ...
        for(i =TPIPE_DRV_CQ01; i < TPIPE_DRV_CQ_NUM;i++) {
            ...
            m_KeepTpipeInfo[i].virtAddr = 0x0;
        }
