...
configMdpOutPort(PortInfo const* oImgInfo, MdpRotDMACfg &a_rotDma)
{
    ...
    a_rotDma.crop.h = 0;
    //
    PIPE_DBG("[a_rotDma]w(%lu),h(%lu),stride(pxl)(%lu),pa(0x%lx),va(0x%lx),size(%d)",(unsigned long)a_rotDma.size.w,(unsigned long)a_rotDma.size.h,(unsigned long)a_rotDma.size.stride,(unsigned long)a_rotDma.memBuf.base_pAddr,(unsigned long)a_rotDma.memBuf.base_vAddr,a_rotDma.memBuf.size);
    PIPE_DBG("[a_rotDma]crop(%d,%d,%lu,%lu)_f(0x%x,0x%x)",a_rotDma.crop.x,a_rotDma.crop.y,(unsigned long)a_rotDma.crop.w,(unsigned long)a_rotDma.crop.h,a_rotDma.crop.floatX,a_rotDma.crop.floatY);
...
PipeImp::configDmaPort(PortInfo const* portInfo,IspDMACfg &a_dma,MUINT32 pixel_Byte,MUINT32 swap, MUINT32 isBypassOffset,EIMAGE_STRIDE planeNum)
{
    ...
    if ( a_dma.size.stride<a_dma.size.w &&  planeNum==ESTRIDE_1ST_PLANE) {
        PIPE_ERR("[Error]:stride size(%lu) < image width(%lu) byte size",(unsigned long)a_dma.size.stride,(unsigned long)a_dma.size.w);
    }
    //
    ...
    PIPE_DBG("w(%lu),h(%lu),stride(%lu),xsize(0x%lx),crop(%d,%d,%lu,%lu)_f(0x%x,0x%x),ofst_addr(0x%x),pb((0x%x)(+<<2)),va(0x%lx),pa(0x%lx),pixel_Byte(%d)",(unsigned long)a_dma.size.w,(unsigned long)a_dma.size.h,(unsigned long)a_dma.size.stride, \
                                                        (unsigned long)a_dma.size.xsize,a_dma.crop.x,a_dma.crop.y,(unsigned long)a_dma.crop.w,(unsigned long)a_dma.crop.h,\
