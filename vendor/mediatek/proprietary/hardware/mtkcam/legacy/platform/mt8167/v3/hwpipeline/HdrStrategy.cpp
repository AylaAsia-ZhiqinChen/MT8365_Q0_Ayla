    ...
    operator()(Params& rParams, IEvaluator const* pEvaluator)
    {
        ...
        for ( uint32_t i=0; i<rParams.subFrameCnt; i++ ) {
            ...
            if ( i==0 ) {   // main frame w/ callback
                pFrame = PipelineFrameT::create(rParams.requestNo, rParams.frameNo+i, 
                                                rParams.pAppCallback, 
                                                pEvaluator->getStreamBufferProvider(), 
                                                pEvaluator->getPipelineNodeCallback() );
