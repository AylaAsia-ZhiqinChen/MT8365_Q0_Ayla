...
#define LOG_TAG "lsc_mgr2_misc_v3"
...
dumpSdblk(const char* table_name, const ISP_NVRAM_LSC_T& LscConfig, const MUINT32 *ShadingTbl)
{
    ...
    LSC_LOG_BEGIN("---- EM: strTblName(%s)", strTblName.c_str());
...
getDebugInfo(DEBUG_RESERVEA_INFO_T &rShadingDbgInfo)
{
...

    {
        ...
        {
            BOOL fgdumpStbl = 0;
            GET_PROP("vendor.debug.em.dump.stbl", "0", fgdumpStbl);
            if(fgdumpStbl)
            {
                char value[128] = {'\0'};
                property_get("vendor.debug.em.filename", value, "0");
                LSC_LOG("---- shading_table_path = %s", value);
                CCTOPSetSdblkFileCfg(MTRUE, value); 
            }
            else
            {
                CCTOPSetSdblkFileCfg(MFALSE, "");
            }
            LSC_LOG("---- EM: m_bDumpSdblk(%d), m_strSdblkFile(%s)", m_bDumpSdblk, m_strSdblkFile.c_str());
        }
    }
