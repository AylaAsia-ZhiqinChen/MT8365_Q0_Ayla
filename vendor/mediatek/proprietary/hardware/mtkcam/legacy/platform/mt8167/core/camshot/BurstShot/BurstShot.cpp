...
createSensorRawImgN(SensorParam const & rSensorParam, ImgBufInfo *rRawImgBufInfo, MUINT32 u4ShotCount, MUINT32 delayFrame)
{
    ...
    for(MUINT32 i=0; i<u4ShotCount; i++) {
        ...
        rQRawOutBuf.vBufInfo.clear();
        mpCamIOPipe->dequeBuf(PortID(EPortType_MemoryOut, 0, 1), rQRawOutBuf);
        MY_LOGD("camio deque VA should be 0x%x", (unsigned int)rRawImgBufInfo[i].u4BufVA);
...
createSensorRawImgN_FixedExposure(SensorParam const & rSensorParam, ImgBufInfo *rRawImgBufInfo, MUINT32 u4ShotCount, MUINT32 delayFrame)
{
    ...
    for(MUINT32 i=0; i<u4VirtualShotCount; i++) {
        ...
        rQRawOutBuf.vBufInfo.clear();
        mpCamIOPipe->dequeBuf(PortID(EPortType_MemoryOut, 0, 1), rQRawOutBuf);
        MY_LOGD("camio dequeBuf VA should be 0x%x", (unsigned int)virtualImgBufInfo[i].u4BufVA);
...
registerImgBufInfo(ECamShotImgBufType const eBufType, ImgBufInfo *pImgBuf, MUINT32 length)
{
     ...
     for(MUINT32 i=0; i<length; i++) {
         ...
         MY_LOGD("[registerImgBufInfo][%d] (VA, PA, Size, ID) = (0x%x, 0x%x, %d, %d)", i, (unsigned int)pImgBuf[i].u4BufVA, (unsigned int)pImgBuf[i].u4BufPA, pImgBuf[i].u4BufSize, pImgBuf[i].i4MemID);
     }
...
MBOOL
BurstShot::
registerCap3AParam(NS3A::CaptureParam_T *pCap3AParam,__attribute__((unused)) MUINT32 length)
{
...
allocImgMem(char const* const pszName, EImageFormat const eFmt, MUINT32 const u4Width, MUINT32 const u4Height, IMEM_BUF_INFO & rMem)
{
    ...
    if (0 == rMem.size)
    {
        ...
        allocMem(rMem);
        MY_LOGD("[allocImgMem] (va, pa, size) = (0x%x, 0x%x, %d)",  (unsigned int)rMem.virtAddr, (unsigned int)rMem.phyAddr, rMem.size);
    }
    else
    {
        if (rMem.size < u4BufSize)
        {
            ...
            MY_LOGD("[allocImgMem] re-allocate (va, pa, size) = (0x%x, 0x%x, %d)", (unsigned int)rMem.virtAddr, (unsigned int)rMem.phyAddr, rMem.size);
        }
    }
    profile.print();
    return MTRUE;
