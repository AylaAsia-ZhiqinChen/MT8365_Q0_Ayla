...
DFS(size_t userId, android::Vector<android::sp<MyUser> >& result) const
{
    ...
    if(0>idx)
    {
        MY_LOGE("userId:%#" PRIxPTR " find idx less than 0 (idx=%d)", userId, idx);
        return BAD_VALUE;
    }
...
queryUser_(UserId_T const userId) const
{
    ...
    if  ( (ssize_t)mUserGroupGraph.size() <= pUser->mGroupIndex ) {
        MY_LOGE(
            "userId:%#" PRIxPTR " has groupIndex:%zu > mUserGroupGraph.size:%zu",
...
)
{
    ...
    {
        MY_LOGW(
            "the same release fence:%s(%d) is set to userId:%#" PRIxPTR ,
...
)   const
{
    ...
    case Category::CONSUMER:{
        ...
        if ( mUserGroupGraph.size() <= groupIndex ) {
            MY_LOGE(
                "[userId:%#" PRIxPTR "] groupIndex:%zu > mUserGroupGraph.size:%zu",
