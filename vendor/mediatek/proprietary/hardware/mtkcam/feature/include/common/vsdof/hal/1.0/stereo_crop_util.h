...
public:
    ...
    static MRect &cropRectByFOV(MRect &rect, const float CROP_RATIO, ENUM_STEREO_RATIO IMAGE_RATIO)
    {
        if(!StereoSettingProvider::isBMVSDoF() ||
           CROP_RATIO >= 1.0f ||
           CROP_RATIO < 0.0f)
        {
            return rect;
        ...
        //To make sure targetSize will be even and fit aspect ratio
        n <<= 1;
        m <<= 1;
        ...
        {
            int baseSize = (int)(rect.s.w * CROP_RATIO) / m;
            targetSize.w = baseSize * m;
            targetSize.h = baseSize * n;
        } else {
            int baseSize = (int)(rect.s.h * CROP_RATIO) / n;
            targetSize.h = baseSize * n;
            targetSize.w = baseSize * m;
        }

        CUST_FOV_CROP_T cropSetting = getFOVCropSetting(StereoSettingProvider::stereoProfile());

        // 0 degree
        if( eRotate_0 == ROTATION )
        {
            const int TOTAL_CROP_SPACE = rect.s.w - targetSize.w;
            ...
            rect.p.y += (rect.s.h - targetSize.h)/2;
        }
        // 180
        ...
        {
            const int TOTAL_CROP_SPACE = rect.s.h - targetSize.h;
            ...
            rect.p.x += (rect.s.w - targetSize.w)/2;
        }
        // 270
        else
        {
