...
int main(int argc, char **argv)
{
    ...
    if( argc == 2 )
    {
        ...
        CAM_LOGD("Use user-specifying test version = %d", ver_num);
    }
    else
    {
        CAM_LOGW("Unknown test version specifying, use default = 0");
    }
...
#undef ROUND_TO_2X
    ...
    do
    {
        ...
        if( ver_num == 1 )
        {
            if( EIS_RETURN_NO_ERROR != mpEisHal->ConfigFFFMEis(eisHalConfigData, eisMode) )
            {
                CAM_LOGE("Test ConfigFFFMEis fail");
                return MFALSE;
            }
        }
        else
        {
            ...
            {
                CAM_LOGE("Test ConfigRSCMEEis fail");
                return MFALSE;
            }
        }
