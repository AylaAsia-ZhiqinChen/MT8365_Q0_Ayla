...
{
    CAM_LOGD("%s - sensor ID:%d", userName, aSensorIdx);
...
{
    CAM_LOGD("%s - sensor ID:%d is destroyed", userName, mSensorIdx);
    delete this;
}
...
MINT32 GisCalibrationImp::Init()
{
    ...
    if(mUsers > 0)
    {
        ...
        CAM_LOGD("snesorIdx(%u) has %d users",mSensorIdx,mUsers);
    ...
    if (m_pGisAlg == NULL)
    {
        CAM_LOGE("MTKGyro::createInstance fail");
        goto create_fail_exit;
    }
...
MINT32 GisCalibrationImp::Uninit()
{
    ...
    if(mUsers == 0)    // there is no user
    {
        ...
        {
            CAM_LOGD("mIsEisPlusConfig(%d)",mIsEisPlusConfig);
            if(mIsEisPlusConfig == 1)
            {
                err = m_pGisAlg->GyroFeatureCtrl(GYRO_FEATURE_SAVE_LOG, (MVOID *)&mTsForAlgoDebug, NULL);
                if(err != S_GYRO_OK)
                {
                    CAM_LOGE("GyroFeatureCtrl(GYRO_FEATURE_SAVE_LOG) fail(0x%x)",err);
                }
            }
        }
        ...
        {
            CAM_LOGD("mpSensorProvider uninit");
    ...
    {
        CAM_LOGD("mSensorIdx(%u) has %d users",mSensorIdx,mUsers);
    }
...
MINT32 GisCalibrationImp::GetSensorInfo(MUINT32 sensorID)
{
    ...
    CAM_LOGD("[mSensorInfo] sensorIdx(%u), mSensorDev(%u)", sensorID, mSensorInfo[sensorID].mSensorDev);
    ...
    if (LIKELY(m_pHalSensor != NULL))
    {
        ...
        {
            CAM_LOGE("SENSOR_CMD_GET_PIXEL_CLOCK_FREQ is fail(0x%x)",err);
    ...
    {
        CAM_LOGE("m_pHalSensorList->createSensor fail, m_pHalSensor == NULL");
    }

    CAM_LOGD("mSensorDev(%u), pixelclock (%d), pixelline(%x)", mSensorInfo[sensorID].mSensorDev,
...
MINT32 GisCalibrationImp::EnableSensor()
{
    ...
    CAM_LOGD("EN:(Acc,Gyro)=(%d,%d)",mAccEnable,mGyroEnable);
...
MINT32 GisCalibrationImp::CalcSensorTrs(MUINT32 sensorID, MUINT32 sensorHeight)
{
    ...
    {
        CAM_LOGW("mSensorPixelClock is 0, so can NOT get tRS");
    }

    CAM_LOGD("tRS in table: %f, calculated tRS: %f", m_pNVRAM_defParameter->gis_deftRS[0], tRS);
    ...
    if (tRS > 0.042f)
    {
        CAM_LOGW("30 fps tRS+tOffset: %f should be small than 0.042 ms", tRS);
    }
    mSensorInfo[sensorID].mtRSTime = (MINT64)((MDOUBLE)tRS*1000000.0f);
    CAM_LOGD("waiting gyro time: %" PRIi64 " ", mSensorInfo[sensorID].mtRSTime);
    return EIS_RETURN_NO_ERROR;
}
...
MINT32 GisCalibrationImp::ConfigCalibration(const LMV_HAL_CONFIG_DATA &aLMVConfig)
{
    ...
    {
        CAM_LOGD("mSensorIdx(%u) not support EIS",mSensorIdx);
        return EIS_RETURN_NO_ERROR;
    }
    ...
    {
        EnableSensor();
        CAM_LOGD("GIS first config for calibration (%u)", aLMVConfig.sensorType);
        ...
        {
            CAM_LOGE("m_pNVRAM_defParameter OR m_pNVRAM_defParameter is NULL\n");
            return  EIS_RETURN_NULL_OBJ;
        }
        mNVRAMRead = MTRUE;
             ...
             (mSensorInfo[mSensorIdx].mSensorDev == SENSOR_DEV_SUB_2))
        {
            ...
            CAM_LOGD("mSensorDev(%u), GYRO data reversed",mSensorInfo[mSensorIdx].mSensorDev);
        }else
        {
            mGyroData[mSensorIdx].mGyroReverse = 0;
            CAM_LOGD("mSensorDev(%u), GYRO data normal",mSensorInfo[mSensorIdx].mSensorDev);
        }
        ...
        CAM_LOGD("def data 0: %f    %f    %f    %f    %f    %f", m_pNVRAM_defParameter->gis_defParameter1[0], m_pNVRAM_defParameter->gis_defParameter1[1],
        ...
        CAM_LOGD("def data 1: %f    %f    %f    %f    %f    %f", m_pNVRAM_defParameter->gis_defParameter2[0], m_pNVRAM_defParameter->gis_defParameter2[1],
        ...
        CAM_LOGD("def data 2: %f    %f    %f    %f    %f    %f", m_pNVRAM_defParameter->gis_defParameter3[0], m_pNVRAM_defParameter->gis_defParameter3[1],
        ...
        CAM_LOGD("sleep_t is (%" PRIu64 ")", gyroAlgoInitData.sleep_t);
        CAM_LOGD("rrz crop w(%d),h(%d)", gyroAlgoInitData.rrz_crop_Width, gyroAlgoInitData.rrz_crop_Height);
        CAM_LOGD("rrzOut w(%d),h(%d)", gyroAlgoInitData.rrz_scale_Width, gyroAlgoInitData.rrz_scale_Height);
        CAM_LOGD("crz offset x(%d),y(%d)", gyroAlgoInitData.crz_crop_X, gyroAlgoInitData.crz_crop_Y);
        CAM_LOGD("crzOut w(%d),h(%d)", gyroAlgoInitData.crz_crop_Width, gyroAlgoInitData.crz_crop_Height);
        CAM_LOGD("aEisConfig  PIXEL_MODE(%d),op h(%d), opv(%d), rp h(%d), rp v(%d)", gyroAlgoInitData.GyroCalInfo.PIXEL_MODE,
        ...
        if(err != S_GYRO_OK)
        {
            CAM_LOGE("GyroInit fail(0x%x)",err);
            return EIS_RETURN_API_FAIL;
        }
        ...
        if(err != S_GYRO_OK)
        {
            CAM_LOGE("get Gyro proc info fail(0x%x)",err);
            return EIS_RETURN_API_FAIL;
        }
        ...
        {
            CAM_LOGE("m_pGisWorkBuf create ImageBuffer fail");
        ...
        if(err != S_GYRO_OK)
        {
            ...
            CAM_LOGE("mGisWorkBuf GYRO_FEATURE_SET_WORK_BUF_INFO fail");
            return EIS_RETURN_API_FAIL;
        }
        mIsEisPlusConfig = 1;
    }
...
MINT32 GisCalibrationImp::DoCalibration(LMV_HAL_CONFIG_DATA *apLMVConfig, MINT64 aTimeStamp, MINT64 aExpTime)
{
    ...
    //CAM_LOGD("crz offset x(%d),y(%d)", gyroSetProcData.crz_crop_X, gyroSetProcData.crz_crop_Y);
    //CAM_LOGD("crzOut w(%d),h(%d)", gyroSetProcData.crz_crop_Width, gyroSetProcData.crz_crop_Height);

    CAM_LOGD("frame_AE(%d), rrz_crop_X(%d), rrz_crop_Y(%d), rrz_crop_Width(%d), rrz_crop_Height(%d), rrz_out_Width(%d), rrz_out_Height(%d)",
    ...
    if (1)
    {
        ...
        if(err != S_GYRO_OK)
        {
            CAM_LOGE("GIS:GYRO_FEATURE_SET_PROC_INFO fail(0x%x)",err);
        ...
        if(err != S_GYRO_OK)
        {
            CAM_LOGE("GIS:GyroMain fail(0x%x)",err);
            err = EIS_RETURN_API_FAIL;
            return err;
        }
    }else
    ...
    {
        CAM_LOGD("Cal:frame(%d) valid (%d) , %f    %f    %f    %f    %f    %f", mDoEisCount, gyroCal.dataValid, gyroCal.paramFinal[0], gyroCal.paramFinal[1], gyroCal.paramFinal[2],
    ...
    {
        CAM_LOGD("Cal:frame(%d), Initial W/H (%d/%d) , Current W/H    (%d/%d)", mDoEisCount, mGisInputW, mGisInputH, aWidth, aHeight);
    }
    ...
    if (UNLIKELY(mEMEnabled == 1))
    {
        if ((gyroCal.dataValid == 0) || (gyroCal.dataValid == 2) || (gyroCal.dataValid == 5))
        {
            if (mbEMSaveFlag == MFALSE)
            {
                ...
                if(err != S_GYRO_OK)
                {
                    CAM_LOGE("GyroFeatureCtrl(GYRO_FEATURE_SAVE_EM_INFO) fail(0x%x)",err);
                }
                mbEMSaveFlag = MTRUE;
            }
        }
...
MVOID GisCalibrationImp::GetSensorData(GYRO_SET_PROC_INFO_STRUCT& gyroSetProcData)
{
    ...
    for (MUINT32 i = 0; i < sensorData.size(); i++)
    {
             ...
             (sensorData[index].gyro[2] < UNREASONABLE_GYRO_VALUE) && (sensorData[index].gyro[2] > -UNREASONABLE_GYRO_VALUE)))
        {
            ...
            if (UNLIKELY(mDebugDump >= 1))
            {
                CAM_LOGD("Gyro(%f,%f,%f,%" PRIi64 ")",sensorData[index].gyro[0], sensorData[index].gyro[1],
        ...
        {
            CAM_LOGE("Unreasonable gyro data(%f,%f,%f,%" PRIi64 ")", sensorData[index].gyro[0], sensorData[index].gyro[1],
                sensorData[index].gyro[2], sensorData[index].timestamp);
    ...
    if (UNLIKELY(mDebugDump >= 1))
    {
        CAM_LOGD("Gyro data num: %d, video ts: %" PRIi64 " ", gyroSetProcData.gyro_num, gyroSetProcData.frame_t);
    }
