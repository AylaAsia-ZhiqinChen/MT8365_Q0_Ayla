...
bool LmvStateMachineUni::needEnque(SensorId sensor, bool willFollow)
{
    ...
    if (need && willFollow) {
        CAM_LOGD("[LmvState] needEnque(): Will enque (mEnqueRemain[%d] = %d)", int(sensor), mEnqueRemain[sensor]);
        mEnqueRemain[sensor]--;
    }
        ...
        SensorId sourcePath, SwitchResult result, Transition *transition)
{
    ...
    if (mCurrentState != newState)
    {
        CAM_LOGD("[LmvState] Result: %d, State: %d -> %d",
        ...
        SensorId sourcePath, bool isValid, Transition *transition)
{
    ...
    case STATE_ON_WIDE:
        ...
        {
            mWrongStateCount += 2;
            CAM_LOGW("[LmvState] State: %d, but valid LMV from %d", mCurrentState, sourcePath);
        }
        break;
    case STATE_ON_TELE:
        if (sourcePath == TELE)
    ...
    if (mWrongStateCount >= 10) // too many wrongs
    {
        ...
        CAM_LOGE("[LmvState] State recovery: %d -> %d because received too many LMV validity from wrong path",
