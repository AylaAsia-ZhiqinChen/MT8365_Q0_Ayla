...
protected:
    virtual int getStereoFeatureMode() = 0;
    ...
    virtual void TearDown() {
        ...
        if(E_N3D_WARP_WPE == N3D_WARP_HW) {
            StereoDpUtil::freeImageBuffer(LOG_TAG, _warpMapMain1);
            StereoDpUtil::freeImageBuffer(LOG_TAG, _warpMapMain2);
        } else {
    ...
    virtual void loadInitInfo()
    {
        ...
        if( checkStereoProperty("debug.STEREO.disable_gpu") == 1 )
        {
            _n3dInitInfo.system_cfg &= ~(1);  //disable GPU
        }

        if( checkStereoProperty("debug.STEREO.enable_cc", 0) == 1 ) {
            _n3dInitInfo.system_cfg &= ~(1<<3);
        }

        if(E_N3D_WARP_WPE == N3D_WARP_HW) {
            _n3dInitInfo.system_cfg &= ~(1);     //disable GPU
            _n3dInitInfo.system_cfg &= ~(3<<13); //disable learning
            _n3dInitInfo.system_cfg &= ~(1<<9);  //Main FF
            _n3dInitInfo.system_cfg &= ~(1<<10); //Auxi FF
            _n3dInitInfo.system_cfg |= (3<<13);  //wpe on
            _n3dInitInfo.system_cfg |= (3<<15);  //wpe on
            _n3dInitInfo.system_cfg &= ~(3<<22); //ldc off
        }
        ...
        if(E_N3D_WARP_WPE == N3D_WARP_HW) {
            MSize outputSize = MSize(_n3dInitInfo.iio_main.wpe_w, _n3dInitInfo.iio_main.wpe_h);
            StereoDpUtil::allocImageBuffer(LOG_TAG, eImgFmt_WARP_3PLANE, outputSize, !IS_ALLOC_GB, _warpMapMain1);
            StereoDpUtil::allocImageBuffer(LOG_TAG, eImgFmt_WARP_3PLANE, outputSize, !IS_ALLOC_GB, _warpMapMain2);
            _previewOutput.warpMapMain1 = _warpMapMain1.get();
            _previewOutput.warpMapMain2 = _warpMapMain2.get();
        } else {
            //Init main1 images
...
protected:
    ...
    sp<IImageBuffer> _warpMapMain1;
    sp<IImageBuffer> _warpMapMain2;
    ...
    bool LOG_ENABLED = StereoSettingProvider::isLogEnabled("debug.STEREO.log.hal.n3d.ut");
};
