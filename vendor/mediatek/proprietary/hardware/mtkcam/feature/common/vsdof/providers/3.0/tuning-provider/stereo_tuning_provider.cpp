...
bool
StereoTuningProvider::getDPEInputMargin(MUINT32 &top, MUINT32 &left, MUINT32 &bottom, MUINT32 &right)
{
    //TODO: get from tuning
    top    = 4;
    left   = 4;
    bottom = 4;
    right  = 4;
...
bool
StereoTuningProvider::getWPEInputMargin(MUINT32 &top, MUINT32 &left, MUINT32 &bottom, MUINT32 &right)
{
    //TODO: get from tuning
    top    = 5;
    left   = 5;
...
bool
StereoTuningProvider::getADPTuningInfo(std::vector<std::pair<std::string, int>> &tuningParams)
{
    TuningQuery_T query;
    query.results[QUERY_KEY_TUNING_PARAMS] = &tuningParams;

    return StereoTuningProviderKernel::getInstance()->getTuningParams(E_TUNING_SW_ADP, query);
}
#endif
