...
CDN_HAL_IMP::__setImages(CDN_HAL_PARAMS &in, CDN_HAL_IO &io)
{
    ...
    __runtimeImages.output = io.output;
...
CDN_HAL_IMP::__initWorkingBuffer()
{
    ...
    StereoArea area = StereoSizeProvider::getInstance()->getBufferSize(E_BM_DENOISE_HAL_OUT_ROT_BACK);
    ...
    {
        AutoProfileUtil profile(LOG_TAG, "CDN working buffer");
        __pDrv->CDNFeatureCtrl(CDN_FEATURE_GET_WORKBUF_SIZE, &__workSize, NULL);
        MY_LOGD_IF(LOG_ENABLED, "Init working buffer size %d", __workSize);
        if(__workSize > 0){
            __workBuf = new char[__workSize];
            MY_LOGD_IF(LOG_ENABLED, "Set working buffer");
            __pDrv->CDNFeatureCtrl(CDN_FEATURE_SET_WORKBUF_ADDR, __workBuf, NULL);
        }
    }
    FUNC_END;
