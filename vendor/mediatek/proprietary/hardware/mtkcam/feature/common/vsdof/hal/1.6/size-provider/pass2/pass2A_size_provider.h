...
public:
    ...
    virtual StereoArea getWDMAArea( ENUM_STEREO_SCENARIO eScenario ) const {
        switch(eScenario) {
            ...
            case eSTEREO_SCENARIO_CAPTURE:
                return StereoArea(StereoSizeProvider::getInstance()->captureImageSize());
                break;
            default:
                break;
        }
    ...
    virtual StereoArea getWROTArea( ENUM_STEREO_SCENARIO eScenario __attribute__((unused)) ) const  {
        StereoArea area(960, 544);
        if(StereoSettingProvider::isDeNoise() ||
           eRatio_4_3 == StereoSettingProvider::imageRatio())
        {
            area.applyRatio(eRatio_4_3);
        }

        ENUM_ROTATION moduleRotation = StereoSettingProvider::getModuleRotation();
        const bool IS_VERTICAL_MODULE = (moduleRotation & 0x2);
        if(IS_VERTICAL_MODULE) {
            area.rotate(moduleRotation);
        }
