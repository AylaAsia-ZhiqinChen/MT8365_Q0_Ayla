...
#define IS_STEREO_MODE(mode) ((mode & E_STEREO_FEATURE_VSDOF) || \
                              ...
                              (mode & E_STEREO_FEATURE_ACTIVE_STEREO))
...
StereoSettingProvider::setImageRatio(ENUM_STEREO_RATIO ratio)
{
    ...
    size_t len = ::property_get("debug.STEREO.ratio", value, NULL);
    if(len > 0) {
        int m, n;
...
{
    MY_LOGD("Set stereo feature mode: %s -> %s",
            _getStereoFeatureModeString(m_stereoFeatureMode).c_str(),
            _getStereoFeatureModeString(stereoMode).c_str());
...
StereoSettingProvider::_getStereoFeatureModeString(int stereoMode)
{
    ...
    if((stereoMode & E_STEREO_FEATURE_MTK_DEPTHMAP) > 0) {
        if(featureCount++ > 0) {
            s += "+mtkdepthmap";
        } else {
            s += "mtkdepthmap";
    ...
    if((stereoMode & E_STEREO_FEATURE_ACTIVE_STEREO) > 0) {
        if(featureCount++ > 0) {
            s += "+activestereo";
        } else {
            s += "activestereo";
...
StereoSettingProvider::getModuleRotation(ENUM_STEREO_SENSOR_PROFILE profile)
{
    ...
    if(NULL != sensorCombination)
    {
        ...
        {
            rotate = (STEREO_SENSOR_PROFILE_FRONT_FRONT != profile || StereoSettingProvider::isActiveStereo())
                     ? eRotate_90 : eRotate_270;
        } else {
            rotate = eRotate_0;
        }
    } else if(!isTestMode()) {
...
StereoSettingProvider::getSensorRawFormat(const int SENSOR_INDEX)
{
    ...
    if( SENSOR_INDEX != SENSOR_DEV_MAIN &&
        ...
        checkStereoProperty("debug.STEREO.debug.main2Mono") == 1 )
    {
...
StereoSettingProvider::isDualCamMode()
{
    ...
    MUINT32 supportedMode = (E_STEREO_FEATURE_CAPTURE|E_STEREO_FEATURE_THIRD_PARTY);
        ...
        supportedMode |= E_STEREO_FEATURE_ACTIVE_STEREO;
    #endif
...
StereoSettingProvider::_updateImageSettings()
{
       ...
       NULL != g_sensorSettings[1])
    {
        ...
        for(int i = 0; i < 2; ++i) {
            ...
            if(g_sensorSettings[i]->irRrzoSize.find(m_imageRatio) != g_sensorSettings[i]->irRrzoSize.end()) {
                irRrzoSizes[i] = g_sensorSettings[i]->irRrzoSize[m_imageRatio];
            }
        }
        ...
        StereoSizeProvider::getInstance()->__setIRRRZOSize(irRrzoSizes[0], irRrzoSizes[1]);
...
StereoSettingProvider::getStereoCameraFOVRatio(ENUM_STEREO_SENSOR_PROFILE profile)
{
    ...
    float main1FOV = sensorSettings[0]->fovHorizontal;
    float main2FOV = sensorSettings[1]->fovHorizontal;
    if(StereoSettingProvider::getModuleRotation() & 0x2) {
        main1FOV = sensorSettings[0]->fovVertical;
        main2FOV = sensorSettings[1]->fovVertical;
    }
...
{
    int dpeRound = checkStereoProperty("debug.STEREO.tuning.dpe_round", DPE_CAPTURE_ROUND);
    if(dpeRound < 0) {
        dpeRound = 0;
    }
...
StereoSettingProvider::getCallbackBufferList()
{
    ...
    size_t len = ::property_get("debug.STEREO.callback_list", value, NULL);
    if(len > 0) {
        result.assign(value);
    }
...
StereoSettingProvider::getCalibrationData(void *calibrationData, ENUM_STEREO_SENSOR_PROFILE profile)
{
    ...
    MY_LOGD("return calibration data in %d bytes", pSC->calibrationData.size());
    return pSC->calibrationData.size();
}

size_t
StereoSettingProvider::getWarpMapSizeInBytes()
{
    MSize size = StereoSizeProvider::getInstance()->getBufferSize(E_WARP_MAP_M);
    return size.w * size.h * 4;
}