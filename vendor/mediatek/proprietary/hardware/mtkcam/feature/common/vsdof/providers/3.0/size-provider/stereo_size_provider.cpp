...
//===============================================================
// 4:3
// Horizontal: 184x124, 24x4, (12,2), content 160x120
// Vertical: 152x174, 24x4, (12,2), content 128x170
const StereoArea STEREO_BASE_SIZE_4_3(184, 124, 24, 4, 12, 2);
// 16:9
// Horizontal: 216x112, 24x4, (12,2), content 192x108
// Vertical: 136x204, 24x4, (12,2), content 112x200

// 1:1
// Horizontal==Vertical: 168x148, 24x4, (12,2), content 144x144

// 19:9
// Horizontal: 232x102, 24x4, (12,2), content 208x98
// Vertical: 120x206, 24x4, (12,2), content 96x202
std::map<int, StereoArea> sizeCache1x;

//ActiveStereo IR sensor
const StereoArea STEREO_BASE_SIZE_4_3_IR(360, 480);
...
StereoSize::getStereoArea1x(const StereoSizeConfig &config)
{
    ...
    if(StereoSettingProvider::isActiveStereo()) {
        area = STEREO_BASE_SIZE_4_3_IR;
        int m, n;
        imageRatioMToN(imageRatio, m, n);
        area.size.w = area.size.h * n / m;
        area.apply16Align();
    } else {
        if(!StereoSizeProvider::getInstance()->__getCustomziedSize(area)) {
            ...
            if(sizeCache1x.find(imageRatio) != sizeCache1x.end()) {
                ...
                area.applyRatio(imageRatio)
                    .rotatedByModule();

                MSize contentSize = area.contentSize();
                applyNAlign(16, contentSize.w);
                                ...
                                )
{
    ...
    if(__pass1TgCropRectMap.find(key) != __pass1TgCropRectMap.end()) {
        ...
        MY_LOGD_IF(LOG_ENABLED, "Use result of (sensor %d, port %s, format %d, sensor scenario %d, ratio %d:%d, profile %d, feature %d): tg (%d,%d)(%dx%d), outSize %dx%d, stride %d",
        ...
        if(err) {
            MY_LOGE("Cannot get sensor crop win info, dev index %d, sensor scanario %d", sendorDevIndex, sensorScenario);
            return false;
        }
        ...
        if(EPortIndex_RRZO == port) {
                ...
                StereoSettingProvider::is3rdParty() )
            {
                ...
                if(StereoSettingProvider::isActiveStereo()) {
                    outSize = (eSTEREO_SENSOR_MAIN1 == sensor) ? __irRrzoSize[0] : __irRrzoSize[1];
                } else {
                ...
                else if( outSize.w > tgCropRect.s.w ||
                         outSize.h > tgCropRect.s.h)
                {
                    MY_LOGD_IF(LOG_ENABLED, "RRZO %dx%d->%dx%d (Cannot scale up)",
                               outSize.w, outSize.h, tgCropRect.s.w, tgCropRect.s.h);
                    outSize = tgCropRect.s;
                }

                MY_LOGD_IF(LOG_ENABLED, "%s RRZO size=%dx%d, sensor=%d",
                           StereoSettingProvider::isActiveStereo()?"ActiveStereo":"3rd Party",
                           outSize.w, outSize.h, sensor);
            }
        ...
        MY_LOGD_IF(LOG_ENABLED, "New result for (sensor %d, port %s, format %d, sensor scenario %d, ratio %d:%d, profile %d, feature %d): tg (%d,%d)(%dx%d), outSize %dx%d, stride %d",
                                sensor-1, (EPortIndex_RRZO == port) ? "RRZO" : "IMGO", format, sensorScenario, m, n,
                                StereoSettingProvider::stereoProfile(),
                                StereoSettingProvider::getStereoFeatureMode(),
                                tgCropRect.p.x, tgCropRect.p.y, tgCropRect.s.w, tgCropRect.s.h, outSize.w, outSize.h, strideInBytes);
