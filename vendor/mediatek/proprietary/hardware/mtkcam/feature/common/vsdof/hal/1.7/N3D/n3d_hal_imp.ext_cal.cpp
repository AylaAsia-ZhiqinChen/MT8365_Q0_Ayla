...
bool
N3D_HAL_IMP::__loadExternalCalibrationData(float *output)
{
    float *externalCalibrationData = NULL;
    size_t calibrationSize         = 0;
    bool result = false;

    //TODO: read calibration to externalCalibrationData and get calibrationSize

    if(calibrationSize > 0 &&
       calibrationSize <= CAM_CAL_Stereo_Data_SIZE)
    {
        ::memcpy(output, externalCalibrationData, calibrationSize);
        result = true;
    } else {
        MY_LOGD("Read external calibration size %zu, CAM_CAL_Stereo_Data_SIZE (%d)", calibrationSize, CAM_CAL_Stereo_Data_SIZE);
    }
