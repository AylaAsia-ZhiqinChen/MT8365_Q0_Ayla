...
N3D_HAL_IMP::_dumpInitInfo(const STEREO_KERNEL_SET_ENV_INFO_STRUCT &initInfo)
{
    ...
    FAST_LOGD("[iio_cmp.src_w]    %d", initInfo.iio_cmp.src_w);
    FAST_LOGD("[iio_cmp.src_h]    %d", initInfo.iio_cmp.src_h);
    FAST_LOGD("[iio_cmp.inp_w]    %d", initInfo.iio_cmp.inp_w);
    FAST_LOGD("[iio_cmp.inp_h]    %d", initInfo.iio_cmp.inp_h);
    FAST_LOGD("[iio_cmp.out_w]    %d", initInfo.iio_cmp.out_w);
    FAST_LOGD("[iio_cmp.out_h]    %d", initInfo.iio_cmp.out_h);

    _dumpAFInitInfo("[af_init_main]", initInfo.af_init_main);
    _dumpAFInitInfo("[af_init_auxi]", initInfo.af_init_auxi);
    ...
    std::string s = std::bitset<16>(initInfo.system_cfg).to_string();
...
void
N3D_HAL_IMP::_dumpAFInitInfo(const char *prefix, const STEREO_KERNEL_AF_INIT_STRUCT &afInitInfo)
{
    if(!LOG_ENABLED) {
        return;
    }
...
N3D_HAL_IMP::_dumpTuningInfo(const char *prefix, const STEREO_KERNEL_TUNING_PARA_STRUCT &tuningInfo)
{
    ...
    FAST_LOGD("%s[s_tune]   %s", prefix, s.c_str());
    FAST_LOG_PRINT;
}
...
N3D_HAL_IMP::_dumpSetProcInfo(const char *prefix, const STEREO_KERNEL_SET_PROC_INFO_STRUCT &setprocInfo)
{
    ...
    FAST_LOGD("%s[Magic]              %d, %d", prefix, m_magicNumber[0], m_magicNumber[1]);
    FAST_LOGD("%s[Request]            %d", prefix, m_requestNumber);
    FAST_LOGD("%s[rrz_offset_x]       %d", prefix, setprocInfo.rrz_offset_x);
    FAST_LOGD("%s[rrz_offset_y]       %d", prefix, setprocInfo.rrz_offset_y);
    FAST_LOGD("%s[rrz_usage_width]    %d", prefix, setprocInfo.rrz_usage_width);
    FAST_LOGD("%s[rrz_usage_height]   %d", prefix, setprocInfo.rrz_usage_height);
    FAST_LOGD("%s[rrz_out_width]      %d", prefix, setprocInfo.rrz_out_width);
    FAST_LOGD("%s[rrz_out_height]     %d", prefix, setprocInfo.rrz_out_height);
    ...
    FAST_LOGD("%s[addr_ml]            %p", prefix, setprocInfo.addr_ml);
    ...
    FAST_LOGD("%s[iio_cap.src_w]      %d", prefix, setprocInfo.iio_cap.src_w);
    FAST_LOGD("%s[iio_cap.src_h]      %d", prefix, setprocInfo.iio_cap.src_h);
    FAST_LOGD("%s[iio_cap.inp_w]      %d", prefix, setprocInfo.iio_cap.inp_w);
    FAST_LOGD("%s[iio_cap.inp_h]      %d", prefix, setprocInfo.iio_cap.inp_h);
    FAST_LOGD("%s[iio_cap.out_w]      %d", prefix, setprocInfo.iio_cap.out_w);
    FAST_LOGD("%s[iio_cap.out_h]      %d", prefix, setprocInfo.iio_cap.out_h);

    if(eSTEREO_SCENARIO_CAPTURE == m_eScenario) {
        FAST_LOGD("%s[src_gb]     GraphicBuffer %p, EGLImage %p", prefix, setprocInfo.src_gb.mGraphicBuffer, setprocInfo.src_gb.mEGLImage);
        FAST_LOGD("%s[dst_gb]     GraphicBuffer %p, EGLImage %p", prefix, setprocInfo.dst_gb.mGraphicBuffer, setprocInfo.dst_gb.mEGLImage);
        FAST_LOGD("%s[eglDisplay] %p", prefix, setprocInfo.eglDisplay);
