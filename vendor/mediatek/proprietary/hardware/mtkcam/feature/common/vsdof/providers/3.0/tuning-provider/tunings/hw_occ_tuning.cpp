...
HW_OCCTuning::_initDefaultValues()
{
    ...
    for(int i = 0; i < SUPPORT_SCENARIO_COUNT; i++) {
        ...
        __updateSkipMode(*tuningBuffer, (0==i)?eSTEREO_SCENARIO_PREVIEW:eSTEREO_SCENARIO_CAPTURE);
...
HW_OCCTuning::_initDocument()
{
    ...
    for(int i = 0; i < SUPPORT_SCENARIO_COUNT; i++) {
        ...
        AddTuning("occ_horz_ds4", tuningBuffer->occ_horz_ds4);
        AddTuning("occ_vert_ds4", tuningBuffer->occ_vert_ds4);

        AddTuning("occ_th_luma", tuningBuffer->occ_th_luma);
        AddTuning("occ_th_h", tuningBuffer->occ_th_h);
        AddTuning("occ_th_v", tuningBuffer->occ_th_v);
        AddTuning("occ_vec_shift", tuningBuffer->occ_vec_shift);
        AddTuning("occ_vec_offset", tuningBuffer->occ_vec_offset);
        AddTuning("occ_invalid_value", tuningBuffer->occ_invalid_value);
        AddTuning("occ_owc_th", tuningBuffer->occ_owc_th);
        AddTuning("occ_owc_en", tuningBuffer->occ_owc_en);
        AddTuning("occ_depth_clip_en", tuningBuffer->occ_depth_clip_en);
        AddTuning("occ_spare", tuningBuffer->occ_spare);

        value->AddMember(VALUE_KEY_VALUES, tuningValue.Move(), _allocator);

        occValue.PushBack(value->Move(), _allocator);
    }
...
void
HW_OCCTuning::_loadValuesFromDocument()
{
    ...
    for(SizeType i = 0; i < occValues.Size(); i++) {
        ...
        tuningBuffer->occ_horz_ds4      = tuningValues["occ_horz_ds4"].GetInt();
        tuningBuffer->occ_vert_ds4      = tuningValues["occ_vert_ds4"].GetInt();

        __updateROI(*tuningBuffer, scenario);
        __updateSkipMode(*tuningBuffer, scenario);

        tuningBuffer->occ_th_luma       = tuningValues["occ_th_luma"].GetInt();
        tuningBuffer->occ_th_h          = tuningValues["occ_th_h"].GetInt();
        tuningBuffer->occ_th_v          = tuningValues["occ_th_v"].GetInt();
        tuningBuffer->occ_vec_shift     = tuningValues["occ_vec_shift"].GetInt();
        tuningBuffer->occ_vec_offset    = tuningValues["occ_vec_offset"].GetInt();
        tuningBuffer->occ_invalid_value = tuningValues["occ_invalid_value"].GetInt();
        tuningBuffer->occ_owc_th        = tuningValues["occ_owc_th"].GetInt();
        tuningBuffer->occ_owc_en        = tuningValues["occ_owc_en"].GetInt();
        tuningBuffer->occ_depth_clip_en = tuningValues["occ_depth_clip_en"].GetInt();
        tuningBuffer->occ_spare         = tuningValues["occ_spare"].GetInt();
    }
}
...
void
HW_OCCTuning::__updateSkipMode(OCCConfig &config, ENUM_STEREO_SCENARIO scenario)
{
    if(scenario == eSTEREO_SCENARIO_CAPTURE) {
        config.occ_h_skip_mode = 0;
    } else {
