...
class Record_16_9_Rear_Rotate0_BM_VSDoF : public StereoSizeProviderUTBase
{
public:
    Record_16_9_Rear_Rotate0_BM_VSDoF() : StereoSizeProviderUTBase() { init(); }

    virtual ~Record_16_9_Rear_Rotate0_BM_VSDoF() {}

    virtual bool ignoreTest()
    {
        return (StereoSettingProvider::getModuleRotation() != eRotate_0);
...
//  PASS 1
//=============================================================================
TEST_F(Record_16_9_Rear_Rotate0_BM_VSDoF, TEST)
{
    if(ignoreTest()) {
    ...
    MYEXPECT_EQ( tgCropRect[1],     MRect(MPoint(0, 190),   MSize(2100, 1180)) );
    ...
    // PASS2A-2
    MYEXPECT_EQ(pass2SizeInfo[PASS2A_2].areaWDMA,           StereoArea(576, 162));
    ...
    // PASS2A'-2
    MYEXPECT_EQ(pass2SizeInfo[PASS2A_P_2].areaWDMA,         StereoArea(864, 244));   //(576x162) * 1.5
    ...
    //=============================================================================
    //N3D Output
    MYEXPECT_EQ(sizeProvider->getBufferSize(E_MV_Y),            StereoArea(544, 144, 64, 8, 32, 4));
    MYEXPECT_EQ(sizeProvider->getBufferSize(E_MASK_M_Y),        StereoArea(544, 144, 64, 8, 32, 4));
    MYEXPECT_EQ(sizeProvider->getBufferSize(E_SV_Y),            StereoArea(544, 144, 64, 8, 32, 4));
    MYEXPECT_EQ(sizeProvider->getBufferSize(E_MASK_S_Y),        StereoArea(544, 144, 64, 8, 32, 4));
    MYEXPECT_EQ(sizeProvider->getBufferSize(E_LDC),             StereoArea(272, 144, 32, 8, 16, 4));

    //DPE Output
    MYEXPECT_EQ(sizeProvider->getBufferSize(E_DMP_H),           StereoArea(544, 144, 64, 8, 32, 4));
    MYEXPECT_EQ(sizeProvider->getBufferSize(E_CFM_H),           StereoArea(544, 144, 64, 8, 32, 4));
    MYEXPECT_EQ(sizeProvider->getBufferSize(E_RESPO),           StereoArea(544, 144, 64, 8, 32, 4));
