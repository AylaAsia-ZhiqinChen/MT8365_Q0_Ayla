...
//stereo_tuning_def.h: ENUM_STEREO_TUNING
...
StereoTuningProviderKernel::init()
{
    ...
    {
        char SENSOR_COMBINATION_KEY[256];
        sprintf(SENSOR_COMBINATION_KEY, "%s+%s", sensorSettings[0]->name.c_str(), sensorSettings[1]->name.c_str());
        char SENSOR_COMBINATION_KEY2[256];
        sprintf(SENSOR_COMBINATION_KEY2, "%s+%s", sensorSettings[1]->name.c_str(), sensorSettings[0]->name.c_str());

        Value sensorCombinationKey;
        sensorCombinationKey.SetString(StringRef(SENSOR_COMBINATION_KEY, strlen(SENSOR_COMBINATION_KEY)));
        Value sensorCombinationKey2;
        sensorCombinationKey2.SetString(StringRef(SENSOR_COMBINATION_KEY2, strlen(SENSOR_COMBINATION_KEY2)));

        // Check if tuning parameters exist for current profile
        if (loadDocument.HasMember(sensorCombinationKey)) {
            MY_LOGD("Init tuning parameters with combination %s", SENSOR_COMBINATION_KEY);
            document.CopyFrom(loadDocument[sensorCombinationKey], loadDocument.GetAllocator());
        } else if (loadDocument.HasMember(sensorCombinationKey2)) {
            MY_LOGD("Init tuning parameters with combination %s", SENSOR_COMBINATION_KEY2);
            document.CopyFrom(loadDocument[sensorCombinationKey2], loadDocument.GetAllocator());
        } else {
            MY_LOGD("Combination %s does not exist, init tuning parameters with whole document", SENSOR_COMBINATION_KEY);
            document.CopyFrom(loadDocument, loadDocument.GetAllocator());
        }
    }
    ...
    __tuningHolderMap[E_TUNING_HW_OCC]     = new HW_OCCTuning(document);
    ...
    __tuningHolderMap[E_TUNING_SW_BOKEH]   = new SW_BOKEHTuning(document);
    __tuningHolderMap[E_TUNING_SW_ADP]     = new SW_ADPTuning(document);
#endif
...
{
    if(__tuningHolderMap.find(tuningID) == __tuningHolderMap.end()) {
        MY_LOGE("Cannot get holder of %s", STEREO_TUNING_NAME[tuningID]);
        return false;
    }

    return __tuningHolderMap[tuningID]->retrieveTuningParams(param);
}
