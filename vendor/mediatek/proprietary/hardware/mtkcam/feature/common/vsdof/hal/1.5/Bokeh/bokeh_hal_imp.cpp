...
int MDPRunner(RF_DP_STRUCT *pPara)
{
    ...
    for(MUINT32 portIndex = 0; portIndex < pPara->outputPortNo; portIndex++) {
        if (pPara->vsfParam[portIndex]) {
            DpPqParam dpParam;
            dpParam.enable |= PQ_REFOCUS_EN;
            dpParam.u.isp.vsdofParam.isRefocus        = true;
            dpParam.u.isp.vsdofParam.defaultUpTable   = pPara->vsfParam[portIndex]->defaultUpTable;
            dpParam.u.isp.vsdofParam.defaultDownTable = pPara->vsfParam[portIndex]->defaultDownTable;
            dpParam.u.isp.vsdofParam.IBSEGain         = pPara->vsfParam[portIndex]->IBSEGain;
