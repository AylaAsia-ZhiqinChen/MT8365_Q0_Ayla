...
N3D_HAL_IMP::_initAFInfo(STEREO_KERNEL_AF_INIT_STRUCT &afMain1, STEREO_KERNEL_AF_INIT_STRUCT &afMain2)
{
    ...
    for(int i = 0; i < 2; ++i) {
        queryResult = pCamCalDrvObj->GetCamCalCalData(devIdx[i], CAMERA_CAM_CAL_DATA_3A_GAIN, (void *)&calibrationData);
        ...
        if(m_isAF[i]) {
            ...
            afInitInfo[i]->dist_mcr = calibrationData.Single2A.S2aAF_t.AF_Macro_pattern_distance;    //unit: mm
            afInitInfo[i]->dist_inf = calibrationData.Single2A.S2aAF_t.AF_infinite_pattern_distance; //unit: mm

            CustomDacToDistance_T::const_iterator dacIt = CUSTOM_DAC_TO_DITANCE.find(sensorIdx[i]);
            if(0 == afInitInfo[i]->dist_mcr &&
               0 == afInitInfo[i]->dist_inf &&
               dacIt != CUSTOM_DAC_TO_DITANCE.end())
            {
                afInitInfo[i]->dist_mcr = dacIt->second[0];
                afInitInfo[i]->dist_inf = dacIt->second[1];
            }
        } else {
...
N3D_HAL_IMP::_prepareSystemConfig()
{
    ...
    config |= (ENABLE_LDC[StereoSettingProvider::stereoProfile()]<<8);
