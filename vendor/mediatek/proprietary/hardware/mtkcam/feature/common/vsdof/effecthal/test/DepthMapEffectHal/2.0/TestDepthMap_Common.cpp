...
String8 WaitingListener::sNotReadyStatus = String8(DEPTHMAP_NOT_READY_KEY);
...
MVOID WaitingListener::CB(MVOID *tag, String8 status, sp<EffectRequest>& request)
{
  ...
  else if(status == sNotReadyStatus)
      {
    ++WaitingListener::sDoneCount;
...
MVOID finishCB_forListener(bool status)
{
    if(status)
    {
        sp<EffectRequest> pRequest = nullptr;
        WaitingListener::CB(NULL, String8(DEPTHMAP_COMPLETE_KEY), pRequest);
    }
}
...
MBOOL setupReqMetadata(MBOOL eisON, sp<IDepthMapEffectRequest> pRequest)
{
    ...
    pRequest->pushRequestMetadata({PBID_IN_APP_META, eBUFFER_IOTYPE_INPUT}, pMetadata);
    // InAppMeta - EIS on
    if(eisON)
    {
    ...
    // InHalMeta Main1
    pMetadata = new IMetadata();
    pRequest->pushRequestMetadata({PBID_IN_HAL_META_MAIN1, eBUFFER_IOTYPE_INPUT}, pMetadata);
    ...
    pMetadata = new IMetadata();
    trySetMetadata<MINT32>(pMetadata, MTK_P1NODE_PROCESSOR_MAGICNUM, 0);
    pRequest->pushRequestMetadata({PBID_IN_HAL_META_MAIN2, eBUFFER_IOTYPE_INPUT}, pMetadata);
    // OutAppMeta PBID_OUT_APP_META
    pMetadata = new IMetadata();
    pRequest->pushRequestMetadata({PBID_OUT_APP_META, eBUFFER_IOTYPE_OUTPUT}, pMetadata);
    // OutHalMeta
    pMetadata = new IMetadata();
    pRequest->pushRequestMetadata({PBID_OUT_HAL_META, eBUFFER_IOTYPE_OUTPUT}, pMetadata);
...
MVOID releaseMetadata(std::vector<sp<IDepthMapEffectRequest> > vRequestVec)
{
    ...
    {
        if(pRequest->getRequestMetadata({PBID_IN_APP_META, eBUFFER_IOTYPE_INPUT}, pMetadata))
        {
            pRequest->popRequestMetadata({PBID_IN_APP_META, eBUFFER_IOTYPE_INPUT});
            delete pMetadata;
        }

        if(pRequest->getRequestMetadata({PBID_IN_HAL_META_MAIN1, eBUFFER_IOTYPE_INPUT}, pMetadata))
        {
            pRequest->popRequestMetadata({PBID_IN_HAL_META_MAIN1, eBUFFER_IOTYPE_INPUT});
            delete pMetadata;
        }

        if(pRequest->getRequestMetadata({PBID_IN_HAL_META_MAIN2, eBUFFER_IOTYPE_INPUT}, pMetadata))
        {
            pRequest->popRequestMetadata({PBID_IN_HAL_META_MAIN2, eBUFFER_IOTYPE_INPUT});
            delete pMetadata;
        }

        if(pRequest->getRequestMetadata({PBID_OUT_APP_META, eBUFFER_IOTYPE_OUTPUT}, pMetadata))
        {
            pRequest->popRequestMetadata({PBID_OUT_APP_META, eBUFFER_IOTYPE_OUTPUT});
            delete pMetadata;
        }

        if(pRequest->getRequestMetadata({PBID_OUT_HAL_META, eBUFFER_IOTYPE_OUTPUT}, pMetadata))
        {
            pRequest->popRequestMetadata({PBID_OUT_HAL_META, eBUFFER_IOTYPE_OUTPUT});
            delete pMetadata;
        }
    }
...
{
    const char * sMain1Filename = "/sdcard/vsdof_data/BID_P2A_IN_RSRAW1_2100x1180_3944.raw";
    const char * sMain2Filename = "/sdcard/vsdof_data/BID_P2A_IN_RSRAW2_1280x720_2400.raw";

    const char * sMain1FSFilename = "/sdcard/vsdof_data/BID_P2A_IN_FSRAW1_4208x3120_5260.raw";
    const char * sMain2FSFilename = "/sdcard/vsdof_data/BID_P2A_IN_FSRAW2_4208x3120_5260.raw";
...
MBOOL prepareReqInputBuffer(sp<IDepthMapEffectRequest> pRequest)
{
    ...
    pRequest->pushRequestImageBuffer({PBID_IN_RSRAW1, eBUFFER_IOTYPE_INPUT}, buf_rs_Main1);
    // prepare input frame info: RSRAW2
    pRequest->pushRequestImageBuffer({PBID_IN_RSRAW2, eBUFFER_IOTYPE_INPUT}, buf_rs_Main2);
    // prepare input frame info: FSRAW1
    pRequest->pushRequestImageBuffer({PBID_IN_FSRAW1, eBUFFER_IOTYPE_INPUT}, buf_fs_Main1);
    // prepare input frame info: FSRAW2
    pRequest->pushRequestImageBuffer({PBID_IN_FSRAW2, eBUFFER_IOTYPE_INPUT}, buf_fs_Main2);
    return MTRUE;
}
...
)
{
    ...
    else
    {
        ...
        pRequest->pushRequestImageBuffer({PBID_OUT_MV_F, eBUFFER_IOTYPE_OUTPUT},buf);
        ...
        if(reqType == eREQTYPE_VR)
        {
            ...
            pRequest->pushRequestImageBuffer({PBID_OUT_FDIMG, eBUFFER_IOTYPE_OUTPUT},buf);
        }
        ...
        pRequest->pushRequestImageBuffer({PBID_OUT_DMBG, eBUFFER_IOTYPE_OUTPUT},buf);
    }
       return MTRUE;
}
