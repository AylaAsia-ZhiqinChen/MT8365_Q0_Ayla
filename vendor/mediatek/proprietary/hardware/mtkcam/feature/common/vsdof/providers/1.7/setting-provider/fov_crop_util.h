...
public:
    static bool updateFOVCropRatios(StereoSensorConbinationSetting_T &sensorCombinatioSetting)
    {
        ...
        float OUTER_FOV;
        if(ROTATION & 0x2) {
            if(sensorCombinatioSetting.sensorSettings[0]->fovVRuntime+2.0f*MODULE_VARIATION >=
            ...
            if(sensorCombinatioSetting.sensorSettings[0]->fovHRuntime+2.0f*MODULE_VARIATION >=
        ...
        if(cropMain1 &&
           sensorCombinatioSetting.isCenterCrop)
        {
            croppedSensor->cropSetting.keepRatio = std::min(INF_RATIO, NEAR_RATIO);
