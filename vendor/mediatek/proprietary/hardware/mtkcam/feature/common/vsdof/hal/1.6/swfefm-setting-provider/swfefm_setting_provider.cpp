...
#undef LOG_TAG
#endif
#define LOG_TAG "SWFEFMSettingProvider"
...
#define CHECK_FREQ_FF 2 //Check trigger every 2 frames
...
Mutex SWFEFMSettingProvider::__Lock;
SWFEFMSettingProvider *SWFEFMSettingProvider::__instance = NULL;
MTKFEFMTrigger *SWFEFMSettingProvider::__triggerInstance= NULL;

SWFEFMSettingProvider *
SWFEFMSettingProvider::getInstance()
{
    Mutex::Autolock lock(__Lock);

    if(NULL == __instance) {
        __instance = new SWFEFMSettingProvider();
...
void
SWFEFMSettingProvider::init(int main1Index, int main2Index)
{
    Mutex::Autolock lock(__Lock);
            ...
            if(NULL == pHal3A) {
                ...
                if(pHal3A->send3ACtrl(NS3Av3::E3ACtrl_GetSupportedInfo, (MUINTPTR)&rFeatureParam, 0)) {
                    ...
                    if(__isAF[i]) {
                        ...
                        __pAFTable[i] = NULL;
                        if(0 == i) {
                            __triggerInstance->FEFMTrigger_set_reqular_trigger(CHECK_FREQ_FF);
                        }
                    }
                }
...
bool
SWFEFMSettingProvider::needToRunSWFEFM(int main1MagicNumber, int main2MagicNumber, bool isCapture)
{
    bool needTrigger = (isCapture ||
                        __triggerInstance->FEFMTrigger_query(
                            main1MagicNumber, main2MagicNumber,
                            (__pAFTable[0]) ? __pAFTable[0]->daf_vec[main1MagicNumber%DAF_TBL_QLEN].is_af_stable : true,
                            (__pAFTable[1]) ? __pAFTable[1]->daf_vec[main2MagicNumber%DAF_TBL_QLEN].is_af_stable : true
                        ));

    MY_LOGD_IF(StereoSettingProvider::isLogEnabled(),
                "MagicNumber %d, %d isCapture %d, AF stable %d, %d need trigger %d",
                main1MagicNumber, main2MagicNumber, isCapture,
                (__pAFTable[0]) ? __pAFTable[0]->daf_vec[main1MagicNumber%DAF_TBL_QLEN].is_af_stable : true,
                (__pAFTable[1]) ? __pAFTable[1]->daf_vec[main2MagicNumber%DAF_TBL_QLEN].is_af_stable : true,
                needTrigger);
...
SWFEFMSettingProvider::SWFEFMSettingProvider()
{
