...
HW_MDPPQTuning::_initDocument()
{
    ...
    for(auto &param : __mdpPQParams) {
        ...
        AddTuning("switchRatio6Tap6nTap", tuning.switchRatio6Tap6nTap);
        AddTuning("switchRatio6nTapAcc", tuning.switchRatio6nTapAcc);
...
HW_MDPPQTuning::_loadValuesFromDocument()
{
    ...
    for(SizeType i = 0; i < mdpPQValues.Size(); i++) {
        ...
        if(tuningValues.HasMember("switchRatio6Tap6nTap")) {
            tuning.switchRatio6Tap6nTap = tuningValues["switchRatio6Tap6nTap"].GetInt();
        }

        if(tuningValues.HasMember("switchRatio6nTapAcc")) {
            tuning.switchRatio6nTapAcc = tuningValues["switchRatio6nTapAcc"].GetInt();
        }
