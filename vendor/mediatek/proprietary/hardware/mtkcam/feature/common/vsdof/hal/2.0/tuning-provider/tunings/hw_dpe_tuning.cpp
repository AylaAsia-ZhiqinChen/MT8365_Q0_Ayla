...
HW_DPETuning::_initDocument()
{
    ...
    for(int i = 0; i < SUPPORT_SCENARIO_COUNT; i++) {
        ...
        AddTuning("DVE_RESPO_SEL", tuningBuffer->DVE_RESPO_SEL);
        AddTuning("DVE_CONFO_SEL", tuningBuffer->DVE_CONFO_SEL);
...
HW_DPETuning::_loadValuesFromDocument()
{
    ...
    for(SizeType i = 0; i < dpeValues.Size(); i++) {
        ...
        tuningBuffer->DVE_RESPO_SEL = tuningValues["DVE_RESPO_SEL"].GetInt();
        tuningBuffer->DVE_CONFO_SEL = tuningValues["DVE_CONFO_SEL"].GetInt();
    }
}
