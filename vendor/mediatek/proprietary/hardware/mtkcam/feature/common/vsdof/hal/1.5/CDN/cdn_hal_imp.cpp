...
CDN_HAL_IMP::CDNHALRun(CDN_HAL_PARAMS &params, CDN_HAL_IO &io)
{
    ...
    const MINT32 checkIndexValueForSInfo = 7;
    int sInfo_7 = params.sInfo[checkIndexValueForSInfo];
    {
        AutoProfileUtil profile(LOG_TAG, "Run CDN Main");
        __pDrv->CDNMain();
    }

    MY_LOGD("value of SInfo, before: %d, after: %d", sInfo_7, __runntimeCheckProcess.SInfo[checkIndexValueForSInfo]);
    params.sInfo[checkIndexValueForSInfo] = __runntimeCheckProcess.SInfo[checkIndexValueForSInfo];
...
void
CDN_HAL_IMP::__setParams(const CDN_HAL_PARAMS &params)
{
    ...
    // CDN_PARAMS
    static_assert(sizeof __runtimeParams.Trans == sizeof params.Trans, "invalidate Trans array size of CDN_PARAMS");

    __runtimeParams.isRotate = params.isRotate;
    __runtimeParams.mode = params.mode;
    __runtimeParams.var = params.var;
    std::memcpy(&__runtimeParams.Trans, &params.Trans, sizeof(__runntimeCheckProcess.Trans));
    ...
    // CDNCheckProcess
    static_assert(sizeof(__runntimeCheckProcess.Trans) == sizeof(params.Trans), "invalidate Trans array size of CDNCheckProcess");
    static_assert(sizeof(__runntimeCheckProcess.CInfo) == sizeof(params.cInfo), "invalidate CInfo array size of CDNCheckProcess");
    static_assert(sizeof __runntimeCheckProcess.SInfo == sizeof(params.sInfo), "invalidate SInfo array size of CDNCheckProcess");

    __runntimeCheckProcess.width = params.width;
    __runntimeCheckProcess.height = params.height;
    __runntimeCheckProcess.isRotate = params.isRotate;
    std::memcpy(&__runntimeCheckProcess.Trans, &params.Trans, sizeof(__runntimeCheckProcess.Trans));
    std::memcpy(&__runntimeCheckProcess.CInfo, &params.cInfo, sizeof(__runntimeCheckProcess.CInfo));
    std::memcpy(&__runntimeCheckProcess.SInfo, &params.sInfo, sizeof(__runntimeCheckProcess.SInfo));
    __runntimeCheckProcess.BOUNDARY_THR =params.boundaryThr;
    __runntimeCheckProcess.VAR_THR = params.varThr;
    __runntimeCheckProcess.MACRO_BOUND_THR =params.macroBoundThr;

    __logCheckProcess();
    __pDrv->CDNFeatureCtrl(CDN_FEATURE_CHECK_PROCESS, &__runntimeCheckProcess, NULL);

    FUNC_END;
}

void
CDN_HAL_IMP::__setImages(const CDN_HAL_PARAMS &in, const CDN_HAL_IO &io)
{
