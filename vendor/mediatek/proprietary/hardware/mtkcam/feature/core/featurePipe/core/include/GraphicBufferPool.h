...
protected:
  ...
  MBOOL init(MUINT32 width, MUINT32 height, android_pixel_format_t format, MUINT32 usage, NativeBufferWrapper::ColorSpace color);
  MVOID uninit();
  virtual android::sp<GraphicBufferHandle> doAllocate();
  virtual MBOOL doRelease(GraphicBufferHandle *handle);

private:
  MBOOL initConfig(MUINT32 width, MUINT32 height, android_pixel_format_t format, MUINT32 usage, NativeBufferWrapper::ColorSpace color);
