...
onThreadLoop()
{
    ...
    pEnqueBufPool->start();
    {
        ...
        sp<IStopWatch> stopWatch = pN3DImgInfo->getRequestPtr()->getStopWatchCollection()->GetStopWatch(eStopWatchType::eSTOPWATCHTYPE_AUTO, "dpe_stream_enque");
...
handleDPEEnqueDone(DVEParams& rParams, EnquedBufPool* pEnqueBufPool)
{
    ...
    SmartImageBuffer smImgBuf_CFM_L, smImgBuf_CFM_R;
    SmartImageBuffer smImgBuf_RESPO_L, smImgBuf_RESPO_R;
    SmartGraphicBuffer smImgBuf_DMP_L, smImgBuf_DMP_R;
    MBOOL bRet;
    ...
    GET_GRABUFFER_FROM_ENQPOOL_AND_INVALIDATE(pEnqueBufPool, BID_DPE_OUT_DMP_R, smImgBuf_DMP_R);
    ...
    TO_DENOISE_ImgBufInfo->addGraphicBuffer(BID_DPE_OUT_DMP_R, smImgBuf_DMP_R);
    TO_DENOISE_ImgBufInfo->addSmartBuffer(BID_N3D_OUT_WARPING_MATRIX, pN3DImgInfo->getSmartBuffer(BID_N3D_OUT_WARPING_MATRIX));
    // Pass sceneInfo down to next node
    TO_DENOISE_ImgBufInfo->addSmartBuffer(BID_N3D_OUT_SCENEINFO,
                            pN3DImgInfo->getSmartBuffer(BID_N3D_OUT_SCENEINFO));
...
prepareDPEEnqueConfig(ImgInfoMapPtr pN3DImgInfo, NSIoPipe::DVEConfig& rDPEConfig, EnquedBufPool *pEnqueBufPool)
{
    ...
    if(mbFirst)
    {
        ...
        mLastDMP_R = mBufPool.getGraphicBufPool(BID_DPE_OUT_DMP_R)->request();
    ...
    MY_LOGD("flush and setup DVE buffer +");
    ...
    MY_LOGD("output buffers +");
    // output buffers
    SmartGraphicBuffer smImgBuf_DMP_L   =  mBufPool.getGraphicBufPool(BID_DPE_OUT_DMP_L)->request();
    SmartGraphicBuffer smImgBuf_DMP_R   =  mBufPool.getGraphicBufPool(BID_DPE_OUT_DMP_R)->request();
    ...
    MY_LOGD("setup output buffers and store to pEnqueBufPool +");
...
initBufferPool()
{
    ...
    Vector<NSBMDN::BufferConfig> vBufConfig;
    {
        NSBMDN::BufferConfig c = {
            "BID_DPE_OUT_DMP_L",
            BID_DPE_OUT_DMP_L,
            (MUINT32)DPE_DMP_SIZE.w*2,  // Kibo+ not support Y16, so wide * 2 and change format to Y8
            (MUINT32)DPE_DMP_SIZE.h,
            eImgFmt_Y8,
            GraphicBufferPool::USAGE_HW_RENDER,
            MTRUE,
            MTRUE,
            (MUINT32)(DPE_WORKING_BUF_SET*2*get_stereo_bmdenoise_capture_buffer_cnt()) //need hold the previous frame's DMP_LR buffer, extra allocate two
        };
