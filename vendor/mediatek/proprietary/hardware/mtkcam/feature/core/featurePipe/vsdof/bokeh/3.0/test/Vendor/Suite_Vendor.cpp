...
TEST(TestBufferPool, P2B_Run_With_G_0)
{
    ...
    sp<Vendor_UT_Thread> ut = new Vendor_UT_Thread(scenario);
    Vendor_UT_Params param;
    param.scenario = scenario;
    param.bokehLevel = 15;
    param.isAFTrigger = MTRUE;
    param.iGSensor = 0;//eTransform_ROT_180;
    MY_LOGD("Eric:ut->init start..");
    EXPECT_EQ(MTRUE, ut->init(param));
    MY_LOGD("Eric:ut->init end..and run");
    EXPECT_EQ(OK, ut->run(LOG_TAG));
    MY_LOGD("Eric:ut->run end");
    Wait(9000);
