...
#define CREATE_IMGBUF_POOL(rBufPool, name, size, format, usage, bSingle) \
    MY_LOGD("Create ImageBufferPool:: "#name " size=%dx%d format=%x usage=%x", \
                size.w, size.h, format, usage); \
    rBufPool = ImageBufferPool::create(name, size.w, size.h, format, usage);
...
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
    ...
    MBOOL queryFEOBufferSize(MSize iBufSize, MUINT iBlockSize, MSize& rFEOSize)
    {
        ...
        MY_LOGD("iBufSize=%dx%d iBlockSize=%d rFEOSize=%dx%d",
    ...
    MBOOL queryFMOBufferSize(MSize szFEOBuffer, MSize& rFMOSize)
    {
        ...
        MY_LOGD("szFEOBuffer=%dx%d rFMOSize=%dx%d", szFEOBuffer.w, szFEOBuffer.h, rFMOSize.w, rFMOSize.h);
        return MTRUE;
    }
};
