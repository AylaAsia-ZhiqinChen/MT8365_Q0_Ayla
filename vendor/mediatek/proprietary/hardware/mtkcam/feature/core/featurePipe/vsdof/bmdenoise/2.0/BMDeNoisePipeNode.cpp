...
onDump(DataID id, ImgInfoMapPtr &data, const char* fileName, const char* postfix)
{
    ...
    // IImageBuffer
    {
        ...
        for(size_t i=0 ; i<set.size() ; ++i){
            ...
            const sp<IImageBuffer>& spBuf = set.valueAt(i);
...
shouldDumpForTuning(BMDeNoiseBufferID BID)
{
    ...
    switch(BID){
        ...
        case BID_INPUT_RSRAW_1:
        case BID_INPUT_RSRAW_2:
        case BID_PRE_PROCESS_OUT_YUV:
        case BID_PRE_PROCESS_OUT_W_1:
        case BID_DENOISE_HAL_OUT:
        case BID_DENOISE_HAL_OUT_ROT_BACK:
        case BID_DENOISE_FINAL_RESULT:
        case BID_JPEG_YUV:
        case BID_SWNR_OUT_YV12:
            return MTRUE;
        default:
            return MFALSE;
    }
