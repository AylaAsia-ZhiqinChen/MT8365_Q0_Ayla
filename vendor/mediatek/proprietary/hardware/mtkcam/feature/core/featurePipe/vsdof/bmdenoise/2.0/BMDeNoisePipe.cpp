...
shouldDoP2Callback()
{
    ...
    if(fastS2S == 1){
        maxCapCnt = get_stereo_bmdenoise_capture_buffer_cnt();
    }

   if(mvRequestPtrMap.size() < maxCapCnt){
        ret = MTRUE;
    }else{
        ret = MFALSE;
    }

    MY_LOGD("fastS2S:%d on-going reqs:%d, maxCapCnt:%d ret:%d",
        fastS2S,
