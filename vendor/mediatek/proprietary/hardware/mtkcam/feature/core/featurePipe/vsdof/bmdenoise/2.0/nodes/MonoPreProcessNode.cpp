    ...
    ImgInfoMapPtr imgInfo)
{
    ...
    SmartImageBuffer smpBuf_small_yuv = mBufPool.getBufPool(BID_MONO_PREPROCESS_SMALL_YUV)->request();

    // applyISP
    {
        trySetMetadata<MUINT8>(&pMeta_InHal_Mod, MTK_3A_ISP_PROFILE, NSIspTuning::EIspProfile_N3D_HighRes_toYUV);
    ...
    bSuccess =
            ...
            .addCrop(eCROP_WROT, MPoint(0,0), pBuf_fullraw_2->getImgSize(), smpBuf_small_yuv->mImageBuffer->getImgSize()) // WROTO
...
#endif
                .insertOutputBuf(iFrameNum, PORT_WROTO, smpBuf_small_yuv->mImageBuffer.get())
    ...
    pEnqueData->addBuffData(BID_MONO_PREPROCESS_SMALL_YUV, smpBuf_small_yuv);
    pEnqueData->start();
    ...
    EnquedBufPool* pEnqueData)
{
    ...
    pEnqueData->mPImgInfo->addSmartBuffer(
        ...
        pEnqueData->mEnquedSmartImgBufMap.valueFor(BID_MONO_PREPROCESS_SMALL_YUV)
    );
...
initBufferPool()
{
    ...
    int allocateSize = get_stereo_bmdenoise_capture_buffer_cnt()*getMFHRCaptureCnt();
    int tuningSize = get_stereo_bmdenoise_capture_buffer_cnt()*getMFHRCaptureCnt();
...

    {
        ...
        NSBMDN::BufferConfig c = {
            ...
            BID_MONO_PREPROCESS_SMALL_YUV,
            (MUINT32)area.size.w/2,
            (MUINT32)area.size.h/2,
            eImgFmt_I422,
            ImageBufferPool::USAGE_HW,
            MTRUE,
            MFALSE,
