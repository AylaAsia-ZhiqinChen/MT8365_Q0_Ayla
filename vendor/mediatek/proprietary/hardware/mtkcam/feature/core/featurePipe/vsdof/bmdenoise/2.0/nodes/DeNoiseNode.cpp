    ...
    , mBufPool(name)
{
    ...
    mDebugPerformanceQualityOption.dsH = ::property_get_int32("vendor.debug.bmdenoise.dsH", -1);
    mDebugPerformanceQualityOption.dsV = ::property_get_int32("vendor.debug.bmdenoise.dsV", -1);
...
MVOID
DeNoiseNode::
collectExifData(map<MINT32, MINT32>& exifData, BWDN_HAL_PARAMS &rInData)
{
    ScopeLogger logger(PIPE_LOG_TAG, __FUNCTION__);
    ...
    StereoArea area;
    area = mSizePrvider->getBufferSize(E_BM_PREPROCESS_FULLRAW_CROP_1);
    exifData[BM_TAG_DENOISE_RATIO_CROP_X] = area.startPt.x;
    exifData[BM_TAG_DENOISE_RATIO_CROP_Y] = area.startPt.y;
    exifData[BM_TAG_DENOISE_RATIO_CROP_W] = area.size.w;
    exifData[BM_TAG_DENOISE_RATIO_CROP_H] = area.size.h;

    area = mSizePrvider->getBufferSize(E_BM_DENOISE_HAL_OUT_ROT_BACK);
    exifData[BM_TAG_DENOISE_FOV_CROP_X] = 0;
    exifData[BM_TAG_DENOISE_FOV_CROP_Y] = 0;
    exifData[BM_TAG_DENOISE_FOV_CROP_W] = area.size.w;
    exifData[BM_TAG_DENOISE_FOV_CROP_H] = area.size.h;
}
/*******************************************************************************
 *
 ********************************************************************************/
...
onThreadLoop()
{
    ...
    ImgInfoMapPtr imgInfo_BMDN = doBMDeNoise(ImgInfo_PreProcess, imgInfo_Depth, pipeRequest);
    ...
    if(imgInfo_BMDN != nullptr){
        ...
        MY_LOGW("ShaneTest temp pass FS_RAW1 to postProcessNode");
        imgInfo_BMDN->addIImageBuffer(
            BID_INPUT_FSRAW_1,
            ImgInfo_PreProcess->getIImageBuffer(BID_INPUT_FSRAW_1)
        );
...
doBMDeNoise(ImgInfoMapPtr imgInfo_PreProcess, ImgInfoMapPtr imgInfo_Depth, PipeRequestPtr request)
{
    ...
    SmartGraphicBuffer smgpBuf_disparityMap = imgInfo_Depth->getGraphicBuffer(BID_DPE_OUT_DMP_L);
    ...
    collectExifData(exifData, input);
    ...
    uint32_t exif_TS = writer.doExifUpdate(request, BMDeNoiseFeatureType::TYPE_BMDN, exifData);
    request->addParam(PID_EXIF_UNIQUE_ID, exif_TS);
...
getDynamicShadingRA(BWDN_HAL_PARAMS &rInData)
{
    ...
    if(pTuningCustom != nullptr){
        ...
        if(ret == -1){
            MY_LOGE("ret == -1 ?, use default instead");
...
initBufferPool()
{
    ...
    if(!mBufPool.init(vBufConfig, TUNING_ALLOC_SIZE)){
