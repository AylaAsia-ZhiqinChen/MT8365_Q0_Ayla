...
#include "Vendor_UT.h"
#include <vsdof/bokeh/3.0/UtilityNode/DebugNode.h>
#include <vsdof/bokeh/3.0/SWNode/VendorFilterNode.h>
namespace VSDOF
{
namespace Bokeh
{
...
namespace UT
{
class Vendor_UT_Thread : public Thread
{
    public:
        Vendor_UT_Thread(MINT32 scenario);
        ~Vendor_UT_Thread();
        MBOOL init(Vendor_UT_Params params);
        MBOOL uninit();
    // thread interface
    ...
    private:
        Vendor_UT*        mVendor_UT = nullptr;
        mutable Mutex     mRequestLock;
        Condition         mRequestCond;
        VendorFilterNode  mVendorNode;
