...
MBOOL TestNodeD::onData(DataID data, const TestBuffer &buffer)
{
  TRACE_FUNC_ENTER();
  ...
  {
  case B2D:
    MY_LOGD("frame %d B2D arrived", buffer.mRequest->mID);
    mBQueue.enque(buffer);
    ret = MTRUE;
    break;
  case C2D:
    MY_LOGD("frame %d C2D arrived", buffer.mRequest->mID);
...
MBOOL TestNodeD::onThreadLoop()
{
  ...
  if( bufferB.mRequest != bufferC.mRequest )
  {
    MY_LOGE("buffer deque out of sync");
    return MFALSE;
  }

  request = bufferB.mRequest;
  MY_LOGD("frame %d in NodeD", request->mID);
