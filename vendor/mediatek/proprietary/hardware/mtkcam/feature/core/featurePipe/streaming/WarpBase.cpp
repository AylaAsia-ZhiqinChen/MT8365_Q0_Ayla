...
MBOOL WarpBase::applyDZWarp(const ImgBuffer &warp, const MSize &inSize, const MSize &outSize, const MRect &zoom, const MSizeF &marginPixel)
{
    TRACE_FUNC_ENTER();
    MBOOL ret = MFALSE;
    ...
    MSize src(inSize.w - marginPixel.w*2, inSize.h - marginPixel.h*2);
    ...
    if( src.w<outSize.w )
    {
        zoomCrop.s.w *= (outSize.w/src.w);
    }
    if( src.h<outSize.h )
    {
        zoomCrop.s.h *= (outSize.h/src.h);
    }

    // Simple checking
    if( marginPixel.w<0 || marginPixel.h<0 )
    {
        MY_LOGW("Invalid marginPixel=(%dx%d)!", marginPixel.w, marginPixel.h);
    }
    else if( grid.w<1 || grid.h<1 )
    {
    ...
    else
    {
        ...
        TRACE_FUNC("GridW=%d, GridH=%d, src_w=%d, src_h=%d, offset_x=%f, offset_y=%f, cropDZ_w=%d, cropDZ_h=%d",
