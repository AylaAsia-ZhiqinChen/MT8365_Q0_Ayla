...
private:
    //For distance calculation in preview
    void    __initAFWinTransform();
    MPoint  __AFToDepthmapPoint(const MPoint &ptAF);
    MRect   __getAFRect(const int AF_INDEX);
    MFLOAT  __getDistance(IImageBuffer *depthmnap, MRect afROI, MFLOAT focalLensFactor);

    const MSize                             AF_ROTATE_DOMAIN;
    const MSize                             DEPTHMAP_SIZE;
    const static size_t                     ROI_STATISTICS_SIZE = 256;  //we'll skip hole value
    int                                     mROIStatistics[ROI_STATISTICS_SIZE];
    int                                     INVALID_DEPTH_VALUE = 255;  //query from tuning later
    ...
    int                                     mAFOffsetY = 0;
    FastLogger                              mLogger;
    bool                                    mDistnaceLogEnabled = false;

    int                                     FOREGROUND_THRESHOLD = property_get_int32("debug.STEREO.dist_th", 25);
};
