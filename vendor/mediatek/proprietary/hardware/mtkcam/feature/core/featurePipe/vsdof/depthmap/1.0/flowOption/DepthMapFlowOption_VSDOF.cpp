...
, YuvQTemplateProvider(pSetting, pOption, this)
...
)
{
    ...
    if(opState == eSTATE_NORMAL)
    {
        if(pRequest->isSkipDepth(mpPipeOption))
            bRet = YuvQTemplateProvider::buildQParams_NORMAL(pRequest, tuningResult, rOutParam);
        else
            bRet = BayerBayerQTemplateProvider::buildQParams_NORMAL(pRequest, tuningResult, rOutParam);
    }
    else if(opState == eSTATE_CAPTURE)
    {
        bRet = BayerBayerQTemplateProvider::buildQParams_CAPTURE(pRequest, tuningResult, rOutParam);
    }
    return bRet;
}
...
)
{
    ...
    if(!pRequest->isSkipDepth(mpPipeOption))
    {
        ...
        //
        pBufferHandler->configOutBuffer(eDPETHMAP_PIPE_NODEID_P2AFM, BID_P2A_OUT_MY_S, eDPETHMAP_PIPE_NODEID_OCC);
        pNode->handleDataAndDump(P2A_TO_OCC_MY_S, pRequest);
    }
    else if(pRequest->isQueuedDepthRequest(mpPipeOption))
    {
        pNode->handleDataAndDump(QUEUED_FLOW_DONE, pRequest);
    }
    // if capture
...
init()
{
    ...
    bRet &= YuvQTemplateProvider::init(mpSizeMgr);
    return bRet;
}
...
buildPipeGraph(DepthMapPipe* pPipe, const DepthMapPipeNodeMap& nodeMap)
{
    ...
    CONNECT_DATA(P2A_OUT_YUV_DONE, *pP2AFMNode, pPipe);
    CONNECT_DATA(P2A_TO_FD_IMG, *pP2AFMNode, *pFDNode);
    CONNECT_DATA(REQUEST_DEPTH_NOT_READY, *pP2AFMNode, pPipe);
    CONNECT_DATA(P2A_OUT_TUNING_BUF, *pP2AFMNode, pPipe);
    CONNECT_DATA(P2A_TO_OCC_MY_S, *pP2AFMNode, *pOCCNode);
    // P2AFM buffer dump
    ...
    {
        CONNECT_DATA(P2A_TO_GF_DMW_MYS, *pP2AFMNode, *pGFNode);
        // queue flow done
