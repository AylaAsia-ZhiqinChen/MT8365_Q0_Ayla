...
doSoftwareNR(PipeRequestPtr request, ImgInfoMapPtr imgInfo)
{
    ...
    // format transform
    {
        AutoProfileUtil proflie(PIPE_LOG_TAG, "doSoftwareNR input format convert");
    ...
    // do SWNR
    {
        AutoProfileUtil proflie(PIPE_LOG_TAG, "do swnr");
        ISwNR::SWNRParam swnrParam;
        swnrParam.isDualCam = MTRUE;
        std::unique_ptr<ISwNR> swnr = std::unique_ptr<ISwNR>(MAKE_SwNR(mSensorIdx_Main1));
        swnrParam.iso = getISOFromMeta(pMeta_InHal_main1);
        MY_LOGD("Run SwNR with iso=%d, sensorIdx:%d", swnrParam.iso, mSensorIdx_Main1);
        {
    ...
    {
        AutoProfileUtil proflie(PIPE_LOG_TAG, "MDP convert result");
        sMDP_Config config;
