...
public:
    ...
    virtual android::sp<IStopWatch> GetStopWatch(MINT32 type, const std::string name) override;
    virtual void BeginStopWatch(const std::string name, void* cookie) override;
    virtual void EndStopWatch(void* cookie) override;
    virtual void PrintLog() override;

    StopWatchCollection& operator=(const StopWatchCollection&) = delete;
    StopWatchCollection(const StopWatchCollection&) = delete;
...
private:
    ...
    std::map<void*, android::sp<ProfileUtil>> mProfileUtilTable;
};

class ImageBufInfoMap;
typedef android::sp<ImageBufInfoMap> ImgInfoMapPtr;
typedef std::map< BMDeNoiseBufferID, sp<IImageBuffer> >                IImageBufferSet;
...
typedef std::map< BMDeNoiseBufferID, sp<mfll::IMfllImageBuffer> >      MfllImageBufferSet;
typedef KeyedVector<BMDeNoiseBufferID, IMetadata* > MetadataPtrSet;
typedef KeyedVector<BMDeNoiseBufferID, IMetadata > MetadataSet;
typedef KeyedVector<BMDeNoiseBufferID, string > ExtBufNameSet;
...
public:
    ...
    // MfllBuffer
    ssize_t addMfllBuffer(const BMDeNoiseBufferID& key, const sp<mfll::IMfllImageBuffer>& value);
    ssize_t delMfllBuffer(const BMDeNoiseBufferID& key);
    const sp<mfll::IMfllImageBuffer>& getMfllBuffer(const BMDeNoiseBufferID& key);
    ...
    // Metadata
    ssize_t addSolidMetadata(const BMDeNoiseBufferID& key, IMetadata const value);
    ssize_t delSolidMetadata(const BMDeNoiseBufferID& key);
    IMetadata getSolidMetadata(const BMDeNoiseBufferID& key);
    // ExtBufName
    ssize_t addExtBufName(const BMDeNoiseBufferID& key, const string& value);
    bool getExtBufName(const BMDeNoiseBufferID& key, string& rString);
...
public:
    RequestDumpHelper()
    {
        mDumpStartIdx = ::property_get_int32("vendor.bmdenoise.pipe.dump.start", 0);
        mDumpBufSize = ::property_get_int32("vendor.bmdenoise.pipe.dump.size", 0);
        mIsDoPorter = ::property_get_int32("vendor.bmdenoise.porter.dump", 0);

        MY_LOGD("startIndex: %d, dumpBufSize: %d, isDoPorter: %d", mDumpStartIdx, mDumpBufSize, mIsDoPorter);
    }
...
* FOVCropCalculator
********************************************************************************/
class FOVCropCalculator
{
public:
    FOVCropCalculator(MBOOL isToCropMain);

    MRect   getResultRegion(const MSize& srcImgSize);
    string  toString();

    void setRatio(float wFactor, float hFactor);
    void setRatio(ENUM_STEREO_RATIO eRatio);
    void setRatio(const MSize& size);
    void setModuleConfig(SensorModuleConfig moduleConfig);
    void setWorkingRangeNear(float workingRangeNear);
    void setRelRotToleranceXY(float relRotToleranceXY);
    void setIsToCropMain(MBOOL isToCropMain);
    void setbaseLine(float baseLine);
    void setSensorFOV(const SensorFOV& main1FOV, const SensorFOV& main2FOV);

private:
    inline float degree2Radian(float degree);

private:
    MBOOL mIsToCropMain = false;
    SensorFOV mMain1FOV = {66.0f, 52.2f};
    SensorFOV mMain2FOV = {66.0f, 52.2f};

    float mBaseline = 0.95f;
    float mWorkingRangeNear = 20.0f; //cm

    float mOutputWidthFactor = 4.0f ;
    float mOutputHeightFactor = 3.0f ;
    float mAdditionalToleranceCoeff = 1.0f;
    float mRelRotToleranceXY = 1.0f;

    SensorModuleConfig mModuleConfig = SensorModuleConfig::MainAtRight;
};
/*******************************************************************************
