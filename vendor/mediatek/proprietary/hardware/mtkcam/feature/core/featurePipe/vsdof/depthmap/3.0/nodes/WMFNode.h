...
private:
     ...
     * @param [in] pBufferHandler Current effect request buffer handler
    ...
    MBOOL prepareWMFEnqueConfig(
                            ...
                            EGNParams<OWMFEConfig> &rEnqueParam);
    /**
     ...
     */
    static MVOID onWMFEnqueDone(EGNParams<OWMFEConfig>& rParams);
     /**
     ...
     */
    MVOID debugWMFParam(EGNParams<OWMFEConfig> &config);
...
private:
    ...
    // DPE stream
    IEgnStream<OWMFEConfig>* mpOweStream = nullptr;
    // TBLI buffer pool
    android::sp<FatImageBufferPool> mpTbliImgBufPool;
    // tuning parameter: WMFECtrl
    vector<NSIoPipe::OWMFECtrl> mvWMFECtrl;
    // tuning buffer
    vector<SmartFatImageBuffer> mvTbliImgBuf;
