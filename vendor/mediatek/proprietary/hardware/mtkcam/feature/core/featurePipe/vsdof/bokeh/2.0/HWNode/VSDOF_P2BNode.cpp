    ...
    SmartImageBuffer dmbgBuf*/)
{
    ...
    /*if(miMode == ADVANCE)
    {
        ...
        setP2BInputPort(frameParams, dmgBuf, BOKEH_ER_BUF_DMBG );
    }*/
    // DMBG setting
    /*dmbgBuf->mImageBuffer->syncCache(eCACHECTRL_FLUSH);
    setP2BInputPort(frameParams, dmbgBuf, BOKEH_ER_BUF_DMG);*/
    /*if(scenarioId == ENUM_STEREO_SCENARIO::eSTEREO_SCENARIO_CAPTURE)
    {
        pWDMABuffer = mpWDMABufPool->request();
        setP2BOutputPort(frameParams, pWDMABuffer, BOKEH_ER_BUF_WDMAIMG);
    }*/
    // get imgi size
    ...
    FrameParams& frameParam)
{
    ...
    for(MINT32 i = 0;i<request->vInputFrameInfo.size();++i)
    {
        ...
        if(BOKEH_ER_BUF_P2_TUNING == pFrameInfo->getFrameNo())
        {
            ...
            {
                pFrameInfo->getFrameBuffer(frame);
            }
            else
            {
                continue;
            }
            if(!StereoTuningProvider::getBokehTuningInfo((void*)frame->getBufVA(0)))
            {
                MY_LOGE("set tuning parameter fail.");
                return MFALSE;
            }
            frameParam.mTuningData = (void*)frame->getBufVA(0);
            return MTRUE;
        }
        else
        {
    ...
    MUINT32 bufferType)
{
    ...
    MY_LOGD("bufDataType(%d)", bufDataType);
    ...
    QParams& rParams)
{
    ...
    if(mbDumpImageBuffer)
    {
            ...
            for(const auto& outParam : frameParam.mvOut)
        {
            ...
            {
                    saveFileName = string("/sdcard/vsdof/bokeh/result/p2b/WDMA_")+
                                   to_string(scenarioId)+ string("_")+
                                   to_string(request->getRequestNo())+
                                   string(".yuv");
                }
                else if(portId == PORT_WROTO)
                {
                    saveFileName = string("/sdcard/vsdof/bokeh/result/p2b/WROT_")+
                                   to_string(scenarioId)+ string("_")+
                                   to_string(request->getRequestNo())+
                                   string(".yuv");
            }
