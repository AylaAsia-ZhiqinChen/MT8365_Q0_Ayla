...
{
    MY_LOGD("P2B_UT_Thread: ctor(0x%x)", this);
    //
    mP2BNode.connectData(P2B_MDP_REQUEST, P2B_MDP_REQUEST, &mDebugNode, CONNECTION_DIRECT);
    mP2BNode.connectData(P2B_MDP_WDMA, P2B_MDP_WDMA, &mDebugNode, CONNECTION_DIRECT);
    mP2BNode.connectData(P2B_OUT, P2B_OUT, &mDebugNode, CONNECTION_DIRECT);
}
/*******************************************************************************
 *
 ********************************************************************************/
...
~P2B_UT_Thread()
{
    MY_LOGD("~P2B_UT_Thread: dctor(0x%x)", this);
}
/*******************************************************************************
 *
 ********************************************************************************/
...
init(P2B_UT_Params params)
{
    ...
    mDebugNode.start();
    //
    MY_LOGD("Eric:mP2B_UT init start");
    mP2B_UT = new P2B_UT();
    mP2B_UT->init(params);

    MY_LOGD("Eric:mP2B_UT init end");
    //
    MY_LOGD("-");
    return MTRUE;
}
...
uninit()
{
    MY_LOGD("P2B_UT_Thread isRunning(%d)", isRunning());
    mbIsExit = MTRUE;
    if(isRunning())
    {
        requestExit();
...
threadLoop()
{
    MY_LOGD("Eric:P2B_UT_Thread isRunning");
    MY_LOGD("+");
    ...
    sp<NSCam::EffectRequest>& request)
{
    ...
    //
    MY_LOGD("requestId(%d), scenario(%d), bokehLevel(%d)",
