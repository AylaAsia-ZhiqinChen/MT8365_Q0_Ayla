...
copyISPProfiles(char* targetPath)
{
    ...
    #define MAKE_CMD(profile_name) \
        ...
        systemCall(cmd);\
        snprintf(cmd, 1024, "mv /sdcard/debug/*-%d-*-%d.bin %s/debug/%s_sensor_%d", sensorDev_Main1, profile_name, targetPath, #profile_name, sensorDev_Main1);\
        systemCall(cmd);\
        snprintf(cmd, 1024, "mkdir %s/debug/%s_sensor_%d", targetPath, #profile_name, sensorDev_Main2);\
        systemCall(cmd);\
        snprintf(cmd, 1024, "mv /sdcard/debug/*-%d-*-%d.bin %s/debug/%s_sensor_%d", sensorDev_Main2, profile_name, targetPath, #profile_name, sensorDev_Main2);\
        systemCall(cmd);
    ...
    MAKE_CMD(EIspProfile_MFNR_Before_Blend);
    MAKE_CMD(EIspProfile_MFNR_Single);
    MAKE_CMD(EIspProfile_MFNR_MFB);
    MAKE_CMD(EIspProfile_MFNR_After_Blend);

    MAKE_CMD(EIspProfile_N3D_MFHR_Before_Blend);
    MAKE_CMD(EIspProfile_N3D_MFHR_Single);
    MAKE_CMD(EIspProfile_N3D_MFHR_MFB);
    MAKE_CMD(EIspProfile_N3D_MFHR_After_Blend);
...
MVOID
BMHelperNode::
copyRawData(char* targetPath, MINT32 reqNo)
{
    ScopeLogger logger(PIPE_LOG_TAG, __FUNCTION__);
    ...
    char cmd[1024];
    snprintf(cmd, 1024, "cp -R /sdcard/bmdenoise/%d/%d/RootNode/*.raw %s", reqNo, reqNo, targetPath);
    systemCall(cmd);

    snprintf(cmd, 1024, "cp -R /sdcard/bmdenoise/%d/%d/RootHRNode/*.raw %s", reqNo, reqNo, targetPath);
    systemCall(cmd);
}
/*******************************************************************************
 *
...
MVOID
BMHelperNode::
copyTuningData(char* targetPath, MINT32 reqNo, MBOOL isMFHR)
{
    ScopeLogger logger(PIPE_LOG_TAG, __FUNCTION__);

    char makeCmd[1024];
    char tuningPath[1024];
    char moveCmd[1024];
    //Copy 0 to temp folder
    snprintf(makeCmd, 1024, "/sdcard/bmdenoise/temp");
    makePath(makeCmd, 0777);
    snprintf(moveCmd, 1024, "cp -r /sdcard/bmdenoise/%d /sdcard/bmdenoise/temp", reqNo);
    systemCall(moveCmd);
    //Remove original folder
    snprintf(moveCmd, 1024, "rm -rf /sdcard/bmdenoise/%d/*", reqNo);
    systemCall(moveCmd);
    //Copy from temp folder to target folder
    snprintf(tuningPath, 1024, "%s", targetPath);
    MY_LOGD("copyTuningData makePath 0: %s", tuningPath);
    makePath(tuningPath, 0777);
    snprintf(moveCmd, 1024, "cp -a %s/* %s", makeCmd, tuningPath);
    MY_LOGD("copyTuningData move data cmd 0: %s", moveCmd);
    systemCall(moveCmd);
    //remove temp folder
    snprintf(makeCmd, 1024, "rm -rf /sdcard/bmdenoise/temp");
    system(makeCmd);

    if(isMFHR)
    {
        //Move other 1,2..etc to target folder
        for(int j=1; j<=getMFHRCaptureCnt(); j++)
        {
            snprintf(moveCmd, 1024, "cp -r /sdcard/bmdenoise/%d %s", reqNo+j, tuningPath);
            MY_LOGD("MFHR move data cmd : %s", moveCmd);
            systemCall(moveCmd);
            //remove folder
            snprintf(moveCmd, 1024, "rm -rf /sdcard/bmdenoise/%d", reqNo+j);
            systemCall(moveCmd);
        }
    }
}
...
doPorterJob(PipeRequestPtr request)
{
    ...
    snprintf(targetPath, 1024, "/sdcard/bmdenoise/%d", iReqIdx);

    // copy isp profiles
    copyISPProfiles(targetPath);

    // copy MFHR debug buffers
    MINT32 isMainFrame = request->getParam(PID_MFHR_IS_MAIN);
    MY_LOGD("MFHR isMainFrame : %d", isMainFrame);
    MINT32 isMFHR = request->getParam(PID_MFHR_IS_ON);
    MY_LOGD("MFHR isON : %d", isMFHR);
    if(isMainFrame == 1)
    {
        copyTuningData(targetPath, iReqIdx, MTRUE);
    }
    else if(isMFHR == -1)
    {
        copyTuningData(targetPath, iReqIdx, MFALSE);
    }

    // copy p1 raw buffers
    snprintf(targetPath, 1024, "/sdcard/bmdenoise/%d_raw", iReqIdx);
    copyRawData(targetPath, iReqIdx);
}
/*******************************************************************************
 *
 ********************************************************************************/
