 ...
 */
#ifndef _MTK_CAMERA_VSDOF_BMDENOISE_FEATURE_PIPE_DCBUFFER_POOL_H_
#define _MTK_CAMERA_VSDOF_BMDENOISE_FEATURE_PIPE_DCBUFFER_POOL_H_
...
struct MfllBufConfig{
    MBOOL isMfllBuffer = MFALSE;
    MINT32 alignment = -1;
};

struct BufferConfig{
    const char* name;
    DualCamMFBufferID bufID;
    MUINT32 width;
    MUINT32 height;
    EImageFormat format;
    MUINT32 usage;
...
class DCBufferPool
{
    public:
         ...
         */
        MBOOL init(Vector<BufferConfig> bufConfig, MUINT32 preAllocTuningBufferCount = 0, MUINT32 maxTuningBufferCount = 0);
         ...
         */
        android::sp<ImageBufferPool> getBufPool(DualCamMFBufferID bufID);
         ...
         *-the pointer indicates success; nullptr indidcated failed
         */
        android::sp<GraphicBufferPool> getGraphicBufPool(DualCamMFBufferID bufID);

        /**
         * @brief to get the MfllBuffer
         * @return
         *-the pointer indicates success; nullptr indidcated failed
         */
        android::sp<mfll::IMfllImageBuffer> getMfllBuffer(DualCamMFBufferID bufID);
         ...
         *-the pointer indicates success; nullptr indidcated failed
         */
        static android_pixel_format_t getPixelFormat(EImageFormat eFmt);

        // clean up mpINormalStream
        MVOID cleanUp();

    private:
        mfll::ImageFormat toMfllImgFmt(EImageFormat eFmt);
    ...
    private:
        ...
        // Image buffers
        KeyedVector< DualCamMFBufferID, ImageBufferSet > mvImageBufSets;

        // Graphic buffers
        KeyedVector< DualCamMFBufferID, GraphicBufferSet > mvGraphicBufSets;

        // tuning buffers
        android::sp<TuningBufferPool>   mpTuningBufferPool;

        // mfll buffers
        KeyedVector< DualCamMFBufferID, BufferConfig > mvMfllBufConfig;
...
#endif // _MTK_CAMERA_VSDOF_BMDENOISE_FEATURE_PIPE_DCBUFFER_POOL_H_