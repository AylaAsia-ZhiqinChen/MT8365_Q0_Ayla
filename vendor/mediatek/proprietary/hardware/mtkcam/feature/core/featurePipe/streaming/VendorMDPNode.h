...
private:
    MBOOL processMDP(const RequestPtr &request, const BasicIOImg &fullIOImg);
    MBOOL prepareMDPOut(const RequestPtr &request, const BasicIOImg &fullIOImg, MDPWrapper::OUTPUT_ARRAY& outputs);
private:
    WaitQueue<BasicIOImgData> mData;
    WaitQueue<RequestPtr> mN3DReadyData;
    MDPWrapper mMDP;
};
