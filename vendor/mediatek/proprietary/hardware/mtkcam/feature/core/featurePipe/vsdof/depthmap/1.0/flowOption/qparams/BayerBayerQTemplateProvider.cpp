...
MVOID
BayerBayerQTemplateProvider::
setupPQParamSetting(VSDOFParam* pVSDOFParam, MUINT iFrameID)
{
    MY_LOGD("+");

    ENUM_PASS2_ROUND pass2Round;
    convertFrameIdToPass2Round(iFrameID, pass2Round);
    StereoTuningProvider::getPass2MDPPQParam(pass2Round, *pVSDOFParam);

    VSDOF_LOGD("setup pVSDOFParam, frame=%d, isRefocus=%d", iFrameID, pVSDOFParam->isRefocus);
    VSDOF_LOGD("setup pVSDOFParam, frame=%d, defaultUpTable=%d", iFrameID, pVSDOFParam->defaultUpTable);
