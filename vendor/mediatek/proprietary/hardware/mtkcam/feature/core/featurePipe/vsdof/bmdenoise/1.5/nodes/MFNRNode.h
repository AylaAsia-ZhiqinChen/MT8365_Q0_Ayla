...
class MFNRNode : public BMDeNoisePipeNode
{
    ...
    private:
        ...
        mfll::MfllMode              getMfbMode(){return mfll::MfllMode_ZsdMfhr;};
        mfll::NoiseReductionType    getPostNRMode(){return mfll::NoiseReductionType_None;};
        MINT32                      getIsFullSizeMc(){return 1;};
        MINT32                      getRealIso(IMetadata* pAppMeta);
        MINT32                      getFinalIso(IMetadata* pAppMeta);
        ...
        MBOOL                       processMixedYuv(ImgInfoMapPtr& ImgInfo_Main);

    private:
        WaitQueue<ImgInfoMapPtr>                        mImgInfoRequests;
        WaitQueue<PipeRequestPtr>                       mRequests;
        WaitQueue< Vector<BM_BSS_RESULT> >              mBSSResults;
        ...
        mutable Mutex                                   mLock;
        mutable Condition                               mCondPushData;

        MBOOL                                           mbUninit = MFALSE;
};
};
};
};
