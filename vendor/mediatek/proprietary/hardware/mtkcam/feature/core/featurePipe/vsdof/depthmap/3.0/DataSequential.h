...
public:
    ...
    SequenceOPState sequentialize(const T& data)
    {
        ...
        VSDOF_LOGD("%s sequentialize reqID=%d mOnGoingDataQueue size=%d",
    ...
    {
        RWLock::AutoWLock _l(mLock);
        VSDOF_LOGD("%s Mark onging reqID=%d", mpUser, mfSeqGetter(data));
        mOnGoingDataQueue.push_back(data);
        miLatestOnGoingStamp = mfSeqGetter(data);
        VSDOF_LOGD("%s, debugSeq=%s", mpUser, debugSeq());
        return MTRUE;
    }
    /**
    ...
    {
        RWLock::AutoWLock _l(mLock);
        VSDOF_LOGD("%s markFinishAndPop=%d, debugSeq=%s",
                    mpUser, iFinishSeq, debugSeq());
        // limitation check
        if(mfSeqGetter(mOnGoingDataQueue.front()) != iFinishSeq)
        {
            MY_LOGE("Error, un-reasonable markFinish iFinishSeq=%d debugSeq=%s",
                    iFinishSeq, debugSeq() );
            return MFALSE;
        }
        ...
        if(mOnGoingDataQueue.size() == 1)
        {
            ...
            while(!mReadyDataQueue.empty())
            {
                ...
                VSDOF_LOGD("%s record all pop_id=%d",
        ...
        else
        {
            ...
            while(!mReadyDataQueue.empty())
            {
                ...
                if(bReverse)
                {
                    ...
                    {
                        rOutDataVec.add(mReadyDataQueue.front());
                        VSDOF_LOGD("%s , reversed, pop_id=%d.",
                            mpUser, mfSeqGetter(mReadyDataQueue.front()));
                        // pop
                        mReadyDataQueue.pop_front();
                    }
...
public:
    ...
    {
        RWLock::AutoWLock _l(mLock);
        VSDOF_LOGD("%s Mark enque reqID=%d", mpUser, mfSeqGetter(data));
    ...
    {
        RWLock::AutoWLock _l(mLock);
        VSDOF_LOGD("%s Mark deque reqID=%d latest:%d",
                    mpUser, mfSeqGetter(data), mfSeqGetter(mRequestQueue.back()));

        if(mfSeqGetter(data) != mfSeqGetter(mRequestQueue.front()))
        {
            MY_LOGE("The enque order does not consistent, deque req=%d front req=%d",
