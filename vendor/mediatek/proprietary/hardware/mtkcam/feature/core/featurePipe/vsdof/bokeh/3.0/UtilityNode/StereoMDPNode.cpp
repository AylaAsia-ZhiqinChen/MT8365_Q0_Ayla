    ...
    , miOpenId(openId)
{
    ...
    this->addWaitQueue(&mRequests_Vendor);
}
/*******************************************************************************
 *
 ********************************************************************************/
    ...
    EffectRequestPtr &request)
{
    ...
    switch(id)
    {
        case P2B_MDP_REQUEST:
            ...
            break;
        case VENDOR_TO_MDP:
            mRequests_Vendor.enque(request);
            ret = MTRUE;
            break;
        default:
            ret = MFALSE;
...
onThreadLoop()
{
    ...
    else if (mRequests_Vendor.deque(request))
    {
        if (!processPreviewCBRequest(request)) {
            MY_LOGE("processPreviewCBRequest fail !!, reqID=%d", request->getRequestNo());
            return MFALSE;
        }
        handleData(MDP_OUT, request);
    }
...
MBOOL
StereoMDPNode::
processPreviewCBRequest(EffectRequestPtr request)
{
    MY_LOGD_IF(miPipeLogEnable, "reqID=%d processPreviewCBRequest", request->getRequestNo());
    auto getOutpuBuffer = [&](BokehEffectRequestBufferType type) -> IImageBuffer*
    {
        sp<IImageBuffer> pBuf = nullptr;
        ssize_t keyIndex = request->vOutputFrameInfo.indexOfKey(type);
        if(keyIndex >=0)
        {
            auto frameInfo = request->vOutputFrameInfo.valueAt(keyIndex);
            frameInfo->getFrameBuffer(pBuf);
        }
        return pBuf.get();
    };

    IImageBuffer* pBuf_PV = getOutpuBuffer(BOKEH_ER_BUF_DISPLAY);
    IImageBuffer* pBuf_CB = getOutpuBuffer(BOKEH_ER_BUF_PRVCB);

    if(pBuf_PV == nullptr || pBuf_CB == nullptr)
    {
        MY_LOGE("Failed to get output buffer, PV:%x PrvCB:%x", pBuf_PV, pBuf_CB);
        return MFALSE;
    }

    if(!processImageByMDP(pBuf_PV, pBuf_CB, 0))
    {
        MY_LOGE("Failed to do MDP copy!");
        return MFALSE;
    }
    ...
    if(mbDumpImageBuffer)
    {
        ...
        // TODO: let this flow can get the main1 hal metadata
        if(!getInputMetadataFromRequest(request, BOKEH_ER_IN_HAL_META_MAIN1, pMeta_InHal))
        {
            ...
            //
            extract(&mDumpHint, pBuf_CB);
            genFileName_YUV(writepath, PATH_SIZE, &mDumpHint,
                            TuningUtils::YUV_PORT_UNDEFINED, "PRVCB");
            pBuf_CB->saveToFile(writepath);
            // DISPLAY
            extract(&mDumpHint, pBuf_PV);
            genFileName_YUV(writepath, PATH_SIZE, &mDumpHint,
                            TuningUtils::YUV_PORT_UNDEFINED, "DISPLAY_PRVCBINPUT");
            pBuf_PV->saveToFile(writepath);
        }
    }
    return MTRUE;
