        ...
        case QUEUED_FLOW_DONE:
        {
            ...
            if(index>=0)
            {
                ...
                if(requestData.status == eREQ_STATUS_WAIT_BG_QUEUED)
                {
                    ...
                    VSDOF_LOGD("Remove request reqID=%d for queue flow finish.", iReqID);
                }
...
onRequestDepthNotReady(DepthMapRequestPtr pRequest)
{
    ...
    if(requestData.status == eREQ_STATUS_FG_FLOW)
    {
        ...
        // callback
        sp<EffectRequest> pReq = (EffectRequest*) pRequest.get();
        pRequest->mpOnRequestProcessed(pRequest->mpTag, String8( DEPTHMAP_NOT_READY_KEY ), pReq);
    }
    ...
    else if(requestData.status == eREQ_STATUS_WAIT_FG_FLOW)
    {
        ...
        VSDOF_LOGD("Remove request reqID=%d for depth not readu.", pRequest->getRequestNo());
    }
    else
    {
...
onErrorNotify(DepthMapRequestPtr &request)
{
    ...
    // remove request
    mvRequestDataMap.removeItem(reqID);
    VSDOF_LOGD("Remove request reqID=%d for error notify.", reqID);
...
onDataSuccess(DepthMapRequestPtr &request)
{
    ...
    if(mvRequestDataMap.valueAt(reqPos).status == eREQ_STATUS_WAIT_BG_QUEUED)
        return MTRUE;
    else if(reqPos < 0)
    {
    ...
    if(request->checkAllOutputReady())
    {
        ...
        {
            mvRequestDataMap.removeItem(reqID);
            VSDOF_LOGD("Remove request reqID=%d for data success.", reqID);
        }
        // callback
        sp<EffectRequest> pReq = (EffectRequest*) request.get();
...
onHandleYUVDone(DepthMapRequestPtr request)
{
    ...
    // callback
    sp<EffectRequest> pReq = (EffectRequest*) request.get();
    request->mpOnRequestProcessed(request->mpTag, String8( DEPTHMAP_YUV_DONE_KEY ), pReq);
