...
execute(MdpConfig& config)
{
    ...
    if( config.pSrcBuffer )
    {
        ...
        MY_LOGD("src(%dx%d_%d) custom(%dx%d) useCusStride(%d) specPlane(%d) => config(%dx%d_%d)",
    ...
    if( config.pDstBuffer )
    {
        ...
        MY_LOGD("dst(%dx%d_%d) custom(%dx%d) useCusStride(%d) specPlane(%d) => config(%dx%d_%d) | trans(%d) => rot(%d) flip(%d) | setPQ(%d)",
