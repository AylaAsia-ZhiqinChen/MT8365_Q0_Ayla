...
onThreadLoop()
{
    ...
    enquedData->start();
    imgInfo->getRequestPtr()->getStopWatchCollection()->BeginStopWatch("do_p2af", (void*)enquedData);

    enquedData->mProcessId = BMDeNoiseProcessId::P2AFM;
    {
        CAM_TRACE_BEGIN("P2AFMNode::NormalStream::enque");
        sp<IStopWatch> stopWatch = imgInfo->getRequestPtr()->getStopWatchCollection()->GetStopWatch(eStopWatchType::eSTOPWATCHTYPE_AUTO, "p2af_node_normalstream_enque");
...
prepareBurstQParams_CAP(MINT32 iModuleTrans)
{
    ...
    //--> frame 2
    {
        ...
        QParamTemplateGenerator(2, mSensorIdx_Main2, ENormalStreamTag_Normal).   // frame 2
            ...
            addCrop(eCROP_WDMA, zeroPos, P2AFM_FE2BO_INPUT_SIZE, P2AFM_RECT_IN2_CAP_SIZE).  // WDMA : Rect_in2
    ...
    //--> frame 3
    {
        ...
        QParamTemplateGenerator(3, mSensorIdx_Main1, ENormalStreamTag_Normal).   // frame 3
            ...
            addCrop(eCROP_WDMA, zeroPos, P2AFM_FE1BO_INPUT_SIZE, P2AFM_RECT_IN1_CAP_SIZE).  // WDMA : Rect_in1
...
buildQParams_CAP(ImgInfoMapPtr &imgInfo, QParams& rEnqueParam, EnquedBufPool* pEnquePoolData)
{
    ...
    MY_LOGD("P2AFM main1 FSRAW(%p) main2 FSRAW(%p)", pBuf_INPUTRAW1.get(), pBuf_INPUTRAW2.get());

    CHECK_OBJECT(pBuf_INPUTRAW1);
    CHECK_OBJECT(pBuf_INPUTRAW2);
	...
	#define ADD_REQUEST_GRAPHIC_BUFFER_WITH_AREA_TO_MVOUT(bufID, reqBuf, validSize, filler, frameID, portID) \
        pEnquePoolData->addGBuffData(bufID, reqBuf); \
    ...
    //--> frame 0
    {
        ...
        ISPTuningConfig ispConfig = {pMeta_InApp, &pMeta_InHal_Mod_main2, mp3AHal_Main2, MFALSE, reqNo};
    ...
    //--> frame 2
    {
        ...
        ADD_REQUEST_GRAPHIC_BUFFER_WITH_AREA_TO_MVOUT(BID_P2AFM_OUT_RECT_IN2_CAP, rectIn2Buf, P2AFM_RECT_IN2_CAP_IMG_SIZE, qParamFiller, iFrameNum, mapToPortID(BID_P2AFM_OUT_RECT_IN2_CAP));
    ...
    //--> frame 3
    {
        ...
        ADD_REQUEST_BUFFER_WITH_AREA_TO_MVOUT(BID_P2AFM_OUT_RECT_IN1_CAP, rectIn1Buf, P2AFM_RECT_IN1_CAP_IMG_SIZE, qParamFiller, iFrameNum, mapToPortID(BID_P2AFM_OUT_RECT_IN1_CAP));
...
)
{
    ...
    MY_LOGD("+, reqID=%d bIsResized=%d", ispConfig.reqNo, ispConfig.bInputResizeRaw);
    ...
    // DONT DO THIS! hal meta write back will be done by other node.
    // (*pMeta_InHal) += resultMeta.halMeta;
    #endif

    MY_LOGD("-, reqID=%d setIsp_time=%f", ispConfig.reqNo, timer.countTimer());
...
handleP2Done(QParams& rParams, EnquedBufPool* pEnqueBufPool)
{
    ...
    request->getStopWatchCollection()->EndStopWatch((void*)pEnqueBufPool); // do_p2af
...
initBufferPool()
{
    ...
    // frame 2
    pSizePrvder->getPass2SizeInfo(PASS2A_P_2, eSTEREO_SCENARIO_CAPTURE, pass2SizeInfo);
    P2AFM_RECT_IN2_CAP_SIZE = pass2SizeInfo.areaWDMA.size;
    P2AFM_RECT_IN2_CAP_IMG_SIZE = pass2SizeInfo.areaWDMA.contentSize();

    // frame 3
    pSizePrvder->getPass2SizeInfo(PASS2A_2, eSTEREO_SCENARIO_CAPTURE, pass2SizeInfo);
    P2AFM_RECT_IN1_CAP_SIZE = pass2SizeInfo.areaWDMA.size;
    P2AFM_RECT_IN1_CAP_IMG_SIZE = pass2SizeInfo.areaWDMA.contentSize();
    ...
    // Capture RECT_IN1
    {
        NSBMDN::BufferConfig c = {
            ...
            (MUINT32)P2AFM_RECT_IN1_CAP_SIZE.w,
            (MUINT32)P2AFM_RECT_IN1_CAP_SIZE.h,
            eImgFmt_YV12,
            ImageBufferPool::USAGE_HW,
            MTRUE,
            MFALSE,
    ...
    // Capture IN2(graphic buffer)
    {
        NSBMDN::BufferConfig c = {
            ...
            (MUINT32)P2AFM_RECT_IN2_CAP_SIZE.w,
            (MUINT32)P2AFM_RECT_IN2_CAP_SIZE.h,
            eImgFmt_YV12,
