...
bool MfllMemc::getForceMe(void* param_addr, size_t param_size)
{
    if ( param_size != sizeof(MFBLL_SET_PROC_INFO_STRUCT)) {
        CAM_LOGE("%s: invalid sizeof param, param_size:%d, sizeof(MFBLL_SET_PROC_INFO_STRUCT):%d",
    ...
    if (r != -1) {
        CAM_LOGI("%s: Force Proc1_bad_range = %d (original:%d)", __FUNCTION__, r, param->Proc1_bad_range);
    ...
    if (r != -1) {
        CAM_LOGI("%s: Force Proc1_bad_TH = %d (original:%d)", __FUNCTION__, r, param->Proc1_bad_TH);
