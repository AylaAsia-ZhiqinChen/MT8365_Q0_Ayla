...
MfllCore::MfllCore(void)
    ...
    , m_internalYuvFmt(InputYuvFmt_Nv12)
...
enum MfllErr MfllCore::doMfll()
{
    ...
    if (CC_UNLIKELY(m_spProperty->isDump())) {
        ...
        /* construct dump file path and increase requestKey as serial number */
        int serialNumber = 0;
        MfllMiddlewareInfo_t info;
        info.requestKey = serialNumber;
...
enum MfllErr MfllCore::setCaptureResolution(unsigned int width, unsigned int height)
{
    ...
    if (m_bDoDownscale && m_downscaleRatio > 0) {
        m_width = width/m_downscaleRatio;
        m_height = height/m_downscaleRatio;
    }
    else {
        m_width = width;
        m_height = height;
...
enum MfllErr MfllCore::setSyncPrivateData(const std::deque<void*>& dataset)
{
    ...
    if (CC_LIKELY( dataset.size() >= 3)) {
        ...
        if (CC_UNLIKELY( m_timestampSync.size() == 1 )) {
            ...
            mfllLogD3("%s:=========================", __FUNCTION__);
            mfllLogD3("%s: Get uniqueKey -> %d, uniqueKey->: %" PRIi64, __FUNCTION__, uniqueKey.result, uniqueKey.val);
            mfllLogD3("%s:=========================", __FUNCTION__);
