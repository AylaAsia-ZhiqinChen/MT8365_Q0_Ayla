...
namespace mfll_property {
    static int _dbgLevel = ::property_get_int32("persist.mtk.camera.log_level", 0);
}
...
bool MfllProperty::getForceBssOrder(vector<int> &bssOrder)
{
    ...
    if (0 == strcmp(str, "-1")) {
        ...
        for (size_t i = 0; i < framesCount; i++) {
            CAM_LOGD("%s: original bss order[%zu]=%d", __FUNCTION__, i, bssOrder[i]);
        }
        // force bss order
        string bssStr(str);

        uint32_t forcedBssOrderByHex = std::stoi(bssStr, 0, 16);
        CAM_LOGD("%s: forced bss order by hex:0x%X", __FUNCTION__, forcedBssOrderByHex);
        ...
        for ( int i = (BSS_ORDER_MAX_FRAMES_COUNT-1) ; i >= 0 ; i--) {
            ...
            CAM_LOGD("%s: forcedBssOrder[%d]:%X", __FUNCTION__, i, indexNum);
        ...
        if (forcedOrderFramesCount != framesCount) {
            CAM_LOGW("%s: forcedOrderFramesCount(%zu) is not equal to original frames count(%zu)",
        ...
        for (size_t i = 0; i < framesCount; i++) {
            if (forcedBssOrder[i] >= framesCount) {
                CAM_LOGE("%s: forcedBssOrder(%d) is out of original frames count(%zu)",
        ...
        for (size_t i = 0; i < framesCount; i++) {
            ...
            CAM_LOGD("%s: forced bss order[%zu]=%d", __FUNCTION__, i, bssOrder[i]);
        }
...
bool MfllProperty::getForceGmv(MfllMotionVector_t (&globalMv)[MFLL_MAX_FRAMES])
{
    ...
    if (0 == strcmp(str, "-1")) {
        ...
        for (int i = 0; i < MFLL_MAX_FRAMES; i++) {
            CAM_LOGD("%s: original GMV[%d] (x,y) = (%d,%d)", __FUNCTION__, i, globalMv[i].x, globalMv[i].y);
            globalMv[i].x = 0;
            globalMv[i].y = 0;
        }
        ...
        char *delim = " ),(;";
        char * pch;
        CAM_LOGD("%s: Force GMV setting: \"%s\"", __FUNCTION__, str);
        ...
        while (pch != NULL)
        {
            ...
            if (setting_count%2 != 0) {
                CAM_LOGD("%s: GMV[%d].x=%d", __FUNCTION__, frame_count, value);
                globalMv[frame_count].x = value;
            }
            else {
                CAM_LOGD("%s: GMV[%d].y=%d", __FUNCTION__, frame_count, value);
                globalMv[frame_count].y = value;

                frame_count++;
                if (frame_count >= MFLL_MAX_FRAMES) {
                    CAM_LOGW("%s: out of globalMv's MFLL_MAX_FRAMES:%d", __FUNCTION__, frame_count);
                    break;
                }
            }
