...
void MfllFeatureDump::doneEvent(enum EventType t, MfllEventStatus_t &status, void *mfllCore, void *param1, void* param2)
{
    ...
    info.requestKey = static_cast<int>(index);
    ...
    case EventType_MotionCompensation:
        if (m_dumpFlag.yuv){
            info.requestKey = static_cast<int>(index + 1);
...
void MfllFeatureDump::dumpMfbStage(IMfllCore *pCore, int index)
{
    ...
    auto dp = [&](IMfllImageBuffer* pBuf, const char* bufferName, const char* extName)
    {
        __dump_image(pBuf, MFLL_DUMP_FILE_NAME_OTHER, "mfb", bufferName, extName, info);
    };

    /* base frame */
    dp( c->retrieveBuffer(MfllBuffer_BaseYuv).get(), "base_in", "nv12");

    /* reference frame */
    dp( c->retrieveBuffer(MfllBuffer_ReferenceYuv).get(), "ref_in", "nv12");
    ...
    /* output frame */
    dp( c->retrieveBuffer(MfllBuffer_BlendedYuv).get(), "blended", "nv12");
}
...
void MfllFeatureDump::dumpMixStage(IMfllCore *pCore)
{
    ...
    /* golden frame */
    dp( c->retrieveBuffer(MfllBuffer_GoldenYuv).get(), "golden", "nv12");

    /* output frame */
    if (c->getMixingBufferType() == MixYuvType_Working) {
        dp( c->retrieveBuffer(MfllBuffer_MixedYuv).get(), "mixed_working");
    }
    else {
        dp( c->retrieveBuffer(MfllBuffer_MixedYuv).get(), "mixed", "nv12");
    }
