...
public:     ////                    Image Attributes.
     ...
     * @param overwrite  If the description exists, overwrite = MTRUE can force to overwrite the value.
     *                   Otherwise, the setting will fail and nothing will be changed.
     * @return MTRUE if the value is set successfully.
     *         MFALSE if id is invalid or description exists but the overwrite = MFALSE.
     */
    virtual MBOOL                   setImgDesc(ImageDescId id, MINT64 value, MBOOL overwrite = MFALSE) = 0;

    /**
     * Get image description by ID.
     * @param id         Description ID
     * @param value      [OUT] The value of the description.
     * @return MTRUE if the description exists and the return value is valid. Otherwise MFALSE.
     */
