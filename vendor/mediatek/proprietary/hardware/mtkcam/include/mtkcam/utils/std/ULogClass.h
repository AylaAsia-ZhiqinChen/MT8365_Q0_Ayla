...
public:
    ...
    {
        get()->onLogDetails(moduleId, noNull(tag), type, content, contentLen);
    }
...
public:
    ...
    virtual void onLogSubreqs(ModuleId byModuleId, const char *tag, RequestTypeId requestTypeId, RequestSerial requestSerial,
        RequestTypeId subrequestTypeId, const RequestSerial *subrequestSerialList, size_t n);
    virtual void onLogDetails(ModuleId moduleId, const char *tag, DetailsType type, const char *content, size_t contentLen);
...
class ULogTimeBomb : public android::RefBase
{
public:
    ...
    virtual ~ULogTimeBomb();

protected:
    ULogTimeBomb(
        Type type, ModuleId moduleId, const char *dispatchKey, int warnMs, int abortMs, int abortMaxBound);
    virtual void onLastStrongRef(const void *id) override final;
    static void registerToGuard(android::sp<ULogTimeBomb> timeBomb);
...
typedef android::sp<ULogTimeBomb> ULogTimeBombHandle;


class ULogTimeBombSerial final : public ULogTimeBomb
{
public:
    // Please use CAM_ULOG_TIMEBOMB_CREATE() to create a time bomb.
    // Do NOT call create() directly.
    static android::sp<ULogTimeBombSerial> create(
    ...
    {
        android::sp<ULogTimeBombSerial> timeBomb = new ULogTimeBombSerial(moduleId, serial, dispatchKey, warnMs, abortMs, abortMaxBound);
        registerToGuard(timeBomb);
        return timeBomb;
    }
...
private:
    ...
    ULogTimeBombSerial(
        ...
        ULogTimeBomb(TYPE_SERIAL, moduleId, dispatchKey, warnMs, abortMs, abortMaxBound),
        mSerial(serial)
    {
    }
...
class ULogTimeBombVerbose final : public ULogTimeBomb
{
public:
    static constexpr size_t ID_STRING_SIZE = 512;

    // Please use CAM_ULOG_TIMEBOMB_CREATE() to create a time bomb.
    // Do NOT call create() directly.
    static android::sp<ULogTimeBombVerbose> create(
        const please_use_CAM_ULOG_TIMEBOMB_CREATE &,
        ModuleId moduleId, const char *dispatchKey, int warnMs, int abortMs, int abortMaxBound,
        const char *format, ...) __attribute__((format (printf, 7, 8)))
    {
        android::sp<ULogTimeBombVerbose> timeBomb = new ULogTimeBombVerbose(moduleId, dispatchKey, warnMs, abortMs, abortMaxBound);

        va_list args;
        va_start(args, format);
        ::vsnprintf(timeBomb->mIdString, sizeof(mIdString), format, args);
        va_end(args);
        timeBomb->mIdString[ID_STRING_SIZE - 1] = '\0';
...
private:
    char mIdString[ID_STRING_SIZE];

    ULogTimeBombVerbose(
            ModuleId moduleId, const char *dispatchKey, int warnMs, int abortMs, int abortMaxBound) :
        ULogTimeBomb(TYPE_VERBOSE, moduleId, dispatchKey, warnMs, abortMs, abortMaxBound)
    {
        mIdString[0] = '\0';
    }
...
class ULogRoutineOvertime
{
private:
    static constexpr unsigned int DYNAMIC_STRING = 0x8000;

public:
    static constexpr std::intptr_t NO_KEY_VALUE = -12345487;

    struct please_use_CAM_ULOG_TAG_OVERTIME { };
    struct please_use_CAM_ULOG_TAG_OVERTIMEF { };

    ULogRoutineOvertime(const please_use_CAM_ULOG_TAG_OVERTIME &,
        ModuleId moduleId, const char *logTag, const char *routineTag,
        int warnMs, unsigned int options = 0, std::intptr_t keyValue = NO_KEY_VALUE);

    ULogRoutineOvertime(const please_use_CAM_ULOG_TAG_OVERTIMEF &,
        ModuleId moduleId, const char *logTag, const char *routineTag,
        const char *dynStr, int warnMs, unsigned int options = 0);

    ~ULogRoutineOvertime();

private:
    const char* const mLogTag;
    const char* const mRoutineTag;
    std::intptr_t mKeyValue;
    const ModuleId mModuleId;
    const int mWarnMs;
    const unsigned int mOptions;
    struct timespec mBeginTime;
};


class ULogPrinter final : public android::Printer
{
private:
    static constexpr size_t LOG_TAG_LEN_MAX = 64;
    static constexpr size_t LOG_BUFFER_MAX = 1024;

    static constexpr android_LogPriority toALogPriority(DetailsType detailsType) {
        switch (detailsType) {
        case DETAILS_ERROR:   return ANDROID_LOG_ERROR;
        case DETAILS_WARNING: return ANDROID_LOG_WARN;
        case DETAILS_INFO:    return ANDROID_LOG_INFO;
        case DETAILS_VERBOSE: return ANDROID_LOG_VERBOSE;
...
public:
    ULogPrinter(
            ModuleId moduleID,
            const char* logtag,
            DetailsType detailsType = DETAILS_DEBUG,
            const char* prefix = nullptr,
            bool ignoreBlankLines = false) :
        mDetailsType(detailsType),
        mModuleId(moduleID),
        mALogPrinter(logtag, toALogPriority(detailsType), prefix, ignoreBlankLines)
    {
        ::strncpy(mLogTag, logtag, LOG_TAG_LEN_MAX);
        mLogTag[LOG_TAG_LEN_MAX - 1] = '\0';
    }

    inline virtual void printLine(const char* string = "") override {
        if (ULogger::isNormalDetailsEnabled(mModuleId, mDetailsType)) {
            if (ULogger::isAndroidModeEnabled())
                mALogPrinter.printLine(string);
            if (ULogger::isFileModeEnabled())
                ULogger::logNormalDetails(mModuleId, mLogTag, mDetailsType, string, ::strlen(string));
        }
    }

    inline virtual void printFormatLine(const char *format, ...) override __attribute__((format (printf, 2, 3))) {
        if (!ULogger::isNormalDetailsEnabled(mModuleId, mDetailsType))
            return;
        ...
        va_end(args);
        buffer[LOG_BUFFER_MAX - 1] = '\0';

        if (ULogger::isAndroidModeEnabled())
            mALogPrinter.printLine(buffer);
        if (ULogger::isFileModeEnabled())
            ULogger::logNormalDetails(mModuleId, mLogTag, mDetailsType, buffer, n);
    }

private:
    DetailsType mDetailsType;
    ModuleId mModuleId;
    char mLogTag[LOG_TAG_LEN_MAX];
    android::LogPrinter mALogPrinter;
};
