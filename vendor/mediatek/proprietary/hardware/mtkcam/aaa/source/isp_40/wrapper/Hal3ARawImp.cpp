        ...
        // ZSD cap
        {
            ...
            */
            ...
            if(!mbIsHDRShot && (i4OperMode != EOperMode_Meta) &&
                (!bAELock|| IAeMgr::getInstance().getEVCompensateIndex(m_i4SensorDev) != 0) && !bSkipZsdCap)
#endif
            {
...
notify4CCU(MUINT32 u4PreFrmId, ISP_NVRAM_OBC_T const &OBCResult)
{
    if(m_bCCUEn && m_pThreadSetCCUPara)
    {
        ...
        MY_LOGD_IF(m_3ALogEnable, "[%s] , i8SensorExposureTime(%ld), i8SensorFrameDuration(%ld), i4SensorSensitivity(%d), OBCResult gain0(%d), offst0(%d)", __FUNCTION__,
        (long)rAESensorInfo.u8ExposureTime, (long)rAESensorInfo.u8FrameDuration, rAESensorInfo.u4Sensitivity, OBCResult.gain0.val, OBCResult.offst0.val);
