...
postCmd(MVOID* pArg)
{
    std::lock_guard<std::mutex> autoLock(m_PoolMtx);
...
{
    std::unique_lock<std::mutex> autoLock(m_PoolMtx);
...
onThreadLoop(MVOID* pArg)
{
    ...
    while(_this->getCmd(rCmd))
    {
        ...
        if(rCmd.i4SubsampleCount > 1)
        {
            ...
            //===== Get new result =====
            AllResult_T *pAllResult = pResultPoolObj->getResultByReqNum(rCmd.i4MagicNumReq, __FUNCTION__);
            if(pAllResult == NULL)
            ...
            else
            {
                ...
                for(MINT32 i=0; i < rCmd.i4SubsampleCount; i++)
                {
                    ...
                    if( (pAEResult->i8SensorExposureTime != rLocalNew3AInfo.i8SensorExposureTime) && (rCmd.i4SubsampleCount >= pAllResult->rOld3AInfo.rGainDelay.u4SutterGain_Delay) )
                        pAEResult->i8SensorExposureTime = rLocalNew3AInfo.i8SensorExposureTime;

                    if( (pAEResult->i4SensorSensitivity != rLocalNew3AInfo.i4SensorSensitivity) && (rCmd.i4SubsampleCount >= pAllResult->rOld3AInfo.rGainDelay.u4SensorGain_Delay) )
                        pAEResult->i4SensorSensitivity = rLocalNew3AInfo.i4SensorSensitivity;

                    AllMetaResult_T *pMetaResult = pResultPoolObj->getMetadata(i4Magic, __FUNCTION__);
                    ...
                    {
                        if(pMetaResult != NULL)
                            pResultPoolObj->convertToMetadata42A(pAllResult, pMetaResult);
                        else
                    ...
                    else // AF convert
                    {
                        if(pMetaResult != NULL)
                        {
                            pResultPoolObj->convertToMetadata4AF(pAllResult, pMetaResult);
                        }
                        else
                            CAM_LOGE("[%s] SMVR  pMetaResult", __FUNCTION__);
                    }
                    pResultPoolObj->returnMetadata(pMetaResult, __FUNCTION__);
                }
            }
        ...
        else //===== Normal =====
        {
            ...
            if(rCmd.eConvertType == E_2A_CONVERT) // 2A convert
            {
                pMetaResult = pResultPoolObj->getMetadata(rCmd.i4MagicNumReq, __FUNCTION__);
                pAllResult = pResultPoolObj->getResultByReqNum(rCmd.i4MagicNumReq, __FUNCTION__);
        ...
        CAM_LOGD_IF(_this->m_u4LogEn, "[%s] 2A & AF ConvertToMetadata done", __FUNCTION__);
        AAA_TRACE_END_HAL;
    }
