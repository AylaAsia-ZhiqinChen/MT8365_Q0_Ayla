...
#define OVERRIDE_AF  (1<<2)
    ...
    MY_LOGD_IF( (i4LogEnable || fgLogEn0), "[%s] Dev(%d) PreSetKey(#%d), StereoParams : DenoiseMode(%d -> %d), Sync2A(%d -> %d), SyncAF(%d -> %d), HwSync(%d -> %d), M&S Idx(%d,%d), bIsByFrame(%d -> %d)", __FUNCTION__,
...
setNormal(const vector<MetaSet_T*>& requestQ)
{
    ...
    else if (it->Dummy || mParams.bDummyBeforeCapture || mParams.bDummyAfterCapture)
    {
        ...
        mStereoParam.i4Sync2AMode = NS3Av3::E_SYNC2A_MODE::E_SYNC2A_MODE_IDLE;
        MY_LOGD("[%s] Req(#%d) FrameNum(#%d) Dummy(%d) Sync2AMode(%d)", __FUNCTION__, i4FrmId, i4FrameNum, it->Dummy, mStereoParam.i4Sync2AMode);
#endif
...
getCur(MUINT32 frmId, MetaSet_T& result)
{
    ...
    {
        MY_LOGW("[%s] Fail to get R[%d], return -1", __FUNCTION__, frmId);
        return -1;
    }
