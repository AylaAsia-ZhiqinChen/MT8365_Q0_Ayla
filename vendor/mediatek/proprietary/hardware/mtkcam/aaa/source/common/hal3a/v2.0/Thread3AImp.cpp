...
protected: //private:
    ...
    std::mutex              mLock;
    pthread_t               mThread;
    std::list<CmdQ_T>       mCmdQ;
    std::mutex              mModuleMtx;
    std::condition_variable mCmdQCond;
    sem_t                   mSem;
    std::thread             create3Athread;
    std::atomic_flag        thread_alive;
};
...
{
    //std::lock_guard<std::mutex> lock(mLock);  // Nelson : mutual exclusive by atomic_flag
    ...
    switch (r3ACmd){
        ...
        case ECmd_PrecaptureEnd:
            ...
            if (!bRet)
            {
                NSCam::Utils::CallStackLogger csl;
                csl.logProcess(LOG_TAG);
                AEE_ASSERT_3A_HAL("type1 to type3 postCommand fail.");
            }
            break;
...
Thread3AImp::clearCommandQ()
{
    ...
    for (std::list<CmdQ_T>::iterator it = mCmdQ.begin(); it != mCmdQ.end();)
    {
        ECmd_T eCmd = it->eCmd;
...
Thread3AImp::getCommand(IThread3A::CmdQ_T &rCmd, MBOOL &bGetCmd, MBOOL en_timeout)
{
    ...
    if (mCmdQ.size() == 0)
    {
        if (en_timeout)
        {
            ...
            mCmdQCond.wait_for(autoLock, std::chrono::nanoseconds(27000000));
            MY_LOGD_IF(fgLogEn, "[%s] mCmdQCond.waitRelative -", __FUNCTION__);
        }
        else
        {
            MY_LOGD_IF(fgLogEn, "[%s] mCmdQCond.wait +", __FUNCTION__);
            mCmdQCond.wait(autoLock);
            MY_LOGD_IF(fgLogEn, "[%s] mCmdQCond.wait -", __FUNCTION__);
        }
    }
    ...
    while (_this->getCommand(rCmd, bGetCmd, MFALSE))
    {
        ...
        switch (rCmd.eCmd)
        {
            case ECmd_Update:
                ...
                {
                    std::lock_guard<std::mutex> autoLock(_this->mModuleMtx);
