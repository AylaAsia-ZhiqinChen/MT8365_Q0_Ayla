...
getInstance(MINT32 i4SensorOpenIndex, MINT32 i4SubsampleCount)
{
    ...
    switch (i4SensorDev)
    {
        ...
        {
            static Hal3AFlowCtrl_SMVR _singleton(SENSOR_DEV_MAIN);
            _singleton.init(i4SensorOpenIndex, i4SubsampleCount);
            return &_singleton;
        }
        case SENSOR_DEV_SUB:
        {
            static Hal3AFlowCtrl_SMVR _singleton(SENSOR_DEV_SUB);
            _singleton.init(i4SensorOpenIndex, i4SubsampleCount);
            return &_singleton;
        }
        case SENSOR_DEV_MAIN_2:
        {
            static Hal3AFlowCtrl_SMVR _singleton(SENSOR_DEV_MAIN_2);
            _singleton.init(i4SensorOpenIndex, i4SubsampleCount);
            return &_singleton;
        }
        case SENSOR_DEV_SUB_2:
        {
            static Hal3AFlowCtrl_SMVR _singleton(SENSOR_DEV_SUB_2);
            _singleton.init(i4SensorOpenIndex, i4SubsampleCount);
            return &_singleton;
        }
        case SENSOR_DEV_MAIN_3:
        {
            static Hal3AFlowCtrl_SMVR _singleton(SENSOR_DEV_MAIN_3);
            _singleton.init(i4SensorOpenIndex, i4SubsampleCount);
            return &_singleton;
        }
    default:
...
Hal3AFlowCtrl_SMVR::
Hal3AFlowCtrl_SMVR(MINT32 i4SensorDev)
    : Hal3AFlowCtrl(i4SensorDev)////fix resultsize
    ...
    if (eCmd == ECmd_Update)
    {
        ...
        if (rParamIspProf.iValidateOpt != ParamIspProfile_T::EParamValidate_None)
        {
            ...
            for (it = rNumberSet.begin(); it != rNumberSet.end(); it++)
            {
                ...
                if(!ret)
                    MY_LOGE("[%s] setRequestInfo fail", __FUNCTION__);
            }
...
on3AProcFinish(const RequestSet_T rRequestSet, MINT32 i4MagicNumCur)
{
    ...
    AllResult_T *pAllResult = m_pResultPoolObj->getResultByReqNum(i4MagicNum, __FUNCTION__);// get result
    ...
    if(pAllResult && pAEResult && pAFResult)
    {
        ...
        MY_LOGD("[%s] pAllResult(%p)/pAEResult(%p)/pAFResult(%p)", __FUNCTION__, pAllResult, pAEResult, pAFResult);

    MY_LOGD_IF(m_fgLogEn, "[%s] u1AfState(%d), u1AeState(%d), u1FlashState(%d), u4AeMode(%d), fgAeBvTrigger(%d)", __FUNCTION__, pAllResult->rEarlyCB.u1AfState, pAllResult->rEarlyCB.u1AeState, pAllResult->rEarlyCB.u1FlashState, pAllResult->rEarlyCB.u1AeMode, pAllResult->rEarlyCB.fgAeBvTrigger);
