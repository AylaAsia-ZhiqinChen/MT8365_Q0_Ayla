...
typedef struct DoSttIn_t
{
    MUINT32 u4ReqMagicNum;
    MINT32 i4AfPos;
    MINT32 i4HWBlkNumX;
    MINT32 i4HWBlkNumY;
    MINT32 i4HWBlkSizeX;
...
typedef struct DoSttOut_t
{
    MUINT32 u4StaMagicNum;   // statistics number : based on request
    MUINT32 u4ConfigHWNum;   // configure number : based on handleaf counter
    MUINT32 u4ConfigLatency; // configure latency count
    MUINT64 u8SofTimeStamp;  // (unit: us) the time stamp of SOF of the current AFO
    AF_STAT_PROFILE_T* pAfSttProfile;
    AF_STAT_T* pAfoBuf;
} DoSttOut_T;
