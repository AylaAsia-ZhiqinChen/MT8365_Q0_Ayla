...
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
AAOBufMgrImp::
AAOBufMgrImp(MINT32 i4SensorDev, MINT32 i4SensorIdx, const ConfigInfo_T& rConfigInfo __unused)
    ...
    , m_rConfigInfo(rConfigInfo)
{
    ...
    CAM_LOGD("AAOBufMgrImp, m_i4SensorDev: %d SubsampleCount:%d", m_i4SensorDev, m_rConfigInfo.i4SubsampleCount);
}
...
//
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
static MBOOL dumpBuf(char* fileName, char* folderName, void* pBuf, MUINT32 u4Size) __attribute__((unused));
...
enqueueHwBuf()
{
    ...
    if(m_pSttPipe != NULL)
    {
        ...
        for (MUINT32 i = 0; i < P1_HW_BUF_MAX_NUM; i++)
        {
           ...
           if(!m_HW_QBuf[QBufIdx].mvOut.empty())
           {
            BufInfo rLastBuf = m_HW_QBuf[QBufIdx].mvOut.at( size-1 );
           ...
           {
            CAM_LOGE("without deque buffer, skip enque");
            break;
           }
        }
        CAM_TRACE_END();
    ...
    if(m_pSttPipe != NULL)
    {
        ...
        if(bWaitDeqHW)
        {
            ...
            if(m_bAbort || m_bPreStop || m_bForceReturn) {
                CAM_LOGD("[%s] AAO buffer return NULL: EnQBufIdx(%d)/DeQBufIdx(%d), m_bAbort: %d, m_bPreStop: %d, m_bForceReturn: %d\n",
                         __FUNCTION__, m_HW_EnQBufIdx, m_HW_DeQBufIdx, m_bAbort, m_bPreStop, m_bForceReturn);
                m_bForceReturn = MFALSE;
                return NULL;
            }
        ...
        //Magic Number remapping
        MINT32 i4RemappingMagic = m_rHwBuf.mMagicNumber;
        if (m_rConfigInfo.i4SubsampleCount > 1)
        {
            i4RemappingMagic = m_rHwBuf.mMagicNumber - m_rConfigInfo.i4SubsampleCount + 1;
        }

        CAM_LOGD_IF( m_bDebugEnable , "[%s] SubsampleCount(%d) HwMagic(%d) remappingMagic(%d)\n", __FUNCTION__, m_rConfigInfo.i4SubsampleCount, m_rHwBuf.mMagicNumber, i4RemappingMagic);

        // update flash state to statistic information.
        IResultPool* pResultPoolObj = IResultPool::getInstance(m_i4SensorDev);
        FLASHResultToMeta_T* pFLASHResult = (FLASHResultToMeta_T*)pResultPoolObj->getResult(i4RemappingMagic,E_FLASH_RESULTTOMETA);
        if(pFLASHResult != NULL)
            m_rHwBuf.mFlashState = pFLASHResult->u1FlashState;
        // update ae mode to statistic information.
        AEResultToMeta_T* pAEResult = (AEResultToMeta_T*)pResultPoolObj->getResult(i4RemappingMagic,E_AE_RESULTTOMETA);
        m_rHwBuf.mu1AeMode = LIB3A_AE_MODE_ON;
        if(pAEResult != NULL)
            m_rHwBuf.mu1AeMode = pAEResult->u1AeMode;
        ...
        if (bEnable) {
            ...
            strncpy(folderName, "/sdcard/aao", FILE_NAME_SIZE);
        ...
        if(bEnableHW){
            char fileName[FILE_NAME_SIZE];
            char folderName[FILE_NAME_SIZE];
            strncpy(folderName, "/sdcard/aaoHwBuf", FILE_NAME_SIZE);
