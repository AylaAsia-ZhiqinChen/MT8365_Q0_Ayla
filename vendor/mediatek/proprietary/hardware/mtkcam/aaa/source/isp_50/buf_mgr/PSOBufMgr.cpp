...
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
PSOBufMgrImp::
PSOBufMgrImp(MINT32 i4SensorDev, MINT32 i4SensorIdx, const ConfigInfo_T& rConfigInfo __unused)
...
enqueueHwBuf()
{
    ...
    if(m_pSttPipe != NULL)
    {
        ...
        {
            CAM_LOGD("[%s] PSO deque fail - EnQBufIdx(%d) -> DeQBufIdx(%d)\n", __FUNCTION__, m_HW_EnQBufIdx, m_HW_DeQBufIdx);
            return -1;
        }
...
dequeueSwBuf()
{
    MBOOL bEnable = property_get_int32("vendor.pso.dump.enable", 0);
    MBOOL bEnableHW = property_get_int32("vendor.psohw.dump.enable", 0);
    ...
    if(m_pSttPipe != NULL)
    {
        MBOOL bWaitDeqHW = MFALSE;
        {
            ...
            if(m_HW_EnQBufIdx >= m_HW_DeQBufIdx) {
                bWaitDeqHW = MTRUE;
                CAM_LOGD_IF(m_bDebugEnable, "[%s] PSO deque fail - EnQBufIdx(%d) -> DeQBufIdx(%d)\n", __FUNCTION__, m_HW_EnQBufIdx, m_HW_DeQBufIdx);
            }
        }
        if(bWaitDeqHW)
        {
            ...
            if(m_bAbort || m_bPreStop || m_bForceReturn) {
                CAM_LOGD("[%s] PSO buffer return NULL: EnQBufIdx(%d)/DeQBufIdx(%d)\n", __FUNCTION__, m_HW_EnQBufIdx, m_HW_DeQBufIdx);
                m_bForceReturn = MFALSE;
                return NULL;
            }

            CAM_LOGD_IF(m_bDebugEnable, "[%s] PSO deque success - EnQBufIdx(%d) -> DeQBufIdx(%d)\n", __FUNCTION__, m_HW_EnQBufIdx, m_HW_DeQBufIdx);
        }
        ...
        if (bEnable) {
            ...
            strncpy(folderName, "/sdcard/pso", FILE_NAME_SIZE);
        ...
        // dump pso hw data for ut
        if(bEnableHW){
            char fileName[FILE_NAME_SIZE];
            char folderName[FILE_NAME_SIZE];
            strncpy(folderName, "/sdcard/psoHwBuf", FILE_NAME_SIZE);
            sprintf(fileName, "/sdcard/psoHwBuf/pso_%d.hwbuf", rLastBuf.mMetaData.mMagicNum_hal);
...
abortDequeue()
{
    CAM_LOGD_IF(m_bDebugEnable, "[%s] m_rBufIndex(%d) +\n", __FUNCTION__, m_rBufIndex);
    m_bAbort = MTRUE;
    m_u4FrameCount = 0;
    if(m_pSttPipe != NULL)
        m_pSttPipe->abortDma(m_rPort,LOG_TAG);
    m_Cond.broadcast();
    CAM_LOGD_IF(m_bDebugEnable, "[%s] abortDma done\n", __FUNCTION__);
    CAM_LOGD_IF(m_bDebugEnable, "[%s] m_rBufIndex(%d) -\n", __FUNCTION__, m_rBufIndex);
}
