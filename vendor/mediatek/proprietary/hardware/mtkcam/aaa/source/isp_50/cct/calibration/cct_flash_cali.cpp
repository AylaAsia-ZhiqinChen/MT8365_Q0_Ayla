...
int AcdkCalibration::flashCalibration(int sensorDev)
{
    logI("%s(): sensorDev(%d), m_SensorDevId(%d)",
            __FUNCTION__, sensorDev, m_SensorDevId);

    int ret;
    m_pAcdkCctBaseObj->startPreview(NULL);
    FlashMgr::getInstance(m_SensorDevId)->cctSetSpModeCalibration();
    FlashMgr::getInstance(m_SensorDevId)->cctSetSpModeLock(1);
    ret = takePicture(ACDK_CDVT_SENSOR_MODE_CAPTURE, PURE_RAW8_TYPE);
    FlashMgr::getInstance(m_SensorDevId)->cctSetSpModeLock(0);
    FlashMgr::getInstance(m_SensorDevId)->cctSetSpModeNormal();
