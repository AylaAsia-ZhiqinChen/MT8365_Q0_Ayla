...
{
    AE_SETTING_LOG("[AeSettingDefault:Construct] SensorDev:%d\n", (MUINT32)m_eSensorDev);
}
...
updateSensor(MVOID* input)
{
    ...
    if (m_bSetFrameRateValue)
    {
        ...
        AE_SETTING_LOG_IF(m_3ALogEnable, "[%s()] Set PreviewMaxFrameRate:%d\n", __FUNCTION__, pinput->u4UpdateFrameRate_x10);
        localTimer.End();
    }
    ...
    if (m_bSetShutterValue)
    {
        if (pinput->bPerframeAndExpChanged)
        {
            ...
            AE_SETTING_LOG_IF(m_3ALogEnable, "[%s()] Set Exposure time:%d\n", __FUNCTION__, pinput->u4UpdateShutterValue);
            localTimer.End();
        }
        m_bSetShutterValue = MFALSE;
    }
    ...
    if(m_bSetGainValue)
    {
        if (pinput->bPerframeAndExpChanged)
        {
            ...
            AE_SETTING_LOG_IF(m_3ALogEnable, "[%s()] Set Sensor gain:%d\n", __FUNCTION__, pinput->u4UpdateGainValue);
            localTimer.End();
        }
        m_bSetGainValue = MFALSE;
    }
...
updateSensorGain(MVOID* input)
{
    ...
    AE_SETTING_LOG("[%s()] setAfe:%d\n", __FUNCTION__, *psensorgain);
    if (!m_pSensorGainThread)
    {
        // 0/0/2 sensor
...
updateSensorExp(MVOID* input)
{
    ...
    AE_SETTING_LOG("[%s()] setExp:%d\n", __FUNCTION__, *psensorExp);
...
~AeSettingDefault()
{
    AE_SETTING_LOG("[AeFlowDefault:Destruct] SensorDev:%d\n", (MUINT32)m_eSensorDev);
}
