...
void svd_tool(unsigned int *src_addr, int nrow, int ncol, int num_term, int num_len, TBL_INFO_T rTableINFO, MUINT8 a_u1Mode)
{
    ...
    switch(a_u1Mode) {
        case 0:
            snprintf(fileNameBin, sizeof(fileNameBin), "%s//Preview_LSC.bin", MEDIA_PATH);
            break;
        case 1:
            snprintf(fileNameBin, sizeof(fileNameBin), "%s//Capture_LSC.bin", MEDIA_PATH);
            break;
        case 2:
            snprintf(fileNameBin, sizeof(fileNameBin), "%s//Video_LSC.bin", MEDIA_PATH);
            break;
    }
...
//                    val[0], val[1], val[2], val[3]);
  ...
  FILE *fin;
    char fileName[100];
    snprintf(fileName, sizeof(fileName), "%s//Source_Gain_Table.dat", MEDIA_PATH); //g_lsc_param.raw_wd, g_lsc_param.raw_ht
    fin  = fopen(fileName, "wt");
#ifdef FLOAT_VERSION
    FILE *fin2,*fin3;
    snprintf(fileName, sizeof(fileName), "%s//Source_Gain_Table_Float.dat", MEDIA_PATH);
    fin2  = fopen(fileName, "wt");
    snprintf(fileName, sizeof(fileName), "%s//Source_Gain_Table_16bit.dat", MEDIA_PATH);
...
#if LSC_Debug_Table_Output
    char fileName[100];
    snprintf(fileName, sizeof(fileName), "%s//%04d_%04d.Table.dat", MEDIA_PATH, g_lsc_param.raw_wd, g_lsc_param.raw_ht);
...
#endif
// debug
    snprintf(fileName, sizeof(fileName), "%s//poly.dat", MEDIA_PATH);
    FILE *finpoly;
    ...
    snprintf(fileNameCoef, sizeof(fileNameCoef), "%s//%04d_%04d.Coeff.c", MEDIA_PATH, g_lsc_param.raw_wd, g_lsc_param.raw_ht);
...
MINT32 mrBPC_Calibrate(BPC_CAL_INI_PARAM_T cali_info)
{
    ...
    {
        snprintf(fileNameBin, sizeof(fileNameBin), "%s//Preview_BPC.bin", MEDIA_PATH);
    }
    else
    {
        snprintf(fileNameBin, sizeof(fileNameBin), "%s//Capture_BPC.bin", MEDIA_PATH);
    }
