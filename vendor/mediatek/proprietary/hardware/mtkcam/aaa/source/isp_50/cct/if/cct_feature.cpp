...
)
{
    ...
    case ACDK_CCT_OP_STROBE_SET_NVDATA_META:
        ...
        ret = FlashMgr::getInstance(mSensorDev)->cctSetNvdataMeta(puParaIn, u4ParaInLen);
        if(ret==0)
            err=0;
        else
            err=-1;
    ...
    case ACDK_CCT_OP_STROBE_READ_NVRAM_TO_PC_META:
        ...
        ret = FlashMgr::getInstance(mSensorDev)->cctReadNvramToPcMeta(puParaOut, pu4RealParaOutLen);
        if(ret==0)
            err=0;
        else
            err=-1;
    ...
    case ACDK_CCT_OP_FLASH_CONTROL:
        ...
        ret = FlashMgr::getInstance(mSensorDev)->cctFlashLightTest((VOID *)puParaIn);
        if(ret==0)
            err=0;
        else
            err=-1;
  ...
  case ACDK_CCT_OP_FLASH_ENABLE:
      MY_LOG("ACDK_CCT_OP_FLASH_ENABLE line=%d\n",__LINE__);
        ret = FlashMgr::getInstance(mSensorDev)->cctFlashEnable(1); //YosenFlash
        if(ret==0)
            err=0;
        else
            err=-1;
    ...
    case ACDK_CCT_OP_FLASH_DISABLE:
        ...
        ret = FlashMgr::getInstance(mSensorDev)->cctFlashEnable(0); //YosenFlash
        if(ret==0)
            err=0;
        else
            err=-1;
    ...
    case ACDK_CCT_OP_FLASH_SET_MANUAL_FLASH:
        ...
        ret = FlashMgr::getInstance(mSensorDev)->setManualFlash(i32In[0], i32In[1]);
        if(ret==0)
            err=0;
        else
            err=-1;
