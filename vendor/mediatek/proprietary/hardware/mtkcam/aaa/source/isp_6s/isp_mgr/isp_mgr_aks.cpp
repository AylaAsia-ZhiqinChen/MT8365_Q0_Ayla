...
{
    ISP_MGR_MODULE_GET_INSTANCE(AKS);
}

template <>
ISP_MGR_AKS_T&
ISP_MGR_AKS_T::
put(MUINT8 SubModuleIndex, ISP_NVRAM_AKS_T const& rParam)
{
    if (SubModuleIndex >= ESubModule_NUM) {
        CAM_LOGE("[PUT] Error Submodule Index: %d", SubModuleIndex);
        return (*this);
    }
    PUT_REG_INFO_MULTI(SubModuleIndex, TUNING,                    tuning);
    return (*this);
}
...
ISP_MGR_AKS_T&
ISP_MGR_AKS_T::
get(MUINT8 SubModuleIndex, ISP_NVRAM_AKS_T& rParam)
{
    if (SubModuleIndex >= ESubModule_NUM) {
        CAM_LOGE("[GET] Error Submodule Index: %d", SubModuleIndex);
        return (*this);
    }
    GET_REG_INFO_MULTI(SubModuleIndex, TUNING,                    tuning);
    return (*this);
}
...
apply_P2(MUINT8 SubModuleIndex, const RAWIspCamInfo& rRawIspCamInfo, dip_a_reg_t* pReg)
{
    ...
    dumpRegInfoP2("AKS", SubModuleIndex);
    return MTRUE;
}
