...
{
    if (i4SensorDev & ESensorDev_Main) {
        return AwbMgr::getInstance(ESensorDev_Main).setStrobeNvramIdx(i4StrobeNvramIdx);
    }
    else if (i4SensorDev & ESensorDev_Sub) {
        return AwbMgr::getInstance(ESensorDev_Sub).setStrobeNvramIdx(i4StrobeNvramIdx);
    }
    else if (i4SensorDev & ESensorDev_MainSecond) {
        return AwbMgr::getInstance(ESensorDev_MainSecond).setStrobeNvramIdx(i4StrobeNvramIdx);
    }
    else if (i4SensorDev & ESensorDev_SubSecond) {
        return AwbMgr::getInstance(ESensorDev_SubSecond).setStrobeNvramIdx(i4StrobeNvramIdx);
    }
    CAM_LOGE("Err IAwbMgr::setStrobeNvramIdx()\n");
...
{
    if (i4SensorDev & ESensorDev_Main) {
        return AwbMgr::getInstance(ESensorDev_Main).setFlashCaliNvramIdx(i4FlashCaliNvramIdx);
    }
    else if (i4SensorDev & ESensorDev_Sub) {
        return AwbMgr::getInstance(ESensorDev_Sub).setFlashCaliNvramIdx(i4FlashCaliNvramIdx);
    }
    else if (i4SensorDev & ESensorDev_MainSecond) {
        return AwbMgr::getInstance(ESensorDev_MainSecond).setFlashCaliNvramIdx(i4FlashCaliNvramIdx);
    }
    else if (i4SensorDev & ESensorDev_SubSecond) {
        return AwbMgr::getInstance(ESensorDev_SubSecond).setFlashCaliNvramIdx(i4FlashCaliNvramIdx);
    }
    CAM_LOGE("Err IAwbMgr::setFlashCaliNvramIdx()\n");
...
MRESULT IAwbMgr::setNVRAMIndex(MINT32 i4SensorDev, MUINT32 a_eNVRAMIndex)
{
    ...
    if (i4SensorDev & ESensorDev_Main)
        ret_main = AwbMgr::getInstance(ESensorDev_Main).setNVRAMIndex(a_eNVRAMIndex);
    if (i4SensorDev & ESensorDev_Sub)
        ret_sub = AwbMgr::getInstance(ESensorDev_Sub).setNVRAMIndex(a_eNVRAMIndex);
    if (i4SensorDev & ESensorDev_MainSecond)
        ret_main2 = AwbMgr::getInstance(ESensorDev_MainSecond).setNVRAMIndex(a_eNVRAMIndex);
    if (i4SensorDev & ESensorDev_SubSecond)
        ret_sub2 = AwbMgr::getInstance(ESensorDev_SubSecond).setNVRAMIndex(a_eNVRAMIndex);
...
{
    if (i4SensorDev & ESensorDev_Main)
        return AwbMgr::getInstance(ESensorDev_Main).CCTOPAWBApplyK71NVRAMParam(a_pAWBNVRAM);
    if (i4SensorDev & ESensorDev_Sub)
        return AwbMgr::getInstance(ESensorDev_Sub).CCTOPAWBApplyK71NVRAMParam(a_pAWBNVRAM);
    if (i4SensorDev & ESensorDev_MainSecond)
        return AwbMgr::getInstance(ESensorDev_MainSecond).CCTOPAWBApplyK71NVRAMParam(a_pAWBNVRAM);
    if (i4SensorDev & ESensorDev_SubSecond)
        return AwbMgr::getInstance(ESensorDev_SubSecond).CCTOPAWBApplyK71NVRAMParam(a_pAWBNVRAM);
    return E_AWB_PARAMETER_ERROR;
}
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
//
...
{
    if (i4SensorDev & ESensorDev_Main)
        return AwbMgr::getInstance(ESensorDev_Main).CCTOPFlashCalibrationSaveNVRAMParam();
    if (i4SensorDev & ESensorDev_Sub)
        return AwbMgr::getInstance(ESensorDev_Sub).CCTOPFlashCalibrationSaveNVRAMParam();
    if (i4SensorDev & ESensorDev_MainSecond)
        return AwbMgr::getInstance(ESensorDev_MainSecond).CCTOPFlashCalibrationSaveNVRAMParam();
    if (i4SensorDev & ESensorDev_SubSecond)
        return AwbMgr::getInstance(ESensorDev_SubSecond).CCTOPFlashCalibrationSaveNVRAMParam();
    return E_AWB_PARAMETER_ERROR;
}
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
//
...
MBOOL IAwbMgr::CallBackAwb(MINT32 i4SensorDev, MVOID* pIn, MVOID* pOut)
{
    ...
    if (i4SensorDev & ESensorDev_Main) {
        return AwbMgr::getInstance(ESensorDev_Main).CallBackAwb(pIn, pOut);
    }
    else if (i4SensorDev & ESensorDev_Sub) {
        return AwbMgr::getInstance(ESensorDev_Sub).CallBackAwb(pIn, pOut);
    }
    else if (i4SensorDev & ESensorDev_MainSecond) {
        return AwbMgr::getInstance(ESensorDev_MainSecond).CallBackAwb(pIn, pOut);
    }
    else if (i4SensorDev & ESensorDev_SubSecond) {
       return AwbMgr::getInstance(ESensorDev_SubSecond).CallBackAwb(pIn, pOut);
    }
...
{
    if (i4SensorDev & ESensorDev_Main) {
        return AwbMgr::getInstance(ESensorDev_Main).SetAWBFlare(Flare, FlareOffset);
    }
    else if (i4SensorDev & ESensorDev_Sub) {
        return AwbMgr::getInstance(ESensorDev_Sub).SetAWBFlare(Flare, FlareOffset);
    }
    else if (i4SensorDev & ESensorDev_MainSecond) {
        return AwbMgr::getInstance(ESensorDev_MainSecond).SetAWBFlare(Flare, FlareOffset);
    }
    else if (i4SensorDev & ESensorDev_SubSecond) {
       return AwbMgr::getInstance(ESensorDev_SubSecond).SetAWBFlare(Flare, FlareOffset);
    }
...
MRESULT IAwbMgr::configReg(MINT32 i4SensorDev, AWBResultConfig_T *pResultConfig)
{
  MINT32 ret_main, ret_sub, ret_main2, ret_sub2;
   ...
   if (i4SensorDev & ESensorDev_Main)
       ret_main = AwbMgr::getInstance(ESensorDev_Main).configReg(pResultConfig);
   if (i4SensorDev & ESensorDev_Sub)
       ret_sub = AwbMgr::getInstance(ESensorDev_Sub).configReg(pResultConfig);
   if (i4SensorDev & ESensorDev_MainSecond)
       ret_main2 = AwbMgr::getInstance(ESensorDev_MainSecond).configReg(pResultConfig);
   if (i4SensorDev & ESensorDev_SubSecond)
       ret_sub2 = AwbMgr::getInstance(ESensorDev_SubSecond).configReg(pResultConfig);
...
{
    if (i4SensorDev & ESensorDev_Main) {
        return AwbMgr::getInstance(ESensorDev_Main).getAWBInfo(rAWBInfo);
    }
    else if (i4SensorDev & ESensorDev_Sub) {
        return AwbMgr::getInstance(ESensorDev_Sub).getAWBInfo(rAWBInfo);
    }
    else if (i4SensorDev & ESensorDev_MainSecond) {
        return AwbMgr::getInstance(ESensorDev_MainSecond).getAWBInfo(rAWBInfo);
    }
    else if (i4SensorDev & ESensorDev_SubSecond) {
       return AwbMgr::getInstance(ESensorDev_SubSecond).getAWBInfo(rAWBInfo);
    }

    CAM_LOGE("Err IAwbMgr::getAWBInfo()\n");
...
{
    if (i4SensorDev & ESensorDev_Main) {
        return AwbMgr::getInstance(ESensorDev_Main).getPostgain(rPostgain);
    }
    else if (i4SensorDev & ESensorDev_Sub) {
        return AwbMgr::getInstance(ESensorDev_Sub).getPostgain(rPostgain);
    }
    else if (i4SensorDev & ESensorDev_MainSecond) {
        return AwbMgr::getInstance(ESensorDev_MainSecond).getPostgain(rPostgain);
    }
    else if (i4SensorDev & ESensorDev_SubSecond) {
       return AwbMgr::getInstance(ESensorDev_SubSecond).getPostgain(rPostgain);
    }

    CAM_LOGE("Err IAwbMgr::getPostgain()\n");
...
{
    if (i4SensorDev & ESensorDev_Main) {
        return AwbMgr::getInstance(ESensorDev_Main).setPostgain(rPostgain);
    }
    else if (i4SensorDev & ESensorDev_Sub) {
        return AwbMgr::getInstance(ESensorDev_Sub).setPostgain(rPostgain);
    }
    else if (i4SensorDev & ESensorDev_MainSecond) {
        return AwbMgr::getInstance(ESensorDev_MainSecond).setPostgain(rPostgain);
    }
    else if (i4SensorDev & ESensorDev_SubSecond) {
       return AwbMgr::getInstance(ESensorDev_SubSecond).setPostgain(rPostgain);
    }

    CAM_LOGE("Err IAwbMgr::setPostgain()\n");
...
MBOOL IAwbMgr::setPostgainLock(MINT32 i4SensorDev, MBOOL bPostgainLock)
{
    if(i4SensorDev & ESensorDev_Main)
    {
        return AwbMgr::getInstance(ESensorDev_Main).setPostgainLock(bPostgainLock);
    }
    else if(i4SensorDev & ESensorDev_Sub)
    {
        return AwbMgr::getInstance(ESensorDev_Sub).setPostgainLock(bPostgainLock);
    }
    else if(i4SensorDev & ESensorDev_MainSecond)
    {
        return AwbMgr::getInstance(ESensorDev_MainSecond).setPostgainLock(bPostgainLock);
    }
    else if(i4SensorDev & ESensorDev_SubSecond)
    {
        return AwbMgr::getInstance(ESensorDev_SubSecond).setPostgainLock(bPostgainLock);
    }

    CAM_LOGE("Err IAwbMgr::setPostgainLock()\n");
...
{
    if (i4SensorDev & ESensorDev_Main) {
        return AwbMgr::getInstance(ESensorDev_Main).getAWBStatInfo(rAWBStatInfo);
    }
    else if (i4SensorDev & ESensorDev_Sub) {
        return AwbMgr::getInstance(ESensorDev_Sub).getAWBStatInfo(rAWBStatInfo);
    }
    else if (i4SensorDev & ESensorDev_MainSecond) {
        return AwbMgr::getInstance(ESensorDev_MainSecond).getAWBStatInfo(rAWBStatInfo);
    }
    else if (i4SensorDev & ESensorDev_SubSecond) {
       return AwbMgr::getInstance(ESensorDev_SubSecond).getAWBStatInfo(rAWBStatInfo);
    }

    CAM_LOGE("Err IAwbMgr::getAWBStatInfo \n");
...
{
    if (i4SensorDev & ESensorDev_Main) {
        return AwbMgr::getInstance(ESensorDev_Main).setAWBStatInfo(rAWBStatInfo);
    }
    else if (i4SensorDev & ESensorDev_Sub) {
        return AwbMgr::getInstance(ESensorDev_Sub).setAWBStatInfo(rAWBStatInfo);
    }
    else if (i4SensorDev & ESensorDev_MainSecond) {
        return AwbMgr::getInstance(ESensorDev_MainSecond).setAWBStatInfo(rAWBStatInfo);
    }
    else if (i4SensorDev & ESensorDev_SubSecond) {
       return AwbMgr::getInstance(ESensorDev_SubSecond).setAWBStatInfo(rAWBStatInfo);
    }

    CAM_LOGE("Err IAwbMgr::setAWBStatInfo()\n");
