...
put(MUINT8 SubModuleIndex, ISP_NVRAM_DM_T const& rParam)
{
    ...
    PUT_REG_INFO_MULTI(SubModuleIndex, SPARE_1,                   spare_1);
...
get(MUINT8 SubModuleIndex, ISP_NVRAM_DM_T& rParam)
{
    ...
    GET_REG_INFO_MULTI(SubModuleIndex, SPARE_1,                   spare_1);
...
#define ISP_MGR_DM_PARAM_SIZE       (16)
			
	if (bEn == 1){
		char strTuningFile[512] = {'\0'};
		sprintf(strTuningFile, "data/vendor/camera_dump/dm_mgr.param");
		FILE* fidTuning = fopen(strTuningFile, "wb");
		if (fidTuning)
		{
		  fwrite(&(m_rIspRegInfo[SubModuleIndex][0]), sizeof(RegInfo_T)*ISP_MGR_DM_PARAM_SIZE, 1, fidTuning);
		  fclose(fidTuning);
		}
	}
	if (bEn == 2){
		FILE* fidTuning = fopen("data/vendor/camera_dump/dm_mgr.param", "rb");
		if (fidTuning)
		{
			RegInfo_T	tempInfo[ISP_MGR_DM_PARAM_SIZE];
			fread((&tempInfo), sizeof(tempInfo), 1, fidTuning);
			fclose(fidTuning);
			rTuning.tuningMgrWriteRegs( (TUNING_MGR_REG_IO_STRUCT*)(&(tempInfo)),
				ISP_MGR_DM_PARAM_SIZE, i4SubsampleIdex);
		}
	} else {
