...
class DceMgrDev : public DceMgr
{
public:
    static
    DceMgr*
    getInstance(ISP_NVRAM_DCE_TUNING_PARAM_T* pIspNvramDcePara)
    {
        static DceMgrDev<eSensorDev> singleton(pIspNvramDcePara);
        return &singleton;
    }
    virtual MVOID destroyInstance() {}

    DceMgrDev(ISP_NVRAM_DCE_TUNING_PARAM_T* pIspNvramDcePara)
        : DceMgr(eSensorDev, pIspNvramDcePara)
    {}
...
#define INSTANTIATE(_dev_id) \
    case _dev_id: return  DceMgrDev<_dev_id>::getInstance(pIspNvramDcePara)

DceMgr*
DceMgr::
createInstance(ESensorDev_T const eSensorDev, ISP_NVRAM_DCE_TUNING_PARAM_T* pIspNvramDcePara)
{
    switch  (eSensorDev)
    {
...
DceMgr::DceMgr(ESensorDev_T const eSensorDev, ISP_NVRAM_DCE_TUNING_PARAM_T* pIspNvramDcePara)
    ...
    , mAlgoEn(MTRUE)
{
    getPropInt("vendor.debug.dce_algo.disable", &mAlgoEn, 0);
    mAlgoEn = !mAlgoEn;
    ...
    memset(&m_DCES_Info,   0, sizeof(DCE2DCES_INFO_T));
...
start()
{
    getPropInt("vendor.debug.dynamic_dce.log", &mLogEn, 0);
...
calculateDCE(RAWIspCamInfo const& rRawIspCamInfo, MUINT16 Idx, MUINT32* pDcesBuffer,
                   const LCE2DCE_INFO_T& rLastLce2DceInfo,
                   ISP_NVRAM_DCE_T* pDCEReg, NSIspExifDebug::IspDceInfo* pDceExif)
{
    ...
    mInInfo.dce_lce_info.dr_idx_l = rLastLce2DceInfo.DRINDEX_L;
    mInInfo.dce_lce_info.dr_idx_h = rLastLce2DceInfo.DRINDEX_H;
    mInInfo.dce_lce_info.face_lo_bound = rLastLce2DceInfo.FACEBOUND_L;
    mInInfo.dce_lce_info.face_hi_bound = rLastLce2DceInfo.FACEBOUND_H;
    ...
    if(m_pDceAlgo){
        ...
        m_DCES_Info.YV_BLD_WT = mOutDCE.yv_bld;
        m_DCES_Info.DC_TH     = mOutDCE.DCES_D1A_DC_TH;
