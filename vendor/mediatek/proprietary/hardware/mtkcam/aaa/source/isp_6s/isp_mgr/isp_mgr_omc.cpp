...
{
    ISP_MGR_MODULE_GET_INSTANCE(OMC);
}

template <>
ISP_MGR_OMC_T&
ISP_MGR_OMC_T::
put(MUINT8 SubModuleIndex, ISP_NVRAM_OMC_T const& rParam)
{
...
ISP_MGR_OMC_T&
ISP_MGR_OMC_T::
get(MUINT8 SubModuleIndex, ISP_NVRAM_OMC_T rParam)
{
    if (SubModuleIndex >= ESubModule_NUM) {
...
MBOOL
ISP_MGR_OMC_T::
apply_MSS(MUINT8 SubModuleIndex, const RAWIspCamInfo& rRawIspCamInfo, mss_reg_t* pReg)
{
    if (SubModuleIndex >= ESubModule_NUM) {
        CAM_LOGE("[GET] Error Submodule Index: %d", SubModuleIndex);
        return MFALSE;
    ...
    dumpRegInfoP2("OMC", SubModuleIndex);
    return MTRUE;
}
