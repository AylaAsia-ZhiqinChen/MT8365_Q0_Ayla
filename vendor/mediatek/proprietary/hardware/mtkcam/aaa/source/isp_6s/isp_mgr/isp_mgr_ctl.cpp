...
{
    ISP_MGR_MODULE_GET_INSTANCE(CAMCTL);
}
...
getInstance(ESensorDev_T const eSensorDev)
{
    ISP_MGR_MODULE_GET_INSTANCE(DIPCTL);
}

template <>
MBOOL
ISP_MGR_DIPCTL_T::
get(ISP_NVRAM_DIPCTL_T& rParam, const dip_a_reg_t* pReg) const
{
    if (pReg)
    {
...
MBOOL
ISP_MGR_DIPCTL_T::
apply(const RAWIspCamInfo& /*rRawIspCamInfo*/, dip_a_reg_t* /*pReg*/)
{
