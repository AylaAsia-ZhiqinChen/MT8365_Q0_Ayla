...
#define ISP_MGR_WB_PARAM_SIZE       (2)
			
				if (bEn == 1){
					char strTuningFile[512] = {'\0'};
					sprintf(strTuningFile, "data/vendor/camera_dump/wb.param");
					FILE* fidTuning = fopen(strTuningFile, "wb");
					if (fidTuning)
					{
					  fwrite(&(m_rIspRegInfo[SubModuleIndex][0]), sizeof(RegInfo_T)*ISP_MGR_WB_PARAM_SIZE, 1, fidTuning);
					  fclose(fidTuning);
					}
				}
				if (bEn == 2){
