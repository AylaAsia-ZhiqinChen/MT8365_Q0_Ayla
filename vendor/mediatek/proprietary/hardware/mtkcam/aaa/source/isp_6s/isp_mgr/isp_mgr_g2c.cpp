...
{
    ISP_MGR_MODULE_GET_INSTANCE(G2C);
}
...
apply_P1(MUINT8 SubModuleIndex, const RAWIspCamInfo& rRawIspCamInfo, TuningMgr& rTuning, MINT32 i4SubsampleIdex)
{
    ...
    {
        case EG2C_R1:
            ISP_MGR_CAMCTL_T::getInstance(m_eSensorDev).setEnable_G2C_R1(bEnable);
            rTuning.updateEngine(eTuningMgrFunc_G2C_R1, bEnable, i4SubsampleIdex);
            break;
        case EG2C_R2:
            ISP_MGR_CAMCTL_T::getInstance(m_eSensorDev).setEnable_G2C_R2(bEnable);
            rTuning.updateEngine(eTuningMgrFunc_G2C_R2, bEnable, i4SubsampleIdex);
            break;
        default:
...
#define ISP_MGR_G2C_PARAM_SIZE       (6)
							
					if (bEn == 1){
						char strTuningFile[512] = {'\0'};
						sprintf(strTuningFile, "data/vendor/camera_dump/g2c_mgr.param");
						FILE* fidTuning = fopen(strTuningFile, "wb");
						if (fidTuning)
						{
						  fwrite(&(m_rIspRegInfo[SubModuleIndex][0]), sizeof(RegInfo_T)*ISP_MGR_G2C_PARAM_SIZE, 1, fidTuning);
						  fclose(fidTuning);
						}
					}
					if (bEn == 2){
						FILE* fidTuning = fopen("data/vendor/camera_dump/g2c_mgr.param", "rb");
						if (fidTuning)
						{
							RegInfo_T	tempInfo[ISP_MGR_G2C_PARAM_SIZE];
							fread((&tempInfo), sizeof(tempInfo), 1, fidTuning);
							fclose(fidTuning);
							rTuning.tuningMgrWriteRegs( (TUNING_MGR_REG_IO_STRUCT*)(&(tempInfo)),
								ISP_MGR_G2C_PARAM_SIZE, i4SubsampleIdex);
						}
					} else {
...
apply_P2(MUINT8 SubModuleIndex, const RAWIspCamInfo& rRawIspCamInfo, dip_a_reg_t* pReg)
{
    ...
    {
        case EG2C_D1:
            ISP_WRITE_ENABLE_BITS(pReg, DIPCTL_D1A_DIPCTL_YUV_EN1, DIPCTL_G2C_D1_EN, bEnable);
            break;
        default:
            CAM_LOGE("Apply Error Submodule Index: %d", SubModuleIndex);
            return  MFALSE;
