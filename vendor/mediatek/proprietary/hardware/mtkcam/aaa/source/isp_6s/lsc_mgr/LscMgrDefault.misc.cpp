...
getGainTable(MUINT32 gridPointNum, MINT32 i4MagicNum, ILscTbl& output) const
{
    ...
    /*else if (m_pTsf->getTbl(gridPointNum, gridPointNum, i4MagicNum, output) == MFALSE)
    {
...
MINT32
LscMgrDefault::
getCurrTbl(std::vector<MUINT8>& rData) const
{
    std::lock_guard<std::mutex> lock(m_Lock);
    if (m_pCurrentBuf)
    {
        ...
        ::memcpy(rData.data(), &rTblCfg, sizeof(ILscTable::Config));
        ::memcpy(rData.data()+sizeof(ILscTable::Config), &m_rRsvdData, sizeof(ILscTbl::RsvdData));
        ::memcpy(rData.data()+sizeof(ILscTable::Config)+sizeof(ILscTbl::RsvdData), m_pCurrentBuf->getTable(), m_pCurrentBuf->getSize());
    }
    return 0;
}
...
MINT32
LscMgrDefault::
getCurrTbl(MUINT32 u4CurrentBufIdx, std::vector<MUINT8>& rData) const
{
    std::lock_guard<std::mutex> lock(m_Lock);
    ILscBuf* pCurrentBuf;
    ...
    if (pCurrentBuf)
    {
        ...
        ::memcpy(rData.data(), &rTblCfg, sizeof(ILscTable::Config));
        ::memcpy(rData.data()+sizeof(ILscTable::Config), &m_rRsvdData, sizeof(ILscTbl::RsvdData));
        ::memcpy(rData.data()+sizeof(ILscTable::Config)+sizeof(ILscTbl::RsvdData), pCurrentBuf->getTable(), pCurrentBuf->getSize());
    }
    return 0;
}
...
MINT32
LscMgrDefault::
getCorresTsfo(MUINT32 u4CurrentBufIdx, std::vector<MUINT8>& rData)
{
    std::vector<MUINT8> &tsfoData = m_TsfoBufRing[u4CurrentBufIdx];

    rData.resize(tsfoData.size());
    ::memcpy(rData.data(), &tsfoData[0], tsfoData.size());
