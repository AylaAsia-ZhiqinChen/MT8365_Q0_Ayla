...
public:
    static
    GmaMgr*
    getInstance(ISP_NVRAM_GMA_STRUCT_T* pIspNvramGmaPara)
    {
        static GmaMgrDev<eSensorDev> singleton(pIspNvramGmaPara);
        return &singleton;
    }
    virtual MVOID destroyInstance() {}

    GmaMgrDev(ISP_NVRAM_GMA_STRUCT_T* pIspNvramGmaPara)
        : GmaMgr(eSensorDev, pIspNvramGmaPara)
    {}
...
#define INSTANTIATE(_dev_id) \
    case _dev_id: return  GmaMgrDev<_dev_id>::getInstance(pIspNvramGmaPara)

GmaMgr*
GmaMgr::
createInstance(ESensorDev_T const eSensorDev, ISP_NVRAM_GMA_STRUCT_T* pIspNvramGmaPara)
{
    switch  (eSensorDev)
    {
...
GmaMgr::GmaMgr(ESensorDev_T const eSensorDev, ISP_NVRAM_GMA_STRUCT_T* pIspNvramGmaPara)
    ...
    , m_bSameResult(0)
{
    getPropInt("vendor.debug.gma_algo.disable", &mAlgoEn, 0);
    mAlgoEn = !mAlgoEn;
...
start()
{
    getPropInt("vendor.debug.dynamic_gma.log", &mLogEn, 0);
                   ...
                   NSIspExifDebug::IspGmaInfo* pGmaExif)
{
    ...
    if (mAlgoEn) m_pGmaAlgo->GmaFeatureCtrl(MTKGMA_FEATURE_GET_GGMD1_RESULT, NULL, &mOutGGM_D1);
    if (mAlgoEn) m_pGmaAlgo->GmaFeatureCtrl(MTKGMA_FEATURE_GET_IGGMD1_RESULT, NULL, &mOutIGGM_D1);
    ...
    if (m_pGmaAlgo)
    {
        ...
        m_bSameResult = !(mOutGGM.bIsCurveChange);
    }
}
