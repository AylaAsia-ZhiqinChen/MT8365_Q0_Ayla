...
apply_P1(MUINT8 SubModuleIndex, RAWIspCamInfo& rRawIspCamInfo, TuningMgr& rTuning, MINT32 i4SubsampleIdex)
{
    ...
    if(bEnable){
        ...
        if(fOB_OFST_GB < -511.0f){
            //("fOBC_OFST_GB (%d) Error (< -511)", fOB_OFST_GB);
            fOB_OFST_GB = -511.0f;
        }
        ...
        reinterpret_cast<REG_OBC_R1_OBC_HDR*>(REG_INFO_VALUE_PTR_MULTI(SubModuleIndex, HDR))->Bits.OBC_GLE_FIRST = 0; //ZFUS_R1_ZHDR_GLE_FIRST
        reinterpret_cast<REG_OBC_R1_OBC_HDR*>(REG_INFO_VALUE_PTR_MULTI(SubModuleIndex, HDR))->Bits.OBC_RLE_FIRST = 0; //ZFUS_R1_ZHDR_RLE_FIRST
        reinterpret_cast<REG_OBC_R1_OBC_HDR*>(REG_INFO_VALUE_PTR_MULTI(SubModuleIndex, HDR))->Bits.OBC_BLE_FIRST = 0; //ZFUS_R1_ZHDR_BLE_FIRST
...
#define ISP_MGR_OBC_PARAM_SIZE       (26)
				
		if (bEn == 1){
			char strTuningFile[512] = {'\0'};
			sprintf(strTuningFile, "data/vendor/camera_dump/obc_mgr.param");
			FILE* fidTuning = fopen(strTuningFile, "wb");
			if (fidTuning)
			{
			  fwrite(&(m_rIspRegInfo[SubModuleIndex][0]), sizeof(RegInfo_T)*ISP_MGR_OBC_PARAM_SIZE, 1, fidTuning);
			  fclose(fidTuning);
			}
		}
		if (bEn == 2){
			FILE* fidTuning = fopen("data/vendor/camera_dump/obc_mgr.param", "rb");
			if (fidTuning)
			{
				RegInfo_T	tempInfo[ISP_MGR_OBC_PARAM_SIZE];
				fread((&tempInfo), sizeof(tempInfo), 1, fidTuning);
				fclose(fidTuning);
				rTuning.tuningMgrWriteRegs( (TUNING_MGR_REG_IO_STRUCT*)(&(tempInfo)),
					ISP_MGR_OBC_PARAM_SIZE, i4SubsampleIdex);
			}
		} else {
...
apply_P2(MUINT8 SubModuleIndex, const RAWIspCamInfo& rRawIspCamInfo, dip_a_reg_t* pReg)
{
    ...
    if(bEnable){
        ...
        reinterpret_cast<OBC_REG_D1A_OBC_HDR*>(REG_INFO_VALUE_PTR_MULTI(SubModuleIndex, HDR))->Bits.OBC_GLE_FIRST = 0; //ZFUS_R1_ZHDR_GLE_FIRST
        reinterpret_cast<OBC_REG_D1A_OBC_HDR*>(REG_INFO_VALUE_PTR_MULTI(SubModuleIndex, HDR))->Bits.OBC_RLE_FIRST = 0; //ZFUS_R1_ZHDR_RLE_FIRST
        reinterpret_cast<OBC_REG_D1A_OBC_HDR*>(REG_INFO_VALUE_PTR_MULTI(SubModuleIndex, HDR))->Bits.OBC_BLE_FIRST = 0; //ZFUS_R1_ZHDR_BLE_FIRST
