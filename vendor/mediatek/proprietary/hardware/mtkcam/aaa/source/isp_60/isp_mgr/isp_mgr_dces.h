...
public:
    ISP_MGR_DCES(MUINT32 const eSensorDev)
        : ISP_MGR_BASE_T(m_rIspRegInfo, ERegInfo_NUM, eSensorDev, ESubModule_NUM)
    {
        ...
        for(int i=0; i<ESubModule_NUM; i++){
            ...
            reinterpret_cast<DCES_REG_D1A_DCES_DC_CTRL*>(REG_INFO_VALUE_PTR_MULTI(i, DC_CTRL))->Bits.DCES_DC_TH = 8;
...
public: // Interfaces.
    ...
    MVOID
    setLastYV_Bld(MUINT8 SubModuleIndex, MUINT32 DCES_yv_bld)
    {
        reinterpret_cast<DCES_REG_D1A_DCES_V_Y_CH_1*>(REG_INFO_VALUE_PTR_MULTI(SubModuleIndex, V_Y_CH_1))->Bits.DCES_VY_BLD_WT = DCES_yv_bld;
    }
