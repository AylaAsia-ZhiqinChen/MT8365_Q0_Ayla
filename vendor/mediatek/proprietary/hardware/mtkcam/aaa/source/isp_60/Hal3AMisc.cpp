...
#define MY_INST NS3Av3::INST_T<NS3Av3::Hal3AMisc>
...
init()
{
    std::lock_guard<std::mutex> lock(LCSlock);
    LCSoutQ.clear();
    return ;
}
...
updateLCSList_Out(const ISP_LCS_OUT_INFO_T& rLCSinfo)
{
    ...
    std::list<ISP_LCS_OUT_INFO_T>::iterator it = LCSoutQ.begin();
...
getLCSListLast_Out(ISP_LCS_OUT_INFO_T& rLCSinfo)
{
    ...
    if (!LCSoutQ.empty())
    {
        std::list<ISP_LCS_OUT_INFO_T>::iterator it = LCSoutQ.end();
        it--;
        rLCSinfo = *it;
        return 0;
    }
