...
#define INIT_LPCNR_ADDR(reg)\
    INIT_REG_INFO_ADDR_P2_MULTI(ELPCNR_D1 ,reg, LPCNR_D1A_LPCNR_)
...
public:
    ISP_MGR_LPCNR(MUINT32 const eSensorDev)
        : ISP_MGR_BASE_T(m_rIspRegInfo, ERegInfo_NUM, eSensorDev, ESubModule_NUM)
    {
        ...
        INIT_LPCNR_ADDR(TOP);
        //INIT_LPCNR_ADDR(ATPG);
        //INIT_LPCNR_ADDR(SIZE);
        ...
        //INIT_LPCNR_ADDR(DUMMY);
        //INIT_LPCNR_ADDR(IN1_CNT);
        //INIT_LPCNR_ADDR(IN2_CNT);
        //INIT_LPCNR_ADDR(IN3_CNT);
        //INIT_LPCNR_ADDR(OUT_CNT);
        //INIT_LPCNR_ADDR(STATUS);
        for(int i=0; i<ESubModule_NUM; i++){
            //Sub module always ENABLE
            reinterpret_cast<LPCNR_REG_D1A_LPCNR_TOP*>(REG_INFO_VALUE_PTR_MULTI(i, TOP))->Bits.LPCNR_lpcnr_core_en = 1;
        }
    }
