...
apply_P1(MUINT8 SubModuleIndex, const RAWIspCamInfo& rRawIspCamInfo, TuningMgr& rTuning, MINT32 i4SubsampleIdex)
{
    ...
    {
        case EFLC_R1:
            ISP_MGR_CAMCTL_T::getInstance(m_eSensorDev).setEnable_FLC_R1(bEnable);
            rTuning.updateEngine(eTuningMgrFunc_FLC_R1, bEnable, i4SubsampleIdex);
            break;
        default:
...
apply_P2(MUINT8 SubModuleIndex, const RAWIspCamInfo& rRawIspCamInfo, dip_x_reg_t* pReg)
{
    ...
    {
        case EFLC_D1:
            ISP_WRITE_ENABLE_BITS(pReg, DIPCTL_D1A_DIPCTL_RGB_EN2, DIPCTL_FLC_D1_EN, bEnable);
            break;
        default:
            CAM_LOGE("Apply Error Submodule Index: %d", SubModuleIndex);
            return  MFALSE;
