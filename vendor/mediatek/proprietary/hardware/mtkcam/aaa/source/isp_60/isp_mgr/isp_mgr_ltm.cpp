...
#define MY_INST NS3Av3::INST_T<ISP_MGR_LTM_T>
...
apply_P2(MUINT8 SubModuleIndex, const RAWIspCamInfo& rRawIspCamInfo, dip_x_reg_t* pReg)
{
    ...
    if(bEnable){
        ...
        ct_alpha_base = (ct_alpha_base<0)? 0 :((ct_alpha_base>1023)? 1023 : ct_alpha_base);
        reinterpret_cast<LTM_REG_D1A_LTM_MAX_DIV*>(REG_INFO_VALUE_PTR_MULTI(SubModuleIndex, MAX_DIV))->Bits.LTM_MAXVALUE_ALPHA_BASE_SHIFT_BIT = ct_alpha_base_shift_bit;
        reinterpret_cast<LTM_REG_D1A_LTM_MAX_DIV*>(REG_INFO_VALUE_PTR_MULTI(SubModuleIndex, MAX_DIV))->Bits.LTM_MAXVALUE_ALPHA_BASE = ct_alpha_base;
