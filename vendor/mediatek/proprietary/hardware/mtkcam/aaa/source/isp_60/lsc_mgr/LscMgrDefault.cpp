...
#define MAX_POLLING_COUNT 100
    ...
    for (i = 0; i < RING_BUF_NUM; i++)
    {
        ...
        MUINT32 id = 0xFFFFFFDF - i;           /*m_pLsc->m_rBufPool.size()*/
...
evaluate_cus_Ratio(LSC_INPUT_INFO_T &rLscInputInfo, MINT32 i4IsLockRto)
{
    ...
    MBOOL fgLockRto =((rLscInputInfo.bAELock || rLscInputInfo.bAELimiter || i4IsLockRto || m_bLockRatio) && !m_fgSuperNightMode);
    ...
    CAM_LOGD_IF(fgLogEn, "[%s] (flash, iso, rto)=(%d, %d, %d), auto(%d), lock(%d,%d,%d), dyn(%d), PreCapFlashIsLockRto(%d), super night En(%d), ev (%d)", __FUNCTION__,
        ...
        m_fgSuperNightMode, rLscInputInfo.i4AEComp);
...
{
    CAM_LOGD("[%s] set app info %d", __FUNCTION__, appParam);
    m_appParam = appParam;
}
