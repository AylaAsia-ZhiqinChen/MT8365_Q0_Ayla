...
getCurrResult4AF(MINT32& i4MagicNumber, AFParam_T& AFParam)
{
    ...
    if(pAllResult)
    {
        ...
        MY_LOGD("[%s] update AF config result to ResutlPool", __FUNCTION__);
        ...
        m_pResultPoolObj->updateResult(LOG_TAG, i4MagicNumber, E_AF_CONFIGRESULTTOISP, &AFResultConfig); // frame id 1 for config

        // update PDO config result to ResutlPool
        MY_LOGD("[%s] update PDO config result to ResutlPool", __FUNCTION__);
        isp_pdo_cfg_t PDOResultConfig;
        IPDMgr::getInstance().getPDOHWCfg(mi4SensorDev, &PDOResultConfig);
        m_pResultPoolObj->updateResult(LOG_TAG, i4MagicNumber, E_PDO_CONFIGRESULTTOISP, &PDOResultConfig); // frame id 1 for config
#endif
    }
    else
