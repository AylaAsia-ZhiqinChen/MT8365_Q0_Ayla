...
//
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
MRESULT IAeMgr::prepareCapParams(MINT32 i4SensorDev, MBOOL bIsEVChange)
{
    MINT32 ret_main, ret_sub, ret_main2, ret_sub2, ret_main3;
    ret_main = ret_sub = ret_main2 = ret_sub2 = ret_main3 = S_AE_OK;

    if (i4SensorDev & ESensorDev_Main)
        ret_main = AeMgr::getInstance(ESensorDev_Main).prepareCapParams(bIsEVChange);
    if (i4SensorDev & ESensorDev_Sub)
        ret_sub = AeMgr::getInstance(ESensorDev_Sub).prepareCapParams(bIsEVChange);
    if (i4SensorDev & ESensorDev_MainSecond)
        ret_main2 = AeMgr::getInstance(ESensorDev_MainSecond).prepareCapParams(bIsEVChange);
    if (i4SensorDev & ESensorDev_SubSecond)
        ret_sub2 = AeMgr::getInstance(ESensorDev_SubSecond).prepareCapParams(bIsEVChange);
    if (i4SensorDev & ESensorDev_MainThird)
        ret_main3 = AeMgr::getInstance(ESensorDev_MainThird).prepareCapParams(bIsEVChange);
