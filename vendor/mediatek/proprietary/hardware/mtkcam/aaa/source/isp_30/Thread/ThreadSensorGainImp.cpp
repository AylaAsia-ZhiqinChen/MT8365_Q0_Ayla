    ...
    , m_u4SensorDev(u4SensorDev)
{
    ...
    const char* const callerName = "ThreadsensorGainImp";
    IHalSensor* pHalSensor = pHalSensorList->createSensor(callerName, i4SensorIndex);
    MY_ASSERT(pHalSensor != nullptr);

    SensorDynamicInfo senInfo;
    MINT32 ret = pHalSensor->querySensorDynamicInfo(u4SensorDev, &senInfo);
    MY_ASSERT(ret == MTRUE);
