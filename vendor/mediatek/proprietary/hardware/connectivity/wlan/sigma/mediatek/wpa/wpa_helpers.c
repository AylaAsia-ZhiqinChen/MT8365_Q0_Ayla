...
char *wpas_ctrl_path = CONFIG_CTRL_IFACE_CLIENT_DIR"/";
#else
...
int get_wpa_cli_events(struct wpa_ctrl *mon, const char **events,
        char *buf, size_t buf_size)
{
	int fd, ret;
	fd_set rfd;
	...
	while (1) {
		...
		ret = select(fd + 1, &rfd, NULL, NULL, &tv);
		if (ret == 0) {
                        printf("Timeout on waiting for event\n");
			return -1;
		}
		if (ret < 0) {
			printf("select: %s\n", strerror(errno));
		...
		len = buf_size;
		if (wpa_ctrl_recv(mon, buf, &len) < 0) {
                        printf("Failure while waiting for event\n");
			return -1;
		}
		if (len == buf_size)
			len--;
