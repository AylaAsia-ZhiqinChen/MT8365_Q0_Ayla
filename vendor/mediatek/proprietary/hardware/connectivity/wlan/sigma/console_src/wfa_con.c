    ...
    for(i=1; i<argc; i++)
    {
        if(argv[i][0]=='-')
        {
            if(argv[i][1]=='t')
            {
                ...
                /*
                 * AP interface name as parameter for ioctl.
                 * eg. when bridge enabled, we use br0 as interface name,
                 * instead, we use ap0 as interface name.
                 */
                i++;
                intf_name = argv[i];
            }
            ...
            {
                DPRINT_INFO(WFA_OUT, "Unrecognized options\n");
                exit(1);
            }
        }
        else
