...
int setup_addr(char *name, struct sockaddr *dst, char *intf_name)
{
    struct hostent *h;
    char *array[5];
    char c;
...
#ifndef __CYGWIN__
    DPRINT_INFO(WFA_OUT,  "setup_addr: entering\n" );
    DPRINT_INFO(WFA_OUT, "Automatically discover IP Address of eth0 interface, since no broadcast IP address has been specified by you.\n");
    /* tells ioctl which interface to query */
#ifdef CONFIG_MTK_AP
    strcpy(interface.ifr_name, intf_name);
#else
    ...
    DPRINT_INFO(WFA_OUT, "IP of eth0: %s\n", IP);
...
void setup_socket(char *intf_name)
{
    char name[256];
    int dscp = 0;
    ...
    set_dscp(dscp);
    /* Setup broadcast*/
    if ((r=setup_addr(name, &dst, intf_name))<0)
    {
...
int set_dscp(int new_dscp)
{
    ...
    if ((r=setsockopt(sd, SOL_IP, IP_TOS, &new_dscp, sizeof(dscp)))<0)
    {
        ...
        DPRINT_INFO(WFA_OUT, "can't set dscp/tos field 0x%x", new_dscp);
        exit(-1);
    }
    dscp = ldscp = new_dscp;
    rmsg[1] = dscp;
...
void ap_create_apts_msg(int msg, unsigned int txbuf[],int id)
{
    ...
    if (traceflag) DPRINT_INFO(WFA_OUT, "ap_create_apts_msg (%s)\n", t->name);
}
