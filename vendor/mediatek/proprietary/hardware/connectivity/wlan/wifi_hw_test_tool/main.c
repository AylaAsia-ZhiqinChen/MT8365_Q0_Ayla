...
static bool checkNeedRemap(void);
static int remapTxDataRateIfNeeded(int TxDataRate);
    ...
    while ((opt = getopt(argc, argv, "A:f:E:e:M:g:G:I:B:R:N:Tm:S:s:p:b:t:hVw:v:k:l:f:c:rOCn:DP:x:Ld:j:qa:o")) != -1) {
	    switch(opt) {
	        ...
	        case 'G':
	            ...
	            break;
	        case 'I':
				 strncpy(WIFI_IF_NAME, optarg, sizeof(WIFI_IF_NAME) - 1);
...
void wifi_tx(void)
{
    ...
    if(txGain != Default_TxGain){
		...
		printf("Set Tx power gain by NVRAM Setting\n");
		if((0< channel)&&(channel < 15)) {
			...
			printf("not support this channel,please checking....\n");
			return;
		}
    }
    ...
    case 0: /* A/B/G mode */
        ...
        newRate = remapTxDataRateIfNeeded(g_rate);
        if (newRate != 0xFFFFFFFF) 
            retval = WIFI_TEST_TxDataRate(newRate);
        else
            retval = 0;
        printf("(%s) Set Tx mode to 11a/b/g, tx rate %s\n", retval ? "success":"fail", bg_rate[g_rate]);
        if (retval == 0) return;
...
static bool checkNeedRemap(void)
{
    return !TURN_OFF_REMAP;
}

/*this function remap CLI-stype abg mode rate to FW-stype abg mode rate*/
static int remapTxDataRateIfNeeded(int TxDataRate)
{
    int newRate = TxDataRate;
    if (checkNeedRemap()){
        switch (TxDataRate) {
            case 4: /*CLI-stype OFDM_6M*/
                newRate = 4;
                break;
            case 5: /*CLI-stype OFDM_9M*/
                newRate = 5;
                break;
            case 6: /*CLI-stype CCK_11M*/
                newRate = 3;
                break;
            case 1: /*CLI-stype CCK_1M*/
            case 2: /*CLI-stype CCK_2M*/
            case 3: /*CLI-stype CCK_5.5M*/
            case 7: /*CLI-stype OFDM_12M*/
            case 8: /*CLI-stype OFDM_18M*/
            case 9: /*CLI-stype OFDM_24M*/
            case 10: /*CLI-stype OFDM_36M*/
            case 11: /*CLI-stype OFDM_48M*/
            case 12: /*CLI-stype OFDM_54M*/
                newRate = TxDataRate - 1;
                break;
            case 0: /*CLI-stype AUTO*/
                newRate = WIFI_TEST_RATE_AUTO;
                break;
            default:
                newRate = 0xFFFFFFFF; /*invalid rate*/
                break;
        }
        //remap txdatarate
        printf("After remap, ori rate = %d and new is %d", TxDataRate, newRate);
    }
    return newRate;
}
