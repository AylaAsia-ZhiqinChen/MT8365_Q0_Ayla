...
getInstance()
{
    MTKWarpExt *pAppGlesWarp = NULL;

    LOGD("[%s] getInstance \n", LOG_TAG);
    if (pAppGlesWarp == NULL) {
        pAppGlesWarp = new AppGlesWarpExt();
    }
/*
#ifndef SIM_MAIN
...
void
AppGlesWarpExt::
destroyInstance(MTKWarpExt* pAppGlesWarp)
{
    if (pAppGlesWarp) {
        delete pAppGlesWarp;
    }
...
MRESULT AppGlesWarpExt::WarpInit(MUINT32 *InitInData, MUINT32 *InitOutData)
{
    LOGD("[%s] WarpInit \n", LOG_TAG);
    ...
    core->source_color_domain = (UTIL_EGLIMAGE_COLOR_SPACE)input->source_color_domain;
...
MRESULT AppGlesWarpExt::WarpMain(void)
{
    //LOGD("[%s] WarpMain \n", LOG_TAG);
...
MRESULT AppGlesWarpExt::WarpReset(void)
{
    LOGD("[%s] WarpReset \n", LOG_TAG);
    WarpState = WARP_STATE_STANDBY;
    //gWarpCurrImageNum = 0;
...
MRESULT AppGlesWarpExt::WarpFeatureCtrl(MUINT32 FeatureID, void* pParaIn, void* pParaOut)
{
    MRESULT RetCode = S_WARP_OK;
    ...
    case WARP_FEATURE_INIT_EGLIMAGE:
         ...
         core->source_color_domain = (UTIL_EGLIMAGE_COLOR_SPACE) ImgInfo->source_color_domain;
