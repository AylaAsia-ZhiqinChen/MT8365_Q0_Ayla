...
tNFC_STATUS RW_MfcWriteNDef(uint16_t buf_len, uint8_t* p_buffer) {
  ...
  if (i < 32) {
    p_mfc->last_block_accessed.block = i * 4;
    p_mfc->next_block.block = i * 4;
  } else {
    p_mfc->last_block_accessed.block = 128 + (i - 32) * 16;
    p_mfc->next_block.block = 128 + (i - 32) * 16;
  }
...
static void rw_mfc_handle_read_op(uint8_t* data) {
  ...
  switch (p_mfc->state) {
    case RW_MFC_STATE_DETECT_MAD:
      ...
      } else if (p_mfc->current_block == 66) {
        DLOG_IF(INFO, nfc_debug_enabled)
