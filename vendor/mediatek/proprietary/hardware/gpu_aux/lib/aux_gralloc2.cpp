...
uint64_t getValid10UsageBits() {
    static const uint64_t valid10UsageBits = []() -> uint64_t {
        ...
        uint64_t bits = 0;
        for (const auto bit : hidl_enum_range<BufferUsage>()) {
            bits = bits | bit;
        }
        return bits;
    }();
...
void AuxGralloc2Mapper::preload() {
    android::hardware::preloadPassthroughService<android::hardware::graphics::mapper::V2_0::IMapper>();
}

AuxGralloc2Mapper::AuxGralloc2Mapper() {
    mMapper = android::hardware::graphics::mapper::V2_0::IMapper::getService();
    if (mMapper == nullptr) {
        ALOGW("@%s: mapper 2.x is not supported", __func__);
        return;
    }
    if (mMapper->isRemote()) {
...
bool AuxGralloc2Mapper::isLoaded() const {
    return mMapper != nullptr;
}

status_t AuxGralloc2Mapper::validateBufferDescriptorInfo(
    ...
    if (descriptorInfo->usage & ~validUsageBits) {
        ALOGE("@%s: buffer descriptor contains invalid usage bits 0x%" PRIx64 " 0x%" PRIx64 " 0x%" PRIx64,
...
status_t AuxGralloc2Mapper::createDescriptor(void* bufferDescriptorInfo,
...
status_t AuxGralloc2Mapper::importBuffer(const hidl_handle& rawHandle,
...
void AuxGralloc2Mapper::freeBuffer(buffer_handle_t bufferHandle) const {
...
status_t AuxGralloc2Mapper::validateBufferSize(buffer_handle_t bufferHandle, uint32_t width,
...
void AuxGralloc2Mapper::getTransportSize(buffer_handle_t bufferHandle, uint32_t* outNumFds,
...
status_t AuxGralloc2Mapper::lock(buffer_handle_t bufferHandle, uint64_t usage, const Rect& bounds,
...
int AuxGralloc2Mapper::unlock(buffer_handle_t bufferHandle) const {
    auto buffer = const_cast<native_handle_t*>(bufferHandle);
...
status_t AuxGralloc2Mapper::isSupported(uint32_t /*width*/, uint32_t /*height*/,
