...
uint64_t getValidUsageBits() {
    static const uint64_t validUsageBits = []() -> uint64_t {
        uint64_t bits = 0;
        for (const auto bit : hidl_enum_range<android::hardware::graphics::common::V1_2::BufferUsage>()) {
            bits = bits | bit;
        }
        return bits;
    }();
...
void AuxGralloc3Mapper::preload() {
    android::hardware::preloadPassthroughService<IMapper>();
}

AuxGralloc3Mapper::AuxGralloc3Mapper() {
    mMapper = IMapper::getService();
    if (mMapper == nullptr) {
        ALOGW("@%s: mapper 3.x is not supported", __func__);
        return;
    }
    if (mMapper->isRemote()) {
        ALOGW("@%s: gralloc-mapper must be in passthrough mode", __func__);
    }
}

bool AuxGralloc3Mapper::isLoaded() const {
    return mMapper != nullptr;
}

status_t AuxGralloc3Mapper::validateBufferDescriptorInfo(
        IMapper::BufferDescriptorInfo* descriptorInfo) const {
    uint64_t validUsageBits = getValidUsageBits();

    if (descriptorInfo->usage & ~validUsageBits) {
        ALOGE("@%s: buffer descriptor contains invalid usage bits 0x%" PRIx64,
              __func__, descriptorInfo->usage & ~validUsageBits);
        return android::BAD_VALUE;
    }
    return android::NO_ERROR;
}

status_t AuxGralloc3Mapper::createDescriptor(void* bufferDescriptorInfo,
...
status_t AuxGralloc3Mapper::importBuffer(const hidl_handle& rawHandle,
...
void AuxGralloc3Mapper::freeBuffer(buffer_handle_t bufferHandle) const {
    auto buffer = const_cast<native_handle_t*>(bufferHandle);
    auto ret = mMapper->freeBuffer(buffer);

    auto error = (ret.isOk()) ? static_cast<Error>(ret) : kTransactionError;
    ALOGE_IF(error != Error::NONE, "@%s: freeBuffer(%p) failed with %d", __func__, buffer, error);
}

status_t AuxGralloc3Mapper::validateBufferSize(buffer_handle_t bufferHandle, uint32_t width,
...
void AuxGralloc3Mapper::getTransportSize(buffer_handle_t bufferHandle, uint32_t* outNumFds,
    ...
    *outNumInts = uint32_t(bufferHandle->numInts);
    ...
    ALOGE_IF(error != Error::NONE, "@%s: getTransportSize(%p) failed with %d", __func__, buffer, error);
}

status_t AuxGralloc3Mapper::lock(buffer_handle_t bufferHandle, uint64_t usage, const Rect& bounds,
    ...
    ALOGW_IF(error != Error::NONE, "@%s: lock(%p, ...) failed: %d", __func__, bufferHandle, error);
...
int AuxGralloc3Mapper::unlock(buffer_handle_t bufferHandle) const {
    auto buffer = const_cast<native_handle_t*>(bufferHandle);
    ...
    auto ret = mMapper->unlock(buffer, [&](const auto& tmpError, const auto& tmpReleaseFence) {
        ...
        if (fenceHandle && fenceHandle->numFds == 1) {
            ...
            if (fd >= 0) {
                ...
                ALOGW("@%s: failed to dup unlock release fence", __func__);
    ...
    if (error != Error::NONE) {
        ALOGE("@%s: unlock(%p) failed with %d", __func__, buffer, error);
    }

    return releaseFence;
}

status_t AuxGralloc3Mapper::isSupported(uint32_t width, uint32_t height, PixelFormat format,
                                        uint32_t layerCount, uint64_t usage,
    ...
    if (error != Error::NONE) {
        ALOGE("@%s: isSupported(%u, %u, %d, %u, ...) failed with %d",
              __func__, width, height, format, layerCount, error);
    }
