...
class VpuStream: public apusysEngine {

public:
    static VpuStream *createInstance();
    static VpuStream *createInstance(const char* callername);

    virtual VpuAlgo *getAlgo(const char *name) = 0;

    virtual VpuRequest *acquire(VpuAlgo *algo, VpuModuleType module = eModuleTypeCv) = 0;

    virtual void release(VpuRequest *request) = 0;

    virtual bool enque(VpuRequest *request, unsigned int core_index = 0xFFFF) = 0;  // deprecated

    virtual bool tryEnque(VpuRequest *request) = 0;  // deprecated

    virtual VpuRequest *deque() = 0;  // deprecated

	virtual bool runReq(VpuRequest *request, bool async = true) = 0;

	virtual bool waitReq(VpuRequest *request) = 0;
    ...
    /* 3.0 vpu create algo*/
    virtual VpuCreateAlgo createAlgo(const char *name, char *buf, uint32_t len, uint32_t core) = 0;
