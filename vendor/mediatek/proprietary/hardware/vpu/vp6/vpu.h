...
public:
	...
	virtual bool freeAlgo(VpuCreateAlgo algoFd) = 0;

	virtual ~VpuStream() { };

	virtual VpuMemBuffer *allocMem(size_t size, uint32_t align, enum vpuMemType type);
	virtual VpuMemBuffer *importMem(int fd, uint32_t len, enum vpuMemType type);
	virtual bool freeMem(VpuMemBuffer *buf);
	virtual bool syncMem(VpuMemBuffer *buf, enum vpuSyncType type);
};
