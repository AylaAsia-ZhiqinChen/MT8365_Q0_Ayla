...
static void* _vpu_coreA_test_thread(void *arg)
{
    ...
    // init test image data
    memcpy((void *)img1_va ,(const void *)pgTest_img1 , gTestImgSize);
#if 1 // sync after setup input and test data
    if(_vpu_sync_buffer(ion_drv_handle, gTestImgSize, img1_fd, img1_va) == false)
    {
        MY_LOGW("[%s] sync img1 fail\n", __func__);
        result |= VPU_TEST_FAIL;
        goto EXIT;
    }
#endif
    ...
    for(i = 0 ; i < que_cnt ; i++)
    {
        ...
        memcpy((void *)data1_va[i],(const void *)pgTest_data1, gTestDataLength);
#if 1 // sync after setup input and test data
        MY_LOGI("[%s][vpu][%d] sync output/test data when init...\n", __func__, __LINE__);
        if(_vpu_sync_buffer(ion_drv_handle, gTestImgSize, img2_fd[i], img2_va[i]) == false)
        {
            MY_LOGW("[%s] sync img2 fail\n", __func__);
            result |= VPU_TEST_FAIL;
            goto EXIT;
        }
        if(_vpu_sync_buffer(ion_drv_handle, gTestDataLength, data1_fd[i], data1_va[i]) == false)
        {
            MY_LOGW("[%s] sync data1 fail\n", __func__);
            result |= VPU_TEST_FAIL;
            goto EXIT;
        }
#endif
...
::sem_wait(&mSem);
    ...
    while(original_cnt > 0)
    {
	    	...
	    	if(g_coreA_test_pattern != 2)
		{
			...
			if(g_coreA_test_pattern == 2)
			{//k7
				MY_LOGD("[coreA] k7 sett");
				d2d_k7_sett_structure_t property;
				property.enable_compare = 1;
				property.result = 0;
				property.cnt_process = 0;
				property.cnt_dma = 0;
				request[i]->setProperty((void*)&property, sizeof(d2d_k7_sett_structure_t));
			}
	    	}
			...
			if(coreA_adjust_opp)
			{
				...
				else
				{
					rand_value = rand() % 3;
					#if 1
					//extraP.opp_step = rand() % 16;
					extraP.boost_value = rand() % 100;

					//extraP.boost_value = 100;
					#else
					extraP.opp_step = rand() % 2;
					extraP.freq_step = rand() % 4;
					#endif
				}
				MY_LOGW("[coreA] value(%d/%d/%d)", rand_value, extraP.opp_step, extraP.boost_value);
		...
		//non_stop for HQA
			...
			if(g_coreA_test_pattern == 0 || g_coreA_test_pattern ==1)
			{//k3
#if 1 // sync after setup output
                    MY_LOGI("[%s][vpu][%d] sync output data to compare...\n", __func__, __LINE__);
                    if(_vpu_sync_buffer(ion_drv_handle, gTestImgSize, img2_fd[i], img2_va[i]) == false)
                    {
                        MY_LOGW("[%s] sync img2 fail\n", __func__);
                        result |= VPU_TEST_FAIL;
			...
			if(g_coreA_test_pattern == 2)
			{//k7
				MY_LOGD("[coreA] check k7 test result");
				d2d_k7_sett_structure_t property;
				property.enable_compare = 1;
				property.result = 0;
				property.cnt_process = 0;
				property.cnt_dma = 0;
				request[i]->getProperty((void*)&property, sizeof(d2d_k7_sett_structure_t));

				if (property.result != 0xc0000001)
					result_cmp = 1;
			}
	            ...
	            {
	                result |= VPU_TEST_FAIL;
	                MY_LOGI("[coreA] queue %d compare failed, %d", i, result_cmp);
				    if(g_non_stop)
					goto EXIT;
	            }
	        }
...
#if 1 // sync after setup output and test data
        MY_LOGI("[%s][vpu][%d] sync output/test data to restart...\n", __func__, __LINE__);
        if(_vpu_sync_buffer(ion_drv_handle, gTestImgSize, img2_fd[i], img2_va[i]) == false)
        {
            MY_LOGW("[%s] sync img2 fail\n", __func__);
            result |= VPU_TEST_FAIL;
    ...
    while(original_cnt > 0)
    {
	    for(i = 0 ; i < que_cnt ; i++)
	    {
			...
			if(coreB_adjust_opp)
			{
				...
				MY_LOGW("[coreB] value(%d/%d/%d)", rand_value, extraP.opp_step, extraP.boost_value);
				usleep(rand_value * 1000);
				request[i]->setExtraParam(extraP);
			}
	    ...
	    for(i = 0 ; i < que_cnt ; i++)
	    {
	        ...
	        if (!flushed)
	        {
	            ...
	            if(g_coreB_test_pattern == 2)
		    {//k7
			MY_LOGD("[coreB] check k7 test result");
			d2d_k7_sett_structure_t property;
			property.enable_compare = 1;
			property.result = 0;
			property.cnt_process = 0;
		...
		for(i = 0 ; i < que_cnt ; i++)
	    {
        	...
        	_vpu_setup_buf(buf_img2[i] , img2_fd[i] , eFormatImageY8, gTestImgWidth, gTestImgHeight, 0, gTestImgWidth, gTestImgSize);
#if 1 // sync after setup output and test data
            MY_LOGI("[%s][vpu][%d] sync output/test data\n", __func__, __LINE__);
            if(_vpu_sync_buffer(ion_drv_handle, gTestImgSize, img2_fd[i], img2_va[i]) == false)
            {
                MY_LOGW("[%s] sync img2 fail\n", __func__);
                result |= VPU_TEST_FAIL;
...
int vpu_Imax_test(UINT32 test_pattern, UINT32 maxCoreNum, UINT32 que_cnt, bool non_stop, int time_exec, int time_sleep)
{
	...
	coreA_random_opp=true;
	coreB_random_opp=true;
...
void vpu_performance_test (INT64 totalLength)
{
    ...
    // flush cache
    if (!_vpu_sync_buffer(ion_drv_handle, ion_total_szie, i4In1FD, prVaIn1))
    {
        MY_LOGW("[%s] sync fail\n", __func__);
        goto EXIT;
    }
    ...
    prAlgo = prVPU->getAlgo("vpu_flo_d2d_kp");
    LOGE("[%s] Start %s, load vpu_flo_d2d_kp \n", LOG_TAG, "Algo2");
    MY_GET_TIME(&sub_end_time, NULL);
    MY_DIFF_TIME(&temp_time, sub_start_time, sub_end_time);
    MY_ADD_TIME(elapse_time[2], temp_time);
    if (prAlgo == NULL)
    {
		LOGE("[%s] fail to get vpu algo handle %s, re-loading !!\n", LOG_TAG, "vpu_flo_d2d_kp");
        goto EXIT;
	...
	extraP.boost_value = 100; //Set fastest

	prRequest->setExtraParam(extraP);
    ...
    MY_GET_TIME(&sub_end_time, NULL);
    MY_DIFF_TIME(&temp_time, sub_start_time, sub_end_time);
    MY_ADD_TIME(elapse_time[9], temp_time);
	...
	MY_DISPLAY_TIME(elapse_time[8],"jack 2nd enque process time ...");


	MY_DISPLAY_TIME(elapse_time[9],"jack 2nd deque process time... ");
...
int main(int argc, char** argv)
{
	...
	if(!vpuCheckVersion())
	{
        MY_LOGD("Check VPU Version Fail\n");
        goto EXIT;
    }
	            ...
	            while(enter_test)
	            {
	                ...
	                switch(testcase)
	                {
	                    case 1: //single core enque test
	                        ...
	                        scanf("%u", &input);
	                        test_cnt = input;
	                        MY_LOGD("Opp(0~15, default is %d):", gVcoreOpp);
	                        scanf("%u", &input);
							gVcoreOpp = input;
							...
							//gFreqOpp = input;
							vpu_cleartime();
							MY_LOGD("VPU Test +check process time !!!");
							...
							MY_DISPLAY_TIME(elapse_time[0],"Auto test total process time ...");
	                    ...
	                    case 4:
	                    {
							...
							scanf("%u", &input);
							loop = input;
							MY_LOGD("fail and stop: (0: do not stop 1:fail and stop");
							scanf("%u", &input);
							if(input == 1)
							non_stop = true;
							else
						...
						case 7:
						{
							...
							MY_LOGD("specify test size (default=1024):");
							scanf("%u", &input);

							MY_LOGD("VPU performance test size = %d !!!:",input);

							vpu_performance_test(input);
							MY_LOGD("VPU performance test 2nd times...");
							vpu_performance_test(input);
