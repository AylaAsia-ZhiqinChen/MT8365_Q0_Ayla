...
protected:
    ...
    virtual void onHandleMessage(const Message& message) {
        RFX_UNUSED(message);
        RFX_LOG_V(RFX_LOG_TAG, "onHandleMessage() begin, this = %p", this);

        // callback to slot
        callback.invoke();

        RFX_LOG_V(RFX_LOG_TAG, "onHandleMessage() end, this = %p", this);
    }
...
TimerHandle RfxTimer::start(const RfxCallback0 &callback, nsecs_t time) {
    ...
    if (looper != NULL) {
        ...
        RFX_LOG_V(RFX_LOG_TAG, "start(), timer = %p", handler.get());
...
void RfxTimer::stop(const TimerHandle &timer_handle) {
    ...
    if (looper != NULL) {
        RFX_LOG_V(RFX_LOG_TAG, "stop(), timer = %p", timer_handle.get());
