...
RpIrMccEntry RpIrMccTable::sMccTable[] = {
    {202,"gr"},   // Greece
...
bool RpIrMccTable::isSameCountryCode(int home_mcc, int mcc) {
    RFX_LOG_D("RpIrMccTable", "isSameCountryCode, home_mcc=[%d]  mcc=[%d]", home_mcc, mcc);
    if (home_mcc <= 0 || mcc <= 0) {
        RFX_LOG_D("RpIrMccTable", "Invalid mcc");
        return false;
    }
    if (home_mcc == mcc) {
        return true;
    ...
    bool bSameCountry = false;
    RpIrMccEntry *homeItem = NULL;
    RpIrMccEntry *item = NULL;
    for (int i = 0; sMccTable[i].mcc != 0; i++) {
        if (home_mcc == sMccTable[i].mcc) {
            homeItem = &sMccTable[i];
        }
        if (mcc == sMccTable[i].mcc) {
            item = &sMccTable[i];
        }
        if (homeItem != NULL && item != NULL) {
            break;
        }
    }
    if (homeItem == NULL || item == NULL) {
        RFX_LOG_D("RpIrMccTable", "Can't find mcc item");
        return false;
    }
    bSameCountry = strcmp(homeItem->country_code, item->country_code) == 0 ? true : false;
    if (bSameCountry) {
        RFX_LOG_D("RpIrMccTable", "is same country");
        return bSameCountry;
    }

    // Special same country cases
    if ((strcmp(homeItem->country_code, "us") == 0 && strcmp(item->country_code, "vi") == 0)
            || (strcmp(homeItem->country_code, "vi") == 0 && strcmp(item->country_code, "us") == 0)) {
        bSameCountry = true;
    }
    return bSameCountry;
}
