...
void checkReturnStatus(int32_t slotId, Return<void>& ret) {
    if (ret.isOk() == false) {
        ...
        assert(ret == 0);
        mtkLogD(LOG_TAG, "radioconfig checkReturnStatus: release r lock %d", slotId);
        // acquire wrlock
        do {
        ...
        assert(ret == 0);
        mtkLogD(LOG_TAG, "radioconfig checkReturnStatus: release lock %d", slotId);
        // Reacquire rdlock
        ret = pthread_rwlock_rdlock(radioServiceRwlockPtr);
        assert(ret == 0);
        mtkLogD(LOG_TAG, "radioconfig checkReturnStatus: got r lock %d", slotId);
    }
}
...
int radioConfig::getSimSlotsStatusResponse(int slotId, android::ClientId clientId __unused,
    ...
    mtkLogE(LOG_TAG, "getSimSlotsStatusResponse: response");
...
int radioConfig::simSlotStatusChangedInd(int slotId,
    ...
    if (radioConfigService != NULL && radioConfigService ->mRadioConfigIndicationV1_2!= NULL) {
        ...
        mtkLogE(LOG_TAG, "simSlotStatusChangedInd: radioConfigService[%d] or mRadioConfigIndication is NULL",
                slotId);
    }
    return 0;
}
