...
class SoftGateKeeperDevice : public GateKeeper {
...
public:
    ...
    virtual uint64_t GetMillisecondsSinceBoot() const {
        ...
        if (res < 0) return 0;
        return ((uint64_t)time.tv_sec * 1000) + (time.tv_nsec / 1000 / 1000);
    }
    ...
    fast_hash_t ComputeFastHash(const SizedBuffer &password, uint64_t salt) {
        ...
        psd_len = password.length;
#else
        psd_len = password.size();
#endif
		    digest_size = psd_len + sizeof(salt);
		    std::unique_ptr<uint8_t[]> digest(new uint8_t[digest_size]);
        memcpy(digest.get(), &salt, sizeof(salt));
#if !(PLATFORM_VERSION == R)
        memcpy(digest.get() + sizeof(salt), password.buffer.get(), psd_len);
#else
        memcpy(digest.get() + sizeof(salt), (void*)password.Data<uint8_t>(), psd_len);
#endif
