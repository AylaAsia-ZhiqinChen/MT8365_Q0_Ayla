...
MTKM4UDrv::MTKM4UDrv(void)
{
    ...
    {
        if(ioctl( mFileDescriptor , MTK_M4U_T_M4UDrv_CONSTRUCT, NULL) < 0)
        {
...
{    
    ...
    {
        if(ioctl(mFileDescriptor, MTK_M4U_T_M4UDrv_DECONSTRUCT, NULL) < 0)
        {
								  ...
								  unsigned int *pRetMVAAddr)
{
    ...
    if(!m4u_check_m4u_en(eModuleID))
    {
        ...
        {
            M4UINFO(" ioctl MTK_M4U_T_ALLOC_MVA pmem! fd=%d, VA:0x%x, MVA:0x%x \n", mFileDescriptor, m4u_module.BufAddr, m4u_module.MVAStart);
    ...
    else
    {
        ...
        {
            M4UINFO(" ioctl MTK_M4U_T_ALLOC_MVA! fd=%d, VA:0x%x, MVA:0x%x \n", mFileDescriptor, m4u_module.BufAddr, m4u_module.MVAStart);        
            *pRetMVAAddr = m4u_module.MVAStart;
            return M4U_STATUS_OK;
        }
    }
                                    ...
                                    const unsigned int MVAStart)
{
    ...
    if(ioctl(mFileDescriptor, MTK_M4U_T_DEALLOC_MVA, &m4u_module) < 0)
    {
		...
		M4UMSG("ioctl MTK_M4U_T_DEALLOC_MVA fail:bufAddr = 0x%lx,bufsize = 0x%x,mva = 0x%x,eModuleID = %d\n ", BufAddr, BufSize, MVAStart, eModuleID);
        if(munlock((void*)BufAddr, BufSize)) {
            M4UMSG("munlock fail! %d, %s \n", errno, strerror(errno));
        }
        return M4U_STATUS_KERNEL_FAULT;
    }
    else
    {
    	M4UINFO("ioctl MTK_M4U_T_DEALLOC_MVA fail:bufAddr = 0x%lx,bufsize = 0x%x,mva = 0x%x,eModuleID = %d\n ", BufAddr, BufSize, MVAStart, eModuleID);
        if(munlock((void*)BufAddr, BufSize)) {
            M4UMSG("munlock fail! %d, %s \n", errno, strerror(errno));
        }
								  ...
								  const unsigned int MVAEnd)
{
    ...
    if(ioctl(mFileDescriptor, MTK_M4U_T_INSERT_WRAP_RANGE, &m4u_wrap) < 0)
    {
...
{
    return (M4U_MODULE_ID_ENUM)portID;    
}
#endif
