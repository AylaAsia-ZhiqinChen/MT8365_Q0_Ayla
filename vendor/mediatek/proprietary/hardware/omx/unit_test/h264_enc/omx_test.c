 ...
 int ConfigOmxComponentPorts(pOMXCoreIfaceTable pOMXCoreIfaceTableInst)
{
     ...
     printf("ConfigOmxComponentPorts++\n");
     if(NULL == pOMXCoreIfaceTableInst){
         iRetValue = -1;
         goto GOOUT;
     }
    ...
    video_def->nStride = pOMXCoreIfaceTableInst->width;
    video_def->nSliceHeight = pOMXCoreIfaceTableInst->height;
    ...
    #endif
    //set prependSPSPPSToIDR
    OMX_INDEXTYPE index;
    err = OMX_GetExtensionIndex(pOMXCoreIfaceTableInst->CompHandle,"OMX.google.android.index.prependSPSPPSToIDRFrames", &index);
    if (OMX_ErrorNone != err) {
        UT_LOG_E(printf("Fail to OMX_GetExtensionIndex (0x%08X)\r\n", err);)
        iRetValue = -110;
        goto GOOUT;
    }
    PrependSPSPPSToIDRFramesParams params;
    memset(&params, 0, sizeof(PrependSPSPPSToIDRFramesParams));
    params.nSize = sizeof(PrependSPSPPSToIDRFramesParams);
    params.nVersion.s.nVersionMajor = 1;
    params.nVersion.s.nVersionMinor = 0;
    params.nVersion.s.nRevision = 0;
    params.nVersion.s.nStep = 0;
    params.bEnable = OMX_TRUE;
    err = OMX_SetParameter(pOMXCoreIfaceTableInst->CompHandle, index, &params);
    if (OMX_ErrorNone != err) {
        UT_LOG_E(printf("Fail to OMX_SetParameter F error (0x%08X)\r\n", err);)
        iRetValue = -129;
        goto GOOUT;
    }
    #if 1
    ...
    for (i = 0 ; i < H264_NUM_INPUT_BUF ; i++) {
        OMX_AllocateBuffer(pOMXCoreIfaceTableInst->CompHandle, &pOMXCoreIfaceTableInst->pInBufHdrs[i], OMX_DirInput, NULL, inBufSize);
        //set owner to IL client
        pOMXCoreIfaceTableInst->InBufOwner[i] = BOS_IL_CLIENT;
        printf("pOMXCoreIfaceTableInst->pInBufHdrs[%d] = %p,inBufSize is %d\n", i, pOMXCoreIfaceTableInst->pInBufHdrs[i],inBufSize);
    }
    // allocate output buffers and provide to OMX
    int outBufSize = MTK_VENC_DEFAULT_OUTPUT_BUFFER_SIZE_AVC;
    if(pOMXCoreIfaceTableInst->width * pOMXCoreIfaceTableInst->height > HD_AREA)
    {
        outBufSize = MTK_VENC_DEFAULT_OUTPUT_BUFFER_SIZE_AVC_1080P;
    }

    for (i = 0 ; i < H264_NUM_OUTPUT_BUF ; i++) {
        OMX_AllocateBuffer(pOMXCoreIfaceTableInst->CompHandle, &pOMXCoreIfaceTableInst->pOutBufHdrs[i], OMX_DirOutput, NULL, outBufSize);
        //set owner to IL client
        pOMXCoreIfaceTableInst->OutBufOwner[i] = BOS_IL_CLIENT;
        printf("pOMXCoreIfaceTableInst->pOutBufHdrs[%d] = %p,outBufSize is %d\n", i, pOMXCoreIfaceTableInst->pOutBufHdrs[i],outBufSize);
    }
    printf("ConfigOmxComponentPorts--\n");
    return 1;
#elif
