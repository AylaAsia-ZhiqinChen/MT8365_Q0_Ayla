...
DP_STATUS_ENUM DpEngineBase::configFrameSout(DpConfig &config)
{
    const uint32_t id = m_identifier;

	if (cfg.enable_MDP_WROT[0])
    {
		// Prepare port config
	...
	if (cfg.enable_MDP_WDMA[0])
	{
		// Prepare port config
		config.inWidth   = cfg.MDP_WDMA_src_w[0];
		config.inHeight  = cfg.MDP_WDMA_src_h[0];
		config.outWidth  = cfg.MDP_WDMA_src_w[0];
		config.outHeight = cfg.MDP_WDMA_src_h[0];
	}
...
DP_STATUS_ENUM DpEngineBase::configFrameWrite(DpCommand &command, DpConfig &config)
{
    ...
    if (feature & eROT) // WROT
    {
        ...
        {
            transferred_cfg.WROT1_width[0] = width;
            transferred_cfg.WROT1_width[1] = (plane_num > 1) ? width : 0;
            transferred_cfg.WROT1_width[2] = (plane_num > 2) ? width : 0;
            transferred_cfg.WROT1_height[0] = height;
            transferred_cfg.WROT1_height[1] = (plane_num > 1) ? height_uv : 0;
            transferred_cfg.WROT1_height[2] = (plane_num > 2) ? height_uv : 0;
            transferred_cfg.WROT1_pitch[0] = pitch_y;
            transferred_cfg.WROT1_pitch[1] = (plane_num > 1) ? pitch_uv : 0;
            transferred_cfg.WROT1_pitch[2] = (plane_num > 2) ? pitch_uv : 0;
            transferred_cfg.WROT1_plane = plane_num;
#ifdef RING_BUFFER
            if (transferred_cfg.RDMA0_ringbuffermode)
                transferred_cfg.WROT1_invalid = 1;
#endif
        }
