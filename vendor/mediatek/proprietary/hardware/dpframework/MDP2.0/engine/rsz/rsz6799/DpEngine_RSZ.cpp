...
#if ENABLE_PQ_RSZ
    void onCalcRsz(DpCommand &command, PQSession *pPQSession, DpConfig &config);
    void initInParam(RszInput *inParam, DpPqParam *pqParam, DpConfig &config);
#endif // ENABLE_PQ_RSZ
};
...
#ifdef tSCL3
static DpEngineBase* SCL3Factory(DpEngineType type)
{
    if (tSCL3 == type)
    {
        return new DpEngine_SCL(3);
    }
    return NULL;
};
#endif
                                           ...
                                           DpConfig  &config)
{
    ...
    onCalcRsz(command, pPQSession, config);
...
#if ENABLE_PQ_RSZ
void DpEngine_SCL::onCalcRsz(DpCommand &command, PQSession *pPQSession, DpConfig &config)
{
    int32_t tmp, q = 0;
    RszInput inParam;
    RszOutput outParam;
    ...
    initInParam(&inParam, &pqParam, config);
...
/*
    ...
    int  cnt = 0;
    cnt += sprintf(buffer + cnt, "____mdp_section_start____\n");
    if (m_identifier == 0)
        cnt += sprintf(buffer + cnt, "Rsz0_hw:\n");
    else if (m_identifier == 1)
        cnt += sprintf(buffer + cnt, "Rsz1_hw:\n");
    else
        DPLOGE("DpEngine_SCL::onCalcRsz Exception: m_identifier = %d", m_identifier);

    if (true == m_bypassEngine)
    {
        cnt += sprintf(buffer + cnt, "rsz_en = 0\n");
    }
    else
    {
        cnt += sprintf(buffer + cnt, "rsz_en = 1\n");
    }
*/
    MM_REG_WRITE(command, PRZ_CONTROL_1, (true == m_use121filter ? 1 : 0) << 26, 0x04000000);
/*
    cnt += sprintf(buffer + cnt, "rsz_c42_interp_en = %d\n", true == m_use121filter ? 1 : 0);

    cnt += sprintf(buffer + cnt, "____mdp_section_end____\n");
    ...
    if (pqParam.u.isp.p_mdpSetting != NULL && pqParam.u.isp.p_mdpSetting->buffer != NULL)
    {
        ...
        {
            DPLOGE("DpEngine_SCL::onCalcRsz mdpSetting buffer overflow\n");
            delete [] buffer;
...
void DpEngine_SCL::initInParam(RszInput *inParam, DpPqParam *pqParam, DpConfig &config)
{
