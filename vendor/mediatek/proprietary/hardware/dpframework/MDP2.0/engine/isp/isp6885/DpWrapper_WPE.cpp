...
DP_STATUS_ENUM DpWrapper_WPE::onInitEngine(DpCommand &command)
{
    ...
    // Direct link
    if (m_ISPConfig.wpe.top_wpe.wpe_mdpcrop_en)
    {
...
DP_STATUS_ENUM DpWrapper_WPE::onWaitEvent(DpCommand &command)
{
    ...
    if (m_ISPConfig.wpe.top_wpe.wpe_mdpcrop_en == 1)
    {
        DPLOGI("[DpWrapper_WPE::onWaitEvent]Enable WPE direct link,\n");
        //Enable WPE direct link
        MM_REG_WRITE_MASK(command, 0x1502004C, 0x00000400, 0x00000C00); // MDP2_DL_SEL: select wpe_a

    }
    DPLOGI("DpWrapper_WPE: onWaitEvent\n");

    MM_REG_WRITE_MASK(command, 0x15011000, 0x0001, 0x00000001);
...
DP_STATUS_ENUM DpWrapper_WPE::onFlushBuffer(FLUSH_TYPE_ENUM type)
{
    ...
    fwrite(m_pTDRIBufVA, 1, m_pTileDesc->used_word_no_wpe_s*4, pFile); //Holmes : make the correct size=>uint word=>4 bytes
    fclose(pFile);
    ...
    {
        tmpOffset = (m_pTileDesc->used_tpipe_no_wpe_s * i) << 2;
                                           ...
                                           DpConfig  &config)
{
    ...
    m_pDLTDRIBufVA = m_ISPConfig.drvinfo.dl_tpipeTableVa_wpe;
#endif // CONFIG_FOR_VERIFY_FPGA
    assert(0 != m_pTDRIBufVA);

    // Initialize hex dump info
    m_pTileDesc->tpipe_config_wpe_s       = m_pTDRIBufVA;
    m_pTileDesc->tpipe_config_wpe         = m_pDLTDRIBufVA;
...
#if !CONFIG_FOR_VERIFY_FPGA
    if (m_inFrameWidth != m_ISPConfig.wpe.wpe.vgen_input_width)
    {
        DPLOGI("Change VECI input width from %d to %d\n",m_inFrameWidth,m_ISPConfig.wpe.wpe.vgen_input_width);
        m_inFrameWidth = m_ISPConfig.wpe.wpe.vgen_input_width;
    }

    if (m_inFrameHeight != m_ISPConfig.wpe.wpe.vgen_input_height)
    {
        DPLOGI("Change VECI input height from %d to %d\n",m_inFrameHeight, m_ISPConfig.wpe.wpe.vgen_input_height);
        m_inFrameHeight = m_ISPConfig.wpe.wpe.vgen_input_height;
    }

    if (0 == m_ISPConfig.wpe.top_wpe.wpe_mdpcrop_en)
    {
        DPLOGI("Change VECI output width/height from WPE config w:%d h:%d\n",m_ISPConfig.wpe.wpe.vgen_output_width, m_ISPConfig.wpe.wpe.vgen_output_height);
        m_outFrameWidth = m_ISPConfig.wpe.wpe.vgen_output_width;
        m_outFrameHeight = m_ISPConfig.wpe.wpe.vgen_output_height;
    }
    ...
    if (m_ISPConfig.wpe.top_wpe.wpe_ispcrop_en == 1)
    {
    ...
    MM_REG_WRITE_MASK(command, WPE_PSP_BORDER_HW, wpecmd->WPE_PSP_BORDER_COLOR, 0xFFFFFFFF);
   ...
   // MM_REG_WRITE_MASK(command, WPE_WPEO_CON3_HW, wpecmd->WPE_WPEO_CON3, 0xFFFFFFFF);
    ...
    MM_REG_WRITE_MASK(command, WPE_VEC2I_XSIZE_HW, wpecmd->WPE_VECI_XSIZE, 0xFFFFFFFF);
    MM_REG_WRITE_MASK(command, WPE_VEC2I_YSIZE_HW, wpecmd->WPE_VECI_YSIZE, 0xFFFFFFFF);
    MM_REG_WRITE_MASK(command, WPE_VEC2I_STRIDE_HW, wpecmd->WPE_VECI_STRIDE, 0xFFFFFFFF);
...
DP_STATUS_ENUM DpWrapper_WPE::initTilePath(TILE_PARAM_STRUCT *p_tile_param, WPE_TPIPE_CONFIG_STRUCT *p_wpe_param)
{
    ...
    ptr_tile_reg_map->last_irq_mode = m_ISPConfig.wpe.sw_wpe.tpipe_irq_mode; // 0 FOR WPE ONLY,2 FOR MDP directtlink
    ptr_tile_reg_map->isp_tile_width  = m_ISPConfig.wpe.sw_wpe.tpipe_width_wpe;
    ptr_tile_reg_map->isp_tile_height = m_ISPConfig.wpe.sw_wpe.tpipe_height_wpe;
    ...
    if (NULL != p_wpe_param)
    {
        memcpy(p_wpe_param, &m_ISPConfig.wpe, sizeof(WPE_TPIPE_CONFIG_STRUCT));
    }
    else
    {
        DPLOGE("DpWrapper_WPE::initTilePath p_wpe_param = NULL\n");
    }
...
#if !CONFIG_FOR_VERIFY_FPGA
    if (m_onlyWPE && m_ISPConfig.wpe.top_wpe.wpe_mdpcrop_en)
    {
...
#endif
*/
    tpipePa = m_TDRIBufPA + m_TDRITileID * m_pTileDesc->config_no_per_tpipe_wpe_s * 4;

    DPLOGI("[onConfigTile]+,m_TDRITileID(%d), config_no_per_tpipe_wpe_s(%d), tpipePa(0x%08x), m_pTDRIBufVA (0x%p)\n",
            m_TDRITileID, m_pTileDesc->config_no_per_tpipe_wpe_s, tpipePa, m_pTDRIBufVA);
    ...
    MM_REG_WRITE_MASK(command, 0x150113C4, tpipePa, 0xFFFFFFFF, &m_TDRITileLabel[m_TDRITileID]);
...
DP_STATUS_ENUM DpWrapper_WPE::onReconfigTiles(DpCommand &command)
{
    ...
    for (i = 0; i < m_pTileDesc->used_tpipe_no_wpe_s; i++) //For WPE MDP DL
    {
        tpipePa = m_TDRIBufPA + m_pTileDesc->used_word_no_wpe_s * i *sizeof(uint32_t);

        DPLOGI("[reConfigTile]+,used_tpipe_no_wpe(%d), used_word_no_wpe(%d), tpipePa(0x%08x)", m_pTileDesc->used_tpipe_no_wpe_s, m_pTileDesc->used_word_no_wpe_s, tpipePa);

        MM_REG_WRITE_MASK(command, 0x150113C4, tpipePa, 0xFFFFFFFF, NULL, m_TDRITileLabel[i]);
    }

    return DP_STATUS_RETURN_SUCCESS;
}

DP_STATUS_ENUM DpWrapper_WPE::onUpdatePMQOS(DpCommand &command, uint32_t& totalPixel, uint32_t& totalDataSzie, int32_t& engineType, struct timeval& endtime)
{
    DP_UNUSED(command);
