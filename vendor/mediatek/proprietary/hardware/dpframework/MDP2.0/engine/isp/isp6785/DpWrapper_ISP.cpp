...
{
    if (m_ISPConfig.top.mdp_crop_en)
        MM_REG_WRITE_MASK(command, 0x15020030, 0x00000000, 0x00000200); // MDP_DL_SEL: select MDP_CROP
    if (m_ISPConfig.top.mdp_crop2_en)
        MM_REG_WRITE_MASK(command, 0x15020030, 0x00000000, 0x00000C00); // MDP2_DL_SEL: select MDP_CROP2
                                           ...
                                           DpConfig  &config)
{
    ...
    if (m_ISPConfig.drvinfo.dip_secure_tag&0x1) // For secure camera path
    {
        ...
        flag = (uint64_t)((1LL<<CMDQ_ENG_ISP_IMGI) | (1LL<<CMDQ_ENG_ISP_LCEI)); // For CQ DMA
        switch (m_ISPConfig.drvinfo.cqIdx) {
            case ISP_DRV_DIP_CQ_THRE0:
...
DP_STATUS_ENUM DpWrapper_ISP::onConfigTile(DpCommand &command)
{
    ...
    uint32_t       memSize[3] = {0};
    uint32_t       planeOffset[3] = {0};
    uint64_t       flag = (uint64_t)(1LL << CMDQ_ENG_ISP_IMGI);
...
DP_STATUS_ENUM DpWrapper_ISP::onUpdatePMQOS(DpCommand &command, uint32_t& totalPixel, uint32_t& totalDataSzie, int32_t& engineFlag, struct timeval& endtime)
{
    DP_UNUSED(command);
