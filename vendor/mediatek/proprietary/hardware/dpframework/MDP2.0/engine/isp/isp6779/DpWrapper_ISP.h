...
public:
          ...
          m_ispPmqosUpdIdx(0)
    {
        ...
        char *buf_mfb = (char*)calloc(1, (MAX_TILE_TOT_NO * sizeof(ISP_TPIPE_INFORMATION_STRUCT)) + // tpipe_info
                                     (((MAX_TILE_TOT_NO + 31) >> 5) * sizeof(unsigned int)) +   // tdr_disable_flag
                                     (((MAX_TILE_TOT_NO + 31) >> 5) * sizeof(unsigned int)));   // last_irq_flag
        assert(NULL != buf_mfb);
        m_pTileInfo_MFB = (ISP_TPIPE_INFORMATION_STRUCT*)(buf_mfb);
...
private:
    ...
    uint32_t                     m_TDRIBufPA_MFB;
    uint32_t                     *m_pTDRIBufVA_MFB;
    ...
    ISP_TPIPE_INFORMATION_STRUCT *m_pTileInfo_MFB;
    uint32_t                     *m_pTdrFlag;
    uint32_t                     *m_pIrqFlag;
    ...
    int32_t                      m_ispPmqosUpdIdx;
    ISP2MDP_STRUCT               m_pIsp2Mdp;

    //pmqos
    std::map<uint32_t, uint32_t>      m_IspPmqosData;
    ...
    DP_STATUS_ENUM onTilePostProc(DpCommand &);
