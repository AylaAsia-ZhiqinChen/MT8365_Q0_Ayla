...
#if 0 //TODO, after mdp add engine flags
	if (m_ISPConfig.drvinfo.dip_secure_tag&0x1) // For secure camera path
	{
        //reset secure bit/domain
        MM_REG_WRITE_MASK(command, 0x150210c0, 0x00000000, 0xFFFFFFFF, &m_frameConfigLabel[index++]);
        MM_REG_WRITE_MASK(command, 0x150210c4, 0x00000000, 0xFFFFFFFF, &m_frameConfigLabel[index++]);
        MM_REG_WRITE_MASK(command, 0x150210c8, 0x00000000, 0xFFFFFFFF, &m_frameConfigLabel[index++]);
        MM_REG_WRITE_MASK(command, 0x150210cc, 0x00000000, 0xFFFFFFFF, &m_frameConfigLabel[index++]);
        MM_REG_WRITE_MASK(command, 0x150210d0, 0x00000000, 0xFFFFFFFF, &m_frameConfigLabel[index++]);
        MM_REG_WRITE_MASK(command, 0x150210d4, 0x00000000, 0xFFFFFFFF, &m_frameConfigLabel[index++]);
        MM_REG_WRITE_MASK(command, 0x150210d8, 0x00000000, 0xFFFFFFFF, &m_frameConfigLabel[index++]);
        MM_REG_WRITE_MASK(command, 0x150210dc, 0x00000000, 0xFFFFFFFF, &m_frameConfigLabel[index++]);
        MM_REG_WRITE_MASK(command, 0x150210e0, 0x00000000, 0xFFFFFFFF, &m_frameConfigLabel[index++]);
        MM_REG_WRITE_MASK(command, 0x150210e4, 0x00000000, 0xFFFFFFFF, &m_frameConfigLabel[index++]);
        MM_REG_WRITE_MASK(command, 0x150210e8, 0x00000000, 0xFFFFFFFF, &m_frameConfigLabel[index++]);
        MM_REG_WRITE_MASK(command, 0x150210ec, 0x00000000, 0xFFFFFFFF, &m_frameConfigLabel[index++]);
        MM_REG_WRITE_MASK(command, 0x150210f0, 0x00000000, 0xFFFFFFFF, &m_frameConfigLabel[index++]);
        MM_REG_WRITE_MASK(command, 0x150210f4, 0x00000000, 0xFFFFFFFF, &m_frameConfigLabel[index++]);

		if (m_ISPConfig.drvinfo.dip_img2o_secure_tag != 0) // IMG2O, CRZO
		{
			secureRegAddr[0] = 0x150216B0;
			secureRegAddr[1] = 0x15021720;
			secureRegAddr[2] = 0;
			planeOffset[0] = 0;
            ...
            //dma secure bit/domain, crzo+crzbo (domain is fixed at 4 as multimedia)
            MM_REG_WRITE_MASK(command, 0x150210f0, 0x00002424, 0x0000FFFF, &m_frameConfigLabel[index++]);
		}
		if (m_ISPConfig.drvinfo.dip_img3o_secure_tag != 0) // IMG3O
		{
			secureRegAddr[0] = 0x15021890;
			secureRegAddr[1] = 0x15021900;
			secureRegAddr[2] = 0x15021970;
			planeOffset[0] = 0;
            ...
            //dma secure bit/domain, img3o+img3bo+img3co (domain is fixed at 4 as multimedia)
            MM_REG_WRITE_MASK(command, 0x150210e8, 0x00242424, 0x00FFFFFF, &m_frameConfigLabel[index++]);
		}
		if (m_ISPConfig.drvinfo.dip_feo_secure_tag != 0) // FEO
		{
            ...
            //dma secure bit/domain(domain is fixed at 4 as multimedia)
            MM_REG_WRITE_MASK(command, 0x150210e8, 0x00240000, 0x00FF0000, &m_frameConfigLabel[index++]);
		}
		if (m_ISPConfig.drvinfo.dip_dceso_secure_tag != 0) // DCESO
		{
			secureRegAddr[0] = 0x15021500;
			secureRegAddr[1] = 0;
			secureRegAddr[2] = 0;
			planeOffset[0] = 0;
			planeOffset[1] = 0;
            ...
            //dma secure bit/domain(domain is fixed at 4 as multimedia)
            MM_REG_WRITE_MASK(command, 0x150210e4, 0x00002400, 0x0000FF00, &m_frameConfigLabel[index++]);
		}
		if (m_ISPConfig.drvinfo.dip_timgo_secure_tag != 0) // TIMGO
		{
			secureRegAddr[0] = 0x15021360;
			secureRegAddr[1] = 0;
			secureRegAddr[2] = 0;
			planeOffset[0] = 0;
			planeOffset[1] = 0;
            ...
            //dma secure bit/domain(domain is fixed at 4 as multimedia)
            MM_REG_WRITE_MASK(command, 0x150210e4, 0x00000024, 0x000000FF, &m_frameConfigLabel[index++]);
		}
		if (m_ISPConfig.drvinfo.dip_imgi_secure_tag != 0) // IMGI (Separate secure setting for each plane due to BPCI and LSCI)
		{
            ...
            //dma secure bit/domain(domain is fixed at 4 as multimedia)
            MM_REG_WRITE_MASK(command, 0x150210c0, 0x00000024, 0x000000FF, &m_frameConfigLabel[index++]);
 
            if(m_ISPConfig.drvinfo.dip_imgbi_secure_tag != 0)
			{    //second frame of imgi, store offset(byte size) in imgbi_base_addr in isp_function
                ...
                //dma secure bit/domain(domain is fixed at 4 as multimedia)
                MM_REG_WRITE_MASK(command, 0x150210d4, 0x00000024, 0x000000FF, &m_frameConfigLabel[index++]);
			}
			if(m_ISPConfig.drvinfo.dip_imgci_secure_tag != 0)
			{    //third frame of imgi, store offset(byte size) in imgci_base_addr in isp_function
				secureRegAddr[0] = 0x15021330;
				secureRegAddr[1] = 0;
				secureRegAddr[2] = 0;
                ...
                //dma secure bit/domain(domain is fixed at 4 as multimedia)
                MM_REG_WRITE_MASK(command, 0x150210d4, 0x00002400, 0x0000FF00, &m_frameConfigLabel[index++]);
			}
		}
		if (m_ISPConfig.drvinfo.dip_vipi_secure_tag != 0) // VIPI
		{
			secureRegAddr[0] = 0x15021800;
			secureRegAddr[1] = 0x15021830;
			secureRegAddr[2] = 0x15021860;
			planeOffset[0] = 0;
            ...
            //dma secure bit/domain, vipi+vipbi+vipci (domain is fixed at 4 as multimedia)
            MM_REG_WRITE_MASK(command, 0x150210cc, 0x00242424, 0x00FFFFFF, &m_frameConfigLabel[index++]);
		}
		if (m_ISPConfig.drvinfo.dip_dmgi_secure_tag != 0) // DMGI
		{
            ...
            //dma secure bit/domain(domain is fixed at 4 as multimedia)
            MM_REG_WRITE_MASK(command, 0x150210d0, 0x00000024, 0x000000FF, &m_frameConfigLabel[index++]);
		}
		if (m_ISPConfig.drvinfo.dip_depi_secure_tag != 0) // DEPI
		{
            ...
            //dma secure bit/domain(domain is fixed at 4 as multimedia)
            MM_REG_WRITE_MASK(command, 0x150210d8, 0x00000024, 0x000000FF, &m_frameConfigLabel[index++]);
		}
		if (m_ISPConfig.drvinfo.dip_lcei_secure_tag != 0) // LCEI
		{
			secureRegAddr[0] = 0x150214D0;
			secureRegAddr[1] = 0;
			secureRegAddr[2] = 0;
			planeOffset[0] = 0;
			planeOffset[1] = 0;
            ...
            //dma secure bit/domain(domain is fixed at 4 as multimedia)
            MM_REG_WRITE_MASK(command, 0x150210c4, 0x00000024, 0x000000FF, &m_frameConfigLabel[index++]);
		}
		if (m_ISPConfig.drvinfo.dip_ufdi_secure_tag != 0) // UFDI
		{
            ...
            //dma secure bit/domain(domain is fixed at 4 as multimedia)
            MM_REG_WRITE_MASK(command, 0x150210d8, 0x00002400, 0x0000FF00, &m_frameConfigLabel[index++]);
		}
		...
		command.secure(CMDQ_ENG_ISP_SMXIO, flag, secureRegAddr, DP_SECURE, m_ISPConfig.drvinfo.smxSecHdl, offset, memSize, planeOffset);
		// SMX5
        secureRegAddr[0] = 0x150219E0;
        planeOffset[0] = m_ISPConfig.drvinfo.smx5iOft;
        memSize[0] = m_ISPConfig.drvinfo.smxSecSize - m_ISPConfig.drvinfo.smx5iOft;
        command.secure(CMDQ_ENG_ISP_SMXIO, flag, secureRegAddr, DP_SECURE, m_ISPConfig.drvinfo.smxSecHdl, offset, memSize, planeOffset);
        secureRegAddr[0] = 0x15021A10;
        command.secure(CMDQ_ENG_ISP_SMXIO, flag, secureRegAddr, DP_SECURE, m_ISPConfig.drvinfo.smxSecHdl, offset, memSize, planeOffset);
        // SMX6
        secureRegAddr[0] = 0x15021A80;
        planeOffset[0] = m_ISPConfig.drvinfo.smx6iOft;
        memSize[0] = m_ISPConfig.drvinfo.smxSecSize - m_ISPConfig.drvinfo.smx6iOft;
        command.secure(CMDQ_ENG_ISP_SMXIO, flag, secureRegAddr, DP_SECURE, m_ISPConfig.drvinfo.smxSecHdl, offset, memSize, planeOffset);
        secureRegAddr[0] = 0x15021AB0;
		command.secure(CMDQ_ENG_ISP_SMXIO, flag, secureRegAddr, DP_SECURE, m_ISPConfig.drvinfo.smxSecHdl, offset, memSize, planeOffset);
        //dma secure bit/domain(domain is fixed at 4 as multimedia)
        //smti_d1 ~ smti_d4
        MM_REG_WRITE_MASK(command, 0x150210c8, 0x24242424, 0xFFFFFFFF, &m_frameConfigLabel[index++]);
        //smti_d5 ~ smti_d6
        MM_REG_WRITE_MASK(command, 0x150210dc, 0x00002424, 0x0000FFFF, &m_frameConfigLabel[index++]);
        //smto_d2 ~ smto_d5
        MM_REG_WRITE_MASK(command, 0x150210e0, 0x24242424, 0xFFFFFFFF, &m_frameConfigLabel[index++]);
        //smto_d1,smto_d6
        MM_REG_WRITE_MASK(command, 0x150210ec, 0x00002424, 0x0000FFFF, &m_frameConfigLabel[index++]);
...
//				command.secure(CMDQ_ENG_ISP_DMGI_DEPI, flag, secureRegAddr, DP_SECURE, m_ISPConfig.drvinfo.cqSecHdl, offset, memSize, planeOffset);
        ...
        //cq, tdr
        MM_REG_WRITE_MASK(command, 0x150210f4, 0x00242424, 0x00FFFFFF, &m_frameConfigLabel[index++]);
	}
#endif
                                           ...
                                           DpConfig  &config)
{
    ...
    {
        //reset secure bit/domain
        MM_REG_WRITE_MASK(command, 0x150210c0, 0x00000000, 0xFFFFFFFF, NULL, m_frameConfigLabel[index++]);
        MM_REG_WRITE_MASK(command, 0x150210c4, 0x00000000, 0xFFFFFFFF, NULL, m_frameConfigLabel[index++]);
        MM_REG_WRITE_MASK(command, 0x150210c8, 0x00000000, 0xFFFFFFFF, NULL, m_frameConfigLabel[index++]);
        MM_REG_WRITE_MASK(command, 0x150210cc, 0x00000000, 0xFFFFFFFF, NULL, m_frameConfigLabel[index++]);
        MM_REG_WRITE_MASK(command, 0x150210d0, 0x00000000, 0xFFFFFFFF, NULL, m_frameConfigLabel[index++]);
        MM_REG_WRITE_MASK(command, 0x150210d4, 0x00000000, 0xFFFFFFFF, NULL, m_frameConfigLabel[index++]);
        MM_REG_WRITE_MASK(command, 0x150210d8, 0x00000000, 0xFFFFFFFF, NULL, m_frameConfigLabel[index++]);
        MM_REG_WRITE_MASK(command, 0x150210dc, 0x00000000, 0xFFFFFFFF, NULL, m_frameConfigLabel[index++]);
        MM_REG_WRITE_MASK(command, 0x150210e0, 0x00000000, 0xFFFFFFFF, NULL, m_frameConfigLabel[index++]);
        MM_REG_WRITE_MASK(command, 0x150210e4, 0x00000000, 0xFFFFFFFF, NULL, m_frameConfigLabel[index++]);
        MM_REG_WRITE_MASK(command, 0x150210e8, 0x00000000, 0xFFFFFFFF, NULL, m_frameConfigLabel[index++]);
        MM_REG_WRITE_MASK(command, 0x150210ec, 0x00000000, 0xFFFFFFFF, NULL, m_frameConfigLabel[index++]);
        MM_REG_WRITE_MASK(command, 0x150210f0, 0x00000000, 0xFFFFFFFF, NULL, m_frameConfigLabel[index++]);
        MM_REG_WRITE_MASK(command, 0x150210f4, 0x00000000, 0xFFFFFFFF, NULL, m_frameConfigLabel[index++]);

        if (m_ISPConfig.drvinfo.dip_img2o_secure_tag != 0) // IMG2O, CRZO
        {
            //dma secure bit/domain, crzo+crzbo (domain is fixed at 4 as multimedia)
            MM_REG_WRITE_MASK(command, 0x150210f0, 0x00002424, 0x0000FFFF, NULL, m_frameConfigLabel[index++]);
        }
        if (m_ISPConfig.drvinfo.dip_img3o_secure_tag != 0) // IMG3O
        {
            //dma secure bit/domain, img3o+img3bo+img3co (domain is fixed at 4 as multimedia)
            MM_REG_WRITE_MASK(command, 0x150210e8, 0x00242424, 0x00FFFFFF, NULL, m_frameConfigLabel[index++]);
        }
        if (m_ISPConfig.drvinfo.dip_feo_secure_tag != 0) // FEO
        {
            //dma secure bit/domain(domain is fixed at 4 as multimedia)
            MM_REG_WRITE_MASK(command, 0x150210e8, 0x00240000, 0x00FF0000, NULL, m_frameConfigLabel[index++]);
        }
        if (m_ISPConfig.drvinfo.dip_dceso_secure_tag != 0) // DCESO
        {
            //dma secure bit/domain(domain is fixed at 4 as multimedia)
            MM_REG_WRITE_MASK(command, 0x150210e4, 0x00002400, 0x0000FF00, NULL, m_frameConfigLabel[index++]);
        }
        if (m_ISPConfig.drvinfo.dip_timgo_secure_tag != 0) // TIMGO
        {
            //dma secure bit/domain(domain is fixed at 4 as multimedia)
            MM_REG_WRITE_MASK(command, 0x150210e4, 0x00000024, 0x000000FF, NULL, m_frameConfigLabel[index++]);
        }
        if (m_ISPConfig.drvinfo.dip_imgi_secure_tag != 0) // IMGI (Separate secure setting for each plane due to BPCI and LSCI)
        {
            //dma secure bit/domain(domain is fixed at 4 as multimedia)
            MM_REG_WRITE_MASK(command, 0x150210c0, 0x00000024, 0x000000FF, NULL, m_frameConfigLabel[index++]);
 
            if(m_ISPConfig.drvinfo.dip_imgbi_secure_tag != 0)
            {    //second frame of imgi, store offset(byte size) in imgbi_base_addr in isp_function
                //dma secure bit/domain(domain is fixed at 4 as multimedia)
                MM_REG_WRITE_MASK(command, 0x150210d4, 0x00000024, 0x000000FF, NULL, m_frameConfigLabel[index++]);
            }
            if(m_ISPConfig.drvinfo.dip_imgci_secure_tag != 0)
            {    //third frame of imgi, store offset(byte size) in imgci_base_addr in isp_function
                //dma secure bit/domain(domain is fixed at 4 as multimedia)
                MM_REG_WRITE_MASK(command, 0x150210d4, 0x00002400, 0x0000FF00, NULL, m_frameConfigLabel[index++]);
            }
        }
        if (m_ISPConfig.drvinfo.dip_vipi_secure_tag != 0) // VIPI
        {
            //dma secure bit/domain, vipi+vipbi+vipci (domain is fixed at 4 as multimedia)
            MM_REG_WRITE_MASK(command, 0x150210cc, 0x00242424, 0x00FFFFFF, NULL, m_frameConfigLabel[index++]);
        }
        if (m_ISPConfig.drvinfo.dip_dmgi_secure_tag != 0) // DMGI
        {
            //dma secure bit/domain(domain is fixed at 4 as multimedia)
            MM_REG_WRITE_MASK(command, 0x150210d0, 0x00000024, 0x000000FF, NULL, m_frameConfigLabel[index++]);
        }
        if (m_ISPConfig.drvinfo.dip_depi_secure_tag != 0) // DEPI
        {
            //dma secure bit/domain(domain is fixed at 4 as multimedia)
            MM_REG_WRITE_MASK(command, 0x150210d8, 0x00000024, 0x000000FF, NULL, m_frameConfigLabel[index++]);
        }
        if (m_ISPConfig.drvinfo.dip_lcei_secure_tag != 0) // LCEI
        {
            //dma secure bit/domain(domain is fixed at 4 as multimedia)
            MM_REG_WRITE_MASK(command, 0x150210c4, 0x00000024, 0x000000FF, NULL, m_frameConfigLabel[index++]);
        }
        if (m_ISPConfig.drvinfo.dip_ufdi_secure_tag != 0) // UFDI
        {
            //dma secure bit/domain(domain is fixed at 4 as multimedia)
            MM_REG_WRITE_MASK(command, 0x150210d8, 0x00002400, 0x0000FF00, NULL, m_frameConfigLabel[index++]);
        }

        //dma secure bit/domain(domain is fixed at 4 as multimedia)
        //smti_d1 ~ smti_d4
        MM_REG_WRITE_MASK(command, 0x150210c8, 0x24242424, 0xFFFFFFFF, NULL, m_frameConfigLabel[index++]);
        //smti_d5 ~ smti_d6
        MM_REG_WRITE_MASK(command, 0x150210dc, 0x00002424, 0x0000FFFF, NULL, m_frameConfigLabel[index++]);
        //smto_d2 ~ smto_d5
        MM_REG_WRITE_MASK(command, 0x150210e0, 0x24242424, 0xFFFFFFFF, NULL, m_frameConfigLabel[index++]);
        //smto_d1,smto_d6
        MM_REG_WRITE_MASK(command, 0x150210ec, 0x00002424, 0x0000FFFF, NULL, m_frameConfigLabel[index++]);

        //cq, tdr
        MM_REG_WRITE_MASK(command, 0x150210f4, 0x00242424, 0x00FFFFFF, NULL, m_frameConfigLabel[index++]);
    }
    #endif
	// DIP settings for GCE cmd
...
DP_STATUS_ENUM DpWrapper_ISP::onConfigTile(DpCommand &command)
{
    ...
    uint32_t       memSize[3] = {0};
    uint32_t       planeOffset[3] = {0};
    uint64_t       flag = (uint64_t)(0LL);//<< CMDQ_ENG_ISP_DMGI_DEPI);
