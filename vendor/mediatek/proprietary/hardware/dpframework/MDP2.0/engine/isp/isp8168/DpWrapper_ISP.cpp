...
#define CAM_IMGI_BASEADDR 	 0x7230
#define CAM_IMG2O_BASEADDR 	 0x7440
...
DP_STATUS_ENUM DpWrapper_ISP::onWaitEvent(DpCommand &command)
{
    ...
    MM_REG_POLL_MASK(command, ISP_BASE_HW + CAM_CTL_START, 0x0, 0x00000013);

    // Use cmdq to update IMGi & IMG2O in secure flow
    // Because the secMVA is translated in TEE, we can't use CQ to update them
    // CQ only update the normal buffer and it causes M4U fault
	if (m_ISPConfig.drvinfo.dip_secure_tag&0x1) {
		if (m_ISPConfig.drvinfo.dip_imgi_secure_tag != 0)
			MM_REG_WRITE_MASK(command, ISP_BASE_HW + CAM_IMGI_BASEADDR, m_ISPConfig.drvinfo.dip_imgi_base_addr, 0xFFFFFFFF);

		if (m_ISPConfig.drvinfo.dip_img2o_secure_tag != 0)
			MM_REG_WRITE_MASK(command, ISP_BASE_HW + CAM_IMG2O_BASEADDR, m_ISPConfig.drvinfo.dip_img2o_base_addr, 0xFFFFFFFF);
	}
                                           ...
                                           DpConfig  &config)
{
    ...
    if (m_ISPConfig.drvinfo.dip_secure_tag&0x1) // For secure camera path
    {
        if (m_ISPConfig.drvinfo.dip_img2o_secure_tag != 0) // IMG2O
        {
            ...
            DPLOGI("[onConfigFrame] command.secure(CMDQ_ENG_ISP_IMG2O(0x%x), flag(0x%x), secureAddr(0x%x), DP_SECURE(%d), dip_imgi_base_addr(0x%x), offset(0x%x), memSize(0x%x), planofst(0x%x)",
				CMDQ_ENG_ISP_IMG2O, flag, secureRegAddr[0], (DpSecure)m_ISPConfig.drvinfo.dip_img2o_secure_tag, m_ISPConfig.drvinfo.dip_img2o_base_addr, offset[0], m_ISPConfig.drvinfo.dip_img2o_size[0], planeOffset[0]);
        ...
        if (m_ISPConfig.drvinfo.dip_imgi_secure_tag != 0) // IMGI
        {
            ...
            DPLOGI("[onConfigFrame] command.secure(CMDQ_ENG_ISP_IMGI(0x%x), flag(0x%x), secureAddr(0x%x), DP_SECURE(%d), dip_imgi_base_addr(0x%x), offset(0x%x), memSize(0x%x), planofst(0x%x)",
				CMDQ_ENG_ISP_IMGI, flag, secureRegAddr[0], (DpSecure)m_ISPConfig.drvinfo.dip_imgi_secure_tag, m_ISPConfig.drvinfo.dip_imgi_base_addr, offset[0], m_ISPConfig.drvinfo.dip_imgi_size[0], planeOffset[0]);
        ...
        planeOffset[1] = 0;
        planeOffset[2] = 0;
        flag = (uint64_t)(1LL<<CMDQ_ENG_ISP_IMGI); // For CQ DMA
        switch (m_ISPConfig.drvinfo.p2Cq) {
	        case ISP_TPIPE_P2_CQ1:
                secureRegAddr[0] = ISP_BASE_HW + CAM_CTL_CQ1_BASEADDR;
                break;
            case ISP_TPIPE_P2_CQ2:
                secureRegAddr[0] = ISP_BASE_HW + CAM_CTL_CQ2_BASEADDR;
                break;
            case ISP_TPIPE_P2_CQ3:
                secureRegAddr[0] = ISP_BASE_HW + CAM_CTL_CQ3_BASEADDR;
                break;
            default:
                DPLOGE("[Error]not support this cqIdx(%d)\n", m_ISPConfig.drvinfo.p2Cq);
                return DP_STATUS_INVALID_PARAX;
        }
        memSize[0] = m_ISPConfig.drvinfo.CQDescSecBufSize;
        DPLOGI("[onConfigFrame] command.secure(CMDQ_ENG_ISP_IMGI(0x%x), flag(0x%x), secureAddr(0x%x), DP_SECURE(%d), CQDescSecBufHdl(0x%x), offset(0x%x), memSize(0x%x), planofst(0x%x)",
			CMDQ_ENG_ISP_IMGI, flag, secureRegAddr[0], DP_SECURE, m_ISPConfig.drvinfo.CQDescSecBufHdl, offset[0], memSize[0], planeOffset[0]);
        command.secure(CMDQ_ENG_ISP_IMGI, flag, secureRegAddr, DP_SECURE, m_ISPConfig.drvinfo.CQDescSecBufHdl, offset, memSize, planeOffset);
    }
#endif // CONFIG_FOR_VERIFY_FPGA
...
DP_STATUS_ENUM DpWrapper_ISP::onConfigTile(DpCommand &command)
{
	...
	if (m_ISPConfig.drvinfo.dip_secure_tag&0x1)
	{
		secureRegAddr[0] = ISP_BASE_HW + CAM_TDRI_BASE_ADDR;
		planeOffset[0] = m_pTileDesc->tpipe_info[m_TDRITileID].dump_offset_no*sizeof(uint32_t);
		memSize[0] = m_ISPConfig.drvinfo.tpipeTableSecSize - planeOffset[0];
		DPLOGI("[onConfigTile] command.secure(CMDQ_ENG_ISP_IMGI(0x%x), flag(0x%x), secureAddr(0x%x), DP_SECURE(%d), tpipeTableSecHdl(0x%x), offset(0x%x), memSize(0x%x), planofst(0x%x)",
			CMDQ_ENG_ISP_IMGI, flag, secureRegAddr[0], DP_SECURE, m_ISPConfig.drvinfo.tpipeTableSecHdl, offset[0], memSize[0], planeOffset[0]);
		...
		m_TDRITileID++;
		MM_REG_WRITE_MASK(command, ISP_BASE_HW + CAM_TDRI_BASE_ADDR, m_ISPConfig.drvinfo.tpipeTableSecHdl, 0xFFFFFFFF);
	} else {
		m_TDRITileID++;
	...
	if (m_ISPConfig.drvinfo.dip_secure_tag&0x1)
		bufPA = m_ISPConfig.drvinfo.CQDescSecBufHdl;
	else
		bufPA = m_ISPConfig.drvinfo.DesCqPa;

    switch (m_ISPConfig.drvinfo.p2Cq) {
        case ISP_TPIPE_P2_CQ1:
            MM_REG_WRITE_MASK(command, ISP_BASE_HW + CAM_CTL_CQ1_BASEADDR, bufPA, 0xFFFFFFFF); //ISP CQ1 base address
            break;
        case ISP_TPIPE_P2_CQ2:
            MM_REG_WRITE_MASK(command, ISP_BASE_HW + CAM_CTL_CQ2_BASEADDR, bufPA, 0xFFFFFFFF); //ISP CQ2 base address
            break;
        case ISP_TPIPE_P2_CQ3:
            MM_REG_WRITE_MASK(command, ISP_BASE_HW + CAM_CTL_CQ3_BASEADDR, bufPA, 0xFFFFFFFF); //ISP CQ3 base address
            break;
        default:
...
DP_STATUS_ENUM DpWrapper_ISP::onReconfigTiles(DpCommand &command)
{
	...
	{
		if (m_ISPConfig.drvinfo.dip_secure_tag&0x1) {
			tpipePa = m_ISPConfig.drvinfo.tpipeTableSecHdl + m_pTileDesc->tpipe_info[i].dump_offset_no*sizeof(uint32_t);
			DPLOGE("Tobe Fixed: tpipePa(%d): 0x%x = tpipeTableSecHdl(0x%x) + ofst(0x%x)", i, tpipePa,
				m_ISPConfig.drvinfo.tpipeTableSecHdl, m_pTileDesc->tpipe_info[i].dump_offset_no*sizeof(uint32_t));
			DPLOGE("Tobe Fixed: After 2nd tile offset can't be added to secure handle directly!!");
		} else {
