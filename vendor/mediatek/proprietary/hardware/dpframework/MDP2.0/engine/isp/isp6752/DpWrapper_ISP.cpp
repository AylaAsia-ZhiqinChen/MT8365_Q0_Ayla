...
//	    if (TPIPE_PASS2_CMDQ_1 == m_ISPPassType)
//	    {
//	        MM_REG_POLL(command, 0x15004000, 0x0, 0x00000001);
//	        MM_REG_CLEAR(command,DpCommand::ISP_P2_0_DONE);
//	    }else if (TPIPE_PASS2_CMDQ_2 == m_ISPPassType)
//	    {
//	        MM_REG_POLL(command, 0x15004000, 0x0, 0x00000002);
//	        MM_REG_CLEAR(command,DpCommand::ISP_P2_1_DONE);
//	    }else if (TPIPE_PASS2_CMDQ_3 == m_ISPPassType)
//	    {
//	        MM_REG_POLL(command, 0x15004000, 0x00, 0x00000010);
...
DP_STATUS_ENUM DpWrapper_ISP::onConfigTile(DpCommand &command)
{
    ...
    if (m_pIMGI->h_end_flag || m_pMDPCrop->h_end_flag)   // the end of tile
    {
    MM_REG_WRITE_MASK(command, ISP_BASE_HW + offsetof(isp_reg_t, CAM_CTL_INT_P2_EN), 0x000000E2, 0xFFFFFFFF);
    // command that must be finish
    }   //other tile
    else
    {
    MM_REG_WRITE_MASK(command, ISP_BASE_HW + offsetof(isp_reg_t, CAM_CTL_INT_P2_EN), 0x00000000, 0xFFFFFFFF);
    }
    return DP_STATUS_RETURN_SUCCESS;
}
