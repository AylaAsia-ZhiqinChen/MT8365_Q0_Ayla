...
DP_STATUS_ENUM DpWrapper_WPE::onWaitEvent(DpCommand &command)
{
    ...
    if (m_ISPConfig.top.wpe_mdpcrop_en == 1)
    {
        ...
        if(m_identifier == 0)
        	MM_REG_WRITE_MASK(command, 0x15020030, 0x00000400, 0x00000C00); // MDP2_DL_SEL: select wpe_a for IP
        else
			MM_REG_WRITE_MASK(command, 0x15020030, 0x00000200, 0x00000200); // MDP_DL_SEL: select wpe_b for IP
    }
