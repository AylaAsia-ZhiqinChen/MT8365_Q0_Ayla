...
DP_STATUS_ENUM DpWrapper_WPE::onInitEngine(DpCommand &command)
{
    ...
    {
        DPLOGI("DpWrapper_WPE: SW_RST\n");
        // Reset MDP_DL_ASYNC2_TX
        // Bit  3: MDP_DL_ASYNC2_TX / MDP_RELAY2
        MM_REG_WRITE(command, MMSYS_SW1_RST_B,     0x0, 0x00800000);
        MM_REG_WRITE(command, MMSYS_SW1_RST_B, 1 << 23, 0x00800000);
        // Reset MDP_DL_ASYNC2_RX
        // Bit  31: MDP_DL_ASYNC2_RX
        MM_REG_WRITE(command, MMSYS_SW1_RST_B,     0x0, 0x40000000);
        MM_REG_WRITE(command, MMSYS_SW1_RST_B, 1 << 30, 0x40000000);
    }
...
DP_STATUS_ENUM DpWrapper_WPE::onWaitEvent(DpCommand &command)
{
    ...
    if (m_ISPConfig.top.wpe_mdpcrop_en == 1)
    {
        ...
        MM_REG_WRITE_MASK(command, 0x15020030, 0x10, 0x10); //Enable WPE direct link
    }
