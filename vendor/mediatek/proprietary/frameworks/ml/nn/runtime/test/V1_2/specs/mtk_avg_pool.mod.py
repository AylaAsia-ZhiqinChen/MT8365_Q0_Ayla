...
i0 = Input("i0", "TENSOR_QUANT8_ASYMM", "{%d, %d, %d, %d}, 0.5f, 128" % (bat, row, col, chn))
...
output = Output("output", "TENSOR_QUANT8_ASYMM",
                "{%d, %d, %d, %d}, 0.25f, 128" % (bat, output_row, output_col, chn))

model = model.Operation(
    "AVERAGE_POOL_2D", i0, padding, padding, padding, padding, stride, stride, filt, filt, act0).To(output)

# Example 1. Input in operand 0,
input_values = [150 for _ in range(bat * row * col * chn)]
input0 = {i0: input_values}
output_values = [172 for _ in range(bat * output_row * output_col * chn)]
output0 = {output: output_values}

# Instantiate an example
Example((input0, output0))

# TEST 3: AVG_POOL quant8_3
# model
model = Model("quant8")
