...
void ATSParser_setHDCP(const sp<IHDCP> &hdcp)
{
#ifdef MTK_WFD_HDCP_RX_SUPPORT
    if (hdcp != NULL)
        gHDCP = hdcp;
...
struct ATSParser::Program : public RefBase {
    ...
    bool getIsProgFromWFD() {
#ifdef MTK_WFD_HDCP_RX_SUPPORT
        return mParser->getIsSourceFromWFD();
#else
        return false;
#endif
    };
    bool getIsProgFromWiDi() {
#ifdef MTK_WFD_HDCP_RX_SUPPORT
        return mParser->getIsSourceFromWiDi();
#else
        return false;
#endif
    };
...
//#endif
    ...
    ALOGI("new stream PID 0x%02x, type 0x%02x, scrambled %d, SampleEncrypted: %d",
    ...
    if (mQueue != NULL) {
#ifdef MTK_WFD_HDCP_RX_SUPPORT
            if (mProgram->getIsProgFromWFD()) {
                mQueue->setIsFromWFD(true);
            }
#endif
        if (mSampleAesKeyItem != NULL) {
...
status_t ATSParser::Stream::parse(
    ...
    char val[PROPERTY_VALUE_MAX] = {0};
	  if (property_get("media.wfd.tslog", val, 0))
    {
        if(strcmp(val,"1")==0)
            _enableLog=1;
    }
...
#ifdef MTK_WFD_HDCP_RX_SUPPORT
            if (STREAMTYPE_H264 == mStreamType && mProgram->getIsProgFromWFD()) {
                if (_enableLog) {
                    ALOGD("[WFD] PES_header_data_length=%d, PES Header Len=%d", mBuffer->data()[8], mBuffer->data()[8]+8+1);
                    ALOGD("[WFD] PES data receive full %d=%d", (int)mBuffer->size(), (mBuffer->data()[4] << 8) + mBuffer->data()[5] + 6);
                }
                if (mBuffer->size() > 0){
                    ALOGD("[WFD] PES data flush +++ Push, %d-%d",(int)mBuffer->size(), (mBuffer->data()[4] << 8) + mBuffer->data()[5] + 6);
                    mPayloadSize = mBuffer->size()-(mBuffer->data()[8]+8+1); // total - pes header 
    ...
    if (STREAMTYPE_H264 == mStreamType && mProgram->getIsProgFromWFD()) {
        ...
        // need move up, otherwise don't run here forever.
        if (mPayloadStarted &&
                (mBuffer->size() == (size_t)(mBuffer->data()[4] << 8) + mBuffer->data()[5] + 6)) {
            if (_enableLog) {
                ALOGD("[WFD] PES_header_data_length=%d, PES Header Len=%d", mBuffer->data()[8], mBuffer->data()[8]+8+1);
                ALOGD("[WFD] PES data receive full %d=%d", (int)mBuffer->size(), (mBuffer->data()[4] << 8) + mBuffer->data()[5] + 6);
            }
            ...
            mPayloadSize = mBuffer->size()-(mBuffer->data()[8]+8+1); // total - pes header
            if (_enableLog) ALOGD("[WFD] PES data flush +++ Match, %d-%d", (int)mBuffer->size(), (mBuffer->data()[4] << 8) + mBuffer->data()[5] + 6);
            flush(event);

        } else if (mPayloadStarted && ((mBuffer->data()[4] << 8) + mBuffer->data()[5]==0) &&
                    (mBuffer->size() > (size_t)(mBuffer->data()[4] << 8) + mBuffer->data()[5] + 6)) {
            /* Mi3 case */
        }
        else if (mPayloadStarted && ((mBuffer->data()[4] << 8) + mBuffer->data()[5]!=0) &&
            ...
            mPayloadSize = (mBuffer->data()[4] << 8) + mBuffer->data()[5]-3-mBuffer->data()[8];
            ALOGD("[WFD] PES data flush +++ More, %d>%d", (int)mBuffer->size(), (mBuffer->data()[4] << 8) + mBuffer->data()[5] + 6);
            flush(event);
        }
        return OK;
    }
...
status_t ATSParser::Stream::parsePES(ABitReader *br, SyncEvent *event) {
    ...
    if (stream_id != 0xbc  // program_stream_map
        ...
        if (mProgram->getIsProgFromWFD()){
            ALOGV("PES_CRC_flag = %u", PES_CRC_flag);
        }
#else
        ...
        if (mProgram->getIsProgFromWFD()){
            ALOGV("PES_extension_flag = %u", PES_extension_flag);
        }
#else
        ...
        if(DSM_trick_mode_flag) {
            if(optional_bytes_remaining < 1)
                return OK;
            MY_LOGV("trick_mode = %u", br->getBits(8));
        ...
        if(additional_copy_info_flag) {
            if(optional_bytes_remaining < 1)
                return OK;
            MY_LOGV("additional_copy_info = %u", br->getBits(8));
            optional_bytes_remaining -= 1;
        }
        ...
        if(PES_CRC_flag) {
            if(optional_bytes_remaining < 2)
                return OK;
            MY_LOGV("PES_CRC = %u", br->getBits(16));
        ...
        if(PES_extension_flag) {
            if(optional_bytes_remaining < 1)
                return OK;
            unsigned PES_private_data_flag = br->getBits(1);
            br->getBits(1);//unsigned pack_header_field_flag
            br->getBits(1);//unsigned program_packet_sequence_counter_flag
            br->getBits(1);//unsigned P_STD_buffer_flag
            unsigned PES_extension_flag_2;
            br->getBits(3);
            PES_extension_flag_2 = br->getBits(1);
            optional_bytes_remaining -= 1;
            ...
            if(PES_private_data_flag){
                ...
                mIsSvpStream = true;
                br->getBits(13);
                mStreamCtr |= (br->getBits(2)&3)<< 30;
                br->getBits(1);

                mStreamCtr |= (br->getBits(15)& 0x7fff) << 15;
                br->getBits(1);

                mStreamCtr |= (br->getBits(15)& 0x7fff);
                br->getBits(1);

                br->getBits(11);
                mInputCtr |= (uint64_t)(br->getBits(4)& 0xf)<< 60;
                br->getBits(1);

                mInputCtr |= (uint64_t)(br->getBits(15)& 0x7fff) << 45;
                br->getBits(1);

                mInputCtr |= (br->getBits(15)& 0x7fff) << 30;
                br->getBits(1);

                mInputCtr |= (br->getBits(15)& 0x7fff) << 15;
                br->getBits(1);

                mInputCtr |= (br->getBits(15)& 0x7fff);
                br->getBits(1);
                ALOGD("mStreamCtr %d, mInputCtr %lld",mStreamCtr,(long long)mInputCtr);
            }else{
                mInputCtr = 0;
                mStreamCtr = 0;
                mIsSvpStream = false;
...
//  duration patch --
    ...
    if (mSource != NULL && mSource->getQueuedBufferSize() > 100*1024*1024) {
        ALOGV("ASource queue too many buffer,stop queueAccessUnit");
        return;
    }
#ifdef MTK_WFD_HDCP_RX_SUPPORT
    if(mIsSvpStream){
        mQueue->setSvpInf(mIsSvpStream, mStreamCtr, mInputCtr);
    }else{
        mQueue->setSvpInf(false, 0, 0);
    }
#endif
    status_t err;
#ifdef MTK_WFD_HDCP_RX_SUPPORT
    if (isAudio() && mIsSvpStream) {
        sp<ABuffer> decryptedAU = new ABuffer(size);
        memcpy(decryptedAU->data(), data, size);
	    if (gHDCP != NULL) {
			ALOGD("Encrypted Audio=%zu, ingore it ", size);
            gHDCP->decrypt(
                        decryptedAU->data(), size,
                        (uint32_t)mStreamCtr, (uint64_t)mInputCtr,
                        decryptedAU->data());
        }
        err = mQueue->appendData(decryptedAU->data(), size, timeUs);
    }
    else
#endif
    {
        err = mQueue->appendData(data, size, timeUs,payloadOffset, PES_scrambling_control);
    }
    char val[PROPERTY_VALUE_MAX] = {0};
    property_get("media.wfd.tslog", val, 0);
    if(atoi(val)){
        ALOGE("appenddata %s PID:%d size:%d  timeUs:%lld",isVideo()?"video":"audio",(int)mElementaryPID,(int)size,(long long)timeUs);
    }
    if (mEOSReached) {
        mQueue->signalEOS();
    }
    ...
    bool found = false;
#ifdef MTK_WFD_HDCP_RX_SUPPORT
    mQueue->mPayloadSize = mPayloadSize;
#endif
    while ((accessUnit = mQueue->dequeueAccessUnit()) != NULL) {
        if (mSource == NULL) {
            ...
            if (meta != NULL) {
                ALOGE("Stream PID 0x%08x of type 0x%02x now has data.",
            ...
            int64_t timeUs = -1;
            if (accessUnit->meta()->findInt64("timeUs", &timeUs) && atoi(val)){
                ALOGE("queueAu %s size:%d timeUs:%lld",isVideo()?"video":"audio",
                    (int)accessUnit->size(),(long long)timeUs);
            }
            mSource->queueAccessUnit(accessUnit);
        }
...
ATSParser::ATSParser(uint32_t flags)
    ...
    mCasManager = new mtk::ATSParser::CasManager();
    mNeedDequeuePES = true;
//#ifdef MTK_WFD_HDCP_RX_SUPPORT
    isSourceFromWFD = false;
    isSourceFromWiDi =false;
    ALOGD("ATSParser");
//#endif
}
...
status_t ATSParser::feedTSPacketEx(const void *data, size_t size,
SyncEvent *event,bool isFromWFD, bool isWiDi) {
#ifdef MTK_WFD_HDCP_RX_SUPPORT 
    if (size != kTSPacketSize)
    {
        ALOGD("[CHECK_ERROR](feedTSPacket)not correct ts packet size=%d",(int)size);
        return ERROR_MALFORMED;
    }   
    setIsSourceFromWFD(isFromWFD);
    setIsSourceFromWiDi(isWiDi);
