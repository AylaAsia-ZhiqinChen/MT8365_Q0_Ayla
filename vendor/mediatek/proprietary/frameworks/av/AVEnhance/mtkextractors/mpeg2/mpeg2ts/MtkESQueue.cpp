...
//  seek patch --
    ...
    mSampleDecryptor = isSampleEncrypted() ? new mtk::HlsSampleDecryptor : NULL;
}
...
#ifdef MTK_WFD_HDCP_RX_SUPPORT
                if (mIsSvpStream) {
                    ALOGD("appendData() don't care about start code, mIsSvpStream=%d ", mIsSvpStream);
                    break;
                }
#endif
#if 0
...
void ElementaryStreamQueue::setSvpInf(bool IsSvpStream, uint32_t StreamCtr, uint64_t InputCtr){
#ifdef MTK_WFD_HDCP_RX_SUPPORT
    mIsSvpStream = IsSvpStream;
    mStreamCtr = StreamCtr;
    mInputCtr = InputCtr;
#else
    UNUSED(IsSvpStream);
    UNUSED(StreamCtr);
...
#ifdef MTK_WFD_HDCP_RX_SUPPORT
            if (getIsFromWFD())
                return dequeueAccessUnitH264forWFD(true);
            else
#endif
...
sp<ABuffer> ElementaryStreamQueue::dequeueAccessUnitAACforWFD() {
#ifdef MTK_WFD_HDCP_RX_SUPPORT
    if (mBuffer->size() == 0) {
        return NULL;
    }
	...
	ALOGD("[WFD] dequeueAccessUnitAACforWFD mBuffer->size=%d", (int)mBuffer->size());
    ...
    if (mIsSvpStream) {
        ...
        accessUnit->meta()->setInt64("timeUs", timeUs);
        accessUnit->meta()->setInt32("StreamCtr", mStreamCtr);
        accessUnit->meta()->setInt64("InputCtr", mInputCtr);
...
sp<ABuffer> ElementaryStreamQueue::dequeueAccessUnitH264forWFD(bool isWFD) {
#ifdef MTK_WFD_HDCP_RX_SUPPORT
	if (accessUnits.empty()) {
        Vector<NALPosition> nals;

    	if (isWFD) {
    		ALOGV("[WFD] dequeueAccessUnitH264forWFD mBuffer->size=%d, PESPayloadSize=%d ", (int)mBuffer->size(), mPayloadSize);
    	  
    	   if ( mBuffer->size()>0 &&
                mBuffer->size()>=(size_t)mPayloadSize) {
    		  

              if (mPayloadSize==0) {
                    ALOGD("[WFD] flush all buffer:%d ", (int)mBuffer->size());
                    mPayloadSize = mBuffer->size(); // flush all buffer
              }

              // WFD quick push flow  
    		  ALOGD("[WFD] WFD quick push flow, mPayloadSize=%d ", mPayloadSize);

    		  sp<ABuffer> accessUnit = new ABuffer(mPayloadSize);
    		  memcpy(accessUnit->data(), mBuffer->data(),mPayloadSize);

    		  // ???
    		  size_t nextScan = 0 + mPayloadSize;
              ...
              if(mIsSvpStream){
                accessUnit->meta()->setInt32("StreamCtr", mStreamCtr);
                accessUnit->meta()->setInt64("InputCtr", mInputCtr);
                accessUnit->meta()->setInt32("Encrypted", 1);
                if (mFormat == NULL) {
                    mFormat = new MetaData;
                    mFormat->setCString(kKeyMIMEType, MEDIA_MIMETYPE_VIDEO_AVC);

                    mFormat->setInt32(kKeyWidth, 1920);
                    mFormat->setInt32(kKeyHeight, 1080);
                 }
              }else{

        		  //ALOGD("[WFD] timeUs=%lld", timeUs);
        		  if (mFormat == NULL) {
        			  ALOGD("[WFD] set formt");

                    /* To support secure buffer only provide basic config because MediaCodec::queueCSDInputBuffer has memcpy operation  */
                    mFormat = new MetaData;
                    mFormat->setCString(kKeyMIMEType, MEDIA_MIMETYPE_VIDEO_AVC);
      		  ...
      		  mBuffer->setRange(0, 0); // clear ABuffer

      		  mPayloadSize = 0; // clear PayloadSize
      		  if (mFormat != NULL){
                   char value[PROPERTY_VALUE_MAX];
                   property_get("esqueue.ts.dump", value, "0");
                   if(atoi(value)){
                      char fileName[PATH_MAX];
                      uint32_t size_fp_output = 0; 
                      snprintf(fileName, PATH_MAX, "/data/dump/esqueue_h264.bin");
                      FILE * fd = fopen(fileName,"ab");
                      if (fd != NULL)
                      {
                          size_fp_output = fwrite("\x00\x00\x00\x01", 1, 4,fd);
                          size_fp_output = fwrite(accessUnit->data(), 1, accessUnit->size(),fd);
                          fclose(fd);
                      }
                   }
               }
           ...
           else if (mBuffer->size()>0 && mPayloadSize>0 && mBuffer->size()< (size_t)mPayloadSize ) {
                // WFD quick push flow
    		    ALOGV("[WFD] WFD next header push case, mBuffer->size()=%d ", (int)mBuffer->size());

                sp<ABuffer> accessUnit = new ABuffer(mBuffer->size());
    		  memcpy(accessUnit->data(), mBuffer->data(),mBuffer->size());

    		  // ???
    		  size_t nextScan = 0 + mBuffer->size();
    	  ...
    	  //ALOGD("[WFD] I am ready to while ");
    	}

    //ALOGE("[TIME] parser packet time:%lld",getESCountMs()-TimeMs);
	}
	if (!accessUnits.empty()) {
		sp<ABuffer> accessUnit = *accessUnits.begin();
		accessUnits.erase(accessUnits.begin());
        //ALOGE("[TIME] fill h264 au time:%lld",getESCountMs()-TimeMs);
		return accessUnit;
	} else {
	    //ALOGE("[TIME] no au time:%lld",getESCountMs()-TimeMs);
		return NULL;
	}
#else
    UNUSED(isWFD);
