...
const char* UibcCapability::getLocalCapabilities()
{
    ...
    int len;
    mLocalCap.clear();
    len = property_get("vendor.media.wfd.uibc.localcap", val, NULL);

    if(len > 0 && !strstr(val, "0")) {
        ALOGI("vendor.media.wfd.uibc.localcap:%s", val);
        bool bNone = true;
...
bool UibcCapability::isHidcPropEnabled()
{
    ...
    if(property_get("vendor.media.wfd.uibc.hidc", val, NULL)) {
        ALOGI("vendor.media.wfd.uibc.hidc:%s", val);
