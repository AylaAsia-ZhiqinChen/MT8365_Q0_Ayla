...
void RTPReceiver::Source::queuePacket(const sp<ABuffer> &packet) {
    ...
    if (mPackets.empty()) {
        ...
        if(mPackets.size()>2)
            ALOGI("mPackets size is %zu",mPackets.size());
        dequeueMore();
        return;
    #if 0
    }
...
RTPReceiver::RTPReceiver(
    ...
    ALOGD("rtpreceiver constructor latency rule %d", mLatencyRule);
...
status_t RTPReceiver::connect(
    ...
    status_t err;
    ALOGD("RTPReceiver::connect remoteRTPPort = %d,remoteRTCPPort = %d",remoteRTPPort,remoteRTCPPort);
    ...
    if (mRTCPMode == TRANSPORT_UDP) {
        ...
        if (remoteRTCPPort>0) {
            ///Add by MTK @{  To avoid sink IOT problem
            // if Source don't support rtcp, there is no remoteRTCPPort here, don't need connect
            err = mNetSession->connectUDPSession(
            ...
            ALOGD("connectUDPSession RTCP successful.");

            mRTCPConnected = true;
        }
        else
            ALOGD("connectUDPSession RTCP fail, remoteRTCPPort=%d", remoteRTCPPort);
    }
...
status_t RTPReceiver::informSender(const sp<AMessage> &params) {
    ...
    ALOGD("RTCP informSender %zu", buf->size());
        ...
        case MtkANetworkSession::kWhatDatagram:
        {
            ...
            if (isRTP) {
                ...
                if(mFirstRTPData)
                {
                    mFirstRTPData = false;
                    int64_t nowUs = ALooper::GetNowUs();
                    ALOGI("WfdApp-1st_data Time:(%lld)us",(long long)nowUs);
                }
#endif
...
/// @}
    ...
    if(BaseOnMbit == mLatencyRule){
        ...
        if(Mbit == 1){
            ...
            ALOGV("rtp in %lld", (long long)timeUs);
        }
...
void RTPReceiver::scheduleSendRR() {
    (new AMessage(kWhatSendRR, this))->post(5000000ll);
}
...
void RTPReceiver::onSendRR() {
    ...
    ALOGD("RTCP onSendRR %zu", buf->size());
    mNetSession->sendRequest(mRTCPSessionID, buf->data(), buf->size());
...
void RTPReceiver::dumpTs(const uint8_t * data, size_t size, bool bdtse)
{
    ...
    if(bdtse)/*dump start*/
    {
        if(!mDumpTsFlag)
        {
            mTsFile = fopen("/data/dump/wfdsink.ts", "w+");
            if(NULL == mTsFile)
            {
                ALOGD("Open file fail[/data/dump/wfdsink.ts]");
            }
            else
            {
                ALOGD("Begin to dump ts file[/data/dump/wfdsink.ts]");
                mDumpTsFlag = true;
            }
...
#else
    ALOGD("data = %p ,size = %zu, bdtse = %d",data,size,(int32_t)bdtse);
#endif
}
...
void RTPReceiver::Source::postRetransmitTimer(int64_t timeUs) {
    ...
    sp<AMessage> msg = new AMessage(kWhatRetransmit, this);
...
void RTPReceiver::Source::postDeclareLostTimer(int64_t timeUs) {
    ...
    int64_t delayUs = timeUs - ALooper::GetNowUs();
    sp<AMessage> msg = new AMessage(kWhatDeclareLost, this);
