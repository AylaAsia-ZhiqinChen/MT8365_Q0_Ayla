...
TimeSyncer::TimeSyncer(
        const sp<MtkANetworkSession> &netSession, const sp<AMessage> &notify)
...
void TimeSyncer::onMessageReceived(const sp<AMessage> &msg) {
    switch (msg->what()) {
        case kWhatStartClient:
        {
            ...
            sp<AMessage> notify = new AMessage(kWhatUDPNotify, this);
...
void TimeSyncer::postSendPacket(int64_t delayUs) {
    (new AMessage(kWhatSendPacket, this))->post(delayUs);
}

void TimeSyncer::postTimeout() {
    sp<AMessage> msg = new AMessage(kWhatTimedOut, this);
...
void TimeSyncer::notifyOffset() {
    ...
    for (size_t i = 0; i < mHistory.size() / 3; ++i) {
        ...
        ALOGV("(%zu) RT: %lld us, offset: %lld us",
              i, (long long)(ti->mT4 - ti->mT1), (long long)offset);
    ...
    if (mNotify == NULL) {
        ALOGI("avg. offset is %lld", (long long)(sum / count));
        return;
    }
