...
SurfaceMediaSource::SurfaceMediaSource(uint32_t bufferWidth, uint32_t bufferHeight,bool isHDCP) :
    mWidth(bufferWidth),
    mHeight(bufferHeight),
    mCurrentSlot(BufferQueue::INVALID_BUFFER_SLOT),
    mNumPendingBuffers(0),
    ...
    uint64_t flags = GRALLOC_USAGE_HW_VIDEO_ENCODER | GRALLOC_USAGE_HW_TEXTURE;
    if(isHDCP)
    {
        ALOGE("isHDCP is true ");
        flags |= GRALLOC_USAGE_SECURE;
    }
