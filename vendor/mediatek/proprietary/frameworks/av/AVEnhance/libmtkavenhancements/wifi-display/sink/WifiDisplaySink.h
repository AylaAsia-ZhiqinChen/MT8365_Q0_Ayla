...
//#endif
    ...
    State mState;
    uint32_t mFlags;
    MtkVideoFormats mSinkSupportedVideoFormats;
...
static const int64_t kRetryDelayUs = 0.2*1000000ll;
    int64_t mRetryCounter;
    bool mRtspConnected;
    bool mkWhatStarted;
    int32_t mSourcePort;
...
//#ifdef WFD_HDCP_RX_SUPPORT
    ///M: Support HDCP2.x RX  @{

    static bool sHDCPAuthentication;
    static bool sHDCPNewConn;
...
//#endif
    ...
    status_t sendGenericMsgByMethod(int32_t methodID);
        void retrykWhatStart(const char *sourceHost, int32_t sourcePort);
    void     onReceiveTestData(const sp<AMessage> &msg);
    void scheduleCheckAlive(int32_t sessionID) ;
    void updateChosenResolution(const char *videoSpec);
    /// @}
