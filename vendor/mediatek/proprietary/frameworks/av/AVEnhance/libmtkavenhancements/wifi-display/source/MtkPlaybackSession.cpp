    ...
    unsigned constraintSet, size_t *numInputBuffers)
{
    ...
    ALOGE("IS HDCP mode? %d",(mHDCP != NULL));
    sp<MtkConverter> converter;
    if(isVideo) {
        converter = new MtkConverter(notify, codecLooper, format, (mHDCP != NULL));
    } else {
        converter = new MtkConverter(notify, codecLooper, format, false);
    }
    ...
    MtkVideoFormats::LevelType videoLevelType)
{
    ...
    sp<SurfaceMediaSource> source = new SurfaceMediaSource(width, height, (mHDCP != NULL));
