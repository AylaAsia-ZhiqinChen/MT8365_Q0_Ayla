...
status_t CAPEWrapper::Create(const Parcel &para __unused, Parcel *reply)
{
...
status_t CAPEWrapper::Destroy(const Parcel &para __unused)
{
    apeHandle = NULL;
...
status_t CAPEWrapper::DeInit(const Parcel &para __unused)
{
    return OK;
}
...
//consumeBS should be returned to the MtkOmxApeDec component...>>>
    ...
    //tempbuffer was decoded to end;why inputBufferUsedLength > Tempbuffersize?
            if ((unsigned int)mApeConfig.inputBufferUsedLength >= Tempbuffersize) ///&& (mSourceRead == false))
            {
    ...
    //tempbuffer suplus
        ...
        else
        {
         //input buffer not docodec end,copy suplus data in input buffer to temp buffer
            ...
            if ((unsigned int)(inAlloclen - inOffset - mApeConfig.inputBufferUsedLength) <= in_size)
            {
                LOGV("tempbuf false1");
