...
public:
    ...
    virtual status_t setUp(EffectHalVersion const &nameVersion)
    {
        ...
        Parcel data, reply;
        data.writeInterfaceToken(IFeatureManager::getInterfaceDescriptor());
        // TODO this api will be phase out, now do nothing
    ...
    virtual status_t getEffectFactory(sp<IInterface> &effectFactory)
    {
        FUNCTION_LOG_START;
        Parcel data, reply;
        data.writeInterfaceToken(IFeatureManager::getInterfaceDescriptor());
        ...
        {
            sp<IBinder> bind = reply.readStrongBinder();
            effectFactory = bind->queryLocalInterface(bind->getInterfaceDescriptor());
        }
