...
#define RSC_ROOT_PROPERTY "ro.vendor.sys.current_rsc_path"
#define SYSTEM_ROOT "system/etc"
#define CFG_FILE_NAME "custom.conf"
...
static int get_browser_version(char *buff)
{
    ...
    ALOGI("[custom_prop]get_browser_version");
...
static int get_release_date(char *buff)
{
    ...
    ALOGI("[custom_prop]get_release_date");
...
static JNIEnv *load_jni_env()
{
    ...
    if (!handle) {
        handle = dlopen("/system/lib64/libcustom_jni.so", RTLD_NOW);
    if (!handle)
        return 0;
    }
    ...
    ALOGI("[custom_prop]getEnv[%p]", env);
...
static jclass load_interface(JNIEnv *env)
{
    ...
    ALOGI("[custom_prop]callMethod[%p]", intrfc);
...
static void reset_buffer(Buffer *buff, char *base, int size)
{
    buff->base = buff->cursor = base;
    buff->size = size;
}
...
static void reset_parser(Parser *parser, int keep_output)
{
    ...
    reset_buffer(&parser->buff, parser->output->key, sizeof(parser->output->key) - 1);
    reset_buffer(&parser->buff2, parser->output->prop, sizeof(parser->output->prop) - 1);
...
static int parse_state_key(const char *in, Parser *parser)
{
    ...
    case '.':
        *parser->buff.cursor = '\0';
        strncpy(item->module, parser->buff.base, sizeof(item->module)-1);
        reset_buffer(&parser->buff, item->key, sizeof(item->key)-1);
    ...
    case '=':
        ...
        *parser->buff.cursor = '\0';
        reset_buffer(&parser->buff, item->value, sizeof(item->value) - 1);
    ...
    default:
        if (parser->buff.cursor - parser->buff.base < parser->buff.size)
            *parser->buff.cursor++ = *in;
        break;
    }
...
static int parse_state_value(const char *in, Parser *parser)
{
    ...
    case '@':
        ...
        reset_buffer(&parser->buff2, item->prop, sizeof(item->prop) - 1);
        break;
...
static int parse_state_property(const char *in, Parser *parser)
{
    ...
    case ' ':
    case '@':
        *parser->buff2.cursor = '\0';
        ALOGI("[custom_prop]prop name:%s", parser->buff2.base);
        /* reserve enough buffer size since there is no way to test actual value length */
        if (parser->buff.cursor - parser->buff.base + PROP_VALUE_MAX <= parser->buff.size) {
            for (i = 0; CustomProperties[i].name; i++) {
                if (strcmp(item->prop, CustomProperties[i].name) == 0) {
                    parser->buff.cursor += CustomProperties[i].finder(parser->buff.cursor);
                    break;
                }
            }
            if (!CustomProperties[i].name) {
                parser->buff.cursor += property_get(item->prop, parser->buff.cursor, 0);
            }
        }
        if ('\n' == *in)
            reset_parser(parser, 1);
    ...
    default:
        if (parser->buff2.cursor - parser->buff2.base < parser->buff2.size)
            *parser->buff2.cursor++ = *in;
        break;
    }
...
static int parse_data(const char *data, Parser *parser)
{
    ...
    else if (parser->state == STATE_PROP) {
        *parser->buff2.cursor = '\0';
        if (PROP_VALUE_MAX + parser->buff.cursor - parser->buff.base <= parser->buff.size)
            parser->buff.cursor += property_get(parser->buff2.base, parser->buff.cursor, 0);
        *parser->buff.cursor = '\0';
    }
    else
        reset_parser(parser, 0);
...
static int get_rsc_file_path(char *buff, int buffSize) {
    property_get(RSC_ROOT_PROPERTY, buff, "");
    if (strlen(buff) > 0) {
        snprintf(&buff[strlen(buff)], buffSize - strlen(buff), "%c%s", '/', CFG_FILE_NAME);
...
static int get_sys_file_path(char *buff, int buffSize) {
    snprintf(buff, buffSize, "%s%c%s", SYSTEM_ROOT, '/', CFG_FILE_NAME);
    return 0;
}
...
{
    ConfigItem *item;
    char cfgPath[PROPERTY_VALUE_MAX];

    pthread_mutex_lock(&parseMutex);
    if (!initDone) {
        if (get_rsc_file_path(cfgPath, sizeof(cfgPath)) < 0) {
            ALOGW("[custom_prop]get cfg from RSC[%s] fail", cfgPath);
            get_sys_file_path(cfgPath, sizeof(cfgPath));
        }
        if (parse_file(cfgPath))
            initDone = 1;
        else {
    ...
    strncpy(value, item->value, sizeof(item->value)-1);
    value[sizeof(item->value) - 1] = '\0';

    return strlen(value);

find_fail:
    if (default_value) {
        strncpy(value, default_value, sizeof(item->value)-1);
        value[sizeof(item->value)-1] = '\0';
        return strlen(value);
    }
