...
public class AnrManagerImpl extends AnrManager {
    ...
    private AnrManagerService mService;

    public AnrManagerImpl() {
        mService = AnrManagerService.getInstance();
    }

    @Override
    public void AddAnrManagerService() {
        ServiceManager.addService("anrmanager", (IBinder) mService, true);
    }

    @Override
    public void startAnrManagerService(int pid) {
        mService.startAnrManagerService(pid);
    }

    @Override
    public boolean isAnrDeferrable() {
        return mService.isAnrDeferrable();
    }
    ...
    public void writeEvent(int event) {
        mService.writeEvent(event);
    }

    @Override
    public void sendBroadcastMonitorMessage(long timeoutTime,
            long timeoutPeriod) {
        mService.sendBroadcastMonitorMessage(timeoutTime, timeoutPeriod);
    }

    @Override
    public void removeBroadcastMonitorMessage() {
        mService.removeBroadcastMonitorMessage();
    }

    @Override
    public void sendServiceMonitorMessage() {
        mService.sendServiceMonitorMessage();
    }

    @Override
    public void removeServiceMonitorMessage() {
        mService.removeServiceMonitorMessage();
    }
    ...
    public boolean startAnrDump(ActivityManagerService service,
        ...
        try {
            return mService.startAnrDump(service, app, activityShortComponentName, aInfo,
                    parentShortComponentName, parentProcess, aboveSystem, annotation,
                    showBackground, anrTime);
        } catch (Exception e) {
            e.printStackTrace();
            return false;
        }
