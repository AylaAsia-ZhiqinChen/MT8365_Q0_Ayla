...
/// M: NeuroPilot @}
  ...
  // 'buffer'.
  virtual void* OpInit(const TfLiteRegistration& op_reg, const char* buffer,
               size_t length) {
  ...
  virtual void OpFree(const TfLiteRegistration& op_reg, void* buffer) {
  ...
  virtual TfLiteStatus OpPrepare(const TfLiteRegistration& op_reg, TfLiteNode* node);
  ...
  std::unique_ptr<MtkNnApiDelegate> mtk_nnapi_delegate_;
};
