...
// M: reference reference_ops::ResolveAxis
void ResolveDuplicateAxis(const int num_dims, const int* axis,
                          const int num_axis, std::vector<int>& resolve_axis) {
  ...
  for (int idx = 0; idx < num_axis; ++idx) {
    ...
    bool is_dup = false;
    for (int j = 0; j < resolve_axis.size(); ++j) {
      if (resolve_axis[j] == current) {
        is_dup = true;
        break;
      }
    }
    if (!is_dup) {
      resolve_axis.push_back(current);
    }
  }
}
// M: @}
...
TfLiteStatus EvalMean(TfLiteContext* context, TfLiteNode* node) {
  ...
  // M: Solve this issue if the axis data number are more than 4.
  // for example: [-3, -3, -3, 0, -1, 3, 0, 3] contains many duplicates.
  // Original ResolveAxis only consider 4. @{
  std::vector<int> resolve_axis;
  ResolveDuplicateAxis(op_context.input->dims->size,
                       GetTensorData<int>(op_context.axis),
                       num_axis, resolve_axis);
  TF_LITE_ENSURE(context, resolve_axis.size() <= 4);
  int resolve_axis_data[4] = {0};
  int resolve_axis_num = resolve_axis.size();
  for (size_t i = 0 ; i < resolve_axis.size(); i++) {
      resolve_axis_data[i] = resolve_axis[i];
  }
  // M: @}
  ...
  if (kernel_type == kGenericOptimized) {
    ...
    switch (op_context.input->type) {
      case kTfLiteInt8: {
        tflite::MeanParams op_params;
        // M: solve duplicate axis
        // op_params.axis_count = num_axis;
        // ResolveAxis(GetTensorData<int>(op_context.axis), num_axis, &op_params);
        op_params.axis_count = resolve_axis_num;
        ResolveAxis(resolve_axis_data, resolve_axis_num, &op_params);
        // M: @}
