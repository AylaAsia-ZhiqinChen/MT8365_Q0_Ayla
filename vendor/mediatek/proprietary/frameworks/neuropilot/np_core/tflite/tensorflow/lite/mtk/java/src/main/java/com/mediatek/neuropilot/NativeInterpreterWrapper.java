...
final class NativeInterpreterWrapper implements AutoCloseable {
  ...
  private void init(long errorHandle, long modelHandle, Interpreter.Options options) {
    ...
    if (options.cacheDir != null) {
      setCacheDir(interpreterHandle, options.cacheDir);
    }
    if (options.encryptionLevel != 0) {
      setEncryptionLevel(interpreterHandle, options.encryptionLevel);
    }
  ...
  private static native void setCacheDir(long interpreterHandle, String cacheDir);

  private static native void setEncryptionLevel(long interpreterHandle, int encryptionLevel);
