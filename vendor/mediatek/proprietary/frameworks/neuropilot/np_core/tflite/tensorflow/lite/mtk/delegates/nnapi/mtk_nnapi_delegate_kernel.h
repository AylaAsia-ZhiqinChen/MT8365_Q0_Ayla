...
#ifndef MTK_TENSORFLOW_LITE_DELEGATES_NNAPI_NNAPI_DELEGATE_KERNEL_H_
#define MTK_TENSORFLOW_LITE_DELEGATES_NNAPI_NNAPI_DELEGATE_KERNEL_H_
...
static const bool kUseAhardwareBuffer = tflite::mtk::IsFeatureSupported(
  tflite::mtk::ANEUROPILOT_FEATURE_ION);
...
struct NNAPIOpMappingArgs {
  ...
  ::tflite::mtk::CustomOpHelper& custom_op_helper;
  ::tflite::mtk::CustomOpHelper::ParameterFunc (*GetParamFunc)(std::string op_name);
  ::tflite::mtk::CustomOpHelper::MtkExtOpParameterFunc (*GetMtkExtOpParamFunc)(
                                                std::string op_name);
  /// M: NeuroPilot @}
};
...
struct NNFreeModelEncryptor {
  void operator()(ANnModelEncryptor* model_encryptor) {
    NnApiImplementation()->ANnModelEncryption_delete(model_encryptor);
  }
};
    ...
    if (name && size > 0) {
      ...
      if (kUseAhardwareBuffer) {
          ...
          if(AHardwareBuffer_allocate(&desc, &buffer_) == 0) {
              nnapi_->ANeuralNetworksMemory_createFromAHardwareBuffer(buffer_, &nn_memory_handle_);
          }
      }
      if (!buffer_) {
...
class NNAPIDelegateKernel {
 ...
 private:
  ...
  std::unique_ptr<ANnModelEncryptor, NNFreeModelEncryptor> nn_model_encryptor_;
...
#endif  // MTK_TENSORFLOW_LITE_DELEGATES_NNAPI_NNAPI_DELEGATE_KERNEL_H_
