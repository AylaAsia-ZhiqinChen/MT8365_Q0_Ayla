...
//This API is only use internal, so we define it in nnapi_implementation.h
typedef int (*ANnModelEncryption_setDebugFlag_fn)(ANnModelEncryptor *encryptor, bool debugOn);
...
struct NnApi {
  ...
  int (*ANnModelEncryption_create)(
  ANnModelEncryptor **encryptor, void *model, EncryptionLevel encryptLevel,
    uint8_t rsaEncryptionKey[16]);

  int (*ANnModelEncryption_getExtensionOperationType)(ANnModelEncryptor *encryptor,
  const char *extensionName, uint16_t operationCodeWithinExtension,
  ANeuralNetworksOperationType *type);

  int (*ANnModelEncryption_addOperation)(ANnModelEncryptor *encryptor,
  ANeuralNetworksOperationType type, uint32_t inputCount,
  const uint32_t *inputs, uint32_t outputCount, const uint32_t *outputs);

  int (*ANnModelEncryption_getExtensionOperandType)(ANnModelEncryptor *encryptor,
  const char *extensionName, uint16_t operandCodeWithinExtension, int32_t *type);

  int (*ANnModelEncryption_addOperand)(ANnModelEncryptor *encryptor,
  const ANeuralNetworksOperandType *type);

  int (*ANnModelEncryption_setOperandValue)(ANnModelEncryptor *encryptor,
  int32_t index, const void *buffer, size_t length);

  int (*ANnModelEncryption_setOperandValueFromFd)(ANnModelEncryptor *encryptor,
  int32_t index, const int fd, size_t offset, size_t length);

  int (*ANnModelEncryption_identifyInputsAndOutputs)(ANnModelEncryptor *encryptor,
  uint32_t inputCount, const uint32_t *inputs, uint32_t outputCount, const uint32_t *outputs);

  int (*ANnModelEncryption_relaxComputationFloat32toFloat16)(
  ANnModelEncryptor *encryptor, bool allow);

  int (*ANnModelEncryption_setDebugFlag)(ANnModelEncryptor *encryptor, bool debugOn);

  int (*ANnModelEncryption_finish)(ANnModelEncryptor *encryptor, bool needCargo);

  void (*ANnModelEncryption_delete)(ANnModelEncryptor *encryptor);

  int (*ANnModelEncryption_setOperandSymmPerChannelQuantParams)(
        ANnModelEncryptor *encryptor, uint32_t index,
        const ANeuralNetworksSymmPerChannelQuantParams *channelQuant);

  int (*ANnModelEncryption_setOperandExtensionData)(ANnModelEncryptor *encryptor,
