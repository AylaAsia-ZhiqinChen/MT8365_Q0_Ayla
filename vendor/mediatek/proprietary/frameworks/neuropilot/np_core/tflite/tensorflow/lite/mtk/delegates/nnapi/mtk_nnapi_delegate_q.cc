...
#define RETURN_TFLITE_ERROR_IF_ION_ERROR(code)                \
  do {                                                        \
    const auto _code = (code);                                \
    if (_code != 0) {                                         \
      TFLITE_MTK_LOG_ERROR(MTK_ENCRYPT_PRINT("Ion lock/unlock fail %d", _code)); \
      return kTfLiteError;                                    \
    }                                                         \
  } while (0)
...
NNAPIDelegateKernel::MappingFn NNAPIDelegateKernel::Map(
    ...
    TFLITE_MTK_LOG_INFO(MTK_ENCRYPT_PRINT("Map OP:%d, version:%d", builtin_code, version));
  switch (builtin_code) {
    case kTfLiteBuiltinAdd:
      if (version <= 2) {
    ...
    case kTfLiteBuiltinCustom:
      if (registration->custom_name == nullptr) {
        TFLITE_MTK_LOG_ERROR(MTK_ENCRYPT_PRINT(
      ...
      if (strcmp(registration->custom_name, "MTK_OPT") == 0) {
        ...
        if (strcmp(registration->custom_name, "MTK_DEPTH_TO_SPACE") == 0) {
          ...
          if(tflite::mtk::CustomOpHelper::GetInstance().GetParamFunc(registration->custom_name)
                  == nullptr) {
              return nullptr;
          }
          ...
          return [](const NNAPIOpMappingArgs& mapping_args)
            ...
            uint8_t* oem_scalar = nullptr;
            const char* target =
                tflite::mtk::CustomOpHelper::GetInstance().FindTarget(mapping_args.registration->custom_name);
            if (target != nullptr) {
    ...
    if((cache_dir = context->cache_dir)) {
        if(auto buffers = reinterpret_cast<const flatbuffers::Vector<flatbuffers::Offset<Buffer>>*>
           (context->buffers)) {
            ...
            buffers_for_token.push_back((uint8_t)0);
...
TfLiteStatus NNAPIDelegateKernel::AddOpsAndTensors(TfLiteContext* context) {
  ...
  for (auto node_index : nodes_) {
        ...
        while (it != fusion_mtk_op_list_.end()) {
          ...
          for (auto j = 0; j < node->inputs->size; j++) {
            if (it->outputs->data[0] == node->inputs->data[j]) {
              TFLITE_MTK_LOG_INFO(MTK_ENCRYPT_PRINT(
    ...
    RETURN_TFLITE_ERROR_IF_NN_ERROR(
        context, nnapi_->ANeuralNetworks_getDeviceCount(&device_count));
    if (device_count > 1 || tflite::mtk::IsForceUseNnApi()) {
