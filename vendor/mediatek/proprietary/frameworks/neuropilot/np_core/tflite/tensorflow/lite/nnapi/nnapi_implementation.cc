...
const NnApi LoadNnApi() {
  ...
  // M: Add Encryption API @{
  void* libneuralnetworks_encrypt = nullptr;
  libneuralnetworks_encrypt = dlopen("libnnmodelencryption.so", RTLD_LAZY | RTLD_LOCAL);
  if (libneuralnetworks_encrypt != nullptr) {
      NNAPI_LOG("encryption library %s open", "libneuralnetworks_encrypt.so");
      LOAD_FUNCTION(libneuralnetworks_encrypt, ANnModelEncryption_create);
      LOAD_FUNCTION(libneuralnetworks_encrypt, ANnModelEncryption_addOperand);
      LOAD_FUNCTION(libneuralnetworks_encrypt, ANnModelEncryption_getExtensionOperationType);
      LOAD_FUNCTION(libneuralnetworks_encrypt, ANnModelEncryption_getExtensionOperandType);
      LOAD_FUNCTION(libneuralnetworks_encrypt, ANnModelEncryption_setOperandValueFromFd);
      LOAD_FUNCTION(libneuralnetworks_encrypt, ANnModelEncryption_setOperandValue);
      LOAD_FUNCTION(libneuralnetworks_encrypt, ANnModelEncryption_addOperation);
      LOAD_FUNCTION(libneuralnetworks_encrypt, ANnModelEncryption_identifyInputsAndOutputs);
      LOAD_FUNCTION(libneuralnetworks_encrypt, ANnModelEncryption_relaxComputationFloat32toFloat16);
      LOAD_FUNCTION(libneuralnetworks_encrypt, ANnModelEncryption_finish);
      LOAD_FUNCTION(libneuralnetworks_encrypt, ANnModelEncryption_delete);
      LOAD_FUNCTION(libneuralnetworks_encrypt, ANnModelEncryption_setDebugFlag);
      LOAD_FUNCTION(libneuralnetworks_encrypt,
                    ANnModelEncryption_setOperandSymmPerChannelQuantParams);
      LOAD_FUNCTION(libneuralnetworks_encrypt, ANnModelEncryption_setOperandExtensionData);
  }
  // M: @}
