...
public class MTKContactObserver {
    ...
    private static HashMap<Long, DirtyContactEvent> sPreDirtyContactMap =
        new HashMap<Long, DirtyContactEvent>();
    ...
    public String toString() {
        // TODO Auto-generated method stub
        String item = action[eventType] + "_ID" + _ID + "CID" + CID + "dt"
            + dirty + "dl" + delete + "v" + version;
        return item;
    }
}
    void deInitState() {
...
*/
    ...
    private void loadCurrentDirtyContact() {
        mCursor = queryDirtyRawContact(null, null);
        if (mCursor == null) {
            Log.d(TAG, "loadCurrentDirtyContact mCursor is null!");
            return;
        }
        ArrayList<ContactListener> oldListener;
        try {
            if (mCursor.getCount() < sPreDirtyContactMap.size()) {
                ...
                Log.d(TAG, "loadCurrentDirtyContact1---");
                synchronized (mListeners) {
                    oldListener = (ArrayList<ContactListener>) mListeners.clone();
                }

                for (ContactListener l : oldListener) {
            ...
            while (mCursor.moveToNext()) {
                 DirtyContactEvent e = getDirtyContactItem(mCursor);
                 if (e != null && !mPreDirtyContactSet.contains(e)) {
                     mPreDirtyContactSet.add(e);
                     mDirtyContactSet.add(e);
                     sPreDirtyContactMap.put(e._ID, e);
                     Log.d(TAG, e.toString());
    ...
    DirtyContactEvent getDirtyContactItem(Cursor c) {
        ...
        try {
            ...
            e = new DirtyContactEvent(_ID, contactID , dirty , delete , version);
        } catch (IllegalStateException ex) {
            Log.d(TAG, "getDirtyContactItem meets IllegalStateException->" + ex);
            e = null;
        }
        return e;
    }
