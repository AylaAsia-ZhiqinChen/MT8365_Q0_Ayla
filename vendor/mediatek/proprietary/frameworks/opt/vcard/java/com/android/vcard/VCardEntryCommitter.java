...
public class VCardEntryCommitter implements VCardEntryHandler {
    ...
    private static final int MAX_BYTE = 1024 * 512;
    ...
    public void onEntryCreated(final VCardEntry vcardEntry) {
        ...
        mTotalPhotoSize += getPhotoSize(vcardEntry);

        if (mOperationList != null &&
                (mOperationList.size() >= 400 || mTotalPhotoSize >= MAX_BYTE)) {
            mCreatedUris.add(pushIntoContentResolver(mOperationList));
            mCounter = 0;
            mTotalPhotoSize = 0;
            mOperationList = null;
    ...
    private int getPhotoSize(final VCardEntry vcardEntry) {
        List<PhotoData> photoList = vcardEntry.getPhotoList();
        if (photoList == null) {
            return 0;
        }
        int photosize = 0;
        for (PhotoData data : photoList) {
            if (!data.isEmpty()) {
                photosize += data.getBytes().length;
            }
        }
        return photosize;
    }
