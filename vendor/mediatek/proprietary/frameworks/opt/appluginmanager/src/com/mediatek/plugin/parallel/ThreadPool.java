...
public class ThreadPool {
    ...
    public static final int PARALLEL_THREAD_NUM =
            PARALLEL_THREAD_NUM_MIN > (CPU_CORES_NUM / 2) ? PARALLEL_THREAD_NUM_MIN
                    : (CPU_CORES_NUM / 2);
    ...
    private static class ResourceCounter {
        ...
        public ResourceCounter(int v) {
            Log.d(TAG, "<ResourceCounter.ResourceCounter> ResourceCounter max thread num " + v
                    + " CPU Core " + CPU_CORES_NUM);
            value = v;
        }
    }
    ...
    public ThreadPool(int initPoolSize, int maxPoolSize) {
        mExecutor =
                new ThreadPoolExecutor(initPoolSize, maxPoolSize, KEEP_ALIVE_TIME,
                        TimeUnit.SECONDS, new LinkedBlockingQueue<Runnable>(),
                        new PriorityThreadFactory("plugin-thread",
    ...
    private class Worker<T> implements Runnable, Future<T>, JobContext {
        ...
        public void run() {
            ...
            if (setMode(MODE_CPU)) {
                try {
                    Log.d(TAG, "<Worker.run> ThreadPool job begin:" + mJob);
                    result = mJob.run(this);
                    Log.d(TAG, "<Worker.run> ThreadPool job end:" + mJob);
                } catch (Throwable ex) {
                    Log.w(TAG, "<Worker.run> Exception in running a job", ex);
                }
            }
        ...
        public synchronized T get() {
            while (!mIsDone) {
                try {
                    ...
                    Log.w(TAG, "<get> ingore exception", ex);
                    // ignore.
                }
            }
            return mResult;
