...
public interface ISmartDataSwitchAssistantOpExt {
    ...
    boolean onServiceStateChanged(int phoneId);

    void onHandoverToWifi();

    void onHandoverToCellular();

    boolean isNeedSwitchCallType(int callType);

    boolean preCheckByCallStateExt(Intent intent, boolean result);

    boolean isSmartDataSwtichAllowed();

    boolean checkIsSwitchAvailable(int phoneId);
}