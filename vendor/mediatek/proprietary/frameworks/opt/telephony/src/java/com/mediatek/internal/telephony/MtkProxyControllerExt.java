...
/**
 * MtkProxyControllerExt is a implements for IMtkProxyControllerExt.
 * It will implements the operator related features.
 */
public class MtkProxyControllerExt implements IMtkProxyControllerExt  {
    ...
    /**
     * Check if need to sim switch based on opertor spec.
     * @return true if sim switch is needed.
     */
    public boolean isNeedSimSwitch(int majorPhoneId, int phoneNum) {
        log("OMisNeedSimSwitch, majorPhoneId = " + majorPhoneId);
        return !RadioCapabilitySwitchUtil.isSkipCapabilitySwitch(majorPhoneId, phoneNum, mContext);
    }
}