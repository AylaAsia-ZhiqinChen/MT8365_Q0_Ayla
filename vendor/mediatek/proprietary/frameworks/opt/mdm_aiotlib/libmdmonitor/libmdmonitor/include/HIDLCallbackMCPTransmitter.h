...
#ifndef __HIDL_CALLBACK_MCP_TRANSMITTER_H__
#define __HIDL_CALLBACK_MCP_TRANSMITTER_H__
#include <unistd.h>
#include <pthread.h>
 ...
 * @see Transmitter
 */
class HIDLCallbackMCPTransmitter : public Transmitter
{
public:
    /**
     * @brief Crate @c HIDLMCPTransmitter instance that with HIDL callback function.
     *
     * @param callback [IN] The callback HIDL function.
     */
    HIDLCallbackMCPTransmitter(const android::sp<IMDMonitorClientCallback>& callback);

    /**
     * @brief Cleanup.
     */
    virtual ~HIDLCallbackMCPTransmitter();
    ...
    class HIDLCallbackMCPTransmitterRecipient : public android::hardware::hidl_death_recipient
    {
    public:
        HIDLCallbackMCPTransmitterRecipient(HIDLCallbackMCPTransmitter *transmitter):mTransmitter(transmitter){}
    ...
    private:
        HIDLCallbackMCPTransmitter *mTransmitter;
    };

protected:    

    // HIDL Hanlder
    android::sp<IMDMonitorClientCallback> m_clientscallback;
    Mutex mLock;  // mutex lock for m_clientscallback
    android::sp<HIDLCallbackMCPTransmitterRecipient> m_deathRecipient;
