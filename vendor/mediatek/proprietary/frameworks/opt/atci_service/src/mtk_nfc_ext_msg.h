...
typedef struct nfc_tag_raw_com_request {
    ...
    /*Length of written data. Unit is byte. The data is stored in the Meta peer buffer*/
    unsigned int    length;
...
typedef struct nfc_card_emulation_request {
    unsigned int   type;           /*Test type: currently, always set 1*/
    unsigned int   action;         /*Test action, 1: start test, 0:stop test*/
    /*bitmask: MifareUL=bit 0, MifareStd=bit1, ISO14443_4A=bit 2,
    ISO14443_4B=bit 3, Jewel=bit 4, Felica=bit 5, ISO15693=bit 6 */
    unsigned short  technology;
    /*bitmask: Iso14443A=bit 0, Iso14443B=bit 1, Felica212=bit 2,
    Felica424=bit 3, Iso15693=bit 4 */
