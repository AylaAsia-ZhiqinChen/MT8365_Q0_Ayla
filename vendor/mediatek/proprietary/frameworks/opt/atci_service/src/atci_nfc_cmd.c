    ...
    while ( count < 800)
    {
        ...
        if (rec_bytes > 0) {
            ...
            if ((pnfc_msg_ptr->msg_type == MTK_NFC_FM_SWP_TEST_RSP)
                    && (type == MTK_NFC_FM_SWP_TEST_RSP)) {
                s_mtk_nfc_fm_swp_test_rsp* pRspData = NULL;

                pRspData = (s_mtk_nfc_fm_swp_test_rsp*)
                        ((unsigned char*)pnfc_msg_ptr + sizeof(s_mtk_nfc_main_msg));
                ...
                break;
            } else if ((pnfc_msg_ptr->msg_type == MTK_NFC_FM_READ_DEP_TEST_RSP)
                    && (type == MTK_NFC_FM_READ_DEP_TEST_RSP)) {
                s_mtk_nfc_em_als_readerm_opt_rsp* pRspData = NULL;

                pRspData = (s_mtk_nfc_em_als_readerm_opt_rsp*)
                        ((unsigned char*)pnfc_msg_ptr + sizeof(s_mtk_nfc_main_msg));
                ...
                break;
            } else if ((pnfc_msg_ptr->msg_type == MTK_NFC_FM_CARD_MODE_TEST_RSP)
                    && (type == MTK_NFC_FM_CARD_MODE_TEST_RSP)) {
                s_mtk_nfc_em_als_cardm_rsp* pRspData = NULL;

                pRspData = (s_mtk_nfc_em_als_cardm_rsp*)
                        ((unsigned char*)pnfc_msg_ptr + sizeof(s_mtk_nfc_main_msg));
                ...
                break;
            } else  if ((pnfc_msg_ptr->msg_type == MTK_NFC_FM_VIRTUAL_CARD_RSP)
                    && (type == MTK_NFC_FM_VIRTUAL_CARD_RSP)) {
                s_mtk_nfc_em_virtual_card_rsp* pRspData = NULL;

                pRspData = (s_mtk_nfc_em_virtual_card_rsp*)
                        ((unsigned char*)pnfc_msg_ptr + sizeof(s_mtk_nfc_main_msg));
...
int ATCMD_NFC_6605_OP(unsigned int  type)
{
    ...
    switch (type)
    {
        case MTK_NFC_FM_SWP_TEST_REQ:
        {
            ...
            pReqData = (s_mtk_nfc_fm_swp_test_req*)
                    ((unsigned char*)pnfc_msg + sizeof(s_mtk_nfc_main_msg));
        ...
        case MTK_NFC_FM_READ_DEP_TEST_REQ:
        {
            ...
            pReqData = (s_mtk_nfc_em_als_readerm_req*)
                    ((unsigned char*)pnfc_msg + sizeof(s_mtk_nfc_main_msg));

            if (pReqData != NULL) {
                pReqData->action = 0x00;
                pReqData->supporttype = EM_ALS_READER_M_TYPE_A;
                pReqData->typeA_datarate = (EM_ALS_READER_M_SPDRATE_106
                        + EM_ALS_READER_M_SPDRATE_212 + EM_ALS_READER_M_SPDRATE_424
                        + EM_ALS_READER_M_SPDRATE_848);
        ...
        case MTK_NFC_FM_CARD_MODE_TEST_REQ:
        {
            ...
            pReqData = (s_mtk_nfc_em_als_cardm_req*)
                    ((unsigned char*)pnfc_msg + sizeof(s_mtk_nfc_main_msg));
        ...
        case MTK_NFC_FM_VIRTUAL_CARD_REQ:
        {
            ...
            pReqData = (s_mtk_nfc_em_virtual_card_req*)
                    ((unsigned char*)pnfc_msg + sizeof(s_mtk_nfc_main_msg));

            if (pReqData != NULL) {
                pReqData->action = 0x00;
                pReqData->supporttype = (EM_ALS_READER_M_TYPE_A
                        + EM_ALS_READER_M_TYPE_B  + EM_ALS_READER_M_TYPE_F);
                pReqData->typeF_datarate = (EM_ALS_READER_M_SPDRATE_212
...
void ATCMD_NFC_Read_CNF(void)
{
    ...
    while(retry_count < 5) {
        ...
        if (rec_bytes > 0) {
            // check msg id
            ALOGI("ATCMD_NFC_Read_CNF:NFC read (msg_id,dest_mod_id) = (%d,%d)",
                    nfc_ilm_rec.msg_id, nfc_ilm_rec.dest_mod_id);

            if ((nfc_ilm_rec.msg_id == MSG_ID_NFC_TEST_RSP)
                    && (nfc_ilm_rec.dest_mod_id == MOD_NFC_APP)) {
                nfc_msg_length = sizeof(nfc_msg_struct);
                memcpy( &nfc_msg, (nfc_msg_struct*)nfc_ilm_rec.local_para_ptr, nfc_msg_length);
                ALOGI("ATCMD_NFC_Read_CNF:NFC read msg_type=%d,length=%d",
                        nfc_msg.msg_type,nfc_msg_length);
                switch (nfc_msg.msg_type) {
                    case MSG_ID_NFC_SWP_SELF_TEST_RSP:
                    {
                        nfc_cnf.op = NFC_OP_SWP_SELF_TEST;
                        memcpy(&nfc_cnf.result.m_script_cnf,
                            (nfc_script_response*)(nfc_ilm_rec.local_para_ptr + nfc_msg_length),
                            sizeof(nfc_script_response));
...
int NFC_Socket_Open(void)
{
    ...
    if(pthread_create(&read_cnf_thread_handle, NULL, META_NFC_read_cnf, NULL) != 0) {
...
void ATCMD_NFC_OP(NFC_REQ *req)
{
    ...
    switch(req->op)
    {
        ...
        case NFC_OP_SWP_SELF_TEST:
        {
            ...
            ALOGI("META_NFC_OP:NFC msg_type,msg_length = (%d,%d)", nfc_msg.msg_type,
                nfc_msg.msg_length);
            ...
            memcpy((nfc_ilm_loc.local_para_ptr + sizeof(nfc_msg_struct)),
                &req->cmd.m_script_req, sizeof(nfc_script_request));
        ...
        case NFC_OP_TAG_UID_RW:
        {
            ...
            memcpy(nfc_ilm_loc.local_para_ptr, (char*)&nfc_msg, sizeof(nfc_msg_struct));
            memcpy((nfc_ilm_loc.local_para_ptr + sizeof(nfc_msg_struct)),
                &req->cmd.m_script_uid_req, sizeof(nfc_script_uid_request));
            AT_NFC_CMD(&nfc_ilm_loc);
            ATCMD_NFC_Read_CNF();
            break;
        }
