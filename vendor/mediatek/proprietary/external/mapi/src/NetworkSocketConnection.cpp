...
#undef TAG
#define TAG "MAPI-NetworkSocketConnection"
...
#define READ_RETRY_TIMES 5
    MDI_LOGD(TAG, "NetworkSocketConnection::Read");
        ...
        if (0 > (readBytes = read(m_fd, buffer, bufferSize))) {
            if (errno == EAGAIN || errno == EINTR) {
                if (retry == READ_RETRY_TIMES) {
                    MDI_LOGE(TAG, "No data to read from fd: [%d], retry [%d] times when err = [%s](%d)", m_fd, retry, strerror(errno), errno);
                    return 0;
                }
                ++retry;
                continue;
            ...
            MDI_LOGE(TAG, "Socket is closed. fd: [%d]", m_fd);
            m_isValid = false;
            return -1;
        }
        MDI_LOGE(TAG, "NetworkSocketConnection::Read done! read [%zd] bytes.", readBytes);
        return readBytes;
    }
}
...
bool NetworkSocketConnection::Write(const uint8_t *data, size_t len, volatile const bool *bStopFlag) {
    ...
    MDI_LOGD(TAG, "NetworkSocketConnection::Write to fd: [%d] size=[%zu]", m_fd, len);
    ...
    MDI_LOGI(TAG, "NetworkSocketConnection::Send to fd: [%d] size=[%zu] successfully.", m_fd, len);
    return true;
}
