...
static const char *TAG = "MAPI-SocketConnection";
...
SocketConnection::~SocketConnection() {
    MDI_LOGD(TAG, "~SocketConnection fd = [%d]", m_fd);
    Reset();
}
...
bool SocketConnection::Connect(void *host) {
    ...
    MDI_LOGD(TAG, "Connect");

    if (NULL == host) {
        MDI_LOGE(TAG, "Input arg is NULL");
        return false;
    }
    Reset();
...
#define READ_RETRY_TIMES 5
    MDI_LOGD(TAG, "SocketConnection::Read");

    if (NULL == buffer || 0 == bufferSize) {
        MDI_LOGE(TAG, "Bad parameters. buffer = [%p], bufferSize = [%zu]", buffer, bufferSize);
        return -2;
    }
        ...
        if (0 > (readBytes = read(m_fd, buffer, bufferSize))) {
            if (errno == EAGAIN || errno == EINTR) {
                if (retry == READ_RETRY_TIMES) {
                    MDI_LOGD(TAG, "No data to read from fd: [%d], retry [%d] times when err = [%s](%d)",
            ...
            MDI_LOGE(TAG, "Failed to read from fd: [%d], err = [%s](%d)", m_fd, strerror(errno), errno);
            return -1;
        } else if (0 == readBytes) {
            MDI_LOGD(TAG, "Socket is closed. fd: [%d]", m_fd);
            m_isValid = false;
            return -1;
        }
        MDI_LOGD(TAG, "Read done! read [%zd] bytes.", readBytes);
        return readBytes;
    }
}
...
bool SocketConnection::Write(const uint8_t *data, size_t len, volatile const bool *bStopFlag) {
    ...
    MDI_LOGD(TAG, "Write to fd: [%d] size=[%zu]", m_fd, len);
    ...
    MDI_LOGD(TAG, "write to fd: [%d]", m_fd);
    ...
    while (totalBytes < len) {
        ...
        if (0 > (writtenBytes = write(m_fd, data + totalBytes, len - totalBytes))) {
            ...
            MDI_LOGE(TAG, "Failed to write to fd: [%d], err = [%s](%d)", m_fd, strerror(errno), errno);
            m_isValid = false;
            return false;
        }
    ...
    MDI_LOGD(TAG, "Send to fd: [%d] size=[%zu] successfully.", m_fd, len);
    return true;
}

bool SocketConnection::CheckValid() {
    if (!IsValid()) {
        MDI_LOGE(TAG, "socket invalid. fd = %d, err = %s", m_fd, strerror(errno));
        return false;
    }
    struct pollfd event;
    ...
    if (ret_poll == 0) {
        MDI_LOGE(TAG, "no event happen");
        return true;
    }
    if (ret_poll < 0) {
        if (errno == EAGAIN || errno == EINTR) {
            MDI_LOGD(TAG, "socket valid, just no data to read from fd = %d, err = %s",
                    m_fd, strerror(errno));
            return true;
        }
        MDI_LOGE(TAG, "poll error, fd = %d, err = %s", m_fd, strerror(errno));
        m_isValid = false;
        return false;
    }
    if (event.revents & POLLHUP || event.revents & POLLNVAL || event.revents & POLLERR) {
        MDI_LOGE(TAG, "socket invalid! revents = 0x%x", event.revents);
        m_isValid = false;
        return false;
    }
    MDI_LOGE(TAG, "socket valid! revents = 0x%x", event.revents);
    return true;
}
...
bool SocketConnection::ReadToBuffer() {
    ...
    MDI_LOGD(TAG, "ReadToBuffer. There're [%zu] bytes of data in dataQ.", m_dataQ.Size());

    if (0 > (readBytes = Read(m_dataQ.SpaceStart(), m_dataQ.SpaceSize()))) {
        MDI_LOGD(TAG, "Failed to read from connection");
        return false;
    }
    m_dataQ.IncSize(readBytes);

    MDI_LOGD(TAG, "ReadToBuffer done! read [%zd] bytes.", readBytes);
    return true;
}
