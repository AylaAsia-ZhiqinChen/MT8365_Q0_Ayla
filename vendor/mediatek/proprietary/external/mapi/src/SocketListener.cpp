...
static const char *TAG = "MAPI-SocketListener";
...
SocketListener::SocketListener(const char *szServerName):
    ...
    if (!SetupDomainSocket(szServerName)) {
        MDI_LOGE(TAG, "Fail to setup domain socket.");
        Cleanup();
        return;
    }
    if (pthread_create(&m_threadId, NULL, SocketListener::ListenThread, this)) {
        MDI_LOGE(TAG, "Fail to create MonitorCmdService's listener thread, error=%s", strerror(errno));
        Cleanup();
    }
    if (0 != pthread_setname_np(m_threadId, "mdm_cmd_serv")) {
        MDI_LOGE(TAG, "Fail to set name for MonitorCmdService's listener thread, error=%s",
                strerror(errno));
    }
}
...
bool SocketListener::SetupDomainSocket(const char *szServerName) {
    ...
    if (NULL == szServerName) {
        MDI_LOGE(TAG, "szServerName is NULL");
        return false;
    }

    MDI_LOGD(TAG, "SetupDomainSocket [%s]", szServerName);

    if (0 > (fd = socket( AF_LOCAL, SOCK_STREAM | SOCK_NONBLOCK, 0 ))) {
        MDI_LOGE(TAG, "Failed to create domain socket.");
        return false;
    }
    MDI_LOGD(TAG, "Domain sockect was created. fd = [%d]", fd);
    ...
    do {
        ...
        if (fd < 0) {
            MDI_LOGE(TAG, "Failed to check donmain socket");
            break;
        }

        if (0 > bind(fd, (struct sockaddr *)&servaddr, addrLen)) {
            MDI_LOGE(TAG, "Failed to bind domain socket to [%s]. err = [%s](%d)",
                    szServerName, strerror(errno), errno);
            break;
        }

        if (0 < listen(fd, 4)) {
            MDI_LOGE(TAG, "Failed to listen domain socket, fd = [%d]. err = [%s](%d)",
                    fd, strerror(errno), errno);
            break;
        }

        AddSocket(SOCKET_TYPE_SERVER, fd, NULL);
        MDI_LOGD(TAG, "SetupDomainSocket done! Start to listen domain sockect, fd =[%d], file = [%s].",
    ...
    MDI_LOGE(TAG, "SetupDomainSocket failed.");
    return false;
}

void *SocketListener::ListenThread(void *obj) {
    MDI_LOGD(TAG, "ListenThread Loop");
...
void SocketListener::Listen() {
    ...
    while (!m_notifyListenThreadStop) {
        ...
        MDI_LOGD(TAG, "Select done! num of fd = [%d]. max fd = [%d].", rc, m_maxFD);
        ...
        for (socketItr = m_socket[SOCKET_TYPE_SERVER].begin(); socketItr != m_socket[SOCKET_TYPE_SERVER].end(); ++socketItr) {
            ...
            MDI_LOGD(TAG, "Begins to accept. fd = [%d]", serverFd);
            ...
            MDI_LOGD(TAG, "Accept a client. fd = [%d]", connectfd);
            AddSocket(SOCKET_TYPE_CLIENT, connectfd, szAddr);
            if (m_notifyListenThreadStop) {
                return;
            }
            MDI_LOGD(TAG, "Accepts one client, fd = [%d] addr = [%s]", connectfd, addr.sun_path+1);
        }
        ...
        for (socketItr = m_socket[SOCKET_TYPE_CLIENT].begin(); socketItr != m_socket[SOCKET_TYPE_CLIENT].end(); ++socketItr) {
            ...
            MDI_LOGD(TAG, "Start to read command from fd = [%d] addr = [%s]", clientFd, clientConn.GetAddr());
            if (m_notifyListenThreadStop) {
                return;
            }
            /**
