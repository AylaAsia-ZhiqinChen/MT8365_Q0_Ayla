...
#undef TAG
#define TAG "MAPI-CommandProcessor"
...
int CommandProcessor::ParseCommand(DataQueue &dataQ) {
    ...
    MDI_LOGD(TAG, "CommandProcessor::ParseCommand");
    ...
    while (NULL != dataQ.Front() && dataQ.Size() >= sizeof(REDIRECTOR_CMD_HEADER_MAGIC) + 2 + 4) {
        if (0 != memcmp(dataQ.Front(), REDIRECTOR_CMD_HEADER_MAGIC, sizeof(REDIRECTOR_CMD_HEADER_MAGIC))) {
            ...
            MDI_LOGE(TAG, "Not magic header. Pop(1).");
            dataQ.Pop(1);
            continue;
        }
        MDI_LOGD(TAG, "Magic header is found.");

        /* Check CMD_CODE */
        const uint16_t *pCmdCode = (const uint16_t *)(dataQ.Front() + sizeof(REDIRECTOR_CMD_HEADER_MAGIC));
        if (NULL == pCmdCode) {
            MDI_LOGE(TAG, "Incorrent command code. Pop(4).");
            dataQ.Pop(4);
            continue;
        }
        MDI_LOGD(TAG, "Command code is found. [%" PRIu16 "]", *pCmdCode);
        ...
        if (NULL == pArgsLen || COMMAND_ARGS_LEN_MAX < *pArgsLen) {
            MDI_LOGE(TAG, "Incorrent command args len. Pop(4).");
            dataQ.Pop(4);
            continue;
        }
        MDI_LOGD(TAG, "Argument length is found. [%" PRIu32 "]", *pArgsLen);
        ...
        if (i == mCmdTableSize) {
            ...
            MDI_LOGE(TAG, "No such command code [%" PRIu16 "]. Pop(4).", *pCmdCode);
            dataQ.Pop(4);
            continue;
        }
