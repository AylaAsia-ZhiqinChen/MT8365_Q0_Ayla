...
#define BOOT_REASON_SYS_PROPERTY "vendor.sys.boot.reason"
#define BOOT_PACKAGE_SYS_PROPERTY "persist.vendor.sys.bootpackage"
...
int update_boot_reason() {
    ...
    if (fd < 0) {
        ...
        if(s <= 0) {
           ...
           boot_reason = '0';
            ...
            // ALARM_BOOT = 7
            SLOGD("[boot_logo_updater]boot_mode = %c\n", boot_mode);
            if ( boot_mode == '7' ) {
                ...
                SLOGE("[boot_logo_updater]boot package = %d\n", package);
                if ( package != 1 ) {
    ...
    SLOGE("[boot_logo_updater]update boot reason = %s, ret = %d\n", propVal, ret);
...
int write_to_file(const char* path, const char* buf, int size) {
    ...
    if (!path) {
        SLOGE("[boot_logo_updater]path is null!\n");
        return 0;
    }

    int fd = open(path, O_RDWR);
    if (fd == -1) {
        SLOGE("[boot_logo_updater]Could not open '%s'\n", path);
        close(fd);
        return 0;
    }
    ...
    if (count != size) {
        SLOGE("[boot_logo_updater]write file (%s) fail, count: %d\n", path, count);
        close(fd);
        return 0;
    }
    close(fd);
