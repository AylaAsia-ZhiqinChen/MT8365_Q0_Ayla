	...
	while (1) {
		...
		case 'S':
			...
			if (stat(context, &context_st))
				return sys_err_msg("bad file context '%s'", context);
			break;
		}
	}
...
static void prepare_node(void *node, int len)
{
	...
	crc = mtd_crc32(UBIFS_CRC32_INIT, (char *)node + 8, len - 8);
...
static int do_pad(void *buf, int len)
{
	...
	memset((char *)buf + len, 0xff, alen - len);
	pad_len = wlen - alen;
	...
	if (pad_len >= (int)UBIFS_PAD_NODE_SZ) {
		...
		crc = mtd_crc32(UBIFS_CRC32_INIT, (char *)buf + 8,
				  UBIFS_PAD_NODE_SZ - 8);
		ch->crc = cpu_to_le32(crc);

		memset((char *)buf + UBIFS_PAD_NODE_SZ, 0, pad_len);
	} else if (pad_len > 0)
			       ...
			       unsigned int data_len, int flags)
{
	...
	if (S_ISLNK(st->st_mode)) {
		ino->compr_type = cpu_to_le16(UBIFS_COMPR_NONE);
		ino->xattr_cnt = 1;
		ino->xattr_size += CALC_DENT_SIZE(strlen(XATTR_NAME_SELINUX));
		ino->xattr_size += CALC_XATTR_BYTES(context_len);
		ino->xattr_names += strlen(XATTR_NAME_SELINUX);
	}
...
static int add_dent_node(ino_t dir_inum, const char *name, ino_t inum,
			 unsigned char type);
static int add_xattr(struct stat *st, ino_t inum, char *var_name,
...
static int add_inode_with_xattr(struct stat *st, ino_t inum, 
				int flags, uint64_t capabilities, char *secontext, int context_len)
{
	struct ubifs_ino_node *ino = node_buf;
	union ubifs_key key;
	int use_flags = 0, ret;
	...
	int len, data_len = sizeof(cap_data);

	memset(&cap_data, 0, data_len);
	...
	if(capabilities > 0) {
		ino->xattr_cnt++;
		ino->xattr_size += CALC_DENT_SIZE(strlen(XATTR_NAME_CAPS));
		ino->xattr_size += CALC_XATTR_BYTES(data_len);
		ino->xattr_names += strlen(XATTR_NAME_CAPS);
	}

	len = UBIFS_INO_NODE_SZ;
 
	ret = add_node(&key, NULL, ino, len) ;
	if (ret) {
 	...
 	if(capabilities > 0) {
		ret = add_xattr(st, inum, XATTR_NAME_CAPS, data, data_len);
		if(ret)
			printf("new cap xattr add failed\n");
	}
	ret = add_xattr(st, inum, XATTR_NAME_SELINUX, secontext, context_len);
	if(ret)
		printf("new selinux xattr add failed\n");

	return ret;
}

static int add_xattr(struct stat *st, ino_t inum, char *var_name, char *data, int data_len)
{
	struct ubifs_ino_node *ino = node_buf;
	...
	ino->uid        = cpu_to_le32(st->st_uid);
	ino->gid        = cpu_to_le32(st->st_gid);
	ino->mode       = cpu_to_le32((st->st_mode & (~S_IFMT)) | (S_IFREG));
...
static int add_dir_inode(DIR *dir, ino_t inum, loff_t size, unsigned int nlink,
			 struct stat *st, const char *path_name, uint64_t capabilities)
{
  ...
  if ( context ) {
     char *sepath = NULL;
     if (asprintf(&sepath, "/%s", &path_name[source_path_len]) < 0)
                                      sepath = NULL;
	...
	return add_inode_with_xattr(st, inum, flags, capabilities, secontext, con_size);
}
			     ...
			     int flags)
{
	...
	if ( context ) {
	 ...
	 context_len = strlen(secontext)+1;
  } else {
	 secontext = NULL;
	 context_len = 0;
  }
  ...
  add_xattr(st, inum, XATTR_NAME_SELINUX, secontext, context_len);
		    ...
		    int flags, uint64_t capabilities)
{
	...
	do {
		...
		compr_type = compress_data(buf, bytes_read, &dn->data,
					   &out_len, use_compr);
		c->org_size += bytes_read;
		c->compr_size += out_len;
			 ...
			 int non_existing, uint64_t capabilities)
{
	...
	err = add_dir_inode(dir, dir_inum, size, nlink, st, dir_name, capabilities);
	if (err)
		goto out_free;
...
static int finalize_leb_cnt(void)
{
	...
	if (verbose) {
		...
		printf("\tleb_cnt:      %d\n", c->leb_cnt);
		printf("\torg_size:     %ld\n", c->org_size);
		printf("\tcompr_size:   %ld\n", c->compr_size);
		printf("\tcompr radio:  %0.2f\n", (c->compr_size*1.0)/c->org_size);
	}
