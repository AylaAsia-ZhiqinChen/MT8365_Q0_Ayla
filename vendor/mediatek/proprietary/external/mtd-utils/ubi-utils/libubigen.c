...
void ubigen_info_init(struct ubigen_info *ui, int peb_size, int min_io_size,
		      int subpage_size, int peb_count, int vid_hdr_offs, int ubi_ver,
		      uint32_t image_seq)
#else
void ubigen_info_init(struct ubigen_info *ui, int peb_size, int min_io_size,
...
#ifdef __MTK_SLC_BUFFER_SUPPORT
	ui->mtbl_size = sizeof(struct ubi_mtbl_record) + peb_count * sizeof(struct ec_map_info);
#endif
}
...
int ubigen_write_volume(const struct ubigen_info *ui,
			...
			long long bytes, int in, int lnum, int out, int *out_lnum)
{
	int len = vi->usable_leb_size, rd;
...
#ifdef __MTK_SLC_BUFFER_SUPPORT
int ubigen_write_maintain_vol(const struct ubigen_info *ui, int peb, int peb_count,
			    long long ec, int fd, struct ubigen_vol_info *avi, int num)
{
	...
	struct ubi_vid_hdr *vid_hdr;
	off_t seek;
	struct ubi_mtbl_record *ti;
	uint32_t crc;

	printf("num %d\n", num);
	crc_sz = peb_count * sizeof(struct ec_map_info);

	vi.bytes = ui->leb_size * UBI_MAINTAIN_VOLUME_EBS;
	vi.id = UBI_MAINTAIN_VOLUME_ID;
	vi.alignment = UBI_MAINTAIN_VOLUME_ALIGN;
	vi.data_pad = ui->leb_size % UBI_MAINTAIN_VOLUME_ALIGN;
	vi.usable_leb_size = ui->leb_size - vi.data_pad;
	vi.data_pad = ui->leb_size - vi.usable_leb_size;
	vi.type = UBI_MAINTAIN_VOLUME_TYPE;
	vi.name = UBI_MAINTAIN_VOLUME_NAME;
	vi.name_len = strlen(UBI_MAINTAIN_VOLUME_NAME);
	vi.compat = UBI_MAINTAIN_VOLUME_COMPAT;
	...
	vid_hdr = (struct ubi_vid_hdr *)(&outbuf[ui->vid_hdr_offs]);
	ti = (struct ubi_mtbl_record *)(&outbuf[ui->data_offs]);
	ti->magic = cpu_to_be32(UBI_MT_EBA_MAGIC);
	ti->peb_count = cpu_to_be32(peb_count);
	memset(ti->info, 0, crc_sz);
	for (i=0; i < num ; i++) {
		if(avi[i].name == NULL || avi[i].start == -1)
			continue;
		printf("Set PEB from %d to %d with vol %d\n", avi[i].start, avi[i].stop, avi[i].id);
		for(j=avi[i].start; j<avi[i].stop; j++) {
			ti->info[j].map=cpu_to_be32(1);
			ti->info[j].vol_id=cpu_to_be32(avi[i].id);
		}
	}
	crc = mtd_crc32(UBI_CRC32_INIT, &ti->info, crc_sz);
	ti->crc = cpu_to_be32(crc);

	seek = peb * ui->peb_size;
	if (lseek(fd, seek, SEEK_SET) != seek) {
		sys_errmsg("cannot seek output file");
		goto out_free;
	}
...
#else
int ubigen_write_maintain_vol(const struct ubigen_info *ui, int peb, int peb_count,
			    long long ec, int fd, struct ubigen_vol_info *vi, int num)
{
	return 0;
}
#endif
