...
uint64 OsLayer::ReadPhysicalLong(uint64 paddr) {
  int fd;
  ...
  if (fd == -1) {
    logprintf(0, "Process Error: failed to open /dev/mem\n");
    return 0xdeaddeaddeaddeadULL;
  }

  map = mmap(NULL, page_sz, PROT_READ | PROT_WRITE, MAP_SHARED, fd, ROUND_DOWN((off_t)addr, page_sz));

  if (map == MAP_FAILED) {
    logprintf(0, "Process Error: failed to mmap /dev/mem\n");
    data = 0xdeaddeaddeaddeadULL;
  } else {
    data = *((volatile uint64 *)map);
    munmap(map, page_sz);
  }
...
uint32 OsLayer::ReadPhysicalWord(uint64 paddr) {
  int fd;
  uint64 *addr = (uint64 *)paddr;
  uint32 data;
  void *map = MAP_FAILED;
