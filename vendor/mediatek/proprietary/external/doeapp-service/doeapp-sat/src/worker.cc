...
void WorkerThread::ProcessError(struct ErrorRecord *error,
  ...
  if (priority < 5) {
    ...
    logprintf(priority,
              ...
              "read phy:0x%016llx, reread phy:0x%016llx\n",
              message,
              core_id,
              CurrentCpusFormat().c_str(),
              error->vaddr,
              ...
              error->reread,
              error->expected,
              os_->ReadPhysicalLong(error->paddr),
              os_->ReadPhysicalLong(error->paddr));
  }
...
#if defined(HAVE_AEE_FEATURE)
  aee_system_warning("DoEapp:sat", NULL, DB_OPT_DEFAULT, "miscompare");
#endif
}
...
void MemoryRegionThread::ProcessError(struct ErrorRecord *error,
  ...
  if (phase_ == kPhaseCopy) {
    ...
    logprintf(0, "Process Error: memory region thread raised an "
              "unexpected error.");
#if defined(HAVE_AEE_FEATURE)
    aee_system_warning("DoEapp:sat", NULL, DB_OPT_DEFAULT, "unexpected error");
#endif
  }
}
