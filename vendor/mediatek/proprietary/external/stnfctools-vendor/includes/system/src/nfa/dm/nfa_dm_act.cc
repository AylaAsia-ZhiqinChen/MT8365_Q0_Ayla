...
static void nfa_dm_set_init_nci_params(void) {
  ...
  if (NFC_GetNCIVersion() == NCI_VERSION_2_0) {
    ...
    for (xx = 0; xx < NFA_CE_LISTEN_INFO_MAX; xx++) {
      for (yy = 4; yy < NCI_SYSTEMCODE_LEN + NCI_NFCID2_LEN; yy++) {
  ...
  */
  ...
  if (NFC_GetNCIVersion() == NCI_VERSION_2_0) {
    /* NCI 2.0 - Begin */
    /* Align core stack with CLF default values, user can set p_nfa_dm_ce_cfg[]
     * to force setting at each parameter at reset. In case of exclusive RF
     * mode, CORE_SET_CONFIG_CMD will be sent if the parameter value set in the
     * NFA API differs from the core stack internal value & if the enable bit
     * for the protocol is set in the API listen parameter
     */
