...
int do_add_mem_test(pid_t pid, uint32_t id,
    ...
    ret = UREE_RegisterSharedmemWithTag(mem_session, &shm_handle, &shm_param, __func__);
    if (ret != TZ_RESULT_SUCCESS) {
...
int do_do_A_B_cm_test(pid_t pid,
    ...
    ret = UREE_AllocSecurechunkmemWithTag(mem_session, &mem_handle, test_align, mem_size, __func__);
    if (ret != TZ_RESULT_SUCCESS) {
...
int do_do_A_B_secure_test(pid_t pid,
    ...
    ret = UREE_AllocSecurememWithTag(mem_session, &mem_handle, test_align, mem_size, __func__);
    if (ret != TZ_RESULT_SUCCESS) {
        printf("    [%d] UREE_AllocSecureMem Error: %d\n", id, (uint32_t) ret);
        error_exit_1("do_do_A_B_test");
    }
...
int do_cm_test(pid_t pid,
    ...
    for (i = 0; i < TEST_MEM_LOOP_NUM; i ++) {
        ret = UREE_AllocSecurechunkmemWithTag(mem_session, &mem_handle[i], test_align, mem_size, __func__);
        if (ret != TZ_RESULT_SUCCESS) {
...
int do_secure_test(pid_t pid,
    ...
    for (i = 0; i < TEST_MEM_LOOP_NUM; i ++) {
        ret = UREE_AllocSecurememWithTag(mem_session, &mem_handle[i], test_align, mem_size, __func__);
        if (ret != TZ_RESULT_SUCCESS) {
            printf("    [%d] UREE_AllocSecuremem Error: %d\n", id, (uint32_t) ret);
...
int do_shm_test(pid_t pid, uint32_t id, UREE_SESSION_HANDLE mem_session) {
    ...
    for (i = 0; i < loop_num; i ++) {
        ...
        shm_param.size = shm_size;
        ret = UREE_RegisterSharedmemWithTag(mem_session, &shm_handle[i], &shm_param, __func__);
        if (ret != TZ_RESULT_SUCCESS) {
            printf("    [%d] UREE_RegisterSharedmem Error: %s\n", id, TZ_GetErrorString(ret));
