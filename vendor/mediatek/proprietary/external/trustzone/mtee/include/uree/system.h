 ...
 * @param ta_uuid UUID of the TA to connect to.
 * @param pHandle Handle for the new session. Return UREE_SESSION_HANDLE_FAIL if fail.
 * @param tag the string to be printed when querying memory usage.
 * @return return code
 */
TZ_RESULT UREE_CreateSessionWithTag(const char *ta_uuid, UREE_SESSION_HANDLE *pHandle, const char *tag);
