...
{
    if (argc < 2) {
        printf("%s argv incorrect, usage: %s [read name | write name value | delete name | all]\n", argv[0], argv[0]);
        return -1;
    }
    if (!strcmp(argv[1], "write")) {
        ...
        if (sysenv_set_static(argv[2], argv[3]) < 0)
            printf("write sysenv fail\n");
