...
private:
    ...
    bool SetupProperty()
    {
        ...
        MDMI_LOGD(TAG, "%s=%s", PROP_SYS_USB_CONFIG, tmp);

        property_get(PROP_SYS_USB_IDX, tmp, "");
        MDMI_LOGD(TAG, "%s=%s", PROP_SYS_USB_IDX, tmp);
        ...
        property_get(PROP_SYS_USB_CONFIG, tmp, "");
        MDMI_LOGD(TAG, "new %s=%s", PROP_SYS_USB_CONFIG, tmp);

        property_get(PROP_SYS_USB_IDX, tmp, "");
        while((tmp[0] == 0 || strchr(tmp, '4') == NULL) && count-- > 0) {
            MDMI_LOGE(TAG, "wait for sys.usb.config to take effect");
            sleep(1);
            property_get(PROP_SYS_USB_IDX, tmp, "");
        }
        MDMI_LOGD(TAG, "new %s=%s", PROP_SYS_USB_IDX, tmp);
        if (count == 0) {
            return false;
        }
        return true;
    ...
    bool OpenComDev()
    {
        ...
        MDMI_LOGD(TAG, "ComPortManager::Open");

        Close();

        if (0 > (m_usbPortFd = open(m_vcomDevName, O_RDWR | O_NOCTTY))) {
            MDMI_LOGE(TAG, "Failed to open dev [%s]. errno = [%d](%s)", m_vcomDevName, errno, strerror(errno));
            return false;
        }

        MDMI_LOGD(TAG, "Open dev [%s] successfully.", m_vcomDevName);
    ...
    void Close()
    {
        MDMI_LOGD(TAG, "ComPortManager::Close");
        if (m_usbPortFd != -1) {
            if (0 != close(m_usbPortFd)) {
                MDMI_LOGE(TAG, "Failed to close port [%d], errno = [%d](%s)", m_usbPortFd, errno, strerror(errno));
            } else {
                MDMI_LOGD(TAG, "Port [%d] was successfully closed.", m_usbPortFd);
            }
            m_usbPortFd = -1;
        }
    }
    ...
    {
        if (!SetupProperty()) {
            MDMI_LOGE(TAG, "Failed to setup property.");
            return false;
        }
        if (!OpenComDev()) {
            MDMI_LOGE(TAG, "Failed to open COM dev [%s]", m_vcomDevName);
            return false;
        }
        return true;
    }
...
public:
    ...
    bool Write(const uint8_t *pData, size_t len)
    {
        ...
        MDMI_LOGD(TAG, "ComPortManager::Write");
        ...
        while (bytes_left > 0 && !gbTerminate) {
            ...
            if (bytes_written == -1) {
                if (errno == EINTR) {
                    MDMI_LOGE(TAG, "write failed: EINTR. Retry.");
                    continue;
                }
                if (errno == EAGAIN) {
                    MDMI_LOGE(TAG, "write failed: EAGAIN. Retry.");
                    continue;
                }
                if (errno == EBADF || errno == EPIPE) {
                    MDMI_LOGE(TAG, "write failed: EBADF or EPIPE. Retry.");
                    Open();
                    continue;
                }
                MDMI_LOGE(TAG, "write failed: errno = %d (%s), total len = [%zu], left = [%zu]", errno, strerror(errno), len, bytes_left);
                return false;
            }
        ...
        MDMI_LOGD(TAG, "Write [%zu] bytes to PC", len);
