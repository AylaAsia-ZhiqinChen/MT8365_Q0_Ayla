...
def process_apkcerts_txt(
    system_target_files_temp_dir,
    other_target_files_temp_dir,
    output_target_files_temp_dir):

  system_apkcerts_txt = os.path.join(
      system_target_files_temp_dir, 'META', 'apkcerts.txt')

  other_apkcerts_txt = os.path.join(
      other_target_files_temp_dir, 'META', 'apkcerts.txt')

  with open(system_apkcerts_txt) as f:
    system_apkcerts = f.read().splitlines()

  with open(other_apkcerts_txt) as f:
    other_apkcerts = f.read().splitlines()

  output_apkcerts = set(system_apkcerts + other_apkcerts)

  output_apkcerts_txt = os.path.join(
      output_target_files_temp_dir, 'META', 'apkcerts.txt')

  with open(output_apkcerts_txt, 'w') as output:
    for cert in sorted(output_apkcerts):
      output.write('%s\n' % cert)


def process_filesystem_config_txt(
    system_target_files_temp_dir,
    other_target_files_temp_dir,
    output_target_files_temp_dir,
    filename):

  system_filesystem_config_txt = os.path.join(
      system_target_files_temp_dir, 'META', filename)

  other_filesystem_config_txt = os.path.join(
      other_target_files_temp_dir, 'META', filename)

  system_filesystem_config = []
  if os.path.exists(system_filesystem_config_txt):
    with open(system_filesystem_config_txt) as f:
      system_filesystem_config = f.read().splitlines()

  other_filesystem_config = []
  if os.path.exists(other_filesystem_config_txt):
    with open(other_filesystem_config_txt) as f:
      other_filesystem_config = f.read().splitlines()

  output_filesystem_config = set(system_filesystem_config + other_filesystem_config)

  output_filesystem_config_txt = os.path.join(
      output_target_files_temp_dir, 'META', filename)

  with open(output_filesystem_config_txt, 'w') as output:
    for config in sorted(output_filesystem_config):
      output.write('%s\n' % config)


def process_build_prop(
    system_target_files_temp_dir,
    other_target_files_temp_dir,
    output_target_files_temp_dir,
    filename):
...
def process_special_cases(
  ...
  if True:
    process_apkcerts_txt(
        system_target_files_temp_dir=system_target_files_temp_dir,
        other_target_files_temp_dir=other_target_files_temp_dir,
        output_target_files_temp_dir=output_target_files_temp_dir)
    process_filesystem_config_txt(
        system_target_files_temp_dir=system_target_files_temp_dir,
        other_target_files_temp_dir=other_target_files_temp_dir,
        output_target_files_temp_dir=output_target_files_temp_dir,
        filename='recovery_filesystem_config.txt')
