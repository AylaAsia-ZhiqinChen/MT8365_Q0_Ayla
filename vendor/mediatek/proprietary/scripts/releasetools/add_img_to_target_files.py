...
def AVBSignBoot(output_zip, image):
  ...
  # AVB-sign the image as needed.
    ...
    part_size = OPTIONS.info_dict[image+"_size"]
    # The AVB hash footer will be replaced if already present.
    cmd = [avbtool, "add_hash_footer", "--image", img.name,
           "--partition_size", str(part_size), "--partition_name", image]
    common.AppendAVBSigningArgs(cmd, image)
    args = OPTIONS.info_dict.get("avb_boot_add_hash_footer_args")
    if args and args.strip():
      cmd.extend(shlex.split(args))
    common.RunAndCheckOutput(cmd)
  else:
    assert os.path.exists(img.input_name), 'Failed to find %s.img in %s' % (
        image, img.input_name)
...
def AddImagesToTargetFiles(filename):
  ...
  if boot_image:
    ...
    if not os.path.exists(partitions['boot']):
      ...
      banner("boot-debug")
      boot_debug_image = common.GetBootableImage(
         "IMAGES/boot-debug.img", "boot-debug.img", OPTIONS.input_tmp, "BOOT_DEBUG")
      assert boot_debug_image, "Failed to create boot-debug.img."
      boot_debug_image_path = os.path.join(OPTIONS.input_tmp, "IMAGES", "boot-debug.img")
      if not os.path.exists(boot_debug_image_path):
        boot_debug_image.WriteToDir(OPTIONS.input_tmp)
        if output_zip:
          boot_debug_image.AddToZip(output_zip)
  ...
  if OPTIONS.info_dict.get("avb_main_vbmeta_in_boot") == "true":
    ...
    partitions['boot'] = AVBSignBoot(output_zip, "boot")
    if has_recovery:
      partitions['recovery'] = AVBSignBoot(output_zip, "recovery")
