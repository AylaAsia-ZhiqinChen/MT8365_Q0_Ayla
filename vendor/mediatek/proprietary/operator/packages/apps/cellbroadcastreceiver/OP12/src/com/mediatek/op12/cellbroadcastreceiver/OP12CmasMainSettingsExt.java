...
public class OP12CmasMainSettingsExt extends DefaultCmasMainSettingsExt {
    ...
    public boolean isSpanishAlert(Context hostContext,
        ...
        Log.i(TAG, "isSpanishAlert Language code:" + languageCode +
            " msgId:" + msgId);
        if (languageCode != null && languageCode.contains("es")) {
            String langList = MtkSmsManager.getDefault().getCellBroadcastLang();
            Log.d(TAG, "updateLanguagesList langList:" + langList);
            if (langList != null) {
                return langList.contains("4");
            }
            return false;
        } else {
            return true;
