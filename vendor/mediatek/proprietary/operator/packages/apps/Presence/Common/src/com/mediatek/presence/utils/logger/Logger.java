...
package com.mediatek.presence.utils.logger;

import com.mediatek.presence.platform.logger.AndroidAppender;
...
public class Logger {
    ...
    private boolean isSlotIdSet = false;
    ...
    private Logger(int slotId, String classname) {
        this.isSlotIdSet = true;
        this.mSlotId = slotId;
        this.classname = classname;
    }
    ...
    private void printTrace(String trace, int level) {
        if ((appenders != null) && (level >= traceLevel)) {
            for(int i=0; i < appenders.length; i++) {
                if (isSlotIdSet) {
                    appenders[i].printTrace(mSlotId, classname, level, trace);
                } else {
    ...
    public static synchronized Logger getLogger(int slotId, String classname) {
        return new Logger(slotId, classname);
    }
