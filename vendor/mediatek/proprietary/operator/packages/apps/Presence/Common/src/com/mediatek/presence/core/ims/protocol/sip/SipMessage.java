...
public abstract class SipMessage {
    ...
    public String getSdpContent() {
        ...
        if (contentType.contains("application/sdp")) {
            if(content.contains("--boundary1")){
                int index1 = content.indexOf("v=0");
                content = content.substring(index1);
                int index2 = content.indexOf("--boundary1--");
                content = content.substring(0,index2-4);
            }
            return content;
        } else {
            return null;
    ...
    public ArrayList<String> getFeatureTags() {
        ...
        for(int i=0; i < temp.size(); i++) {
            ...
            if (!tag.startsWith("+")
                    && 
                    !(
                    tag.equalsIgnoreCase(FeatureTags.FEATURE_CPM_BURNED_MSG)
