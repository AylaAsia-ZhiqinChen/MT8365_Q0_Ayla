...
public class RlmiParser extends DefaultHandler {
    ...
    */
    ...
    private boolean isInResourceTag = false;
    ...
    public void characters(char buffer[], int start, int length) {
        ...
        if (isNameTag) {
            String name = new String(buffer, start, length);
            if (isInResourceTag) {
                //Set resource instance name
                resourceInstance.setName(name.trim());
            } else {
                //Set list name
    ...
    public void startElement(String namespaceURL, String localName,    String qname, Attributes attr) {
        ...
        if (localName.equals("list")) {
            ...
            String docVersion = attr.getValue("version").trim();
            String fullState = attr.getValue("fullState").trim();
            resourceInfo = new RlmiDocument(uri,docVersion,fullState);
            
        } else
        if (localName.equals("resource")) {
            String uri = attr.getValue("uri").trim();
            resourceInstance = new ResourceInstance(uri);
            isInResourceTag = true;
        } else
        if (localName.equals("instance")) {
            ...
            if ((resourceInstance != null) && (reason != null)) {
                resourceInstance.setId(id.trim());
            }
        } else
