...
public class PhoneUtils {
    ...
    public static synchronized void initialize(Context context) {
        ...
        int countryCode = PhoneNumberUtil.getInstance().
                getCountryCodeForRegion(countryIso.toUpperCase());
        COUNTRY_CODE = COUNTRY_CODE_PLUS + countryCode;
        Log.d(TAG, "initialize() countryIso: " + countryIso + " sInternationalPrefix: "
                + sInternationalPrefix + " COUNTRY_CODE: " + COUNTRY_CODE
                + " COUNTRY_AREA_CODE: " + COUNTRY_AREA_CODE);
        /** @} */
    }
    ...
    */
     ...
     * Get the area code.
     *
     * @param imsUser Ims User Number
     * @param countryCode Country Code
     * @return area code
     */
    public static String getAreaCode(String imsUser, String countryCode) {
        String result = "000";
        Log.d(TAG, "getAreaCode imsUser: " + imsUser);
        if (TextUtils.isEmpty(imsUser)) {
            return result;
        }
        if (imsUser.length() == ContactNumberUtils.NUMBER_LENGTH_NORMAL + 1
                && imsUser.startsWith(countryCode)) {
            //Get area code behind country code, length is three.
            result = imsUser.substring(1, 4);
        }
        Log.d(TAG, "getAreaCode areaCode: " + result);
        return result;
    }
    ...
    public static String formatNumberToInternational(String number) {
        ...
        if (!phoneNumber.startsWith(COUNTRY_CODE_PLUS)) {
            // National number
            if (phoneNumber.length() == ContactNumberUtils.NUMBER_LENGTH_NORMAL) {
                 if (!phoneNumber.startsWith("+1") && !phoneNumber.startsWith("1")) {
                    phoneNumber = "+1" + phoneNumber;
                }
            } else if (phoneNumber.length() == ContactNumberUtils.NUMBER_LENGTH_NORMAL + 1) {
                if (phoneNumber.startsWith("1")) {
                    phoneNumber = "+" + phoneNumber;
                }
            } else if (phoneNumber.length() >= ContactNumberUtils.NUMBER_LENGTH_NORMAL + 4) {
    ...
    public static String formatNumberToSipUri(String number) {
        ...
        return "sip:" + formatNumberToInternational(number) +
                ...
                .substring(ImsModule.IMS_USER_PROFILE.getPublicUri().indexOf('@'));
    }

    /**
     * Format a phone number to a TEL URI
     *
     * @param number Phone number
     * @return SIP URI
     */
     ...
     */
    public static boolean isUsingDummyAccount(int slotId) {
        boolean usingDummyAccount = RcsSettingsManager.getRcsSettingsInstance(slotId)
                .getAutoConfigMode() == RcsSettingsData.NO_AUTO_CONFIG ? true : false;
