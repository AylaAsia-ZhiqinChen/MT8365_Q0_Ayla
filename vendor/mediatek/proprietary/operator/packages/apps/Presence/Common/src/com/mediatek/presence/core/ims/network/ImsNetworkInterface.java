...
import com.mediatek.presence.core.access.NetworkAccess;
import com.mediatek.presence.core.access.WifiNetworkAccess;
import com.mediatek.presence.core.ims.ImsModule;
import com.mediatek.presence.core.ims.network.registration.AKADigestRegisterationProcedure;
import com.mediatek.presence.core.ims.network.registration.HttpDigestRegistrationProcedure;
import com.mediatek.presence.core.ims.network.registration.RegistrationManager;
import com.mediatek.presence.core.ims.network.registration.RegistrationProcedure;
...
import com.mediatek.presence.core.ims.userprofile.GibaUserProfileInterface;
import com.mediatek.presence.core.ims.userprofile.SettingsUserProfileInterface;
import com.mediatek.presence.core.ims.userprofile.VolteUserProfileInterface;
import com.mediatek.presence.core.ims.userprofile.UserProfile;
import com.mediatek.presence.core.ims.userprofile.UserProfileInterface;
...
public abstract class ImsNetworkInterface {
    ...
    public ImsNetworkInterface(ImsModule imsModule, int type, NetworkAccess access,
        ...
        int length = RcsSettingsManager.getRcsSettingsInstance(mSlotId).getNumberOfPCssfAddresses();
    ...
    protected void UpdateVolteDetails(String[] imsDestinationAddress, int[] imsDestinationPort){

        logger.info("UpdateVolteDetails");
        logger.info("imsDestinationAddress : "+imsDestinationAddress[0] + " , imsDestinationPort : "+imsDestinationPort[0]);

        imsProxyAddr = imsDestinationAddress;
        imsProxyPort = imsDestinationPort;
    }
    ...
    public boolean register(int currentPCscfAddress) {
        ...
        try {
            ...
            if (resolvedPort != imsProxyPort[currentPCscfAddress]) {
                ...
                RcsSettingsManager.getRcsSettingsInstance(mSlotId).setImsProxyPortForWifi(resolvedPort,currentPCscfAddress);
            }
            /**
             * @}
             */
        ...
        if (registered) {
            ...
            if (isBehindNat() && RcsSettingsManager.getRcsSettingsInstance(mSlotId).isSipKeepAliveEnabled()) {
    ...
    public boolean volteRegister(int currentPCscfAddress){
        if (logger.isActivated()) {
            logger.info("Start register() for Volte ");
        }

        if(!RcsUaAdapterManager.getRcsUaAdapter(mSlotId).isImsRegistered() || (access.getIpAddress() ==null)  ){
        ...
        try {
            ...
            // Initialize the SIP stack
            sip.initStack(access.getIpAddress(), resolvedIpAddress, resolvedPort, imsProxyProtocol, getType());
            int sipStackListeningPort = 0;
            sipStackListeningPort = sip.getSipStack().getListeningPort();

            if (logger.isActivated()) {
                logger.debug("sipStackListeningPort for listening from UCE_UA is : " + sipStackListeningPort);
            }

            //set the RCS SIP stack listening port
            RcsUaAdapterManager.getRcsUaAdapter(mSlotId).setRCSSipStackPort(sipStackListeningPort);

            sip.getSipStack().addSipEventListener(imsModule);

            if (logger.isActivated()) {
                logger.debug("IMS VOLTE registration successful");
            }
            ...
            getImsModule().getCore().getListener().handleRegistrationSuccessful(mSlotId);
        } catch(Exception e) {
                if (logger.isActivated()) {
                    logger.error("Can't instanciate the SIP stack", e);
                }
    ...
    public void volteUnregister(){
        ...
        try{
            RcsUaAdapterManager.getRcsUaAdapter(mSlotId).terminate();

            sip.closeStack();
            // Notify event listener
            getImsModule().getCore().getListener().handleRegistrationTerminated(mSlotId);
        } catch(Exception e){
            e.printStackTrace();
        }
    }
    ...
    */
    ...
    public String getGsmCellLocation() {
        //TODO: The code flow will not ge in here. If it does int the future, need to modify TelephonyManager
        // to support DSDS
   ...
   /*
        ...
        try {
            ...
            if (!TextUtils.isEmpty(mLastAccessNetworkInfo)
                ...
                RcsSettingsManager.getRcsSettingsInstance(slotId)
                        .writeParameter(RcsSettingsData.LAST_ACCESS_NETWORKINFO, mLastAccessNetworkInfo);
            } else {
                logger.debug("There isn't the last celluar network infomation.");
            }

            RcsSettingsManager.getRcsSettingsInstance(slotId)
                    .writeParameter(RcsSettingsData.CURRENT_ACCESS_NETWORKINFO, mCurrentAccessNetworkInfo);
    ...
    /**
     * M:Add for Single registration. . Interfaces
     */
