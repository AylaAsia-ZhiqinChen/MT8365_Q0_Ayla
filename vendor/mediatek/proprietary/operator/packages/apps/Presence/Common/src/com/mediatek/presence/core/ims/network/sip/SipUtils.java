...
import com.mediatek.presence.core.TerminalInfo;
...
public class SipUtils {
    ...
    /**
     * P-Preferred-Association header
     */
    public static final String HEADER_P_PREFERRED_ASSOCIATION = "P-Preferred-Association";
    ...
    private static Logger logger = Logger.getLogger(SipUtils.class.getName());
     ...
     * @throws Exception
     */
    public static Header buildCpimUserAgentHeader(int slotId) throws Exception {
        if(SimUtils.isTmoSimCard(slotId)) {
            String value = "";
            if(RcsUaAdapterManager.getRcsUaAdapter(slotId).isImsRegistered() &&
                    !RcsUaAdapterManager.getRcsUaAdapter(slotId).getUserAgent().equals("")) {
                value = RcsUaAdapterManager.getRcsUaAdapter(slotId).getUserAgent();
            }
    ...
    /**
        ...
        * @throws Exception
        */
    public static Header buildUserAgentHeader(int slotId) throws Exception {
        String value = "";
        if (RcsSettingsManager.getRcsSettingsInstance(slotId).isSingleRegistrationFeasible()) {
            if (RcsUaAdapterManager.getRcsUaAdapter(slotId).isImsRegistered() &&
                    !RcsUaAdapterManager.getRcsUaAdapter(slotId).getUserAgent().equals("")) {
                value = RcsUaAdapterManager.getRcsUaAdapter(slotId).getUserAgent();
            }
        ...
        //maybe not sync.
        if (value.equals("")) {
            value = "IM-client/OMA1.0 " + TerminalInfo.getProductInfo();
        }

        if (SimUtils.isTmoSimCard(slotId) &&
                !TextUtils.isEmpty(RcsUaAdapterManager.getRcsUaAdapter(slotId).getPidentifier())) {
            // add PIDENTIFIER from RcsUaAdapterManager.getRcsUaAdapter(slotId) if reveals
            value += " " + RcsUaAdapterManager.getRcsUaAdapter(slotId).getPidentifier();
        }
        Header userAgentHeader = HEADER_FACTORY.createHeader(UserAgentHeader.NAME, value);
        return userAgentHeader;
    }
    ...
    public static String getPAccessNetworkInfo(int slotId, int type) {
        String result = null;
        if (SimUtils.isAttSimCard(slotId)  || SimUtils.isVzwSimCard(slotId)) {
            if (type == ConnectivityManager.TYPE_MOBILE) {
                int[] subIds = SubscriptionManager.getSubId(slotId);
                if (subIds == null) {
                    logger.debug("getPAccessNetworkInfo fail. subIds is null");
                    return null;
                }
                TelephonyManager telephonyManager = TelephonyManager.from(
                ...
                while (true) {
                    if (cellInfos != null && cellInfos.size() > 0) {
                        ...
                        logger.debug("cellInfos is null, waitting 1sec");
                        SystemClock.sleep(1000);
                        cellInfos = telephonyManager.getAllCellInfo();
                    }
                }

                int cid = 0; // Cell Identity
                int lac = 0; // Location Area Code
                for (CellInfo info : cellInfos) {
                    if (info.isRegistered()) {
                        if (info instanceof CellInfoLte) {
                            logger.debug("CellInfoLte");
                            cid = ((CellInfoLte) info).getCellIdentity().getCi();
                            lac = ((CellInfoLte) info).getCellIdentity().getTac();
                        } else if (info instanceof CellInfoWcdma) {
                            logger.debug("CellInfoWcdma");
                            cid = ((CellInfoWcdma) info).getCellIdentity().getCid();
                            lac = ((CellInfoWcdma) info).getCellIdentity().getLac();
                        } else if (info instanceof CellInfoGsm) {
                            logger.debug("CellInfoGsm");
                            cid = ((CellInfoGsm) info).getCellIdentity().getCid();
                            lac = ((CellInfoGsm) info).getCellIdentity().getLac();
                        } else if (info instanceof CellInfoCdma) {
                            logger.debug("CellInfoCdma");
                            CellIdentityCdma identifyCdma = ((CellInfoCdma) info).getCellIdentity();
                            logger.debug("networkId:" + identifyCdma.getNetworkId()
                                    + " systemId:" + identifyCdma.getSystemId()
                                    + " basestationId:" + identifyCdma.getBasestationId()
                                    + " longitude:" + identifyCdma.getLongitude()
                                    + " latitude:" + identifyCdma.getLatitude());
                        }
                    }
                }
                String mccMnc = telephonyManager.getNetworkOperator();
                ServiceState state =
                        TelephonyManager.getDefault().getServiceStateForSubscriber(subIds[0]);
                int networkClass = TelephonyManager.getNetworkClass(state.getDataNetworkType());
                logger.debug("service state: " + state + "network class: " + networkClass);

                if (networkClass == TelephonyManager.NETWORK_CLASS_2_G) {
                    result = "3GPP-GERAN;cgi-3gpp=" + mccMnc +
                             String.format("%04x%04x", lac, cid);
                } else if (networkClass == TelephonyManager.NETWORK_CLASS_3_G) {
                    result = "3GPP-UTRAN-FDD;utran-cell-id-3gpp=" + mccMnc +
                             String.format("%04x%07x", lac, cid);
                } else if (networkClass == TelephonyManager.NETWORK_CLASS_4_G) {
                    result = "3GPP-E-UTRAN-FDD;utran-cell-id-3gpp=" + mccMnc +
                             String.format("%04x%07x", lac, cid);
                }
            } else if (type == NetworkCapabilities.TRANSPORT_WIFI) {
                WifiManager wifiManager =
                    (WifiManager) AndroidFactory.getApplicationContext().getSystemService(
                        Context.WIFI_SERVICE);
                WifiInfo info = wifiManager.getConnectionInfo();
                if (info != null) {
                    String macAddress = info.getBSSID().replaceAll(":", "").replaceAll("-", "");
                    if (info.getLinkSpeed() <= 11) {
                        result = "IEEE-802.11b;i-wlan-node-id=" + macAddress;
                    } else {
                        result = "IEEE-802.11a;i-wlan-node-id=" + macAddress;
                    }
                }
            }
        }
