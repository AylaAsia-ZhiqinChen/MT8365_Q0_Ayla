...
import com.mediatek.presence.core.ims.service.presence.VzwPublishManager;
import com.mediatek.presence.utils.SimUtils;

public class PublishManagerFactory {
    public static PublishManager newInstance(ImsModule parent) {
        int slotId = parent.getSlotId();
        if (SimUtils.isAttSimCard(slotId)) {
            return new PublishManagerEx(parent);
        } else {
            return new PublishManager(parent);
        }
    }
}
