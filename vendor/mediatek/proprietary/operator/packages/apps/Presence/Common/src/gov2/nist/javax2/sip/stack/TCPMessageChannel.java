...
public class TCPMessageChannel extends MessageChannel implements SIPMessageListener, Runnable,
    ...
    public final static String TAG_LOG = "TCPMessageChannel";
    ...
    private Logger SipLogger = Logger.getLogger("sip_msg");
    ...
    protected TCPMessageChannel(int slotId, Socket sock, SIPTransactionStack sipStack,
     ...
     */
    protected TCPMessageChannel(int slotId, InetAddress inetAddr, int port, SIPTransactionStack sipStack,
    ...
    private void sendMessage(byte[] msg, boolean retry) throws IOException {
        if(isSingleRegisteration()) {
            ...
            SipLogger.debug("--> sendMessage err!");
        }
    ...
    public void processMessage(byte[] msg) throws Exception {
        //create sip message from byte array
        SIPMessage sipMessage = null;
        StringMsgParser smp = new StringMsgParser();
        try {
            sipMessage = smp.parseSIPMessage(msg);
            processIMSMessage(sipMessage);
        } catch (Exception ex) {
            ex.printStackTrace();
        }
    }
    ...
    public void processIMSMessage(SIPMessage sipMessage) throws Exception {
        try {
            if (sipMessage instanceof SIPRequest) {
                ...
                SipLogger.debug("<-- " + sipRequest.toString());
                SipLogger.debug(TRACE_SEPARATOR);
                ...
                SipLogger.debug("<-- " + sipResponse.toString());
                SipLogger.debug(TRACE_SEPARATOR);
    ...
    private void raiseIOException() {

        if (sipStack instanceof SipStackImpl) {
            SipStackImpl stack = (SipStackImpl)sipStack;
            SipListener listener = stack.getSipListener();
            if (listener != null) {
                IOExceptionEvent event = new IOExceptionEvent(
                        this, peerAddress.toString(), peerPort, "TCP");
                listener.processIOException(event);
            }
        }
    }
