...
public class SipStackImpl extends SIPTransactionStack implements
	 ...
	 */
	public SipStackImpl(int slotId, Properties configurationProperties)
			throws PeerUnavailableException {
		this();
        mSlotId = slotId;
		...
		boolean computeContentLength = configurationProperties.getProperty(
				...
				"false").equalsIgnoreCase("true");
                    ...
                    //needed fro TMO as in case of length mistmatch parse exption is thrown
                    //StringMsgParser
			//	.setComputeContentLengthFromMessage(computeContentLength);
		StringMsgParser
		.setComputeContentLengthFromMessage(true);
		...
		if (lip != null) {
			...
			try {
				...
				MessageProcessor messageProcessor = this
						.createMessageProcessor(mSlotId, inetAddr, port, transport);
