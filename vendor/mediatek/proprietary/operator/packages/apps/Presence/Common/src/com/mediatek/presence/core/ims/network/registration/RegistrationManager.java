...
import com.mediatek.presence.core.ims.network.VolteNetworkInterface;
...
import com.mediatek.presence.core.ims.service.SubscribeRequest;
...
public class RegistrationManager extends PeriodicRefresher {
    ...
    public RegistrationManager(
        ...
        super("Registration");
        mSlotId = networkInterface.getImsModule().getSlotId();
        this.networkInterface = networkInterface;
        this.registrationProcedure = registrationProcedure;
        this.featureTags = RegistrationUtils.getSupportedFeatureTags(mSlotId);
        this.expirePeriod = RcsSettingsManager.getRcsSettingsInstance(mSlotId).getRegisterExpirePeriod();

        if (RcsSettingsManager.getRcsSettingsInstance(mSlotId).isGruuSupported()) {

            //DeviceUtils get instanceid handles whcih id needs to be used
            this.instanceId = DeviceUtils.getInstanceId(mSlotId, AndroidFactory.getApplicationContext());
        }

        int defaultExpirePeriod = RcsSettingsManager.getRcsSettingsInstance(mSlotId).getRegisterExpirePeriod();
    ...
    public synchronized boolean registration() {
         ...
         */
        String version = RcsSettingsManager.getRcsSettingsInstance(mSlotId)
        ...
        if (disabled) {
            if (!PhoneUtils.isUsingDummyAccount(mSlotId)) {
                if (logger.isActivated()) {
        ...
        long validity = RcsSettingsManager.getRcsSettingsInstance(mSlotId).getProvisionValidity();
        if (validity < 0) {
            if (!PhoneUtils.isUsingDummyAccount(mSlotId)) {
                if (logger.isActivated()) {
        ...
        for (int retryCount = 0; retryCount < MAX_RETRY_COUNT; retryCount++) {
            try {
                ...
                SipRequest register = SipMessageFactory.createRegister(
                                    mSlotId, dialogPath, featureTags, expirePeriod, instanceId);
        ...
        if (SimUtils.isCmccSimCard(mSlotId) && registered)
            subscription();
 ...
 public  synchronized void doPreReRegistrationProcess() {
        ...
        int slotId = networkInterface.getImsModule().getSlotId();
        networkInterface.getImsModule().getCore().getListener()
                .handleRegistrationTerminated(slotId);
    }
    ...
    private synchronized void doUnRegistration() {
        ...
        try {
            ...
            SipRequest register = SipMessageFactory.createRegister(mSlotId, dialogPath,
                    featureTags,
                    0,
                    instanceId);
        ...
        // Notify event listener
        int slotId = networkInterface.getImsModule().getSlotId();
        networkInterface.getImsModule().getCore().getListener().handleRegistrationTerminated(slotId);
    }
    ...
    private void handle200OK(SipTransactionContext ctx) throws Exception {
        ...
        RcsSettingsManager.getRcsSettingsInstance(mSlotId).setPublicUri(publicUri);
        ...
        registrationProcedure.readSecurityHeader(mSlotId, resp);
       ...
       // startTimer(expirePeriod - 60, 1);
        ...
        // Notify event listener
        int slotId = networkInterface.getImsModule().getSlotId();
        networkInterface.getImsModule().getCore().getListener().handleRegistrationSuccessful(slotId);
    ...
    private void handle401Unauthorized(SipTransactionContext ctx) throws Exception {
        ...
        SipRequest register = SipMessageFactory.createRegister(mSlotId,
                dialogPath,
                featureTags,
                ctx.getTransaction().getRequest().getExpires().getExpires(),
                instanceId);
    ...
    private void handleError(ImsError error) {
        ...
        int slotId = networkInterface.getImsModule().getSlotId();
        networkInterface.getImsModule().getCore().getListener()
                .handleRegistrationFailed(slotId, error);
    }
    ...
    private synchronized void doUnNormalUnRegistration() {
        ...
        try {
            ...
            dialogPath.incrementCseq();
            // Create REGISTER request with expire 0
            SipRequest register = SipMessageFactory.createRegister(mSlotId, dialogPath, featureTags, 0,
                    instanceId);
            // Send REGISTER request
