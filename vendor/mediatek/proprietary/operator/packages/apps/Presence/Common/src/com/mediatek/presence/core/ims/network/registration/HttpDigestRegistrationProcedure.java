...
import com.mediatek.presence.core.ims.security.HttpDigestMd5Authentication;
...
public class HttpDigestRegistrationProcedure extends RegistrationProcedure {
    ...
    public void writeSecurityHeader(SipRequest request) throws CoreException {
      ...
      //  if(!RcsSettings.getInstance().getNetworkOperator().equalsIgnoreCase("CMCC"))
      //  {
        try {
            // Get Realm
            String realm = "";
     ...
     * @throws CoreException
     */
    public void readSecurityHeader(int slotId, SipResponse response) throws CoreException {
        if (digest == null) {
            return;
        }

        WWWAuthenticateHeader wwwHeader = (WWWAuthenticateHeader)response.getHeader(
                WWWAuthenticateHeader.NAME);
        AuthenticationInfoHeader infoHeader =  (AuthenticationInfoHeader)response.getHeader(
