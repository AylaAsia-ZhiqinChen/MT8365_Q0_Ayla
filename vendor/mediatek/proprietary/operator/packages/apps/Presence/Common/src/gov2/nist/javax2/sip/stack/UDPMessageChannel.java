...
public class UDPMessageChannel extends MessageChannel implements
     ...
     public final static String TAG_LOG = "UDPMessageChannel";
    /**
     ...
     */
    protected UDPMessageChannel(int slotId, SIPTransactionStack stack,
            UDPMessageProcessor messageProcessor) {
        super.messageProcessor = messageProcessor;
        this.sipStack = stack;

        Thread mythread = new Thread(this);
        if (isSingleRegisteration())
        {
	        eventListener a = new eventListener();
            RcsUaAdapterManager.getRcsUaAdapter(slotId).addRcsSipEventListener(a);
	    }
        this.myAddress = messageProcessor.getIpAddress().getHostAddress();
        this.myPort = messageProcessor.getPort();
        this.mSlotId = slotId;
        if (sipStack.isLoggingEnabled()) {
            this.sipStack.getStackLogger().logDebug("UDPMessageChannel: MyportNumber :  " + myPort);
     ...
     *            our SIP Stack.
     */
    protected UDPMessageChannel(int slotId, InetAddress targetAddr, int port,
     ...
     */
    /*
    private void processIncomingDataPacket(byte[] packet , InetAddress peerAddress,int peerPort )
            throws Exception {
    	Log.i(TAG_LOG,"UDPMessageChannel : processIncomingDataPacket");
     ...
     // Read bytes and put it in a eueue.
        ...
        Log.i(TAG_LOG,"UDPMessageChannel: processIncomingDataPacket : peerAddress = "
                + peerAddress.getHostAddress() + "/"
                + peerPort + " Length = " + packetLength);
        ...
        try {
            ...
            sipMessage = myParser.parseSIPMessage(packet);
            myParser = null;
        } catch (ParseException ex) {
        	Log.i(TAG_LOG,"UDPMessageChannel : ParseException : " + ex.getMessage());
            ...
            String msgString = new String(packet, 0, packetLength);
...
*/
    ...
    public class eventListener extends RcsUaAdapter.RcsSipEventListener{
		...
		@Override
		public  void notifySipMessage(byte[] bytes , InetAddress address,int port )
    	{
    		/* creating a UDP packet here and forward to handler */
			DatagramPacket datagram = new DatagramPacket(bytes, 0, bytes.length, address, port);

			try {
				Log.i(TAG_LOG,"notifySIPMessage");
				DatagramPacket udpPacket = new DatagramPacket(bytes, bytes.length, address,  port);
				processIncomingDataPacket(udpPacket);
			} catch (Exception e) {
				e.printStackTrace();
				//Log.i(TAG_LOG,"error : "+err.getMessage());
			}
    	}
    }

    @Override
    public boolean isSingleRegisteration() {
        return RcsUaAdapterManager.getRcsUaAdapter(mSlotId).isImsRegistered();
    }

    @Override
    public void sendSipMessage(byte[] msg) {
        RcsUaAdapterManager.getRcsUaAdapter(mSlotId).sendSipMessage(msg);
    }
