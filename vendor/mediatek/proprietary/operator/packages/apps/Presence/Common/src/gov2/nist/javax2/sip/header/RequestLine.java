...
public class RequestLine extends SIPObject implements SipRequestLine {
    ...
    */
    ...
    public void setPresList(String listName) {
        NameValue nv = parameters.getNameValue(ParameterNames.PRES_LIST);
        if (nv != null) {
            nv.setValueAsObject(listName);
        } else {
            nv = new NameValue(ParameterNames.PRES_LIST, listName);
            parameters.set(nv);
        }
    }
