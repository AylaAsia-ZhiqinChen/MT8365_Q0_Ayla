...
public class SubscriptionManager {
    ...
    private static ImsModule mImsModule = null;
     ...
     * @param imsModule
     */
    public SubscriptionManager(ImsModule imsModule) {
        mImsModule = imsModule;
        mSlotId = imsModule.getSlotId();
        logger = Logger.getLogger(mSlotId, "SubscriptionManager");
    }
    ...
    public Subscription getSubscription(EventCallback callback) {
        if (callback == null)
            return null;
    ...
    private static class Subscription extends PeriodicRefresher {
        ...
        public Subscription(
            ...
            int slotId = manager.getImsModule().getSlotId();

            this.authenticationAgent = new SessionAuthenticationAgent(mImsModule);
            ...
            if (pollOnce || request.getExpirePeriod() == 0) {
                ...
                if (request.getExpirePeriod() > 0)
                    ...
                    defaultExpire = RcsSettingsManager.getRcsSettingsInstance(slotId).getSubscribeExpirePeriod();
            ...
            this.mIdentity = mImsModule.getSipManager().getSipStack().generateCallId();
        }
        ...
        private void subscribe() {
            ...
            if (dialogPath == null) {
                ...
                dialogPath = new SipDialogPath(
                        mImsModule.getSipManager().getSipStack(),
                        callId,
                        1,
                        target,
                        localParty,
        ...
        private SipRequest createSubscribe(int expirePeriod) throws Exception {
            ...
            subscribe = SipMessageFactory.createCpimSubscribe(
                    slotId, dialogPath, expirePeriod, mRequest.getContentType(), mRequest.getContent());
