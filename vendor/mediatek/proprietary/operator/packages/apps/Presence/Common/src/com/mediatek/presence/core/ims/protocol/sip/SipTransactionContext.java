...
import com.mediatek.presence.service.RcsCoreService;
...
public class SipTransactionContext extends Object {
    ...
    private boolean wasSignalled = false;   //to see if thread  had been notified
     ...
     */
    public SipTransactionContext(int slotId, Transaction transaction) {
        this.transaction = transaction;
        mSlotId = slotId;
        mReceiver = new RegistrationStatusReceiver();
        IntentFilter filter = new IntentFilter();
        filter.addAction(RcsCoreService.CORE_SERVICE_CURRENT_STATE_NOTIFICATION);
    ...
    public void waitResponse(int timeout) {
        try {
            ...
            Log.e("SipTransactionContext", "Send Message receive interrupted~");
            recvMsg = null;
             if(mReceiver != null){
                 LocalBroadcastManager.getInstance(AndroidFactory.getApplicationContext())
                        .unregisterReceiver(mReceiver);
    ...
    private class RegistrationStatusReceiver extends BroadcastReceiver {
        ...
        public void onReceive(final Context context,final Intent intent) {
            int status = intent.getIntExtra("status", RcsCoreService.RCS_CORE_IMS_CONNECTED);
            int slotId = intent.getIntExtra("slotId", 0);
            if (slotId != mSlotId) {
                return;
            }
            if(status == RcsCoreService.RCS_CORE_IMS_DISCONNECTED ||
                status == RcsCoreService.RCS_CORE_IMS_CONNECTION_FAILED)
                lostRegistration();
        }
    }

    //Add an intent filter for registeration state and call this api from there 
    public void lostRegistration(){
        ...
        synchronized(this) {
            ...
            Log.e("SipTransactionContext", "lostRegistration~");
            super.notify();
        }
    }
