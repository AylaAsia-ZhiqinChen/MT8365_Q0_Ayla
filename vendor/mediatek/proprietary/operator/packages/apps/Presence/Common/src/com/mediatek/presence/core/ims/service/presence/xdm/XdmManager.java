...
package com.mediatek.presence.core.ims.service.presence.xdm;
...
import com.mediatek.presence.core.ims.protocol.http.HttpAuthenticationAgent;
import com.mediatek.presence.core.ims.protocol.http.HttpDeleteRequest;
import com.mediatek.presence.core.ims.protocol.http.HttpGetRequest;
import com.mediatek.presence.core.ims.protocol.http.HttpPutRequest;
import com.mediatek.presence.core.ims.protocol.http.HttpRequest;
...
import com.mediatek.presence.core.ims.service.presence.directory.Folder;
import com.mediatek.presence.core.ims.service.presence.directory.XcapDirectoryParser;
import com.mediatek.presence.platform.network.AndroidDefaultNetwork;
...
import com.mediatek.presence.platform.network.SocketConnection;
...
import com.mediatek.presence.utils.HttpUtils;
...
public class XdmManager {
    ...
    public XdmManager(ImsModule parent) {
        ...
        logger = Logger.getLogger(mSlotId, "XdmManager");

        // check GBA suport
        if (RcsSettingsManager.getRcsSettingsInstance(mSlotId)
                .getXDMAuth().equalsIgnoreCase(AUTH_GBA)) {
    ...
    public HttpResponse getXcapDocuments() {
        try {
            ...
            if (response == null) {
                return null;
            } else if (response.isSuccessfullResponse()) {
                if (logger.isActivated()) {
    ...
    public HttpResponse addContactToGrantedList(String contact) {
        boolean isInitialize = StartService.getXdmInitializeFlag(mSlotId, AndroidFactory.getApplicationContext());
        if (logger.isActivated()) {
    ...
    public List<String> getGrantedContacts() {
        ...
        try {
            ...
            if (logger.isActivated()) {
                logger.error(
                        "Can't get granted contacts list: unexpected exception", e);
            }
        }
        return result;
    }
    ...
    private boolean setUpDefaultNetwork() {
        ...
        if (mActiveNetworkConnection == null) {
            try {
                ...
                if (subIds == null) {
                    logger.debug("setUpDefaultNetwork fail. subIds is null");
                    return false;
                }
                mActiveNetworkConnection.acquireNetwork(subIds[0]);
