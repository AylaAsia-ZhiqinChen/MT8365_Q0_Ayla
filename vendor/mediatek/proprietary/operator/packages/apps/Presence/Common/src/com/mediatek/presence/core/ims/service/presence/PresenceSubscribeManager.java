...
public class PresenceSubscribeManager extends SubscribeManager {
    ...
    public SipRequest createSubscribe(SipDialogPath dialog, int expirePeriod) throws SipException {
        ...
        if(!RcsSettingsManager.getRcsSettingsInstance(slotId).isCPMSupported()){
            subscribe = SipMessageFactory.createSubscribe(slotId, dialog, expirePeriod);
        }
        else{
            if (logger.isActivated()) {
                logger.info("createSubscribe CPMS");
            }
            subscribe = SipMessageFactory.createPresenceSubscribe(slotId, dialog, expirePeriod, null);
        }
                ...
                if (multi.isMultipart()) {
                    ...
                    try {
                        ...
                        getImsModule().getCore().getListener().handlePresenceInfoNotification(
                                slotId, presenceInfo.getEntity(), presenceInfo);
                    } catch(Exception e) {
                        if (logger.isActivated()) {
