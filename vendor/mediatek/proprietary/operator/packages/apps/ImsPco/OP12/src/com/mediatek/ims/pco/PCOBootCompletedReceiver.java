...
public class PCOBootCompletedReceiver extends BroadcastReceiver {
    ...
    public void onReceive(Context context, Intent intent) {
        ...
        log("isOp12BrandedDevice() = " + mIsBrandedDevice);
        if (mIsBrandedDevice == false) {
            return;
        }
        if (intent.getAction().equals("android.intent.action.BOOT_COMPLETED")) {
            log("BOOT_COMPLETED: start PCO check");
            Intent i = new Intent(context, PCOService.class);
            i.setAction(PCOService.ACTION_PCO_CHECK);
            i.putExtra("service", PCO_SERVICE);
