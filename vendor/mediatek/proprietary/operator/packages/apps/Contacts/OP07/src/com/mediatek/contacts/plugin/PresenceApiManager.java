...
public class PresenceApiManager {
    ...
    */
     ...
     *            the number list
     */
    public void requestContactsPresence(final List<String> numbers) {
        ...
        AsyncTask.execute(new Runnable() {
            ...
            public void run() {
                ...
                for (Iterator<String> it = set.iterator(); it.hasNext(); ) {
                    formatNumber = ContactNumberUtils.getDefault().getFormatNumber(it.next());
                    if (formatNumber != null) {
                        contactIds.add(createContactIdFromNumber(formatNumber));
                    }
                }
                    ...
                    try {
                        if (count >= MAX_REQUEST_COUNT) {
                            ...
                            for (ContactId contactId : contactIds) {
                                mCapabilitiesApi.requestContactCapabilities(contactId);
                            }
                        }
                    } catch (Exception e) {
                        e.printStackTrace();
    ...
    private void queryNumbersPresence(List<String> numbers) {
        if (mCapabilitiesApi != null) {
            for (String number : numbers) {
                requestContactPresenceDelay(number, false);
            }
        }
    }
