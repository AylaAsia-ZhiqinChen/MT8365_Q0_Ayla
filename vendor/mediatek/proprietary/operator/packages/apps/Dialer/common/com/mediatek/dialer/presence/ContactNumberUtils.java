...
public class ContactNumberUtils {
    ...
    private static final String TAG = "Dialer/ContactNumberUtils";
    ...
    public String format(final String phoneNumber) {
        ...
        if (len == NUMBER_LENGTH_NORMAL || len == NUMBER_LENGTH_NO_AREA_CODE) {
    ...
    public String getFormatNumber(final String phoneNumber) {
        ...
        LogUtils.printSensitiveInfo(TAG, "formatNumber: "
                + formatResult.getFormatNumber()
    ...
    public FormatNumberResult validate(final String phoneNumber) {
        ...
        int len = number.length();
        if (len > "+1-nnn-nnn-nnnn".length() || len < NUMBER_LENGTH_NO_AREA_CODE) {
            result.setResultCode(NUMBER_INVALID);
            return result;
        }
        ...
        if (number.startsWith("+1")) {
            len = number.length();
            if (len == NUMBER_LENGTH_NORMAL + 2 || len == NUMBER_LENGTH_NO_AREA_CODE + 2) {
