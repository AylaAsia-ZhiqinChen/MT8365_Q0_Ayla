...
package com.mediatek.dialer.presence;
...
public class PresenceApiManager {
    public static final String TAG = "Dialer/PresenceApiManager";
    ...
    */
    ...
    public boolean isVideoCallCapable(String number) {
        ...
        number = number.trim();
        LogUtils.printSensitiveInfo(TAG, "isVideoCallCapable number: " + number);
    ...
    private void getContactPresence(final String number) {
        LogUtils.printSensitiveInfo(TAG, "getContactPresence entry, number is " + number);
                    ...
                    try {
                        ...
                        if (capabilities == null) {
                            LogUtils.printSensitiveInfo(TAG,
                                    "getContactPresence capabilities null, " + number);
                           return;
                        }
                        ContactInformation info = new ContactInformation();
                        ...
                        synchronized (mContactsCache) {
                            LogUtils.printSensitiveInfo(TAG, "getContactPresence: " + number
    ...
    public void requestContactPresenceDelay(final String number, final boolean force) {
        if (isServiceConnected()) {
            ...
            LogUtils.printSensitiveInfo(TAG, "requestContactPresenceDelay, number is " + number);
    ...
    public void requestContactPresence(final String number, final boolean force) {
        LogUtils.printSensitiveInfo(TAG, "requestContactPresence, number: "
                + number + " force: " + force);
        if (!isServiceConnected()) {
            return;
        }
                ...
                public void run() {
                    if (mCapabilitiesApi != null) {
                        try {
                            if (force) {
                                mCapabilitiesApi
                                .forceRequestContactCapabilities(
                            ...
                            synchronized (mContactsCache) {
                                if (mContactsCache.get(formatNumber) == null) {
                                    LogUtils.printSensitiveInfo(TAG, 
                                            "requestContactPresence cache: " + formatNumber);
                                    ContactInformation info = new ContactInformation();
                                    info.isVideoCapable = false;
     ...
     *            the number list
     */
    public void requestContactsPresence(List<String> numbers) {
        if (numbers == null || numbers.isEmpty()) {
            Log.e(TAG, "requestContactsPresence invalid list");
            return;
        }
        if (!isServiceConnected()) {
            return;
        }
        Log.d(TAG, "requestContactsPresence entry, size is " + numbers.size());
        AsyncTask.execute(new Runnable() {
            @Override
            public void run() {
                //remove duplicated contact
                HashSet<String> set = new HashSet<String>(numbers);
                numbers.clear();
                ...
                for (Iterator<String> it = set.iterator(); it.hasNext(); ) {
                    number = it.next();
                    formatNumber = ContactNumberUtils.getDefault().getFormatNumber(number);
                    if (formatNumber != null) {
                        numbers.add(formatNumber);
                    }
                }
                int count = numbers.size();
                Log.d(TAG, "requestContactsPresence filter, size is " + count);
                if (mCapabilitiesApi != null && count > 0) {
                    try {
                        if (count >= MAX_REQUEST_COUNT) {
                            mCapabilitiesApi.requestContactsCapabilities(
                                    createContactIsdFromNumbers(numbers));
                        } else {
                            for (String singleNumber : numbers) {
                                mCapabilitiesApi.requestContactCapabilities(
                                        createContactIdFromNumber(singleNumber));
                            }
                        }
                    } catch (Exception e) {
                        e.printStackTrace();
        ...
        Log.d(TAG, "requestContactsPresence exit");
    }
    ...
    private void queryContactsPresence() {
        ...
        Cursor cursor = mContext.getContentResolver().query(Data.CONTENT_URI, projection,
        ...
        try {
            if (cursor != null) {
                while (cursor.moveToNext()) {
                    ...
                    info.isVideoCapable = true;
                    synchronized (mContactsCache) {
                        LogUtils.printSensitiveInfo(TAG, "queryContactsPresence number: " + number);
                        mContactsCache.put(number, info);
                    }
                }
            } else {
    ...
    public class MyCapabilitiesListener extends CapabilitiesListener {
        ...
        public void onCapabilitiesReceived(final String contact,
            ...
            LogUtils.printSensitiveInfo(TAG, "onCapabilityReceived(), contact = " + contact
         ...
         public void onErrorReceived(String contact, int type, int status, String reason) {
             ...
             if (contact != null) {
                for (CapabilitiesChangeListener listener :
                        mCapabilitiesChangeListenerList) {
                    if (listener != null) {
                        Log.i(TAG, "onErrorReceived, Notify the listener");
    ...
    public List<String> getNumbersByContactId(long contactId) {
        ...
        Cursor cur = null;
        try {
            cur = mContext.getContentResolver().query(Phone.CONTENT_URI, projection, selection,
                    selectionArgs, null);
        ...
        LogUtils.printSensitiveInfo(TAG, "getNumbersByContactId exit, list: " + list);
        return list;
    }

    /**
     * Request Capabilities By Id.
     * @param contactId Contact Id
     */
    public void requestCapabilitiesById(long contactId) {
        Log.d(TAG, "requestCapabilitiesById contactId: " + contactId);
        queryNumbersPresence(getNumbersByContactId(contactId));
    }

    private void queryNumbersPresence(List<String> numbers) {
        if (mCapabilitiesApi != null) {
            for (String number : numbers) {
                requestContactPresence(number, false);
            }
        }
    }
    ...
    /**
     * Creates a ContactId list from number list.
     *
     * @param number list
     * @return contactid list
     */
     private List<ContactId>  createContactIsdFromNumbers(List<String> numbers) {
        ContactUtil cu = ContactUtil.getInstance(mContext);
        List<ContactId> contactIds = new ArrayList<ContactId>();

        try {
            for (String number : numbers) {
                contactIds.add(cu.formatContact(number));
            }
            return contactIds;
        } catch (Exception e) {
            e.printStackTrace();
        }
        contactIds = new ArrayList<ContactId>();
        for (String number : numbers) {
             contactIds.add(new ContactId(number));
        }
        return contactIds;
    }
