 ...
 *
 */
public class Op01SmsReceiver {
    private static final String TAG = "Mms/Op01SmsReceiver";

    /**
     * Extract sms body.
     * @param msgs SmsMessage array.
     * @param sms  SmsMessage.
     * @param values ContentValues.
     */
    public void extractSmsBody(MtkSmsMessage[] msgs, MtkSmsMessage sms, ContentValues values) {
        Log.d(TAG, "Op01SmsReceiverExt.extractSmsBody");

        int pduCount = msgs.length;
        boolean hasMissedSegments = checkConcateRef(sms.getUserDataHeader(), pduCount);

        Log.d(TAG, "pduCount=" + pduCount);

        if (hasMissedSegments) {
            int totalParts = sms.getUserDataHeader().concatRef.msgCount;
            Log.v(TAG, "[fake process missed segment(s) " + pduCount + "/" + totalParts);
    ...
    private String handleMissedParts(MtkSmsMessage[] parts) {
        if (parts == null || parts.length <= 0) {
            Log.e(TAG, "[fake invalid message array");
            return null;
        }
