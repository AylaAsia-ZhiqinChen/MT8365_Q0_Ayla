...
public class SDCardUtils {
    ...
    /*
     * If SD card is removed or full, broadcast intent
     */
    public static void broadcastSDRemovedOrFullIfNecessary(Context context) {
        if (isSdCardMissing(context)) {
            Log.i(CLASS_TAG, "[broadcast] SD card removed");
            Intent intent = new Intent("com.mediatek.mms.plugin.sd_removed_or_full");
            context.sendBroadcast(intent);
        } else {
            String path = getStoragePath(context);
            if (getAvailableSize(path) <= MINIMUM_SIZE) {
                Log.i(CLASS_TAG, "[broadcast] SD full");
                Intent intent = new Intent("com.mediatek.mms.plugin.sd_removed_or_full");
                context.sendBroadcast(intent);
            }
        }
