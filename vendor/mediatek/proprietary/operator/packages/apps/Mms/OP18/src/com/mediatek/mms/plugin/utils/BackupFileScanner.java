...
import com.mediatek.op18.mms.modules.MmsXmlComposer;
import com.mediatek.op18.mms.modules.MmsXmlInfo;
...
public class BackupFileScanner {
    ...
    public void quitScan() {
        synchronized (object) {
            if (mScanThread != null) {
                mScanThread.cancel();
                mScanThread = null;
                MyLogger.logD(CLASS_TAG, "quitScan");
            }
        }
    }
    ...
    public boolean isRunning() {
        if (mScanThread != null) {
            return mScanThread.mIsCanceled;
        }
        return false;
    }
    ...
    private class ScanThread extends Thread {
        ...
        public void run() {
            if (!SDCardUtils.isSdCardAvailable(mContext)) {
                MyLogger.logW(CLASS_TAG, "ScanThread.run(): No SD card found of SD card is full");
                if (mHandler != null) {
                    Message msg = mHandler.obtainMessage(MessageID.SCANNER_FINISH, null);
        ...
        private void detectSDcardForOtherBackups() {
            ...
            for (BackupsHandler backupsHandler : scanTaskHandlers) {
                ...
                if (backupType.equals(Constants.BackupScanType.DATATRANSFER)) {
                    MyLogger.logD(CLASS_TAG, "Type is DATATRANSFER");
            ...
            if (notifyType == NotifyManager.FP_NEW_DETECTION_NOTIFY_TYPE_LIST
             || !result.isEmpty()) {
                MyLogger.logD(CLASS_TAG, "notifyType = " + notifyType + "  path = " + path);
                NotifyManager.getInstance(mContext).showNewDetectionNotification(notifyType, path);
            }
            mIsCanceled = (mIsCanceled ? !mIsCanceled : mIsCanceled);
        }
 ...
 /*               if (name.endsWith(".vcf")) { // find it is a contact file
     ...
     */       }
        ...
        private void initDataTransferBackupForList() {
            ...
            synchronized (object) {
                if (!mIsCanceled && mHandler != null) {
                    ...
                    MyLogger.logD(CLASS_TAG, "msg.hashCode() = " + msg.hashCode());
                    MyLogger.logD(CLASS_TAG, "msg.toString() = " + msg.toString());
                    mHandler.sendMessage(msg);
                }
            }
            mScanThread = null;
        ...
        private HashMap<String, List<BackupFilePreview>> getDataTransferBackups() {
            File[] files = scanPersonalBackupFiles();
            HashMap<String, List<BackupFilePreview>> result
             = new HashMap<String, List<BackupFilePreview>>();
            ...
            MyLogger.logD(CLASS_TAG, "getDataTransferBackups path = " + path);
            //path = null;
            ...
            MyLogger.logD(CLASS_TAG, "getDataTransferBackups result = " + result);
            return result;
        }
