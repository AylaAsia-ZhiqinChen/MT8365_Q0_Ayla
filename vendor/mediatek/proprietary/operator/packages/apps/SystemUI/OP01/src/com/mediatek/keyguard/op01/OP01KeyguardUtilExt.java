 ...
 */
public class OP01KeyguardUtilExt extends DefaultKeyguardUtilExt {
    public static final String TAG = "OP01KeyguardUtilExt";
     ...
     */
    public OP01KeyguardUtilExt(Context context) {
        super();
        mContext = context;
    }
    ...
    public boolean lockImmediatelyWhenScreenTimeout() {
        boolean ret = false;
        int type = -1;
        type = Settings.System.getInt(mContext.getContentResolver(),
                    "op01_lock_screen_type", 1);
        if (type == 1) {
            ret = true;
        }
        Log.d(TAG, "lockImmediatelyWhenScreenTimeout, ret=" + ret
            + ", type=" + type);
        return ret;
    }
}
