...
public class ChatAdapter extends BaseAdapter implements SectionIndexer {
    ...
    private static class ItemBinderFactory {
        /**
         * Creates a new ItemBinder object.
         * @param item the item
         * @param position the position
         * @return the abs item binder
         */
        private static AbsItemBinder createItemBinder(Object item, int position) {
            if (item instanceof Date) {
                ...
                if ((item instanceof GroupChatFragment.SentMessage || item instanceof MultiChatFragment.SentMessage)
                        && Logger.getIsIntegrationMode()) {
                    return new PluginGroupSendMesItemBinder(
                            (ISentChatMessage) item);
                } else {
                    return new SentMessageItemBinder((ISentChatMessage) item);
                }
            } else if (item instanceof IReceivedChatMessage) {
                if (item instanceof GroupChatFragment.ReceivedMessage
                        && Logger.getIsIntegrationMode()) {
                    return new PluginGroupReceivedMesItemBinder(
                            (IReceivedChatMessage) item);
                } else {
                    return new ReceivedMessageItemBinder(
                            (IReceivedChatMessage) item);
                }
            } else if (item instanceof One2OneChatFragment.SentFileTransfer
                    || item instanceof GroupChatFragment.SentFileTransfer
                    || item instanceof MultiChatFragment.SentFileTransfer) {
                return new SentFileTransferItemBinder((FileTransfer) item);
            } else if (item instanceof One2OneChatFragment.ReceivedFileTransfer
                    || item instanceof GroupChatFragment.ReceivedFileTransfer) {
                return new ReceivedFileTransferItemBinder((FileTransfer) item);
    ...
    public ChatAdapter(ListView listView) {
        if (null != listView) {
            ...
            mHeaderView = (RelativeLayout) mInflater.inflate(
                    R.layout.load_history_header, null);
            if (mHeaderView != null) {
                mListView.addHeaderView(mHeaderView);
                mHeaderText = (TextView) mHeaderView
                        .findViewById(R.id.load_chat_history);
    ...
    public final void updateAdapter() {
        ...
        if (context != null) {
            ...
            try {
                ...
                field = fastScroller.getClass().getDeclaredField(
                        OVERLAY_DRAWABLE);
                field.setAccessible(true);
                Drawable myOverlayDrawable = resources
                        .getDrawable(R.drawable.fast_scroller);
            ...
            int fastScrollerFontSize = resources
                    .getDimensionPixelSize(R.dimen.fast_scroll_font_size);
            // mListView.setFastScrollerPopupTextSize(fastScrollerFontSize);
            int fastScrollerWidth = resources
                    .getDimensionPixelSize(R.dimen.fast_scroll_overlay_width);
            int fastScrollerHeight = resources
                    .getDimensionPixelSize(R.dimen.fast_scroll_overlay_height);
            // mListView.setFastScrollerPopupSize(fastScrollerWidth,
            // fastScrollerHeight);
        ...
        if (null != item) {
            ...
            if (null != itemBinder) {
                ...
                if (THREAD_ID_MAIN == currentThread.getId()) {
                    ...
                    Logger.d(TAG, "addMessage() run in a background thread ");
    ...
    public int getItemViewType(int position) {
        ...
        Logger.d(TAG, "getItemViewType() the item type of position " + position
                + " is " + type);
        return type;
    }
     ...
     * @return the position for section
     */
    @Override
    public int getPositionForSection(int section) {
        Logger.d(TAG, "getPositionForSection() entry ,the section is "
                + section);
        int position = 0;
        Logger.d(TAG,
                "getPositionForSection() entry ,the mSectionToPosition is "
                        + (mSectionToPosition == null ? "null" : "not null"));
        if (null != mSectionToPosition) {
            boolean isContain = mSectionToPosition.containsKey(section);
            Logger.d(TAG, "getPositionForSection() entry ,the isContain is "
                    + isContain);
            if (isContain) {
                Logger.d(TAG,
                        "getPositionForSection() entry ,the mSectionToPosition size is "
                                + mSectionToPosition.size());
                position = mSectionToPosition.get(section);
            }
        }
        Logger.d(TAG, "getPositionForSection() exit,the position is "
                + position);
        return position;
    }
     ...
     *
     * @param position the position
     * @return the section for position
     */
    @Override
    public int getSectionForPosition(int position) {
        Logger.d(TAG, "getSectionForPosition() entry,the position is "
                + position);
        int sectionIndex = 0;
        int size = (mItemBinderList == null) ? 0 : mItemBinderList.size();
        Logger.d(TAG,
                "getSectionForPosition() entry ,the mItemBinderList size is "
                        + size);
        ...
        Logger.d(TAG, "getSectionForPosition() exit, the sectionIndex is "
                + sectionIndex);
        return sectionIndex;
    }
    ...
    public Object[] getSections() {
        ...
        for (int i = 0; i < size; i++) {
            ...
            if (item instanceof DateLabelItemBinder) {
                ...
                Logger.d(TAG, "getSections() entry, at position " + i
        ...
        Logger.d(TAG,
                "getSections(), the size of sections = " + sections.size()
                        + "\n"
                        + "getSections(), the size of mItemBinderList = "
                        + mItemBinderList.size() + "\n"
                        + "getSections(), the size of mSectionToPosition = "
                        + mSectionToPosition.size());
