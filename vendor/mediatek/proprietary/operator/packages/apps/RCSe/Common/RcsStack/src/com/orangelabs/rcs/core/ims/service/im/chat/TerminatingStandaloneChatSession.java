 ...
 * @author jexa7410
 */
public class TerminatingStandaloneChatSession extends StandaloneChatSession {
    /**
     * The logger
     */
     ...
     */
    public TerminatingStandaloneChatSession(ImsService parent, SipRequest invite) {
        super(parent, SipUtils.getAssertedIdentity(invite),
              PhoneUtils.generateContactsList(SipUtils.getAssertedIdentity(invite)),
              invite.getFeatureTags());
        ...
        if (RcsSettings.getInstance().isCPMSupported()) {
            if (logger.isActivated()) {
                logger.info("TerminatingOne2OneLargeSession  CPMS");
            }
        ...
        if (logger.isActivated()) {
            logger.info("TerminatingOne2OneLargeSession From: " + ChatUtils.getFromAias(invite));
            logger.info("TerminatingOne2OneLargeSession Display name: " + this.getRemoteDisplayName());
        }
    ...
    public void run() {
        try {
            if (logger.isActivated()) {
                logger.info("TerminatingOne2OneLargeSession Initiate a new 1-1 chat session as terminating");
            }
            ...
            if ((ChatUtils.isImdnDeliveredRequested(getDialogPath().getInvite())) ||
                ...
                if (msgId != null && !(RcsSettings.getInstance().supportOP01())) {
            ...
            if (logger.isActivated()) {
                logger.info("Auto accept large message invitation");
            }
            ...
            resp = SipMessageFactory.createCpm200OkInviteResponse(
                    getDialogPath(), InstantMessagingService.CPM_LARGE_MESSAGE_FEATURE_TAGS, sdp);
