...
public class ChatActivityManager extends PeriodicRefresher {
    ...
    public void periodicProcessing() {
        ...
        if (logger.isActivated()) {
            logger.info("Check inactivity period: inactivity=" + inactivityPeriod + ", remaining=" + remainingPeriod);
        }

        if (inactivityPeriod >= timeout) {
            if (logger.isActivated()){
                logger.info("No activity on the session during " + timeout + "s: abort the session");
            }
