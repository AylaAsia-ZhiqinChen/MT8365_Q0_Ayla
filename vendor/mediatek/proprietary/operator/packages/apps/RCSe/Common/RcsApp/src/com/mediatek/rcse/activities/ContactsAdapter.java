 ...
 */
public class ContactsAdapter extends ContactBaseAdapter implements
        OnScrollListener {
     ...
     * @param contactListFragment the contact list fragment
     */
    ...
    static class CapabilityHandler {
        ...
        /**
         * The Class CapabilityTask.
         */
        private class CapabilityTask extends
        ...
        private void queryContactCapabilities(String contact) {
            ...
            try {
                capabilityApi.getContactCapabilities(ContactIdUtils.createContactIdFromTrustedData(contact));
        ...
        public synchronized void cancel() {
            ...
            if (mCapabilityTask != null) {
                ...
                Logger.d(TAG,
                        "cancel operation falied because the mCapabilityTask is null");
            }
        }
    ...
    public void onScrollStateChanged(AbsListView view, int scrollState) {
        if (scrollState == OnScrollListener.SCROLL_STATE_IDLE) {
            ...
            RcsContact firstRcsContact = (RcsContact) view
                    .getItemAtPosition(firstPos);
            int sectionIndex = -1;
            ...
            Logger.v(TAG, "sectionIndex = " + sectionIndex + ",position = "
                    + position);
            firstPos = view.getFirstVisiblePosition();
            lastPos = view.getLastVisiblePosition();
            Logger.v(TAG, "firstPos = " + firstPos + ",lastPos = " + lastPos
                    + ",the num = " + (lastPos - firstPos) + ",position = "
                    + position);
    ...
    @Override
    public ViewHolder onConfiguerItemView(View view, int position,
            boolean isIndexer) {
        Logger.v(TAG, "onConfiguerItemView(),isIndexer = " + isIndexer
                + ",position = " + position);
    ...
    /**
     * Infalte contact item view.
     *
     * @param view the view
     * @param holder the holder
     * @param position the position
     */
    private void infalteContactItemView(View view, ViewHolder holder,
            int position) {
        holder.name = (TextView) view.findViewById(R.id.contact_list_item_name);
        holder.number = (TextView) view
                .findViewById(R.id.contact_list_item_number);
        holder.header = (AsyncAvatarView) view
                .findViewById(R.id.contact_list_item_icon);
        holder.firstLetter = (TextView) view.findViewById(R.id.alpha_indexer);
        holder.firstLetter.setVisibility(View.GONE);
        holder.contactItemLayout = (RelativeLayout) view
                .findViewById(R.id.contact_item_layout);
    ...
    private void infalteIndexerItemView(View view, ViewHolder holder,
            int position) {
        holder.firstLetter = (TextView) view.findViewById(R.id.alpha_indexer);
    ...
    /**
     * Update contact item view.
     *
     * @param view the view
     * @param holder the holder
     * @param position the position
     */
    private void updateContactItemView(View view, ViewHolder holder,
            int position) {
        holder.name.setText(mContactsList.get(position).mDisplayName);
        holder.number.setText(mContactsList.get(position).mNumber);
        holder.firstLetter.setVisibility(View.GONE);
    ...
    /**
     * Update indexer item view.
     *
     * @param view the view
     * @param holder the holder
     * @param position the position
     */
    private void updateIndexerItemView(View view, ViewHolder holder,
            int position) {
        holder.firstLetter.setText(mContactsList.get(position).mSortKey);
        holder.firstLetter.setVisibility(View.VISIBLE);
        holder.number = null;
        ...
        TextView numberView = (TextView) holder.contactItemLayout
                .findViewById(R.id.contact_list_item_number);
