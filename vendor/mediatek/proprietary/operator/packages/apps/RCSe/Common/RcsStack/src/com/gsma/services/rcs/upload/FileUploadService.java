...
public final class FileUploadService extends RcsService {
    ...
    public final void connect() throws RcsPermissionDeniedException {
       /* if (!sApiCompatible) {
        ...
        intent.setAction(IFileUploadService.class.getName());
    ...
    private ServiceConnection apiConnection = new ServiceConnection() {
        public void onServiceConnected(ComponentName className, IBinder service) {
            ...
            IBinder binder = null;
            try {
                binder = mCoreServiceWrapperBinder.getUploadServiceBinder();
            } catch (RemoteException e1) {
                Logger.i(TAG, "onServiceConnected exception");
                e1.printStackTrace();
            }
            setApi(IFileUploadService.Stub.asInterface(binder));
            if (mListener != null) {
