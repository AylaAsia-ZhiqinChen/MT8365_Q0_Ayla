...
public class ChunkReceiver extends Thread {
    ...
    public void run() {
        try {
            ...
            while(!terminated) {
                ...
                if (responseCode != -1) {
                    ...
                    if (method.toString().equals(MsrpConstants.METHOD_SEND)) {
                        ...
                        connection.getSession().receiveMsrpSend(
                                txId, headers, continuationFlag, data, totalSize);
                    } else
                        if (method.toString().equals(MsrpConstants.METHOD_REPORT)) {
                            if (logger.isActivated()) {
                                logger.debug("Receive MSRP REPORT request " + method);
                                String lines[] = trace.toString().split("\\r?\\n");
                                logger.debug("{");
                                for(int i = 0;i<lines.length;i++ )
                                    logger.debug("===>  "+lines[i]);
            ...
            if (terminated) {
                ...
                connection.getSession().getMsrpEventListener().msrpTransferError(
                        null, /*e.getMessage()*/"timeout");
            }
            terminated = true;
        }
    }
    ...
    private StringBuffer readLine() throws IOException {
        ...
        try {
            ...
            throw new IOException("Reading from socket failed");
        }
        /** @} */
            ...
            for (int j = 0; (b = stream.read()) != -1; j++) {
                ...
                if (b != match[0]) {
                    ...
                    if (tagFound) {
                        ...
                        // j+1 characters read; remove tagLength characters
                        // and CR/LF plus one extra character for continuation flag
                        result = new byte[j - tagLength];
                        // remove tag and CR/LF
                        System.arraycopy(buffer, 0, result, 0, j - tagLength - 1);
