 ...
 */
public final class ChatListView extends View implements OnClickListener,
        ContactsListManager.OnDisplayNameChangedListener {
    ...
    public ChatListView(Activity activity, OnSelectedItemListener listener) {
        ...
        mRootView = LayoutInflater.from(mActivity).inflate(
                R.layout.chats_list_layout, null);
        mListener = listener;
        mInviteContainer = (LinearLayout) mRootView
                .findViewById(R.id.invite_container_layout);
        mInviteHeader = mRootView.findViewById(R.id.invite_header_layout);
        mChatContainer = (LinearLayout) mRootView
                .findViewById(R.id.chat_container_layout);
    }

    /**
     * Gets the chat container.
     *
    ...
    /**
     * Gets the invite container.
     *
     * @return the invite container
     */
     ...
     */
    private void updateInvitations(final View view,
            final InvitationStruct inviteStructAfterChange) {
            ...
            mMainHandler.post(new Runnable() {
                ...
                public void run() {
                    ...
                    if (inviteStructAfterChange != null) {
                        ...
                        Logger.e(TAG,
                                "updateInvitations the inviteStructAfterChange is null");
                    }
                }
            });
        }
    ...
    public void addInviteItem(Object obj) {
        ...
        if (obj != null) {
            ...
            View view = LayoutInflater.from(mActivity).inflate(
                    R.layout.invite_item, null);
            AsyncAvatarView avatarImageView = (AsyncAvatarView) view
                    .findViewById(R.id.invite_contact_quick);
            try {
                String phoneNumUri = invite.getChatSession().getRemoteContact().toString();
                String phoneNum = Utils.extractNumberFromUri(phoneNumUri);
                if (phoneNum != null) {
            ...
            DateView textViewDate = (DateView) view
                    .findViewById(R.id.invite_date);
    ...
    public void addChatItem(Object obj) {
        ...
        View view = LayoutInflater.from(mActivity).inflate(R.layout.chat_item,
                null);
        view.setOnClickListener(this);
        ...
        if (chat != null) {
            if (chat.isGroupChat()) {
                ...
                if (chatMap != null) {
                    ...
                    List<String> nunList = (List<String>) chatMap
                            .get(ChatListFragment.NUMBER);
                ...
                StringBuilder chatNum = new StringBuilder();
                chatNum.append(LEFT_BRACKET).append(chat.getParticipantNum())
                        .append(RIGHT_BRACKET);
                participantNum.setText(chatNum.toString());
            } else {
                ...
                if (chatMap != null) {
                    ...
                    if (phoneNum != null) {
                        Logger.w(TAG, "addChatItem  the phonenum is "
                                + phoneNum);
                        avatarImageView.setAsyncContact(phoneNum);
                    } else {
            ...
            TextView unreadMessage = (TextView) view
                    .findViewById(R.id.unread_message_num);
            unreadMessage.setVisibility(View.GONE);
        } else {
     ...
     *
     * @param sessionId
     *            The session id that identify the invitation item.
     */
        ...
        for (int i = 0; i < viewList.length; i++) {
            ...
            if (view != null) {
                ...
                if (obj instanceof InvitationStruct) {
                    ...
                    Logger.d(TAG,
                            "Current obj data is not a InvitationStruct instance");
                }
            } else {
    ...
    public static class InvitationStruct {
         ...
         */
        public InvitationStruct(Drawable drawable, Date date,
                String information, Intent intent) {
    ...
    /**
     * The Class MessageItemStructForContact.
     */
    public static class MessageItemStructForContact {
        private String mLatestMessageId = "";

        /**
         * Gets the mlatest message id.
         *
         * @return the mlatest message id
         */
        ...
        /**
         * Sets the mlatest message id.
         *
         * @param mlatestMessageId the new mlatest message id
         */
        ...
        /**
         * Gets the mlatest message.
         *
         * @return the mlatest message
         */
        ...
        /**
         * Sets the mlatest message.
         *
         * @param mlatestMessage the new mlatest message
         */
    ...
    public static class ChatsStruct {
        ...
        public ArrayList<MessageItemStructForContact> chatItems =
                new ArrayList<MessageItemStructForContact>();
         ...
         * Sets the m subject.
         *
         * @param mSubject the new m subject
         */
         ...
         * Structure method.
         *
         * @param isGroupChat the is group chat
         ...
         *
         * @param isFileTransfer            True for file transfer, false for none file transfer
         */
        ...
        /**
         * Gets the one2 one display name.
         *
         * @return the one2 one display name
         */
        ...
        public TreeSet<String> getContacts() {
            if (mIsGroupChat) {
                ...
                if (null != mChatMap) {
                    ...
                    Logger.e(TAG,
                            "getContacts() this is a group chat, but mChatMap is null");
                    return null;
                }
            } else {
    ...
    /**
     * Gets the chat pair with tag.
     *
     * @param chatWindowTag the chat window tag
     * @return the chat pair with tag
     */
     ...
     *
     * @param chatWindowTag
     *            the specific window Tag whose message need to be added.
     * @param message
     *            the message need to be added
     * @param isRead
     *            the message is read or not
     */
    public void addMessage(Object chatWindowTag, ChatMessage message,
            boolean isRead) {

        Logger.d(TAG, "addMessage() entry chatWindowTag is " + chatWindowTag
                + " message is " + message);
        Pair<View, ChatsStruct> findPair = getChatPairWithTag(chatWindowTag);
        if (null == findPair) {
            Logger.e(TAG,
        ...
        if (null != message) {
            ...
            //chatStruct.setDate(message.getTimestampDelivered());
            chatStruct.setLatestMessage(message.getContent());

            MessageItemStructForContact newMessage = new MessageItemStructForContact();
            //newMessage.setmDate(message.getTimestampDelivered());
            newMessage.setMlatestMessage(message.getContent());
            ...
            for (int i = 0; i < chatStruct.chatItems.size(); i++) {
                Logger.d(TAG, "addMessage() chatStruct is " + chatStruct
                        + "msgID : " + message.getId());

                if (!chatStruct.chatItems.isEmpty()
                        && message.getId() != null
                        && (message.getId().equals(chatStruct.chatItems.get(i)
                                .getMlatestMessageId()))) {
     ...
     *
     * @param chatWindowTag            the specific window Tag whose history need to be clear.
     ...
     * add a message in a particular Window.
     *
     * @param chatWindowTag            the specific window Tag whose message need to be added.
     ...
     *
     * @param chatWindowTag
     *            the specific window Tag whose history need to be clear.
     */
    ...
    /**
     * On file transfer state changed.
     *
     * @param chatWindowTag the chat window tag
     * @param isActive the is active
     */
    public void onFileTransferStateChanged(Object chatWindowTag,
            boolean isActive) {
        Logger.d(TAG, "onFileTransferStateChanged() entry the window tag is "
                + chatWindowTag + " isActive " + isActive);
        Pair<View, ChatsStruct> findPair = getChatPairWithTag(chatWindowTag);
        if (null == findPair) {
            Logger.e(TAG,
