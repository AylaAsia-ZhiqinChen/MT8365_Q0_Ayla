...
import com.mediatek.rcse.service.binder.IRemoteMultiChatWindow;
 ...
 */
public class PluginMultiChatWindow extends IRemoteMultiChatWindow.Stub
        implements WindowTagGetter, PhotoLoaderManager.OnPhotoChangedListener {
    /**
     * The Constant TAG.
     */
    private static final String TAG = "PluginMultiChatWindow";
    /**
     * The Constant MULTI_CONTACT_STRING_BEGINNER.
     */
    public static final String MULTI_CONTACT_STRING_BEGINNER = "7+++";
    ...
    public List<ParticipantInfo> mParticipantList = new ArrayList<ParticipantInfo>();
    ...
    public String getDisplayName() {
        ...
        String displayname = mSubject;
        if(displayname == null )
            return "";
        if(!displayname.equals("")) {
            return displayname;
        }
        for (ParticipantInfo parInfo : mParticipantList) {
           displayname = displayname + "[" + parInfo.getParticipant().getDisplayName() + "]";
        }
        Logger.d(TAG, "getDisplayName() exit mKey is " + mKey
    ...
    /**
     * Instantiates a new plugin multi chat window.
     *
     * @param windowTag the window tag
     * @param messageManager the message manager
     * @param participantList the participant list
     */
    public PluginMultiChatWindow(String windowTag,
        ...
        mContactString = MULTI_CONTACT_STRING_BEGINNER + windowTag;
        Logger.d(TAG, "PluginMultiChatWindow() constructor, windowTag: "
    ...
    public IRemoteReceivedChatMessage addReceivedMessage(ChatMessage message,
       ...
       if (null != message) {
           ...
           /*ChatMessage newMessage = new ChatMessage(message.getId(),
                   mContactString, message.getContent(), message.getMimeType(), message.getDisplayName(), message.getDirection().toInt(),
                   message.getTimestamp(), message.getTimestampSent(), message.getTimestampDelivered(), message.getTimestampDisplayed(),
                   message.getStatus().toInt(), message.getReasonCode().toInt(),message.getChatId(), message.isRead(), message.isExpiredDelivery() );*/
    ...
    public static String generateGroupChatInvitationContact(String sessionId) {
        ...
        String contact = MULTI_CONTACT_STRING_BEGINNER + groupChatInvitationTag;
    ...
    private static boolean isContactContainsInvitationBeginner(String contact) {
        Logger.d(TAG, "isContactContainsInvitation() entry contact is "
                + contact);
        boolean ret = false;
        if (contact.contains(MULTI_CONTACT_STRING_BEGINNER
    ...
    public static void removeAllGroupChatInvitationsInMms() {
        ...
        contentResolver.delete(PluginUtils.SMS_CONTENT_URI, Sms.ADDRESS
                + " LIKE '" + MULTI_CONTACT_STRING_BEGINNER
