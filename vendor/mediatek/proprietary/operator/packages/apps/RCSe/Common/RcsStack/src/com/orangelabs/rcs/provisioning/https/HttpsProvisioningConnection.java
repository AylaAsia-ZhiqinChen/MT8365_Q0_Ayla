...
public class HttpsProvisioningConnection {
    ...
    /**
     * HttpsProvisioningManager manages HTTP and SMS reception to load provisioning from
     * network
     */
    ...
    /**
     * Separate Network state listener
     */
    private PhoneStateListener separateNetworkStateListener = null;
    ...
    public HttpsProvisioningConnection(HttpsProvisioningManager httpsProvisioningManager) {
        ...
        if (connMgr == null) {
            connMgr = (ConnectivityManager)httpsProvisioningManager.getContext()
                .getSystemService(Context.CONNECTIVITY_SERVICE);
        }
        if (telMgr == null) {
            telMgr = (TelephonyManager)httpsProvisioningManager.getContext()
     ...
     * Register the broadcast receiver for network state
     */
    protected void registerSeparateNetworkStateListener(Looper looper) {
        // Check if network state listener is already registered
        if (separateNetworkStateListener != null) {
            if (logger.isActivated()) {
                logger.debug("Separete Network state listener already registered");
            }
            return;
        }

        if (logger.isActivated()) {
            logger.debug("Registering Separete network state listener");
        }

        // Instantiate the network state listener
        separateNetworkStateListener = new PhoneStateListener(
                SubscriptionManager.getDefaultDataSubscriptionId(), looper) {
            @Override
            public void onServiceStateChanged(ServiceState state) {
                if (logger.isActivated()) {
                    logger.debug("OnServiceStateChanged->dataState:"+ state.getDataRegState());
                }
                ...
                if (state.getDataRegState() == ServiceState.STATE_IN_SERVICE) {
                    Thread t = new Thread() {
                        ...
                        public void run() {
                            manager.connectionEvent(ConnectivityManager.CONNECTIVITY_ACTION);
                        }
                    };
                    t.start();
                }
        ...
        telMgr.listen(separateNetworkStateListener, PhoneStateListener.LISTEN_SERVICE_STATE);
    }

    /**
     * Unregister the broadcast receiver for network state
     */
    protected void unregisterSeparateNetworkStateListener() {
        if (separateNetworkStateListener != null) {
            if (logger.isActivated()) {
                logger.debug("Unregistering Separete network state listener");
            }

            telMgr.listen(separateNetworkStateListener, PhoneStateListener.LISTEN_NONE);

            separateNetworkStateListener = null;
        }
    }
}
