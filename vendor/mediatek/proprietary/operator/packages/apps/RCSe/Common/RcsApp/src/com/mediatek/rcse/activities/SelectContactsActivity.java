...
public class SelectContactsActivity extends Activity {
    ...
    public static final String KEY_ONLY_NEED_ORIGINAL_CONTACTS = "only_need_original_contacts";
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mCallBackActivity = intent
                .getStringExtra(ChatMainActivity.KEY_ADD_CONTACTS);
        mParticipantList = intent
        ...
        if (mParticipantList != null) {
            Logger.d(TAG, "onCreate() the mParticipantlist size is "
                    + mParticipantList.size());
            mContactsListFragment.setExistingContacts(mParticipantList);
            mContactsListFragment.setIsNeedOriginalContacts(intent
                    .getBooleanExtra(KEY_IS_NEED_ORIGINAL_CONTACTS, false));
            mContactsListFragment.setIsOnlyNeedOriginalContacts(intent
                    .getBooleanExtra(KEY_ONLY_NEED_ORIGINAL_CONTACTS, false));
        } else {
        ...
        if (null != action
                && action
                        .equals(PluginApiManager.RcseAction.SELECT_CONTACT_ACTION)) {
        ...
        fm.beginTransaction().add(android.R.id.content, mContactsListFragment)
                .commit();
    }
}
