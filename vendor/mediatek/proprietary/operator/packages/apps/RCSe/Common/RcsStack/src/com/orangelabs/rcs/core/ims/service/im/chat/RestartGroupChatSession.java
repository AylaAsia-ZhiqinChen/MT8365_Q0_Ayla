...
public class RestartGroupChatSession extends GroupChatSession {
     ...
     */
    public RestartGroupChatSession(ImsService parent, String conferenceId, String subject, Map<String, ParticipantStatus> participants, String contributionId) {
        ...
        if (RcsSettings.getInstance().isCPMSupported()) {
            if (logger.isActivated()) {
                logger.info("CPMS RestartGroupChatSession old call id: "
                        + getDialogPath().getCallId());
            }
            RichMessagingHistory rmHistory = RichMessagingHistory.getInstance();
            String conversationId = rmHistory.getCoversationID(contributionId, 2);
            if (logger.isActivated()) {
                logger.info("CPMS RestartGroupChatSession1 ");
            }
            if (conversationId == null || conversationId.equals("")) {
                String callId = getImsService().getImsModule().getSipManager().getSipStack()
                        .generateCallId();
                if (logger.isActivated()) {
                    logger.info("CPMS RestartGroupChatSession call id: " + callId);
                }
                // Set conversation ID
                conversationId = ContributionIdGenerator.getContributionId(callId);
                setConversationID(conversationId);
                rmHistory.UpdateCoversationID(contributionId, conversationId, 1);
            } else {
                setConversationID(conversationId);
            }
        }
    ...
    private SipRequest createInviteRequest(String content) throws SipException {
        ...
        if(!RcsSettings.getInstance().isCPMSupported()){
            ...
            if (logger.isActivated()) {
                logger.info("RestartGroupChatSession createInviteRequest CPMS");
            } 
        ...
        if(RcsSettings.getInstance().isCPMSupported()){
            if (logger.isActivated()) {
                logger.info("RestartGroupChatSession createInviteRequest0 CPMS");
            } 
