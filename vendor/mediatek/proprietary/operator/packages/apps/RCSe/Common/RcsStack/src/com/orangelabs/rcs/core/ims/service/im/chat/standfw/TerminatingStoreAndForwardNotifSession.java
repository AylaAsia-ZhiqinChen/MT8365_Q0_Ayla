...
public class TerminatingStoreAndForwardNotifSession extends OneOneChatSession implements MsrpEventListener {
    ...
    public void run() {
        try {
            ...
            if(!RcsSettings.getInstance().isCPMSupported()){
                 ...
                 if (logger.isActivated()) {
                    logger.info("TerminatingStoreAndForwardNotifSession  CPMS");
                 }
            ...
            if(RcsSettings.getInstance().isCPMSupported()){
                if (logger.isActivated()) {
                    logger.info("TerminatingStoreAndForwardNotifSession1  CPMS");
                 }
     ...
     * @param error Error
     */
    public void handleInviteError(SipResponse resp) {
        ...
        // Error    
        if (logger.isActivated()) {
            logger.info("Session Invite error: " + error.getErrorCode() + ", reason=" + error.getMessage());
        }
            ...
            try {
                ...
                if (cpimMsg != null) {
                    ...
                    if(isGroupChat()){
                        ...
                        String number = getRemoteContact();
                        from = PhoneUtils.extractNumberFromUri(number);
                    }
                    if (logger.isActivated()) {
                        logger.debug("ABC from is: " + from);
                    }
    ...
    public void receiveMessageDeliveryStatus(String contact, String xml) {
        try {
            ...
            if (imdn != null) {
                ...
                if (logger.isActivated()) {
                    logger.info("ABC receiveMessageDeliveryStatus: " + imdn.getMsgId() + "Notify outside session");
                }
                getImsService().getImsModule().getCore().getListener().handleMessageDeliveryStatus(contact,
                        imdn.getMsgId(), imdn.getStatus(), null);
            }
        } catch(Exception e) {
            if (logger.isActivated()) {
