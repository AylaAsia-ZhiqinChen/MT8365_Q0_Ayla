...
public class NetworkChangedReceiver extends BroadcastReceiver {
    ...
    public void onReceive(final Context context, Intent intent) {
        ...
        if (action.equals(ConnectivityManager.CONNECTIVITY_ACTION)) {
            ...
            if (info.getType() == ConnectivityManager.TYPE_MOBILE) {
                ...
                Logger.d(TAG,
                        "onReceive() getActiveNetworkInfo activeNetworkInfo: "
    ...
    private void handleFtCapabilityChanged(NetworkInfo info) {
        ...
        Logger.d(TAG,
                "handleFtCapabilityExchange()-current network sub-type name = "
                        + type);
        if (networkType == ConnectivityManager.TYPE_MOBILE) {
            if (type.toLowerCase().equals(GSM)
                ...
                if (RcsSettings.getInstance().isFileTransferSupported()) {
                    ...
                    || type.toLowerCase().startsWith(CDMA)
                ...
                Logger.d(TAG,
                        "handleFtCapabilityExchange()-other network sub-type name.");
            }
    ...
    private void exchangeMyFtCapability(boolean isFtCapabilityEnable) {
        ExchangeMyCapability exchangeMyCapability = ExchangeMyCapability
                .getInstance(mContext);
        if (exchangeMyCapability == null) {
            Logger.d(TAG, "exchangeMyCapability()-exchangeMyCapability is null");
        } else {
            Logger.d(TAG,
                    "exchangeMyCapability()-exchangeMyCapability is not null");
            exchangeMyCapability.notifyCapabilityChanged(
                    ExchangeMyCapability.FILE_TRANSFER_CAPABILITY_CHANGE,
                    isFtCapabilityEnable);
        }
    }


    /**
     * This interface defines a listener to handle the network connection status
     * changes.
     */
