 ...
 */
public class SentFileTransferItemBinder extends AbsItemBinder implements
        OnClickListener {
    ...
    /**
     * Constructor.
     * @param fileTransfer fileTransfer object
     */
    ...
    private void bindProgressFile(FileStruct fileStruct, View itemView) {
        Logger.d(TAG, "bindProgressFile: itemView = " + itemView
        ...
        Status status = mFileTransfer.getStatue();
        TextView fileSize = (TextView) itemView
                .findViewById(R.id.file_transfer_size);
        RelativeLayout statusLayout = (RelativeLayout) itemView
                .findViewById(R.id.file_transfer_layout_status);
        TextView statusText = (TextView) itemView
                .findViewById(R.id.file_transfer_status);
        ...
        ImageView playPauseView = (ImageView) itemView
                .findViewById(R.id.file_transfer_btn_play_pause);
        ...
        if (progressBar != null) {
            ...
            case WAITING:
                ...
                playPauseView.setVisibility(View.INVISIBLE);
                fileSize.setText(Utils.formatFileSizeToString(fileStruct.mSize,
                        0));
        ...
        ImageView cancelView = (ImageView) itemView
                .findViewById(R.id.file_transfer_btn_cancel);
        if (cancelView != null && status != Status.PENDING) {
    ...
    private void bindCancelView(View itemView) {
        ...
        Logger.v(TAG, "bindCancelView: itemView = " + itemView + ", context = "
                + context);
        ...
        if (mFileTransfer != null) {
            ...
            stringBuilder.append(context
                    .getString(R.string.file_transfer_canceled));
            stringBuilder.append(WHITE_SPACE);
            FileStruct fileStruct = mFileTransfer.getFileStruct();
            if (fileStruct != null) {
                ...
                stringBuilder.append(WHITE_SPACE);
                stringBuilder.append(context
                        .getString(R.string.file_transfer_at));
                stringBuilder.append(WHITE_SPACE);
    ...
    private void bindCanceledView(View itemView) {
        Context context = ApiManager.getInstance().getContext();
        Logger.v(TAG, "bindCanceledView: itemView = " + itemView
                + ", context = " + context);
        if (itemView == null || context == null) {
            return;
        }
        ...
        if (mFileTransfer != null) {
            ...
            if (mFileTransfer instanceof One2OneChatFragment.SentFileTransfer) {
                stringBuilder
                        .append(((One2OneChatFragment.SentFileTransfer) mFileTransfer)
                                .getContactName());
            }
            stringBuilder.append(WHITE_SPACE);
            stringBuilder.append(context
    ...
    private void bindFailedView(View itemView) {
        Logger.v(TAG, "bindFailedView: itemView = " + itemView);
        if (null == itemView) {
            return;
        }

        DateView date = (DateView) itemView.findViewById(R.id.chat_item_date);
        Button resendView = (Button) itemView
                .findViewById(R.id.file_transfer_resent);
        if (resendView != null) {
        ...
        if (context != null) {
            failedText = context
                    .getString(R.string.file_transfer_upload_failed);
            if (textView != null) {
                textView.setText(failedText);
            } else {
                Logger.w(TAG, "bindFailedView(), TextView is null");
        }
        } else {
        ...
        if (mFileTransfer != null && mFileTransfer.getFileStruct() != null) {
            ...
            if (fileTransferView != null) {
                ...
                Logger.w(TAG, "bindFailedView(), fileTransferView is null ");
            }
        } else {
    ...
    private void bindTimeoutView(View itemView) {
        Logger.v(TAG, "bindTimeoutView: itemView = " + itemView);
        if (null == itemView) {
            return;
        }
        ...
        if (context != null) {
            timeoutText = context
                    .getString(R.string.file_transfer_receiver_timeout);
            if (textView != null) {
                textView.setText(timeoutText);
            } else {
                Logger.w(TAG, "bindTimeoutView(), TextView is null");
            }
        } else {
            Logger.w(TAG, "bindTimeoutView(), context is null");
        }
        FileStruct fileStruct = null;
        if (mFileTransfer != null && mFileTransfer.getFileStruct() != null) {
            fileStruct = mFileTransfer.getFileStruct();
            ...
            if (fileTransferView != null) {
                ...
                Logger.w(TAG, "bindTimeoutView(), fileTransferView is null ");
            }
        } else {
            Logger.w(TAG, "mFileTransfer is null, or fileStruct is null ");
        }
    ...
    private void setRejectText(TextView textView) {
        ...
        if (context != null) {
            rejectTextString = context
                    .getString(R.string.file_transfer_rejected);
        } else {
    ...
    private void handleCancelFileTransfer() {
        Logger.v(TAG, "handleCancelFileTransfer: mFileTransfer = "
                + mFileTransfer);
        if (mFileTransfer != null) {
            ...
            if (fileStruct != null) {
                Message message = ControllerImpl.getInstance().obtainMessage(
                        ...
                        mFileTransfer.getTag(), fileStruct.mFileTransferTag);
                if (message != null) {
    ...
    private void handlePlayPauseFileTransfer(View v) {
        Logger.v(TAG, "handlePlayPauseFileTransfer: mFileTransfer = "
                + mFileTransfer);
        if (mFileTransfer != null) {
            // mFileTransfer.setStatus(Status.CANCEL);
            FileStruct fileStruct = mFileTransfer.getFileStruct();
            if (fileStruct != null) {
                if (mIsPause == false) {
                    Message message = ControllerImpl.getInstance()
                            .obtainMessage(
                                    ChatController.EVENT_FILE_TRANSFER_PAUSE,
                                    mFileTransfer.getTag(),
                    ...
                    Message message = ControllerImpl.getInstance()
                            .obtainMessage(
                                    ChatController.EVENT_FILE_TRANSFER_RESUME,
                                    mFileTransfer.getTag(),
                                    fileStruct.mFileTransferTag);
                    message.arg1 = 0; // sent file case
                    if (message != null) {
    ...
    private void handleStatusFileTransfer(Context context) {
        Logger.v(TAG, "handleStatusFileTransfer: mFileTransfer = "
                + mFileTransfer);
        ChatScreenWindow window = ChatScreenWindowContainer.getInstance()
                .getFocusWindow();
        List<Participant> participants = ((GroupChatWindow) window)
                .getmGroupChatFragment().getParticipants();
    ...
    private void bindSendFileFinished(FileStruct fileStruct, View itemView) {
        ...
        ImageView statusView = (ImageView) itemView
                .findViewById(R.id.file_transfer_btn_status);
        if (statusView != null) {
