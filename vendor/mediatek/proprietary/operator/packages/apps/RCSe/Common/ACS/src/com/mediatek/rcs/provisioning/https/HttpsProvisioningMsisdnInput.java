...
public final class HttpsProvisioningMsisdnInput {
    public static final String TAG = "HttpsProvisioningMsisdnInput";
     ...
     * @return ContactId
     */
    public ContactId displayPopupAndWaitResponse(Context context, ContactId contact) {
        ...
        try {
            Log.d(TAG, "displayPopupAndWaitResponse before wait:");
            synchronized (sInstance) {
                super.wait();
            }
            Log.d(TAG, "displayPopupAndWaitResponse after wait:");
    ...
    protected void responseReceived(ContactId contact) {
        synchronized (sInstance) {
            Log.d(TAG, "responseReceived contact:" + contact);
            mMsisdn = contact;
            super.notify();
        }
    }
