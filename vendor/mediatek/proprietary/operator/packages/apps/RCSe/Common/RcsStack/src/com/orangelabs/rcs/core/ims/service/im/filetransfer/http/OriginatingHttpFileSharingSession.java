        ...
        if (result != null) {
            ...
            if (chatSession != null) {
                ...
                try {
                    ...
                    final ChatSession newChatSession = chatSession;

                    // Update rich messaging history
                    RichMessagingHistory.getInstance().addChatMessage(firstMsg, RcsService.Direction.OUTGOING.toInt());

                    // Start the session
                    Thread t = new Thread() {
                        public void run() {
                            newChatSession.startSession();
                        }
                    };
                    t.start();
                ...
                RichMessagingHistory.getInstance().addFileTransfer(getRemoteContact(), getSessionID(), RcsService.Direction.OUTGOING.toInt(), getContent());
                
                // Add session in the list
                OneToOneChatImpl sessionApi = new OneToOneChatImpl(getRemoteContact(), (OneOneChatSession)chatSession);
