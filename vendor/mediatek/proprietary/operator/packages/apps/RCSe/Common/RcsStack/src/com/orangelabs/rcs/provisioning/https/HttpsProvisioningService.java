...
public class HttpsProvisioningService extends Service {
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        ...
        // useless code, comment for security reason
        // registerReceiver(retryReceiver, new IntentFilter(ACTION_RETRY));
        httpsProvisioningMng = new HttpsProvisioningManager(
                AndroidFactory.getApplicationContext(), retryIntent, first, user);
        if (logger.isActivated()) {
            logger.debug("Provisioning parameter: boot=" + first + ", user=" + user +
                    ", version=" + version);
        }
        ...
        if (first) {
            ...
            } else if (ProvisioningInfo.Version.DISABLED_DORMANT.equals(version) && user) {
                requestConfig = true;
            } else { // version > 0
