...
//import com.orangelabs.rcs.utils.TimerUtils;
...
public abstract class PeriodicRefresher {
    ...
    private String INTENT_PERMISSION = "com.orangelabs.rcs.PeriodicRefresher";
...
*/
    ...
    public synchronized void startTimer(long currentTime, long expirePeriod, double delta) {
        ...
        if (logger.isActivated()) {
            logger.debug("Start timer at period=" + pollingPeriod + "ms (expiration="
                    + expirePeriod + "ms)");
        }
        ...
        AndroidFactory.getApplicationContext().registerReceiver(alarmReceiver,
                new IntentFilter(action), INTENT_PERMISSION, null);
        timerStarted = true;
    }
    ...
    public PeriodicRefresher() {
        ...
        intent.setPackage("com.orangelabs.rcs");
    ...
    public void startTimer(int expirePeriod, double delta) {
        ...
        if (logger.isActivated()) {
            logger.debug("Start timer at period="
                    + pollingPeriod +  "s (expiration=" + expirePeriod + "s)");
        }
        /**
        ...
        if (!timerStarted) {
            synchronized (timerStarted) {
                ...
                if (!timerStarted) {
                    logger.debug("startTimer() begin to register, PeriodicRefresher is "
                            + this);
                    ...
                    logger.error("startTimer() timerStarted is true, PeriodicRefresher is "
                                    + this);
                }
            }
        } else {
    ...
    public void stopTimer() {
        logger.debug("stopTimer() entry, Refresher is " + this);
        if (timerStarted) {
            synchronized (timerStarted) {
                ...
                if (timerStarted) {
                    ...
                    logger.debug("stopTimer() begin to stop, Refresher is " + this);
                    ...
                    logger.error("stopTimer() timerStarted is false, Refresher is " + this);
                }
            }
        } else {
            logger.error("stopTimer() timerStarted is false, Refresher is " + this);
