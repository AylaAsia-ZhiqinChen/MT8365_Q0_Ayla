...
public final class ImageLoader {
    ...
    private static final String[] PROJECTION = new String[] { Contacts._ID,
            Contacts.LOOKUP_KEY, Contacts.PHOTO_URI };
    ...
    static {
        ...
        if (null != resource) {
            ...
            BLANK_BITMAP = BitmapFactory.decodeResource(resource,
    ...
    public static Bitmap getImage(Object key) {
        ...
        if (getInstance().isImageLoaded(key)) {
            ...
            Logger.d(TAG, "getImage() the image " + key
                    + " has not been loaded, just return null");
            return null;
        }
    }
     ...
     *
     * @param key            The key of this image
     * @param listener            The callback listener user to notify the image has been loaded
     *            successfully, null indicates that the called needs
     *            synchronized loading
     * @return The Bitmap instance if the image has already been decoded, null
     *         if the image has not been decoded
     */
    public static Bitmap requestImage(Object key,
            OnLoadImageFinishListener listener) {
        Logger.d(TAG, "requestImage() the key is " + key + " the listener is "
                + listener);
            ...
            if (null != listener) {
                ...
                Logger.d(
                        ...
                        "requestImage() the image "
                                + key
                                + " has not been loaded, " +
                                "sync load the requested image and return it");
     ...
     * @param key the key
     * @param listener the listener
     * @return the load image task
     */
    private LoadImageTask generateTask(Object key,
            OnLoadImageFinishListener listener) {
    ...
    private Bitmap getExistingImage(Object key) {
        ...
        if (null != reference) {
            ...
            Logger.e(TAG, "getExistingImage() reference is null, the key is "
                    + key);
            return null;
        }
    }
    ...
    /**
     * Checks if is participants image loaded.
     *
     * @param key the key
     * @return true, if is participants image loaded
     */
    ...
    private void setImage(Object key, Bitmap image) {
        ...
        if (key instanceof TreeSet<?>) {
            mParticipantsImageMap.put((TreeSet<String>) key,
                    new BitmapReference(image, mBitmapReferenceQueue, key));
        } else {
            mImageMap.put(key, new BitmapReference(image,
    ...
    /**
     * Clear unreachable bitmap.
     */
    private void clearUnreachableBitmap() {
        BitmapReference reference = null;
        while (null != (reference = (BitmapReference) mBitmapReferenceQueue
                .poll())) {
            Object key = reference.mKey;
            Logger.d(TAG, "clearUnreachedBitmap () remove unreached Bitmap "
                    + key);
            if (key instanceof TreeSet<?>) {
    ...
    private static class BitmapReference extends SoftReference<Bitmap> {
        ...
        /**
         * Instantiates a new bitmap reference.
         *
         * @param r the r
         * @param q the q
         * @param key the key
     ...
     */
    private abstract static class LoadImageTask extends
        ...
        public static void interrupt(Object key) {
            ...
            if (task != null) {
                ...
                case FINISHED:
                    ...
                    Logger.d(TAG, "interrupt() the image " + key + " is "
                            + status + ", remove it from LIST and MAP");
                    break;
                case RUNNING:
                ...
                Logger.e(TAG, "interrupt() TASK_MAP doesn't contain the key "
                        + key);
            }
        }
        ...
        protected void onPostExecute(Bitmap result) {
            ...
            if (mIsActive) {
                ...
                Logger.d(TAG, "onPostExecute() the loading " + mKey
                        + "has been disabled");
            }
            ...
            int size = TASK_LIST.size();
            if (size > 0) {
                Logger.v(TAG, "onPostExecute() there is still " + size
                        + " waiting images to load");
                TASK_LIST.get(0).executeOnExecutor(
                        AsyncTask.THREAD_POOL_EXECUTOR);
            } else {
                Logger.v(TAG,
                        "onPostExecute() there is not any waiting images to load");
            }
        }
    }
    ...
    private class LoadImageFileTask extends LoadImageTask {
        ...
        /**
         * Instantiates a new load image file task.
         *
         * @param path the path
         * @param listener the listener
         */
        private LoadImageFileTask(String path,
                OnLoadImageFinishListener listener) {
            super(path, listener);
        }
        ...
        protected Bitmap doInBackground(Void... params) {
            ...
            if (null == image) {
                Logger.d(TAG, "doInBackground() the file " + mKey
                        + " has not been loaded yet");
                if (com.mediatek.rcse.service.Utils.isFileExist((String) mKey)) {
                    image = ThumbnailUtils.createImageThumbnail((String) mKey,
                    ...
                    Logger.e(TAG, "doInBackground() the file " + mKey
                            + " doesn't exist!");
                }
            } else {
                Logger.w(TAG, "doInBackground() the file " + mKey
    ...
    private class LoadImageUriTask extends LoadImageTask {
        ...
        /**
         * Instantiates a new load image uri task.
         *
         * @param uri the uri
         * @param listener the listener
         */
     ...
     */
    private static class LoadParticipantsImageTask extends
        ...
        /**
         * Instantiates a new load participants image task.
         *
         * @param key the key
         * @param listener the listener
         */
        private LoadParticipantsImageTask(TreeSet<String> key,
                OnLoadImageFinishListener listener) {
            mListener = listener;
            mKey = key;
            if (null != mListener) {
        ...
        private Bitmap processBitmaps(List<Bitmap> bitMapList) {
            ...
            one = Bitmap.createBitmap(one, block * 4, 0, block * 9,
                    one.getHeight());
            ...
            canvas.drawBitmap(three, one.getWidth() + 1, two.getHeight() + 1,
                    null);
         ...
         * @return the list
         */
        private static List<Bitmap> fillBitmapList(int count,
        ...
        protected void onPostExecute(List<Bitmap> result) {
            ...
            if (mIsActive) {
                if (null != mListener) {
                    ...
                    Logger.d(TAG, "onPostExecute the mKey is prossedMap is "
                            + mKey + prossedMap);
                    getInstance().setImage(mKey, prossedMap);
                    mListener.onLoadImageFished(prossedMap);
                } else {
                    Logger.e(TAG, "onPostExecute() result is " + result
                            + " listener is " + mListener);
                }
            } else {
                Logger.d(TAG, "onPostExecute() the loading " + mKey
                        + "has been disabled");
        ...
        private Bitmap parsePhoto(Cursor cursor) {
            ...
            if (cursor.moveToFirst()) {
                ...
                String photoUriString = cursor.getString(cursor
                ...
                if (photoUriString != null) {
                    ...
                    if (INSTANCE.isImageLoaded(photoUri)) {
                        ...
                        if (apiManager != null) {
                            try {
                                avatar = android.provider.MediaStore.Images.Media
                                        .getBitmap(apiManager.getContext()
            ...
            if (mKey != null) {
                ...
                for (String number : mKey) {
                    ...
                    if (cursor != null) {
                        try {
                            ...
                            if (bitMap != null) {
                                ...
                                Logger.d(TAG,
                                        "doInBackground() the bitMap is null");
                            }
                        } finally {
                            cursor.close();
                        }
                    ...
                    if (size == PARTICIPANT_NUM_THREE) {
                        Logger.d(TAG, "doInBackground() the size is "
                                + PARTICIPANT_NUM_THREE);
                        break;
                    }
