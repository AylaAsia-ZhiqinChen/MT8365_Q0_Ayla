...
import com.orangelabs.rcs.core.ims.service.presence.extension.PresenceExtension;
import com.orangelabs.rcs.core.ims.service.presence.extension.PresenceExtensionListener;
import com.orangelabs.rcs.core.ims.service.presence.extension.ViLTEExtension;
import com.orangelabs.rcs.core.ims.service.presence.xdm.XDMContact;
...
public class PresenceService extends ImsService implements AddressBookEventListener , PresenceExtensionListener{
    ...
    //vilteExtension
    private PresenceExtension vilteService = null;
    ...
    public PresenceService(ImsModule parent) throws CoreException {
        ...
        //vilte
        vilteService = new ViLTEExtension();
    ...
    public synchronized void start() {
        ...
        boolean isXDMOperationRequired = StartService
                .getXDMBootFlag(AndroidFactory.getApplicationContext());
        ...
        if (isXDMOperationRequired) {
            ...
            try {
                ...
                StartService.setXdmInitializeFlag(AndroidFactory.getApplicationContext(), mIsInitialize);
                if (logger.isActivated()) {
                ...
                if (StartService.getNewUserAccount(AndroidFactory
                ...
                StartService.setXDMBootFlag(
                        AndroidFactory.getApplicationContext(), false);
            } catch (Exception e) {
                if (logger.isActivated()) {
        ...
        //SEND SUBSCRIBE LIST AFTER PUBLISHING IS DONE    
        PreseneSubscribe();
        
        //commented as Social presence not supported currently
...
private String buildCapabilities(String timestamp, Capabilities capabilities) {
    ...
    if(RcsSettings.getInstance().isStandaloneMsgSupport()){
  //Standalone Mode Messaging
    publishXML+="<tuple id=\""+Standalone+"\">" + SipUtils.CRLF +
    "  <status><basic>" + buildBooleanStatus(RcsSettings.getInstance().isStandaloneMsgSupport()) + "</basic></status>" + SipUtils.CRLF +
   ...
   if(capabilities.isIR94_VoiceCallSupported()){ 
    ...
    publishXML+="<tuple id=\""+VOLTE+"\">" + SipUtils.CRLF +
        ...
        if(capabilities.isIR94_VideoCallSupported() && ((ViLTEExtension)vilteService).isActive()){
            publishXML +=" <caps:video>"+capabilities.isIR94_VoiceCallSupported()+"</caps:video>" + SipUtils.CRLF;
        }
        
        if(capabilities.isIR94_DuplexModeSupported() && ((ViLTEExtension)vilteService).isDuplex()){
            publishXML +=" <caps:duplex>"+"<caps:supported>" + SipUtils.CRLF +
 ...
 " xmlns:ci=\"urn:ietf:params:xml:ns:pidf:cipid\""
    ...
    public void handleAddressBookHasChanged() {
        ...
        isFirstBoot = StartService.getFirstBootFlag(AndroidFactory.getApplicationContext());
        mIsInitialize = StartService.getXdmInitializeFlag(AndroidFactory.getApplicationContext());
        if (logger.isActivated()){
        ...
        StartService.setFirstBootFlag(AndroidFactory.getApplicationContext(), false);
    ...
    private void publishCapability()
    {
