...
public class CpimParser {
    ...
    private void parse(String temp_data) throws Exception {
        ...
        try {
            Matcher typeMatcher =
                Pattern.compile("Content-[tT]ype: *message/cpim").matcher(temp_data);
            if (typeMatcher.find()) {
                String separator = temp_data.substring(0, temp_data.indexOf(CRLF));
                Matcher bMatcher =
                    Pattern.compile("(?m)^$").matcher(temp_data.substring(typeMatcher.start()));
                if (bMatcher.find()) {
                    int from = typeMatcher.start() + bMatcher.start() + 2;
                    int to = temp_data.indexOf(CRLF + separator, from);
                    if (to >= from) {
                        temp_data = temp_data.substring(from, to);
                        logger.debug("get cpim body " + from + "->" + to);
                    }
                }
            }
