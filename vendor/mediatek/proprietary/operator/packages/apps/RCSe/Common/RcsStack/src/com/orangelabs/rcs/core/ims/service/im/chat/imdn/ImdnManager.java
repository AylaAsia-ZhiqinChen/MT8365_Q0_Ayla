...
public class ImdnManager extends Thread {
    ...
    public void sendMessageDeliveryStatus(String contact, String msgId, String status) {
        ...
        if (logger.isActivated()) {
            logger.info("sendMessageDeliveryStatus msgId: " + msgId + ";status: " + status);
        }
    ...
    public void sendMessageDeliveryStatusImmediately(String contact, String msgId, String status, final String remoteInstanceId) {
        ...
        if (logger.isActivated()) {
            logger.info("sendMessageDeliveryStatusImmediately ID: " + remoteInstanceId);
        }
    ...
    private void sendCpimSipMessageDeliveryStatus(DeliveryStatus deliveryStatus, String remoteInstanceId) {
        try {
            if (logger.isActivated()) {
                logger.debug("sendCpimSipMessageDeliveryStatus0 " + deliveryStatus.getStatus() + " for message " + deliveryStatus.getMsgId());
            }
            ...
            if (RcsSettings.getInstance().supportOP01()) {
                ...
                to = ChatUtils.formatCpimSipUri(deliveryStatus.getContact());
            } else {
            ...
            if(RcsSettings.getInstance().isCPMSupported()) {
                if (logger.isActivated()) {
                    logger.info("send CPM Message1");
                }
                msg = SipMessageFactory.createCpimMessage(dialogPath,
                    FeatureTags.FEATURE_RCSE_PAGER_MSG, CpimMessage.MIME_TYPE, cpim.getBytes());

                String contribId = ContributionIdGenerator.getContributionId(dialogPath.getCallId());
                msg.addHeader(ChatUtils.HEADER_CONTRIBUTION_ID, contribId);

                String convsId = RichMessagingHistory.getInstance().getCoversationID(deliveryStatus.getContact(), 1);
                if (convsId.isEmpty()) {
                    String callId = imsService.getImsModule().getSipManager().getSipStack().generateCallId();
                    convsId = ContributionIdGenerator.getContributionId(callId);
                }
                msg.addHeader(ChatUtils.HEADER_CONVERSATION_ID, convsId);

            } else {
                if (logger.isActivated()) {
                    logger.info("send OMA IM Message1");
            }
