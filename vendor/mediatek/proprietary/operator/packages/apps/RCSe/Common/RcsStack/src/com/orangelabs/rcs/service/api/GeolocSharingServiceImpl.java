...
import com.orangelabs.rcs.provider.sharing.GeolocSharingPersistedStorageAccessor;
...
public class GeolocSharingServiceImpl extends IGeolocSharingService.Stub {
     ...
     * Notifies registration event
     */
    public void notifyRegistrationEvent(boolean status) {
        // Notify listeners
        ...
        if(status){
            ...
            synchronized (mLock) {
                mRcsServiceRegistrationEventBroadcaster.broadcastServiceUnRegistered(RcsServiceRegistration.ReasonCode.UNSPECIFIED);
            }
        }
    }
    ...
    public void receiveGeolocSharingInvitation(GeolocTransferSession session) {
        ...
        ContactId contact = session.getRemoteContactId();
    ...
    public IGeolocSharing shareGeoloc(ContactId contact, Geoloc geoloc) throws RemoteException {
        ...
        try {
            ...
            String geolocDoc = ChatUtils.buildGeolocDocument(geoloc, ImsModule.IMS_USER_PROFILE
                    .getPublicUri(), msgId);
