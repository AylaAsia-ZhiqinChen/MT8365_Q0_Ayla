...
public class MobileNetworkInterface extends ImsNetworkInterface {
    ...
    public MobileNetworkInterface(ImsModule imsModule) throws CoreException {
        super(imsModule, ConnectivityManager.TYPE_MOBILE,
                ...
                RcsSettings.getInstance().getAllImsProxyAddrForMobile(),
                RcsSettings.getInstance().getAllImsProxyPortForMobile(),
    ...
    public boolean isRegistered() {
        logger.info("isRegistered: " + mRegistered);
        return mRegistered;
    }

    public void registrationTerminated() {
        logger.debug("registrationTerminated");
        if (mRegistered) {
            getSipManager().closeStack();
            mRegistered = false;
            getImsModule().getCore().getListener().handleRegistrationTerminated();
            logger.info("set mRegistered: " + mRegistered);
        }
    }
}
