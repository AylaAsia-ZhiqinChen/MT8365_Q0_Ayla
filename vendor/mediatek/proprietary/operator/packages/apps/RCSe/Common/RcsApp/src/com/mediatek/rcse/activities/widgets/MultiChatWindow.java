...
import com.mediatek.rcse.fragments.MultiChatFragment.SentMessage;
 ...
 */
public class MultiChatWindow extends ChatScreenWindow implements
        ChatView.IMultiChatWindow {
    private static final String TAG = "MultiChatWindow";
    private MultiChatFragment mMultiChatFragment = null;

    /**
     * @return group fragment instance
     */
    public MultiChatFragment getmMultiChatFragment() {
        return mMultiChatFragment;
    }
     ...
     */
    public MultiChatWindow(Object tag,
            CopyOnWriteArrayList<ParticipantInfo> participants) {
        mTag = tag;
    ...
    public IFileTransfer addSentFileTransfer(FileStruct file) {
        Logger.d(TAG, "addSentFileTransfer entry: mTag = " + mTag
                + ", mMultiChatFragment = " + mMultiChatFragment);
        IFileTransfer iFileTransfer = null;
        if (shouldLoadData()) {
            if (mMultiChatFragment != null) {
                iFileTransfer = mMultiChatFragment.addSentFileTransfer(file);
            }
        }
    ...
    public MultiChatFragment getFragment() {
        ...
        if (mMultiChatFragment == null) {
            ...
            if (mParticipantInfos != null) {
                ...
                arguments
                        .putParcelableArrayList(Utils.CHAT_PARTICIPANTS, infos);
            }
            mMultiChatFragment.setArguments(arguments);
        }
    ...
    public void pause() {
        ...
        if (null != controller) {
            Message controllerMessage = controller.obtainMessage(
                    ChatController.EVENT_HIDE_WINDOW, mTag, null);
    ...
    public void resume() {
        ...
        if (null != controller) {
            Message controllerMessage = controller.obtainMessage(
                    ChatController.EVENT_SHOW_WINDOW, mTag, null);
            controllerMessage.sendToTarget();
        } else {
            Logger.e(TAG, "resume controller is null");
        }
        if (mMultiChatFragment != null) {
            mMultiChatFragment.resume();
        } else {
            Logger.d(TAG, "resume mMultiChatFragment is null");
        }
    }
    ...
    public ISentChatMessage addSentMessage(ChatMessage message, int messageTag) {
        Logger.d(TAG,
                "addSentMessage entry, the tag is: " + mTag + " message is: "
                        + ((message == null) ? null : message.getContent()));
        SentMessage msg = null;
        if (!shouldLoadData()) {
            Logger.d(TAG,
                    "addSentMessage this window is not focused, the tag is: "
                            + mTag);
            return null;
        }
        if (mMultiChatFragment != null) {
            msg = (SentMessage) mMultiChatFragment.addSentMessage(message,
                    messageTag);
        } else {
            Logger.d(TAG, "addSentMessage mMultiChatFragment is null");
        }
        return msg;
    }
    ...
    public void removeAllMessages() {
        Logger.d(TAG, "removeAllMessages entry, the tag is: " + mTag
                + " ,mMultiChatFragment is :" + mMultiChatFragment);
        if (mMultiChatFragment != null) {
            mMultiChatFragment.removeAllMessages();
        } else {
            Logger.d(TAG, "removeAllMessages mMultiChatFragment is null");
        }
    }
    ...
    public IChatWindowMessage getSentChatMessage(String messageId) {
        ...
        if (mMultiChatFragment != null) {
            return mMultiChatFragment.getSentChatMessage(messageId);
        } else {
            Logger.d(TAG, "getSentChatMessage mMultiChatFragment is null");
            return null;
        }
    }
    ...
    public void addLoadHistoryHeader(boolean showLoader) {
        Logger.d(TAG, "addLoadHistoryHeader entry: mTag = " + mTag
                + ", mMultiChatFragment = " + mMultiChatFragment);

        if (shouldLoadData()) {
            if (mMultiChatFragment != null) {
                mMultiChatFragment.addLoadHistoryHeader(showLoader);
            }
        }
    ...
    public void updateParticipants(List<ParticipantInfo> participantList) {
        ...
        if (mMultiChatFragment != null) {
            mMultiChatFragment.updateParticipants(participantList);
        } else {
            Logger.d(TAG, "updateParticipants mMultiChatFragment is null");
        }
    }
    ...
    public void expandGroupChat() {
        Logger.d(TAG, "expandGroupChat entry, the tag is: " + mTag
                + " ,mGroupChatFragment is :" + mMultiChatFragment);
        if (mMultiChatFragment != null) {
            mMultiChatFragment.expandGroupChat();
        }
    }
    ...
    public void collapseGroupChat() {
        Logger.d(TAG, "collapseGroupChat entry, the tag is: " + mTag
                + " ,mMultiChatFragment is :" + mMultiChatFragment);
        if (mMultiChatFragment != null) {
            mMultiChatFragment.collapseGroupChat();
        }
    }

    @Override
    public void addContacts() {
        Logger.d(TAG, "addContacts mMultiChatFragment is: "
                + mMultiChatFragment);
        if (mMultiChatFragment != null) {
            mMultiChatFragment.addContacts();
        }
    }
    
    @Override
    public void removeContacts() {
        Logger.d(TAG, "removeContacts mMultiChatFragment is: "
                + mMultiChatFragment);
        if (mMultiChatFragment != null) {
            mMultiChatFragment.removeContacts();
        }
    }

    @Override
    public void removeChatUi() {
        Logger.d(TAG, "removeChatUi mMultiChatFragment is : "
                + mMultiChatFragment);
        if (mMultiChatFragment != null) {
            mMultiChatFragment.removeChatUi();
        }
    }
    ...
    public boolean clearHistory() {
        ...
        if (mMultiChatFragment != null) {
            return mMultiChatFragment.clearHistory();
        } else {
            Logger.d(TAG, "clearHistory mMultiChatFragment is null");
            return false;
        }
    }
    ...
    public void addUnreadMessage(ChatMessage message) {
        Logger.d(
                TAG,
                "addUnreadMessage entry, notice null check is in outer, "
                        + "the tag is: " + mTag + " message is: "
                        + message.getContent());
        if (Logger.getIsIntegrationMode()) {
            ...
            if (mMultiChatFragment != null) {
                mMultiChatFragment.addUnreadMessage(message);
            } else {
                Logger.d(TAG, "removeChatUi mMultiChatFragment is null");
            }
        }
    ...
    public void loadMessageIdList() {
        if (mMultiChatFragment != null) {
            mMultiChatFragment.loadMessageIdList();
        } else {
            Logger.d(TAG, "loadMessageIdList() mMultiChatFragment is null");
        }
