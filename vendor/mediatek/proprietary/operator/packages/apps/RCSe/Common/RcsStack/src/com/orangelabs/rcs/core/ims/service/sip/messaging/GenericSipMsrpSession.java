...
import com.orangelabs.rcs.core.ims.service.SessionActivityManager;
...
public abstract class GenericSipMsrpSession extends GenericSipSession implements MsrpEventListener {
     ...
     */
    public GenericSipMsrpSession(SipService parent, ContactId contact, String featureTag) throws CoreException {
        super(parent, contact, featureTag);

        mMaxMsgSize = RcsSettings.getInstance().getMaxMsrpLengthForExtensions();
        ...
        if (EnrichCallingService.CALL_COMPOSER_FEATURE_TAG.equals(featureTag)) {
            timeout = RcsSettings.getInstance().getCallComposerInactivityTimeout();
        } else {
            timeout = RcsSettings.getInstance().getChatIdleDuration();
        }
        ...
        mMsrpMgr = new MsrpManager(localIpAddress, localMsrpPort);
    }
