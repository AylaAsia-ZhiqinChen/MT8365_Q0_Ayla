...
public class UnreadMessagesContainer {
    ...
    public static synchronized UnreadMessagesContainer getInstance() {
        ...
        if (sUnreadMessagesContainer == null) {
            ...
            Logger.v(TAG, "getInstance exit with not null");
        }
     ...
     *
     * @param tag
     *            The tag of the unread message' chat.
     */
    public synchronized void add(ParcelUuid tag) {
        Logger.v(TAG, "add entry, the tag is: " + tag);
        if (tag == null) {
            Logger.v(TAG, "add tag is null");
            return;
        }
        ...
        if (mHead == null) {
            ...
            Logger.w(TAG, "add() invalid mHead: " + mHead + " , mTail: "
                    + mTail);
        }
        Logger.v(TAG, "add exit");
    }
    ...
    public synchronized void remove(ParcelUuid tag) {
        Logger.v(TAG, "remove entry, the tag is: " + tag);
        if (tag == null) {
            Logger.v(TAG, "remove tag is null");
            return;
        }
        if (mHead == null) {
            Logger.v(TAG, "remove mHead is null");
            return;
        }
    ...
    public synchronized ParcelUuid getLatestUnreadMessageChat() {
        Logger.v(TAG, "getLatestUnreadMessageChat entry");
        if (mTail == null) {
            Logger.v(TAG, "getLatestUnreadMessageChat mTail is null");
            return null;
        }
        Logger.v(TAG, "getLatestUnreadMessageChat exit");
        return mTail.tag;
    }
    ...
    public synchronized ChatMessage getLatestUnreadMessage() {
        ...
        if (instance == null) {
            Logger.v(TAG, "getLatestUnreadMessageChat instance is null");
            return null;
        } else {
            ...
            if (messages == null) {
                Logger.v(TAG, "getLatestUnreadMessage messages is null");
                return null;
            } else {
                Object[] messageArray = messages.toArray();
                int length = messageArray.length;
                if (length == 0) {
                    Logger.v(TAG, "getLatestUnreadMessage messages size is 0");
                    return null;
                } else {
                    ...
                    if (null != message) {
                        Logger.v(
                                TAG,
                                "getLatestUnreadMessage exit, the message text is "
                                        + message.getContent()
                                        + " the remote contact is "
                                        + message.getRemoteContact().toString());
                        return message;
                    } else {
                        Logger.e(TAG,
                                "getLatestUnreadMessage exit, the message is null");
                        return null;
                    }
                }
            }
    ...
    public synchronized void loadLatestUnreadMessage() {
        Logger.v(TAG, "loadLatestUnreadMessage entry");
        ChatMessage unreadMessage = getLatestUnreadMessage();
        ChatScreenWindow window = ChatScreenWindowContainer.getInstance()
                .getFocusWindow();
        if (mTail != null) {
            ParcelUuid currentTag = ChatScreenWindowContainer.getInstance()
                    .getCurrentTag();
            if (mTail.tag.equals(currentTag)) {
                Logger.v(TAG,
                        "loadLatestUnreadMessage should not load unread message");
                return;
            } else {
                Logger.v(TAG, "loadLatestUnreadMessage " + mTail.tag);
                if (unreadMessage != null && window != null) {
                    window.addUnreadMessage(unreadMessage);
                }
            }
        } else {
            Logger.v(TAG, "loadLatestUnreadMessage mTail is null");
        }
    }
     ...
     *
     * @param tag
     *            The tag of chat window to be switched to.
     * @param windowManager
     *            The window manager in which the window will be switched.
     */
    public synchronized void switchTo(ParcelUuid tag,
            ChatScreenActivity.ChatWindowManager windowManager) {
        Logger.v(TAG, "switchTo() entry, target tag: " + tag);

        ViewImpl instance = ViewImpl.getInstance();
        if (instance == null | null == tag) {
            Logger.e(TAG, "switchTo() ViewImpl instance is " + instance
                    + ", tag is" + tag);
            return;
        }
        ...
        if (window != null) {
            ...
            Logger.v(TAG, "switchTo() current window is " + currentWindowTag);
