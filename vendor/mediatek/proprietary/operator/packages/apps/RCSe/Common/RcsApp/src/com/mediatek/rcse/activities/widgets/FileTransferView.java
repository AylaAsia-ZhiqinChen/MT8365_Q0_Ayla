...
public class FileTransferView extends RelativeLayout {
    ...
    /**
     * Instantiates a new file transfer view.
     *
    ...
    public void setFile(final String filePath) {
        ...
        if (AsyncGalleryView.isPictureFile(filePath)) {
            ...
            String fileName = ModelImpl.SentFileTransfer
    ...
    private class SetIconAsyncTask extends AsyncTask<String, Void, Boolean> {
        ...
        protected Boolean doInBackground(String... params) {
            ...
            String fileName = ModelImpl.SentFileTransfer
                    .extractFileNameFromPath(mFilePath);
            ...
            WeakReference<Bitmap> iconBitmap = AsyncGalleryView.APP_ICON_MAP
                    .get(mMimeType);
            if (null != iconBitmap && null != iconBitmap.get()) {
                ...
                if (size == 0) {
                    Logger.d(TAG,
                            "doInBackground(), no icon found for this file type , " +
                            "use the default icon");
                } else {
                    Drawable icon = list.get(0).activityInfo
                            .loadIcon(packageManager);
                    if (icon instanceof BitmapDrawable) {
                    mTempIcon = ((BitmapDrawable) icon).getBitmap();
                    AsyncGalleryView.APP_ICON_MAP.put(mMimeType,
                            new WeakReference<Bitmap>(mTempIcon));
                    isIconFound = true;
                }
            }
            }
        ...
        protected void onPostExecute(Boolean result) {
            ...
            if (result) {
                ...
                mFileIcon
                        .setImageResource(R.drawable.rcs_ic_ft_default_preview);
            }
            OnClickListener onClickListener = new OnClickListener() {
                ...
                public void onClick(View v) {
                    if (null != mMimeType) {
                        ...
                        String fileName = mFilePath.startsWith(filesDirPath) ? mFilePath
                            : (filesDirPath + File.separator + mFilePath);
                        File file = new File(fileName);
                        try {
                            Uri tmpUri = FileProvider.getUriForFile(mContext,
                                                                    "com.mediatek.rcs.fileprovider",
                                                                    file);
                            intent.setDataAndType(tmpUri, mMimeType);
                            intent.addFlags(Intent.FLAG_GRANT_READ_URI_PERMISSION);
                            mContext.startActivity(intent);
                        } catch (android.content.ActivityNotFoundException e) {
                            Toast.makeText(
                                    ApiManager.getInstance().getContext(),
                                    R.string.file_formats_not_support,
        ...
        private String getFileExtension(String fileName) {
            Logger.d(TAG, "getFileExtension() entry, the fileName is "
                    + fileName);
            ...
            if ((lastDot > INDEX_ZERO)
                    && (lastDot < fileLength - AsyncGalleryView.INDEX_OFFSET)) {
                extension = fileName.substring(
                        lastDot + AsyncGalleryView.INDEX_OFFSET).toLowerCase();
            }
            Logger.d(TAG, "getFileExtension() entry, the extension is "
                    + extension);
            return extension;
        }
    }
