...
public class ChatMessagePersistedStorageAccessor {
     ...
     * @param id Message Id
     */
    public ChatMessagePersistedStorageAccessor(RichMessagingHistory messagingLog, String id) {
     ...
     */
    public ChatMessagePersistedStorageAccessor(RichMessagingHistory messagingLog, String id,
            String remoteContact, String content, String mimeType, String chatId,
            Direction direction) {
        mMessagingLog = messagingLog;
        mId = id;
        try {
            mRemoteContact = ContactIdUtils.createContactIdFromTrustedData(remoteContact);
        } catch(Exception e){
            if (logger.isActivated()) {
                logger.info("ChatMessagePersistedStorageAccessor exception");
            }
            mRemoteContact = null;
            e.printStackTrace();
        }
    ...
    private void cacheData() {
        ...
        try {
            ...
            if (contact != null) {
                ...
                mRemoteContact = ContactIdUtils.createContactIdFromTrustedData(contact);
            }
            } catch (Exception e) {
                if (logger.isActivated()) {
                    logger.info("ChatMessagePersistedStorageAccessor exception1");
                }
                e.printStackTrace();
            }
    ...
    public Status getStatus() {
        int mstatus = mMessagingLog.getMessageStatus(mId);
        Status status = Status.valueOf(mstatus);
        if (status == null) {
