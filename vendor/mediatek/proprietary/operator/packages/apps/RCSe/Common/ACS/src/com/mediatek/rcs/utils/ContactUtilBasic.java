 ...
 * @author YPLO6403
 */
public class ContactUtilBasic {

    /**
     * Singleton of ContactUtilBasic
     */
    private static volatile ContactUtilBasic sInstance;
     ...
     */
    private ContactUtilBasic(Context ctx) {
        mCtx = ctx;
    }

    /**
     * Gets a singleton instance of ContactUtilBasic.
     *
     * @param context the context.
     * @return the singleton instance.
     */
    public static ContactUtilBasic getInstance(Context context) {
        if (sInstance != null) {
            return sInstance;
        }
        synchronized (ContactUtilBasic.class) {
            if (sInstance == null) {
                if (context == null) {
                    throw new IllegalArgumentException("Context is null");
                }
                sInstance = new ContactUtilBasic(context);
            }
        }
        return sInstance;
    }
    ...
    private void tryToDetermineAndCacheCountryAndAreaCodes() throws RcsPermissionDeniedException {
        synchronized (ContactUtilBasic.class) {
            ...
            int mcc = RcsSettings.getInstance().getMobileCountryCode();
            /* Get the country code information associated to the mobile country code */
            String[] countryCodeInfo = sCountryCodes.get(mcc);
            if (countryCodeInfo == null) {
                throw new RcsPermissionDeniedException("Failed to get mobile country code (" + mcc + ")!");
            }
