...
public class StringMsgParser {
	...
	private static byte[] decompress(byte[] compressed) {
		...
		try {
			...
			GZIPInputStream gis;
			try {
				gis = new GZIPInputStream(in, BUFFER_SIZE);
			} catch (Exception e) {
				e.printStackTrace();
				System.out.println("Not in GZIP format  ");
				return null;
			}
			Base64InputStream b64is = new Base64InputStream(gis, 0);
        ...
        gis.close();
				...
				in.read(array);
				in.close();
				a = buffer.toByteArray();
				System.out.println("Output of unzip" + new String(a));
				return a;
			}
			buffer.flush();
			gis.close();
			...
			System.out.println("exception Output of unzip" + new String(a));
			e.printStackTrace();
			// for invalid stored block lengths errors
			return a;
		...
		if (readBody && message.getContentLength() != null
			...
			if (message.getContentEncoding() == null) {
				message.setMessageContent(body,
						computeContentLengthFromMessage, message
								.getContentLength().getContentLength());
				return message;
			} else if (message.getContentEncoding().getEncoding()
					.contains("gzip")) {
				...
				if ((body[0] == '-') && (body[1] == '-')) {
					...
					for (j = 0; j < bodyLength; j++) {
						if (body[j] == '\r' && body[j + 1] == '\n')
							break;
						System.out.println("Before Magic" + body[j]);
					}

					j = j++;// to skip \n
					System.out.println("Before Magic" + body[j]);
					j = j++;// for considering 0th
					System.out.println("Magic1" + body[j]);
					System.out.println("Magic 2" + body[j++]);
					byte[] hackyBody = new byte[bodyLength - j * 2 - 2];
					System.arraycopy(body, j, hackyBody, 0, hackyBody.length);
					decoded = decompress(hackyBody);
				} else {
					decoded = decompress(body);
                }

				message.setMessageContent(decoded,
						computeContentLengthFromMessage, message
								.getContentLength().getContentLength());
            } else {
				message.setMessageContent(body,
