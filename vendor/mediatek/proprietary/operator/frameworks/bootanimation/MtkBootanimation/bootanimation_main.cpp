...
//}
        ...
        ALOGD("[MtkBootAnimation %s %d]setBoot=%d,sePaly=%d,setRotated=%d",__FUNCTION__,__LINE__,setBoot,sePaly,setRotated);
        char volume[PROPERTY_VALUE_MAX];
        property_get("persist.sys.mute.state", volume, "-1");
        int nVolume = -1;
        nVolume = atoi(volume);
        ALOGD("[MtkBootAnimation %s %d]nVolume=%d",__FUNCTION__,__LINE__,nVolume);
        if(nVolume == 0 || nVolume == 1 ){
            sePaly = false;
        }
        ALOGD("before new MtkBootAnimation...");
        ALOGD("[MtkBootAnimation %s %d]before new MtkBootAnimation...",__FUNCTION__,__LINE__);
        sp<BootAnimation> boot = new BootAnimation(new AudioAnimationCallbacks(),setBoot,sePaly,setRotated);
        ALOGD("joinThreadPool...");
        ALOGD("[MtkBootAnimation %s %d]before joinThreadPool...",__FUNCTION__,__LINE__);
        IPCThreadState::self()->joinThreadPool();
    }
    ALOGD("[MtkBootAnimation %s %s %d]end",__FILE__,__FUNCTION__,__LINE__);
    return 0;
}
