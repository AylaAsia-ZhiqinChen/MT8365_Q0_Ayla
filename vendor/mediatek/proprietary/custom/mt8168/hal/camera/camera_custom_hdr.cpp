...
MVOID getHDRExpSetting(const HDRExpSettingInputParam_T& rInput, HDRExpSettingOutputParam_T& rOutput)
{
    ...
    if(rInput.u4AOEMode==1) //Dynamic EV Bracket
    {
        ...
        {
            ALOGD("SWHDR Error calculation of GainDiff = %f %f", dfGainDiff[0], dfGainDiff[1]);
        }
        ...
        {
            ALOGD("getHDRExpSetting dfGainDiff Error ! %f %f", dfGainDiff[0], dfGainDiff[1]);
        }

        //Decide taking 2 or 3 by EV diff of SE and LE
        if(dfGainDiff[0]!=0)
        {
            ALOGD("SWHDR GainDiff = %f", dfGainDiff[1]/dfGainDiff[0]);
            ...
            if((dfGainDiff[1]/dfGainDiff[0]) < dfGainDiffTh)//Use SE, LE only
            {
                ...
                {
                    ALOGD("SWHDR Error calculation of SE = %d %d", rOutput.u4ExpTimeInUS[0], rOutput.u4SensorGain[0]);
                }
                if(dfGainDiff[0]!=0)
                {
                    rOutput.u4FinalGainDiff[0] = static_cast<MUINT32>(1024 / dfGainDiff[0] + 0.5);
                ...
                {
                    ALOGD("SWHDR Error calculation of 2 frm dfGainDiff = %f", dfGainDiff[0]);
                }
            }
            else
            {
        ...
        {
            ALOGD("getHDRExpSetting dfGainDiff Error ! %f", dfGainDiff[0]);
        }
    ...
    if(hdr_debug_mode)
    {
        ...
        ALOGD("SWHDR AE Mode = %d", rInput.u4AOEMode);
        ALOGD("SWHDR FrmNum = %d", rOutput.u4OutputFrameNum);
        ALOGD("SWHDR Input delta EV(Bottom) = %f, TopEV = %f, Avg0_40 = %d Avg0_40_2 = %f", dfTargetBottomEV, dfTargetTopEV, rInput.i4LE_LowAvg, DarkAvg);

        ALOGD("SWHDR AE HDRFlag Mode = %d", HDRFlag);
        ALOGD("SWHDR Input 0EV Time = %d, Gain = %d Flare = %d", rInput.u4ExpTimeInUS0EV, rInput.u4SensorGain0EV,rInput.u1FlareOffset0EV);
