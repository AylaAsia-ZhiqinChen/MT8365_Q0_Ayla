...
public:     ////    Attributes
    virtual ESensorDev_T   getSensorDev() const { return ESensorDev_Main; }
    virtual MUINT32   getSensorID() const { return m_u4SensorID; }
...
#define INSTANTIATE(_dev_id,_sensor_id) \
    case _dev_id: return  CTIspTuningCustomBySensor<_dev_id,_sensor_id>::getInstance(u4SensorID)

#define INSTANTIATE_DEV(_dev_id) \
...
createInstance(ESensorDev_T const eSensorDev, MUINT32 const u4SensorID)
{
    switch  (eSensorDev)
    {
    //INSTANTIATE(ESensorDev_Main,IMX258_SENSOR_ID);       //  Main Sensor
    //INSTANTIATE(ESensorDev_MainSecond,IMX258_SENSOR_ID); //  Main Second Sensor
    //INSTANTIATE(ESensorDev_Sub,IMX258_SENSOR_ID);        //  Sub Sensor
    //INSTANTIATE(ESensorDev_SubSecond,IMX258_SENSOR_ID);  //  Sub Sensor
    INSTANTIATE_DEV(ESensorDev_Main);       //  Main Sensor
    INSTANTIATE_DEV(ESensorDev_MainSecond); //  Main Second Sensor
    INSTANTIATE_DEV(ESensorDev_Sub);        //  Sub Sensor
    INSTANTIATE_DEV(ESensorDev_SubSecond);  //  Sub Sensor
    INSTANTIATE_DEV(ESensorDev_MainThird);  //  Main Third Sensor
    default:
        break;
    }
