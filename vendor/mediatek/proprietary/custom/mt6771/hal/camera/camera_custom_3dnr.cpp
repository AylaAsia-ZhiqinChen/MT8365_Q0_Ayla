...
MBOOL NR3DCustom::isEnabledRSC(MUINT32 mask)
{
    ...
    if (isSupportRSC() && !isNR3DUsageMaskEnable(mask, USAGE_MASK_DUAL_ZOOM) &&
        !isNR3DUsageMaskEnable(mask, USAGE_MASK_MULTIUSER) && !isNR3DUsageMaskEnable(mask, USAGE_MASK_HIGHSPEED))
        ret = true;
