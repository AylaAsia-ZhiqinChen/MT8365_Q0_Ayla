...
#define S1714_TO_S3100_F(x) ((int)(1)*((x)/(1<<14)))  // Flip
#define S1714_TO_S3100_N(x) ((int)(-1)*((x)/(1<<14))) // Normal
...
SonyIMX519PdafLibraryWrapper::SonyIMX519PdafLibraryWrapper()
{
    ...
    {
        error = dlerror();
        AAA_LOGE("dlopen libSonyIMX519PdafLibrary.so fail : %s", error);
    }
    ...
    if( m_bLdDn)
    {
        ...
        m_pTransRectArgIn     = new PdLibSensorCoordSetting_t;
        ...
        memset( m_pTransRectArgIn,             0, sizeof(PdLibSensorCoordSetting_t));
...
SonyIMX519PdafLibraryWrapper::~SonyIMX519PdafLibraryWrapper()
{
    if( m_bLdDn)
    {
        ...
        if( m_pTransRectArgIn)
            delete ((PdLibSensorCoordSetting_t *)m_pTransRectArgIn);
...
MBOOL SonyIMX519PdafLibraryWrapper::DbgDumpSetting()
{
    if( m_bLdDn)
    {
        if( m_bDmpSetting)
        {
            ...
            PdLibSensorCoordSetting_t *pSensorSetting = (PdLibSensorCoordSetting_t *)m_pTransRectArgIn;
            ...
            AAA_LOGD("scale_n               : %d", pSensorSetting->scale_n                );
...
MBOOL SonyIMX519PdafLibraryWrapper::Init( SPDProfile_t &iPdProfile)
{
    if( m_bLdDn)
    {
         ...
         *       }eAETargetMODE;
         */
        AAA_LOGD("%s sensor scenario:%d, ae_mode:%d", __FUNCTION__, iPdProfile.i4SensorMode, iPdProfile.AETargetMode);
...
MBOOL SonyIMX519PdafLibraryWrapper::SetCaliData( MUINT32 u4CaliDaSz, MUINT8 *pCaliDaBuf, MUINT32 u4RegAddDaPairSz, MUINT16 *pRegAddDaPairBuf)
{
    ...
    if( m_bLdDn)
    {
        ...
        if( ret)
        {
            ...
            PdLibSensorCoordRegData_t RegData;
            memset( &RegData, 0, sizeof(PdLibSensorCoordRegData_t));
            for( MUINT32 i=0; i<pairsz; i++)
            {
                MUINT32 idx = 2*i;
                MUINT16 reg_addr = pRegAddDaPairBuf[idx+0];
                MUINT16 reg_data = pRegAddDaPairBuf[idx+1];

                //
                AAA_LOGD_IF( m_u8DbgEn, "sensor register setting : addr=0x%x, data=0x%x", reg_addr, reg_data);

                //
                switch( reg_addr)
                {
                    case (0x0101) :
                        RegData.reg_addr_0x0101 = reg_data;
                        break;
                    case (0x0220) :
                        RegData.reg_addr_0x0220 = reg_data;
                        break;
                    case (0x0221) :
                        RegData.reg_addr_0x0221 = reg_data;
                        break;
                    case (0x0344) :
                        RegData.reg_addr_0x0344 = reg_data;
                        break;
                    case (0x0345) :
                        RegData.reg_addr_0x0345 = reg_data;
                        break;
                    case (0x0346) :
                        RegData.reg_addr_0x0346 = reg_data;
                        break;
                    case (0x0347) :
                        RegData.reg_addr_0x0347 = reg_data;
                        break;
                    case (0x0348) :
                        RegData.reg_addr_0x0348 = reg_data;
                        break;
                    case (0x0349) :
                        RegData.reg_addr_0x0349 = reg_data;
                        break;
                    case (0x034A) :
                        RegData.reg_addr_0x034A = reg_data;
                        break;
                    case (0x034B) :
                        RegData.reg_addr_0x034B = reg_data;
                        break;
                    case (0x034C) :
                        RegData.reg_addr_0x034C = reg_data;
                        break;
                    case (0x034D) :
                        RegData.reg_addr_0x034D = reg_data;
                        break;
                    case (0x034E) :
                        RegData.reg_addr_0x034E = reg_data;
                        break;
                    case (0x034F) :
                        RegData.reg_addr_0x034F = reg_data;
                        break;
                    case (0x0381) :
                        RegData.reg_addr_0x0381 = reg_data;
                        break;
                    case (0x0383) :
                        RegData.reg_addr_0x0383 = reg_data;
                        break;
                    case (0x0385) :
                        RegData.reg_addr_0x0385 = reg_data;
                        break;
                    case (0x0387) :
                        RegData.reg_addr_0x0387 = reg_data;
                        break;
                    case (0x0401) :
                        RegData.reg_addr_0x0401 = reg_data;
                        break;
                    case (0x0404) :
                        RegData.reg_addr_0x0404 = reg_data;
                        break;
                    case (0x0405) :
                        RegData.reg_addr_0x0405 = reg_data;
                        break;
                    case (0x0408) :
                        RegData.reg_addr_0x0408 = reg_data;
                        break;
                    case (0x0409) :
                        RegData.reg_addr_0x0409 = reg_data;
                        break;
                    case (0x040A) :
                        RegData.reg_addr_0x040A = reg_data;
                        break;
                    case (0x040B) :
                        RegData.reg_addr_0x040B = reg_data;
                        break;
                    case (0x040C) :
                        RegData.reg_addr_0x040C = reg_data;
                        break;
                    case (0x040D) :
                        RegData.reg_addr_0x040D = reg_data;
                        break;
                    case (0x040E) :
                        RegData.reg_addr_0x040E = reg_data;
                        break;
                    case (0x040F) :
                        RegData.reg_addr_0x040F = reg_data;
                        break;
                    case (0x0900) :
                        RegData.reg_addr_0x0900 = reg_data;
                        break;
                    case (0x0901) :
                        RegData.reg_addr_0x0901 = reg_data;
                        break;
                    case (0x3F42) :
                        RegData.reg_addr_0x3F42 = reg_data;
                        break;
                    case (0x3F43) :
                        RegData.reg_addr_0x3F43 = reg_data;
                        break;
                    case (0x38A4) :
                        reg_addr_0x38A4 = reg_data;
                        break;
                    case (0x38A5) :
                        reg_addr_0x38A5 = reg_data;
                        break;
                    case (0x38A6) :
                        reg_addr_0x38A6 = reg_data;
                        break;
                    case (0x38A7) :
                        reg_addr_0x38A7 = reg_data;
                        break;
                    case (0x38A8) :
                        reg_addr_0x38A8 = reg_data;
                        break;
                    case (0x38A9) :
                        reg_addr_0x38A9 = reg_data;
                        break;
                    case (0x38AA) :
                        reg_addr_0x38AA = reg_data;
                        break;
                    case (0x38AB) :
                        reg_addr_0x38AB = reg_data;
                        break;
                    default :
                        break;
            ...
            //
            int reg_data_status = PdLibInterpretRegData( &RegData, (PdLibSensorCoordSetting_t *)m_pTransRectArgIn);
            ...
            //
            if( reg_data_status!=D_PD_LIB_REGDATA_IS_OK)
            {
                AAA_LOGE("Stauts of interpret register data (%d)", reg_data_status);
                ...
                AAA_LOGE("img_orientation_h     : %d", pSensorSetting->img_orientation_h      );
                AAA_LOGE("img_orientation_v     : %d", pSensorSetting->img_orientation_v      );
                AAA_LOGE("hdr_mode_en           : %d", pSensorSetting->hdr_mode_en            );
                AAA_LOGE("hdr_reso_redu_h       : %d", pSensorSetting->hdr_reso_redu_h        );
                AAA_LOGE("hdr_reso_redu_v       : %d", pSensorSetting->hdr_reso_redu_v        );
                AAA_LOGE("x_add_sta             : %d", pSensorSetting->x_add_sta              );
                AAA_LOGE("y_add_sta             : %d", pSensorSetting->y_add_sta              );
                AAA_LOGE("x_add_end             : %d", pSensorSetting->x_add_end              );
                AAA_LOGE("y_add_end             : %d", pSensorSetting->y_add_end              );
                AAA_LOGE("x_out_size            : %d", pSensorSetting->x_out_size             );
                AAA_LOGE("y_out_size            : %d", pSensorSetting->y_out_size             );
                AAA_LOGE("x_evn_inc             : %d", pSensorSetting->x_evn_inc              );
                AAA_LOGE("x_odd_inc             : %d", pSensorSetting->x_odd_inc              );
                AAA_LOGE("y_evn_inc             : %d", pSensorSetting->y_evn_inc              );
                AAA_LOGE("y_odd_inc             : %d", pSensorSetting->y_odd_inc              );
                AAA_LOGE("scale_mode            : %d", pSensorSetting->scale_mode             );
                AAA_LOGE("scale_m               : %d", pSensorSetting->scale_m                );
                AAA_LOGE("scale_n               : %d", pSensorSetting->scale_n                );
                AAA_LOGE("dig_crop_x_offset     : %d", pSensorSetting->dig_crop_x_offset      );
                AAA_LOGE("dig_crop_y_offset     : %d", pSensorSetting->dig_crop_y_offset      );
                AAA_LOGE("dig_crop_image_width  : %d", pSensorSetting->dig_crop_image_width   );
                AAA_LOGE("dig_crop_image_height : %d", pSensorSetting->dig_crop_image_height  );
                AAA_LOGE("binning_mode          : %d", pSensorSetting->binning_mode           );
                AAA_LOGE("binning_type_h        : %d", pSensorSetting->binning_type_h         );
                AAA_LOGE("binning_type_v        : %d", pSensorSetting->binning_type_v         );
                AAA_LOGE("binning_type_ext_en   : %d", pSensorSetting->binning_type_ext_en    );
                AAA_LOGE("binning_type_h_ext    : %d", pSensorSetting->binning_type_h_ext     );
                AAA_LOGE("pd_area_x_offset      : %d", m_u8PDArea_XOffset                     );
                AAA_LOGE("pd_area_y_offset      : %d", m_u8PDArea_YOffset                     );
                AAA_LOGE("pd_area_width         : %d", m_u8PDArea_Width                       );
                AAA_LOGE("pd_area_height        : %d", m_u8PDArea_Height                      );
...
MBOOL SonyIMX519PdafLibraryWrapper::SetPDCLData( MUINT32 u4PDCLDaSz, MUINT8 *pPDCLDaBuf, MINT32 &oWinMode, MUINT32 &oWinEn)
{
    ...
    if(  /* Sensor output data size is {16*12*5(PD/CL data)+5(Header)}=965 Bytes, no matter what kind of sensor setting is. */
        (u4PDCLDaSz>=965) && (pPDCLDaBuf!=NULL))
    {
        ...
        //
        MUINT8  cur_fiexible_win_en =  m_pPDCLDa[0];
        MUINT8  cur_win_mode        = (m_pPDCLDa[1]>>6)&0x3; // 0:Fixed area(16x12), 1:Fixed area(8x6), 2:Fiexible window, 3:Reserved.

        if( (m_u8FiexibleWindowEn!=cur_fiexible_win_en) || (m_u8WindowModeSelect!=cur_win_mode))
        {
            AAA_LOGD("PDCL Data:{ 0x%02x 0x%02x ....}, Fiexible window enable(0x%02x->0x%02x), window mode select(0x%02x->0x%02x)",
                     m_pPDCLDa[0],
                     m_pPDCLDa[1],
        ...
        //
        oWinEn   = m_u8FiexibleWindowEn = cur_fiexible_win_en;
        oWinMode = m_u8WindowModeSelect = cur_win_mode;

        ret = MTRUE;
    }
    else
    {
        AAA_LOGE("[%s] PDCL data is not valid : %p sz=%d. Please check camsv setting in sensor driver.", __FUNCTION__, pPDCLDaBuf, u4PDCLDaSz);

        /* error : reset parameters. */
        m_pPDCLDa    = NULL;
        m_u4PDCLDaSz = 0;
    }
    ...
    if( m_pfunGetDefocus && m_bLdDn && m_bInitDn)
    {
        ...
        if( (m_u8WindowModeSelect==0x0) || (m_u8WindowModeSelect==0x1))
        {
            ...
            if( iPDInputData.mode==0x0 || iPDInputData.mode==0x1 )
            {
                ...
                AAA_LOGD("[%s] i4X = %d i4Y = %d i4W = %d i4H = %d ",
                         __FUNCTION__,
                         iPDInputData.ROI.sPDROI.i4X,
                         iPDInputData.ROI.sPDROI.i4Y,
                         iPDInputData.ROI.sPDROI.i4W,
                         iPDInputData.ROI.sPDROI.i4H);
        ...
        //
#define _PD_CL_OFFSET_IN_BYTE_ 10
        MUINT8 *ptrPDCLDa = &m_pPDCLDa[_PD_CL_OFFSET_IN_BYTE_];
        ...
        if( ((PdLibSensorCoordSetting_t *)m_pTransRectArgIn)->img_orientation_h==1)
        {
            /*
         ...
         * In any image sensor mode, address is required to be converted into all-pixel mode address before scaling, cropping, mirroring and flipping.
         */
        PdLibRect_t ImgSz_I, ImgSz_O, WinAddr_I, WinAddr_O;

        ImgSz_I.sta.x = 0;
        ImgSz_I.sta.y = 0;
        ImgSz_I.end.x = ( m_PdProfile.uFullXsz - ImgSz_I.sta.x - 1);
        ImgSz_I.end.y = ( m_PdProfile.uFullYsz - ImgSz_I.sta.y - 1);
        ImgSz_O = PdLibTransOutputRectToPdafRect( ImgSz_I, (PdLibSensorCoordSetting_t *)m_pTransRectArgIn);
        ...
        if( m_u8WindowModeSelect==0x2)
        {
            ...
            WinAddr_O = PdLibTransOutputRectToPdafRect( WinAddr_I, (PdLibSensorCoordSetting_t *)m_pTransRectArgIn);
        }
        else
        {
            // Fix window mode
        ...
        {
            long defocus_14bit_shift = (((PdLibSensorCoordSetting_t *)m_pTransRectArgIn)->img_orientation_h == 1) ? S1714_TO_S3100_F(out.Defocus) : S1714_TO_S3100_N(out.Defocus);
            ...
            oPdOutputData.Confidence      = (int)(in->ConfidenceLevel*100/2047);
            //(3) normalize to fit API spec : confidence level is in ragne 0~100.
            oPdOutputData.ConfidenceLevel = (int)(in->ConfidenceLevel*100/2047);
        ...
        else
        {
            ...
            AAA_LOGE("[%s] defocus is not valid : failed to call SonyPdLibGetDefocus : %d",
                     __FUNCTION__,
                     (int)(err));
        }
    ...
    {
        AAA_LOGW("[%s] defocus is not valid : pfunGetDefocus(%p) bLdDn(%d), bInitDn(%d)", __FUNCTION__, m_pfunGetDefocus,m_bLdDn,m_bInitDn);
    }
    return 0;
}
