...
MUINT16* PD_IMX398MIPIRAW::ConvertPDBufFormat( MUINT32 /*i4Size*/, MUINT32 i4Stride, MUINT8 *ptrBufAddr, MUINT32 u4BitDepth, PD_AREA_T* ptrPDRegion)
{
    ...
    MUINT32 XSz = (m_sPDOHWInfo.u4Pdo_xsz+1)/2/sizeof(MUINT16); // (Pixle)
    ...
    AAA_LOGD_IF(m_bDebugEnable, "%s: va(%p) stride(%d), bit depth(%d), pd region(%p)", __FUNCTION__, ptrBufAddr, i4Stride, u4BitDepth, ptrPDRegion);
