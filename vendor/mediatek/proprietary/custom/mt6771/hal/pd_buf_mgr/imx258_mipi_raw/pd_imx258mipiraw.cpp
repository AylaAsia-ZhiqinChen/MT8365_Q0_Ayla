...
MUINT16* PD_IMX258MIPIRAW::ConvertPDBufFormat( MUINT32 i4Size, MUINT32 i4Stride, MUINT8 *ptrBufAddr, MUINT32 u4BitDepth, PD_AREA_T *ptrPDRegion)
{
    //Input :
    ...
    if (m_eBufType==EPDBUF_PDO)
    {
        ...
        {
            //vaild pd data size
            m_PDBufXSz = (m_sPDOHWInfo.u4Pdo_xsz+1)/sizeof(MUINT16);
            m_PDBufYSz = (m_sPDOHWInfo.u4Pdo_ysz+1);
            m_PDBufSz  = m_PDBufXSz*m_PDBufYSz;
            m_PDBuf    = new MUINT16 [m_PDBufSz];
        }
        separateLR(i4Stride, ptrBufAddr, m_PDBufXSz, m_PDBufYSz, m_PDBuf, u4BitDepth, ptrPDRegion);
    }
