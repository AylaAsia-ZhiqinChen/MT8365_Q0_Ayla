...
typedef struct aurisys_lib_handler_t {
	...
	struct AurisysLibInterface
		*api;        /* only pointer to library_config's api */
	...
	audio_buf_t
	*p_ul_ref_buf_aec; /* only pointer to aec in lib_config->p_ul_ref_bufs */
	...
	struct aurisys_lib_handler_t
		**head; /* point to the parent component's lib_handler_list_for_adb_cmd */
