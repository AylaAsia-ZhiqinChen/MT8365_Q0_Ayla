	...
	void        *p_handler)
{
	...
	copy_size = p_ul_buf_in->data_buf.data_size / p_ul_buf_in->num_channels *
		    p_ul_buf_out->num_channels;
...
void demo_arsi_assign_lib_fp(AurisysLibInterface *lib)
{
	...
	lib->arsi_query_max_debug_dump_buf_size =
		demo_arsi_query_max_debug_dump_buf_size;
	...
	lib->arsi_query_param_buf_size_by_custom_info =
		demo_arsi_query_param_buf_size_by_custom_info;
	lib->arsi_parsing_param_file_by_custom_info =
		demo_arsi_parsing_param_file_by_custom_info;
}
/* <-- For static link, like libXXX.a in FreeRTOS */
