...
#define DMA_ADDR_CHK(addr)  (((unsigned int) addr) & 0x00000000) //0xEC000001)  // 2 bytes align
...
#define DMA_TCM_ADDR(addr)  ((addr) + 0x20000000)
...
extern const unsigned int __DM_END_ADDR;
static struct dma_descr_t dma_descr[DMA_NUM];

// workaround for compiler
static inline void _dma_addr(unsigned int *addr)
{
    if (*addr < __DM_END_ADDR)
        *addr = DMA_TCM_ADDR(*addr);
}
                    ...
                    SemaphoreHandle_t sem, unsigned int flags)
{
    ...
    _dma_addr(&src->current);
    _dma_addr(&dst->current);
