...
unsigned int EINT_Domain_Store[EINT_REG_SET_SIZE];
...
/*
 * mt_set_domain_all: Enable all EINT on domain.
 */
void mt_eint_domain_set_all(void)
{
	unsigned int val = 0xFFFFFFFF, i;

	for (i = 0; i < EINT_REG_SET_SIZE; i++) {
		WRITE_REG(val, (EINT_DOMAIN_EN_BASE + (i * 4)));
	}
...
/*
 * mt_set_domain_all: Disable all EINT on domain.
 */
void mt_eint_domain_clr_all(void)
{
...
/*
 * mt_eint_domain_save_all: Save and disable all EINT on domain.
 * @eint_num: the EINT number to set
 */
void mt_eint_domain_save_all(void)
{
	unsigned int val = 0x0, i;

	for (i = 0; i < EINT_REG_SET_SIZE; i++) {
		EINT_Domain_Store[i] = READ_REG((EINT_DOMAIN_EN_BASE + (i * 4)));
		WRITE_REG(val, (EINT_DOMAIN_EN_BASE + (i * 4)));
	}
...
/*
 * mt_eint_domain_restore: Restore all EINT on domain.
 * @eint_num: the EINT number to clear
 */
void mt_eint_domain_restore_all(void)
{
	unsigned int i;

	for (i = 0; i < EINT_REG_SET_SIZE; i++) {
		WRITE_REG(EINT_Domain_Store[i], (EINT_DOMAIN_EN_BASE + (i * 4)));
	}
...
/*
 * mt_set_domain_all: Enable specific EINT on domain.
 * @eint_num: the EINT number to set
 */
void mt_eint_domain_set(unsigned int eint_num)
{
	unsigned int st;

	if (eint_num < EINT_MAX_CHANNEL) {
		st = READ_REG(((eint_num / 32) * 4 + EINT_DOMAIN_EN_BASE));
		st |= (1 << (eint_num % 32));
		WRITE_REG(st, ((eint_num / 32) * 4 + EINT_DOMAIN_EN_BASE));
	} else {
		error_msg("[EINT]num err\n\r");
	}
...
/*
 * mt_domain_clr: Disable specific EINT on domain.
 * @eint_num: the EINT number to clear
 */
void mt_eint_domain_clr(unsigned int eint_num)
{
	unsigned int st;
...
/*
 * mt_eint_domain_save: Save and diable specific EINT on domain.
 * @eint_num: the EINT number to set
 */
unsigned int mt_eint_domain_save(unsigned int eint_num)
{
	unsigned int st;
	unsigned int bit = 1 << (eint_num % 32);
	...
	if (eint_num < EINT_MAX_CHANNEL) {
		st = READ_REG(((eint_num / 32) * 4 + EINT_DOMAIN_EN_BASE));
		WRITE_REG((st & (~bit)),
			  ((eint_num / 32) * 4 + EINT_DOMAIN_EN_BASE));
		return (st & bit);
	} else {
		error_msg("[EINT]num err\n\r");
		return 0;
...
/*
 * mt_eint_domain_restore: Restore specific EINT on domain.
 * @eint_num: the EINT number to clear
 */
void mt_eint_domain_restore(unsigned int eint_num, unsigned int val)
{
	unsigned int st;
	unsigned int bit = 1 << (eint_num % 32);
	...
	for (index = 0; index < EINT_MAX_CHANNEL; index++) {
		...
		if (status_check) {
			...
			if (EINT_FUNC.eint_func[index]) {
				...
				/*record max duration */
				if (xLastExecutionStamp >
				    max_eint_isr_duration_time) {
					max_eint_isr_duration_time =
					    xLastExecutionStamp;
...
unsigned int mt_eint_get_def_pol(unsigned int eint_num)
{
	...
	domain = mt_eint_domain_save(eint_num);
	...
	mt_eint_domain_restore(eint_num, domain);
...
int mt_eint_init(void)
{
	...
	mt_eint_mask_all();
	intc_irq_request(&INTC_IRQ_EINT, (void *)mt_eint_lisr, (void *) 0);
...
void mt_eint_dump_config(unsigned int eint_num)
{
	...
	if (eint_num < EINT_MAX_CHANNEL) {
		...
		debug_msg
		    ("[EINT]number=%d,mask=%d,sens=%d,pol=%d,deb_en=%d,deb_time=%d,deb_reg_value=%d,eint_func=%p,int_status=%d\n\r",
		     eint_num, eint_mask, eint_sens, eint_pol, is_deb_en,
		     deb_time, deb_reg_value, eint_func, int_status);
