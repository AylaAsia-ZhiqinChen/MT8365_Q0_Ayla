...
from .enums import ENUM_ST_INFO_BIND
from .structs import ELFStructs
from .sections import (
        Section, StringTableSection, SymbolTableSection,
        SUNWSyminfoTableSection, NullSection)
from .descriptions import describe_symbol_bind, _DESCR_ST_INFO_BIND
...
class ELFFile(object):
    ...
    def _format_hex(self, addr, fieldsize=None, fullhex=False, lead0x=True):
        ...
        s = '0x' if lead0x else ''
        if fullhex:
            fieldsize = 8 if self.elfclass == 32 else 16
    ...
    def get_und_symbols_map(self):
        ...
        for nsym, symbol in enumerate(symtab.iter_symbols()):
            if symbol['st_shndx'] == 'SHN_UNDEF' and symbol.name != "":
                und_symbols_map[symbol.name] = nsym

        return und_symbols_map

    def get_weak_symbols_map(self):
        weak_symbols_map = {}
        symtab = self.get_section_by_name('.symtab')
        self._check_if_valid_symtab_section(symtab)

        for nsym, symbol in enumerate(symtab.iter_symbols()):
            if describe_symbol_bind(symbol['st_info']['bind']) == _DESCR_ST_INFO_BIND['STB_WEAK']:
                weak_symbols_map[symbol.name] = nsym

        return weak_symbols_map

    def get_weak_und_symbols_map(self):
        weak_und_symbols_map = {}
        symtab = self.get_section_by_name('.symtab')
        self._check_if_valid_symtab_section(symtab)

        for nsym, symbol in enumerate(symtab.iter_symbols()):
            if (describe_symbol_bind(symbol['st_info']['bind']) == _DESCR_ST_INFO_BIND['STB_WEAK'] and
                symbol['st_shndx'] == 'SHN_UNDEF' and symbol.name != ""):
                weak_und_symbols_map[symbol.name] = nsym

        return weak_und_symbols_map

    def _check_if_valid_symtab_section(self, section):
