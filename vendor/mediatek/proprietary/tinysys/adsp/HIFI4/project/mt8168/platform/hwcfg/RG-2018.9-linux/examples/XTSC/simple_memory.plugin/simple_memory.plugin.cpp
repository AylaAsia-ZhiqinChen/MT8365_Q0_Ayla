...
u32 xtsc_component::simple_memory::get_bit_width(const string& port_name, u32 interface_num) const {
  return m_width8*8;
}



sc_object *xtsc_component::simple_memory::get_port(const string& port_name) {
  return ((port_name == "m_respond_port") ? (sc_object*) &m_respond_port : (sc_object*) &m_request_export);
}



xtsc_port_table xtsc_component::simple_memory::get_port_table(const string& port_table_name) const {
...
/*
 * The code below here was added to make the simple_memory model available as a plugin 
 * to xtsc-run.
 *
 * Although this is a simple example with only one plugin model, the code here can serve
 * as a convenient template for how to add your own model to xtsc-run as a plugin.  
 *
 * An additional requirement for adding a model to xtsc-run as a plugin, is that the
 * model be supported by an xtsc_connection_interface implementation.  One way of doing
 * this is to modify your model to inherit from the xtsc_connection_interface (or,
 * better yet, the xtsc_module interface which itself inherits from the
 * xtsc_connection_interface).  To see how the original simple_memory model was modified
 * to inherit from xtsc_module, you may wish to do a side-by-side comparison of the 
 * following pairs of files:
 *     In XTSC/simple_memory    In XTSC/simple_memory.plugin
 *     ---------------------    ----------------------------
 *     simple_memory.h          simple_memory.h
 *     simple_memory.cpp        simple_memory.plugin.cpp  (this file)
 *
 * For a more complex example which has several plugin models, some of which have
 * non-Xtensa TLM interfaces (and hence require user-defined ports), and which does not
 * modify the original model code and which includes more discussion of how the plugin
 * mechanism works, please see the neighboring pif2sb_bridge.plugin example, especially
 * the file:
 *    ../pif2sb_bridge.plugin/pif2sb_bridge.plugin.cpp  (Linux)
 *    ..\pif2sb_bridge.plugin\pif2sb_bridge.plugin.cpp  (MS Windows)
 *
 * For more information about xtsc_connection_interface, xtsc_module, and
 * xtsc_plugin_interface, see the documentation for these classes in xtsc.h or
 * xtsc_rm.pdf and in xtsc_ug.pdf.
 */
...
class simple_memory_plugin_interface : public xtsc_plugin_interface {
public:

  virtual set<string> get_plugin_names() {
    set<string> plugin_names;
    plugin_names.insert("simple_memory");
    return plugin_names;
  }


  virtual void help(const string& plugin_name, bool verbose, ostream& os) {
    if (plugin_name == "simple_memory") {
      os << "simple_memory is a single-ported memory model of configurable byte width (default 4)," << endl;
      os << "start address (default 0x00000000), and size (default 4KB)." << endl;
    }
    else {
      ostringstream oss;
      oss << "simple_memory_plugin_interface::help() called with unknown plugin_name of \"" << plugin_name << "\"";
      throw xtsc_exception(oss.str());
    }
  }
  ...
  virtual xtsc_parms *create_parms(const string& plugin_name) {
    if (plugin_name == "simple_memory") {
      return new simple_memory_parms(4, 1024*4);
    }
    else {
      ostringstream oss;
      oss << "simple_memory_plugin_interface::create_parms() called with unknown plugin_name of \"" << plugin_name << "\"";
      throw xtsc_exception(oss.str());
    }
  }
  ...
  virtual sc_core::sc_module& create_module(const string& plugin_name, const string& instance_name, const xtsc_parms *p_parms) {
    if (plugin_name == "simple_memory") {
      simple_memory *p_mem = new simple_memory(instance_name.c_str(), *(simple_memory_parms*)p_parms);
      m_instance_map[p_mem->name()] = p_mem;
      return *p_mem;
    }
    else {
      ostringstream oss;
      oss << "simple_memory_plugin_interface::create_module() called with unknown plugin_name of \"" << plugin_name << "\"";
      throw xtsc_exception(oss.str());
    }
  }
  ...
  virtual xtsc_connection_interface *get_connection_interface(const string& hierarchical_name) {
    if (m_instance_map.find(hierarchical_name) == m_instance_map.end()) {
      ostringstream oss;
      oss << "simple_memory_plugin_interface::get_connection_interface() called with unknown hierarchical_name of \""
          << hierarchical_name << "\"";
      throw xtsc_exception(oss.str());
    }
...
xtsc_plugin_interface& xtsc_get_plugin_interface() {
  static simple_memory_plugin_interface *p_simple_memory_plugin = 0;
  if (!p_simple_memory_plugin) {
    p_simple_memory_plugin = new simple_memory_plugin_interface;
  }
  return *p_simple_memory_plugin;
}
