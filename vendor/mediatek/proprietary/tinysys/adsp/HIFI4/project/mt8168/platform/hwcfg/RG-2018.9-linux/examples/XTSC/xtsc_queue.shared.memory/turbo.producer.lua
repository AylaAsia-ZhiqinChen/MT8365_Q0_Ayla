...
-- period for the queue to be become non-full and then notify m_nonfull_event.
-- This is only needed if the queue consumer is in another host OS process.
...
queue           = "Q" .. string.format("%X", core) .. string.format("%X", core+1)
driver_wait     = 0
driver_wait_set = "xtsc xtsc_user_state_set " .. queue .. ".producer.counter.driver_wait "
cycle_wait      = 0
cycle_wait_set  = "xtsc xtsc_user_state_set " .. queue .. ".producer.counter.cycle_wait "
notify          = 0
notify_set      = "xtsc xtsc_user_state_set " .. queue .. ".producer.counter.notify "
...
while true do
  ...
  while true do
    if xtsc.cmd(queue .. " can_push") == "1" then
      xtsc.cmd("xtsc xtsc_event_notify " .. queue .. ".m_nonfull_event")
