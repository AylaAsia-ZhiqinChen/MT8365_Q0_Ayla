    ...
    for (p=xtsc_core::MEM_FIRST; p<=xtsc_core::MEM_LAST; ++p) {
      ...
      if (core0.has_memory_port(p)) {
        ...
        if (xtsc_core::is_multi_port_zero(p)) {
          ...
          string memory_trace_name = string("trace_") + port_name;
          ...
          memory_trace_parms.set("track_latency", true);
          ...
          xtsc_connect(core0, port_name, "slave_ports", *p_trace);
          // Connect the xtsc_memory_trace to the xtsc_memory
          xtsc_connect(*p_trace, "master_ports", "", *p_mem);
