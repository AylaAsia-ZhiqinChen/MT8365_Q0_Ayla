...
_C_LIB_DECL
typedef void (_CDECL *_Tlsdtor_t)(void *);
_CRTIMP2 int _Atthreadexit(void (_CDECL *)(void));
_CRTIMP2 void _CDECL _Destroytls(void);
...
_CRTIMP2 int _Tlsalloc(_Tlskey_t *, _Tlsdtor_t);
_CRTIMP2 int _Tlsfree(_Tlskey_t);
_CRTIMP2 int _Tlsset(_Tlskey_t, void *);
_CRTIMP2 void *_Tlsget(_Tlskey_t);
 #endif	/* _COMPILER_TLS */
...
#define _XTLS_SETUPX(name)	_Tls_setupx_ ## name
  ...
  #define _TLS_DATA_DECL(type, name) \
 	extern int (_CDECL *_XTLS_SETUP(name))(void); \
  ...
  #define _TLS_DEFINE_NO_INIT(scope, type, name) \
	scope int (_CDECL *_XTLS_SETUP(name))(void) = 0
  ...
  #define _TLS_DEFINE_NO_INIT_DT(scope, type, name, dtor) \
	...
	static void _CDECL _XTLS_DTOR(name)(void) \
	{ \
	dtor(&(name)); \
	} \
	static void _CDECL _XTLS_REG(name)(void) \
	{ \
	_CLEANUP(_XTLS_DTOR(name)); \
	} \
	static int _CDECL _XTLS_SETUPX(name)(void) \
	{ \
	_Once(&_XTLS_ONCE(name), _XTLS_REG(name)); \
	return 1; \
	} \
	scope int (_CDECL *_XTLS_SETUP(name))(void) = _XTLS_SETUPX(name)
...
#define _TLS_DATA_DECL(type, name)		\
  ...
  #define _XTLS_DEFINE_NO_INIT(scope, type, name, elts, dtor) \
	...
	static void _CDECL _XTLS_SETUP(name)(void) \
	{ \
  ...
  #define _XTLS_DEFINE_NO_INIT_DT(scope, type, name, elts, dtor) \
	static void _CDECL _XTLS_DTOR(name)(void *_Ptr) \
	{ \
	(dtor)(_Ptr); \
	free(_Ptr); \
	} \
