...
int sc_main(int argc, char *argv[]) {
  ...
  try {
    ...
    u32 bits = core0.get_xtsc_core().get_pin_bit_width("TIE_status");
    xtsc_signal_sc_bv_base core0_to_core1("core0_to_core1", bits);
    sc_trace(p_tf, core0_to_core1, "core0_to_core1");
    core0.TIE_status (core0_to_core1);
    core1.TIE_control(core0_to_core1);

    // Connect a dummy signal the other way to prevent port not bound error
    xtsc_signal_sc_bv_base dummy("dummy", bits);
    core1.TIE_status (dummy);
    core0.TIE_control(dummy);
