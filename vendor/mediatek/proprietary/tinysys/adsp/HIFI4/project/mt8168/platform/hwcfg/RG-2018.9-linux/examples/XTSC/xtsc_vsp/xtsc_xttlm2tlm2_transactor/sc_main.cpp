// Customer ID=13943; Build=0x75f5e; Copyright (c) 2005-2014 by Tensilica Inc.  ALL RIGHTS RESERVED.
// These coded instructions, statements, and computer programs are the
// copyrighted works and confidential proprietary information of
// Tensilica Inc.  They may be adapted and modified by bona fide
// purchasers for internal use, but neither the original nor any adapted
// or modified version may be disclosed or distributed to third parties
// in any manner, medium, or form, in whole or in part, without the prior
// written consent of Tensilica Inc.

#include <ctime>
#include "xtsc_core_vsp/xtsc_core_vsp.h"
#include <xtsc/xtsc_xttlm2tlm2_transactor.h>
#include <xtsc/xtsc_memory_tlm2.h>


using namespace std;
using namespace sc_core;
using namespace log4xtensa;
using namespace xtsc;
using namespace xtsc_component;
using namespace xtsc_vsp;


// Create a logger for sc_main
static TextLogger& logger = TextLogger::getInstance("sc_main");



int sc_main(int argc, char *argv[]) {

  try {

    // Initialize
    xtsc_vsp_initialize_parms init_parms("../../TextLogger.txt");
    // Next line internally calls extract_parms()
    xtsc_vsp_initialize(init_parms);

    // Configuration parameters for xtsc_core
    xtsc_core_vsp_parms core_parms;
    core_parms.set("SimTargetOutput", "core0_output.log");

    // Construct the core
    // Next line internally calls extract_parms
    xtsc_core_vsp core0("core0", core_parms);

    // Iterate all possible memory ports 
    xtsc_core::memory_port p;
    xtsc_xttlm2tlm2_transactor *p_tran = NULL;
    xtsc_memory_tlm2           *p_mem  = NULL;
    for (p=xtsc_core::MEM_FIRST; p<=xtsc_core::MEM_LAST; ++p) {
      // Does this core have such a memory port?
      if (core0.get_xtsc_core().has_memory_port(p)) {
        // Skip all but the first port in a multi-ported interface because
        // they have already been taken care of.
        if (xtsc_core::is_multi_port_zero(p)) {
          // Get the memory port name
          const char *port_name = xtsc_core::get_memory_port_name(p, true);
          // Create an Xtensa TLM to OSCI TLM2 transactor name
          string tran_name = string("tran_") + string(xtsc_core::get_memory_port_name(p, true));
          // Create transactor parameters corresponding to this memory port
          xtsc_xttlm2tlm2_transactor_parms tran_parms(core0.get_xtsc_core(), port_name);
          tran_parms.extract_parms(argc, argv, tran_name);
          // Construct the transactor
          p_tran = new xtsc_xttlm2tlm2_transactor(tran_name.c_str(), tran_parms);

          // Create a TLM2 memory name
          string mem_name = string("mem_") + string(xtsc_core::get_memory_port_name(p, true));
          // Create TLM2 memory parameters corresponding to this memory port
          xtsc_memory_tlm2_parms memory_parms(core0.get_xtsc_core(), port_name);
          memory_parms.extract_parms(argc, argv, mem_name);
          // Construct the TLM2 memory
          p_mem = new xtsc_memory_tlm2(mem_name.c_str(), memory_parms);

          // Connect the transactor
          xtsc_connect(core0, port_name, "slave_ports", *p_tran);

          // Connect the TLM2 memory
          xtsc_connect(*p_tran, "initiator_sockets", "target_sockets", *p_mem);
        }
      }
    }

    // Load program
    core0.get_xtsc_core().load_program("target/memory_test.out");

    // Set-up debugging according to command line arguments, if any
    xtsc_core::setup_multicore_debug(argc, argv);

    // Start and run the simulation
    sc_start();

    // Clean-up
    xtsc_vsp_finalize();

  }
  catch (const exception& error) {
    ostringstream oss;
    oss << "std::exception caught: " << endl;
    oss << error.what() << endl;
    xtsc_log_multiline(logger, FATAL_LOG_LEVEL, oss.str(), 2);
    cerr << oss.str();
  }

  return 0;
}

