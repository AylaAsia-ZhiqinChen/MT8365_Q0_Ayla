...
/*****************************************************************************
  ...
  Description of Modification: Use log4xtensa and xtsc_exception. Also, 
                               notify transfer_done before return from
                               handle_request with SIMPLE_BUS_ERROR.
  ...
  for (int i = 1; i < slave_port.size(); ++i) {
    ...
    for (int j = 0; j < i; ++j) {
      ...
      if ( !no_overlap ) {
        ...
        oss << "Error in '" << name() << "': overlapping address spaces of 2 slaves :" << endl;
        oss << "  slave " << i << " 0x" << hex << slave1->start_address() << "-0x" << slave1->end_address() << endl;
        oss << "  slave " << j << " 0x" << hex << slave2->start_address() << "-0x" << slave2->end_address() << endl;
...
void simple_bus::main_action()
{
  ...
  if (!m_current_request)
    ...
    XTSC_VERBOSE(m_text, "SLV [0x" << hex << m_current_request->address << "]");
  if (m_current_request)
    handle_request();
  if (!m_current_request)
    clear_locks();
...
bool simple_bus::direct_read(int *data, unsigned int address)
{
  if (address%4 != 0 ) {// address not word alligned
    XTSC_INFO(m_text, "BUS ERROR --> address 0x" << hex << address << " not word alligned");
    return false; 
  }
...
void simple_bus::read(unsigned int unique_priority
		      ...
		      , bool lock)
{
  XTSC_VERBOSE(m_text, "read(" << unique_priority << " @ 0x" << hex << address);
...
void simple_bus::write(unsigned int unique_priority
		       ...
		       , bool lock)
{
  XTSC_VERBOSE(m_text, "write(" << unique_priority << ") @ 0x" << hex << address);
...
simple_bus_status simple_bus::burst_read(unsigned int unique_priority
					 ...
					 , bool lock)
{
  XTSC_VERBOSE(m_text, "burst_read(" << unique_priority << ") @ 0x" << hex << start_address);
...
simple_bus_status simple_bus::burst_write(unsigned int unique_priority
					  ...
					  , bool lock)
{
  XTSC_VERBOSE(m_text, "burst_write(" << unique_priority << ") @ 0x" << hex << start_address);
...
{
  XTSC_VERBOSE(m_text, "Handle Slave(" << m_current_request->priority << ")");
  ...
  if ((m_current_request->address)%4 != 0 ) {// address not word alligned
    XTSC_INFO(m_text, "BUS ERROR --> address 0x" << hex << m_current_request->address << " not word aligned");
    m_current_request->status = SIMPLE_BUS_ERROR;
    m_current_request->transfer_done.notify();          // Tensilica
    m_current_request = (simple_bus_request *)0;
    return;
  }
  if (!slave) {
    XTSC_INFO(m_text, "BUS ERROR --> no slave for address 0x" << hex << m_current_request->address);
    m_current_request->status = SIMPLE_BUS_ERROR;
    m_current_request->transfer_done.notify();          // Tensilica
    m_current_request = (simple_bus_request *)0;
    return;
  ...
  XTSC_VERBOSE(m_text, "--> status=(" << simple_bus_status_str[slave_status] << ")");
...
simple_bus_request * simple_bus::get_next_request()
{
  ...
  for (int i = 0; i < m_requests.size(); ++i)
	...
	{
          XTSC_VERBOSE(m_text, "request (" << request->priority << ") [" << simple_bus_status_str[request->status] << "]");
	  Q.push_back(request);
	}
    }
  if (Q.size() > 0)
