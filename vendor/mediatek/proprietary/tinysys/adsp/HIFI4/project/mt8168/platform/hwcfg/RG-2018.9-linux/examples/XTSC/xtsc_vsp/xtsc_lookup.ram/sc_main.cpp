...
int sc_main(int argc, char *argv[]) {
  ...
  try {
    ...
    xtsc_lookup_parms lram_parms(core0.get_xtsc_core(), "lookup_ram", NULL, "0xBAD1BAD1", true);
    lram_parms.set("enforce_latency", false);  // TurboXim (turbo = true) ignores latency
    ...
    // Load program
    core0.get_xtsc_core().load_program("target/ram_test.out");
