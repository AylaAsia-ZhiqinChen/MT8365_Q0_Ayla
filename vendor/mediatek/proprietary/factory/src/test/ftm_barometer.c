...
#define INVALID_T_VAL -273.150*100 //0K = -273.15
#define INVALID_P_VAL -1.0*100
#define MAX_T_VAL 100.0 * 100
#define MIN_T_VAL 0.0 * 100
#define MAX_P_VAL 1013.25*1.2*100
#define MIN_P_VAL 1013.25*0.8*100
...
{
    int err, max_retry = 3, retry_period = 100, retry = 0;
    uint32_t flags = 1;
    err = 0;
    if (baro->fd == -1)
	{
		  ...
		  //
		  while ((err = ioctl(baro->fd, BAROMETER_IOCTL_INIT, &flags)) && (retry ++ < max_retry))
...
static int barometer_close(struct baro_priv *baro)
{
	...
	{
        flags = false;
        ioctl(baro->fd, BAROMETER_IOCTL_INIT, &flags);
        close(baro->fd);
    }
    memset(baro, 0x00, sizeof(*baro));
    baro->fd = -1;
...
int barometer_entry(struct ftm_param *param, void *priv)
{
    ...
    if (FTM_AUTO_ITEM == param->test_type) {
        ...
        usleep(1000000);
        press_raw_tmp = dat->baro.press_raw;
        for (i=0; i<400; i++) {
            if (dat->baro.temp_raw > MAX_T_VAL || dat->baro.temp_raw < MIN_T_VAL ) {
                LOGE(TAG "Error: Temperature = %.4f out of range", dat->baro.temp_raw/100);
                dat->mod->test_result = FTM_TEST_FAIL;
                break;
            }
            if (dat->baro.press_raw > MAX_P_VAL || dat->baro.press_raw < MIN_P_VAL ) {
                LOGE(TAG "Error: Pressure = %.4f out of range", dat->baro.press_raw/100);
                dat->mod->test_result = FTM_TEST_FAIL;
                break;
            }
            if (dat->baro.press_raw != press_raw_tmp)
                press_constant = false;
            usleep(5000);
        }
        if (press_constant == true) { // press value not change, maybe something wrong
            LOGE(TAG "Error: Pressure = %.4f is constant!!! Maybe something wrong.", dat->baro.press_raw/100);
            dat->mod->test_result = FTM_TEST_FAIL;
        }
        dat->exit_thd = true;
    } else {
