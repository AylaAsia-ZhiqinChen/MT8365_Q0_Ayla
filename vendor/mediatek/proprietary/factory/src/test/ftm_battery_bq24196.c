...
#define AUTO_TEST_THD_VALUE 4350
...
#define OTG_STATE_PATH "/sys/bus/usb/devices/1-1/speed"

int get_otg_online(void)
{
	int fd = -1;
...
static void battery_update_info(struct batteryFTM *batt, char *info)
{
	...
	LOGD(TAG "OTG battery_update_info\n");
	temp_otg = get_otg_online();
	if (temp_otg == 49)
	{
		LOGD(TAG "OTG online 49 = %d , return fail\n", temp_otg);
		batt->charger_voltage = -1;
		batt->charger_exist = false;
		batt->is_charging = false;
	}
...
int battery_entry(struct ftm_param *param, void *priv)
{
	...
	return_data.battery.current = 500;
  ...
  if(temp_v_chr < AUTO_TEST_VCHR_VALUE)
  {
	...
	if (temp_otg == 49)
	{
		LOGD(TAG "[FTM_OTG 3] OTG = %d , return fail\n", temp_v_chr);
		batt->mod->test_result = FTM_TEST_FAIL;
		return 0;
	}
    ...
    {
       LOGD(TAG "[FTM_BAT 4] VBatTemp = %d , return fail\n", temp_v_chr);
        batt->mod->test_result = FTM_TEST_FAIL;
    return 0;
}
#endif
