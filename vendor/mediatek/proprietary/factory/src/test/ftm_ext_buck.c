...
#endif //#ifdef EXT_BUCK3_EXIST


static void ext_buck_update_info(char *info)
{
    char *ptr;
    ...
    ptr += sprintf(ptr, "%s : %s \n%s: %s \n"
		   ...
		   , (is_hw_ext_buck_exist()) ? "Connect" : "NO connect"
		   ,uistr_info_title_ext_buck2_chip
		   , (is_hw_ext_buck2_exist()) ? "Connect" : "NO connect");
#else
    ptr += sprintf(ptr, "%s : %s \n"
		   , uistr_info_title_ext_buck_chip
		   , (is_hw_ext_buck_exist()) ? "Connect" : "NO connect");
    ...
    ptr += sprintf(ptr, "%s : %s \n"
		   ...
		   , (is_hw_ext_buck3_exist()) ? "Connect" : "NO connect");
#endif
    return;
}
...
int ext_buck_entry(struct ftm_param *param, void *priv)
{
    ...
    if(is_hw_ext_buck3_exist())
    {
    /*
        LOGD(TAG "[ITEM_EXT_BUCK3] is_hw_ext_buck3_exist : YES\n");
        extbk->mod->test_result = FTM_TEST_PASS;
        LOGD(TAG "[ITEM_EXT_BUCK3] Final : PASS\n");
        return 0;
     */
    }
    else
...
#ifdef FEATURE_FTM_EXT_BUCK2
    //auto test - check external buck
    if(is_hw_ext_buck2_exist())
    {
    /*
        LOGD(TAG "[ITEM_EXT_BUCK2] is_hw_ext_buck2_exist : YES\n");
        extbk->mod->test_result = FTM_TEST_PASS;
        LOGD(TAG "[ITEM_EXT_BUCK2] Final : PASS\n");
        return 0;
     */
    }
    else
