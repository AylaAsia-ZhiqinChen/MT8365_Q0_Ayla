...
{    
    ...
    *gr_font = (GRFont *)calloc(sizeof(GRFont), 1);
    ...
    (*gr_font)->texture = (GGLSurface *)malloc(surface_num*sizeof(GGLSurface));
   ...
   // fprintf(stderr, "block over\n");
    if(residue != 0)
    {
        bits_split = (void*)malloc(residue * font->height);
        (*gr_font)->texture[i].version = sizeof(GGLSurface);
        ...
        for(j=0; j<font->height; j++)
        {
          memcpy((unsigned char*)bits_split+ j*residue, (unsigned char*)(org_bits+block*pivotal+j*font->width), residue);
        }
    }
...
static int get_framebuffer(GGLSurface *fb)
{
    ...
    //LOGD(TAG "fb->width=%d, fb->height=%d\n", fb->width, fb->height);
    //LOGD(TAG "define FACTORY_MODE_SUPPORT_BGRA_8888\n");
        if(redoffset_32bit){
...
#else
    //LOGD(TAG "not define FACTORY_MODE_SUPPORT_BGRA_8888\n");
