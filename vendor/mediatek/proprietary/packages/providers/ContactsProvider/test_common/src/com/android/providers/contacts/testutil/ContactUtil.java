...
public class ContactUtil {
    ...
    /**
     * M: Add to support multi-delete
     */
    public static void deleteInBatch(ContentResolver resolver, long... contactIds) {
        if (contactIds == null) {
            return;
        }
        Uri uri = URI.buildUpon().appendQueryParameter("batch", "true").build();
        String[] selectionArg = new String[contactIds.length];
        int i = 0;
        for (long id : contactIds) {
            selectionArg[i++] = String.valueOf(id);
        }
        resolver.delete(uri, null, selectionArg);
    }
}
