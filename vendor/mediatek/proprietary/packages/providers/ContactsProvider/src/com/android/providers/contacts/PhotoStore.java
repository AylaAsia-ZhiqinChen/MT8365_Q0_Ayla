...
public class PhotoStore {
    ...
    public Set<Long> cleanup(Set<Long> keysInUse) {
        ...
        if (!keysToRemove.isEmpty()) {
            ...
            for (long key : keysToRemove) {
                /// M: ALPS03769109: remove if created 2+ minutes ago
                // remove(key);
                removeIfNeed(key);
                /// @}
            }
            ...
            try {
                ...
                if (id != 0) {
                    ...
                    if (file.renameTo(target)) {
                        ...
                        putEntry(entry.id, entry);
                        return id;
                    /// M: if rename fail, trigger exception to catch stack @{
                    } else {
    ...
    /// M: for CR ALPS03769109 @{
    /**
     * Removes the specified photo file, if created more than 2 minutes ago
     */
    private void removeIfNeed(long id) {
        File file = getFileForPhotoFileId(id);
        long duration = Math.abs(file.lastModified() - System.currentTimeMillis());
        if (duration < 2 * 60 * 1000) {
            Log.e(TAG, "File " + id + " newly created, ignore remove here");
            return;
        }
        remove(id);
    }
