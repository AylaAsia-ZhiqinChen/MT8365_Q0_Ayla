...
public class CallLogDatabaseHelper {
    ...
    private final class OpenHelper extends SQLiteOpenHelper {
        ...
        public void onCreate(SQLiteDatabase db) {
            ...
            db.execSQL("CREATE TABLE " + Tables.CALLS + " (" +
                    ...
                    /// M: sim indicate, is sdn @{
                    MtkCallLog.Calls.CACHED_INDICATE_PHONE_SIM + " INTEGER NOT NULL DEFAULT -1," +
                    MtkCallLog.Calls.CACHED_IS_SDN_CONTACT + " INTEGER NOT NULL DEFAULT 0" +
                    /// @}
                    ");");
    ...
    /// M: add MTK customized columns, if need @{
    /**
     * Add two customized columns if not found (upgrade from AOSP version)
     * @param db the writable database
     */
    private static void addMissingColumnsIfNeed(SQLiteDatabase db) {
        Log.i(TAG, "addMissingColumnsIfNeed");
        try {
            db.rawQuery("SELECT " + MtkCallLog.Calls.CACHED_INDICATE_PHONE_SIM + " FROM "
                    + Tables.CALLS, null);
            ...
            if (e.toString().contains(pattern)) {
                db.execSQL("ALTER TABLE " + Tables.CALLS + " ADD " +
                        MtkCallLog.Calls.CACHED_INDICATE_PHONE_SIM +
