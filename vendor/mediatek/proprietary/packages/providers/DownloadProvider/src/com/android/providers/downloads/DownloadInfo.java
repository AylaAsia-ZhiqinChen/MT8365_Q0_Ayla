...
public class DownloadInfo {
    ...
    public static class Reader {
        ...
        public void updateFromDatabase(DownloadInfo info) {
            ...
            info.mUsername = getString(MtkDownloads.Impl.COLUMN_USERNAME);
            info.mPassword = getString(MtkDownloads.Impl.COLUMN_PASSWORD);
            /// @}
            /// M: Add these to support OMA DL @{
            if (MtkDownloads.Impl.OMA_DOWNLOAD_SUPPORT) {
                info.mOmaDownload = getInt(MtkDownloads.Impl.COLUMN_OMA_DOWNLOAD_FLAG);
                info.mOmaDownloadStatus = getInt(MtkDownloads.Impl.COLUMN_OMA_DOWNLOAD_STATUS);
                info.mOmaDownloadNextUrl = getString
                (MtkDownloads.Impl.COLUMN_OMA_DOWNLOAD_NEXT_URL);
                info.mOmaDownloadInsNotifyUrl =
                 getString(MtkDownloads.Impl.COLUMN_OMA_DOWNLOAD_INSTALL_NOTIFY_URL);
            }
            /// @}
            /// M: Operator Feature check whether can continue download
            /// and get the download path. @{
            ...
            sDownloadProviderFeatureExt.setContinueDownload(info.mContinueDownload,
                    getInt(MtkDownloads.Impl.COLUMN_CONTINUE_DOWNLOAD_WITH_SAME_FILENAME) == 1);

            sDownloadProviderFeatureExt.setDownloadPath(info.mDownloadPath,
                    getString(MtkDownloads.Impl.COLUMN_DOWNLOAD_PATH_SELECTED));
            /// @}
            ...
            return (mMediaScanned == Downloads.Impl.MEDIA_NOT_SCANNED)
                && (mDestination == Downloads.Impl.DESTINATION_EXTERNAL ||
                        ...
                        mDestination == Downloads.Impl.DESTINATION_NON_DOWNLOADMANAGER_DOWNLOAD)
                            ...
                            /// M: add to fix 821679. @{
                            && ((mMimeType != null) && !mMimeType.equalsIgnoreCase
                            (Constants.DD_FILE_MIMETYPE));
                            /// @}
        } else {
            /// @}
        return (mMediaScanned == Downloads.Impl.MEDIA_NOT_SCANNED)
