...
class RILRequest {
     ...
     *
     * @param request MwiRILConstants.RIL_REQUEST_*
    ...
    onError(int error, Object ret) {
        ...
        if (MwiRIL.MWI_RILA_LOGD) Rlog.d(LOG_TAG, serialString() + "< "
            + MwiRIL.requestToString(mRequest)
            + " error: " + ex + " ret=" + MwiRIL.retToString(mRequest, ret));
...
/**
 * Mwi RIL Adapter implementation.
 *
 * {@hide}
 */
public final class MwiRIL extends MwiBaseCommands implements MwiCommandsInterface {
    ...
    static final boolean MWIRIL_LOGV = false; // STOPSHIP if true
    ...
    volatile IMtkRadioEx mRadioProxy = null;
            ...
            switch (msg.what) {
                    ...
                    synchronized (mRequestList) {
                        if (msg.arg1 == mWlSequenceNum && clearWakeLock(FOR_WAKELOCK)) {
                            if (MWIRIL_LOGD) {
                                ...
                                Rlog.d(MWIRIL_LOG_TAG, "WAKE_LOCK_TIMEOUT " +
                                        " mRequestList=" + count);
                                for (int i = 0; i < count; i++) {
                                    rr = mRequestList.valueAt(i);
                                    Rlog.d(MWIRIL_LOG_TAG, i + ": [" + rr.mSerial + "] "
                ...
                case EVENT_ACK_WAKE_LOCK_TIMEOUT:
                    if (msg.arg1 == mAckWlSequenceNum && clearWakeLock(FOR_ACK_WAKELOCK)) {
                        if (MWIRIL_LOGV) {
                            Rlog.d(MWIRIL_LOG_TAG, "ACK_WAKE_LOCK_TIMEOUT");
                        }
                    }
                    break;
    ...
    private IMtkRadioEx getRadioProxy(Message result) {
        if (!mIsMobileNetworkSupported) {
            if (MWIRIL_LOGV) riljLog("getRadioProxy: Not calling getService(): wifi-only");
            return null;
        }
        ...
        try {
            mRadioProxy = IMtkRadioEx.getService(
            ...
            if (mRadioProxy != null) {
                ...
                mRadioProxy.setResponseFunctionsMwi(mRadioResponse, mRadioIndication);
    ...
    public MwiRIL(Context context, int instanceId, Looper looper) {
        ...
        mRadioResponse = new MwiRadioResponse(this, instanceId);
        mRadioIndication = new MwiRadioIndication(this, instanceId);

        mRilHandler = new RilHandler(looper);
        mRadioProxyDeathRecipient = new RadioProxyDeathRecipient();
        PowerManager pm = (PowerManager)context.getSystemService(Context.POWER_SERVICE);
        mWakeLock = pm.newWakeLock(PowerManager.PARTIAL_WAKE_LOCK, MWIRIL_LOG_TAG);
        ...
        IMtkRadioEx proxy = getRadioProxy(null);
        if (MWI_RILA_LOGD) {
            riljLog("MwiRIL: proxy = " + (proxy == null));
        }
    }
    ...
    public void rilAction(Message response) {
        ...
        if (radioProxy != null) {
            RILRequest rr = obtainRequest(MwiRILConstants.RIL_REQUEST_XXX_XXX, result,
    ...
    ******************************************************************************/
    ...
    /// Public APIs for MwiRIL ///////////////////////////////////////////////////

    @Override
    public void setWifiEnabled(String ifName, int isWifiEnabled, int isFlightModeOn,
            Message response) {
        IMtkRadioEx radioProxy = getRadioProxy(response);
        if (radioProxy != null) {
            RILRequest rr = obtainRequest(MwiRILConstants.RIL_REQUEST_SET_WIFI_ENABLED,
                    response, mRILDefaultWorkSource);

            if (MWI_RILA_LOGD) {
                riljLog(rr.serialString() + "> " + requestToString(rr.mRequest) +
                    " ifName:" + ifName + " isWifiEnabled:" + isWifiEnabled +
                    " isFlightModeOn:" + isFlightModeOn);
            }

            try {
                radioProxy.setWifiEnabled(rr.mSerial, ifName,
    ...
    public void setWifiAssociated(String ifName, boolean associated, String ssid,
            String apMac, int mtuSize, String ueMac, Message response) {
        IMtkRadioEx radioProxy = getRadioProxy(response);
        if (radioProxy != null) {
            RILRequest rr = obtainRequest(MwiRILConstants.RIL_REQUEST_SET_WIFI_ASSOCIATED,
                    response, mRILDefaultWorkSource);

            if (MWI_RILA_LOGD) {
                riljLog(rr.serialString() + "> " + requestToString(rr.mRequest) +
                        " ifName:" + ifName + " associated:" + associated + ", mtu: " + mtuSize);
            }

            try {
                radioProxy.setWifiAssociated(rr.mSerial,
                        ifName, (associated ? 1 : 0), ssid, apMac, mtuSize, ueMac);
            } catch (RemoteException | RuntimeException e) {
    ...
    public void setWifiSignalLevel(int rssi, int snr, Message response) {
        IMtkRadioEx radioProxy = getRadioProxy(response);
        if (radioProxy != null) {
            RILRequest rr = obtainRequest(MwiRILConstants.RIL_REQUEST_SET_WIFI_SIGNAL_LEVEL, response,
                    mRILDefaultWorkSource);
    ...
    @Override
    public void setWifiIpAddress(String ifName, String ipv4Addr, String ipv6Addr,
        ...
        IMtkRadioEx radioProxy = getRadioProxy(response);
        if (radioProxy != null) {
            RILRequest rr = obtainRequest(MwiRILConstants.RIL_REQUEST_SET_WIFI_IP_ADDRESS,
                    response, mRILDefaultWorkSource);
            ...
            if (MWI_RILA_LOGD) {
                riljLog(rr.serialString() + "> " + requestToString(rr.mRequest) +
                        ...
                        " dnsCount: " + dnsCount + " dnsAddresses: " + dnsAddresses);
            }

            try {
                radioProxy.setWifiIpAddress(rr.mSerial,
                        ifName, ipv4Addr, ipv6Addr,
                        ipv4PrefixLen, ipv6PrefixLen, ipv4Gateway, ipv6Gateway,
    ...
    public void setWfcConfig(int setting, String ifName, String value, Message response) {
        IMtkRadioEx radioProxy = getRadioProxy(response);
        if (radioProxy != null) {
            RILRequest rr = obtainRequest(MwiRILConstants.RIL_REQUEST_SET_WFC_CONFIG,
                    response, mRILDefaultWorkSource);
            ...
            try {
                radioProxy.setWfcConfig(rr.mSerial,
                        setting, ifName, value);
            } catch (RemoteException | RuntimeException e) {
    ...
    public void setLocationInfo(String accountId, String broadcastFlag, String latitude,
                ...
                String zip, String countryCode, String ueWlanMac, Message response) {
        IMtkRadioEx radioProxy = getRadioProxy(response);
        if (radioProxy != null) {
            RILRequest rr = obtainRequest(MwiRILConstants.RIL_REQUEST_SET_GEO_LOCATION,
                    response, mRILDefaultWorkSource);

            if (MWI_RILA_LOGD) {
                riljLog(rr.serialString() + "> " + requestToString(rr.mRequest) +
                    " accountId:" + accountId + " broadcastFlag:" + broadcastFlag +
                    // " latitude:" + latitude + " longitude:" + longitude +
                    " accuracy:" + accuracy + " method:" + method +
                    " city:" + city + " state:" + state +
                    " zip:" + zip + " countryCode:" + countryCode);
            }

            try {
                radioProxy.setLocationInfo(rr.mSerial,
    ...
    public void setEmergencyAddressId(String aid, Message response) {
        IMtkRadioEx radioProxy = getRadioProxy(response);
        if (radioProxy != null) {
            RILRequest rr = obtainRequest(MwiRILConstants.RIL_REQUEST_SET_EMERGENCY_ADDRESS_ID,
                    response, mRILDefaultWorkSource);
    ...
    public void setNattKeepAliveStatus(String ifName, boolean enable,
        ...
        IMtkRadioEx radioProxy = getRadioProxy(response);
        if (radioProxy != null) {
            RILRequest rr = obtainRequest(MwiRILConstants.RIL_REQUEST_SET_NATT_KEEPALIVE_STATUS,
                    response, mRILDefaultWorkSource);

            if (MWI_RILA_LOGD) {
                riljLog(rr.serialString() + "> " + requestToString(rr.mRequest) +
                    " ifName:" + ifName + " enable:" + enable);
            }

            try {
                radioProxy.setNattKeepAliveStatus(rr.mSerial,
    ...
    public void setWifiPingResult(int rat, int latency, int pktloss, Message response) {
        IMtkRadioEx radioProxy = getRadioProxy(response);
        if (radioProxy != null) {
            RILRequest rr = obtainRequest(MwiRILConstants.RIL_REQUEST_SET_WIFI_PING_RESULT,
                    response, mRILDefaultWorkSource);

            if (MWI_RILA_LOGD) {
                riljLog(rr.serialString() + "> " + requestToString(rr.mRequest) +
                    " rat = " + rat + " latency = " + latency + " pktloss = " + pktloss);

            }

            try {
                radioProxy.setWifiPingResult(rr.mSerial, rat, latency,
                    pktloss);
            } catch (RemoteException | RuntimeException e) {
    ...
    public void notifyEPDGScreenState(int state, Message response) {
        IMtkRadioEx radioProxy = getRadioProxy(response);
        if (radioProxy != null) {
            RILRequest rr = obtainRequest(MwiRILConstants.RIL_REQUEST_NOTIFY_EPDG_SCREEN_STATE,
                        response, mRILDefaultWorkSource);
    ...
    void processIndication(int indicationType) {
        if (indicationType == RadioIndicationType.UNSOLICITED_ACK_EXP) {
            sendAck();
            if (MWIRIL_LOGD) riljLog("Unsol response received; Sending ack to ril.cpp");
        } else {
    ...
    RILRequest processResponse(RadioResponseInfo responseInfo) {
        ...
        rr = findAndRemoveRequestFromList(serial);
        if (rr == null) {
            Rlog.e(MwiRIL.MWIRIL_LOG_TAG, "processResponse: Unexpected response! serial: " + serial
                    + " error: " + error);
            return null;
        }
        ...
        // Here and below fake MwiRILConstants.RIL_UNSOL_RESPONSE_SIM_STATUS_CHANGED, see b/7255789.
    ...
    private void acquireWakeLock(RILRequest rr, int wakeLockType) {
        synchronized (rr) {
            if (rr.mWakeLockType != INVALID_WAKELOCK) {
                Rlog.d(MWIRIL_LOG_TAG, "Failed to aquire wakelock for " + rr.serialString());
                return;
            }
            ...
            switch(wakeLockType) {
                ...
                case FOR_ACK_WAKELOCK:
                    ...
                    break;
                default: //WTF
                    Rlog.w(MWIRIL_LOG_TAG, "Acquiring Invalid Wakelock type " + wakeLockType);
                    return;
            }
    ...
    private boolean clearWakeLock(int wakeLockType) {
        if (wakeLockType == FOR_WAKELOCK) {
            synchronized (mWakeLock) {
                ...
                Rlog.d(MWIRIL_LOG_TAG, "NOTE: mWakeLockCount is " + mWakeLockCount
                        + "at time of clearing");
                mWakeLockCount = 0;
                mWakeLock.release();
    ...
    private void clearRequestList(int error, boolean loggable) {
        ...
        synchronized (mRequestList) {
            int count = mRequestList.size();
            if (MWIRIL_LOGD && loggable) {
                Rlog.d(MWIRIL_LOG_TAG, "clearRequestList " + " mWakeLockCount="
    ...
    static String responseToString(int request)
    {
        switch(request) {
            ...
            case MwiRILConstants.RIL_UNSOL_WIFI_LOCK:
    ...
    static String requestToString(int request) {
        switch(request) {
            case MwiRILConstants.RIL_REQUEST_SET_WIFI_ENABLED:
                return "MwiRILConstants.RIL_REQUEST_SET_WIFI_ENABLED";
            case MwiRILConstants.RIL_REQUEST_SET_WIFI_ASSOCIATED:
                return "MwiRILConstants.RIL_REQUEST_SET_WIFI_ASSOCIATED";
            case MwiRILConstants.RIL_REQUEST_SET_WIFI_SIGNAL_LEVEL:
                return "MwiRILConstants.RIL_REQUEST_SET_WIFI_SIGNAL_LEVEL";
            case MwiRILConstants.RIL_REQUEST_SET_WIFI_IP_ADDRESS:
                return "MwiRILConstants.RIL_REQUEST_SET_WIFI_IP_ADDRESS";
            case MwiRILConstants.RIL_REQUEST_SET_GEO_LOCATION:
                return "MwiRILConstants.RIL_REQUEST_SET_GEO_LOCATION";
            case MwiRILConstants.RIL_REQUEST_SET_EMERGENCY_ADDRESS_ID:
                return "MwiRILConstants.RIL_REQUEST_SET_EMERGENCY_ADDRESS_ID";
            case MwiRILConstants.RIL_REQUEST_SET_NATT_KEEPALIVE_STATUS:
                return "MwiRILConstants.RIL_REQUEST_SET_NATT_KEEPALIVE_STATUS";
            case MwiRILConstants.RIL_REQUEST_SET_WIFI_PING_RESULT:
                return "MwiRILConstants.RIL_REQUEST_SET_WIFI_PING_RESULT";
            // Notify ePDG screen state
            case MwiRILConstants.RIL_REQUEST_NOTIFY_EPDG_SCREEN_STATE:
                return "MwiRILConstants.RIL_REQUEST_NOTIFY_EPDG_SCREEN_STATE";
            case MwiRILConstants.RIL_REQUEST_SET_WFC_CONFIG:
    ...
    void riljLog(String msg) {
        Rlog.d(MWIRIL_LOG_TAG, msg
                + (mPhoneId != null ? (" [SUB" + mPhoneId + "]") : ""));
    }

    void riljLoge(String msg) {
        Rlog.e(MWIRIL_LOG_TAG, msg
                + (mPhoneId != null ? (" [SUB" + mPhoneId + "]") : ""));
    }
