...
public class AtParser {
    ...
    public AtCommandResult process(String raw_input) {
        ...
        if (!input.regionMatches(0, "AT", 0, 2)) {
            ...
            return new AtCommandResult(AtCommandResult.UNKNOWN_COMMAND);
        }
        ...
        while (index < input.length()) {
            ...
            if (isAtoZ(c)) {
                ...
                if (mBasicHandlers.containsKey((Character) c)) {
                    ...
                    result.addResult(
                            new AtCommandResult(AtCommandResult.UNKNOWN_COMMAND));
                    return result;
                }
            ...
            if (c == '+' || c == '%') {
                ...
                Log.d(LOG_TAG, "mExtHandlers cmd [" + index + "]" + input + "+["
                    + commandName + "]");
                ...
                case TYPE_READ:
                    if (i + 1 < endIndex) {
                        try {
                            ...
                            String rspStr = input.substring(i + MTKATCIRIL_PATTERN.length() + 1,
                                endIndex);
                ...
                case TYPE_TEST:
                    if (i + 2 < endIndex) {
                        try {
                            ...
                            String rspStr = input.substring(i + MTKATCIRIL_PATTERN.length() + 2,
                                endIndex);
                            if (subStr.startsWith(MTKATCIRIL_PATTERN)) {
                                args = new String[1];
                                args[0] = rspStr;
                ...
                case TYPE_SET:
                    ...
                    if (i + 1 < endIndex) {
                        ...
                        try {
                            ...
                            Log.e(LOG_TAG,
                                "IndexOutOfBoundsException: treat as normal mmi command.");
                            isGenerateArgs = 1;
                        }
                ...
                Log.d(LOG_TAG, "mExtHandlers,resultCode=" + result.getResultCode() + ",length="
                    + input.length());
                if (result.getResultCode() != AtCommandResult.OK &&
                        result.getResultCode() != AtCommandResult.RILRSP) {
                    return result;   // short-circuit
                }
                if (result.getResultCode() == AtCommandResult.RILRSP) {
                    if (input.regionMatches(endIndex + 1, "AT", 0, 2)) {
                        Log.d(LOG_TAG, "regionMatches matches another RILESP."
