...
public class SendRequest extends MmsRequest {
    ...
    protected Uri persistIfRequired(Context context, int result, byte[] response) {
        final String requestId = getRequestId();
        // just print the send messageId in log file
        if (response != null && response.length > 0) {
            ...
            if (sendConfPdu != null
                ...
                LogUtil.d(requestId, "persistIfRequired: messageId = "
                        + PduPersister.toIsoString(((SendConf)sendConfPdu).getMessageId()));
            }
        }
        // if send from com.android.phone, also not persist into DB
        if (!SmsApplication.shouldWriteMessageForPackage(mCreator, context) ||
                TextUtils.equals("com.android.phone", mCreator)) {
