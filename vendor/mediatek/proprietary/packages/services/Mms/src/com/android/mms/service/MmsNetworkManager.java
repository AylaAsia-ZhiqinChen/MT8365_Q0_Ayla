...
public class MmsNetworkManager {
    ...
    // for mtk solution, data fwk retry time is usually not reach 10min limit, so reduce the pdn wait time.
    private static final int NETWORK_REQUEST_TIMEOUT_MILLIS = 5 * 60 * 1000;
        ...
        synchronized (this) {
            if (mMmsRequestCount > 0) {
                ...
                LogUtil.d(requestId, "MmsNetworkManager: release, count=" + mMmsRequestCount  +
                        ", delayRelease=" + shouldDelayRelease);
                if (mMmsRequestCount < 1) {
    ...
    private void startNewNetworkRequestLocked() {
        ...
        int networkType = getTelephonyManager().getDataNetworkType(mSubId);
