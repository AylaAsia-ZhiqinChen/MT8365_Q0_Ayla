...
public abstract class ServiceBinder {
        ...
        public void onServiceConnected(ComponentName componentName, IBinder binder) {
            try {
                ...
                synchronized (mLock) {
                    ...
                    if (binder != null) {
                        /// M: needn't listen binder die event, when binder is dead, @{
                        /// onServiceDisconnected will be invoked from main thread.
                        mServiceDeathRecipient = null;
                        // mServiceDeathRecipient = new ServiceDeathRecipient(componentName);
                        // try {
                        /// @}
                        /// M: record mediatek binder first, our service return mtk binder @{
                        setMtkBinder(binder);
                        binder = getAospBinder(binder);
                        // binder.linkToDeath(mServiceDeathRecipient, 0);
                        /// @}
                        mServiceConnection = this;
                        setBinder(binder);
                        handleSuccessfulConnection();
                        /// M: since we comment linkToDeath, there is no Exception. @{
                        /*
        ...
        public void onServiceDisconnected(ComponentName componentName) {
            try {
                ...
                synchronized (mLock) {
                    ...
                    /// M: unbindService, otherwise it will auto-bind again after restart.
    ...
    // M: For mediatek's owner interface.
    ...
    /**
     * M: Sets the mediatek service interface after the service is bound.
     *
     * @param binder The new binder interface that is being set.
     */
    protected abstract void setMtkServiceInterface(IBinder binder);

    /**
     * M: Removes the mediatek service interface before the service is unbound.
     */
    protected abstract void removeMtkServiceInterface();

    /**
     * M: Check whether mediatek binder is valid or not.
     */
    public boolean isMtkServiceValid(String actionName) {
        if (mMtkBinder == null) {
            Log.w(this, "%s invoked while mediatek service is unbound", actionName);
            return false;
        }
    ...
    /**
     * M: Set mediatek service binder, invoke it before aosp setBinder.
     */
    private void setMtkBinder(IBinder binder) {
        if (!TelephonyUtil.isPstnComponentName(mComponentName)) {
            Log.d(this, "not mediatek service, binder is aosp.");
            return;
        }
        if (mMtkBinder != binder) {
            if (binder == null) {
                removeMtkServiceInterface();
                mMtkBinder = null;
            } else {
                mMtkBinder = binder;
                setMtkServiceInterface(binder);
            }
        }
    }

    /**
     * M: get aosp binder, it is a blocking method.
     */
    private IBinder getAospBinder(IBinder binder) {
        if (mMtkBinder == null) {
            Log.w(this, "binder is aosp, return directly.");
            return binder;
        }
        try {
            IMtkConnectionService service = IMtkConnectionService.Stub.asInterface(binder);
            return service.getBinder();
        } catch (RemoteException e) {
            Log.e(this, e, "get aosp binder error! it shouldn't be!");
            return null;
        }
    }
}
