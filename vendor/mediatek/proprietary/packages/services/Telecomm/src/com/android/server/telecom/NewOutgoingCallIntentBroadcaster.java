...
public class NewOutgoingCallIntentBroadcaster {
        ...
        public void onReceive(Context context, Intent intent) {
            try {
                ...
                synchronized (mLock) {
                    ...
                    /// M: Performance enhancement. Send a special intent to UnDemote.
                    broadcastToUndemote(resultNumber, mCall.getInitiatingUser());
                    ...
                    Uri resultHandleUri = Uri.fromParts(
                            mPhoneNumberUtilsAdapter.isUriNumber(resultNumber)
                                    /// M: If specified "tel:" on a uri number,
                                    /// don't replace it with "sip"
                                    && !MtkUtil.isImsCallIntent(mIntent) ?
    ...
    public void processCall(CallDisposition disposition) {
        ...
        /// M: Set broadcast done if no broadcast sent or no self receiver. @{
        if (disposition.callImmediately || !disposition.sendBroadcast) {
    ...
    private void broadcastIntent(
        ...
        /// M: Performance enhancement. Send a special intent to UnDemote. @{
        if (!receiverRequired) {
            broadcastToUndemote(number, targetUser);
        }
        /// M: @}
    }
     ...
     * Telecom will send a broadcast of ACTION_NEW_OUTGOING_CALL when make an outgoing call,
     * Telecom is the last receiver of this broadcast and maybe blocked by
     * UnDemote(previous receiver) becuase UnDemote runs in Dialer process which is very busy.
     * For not to block Telecom to start outgoing call, broadcast a special intent to UnDemote.
     */
    private void broadcastToUndemote(String number, UserHandle targetUser) {
        Intent intent = new Intent(MtkTelecomManager.ACTION_NEW_OUTGOING_CALL_UNDEMOTE);
