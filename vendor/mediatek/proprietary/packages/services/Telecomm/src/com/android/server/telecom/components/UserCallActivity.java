...
public class UserCallActivity extends Activity implements TelecomSystem.Component {
    ...
    protected void onCreate(Bundle bundle) {
        ...
        try {
            ...
            /// M: Not support launch conference invitation by send Intent.ACTION_CALL directly. @{
            if (MtkUtil.isConferenceInvitation(intent.getExtras())) {
                Log.w(this, "[onCreate] Unsupported conference invitation via startActivity");
                finish();
                return;
            }
            /// @}
            ...
            if (!MtkUtil.checkCallForwardForActivity(this, intent)) {
                Log.w(this, "No call forward permission!");
                MtkUtil.showCallForwardPermissionDialog(this, userHandle);
                finish();
                return;
            }
            /// M: @}
