...
public class CallAudioManager extends CallsManagerListenerBase {
    ...
    public void onCallStateChanged(Call call, int oldState, int newState) {
        ...
        ///M: ALPS02797621 ECC retry @{
        // Set audio mode to normal for ecc retry.
        // Speech driver request to reset audio mode after MD reset.
    ...
    public void onCallRemoved(Call call) {
        ...
        ///M: ALPS03481174 @{
        // disable mute when add ECC call
        // under normal call which has open mute.
        if (mCallsManager.hasEmergencyCall()) {
            Log.d(this, "unmute if there has ecc");
            mute(false);
        }
        /// @}
    }
    ...
    public void onVideoStateChanged(Call call, int previousVideoState, int newVideoState) {
        ...
        if (!VideoProfile.isVideo(previousVideoState) &&
                /// M: [ALPS03800837] disable speaker when voice call playing video ringtone. @{
    ...
    public void mute(boolean shouldMute) {
        Log.v(this, "mute, shouldMute: %b, hasEmergency: %b",
                shouldMute, mCallsManager.hasEmergencyCall());
    ...
    private void onCallLeavingActiveDialingOrConnecting() {
        ...
        if (mActiveDialingOrConnectingCalls.size() == 1 && MtkUtil.isInDsdaMode()) {
    ...
    private void onCallEnteringActiveDialingOrConnecting() {
        if (mActiveDialingOrConnectingCalls.size() == 1) {
            // M: ALPS01781841
            // not set Ecc audio mode when state is CONNECTING @{
    ...
    private void updateForegroundCall() {
        ...
        if (mForegroundCall != oldForegroundCall) {
            ...
            mCallsManager.onForegroundCallChanged(oldForegroundCall, mForegroundCall);
            /// @}
        }
    }
