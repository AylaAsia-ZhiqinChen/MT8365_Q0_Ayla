...
public class LogUtils {
    ...
    private static void eventRecordAdded(EventManager.EventRecord eventRecord) {
        ...
        if (recordEntry instanceof Call) {
            ...
            Log.i(LOGUTILS_TAG, "EventRecord added as Call: " + callRecordEntry);
            Analytics.CallInfo callInfo = callRecordEntry.getAnalytics();
            if(callInfo != null) {
                callInfo.setCallEvents(eventRecord);
            } else {
                Log.w(LOGUTILS_TAG, "Could not get Analytics CallInfo.");
            }
        } else {
            Log.w(LOGUTILS_TAG, "Non-Call EventRecord Added.");
        }
        /// @}
    }
    ...
    public static void initLogging(Context context) {
        /// M: Using mtk log instead of AOSP log. @{
        Log.setTag(TAG);
        /// M: Setup Log level based on property.
        Log.setupLoggableFlags();
        Log.setSessionContext(context);
        for (EventManager.TimedEventPair p : Events.Timings.sTimedEvents) {
            Log.addRequestResponsePair(p);
        }
        Log.registerEventListener(LogUtils::eventRecordAdded);
        // Store analytics about recently completed Sessions.
        Log.registerSessionListener(Analytics::addSessionTiming);
        /// @}
    }
}
