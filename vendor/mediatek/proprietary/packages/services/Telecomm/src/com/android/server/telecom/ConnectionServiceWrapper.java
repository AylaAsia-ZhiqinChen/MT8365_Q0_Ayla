...
public class ConnectionServiceWrapper extends ServiceBinder implements
    ...
    private final class Adapter extends IConnectionServiceAdapter.Stub {
        ...
        public void setAddress(String callId, Uri address, int presentation,
            ...
            try {
                synchronized (mLock) {
                    /// M: Encrypt uri to avoid sensitive information leak. @{
        ...
        public void setCallerDisplayName(String callId, String callerDisplayName, int presentation,
            ...
            try {
                synchronized (mLock) {
                    /// M: Encrypt string to avoid sensitive information leak. @{
        ...
        public void addExistingConnection(String callId, ParcelableConnection connection,
            ...
            try {
                synchronized (mLock) {
                    /// M: If the service is already unbinded, return directly. @{
                    /*
                     * The issue happen in the following scenario.
                     * 1. There is only one call with the Teleservice
                     * 2. Telecom->Teleservice: abort the call
                     * 3. Telecom->Teleservice: unbind(binder is set as NULL)
                     * 4. Teleservice->Telecom: addExistingConnection
                     * 5. Teleservice executes the abort
                     * 6. Teleservice executes the unbind
                     * 7. Telecom executes addExistingConnection
                     *
                     * If we don't return here, the UI will always display dialing.
                     */
    ...
    public void createConnection(final Call call, final CreateConnectionResponse response) {
        ...
        if (call.isConferenceInvitation()) {
            createConferenceConnection(call, response);
            return;
        }
        /// M: @}
        ...
        BindCallback callback = new BindCallback() {
            ...
            public void onSuccess() {
                ...
                try {
                    /// M: mServiceInterface maybe be null in MTBF.
                    if (mServiceInterface != null) {
    ...
    public void onCallAudioStateChanged(Call activeCall, CallAudioState audioState) {
        ...
        if (callId != null && isServiceValid("onCallAudioStateChanged")) {
            try {
                ...
                /// M: mServiceInterface may be null, need check.
                if (mServiceInterface != null) {
    ...
    private void handleCreateConnectionComplete(
        ...
        if (connection.getState() == Connection.STATE_DISCONNECTED) {
            ...
            /// M: Need to disconnect the call in the following scenario. {@
            /*
             * When ECC retry happens, teleservice will invoke
             * addExistingConnection to change call Id.
             * If the ECC cannot be made successfully, teleservice
             * will invoke handleCreateConnectionComplete with a
            ...
            */
            ...
            if (call != null &&
                ...
                Log.d(this, "disconnect call when create connection complete %s %s",
                    connection.getDisconnectCause(), call);
                mCallsManager.markCallAsDisconnected(call, connection.getDisconnectCause());
                mCallsManager.markCallAsRemoved(call);
            } else {
                Log.d(this, "remove Call when create connection complete");
                removeCall(callId, connection.getDisconnectCause());
            }
            /// M: @}
        } else {
    ...
    /// M: Add APIs for call related operations. @{
    private MtkAdapter mMtkAdapter = new MtkAdapter();
    private IMtkConnectionService mMtkServiceInterface;

    @Override
    protected void setMtkServiceInterface(IBinder binder) {
        mMtkServiceInterface = IMtkConnectionService.Stub.asInterface(binder);
        Log.v(this, "Adding MTK Connection Service Adapter.");
        addMtkConnectionServiceAdapter(mMtkAdapter);
    }

    @Override
    protected void removeMtkServiceInterface() {
        Log.v(this, "Removing MTK Connection Service Adapter.");
        removeMtkConnectionServiceAdapter(mMtkAdapter);
        mMtkServiceInterface = null;
    }

    /**
     * M: Adapter for MTK TeleService to call MTK Telecom APIs.
     */
    private final class MtkAdapter extends IMtkConnectionServiceAdapter.Stub {
        @Override
        public void handleCreateConferenceComplete(
                String conferenceId, ConnectionRequest request,
                ParcelableConference conference,
                DisconnectCause disconnectCause) throws RemoteException {
            long token = Binder.clearCallingIdentity();
            try {
                synchronized (mLock) {
                    logIncoming("handleCreateConferenceComplete");
    ...
    private void addMtkConnectionServiceAdapter(IMtkConnectionServiceAdapter adapter) {
        if (isMtkServiceValid("addMtkConnectionServiceAdapter")) {
            try {
                logOutgoing("addMtkConnectionServiceAdapter %s", adapter);
                mMtkServiceInterface.addMtkConnectionServiceAdapter(adapter);
            } catch (RemoteException e) {
                Log.e(this, e, "Failed to addMtkConnectionServiceAdapter via " +
                        "IMtkConnectionService.addMtkConnectionServiceAdapter");
            }
        }
    }

    private void removeMtkConnectionServiceAdapter(IMtkConnectionServiceAdapter adapter) {
        if (isMtkServiceValid("removeMtkConnectionServiceAdapter")) {
            try {
                logOutgoing("removeMtkConnectionServiceAdapter %s", adapter);
                mMtkServiceInterface.clearMtkConnectionServiceAdapter();
            } catch (RemoteException e) {
                Log.e(this, e, "Failed to removeMtkConnectionServiceAdapter via " +
                        "IMtkConnectionService.clearMtkConnectionServiceAdapter");
            }
        }
    }

    /**
     * M: Disconnect active and answer waiting call.
     *
     * @param call The active call.
     * @param pendingAction The pending action that need to be handled after disconnect.
     */
    void disconnectWithPendingAction(Call call, String pendingAction) {
        final String callId = mCallIdMapper.getCallId(call);
        if (callId != null && isServiceValid("disconnectWithPendingAction")) {
            try {
                logOutgoing("disconnectWithPendingAction %s pending action: %s",
                        callId, pendingAction);
                mMtkServiceInterface.handleOrderedOperation(callId,
                        MtkConnection.OPERATION_DISCONNECT_CALL, pendingAction);
            } catch (RemoteException e) {
                Log.e(this, e, "disconnectWithPendingAction handleOrderedOperation");
            }
        }
    }
     ...
     *
     * @param call The call to be hangupped.
     */
    void hangupAll(Call call) {
        if (isServiceValid("hangupAll")) {
            try {
                logOutgoing("hangupAll %s", mCallIdMapper.getCallId(call));
                mMtkServiceInterface.hangupAll(mCallIdMapper.getCallId(call));
            } catch (RemoteException e) {
                Log.e(this, e, "hangupAll");
            }
        }
    }

    /**
     * M: Create VoLTE invitation conference.
     *
     * @param call The VoLTE invitation conference call.
     * @param response The response from TeleService.
     */
    void createConferenceConnection(Call call, CreateConnectionResponse response) {
        Log.d(this, "createConferenceConnection(%s) via %s.", call, getComponentName());
        BindCallback callback = new BindCallback() {
            @Override
            public void onSuccess() {
                String callId = mCallIdMapper.getCallId(call);
                ...
                if (call.isIncoming() && mCallsManager.getEmergencyCallHelper()
                        .getLastEmergencyCallTimeMillis() > 0) {
                    // Add the last emergency call time to the connection request for
                    // incoming calls
                    if (extras == call.getIntentExtras()) {
                        extras = (Bundle) extras.clone();
                    }
                    extras.putLong(android.telecom.Call.EXTRA_LAST_EMERGENCY_CALLBACK_TIME_MILLIS,
                            mCallsManager.getEmergencyCallHelper()
                ...
                try {
                    /// M: mMtkServiceInterface maybe be null in MTBF.
                    if (mMtkServiceInterface != null) {
                        mMtkServiceInterface.createConference(
                                call.getConnectionManagerPhoneAccount(),
                                callId,
                                connectionRequest,
                                call.getConferenceInvitationNumbers(),
                                call.isIncoming(),
                                Log.getExternalSession());
                    }
    ...
    /**
     * M: Handle parcelable conference response.
     */
    private void handleCreateConferenceComplete(
            String conferenceId, ConnectionRequest request, ParcelableConference conference,
        ...
        if (conference.getState() == Connection.STATE_DISCONNECTED) {
            // A connection that begins in the DISCONNECTED state is an indication of
            // failure to connect.
            removeCall(conferenceId, disconnectCause);
        } else {
            // Successful connection
            if (mPendingResponses.containsKey(conferenceId)) {
                mPendingResponses.remove(conferenceId);
                Call call = mCallIdMapper.getCall(request.getTelecomCallId());
                call.handleCreateConferenceSuccess(conference);
            }
        }
    }

    /**
     * M: Call inviteConferenceParticipants to TeleService.
     *
     * @param call The VoLTE invitation conference call.
     * @param numbers The invitation numbers.
     */
    public void inviteNumbersToConference(Call call, List<String> numbers) {
        if (isServiceValid("inviteNumbersToConference")) {
            try {
                logOutgoing("inviteNumbersToConference %s", mCallIdMapper.getCallId(call));
                mMtkServiceInterface.inviteConferenceParticipants(
                        mCallIdMapper.getCallId(call), numbers);
            } catch (RemoteException e) {
                Log.e(this, e, "inviteNumbersToConference");
            }
        }
    }

    /**
     * M: Explicit call transfer.
     *
     * @param call The call want to be explicit transferred.
     */
    void explicitCallTransfer(Call call) {
        if (isServiceValid("explicitCallTransfer")) {
            try {
                logOutgoing("explicitCallTransfer %s", mCallIdMapper.getCallId(call));
                mMtkServiceInterface.explicitCallTransfer(mCallIdMapper.getCallId(call));
            } catch (RemoteException e) {
                Log.e(this, e, "explicitCallTransfer");
            }
        }
    }

    // blind/assured explicit call transfer.
    void explicitCallTransfer(Call call, String number, int type) {
        ...
        if (callId != null && isServiceValid("explicitCallTransfer")) {
            try {
                ...
                mMtkServiceInterface.blindAssuredEct(callId, number, type);
            } catch (RemoteException e) {
                Log.e(this, e, "blindAssuredEct");
            }
        }
    }
    /// M: @}

    /// M: For call rejection requirement. @{
    void rejectWithCause(Call call, int cause) {
        final String callId = mCallIdMapper.getCallId(call);
        if (callId != null && isServiceValid("rejectWithCause")) {
            try {
                logOutgoing("rejectWithCause %s", callId);
                mMtkServiceInterface.rejectWithCause(callId, cause);
            } catch (RemoteException e) {
                Log.e(this, e, "rejectWithCause");
            }
        }
    }
    /// M: @}
