...
/// M: import Mockito.mock for extended cases.
...
/// M: import InCallController for extended cases.
...
public class NewOutgoingCallIntentBroadcasterTest extends TelecomTestCase {
    ...
    /// M: test tel: xxx@xxx.xxx format number should be kept
    //  after NewOutgoingCallIntentBroadcaster.processCall
    @SmallTest
    @Test
    public void testUnmodifiedImsCall1() {
        Uri handle = Uri.parse("tel:test@test.com");
        Intent intent = new Intent(Intent.ACTION_CALL, handle);
        doReturn(true).when(mPhoneNumberUtilsAdapterSpy).isPotentialLocalEmergencyNumber(
                any(Context.class), eq(handle.getSchemeSpecificPart()));
        ...
        Uri encHandle = Uri.fromParts(handle.getScheme(),
                handle.getSchemeSpecificPart(), null);
        verify(mCallsManager).placeOutgoingCall(any(Call.class), eq(encHandle),
                isNull(GatewayInfo.class), anyBoolean(), anyInt());
    }

    /// M: test tel: xxx@xxx.xxx format number should be kept
    //  after NewOutgoingCallIntentBroadcaster.onReceive
    @SmallTest
    @Test
    public void testUnmodifiedImsCall2() {
        Uri handle = Uri.parse("tel:test@test.com");
        Intent intent = new Intent(Intent.ACTION_CALL, handle);

        doReturn(false).when(mPhoneNumberUtilsAdapterSpy).isPotentialLocalEmergencyNumber(
                any(Context.class), eq(handle.getSchemeSpecificPart()));
        ReceiverIntentPair result = regularCallTestHelper(intent, null);
        verify(mCallsManager, never()).placeOutgoingCall(any(Call.class), any(Uri.class),
                any(GatewayInfo.class), anyBoolean(), anyInt());
        result.receiver.setResultData(
                result.intent.getStringExtra(Intent.EXTRA_PHONE_NUMBER));
    ...
    /// M: test tel: xxx@xxx.xxx format number should be kept
    //  after CallIntentProcessor.processOutgoingCallIntent
    @SmallTest
    @Test
    public void testUnmodifiedImsCall3() {
        Uri handle = Uri.parse("tel:test@test.com");
        Intent intent = new Intent(Intent.ACTION_CALL, handle);
        intent.putExtra(com.android.server.telecom.CallIntentProcessor.KEY_INITIATING_USER,
                UserHandle.CURRENT);
        InCallController icc = mock(InCallController.class);
        when(icc.isBoundAndConnectedToServices()).thenReturn(true);
        when(mCallsManager.getInCallController()).thenReturn(icc);
        when(mCallsManager.isSelfManaged(isNull(android.telecom.PhoneAccountHandle.class),
                                         any(UserHandle.class))).thenReturn(true);
        /// M: Mock CallsManager.startOutgoingCall to avoid NullPointerException.
        when(mCallsManager.startOutgoingCall(any(Uri.class),
                isNull(android.telecom.PhoneAccountHandle.class),
                any(Bundle.class), any(UserHandle.class), any(Intent.class), anyString()))
                .thenReturn(CompletableFuture.completedFuture(null));
        new com.android.server.telecom.CallIntentProcessor(mContext, mCallsManager).processIntent(
                intent, mContext.getOpPackageName());
        Uri encHandle = Uri.fromParts(handle.getScheme(),
                handle.getSchemeSpecificPart(), null);
        verify(mCallsManager).startOutgoingCall(eq(encHandle),
                isNull(android.telecom.PhoneAccountHandle.class),
                any(Bundle.class), any(UserHandle.class), any(Intent.class), anyString());
    }
}
