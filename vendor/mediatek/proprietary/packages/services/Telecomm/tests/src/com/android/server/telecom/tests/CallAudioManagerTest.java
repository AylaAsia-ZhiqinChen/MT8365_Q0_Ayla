...
/// M: Used in added case: testEmergencyCallWithRetry. @{
...
public class CallAudioManagerTest extends TelecomTestCase {
    ...
    /**
     * M: Add auto test case for emergency call retry. @{
     */
    @MediumTest
    public void testEmergencyCallWithRetry() {
        Call call = mock(Call.class);
        ...
        when(call.getState()).thenReturn(CallState.CONNECTING);
        mCallAudioManager.onCallStateChanged(call, CallState.DIALING, CallState.CONNECTING);
        verify(mCallAudioModeStateMachine, times(1)).sendMessageWithArgs(
                eq(CallAudioModeStateMachine.NEW_ACTIVE_OR_DIALING_CALL), captor.capture());
        assertMessageArgEquality(expectedArgs, captor.getValue());
        verify(mCallAudioModeStateMachine).sendMessageWithArgs(
                eq(CallAudioModeStateMachine.ABANDON_FOCUS), captor.capture());
        assertMessageArgEquality(expectedArgs, captor.getValue());
