...
public class CallsManagerTest extends TelecomTestCase {
    ...
    /// M: Need sleep test thread to avoid mock multi-thread issue. @{
    private static final int DELAY_RUN_CASE_MS = 300;
    /// @}
    ...
    public void testAnswerCallWhenOngoingCallCanBeHeld() {
        ...
        /// M: we change ActionAnswerCall.performAction to invoke Call.doAnswer for DSDA.
        verify(incomingCall).doAnswer(VideoProfile.STATE_AUDIO_ONLY);
    }
    ...
    public void testAnswerCallWhenMultipleHeldCallsExisted() {
        ...
        /// M: HeldCall using real state to fit MTK WaitingCallAction mechanism.
        // In 1A1H1W case, we need waiting heldCall state change from ON_HOLD to DISCONNECTED,
        // then answer the ringing call. So heldCall must use real state.
        // doReturn(CallState.ON_HOLD).when(heldCall).getState();
        heldCall.setState(CallState.ON_HOLD, "just cuz");
        ...
        /// M: Mock incomingCall state to RINGING, because default mock state is ACTIVE.
        // In 1A1H1W case, MTK WaitingCallAction mechanism will check incomingCall state before
        // execute waitingAction. So incomingCall change to using state RINGING not default ACTIVE.
        ...
        /// M: Add invoke markCallAsDisconnected to trigger MTK WaitingCallAction mechanism running.
        mCallsManager.markCallAsDisconnected(heldCall, new DisconnectCause(DisconnectCause.LOCAL,
                    "LOCAL"));
    ...
    public void testAnswerAlreadyActiveCall() {
        ...
        // and the incoming call is answered.
        /// M: We change ActionAnswerCall.performAction to invoke Call.doAnswer.
        verify(incomingCall).doAnswer(VideoProfile.STATE_AUDIO_ONLY);
    ...
    private Call addSpyCall(PhoneAccountHandle targetPhoneAccount) {
        ...
        /// M: aosp missing these lines.
        doNothing().when(callSpy).unhold(Matchers.anyString());
        doNothing().when(callSpy).hold(Matchers.anyString());
        doNothing().when(callSpy).disconnect(Matchers.anyString());
        /// M: we change Call.answer to Call.doAnswer.
        doNothing().when(callSpy).doAnswer(Matchers.anyInt());

        mCallsManager.addCall(callSpy);
        /// M: Sleep test thread to avoid mock multi-thread issue. @{
        //  Because mock cannot support multi-thread well, need wait addCall which use some
        //  mock mothods execute done here.
        try {
            Thread.sleep(DELAY_RUN_CASE_MS);
        } catch (InterruptedException e) {
        }
        /// @}
        return callSpy;
