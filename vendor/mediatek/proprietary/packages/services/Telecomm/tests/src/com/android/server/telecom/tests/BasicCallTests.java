...
/// M: MTK added cases need reflection
...
public class BasicCallTests extends TelecomSystemTest {
    ...
    public void testOutgoingThenIncomingCalls() throws Exception {
        ...
        mInCallServiceFixtureX.mInCallAdapter.disconnectCall(incoming.mCallId);
        /// M: MTK proprietary design, check hold call auto change to active after disconnect active call.
        mConnectionServiceFixtureA.sendSetDisconnected(incoming.mConnectionId, DisconnectCause.LOCAL);
        mConnectionServiceFixtureA.sendRemoveCall(incoming.mConnectionId);
        /// M: FakeConnectionService.unhold is running in another thread not test case thread,
        //  since mock cannot support multi-thread case well, add timeout parameter in verify
        //  method then case will wait enough time for FakeConnectionService.unhold.
        verify(mConnectionServiceFixtureA.getTestDouble(), timeout(TEST_TIMEOUT))
                .unhold(eq(outgoing.mConnectionId), any());
        mInCallServiceFixtureX.mInCallAdapter.disconnectCall(outgoing.mCallId);
    }
    ...
    public void testAddCallToConference1() throws Exception {
        ...
        /// M: change sleep time 200 to 500 for low-end chips
        Thread.sleep(500);
    ...
    private void blockNumber(String phoneNumber) throws Exception {
        blockNumberWithAnswer(phoneNumber, new Answer<Bundle>() {
            ...
            public Bundle answer(InvocationOnMock invocation) throws Throwable {
                Bundle bundle = new Bundle();
                bundle.putBoolean(BlockedNumberContract.RES_NUMBER_IS_BLOCKED, true);
                /// M:  BlockedNumberContract.shouldSystemBlockNumber change to use
                // RES_BLOCK_STATUS to check block status on Q, fit it.
    ...
    private void blockNumberWithAnswer(String phoneNumber, Answer answer) throws Exception {
        when(getBlockedNumberProvider().call(
                anyString(),
                /// M: IContentProvider.call add argument authority on Q, fit it.
                eq(BlockedNumberContract.AUTHORITY),
     ...
     * @throws Exception
     */
    /// M: we change emergency call behavior, normal call will be disconnected first, so this
    /// case is not suitable for our design.
    /*
    @LargeTest
    @Test
    @FlakyTest
    ...
    */
    ...
    /// M: test headset swap calls, hold call can change to active.
    @LargeTest
    @Test
    public void test1Active1HoldHeadsetSwapCalls() throws Exception {
        IdPair outgoing = startAndMakeActiveOutgoingCall("650-555-1212",
                mPhoneAccountA0.getAccountHandle(), mConnectionServiceFixtureA);
        IdPair incoming = startAndMakeActiveIncomingCall("650-555-2323",
                mPhoneAccountA0.getAccountHandle(), mConnectionServiceFixtureA);
        ...
        Class classCallManager = Class.forName("com.android.server.telecom.CallsManager");
        Method m = classCallManager.getDeclaredMethod("onMediaButton", int.class);
        m.setAccessible(true);
        // simulate headset swap calls, SHORT_PRESS = 1.
        m.invoke(mTelecomSystem.getCallsManager(), 1);

        // AOSP behavior changes to swap calls in 1A1H case on Q, behavior on P is disconnect call.
        verify(mConnectionServiceFixtureA.getTestDouble(), timeout(TEST_TIMEOUT))
                .hold(eq(incoming.mConnectionId), any());

        verify(mConnectionServiceFixtureA.getTestDouble(), timeout(TEST_TIMEOUT))
                .unhold(eq(outgoing.mConnectionId), any());

        mConnectionServiceFixtureA.sendSetOnHold(incoming.mConnectionId);
        mConnectionServiceFixtureA.sendSetActive(outgoing.mConnectionId);
        ...
        mInCallServiceFixtureX.mInCallAdapter.disconnectCall(outgoing.mCallId);
        /// M: MTK proprietary design, check hold call auto change to active after disconnect
        //  active call.
        mConnectionServiceFixtureA.sendSetDisconnected(outgoing.mConnectionId,
                DisconnectCause.LOCAL);
        mConnectionServiceFixtureA.sendRemoveCall(outgoing.mConnectionId);
        verify(mConnectionServiceFixtureA.getTestDouble(), timeout(TEST_TIMEOUT))
                .unhold(eq(incoming.mConnectionId), any());
        mInCallServiceFixtureX.mInCallAdapter.disconnectCall(incoming.mCallId);
    }
}
