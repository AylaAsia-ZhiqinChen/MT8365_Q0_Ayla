...
public class AnalyticsTests extends TelecomSystemTest {
    ...
    public void testAnalyticsDumping() throws Exception {
        ...
        mConnectionServiceFixtureA.
                sendSetDisconnected(testCall.mConnectionId, DisconnectCause.ERROR);
        /// M: sleep thread for 500ms to wait disconnect related events finish. Audio thread will
        /// add events in Analytics by different thread, which may trigger concurrent exception.
        Thread.sleep(500);
    ...
    public void testAnalyticsAudioRoutes() throws Exception {
        ...
        /// M: Fix Google AT issue: After call added and answered, AudioRouteStateMachine may not
        /// handle audio route state switch as expected in test case. Immediately assert after
        /// switch speaker may failed.
        /// Before assert, AudioModeStateMachine should handle 2 message:
        /// 1.NEW_RINGING_CALL message sent from CallAudioManager after entring ringing state.
        /// 2.NO_MORE_RINGING_CALLS message sent from CallAudioManager after call leaving ringing
        ///   state.
        ///AudioRouteStateMachine should handle 4
        /// message before switch to ActiveEarpiece state.
        /// 1.CallAudioManager.onCallAdded will trigger foreground change, and ask
        ///   AudioRouteStateMachine to force setSystemAudioState, UPDATE_SYSTEM_AUDIO_ROUTE
        ///   message is handled.
        /// 2.After AudioModeStateMachine enter RinigngFocusState, it will send SWITCH_FOCUS
        ///   message to AudioRouteStateMachine, then SWITCH_FOCUS(RINGING_FOCUS) is handled.
        /// 3.startAndMakeActiveIncomingCall will call answerCall immediately after call added,
        ///   then CallAudioManager.onIncomingCallAnswered will call mute(false), then
        ///   MUTE_OFF message is handled.
        /// 4.After call answered,  AudioModeStateMachine enter SimCallFocusState, it will
        ///   send SWITCH_FOCUS message to AudioRouteStateMachine, then SWITCH_FOCUS(ACTIVE_FOCUS)
        ///   is handled.
