...
public class BluetoothRouteManagerTest extends TelecomTestCase {
    ...
    public void testConnectHfpRetryWhileNotConnected() {
        ...
        waitForHandlerAction(sm.getHandler(), TEST_TIMEOUT);
        waitForHandlerAction(sm.getHandler(), TEST_TIMEOUT);
        /// M: Add for fix testConnectHfpRetryWhileNotConnected case fail. @{
        /// Use waitForHandleAction is not safely enough, state machine actions still may not finish
        /// after waitForHandlerAction 4 times.
        /// Solution:
        /// Sleep test thread to ensure all state machine actions execute done.
        try {
    ...
    public void testConnectHfpRetryWhileConnectedToAnotherDevice() {
        ...
        waitForHandlerAction(sm.getHandler(), TEST_TIMEOUT);
        waitForHandlerAction(sm.getHandler(), TEST_TIMEOUT);
        /// M: Add for fix testConnectHfpRetryWhileConnectedToAnotherDevice case fail. @{
        /// Use waitForHandleAction is not safely enough, state machine actions still may not
        /// finish after waitForHandlerAction 4 times.
        /// Solution:
        /// Sleep test thread to ensure all state machine actions execute done.
        try {
            Thread.sleep(TEST_TIMEOUT);
