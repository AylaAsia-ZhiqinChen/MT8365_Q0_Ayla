...
public class PhoneAccountRegistrarTest extends TelecomTestCase {
    ...
    /**
     * M: Test MTK phoneAcountHandle mota upgrade case. @{
     * After upgrade from MTK o0 version, some modules would try to query phoneAccount by the
     * MTK phoneAccountHandle. Telecom should compatible for this requirement.
     * getPhoneAccountUnchecked method in PhoneAccountRegistrar will convert the MTK
     * phoneAccountHandle to AOSP version. Add test case for it.
     */

    @MediumTest
    public void testGetPhoneAccountUnchecked() throws Exception {
        final ComponentName aospPstnComponentName = new ComponentName(
            "com.android.phone",
            "com.android.services.telephony.TelephonyConnectionService");
        final ComponentName mtkPstnComponentName = new ComponentName(
            "com.android.phone",
            "com.mediatek.services.telephony.MtkTelephonyConnectionService");

        mComponentContextFixture.addConnectionService(aospPstnComponentName,
                Mockito.mock(IConnectionService.class));

        PhoneAccountHandle telAccount = makeQuickAccountHandle(aospPstnComponentName,
                "tel_acct");
        registerAndEnableAccount(new PhoneAccount.Builder(telAccount, "tel_acct")
                .setCapabilities(PhoneAccount.CAPABILITY_CALL_PROVIDER)
                .build());
        assertNotNull(mRegistrar.getPhoneAccountUnchecked(telAccount));

        PhoneAccountHandle mtkTelAccount = makeQuickAccountHandle(mtkPstnComponentName,
                "tel_acct");
        assertEquals(mRegistrar.getPhoneAccountUnchecked(telAccount),
                mRegistrar.getPhoneAccountUnchecked(mtkTelAccount));
    }
    /** @} */
}
