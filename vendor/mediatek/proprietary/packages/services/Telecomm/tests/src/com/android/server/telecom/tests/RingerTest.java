...
/// M: Add for fix testCustomVibrationForRingtone case fail. @{
...
public class RingerTest extends TelecomTestCase {
    ...
    @SmallTest
    @Test
    /// M: Modify for fix testCustomVibrationForRingtone case fail. @{
    ...
    /// @}
        /// M: Add for fix testCustomVibrationForRingtone case fail. @{
        /// contentProvider.uncanonicalize() will be invoked in VibrationEffect.get(),
        /// but this method has not been mocked, cause test flow unexpected.
        IContentProvider contentProvider =
                mContext.getContentResolver().acquireProvider(FAKE_RINGTONE_URI);
        doAnswer(new Answer<Uri>() {
            @Override
            public Uri answer(InvocationOnMock invocation) throws Throwable {
                return (Uri) invocation.getArguments()[1];
            }
        }).when(contentProvider).uncanonicalize(anyString(), any(Uri.class));
        /// @}
