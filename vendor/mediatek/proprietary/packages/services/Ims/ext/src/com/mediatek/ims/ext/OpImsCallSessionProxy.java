...
public interface OpImsCallSessionProxy {
    ...
    boolean handleDeviceSwitchResult(
            String callId, IMtkImsCallSession imsCallSession, AsyncResult result);
    boolean isValidVtDialString(String number);
    String normalizeVtDialString(String number);
    boolean isDeviceSwitching();
    ImsReasonInfo getImsReasonInfo(SipMessage sipMsg);
    boolean handleCallStartFailed(
            IMtkImsCallSession imsCallSession, Object imsRILAdapter, boolean hasHoldingCall);
    boolean handleHangup();

    void sendCallEventWithRat(Bundle extras);
}
