 ...
 */
public class NlpServiceWatcher implements ServiceConnection {

    private static final String TAG = "NlpServiceWatcher";
    private static final boolean D = true;

    public static final String EXTRA_SERVICE_VERSION = "serviceVersion";
    public static final String EXTRA_SERVICE_IS_MULTIUSER = "serviceIsMultiuser";
    /// M: mtk add start
    private final ArrayList<String> mInitialPackageNames;
    private String[] mPreferPkgs = null;
    ...
    public static ArrayList<HashSet<Signature>> getSignatureSets(Context context,
        ...
        ArrayList<HashSet<Signature>> signatureSets = new ArrayList<>(initialPackageNames.size());
        for (int i = 0, size = initialPackageNames.size(); i < size; i++) {
            String packageName = initialPackageNames.get(i);
            try {
    ...
    private final String mServicePackageName = null;
    ...
    public NlpServiceWatcher(Context context, String logTag, String action,
            String gmsPackageName,
        ...
        if (gmsPackageName != null) initialPackageNames.add(gmsPackageName);
        if (vendorPackageName != null) initialPackageNames.add(vendorPackageName);
        mInitialPackageNames = initialPackageNames;

        Log.d(mTag, "NlpServiceWatcher, gms package=" + gmsPackageName);
        Log.d(mTag, "NlpServiceWatcher, vendor package=" + vendorPackageName);
        Log.d(mTag, "NlpServiceWatcher, prefer package=" + preferPackageName);
        mSignatureSets = getSignatureSets(context, initialPackageNames);

        mPreferPkgs = new String[1];
        mPreferPkgs[0] = preferPackageName;
        /// M: mtk add end
    ...
    private boolean isPackageMatch(String packageName) {
        for (int i = 0, size = mInitialPackageNames.size(); i < size; i++) {
            String pkg = mInitialPackageNames.get(i);
    ...
    private boolean isServiceMissing() {
        List<ResolveInfo> rInfos =  mContext.getPackageManager().queryIntentServicesAsUser(
                new Intent(mAction),
                PackageManager.MATCH_DIRECT_BOOT_AWARE
                        | PackageManager.MATCH_DIRECT_BOOT_UNAWARE,
                UserHandle.USER_SYSTEM);
        if (rInfos.size() == 1) { // only MtkNlp package matches.
            return true;
        }
        return false;
    }
    ...
    private void bindBestPackage(boolean forceRebind) {
        ...
        for (ResolveInfo rInfo : rInfos) {
            ...
            try {
                ...
                /// M: mtk add start
                if (!isPackageMatch(packageName)) {
                    Log.w(mTag, packageName + " resolves service " + mAction
                            + ", but not in provider package list, ignoring");
                    continue;
                }
                /// M: mtk add end
            } catch (NameNotFoundException e) {
