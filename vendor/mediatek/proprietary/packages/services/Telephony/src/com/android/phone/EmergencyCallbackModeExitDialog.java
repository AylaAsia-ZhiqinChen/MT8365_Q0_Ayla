...
public class EmergencyCallbackModeExitDialog extends Activity implements OnCancelListener {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        if (mPhone == null || !mPhone.isInEcm()) {
            ...
            /// M: ALPS03624340, Must setResult before invoke finish().@{
            //  Otherwise it will cause NullPointerException when settings invoke onActivityResult.
    ...
    private void showEmergencyCallbackModeExitDialog() {
        if (!this.isResumed()) {
            ...
            /// M: ALPS03624340, Need finish activity here to fix click invalid issue. @{
            //  If current activity is onPause state it will not show dialog, but after
            //  activity change to onResume state later, there will still exist an transparent
            //  activity, cause user click event invalid. So we finish it here.
    ...
    private CharSequence getDialogText(long millisUntilFinished) {
        ...
        ///M: Plug-in call to change dialog text. @{
        String dialogText = ExtensionManager.getEmergencyDialerExt()
                .getDialogText(mPhone, mDialogType, millisUntilFinished);
        if (dialogText != null) {
            return dialogText;
        }
        ///  @}
