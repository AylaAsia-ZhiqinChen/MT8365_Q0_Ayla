...
public class ADNList extends ListActivity {
    ...
    /// Revise cusor index for IccProvider. @{
    ...
    protected void onCreate(Bundle icicle) {
        ...
        /// Get sub id for parse uri. @{
        mSubId = getIntent().getIntExtra(
                PhoneConstants.SUBSCRIPTION_KEY, SubscriptionInfoHelper.NO_SUB_ID);
        /// @}
    }
    ...
    protected void onStop() {
        super.onStop();
        /// ALPS03828614 Do not deactive cursor for multiple windows. @{
        if (mCursor != null && !this.isInMultiWindowMode()) {
    ...
    protected Uri resolveIntent() {
        ...
        if (intent.getData() == null) {
            /// Set sub id for parse uri. @{
            intent.setData(Uri.parse(ICC_ADN_SUBID_URI + mSubId));
            /// @}
        }
    ...
    private void displayProgress(boolean loading) {
        ...
        /// Add airplane mode tips. @{
        mEmptyText.setText(loading ? R.string.simContacts_emptyLoading :
            (TelephonyUtils.isAirplaneModeOn(this) ? R.string.simContacts_airplaneMode :
    ...
    public static final String ICC_ADN_SUBID_URI = ICC_ADN_URI + "/subId/";

    protected int mSubId  = SubscriptionInfoHelper.NO_SUB_ID;
