...
public class PhoneAccountSettingsFragment extends PreferenceFragment
    ...
    private final SubscriptionManager.OnSubscriptionsChangedListener
        ...
        public void onSubscriptionsChanged() {
            ///M: Do not updateAccounts when activity is null or finishing. @{
    ...
    public void onPause() {
        SubscriptionManager.from(getActivity()).removeOnSubscriptionsChangedListener(
                mOnSubscriptionsChangeListener);
        super.onPause();
        /// M: AccountSelectionPreference pause @{
        if (mDefaultOutgoingAccount != null) {
            mDefaultOutgoingAccount.doPause();
        }
        /// @}
    }
    ...
    public boolean onAccountSelected(AccountSelectionPreference pref, PhoneAccountHandle account) {
        if (pref == mDefaultOutgoingAccount) {
            ...
            ///M: Debug info @{
            Log.d(LOG_TAG, "onAccountSelected:" + account);
            /// @}
            return true;
        }
        return false;
    ...
    private void initAccountList(List<PhoneAccountHandle> enabledAccounts) {
        ...
        for (PhoneAccount account : accounts) {
            ...
            if (icon != null) {
                ///M: Debug info @{
                Drawable d = icon.loadDrawable(getActivity());
                if (d != null) {
                    Log.d("PhoneAccountSettingsFragment", "icon width:" + d.getIntrinsicWidth() +
                            "icon height:" + d.getIntrinsicHeight() +
                            "account label:" + accountLabel);
                }
                accountPreference.setIcon(d);
                /// @}
            }
    ...
    private void updateAccounts() {
        if (mAccountList != null) {
            ...
            ///M: Debug info @{
            Log.d(LOG_TAG, "allNonSimAccounts size: " +
                    (allNonSimAccounts != null ? allNonSimAccounts.size() : null));
            /// @}
            ...
            if (shouldShowConnectionServiceList(allNonSimAccounts)) {
                List<PhoneAccountHandle> enabledAccounts =
                        getCallingAccounts(true /* includeSims */, false /* includeDisabled */);
                ///M: Debug info @{
                Log.d(LOG_TAG, "enabledAccounts size: " +
                        (enabledAccounts != null ? enabledAccounts.size() : null));
                /// @}
