...
public class PhoneUtils {
    ...
    static void displayMMIComplete(final Phone phone, Context context, final MmiCode mmiCode,
        ...
        if ((app.getPUKEntryActivity() != null) && (state == MmiCode.State.COMPLETE)) {
            ...
            if (state != MmiCode.State.PENDING) {
                createUssdDialog(app, context, text,
                        /// M: MMI dialog does not appear in ECC call view. @{
                        /// When dial MMI from EmergencyDial, the dialog will be coverd by
                        /// keyguard, change dialog type to ensure it can be displayed normally.
                        WindowManager.LayoutParams.TYPE_KEYGUARD_DIALOG);
                        /// @}
            } else {
    ...
    public static void createUssdDialog(PhoneGlobals app, Context context, CharSequence text,
        ...
        /// M: Destroy old ussd dialog befor show it. For fix issue that "OK" of
        /// dialog positive button has no translate after switch language.
        if (sUssdDialog != null) {
            sUssdDialog.dismiss();
            sUssdDialog = null;
        }
    ...
    static CallerInfoToken startGetCallerInfo(Context context, Connection c,
        ...
        if (userDataObject instanceof Uri) {
            ...
            cit = new CallerInfoToken();
            /// M: CC: MtkCallerInfo add-on
            cit.currentInfo = new MtkCallerInfo();
