    ...
    private void sendIncomingCallIntent(Connection connection) {
        ...
        if (connection.getNumberPresentation() == TelecomManager.PRESENTATION_ALLOWED &&
            ...
            if (connection instanceof MtkImsPhoneConnection) {
                ...
                boolean isIncomingMpty =
                      ((MtkImsPhoneConnection)connection).isIncomingCallMultiparty();
                Log.d(this, "isIncomingMpty: " + isIncomingMpty);
                extras.putBoolean(MtkTelecomManager.EXTRA_INCOMING_VOLTE_CONFERENCE, isIncomingMpty);
                /// @}
            }
        }
        ...
        if (connection instanceof MtkGsmCdmaConnection) {
            boolean isIncomingGwsd = ((MtkGsmCdmaConnection) connection).isIncomingCallGwsd();
            Log.d(this, "isIncomingGwsd: " + isIncomingGwsd);
            extras.putBoolean(MtkTelecomManager.EXTRA_INCOMING_GWSD, isIncomingGwsd);
        }
        /// @}
        /// M: IMS CC: GWSD @{
        if (connection instanceof MtkImsPhoneConnection) {
            boolean isIncomingGwsd = ((MtkImsPhoneConnection) connection).isIncomingCallGwsd();
            Log.d(this, "isIncomingGwsd: " + isIncomingGwsd);
            extras.putBoolean(MtkTelecomManager.EXTRA_INCOMING_GWSD, isIncomingGwsd);
        }
        /// @}
        ...
        if (connection.getPhoneType() == PhoneConstants.PHONE_TYPE_IMS) {
            ...
            ///M: For TMO RTT EMC Guard Timer
            if (((MtkImsPhoneConnection) connection).isIncomingCallDuringRttEmcGuard()) {
                extras.putBoolean(MtkTelecomManager.EXTRA_IS_RTT_EMERGENCY_CALLBACK, true);
                Log.d(this, "sendIncomingCallIntent: put EXTRA_IS_RTT_EMERGENCY_CALLBACK");
            }
        }
        PhoneAccountHandle handle = findCorrectPhoneAccountHandle();
        if (handle == null) {
