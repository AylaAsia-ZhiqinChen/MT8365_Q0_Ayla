...
public class CallNotifier extends Handler {
    ...
    private void onDisplayInfo(AsyncResult r) {
        ...
        if (displayInfoRec != null) {
            ...
            // M: CC: Remove old timer to avoid new dialog canceled by old timer
            removeMessages(INTERNAL_SHOW_MESSAGE_NOTIFICATION_DONE);
    ...
    private void onSuppServiceFailed(AsyncResult r) {
        ...
        if (r.result == Phone.SuppService.CONFERENCE) {
            ...
            if (DBG) log("onSuppServiceFailed: displaying hangup failure message");
            mergeFailedString = mApplication.getResources().getString(
                    R.string.incall_error_supp_service_hangup);
        } else if (r.result == Phone.SuppService.UNKNOWN) {
            if (DBG) log("onSuppServiceFailed: displaying unknown failure message");
            mergeFailedString = mApplication.getResources().getString(
    ...
    public void updatePhoneStateListeners(boolean isRefresh, int updateType, int subIdToUpdate) {
        ...
        for (int subIdCounter = (subIdList.size() - 1); subIdCounter >= 0; subIdCounter--) {
            ...
            if (subInfos == null || !containsSubId(subInfos, subId)) {
                ...
                /// ALPS04183402 Remove mCFIStatus if mPhoneStateListeners is removed by subId. @{
                mCFIStatus.remove(subId);
                mMWIStatus.remove(subId);
                /// @}
            } else {
        ...
        for (int i = 0; i < subInfos.size(); i++) {
            ...
            if (!mPhoneStateListeners.containsKey(subId)) {
                ...
                /// Debug info. @{
                log("updatePhoneStateListeners: add listener for subId=" + subId);
                /// @}
            }
        }
    }
    ...
    private class CallNotifierPhoneStateListener extends PhoneStateListener {
        ...
        public void onMessageWaitingIndicatorChanged(boolean visible) {
            /// Debug info. @{
            log("onMessageWaitingIndicatorChanged(): " + this.mSubId + " " + visible);
            /// @}
