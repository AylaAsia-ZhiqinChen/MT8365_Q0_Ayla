...
public class CallBarringEditPreference extends EditPinPreference {
    ...
    void init(TimeConsumingPreferenceListener listener, boolean skipReading, Phone phone) {
        ...
        if (!skipReading) {
            ...
            mPhone.getCallBarring(mFacility, "", mHandler.obtainMessage(
                    MyHandler.MESSAGE_GET_CALL_BARRING), mServiceClass);
            /// @}
    ...
    protected void onDialogClosed(boolean positiveResult) {
        ...
        if (mButtonClicked != DialogInterface.BUTTON_NEGATIVE) {
            ...
            //}
            CallSettingUtils.sensitiveLog(LOG_TAG, "onDialogClosed: password=", password);
            /// @}
            // Send set call barring message to RIL layer.
            /// M: Handle call forward option by VT type or default. @{
            mPhone.setCallBarring(mFacility, !mIsActivated, password,
                    mHandler.obtainMessage(MyHandler.MESSAGE_SET_CALL_BARRING), mServiceClass);
            /// @}
            if (mTcpListener != null) {
    ...
    private void setShowPassword() {
        ...
        ///M: Show Call Barring password when ims is avaiable for CSFB operators. @{
        PersistableBundle carrierConfig =
                PhoneGlobals.getInstance().getCarrierConfigForSubId(mPhone.getSubId());
        if (!mShowPassword && carrierConfig.getBoolean(
                MtkCarrierConfigManager.MTK_KEY_SHOW_CALL_BARRING_PASSWORD_BOOL)) {
    ...
    /// M: Remove static to access mServiceClass.
        ...
        private void handleGetCallBarringResponse(Message msg) {
            ...
            if (ar.exception != null) {
                ...
                if (ints.length == 0) {
                    ...
                    /// M: Check service class @{
                    int result = ints[0];
                    Log.d(LOG_TAG, "handleGetCallBarringResponse: result = " + result +
                            " serviceClass = " + mServiceClass);
                    result = result & mServiceClass;
                    pref.handleCallBarringResult(result != 0);
                    /// @}
                }
            }
    ...
    private void handleCommandException(AsyncResult ar) {
        if (ar.exception instanceof CommandException) {
            CommandException ce = (CommandException) ar.exception;
            Log.d(LOG_TAG, "handleCommandException : Error =" + ce.getCommandError());

            //  Should disable item if request is not supported
            if (ce.getCommandError()
                    == CommandException.Error.REQUEST_NOT_SUPPORTED) {
                setEnabled(false);
                setSummary(mSummaryOff);
            } else {
                if (mTcpListener != null) {
                    mTcpListener.onException(this, ce);
                }
            }
        } else {
            ...
            if (mTcpListener != null) {
                mTcpListener.onError(this, TimeConsumingPreferenceActivity.EXCEPTION_ERROR);
            }
        }
    }
}
