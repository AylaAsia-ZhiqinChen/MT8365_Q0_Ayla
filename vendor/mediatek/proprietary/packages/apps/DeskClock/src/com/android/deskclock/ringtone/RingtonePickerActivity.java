...
public class RingtonePickerActivity extends BaseActivity
    ...
    /// M: [ALPS03490964]  filter in only FL types to open out of drm files @{
            ...
            if (mAlarmId != -1) {
                ...
                new AsyncTask<Void, Void, Alarm>() {
                    ...
                    protected void onPostExecute(Alarm alarm) {
                        /// M: [ALPS03484966] Adding null check while accessing alarm
                        /// same to doInBackground @{
                        if (alarm != null) {
    ...
    private RingtoneHolder getRingtoneHolder(Uri uri) {
        /// M: [ALPS03472504] Adding null check before starting iteration @{
        if (mRingtoneAdapter != null && mRingtoneAdapter.getItemCount() > 0) {
            ...
            switch (id) {
                case AddCustomRingtoneViewHolder.CLICK_ADD_NEW:
                    ...
                    startActivityForResult(new Intent(Intent.ACTION_OPEN_DOCUMENT)
                            ...
                            .putExtra(EXTRA_DRM_LEVEL, LEVEL_FL)
                            .setType("audio/*"), 0);
                    ///@}
                    break;
