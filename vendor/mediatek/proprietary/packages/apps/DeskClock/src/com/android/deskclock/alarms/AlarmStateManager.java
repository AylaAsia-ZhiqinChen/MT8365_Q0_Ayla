...
public final class AlarmStateManager extends BroadcastReceiver {
    ...
    /// M: [ALPS03715614] Defining complete broadcast name @{
    ...
    /// M: [ALPS03269543] [Power off Alarm] Set the type of power off alarm @{
    ...
    private static void updateParentAlarm(Context context, AlarmInstance instance) {
        ...
        if (!alarm.daysOfWeek.isRepeating()) {
            ...
            /// M: [ALPS03269543] [Power off Alarm] It is no need to call
            /// updateNextAlarm, the caller would do it later, so set the flag
            /// updateNextAlarm to false to avoid endless loop. @{
    ...
    public static void fixAlarmInstances(Context context) {
       LogUtils.i("Fixing alarm instances");
        ...
        /// M: [ALPS03432560] record for duplicated instance, and delete them
    ...
    private static class AlarmManagerStateChangeScheduler implements StateChangeScheduler {
        ...
        public void scheduleInstanceStateChange(Context context, Calendar time,
            ...
            PendingIntent pendingIntent = PendingIntent.getService(context,
                    instance.hashCode(), stateChangeIntent, PendingIntent.FLAG_UPDATE_CURRENT);
            ...
            /// M: Cancel the alarm in alarmManager for power off alarm @{
        ...
        public void cancelScheduledInstanceStateChange(Context context, AlarmInstance instance) {
            ...
            PendingIntent pendingIntent = PendingIntent.getService(context,
                    instance.hashCode(), createStateChangeIntent(context,
                    ALARM_MANAGER_TAG, instance, null), PendingIntent.FLAG_NO_CREATE);

            if (pendingIntent != null) {
                AlarmManager am = (AlarmManager) context.getSystemService(ALARM_SERVICE);
                am.cancel(pendingIntent);
                /// M: [ALPS03269543] [Power off Alarm] Cancel the alarm list in alarmManager
                /// for power off alarm @{
                am.cancelPoweroffAlarm(context.getPackageName());
                ///@}
    ...
    /** M: [ALPS03432560] Update the alarmInstances who can be set a nearly time @{ */
    ...
    /**
     * M: In N, the launch for second alarm doesn't happen as AlarmService
     *    is not able to start. Resetting the launch time for alarmService
     *    ensures that the second alarm is triggered successfully.
     */
    public static void resetServiceForAlarm(Context context, AlarmInstance nextAlarm) {

        LogUtils.v("Resetting service launch for 2nd alarm");
        ...
        Intent stateChangeIntentAlarm = createStateChangeIntent(context, ALARM_MANAGER_TAG,
                nextAlarm, AlarmInstance.FIRED_STATE);
        // Treat alarm state change as high priority, use foreground broadcasts
        stateChangeIntentAlarm.addFlags(Intent.FLAG_RECEIVER_FOREGROUND);
        PendingIntent pendingIntentAlarm = PendingIntent.getService(context,
                nextAlarm.hashCode(), stateChangeIntentAlarm, PendingIntent.FLAG_UPDATE_CURRENT);

        AlarmManager am = (AlarmManager) context.getSystemService(Context.ALARM_SERVICE);
        am.setExact(AlarmManager.RTC_WAKEUP, timeInMillis, pendingIntentAlarm);
