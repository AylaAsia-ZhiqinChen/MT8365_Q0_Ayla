...
public class CallbackHandler extends Handler implements EmergencyListener, SignalCallback {
    ...
    public void handleMessage(Message msg) {
        switch (msg.what) {
            ...
            case MSG_SUBS_CHANGED:
                if (CHATTY) {
                    Log.d(TAG, "handleMessage(MSG_SUBS_CHANGED), mSignalCallbacks = "
                        + mSignalCallbacks);
                }
            ...
            case MSG_ADD_REMOVE_SIGNAL:
                if (CHATTY) {
                    Log.d(TAG, "handleMessage(MSG_ADD_REMOVE_SIGNAL), arg1 = "
                        + msg.arg1 + ", obj = " + msg.obj);
                }
                if (msg.arg1 != 0) {
    ...
    public void setMobileDataIndicators(final IconState statusIcon, final IconState qsIcon,
            ...
            boolean isDefaultData, int customizedState) {
        post(new Runnable() {
            @Override
            public void run() {
                for (SignalCallback signalCluster : mSignalCallbacks) {
