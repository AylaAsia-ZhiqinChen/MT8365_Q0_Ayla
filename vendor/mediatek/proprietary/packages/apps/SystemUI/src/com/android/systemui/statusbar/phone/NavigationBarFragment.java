...
///yuntian longyao add
///Description:为工厂测试中按键测试修改拦截Recent button 
...
public class NavigationBarFragment extends LifecycleFragment implements Callbacks,
    ...
    private void onRecentsClick(View v) {
        ...
        if (tasksInfo.size() > 0) {
            String clsName = tasksInfo.get(0).topActivity.getClassName();
            String pkgName = tasksInfo.get(0).topActivity.getPackageName();
            if (null != pkgName && null != clsName) {
                if (("com.android.factorytest").equals(pkgName)
        ...
        if (flag) {
            ...
            sendEvent(KeyEvent.ACTION_DOWN, 0, mDownTime, mDownTime, KeyEvent.KEYCODE_MENU);
            sendEvent(KeyEvent.ACTION_UP, 0, mDownTime, mDownTime, KeyEvent.KEYCODE_MENU);
            return; 
        }
        /// &&}}
        mStatusBar.awakenDreams();
