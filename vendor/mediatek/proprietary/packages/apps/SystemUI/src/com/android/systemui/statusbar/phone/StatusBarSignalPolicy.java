...
public class StatusBarSignalPolicy implements NetworkControllerImpl.SignalCallback,
        ...
        ISystemUIStatusBarExt.StatusBarCallback {
    ...
    public StatusBarSignalPolicy(Context context, StatusBarIconController iconController) {
        ...
        ///M: Add for CTA. If CTA support, need to show by default. @{
        ...
        mStatusBarExt.addCallback(this);
        /// @ }
    ...
    public void setMobileDataIndicators(IconState statusIcon, IconState qsIcon, int statusType,
        ...
        state.mCustomizedState = customizedState;
        /// @ }
    ...
    public void setSubs(List<SubscriptionInfo> subs) {
        if (DEBUG) {
            Log.d(TAG, "setSubs: size = " + subs.size()
                + ", subs = " + subs);
        }
        ...
        for (int i = 0; i < n; i++) {
            mMobileStates.add(new MobileIconState(
                subs.get(i).getSubscriptionId(), subs.get(i).getSimSlotIndex()));
        }
    }
    ...
    public void setNoSims(boolean show, boolean simDetected) {
        // Noop yay!
        /// add for show op nosim view. @{
        mNoSimsVisible = show && !mBlockMobile && !mIsAirplaneMode;
        mStatusBarExt.setCustomizedNoSimView(mNoSimsVisible);
        /// @}
    }
    ...
    public void setEthernetIndicators(IconState state) {
        ...
        //yuntian longyao add Ethernet settings以太网图标显示异常
        //if (resId > 0) {
        if (visible && resId > 0) {
    ...
    public static class MobileIconState extends SignalIconState {
        ...
        /// @}
        /// M: Add for plugin features. @ {
        public boolean mDataActivityIn, mDataActivityOut;
        public boolean mDefaultData;
        public int mCustomizedState;
        /// @ }
        /// M: Used for sorting @{
        public int phoneId;
        /// @}
        ...
        private MobileIconState(int subId, int phoneId) {
            this(subId);
        ...
        public boolean equals(Object o) {
            ...
            return subId == that.subId &&
                    ...
                    networkIcon == that.networkIcon &&
                    mDataActivityIn == that.mDataActivityIn &&
                    mDataActivityOut == that.mDataActivityOut &&
                    mDefaultData == that.mDefaultData &&
                    mCustomizedState == that.mCustomizedState &&
                    volteIcon == that.volteIcon &&
                    phoneId == phoneId;
        }
        ...
        public int hashCode() {
            ...
            return Objects
                    .hash(super.hashCode(), subId, strengthId, typeId, roaming, needsLeadingPadding,
                            typeContentDescription, networkIcon, mDataActivityIn, mDataActivityOut,
                            mDefaultData, mCustomizedState, volteIcon, phoneId);
        }
        ...
        public void copyTo(MobileIconState other) {
            ...
            other.networkIcon = networkIcon;
            other.mDataActivityIn = mDataActivityIn;
            other.mDataActivityOut = mDataActivityOut;
            other.mDefaultData = mDefaultData;
            other.mCustomizedState = mCustomizedState;
            other.volteIcon = volteIcon;
        ...
        @Override public String toString() {
            return "MobileIconState(subId=" + subId + ", strengthId=" + strengthId + ", roaming="
                    + roaming + ", typeId=" + typeId + ", networkIcon = " + networkIcon
                    + ", mDataActivityIn: " + mDataActivityIn
                    + ", mDataActivityOut: " + mDataActivityOut
                    + ", mDefaultData: " + mDefaultData
                    + ", mCustomizedState: " + mCustomizedState
                    + ", volteIcon = " + volteIcon + ", visible=" + visible
                    + ", phoneId = " + phoneId + ")";
        }
    }

    public void setSystemIcon(String slot, int resourceId,
            CharSequence contentDescription, boolean isVisible) {
        if (slot.equals("hdvoice")) {
            resourceId = R.drawable.stat_sys_hd_voice_call;
        } else if (slot.equals("nosim")) {
            int count = TelephonyManager.getDefault().getSimCount();
            if (count == 1) {
                resourceId = R.drawable.stat_sys_signal_null_one_sim;
            } else if (count == 2) {
                resourceId = R.drawable.stat_sys_signal_null_two_sims;
            }
        }
        if (DEBUG) {
            Log.d(TAG, "setSystemIcon, slot = " + slot
                    + ", resourceId = " + resourceId
                    + ", contentDescription = " + contentDescription
                    + ", isVisible = " + isVisible);
        }
        mIconController.setIcon(slot, resourceId, contentDescription);
        mIconController.setIconVisibility(slot, isVisible);
    }
}
