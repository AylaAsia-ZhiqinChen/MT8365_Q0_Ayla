...
public class NetworkControllerImpl extends BroadcastReceiver
    ...
    // M: The broadcast name is come from MtkImsConstants.java#ACTION_MTK_IMS_SERVICE_UP.
    // Because the definition of MtkImsConstants dose not exist on some branch, so we write
    // hard code at here to keep platform independent, it should be consistent with
    // MtkImsConstants.java @{
    ...
    private void addCustomizedAction(IntentFilter filter) {
        ...
        // add for ims listener
        filter.addAction(ACTION_MTK_IMS_SERVICE_UP);
    }
    /// @ }
    ...
    public void removeCallback(SignalCallback cb) {
        if (CHATTY) {
            Log.d(TAG, "removeCallback, cb = " + cb);
        }
    ...
    public void onReceive(Context context, Intent intent) {
        ...
        switch (action) {
            ...
            case CarrierConfigManager.ACTION_CARRIER_CONFIG_CHANGED:
                ...
                mReceiverHandler.post(this::handleConfigurationChanged);
                break;
            case NetworkTypeUtils.LWA_STATE_CHANGE_ACTION:
                /// M: Add for 4G+W
                handleLwaAction(intent);
                break;
            case ACTION_MTK_IMS_SERVICE_UP:
                handleIMSServiceUp();
                break;
            default:
    ...
    void doUpdateMobileControllers() {
        ...
        if (CHATTY) {
            Log.d(TAG, "doUpdateMobileControllers, subscriptions = " + subscriptions);
        }
    ...
    public void setCurrentSubscriptionsLocked(List<SubscriptionInfo> subscriptions) {
        ...
        if (CHATTY) {
            Log.d(TAG, "setCurrentSubscriptions, controllers before update = " + mMobileSignalControllers);
        }
        ...
        if (CHATTY) {
            Log.d(TAG, "setCurrentSubscriptions, removed controllers = " + cachedControllers);
            Log.d(TAG, "setCurrentSubscriptions, controllers after update = " + mMobileSignalControllers);
        }
        if (mListening) {
    ...
    boolean hasCorrectMobileControllers(List<SubscriptionInfo> allSubscriptions) {
        ...
        /// @}
        if (mSignalCallbackCount != mCallbackHandler.getSignalCallbackCount()) {
            Log.d(TAG,"signal callbacks count not equals, reset subInfo, "
                + "old = " + mSignalCallbackCount + ", new = "
                + mCallbackHandler.getSignalCallbackCount());
            mSignalCallbackCount = mCallbackHandler.getSignalCallbackCount();
            return false;
        }
        return true;
    }
    ...
    private void updateAirplaneMode(boolean force) {
        ...
        if (CHATTY) {
            Log.d(TAG, "updateAirplaneMode, airplaneMode =" + airplaneMode);
        }
        /**
    ...
    /// M: Add for ims service up
    private void handleIMSServiceUp() {
        Log.d(TAG, "handleIMSServiceUp");
        for (int i = 0; i < mMobileSignalControllers.size(); i++) {
            MobileSignalController controller = mMobileSignalControllers.valueAt(i);
            controller.registerImsListener();
        }
    }
    /// @}
}
