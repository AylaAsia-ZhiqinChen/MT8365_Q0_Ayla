...
public class ImageWallpaperRenderer implements GLWallpaperRenderer,
   ...
   // M:Low RAM wallpaper drawing handling @{
    ...
    public ImageWallpaperRenderer(Context context, SurfaceProxy proxy) {
        ...
        mWindowManager.getDefaultDisplay().getDisplayInfo(displayInfo);
        ...
        if (mWallpaperManager != null && mBitmap == null) {
          ...
          if (MTK_LOW_RAM_SUPPORT) {
              ...
              if (Build.VERSION.SDK_INT >= 19) {
                // contain virtual keyboard
                 display.getRealSize(outPoint);
              } else {
                // no virtual keyboard
                 display.getSize(outPoint);
             }
              mBitmap = scaleBitmap(mWallpaperManager.getBitmap(), outPoint.x, outPoint.y);
         } else {
         //M:@}   
