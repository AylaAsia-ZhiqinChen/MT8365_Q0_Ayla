...
public class OmacpProvider extends ContentProvider {
    ...
    private final BroadcastReceiver mBroadcastReceiver = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            MTKlog.d(TAG, "[onReceive] action=" + intent.getAction());

            if (mOmacpDatabaseHelper != null) {
                mOmacpDatabaseHelper.close();
            }

            final Context deviceContext = context.createDeviceProtectedStorageContext();
            if (!deviceContext.moveDatabaseFrom(context, OmacpDatabaseHelper.DATABASE_NAME)) {
                Log.wtf(TAG, "[onReceive] Failed to move database");
            }
            MTKlog.d(TAG, "[onReceive] Move database successfully");

            if (mOmacpDatabaseHelper != null) {
                mOmacpDatabaseHelper = new OmacpDatabaseHelper(deviceContext);
            }
    ...
    public boolean onCreate() {
        ...
        if (UserManager.get(context).isUserUnlocked()) {
            ...
            MTKlog.d(TAG, "[onCreate] Move database successfully");
        } else {
            MTKlog.d(TAG, "[onCreate] User locked, register receiver for BOOT_COMPLETED");
