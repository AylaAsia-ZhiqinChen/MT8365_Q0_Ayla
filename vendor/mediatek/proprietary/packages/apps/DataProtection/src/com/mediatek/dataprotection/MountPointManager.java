...
public final class MountPointManager {
    ...
    public void init(Context context) {
        ...
        Log.s(TAG, "init,defaultPath = " + defaultPath);
        ...
        if (storageVolumeList != null) {
            for (StorageVolume volume : storageVolumeList) {
                ...
                Log.s(TAG, "init,addpath = " + mountPoint.mPath);
    ...
    public List<FileInfo> getMountPointFileInfo() {
        ...
        for (MountPoint mp : mMountPathList) {
            if (mp.mIsMounted) {
                fileInfos.add(new FileInfo(new File(mp.mPath)));
            }
        }
        return fileInfos;
    }
    ...
    protected boolean isRootPathMount(String path) {
        Log.s(TAG, "isRootPathMount,  path = " +  path);
        boolean ret = false;
        if (path == null) {
            return ret;
        }
    ...
    public String getRealMountPointPath(String path) {
        ...
        Log.s(TAG, "getRealMountPointPath = \"\" ");
        return "";
    }
    ...
    public boolean isFat32Disk(String path) {
        Log.s(TAG, "isFat32Disk ,path =" + path);
        for (MountPoint mountPoint : mMountPathList) {
            if ((path + SEPARATOR).startsWith(mountPoint.mPath + SEPARATOR)) {
               Log.s(TAG, "isFat32Disk = " + mountPoint.mPath);
    ...
    public boolean isMountPoint(String path) {
        boolean ret = false;
        Log.s(TAG, "isMountPoint ,path =" + path);
        if (path == null) {
            return ret;
        }
        for (MountPoint mountPoint : mMountPathList) {
    ...
    public boolean isExternalMountPath(String path) {
        Log.s(TAG, "isExternalMountPath ,path =" + path);
        if (path == null) {
            return false;
        }
        for (MountPoint mountPoint : mMountPathList) {
            Log.s(TAG, "mountPoint ,in path =" + mountPoint.mPath);
            if (mountPoint.mIsExternal && mountPoint.mPath.equals(path)) {
                return true;
            }
        }
    ...
    public String getMountPointPath(String filePath) {
        String path = null;
        if (filePath == null) {
            return path;
        }
        for (MountPoint mountPoint : mMountPathList) {
            if (mountPoint.mIsMounted && filePath.startsWith(mountPoint.mPath)) {
                path = mountPoint.mPath;
            }
        }
        return path;
    }

    public String getMountPointDescription(String filePath) {
        ...
        for (MountPoint mountPoint : mMountPathList) {
            if (mountPoint.mIsMounted && filePath.startsWith(mountPoint.mPath)) {
                description = mountPoint.mDescription;
            }
        }
        return description;
    }
    ...
    public boolean isFilePathValid(String path) {
        ...
        Log.s(TAG, "isFilePathValid " + path + " isValid " + res);
        return res;
    }

    public boolean isPrimaryVolume(String path) {
        Log.s(TAG, "isPrimaryVolume ,path =" + path);
    ...
    public void updateMountPointSpaceInfo() {
        Log.s(TAG, "updateMountPointSpaceInfo...");
        for (MountPoint mp : mMountPathList) {
            if (mp.mIsMounted) {
                // fileInfos.add(new FileInfo(mp.mPath));
                File file = new File(mp.mPath);
                mp.mFreeSpace = file.getFreeSpace();
    ...
    public long getMountPointFreeSpace(String path) {
        Log.s(TAG, "getMountPointFreeSpace " + path);
        long freeSpace = 0;
        for (MountPoint mp : mMountPathList) {
    ...
    public long getMountPointTotalSpace(String path) {
        Log.s(TAG, "getMountPointTotalSpace " + path);
        long totalSpace = 0;
        for (MountPoint mp : mMountPathList) {
            if (mp.mPath.equalsIgnoreCase(path)) {
    ...
    public boolean isRootSafePath(String path) {
        if (path == null) {
            return false;
        }
        return path.equalsIgnoreCase(getDefaultPath() + File.separator + ".lockedfiles");
    }
    ...
    public String getDestinationPath(File file) {
        ...
        return getDefaultPath() + subFolder + file.getName();
    }

    public String getDecryptDestinationPath(File oFile) {
        String rootPath = getDefaultPath();
        File file = new File(rootPath + File.separator + ".unlockedfiles");
        if (!file.exists()) {
            file.mkdirs();
        }
        if (file.exists()) {
            //String path = oFile.getName();
            return file.getAbsolutePath() + File.separator + oFile.getName();
        }
        return null;
    }
}
