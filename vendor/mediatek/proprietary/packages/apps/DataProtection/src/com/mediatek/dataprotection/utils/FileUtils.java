...
public final class FileUtils {
    ...
    public static int checkFileName(String fileName) {
         ...
         * return OperationEventListener.ERROR_CODE_NAME_EMPTY; } else { try {
         * int length = fileName.getBytes("UTF-8").length; // int length =
         * fileName.length(); LogUtils.d(TAG, "checkFileName: " + fileName +
         * ",lenth= " + length); if (length > FileInfo.FILENAME_MAX_LENGTH) {
         * LogUtils.d(TAG, "checkFileName,fileName is too long,len=" + length);
         * return OperationEventListener.ERROR_CODE_NAME_TOO_LONG; } else {
         * return OperationEventListener.ERROR_CODE_NAME_VALID; } } catch
         * (UnsupportedEncodingException e) { e.printStackTrace(); return
         * OperationEventListener.ERROR_CODE_NAME_EMPTY; } }
         */
    }
    ...
    public static File genrateNextNewName(File file) {
        ...
        if (fileName.endsWith(")")) {
            ...
            if (leftBracketIndex != -1) {
                String numeric = fileName.substring(leftBracketIndex + 1,
                        fileName.length() - 1);
                if (numeric.matches("[0-9]+")) {
                    /*
                     * LogUtils.v(TAG,
                     * "Conflict folder name already contains (): " + fileName +
                    ...
                    try {
                        ...
                        /*
                         * LogUtils.e(TAG, "Fn-findSuffixNumber(): " +
                         * e.toString());
                         */
                    }
                }
        ...
        sb.append(fileName).append("(").append(newNumber).append(")")
                .append(ext);
    ...
    public static String sizeToString(long size) {
        ...
        // // strict to two
        long sizeInt = (long) ((sizeDouble) * DECIMAL_NUMBER);
        // decimal places
     ...
     */
    /*
     * public static String getMultipleMimeType(FileManagerService service,
     * String currentDirPath, List<FileInfo> files) { String mimeType = null;
     *
     * for (FileInfo info : files) { mimeType = info.getFileMimeType(service);
     * if ((null != mimeType) && (mimeType.startsWith("image/") ||
     * mimeType.startsWith("video/"))) { break; } }
     *
     * if (mimeType == null || mimeType.startsWith("unknown")) { mimeType =
     * FileInfo.MIMETYPE_UNRECOGNIZED; } LogUtils.d(TAG,
     * "Multiple files' mimetype is " + mimeType); return mimeType; }
     */
     ...
     *
     * @param newFilePath
     *            path to file before modified.(Here modify means rename).
     * @param oldFilePath
     *            path to file after modified.
     * @return true for extension changed, false for not changed.
     */
    public static boolean isExtensionChange(String newFilePath,
            String oldFilePath) {
        ...
        if (((origFileExtension != null) && (!origFileExtension
                .equals(newFileExtension)))
                || ((newFileExtension != null) && (!newFileExtension
     ...
     */
    public static String tryContentMediaMimeType(Context context, Uri fileUri) {
        if (null == fileUri) {
            return null;
        }
        ...
        if (!new File(path).exists()) {
            // LogUtils.w(TAG,
            // "tryContentMediaMimeType(),file is not exist! file:" + path);
            return null;
        }
        ...
        try {
            ...
            Uri uri = android.provider.MediaStore.Files
                    .getContentUri("external");
            cursor = context.getContentResolver().query(uri,
                    new String[] { FileColumns.MIME_TYPE },
                    FileColumns.DATA + "=?",
    ...
    public static void tryContentMediaUri(Context context, Intent intent,
            String path) {
        if (path == null) {
            return;
        }
        ...
        try {
            ...
            if (cursor != null && cursor.moveToFirst()) {
                ...
                if (mediaType == FileColumns.MEDIA_TYPE_IMAGE) {
                    ...
                    mimeType = null;
                    // LogUtils.d(TAG,
                    // "tryContentMediaUri ,the file is not a media file.");
                }
            }
        } finally {
            if (cursor != null) {
        ...
        // LogUtils.d(TAG, "tryContentMediaUri with real mimeType = " + mimeType
        // + ", uri = " + uri);
    }
    ...
    public static String getFileMimeType(Context context, Uri fileUri) {
        ...
        if (mimeType == null) {
            mimeType = MediaFile.getMimeTypeForFile(fileUri.getPath());
        }
        return mimeType;
    }

    public static String getFileMimeType(Context context, String fileUri) {
        ...
        try {
            ...
            //e.printStackTrace();
            Log.d(TAG, "get mime type from data base exception: " + e);
        } finally {
            if (null != cursor) {
                cursor.close();
            }
        ...
        if (mimeType == null) {
            mimeType = MediaFile.getMimeTypeForFile(fileUri);
        }
        return mimeType;
    }
    ...
    public static void closeQuietly(ParcelFileDescriptor file) {
        if (null != file) {
            try {
                file.close();
            } catch (IOException e) {
                Log.d(TAG, "close file exception: " + e);
            }
        }
    }

    public static String trimDot(String file) {
        String res = null;
        if (file != null) {
            String fileName = file;
            while (fileName != null && fileName.startsWith(".")) {
                fileName = fileName.substring((fileName.length() > 0) ? 1 : 0);
                Log.s(TAG, "trimDot " + fileName);
            }
            while (fileName != null && fileName.endsWith(".")) {
                fileName = fileName.substring(0,
                        fileName.length() > 0 ? fileName.length() - 1 : 0);
                Log.s(TAG, "trimDot " + fileName);
            }
            res = fileName;
        }
    ...
    public static String formatTimeStampStringExtend(Context context, long when) {
        Log.d(TAG, "formatTimeStampStringExtend: when = " + when);
        Log.d(TAG, "formatTimeStampStringExtend:  now = " + System.currentTimeMillis());
        Time then = new Time();
        then.set(when);
        Time now = new Time();
        now.setToNow();
        Log.d(TAG, "formatTimeStampStringExtend: now = " + now + ", then = " + then);
    ...
    public static boolean isImage(String mimeType) {
        boolean res = false;
        res = MediaFile.isImageMimeType(mimeType);
        return res;
    }

    public static boolean isAudio(String mimeType) {
        boolean res = false;
        res = MediaFile.isAudioMimeType(mimeType);
        return res;
    }

    public static boolean isVideo(String mimeType) {
        boolean res = false;
        res = MediaFile.isVideoMimeType(mimeType);
        return res;
    }
