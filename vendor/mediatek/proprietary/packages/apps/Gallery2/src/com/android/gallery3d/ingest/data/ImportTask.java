...
public class ImportTask implements Runnable {
  ...
  private static final String TAG = "Gallery2/ImportTask";
  ...
  public void run() {
    ...
    try {
      ...
      for (IngestObjectInfo object : mObjectsToImport) {
        ...
        /// M: [BUG.MODIFY] @{
        // Add null pointer check to avoid USB disconnect case
        if (object != null && hasSpaceForSize(object.getCompressedSize())) {
          try {
            importedPath = new File(dest, object.getName(mDevice)).getAbsolutePath();
          } catch (NullPointerException e) {
            importedPath = null;
            Log.w(TAG, "can not get name fom mtp device, catch null pointer exception, " + e);
          }
          if (importedPath != null && !mDevice.importFile(object.getObjectHandle(), importedPath)) {
            importedPath = null;
          }
        }
        /// @}
