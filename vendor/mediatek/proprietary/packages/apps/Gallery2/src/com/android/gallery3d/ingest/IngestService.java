...
public class IngestService extends Service implements ImportTask.Listener,
  ...
  public void onCreate() {
    ...
    createChannel(mNotificationBuilder);
    /// @}
  ...
  private static final class ScannerClient implements MediaScannerConnectionClient {
    ...
    /// M: [BUG.ADD] create notification channel for android O.@{
    private static final String TAG = "Gallery2/IngestService";

    @TargetApi(Build.VERSION_CODES.O)
    private void createChannel(NotificationCompat.Builder builder) {
        Log.i(TAG, "<createChannel> SDK: " + Build.VERSION.SDK_INT);
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
            String channelId = "mtp_channel";
            NotificationChannel channel = new NotificationChannel(channelId, "mtp",
