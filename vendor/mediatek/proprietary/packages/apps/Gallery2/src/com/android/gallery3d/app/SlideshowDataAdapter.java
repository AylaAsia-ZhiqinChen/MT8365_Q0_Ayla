...
public class SlideshowDataAdapter implements SlideshowPage.Model {
    ...
    public interface SlideshowSource {
        ...
        /// M: [BUG.ADD] @{
        /**
         * Stop ClusterAlbum and ClusterAlbumset reload.
         */
        ...
        public Void run(JobContext jc) {
            while (true) {
                ...
                if (bitmap != null) {
                    ...
                    //item's Uri have already in mUriOfObtainBitmapItem.
                    // mean this is the second time to decode bitmap.
                    // If at the same time mUriOfObtainBitmapItem.size() == 0,
                    // mean there is not bitmap to show. So should
                    // quit the slideShow.
                    if (mUriNotGetBitmapItem.contains(item.getContentUri()
                            .toString())) {
                        if (mUriOfObtainBitmapItem.size() == 0) {
                            synchronized (SlideshowDataAdapter.this) {
                                /// M: new Slide(item, -1, null), as quit
                                // thread signal .
                        ...
                        //add the Uri to set.
                        mUriNotGetBitmapItem.add(item.getContentUri()
                                .toString());
                    }
                    /// @}
                }
    ...
    //mUriOfObtainBitmapItem save URI of item which get bitmap.
    // mUriNotGetBitmapItem save URI of item which do not get bitmap.
