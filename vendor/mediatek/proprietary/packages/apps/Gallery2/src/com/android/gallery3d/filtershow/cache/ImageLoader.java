...
public final class ImageLoader {
    ...
    public static Bitmap loadBitmap(Context context, Uri uri, BitmapFactory.Options o) {
        ...
        /// M: [FEATURE.ADD] @{
        Bitmap res = loadBitmapFromExt(context, uri, o);
        if (res != null) {
            return res;
        }
        /// @}

        /// M: [BUG.ADD] wbmp, get mime type @{
        Cursor c = null;
        String mimeType = "";
        try {
            ...
            Log.e(LOGTAG, "<loadBitmap> Exception when trying to fetch MIME_TYPE info", e);
        } finally {
            if (c != null) {
                c.close();
            }
        ...
        try {
            ...
            if (result != null && null == result.getConfig()
                ...
                Log.d(LOGTAG, "<loadBitmap> wbmp, resizeBitmapByScale");
...
/// @}
    ...
    public static Bitmap resizeBitmapByScale(
        ...
        if (width < 1 || height < 1) {
            Log.d(LOGTAG, "scaled width or height < 1, no need to resize");
            return bitmap;
        }
    ...
    private static Bitmap loadBitmapFromExt(Context context, Uri uri, BitmapFactory.Options o) {
        if (sExtLoaders == null) {
            sExtLoaders =
