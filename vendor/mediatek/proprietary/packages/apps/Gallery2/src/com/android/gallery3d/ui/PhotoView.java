...
/// @}
    ...
    public PhotoView(AbstractGalleryActivity activity) {
        ...
        /// M: [BUG.MODIFY] Add GLRoot parameter @{
    ...
    class FullPicture implements Picture {
        ...
        private void drawTileView(GLCanvas canvas, Rect r) {
            ...
            /// M: [FEATURE.ADD] @{
            for (IPhotoRenderer renderer : mPhotoRenderExts) {
                if (mModel.getMediaItem(0) != null) {
                    renderer.renderOverlay(canvas.getMGLCanvas(), r.width(), r.height(), mModel
                            .getMediaItem(0).getMediaData());
                }
            }
            /// @}
    ...
    private class ScreenNailPicture implements Picture {
        ...
        public void draw(GLCanvas canvas, Rect r) {
            ...
            /// M: [FEATURE.ADD] @{
            for (IPhotoRenderer renderer : mPhotoRenderExts) {
                if (mModel.getMediaItem(mIndex) != null) {
                    renderer.renderOverlay(canvas.getMGLCanvas(), r.width(), r.height(), mModel
                            .getMediaItem(mIndex).getMediaData());
                }
    ...
    private void hideUndoBar() {
        ...
        /// M: [BUG.ADD] Hide action bar menu list.@{
            ...
            if (curIndex + hitIndex < 0) {
                ...
                if (mModel instanceof PhotoDataAdapter) {
                    ...
                    if (totalCount > 0 && (curIndex + hitIndex) >= totalCount) {
                        Log.d(TAG, "<switchToHitPicture> adjust targetIndex from " + targetIndex
                                + " to " + (totalCount - 1));
                        targetIndex = totalCount - 1;
                    }
                }
                mModel.moveTo(targetIndex);
                Log.d(TAG, "<switchToHitPicture> move to " + targetIndex);
            }
        }
        /// @}
    }
...
//********************************************************************
    ...
    private void extConstruct(Activity activity) {
        ...
        mPhotoRenderExts = FeatureManager.getInstance().getImplement(IPhotoRenderer.class,
...
//        }
    ...
    public boolean freshLayers(boolean onActionPresentationMode) {
        return mLayerManager.freshLayers(onActionPresentationMode);
    }
