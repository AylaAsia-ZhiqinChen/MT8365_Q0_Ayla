...
/// @}
        ...
        */
        ...
        while (noBitmap) {
            try {
                ...
                updateExifData(exif, time);
                /// M: [FEATURE.ADD] @{
                updateExifDataFromExt(newSourceUri);
                /// @}
                updateProgress();
    ...
    public static Uri linkNewFileToUri(Context context, Uri sourceUri,
        ...
        if (fileUri || oldSelectedFile == null || !deleteOriginal) {
            Uri uri = MediaStore.Files.getContentUriForPath(file.getPath());
            result = context.getContentResolver().insert(
                    uri, values);
        } else {
            result = MediaStore.scanFile(context, file);
            if (oldSelectedFile.exists()) {
                oldSelectedFile.delete();
                MediaStore.scanFile(context, oldSelectedFile);
            }
        }
        return result;
    }
    ...
    public static boolean updataImageDimensionInDB(Context context,
        ...
        values.put(Images.Media.SIZE, file.length());
        /// M: [FEATURE.ADD] @{
        updateMediaDatabaseFromExt(file, values);
        /// @}
    ...
    private static void updateExifDataFromExt(Uri uri) {
        if (sExtSavers == null) {
            sExtSavers =
                    (IFilterShowImageSaver[]) FeatureManager.getInstance().getImplement(
                            IFilterShowImageSaver.class);
        }
        for (IFilterShowImageSaver saver : sExtSavers) {
            saver.updateExifData(uri);
        }
    }

    private static void updateMediaDatabaseFromExt(File file, ContentValues values) {
        if (sExtSavers == null) {
            sExtSavers =
                    (IFilterShowImageSaver[]) FeatureManager.getInstance().getImplement(
                            IFilterShowImageSaver.class);
        ...
        for (IFilterShowImageSaver saver : sExtSavers) {
            saver.updateMediaDatabase(file, values);
        }
    }
