...
public class FilterShowActivity extends FragmentActivity implements OnItemClickListener,
    ...
    private void loadXML() {
        ...
        /// M: [TESTCASE.ADD] @{
        if (SystemPropertyUtils.get("prop.filtershow.imagetest").equals("1")) {
            View mainView = findViewById(R.id.mainView);
            if (mainView != null) {
                int color = SystemPropertyUtils.getInt("prop.filtershow.imagetest.color", 0);
                mainView.setBackgroundColor(Color.rgb(color, color, color));
            }
        }
        /// @}
    ...
    public void setupMasterImage() {
        ...
        /// M: [BUG.ADD] @{
        //add for small heap size and small screen
        if (Runtime.getRuntime().maxMemory() <= LIMIT_SUPPORTS_HIGHRES * 2
                && getScreenImageSize() <= SMALL_SCREEN_SIZE) {
            mMasterImage.setSupportsHighRes(false);
        }
        /// @}
    }
    ...
    private String loadAlbumNameForSaving() {
        if (mSelectedImageUri == null) {
            Log.d(TAG, "<loadAlbumNameForSaving> mSelectedImageUri is null");
            return null;
        }
        File saveDir = SaveImage.getFinalSaveDirectory(this, mSelectedImageUri);
        int bucketId = GalleryUtils.getBucketId(saveDir.getPath());
