        ...
        public Void call() throws Exception {
            ...
            for (int i = start; i < end; ++i) {
                ...
                if (mItemVersion[index] != itemVersion) {
                    ...
                    /// M: [BUG.ADD] add listener for DRM item.
                    if (updateItem.getExtItem() != null) {
                        ExtItem.DataChangeListener listener =
                                new ExtItem.DataChangeListener() {
                            @Override
                            public void onExtItemDataChange(ArrayList<String> updateContentList) {
                                Log.d(TAG, "<onExtItemDataChange>,size:"
                                    + updateContentList.size() + ",mContentEnd:" + mContentEnd);
                                for (int i = 0; i < mContentEnd; i++) {
                                    MediaItem item = mData[i];
                                    if (item != null && item.getExtItem() != null) {
                                        String path = item.getMediaData().filePath;
                                        if (updateContentList.indexOf(path) != -1) {
                        ...
                        updateItem.getExtItem().registerListener(listener);
                    }
                    /// @}
