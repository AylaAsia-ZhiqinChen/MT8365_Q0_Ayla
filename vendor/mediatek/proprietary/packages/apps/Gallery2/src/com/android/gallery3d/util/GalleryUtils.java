...
public class GalleryUtils {
    ...
    public static void initialize(Context context) {
        ...
        Log.d(TAG, "<initialize> device width " + reMetrics.widthPixels + " height "
                + reMetrics.heightPixels);

        initializeTiledTxtureSize(metrics);

        FeatureConfig.sIsLowRamDevice = DecodeSpecLimitor.sIsLowRamDevice = isLowRamDevice(context);
        Log.d(TAG, "<initialize> sIsLowRamDevice = " + FeatureConfig.sIsLowRamDevice);
        /// @}
    ...
    // FHD PLUS height size in pixels
    private static final int FHD_PLUS_HEIGHT = 2000;
    // TILE_SIZE for FHD PLUS display case
    private static final int TILE_SIZE_HIGH_RESOLUTION_FHDP = 511;
    // CONTENT_SIZE for FHD PLUS display case
    private static final int CONTENT_SIZE_HIGH_RESOLUTION_FHDP = 509;

    private static void initializeTiledTxtureSize(DisplayMetrics metrics) {
        int maxPixels = Math.max(metrics.heightPixels, metrics.widthPixels);
        if (maxPixels > HIGH_RESOLUTION_SIZE) {
            if (metrics.heightPixels > FHD_PLUS_HEIGHT) {
                TiledTexture.TILE_SIZE = TILE_SIZE_HIGH_RESOLUTION_FHDP;
                TiledTexture.CONTENT_SIZE = CONTENT_SIZE_HIGH_RESOLUTION_FHDP;
            } else {
