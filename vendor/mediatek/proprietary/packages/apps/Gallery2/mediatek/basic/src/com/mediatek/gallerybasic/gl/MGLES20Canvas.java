...
public class MGLES20Canvas implements MGLCanvas {
    ...
    // Bound textures.
    private ArrayList<MRawTexture> mTargetTextures = new ArrayList<MRawTexture>();
    ...
    private float[] mTempTextureMatrix = new float[MATRIX_SIZE];
    private int[] mTempIntArray = new int[1];

    public void
            syncState(float[] matrics, int matricsIndex, float[] projectMatrics, int width,
                      int height, int screenWidth, int screenHeight, float alpha) {
        reset();

        Matrix.setIdentityM(mTempTextureMatrix, 0);
        mAlphas[mCurrentAlphaIndex] = alpha;

        mMatrices = matrics.clone();
        mCurrentMatrixIndex = matricsIndex;
        mProjectionMatrix = projectMatrics.clone();
        mWidth = width;
        mHeight = height;
    ...
    private void reset() {
        mAlphas = new float[INITIAL_RESTORE_STATE_SIZE];
        // mBoxCoordinates = 0;
        mCountDrawLine = 0;
        mCountDrawMesh = 0;
        mCountFillRect = 0;
        mCountTextureRect = 0;
        mCurrentAlphaIndex = 0;
        mCurrentMatrixIndex = 0;
        // mDeleteBuffers = new IntArray();
        // mFrameBuffer[0] = INVALID_FRAME_BUFFER;
        mHeight = 0;
        mMatrices = new float[INITIAL_RESTORE_STATE_SIZE * MATRIX_SIZE];
        ;
        mProjectionMatrix = new float[MATRIX_SIZE];
        mSaveFlags = new IntArray();
        ;
        mScreenHeight = 0;
        mScreenWidth = 0;
        // mTargetTextures.clear();
        mTempColor = new float[4];
        mTempIntArray = new int[1];
        mTempMatrix = new float[32];
        ;
        mTempSourceRect = new RectF();
        mTempTargetRect = new RectF();
        mTempTextureMatrix = new float[MATRIX_SIZE];
    }
    ...
    private void init() {
        ...
        int textureFragmentShader =
                loadShader(GLES20.GL_FRAGMENT_SHADER, TEXTURE_FRAGMENT_SHADER);
        int oesTextureFragmentShader =
                loadShader(GLES20.GL_FRAGMENT_SHADER, OES_TEXTURE_FRAGMENT_SHADER);

        mDrawProgram = assembleProgram(drawVertexShader, drawFragmentShader, mDrawParameters);
        mTextureProgram =
                assembleProgram(textureVertexShader, textureFragmentShader, mTextureParameters);
        mOesTextureProgram =
                assembleProgram(textureVertexShader, oesTextureFragmentShader,
                        mOesTextureParameters);
        mMeshProgram =
                assembleProgram(meshVertexShader, textureFragmentShader, mMeshParameters);
        GLES20.glBlendFunc(GLES20.GL_ONE, GLES20.GL_ONE_MINUS_SRC_ALPHA);
        checkError();

        textureFragmentShader = loadShader(GLES20.GL_FRAGMENT_SHADER, TEXTURE_FRAGMENT_SHADER);
        int meshVertex3DShader = loadShader(GLES20.GL_VERTEX_SHADER, MESH_VERTEX_3D_SHADER);
        mMesh3DProgram =
                assembleProgram(meshVertex3DShader, textureFragmentShader, mMeshParameters);
        checkError();

        // set default value of frame buffer
        mFrameBuffer[0] = INVALID_FRAME_BUFFER;
    }
    ...
    private int
            assembleProgram(int vertexShader, int fragmentShader, ShaderParameter[] params) {
        ...
        if (mLinkStatus[0] != GLES20.GL_TRUE) {
            ...
            Log.e(TAG, "<assembleProgram>" + GLES20.glGetProgramInfoLog(program));
    ...
    public void save(int saveFlags) {
        ...
        if (saveMatrix) {
            ...
            System.arraycopy(mMatrices, currentIndex, mMatrices, mCurrentMatrixIndex,
                    MATRIX_SIZE);
        }
    ...
    private void draw(int type, int offset, int count, float x, float y, float width,
                      float height, int color, float lineWidth) {
    ...
    private void draw(ShaderParameter[] params, int type, int count, float x, float y,
    ...
    private void setMatrix(ShaderParameter[] params, float x, float y, float width,
                           float height) {
        Matrix.translateM(mTempMatrix, 0, mMatrices, mCurrentMatrixIndex, x, y, 0f);
        Matrix.scaleM(mTempMatrix, 0, width, height, 1f);
        Matrix.multiplyMM(mTempMatrix, MATRIX_SIZE, mProjectionMatrix, 0, mTempMatrix, 0);
        GLES20.glUniformMatrix4fv(params[INDEX_MATRIX].handle, 1, false, mTempMatrix,
                MATRIX_SIZE);
        checkError();
    }
    ...
    public void fillRect(float x, float y, float width, float height, int color) {
        draw(GLES20.GL_TRIANGLE_STRIP, OFFSET_FILL_RECT, COUNT_FILL_VERTEX, x, y, width,
                height, color, 0f);
        mCountFillRect++;
    }

    @Override
    public void drawTexture(MBasicTexture texture, int x, int y, int width, int height) {
        if (width <= 0 || height <= 0) {
            return;
        }
    ...
    private static void copyTextureCoordinates(MBasicTexture texture, RectF outRect) {
        int left = 0;
        int top = 0;
    ...
    @Override
    public void drawTexture(MBasicTexture texture, float[] textureTransform, int x, int y,
                            int w, int h) {
        if (w <= 0 || h <= 0) {
            return;
        }
    ...
    private void drawTextureRect(MBasicTexture texture, RectF source, RectF target) {
    ...
    private static void convertCoordinate(RectF source, RectF target, MBasicTexture texture) {
        ...
        if (source.right > xBound) {
            target.right =
                    target.left + target.width() * (xBound - source.left) / source.width();
            source.right = xBound;
        }
        float yBound = (float) height / texHeight;
        if (source.bottom > yBound) {
            target.bottom =
                    target.top + target.height() * (yBound - source.top) / source.height();
            source.bottom = yBound;
        }
    }

    private void drawTextureRect(MBasicTexture texture, float[] textureMatrix, RectF target) {
    ...
    private ShaderParameter[] prepareTexture(MBasicTexture texture) {
    ...
    private void prepareTexture(MBasicTexture texture, int program, ShaderParameter[] params) {
    ...
    @Override
    public void drawMesh(MBasicTexture texture, int x, int y, int xyBuffer, int uvBuffer,
    ...
    @Override
    public void drawMixed(MBasicTexture texture, int toColor, float ratio, int x, int y,
                          int w, int h) {
        copyTextureCoordinates(texture, mTempSourceRect);
        mTempTargetRect.set(x, y, x + w, y + h);
    ...
    @Override
    public void drawMixed(MBasicTexture texture, int toColor, float ratio, RectF source,
                          RectF target) {
        if (target.width() <= 0 || target.height() <= 0) {
            return;
        }
    ...
    @Override
    public boolean unloadTexture(MBasicTexture texture) {
    ...
    public void dumpStatisticsAndClear() {
        String line =
                String.format("MESH:%d, TEX_RECT:%d, FILL_RECT:%d, LINE:%d", mCountDrawMesh,
    ...
    public void endRenderTarget() {
        MRawTexture oldTexture = mTargetTextures.remove(mTargetTextures.size() - 1);
        MRawTexture texture = getTargetTexture();
    ...
    public void beginRenderTarget(MRawTexture texture) {
        ...
        MRawTexture oldTexture = getTargetTexture();
    ...
    private void setRenderTarget(MBasicTexture oldTexture, MRawTexture texture) {
    ...
    public static void checkError() {
        ...
        if (error != 0) {
            ...
            Log.e(TAG, "<checkError> GL error: " + error, t);
        }
    }
    ...
    private static void printMatrix(String message, float[] m, int offset) {
        ...
        Log.v(TAG, "<printMatrix> " + b.toString());
    }
    ...
    private static final String MESH_VERTEX_3D_SHADER = "" + "uniform mat4 " + MATRIX_UNIFORM
    ...
    @Override
    public void drawMesh(MBasicTexture tex, float degree, float x, float y, float z,
        ...
        checkError();
        int positionHandle = mMeshParameters[INDEX_POSITION].handle;
        GLES20.glVertexAttribPointer(positionHandle, COORDS_PER_VERTEX_3, GLES20.GL_FLOAT,
                false, VERTEX_STRIDE_3, 0);
        checkError();
    ...
    @Override
    public void readPixels(int x, int y, int width, int height, int type, int format,
                           Buffer buffer) {
    ...
    class GLES20Generator implements Generator {
    ...
    Generator mGenerator = new GLES20Generator();
    ...
    public Bitmap saveTexture(int texture, int width, int height) {
        ByteBuffer buffer = ByteBuffer.allocate(width * height * BYTE_ARGB);
