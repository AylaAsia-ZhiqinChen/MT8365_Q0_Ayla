...
public class ThreadPool {
    ...
    /// M: [BUG.ADD] CPU_CORES_NUM:Dynamic control thread number
    //according to CPU Cores from constant number, if the CPU Cores <= 2,
    //then let the minium parallel process be 2
    //to avoid multi_thread activity error.@{
    ...
    public static final int PARALLEL_THREAD_NUM = PARALLEL_THREAD_NUM_MIN >
        (CPU_CORES_NUM / 2) ? PARALLEL_THREAD_NUM_MIN : (CPU_CORES_NUM / 2);
    /// @}
    ...
    private static class ResourceCounter {
        ...
        public ResourceCounter(int v) {
            /// M: [DEBUG.ADD] @{
            Log.d(TAG,
                    "<ResourceCounter.ResourceCounter> ResourceCounter max thread num "
                            + v + " CPU Core " + CPU_CORES_NUM);
            /// @}
            value = v;
        }
    }
