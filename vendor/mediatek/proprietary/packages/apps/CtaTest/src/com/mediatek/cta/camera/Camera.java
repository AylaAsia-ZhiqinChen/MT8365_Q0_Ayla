...
public class Camera extends ActivityBase implements View.OnClickListener, SurfaceHolder.Callback {
    ...
    private static final int DIALOG_WAIT_STOP = 4;
    ...
    private ProgressDialog mDialogWait = null;
    ...
    private void record() {
        ...
        try {
            ...
            mRecorder.setOrientationHint(info.orientation);
            mRecorder.prepare();
            mRecorder.start();
        } catch (IOException e) {
            if (mRecorder != null) {
...
//        mWorkHandler.sendEmptyMessage(MSG_START_PREVIEW);
    ...
    protected Dialog onCreateDialog(int id) {
        ...
        case DIALOG_WAIT:
            ...
            return mDialog;
        case DIALOG_WAIT_STOP:
            mDialogWait = new ProgressDialog(this);
            mDialogWait.setMessage("Wait for stop");
    ...
    public void onBackPressed() {
        Log.i(TAG, "onBackPressed() " + mCameraState);
        if (mRecorderButton.getText().toString().equals(getString(R.string.cta_test_stop))) {
            // Stop recording
            mWorkHandler.sendEmptyMessage(MSG_STOP_RECORD);
            mBackPressed = true;
    ...
    private void setCameraParameters() {
        ...
        CameraInfo info = new CameraInfo();
        android.hardware.Camera.getCameraInfo(0, info);
        mParameters.setRotation(info.orientation);
        mCameraDevice.setParameters(mParameters);
    }
