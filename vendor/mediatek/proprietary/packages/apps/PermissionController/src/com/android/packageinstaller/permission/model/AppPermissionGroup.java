    ...
    public static AppPermissionGroup create(Context context, PackageInfo packageInfo,
        ...
        for (int i = 0; i < permissionCount; i++) {
            ...
            if (packageInfo.applicationInfo.targetSdkVersion <= Build.VERSION_CODES.LOLLIPOP_MR1
                    && !manager.isPlatformPermissionGroup(groupInfo.packageName, groupInfo.name)) {
                continue;
            }
            ///@}
            ...
            // / M: CTA requirement - permission control @{
            final String appOp = manager.isPlatformPermission(
                    requestedPermissionInfo.packageName,
                    requestedPermissionInfo.name) ? AppOpsManager
                    .permissionToOp(requestedPermissionInfo.name) : null;
            // /@}
    ...
    public boolean grantRuntimePermissions(boolean fixedByTheUser, String[] filterPermissions) {
        ...
        for (Permission permission : mPermissions.values()) {
            ...
            if (mAppSupportsRuntimePermissions) {
                ...
                CtaManager manager = CtaManagerFactory.getInstance().makeCtaManager();
                /// }@
                if (!manager.isCtaSupported() && permission.isReviewRequired()) {
                    permission.unsetReviewRequired();
                }
            }
    ...
    void persistChanges(boolean mayKillBecauseOfAppOpsChange) {
        ...
        for (int i = 0; i < numPermissions; i++) {
            ...
            int flags = (permission.isUserSet() ? PackageManager.FLAG_PERMISSION_USER_SET : 0)
                    ...
                    | ((!manager.isCtaSupported() && permission.isReviewRequired())
