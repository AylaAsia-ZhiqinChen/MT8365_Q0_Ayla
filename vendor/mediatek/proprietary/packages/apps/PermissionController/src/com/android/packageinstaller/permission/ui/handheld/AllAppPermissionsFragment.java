...
import com.mediatek.packageinstaller.PermissionState;
///@}
...
public final class AllAppPermissionsFragment extends SettingsWithLargeHeader {
            ...
            if (sCtaManager.isCtaSupported()) {
                setSwitchChangeListener((pref, newValue) -> {
                    if (mLaunchedFromReview) {
                        ...
                        Log.d(LOG_TAG,
                                "onPreferenceChange setPermStateCache " + permission + " " + val);
                        UtilsExt.setPermStateCache(appPermissionGroup.getPermission(permission),
    ...
    private static void revokePermissions(AppPermissionGroup appPermissionGroup,
            String permission) {
        /// M: CTA requirement - permission control @{
        if (sCtaManager.isCtaSupported()) {
            appPermissionGroup.revokeRuntimePermissions(false, new String[] { permission });
