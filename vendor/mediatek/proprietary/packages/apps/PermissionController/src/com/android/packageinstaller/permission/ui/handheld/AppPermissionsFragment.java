...
public final class AppPermissionsFragment extends SettingsWithLargeHeader {
    ...
    private void updatePreferences() {
        ...
        for (int i = 0; i < groups.size(); i++) {
            ...
            boolean isPlatform = manager.isPlatformPermissionGroup(group.getDeclaringPackage(),
                    group.getName());
            /// @}
        ...
        /// M: CTA requirement - permission control @{
        CtaManager manager = CtaManagerFactory.getInstance().makeCtaManager();
        // Add "manage all permissions" in this view based on CTA feedback
        if (manager.isCtaSupported()) {
            PreferenceCategory manageAllPermsCategory = (PreferenceCategory) findPreference(
                    "allperms");
            manageAllPermsCategory.removeAll();
            Preference manageAllPermsPreference = new Preference(context);
            manageAllPermsPreference.setIcon(R.drawable.ic_toc);
            manageAllPermsPreference.setTitle(R.string.pref_manage_all_perms);
            manageAllPermsPreference.setOnPreferenceClickListener(preference -> {
                    Fragment frag = AllAppPermissionsFragment
                    ...
                    getFragmentManager().beginTransaction().replace(android.R.id.content, frag)
                            .addToBackStack("AllPerms").commit();
                    return true;
            });
            manageAllPermsCategory.addPreference(manageAllPermsPreference);
        }
        /// @}
