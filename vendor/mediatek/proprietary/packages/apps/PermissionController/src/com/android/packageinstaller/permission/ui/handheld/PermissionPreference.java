...
class PermissionPreference extends MultiTargetSwitchPreference {
    ...
    void onDenyAnyWay(@ChangeTarget int changeTarget) {
        ...
        if (UtilsExt.sCtaManager.isCtaSupported()) {
            mCallBacks.hasConfirmDefaultPermissionRevoke();
            setChecked(false);
            UtilsExt.setPermGroupStateCache(mGroup, false);
        /// @}
        } else {
        mCallBacks.onPreferenceChanged(getKey());
     ...
     */
    void updateUiforCTA() {
        setChecked(UtilsExt.isPermGroupGrantedByCache(mGroup));
        setSummary(mGroup.getDescription());
        setPersistent(false);
        setOnPreferenceChangeListener(
                (pref, newValue) -> requestChangeforCTA((Boolean) newValue, CHANGE_BOTH));
    ...
    /**
     * Update the preference after the state might have changed for CTA.
     */
    private boolean requestChangeforCTA(boolean requestGrant, @ChangeTarget int changeTarget) {
        if (!mCallBacks.shouldConfirmDefaultPermissionRevoke()) {
            UtilsExt.setPermGroupStateCache(mGroup, requestGrant);
            return true;
        }
        if (isChecked()) {
            showDefaultDenyDialog(changeTarget);
        } else {
            UtilsExt.setPermGroupStateCache(mGroup, true);
            return true;
        }
        return false;
    }
