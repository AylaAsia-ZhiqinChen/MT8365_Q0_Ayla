...
public class ConversationPagerAdapter extends FragmentStatePagerAdapter2
    ...
    /// M: Initialize as true to make sure page change could be listened
    private boolean mPageChangeListenerEnabled = true;
    ...
    /**
     * M: returns the conversation in specific position(abstracts from
     * method "getItem" for reusing).
     * @param position
     * @return
     */
    private Conversation getConversation(int position) {
        ...
        if (isPagingDisabled(cursor)) {
            ...
            if (position != 0) {
                /**
                 * M: Due to mechanism changed, so it should not use
                 * LogUtils.wtf to notify a serious exception, just logging it.
                 *
                 * @{
                 */
                LogUtils.w(LOG_TAG, "pager cursor is null and position is non-zero: %d",
                        position);
                /** @} */
            }
            ...
            if (!cursor.moveToPosition(position)) {
                /** M: Use LogUtils.w to instead of LogUtils.wtf. */
                LogUtils.w(LOG_TAG, "unable to seek to ConversationCursor pos=%d (%s)", position,
                        cursor);
                return null;
            }
    ...
    public Fragment getItem(int position) {
        final Conversation c = getConversation(position);
            ...
            if (pos == POSITION_NONE && cursor != null && currConversation != null) {
                ...
                if (frag != null && cursor.moveToPosition(pos) && frag.isUserVisible()) {
                    ...
                    if (!isPagingDisabled(cursor)) {
                        ...
                        /// M: need update conversation info, such as conversationheader
                        frag.onConversationUpdated(frag.getConversation());
                    }
                    /** M: @} */
                }
            }
    ...
    public boolean dispatchKeyEvent(KeyEvent event) {
        if (null != mPager) {
            ...
            final AbstractConversationViewFragment fragment =
                    (AbstractConversationViewFragment) getFragmentAt(currentItem);
            if (null != fragment && fragment instanceof SecureConversationViewFragment) {
                LogUtils.v(LogTag.getLogTag(), "ConvPagerAdapter dispatchKeyEvent event: %s", event);
                return ((SecureConversationViewFragment) fragment).dispatchKeyEvent(event);
            }
        }
        return false;
    }
    ...
    /**
     * M: retrieves fragment by its conversation instead of position, because the position would
     * change in singleton mode(from 0 to actual value), this way is more make sense.
     */
    @Override
    public Fragment getFragmentAt(int position) {
        Conversation c = getConversation(position);
        /**
         * M: The getConversation would return null, so it should do null
         * pointer check firstly.
         * @{
         */
        if (c == null) {
            return null;
        ...
        for (Fragment f : mFragments) {
            if (c.equals(((AbstractConversationViewFragment) f).getConversation())) {
                LogUtils.d(LOG_TAG, "pager adapter find existing fragment of position: %d", position);
                return f;
            }
        }
        return null;
