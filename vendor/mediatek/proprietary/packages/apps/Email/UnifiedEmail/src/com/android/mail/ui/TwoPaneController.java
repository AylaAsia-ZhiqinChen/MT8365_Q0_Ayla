...
public class TwoPaneController extends AbstractActivityController implements
    ...
    private void initializeConversationListFragment() {
        if (Intent.ACTION_SEARCH.equals(mActivity.getIntent().getAction())) {
            ...
            /// M: we also need call this in TwoPaneController, just like in OnePaneController, Otherwise,
            /// the UI would be incorrect.
     ...
     */
    /*M: package*/ void renderConversationList() {
        if (mActivity == null) {
            return;
        }
    ...
    public boolean shouldShowSearchBarByDefault(int viewMode) {
        /// M: disable search bar in remote search(affected by local search feature)
        return /*viewMode == ViewMode.SEARCH_RESULTS_LIST ||
                (mIsTabletLandscape && viewMode == ViewMode.SEARCH_RESULTS_CONVERSATION)*/ false;
    }
     ...
     */
    @Override
    public void updateConversation(Collection<Conversation> target,
            String columnName, boolean value) {
        super.updateConversation(target, columnName, value);
        if (columnName.equals(UIProvider.ConversationColumns.STARRED)) {
            final ConversationListFragment convList = getConversationListFragment();
            if (convList == null) {
                return;
            }
            SwipeableListView listView = convList.getListView();
            if (listView != null) {
