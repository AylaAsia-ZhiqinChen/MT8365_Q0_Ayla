...
public class EmailCopyContextMenu implements OnCreateContextMenuListener {
    ...
    public void onCreateContextMenu(ContextMenu menu, View v, ContextMenuInfo info) {
        if (!TextUtils.isEmpty(mAddress)) {
            ...
            Intent intent = new Intent(Intent.ACTION_VIEW,Uri.parse(WebView.SCHEME_MAILTO + mAddress));
            //intent.setPackage(mContext.getPackageName());
            intent.setFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
            menu.findItem(SEND_EMAIL_ITEM).setIntent(intent);
            LogUtils.e("UnifiedEmail - EmailCopy", "mAddress: %s ",mAddress);
