...
public class AbstractConversationWebViewClient extends WebViewClient {
    ...
    protected final WebResourceResponse loadCIDUri(Uri uri, ConversationMessage message) {
        ...
        /// M: truncated reload trigger part from cid uri.
        /// The eml mail not truncated,it parsed from eml attachment,so igrone
        /// the reload_trigger tag. @{
        String realCid;
        int triggerIndex = cid.indexOf("?reload_trigger");
        if (triggerIndex < 0) {
            realCid = cid;
            LogUtils.i(LOG_TAG, "The message may not truncated,may be an eml mail");
        } else {
            realCid = cid.substring(0, triggerIndex);
        }
        /// @}
        final Uri queryUri = Uri.withAppendedPath(message.attachmentByCidUri, realCid);
    ...
    public boolean shouldOverrideUrlLoading(WebView view, String url) {
        ...
        /// M: block some wrong link, which with 'x-thread' scheme. @{
        if (!TextUtils.isEmpty(url) && url.startsWith("x-thread:")) {
            LogUtils.d(LOG_TAG, "can't open this link [%s]", url);
            Toast.makeText(mActivity, R.string.open_link__error_toast, Toast.LENGTH_LONG).show();
            return true;
        }
        /// @}
