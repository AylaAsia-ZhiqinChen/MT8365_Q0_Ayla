...
public abstract class FragmentStatePagerAdapter2 extends PagerAdapter {
    ...
    /// M: uses HashSet instead of SparseArrayCompat to cache fragments, ignores the fragment's
    /// position, because the position was not stable enough, it is volatile, especially when singleton mode.
    protected HashSet<Fragment> mFragments = new HashSet<Fragment>();
    ...
    public Object instantiateItem(ViewGroup container, int position) {
        ...
        if (DEBUG) LogUtils.v(TAG, "instantiateItem fragments[" + mFragments.size() + "]: " + mFragments);
        /// M: using override method getFragmentAt() instead original one.
        final Fragment existing = getFragmentAt(position);
        if (existing != null) {
            return existing;
        }
    ...
    public void destroyItem(ViewGroup container, int position, Object object) {
        ...
        /// M: deletes fragment by fragment object not position(we deprecated that way)
    ...
    public void finishUpdate(ViewGroup container) {
        /// M: If the activity had been destroyed, it should do nothing, just
        // only return.
    ...
    public void notifyDataSetChanged() {
        ...
        /// M: forgets updating the position cause the position was not stable enough, it is volatile,
        /// especially when singleton mode, and we has decided ignore it.
/*
...
*/
    ...
    abstract public Fragment getFragmentAt(int position);

    /**
     * M: deletes fragment in cache set(subclass could override it)
     * @param deletingFragment
     */
    public void deleteFragment(Fragment deletingFragment) {
        mFragments.remove(deletingFragment);
    }
}