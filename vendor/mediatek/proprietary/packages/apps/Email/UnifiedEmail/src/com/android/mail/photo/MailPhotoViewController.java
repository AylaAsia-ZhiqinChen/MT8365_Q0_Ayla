...
public class MailPhotoViewController extends PhotoViewController {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        mIsViewOnlyMode = intent.getBooleanExtra(
                MailPhotoViewActivity.EXTRA_IS_VIEW_ONLY, false);
        if (savedInstanceState != null) {
            mIsViewOnlyMode = savedInstanceState
                    .getBoolean(MailPhotoViewActivity.EXTRA_IS_VIEW_ONLY);
        }
        /// @}
    ...
    public void onSaveInstanceState(Bundle outState) {
        ...
        outState.putBoolean(MailPhotoViewActivity.EXTRA_IS_VIEW_ONLY,
                mIsViewOnlyMode);
    }
    ...
    public void updateActionItems() {
        ...
        if (mIsViewOnlyMode) {
            if (mMenu != null) {
                ...
                /// M: Set ExtraOption1Item to invisible because it is blank
        ...
        if (attachment != null && mSaveItem != null && mShareItem != null) {
            ...
            if (mMenu != null) {
                /**
                 * M: In some case,the attachment info not be downloaded with
                 * poor network but user clicked the attachment,then the
                 * mExtraOption1Item will be shown blank. {@
                 */
                mExtraOption1Item.setVisible(false);
                /** @} */
                mMenu.setGroupEnabled(R.id.photo_view_menu_group, false);
    ...
    private void updateProgressAndEmptyViews(
        ...
        if (attachment.isDownloadFailed()) {
            ...
            /**
             * M: For the case that closing the network when down loading
             * attachment, and after opening network, the attachment will be
             * auto re-download or not clicking retry button to re-load, but not
             * according to clicking retry button, the emptyView and retry
             * button will not be gone. @{
             */
    ...
    private void saveAttachment(final Attachment attachment) {
        if (attachment != null && attachment.canSave()) {
            ...
            if (!AttachmentUtils.isSDCardSpaceEnough(mMailActivity.getContext(), attachment.size, true)) {
            ...
            if (Utility.fileExists(mMailActivity.getContext(), attachment.contentUri)) {
                AttachmentUtilities.copyAttachmentFromInternalToExternal(mMailActivity.getContext(), attachment.uri, attachment.contentUri);
            } else {
                /// M: Can't download when no network available, prompt a dialog @{
                if (!AttachmentUtils.canDownloadAttachment(mMailActivity.getContext(), null)) {
                    UiUtilities.showConnectionAlertDialog(mMailActivity.getFragmentManager());
                    return;
                }
                /// @}
                mActionHandler.setAttachment(attachment);
    ...
    /**
     * M: Log for Performance case.
     */
    public void onFragmentPhotoLoadComplete(PhotoViewFragment fragment, boolean success) {
        super.onFragmentPhotoLoadComplete(fragment, success);
        LogUtils.i(LogTag.getLogTag(),
                "[Performance test][Email][Email_020] <<< Open image end [%d]",
