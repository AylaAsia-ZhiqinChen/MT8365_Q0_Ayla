...
public abstract class FolderSelectionDialog extends DialogFragment implements OnClickListener {
    ...
    /// M: add tag for fragment query.
    public static final String TAG = "FolderSelectionDialog";
    ...
    public void onCreate(final Bundle savedInstanceState) {
        ...
        Parcelable[] conversations = args.getParcelableArray(ARG_TARGET_TAG);
        /// M: Parcelable[] cannot be cast to com.android.mail.providers.Conversation[] directly.
        /// Just cast them one by one. @{
        mTarget = new ArrayList<Conversation>();
        for (Parcelable c : conversations) {
            mTarget.add((Conversation) c);
        }
        /// @}
    }
    ...
    protected ConversationUpdater getConversationUpdater() {
        if (!isResumed()) {
            /// M: return null and ignore this function call, not thrown an exception.
            LogUtils.e(TAG, "Tried to update conversations while fragment is not running");
            return null;
        }
