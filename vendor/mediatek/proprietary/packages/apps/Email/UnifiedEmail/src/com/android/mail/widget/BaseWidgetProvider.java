...
public abstract class BaseWidgetProvider extends AppWidgetProvider {
    ...
    /// M: Add a extra key to save a string type uri, just using for save the uri to intent's toUri method.
    public static final String EXTRA_FOLDER_URI_STRING = "folder-uri-string";
    ...
    public void onReceive(Context context, Intent intent) {
        ...
        if (ACTION_UPDATE_WIDGET.equals(action)) {
            ...
            validateAllWidgetInformation(context);
                ...
                /** M: When date or time changed, we need update the widget @{ */
            ...
            final Uri folderUri;
            boolean updateAllWidgets;
            if (Utils.ACTION_NOTIFY_DATASET_CHANGED.equals(action)) {
                accountUri = extras.getParcelable(Utils.EXTRA_ACCOUNT_URI);
                folderUri = extras.getParcelable(Utils.EXTRA_FOLDER_URI);
                updateAllWidgets = extras.getBoolean(EXTRA_UPDATE_ALL_WIDGETS, false);
            } else {
                accountUri = null;
                folderUri = null;
            ...
            /// M: Not configured widgets set.
            final Set<Integer> widgetsNotConfig = Sets.newHashSet();
            for (int id : getCurrentWidgetIdsIfSupported(context)) {
                ...
                if (accountFolder != null) {
                    ...
                    /// M: Not configured widgets set.
                    widgetsNotConfig.add(id);
                }
            }
            /// M: Show set up for the not configured widgets. @{
            if (widgetsNotConfig.size() > 0) {
                AppWidgetManager appWidgetManager = AppWidgetManager.getInstance(context);
                for (Integer widgetId : widgetsNotConfig) {
                    final RemoteViews remoteViews = new RemoteViews(
                            context.getPackageName(), R.layout.widget);
                    configureRemoteViewAsDefault(context, widgetId, remoteViews);
            ...
            if (widgetsToUpdate.size() > 0) {
                ...
                LogUtils.d(LOG_TAG, "dataset changed, update all widget(%s)",
                        widgetsToUpdate);
            }
        }
    }
    ...
    private class BulkUpdateAsyncTask extends AsyncTask<Void, Void, Void> {
        ...
        protected Void doInBackground(final Void... params) {
            for (int i = 0; i < mAppWidgetIds.length; ++i) {
                ...
                /// M: we also need notify the remote view factory to update its data.
                /// and there was another weird problem that if I put these code into onUpdate
                /// it did not work...why? @{
                LogUtils.d(LOG_TAG, "BaseWidgetProvider : BulkUpdateAsyncTask()");
                AppWidgetManager.getInstance(mContext).notifyAppWidgetViewDataChanged(mAppWidgetIds[i],
                        R.id.conversation_list);
                /// @}
            }
...
*/
            /// M: Extracted as a method to update widgets.
            configureRemoteViewAsDefault(context, appWidgetId, remoteViews);
            LogUtils.d(LOG_TAG, "BaseWidgetProvider.updateWidgetInternal invalid handling: %s, %s",
                    account != null ? account.uri : null, folderUri);
        } else {
            LogUtils.d(LOG_TAG, "BaseWidgetProvider.updateWidgetInternal valid handling: %s, %s",
                    account != null ? account.uri : null, folderUri);
    ...
    /// M: Update widgets which are not configured.
    private void configureRemoteViewAsDefault(Context context, int widgetId,
        ...
        configureIntent.setData(Uri.parse(configureIntent.toUri(Intent.URI_INTENT_SCHEME)));
        /// M: it seems no need using FLAG_ACTIVITY_NO_HISTORY for MailboxSelectionActivity
        //configureIntent.setFlags(Intent.FLAG_ACTIVITY_NO_HISTORY);
        PendingIntent clickIntent = PendingIntent.getActivity(context, 0, configureIntent,
                PendingIntent.FLAG_UPDATE_CURRENT);
        remoteViews.setOnClickPendingIntent(R.id.widget_configuration, clickIntent);
        LogUtils.d(LOG_TAG,
                "BaseWidgetProvider.updateNotConfigWidget invalid handling, widgetId %d", widgetId);
    }
