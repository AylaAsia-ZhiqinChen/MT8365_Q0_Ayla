...
public final class ConversationListFragment extends Fragment implements
        ...
        AbsListView.OnScrollListener, /*M:*/OnTouchListener {
                ...
                if (keyEvent.getAction() == KeyEvent.ACTION_UP) {
                    if (mKeyInitiatedFromList) {
                        ...
                        if (currentPos >= 0) {
                            ...
                            /// M: Ignore anything that is not a conversation item.
                            /// Could be a footer or other special views. @{
                            final Object item = getAnimatedAdapter().getItem(currentPos);
...
//            outState.putInt(CHOICE_MODE_KEY, mListView.getChoiceMode());
    ...
    private void viewConversation(final int position) {
        ...
        /**
         * M: In some case, the cursor would be null, it should ignore these
         * cases. for example, in local search mode, click a conversation list
         * to view this conversation, and then quick click the back image to
         * quit the the local search, in this case conversation cursor would be
         * reset to null, so some NPE would occur. @{
         */
        if (cursor == null) {
            LogUtils.d(LOG_TAG, "fail to view conversation, due to cursor is null");
            return;
        }
        /** @} */
        /// M: If the UI list count not match to cursor, skip it and request refresh @{
        if (position - getAnimatedAdapter().getPositionOffset(position) >= cursor.getCount()) {
            LogUtils.w(LOG_TAG,
                    "unable to open exceed conv at cursor pos=%s cursor=%s getPositionOffset=%s",
    ...
    public void requestDelete(int actionId, final Collection<Conversation> conversations,
        ...
        if (mListView.getSwipeAction() == actionId && /* M: do not swipe when id was 0*/ actionId != 0) {
    ...
    public void onConversationListStatusUpdated() {
        ...
        /// M: add params of current footer view shown state
    ...
    private void hideLoadingViewAndShowContents() {
        ...
        /// M: add params of current footer view shown state
        final boolean showFooter = mFooterView.updateStatus(cursor,
                mListAdapter.getFooterViewShowState());        // Update the sync status bar with sync results if needed
        checkSyncStatus();
        mListAdapter.setFooterVisibility(showFooter);
        mLoadingViewPending = false;
    ...
    private void setSwipeAction() {
        ...
        LogUtils.d(LOG_TAG, "setSwipeAction.swipeSetting : %d", swipeSetting);
    ...
    private void onCursorUpdated() {
        ...
        /**
         * M: When selectSet is not null, update the selectSet values to make
         * sure having the latest data.
         */
        if (mListAdapter.isInCabMode()) {
            mListAdapter.updateSelectSet();
        }
        /** @} */
        ...
        if (newCursor != null) {
            ...
            /// M: Show total count for remote search
    ...
    /**
     * M: The sync status bar is shown by default, in some case, for example no
     * network, it shouldn't do any update, so it should hide the sync status
     * bar.
     */
