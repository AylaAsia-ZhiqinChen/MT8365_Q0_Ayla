...
public class ConversationViewFragment extends AbstractConversationViewFragment implements
    ...
    public static ConversationViewFragment newInstance(Bundle existingArgs,
        ...
        f.setArguments(args);
        /// M: fixes bug that we can not get conversation until fragment has call onCreated().
        f.mConversation = conversation;
        return f;
    }
    ...
    public void onAccountChanged(Account newAccount, Account oldAccount) {
        ...
        // depend on settings
        /// M: Just do the judgement in case the Adapter is null
        if (null != mAdapter) {
            mAdapter.notifyDataSetChanged();
        } else {
            LogUtils.w(LOG_TAG, "IN CVF.onAccountChanged, the adapter is null and we do nothing,"
                    + "this may cause the wrong account is displayed.");
        }
    }
    ...
    protected void loadContent() {
        /// M: Because this function call has been deferred from onActivityCreated, there was a
        /// risk that the loader has been stopped before initLoader here. Thus, the loader's
        /// callback will never be called, and the loading lost.
        /// e.g. User rotate device and pause the activity immediately.
        /// Therefore, using restartLoader instead of initLoader when this case happened. @{
        boolean messageLoaderStopped = false;
        if (getLoaderManager().getLoader(MESSAGE_LOADER) != null) {
            messageLoaderStopped = !getLoaderManager().getLoader(MESSAGE_LOADER).isStarted();
        }
        if (messageLoaderStopped) {
            LogUtils.d(LOG_TAG,
                    "SHOWCONV: CVF MESSAGE_LOADER stopped, loadContent by restart loader (%s)",
                    this);
            getLoaderManager().restartLoader(MESSAGE_LOADER, Bundle.EMPTY,
                    getMessageLoaderCallbacks());
        /// @}
        } else {
            getLoaderManager()
                    .initLoader(MESSAGE_LOADER, Bundle.EMPTY, getMessageLoaderCallbacks());
        }
    }
    ...
    /**
     * M: when attachments layout changed, we have to update whole conversation and render correct
     * UI layout.
     */
    @Override
    public void onAttachmentsLayoutChanged() {
        LogUtils.d(LOG_TAG, "CONV RENDER: attachments layout changed, render content. (%s)",
                this);
        renderContent(getMessageCursor());
    }
    ...
    public boolean onKey(View view, int keyCode, KeyEvent keyEvent) {
        ...
        if (mOriginalKeyedView != null) {
            ...
            if (keyCode == KeyEvent.KEYCODE_BACK
                    ...
                    /// M: avoid the subject and folder view's consume back key event. @{
    ...
    public class ConversationWebViewClient extends AbstractConversationWebViewClient {
        ...
        public WebResourceResponse shouldInterceptRequest(WebView view, String url) {
            ...
            if (cm != null) {
                ...
                LogUtils.d(LOG_TAG, "shouldInterceptRequest uri=%s cm=%s, response %s", uri, cm,
                        response);
                if (response != null) {
                    return response;
                }
    ...
    private class MailJsBridge {
        ...
        public void onInlineAttachmentsParsed(final String[] urls, final String[] messageIds) {
            try {
                /// M: Fix timing issue, no need post run the inline parsed callback. Cause it
                /// would be used immediately in shouldInterceptRequest on background thread.
...
//                    public void go() {
                        try {
                            for (int i = 0, size = urls.length; i < size; i++) {
                                ...
                                LogUtils.d(LOG_TAG,
                                        "onInlineAttachmentsParsed url : %s messageid : %s",
                                        urls[i], messageIds[i]);
                            }
...
//                });
    ...
    private boolean processInPlaceUpdates(MessageCursor newCursor, MessageCursor oldCursor) {
        ...
        while (true) {
            ...
            /// M: update changed message bodies in-place, and add situation
            // for exchange partial download, also add situation for inline
            // attachment download finish update
            if (!TextUtils.equals(newMsg.bodyHtml, oldMsg.bodyHtml) ||
                    !TextUtils.equals(newMsg.bodyText, oldMsg.bodyText) ||
                    oldMsg.flagLoaded != newMsg.flagLoaded ||
                    !TextUtils.equals(newMsg.attachmentsJson, oldMsg.attachmentsJson)) {
        ...
        if (!changedOverlayPositions.isEmpty()) {
            /// M: make to update message footer view
            mAdapter.notifyDataSetChanged();
    ...
    public void onConversationUpdated(Conversation conv) {
        ...
        mConversation = conv;
        /// M: update saveInstance argument at same time to avoid data inconsistent.
        getArguments().putParcelable(ARG_CONVERSATION, conv);
        LogUtils.d(LOG_TAG, "____________________________onConversationUpdated, headerview: %s",
                headerView);
        if (headerView != null) {
