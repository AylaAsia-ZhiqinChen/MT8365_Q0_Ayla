...
public class ConversationPagerController {
     ...
     * <p>
     * Side Effect: set true, the fragment's position would become constant value(0 or -2) during
     * singleton mode,thus, the fragments save/restore would not work normally, fragment leakage
     * would happen.
     */
    ...
    protected ConversationPagerAdapter createPagerAdapter(Account account, Folder folder,
        ...
        return new ConversationPagerAdapter(mPager.getContext(), mFragmentManager,
    ...
    public boolean dispatchKeyEvent(KeyEvent event) {
        if (null != mPagerAdapter) {
            LogUtils.v(LogTag.getLogTag(), "ConvPagerController dispatchKeyEvent %s", event);
            return mPagerAdapter.dispatchKeyEvent(event);
        }
        return false;
    }
}
