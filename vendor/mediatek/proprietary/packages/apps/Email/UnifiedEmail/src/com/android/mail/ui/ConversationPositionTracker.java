...
public class ConversationPositionTracker {
    ...
    private int calculatePosition() {
        ...
        if (foundPosition >= 0) {
            ...
            /**
             * M: If it couldn't find the current conversation from
             * ConversationCursor yet, it should reset the position of current
             * conversation to 0, because ConversationPager maybe enter detach
             * mode.
             * ex.
             * 1. View a mail
             * 2. use "adb shell input keyevent 84" to force enter local-search
             *    mode.
             * 3. Input some key words to search some mails, but the current
             *    conversation wasn't included in, so the ConversationPager
             *    would enter detach mode.
             * 4. Re-input some key words to search again, this time,
             *    the current conversation should be included in.
             * 5. Touch "mail box" indicator on conversation header view or
             * "move to" item of option menu once.
             * 6. Re-input some key words to search again, this time,
             *    the current conversation shouldn't be included in again.
             * 7. Touch "mail box" indicator on conversation header view or
             * "move to" item of option menu twice, and then JE would occur.
             *
             * @{
             */
        } else {
            mConversation.position = 0;
            LogUtils.d(LOG_TAG, "ConversationPositionTracker: reset to 0, %s",
                    mConversation);
            /** @} */
        }
