...
public class WidgetService extends RemoteViewsService {
    ...
    private static final String LOG_TAG = LogTag.getLogTag() + "/Widget";

    @Override
    public RemoteViewsFactory onGetViewFactory(Intent intent) {
        LogUtils.d(LOG_TAG, "onGetViewFactory : %s", intent.toString());
    ...
    public static void configureValidWidgetIntents(Context context, RemoteViews remoteViews,
        ...
        /**
         * M: We only need bind ViewFactory to the widget id and folder uri, so don't set other fields into intent data,
         * such as folderDisplayName , account,etc. Otherwise, the changes of other field would cause framework create a
         * new ViewFactory, this would cause a lot of race condition errors and leakages such as ViewFacotry and
         * loaders, etc. @{
         */
        intent.putExtra(AppWidgetManager.EXTRA_APPWIDGET_ID, appWidgetId);
        intent.putExtra(BaseWidgetProvider.EXTRA_FOLDER_URI_STRING, folderUri.toString());
        Uri dataUri = Uri.parse(intent.toUri(Intent.URI_INTENT_SCHEME));
        // only appWidgetId and folderUri should be set in Data
        intent.setData(dataUri);
        /// M: Log the folder display name for debugging.
        LogUtils.d(
                LOG_TAG,
                "configureValidWidgetIntents=>intnet: %s , data : %s, folderDisplayName: %s",
                intent, dataUri.toString(), folderDisplayName);
        ...
        /// M: modify widget_header to widget_folder, widget_header is the parent layout.
        /// It covered and response of the click of entire view, even user clicked the empty area.
        remoteViews.setOnClickPendingIntent(R.id.widget_folder, clickIntent);
        ...
        /**
         * M: Use the appWidgetId as the requestCode to make sure creating
         * different PendingIntent record for AMS, and avoid different widget
         * use the same PendingIntent and cause all the widgets use the same folderUri. @{
         */
    ...
    protected boolean isAccountValid(Context context, Account account) {
        if (account != null) {
            ...
            /// M: Treat combined account is valid when email has at least one account.
            LogUtils.d(LOG_TAG, "Combined account uri(%s)", account.uri.toString());
            return accounts.length > 0 && account.uri.toString().contains(
                    "content://com.android.email.provider/uiaccount/268435456");
        }
        /// M: Account is null mostly set when creating new MailFactory.
        LogUtils.d(LOG_TAG, "Account is null");
        return false;
    }
    ...
    protected static class MailFactory
        ...
        /// M: remove the final modifier, we need update this filed in some case.
        ...
        public void onCreate() {
            ...
            /// M: Check for valid account before saving widget config preference.
            if(mService.isAccountValid(mContext, mAccount)) {
                saveWidgetInformation(mContext, mAppWidgetId, mAccount, mFolderUri.toString());
            }
            else {
                /// M: Account was mostly deleted when creating new MailFactory.
                LogUtils.d(LOG_TAG, "Widget pref not saved because of an invalid aacount");
            }
            ...
            mAccountLoader = new CursorLoader(mContext, mAccount.uri,
                    UIProvider.ACCOUNTS_PROJECTION/* M:ACCOUNTS_PROJECTION_NO_CAPABILITIES*/, null, null, null);
        ...
        public void onDestroy() {
            LogUtils.d(LOG_TAG, "MailFactory(%s) : onDestroy()", this);
        ...
        public void onDataSetChanged() {
            ...
            LogUtils.d(LOG_TAG, "MailFactory(%s) : onDataSetChanged()", this);
            mFolderUpdateHandler.scheduleTask();
        }
        ...
        public int getCount() {
            synchronized (sWidgetLock) {
                ...
                /// M: add log
                LogUtils.d(LOG_TAG, "getCount : count = %d : cursorCount : %d : mFolderCount : %d mShouldShowViewMore = %s",
                        count, cursorCount, mFolderCount, mShouldShowViewMore);
        ...
        public RemoteViews getViewAt(int position) {
            synchronized (sWidgetLock) {
                ...
                if (mConversationCursor == null || mConversationCursor.isClosed()
                    ...
                    /// M: add log
                    LogUtils.d(LOG_TAG, "Show View More conversations : position = %d : getConversationCount"
                            + " : %d : mShouldShowViewMore = %s", position, getConversationCount(), mShouldShowViewMore);
                ...
                LogUtils.d(LOG_TAG, "getViewAt %s", position);
                return remoteViews;
            }
        }
        ...
        public void onLoadComplete(Loader<Cursor> loader, Cursor data) {
            ...
            /// M: add log for widget updating debugging. @{
            String loaderName = null;
            if (loader == mFolderLoader) {
                loaderName = "mFolderLoader";
            } else if (loader == mConversationCursorLoader) {
                loaderName = "mConversationCursorLoader";
            } else if (loader == mAccountLoader) {
                loaderName = "mAccountLoader";
            }
            LogUtils.d(LOG_TAG, "MailFactory(%s) : %s : onLoadComplete : mAppWidgetId : %d"
                    + " : mFolderDisplayName : %s isDataValid : %s getCount : %s", this, loaderName, mAppWidgetId,
                    mFolderDisplayName, isDataValid(data), data == null ? null : data.getCount());
            /// @}
            ...
            if (!mService.isWidgetConfigured(mContext, mAppWidgetId, mAccount)) {
                BaseWidgetProvider.updateWidget(mContext, mAppWidgetId, mAccount, mFolderType,
                        mFolderCapabilities, mFolderUri, mFolderConversationListUri,
                        mFolderDisplayName);
                /// M: If do full update, there is no need to do anything else,
                // just only return.
                /// M: add log for debugging.
                LogUtils.d(LOG_TAG, "widget(%d) isn't configured, just return",
                        mAppWidgetId);
                return;
            }

            /// M: Need re-notify if folder count changed @{
            ...
            if (loader == mFolderLoader) {
                if (!isDataValid(data)) {
                    ...
                    BaseWidgetProvider.updateWidget(mContext, mAppWidgetId, mAccount, mFolderType,
                            ...
                            mFolderDisplayName);
                    /// M: add log for debugging.
                    LogUtils.d(LOG_TAG, "data is invalid, just return, mAppWidgetId=%d",
                            mAppWidgetId);
                ...
                /// M: Need re-notify if folder count changed @{
                final int newFolderCount = data.getInt(UIProvider.FOLDER_TOTAL_COUNT_COLUMN);
                folderCountChanged = mFolderCount != newFolderCount;
                mFolderCount = newFolderCount;
                /// @}
                /// also need update folder name field.
                mFolderDisplayName = folderName;
                /// M: add log
                LogUtils.d(LOG_TAG,
                        "MailFactory : update folder (%s) total count : %d",
                        folderName, mFolderCount);
                ...
                LogUtils.d(
                        LOG_TAG,
                        "mConversationCursorLoader returned, update widget: %d",
                        mAppWidgetId);

            } else if (loader == mAccountLoader) {
                // / M: we need update mAccount, Cause it would has been
                // changed, such as account name @{
                if (isDataValid(data)) {
                    mAccount = Account.builder().buildFrom(data);
                } else {
                    LogUtils.e(LOG_TAG, "Account isn't valid, old is %s",
                            mAccount);
                }
                BaseWidgetProvider.updateWidget(mContext, mAppWidgetId,
                        mAccount, mFolderType, mFolderCapabilities, mFolderUri,
                        mFolderConversationListUri, mFolderDisplayName);
                /// If do full update, just only return.
                return;
                /// @}
            }

            /// M: move these code here, cause the empty view always need to be filled no matter
            /// what the loader was. @{
            if (mConversationCursor == null || mConversationCursor.getCount() == 0) {
                ...
                appWidgetManager.partiallyUpdateAppWidget(mAppWidgetId, remoteViews);
                /// M: Need to notify to update
                if (folderCountChanged) {
                    appWidgetManager.notifyAppWidgetViewDataChanged(mAppWidgetId,
                            R.id.conversation_list);
                  /// M: add log for debugging.
                    LogUtils.d(LOG_TAG, "folder changed, update widget: %d",
                            mAppWidgetId);
                }
            }
            /// @}
        ...
        private class FolderUpdateHandler extends DelayedTaskHandler {
            ...
            protected void performTask() {
                ...
                if (mFolderLoader != null) {
                   /** M: We should reset the folderLoader, so it will reload folders data.
                     * By do this, folder name in widget can change when switching language,
                     * rather than shows the cached data of mFolderLoader.
                     */
                    mFolderLoader.reset();
