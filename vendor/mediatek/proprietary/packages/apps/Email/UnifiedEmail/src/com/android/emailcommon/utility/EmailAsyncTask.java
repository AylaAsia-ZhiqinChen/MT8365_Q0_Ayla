...
public abstract class EmailAsyncTask<Params, Progress, Result> {
    ...
    public static final Executor SERIAL_EXECUTOR_FOR_AUTO_CLEAR_ATTACH = new EmailSerialExecutor();
    /// @}

    /**
     * M: adding for using independent customized executors for specified email task.
     */
    private static class EmailSerialExecutor implements Executor {
    ...
    private static class InnerTask<Params2, Progress2, Result2>
        ...
        /// M: Change mOwner to non-constant to reset it if necessary
        private EmailAsyncTask<Params2, Progress2, Result2> mOwner;
        ...
        public void onCancelled(Result2 result) {
            ...
            /// M: Reset mOwner to null to avoid hold EmailAsyncTask object
            mOwner = null;
        }
    ...
    /**
     * M : execute on a customized executor.
     *
     * @see AsyncTask#execute
     */
    public final EmailAsyncTask<Params, Progress, Result> executeExecutor(Executor executor, Params... params) {
        return executeInternal(executor, false, params);
    }
