...
public class ConversationViewHeader extends LinearLayout implements OnClickListener,
    ...
    public void bind(ConversationHeaderItem headerItem) {
        ...
        mConversation = mHeaderItem.getConversation(); /// M: conversation
    ...
    public void onConversationUpdated(Conversation conv) {
        /// M: Add logs @{
        LogUtils.d(LOG_TAG, "CVH. onConversationUpdated for conversation: %s", conv);
        /// @}
        ...
        if (mHeaderItem != null) {
            /// M: Update this header item's conversation here, cause conversation
            // has been updated @{
            mHeaderItem.setConversation(mConversation);
            /// @}
    ...
    public boolean onLongClick(View v) {
        /// M: Adjust null for eml attachment
        if (mCallbacks != null) {
            final DialogFragment frag = CopySubjectDialog.newInstance(mSubjectAndFolderView
                    .getSubject());
