...
//                ev.getActionMasked(), ev.getX(), ev.getY(), ev.getPointerCount());
    ...
    /**
     * M: Re-position overlap items in some case such as zoom in a special conversation.
     * The spacer's height would be shorter than total item's. Then items must be overlap.
     * @param spacerTop spacer Y position
     * @param totalItemsHeight total height of all items in the spacer
     * @param currentSpacerItems items index
     * @param forceGravity
     * @param postAddView
     */
    private void repositionOverlapOverlays(int spacerTop, int totalItemsHeight,
            ArrayList<Integer> currentSpacerItems, int forceGravity, boolean postAddView) {
        OverlayPosition itemPos = null;
        for (int itemIndex : currentSpacerItems) {
            final int regionTop = spacerTop;
            // The first item's (itemPos is null) bottom should be actual items total height.
            final int regionBottom = itemPos == null ? spacerTop + totalItemsHeight : itemPos.top;
            ConversationOverlayItem adapterItem = mOverlayAdapter.getItem(itemIndex);
            itemPos = calculatePosition(adapterItem, regionTop, regionBottom, forceGravity);
            positionOverlay(itemIndex, itemPos.top, itemPos.bottom, postAddView);
        }
    }
    ...
    private void positionOverlays(int y, boolean postAddView) {
        ...
        while (spacerIndex >= 0 && adapterLoopIndex >= 0) {
            ...
            /// M: use for save items and total height in current spacer. @{
            final ArrayList<Integer> currentSpacerItems = new ArrayList<Integer>();
            ...
            traceLayout("in loop, spacer=%d t/b=%d/%d, overlay=%d, t/b=%d/%d (%s)", spacerIndex,
                    spacerTop, spacerBottom, adapterIndex, itemPos.top, itemPos.bottom,
                    adapterItem);
            positionOverlay(adapterIndex, itemPos.top, itemPos.bottom, postAddView);

            /// M: save this item's index and height.
            currentSpacerItems.add(adapterIndex);
            totalItemsHeight += adapterItem.getHeight();
            ...
            while (--adapterLoopIndex >= 0) {
                ...
                if (spacerIndex > 0 && !adapterItem.isContiguous()) {
                    /// M: total height of current spacer's items has out of bound. The will cause
                    /// items overlap with others. So re-do position with new spacer position. @{
                    if (totalItemsHeight > spacerBottom - spacerTop) {
                        traceLayout("break loop, totalItemsHeight=%d, spacer t/b=%d/%d," +
                                " spacer height=%d)", totalItemsHeight, spacerTop, spacerBottom,
                                spacerBottom - spacerTop);
                        repositionOverlapOverlays(spacerTop, totalItemsHeight, currentSpacerItems,
                                forceGravity, postAddView);
                    }
                    /// @}
