...
public class Message implements Parcelable, HtmlMessage,
    /** M: [LBE] enable clone a message @{ */Cloneable /** @}*/ {
    /**
    ...
    /**
     * M: @see UIProvider.MessageColumns#FLAG_LOADED
     */
    public int flagLoaded;
    /**
     * M: @see UIProvider.MessageColumns#STATE
     */
    public int state;
    ...
    public boolean isEqual(Message o) {
        return TextUtils.equals(this.getFrom(), o.getFrom()) &&
                /// M: Add flagLoad for exchange partial download
                this.flagLoaded == o.flagLoaded &&
    ...
    public Message(Cursor cursor) {
        if (cursor != null) {
            ...
            /// M: It should bring the variable attachmentJson correspondence
            // with variable hasAttachments, in some case, the hasAttachments is
            // false, but the attachmentJson is not null, it would make the
            // conversation view display abnormally. For example, for pop3
            // protocol, view one mail firstly, and then perform download
            // remaining, if the message loader do re-query after the
            // attachment saved but before the flag
            // EmailContent.MessageColumns.FLAG_ATTACHMENT updated, the issue
            // would occur. Add some sleep Utilities.copyOneMessageToProvider
            // would reproduce this issue easily. @{
            if ((!hasAttachments && attachmentsJson != null)
                    || (hasAttachments && attachmentsJson == null)) {
                LogUtils.e(TAG,
                        "Conflicting, hasAttachments=%s, attachmentsJson=%s",
                        hasAttachments, attachmentsJson);
            }
            attachmentsJson = hasAttachments ? attachmentsJson : null;
            // @}
    ...
    public Message(Context context, MimeMessage mimeMessage, Uri emlFileUri)
        ...
        /// M: fix bug, using toString instead of toHeader. Cause toHeader would encoding address.
        setFrom(Address.toString(mimeMessage.getFrom()));
        setTo(Address.toString(mimeMessage.getRecipients(
                com.android.emailcommon.mail.Message.RecipientType.TO)));
        setCc(Address.toString(mimeMessage.getRecipients(
                com.android.emailcommon.mail.Message.RecipientType.CC)));
        setBcc(Address.toString(mimeMessage.getRecipients(
                com.android.emailcommon.mail.Message.RecipientType.BCC)));
        setReplyTo(Address.toString(mimeMessage.getReplyTo()));
    ...
    public static String[] tokenizeAddresses(String addresses) {
         ...
         */
        Address[] unpackedAddresses = Address.fromHeader(addresses);
        if (unpackedAddresses.length == 0) {
            return new String[0];
        }
        /// M: We has unpack addresses above, so can not use Rfc822Tokenizer.tokenize() any more.
        /// Because the address has been tokenized in parse(), and tokenize() could not process a tokenized string again.
        /// Otherwise, some special character like \ would lost.
        /// Finally, we need revert the tokenized string to suitable one for tokenize again by call Rfc822Token.toString
        String[] strings = new String[unpackedAddresses.length];
        for (int i = 0; i < unpackedAddresses.length; i++) {
            Rfc822Token token = new Rfc822Token(unpackedAddresses[i].getSimplifiedName(),
                    unpackedAddresses[i].getAddress(), null);
    ...
    public String getBodyAsHtml() {
        ...
        if (!TextUtils.isEmpty(bodyHtml)) {
            ...
            LogUtils.d(TAG, "getBodyAsHtml parse link for text body, messageId %d ", id);
            /// M: Support parse link feature.
            /// There are two reason not adding parse link option in creating body file:
            /// 1.want to keep text orignal body content.
            /// 2.workaroud Html converting conflict bug with parse link.
            /// @{
            LinkParserTask.addWebLinkSpan(spannable);
            /// @}
