...
function rewriteRelativeImageSrc(imgElement) {
    ...
    if (src.substring(0, 4) == "cid:") {
        /// M: just modified orignal src value, make sure the resource could be reload
        /// everytime call this function. @{
        src = src + "?reload_trigger=" + Math.random();
        imgElement.src = src;
        /// @}
        return src;
    }
...
function setupContentReady() {
    ...
    if (ENABLE_CONTENT_READY) {
        ...
        /// M: Moved from template_conversation_lower.html to hook appropriately
...
function unblockImages(messageDomIds) {
    ...
    for (j = 0, len = messageDomIds.length; j < len; j++) {
        ...
        msgContentDiv = document.querySelector("#" + messageDomId + " > .mail-message-content");
        /// M: Add class mail-show-images which indicates should show images. @{
        if (msgContentDiv) {
            msgContentDiv.classList.add("mail-show-images");
            console.log("unblockImages : add class mail-show-images");
        } else {
            // There's no message div, just skip it.
            console.log("unblockImages : Mail message content for msg " + messageDomId +
                    " to replace not found.");
        }
        /// @}
