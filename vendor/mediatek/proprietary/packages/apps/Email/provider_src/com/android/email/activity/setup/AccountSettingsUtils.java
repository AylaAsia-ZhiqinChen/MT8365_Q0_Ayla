...
public class AccountSettingsUtils {
   ...
   /**
     ...
     * capability.
     * M: Check email plugin config first.
     *
    ...
    public static Provider findProviderForDomain(Context context, String domain) {
        Provider p = EmailPluginUtils.findProviderForDomain(context, domain);

        if (p == null) {
            p = findProviderForDomain(context, domain, R.xml.providers_product, null);
        }
        if (p == null) {
            p = findProviderForDomain(context, domain, R.xml.providers, null);
        }
        return p;
    }

    /**
     * M: Search the list of known Email providers looking for one that matches the user's email
     * domain.  We check Vendor supplied values and CT plugin values first, then we look in
     * providers_product.xml, and finally by the entries in platform providers.xml.  This provides
     * a nominal override capability.
     *
     * A match is defined as any provider entry for which the "domain" attribute and "protocol prefix" matches.
     * @param context
     * @param domain
     * @param protocol
     * @return
     */
    public static Provider findProviderForDomainProtocol(Context context, String domain, String protocol) {
        // If no protocol prefix specific
        if (TextUtils.isEmpty(protocol)) {
            return findProviderForDomain(context, domain);
        }

        Provider p = EmailPluginUtils.findProviderForDomain(context, domain, protocol);

        if (p == null) {
            p = findProviderForDomain(context, domain, R.xml.providers_product, protocol);
        }
        if (p == null) {
            p = findProviderForDomain(context, domain, R.xml.providers, protocol);
        }
     ...
     * @param M: protocol The protocol which was finding(adding to support finding manual configurations).
     * @return suitable Provider definition, or null if no match found
     */
    /*Package*/ static Provider findProviderForDomain(
            Context context, String domain, int resourceId, String protocol) {
        try {
            ...
            while ((xmlEventType = xml.next()) != XmlResourceParser.END_DOCUMENT) {
                if (xmlEventType == XmlResourceParser.START_TAG
                    ...
                    /// M: supports manual setup configuration @{
                    if (protocol != null) {
                        URI uri = new URI(provider.incomingUriTemplate);
                        if (uri != null) {
                            String scheme = uri.getScheme();
                            String[] schemeParts = scheme.split("\\+");
                            String supportProtocol = schemeParts[0];
                            if (!protocol.equals(supportProtocol)) {
                                // if the protocol not match, just ignore this provider and continue searching
                                provider = null;
                            }
                        }
                    }
    ...
    /**
     * M: Auto setup the manual configurations for email account.
     * Search the default server configuration when user setup account manually.
     * We check plugin first , then manual_setup_helper, then we look in providers_product.xml,
     * and finally by the entries in platform providers.xml.  This provides a nominal override
     * capability.
     * @param context The application context.
     * @param domain The domain portion of the user's email address.
     * @param protocol The protocol that user selected.
     * @return suitable server uri, or null if no match found.
     */
    public static Provider findManualInfoForDomain(Context context, String domain, String protocol) {
        Provider p = EmailPluginUtils.findProviderForDomain(context, domain, protocol);

        if (p == null) {
            p = findProviderForDomain(context, domain, R.xml.manual_setup_helper, protocol);
        }
