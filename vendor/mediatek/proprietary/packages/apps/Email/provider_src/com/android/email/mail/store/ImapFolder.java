...
//                        + " not yet supported.");
            ...
            /** @} */
            /** M: get to support text/calendar @{ */
            boolean isCalendarMsg = false;
            if (MimeUtility.mimeTypeMatches(mimeType, MimeUtility.MIME_TYPE_CALENDAR)) {
                LogUtils.w(LogUtils.TAG, "BODYSTRUCTURE " + MimeUtility.MIME_TYPE_CALENDAR
                        + " now supported but still not so complete");
                isCalendarMsg = true;
            }
            /** M: @} */
            ...
            /**
             * M: Parse and set required content-disposition for message/rfc822
             * and text/calendar part @{
             */
            } else if (isRfc822Msg || isCalendarMsg) {
                String fileName = String.format("%s", isCalendarMsg ?
                        "invite.ics" : bs.getListOrEmpty(7).getStringOrEmpty(1));
                if (!TextUtils.isEmpty(fileName)) {
                    if (fileName.startsWith("\"") && fileName.endsWith("\"")) {
                        fileName = fileName.substring(1, fileName.length() - 1);
                    }
                    // Set message/rfc822 or text/calendar content-disposition: filename
                    contentDisposition.append(String.format(MimeHeader.HEADER_CONTENT_DISPOSITION
                            + ": attachment; %s=\"%s\"", "filename", fileName));
                }
            }
            LogUtils.d(LogUtils.TAG, "BODYSTRUCTURE with size %d", size);
            /** @} */
    ...
    /// M: get the mailbox id of the folder
    public long getMailboxId() {
        return mMailbox.mId;
    }

    /// M: set the local mailbox associated with this remote folder
    public void setLocalMailbox(Mailbox localMailbox) {
        mMailbox = localMailbox;
    }

    /// M: get the local set of hashes that can be used to track dirtiness
    public Object[] getLocalHashes() {
        return mHash;
    }

    /// M: set the local set of hashes that can be used to track dirtiness
    public void setLocalHashes(Object[] localHashes) {
        mHash = localHashes;
    }
}
