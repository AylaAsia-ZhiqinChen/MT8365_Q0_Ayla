...
public class AttachmentInfo {
    ...
    public static final String[] PROJECTION = {
            ...
            /// M: add content uri for viewable attachemnt check.
    ...
    public AttachmentInfo(Context context, Attachment attachment) {
        this(context, attachment.mId, attachment.mSize, attachment.mFileName, attachment.mMimeType,
                ...
                /* M: add content uri for viewable attachemnt check.*/ attachment.getContentUri());
    }
    ...
    public AttachmentInfo(Context context, Cursor c) {
        this(context, c.getLong(COLUMN_ID), c.getLong(COLUMN_SIZE), c.getString(COLUMN_FILENAME),
                ...
                c.getInt(COLUMN_FLAGS), /* M: add content uri for viewable attachemnt check.*/
                c.getString(COLUMN_CONTENT_URI));
    }
    ...
    public AttachmentInfo(Context context, AttachmentInfo info) {
        this(context, info.mId, info.mSize, info.mName, info.mContentType, info.mAccountKey,
                info.mFlags,
                /* M: add content uri for viewable attachemnt check.*/ info.mContentUri);
    }
    ...
    public AttachmentInfo(Context context, long id, long size, String fileName, String mimeType,
        ...
        /** M: disable installable attachment download check, always allow download.
        ...
        /// M: Check to see if any activities can view this attachment; if none, we can't view it
        /// Align check condition with UnifiedEmail:
        /// if the mimetype from server is unsupported, check mimetype from extension again.@{
        final Uri uri = mContentUri != null ? Uri.parse(mContentUri) : null;
        /// M: if attachment is installable, such as apk do not check isViewable or not,always
        /// allow download, due to packageInastaller not accept content type uri.@{
        if (!(!TextUtils.isEmpty(extension) &&
                Utility.arrayContains(AttachmentUtilities.INSTALLABLE_ATTACHMENT_EXTENSIONS,
                        extension)) && !MimeType.isViewable(context, uri, mContentType, mName)) {
        /// @}
            canView = false;
            canSave = false;
