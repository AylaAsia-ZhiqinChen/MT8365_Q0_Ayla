...
public class AuthenticatorService extends Service {
    ...
    /// M: Exchange account type defition @{
    public static final String EAS_ACCOUNT_TYPE = "com.android.exchange";
    /// @}
    /// M: Sync Interval set by user @{
    public static final String OPTIONS_EMAIL_SYNC_INTERVAL = "syncInterval";
    /// @}
    class Authenticator extends AbstractAccountAuthenticator {
        ...
        public Bundle addAccount(AccountAuthenticatorResponse response, String accountType,
            ...
            if (options != null && options.containsKey(OPTIONS_PASSWORD)
                ...
                /// M: Get whether account is eas type @{
                boolean isEasAccount = EAS_ACCOUNT_TYPE.equals(accountType);
                /// @}
                ...
                ContentResolver.setSyncAutomatically(account, EmailContent.AUTHORITY,
                        syncEmail);
                /// M: Add(update) periodic sync with the user selected syncInterval @{
                if (syncEmail) {
                        if (options.containsKey(OPTIONS_EMAIL_SYNC_INTERVAL)) {
                            int syncInterval;
                            syncInterval = options.getInt(OPTIONS_EMAIL_SYNC_INTERVAL);
                            // Positive syncInterval needs an update here as setIsSyncable resets
                          if (syncInterval > 0) {
                            // Add(update) periodic syncs
                           ContentResolver.addPeriodicSync(account,
                            EmailContent.AUTHORITY, Bundle.EMPTY,
