...
public class ImapService extends Service {
    ...
    public void onCreate() {
        super.onCreate();
        EmailApplication.checkAndStartForeground(this, 6, "ImapService, onCreate");
...
//            final long queryEndDate = endDate - DateUtils.DAY_IN_MILLIS - DateUtils.HOUR_IN_MILLIS;
        ...
        for (int i = remoteMessages.length - 1; i >= 0; i--) {
            ...
            if (localMessage == null ||
                    /// M: add FLAG_LOADED_ENVELOPE, remove FLAG_LOADED_PARTIAL.
                    (localMessage.mFlagLoaded == EmailContent.Message.FLAG_LOADED_UNLOADED ||
                    localMessage.mFlagLoaded == EmailContent.Message.FLAG_LOADED_ENVELOPE)) {
                unsyncedMessages.add(message);
            }
        }
        LogUtils.d(Logging.LOG_TAG, " unsyncedMessages size " + unsyncedMessages.size());
    ...
    private static void processPendingDeletesSynchronous(Context context, Account account,
        ...
        LogUtils.d(Logging.LOG_TAG,
                "processPendingDeletesSynchronous delete message count [%d]",
                deletes.getCount());
        try {
            ...
            while (deletes.moveToNext()) {
                ...
                if (oldMessage != null) {
                    ...
                    /**
                     * M: some deleted message not sync with server, use delete
                     * flag. @{
                     */
                    final boolean deleteFromTrash = (mailbox.mType == Mailbox.TYPE_TRASH
                            || oldMessage.isDeleteFromServer());
                    /** @} */
    ...
    private static void processPendingDeleteFromTrash(Store remoteStore,
        ...
        if (oldMailbox.mType != Mailbox.TYPE_TRASH && !oldMessage.isDeleteFromServer()) {
            return;
        }
        /** @} */
        ...
        remoteTrashFolder.close(false);
        LogUtils.d(Logging.LOG_TAG, "processPendingDeleteFromTrash [%s] [%s] ",
                oldMailbox.mDisplayName, oldMessage.mServerId);
    }
                ...
                public void messageRetrieved(Message message) {
                    try {
                        ...
                        LegacyConversions.updateMessageFields(localMessage, message, account.mId,
                                mailbox.mId);
                        // Save off the mailbox that this message *really*
                        // belongs in.
                        // We need this information if we need to do more
                        // lookups
                        // (like loading attachments) for this message. See
                        // b/11294681
                        ...
                        // We store the serverId of the source mailbox into
                        // protocolSearchInfo
                        // This will be used by loadMessageForView, etc. to use
                        // the proper remote
                        // folder
                        ...
                        LogUtils.e(Logging.LOG_TAG, me, "Error while copying downloaded message.");
                    } catch (Exception e) {
                        LogUtils.e(Logging.LOG_TAG, e, "Error while storing downloaded message.");
                    }
                }
    ...
    public void onDestroy() {
        LogUtils.e(Logging.LOG_TAG, "onDestroy");
        EmailApplication.checkAndStopForeground(this, "ImapService, onDestroy");
    }
}
