...
public class AttachmentService extends Service implements Runnable {
    ...
    class ServiceCallback extends IEmailServiceCallback.Stub {
        ...
        ContentValues getAttachmentUpdateValues(final Attachment attachment,
            ...
            if (attachment != null) {
                if (statusCode == EmailServiceStatus.IN_PROGRESS) {
                    ...
                    /// M: should not always update the UI_STATE to DOWNLOADING here, user
                    /// cancel downloading we should keep UI_STATE be NOT_SAVED not DOWNLOADING.
                    //values.put(AttachmentColumns.UI_STATE, AttachmentState.DOWNLOADING);
                    if (attachment.mUiState == AttachmentState.DOWNLOADING) {
                        values.put(AttachmentColumns.UI_DOWNLOADED_SIZE, attachment.mSize
                                * progress / 100);
                    }
                }
            }
        ...
        public void loadAttachmentStatus(final long messageId, final long attachmentId,
            ...
            LogUtils.d(
                    LOG_TAG,
                    "messageId %s, attachmentId %s, statusCode %s, progress %s",
                    messageId, attachmentId, statusCode, progress);
            /// @}
    ...
    public static void attachmentChanged(final Context context, final long id, final int flags) {
        ...
        //EmailApplication.checkAndStartForegroundService(context, intent, "attachmentChanged");
        try {
    ...
    public int onStartCommand(final Intent intent, final int flags, final int startId) {
        ...
        EmailApplication.checkAndStartForeground(this, 15, "AttachmentService, onStartCommand");
        } else {
            debugTrace("Received a null intent in onStartCommand");
        }
        EmailApplication.checkAndStopForeground(this, "AttachmentService, onStartCommand");
    ...
    public void onCreate() {
        ...
        EmailApplication.checkAndStartForeground(this, 15, "AttachmentService, onCreate");
    }
    ...
    public void onDestroy() {
        ...
        EmailApplication.checkAndStopForeground(this, "AttachmentService, onDestroy");
    }
    ...
    public void run() {
        ...
        while (!mStop) {
            ...
            if (mDownloadQueue.isEmpty() && (mDownloadsInProgress.size() < 1)) {
                ...
                EmailApplication.checkAndStopForeground(this, "AttachmentService, run");
                stopSelf();
                break;
            }
    ...
    synchronized void processQueue() {
        ...
        /**
         * M: L default design, mDownloadQueue.getNextRequest() will remove the request, event the
         * the request not ready for download, which will break down the request loop.
         * And the other request never start after the the progress one finished.
         *
         * Current modification only loop and check it, keep the original queue. @{
         */
        Iterator<DownloadRequest> iterator = mDownloadQueue.mRequestQueue.iterator();
