...
public class AccountReconciler {
    ...
    public static synchronized void reconcileAccounts(final Context context) {
        /// M: For MOTA, It should get the accounts in email provider firstly,
        // and then get the accounts in account manager. Otherwise, the
        // reconcile would make a mistake to delete the pop account, because the
        // account type of pop account would change from com.android.email to
        // com.android.email.pop3. @{
        ...
        for (Account epAccount : providerAccounts) {
            LogUtils.d(LogUtils.TAG, "Account in email provider, %s", epAccount);
        }
        for (android.accounts.Account amAccount : amAccounts) {
            LogUtils.d(LogUtils.TAG, "Account in account manager, %s",
                    amAccount);
        }
        // @}
    ...
    private static boolean reconcileAccountsInternal(
       ...
       // See if we should have the Eas authenticators enabled.
        ...
        for (final Account providerAccount : emailProviderAccounts) {
            ...
            if (infoForAccount == null || !hasAmAccount(
                ...
                if (performReconciliation) {
                    ...
                    accountsDeleted++;
                    NotificationActionUtils.deleteEmailNotificationChannel(context, providerAccountName);
        ...
        if (!TextUtils.isEmpty(composeActivityName)) {
            ...
            int extComponentCurrentState = context.getPackageManager().
                                                getComponentEnabledSetting(componentExtName);
            LogUtils.e(LogUtils.TAG, " initial enabled state of external composer = "
                    + extComponentCurrentState);
            if ((extComponentCurrentState == PackageManager.COMPONENT_ENABLED_STATE_ENABLED &&
                     !enableCompose) || ((extComponentCurrentState
                            == PackageManager.COMPONENT_ENABLED_STATE_DISABLED ||
                            extComponentCurrentState == PackageManager.COMPONENT_ENABLED_STATE_DEFAULT)
                            && enableCompose)){
                ...
                LogUtils.e(LogUtils.TAG, "Setting external compose activity to "
                    + (enableCompose ? "enabled" : "disabled"));
            } else {
                LogUtils.e(LogUtils.TAG, "do not enable/disable already enabled/disabled compose activity");
            }
            // in Q Internal compose activity will always be enabled but for APK push across different
            // Android versions(in which internal composer is disabled initially by MTK)
            //below case may arise
                        ...
                        == PackageManager.COMPONENT_ENABLED_STATE_DISABLED) && enableCompose)
            {
                ...
                LogUtils.e(LogUtils.TAG, "Enabling internal compose activity to ");
            }
        }
