...
public class SecurityPolicy {
    ...
    public synchronized Policy getAggregatePolicy() {
        /// M: To avoid the following case:
        // 1. delete the current exchange account
        // 2. Add the same exchange account again
        // 3. Then after the account had been setup successfully, the
        // requirement for security update would pop up always.
        // Root cause: Delete the account, it would remove the admin policy and
        // make the email process restart, but PolicyAdimin.onDisabled DPM
        // callback for admin changed wouldn't be implemented, so if it was
        // implemented on the new process, the mAggregatePolicy would be set to
        // Policy.NO_POLICY, the SecurityPolicy.isActive would return true, and
        // the new device policy wouldn't be set to device, so the error would
        // happen.
        if (mAggregatePolicy == null || mAggregatePolicy == Policy.NO_POLICY) {
            ...
            while (c.moveToNext()) {
                ...
                for (android.accounts.Account acct : amAccounts) {
                    if (address != null && address.equalsIgnoreCase(acct.name)) {
                        LogUtils.w(TAG, "Email administration disabled; remove system acount [%s]",
                                address);
