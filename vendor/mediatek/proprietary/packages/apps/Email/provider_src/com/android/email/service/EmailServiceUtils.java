...
public class EmailServiceUtils {
    ...
    public static void killService(Context context, String protocol) {
        ...
        if (info != null && info.intentAction != null) {
            ...
            //context.startService(serviceIntent);
            //EmailApplication.checkAndStartForegroundService(context, serviceIntent, "EmailServiceUtils, killService, info = "+info);
        }
    }
    ...
    public static void startService(Context context, String protocol) {
        ...
        if (info != null && info.intentAction != null) {
            ...
            context.startService(serviceIntent);
                ...
                LogUtils.d(LogUtils.TAG, "startService, unable to start service due to illegal state");
                return;
            }
            //EmailApplication.checkAndStartForegroundService(context, serviceIntent, "EmailServiceUtils, startService, info= "+info);
        }
    }
    ...
    public static void startRemoteServices(Context context) {
        for (EmailServiceInfo info: getServiceInfoList(context)) {
            if (info.intentAction != null) {
                final Intent serviceIntent = getServiceIntent(info);
                //EmailApplication.checkAndStartForegroundService(context, serviceIntent, "EmailServiceUtils, startRemoteServices, info = "+info);
                try {
                context.startService(serviceIntent);
                } catch (IllegalStateException e) {
                    LogUtils.d(LogUtils.TAG, "startRemoteServices, unable to start service due to illegal state");
                    return;
                }
            }
        }
    ...
    public static AccountManagerFuture<Bundle> setupAccountManagerAccount(final Context context,
        ...
        final Bundle options = new Bundle(6);
        ...
        /// M: sync interval option added in bundle
        options.putInt(EasAuthenticatorService.OPTIONS_EMAIL_SYNC_INTERVAL, account.mSyncInterval);
    ...
    public static void enableExchangeComponent(final Context context) {
        if (VendorPolicyLoader.getInstance(context).useAlternateExchangeStrings()) {
            ...
            /// M: No need of Enabling EAS authenticator now. Manifest file takes care of it.
    ...
    public static void disableExchangeComponents(final Context context) {
        ...
        /// M: avoid disabling this to make Exchange always visible in AccountManager List
        //setComponentStatus(context, EasAuthenticatorService.class, false);
    }
