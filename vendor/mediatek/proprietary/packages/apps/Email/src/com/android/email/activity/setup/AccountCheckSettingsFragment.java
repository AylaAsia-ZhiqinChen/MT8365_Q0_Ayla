...
public class AccountCheckSettingsFragment extends Fragment {
    ...
    /* M: update the progress of check Settings from onPostResume rather than onResume */
    public void checkSettingsFragmentcheckResult(){
    ...
    /**
     * M: During Check outgoing settings the Socket might throw SecurityException
     * when checking SMTP settings. As Email TargetSDK is < 23 we should throw this SE
     *
     * To throw the SE modify the signature of AccountCheckTask to return Exception object
     * Exception is parent class of both ME and SE. @{
     */
    private static class AccountCheckTask extends AsyncTask<Void, Integer, Exception> {
    /** @} */
        ...
        public AccountCheckTask(Context context, AccountCheckSettingsFragment callback, int mode,
            ...
            /**
             * M: The setup data may not complete, e.g, for Outgoing checking
             * only init mHostAuthSend, while for Incoming checking only init
             * mHostAuthRecv, refer to
             * {@link AccountSetupIncomingFragment#loadSettings()},
             * {@link AccountSetupOutgoingFragment#loadSettings()}
             *
             * Google default always get address and password from mHostAuthRecv,
             * NPE happend for Outgoing settting.
             * Should recover mHostAuthRecv for this case. @{
             */
            if (mAccount.mHostAuthRecv == null) {
                /// M: generate a new HostAuthRecv if failed to restore it by mHostAuthKeyRecv
                if (mAccount.getOrCreateHostAuthRecv(mContext) == null) {
                    mAccount.mHostAuthRecv = new HostAuth();
                }
            }
            /** @} */
            mStoreHost = mAccount.mHostAuthRecv.mAddress;
        ...
        protected Exception doInBackground(Void... params) {
            try {
                ...
                /// M: return SecurityException for it to be thrown from UI thread.
                // Socket enforce Email SendPermission. We do not handle as we target SDK 21,
                // Catch and return to PostExecute to throw it in UI thread.
                return se;
            }
        }
        ...
        protected void onPostExecute(Exception result) {
            ...
            if (result == null) {
                ...
                /// M: throw SecurityException for framework to handle it.
                throw new SecurityException(result.getCause());
            } else {
                int progressState = STATE_CHECK_ERROR;
                final int exceptionType = ((MessagingException)result).getExceptionType();
