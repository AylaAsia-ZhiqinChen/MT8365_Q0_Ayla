...
public class AccountSetupFinal extends AccountSetupActivity
    ...
    /// M: add to save account creation state, it's used to set result to pending activity. @{
    private static final String SAVESTATE_KEY_ACCOUNT_CREATION_COMPLETE =
            "AccountCreationFragmentComplete";
    private boolean mAccountCreationComplete = false;
    /// @}

    /// M: Some Exchange servers don't support smart reply/forward.
    /// Record them for removing the smart forward flag.
    private static final String[] SERVERS_NOT_SUPPORT_SMART_FORWARD = new String[]{"139.com"};
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        if (savedInstanceState != null) {
            ...
            /// M: restores this state, otherwise activity result would be lost after screen
            /// rotate or other operations. @{
            ...
            ///M: If we're not restoring from a previous state
            // we want to configure the initial screen
            LogUtils.d(LogUtils.TAG, "No previous state, init for setup Account.");
        ...
        if(!(((EmailApplication)getApplication()).getPermissionCheckingStateFlag())){
        ...
        getLoaderManager().initLoader(OWNER_NAME_LOADER_ID, null,
                new AccountDisplayNameLoaderCallbacks());
        ...
        getLoaderManager().initLoader(EXISTING_ACCOUNTS_LOADER_ID, null,
                new ExistedAccountNameLoaderCallbacks());
    }
    }

    /**
     * M: loader to get the owner name with the corresponding email address.
     * if the one has been existed in contacts.
     */
    private class AccountDisplayNameLoaderCallbacks implements
    ...
    /**
     * M: loader to get all existed system account name for duplicate account checking.
     */
    private class ExistedAccountNameLoaderCallbacks implements
    LoaderManager.LoaderCallbacks<Cursor> {
        @Override
        public Loader<Cursor> onCreateLoader(final int id, final Bundle args) {
    ...
    /// M: After onResume, we need to update the progress on UI .
    @Override
    protected void onPostResume() {
        ...
        if (f != null) {
            f.checkSettingsFragmentcheckResult();
        }
    }
    ...
    public void onSaveInstanceState(@NonNull Bundle outState) {
        ...
        /// M: save the account creation complete state.
        outState.putBoolean(SAVESTATE_KEY_ACCOUNT_CREATION_COMPLETE, mAccountCreationComplete);
        /// M: save the status of auto discover
        outState.putBoolean(SAVESTATE_KEY_SKIP_AUTO_DISCOVER, mSkipAutoDiscover);
    }
    ...
    private void updateContentFragment(boolean addToBackstack) {
        ...
        /// M: Add for debug logging about AccountSetupBasics
        LogUtils.d(LogUtils.TAG, "updateContentFragment account, proceed status [%s]", mState);
        switch (mState) {
            case STATE_BASICS:
            ...
            case STATE_MANUAL_INCOMING:
                /// M: try to find incoming manual setup configurations.
                /// TODO : don't set default HostAuth value if auto-discover been enabled. Now, this
                /// feature not really work. @{
                final Account account = mSetupData.getAccount();
                final HostAuth recvAuth = account.getOrCreateHostAuthRecv(this);
                if (TextUtils.isEmpty(mSetupData.getIncomingProtocol())) {
                    mSetupData.setIncomingProtocol(this, recvAuth.mProtocol);
                }
                setupManualInfo(account, mSetupData.getIncomingProtocol());
                // Populate credential again, because it is reseted in above step.
        ...
        /// M: Use commitAllowingStateLoss instead of commit, because commit
        /// can't be invoked after onSaveInstance.
     ...
     */
    protected AccountSetupFragment getContentFragment() {
    ...
    protected void proceed() {
        LogUtils.d(LogUtils.TAG, "validation account, proceed status [%s]", mState);
        mIsProcessing = false;
        /** M: not suitable for MTK solution. @{ */
        ...
        switch (mState) {
            ...
            case STATE_CHECKING_PRECONFIGURED:
                if (mPreConfiguredFailed) {
                    if (mPasswordFailed) {
                        ...
                        mState = STATE_MANUAL_INCOMING;
                        /// M: Clear the flag and reset the incoming protocal to
                        // keep using manual incoming to avoid the following case.
                        // 1. Pre-configuration failed
                        // 2. Enter into manual incoming setting
                        // 3. back to credentials
                        // 4. press next button, then due to the port isn't
                        // configured, the JE would happen.
                        mIsPreConfiguredProvider = false;
                        mSkipAutoDiscover = true;
                        updateContentFragment(true /* addToBackstack */);
                    }
            ...
            case STATE_CHECKING_OUTGOING:
                ...
                break;
            case STATE_OPTIONS:
                /// M: it's a double insurance before account creation, checking duplicated account
                /// again, because the first checking could be bypassed in some special cases.
                /// such as starting multiple account setup concurrently. @{
                final String existingAccountName =
                mExistingAccountsMap != null ? mExistingAccountsMap
                        .get(mSetupData.getEmail()) : null;
    ...
    private boolean onBasicsComplete() {
        ...
        final String[] emailParts = email.split("@");
        final String domain = emailParts[1].trim();
        /// M: Here we should try to find specific provider if account type is set, ex. only find pop
        // provider for pop account, etc. Then try to find a provider for the domain if no appropriate
        // protocol's provider is supplied. @{
        String protocol = mSetupData.getAmProtocol();
        if (!TextUtils.isEmpty(protocol)) {
            mProvider = AccountSettingsUtils.findProviderForDomainProtocol(
                    this, domain, protocol);
        }
        if (TextUtils.isEmpty(protocol) || mProvider == null) {
            mProvider = AccountSettingsUtils
                    .findProviderForDomain(this, domain);
        }

        if (!FeatureOption.MTK_EXCHNAGE_SUPPORT) {
            if (mProvider != null) {
                mProvider.expandTemplates(mSetupData.getEmail());
                ...
                if(info != null && "com.android.exchange".equals(info.accountType)){
                    mProvider = null;
                }
            }
        }
        ...
        /// @}
        if (mProvider != null && /* M: don't auto setup in manual*/ !shouldDivertToManual()) {
            mIsPreConfiguredProvider = true;
    ...
    private void showDuplicateAccountDialog(final String existingAccountName) {
        /// M: should disable "Next" button while showing this dialog.
    ...
    private boolean finishAutoSetup() {
        ...
        try {
            ...
            /// M: check info is valid before use in case of NPE
            if (info != null
                    && !info.isGmailStub
                    && !EmailServiceUtils.isServiceAvailable(this,
                            info.protocol)) {
    ...
    private void populateHostAuthsFromSetupData() {
        ...
        if (info.usesSmtp) {
            ...
            /// M: Use the verified host address to try and
            /// pick a smarter outgoing address.
            final String hostName =
                    AccountSettingsUtils.inferServerName(this, recvAuth.mAddress, null, "smtp");
            sendAuth.setConnection(HostAuth.LEGACY_SCHEME_SMTP, hostName,
                    HostAuth.PORT_UNKNOWN, HostAuth.FLAG_NONE);
            AccountSetupCredentialsFragment.populateHostAuthWithResults(this, sendAuth,
                    mSetupData.getCredentialResults());
            mSetupData.setOutgoingCredLoaded(true);
    ...
    private void dismissCheckSettingsFragment() {
        ...
        getFragmentManager().beginTransaction()
                .remove(f)
                .remove(d)
                .commitAllowingStateLoss(); //M: avoid exception after called onSaveInstanceState
    }
    ...
    public void onCheckSettingsError(int reason, String message) {
        ...
        /// M: Block quick press on "Next" button here, to avoid wrong proceed between fragment
    ...
    /** M: setup manual provider configurations(incoming & outgoing) by searching the
     * manual_setup_helper.xml and providers.xml
     * @param account
     * @param auth
     * @param protocol which protocol to found
     * @return true if setup manual informations successfully, otherwise return false
     */
    private boolean setupManualInfo(Account account, String protocol) {
        boolean result = false;
        final HostAuth recvAuth = account.getOrCreateHostAuthRecv(this);
        final HostAuth sendAuth = account.getOrCreateHostAuthSend(this);
        final String emailAddress = account.getEmailAddress();
        final String serverDomain = emailAddress != null ? emailAddress.split("@")[1].trim() : null;
        Provider manualProvider = AccountSettingsUtils.findManualInfoForDomain(this, serverDomain,
                protocol);
        if (manualProvider != null) {
            manualProvider.expandTemplates(emailAddress);
            try {
                recvAuth.setHostAuthFromString(manualProvider.incomingUri);
                /// M: to set authentication flag
                recvAuth.setUserName(manualProvider.incomingUsername);
                sendAuth.setHostAuthFromString(manualProvider.outgoingUri);
                /// M: to set authentication flag
                sendAuth.setUserName(manualProvider.outgoingUsername);
                result = true;
            } catch (URISyntaxException e) {
                LogUtils.e(LogUtils.TAG, e, "setup from maual helper failed!");
            }
        } else {
            // enhance default HostAuth values setting.
            if (!TextUtils.isEmpty(protocol)) {
                final String[] emailParts = emailAddress.split("@");
                final String user = emailParts[0].trim();
                final String domain = emailParts[1];
                recvAuth.setConnection(protocol, domain, HostAuth.PORT_UNKNOWN,
                        (HostAuth.LEGACY_SCHEME_EAS.equals(protocol)) ? HostAuth.FLAG_SSL
                                : HostAuth.FLAG_NONE);
                if (!(HostAuth.LEGACY_SCHEME_EAS.equals(protocol))) {
                    final EmailServiceUtils.EmailServiceInfo info = mSetupData
                            .getIncomingServiceInfo(this);
                    // pop3 and imap account, set Email address as the login name.
                    recvAuth.mLogin = user + "@" + recvAuth.mAddress;
                    recvAuth.mAddress = AccountSettingsUtils.inferServerName(this,
                            recvAuth.mAddress, info.inferPrefix, null);
                } else {
                    recvAuth.mLogin = user;
                }
            }
        }
    ...
    private void initiateAccountCreation() {
        ...
        if (hostAuth.mProtocol.equals(getString(R.string.protocol_eas))) {
            try {
                ...
                /// M: Some Exchange servers don't support smart reply/forward.
                /// So, remove the smart forward flag for these servers
                for (String server : SERVERS_NOT_SUPPORT_SMART_FORWARD) {
                    if (account.getEmailAddress().endsWith(server)) {
                        LogUtils.d(LogUtils.TAG, "Remove the smart forward flag due to" +
                                " the server don't support for %s", account.getEmailAddress());
                        newFlags &= ~Account.FLAGS_SUPPORTS_SMART_FORWARD;
                        break;
                    }
                }
        ...
        int frequency = fragment.getCheckFrequencyValue();
        if (frequency == AccountSetupOptionsFragment.SMART_PUSH_MENU_ITEM_VALUE) {
            account.setSyncInterval(AccountSetupOptionsFragment.PUSH_MENU_ITEM_VALUE);
    ...
    public void onAccountCreationFragmentComplete() {
        ...
        /// M: record current state, it will be saved when onSaveInstacneState called.
    ...
    public void resetActivityAfterPermission() {
        // super.resetActivityAfterPermission();
        // Launch a loader to look up the owner name.  It should be ready well in advance of
        // the time the user clicks next or manual.
        getLoaderManager().initLoader(OWNER_NAME_LOADER_ID, null,
                new AccountDisplayNameLoaderCallbacks());
