...
public abstract class EmailContent {
    ...
    public static final class Body extends EmailContent {
        ...
        /** M: using to restore the body for local search @{ */
        public static final String[] LOCALSEARCH_CONTENT_PROJECTION = new String[] {
                BaseColumns._ID,
                BodyColumns.MESSAGE_KEY,
                BodyColumns.HTML_CONTENT_URI,
                BodyColumns.TEXT_CONTENT_URI,
        ...
        /** M: save text body into body table(only for localsearch)
        @{ */
        ...
        private static String readBodyFromProvider(final Context context, final String uri) {
            ...
            try {
                ...
                LogUtils.v(LogUtils.TAG, "Exception while reading body content : %s",
                        e.getMessage());
            }
            return content;
        }
        ...
        /**
         * M: notify the message's update , if the body's content changed. the
         * Webview will miss the update of the body without the notification@{
         */
    ...
    public static final class Message extends EmailContent {
        ...
        /**
         * M: lable case of need delete server side message. @{
         */
        public static final int FLAG_DELETE_SERVER_MESSAGE = 1 << 30;
        /** @} */
        ...
        public void addSaveOps(ArrayList<ContentProviderOperation> ops) {
            ...
            if (isBodyNeedCompress()) {
                if (mText != null) {
                    cv.put(BodyColumns.TEXT_CONTENT, StringCompressor.compressToString(mText));
                }
                if (mHtml != null) {
                    cv.put(BodyColumns.HTML_CONTENT, StringCompressor.compressToString(mHtml));
                }
        ...
        private boolean isBodyNeedCompress() {
            ...
            return length > MimeUtility.NEED_COMPRESS_BODY_SIZE
                    && !EmailFeatureOptions.LAEGE_BODY_ENHANCEMENT;
        }

        /**
         * M: flag to lable case of need delete server side message,
         * event mailbox is not trash.
         * @return flag delete server message or not.
         */
        public boolean isDeleteFromServer() {
            return (mFlags & FLAG_DELETE_SERVER_MESSAGE) != 0;
        }
        ...
        /**
         * M: build local search selection from body text with query term
         *
        ...
        /**
         * M: Match the query terms from body text
         *
         * @param messageIds the message id array to query
         * @param queryTermsForBody query terms need to be queried
         */
        private static Set<String> matchBodyQueryFromText(Context context, long[] messageIds, String[] queryTermsForBody) {
            LogUtils.logFeature(
                    LogTag.SEARCH_TAG,
                    "matchBody messageIds [%s] or queryTermsForBody [%s] ",
            ...
            try {
                ...
                c = context.getContentResolver().query(Body.CONTENT_URI,
                        Body.LOCALSEARCH_CONTENT_PROJECTION, bodySelection.toString(), null, null);
                ...
                while (c.moveToNext()) {
                    ...
                    if (!TextUtils.isEmpty(messageContent)) {
                        ...
                        for (int i = 0; i < queryTermsForBody.length; i++) {
                            ...
                            LogUtils.logFeature(
                                    LogTag.SEARCH_TAG,
                                    "matchBody queryTermsforBody[%d]: %s contents: %s", i, queryTermsForBody[i], lowerContent);
