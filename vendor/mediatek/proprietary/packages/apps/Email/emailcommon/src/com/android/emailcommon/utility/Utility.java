...
///M: For LowStorage external interface
...
/** @} */
    ...
    /**
     * M: check whether the system is in LOW STORAGE state by checking the STICKY broadCast.
     * Wrapper for mail utility api for external apps
     * @return true if in LowStorage, false otherwise
     */
    public static boolean isStorageLowExt(Context cntx) {
        final IntentFilter filter = new IntentFilter(Intent.ACTION_DEVICE_STORAGE_LOW);
        final Intent result = cntx.registerReceiver(null, filter);
        return (result != null);
    }

    /**
     * M: Runs the specified action on the UI thread. If the current thread is the UI
    ...
    /**
     * M: Append a bold span to a {@link SpannableStringBuilder}.
     */
    ...
    /**
     * [LBE] Large body enhancement, write body file for large mail.
     * @param cr ContentResolver
     * @param messageId, the message key of body.
     * @param html, true: html type, false: text body.
     * @param bodyString, body of mail
     * @throws IOException
     */
    public static void writeBodyFile(final ContentResolver cr, final long messageId,
            final boolean isHtml, final String bodyString) throws IOException {
        BufferedOutputStream os = null;
        Uri bodyUri = isHtml ? EmailContent.Body.getBodyHtmlUriForMessageWithId(messageId)
                : EmailContent.Body.getBodyTextUriForMessageWithId(messageId);
        ByteArrayInputStream is = new ByteArrayInputStream(bodyString.getBytes());
        byte[] readBuffer = new byte[512];
        try {
            int readCount = 0;
            long startTime = System.currentTimeMillis();
            os = new BufferedOutputStream(cr.openOutputStream(bodyUri), 512);
            while ((readCount = is.read(readBuffer)) != -1) {
                os.write(readBuffer, 0, readCount);
            }
            os.flush();
            Logging.w("[LBE] write body file sucess, for message: " + messageId + " "
                    + (System.currentTimeMillis() - startTime)
                    + " ms, size " + bodyString.length());
        } catch (IOException io) {
            Logging.w("[LBE] write body file failed for message: " + messageId, io);
            throw io;
        } finally {
            if (os != null) {
                os.close();
