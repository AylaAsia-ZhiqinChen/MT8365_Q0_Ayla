    ...
    private boolean parseStore() throws IOException {
        ...
        while (nextTag(Tags.SEARCH_STORE) != END) {
            if (tag == Tags.SEARCH_STATUS) {
                ...
                LogUtils.d(Logging.LOG_TAG, "Search results total [%d]", mTotalResults);
            } else if (tag == Tags.SEARCH_RESULT) {
                parseResult(parser, ops);
                /// M: record message count of the result
                mCurrentResults++;
            } else {
                skipTag();
            }
        ...
        try {
            ...
            LogUtils.d(Logging.LOG_TAG, "Saved %s search results as %d messages", ops.size(), mCurrentResults);
    ...
    private boolean parseResult(EmailSyncParser parser,
        ...
        while (nextTag(Tags.SEARCH_RESULT) != END) {
            if (tag == Tags.SYNC_CLASS) {
                ...
                LogUtils.d(Logging.LOG_TAG, "long id of searched message [%s]", msg.mProtocolSearchInfo);
                ...
                /// M: Tag stack is dirty, need to remove first tag to clean it up
                popTag(tag);
                if (msg.mHtml != null) {
