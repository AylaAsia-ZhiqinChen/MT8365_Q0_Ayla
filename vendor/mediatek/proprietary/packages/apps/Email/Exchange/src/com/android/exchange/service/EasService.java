...
public class EasService extends Service {
    ...
    private final IEmailService.Stub mBinder = new IEmailService.Stub() {
        ...
        public void sendMail(final long accountId) {
            ...
            /// M: change log type from wtf to w, not throw exception.
            LogUtils.w(TAG, "unexpected call to EasService.sendMail");
        }
        ...
        public void pushModify(final long accountId) {
            ...
            /// M: change for fit pingNeededForAccount
        ...
        public void fetchMessage(long messageId) {
            ...
            EasItemOperations.fetchMessage(EasService.this, messageId);
        }
    ...
    public void onCreate() {
        ...
        /* M: Dont start ping as service will be stoped next */
        if (Utility.mayLowStorage() ||
                Utility.isStorageLowExt(EasService.this)) {
            LogUtils.d(TAG, "Dont start ping in low storage mode");
            return;
        }
    ...
    public void onDestroy() {
        ...
        /**
         * M: move this ping task stop operation to work thread to avoid
    ...
    public int onStartCommand(final Intent intent, final int flags, final int startId) {
        ...
        /** M: Stop the exchange service in the low storage mode @{ */
        if (Utility.mayLowStorage() ||
                Utility.isStorageLowExt(EasService.this)) {
            LogUtils.d(TAG, "Stops service in the low storage mode");
            stopSelf();
        ...
        if (intent != null &&
            ...
            if (intent.getBooleanExtra(ServiceProxy.EXTRA_FORCE_SHUTDOWN, false)) {
                ...
                System.exit(-1);
            } else if (intent.getBooleanExtra(Eas.EXTRA_START_PING, false)) {
                /** M: Restart a delayed ping for account @{ */
                Account account = intent.getParcelableExtra(Eas.EXTRA_PING_ACCOUNT);
                LogUtils.d(TAG, "Restart a delayed ping for account[%d]",
                        (account != null) ? account.mId : -1);
                if (account != null) {
    ...
    public int doOperation(final EasOperation operation, final String loggingName) {
        LogUtils.d(TAG, "loggingName : %s, accountId : %d", loggingName, operation.getAccountId());
        ...
        try {
            ...
            /// M: Just leave if EmailProvider is unavailable
            LogUtils.e(TAG, pue, "doOperation failed; email provider unavailable");
            return result;
        /**
         * M: Ping synchronizer exception for sync await failed.
         * catch and skip it, but not signal other sync operation in the queue.@{
         */
        } catch (InterruptedException e) {
            LogUtils.e(TAG, "doOperation, PSS InterruptedException acct:%d", operation
                    .getAccountId());
            result = EasOperation.RESULT_SYNC_INTERRUPT;
            return result;
        /** @} */
        } finally {
            LogUtils.d(TAG, "doOperation finally account = %s, result = %d",
                    operation.getAccount(), result);
            /// M: sync failed for await interrupted exception, not signal other operation.
            if (result != EasOperation.RESULT_SYNC_INTERRUPT) {
                /// M: Any non-failure result should be greater or equal to RESULT_MIN_OK_RESULT @{
    ...
    /**
     * M: Change to static method to call other place for easy use
    ...
    public static boolean pingNeededForAccount(final Context context, final Account account) {
        ...
        LogUtils.d(TAG, "pingNeededForAccount syncInterval[%d]", account.mSyncInterval);
    ...
    /// M: Modify the oder of the modifier
    public static GalResult searchGal(final Context context, final long accountId,
    ...
    /**
     * M: do ping task stop in a worker thread to avoid
     * android.os.NetworkOnMainThreadException.
     */
    private void stopPingTask() {
        Runnable runable = new Runnable() {
            @Override
            public void run() {
                LogUtils.d(TAG, "Do stop ping task.");
                mSynchronizer.stopAllPings();
            }
        };
        Thread t = new Thread(runable, "PingTaskStopThread");
        t.start();
    }

    @Override
    protected void dump(FileDescriptor fd, PrintWriter writer, String[] args) {
        /// M: Dump for debugging wakelock histories
        mSynchronizer.dump(fd, writer, args);
    }

    @Override
    public boolean onUnbind (Intent intent) {
        LogUtils.i(TAG, "EAS service unbind called");
        return false;
    }
}
