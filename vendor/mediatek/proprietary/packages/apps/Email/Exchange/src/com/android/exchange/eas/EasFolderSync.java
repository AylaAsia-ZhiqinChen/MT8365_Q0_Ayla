...
public class EasFolderSync extends EasOperation {
    ...
    public int performOperation() {
        if (mStatusOnly) {
            ...
            /**
             * M: load latest account from database again, to check if current
             * foldersync is dirty or not. e.g. deplicate initial foldersync
             * command, the first command finished and account synckey has
             * updated, while other sync command should skip. @{
             */
            Account cacheAccount = getAccount();
            Account account = loadAccount(mContext, cacheAccount.getId());
            if (account != null && !TextUtils.equals(cacheAccount.mSyncKey, account.mSyncKey)) {
                LogUtils.w(LOG_TAG, "Skip dirty initial FolderSync for account [%s]",
                        getAccountId());
                return RESULT_MIN_OK_RESULT;
            }
            /** @} */

            /**
             * M: Performance improvement, If the securitySyncKey is null, try provision firstly.
             * Because the first folderSync command will spend much time. So we send provision
             * command to server directly. Otherwise, provision operation have to waiting until
             * handleProvisionError be called. The spent time of folder sync was wasted. @{
             */
            ...
            // Use mPolicyKey to determine if we have been asked to try
            // provision during validate process
            if (mAccount.mPolicyKey > 0
                ...
                while (retryCount <= TRY_PROVISION_MAX_RETRY_COUNT) {
                    ...
                    boolean isSuccess = super.tryProvision();
                    if (isSuccess) {
    ...
    /**
     * M: Load latest database account.
     *
     * @param context
     * @param accountId
     * @return latest acount.
     */
    private Account loadAccount(final Context context, final long accountId) {
        Account account = Account.restoreAccountWithId(context, accountId);
        if (account == null) {
            LogUtils.e(LOG_TAG, "Could not load account, for account %d", accountId);
        }
        return account;
    }
}
