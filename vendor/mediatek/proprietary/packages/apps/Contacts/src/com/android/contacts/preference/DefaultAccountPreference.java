...
public class DefaultAccountPreference extends DialogPreference {
    ...
    public CharSequence getSummary() {
        ...
        if (defaultAccount == null || mAccounts == null ||
            ...
            AccountInfo accountInfo = AccountInfo.getAccount(mAccounts, defaultAccount);
            String accountName = accountInfo.getAccount().name;
            if (AccountTypeUtils.getSubIdBySimAccountName(getContext(), accountName) >= 0) {
                return (CharSequence)AccountTypeUtils.getDisplayAccountName(
                            getContext(), accountName);
            ///M: [ALPS03595082] ensure get string with right language.
            } else if (accountInfo.getType() instanceof LocalPhoneAccountType) {
                return accountInfo.getType().getDisplayLabel(getContext());
            } else {
                return accountInfo.getNameLabel();
            }
            /// @}
        }
    }
    ...
    protected void onDialogClosed(boolean positiveResult) {
        ...
        /// M: [ALPS03725656] mListAdapter maybe changed caused by plugout sim card
        // while user select account so need more check. @{
        if (mChosenIndex != -1 && mChosenIndex < mListAdapter.getCount()) {
        /// @}
