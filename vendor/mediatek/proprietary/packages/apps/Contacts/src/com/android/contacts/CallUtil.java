...
public class CallUtil {
    ...
    public static int getVideoCallingAvailability(Context context) {
        if (!PermissionsUtil.hasPermission(context, android.Manifest.permission.READ_PHONE_STATE)
            ...
            Log.d(TAG, "[getVideoCallingAvailability] do not have read phone state permission");
            return VIDEO_CALLING_DISABLED;
        }
        TelecomManager telecommMgr = (TelecomManager)
                context.getSystemService(Context.TELECOM_SERVICE);
        if (telecommMgr == null) {
            Log.d(TAG, "[getVideoCallingAvailability] telecommMgr is null");
            return VIDEO_CALLING_DISABLED;
        }
        ...
        try {
                ...
                if (account != null) {
                    if (account.hasCapabilities(PhoneAccount.CAPABILITY_VIDEO_CALLING)) {
                        ...
                        if (!CompatUtils.isVideoPresenceCompatible()) {
                            Log.d(TAG, "[getVideoCallingAvailability] not videoPresenceCompatible");
            ...
            Log.d(TAG, "[getVideoCallingAvailability] Security exception");
            return VIDEO_CALLING_DISABLED;
        }
    }
