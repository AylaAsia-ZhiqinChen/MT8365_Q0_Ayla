...
public class UpdateGroupMembersAsyncTask extends AsyncTask<Void, Void, Intent> {
    ...
    private static final String TAG = "UpdateGroupMembersAsyncTask";
    ...
    protected Intent doInBackground(Void... params) {
        ...
        Log.d(TAG, "[doInBackground] rawContactIds count = " + rawContactIds.length
                + ", mType = " + mType + ", mGroupId = " + mGroupId
                + ", mAccountName = " + Log.anonymize(mAccountName)
    ...
    private long[] getRawContactIds() {
        ...
        /** M: [MTK Multi Opr] fixe large number contacts JE.
         * ori code:
        ...
        for (int i = 0; i < mContactIds.length; i++) {
            selection.append(String.valueOf(mContactIds[i]));
            if (i < mContactIds.length - 1) {
                selection.append(",");
            }
        }
        selection.append(")");
        Log.d(TAG, "[getRawContactIds] selection=" + selection.toString());
        final Cursor cursor = mContext.getContentResolver().query(
                 rawContactUri, projection, selection.toString(), null, null);
        /* @} */
    ...
    /// M: [Sim Group] add for UpdateSimGroupMembersAsyncTask @{
    ...
    protected boolean isTypeAdd() {
        return getType()==TYPE_ADD;
    }

    protected boolean isTypeRemove() {
        return getType()==TYPE_REMOVE;
    }
    /// @}
}
