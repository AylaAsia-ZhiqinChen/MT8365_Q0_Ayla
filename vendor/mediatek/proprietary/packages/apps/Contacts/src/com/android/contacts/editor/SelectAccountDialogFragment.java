...
public final class SelectAccountDialogFragment extends DialogFragment
    ...
    public Dialog onCreateDialog(Bundle savedInstanceState) {
        ...
        final DialogInterface.OnClickListener clickListener =
            ...
            public void onClick(DialogInterface dialog, int which) {
                dialog.dismiss();
                /* M: [Sim Contact Flow] fix ALPS00402174
                 * Original code: @{
                onAccountSelected(mAccountsAdapter.getItem(which));
                 * @} New code: @{ */
                AccountWithDataSet account = mAccountsAdapter.getItem(which);
                if (account instanceof AccountWithDataSetEx
                        && !SimCardUtils.isPhoneBookReady(getActivity(),
    ...
    public void onAccountsLoaded(List<AccountInfo> accounts) {
        ...
        /// M: [Sim Group] @{
        if (mFilter == AccountFilter.GROUPS_WRITABLE) {
            accounts = AccountsListAdapterUtils.getGroupAccount(accounts);
        }
        /// @}
