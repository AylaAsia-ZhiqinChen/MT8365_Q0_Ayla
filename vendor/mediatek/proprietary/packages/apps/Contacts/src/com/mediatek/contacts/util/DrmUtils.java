...
public class DrmUtils {
    ...
    public static boolean isDrmImage(Context context, Uri uri) {
        if (!ContactsSystemProperties.MTK_DRM_SUPPORT) {
            Log.w(TAG, "[isDrmImage] not support drm...");
            return false;
        }
        boolean isDrm = false;
        ...
        try {
            ...
            Log.e(TAG, "isDrmImage: getMetadata fail with uri " + uri, e);
        }
        final String key = "isdrm";
        Integer isDrmImage = cv == null ? null : cv.getAsInteger(key);
        if (isDrmImage != null && isDrmImage > 0) {
            isDrm = true;
        }
        drmClient.release();
        drmClient = null;
