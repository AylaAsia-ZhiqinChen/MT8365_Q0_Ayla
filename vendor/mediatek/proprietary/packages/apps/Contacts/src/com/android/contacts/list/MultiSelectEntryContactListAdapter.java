...
public abstract class MultiSelectEntryContactListAdapter extends ContactEntryListAdapter {
    ...
    private void bindCheckBox(ContactListItemView view, Cursor cursor, boolean isLocalDirectory) {
        ...
        /* M: [SDN] If is sdn contact, do not show the checkbox in multiselectFragment @{*/
        if (ContactsPortableUtils.MTK_PHONE_BOOK_SUPPORT && cursor.getInt(cursor.
                getColumnIndex(MtkContactsContract.ContactsColumns.IS_SDN_CONTACT)) == 1) {
    ...
    ///M: [SDN] @{
    public FavoritesAndContactsLoader mSDNLoader = null;
    @Override
