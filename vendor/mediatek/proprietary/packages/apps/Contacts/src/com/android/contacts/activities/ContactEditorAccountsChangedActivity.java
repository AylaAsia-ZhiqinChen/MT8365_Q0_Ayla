...
public class ContactEditorAccountsChangedActivity extends Activity
    ...
    /// M: [Sim Contact Flow] get sim info for create sim contact. @{
    ...
    private final OnItemClickListener mAccountListItemClickListener = new OnItemClickListener() {
        ...
        public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
            ...
            if (mAccountListAdapter == null) {
                Log.w(TAG, "mAccountListAdapter = " + mAccountListAdapter);
                return;
            }
            /** M: [Google Issue] ignore multiple click at short time @{ */
            if (mCheckCount >= 1) {
                Log.w(TAG, "ignore multiple click, just return.");
                return;
            }
            mCheckCount++;
            /** @} */
    ...
    private final OnClickListener mAddAccountClickListener = new OnClickListener() {
        ...
        public void onClick(View v) {
            /** M:[ALPS03436806]TK solution do not support GMS default,
             * change behavior as Handroid N to android O response the new account reqeust.
             * ori: final Intent intent = ImplicitIntentsUtil.getIntentForAddingGoogleAccount();
             * new code:@{ */
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        /// M: [Google Issue]ALPS00251666, can not add contact when Contacts busy. @{
    ...
    private void updateDisplayedAccounts(List<AccountInfo> accounts) {
        ...
        if (numAccounts >= 2) {
            ...
            /* M: [Sim Contact Flow] change string
             * Original code: @{
    ...
    private void saveAccountAndReturnResult(AccountWithDataSet account) {
        /// M: [Sim Contact Flow] get sim info for creating sim contact @{
        if (account != null && account.type != null)
        {
            ...
            if (AccountTypeUtils.isAccountTypeIccCard(accountType)) {
                ...
                if (!SimCardUtils.checkPHBStateAndSimStorage(this, mSubId)) {
                    Log.d(TAG, "[saveAccountAndReturnResult] finish for PHB not ready");
                    finish();
                    return;
                }
            }
        } else if (account != null && account.isNullAccount()) {
        ...
        Log.d(TAG, "[saveAccountAndReturnResult] account is " + account
                + ", mSubId is " + mSubId);
        /// @}
