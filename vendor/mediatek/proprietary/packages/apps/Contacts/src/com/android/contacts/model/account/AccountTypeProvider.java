...
public class AccountTypeProvider {
    ...
    public List<AccountType> getAccountTypes(String accountType) {
        ...
        if (types == null) {
            ...
            /// M: [Local Account Type] get local account @{
            if (types.isEmpty() && isLocalAccountType(mLocalAccountTypeFactory, accountType)) {
                AccountType type = mLocalAccountTypeFactory.getAccountType(accountType);
                final ImmutableList.Builder<AccountType> result = ImmutableList.builder();
                types = result.add(type).build();
            }
            /// @}
            mCache.put(accountType, types);
            Log.i(TAG, "[getAccountTypes]String:" + accountType + ", types=" + types);
        }
        return types;
    }
    ...
    public AccountType getType(String type, String dataSet) {
        ...
        Log.i(TAG, "[getType]unkown type:" + type + ", dataSet:" + dataSet);
        return null;
    }
