...
public class DisplayOptionsPreferenceFragment extends PreferenceFragment
    ...
    private static final String TAG = "DisplayOptionsPreferenceFragment";
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        if (!RequestPermissionsActivity.hasRequiredPermissions(getActivity())) {
            Log.w(TAG, "[onCreate] no permissions, return and wait activity to finish");
            return;
        }
        ...
        ///M:[ALPS03696823] import preference maybe hide, so need check. @{
        ...
        Log.w(TAG, "[onCreate] getPreferenceManager()=" + getPreferenceManager());
    }
    ...
    private void removeUnsupportedPreferences() {
        ...
        /// M: [MTK IMPORT/EXPORT] add for mtk solution. @{
        if (!ActivitiesUtils.showImportExportMenu(getActivity())) {
            getPreferenceScreen().removePreference(findPreference(KEY_IMPORT));
            getPreferenceScreen().removePreference(findPreference(KEY_EXPORT));
        }
        /// @}
    }
    ...
    public void onActivityResult(int requestCode, int resultCode, Intent data) {
        if (requestCode == REQUEST_CODE_CUSTOM_CONTACTS_FILTER
            ...
            /// M:[ALPS04218802] mPrefererenceManager maybe is null here, so check
            /// mPreferenceManager validation and add some log for debug. @{
            if (getPreferenceManager() != null) {
                super.onActivityResult(requestCode, resultCode, data);
            } else {
                Log.e(TAG, "[onActivityResult] mPreferenceManager is null. requestCode="
