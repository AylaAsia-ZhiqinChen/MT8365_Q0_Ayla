...
public class TimingStatistics {
    ...
    public TimingStatistics(String targetClassName) {
        mTargetClassName = targetClassName == null ?
                TimingStatistics.class.getSimpleName() : targetClassName;
    }

    public void timingStart() {
        if (!isTimingEnable()) {
            Log.i(TAG, "[timingStart] fail.");
            return;
        }
    ...
    public void log(String... extraInfo) {
        ...
        Log.i(TAG, "[Performance test][Contacts][Timing][ " + mTargetClassName + "]" + info +
                " mTimeTotalUsed:"
                + getTimingTotalUsed() + ",mTimingCount:" + mTimingCount + ",average:" +
                getTimingTotalUsed() * 1.0 / getTimingCount());
    }
}
