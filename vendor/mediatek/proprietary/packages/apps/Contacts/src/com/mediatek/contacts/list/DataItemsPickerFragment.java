...
public class DataItemsPickerFragment extends DataKindBasePickerFragment {
    ...
    public void onCreate(Bundle savedState) {
        ...
        mIntent = this.getArguments().getParcelable(MultiBasePickerFragment.FRAGMENT_ARGS);
    }

    @Override
    protected DataItemsPickerAdapter createListAdapter() {
        DataItemsPickerAdapter adapter = new DataItemsPickerAdapter(getActivity(), getListView());
        adapter.setFilter(ContactListFilter
                .createFilterWithType(ContactListFilter.FILTER_TYPE_ALL_ACCOUNTS));
        if (mIntent != null) {
            long[] phoneIds = mIntent.getLongArrayExtra(
                    ContactListMultiChoiceActivity.RESTRICT_LIST);
