...
public class Cp2DeviceLocalAccountLocator extends DeviceLocalAccountLocator {
    ...
    public List<AccountWithDataSet> getDeviceLocalAccounts() {
        ...
        /** M:[Local Account Type] do not load local account from provider
        ...
        /// M: [Local Account Type] add MTK local account here @{
        int beforeSize = localAccounts.size();
        AccountTypeManagerEx.loadSimAndLocalAccounts(localAccounts);
        Log.i(TAG, "getDeviceLocalAccounts() size: before=" + beforeSize +
                ", after=" + localAccounts.size());
        /// @}
    ...
    private void addAccountsFromCursor(Cursor cursor, Set<AccountWithDataSet> accounts) {
        while (cursor.moveToNext()) {
            ...
            Log.i("Cp2DeviceLocalAccountLocator", "query result=" +
                    name + ", " + type + ", " + dataSet);
