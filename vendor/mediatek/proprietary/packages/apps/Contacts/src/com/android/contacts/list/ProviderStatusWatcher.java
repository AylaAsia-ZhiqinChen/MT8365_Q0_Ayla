...
public class ProviderStatusWatcher extends ContentObserver {
    ...
    public int getProviderStatus() {
        ...
        if (mProviderStatus == null) {
            Log.d(TAG, "[getProviderStatus]mProviderStatus is null, return STATUS_BUSY");
    ...
    private void waitForLoaded() {
        Log.d(TAG, "[waitForLoaded] mProviderStatus=" + mProviderStatus +
        ...
        if (mProviderStatus == null) {
            ...
            synchronized (mSignal) {
                try {
                    ...
                    Log.e(TAG, "[getProviderStatus]InterruptedException", ignore);
                }
            }
        }
    }
    ...
    private void startLoading() {
        if (mLoaderTask != null) {
            Log.d(TAG, "[startLoading]mLoaderTask != null");
            /**
             * M:[ALPS03580656] still reload status to ensure status update correctly.
             * ori code: @{
            return; // Task already running.
             * @} */
        }

        if (DEBUG || Log.ENG_DEBUG) {
                ...
                if (cursor != null) {
                    try {
                        if (cursor.moveToFirst()) {
                            ...
                            Log.d(TAG, "[LoaderTask]mProviderStatus=" + mProviderStatus);
                            return true;
                        }
                        Log.e(TAG, "[LoaderTask]cursor is empty");
                    } finally {
                        cursor.close();
                    }
                }
                Log.e(TAG, "[LoaderTask]cursor is null");
                return false;
