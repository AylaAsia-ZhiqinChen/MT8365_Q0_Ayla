...
class CMT66xx : public CAdapter{
protected:
    ~CMT66xx(void){}

public:
    CMT66xx(const char *Name, const char *Desc, COID* oid, const char* wrapperName, const UINT_32 chipId=0x6620):CAdapter(Name, Desc, oid, wrapperName, chipId)/*, fgAnritsuSupportEn(false)
       , u4OriRoamingEn(0)*/ {isEepromUsed = false;}

    //For MP====================================================================
    //In ATE
    INT_32 setChannel(INT_32 channelFreq);
    INT_32 getPacketRxStatus(INT_32 * rxOk_p, INT_32 * rxCrcErr_p, INT_32 *rxRssi_p);
    ...
    INT_32 getXtalTrimToCr (UINT_32 *pu4Value);
    INT_32 isWifiLogUiEnable(INT_32 module);
    INT_32 getWifiLogLevel(INT_32 module);
    INT_32 setWifiLogLevel(INT_32 module, INT_32 level);
    INT_32 isAntSwapSupport();
private:
