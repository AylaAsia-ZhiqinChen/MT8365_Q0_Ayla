...
CAdapter::CAdapter(const char* AdapterName, const char* Desc, COID * oid, const char* wrapperName, const UINT_32 chipId) :
m_referCnt(0)
{
    UNUSED(Desc, chipId);
#if defined (HAVE_ANDROID_OS)
	m_OID = oid;
    strncpy(ProtocolName, AdapterName, NAMESIZE - 1);


	//Name = AdapterName.ws2s() + string("@") +  Wrapper.getName();
    strncpy(Name, AdapterName, NAMESIZE - 1);
    strncat(Name, "@", NAMESIZE - strlen(Name) - 1);
    strncat(Name, wrapperName, NAMESIZE - strlen(Name) - 1);
	...
	)
{
    ...
    memcpy(m_regDomain.countryCode, chregDomain, ((sizeof(chregDomain) > 2) ? 2 : sizeof(chregDomain)));
...
INT_32 CAdapter::setDPDFromFile(TCHAR* fileName)
{
    UNUSED(fileName);
    em_printf(MSG_ERROR, "CHIP 0x%04x doesn't support setDPDFromFile\n", m_ChipID);
    return 0;
}
INT_32 CAdapter::getDPDLength (INT_32* dpdLength)
{
    UNUSED(dpdLength);
    em_printf(MSG_ERROR,"CHIP 0x%04x doesn't support getDPDLength\n", m_ChipID);
    return 0;
}
INT_32 CAdapter::readDPD32 (UINT_32 offset, UINT_32 * value)
{
    UNUSED(offset, value);
    em_printf(MSG_ERROR,"CHIP 0x%04x doesn't support readDPD32\n", m_ChipID);
    return 0;
}
INT_32 CAdapter::writeDPD32 (UINT_32 offset, UINT_32 value)
{
    UNUSED(offset, value);
    em_printf(MSG_ERROR,"CHIP 0x%04x doesn't support writeDPD32\n", m_ChipID);
    return 0;
}
INT_32 CAdapter::getXtalTrimToCr (UINT_32 *pu4Value)
{
    UNUSED(pu4Value);
    em_printf(MSG_ERROR,"CHIP 0x%04x doesn't support getXtalTrimToCr\n", m_ChipID);
    return 0;
}
	...
	)
{
    ...
    UINT_32 u4Tail;
	INT_32  ret = 0;
    em_printf(MSG_DEBUG, (char*)"u4Addr:%d and u4Length:%d \n", u4Addr, u4Length);
    ...
    for(u4I = u4StartAddrWD; u4I <= u4EdAddrWD;  u4I++) {
        ...
        em_dump(acTmp, 8)
        if (u4Tail >= 512) {
            em_printf(MSG_DEBUG, (char*)"outof size and u4Tail:%d\n", u4Tail);
            return -1;
        }
        memcpy(&szTmp[u4Tail], acTmp, 4 );
        u4Tail +=4;
    }
    em_printf(MSG_DEBUG, (char*)"u4Tail:%d\n", u4Tail);
    ...
    em_printf(MSG_DEBUG, (char*)"u4Head:%d\n", u4Head);
    ...
    szTmp[u4Tail] = '\0';
    em_printf(MSG_DEBUG, (char*)"szTmp:%s\n", szTmp);
    if ((u4Head >= 512)) {
        em_printf(MSG_DEBUG, (char*)"outof size and u4Head:%d\n", u4Head);
        return -1;
    }
...
//    UINT_16 u2Tmp;
    ...
    memset(szTmp, 0, sizeof(szTmp));
    strncpy(szTmp, pszStr, sizeof(szTmp) - 1);
...
CAdapter::setPnpPower(INT_32 i4PowerMode)
{
    ...
    if (!m_OID->setOID(this, OID_PNP_SET_POWER, (CHAR*) &rPowerState,
...
INT_32
CAdapter::getFwManifestVersion(char *manifest) {
    struct iwreq wrq;
    int skfd, ret = 0;
    char auManifest[256];
    /* initialize socket */
    ...
    /* initialize WEXT request */
    wrq.u.data.pointer = &(auManifest[0]);
    wrq.u.data.length = sizeof(char) * 256;
    wrq.u.data.flags = PRIV_CMD_GET_FW_VERSION;
    strncpy(wrq.ifr_name, "wlan0", IFNAMSIZ - 1);
    /* do IOCTL */
    if (ioctl(skfd, IOCTL_GET_INT, &wrq) < 0) {
        em_error((char*)"%s ioctl failed, %s", __FUNCTION__, strerror(errno));
    ...
    em_printf(MSG_DEBUG, (char*)"%s:operation succeed", __FUNCTION__);
    memcpy(manifest, auManifest, wrq.u.data.length * sizeof(char));
    manifest[wrq.u.data.length] = '\0';
    ret = ERROR_RFTEST_SUCCESS;
exit:
    close(skfd);
    return ret;
