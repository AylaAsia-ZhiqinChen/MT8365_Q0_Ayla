    ...
    UINT_32 bufLen)
{
    ...
    strncpy(iwr.ifr_name, ptcDeviceName, IFNAMSIZ - 1);

    memset(&ndis_ts, 0, sizeof(ndis_ts));
...
CLocalOID::CLocalOID(const char* ifname, const char *chipName)
{
	if(strlen(ifname) < IFNAMSIZ)	{
        strncpy(m_name, ifname, IFNAMSIZ);
    ...
    strncpy(this->chipName, chipName, sizeof(this->chipName) - 1);
	//open rfkill
