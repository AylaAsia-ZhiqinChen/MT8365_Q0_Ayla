...
public class DesensePllsActivity extends Activity implements
    ...
    private static final String TAG = "Desense/Plls";
    ...
    private boolean updateData() {
        ...
        if (null != list) {
            mListData.clear();
            int size = list.size();
            Elog.v(TAG, "PLLGetAllInfo list size = " + size);
            ...
            for (int i = 0; i < size; i++) {
                ...
                Elog.v(TAG, "PLLGetAllInfo list.get(i) = " + item.toString());
            }
            result = true;
        }
        return result;
        ...
        try {
            if (ShellExe.RESULT_SUCCESS == ShellExe
                ...
                try {
                    ...
                    Elog.w(TAG, "getAllPllInfo IllegalStateException: "
                            + e.getMessage());
                } catch (NumberFormatException e) {
                    Elog.w(TAG, "getAllPllInfo NumberFormatException: "
                            + e.getMessage());
                } catch (IndexOutOfBoundsException e) {
                    Elog.w(TAG, "getAllPllInfo IndexOutOfBoundsException: "
                            + e.getMessage());
                }
            }
        } catch (IOException e) {
            Elog.w(TAG, "getAllPllInfo IOException: " + e.getMessage());
        }
        return listResult;
    }
