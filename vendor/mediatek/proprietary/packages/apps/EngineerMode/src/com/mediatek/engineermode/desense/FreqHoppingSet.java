...
public class FreqHoppingSet extends Activity {
    ...
    private static final String TAG = "Desense/FreqHoppingSet";
    ...
    private static final String PLL_NAME_PATTERN = "\\=\\w*PLL\\w*|\\=NOTSUPPORT";
    ...
    private final OnItemSelectedListener mSpinnerSelectListener = new OnItemSelectedListener() {
        ...
        public void onItemSelected(AdapterView<?> parent, View view,
            ...
            if("NOTSUPPORT".equals(parent.getSelectedItem().toString())) {
                disableAllBtn();
            } else {
                selectItem(position);
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (!new File(FILE_FREQ_HOPPING_DEBUG).exists()) {
            ...
            Elog.w(TAG, "FreqHoppingSet file not exists");
            finish();
            return;
        }
    ...
    private int initPllSpinnerItems(Spinner pllSpn) {
        ...
        if (debugMsg != null) {
            ...
            if (mPllCount > 0) {
                ...
                Elog.d(TAG, "init pll spinner fail; mPllCount:" + mPllCount);
                return -1;
            }

        } else {
            Elog.d(TAG, "init pll spinner fail; debugMsg = null");
            return -1;
        }
    }
    ...
    private void enableFreqHopping() {
        ...
        if (bEmpty) {
            ...
            strBuilder.append(Integer.toHexString(mSpPlls.getSelectedItemPosition()));
            strBuilder.append(" > ");
            strBuilder.append(FILE_FREQ_STATUS);
            Elog.v(TAG, "enable command: " + strBuilder.toString());
        } else {
            ...
            Elog.v(TAG, "enable command 1: " + strBuilder.toString());
        }
        try {
            if (ShellExe.RESULT_SUCCESS == ShellExe.execCommand(strBuilder
                ...
                if (bEmpty) {
                    ...
                    strBuilder.append(FILE_FREQ_HOPPING_DEBUG);
                    Elog.v(TAG, "enable command 2: " + strBuilder.toString());
            ...
            Elog.w(TAG, "enable freqhopping IOException: " + e.getMessage());
        }
        Toast.makeText(
                this,
    ...
    private void disableFreqHopping() {
        ...
        strBuilder.append(FILE_FREQ_HOPPING_DEBUG);
        Elog.v(TAG, "disable command: " + strBuilder.toString());
        try {
            if (ShellExe.RESULT_SUCCESS == ShellExe.execCommand(strBuilder
                    .toString())) {
                bSuccess = true;
            ...
            Elog.w(TAG, "disable freqhopping IOException: " + e.getMessage());
        }
        Toast.makeText(
                this,
                getString(R.string.desense_freqhopping_set)
    ...
    private boolean validateInput(boolean bEtInput) {
        ...
        if (bEtInput) {
            try {
                ...
                Elog.w(TAG, "validate input NumberFormatException: "
                        + e.getMessage());
                result = false;
            }
        }
    ...
    private boolean updateHoppingStatus() {
        ...
        if (null != debugMsg) {
            ...
            try {
                ...
                Elog.w(TAG, "updateHoppingStatus IllegalStateException: "
                        + e.getMessage());
            } catch (IndexOutOfBoundsException e) {
                Elog.w(TAG, "updateHoppingStatus IndexOutOfBoundsException: "
                        + e.getMessage());
            } catch (NumberFormatException e) {
                Elog.w(TAG, "updateHoppingStatus NumberFormatException: "
                        + e.getMessage());
            }
        }
        return result;
    ...
    private String getFreqHopDebugMsg() {
        ...
        try {
            ...
            Elog.w(TAG, "getFreqHopDebugMsg IOException: " + e.getMessage());
        }
        return result;
    }
    ...
    private String getFreqHopMsg() {
        ...
        try {
            ...
            Elog.w(TAG, "getFreqHopMsg IOException: " + e.getMessage());
        }
        return result;
    }
