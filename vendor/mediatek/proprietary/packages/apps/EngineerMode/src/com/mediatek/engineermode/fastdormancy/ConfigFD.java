...
/**
 * Config FastDormancy support type.
 */
public class ConfigFD extends Activity {
            ...
            if (v.getId() == R.id.fd_set_button) {
                ...
                if (checkedId == R.id.fd_on_radio) {
                    if (null != mReturnData && null != mReturnData[0] && mFdValue != -1) {
                        ...
                        cmdStr[1] = "";
                        sendAtCommand(cmdStr,EVENT_FD_SET);
                        Elog.i(TAG, "invoke cmdStr :" + cmdStr[0]);
                    } else {
                        Toast.makeText(ConfigFD.this, "Get FD data fail!", Toast.LENGTH_SHORT)
                                .show();
    ...
    private void init() {
        sendAtCommand(QUERY_FD,EVENT_FD_QUERY);
    }
    ...
    private String[] parseData(String[] data) {
        ...
        if (data[0] != null && data[0].startsWith(FORE_CMD)) {
            ...
            Elog.d(TAG, "parseData " + data[0] + " " + data[1]);
            data[0].trim();
            data[1].trim();
            return data;
        }
