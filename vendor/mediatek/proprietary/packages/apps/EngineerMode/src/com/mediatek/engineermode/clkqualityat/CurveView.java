...
public class CurveView extends View {
    private static final String TAG = "ClkQualityAt/CurveView";
    ...
    private void calculateAxis() {
        ...
        Elog.v(TAG, "mYMin = " + mYMin + " mYMax = " + mYMax +
                " mXMin = " + mXMin + " mXMax = " + mXMax);
    }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        for (int i = 0; i <= mXCount; i++) {
            ...
            Elog.v(TAG, "before mapPoints: px = " + p[0] + "py = " + p[1]);
            m.mapPoints(p);
            Elog.v(TAG, "after mapPoints: px = " + p[0] + "py = " + p[1]);
            p[1] += TEXT_OFFSET_Y;
        ...
        float[] pX = new float[] {mXMin, mYMin, mXMax, mYMin};
        float[] pY = new float[] {mXMin, mYMin, mXMin, mYMax};
        m.mapPoints(pX);
        m.mapPoints(pY);
        // Draw X axis
        canvas.drawLine(pX[0], pX[1], pX[2], pX[3], paint);

        // Draw Y axis
        canvas.drawLine(pY[0], pY[1], pY[2], pY[3], paint);


        // Draw other lines
        paint.setColor(Color.LTGRAY);
        for (int x = mXMin + mXStep; x <= mXMax; x += mXStep) {
            float[] point = new float[] {x, mYMin, x, mYMax};
            m.mapPoints(point);
            canvas.drawLine(point[0], point[1], point[2], point[3], paint);
        }
        for (int y = mYMin + mYStep; y <= mYMax; y += mYStep) {
            float[] point = new float[] {mXMin, y, mXMax, y};
            m.mapPoints(point);
            canvas.drawLine(point[0], point[1], point[2], point[3], paint);
        }
        ...
        canvas.concat(m);
        if (mData.length > 2) {
            canvas.drawLines(mData, paint);
            canvas.drawLines(mData, 2, mData.length - 2, paint);
        } else {
