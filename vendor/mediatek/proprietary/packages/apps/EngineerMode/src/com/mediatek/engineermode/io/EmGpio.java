...
public class EmGpio {
    ...
    private static final String TAG = "IO/EmGpio";
    ...
    public static int newGetCurrent(int hostNumber, int opcode) {
        ...
        Elog.d(TAG, "newGetCurrent: " + response);
        int idx = -1;
        try {
            idx = Integer.parseInt(response);
        } catch (NumberFormatException e) {
            Elog.i(TAG, "parseInt failed--invalid number!");
        }
        return idx;
    }
    ...
    private static String runCmdInNative(int index, int paramNum, int... param) {
        ...
        if (result) {
            ...
            if (r.mReturnCode == AFMFunctionCallEx.RESULT_IO_ERR) {
                Elog.d(TAG, "AFMFunctionCallEx: RESULT_IO_ERR");
                build.replace(0, build.length(), RESULT_ERROR);
            }
        } else {
            Elog.d(TAG, "AFMFunctionCallEx return false");
