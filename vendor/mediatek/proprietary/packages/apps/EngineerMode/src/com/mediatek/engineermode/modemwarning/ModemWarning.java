...
/**
 * Class for setting modem warning.
 *
 */
public class ModemWarning extends Activity {
    private static final String TAG = "ModemWarning";
    private static final String PREF_KEY = "modem_warning_key";
    private static final String PROPERTY = "persist.vendor.radio.modem.warning";
    ...
    private Handler mResponseHander = new Handler() {
        public void handleMessage(Message msg) {
            ...
            case MSG_MODEM_SET:
                ar = (AsyncResult) msg.obj;
                if ((ar.exception != null) && (isActivityAlive())) {
    ...
    private String parseData(String[] data) {
        ...
        Elog.i(TAG, "parseData() return " + ret);
        return ret;
    }

    private void sendAtCommand(String[] cmd, Message msg) {
        Elog.i(TAG, "send AT Command:" + cmd[0]);
        EmUtils.invokeOemRilRequestStringsEm(cmd, msg);
    }

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.modem_warning);
        mCbMdWarning = findViewById(R.id.cbMdWarning);

        mCbMdWarning.setOnClickListener(new OnClickListener() {

            @Override
            public void onClick(View v) {
                // TODO Auto-generated method stub
                String status = mCbMdWarning.isChecked() ? ENABLE : DISABLE;
                Elog.i(TAG, "Set " + PROPERTY + " = " + status);

                try {
                    EmUtils.getEmHidlService().setModemWarningEnable(status);
    ...
    protected void onResume() {
        ...
        Elog.i(TAG, "Get " + PROPERTY + " = " + status);
        mCbMdWarning.setChecked(status.equals(ENABLE));
    }
    ...
    private boolean isActivityAlive() {
        boolean isFinishing = ModemWarning.this.isFinishing();
        boolean isDestroyed = ModemWarning.this.isDestroyed();
        Elog.d(TAG, "isFinishing: " + isFinishing + " isDestroyed: " + isDestroyed);
