...
public class AudioModeSetting extends Activity implements OnClickListener {
    ...
    private class ValLevelItemSelectListener implements OnItemSelectedListener {
        ...
        public void onItemSelected(AdapterView<?> arg0, View arg1,
            ...
            setMaxVolEdit();
            Elog.v(TAG, "SLevel: " + mCurrentMode + " " + mTypeIndex + " "
                    + mLevelIndex);

        }
        @Override
        public void onNothingSelected(AdapterView<?> arg0) {
            Elog.v(TAG, "noting selected.");
        }
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        Elog.v(TAG, "mCurrentMode: " + mCurrentMode + "mSupportEnhance: "
                + mSupportEnhance);
        if (mSupportEnhance) {
            ...
            int dataSize = AudioTuningJni.getAudioCommand(GET_CUSTOMD_DATASIZE);
        ...
        mVolTypeSpinner.setOnItemSelectedListener(new OnItemSelectedListener() {
            ...
            public void onItemSelected(AdapterView<?> arg0, View arg1,
                ...
                Elog.d(TAG, "mTypeIndex is:" + mTypeIndex);
                // Mode:Sph/Sid/Mic
                ...
                mValueEdit.setText(String.valueOf(getValue(mData, mCurrentMode,
                        mTypeIndex, mLevelIndex)));
                setMaxVolEdit();
                Elog.v(TAG, "SMode: " + mCurrentMode + " " + mTypeIndex + " "
                        + mLevelIndex);
                // update level spinner for 6589
                if (mSupportEnhance) {
                    Elog.d(TAG, "itemName: " + itemName);
            ...
            mFirSpinner.setOnItemSelectedListener(new OnItemSelectedListener() {
                ...
                public void onItemSelected(AdapterView<?> arg0, View arg1,
                    ...
                    if (mCurrentMode == 0) { // normal mode
                        ret = AudioTuningJni.setAudioCommand(AUDIO_COMMAND_PARAM0,
                                arg2);
                        Elog.v(TAG, "set normal fir Z" + arg2);
                        // headset mode
                    } else if (mCurrentMode == 1) {
                        ret = AudioTuningJni.setAudioCommand(AUDIO_COMMAND_PARAM1,
                                arg2);
                        Elog.v(TAG, "set headset fir Z" + arg2);
                        // loudspeaker mode
                    } else if (mCurrentMode == 2) {
                        ret = AudioTuningJni.setAudioCommand(AUDIO_COMMAND_PARAM2,
                                arg2);
                        Elog.v(TAG, "set loudspeaker fir Z" + arg2);
                    }
                    if (-1 == ret) {
    ...
    private void setAudioData() {
        int result = AudioTuningJni.setAudioData(sSetCustomerData, sStructSize,
                mData);
        int ret = AudioSystem.setParameters("ReloadAudioVolume");
        if (0 == result || -38 == result) {
            showDialog(DIALOG_ID_SET_SUCCESS);
        } else {
            showDialog(DIALOG_ID_SET_ERROR);
            Elog.i(TAG, "AudioModeSetting SetAudioData return value is : "
                    + result);
        }
    }

    private boolean checkEditNumber(EditText edit, int maxValue) {
        if (null == edit || null == edit.getText()) {
