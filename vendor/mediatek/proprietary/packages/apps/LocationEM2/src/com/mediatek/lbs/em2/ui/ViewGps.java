...
public class ViewGps extends View {
    ...
    public void setGpsStatus(GpsStatus gpsStatus) {
        ...
        for (GpsSatellite sat : gpsSat) {
            if (satCount >= MAX_SATELLITE_NUM) {
                log("ERR: the number of satellite is over 32");
                break;
            }
            SvData svData = new SvData();
            svData.svid = sat.getPrn();
            svData.snr = sat.getSnr();
            svData.ele = sat.getElevation();
            svData.azi = sat.getAzimuth();
            svData.used = sat.usedInFix();
            svData.type = getSvType(svData.svid);
            svData.color = getSvColor(svData.svid);
    ...
    private int getSvColor(int svid) {
    ...
    public void resetGpsView() {
        mSatInView = 0;
        invalidate();
    }

    public ViewGps(Context context) {
        this(context, null, 0);
    }
    public ViewGps(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }
    public ViewGps(Context context, AttributeSet attrs, int defStyle) {
