        ...
        if (isValueChanged) {
            ...
            if (!isNullToOff) {
                ...
                if (mCameraProxy == null) {
                    ...
                    if (MTK_CAM_MODE_ON.equals(
                            mSceneMode.getSettingController().queryValue(KEY_MTK_CAM_MODE))
                            && !isThirdPartyIntent(mActivity)) {
                        if (ICameraMode.ModeType.PHOTO.equals(mSceneMode.getCurrentModeType())) {
                            LogHelper.i(TAG, "[configParameters] set " + MTK_CAMERA_MODE
                                    + " as " + CAMERA_MODE_MTK_PRV);
                            parameters.set(MTK_CAMERA_MODE, CAMERA_MODE_MTK_PRV);
                        } else if (ICameraMode.ModeType.VIDEO.equals(mSceneMode.getCurrentModeType())) {
                            LogHelper.i(TAG, "[configParameters] set " + MTK_CAMERA_MODE
                                    + " as " + CAMERA_MODE_MTK_PVO);
