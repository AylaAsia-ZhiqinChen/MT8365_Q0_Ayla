...
public class PSensorCalibration extends Activity implements OnClickListener {
    private static final String TAG = "SH/PSensorCali";
    ...
    private void getCurrentData() {
        ...
        Log.d(TAG, String.format("getPsensorData(), ret %d", mData));

        mMin = EmSensor.getPsensorMinValue();
        Log.d(TAG, String.format("getPsensorMinValue(), ret %d", mMin));

        mMax = EmSensor.getPsensorMaxValue();
        Log.d(TAG, String.format("getPsensorMaxValue(), ret %d", mMax));

        int[] result = new int[2];
        EmSensor.getPsensorThreshold(result);
        Log.d(TAG, String.format("getPsensorThreshold(), ret %d, %d", result[0], result[1]));
        mHigh = result[0];
        mLow = result[1];
    ...
    private void enableButtons(boolean enable) {
        mGetMin.setEnabled(enable);
        mGetMax.setEnabled(enable);
        mDoCalibration.setEnabled(enable);
        mClearCalibration.setEnabled(enable);
    }
