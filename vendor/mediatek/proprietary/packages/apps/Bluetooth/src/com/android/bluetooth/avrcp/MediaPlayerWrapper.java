...
class MediaPlayerWrapper {
    ...
    public interface Callback {
        ...
        void sessionDestroyed(String packageName);
    }
    ...
    PlaybackState getPlaybackState() {
        PlaybackState conState = mMediaController.getPlaybackState();
        PlaybackState.Builder pbb = new PlaybackState.Builder();
        PlaybackState newState = conState;
        if (conState != null && mPackageName.equals("cn.kuwo.player")
               && conState.getState() == PlaybackState.STATE_BUFFERING)
            // when kuwo switch songs, change buffering to paused
            newState = pbb.setState(PlaybackState.STATE_PAUSED,
                                    conState.getPosition(), 1).build();
        return newState;
    }
    ...
    class MediaControllerListener extends MediaController.Callback {
        ...
        public void onSessionDestroyed() {
            ...
            synchronized (mCallbackLock) {
                if (mRegisteredCallback != null) {
                    mRegisteredCallback.sessionDestroyed(mPackageName);
                }
            }
        }
