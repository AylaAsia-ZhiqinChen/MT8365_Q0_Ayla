        ...
        try {
            ...
            if (type.equals(TYPE_MESSAGE_UPDATE)) {
                ...
                if (mSimManager.getSubCount() == 0) {
                    ...
                    //long messageSubId = SubscriptionManager.getDefaultSmsSubscriptionId();
                    ...
                    if (messageSubId == SubscriptionManager.INVALID_SUBSCRIPTION_ID
                            || messageSubId == MtkSettingsExt.System.DEFAULT_SIM_SETTING_ALWAYS_ASK) {
                        // always ask, show SUB selection dialog
                        return pushMessage(op, name, appParams, mMessageVersion, false);
                    } else if (messageSubId == (int) MtkSettingsExt.System.SMS_SIM_SETTING_AUTO) {
                        return pushMessage(op, name, appParams, mMessageVersion, true);
                    } else {
                        subId = messageSubId;
                    }
                }
                ...
                return pushMessageGemini(op, name, appParams, mMessageVersion, subId);
                /** @} */
...
/** M: New Feature for MAP Gemini @{ */
    private int pushMessage(final Operation op, String folderName,
            BluetoothMapAppParams appParams, String messageVersion, boolean isAutoSelectSub) {
        if (appParams.getCharset() == BluetoothMapAppParams.INVALID_VALUE_PARAMETER) {
            if (D) Log.d(TAG, "Missing charset - unable to decode message content." +
            ...
            */
            ...
            if (subId <= BluetoothMapSimManager.INVALID_SUBID) {
                ...
                mCallback.post(new Runnable() {
                    @Override
                    public void run() {
                        showSubSelectDialog(op, finalFolderName, finalAppParams, finalMessage,
                                            finalObserver, finalMsgStream);
                    }
                });
            } else {
                /// M: VALID Originators subId
                return sendMessage(op, folderName, appParams, message, mObserver, bMsgStream,
                                   subId);
            }
        } catch (IllegalArgumentException e) {
    ...
    private int pushMessageGemini(final Operation op, String folderName,
            BluetoothMapAppParams appParams, String messageVersion, long subId) {
/** @} */
        if(appParams.getCharset() == BluetoothMapAppParams.INVALID_VALUE_PARAMETER) {
        ...
        try {
            ...
            /** M: New Feature for MAP Gemini @{ */
            long handle = mObserver.pushMessage(message, folderElement,
                    appParams, mBaseUriString, subId);
            /** @} */
            if (D) {
    ...
    private int sendMessage(final Operation op, final String folderName,
        ...
        try {
            BluetoothMapFolderElement folderElement = getFolderElementFromName(folderName);
            long handle = observer.pushMessage(message, folderElement,
                    appParams, mBaseUriString, subId);
            if (D)
            ...
            String handleStr = BluetoothMapUtils.getMapHandle(handle, message.getType());
            if (D)
                Log.d(TAG, "[sendMessage] handleStr: " + handleStr + " message.getType(): " +
                      message.getType());
            replyHeaders.setHeader(HeaderSet.NAME, handleStr);
            op.sendHeaders(replyHeaders);
    ...
    public void showSubSelectDialog(final Operation op, final String folderName,
            ...
            public final void onClick(DialogInterface dialog, int which) {
                ...
                new Thread(new Runnable() {
                    ...
                    public void run() {
                        ...
                        sendMessage(op, folderName, appParams, message, observer, bMsgStream,
                                    subId);
                    }
                }).start();
                dialog.dismiss();
        ...
        subSelectDialog.getWindow().setType(WindowManager.LayoutParams.TYPE_APPLICATION_OVERLAY);
