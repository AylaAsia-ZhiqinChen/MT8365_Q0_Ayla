...
public class ArpPeer {
    ...
    public ArpPeer(String interfaceName, InetAddress myAddr, InetAddress peer)
        ...
        /// M: ALPS03774229: prevent init() error cause JE  @{
    ...
    private boolean initInterface() {
        try {
            ...
            /// M: ALPS04021407: WIFI destroy interface due to flight mode turning on,
            /// So check mIface is not null to prevent NullPointerExcpetion error. @{
            if (mIface != null) {
                mHwAddr = mIface.getHardwareAddress();
            } else {
                Log.e(TAG, "mIface is null for name:" + mIfaceName);
                return false;
            }
            /// @}
            Log.i(TAG, "mac addr:" + HexDump.dumpHexString(mHwAddr) + ":" + mIface.getIndex());
            mInterfaceBroadcastAddr = makePacketSocketAddress(mIface.getIndex(),
                    L2_BROADCAST);
    ...
    private boolean initSocket() {
        try {
            ...
            SocketAddress addr = makePacketSocketAddress((short) ETH_P_ARP,
                                                        mIface.getIndex());
