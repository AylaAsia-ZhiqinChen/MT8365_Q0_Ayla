...
public class A2dpSinkStateMachine extends StateMachine {
    ...
    protected void onConnectionStateChanged(int currentState) {
        ...
        if(currentState == BluetoothProfile.STATE_DISCONNECTED) {
            boolean conection_timeout = SystemProperties.get("bluetooth.autoreconnect","").equals("1");
            if (conection_timeout) {
                log("A2DP sink disconnect by connection timeout!");
                intent.putExtra(BluetoothProfile.EXTRA_DISCONNECT_REASON, 8);
                SystemProperties.set("bluetooth.autoreconnect", "0");
            }
        }
