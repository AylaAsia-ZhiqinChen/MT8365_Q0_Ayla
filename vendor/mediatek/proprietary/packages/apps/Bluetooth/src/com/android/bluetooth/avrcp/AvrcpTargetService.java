...
public class AvrcpTargetService extends ProfileService {
    ...
    class ListCallback implements MediaPlayerList.MediaUpdateCallback,
                                ...
                                MediaPlayerSettings.PlayerSettingsUpdateCallback{
        @Override
        ...
        @Override
        public void run(boolean playerSettings) {
            if (mNativeInterface == null) return;
            mNativeInterface.sendPlayerAppSettingsUpdate(playerSettings);
        }
    }
    ...
    protected boolean start() {
        ...
        mMediaPlayerSettings = new MediaPlayerSettings(Looper.myLooper(), this);
        ...
        if (userManager.isUserUnlocked()) {
            mMediaPlayerList.init(new ListCallback());
            mMediaPlayerSettings.init(new ListCallback());
        }
    ...
    protected boolean stop() {
        ...
        sInstance = null;
        //** M: Change for bug fix: JE when do stop AvrcpTargetService without start.@{*/
        try {
            unregisterReceiver(mReceiver);
        } catch (IllegalArgumentException e) {
            Log.e(TAG, "Avrcp Broadcast Receiver not registered.");
        }
        //** @} */

        // We check the interfaces first since they only get set on User Unlocked
        if (mMediaPlayerList != null) mMediaPlayerList.cleanup();
        if (mMediaPlayerSettings != null) mMediaPlayerSettings.cleanup();
        if (mNativeInterface != null) mNativeInterface.cleanup();

        mMediaPlayerList = null;
        mMediaPlayerSettings = null;
    ...
    void setVolume(int avrcpVolume) {
        ...
        //M: UI change when sco + avrcp both change volume
        HeadsetService service = HeadsetService.getHeadsetService();
        if (service != null) {
            isAudioOn = service.isAudioOn();
        }
        if (isAudioOn) {
            Log.d(TAG, "setVolume: SCO on, not show volume UI");
    ...
    //PDU ID 0x11
    void getListPlayerAttribute(MediaPlayerSettings.GetListPlayerAttributeCallback cb){
        mMediaPlayerSettings.getListPlayerAttribute(cb);
    }

    //PDU ID 0x12
    void getListPlayerAttributeValues(byte attr,
            MediaPlayerSettings.GetListPlayerAttributeValuesCallback cb){
        mMediaPlayerSettings.getListPlayerAttributeValues(attr, cb);
    }

    //PDU ID 0x13
    void getPlayerAttributeValue(byte attr,int[] arr,
            MediaPlayerSettings.GetPlayerAttributeValueCallback cb){
        mMediaPlayerSettings.getPlayerAttributeValue(attr, arr, cb);
    }

    //PDU ID 0x14
    void setPlayerAppSetting(byte num, byte [] attrId, byte [] attrVal,
            MediaPlayerSettings.SetPlayerAppSettingCallback cb){
        mMediaPlayerSettings.setPlayerAppSetting(num, attrId, attrVal, cb);
    }

    //PDU ID 0x15
    void getPlayerAttributeText(byte attr, byte [] attrIds,
            MediaPlayerSettings.GetPlayerAttributeTextCallback cb){
        mMediaPlayerSettings.getPlayerAttributeText(attr, attrIds, cb);
    }

    //PDU ID 0x16
    void getPlayerAttributeTextValue(byte attrId, byte numValue, byte [] value,
            MediaPlayerSettings.GetPlayerAttributeValueTextCallback cb){
        mMediaPlayerSettings.getPlayerAttributeTextValue(attrId, numValue, value, cb);
    }

    //Get app setting change
    void getAppSettingChange(MediaPlayerSettings.GetAppSettingChangeCallback cb){
        mMediaPlayerSettings.getAppSettingChange(cb);
    }
    /** @} */
