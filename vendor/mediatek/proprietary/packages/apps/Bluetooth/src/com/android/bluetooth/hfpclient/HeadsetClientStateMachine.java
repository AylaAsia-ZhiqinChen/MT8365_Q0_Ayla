...
/// M ALPS04899018: add this to forbide no calling dialog when we got unexpected call  @{
...
/// @}
    ...
    // Timeouts.
    @VisibleForTesting
    static final int CONNECTING_TIMEOUT_MS = 12000;  // 10s
...
/// M ALPS04899018: add for app sdk version decrease @{
...
/// @}
    ...
    private void returnAudioFocusIfNecessary() {
        ...
        if (mAudioManager.getMode() != AudioManager.MODE_NORMAL) {
            mAudioManager.setMode(AudioManager.MODE_NORMAL);
            Log.d(TAG, "abandon audiofocus for HFP CALL");
            mAudioManager.abandonAudioFocusForCall();
        }
...
///@}
            /// M: stop ringtone when disconnected
            if (mRingtone != null && mRingtone.isPlaying()) {
                Log.d(TAG, "stopping ring when disconnected");
                mRingtone.stop();
                //abandon audio focus
                if (mAudioManager.getMode() != AudioManager.MODE_NORMAL) {
                    mAudioManager.setMode(AudioManager.MODE_NORMAL);
    ...
    class Connected extends State {
        ...
        public synchronized boolean processMessage(Message message) {
            ...
            switch (message.what) {
                ...
                case SET_MIC_VOLUME:
                    int ammicVol = message.arg1;
                    NativeInterface.setVolumeNative(getByteAddress(mCurrentDevice),
                            HeadsetClientHalConstants.VOLUME_TYPE_MIC, ammicVol);
                    break;
...
/// M ALPS04899018: add this to forbide no calling dialog when we got unexpected call  @{
                            if (unexpected_call) {
                                Log.d(TAG, "unexpected incoming call");
...
/// @}
                        ...
                        case StackEvent.EVENT_TYPE_IN_BAND_RINGTONE:
                            ...
                            mInBandRingtone = HeadsetClientHalConstants.IN_BAND_RING_PROVIDED;
                            break;
                        case StackEvent.EVENT_TYPE_RING_INDICATION:
                            ...
                            if (currMode != newAudioMode) {
                                 ...
                                 Log.d(TAG, "mAudioManager Setting audio mode from "
                                    + currMode + " to " + newAudioMode);
                                 mAudioManager.setMode(newAudioMode);
                            }
                            if (mInBandRingtone == HeadsetClientHalConstants.IN_BAND_RING_NOT_PROVIDED && mRingtone != null) {
                                Log.d(TAG,"ring start playing");
                                mRingtone.play();
                            }
                            break;
                        default:
