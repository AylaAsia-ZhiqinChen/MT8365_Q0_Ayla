...
public class AdapterService extends Service {
    ...
    private static final boolean VERBOSE = SystemProperties
    ...
    public void onCreate() {
        ...
        mHandlerThread = new HandlerThread("CLEANUP_NATIVE_TIMEOUT_THREAD");
        mHandlerThread.start();
        mHandlerForCleanUpTimeout = new Handler(mHandlerThread.getLooper()) {
            @Override
            public void handleMessage(Message msg) {
                debugLog("handleMessage() - Message: " + msg.what);

                switch (msg.what) {
                    case MESSAGE_CLEANUP_NATIVE_TIMEOUT:
                        debugLog("MESSAGE_CLEANUP_NATIVE_TIMEOUT");
                        onDestroy();
                        break;
                }
            }
    ...
    void stateChangeCallback(int status) {
        if (status == AbstractionLayer.BT_STATE_OFF) {
            ...
            /** M: Send BLE_STOPPED after disableNative completed. @{ */
            debugLog("send BLE_STOPPED after disableNative() completed.");
    ...
    void updateAdapterState(int prevState, int newState) {
        ...
        if (newState == BluetoothAdapter.STATE_BLE_ON
            ...
            /// M: reset adapter and profile connection state
            mAdapterProperties.onBluetoothDisableDone();
        }
    ...
    void cleanup() {
        ...
        if (mNativeAvailable) {
            ...
            Message m = mHandlerForCleanUpTimeout.obtainMessage(MESSAGE_CLEANUP_NATIVE_TIMEOUT);
            mHandlerForCleanUpTimeout.sendMessageDelayed(m, CLEANUP_NATIVE_TIMEOUT_DELAY);
            cleanupNative();
            mHandlerForCleanUpTimeout.removeMessages(MESSAGE_CLEANUP_NATIVE_TIMEOUT);
    ...
    boolean disable() {
        ...
        // M: ALPS03759771: Tell BT Stack that we are closing.
    ...
    boolean createBond(BluetoothDevice device, int transport, OobData oobData) {
        ...
        // M: ALPS03386888: make sure BT ON when pairing
        if (getState() == BluetoothAdapter.STATE_OFF || getState() ==
                BluetoothAdapter.STATE_BLE_TURNING_ON || getState() ==
