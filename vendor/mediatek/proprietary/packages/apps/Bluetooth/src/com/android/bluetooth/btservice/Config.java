...
public class Config {
    ...
    private static final String SAP_ENABLE_PROPERTY = "ro.vendor.mtk.bt_sap_enable";
    ...
    private static final ProfileConfig[] PROFILE_SERVICES_AND_FLAGS = {
            ...
            new ProfileConfig(MeshService.class, R.bool.profile_supported_mesh,
                    (1 << BluetoothProfile.MESH))
    };
    ...
    static void init(Context ctx) {
        ...
        for (ProfileConfig config : PROFILE_SERVICES_AND_FLAGS) {
            ...
            /// * M for enabling SAP profile by project * ///
            if (SystemProperties.getBoolean(SAP_ENABLE_PROPERTY, false)
                 && (config.mClass == SapService.class)
                 && !isProfileDisabled(ctx, config.mMask)
                 && supported) {
                Log.v(TAG, "SAP Profile is enabled");
                Log.v(TAG, "Adding " + config.mClass.getSimpleName());
                profiles.add(config.mClass);
            /// * M for enabling SAP profile by project * ///
            } else if (supported && !isProfileDisabled(ctx, config.mMask)) {
                Log.v(TAG, "Adding " + config.mClass.getSimpleName());
                profiles.add(config.mClass);
            }
