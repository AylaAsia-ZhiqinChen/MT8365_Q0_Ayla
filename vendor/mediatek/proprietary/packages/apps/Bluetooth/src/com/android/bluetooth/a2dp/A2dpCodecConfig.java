...
class A2dpCodecConfig {
    ...
    void setCodecConfigPreference(BluetoothDevice device,
        ...
        /// M : Fix LDAC switch to AAC/SBC fail.
        /*
        ...
        if (prioritizedCodecType == currentCodecConfig.getCodecType()
                ...
                /// M :Check LDAC's mCodecSpecific1 @{
                if ((BluetoothCodecConfig.SOURCE_CODEC_TYPE_LDAC == codecConfig.getCodecType())
                        && (currentCodecConfig.getCodecSpecific1() == codecConfig.getCodecSpecific1())) {
                    // Same codec with same parameters, no need to send this request to native.
                    Log.i(TAG, "LDAC setCodecConfigPreference: codec not changed.");
                    return;
                } else if (BluetoothCodecConfig.SOURCE_CODEC_TYPE_LDAC != codecConfig.getCodecType()) {
                    // Same codec with same parameters, no need to send this request to native.
