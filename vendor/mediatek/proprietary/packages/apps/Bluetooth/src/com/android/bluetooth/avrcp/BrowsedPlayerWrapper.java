...
class BrowsedPlayerWrapper {
    ...
    private boolean getFolderItemsInternal(String mediaId, BrowseCallback cb) {
        /** M: MediaID cannot be empty @{ */
        if (mediaId.equals("")) {
            Log.i(TAG, "getFolderItemsInternal: mediaID = " + mediaId);
            return false;
        }
        /** @} */
            ...
            for (MediaItem item : children) {
                ...
                if (item.isBrowsable()) {
                    ...
                    if (titleCharSequence != null) {
                        /** M: Cut browse long attibutes to 150 avoid browse fail @{ */
                        title = Util.toMetadata(item, true).title;
                    }
                    ...
                    return_list.add(new ListItem(Util.toMetadata(item, false)));
                }
            }
