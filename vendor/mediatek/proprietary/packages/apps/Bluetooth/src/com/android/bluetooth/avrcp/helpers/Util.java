...
class Util {
    ...
    public static boolean DEBUG = SystemProperties
    ...
    ///M: Cut long attibutes to 155 avoid carkits IOT issue
    private static final int MAX_ATTRID_TITLE_ALBUM_ARTIST_LENGTH = 155;

    ///M: Cut browse long attibutes to 60 avoid browse fail
    ...
    /** M: Cut long attibutes to 150/60 avoid carkit IOT issue @{ */
    public static Metadata bundleToMetadata(Bundle bundle, int maxAttridLength) {
        if (bundle == null) return empty_data();

        Metadata temp = new Metadata();
        temp.title = bundle.getString(MediaMetadata.METADATA_KEY_TITLE, "Not Provided");
        if (temp.title.length() > maxAttridLength) {
            Log.d(TAG, "title too long " + temp.title.length());
            temp.title = temp.title.substring(0, maxAttridLength);
        }
        temp.artist = bundle.getString(MediaMetadata.METADATA_KEY_ARTIST, "");
        if (temp.artist.length() > maxAttridLength) {
            Log.d(TAG, "artist too long " + temp.artist.length());
            temp.artist = temp.artist.substring(0, maxAttridLength);
        }
        temp.album = bundle.getString(MediaMetadata.METADATA_KEY_ALBUM, "");
        if (temp.album.length() > maxAttridLength) {
            Log.d(TAG, "album too long " + temp.album.length());
            temp.album = temp.album.substring(0, maxAttridLength);
        }
        ...
        if (temp.genre.length() > maxAttridLength) {
            Log.d(TAG, "genre too long " + temp.genre.length());
            temp.genre= temp.genre.substring(0, maxAttridLength);
        }
    ...
    public static Metadata toMetadata(MediaSession.QueueItem item) {
        ...
        Metadata ret = bundleToMetadata(bundle, MAX_BROWSE_ATTRID_TITLE_ALBUM_ARTIST_LENGTH);
    ...
    public static Metadata toMetadata(MediaMetadata data) {
        ...
        Metadata ret = bundleToMetadata(bundle, MAX_ATTRID_TITLE_ALBUM_ARTIST_LENGTH);
    ...
    /** M: Cut browse long attibutes to 60 avoid browse fail @{ */
    public static Metadata toMetadata(MediaItem item, boolean Browsable) {
        if (item == null) {
            return empty_data();
        }
        ...
        int maxAttridLength = MAX_BROWSE_ATTRID_TITLE_ALBUM_ARTIST_LENGTH;
        if (Browsable) {
            maxAttridLength = MAX_ATTRID_TITLE_ALBUM_ARTIST_LENGTH;
        }

        Metadata ret = bundleToMetadata(bundle, maxAttridLength);
        ret.mediaId = item.getMediaId();
