...
public class BluetoothMapMessageListingElement
    ...
    public void encode(XmlSerializer xmlMsgElement, boolean includeThreadId)
        ...
        /// fix bug ALPS01427672, check and remove invaild characters for XmlSerializer
        mSubject = BluetoothMapUtils.removeInvalidChar(mSubject);
        mRecipientName = BluetoothMapUtils.removeInvalidChar(mRecipientName);
        mRecipientAddressing = BluetoothMapUtils.removeInvalidChar(mRecipientAddressing);
        mSenderName = BluetoothMapUtils.removeInvalidChar(mSenderName);
        mSenderAddressing = BluetoothMapUtils.removeInvalidChar(mSenderAddressing);
        mReplytoAddressing = BluetoothMapUtils.removeInvalidChar(mReplytoAddressing);

        if (!BluetoothMapUtils.isLegalArgument(mSubject)
                || !BluetoothMapUtils.isLegalArgument(mRecipientName)
                || !BluetoothMapUtils.isLegalArgument(mRecipientAddressing)
                || !BluetoothMapUtils.isLegalArgument(mSenderName)
                || !BluetoothMapUtils.isLegalArgument(mSenderAddressing)
                || !BluetoothMapUtils.isLegalArgument(mReplytoAddressing)) {
            return;
        }
