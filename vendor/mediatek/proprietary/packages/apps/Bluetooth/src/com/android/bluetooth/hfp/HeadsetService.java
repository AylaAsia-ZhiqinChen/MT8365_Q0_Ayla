...
public class HeadsetService extends ProfileService {
    ...
    private Timer mPhoneStateChangeTimer;
    ...
    /* IOT problem: CLCC response delay 50 millisecond after sco created */
    private static final int CLCC_RESPONSE_DELAY_TIME_MSEC = 50;
    ...
    protected boolean start() {
        Log.i(TAG, "start()");
        if (mStarted) {
            /** M: Remove unnecessary exception when HeadsetService start() called twice */
    ...
    protected boolean stop() {
        ...
        synchronized (mStateMachines) {
            ...
            mActiveDevice = null;
            ///M: lets telecom clean the active device @{
        ...
        /// cancel phone state change timer thread when stop handler thread @{
        synchronized (mStateMachines) {
            mStateMachinesThread.quitSafely();
            mStateMachinesThread = null;
            if (mPhoneStateChangeTimer != null) {
                mPhoneStateChangeTimer.cancel();
                mPhoneStateChangeTimer = null;
            }
        }
        /// @}
        // Step 1: Clear
    ...
    ///M: IOT need process CLCC ressponse on connecting state @{ 1/2
        ...
        synchronized (mStateMachines) {
            for (HeadsetStateMachine stateMachine : mStateMachines.values()) {
                if (stateMachine.getConnectionState() == BluetoothProfile.STATE_CONNECTED
                        || stateMachine.getConnectionState() == BluetoothProfile.STATE_CONNECTING) {
                    devices.add(stateMachine.getDevice());
                }
            }
        }
        Log.d(TAG, "getConnectingConnectedDevices: " + devices);
        return devices;
    }

    private void doForEachConnectingConnectedStateMachine(StateMachineTask task) {
        synchronized (mStateMachines) {
            for (BluetoothDevice device : getConnectingConnectedDevices()) {
                task.execute(mStateMachines.get(device));
            }
        }
    }
                ...
                case BluetoothDevice.ACTION_CONNECTION_ACCESS_REPLY: {
                    ...
                    if (requestType == BluetoothDevice.REQUEST_TYPE_PHONEBOOK_ACCESS) {
                        synchronized (mStateMachines) {
                            ...
                            if (stateMachine == null) {
                                // M: remove wtfcheck to avoid crash, add Log.w instead.
                                //Log.wtfStack(TAG, "Cannot find state machine for " + device);
                                Log.w(TAG, "Cannot find state machine for " + device);
                                return;
                            }
        ...
        synchronized (mStateMachines) {
            ...
            if (connectingConnectedDevices.size() >= mMaxHeadsetConnections) {
                ...
                if (mMaxHeadsetConnections == 1) {
                    /* M: if not the same device, disconnect it {@ */
                    if (!connectingConnectedDevices.get(0).equals(device))
    ...
    private boolean startScoUsingVirtualVoiceCall() {
        ...
        synchronized (mStateMachines) {
            ...
            ///M: Add delay to phone state changed@{
            phoneStateChangedDelay();
            /// @}
            return true;
        }
    }

    private void phoneStateChangedDelay() {
        mPhoneStateChangeTimer = new Timer(true);
        mPhoneStateChangeTimer.schedule(new TimerTask() {
            public void run() {
                synchronized (mStateMachines) {
                    phoneStateChanged(0, 0, HeadsetHalConstants.CALL_STATE_ALERTING, "",
                        0, "", true);
                }
            }
        }, VOIP_CALL_ALERTING_DELAY_TIME_MSEC);
        mPhoneStateChangeTimer.schedule(new TimerTask() {
            public void run() {
                synchronized (mStateMachines) {
        ...
        }, VOIP_CALL_ACTIVE_DELAY_TIME_MSEC);
    }
    ...
    boolean stopScoUsingVirtualVoiceCall() {
        ...
        synchronized (mStateMachines) {
            ...
            ///M: cancel phone state change timer when stop virtual call @{
            if (mPhoneStateChangeTimer!= null) {
                Log.d(TAG, "cancel phone state change timer when stop virtual call");
                mPhoneStateChangeTimer.cancel();
                mPhoneStateChangeTimer = null;
            }
            /// @}
    ...
    private void phoneStateChanged(int numActive, int numHeld, int callState, String number,
        ...
        Log.d(TAG, "numActive=" + numActive + ", numHeld=" + numHeld + ", callState="
                + callState + ", type =" + type + ", name="+ name + ", isVirtual="
                + isVirtualCall);
        ///M: To avoid problems with some headsets/carkits, it needs to suspend A2DP
        // before establishing SCO. And resume A2DP after the call ends@{1/2
    ...
    ///M: IOT problem: need CLCC response after sco created. @{
    private boolean needDelayClcc(HeadsetStateMachine stateMachine) {
        BluetoothDevice device = stateMachine.getDevice();
        String devAddrPrefix = device.getAddress().substring(0, 8);
        if (devAddrPrefix.compareTo("F4:15:FD") == 0) {
            Log.d(TAG, devAddrPrefix + ": IOT, need delay clcc response!!!");
            return true;
            }
        return false;
    }
    ...
    private void clccResponse(int index, int direction, int status, int mode, boolean mpty,
            String number, int type) {
        enforceCallingOrSelfPermission(MODIFY_PHONE_STATE, "Need MODIFY_PHONE_STATE permission");
        ///M: IOT need process CLCC ressponse on connecting state 2/2 @{
        doForEachConnectingConnectedStateMachine((stateMachine) -> {
            if(needDelayClcc(stateMachine) == true){
                ///M: IOT problem: need CLCC response after sco created. @{
                stateMachine.sendMessageDelayed(HeadsetStateMachine.SEND_CCLC_RESPONSE,
                    new HeadsetClccResponse(index, direction, status, mode, mpty, number, type),
                    CLCC_RESPONSE_DELAY_TIME_MSEC);
                /// @}
                }else{
                    stateMachine.sendMessage(HeadsetStateMachine.SEND_CCLC_RESPONSE,
