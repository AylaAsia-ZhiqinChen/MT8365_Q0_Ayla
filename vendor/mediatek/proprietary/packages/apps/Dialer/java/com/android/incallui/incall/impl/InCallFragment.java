...
public class InCallFragment extends Fragment
  ...
  public View onCreateView(
    ...
    phoneType = TelephonyManager.getPhoneType(voiceNetworkType);
    /// @}
  ...
  public void setSecondary(@NonNull SecondaryInfo secondaryInfo) {
    ...
    if (needInsetSecondaryView) {
      getActivity().getWindow().getDecorView().requestApplyInsets();
      needInsetSecondaryView = false;
      LogUtil.i("InCallFragment.setSecondary", "requestApplyInsets");
    }
    /// @}

    /// M: [Performance] Do not update button state every time. @{
    /// Google code: updateButtonStates();
  ...
  public void onAudioRouteSelected(int audioRoute) {
    /// M: Add debug info
    LogUtil.d("InCallFragment.onAudioRouteSelected", "audioRoute: " + audioRoute);
  ...
  /// M: ----------------------------MediaTek feature --------------------------
  private boolean needInsetSecondaryView = true;
  ...
  private void updateInfoForButtonChooser(int state) {
    ...
    // dut to performance. @{
    if (call != null && !DialerCallState.isConnecting(state)) {
  ...
  /**
   * M: [Voice Record] updateRecordStateUi
   * @param isRecording
   */
  @Override
  public void updateRecordStateUi(boolean isRecording) {
    LogUtil.d("InCallFragment.updateRecordStateUi", "[updateRecordStateUi]... " + isRecording);

    DialerCall ringCall = CallList.getInstance().getIncomingCall();
    if (isRecording && (ringCall == null)) {
  ...
  private ImageView voiceRecorderIcon;

  /**
   * M: [Voice Record] initVoiceRecorderIcon
   * @param view
   */
  private void initVoiceRecorderIcon(View view) {
    voiceRecorderIcon = (ImageView) view.findViewById(R.id.voiceRecorderIcon);
    voiceRecorderIcon.setImageResource(R.drawable.voice_record_indicator);
    voiceRecorderIcon.setVisibility(View.INVISIBLE);
  }

  /**
   * M: [Voice Record] updateVoiceRecordIcon
   * @param show need to show
   */
  public void updateVoiceRecordIcon(boolean show) {
    voiceRecorderIcon.setVisibility(show ? View.VISIBLE : View.INVISIBLE);
    AnimationDrawable ad = (AnimationDrawable) voiceRecorderIcon.getDrawable();
    if (ad != null) {
