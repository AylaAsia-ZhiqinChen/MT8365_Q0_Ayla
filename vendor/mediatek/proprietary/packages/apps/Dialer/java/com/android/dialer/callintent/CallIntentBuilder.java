...
public class CallIntentBuilder implements Parcelable {
  ...
  private final String TAG = "CallIntentBuilder";
  ///M: For RTT call @{
  ...
  /// M: For RTT call @{
  public CallIntentBuilder setIsRTTCall(boolean isRTTCall) {
    LogUtil.i(TAG, "setIsRTTCall, isRTTCall = " + isRTTCall);
  ...
  public Intent build() {
    ...
    /// M: For RTT call @{
    if (isRTTCall) {
        LogUtil.i(TAG, "put EXTRA_START_CALL_WITH_RTT = true in EXTRA_OUTGOING_CALL_EXTRAS");
        inCallUiIntentExtras.putBoolean(TelecomManager.EXTRA_START_CALL_WITH_RTT, true);
    }
    /// @}
