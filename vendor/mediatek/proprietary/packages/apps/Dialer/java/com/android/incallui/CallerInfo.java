...
public class CallerInfo {
  ...
  public static CallerInfo getCallerInfo(Context context, Uri contactRef, Cursor cursor) {
    ...
    if (columnIndex != -1) {
      ...
      if (typeColumnIndex != -1) {
        ...
        ///         .toString();
        info.phoneLabel =
            PhoneCompat.getTypeLabel(context, info.numberType, info.numberLabel)
                .toString();
        /// @}
      }
    }
  ...
  private static int getColumnIndexForPersonId(Uri contactRef, Cursor cursor) {
    ...
    if (url.startsWith("content://com.android.contacts/data/phones")) {
      ...
      Log.v(TAG, "Unexpected prefix for contactRef");
      /// @}
    }
  ...
  /* package */ CallerInfo markAsVoiceMail(Context context) {
    ...
    try {
      ...
      phoneNumber = null;
      /// M: ALPS03605975: Add debug info for voice mail @{
  ...
  /**
   * Enhance for markAsVoiceMail(Context context) by subscription.
   *
  ...
  /* package */ CallerInfo markAsVoiceMail(Context context, int subId) {
    ...
    try {
      name = telephonyManager.getVoiceMailAlphaTag(subId);
      phoneNumber = null;
      Log.d(TAG, "getVoiceMailAlphaTag:" + name + ", subId:" + subId);
    } catch (SecurityException se) {
      // Should never happen: if this process does not have
      // permission to retrieve VM tag, it should not have
      // permission to retrieve VM number and would not call
