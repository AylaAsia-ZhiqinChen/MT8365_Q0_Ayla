...
public class ContactsCommonListUtils {
    ...
    public static DefaultImageRequest getDefaultImageRequest(Cursor cursor, String displayName,
        ...
        DefaultImageRequest request = new DefaultImageRequest(displayName, lookupKey,
            circularPhotos);
        int subIdIndex = cursor.getColumnIndex(RawContactsCompat.INDICATE_PHONE_SIM);
        if (subIdIndex != -1) {
            final int subId = cursor.getInt(subIdIndex);
            if (subId > 0) {
                request.subId = subId;
    ...
    private static int getSdnPhotoId(Cursor cursor) {
        int sdnId = 0;
        int isSdnContact = cursor.getInt(
            cursor.getColumnIndexOrThrow(RawContactsCompat.IS_SDN_CONTACT));
        if (isSdnContact > 0) {
