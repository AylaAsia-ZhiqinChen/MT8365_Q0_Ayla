...
interface ButtonController {
  ...
  abstract class CheckableButtonController implements ButtonController, OnCheckedChangeListener {
    ...
    public void setButton(CheckableLabeledButton button) {
      ...
      if (button != null) {
        button.setEnabled(isEnabled);
        /// M: [Button Extension] set to GONE @{
        // button.setVisibility(isAllowed ? View.VISIBLE : View.INVISIBLE);
        button.setVisibility(isAllowed ? View.VISIBLE : View.GONE);
        /// @}
        button.setChecked(isChecked);
  ...
  class SpeakerButtonController
    ...
    public void onCheckedChanged(CheckableLabeledButton checkableLabeledButton, boolean isChecked) {
      ...
      /// M: Add debug info
      LogUtil.d("SpeakerButtonController.onCheckedChanged", null);
  ...
  class UpgradeToVideoButtonController extends SimpleNonCheckableButtonController {
    ...
    public UpgradeToVideoButtonController(@NonNull InCallButtonUiDelegate delegate) {
      super(
          ...
          /// M: [One way video] Change two way icon if one way video call is supported. @{
          (ExtensionManager.getInCallButtonExt().isOneWayVideoSupported() ?
              R.drawable.mtk_ic_two_way_video : R.drawable.quantum_ic_videocam_vd_theme_24));
          /// @}
      Assert.isNotNull(delegate);
    }
  ...
  class HangupAllButtonController extends SimpleNonCheckableButtonController {
    ...
    public HangupAllButtonController(@NonNull InCallScreenDelegate delegate) {
      ...
      Assert.isNotNull(delegate);
          inCallScreenDelegate = delegate;
    }

    @Override
    public void onClick(View v) {
      inCallScreenDelegate.onHangupAllClicked();
    }
  }
  ...
  class HangupHoldButtonController extends SimpleNonCheckableButtonController {
    ...
    @Override
    public void onClick(View v) {
      inCallScreenDelegate.onHangupHoldClicked();
    }
  }

  /**
   * M: [One way video]
   * One way video button controller.
   */
  class UpgradeToOneWayVideoButtonController extends SimpleNonCheckableButtonController {

    public UpgradeToOneWayVideoButtonController(@NonNull InCallButtonUiDelegate delegate) {
      super(
          delegate,
          InCallButtonIds.BUTTON_ONE_WAY_VIDEO,
          0,
          R.string.menu_oneway_videocall,
          R.drawable.mtk_ic_one_way_video);
      Assert.isNotNull(delegate);
    }

    @Override
    public void onClick(View view) {
      delegate.changeToOneWayVideoClicked();
    }
  }
