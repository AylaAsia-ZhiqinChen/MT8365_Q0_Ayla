...
public final class SearchAdapter extends RecyclerView.Adapter<ViewHolder> {
  ...
  public ViewHolder onCreateViewHolder(ViewGroup root, @RowType int rowType) {
    switch (rowType) {
      ///M: For smart search @{
      case RowType.MTK_CONTACT_ROW:
        return new SmartSearchContactViewHolder(
            LayoutInflater.from(context).inflate(R.layout.mtk_dialer_search_item_view, root, false),
            rowClickListener);
      ///@}
  ...
  public void onBindViewHolder(ViewHolder holder, int position) {
    ///M: for smart search @{
    if (holder instanceof SmartSearchContactViewHolder) {
        ((SmartSearchContactViewHolder) holder).bind(searchCursorManager.getCursor(position), context);
        
       // ((SmartSearchContactViewHolder) holder).bindContactView(searchCursorManager.getMtkCursor(position), context,query);
    } /*else if (holder instanceof CallLogSearchViewHolder) {
        ((SmartSearchContactViewHolder) holder).bindCallLogView(searchCursorManager.getMtkCursor(position), context,query);
    }*/
    //@}
    else 
  ...
  public void setIsSmartSearch(boolean isSmartSearch) {
    searchCursorManager.setIsSmartSearch(isSmartSearch);
  }
  /// @}
