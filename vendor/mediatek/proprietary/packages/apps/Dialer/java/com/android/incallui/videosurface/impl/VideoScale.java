...
public class VideoScale {
  ...
  /// call's quality test. @{
  public static void scaleVideoMaintainingAspectRatioWithOrientation(
      TextureView textureView, float videoWidth, float videoHeight, float rotationDegrees) {
    float viewWidth = textureView.getWidth();
    float viewHeight = textureView.getHeight();
    float scaleWidth = 1.0f;
    ...
    if (viewWidth > viewHeight) {
      ...
      if (viewHeight * videoWidth > viewWidth * videoHeight) {
        // Current display height is too much. Correct it.
        float desiredHeight = viewWidth * videoHeight / videoWidth;
        scaleWidth = desiredHeight / viewHeight;
      } else if (viewHeight * videoWidth < viewWidth * videoHeight) {
        // Current display width is too much. Correct it.
        float desiredWidth = viewHeight * videoWidth / videoHeight;
        scaleWidth =  desiredWidth / viewWidth;
      }
    } else {
      // Portrait layout.
      if (viewHeight * videoWidth > viewWidth * videoHeight) {
        // Current display height is too much. Correct it.
        float desiredHeight = viewWidth * videoHeight / videoWidth;
        scaleHeight = desiredHeight / viewHeight;
      } else if (viewHeight * videoWidth < viewWidth * videoHeight) {
        // Current display width is too much. Correct it.
        float desiredWidth = viewHeight * videoWidth / videoHeight;
        scaleHeight = desiredWidth / viewWidth;
      }
    }

    LogUtil.i(
        "VideoScale.scaleVideoMaintainingAspectRatioWithOrientation",
        "view: %f x %f, video: %f x %f scale: %f x %f, rotation: %f",
        viewWidth,
        viewHeight,
        videoWidth,
