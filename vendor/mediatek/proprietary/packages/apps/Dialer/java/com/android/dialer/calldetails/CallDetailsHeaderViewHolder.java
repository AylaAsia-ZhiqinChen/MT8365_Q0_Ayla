...
public class CallDetailsHeaderViewHolder extends RecyclerView.ViewHolder
  ...
  CallDetailsHeaderViewHolder(
    ...
    /// M:For op01,op07,op08,op09 video call button customization. @{
    callDetailsHeaderViewHolderExt = ExtensionManager.getCallDetailsExt()
            .getCallDetailsHeaderViewHolderExt(context, this);
    /// @}
  }
  ...
  private void setCallbackAction(@CallbackAction int callbackAction) {
    ...
    switch (callbackAction) {
      ...
      case CallbackAction.IMS_VIDEO:
        ...
        /// M:For op01,op07,op08,op09 video call button customization.
        callDetailsHeaderViewHolderExt.setCallbackAction(
            callbackButton, callbackAction, number);
        /// @}
        break;
  ...
  public void onClick(View view) {
    if (view == callbackButton) {
      switch (callbackAction) {
        case CallbackAction.IMS_VIDEO:
          callDetailsHeaderListener.placeImsVideoCall(number + postDialDigits);///M: add post digit
          break;
        case CallbackAction.DUO:
