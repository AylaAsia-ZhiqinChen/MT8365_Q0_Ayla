...
public class PostCall {
  ...
  /// M: use called phone number to send message
  ...
  private static void promptUserToSendMessage(Activity activity, View rootView) {
    ...
    int subId = getSubid(activity);//M:add
  ...
  private static void clear(Context context) {
    ...
    StorageComponent.get(context)
        ...
        .remove(KEY_POST_CALL_CALL_SUBID)
        .apply();
  }
  ...
  private static boolean shouldPromptUserToSendMessage(Context context) {
    ...
    ///M:fix ALPS03946884
    manager.edit().remove(KEY_POST_CALL_DISCONNECT_PRESSED).apply();
    ...
    return disconnectTimeMillis != -1
        ...
        /// M: if sub_id <0, dont show send message view
        && (getSubid(context) > 0);
  }
  ...
  ///M:add send message use phone number accroding to subid
  private static int getSubid(Context context) {
    int subId = StorageComponent.get(context)
        .unencryptedSharedPrefs()
        .getInt(KEY_POST_CALL_CALL_SUBID, -1);
    LogUtil.i("PostCall", "getSubid:" + subId);
    return subId;
  }
