...
public class PhoneNumberInteraction implements OnLoadCompleteListener<Cursor> {
  ...
  ///M: Add this method for ALPS04414094 @{
  public interface StartCallListener {
    void onStartCall();
  }
  /// @}
  ...
  private static void performAction(
    ...
    if (activity != null && activity instanceof StartCallListener) {
        LogUtil.i("PhoneNumberInteraction","call onStartCall");
      ((StartCallListener) activity).onStartCall();
    }
    /// @}
  }
  ...
  /*private*/ void startInteraction(Uri uri) {
    ...
    // for the permission.
    ///M: Call hasPhoneCallPermission instead of hasPhonePermissions for fix ALPS04649007
    if (!PermissionsUtil.hasPermission(context,new String[] {permission.CALL_PHONE})) {
  ...
  private static class PhoneItemAdapter extends ArrayAdapter<PhoneItem> {
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
      ...
      if (SipAddress.CONTENT_ITEM_TYPE.equals(item.mimeType) ||
        ...
        if (ContactDisplayUtils.INTERACTION_CALL == interactionType) {
