...
public class AudioModeProvider {
  ...
  public void initializeAudioState(Context context) {
    onAudioStateChanged(
        new CallAudioState(false,
            /// M: [Performance] Using default audio route to increase performance.
            /// It cost alot of time when getAudioManager, it cost about 250ms+, so just
            /// use default audio state CallAudioState.ROUTE_EARPIECE instead.
            /// Telecom will notify correct AUDIO_ROUTE immediately. @{
            /// Google code:
            /// getApproximatedAudioRoute(context),
            CallAudioState.ROUTE_EARPIECE,
            /// @}
