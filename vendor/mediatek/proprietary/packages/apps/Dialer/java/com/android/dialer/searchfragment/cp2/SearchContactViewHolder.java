...
public final class SearchContactViewHolder extends ViewHolder implements OnClickListener {
  ...
  public void bind(SearchCursor cursor, String query) {
    ...
    ///M: Add these projection for search sim contacts @{
    int subId = -1;
    int isSdnContact = 0;

    int subIdIndex = cursor.getColumnIndex(ContactsColumns.INDICATE_PHONE_SIM);
    if (subIdIndex != -1) {
        subId = cursor.getInt(subIdIndex);
        LogUtil.i("SearchContactViewHolder.bind", "subId = " + subId);
    }

    int sdnIndex =cursor.getColumnIndex(ContactsColumns.IS_SDN_CONTACT);
    if (sdnIndex != -1) {
	isSdnContact = cursor.getInt(sdnIndex);
        LogUtil.i("SearchContactViewHolder.bind", "isSdnContact = " + isSdnContact);
    }
    /// @}
  ...
  private String getLabel(Resources resources, Cursor cursor) {
    int numberType = cursor.getInt(Projections.PHONE_TYPE);
    String numberLabel = cursor.getString(Projections.PHONE_LABEL);

    ///M: fix issue ALPS04414247. @{
    CharSequence label = null;
    if (DialerFeatureOptions.isSimContactsSupport()) {
        label = ContactsCompat.PhoneCompat.getTypeLabel(context, numberType, numberLabel);
    } else {
        label = Phone.getTypeLabel(resources, numberType, numberLabel);
    }
    /// M: @}
