...
public class VideoCallPresenter
   ...
   */
  private boolean currentVideoRingtoneState = false;
  /**
  ...
  private static void updateCameraSelection(DialerCall call) {
    ...
    /// M: [One way video] do not update camera dir of call if it is already set.
    /// In one way video need to make back facing camera as default for
    /// one way upgrade. But the camera dir gets overridden here. @{
    else if (call.getCameraDir() != CameraDirection.CAMERA_DIRECTION_UNKNOWN) {
      LogUtil.v("VideoCallPresenter.updateCameraSelection",
          " hasSentVideoUpgradeRequest " + call.getCameraDir());
      cameraDir = call.getCameraDir();
    }
    /// @}
  ...
  /**
   * M: ALPS03461190 changed google's default behavior.
  ...
  public void onVideoCallScreenUiUnready() {
    ...
    if (primaryCall != null) {
      ...
      /// call's quality test. @{
      primaryCall.setLargePreview(false);
      /// @}
      /// M: [ALPS04143953] Set camera is null when videocallpresenter remove listener. @{
      if (!primaryCall.isVideoCall()
          && !InCallPresenter.getInstance().isChangingConfigurations()) {
        LogUtil.i(
            "VideoCallPresenter.onVideoCallScreenUiUnready",
            "screen is destoryed and not configure change, closing the camera.");
        enableCamera(primaryCall, false);
        /// M: [ALPS04277428] Reset full screen when videocallfragment destory. @{
  ...
  private void onSurfaceClick() {
    ...
    if (!InCallPresenter.getInstance().isFullscreen()) {
      /// M: ALPS03792907 No need set full screen if call exit video mode. @{
      if (isVideoMode()) {
        InCallPresenter.getInstance().setFullScreen(true);
      } else {
        LogUtil.d("VideoCallPresenter.onSurfaceClick", "no video mode");
      }
      /// @}
    } else {
      InCallPresenter.getInstance().setFullScreen(false);
  ...
  public void onSystemUiVisibilityChange(boolean visible) {
    ...
    LogUtil.d("VideoCallPresenter.onSystemUiVisibilityChange", "visible: " + visible);
    if (visible) {
      InCallPresenter.getInstance().setFullScreen(false);
      maybeAutoEnterFullscreen(primaryCall);
    }
  ...
  public void onCameraPermissionGranted() {
    LogUtil.d("VideoCallPresenter.onCameraPermissionGranted", "");
    PermissionsUtil.setCameraPrivacyToastShown(context);
    /// M: ALPS03441774 fix null point exception @{
    if (primaryCall != null) {
    ///@}
      enableCamera(primaryCall, isCameraRequired());
      showVideoUi(
          /** M: Pass Call to check more infor @{*/ primaryCall, /** @}*/
  ...
  public void onStateChange(
    ...
    if (newState == InCallPresenter.InCallState.INCOMING) {
      ...
      /// M: ALPS03509465 The primary call need change to incoming call just like voice call @{
  ...
  private void checkForCallStateChange(DialerCall call) {
    ...
    if (shouldShowVideoUi) {
      ...
      if ((!Objects.equals(prevCameraId, newCameraId) && isActiveVideoCall(call))
        /// M: ALPS04201744] add set camera when call state change. Answerfragment excutes onstop
        /// trigger disable camera.On Answerfragment start have no chance for enable camera again.
        /// so local video shows black.When Answerfragment do onstart will trigger
        /// onVideoCallScreenUiReady,the currentCallstate will be reset.so the call state always
        ///change.Add enable camera to CheckForCallStateChange. @{
        || (previewSurfaceState == PreviewSurfaceState.NONE && isCameraRequired())) {
        /// @}
  ...
  private void checkForVideoRingtoneStateChange(@Nullable DialerCall call) {
    ...
    final boolean isVideoRingtoneStateChanged = currentVideoRingtoneState != hasVideoRingtone;
    LogUtil.v("VideoCallPresenter.checkForVideoRingtoneStateChange",
        "hasVideoRingtone = " + hasVideoRingtone +
        ", isVideoRingtoneStateChanged = " + isVideoRingtoneStateChanged);
    if (!isVideoRingtoneStateChanged) {
      return;
    }
    currentVideoRingtoneState = hasVideoRingtone;
    // Make sure we hide or show the video UI if needed.
    showVideoUi(
        /** M: Pass Call to check more infor @{*/ call, /** @}*/
        call.getVideoState(),
  ...
  private void adjustVideoMode(DialerCall call) {
    ...
    /** M: [Voice Record][ALPS02812002] stop voice recording during vt call @{ */
  ...
  private void enableCamera(DialerCall call, boolean isCameraRequired) {
    ...
    final DialerCall activeCall = CallList.getInstance().getActiveCall();
    if (((primaryCall != null) && (primaryCall.getVideoPauseState() == true)) ||
        ((activeCall != null) && (activeCall.getVideoPauseState() == true))) {
      LogUtil.i("VideoCallPresenter.enableCamera", " Video is paused by making camera null");
      return;
    }
    /// @}
  ...
  private void showVideoUi(
    ...
    boolean showOutgoingVideo = showOutgoingVideo(context, videoState, sessionModificationState)
        /// M: Pause video without sip message on CT network. @{
        && (call.getVideoPauseState() == false);
        /// @}
    LogUtil.d(
  ...
  public void onUpdatePeerDimensions(DialerCall call, int width, int height) {
    LogUtil.d("VideoCallPresenter.onUpdatePeerDimensions", "width: %d, height: %d", width, height);
    if (videoCallScreen == null) {
  ...
  protected void maybeAutoEnterFullscreen(DialerCall call) {
    ...
    if (call == null
        ...
        /// M: [ALPS03749536] cancel auto full screen when incallactivity onstop. @{
        || !isScreenInForground()) {
        /// @}
  ...
  public boolean shouldShowCameraPermissionToast() {
    ...
    if (!primaryCall.isVideoCallExcludeVideoRingtone()
        /// M: ALPS04427569  Fix can't enable camera permission when upgrading to video or receiving
        // upgrade video request. @{
        && !primaryCall.hasSentVideoUpgradeRequest()
        && !primaryCall.hasReceivedVideoUpgradeRequest()) {
        /// @}
      LogUtil.i("VideoCallPresenter.shouldShowCameraPermissionToast", "earlymedia without camera");
      return false;
    }
  ...
  private class LocalDelegate implements VideoSurfaceDelegate {
    ...
    public void onSurfaceClick(VideoSurfaceTexture videoCallSurface) {
      /// M: [ALPS03837665] add local and peer swap video fuction for support CMCC and CT do video
      /// call's quality test. @{
      LogUtil.i("VideoCallPresenter.LocalDelegate.onSurfaceClick", null);
      if (InCallUtils.MTK_CMCC_VILTE_LAB_TEST || InCallUtils.MTK_CT_VILTE_LAB_TEST) {
        LogUtil.i("VideoCallPresenter.onSurfaceClick", "in cmcc or ct lab test" );
        if (primaryCall.getState() != DialerCallState.ACTIVE) {
          return;
        }

        boolean oldstate = primaryCall.isLargePreview();
        primaryCall.setLargePreview(!oldstate);
        videoCallScreen.switchLocalAndPeer(!oldstate);
      } else {
      /// @}
  ...
  /// call's quality test. @{
  @Override
  public boolean isPreviewLarge() {
    boolean previewLarge = primaryCall!= null && primaryCall.isLargePreview();
    LogUtil.d("VideoCallPresenter.isPreviewLarge", ":" + previewLarge);
    return previewLarge;
  }
  /// @}
  ...
  public void onHidePreviewRequest(boolean hide) {
    ...
    if (primaryCall == null) {
      LogUtil.e("VideoCallPresenter.onHidePreviewRequest",
          "primaryCall is null returning, " + hide);
      return;
    }
    primaryCall.setHidePreview(hide);
    videoCallScreen.hidePreview(hide);
  }
  ...
  private void updatePreviewHideState(DialerCall call) {
    ...
    if (call == null) {
      LogUtil.w("VideoCallPresenter.updatePreviewHideState",
          "primaryCall is null returning");
      return;
    }
    videoCallScreen.hidePreview(isVideoCall(call)
        && (call.isHidePreview() /** M: ALPS04001319 @{ || call.isConferenceCall() @} */
        /// M: CMCC Video ringtone support. @{
        || (currentVideoRingtoneState
    ...
    LogUtil.i("VideoCallPresenter.updatePreviewHideState",
        ...
        isVideoCall(call), call.isHidePreview(), call.isConferenceCall(),
        currentVideoRingtoneState);
  }
  ...
  public void onCallDataUsageChanged(long dataUsage) {
    if (videoCallScreen != null) {
      videoCallScreen.updateVideoDebugInfo(dataUsage);
    }
  }

  /**
   * M: update Local Preview according to the state pause.
   */
  @Override
  public void onPausePreviewRequest(boolean pause) {
    LogUtil.d("VideoCallPresenter.onPausePreviewRequest", "local video in pause : " + pause);
    if (videoCallScreen == null) {
      LogUtil.e("VideoCallPresenter.onPausePreviewRequest",
          "videoCallScreen is null returning");
      return;
    }
    if (primaryCall == null) {
  ...
  public void onSessionModificationStateChanged(int state) {
    ...
    if (state == SessionModificationState.NO_REQUEST) {
      ...
      adjustVideoMode(primaryCall);
    }
  }

  /**
   * M: ALPS03749536, Videocallscreen is in forground or not.
   * @return videocallscreen is in forground or not
   */
  private boolean isScreenInForground() {
    if (videoCallScreen == null) {
      return false;
    }
    InCallActivity incallactivity =  (InCallActivity) videoCallScreen.
        getVideoCallScreenFragment().getActivity();
    if (incallactivity != null && incallactivity.isVisible()) {
      return true;
    }
    LogUtil.d("VideoCallPresenter.isScreenInForground", "false");
    return false;
  }

  /**
   * M: [ALPS03855504] update greenscreen or fullscreen when UI resume
   */
  @Override
  public void updateFullscreenAndGreenScreenModeWhenUIStart() {
    if (primaryCall == null) {
      return;
    }
    updateFullscreenAndGreenScreenMode(
