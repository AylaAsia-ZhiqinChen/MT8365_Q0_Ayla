...
public final class SearchCursorManager {
  ...
  @interface RowType {
    ...
    ///M:  @ {
    int MTK_CONTACT_ROW = 9;
    int MTK_CALLLOG_ROW = 10;
    /// @}
  }
  ...
  ///M: @{
  private boolean mIsSmartSearch = false;

  public void setIsSmartSearch(boolean isSmartSearch) {
    mIsSmartSearch = isSmartSearch;
  }

  private static final String TAG = "SearchCursorManager";
  /// @}
  ...
  private final int CALL_LOG_ID_INDEX = 2;
  ...
  int getRowType(int position) {
    ...
    LogUtil.i("getRowType", "YF: position = " + position);
    if (cursor == contactsCursor) {
       /// M: @{
       if (DialerFeatureOptions.isDialerSearchEnabled() && mIsSmartSearch) {
            LogUtil.i("getRowType", "cursor.isHeader() = " + cursor.isHeader());
            return cursor.isHeader() ? RowType.CONTACT_HEADER : RowType.MTK_CONTACT_ROW;
       }
       /// @}
