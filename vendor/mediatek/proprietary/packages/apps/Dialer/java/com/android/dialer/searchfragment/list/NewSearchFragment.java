...
public final class NewSearchFragment extends Fragment
  ...
  /// M: add for op01/op07/op08/op12 plug-in.
    ...
    private final BroadcastReceiver smartDialUpdatedReceiver =
      new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
          LogUtil.i("NewSearchFragment.onReceive", "smart dial update broadcast received");
          initLoaders();
  ...
  public View onCreateView(
    ...
    /// M: add for op01 plug-in. @{
    searchFragmentExt = ExtensionManager.getDialerSearchExtension()
        .getSearchFragmentExt(this, adapter, adapter::setSearchActions);
    searchFragmentExt.setActions(getActions(), query);//@}
    ...
    if (savedInstanceState != null) {
      setQuery(
          ...
          ///M: Add for ALPS04409841  @{
          isDialpadShown = savedInstanceState.getBoolean(IS_DIALPAD_SHOWN,false);
          /// @}
    }

     ///M: Add for ALPS04409841  @{
     if (emptyContentView != null && !PermissionsUtil.hasContactsReadPermissions(getContext())) {
        LogUtil.i("NewSearchFragment.onCreateView", "isDialpadShown = " + isDialpadShown);
        emptyContentView.showDialpadSpace(isDialpadShown);
     }
     /// @}
  ...
  public void onSaveInstanceState(Bundle outState) {
    ...
    ///M: Add for ALPS04409841  @{
    outState.putBoolean(IS_DIALPAD_SHOWN,isDialpadShown);
    /// @}
  }
  ...
  public Loader<Cursor> onCreateLoader(int id, Bundle bundle) {
    ...
    if (id == CONTACTS_LOADER_ID) {
      ...
      if (DialerFeatureOptions.isDialerSearchEnabled() && !isRegularSearch()) {
        ...
        loader.configureQuery(query,true);
        return loader;
      }
      /// @}
  ...
  public void onLoadFinished(Loader<Cursor> loader, Cursor cursor) {
    ...
    LogUtil.i("NewSearchFragment.onLoadFinished", "cursor:::: " + cursor);
    if (cursor != null
    ...
    if (loader instanceof DialerSearchCursorLoader) {
        if (DialerFeatureOptions.isDialerSearchEnabled() && !isRegularSearch()) {
            LogUtil.i("NewSearchFragment.onLoadFinished", "adapter.setIsSmartSearch(true)");
            adapter.setIsSmartSearch(true);
            adapter.setContactsCursor((SearchCursor)cursor);
        }
    }
    /// @}
    else if (loader instanceof SearchContactsCursorLoader) {
      LogUtil.i("NewSearchFragment.onLoadFinished", "adapter.setIsSmartSearch(false)");
      adapter.setIsSmartSearch(false);
      adapter.setContactsCursor((SearchCursor) cursor);
  ...
  public void onDestroy() {
    super.onDestroy();
    LogUtil.i("NewSearchFragment.onDestroy", "enter");
    ...
    /// M: add for op01/op07/op08/op12 plug-in
    searchFragmentExt.onDestroy();
     ///M: For ALPS03754339, add try-catch to avoid MTBF issue JE occur.
    try {
        getActivity().unregisterReceiver(smartDialUpdatedReceiver);
    } catch (IllegalArgumentException e) {
        LogUtil.i("NewSearchFragment.onDestroy",
                "Catch IllegalArgumentException, it maybe Receiver not registered");
    }
  }
   ...
   public void showDialpad() {
     isDialpadShown = true;
     if (emptyContentView != null && !PermissionsUtil.hasContactsReadPermissions(getContext())) {
       LogUtil.i("NewSearchFragment.showDialpad()", "isDialpadShown = " + isDialpadShown);
       emptyContentView.showDialpadSpace(true);
     }
   }
   ...
   public void hideDialpad() {
     ...
     if (emptyContentView != null && !PermissionsUtil.hasContactsReadPermissions(getContext())) {
       LogUtil.i("NewSearchFragment.showDialpad()", "isDialpadShown = " + isDialpadShown);
       emptyContentView.showDialpadSpace(false);
     }
   }
   /// @}
  ...
  public void onRequestPermissionsResult(
    ...
    FragmentUtils.getParentUnsafe(this, SearchFragmentListener.class).unRequestingPermission();
    ///@}
  ...
  private void requestLocationPermission() {
    ///M: Remove assert(). Requesting location permission will
    // need LOCATION_FULL_GROUP, and it contains ACCESS_FINE_LOCATION.
    // But at this, it will only assert ACCESS_FINE_LOCATION, so if
    // other permission has not been allowed, but ACCESS_FINE_LOCATION have been
    // allowed, it will occur exception. @{
    ...
    LogUtil.i("requestLocationPermission()"," deniedPermissions.length = " +
  ...
  public void onStart() {
    super.onStart();
    LogUtil.i("NewSearechFragment.onStart", "registering smart dial update receiver");
    getActivity()
        .registerReceiver(
  ...
  public void onResume() {
    super.onResume();
    LogUtil.i("NewSearchFragment.onResume", "enter");
    ...
    if (PermissionsUtil.hasContactsReadPermissions(getContext())) {
        if (emptyContentView != null) {
            LogUtil.i("NewSearchFragment.onResume", "Has contacts permission");
            emptyContentView.setVisibility(View.GONE);
            initLoaders();
        }
    }///@}
  ...
  public void onPause() {
    super.onPause();
    LogUtil.i("NewSearchFragment.onPause", "enter");
    EnrichedCallComponent.get(getContext())
        .getEnrichedCallManager()
  ...
  private List<Integer> getActions() {
    ...
    ///M: For RTT call @ {
    if (DialerVolteUtils.isRttCallSupport(getContext())) {
        actions.add(Action.MAKE_RTT_CALL);
    }
    /// @}
    return actions;
  }
  ...
  ///M: For RTT call @{
  @Override
  public void placeRTTCall(String phoneNumber, int ranking) {
    CallSpecificAppData callSpecificAppData =
    CallSpecificAppData.newBuilder()
        .setCallInitiationType(callInitiationType)
              .setPositionOfSelectedSearchResult(ranking)
              .setCharactersInSearchString(query == null ? 0 : query.length())
        .setAllowAssistedDialing(true)
        .build();
    LogUtil.i("placeRTTCall","PreCall.start");
    PreCall.start(
        getContext(),
