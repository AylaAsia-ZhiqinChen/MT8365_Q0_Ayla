...
public class CallerInfoUtils {
  ...
  static CallerInfo buildCallerInfo(Context context, DialerCall call) {
    ...
    /// M: [Performance] Using cached countryIso to increase performance. @{
    /// Google code: info.countryIso =
    ///     PhoneNumberHelper.getCurrentCountryIso(context, call.getAccountHandle());
    if (call.getCountryIso() != null &&
        call.getCountryIso() != CountryDetector.DEFAULT_COUNTRY_ISO) {
      info.countryIso = call.getCountryIso();
    } else {
    ...
    if (call.isVoiceMailNumber()) {
      ...
      info.markAsVoiceMail(context, call.getCallSubId());
      /// @}
    }
  ...
  static String toLogSafePhoneNumber(String number) {
    ...
    /// M: ALPS03484999 log enhancement for eng or user debug load. @{
    if (!("user".equals(Build.TYPE))) {
        return InCallUtils.formatSensitiveValue(number);
    }
    /// @}
