...
public class ConferenceParticipantListAdapter extends BaseAdapter {
  ...
  private void refreshView(String callId) {
    ...
    for (int position = 0; position <= last - first; position++) {
      ...
      if (rowCallId.equals(callId)) {
        /// M: ALPS03674993, position leak JE. @{
        if (position + first >= conferenceParticipants.size()) {
          LogUtil.e("ConferenceParticipantListAdapter.refreshView", "call [" + callId
  ...
  public View getView(int position, View convertView, ViewGroup parent) {
    ...
    /// M: [VoLTE Conference] Added for Volte conference feature. @{
  ...
  private void updateParticipantInfo(List<DialerCall> conferenceParticipants) {
    ...
    for (DialerCall call : conferenceParticipants) {
      ...
      if (participantsByCallId.containsKey(callId)) {
        ...
        /// M: ALPS03674993, add log for conferenceParticipants size exception. @{
        LogUtil.d("ConferenceParticipantListAdapter.updateParticipantInfo",
            "Add CallId: " + call.getId() + " after added , size: " +
            conferenceParticipants.size() + " / " + participantsByCallId.size());
        /// @}
      }
    }
    ...
    while (it.hasNext()) {
      ...
      if (!newCallIds.contains(existingCallId)) {
        ...
        /// M: ALPS03674993, add log for conferenceParticipants size exception. @{
        LogUtil.d("ConferenceParticipantListAdapter.updateParticipantInfo",
            "Remove CallId: " + existingCallId + " after removed , size: " +
            conferenceParticipants.size() + " / " + participantsByCallId.size());
        /// @}
      }
    }
  ...
  /// M: -------------------------------------Mediatek--------------------------------------------
  /**
   * M: [VoLTE Conference] Set call status if needed.
   * @param view View
