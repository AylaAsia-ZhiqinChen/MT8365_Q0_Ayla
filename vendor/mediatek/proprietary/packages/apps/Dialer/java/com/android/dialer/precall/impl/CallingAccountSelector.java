...
public class CallingAccountSelector implements PreCallAction {
  ...
  public boolean requiresUi(Context context, CallIntentBuilder builder) {
    ...
    ///M: Fix dialog  show abnormal issue when switch user to guest@{
    UserHandle myUserHandle = Process.myUserHandle();
    UserHandle phoneAcountUserHandle;
    boolean hasCallPermission = false;
    for (PhoneAccountHandle phoneAccountHandle : accounts) {
      phoneAcountUserHandle = phoneAccountHandle.getUserHandle();
      if (phoneAcountUserHandle.equals(myUserHandle)) {
    ...
    if (!hasCallPermission) {
        if (PhoneAccount.SCHEME_VOICEMAIL.equals(builder.getUri().getScheme())) {
            ...
            LogUtil.e(TAG_CALLING_ACCOUNT_SELECTOR,
                    "this user have not call permission, return false and let telecom to check");
            return false;
        }
    }
    /// @}
