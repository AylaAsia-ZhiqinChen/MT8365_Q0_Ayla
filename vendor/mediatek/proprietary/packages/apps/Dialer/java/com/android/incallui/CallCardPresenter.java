...
public class CallCardPresenter
    implements InCallStateListener,
        ...
        DialerCallListener,
        /** M: add for auto decline */
        AutoDeclineTimerListener {
  ...
  /// M: ALPS03852365, update secondary info when video state changed @{
  private int secondaryVideoState;
  /// @}

  /// M: ALPS04543764, update secondary info when merge state changed @{
  private boolean secondaryInMerge;
  /// @}

  /// M: ALPS04668544, force to upgrade primary info when conference status changed. @{
  private boolean primaryIsConference;
  /// @}
  ...
  public void onInCallScreenDelegateInit(InCallScreen inCallScreen) {
    ...
    if (call != null) {
      ...
      if (!call.isConferenceCall()
          /// M: [VoLTE Conference] incoming conference should show the invite number
          // when incoming or waiting @{
          || isIncomingVolteConference(primary)
          /// @}
          ) {
  ...
  public void onInCallScreenReady() {
    LogUtil.d("CallCardPresenter.onInCallScreenReady", null);
    ...
    ContactInfoCache.getInstance(context)
        .addContactInfoUpdatedListener(contactInfoUpdatedListener);
    /// @}
  ...
  public void onInCallScreenUnready() {
    LogUtil.d("CallCardPresenter.onInCallScreenUnready", null);
    Assert.checkState(isInCallScreenReady);
    ...
    /// M: [VoLTE Conference] @{
    ContactInfoCache.getInstance(context)
        .removeContactInfoUpdatedListener(contactInfoUpdatedListener);
    /// @}
    if (primary != null) {
    ...
    /// M: ALPS04386064 fix exception when hold RTT call. the function of updatePrimaryCallState
    /// will trigger RTT fragment destoryed when CallCardPresenter excutes onstatechange. But
    /// CallCardPresenter still keep incallscreen. it leads exception happen. Clear incallscreen
    /// when CallCardPresenter excutes onInCallScreenUnready. @{
    inCallScreen = null;
    /// @}
  }
  ...
  public void onStateChange(InCallState oldState, InCallState newState, CallList callList) {
    ...
    /// M: ALPS03852365, update secondary info when video state changed @{
    int secondaryVideoState = VideoProfile.STATE_AUDIO_ONLY;
    /// @}

    /// M: ALPS04543764, update secondary info when merge state changed @{
    boolean secondaryInMerge = false;
    /// @}

    /// M: ALPS04668544, force to upgrade primary info when conference status changed. @{
    boolean primaryIsConference = false;
    /// @}
    ...
    if (newState == InCallState.INCOMING) {
      ...
      /// M: ALPS03787616, using CT card to make another MO call with one active call exists.
      /// After the secondary MO call turning to be actived, the primary call will change to first
      /// active call. This will recreate hold fragment again. But both the active call will be
      /// merged immediately to remove the hold fragment. There maybe timing issue that the
      /// hold fragment can not be removed.
      /// So swap the primary and secondary call manually to avoid recreate of holdfragment. @{
      if (primary != null && primary.getState() == DialerCallState.ACTIVE
          && secondary != null && secondary.getState() == DialerCallState.ACTIVE) {
        DialerCall tempCall = primary;
        primary = secondary;
        secondary = tempCall;
      }
      /// @}
    ...
    /// M: ALPS04668544, force to upgrade primary info when conference status changed. @{
    if (primary != null) {
      primaryIsConference = primary.isConferenceCall();
    }
    /// @}
    ...
    /// M: ALPS04543764, update secondary info when merge state changed @{
    if (secondary != null) {
      secondaryVideoState = secondary.getVideoState();
      secondaryInMerge = secondary.isMergeInProcess();
    }
    /// @}
    ...
    final boolean primaryChanged =
        !(DialerCall.areSame(this.primary, primary)
            ...
            /// M: ALPS04668544, force to upgrade primary info when conference status changed. @{
            && (this.primaryIsConference == primaryIsConference));
            /// @}
    final boolean secondaryChanged =
        !(DialerCall.areSame(this.secondary, secondary)
            ...
            /// M: ALPS03852365, update secondary info when video state changed @{
            && (this.secondaryVideoState == secondaryVideoState)
            /// @}
            /// M: ALPS04543764, update secondary info when merge state changed @{
            && (this.secondaryInMerge == secondaryInMerge));
            /// @}
    ...
    if (shouldRefreshPrimaryInfo(primaryChanged)
        ...
        || needUpdatePrimaryForVolte(oldState, newState, primary)
        /// @}
        ) {
    ...
    /// M: ALPS04056868 update secondaryDisplayInfo when seconday is null is for trigger
    /// requestApplyInserts execute at InCallFragment.setSecondary and in order to fix endbutton
    /// position show wrong. @{
    ...
    if (getUi() != null) {
      ...
      ExtensionManager.getCallCardExt().setPrimary(
          context, this.primary, inCallScreen.getInCallScreenFragment());
      /// @}
    }
    /// @}
    Trace.endSection();
  ...
  private void updatePrimaryCallState() {
    if (getUi() != null && primary != null) {
      ...
      boolean isAttemptingHdAudioCall =
          !isHdAudioCall
              /// M: [Performance]. @{
              && !DialerCallState.isConnecting(primary.getState())
              /// @}
      ...
      getUi()
          .setCallState(
              PrimaryCallState.builder()
                  ...
                  /// M: [ALPS03753763] don't show video state when playing video ringtone. @{
                  // Google code:
                  //.setIsVideoCall(primary.isVideoCall())
                  .setIsVideoCall(primary.isVideoCallExcludeVideoRingtone())
                  /// @}
                  ...
                  /// M: ALPS03420995, do not show the callback number. @{
                  /// Googgle code:
                  /// .setCallbackNumber(
  ...
  private void updatePrimaryDisplayInfo() {
    ...
    /// M: [WWOP] STIR/SHAKEN support for OP08. @{
    ExtensionManager.getCallCardExt().setPrimary(
        context, primary, inCallScreen.getInCallScreenFragment());
    /// @}
    ...
    if (isInCallScreenReady) {
      ...
      LogUtil.d("CallCardPresenter.updatePrimaryDisplayInfo", "UI not ready, not showing location");
    }
  }
  ...
  private boolean shouldShowLocation() {
    ...
    if (!isPotentialEmergencyCall()) {
      LogUtil.d("CallCardPresenter.getLocationFragment", "shouldn't show location");
      return false;
    }
    if (!hasLocationPermission()) {
  ...
  private void updateSecondaryDisplayInfo() {
    ...
    if (secondary == null) {
      ...
      /// M: add init color for secondary. @{
      inCallScreen.setSecondary(SecondaryInfo.builder()
          .setColor(PhoneAccount.NO_HIGHLIGHT_COLOR).setIsFullscreen(isFullscreen).build());
      /// @}
      return;
    }
    ...
    if (secondary.isConferenceCall()) {
      inCallScreen.setSecondary(
          SecondaryInfo.builder()
              ...
              .setColor(InCallPresenter.getInstance().getPrimaryColorFromCall(secondary))
              .build());
  ...
  private String getConnectionLabel() {
    /// M: [Performance]. Move the permission check to the position when it really need.@{
    /// Google code:
  ...
  public void onSecondaryInfoClicked() {
    ...
    if (DialerCallState.isIncomingOrWaiting(secondary.getState())) {
      LogUtil.i(
          "CallCardPresenter.onSecondaryInfoClicked", "handle DSDA call: " + secondary);
  ...
  /**
   * M: [VoLTE Conference] Check whether the incoming call is conference.
   * @param call
  ...
  /**
   * M: [VoLTE Conference] Need to update primary for volte case.
   * @param oldState
   * @param newState
   * @param call
   * @return
  ...
  /**
   * M: [VoLTE Conference] Listner onContactInfoUpdated(),
   * will be notified when ContactInfoCache finish re-query, triggered by some
   * call's number change.
   */
  private final ContactInfoUpdatedListener contactInfoUpdatedListener
  ...
  /**
   * M: [VoLTE Conference] Ask for new ContactInfo to update UI when re-query complete
   * by ContactInfoCache.
   */
  private void handleContactInfoUpdated(String callId) {
    LogUtil.d("CallCardPresenter.handleContactInfoUpdated",
        "handleContactInfoUpdated()... callId = " + callId);
    DialerCall call = null;
    boolean isPrimary = false;
    if (primary != null && primary.getId() == callId) {
      isPrimary = true;
      call = primary;
    } else if (secondary != null && secondary.getId() == callId) {
      isPrimary = false;
      call = secondary;
    }
