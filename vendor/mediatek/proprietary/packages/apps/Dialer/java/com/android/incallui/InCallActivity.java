...
public class InCallActivity extends TransactionSafeFragmentActivity
  ...
  protected void onCreate(Bundle bundle) {
   ...
   /// M: ALPS03504688 Force reset Device Orientation on new Call @{
    if (bundle == null) {
    ...
    /// M: [ppl Lock] @{
    dMLockReceiver = DMLockBroadcastReceiver.getInstance(this);
    dMLockReceiver.register(this);
    /// @}
  }
  ...
  private void relaunchedFromDialer(boolean showDialpad) {
    ...
    if (showDialpadRequest == DIALPAD_REQUEST_SHOW) {
      ...
      if (call != null && call.getState() == DialerCallState.ONHOLD
          /// M:ALPS03387938 If has PendingOutgoing, Outgoing or Incoming call, don't unhold call @{
  ...
  private boolean showPhoneAccountSelectionDialog() {
    ...
    if (waitingForAccountCall == null
        ...
        || waitingForAccountCall.getStateEx() == DialerCallState.WAIT_ACCOUNT_RESPONSE
        /// @}
        ) {
      return false;
    }
     ...
     * @{
     */
    SelectPhoneAccountDialogFragment selectAccountDialog =
        (SelectPhoneAccountDialogFragment) this.getFragmentManager().
            findFragmentByTag(Tags.SELECT_ACCOUNT_FRAGMENT);
    if (selectAccountDialog != null) {
      Bundle args = selectAccountDialog.getArguments();
      if (args != null) {
        args.putString(InCallUtils.ARG_CALL_ID, waitingForAccountCall.getId());
        LogUtil.i("showPhoneAccountSelectionDialog", "existed, just update call id " +
            waitingForAccountCall.getId());
      } else {
        LogUtil.e("showPhoneAccountSelectionDialog", "there is no args, ignore update");
      }
      return true;
    }
    /// @}
    ...
    preferredAccountWorkerResultListener.listen(
        ...
        result -> {
          ...
          if (selectPhoneAccountDialogFragment != null) {
            LogUtil.d("InCallActivity.showPhoneAccountSelectionDialog",
  ...
  protected void onPause() {
    ...
    /// M: ALPS04032116, Just unset activity when activity paused by user finishing.
    /// There is the case new activity create before the last activity destroy, and
    /// the incallactivity will get ui problem. @{
    if (isFinishing()) {
  ...
  protected void onDestroy() {
    ...
    /// M: [ppl Lock] @{
    dMLockReceiver.unregister(this);
    /// @}
  ...
  protected void onNewIntent(Intent intent) {
    ...
    /// M: ALPS04413919 the screen will be recreated after add call with existing active call.
    /// so the screen will show black a moment.To avoid the case, we don't recreate. And for the
    /// call waiting case,we will handle in ALPS04415607. @{
    //if (!isVisible) {
  ...
  private void showDialpadFragment() {
    ...
    /// M: ALPS04525105 Google don't support show dialpad during video call.
    ///MTK implement it by itself.we need show end call space for video on dialpad. @{
  ...
  public void onForegroundCallChanged(DialerCall newForegroundCall) {
    ...
    if (newForegroundCall == null || !didShowAnswerScreen) {
      ...
      if (isDialpadVisible()) {
        getDialpadFragment().updateColors();
      }
      /// @}
    }
  }
  ...
  public void showDialogOrToastForDisconnectedCall(DisconnectMessage disconnectMessage) {
    ...
    /// M: ALPS04426863 Restore to Google logic, if there is no disconnect dialog to show,
    /// the pending dialogs is no need to dismiss. @{
    //dismissPendingDialogs();
    /// @}
    ...
    if (cause.getCode() == MtkDisconnectCause.WFC_CALL_ERROR
        || ExtensionManager.getInCallExt().maybeShowErrorDialog(cause)) {
      /// M: ALPS04426863 Restore to Google logic, if there is no disconnect dialog to show,
      /// the pending dialogs is no need to dismiss. @{
      dismissPendingDialogs();
      /// @}
      Intent intent = new Intent(this, WfcDialogActivity.class);
    ...
    if(ExtensionManager.getInCallExt().showCongratsPopup(cause)) {
      /// M: ALPS04426863 Restore to Google logic, if there is no disconnect dialog to show,
      /// the pending dialogs is no need to dismiss. @{
      dismissPendingDialogs();
      /// @}
    ...
    if (!isVisible()) {
      ...
      toast = Toast.makeText(
          getApplicationContext(), disconnectMessage.toastMessage, Toast.LENGTH_LONG);
      toast.show();
      /// @}
      return;
    }
    ...
    /// M: ALPS04654044, just finish correct dialog and set the reference as NULL. @{
    final Dialog dialog = disconnectMessage.dialog;
    /// @}
  ...
  private void showMainInCallFragment() {
    ...
    /// M: ALPS03414810 Hide incall screen when exist waiting account call. @{
  ...
  /// M: Change the class to none-static. @{
  /// Google code: private static final class SelectPhoneAccountListener
  private final class SelectPhoneAccountListener
  /// @}
      extends SelectPhoneAccountDialogFragment.SelectPhoneAccountListener {
    /// M: Change the type to none-static. @{
    /// Google code: private static final String TAG =
    ///     SelectPhoneAccountListener.class.getCanonicalName();
    private final String TAG = SelectPhoneAccountListener.class.getCanonicalName();
    /// @}
    ...
    public void onPhoneAccountSelected(
      ...
      if (call != null) {
        ...
        call.setState(DialerCallState.WAIT_ACCOUNT_RESPONSE);
        /// @}
  ...
  /// M: --------------- MediaTek features ---------------------
  private DMLockBroadcastReceiver dMLockReceiver;
  /// M: ALPS03459974 Toast and dialog show at the same time @{
  private Toast toast = null;
  /// @}

  /**
   * M: ALPS04025085 notify videocallfragment or incallfragment dialpad is show according to
   * which the current screen shows.
   */
  private void notifyDialpadShow() {
    if (didShowVideoCallScreen) {
      getVideoCallScreen().onVideoCallScreenDialpadVisibilityChange(true);
    } else {
      InCallScreen inCallScreen = getInCallOrRttCallScreen();
      if (inCallScreen != null) {
  ...
  /**
   * M: ALPS04025085 notify videocallfragment and incallfragment dialpad is hide.
   */
  private void notifyDialpadHide() {
    VideoCallScreen videoScreen = getVideoCallScreen();
    if (videoScreen != null) {
      videoScreen.onVideoCallScreenDialpadVisibilityChange(false);
    }
    InCallScreen inCallScreen = getInCallOrRttCallScreen();
    if (inCallScreen != null) {
  ...
  private boolean isAllowOrientation(ShouldShowUiResult answerUi, ShouldShowUiResult videoUi) {
    ...
    if (answerUi.shouldShow) {
      call = answerUi.call;
      if (call != null && call.hasReceivedVideoUpgradeRequest()) {
        LogUtil.d("InCallActivity.isAllowOrientation ", "is true");
        return true;
      }
    } else if (videoUi.shouldShow) {
      call = videoUi.call;
      if (call != null && call.isVideoCall() && call.getState() != DialerCallState.ACTIVE) {
        return false;
      } else {
        LogUtil.d("InCallActivity.isAllowOrientation ", "is true");
        return true;
  ...
  public void checkResetOrientation() {
    if (!inCallOrientationEventListener.isEnabled()) {
      LogUtil.d(
          "InCallActivity.checkResetOrientation", "orientation disallow" +
          ", orientation" + inCallOrientationEventListener.getCurrentOrientation());
      inCallOrientationEventListener.resetDeviceOrientation();
      LogUtil.d(
          "InCallActivity.checkResetOrientation", "reset orientation");
    }
  }

  /// M: ALPS04654044, just finish correct dialog and set the reference as NULL. @{
  private void onDialogDismissed(Dialog dialoag) {
    LogUtil.i("InCallActivity.onDialogDismissed", "");
    if (dialoag == errorDialog) {
      LogUtil.i("InCallActivity.onDialogDismissed", "set as null");
      errorDialog = null;
    }
    CallList.getInstance().onErrorDialogDismissed();
  }
