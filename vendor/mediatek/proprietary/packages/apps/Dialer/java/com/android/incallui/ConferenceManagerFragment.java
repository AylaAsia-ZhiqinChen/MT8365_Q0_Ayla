...
public class ConferenceManagerFragment
  ...
  /// M: ------------------------- MediaTek features ------------------------
  /// M: [VoLTE Conference] Add member part.
  private View floatingAddMemberButtonContainer;
  private ImageButton floatingAddMemberButton;

  /**
   * M: [VoLTE Conference] Init add member button.
   * @param view
   */
  private void initAddMemberButton(View view) {
    floatingAddMemberButtonContainer = view.findViewById(
        R.id.floating_add_member_action_button_container);
    floatingAddMemberButton = (ImageButton) view
        .findViewById(R.id.floating_add_member_action_button);
    floatingAddMemberButton.setOnClickListener(
      new View.OnClickListener() {
        @Override
        public void onClick(View v) {
          LogUtil.d("ConferenceManagerFragment.initAddMemberButton",
              "onVolteAddConfMemberClicked()...");
          getPresenter().showAddMemberScreen();
        }
      });
    floatingAddMemberButtonContainer.setVisibility(View.GONE);
  }

  /**
   * M: [VoLTE Conference]  Show add member button if necessary.
   * @param visible if show
   */
  @Override
  public void showAddMemberButton(boolean visible) {
    floatingAddMemberButtonContainer.setVisibility(visible ? View.VISIBLE : View.GONE);
  }
}
