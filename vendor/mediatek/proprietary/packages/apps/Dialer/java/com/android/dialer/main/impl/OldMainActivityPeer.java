...
///M: @{
     mContactsObserver = new ThrottleContentObserver(mHandler,
     activity.getApplicationContext(), new Runnable() {
        @Override
        public void run() {
          DialerDatabaseHelperEx dbHelper = Database.get(activity.getApplicationContext())
              .getDialerSearchDbHelper(activity.getApplicationContext());
          dbHelper.startContactUpdateThread();
        }
      }, "ContactsObserver");
  mCallLogObserver = new ThrottleContentObserver(mHandler,
      activity.getApplicationContext(), new Runnable() {
        @Override
        public void run() {
          DialerDatabaseHelperEx dbHelper = Database.get(activity.getApplicationContext())
  ...
  public void onActivityCreate(Bundle savedInstanceState) {
    ...
    if (DialerFeatureOptions.isDialerSearchEnabled()) {
      mDialerDatabaseHelperEx = Database.get(activity.getApplicationContext())
                .getDialerSearchDbHelper(activity.getApplicationContext());
      ...
      if(PermissionsUtil.hasPhonePermissions(activity.getApplicationContext())) {
        mCallLogObserver.register(Calls.CONTENT_URI);
      } else {
        LogUtil.i("OldMainActivityPeer", "can not register CallLog observer without permission.");
      }
      // Monitor this so that we can update contact info
      // when importing a large number of contacts
      if(PermissionsUtil.hasContactsReadPermissions(activity.getApplicationContext())) {
        mContactsObserver.register(ContactsContract.Contacts.CONTENT_URI);
      } else {
        LogUtil.i("OldMainActivityPeer", "can not register Contacts observer without permission.");
      }
    }
    /// @}
  }
    ...
    if (DialerFeatureOptions.isDialerSearchEnabled()) {
      ...
      if (!mCallLogObserver.isRegistered()) {
        if(PermissionsUtil.hasPhonePermissions(activity.getApplicationContext())) {
          ...
          LogUtil.w("OldMainActivityPeer", "can not register CallLog observer without permission on Resume");
        }
      }
      ...
      // when importing a large number of contacts
        if(PermissionsUtil.hasContactsReadPermissions(activity.getApplicationContext())) {
          LogUtil.i("OldMainActivityPeer", "YF: mDialerDatabaseHelperEx.startContactUpdateThread()");
          mDialerDatabaseHelperEx.startContactUpdateThread();
          mContactsObserver.register(ContactsContract.Contacts.CONTENT_URI);
        } else {
          LogUtil.i("OldMainActivityPeer", "can not register Contacts observer without permission onResume");
        }
      }
    } 
    /// @}
  ...
  public void onActivityDestroyed() {
    ///M: Add this for unregister observer @{
    LogUtil.enterBlock("MainCallLogFragmentListener.onActivityDestroyed");
    if (DialerFeatureOptions.isDialerSearchEnabled()
          && mCallLogObserver != null && mContactsObserver != null) {
      LogUtil.i("onActivityDestroyed", "Unregister calllog and contact observer");
  ...
  public void onActivityResult(int requestCode, int resultCode, Intent data) {
    ...
    ///M: Add for one key conference call @{
    else if (requestCode == DialerVolteUtils.ACTIVITY_REQUEST_CODE_PICK_PHONE_CONTACTS) {
      if (resultCode == AppCompatActivity.RESULT_OK) {
        LogUtil.i("OldMainActivityPeer.onActivityResult", "launchVolteConfCall");
        DialerVolteUtils.launchVolteConfCall(activity, data);
      } else {
        LogUtil.i("OldMainActivityPeer.onActivityResult",
                         "No contacts picked, Volte conference call cancelled.");
      }
    }
    ///@}
  ...
  private static final class MainSearchFragmentListener implements SearchFragmentListener {
    ...
    public void unRequestingPermission() {
        searchController.unRequestingPermission();
    }
    ///@}
  }
  ...
  private final ThrottleContentObserver mContactsObserver;
  /*
    = new ThrottleContentObserver(mHandler,
     activity.getApplicationContext(), new Runnable() {
        @Override
        public void run() {
          DialerDatabaseHelperEx dbHelper = Database.get(activity.getApplicationContext())
      ...
      */
  private final ThrottleContentObserver mCallLogObserver ;
/*
  = new ThrottleContentObserver(mHandler,
      activity.getApplicationContext(), new Runnable() {
        @Override
...
//    LogUtil.i("OldMainActivityPeer", "onDestroy()");

    /// M: for Plug-in. @{
    //ExtensionManager.getDialPadExtension().onDestroy(this);
    /// @}
...
//  }
  ...
  //    LogUtil.i("DialtactsActivity.onStart", "begin ....");
  //    super.onStart();
  //    LogUtil.i("DialtactsActivity.onStart", "end ....");
  //}
  /// @}
  /// M: Mediatek end.
}
