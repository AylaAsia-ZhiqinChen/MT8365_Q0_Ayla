...
public class CompositeSmartDialMap {
  ...
  /// M: [MTK Dialer Search] , if it is MTK Search,
   /// DEFAULT_MAP should be SmartDialPadMap@ {
   private static final SmartDialMap DEFAULT_MAP;
   static {
       if (DialerFeatureOptions.isDialerSearchEnabled()) {
           DEFAULT_MAP = new SmartDialPadMap();
       } else {
           DEFAULT_MAP = LatinSmartDialMap.getInstance();
       }
   }
   /// @}
