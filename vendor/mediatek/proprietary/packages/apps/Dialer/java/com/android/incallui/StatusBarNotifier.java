...
public class StatusBarNotifier
  ...
  private void buildAndSendNotification(
    ...
    if(InCallUtils.isTwoIncomingCalls()) {
      builder.setContentTitle(context.getString(R.string.two_incoming_calls));
    }
    /// @}
    ...
    if (!InCallUtils.isTwoIncomingCalls()) {
      if (isVideoUpgradeRequest) {
        ...
        appendCountdown(builder, content.toString());
  ...
  private void createIncomingCallNotification(
    ...
    if (state == DialerCallState.ACTIVE
      ...
      if (call.isVideoCall()) {
        /// M: add voice button for MT ViLTE call
    ...
    } else if (DialerCallState.isConnecting(state)) {
      addHangupAction(builder);
    }
    /// @}
  }
  ...
  private boolean checkForChangeAndSaveData(
    ...
    ///M: [VideoCall] ALPS03462589 when call is waiting for upgrade to video call , we should
    //update the timer for 20 seconds. //@{
    final boolean countDownChanged = savedCountDown !=
    ...
    boolean retval =
        (savedIcon != icon)
            ...
            /// M: [VideoCall] ALPS03462589 update the timer
            || countDownChanged
            /// M: [1A1H2W] add check 2W state @{
            || (isTwoIncoming != InCallUtils.isTwoIncomingCalls()) || (savedColor != color);
    isTwoIncoming = InCallUtils.isTwoIncomingCalls();
    savedColor = color;
    /// @}

    /// M: ALPS04156806, need update call time of CDMA call. @{
    if (InCallUtils.isCdmaCall(call)) {
      long currentConnectTime = call.getConnectTimeMillis();
      LogUtil.d("StatusBarNotifier.checkForChangeAndSaveData",
          "currentConnectTime = " + currentConnectTime +
          ", savedConnectTime = " + savedConnectTime);
      retval = retval || (currentConnectTime != savedConnectTime);
      savedConnectTime = currentConnectTime;
    }
    /// @}
    ...
    /**
     * M: ALPS03387938 Need update notification after InCall activity shown or hidden.
    ...
    if (this.isCallUiShown != isInCallShown){
      this.isCallUiShown = isInCallShown;
    ...
    ///[video call] ALPS03462589 store the countdown time
    savedCountDown = InCallPresenter.getInstance().getAutoDeclineCountdown();
  ...
  String getContentTitle(ContactCacheEntry contactInfo, DialerCall call) {
    ...
    boolean isIncomingOrWaiting = call.getState() == DialerCallState.INCOMING
        || call.getState() == DialerCallState.CALL_WAITING;
    /// @}

    if (call.isConferenceCall()
        /// M: [VoLTE Conference] Incoming conference should show the invited number
        // when incoming or waiting @{
        && !isIncomingOrWaiting
        /// @}
        ) {
  ...
  public int getIconToDisplay(DialerCall call) {
    ...
    if (call.getState() == DialerCallState.ONHOLD) {
      return R.drawable.quantum_ic_phone_paused_vd_theme_24;
            ...
            == SessionModificationState.RECEIVED_UPGRADE_TO_VIDEO_REQUEST
        /// M: ALPS04382374, Check ringtone to show notificaiton.
        /// If is an audio call with video ringtone, should not show video call icon. @{
        /// Google code: || call.isVideoCall()) {
        || call.isVideoCallExcludeVideoRingtone()) {
        /// @}
  ...
  private CharSequence getContentString(DialerCall call, @UserType long userType) {
    ...
    if (InCallUIVolteUtils.isIncomingVolteConferenceCall(call)) {
      return context.getString(R.string.card_title_incoming_conference);
    }
    /// @}
    ...
    /// M: ALPS04382374, Check ringtone to show notificaiton.@{
    /// Google code: } else if (call.isVideoCall()) {
    } else if (call.isVideoCallExcludeVideoRingtone()) {
    /// @}
      resId =
  ...
  private CharSequence getMultiSimIncomingText(DialerCall call) {
    ...
    string.setSpan(
        /// M: [Performance] Using cached account color. @{
        /// Google code: new ForegroundColorSpan(phoneAccount.getHighlightColor()),
        new ForegroundColorSpan(call.getAccountColor()),
        /// @}
        accountStart,
        accountEnd,
  ...
  private DialerCall getCallToShow(CallList callList) {
    ...
    if (call == null) {
      ...
      /// M: ALPS03567937, show notification of connecting call will cost more than 200ms.
  ...
  private Spannable getActionText(
    ...
    /// M: ALPS03433240, use account color to show action button. @{
    /// Google code:
  ...
  private void addAnswerAction(Notification.Builder builder) {
    ...
    builder.addAction(
        new Notification.Action.Builder(
                ...
                getActionText(
                    R.string.notification_action_answer, savedColor),
                /// @}
  ...
  private void addDismissAction(Notification.Builder builder) {
    ...
    builder.addAction(
        new Notification.Action.Builder(
                ...
                getActionText(
                    R.string.notification_action_dismiss, savedColor),
                /// @}
  ...
  private void addVideoCallAction(Notification.Builder builder) {
    ...
    builder.addAction(
        new Notification.Action.Builder(
                ...
                getActionText(
                    R.string.notification_action_answer_video, savedColor),
                /// @}
  ...
  private void addAcceptUpgradeRequestAction(Notification.Builder builder) {
    ...
    builder.addAction(
        new Notification.Action.Builder(
                ...
                getActionText(
                    R.string.notification_action_accept, savedColor),
                /// @}
  ...
  /// M: ------------------ MediaTek features ---------------------------
  /// M:[VideoCall] ALPS03462589 add to identify upgrade condition
  private long savedCountDown = -1;
  /// M: ALPS03387938 InCall activity state for notification update
  ...
  public void onIncomingCall(InCallState oldState, InCallState newState,
      DialerCall call) {
    /// M: ALPS04415607 If the screen is off, we need to make sure screen light on for incoming calls.
    /// In Q version, incallpresenter don't trigger statusbar update notification in function of
    /// StartOrFinishUI. so we need trigger update on incoming call comes. @{
    //if (InCallUtils.isTwoIncomingCalls()) {
    showNotification(call);
    //}
    /// @}
  ...
  /**
   * M: [VoLTE Conference] Listen onContactInfoUpdated(),
  ...
  /**
   * M: [VoLTE Conference] When contact info changes, update status bar if necessary.
  ...
  /**
   * M: [VoLTE Conference] Unregister ContactInfoUpdatedListener.
   */
  ...
  /// M: [ViLTE call] add voice button for incoming vilte call
  private void addVoiceAction(Notification.Builder builder) {
    ...
    PendingIntent answerVoicePendingIntent = createNotificationPendingIntent(
        context, ACTION_ANSWER_VOICE_INCOMING_CALL);

    builder.addAction(
        new Notification.Action.Builder(
            Icon.createWithResource(context, R.drawable.quantum_ic_call_white_24),
                getActionText(
                    R.string.notification_action_answer_voice, savedColor),
                answerVoicePendingIntent)
            .build());
  }
}
