...
public final class CallLogListItemViewHolder extends RecyclerView.ViewHolder
  ...
  ///M:For op01,op07,op08,op09 video call button customization.
  ...
  private CallLogListItemViewHolder(
    ...
    ///M:For op01,op07,op08,op09 video call button customization.
    mCallLogListItemViewHolderExt = ExtensionManager.getCallLogExt()
        .getCallLogListItemViewHolderExt(context, this);
    /// @}
  }
  ...
  public boolean onMenuItemClick(MenuItem item) {
    ...
    if (resId == R.id.context_menu_copy_to_clipboard) {
      ...
      Logger.get(context)
          ...
          if (!((Activity) context).isResumed() || info == null) {
             return true;
          }
          /// @}
    ...
    blockReportListener.onUnblock(
      ...
      AsyncTaskExecutors.createAsyncTaskExecutor()
          .submit(TASK_DELETE, new DeleteCallTask(context, callIds
              /*M add for rtt msg delete*/, callDetailsEntries));
    }
    return false;
  }
  ...
  private void updatePrimaryActionButton(boolean isExpanded) {
    ...
    switch (callbackAction) {
      case CallbackAction.IMS_VIDEO:
        primaryActionButtonView.setTag(
            IntentProvider.getReturnVideoCallIntentProvider(number + postDialDigits,
            accountHandle));
        primaryActionButtonView.setContentDescription(
            TextUtils.expandTemplate(
    ...
    ///M:For op01,op07,op08,op09 video call button customization.
    mCallLogListItemViewHolderExt.updatePrimaryActionButton(isExpanded);
    /// @}
  }
  ...
  private void bindActionButtons() {
    ...
    if (canPlaceCallToNumber) {
      if (canSupportAssistedDialing()) {
        callButtonView.setTag(
            IntentProvider.getAssistedDialIntentProvider(
                number + postDialDigits,
                context, context.getSystemService(TelephonyManager.class)));
      } else {
        callButtonView.setTag(IntentProvider.getReturnCallIntentProvider(number + postDialDigits));
      }
    ...
    ///M:For op01,op07,op08,op09 video call button customization.
    mCallLogListItemViewHolderExt.bindActionButtons();
    /// @}
  }
  ...
  private boolean hasPlacedCarrierVideoCall() {
    /// M:For a voice call, set the secondary callback action to be an IMS video call if it is
    // available. But type icons of voice call log will never containing video icon.
  ...
  private static class DeleteCallTask extends AsyncTask<Void, Void, Void> {
    ...
    protected Void doInBackground(Void... params) {
      ...
      /// M: Delete rtt messages associated with these call logs. @{
      ImmutableSet.Builder<String> builder = ImmutableSet.builder();
      for (CallDetailsEntry entry : callDetailsEntriesForDelete.getEntriesList()) {
