...
public class CallList implements DialerCallDelegate {
  ...
  public DialerCall getIncomingCall() {
     ...
     */
    if (incomingCallList.size() >= 1) {
      return incomingCallList.get(0);
    }
    /** @} **/
  ...
  private boolean updateCallInMap(DialerCall call) {
    ...
    /// M: ALPS04081789, clear call map according to real call state. @{
    /// Google code:
    // if (call.getState() == DialerCallState.DISCONNECTED) {
    if (call.getNonConferenceState() == DialerCallState.DISCONNECTED) {
    /// @}
  ...
  private boolean isCallDead(DialerCall call) {
    ...
    /// Google code:
    // final int state = call.getState();
    final int state = call.getNonConferenceState();
    /// @}
  ...
  private void finishDisconnectedCall(DialerCall call) {
    ...
    /// M: ALPS04701680 [Performance] Improve end call performance. @{
    if (call.getNonConferenceState() == DialerCallState.IDLE) {
      LogUtil.i("CallList.finishDisconnectedCall", "call is idle,ignore update again");
      return;
    }
    /// @}
  ...
  public void onInCallUiShown(boolean forFullScreenIntent) {
    ...
    /// M: ALPS03908864, can not show ui after account selection.
    /// Android AOSP move onUIShowing to onStart and onStop.
    /// Previous ui showing flow after account selecion is onNewIntent -> onResume
    /// ->onUIShowing -> onPrimaryCallStateChange. So need force refreshUI here. @{
  ...
  private List<DialerCall> incomingCallList = new ArrayList<DialerCall>();
  /**
  ...
  /**
   * M: [Hang Up]Get the number of the active and background calls.
  ...
  */
  private void updateIncomingCallList(DialerCall call) {
    ...
    if (DialerCallState.isIncomingOrWaiting(state) && !incomingCallList.contains(call)) {
      incomingCallList.add(0, call);
      isIncomingCallListChanged = true;
    } else if (!DialerCallState.isIncomingOrWaiting(state)) {
      Iterator<DialerCall> it = incomingCallList.iterator();
      while (it.hasNext()) {
  ...
  */
  private ArrayList<String> getIncomingCallIdList() {
    ...
    for (DialerCall call: incomingCallList) {
  ...
  */
  public DialerCall getSecondaryIncomingCall() {
    if (incomingCallList.size() >= 2) {
      return incomingCallList.get(1);
    }
    return null;
  }
  ...
  public void switchIncomingCalls() {
    if (incomingCallList.size() >= 2) {
      DialerCall call = incomingCallList.get(0);
      incomingCallList.remove(0);
      incomingCallList.add(1, call);
    }
  ...
  /**
   * M: [VoLTE Conference] Get dialer call by telecom call.
   * @param telecomCall the telecom call
   * @return dialer call
   */
