...
public class CellBroadcastConfigService extends IntentService {
     ...
     */
    @VisibleForTesting
    public void setCellBroadcastOnSub(SmsManager manager, int subId, boolean enableForSub) {
        ...
        if (enableEtwsAlerts) {
            log("enable etws, sub-id=" + subId);
            mtkSmsManager.setEtwsConfig(0x1 | 0x4);
        } else {
            log("disable etws, sub-id=" + subId);
            mtkSmsManager.setEtwsConfig(0);
        }
