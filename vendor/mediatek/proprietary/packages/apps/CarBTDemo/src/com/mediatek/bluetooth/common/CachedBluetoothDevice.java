...
public class CachedBluetoothDevice implements Comparable<CachedBluetoothDevice> {
    ...
    private BluetoothPbapClientManager mBluetoothPbapClientManager = null;
    ...
    private void connectWithoutResettingTimer(boolean connectAllProfiles) {
        synchronized (mProfileLock) {
            ...
            if (Utils.D) Log.d(TAG, "Preferred profiles = " + preferredProfiles);
    ...
    public void connectProfileName(String profileName) {
        ...
        if (profileName.equals(res.getString(R.string.bluetooth_profile_a2dp))){
            ...
            mA2dpSinkProfile.setPreferred(mDevice, true);
            connectProfile(mA2dpSinkProfile);
        }else if (profileName.equals(res.getString(R.string.bluetooth_profile_pbap))){
            PbapClientProfile mPbapProfile = mProfileManager.getPbapClientProfile();
            mPbapProfile.setPreferred(mDevice, true);
            connectProfile(mPbapProfile);
        }else if(profileName.equals(res.getString(R.string.bluetooth_profile_headset))){
            HfpClientProfile mHfpClientProfile = mProfileManager.getHfpClientProfile();
            mHfpClientProfile.setPreferred(mDevice, true);
            connectProfile(mHfpClientProfile);
        }

    }


    public void disconnectProfileName(String profileName) {
        Resources res = mContext.getResources();
        mConnectAttempted = SystemClock.elapsedRealtime();

        Log.d(TAG, "disconnectProfileName : " + profileName);
        if (profileName.equals(res.getString(R.string.bluetooth_profile_a2dp))){
            A2dpProfile mA2dpProfile = mProfileManager.getA2dpProfile();
            disconnect(mA2dpProfile);
        }else if (profileName.equals(res.getString(R.string.bluetooth_profile_a2dp_sink))){
            A2dpSinkProfile mA2dpSinkProfile = mProfileManager.getA2dpSinkProfile();
            disconnect(mA2dpSinkProfile);
        }else if (profileName.equals(res.getString(R.string.bluetooth_profile_pbap))){
            PbapClientProfile mPbapProfile = mProfileManager.getPbapClientProfile();
            disconnect(mPbapProfile);
        }else if(profileName.equals(res.getString(R.string.bluetooth_profile_headset))){
            HfpClientProfile mHfpClientProfile = mProfileManager.getHfpClientProfile();
            disconnect(mHfpClientProfile);
        }
    ...
    public int getConnectedState(){
        ...
        int mPbapConnectedCount = 0;
        mBluetoothPbapClientManager = BluetoothPbapClientManager.getInstance();
        ...
        Log.d(TAG,"mBluetoothPbapClientManager state ==" + mBluetoothPbapClientManager.getConnectState());
        int PbapState = mBluetoothPbapClientManager.getConnectState();
        if((1 == PbapState || 2 == PbapState)
                && (mBluetoothPbapClientManager.getDevice()) == mdevice){
            mPbapConnectedCount = 1;
            Log.d(TAG,"mConnectedProfiles the same ==");
            if(mConnectedProfiles.size() == 0){
                return R.string.bluetooth_profile_pbap;
            }
        }
        if((mConnectedProfiles.size() + mPbapConnectedCount) <=0){
            return R.string.bt_status_paired;
        }
        if((mConnectedProfiles.size() + mPbapConnectedCount) > 1){
   ...
   List<LocalBluetoothProfile> getConnectedProfiles(){
        ...
        for (LocalBluetoothProfile profile : mProfiles) {
            Log.v(TAG,"isConnectedProfile(profile) " + isConnectedProfile(profile));
    ...
    public String getConnectionSummary(boolean shortSummary) {
                ...
                switch (connectionStatus) {
                    ...
                    case BluetoothProfile.STATE_DISCONNECTING:
                        return mContext.getString(
                                Utils.getConnectionStateSummary(connectionStatus));
        ...
        if (profileConnected) {
            ...
            if (Utils.getBooleanMetaData(
                    mDevice, BluetoothDevice.METADATA_IS_UNTETHERED_HEADSET)) {
                leftBattery = Utils.getIntMetaData(mDevice,
                        BluetoothDevice.METADATA_UNTETHERED_LEFT_BATTERY);
                rightBattery = Utils.getIntMetaData(mDevice,
