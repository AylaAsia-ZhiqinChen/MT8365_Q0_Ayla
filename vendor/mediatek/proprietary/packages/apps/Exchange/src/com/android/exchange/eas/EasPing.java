...
public class EasPing extends EasOperation {
    ...
    /// M: PingSyncSynchronizer to control wake lock for ping
    ...
    public EasPing(final Context context, final Account account,
            final android.accounts.Account amAccount, PingSyncSynchronizer pingSynchronizer) {
        this(context, account, amAccount);
        mPingSynchronizer = pingSynchronizer;
    }

    public final int doPing() {
        /// M: Release wake lock before issue http request to server for sleep
        mPingSynchronizer.releaseWakeLock(getAccountId());
    ...
    protected int handleResponse(final EasResponse response) throws IOException {
        /// M: Acquire wake lock to make sure to handle response completely
        mPingSynchronizer.acquireWakeLock(getAccountId());
