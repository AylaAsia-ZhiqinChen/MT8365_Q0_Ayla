...
public class EasFullSyncOperation extends EasOperation {
    ...
    public int performOperation() {
        ...
        /**
         * M: Only do folder sync when account has released security hold,
         * otherwise we may encounter bad circumstances: folder sync
         * successfully while we don't do mailbox sync, so make return. @{
         */
        ...
        if (mailbox.mType == Mailbox.TYPE_OUTBOX || mailbox.isSyncable()) {
            ...
            try {
                if (mailbox.mType == Mailbox.TYPE_OUTBOX) {
                    /// M: Modify for send mail notification
                    return syncOutbox(mailbox);
                }
    ...
    private int syncOutbox(final Mailbox mailbox) {
        ...
        try {
            ...
            int sendableCount = getSendableCount(mContext, mailbox);
            new AccountServiceProxy(mContext).notifySendingStarted(mailbox.mAccountKey,
                    sendableCount);
     ...
     */
    private int getSendableCount(Context context, Mailbox mailbox) {
        int sendableCount = 0;
