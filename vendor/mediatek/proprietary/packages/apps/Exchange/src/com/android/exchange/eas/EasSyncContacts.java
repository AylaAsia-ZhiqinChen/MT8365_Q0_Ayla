...
public class EasSyncContacts extends EasSyncCollectionTypeBase {
     ...
     */
    private void applyBatch(ContentResolver cr, ArrayList<ContentProviderOperation> ops)
            throws IllegalStateException {
        try {
            ContentProviderResult[] results = cr.applyBatch(
                    ContactsContract.RawContacts.CONTENT_URI.getAuthority(), ops);
            ops.clear();
            for (int i = 0; i < results.length; ++i) {
                if (results[i].count <= 0) {
                    LogUtils.d(TAG, "Update local contact db failed: %s",
                            results[i].uri.toString());
                }
            }
        } catch (RemoteException e) {
            LogUtils.d(TAG, "RemoteException: %s", e.toString());
            throw new IllegalStateException("save contact sync id failed!");
        } catch (OperationApplicationException e) {
            LogUtils.d(TAG, "OperationApplicationException: %s", e.toString());
            throw new IllegalStateException("save contact sync id failed!");
        }
    }

    /// M: Max ops per yield point for contacts db
    private static final int MAX_CONTACTS_OPS_PER_BATCH = 500;
    ...
    private void setUpsyncCommands(final Serializer s, final ContentResolver cr,
        ...
        try {
            ...
            /// M: use apply batch for new contact update @{
            ...
            while (ei.hasNext()) {
                ...
                if (serverId == null) {
                    ...
                    /// M: use apply batch to avoid huge amount of notify change @{
...
//                            cidValues, null, null);
                    Uri contentUri = ContentUris.withAppendedId(rawContactUri,
                            entityValues.getAsLong(ContactsContract.RawContacts._ID));
                    ops.add(ContentProviderOperation.newUpdate(contentUri)
                            .withValue(ContactsContract.RawContacts.SYNC1, clientId).build());
                    if (ops.size() + 1 >= MAX_CONTACTS_OPS_PER_BATCH) {
                        applyBatch(cr, ops);
                    }
                    /// @}
                } else {
            ...
            /**
             * M: Once the cursor window was full, and some delete operations
             * were doing background, the re-query maybe get error results, and
             * then it would resulted in IllegalStateException exception, so
             * catch the exception and throw a IOException to force re-sync
             * later.
             *
             * @{
             */
            ...
            LogUtils.e(
                    TAG,
                    "setUpsyncCommands failed, IllegalStateException [%s] occurred",
                    e);
            throw new IOException(
                    "IllegalStateException occurred, re-do the contacts sync later.");
            /** @} */
        } finally {
            ei.close();
        }
    ...
    public void cleanup(final Context context, final Account account) {
        ...
        for (final Long id: mDeletedContacts) {
            /** M: Modified for large binder transaction fail @{ */
            Builder builder = ContentProviderOperation.newDelete(ContentUris.withAppendedId(
        ...
        if (mParser != null && mParser.isGroupsUsed()) {
            ...
            /// M: To avoid NPE caused by binder transaction failed. @{
            if (c == null) {
                LogUtils.e(TAG,
                        "failed to do the following things, due to fail to get the cursor");
                return;
            }
            /// M: @}
