...
public class ContactsSyncParser extends AbstractSyncParser {
    ...
    public void addData(String serverId, ContactOperations ops, Entity entity)
        ...
        /** M: Mark the end of the whole contact @{ */
        addSeparatorOperation(ops, RawContacts.CONTENT_URI);
        /** @} */
        ...
        if (entity != null) {
            ...
            for (NamedContentValues ncv: ncvList) {
                ...
                /** M: Modified for large binder transaction fail @{ */
                ops.add(new Operation(ContentProviderOperation
                        .newDelete(addCallerIsSyncAdapterParameter(u))));
                /** @} */
            }
        }
    }
            ...
            switch (tag) {
                case Tags.SYNC_SERVER_ID:
                    ...
                    try {
                        ...
                        c.close();
                        /// M: Close Iterator to release cursor after be used @{
            ...
            if (c.moveToFirst()) {
                ...
                Builder builder = ContentProviderOperation.newUpdate(
                        ContentUris.withAppendedId(
                                ...
                                c.getLong(0))).withValues(cv);
                ops.add(new Operation(builder));
                /** @} */
    ...
    private static class RowBuilder {
        ...
        /**
         * M: used to backup back reference key and previous result.
         *
         * @{
         */
        String mBackRefKey;
        ...
        RowBuilder withValueBackReference(String key, int previousResult) {
            /**
             * M: backup the back reference key and previous result for later
             * using.
             *
             * @{
             */
        ...
        /**
         * M: build a Operation object instead of the origin
         * ContentProviderOperation object in order to fix the binder
         * transaction fail.
         ...
         */
        Operation build() {
            return new Operation(builder, mBackRefKey, mPreviousResult);
        }
        /** @} */
    ...
    public static class ContactOperations extends ArrayList<Operation> {
         ...
         * M:
         *
         * Add ContentProviderOperation that assigns the data array as the sync
         * state for the given account to the ContactOperations. This function
         * is just the same as the SyncStateContract.Helpers.newSetOperation
         *
         * @param uri
         ...
         * @return true if success added to the ContactOperations.
         */
        public boolean add(Uri uri, android.accounts.Account account, byte[] data) {
        ...
        public void newContact(final String serverId, final String emailAddress) {
            ...
            /** M: Modified for large binder transaction fail @{ */
            add(new Operation(builder));
            /** @} */
        }

        public void delete(long id) {
            Builder builder = ContentProviderOperation.newDelete(ContentUris
        ...
        public void execute(final Context context) {
            try {
                if (!isEmpty()) {
                    /** M: Modified for large binder transaction fail @{ */
                    mResults = safeExecute(context.getContentResolver(),
    ...
    /**
     * M: Called by a sync adapter to indicate a relatively safe place to split
     * a batch of CPO's. The value which is greater than or equal to the
     * ContactsContract.Profile.MIN_ID would be regarded as the Profile ID, but
     * there is no android.permission.WRITE_PROFILE granted by the Exchange, so
     * it should use "ContactsContract.Profile.MIN_ID - 1" as the empty delete
     * operation.
     *
     * @{
     */
    protected static void addSeparatorOperation(ArrayList<Operation> ops, Uri uri) {
        Operation op = new Operation(
                ContentProviderOperation.newDelete(ContentUris.withAppendedId(
                        uri, (ContactsContract.Profile.MIN_ID - 1))));
        op.mSeparator = true;
        ops.add(op);
    }
    /** @} */
