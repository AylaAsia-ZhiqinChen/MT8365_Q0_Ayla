...
public class StkLauncherActivityII extends Activity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (!isStkAvailable(PhoneConstants.SIM_ID_2)) {
            finish();
            return;
        }
        ...
        args.putInt(StkAppService.SLOT_ID, PhoneConstants.SIM_ID_2);
    ...
    private boolean isStkAvailable(int slotId) {
        ...
        if (true == isOnFlightMode()) {
            MtkCatLog.d("Stk-LAII", "isOnFlightMode");
            if( isOP154 && isWfcEnabled ){
                MtkCatLog.d("Stk-LAII", "OP154, wfc enabled");
                return true;
            }else{
            ...
            MtkCatLog.d("Stk-LAII", "isOnLockMode or radio off or sim off");
            showTextToast(getApplicationContext(), resId);
            return false;
        }
        ...
        if (service != null && service.StkQueryAvailable(slotId) !=
            ...
            MtkCatLog.d("Stk-LAII", "slotId: " + slotId + "is not available simState:" + simState);
            showTextToast(getApplicationContext(), resId);
            return false;
        }
        return true;
    ...
    private int getSubIdUsingPhoneId(int phoneId) {
        ...
        MtkCatLog.d("Stk-LAII", "[getSubIdUsingPhoneId] subId " + subId + ", phoneId " + phoneId);
        return subId;
    }
    ...
    boolean isOnFlightMode() {
        ...
        try {
            ...
            MtkCatLog.e("Stk-LAII", "fail to get airlane mode");
            mode = 0;
        }

        MtkCatLog.v("Stk-LAII", "airlane mode is " + mode);
        return (mode != 0);
    }

    boolean isOnLockMode(int slotId) {
        int simState = TelephonyManager.getDefault().getSimState(slotId);
        MtkCatLog.v("Stk-LAII", "lock mode is " + simState);
    ...
    private boolean isRadioOnState(int slotId) {
        ...
        try {
            ...
            if (phone != null) {
                ...
                radioOn = phone.isRadioOnForSubscriber(subId[0], getApplicationContext()
                        .getOpPackageName());
            }
            MtkCatLog.v("Stk-LAII", "slotId: "  + slotId
                      + "isRadioOnState - radio_on[" + radioOn + "]");
        } catch (RemoteException e) {
            e.printStackTrace();
            MtkCatLog.w("Stk-LAII", "isRadioOnState - Exception happen ====");
        }
        return radioOn;
    }
    ...
    private boolean isSimOnState(int slotId) {
        ...
        MtkCatLog.d("Stk-LAII", "isSimOnState - slotId[" + slotId + "], sim_on[" + simOn + "]");
    ...
    private boolean checkSimRadioState(int slotId) {
        ...
        MtkCatLog.d("Stk-LAII", "checkSimRadioState - slotId[" + slotId +
