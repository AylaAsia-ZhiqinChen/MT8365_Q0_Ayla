...
public class StkMenuConfig {
    ...
    public String getLabel(int slotId) {
        ...
        if (DBG) MtkCatLog.d(LOG_TAG, "getLabel: " +
                mConfigs[slotId].label + ", slot id: " + slotId);
    ...
    public Bitmap getIcon(int slotId) {
        ...
        if (DBG) MtkCatLog.d(LOG_TAG, "getIcon: " + mConfigs[slotId].icon + ", slot id: " + slotId);
        return bitmap;
    }
    ...
    private void findConfig(int slotId) {
        ...
        if (mConfigs[slotId] != null && mConfigs[slotId].mcc == mcc
            ...
            if (DBG) MtkCatLog.d(LOG_TAG, "Return the cached config, slot id: " + slotId);
            return;
        }

        if (DBG) MtkCatLog.d(LOG_TAG, "Find config and create the cached config," +
                " slot id: " + slotId);
    ...
    private void initialize(Context context) {
        ...
        try {
            ...
            MtkCatLog.e(LOG_TAG, "Something wrong happened while interpreting the xml file" + e);
        } finally {
            parser.close();
        }
    }
