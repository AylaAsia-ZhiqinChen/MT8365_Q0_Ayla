...
public class StkCmdReceiver extends BroadcastReceiver {
    ...
    public void onReceive(Context context, Intent intent) {
        ...
        if (action.equals(MtkAppInterface.MTK_CAT_CMD_ACTION)) {
            handleAction(context, intent, StkAppService.OP_CMD);
        } else if (action.equals(MtkAppInterface.CAT_SESSION_END_ACTION)) {
            handleAction(context, intent, StkAppService.OP_END_SESSION);
        } else if (action.equals(MtkAppInterface.CAT_ICC_STATUS_CHANGE)) {
            handleAction(context, intent, StkAppService.OP_CARD_STATUS_CHANGED);
        } else if (action.equals(MtkAppInterface.CAT_ALPHA_NOTIFY_ACTION)) {
            //L-MR1: ToDo: We can use broadcast alpha notify to show the string instead.
    ...
    private void handleAction(Context context, Intent intent, int op) {
        ...
        if (StkAppService.OP_CMD == op) {
            ...
            if ((intent.getBooleanExtra(MtkAppInterface.CARD_STATUS, false) == false)
                    && StkAppService.getInstance() == null) {
                MtkCatLog.w("StkCmdReceiver", "lost card state == false");
                return;
            }

            args.putBoolean(MtkAppInterface.CARD_STATUS,
                    intent.getBooleanExtra(MtkAppInterface.CARD_STATUS, true));
            args.putInt(MtkAppInterface.REFRESH_RESULT,
                    intent.getIntExtra(MtkAppInterface.REFRESH_RESULT,
                    IccRefreshResponse.REFRESH_RESULT_FILE_UPDATE));
        } else if (StkAppService.OP_ALPHA_NOTIFY == op) {
            String alphaString = intent.getStringExtra(MtkAppInterface.ALPHA_STRING);
            args.putString(MtkAppInterface.ALPHA_STRING, alphaString);
        }

        MtkCatLog.d("StkCmdReceiver", "handleAction, op: " + op
                + " args: " + (Build.IS_DEBUGGABLE ? args : "********") + ", slot id: " + slot_id);
