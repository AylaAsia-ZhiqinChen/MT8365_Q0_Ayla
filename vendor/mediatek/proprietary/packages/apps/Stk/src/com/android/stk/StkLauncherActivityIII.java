...
public class StkLauncherActivityIII extends Activity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (!isStkAvailable(PhoneConstants.SIM_ID_3)) {
            finish();
            return;
        }
    ...
    private boolean isStkAvailable(int slotId) {
        ...
        if (true == isOnFlightMode()) {
            MtkCatLog.d("Stk-LAIII", "isOnFlightMode");
            if( isOP154 && isWfcEnabled ){
                MtkCatLog.d("Stk-LAIII", "OP154, wfc enabled");
                return true;
            }else{
            ...
            MtkCatLog.d("Stk-LAIII", "isOnLockMode or radio off or sim off");
            showTextToast(getApplicationContext(), resId);
            return false;
        }
        ...
        if (service != null && service.StkQueryAvailable(slotId) !=
            ...
            MtkCatLog.d("Stk-LAIII", "slotId: " + slotId + "is not available simState:" + simState);
            showTextToast(getApplicationContext(), resId);
            return false;
        }
        return true;
    ...
    private int getSubIdUsingPhoneId(int phoneId) {
        ...
        MtkCatLog.d("Stk-LAIII", "[getSubIdUsingPhoneId] subId " + subId + ", phoneId " + phoneId);
        return subId;
    }
    ...
    boolean isOnFlightMode() {
        ...
        try {
            ...
            MtkCatLog.v("Stk-LAIII", "fail to get airlane mode");
            mode = 0;
        }

        MtkCatLog.v("Stk-LAIII", "airlane mode is " + mode);
        return (mode != 0);
    }

    boolean isOnLockMode(int slotId) {
        int simState = TelephonyManager.getDefault().getSimState(slotId);
        MtkCatLog.v("Stk-LAIII", "lock mode is " + simState);
    ...
    private boolean isRadioOnState(int slotId) {
        ...
        try {
            ...
            MtkCatLog.v("Stk-LAIII", "slotId: " + slotId
                     + "isRadioOnState - radio_on[" + radioOn + "]");
        } catch (RemoteException e) {
            e.printStackTrace();
            MtkCatLog.w("Stk-LAIII", "isRadioOnState - Exception happen ====");
        }
        return radioOn;
    }
    ...
    private boolean isSimOnState(int slotId) {
        ...
        MtkCatLog.d("Stk-LAIII", "isSimOnState - slotId[" + slotId + "], sim_on[" + simOn + "]");
    ...
    private boolean checkSimRadioState(int slotId) {
        ...
        MtkCatLog.d("Stk-LAIII", "checkSimRadioState - slotId[" + slotId +
