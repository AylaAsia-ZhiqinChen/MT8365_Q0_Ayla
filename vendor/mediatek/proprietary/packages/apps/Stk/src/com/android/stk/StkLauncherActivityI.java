 ...
 *
 */
public class StkLauncherActivityI extends Activity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (!isStkAvailable(PhoneConstants.SIM_ID_1)) {
            finish();
            return;
        }
        ...
        args.putInt(StkAppService.SLOT_ID, PhoneConstants.SIM_ID_1);
    ...
    private boolean isStkAvailable(int slotId) {
        ...
        if (true == isOnFlightMode()) {
            MtkCatLog.d("Stk-LAI", "isOnFlightMode");
            if( isOP154 && isWfcEnabled ){
                MtkCatLog.d("Stk-LAI", "OP154, wfc enabled");
                return true;
            }else{
            ...
            MtkCatLog.d("Stk-LAI", "isOnLockMode or radio off or sim off");
            showTextToast(getApplicationContext(), resId);
            return false;
        }
        ...
        if (service != null && service.StkQueryAvailable(slotId) !=
            ...
            MtkCatLog.d("Stk-LAI", "slotId: " + slotId + "is not available simState:" + simState);
            showTextToast(getApplicationContext(), resId);
            return false;
        }
        return true;
    ...
    private int getSubIdUsingPhoneId(int phoneId) {
        ...
        MtkCatLog.d("Stk-LAI", "[getSubIdUsingPhoneId] subId " + subId + ", phoneId " + phoneId);
        return subId;
    }
    ...
    boolean isOnFlightMode() {
        ...
        try {
            ...
            MtkCatLog.d("Stk-LAI", "fail to get airlane mode");
            mode = 0;
        }

        MtkCatLog.d("Stk-LAI", "airplane mode is " + mode);
        return (mode != 0);
    }
    boolean isOnLockMode(int slotId) {
        int simState = TelephonyManager.getDefault().getSimState(slotId);
        MtkCatLog.d("Stk-LAI", "lock mode is " + simState);
    ...
    private boolean isRadioOnState(int slotId) {
        ...
        try {
            ...
            MtkCatLog.d("Stk-LAI", "slotId: " + slotId
                    + " isRadioOnState - radio_on[" + radioOn + "]");
        } catch (RemoteException e) {
            e.printStackTrace();
            MtkCatLog.e("Stk-LAI", "isRadioOnState - Exception happen ====");
        }
        return radioOn;
    }
    ...
    private boolean isSimOnState(int slotId) {
        ...
        MtkCatLog.d("Stk-LAI", "isSimOnState - slotId[" + slotId + "], sim_on[" + simOn + "]");
    ...
    private boolean checkSimRadioState(int slotId) {
        ...
        MtkCatLog.d("Stk-LAI", "checkSimRadioState - slotId[" + slotId +
