...
public final class HeifWriter implements AutoCloseable {
    ...
    public static final class Builder {
        ...
        public Builder( String path,
                       int width, int height, int inputMode) {
         ...
         *                     {@link MediaMuxer} or {@link MediaCodec}.
         */
    ...
    private HeifWriter( String path,
                        FileDescriptor fd,
                       int width,
                       int height,
                       int rotation,
    ...
    private class HeifCallback extends HeifEncoder.Callback {
        ...
        public void onOutputFormatChanged(
                 HeifEncoder encoder,  MediaFormat format) {
        ...
        public void onDrainOutputBuffer(
                 HeifEncoder encoder,  ByteBuffer byteBuffer) {
            if (encoder != mHeifEncoder) return;
        ...
        @Override
        public void onComplete( HeifEncoder encoder) {
            if (encoder != mHeifEncoder) return;
        ...
        @Override
        public void onError( HeifEncoder encoder,  MediaCodec.CodecException e) {
            if (encoder != mHeifEncoder) return;

            stopAndNotify(e);
        }

        private void stopAndNotify(  Exception error) {
            try {
                closeInternal();
            } catch (Exception e) {
    ...
    private static class ResultWaiter {
        ...
        synchronized void signalResult(  Exception e) {
            if (!mDone) {
                mDone = true;
                mException = e;
                notifyAll();
