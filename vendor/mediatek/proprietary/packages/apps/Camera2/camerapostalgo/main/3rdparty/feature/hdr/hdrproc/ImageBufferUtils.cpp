...
#define LOG_TAG "HDR_ImageBufferUtils"
...
#include <utils/def/ImageFormat.h>
...
MBOOL ImageBufferUtils::allocBuffer(sp<IImageBuffer>& imageBuf, MUINT32 w,
        MUINT32 h, MUINT32 format, MBOOL isContinuous) {
    MBOOL ret = MTRUE;
    CAM_LOGD("allocBuffer: +");
    MINT planecount = queryPlaneCount(format);
    MINT usage = eBUFFER_USAGE_SW_READ_OFTEN | eBUFFER_USAGE_SW_WRITE_OFTEN;
    MINT32 bufBoundaryInBytes[3] = { 0, 0, 0 };
    MINT32 bufStridesInBytes[3];
    MSize imgSize(w, h);
    for (MINT i = 0; i < planecount; i++) {
        bufStridesInBytes[i] = (queryPlaneWidthInPixels(format, i, imgSize.w)
                * queryPlaneBitsPerPixel(format, i)) >> 3;
    }

    IImageBufferAllocator::ImgParam imgParam(format, imgSize, bufStridesInBytes,
            bufBoundaryInBytes, planecount);
    IImageBufferAllocator::ExtraParam extParam(usage);
    sp<IImageBuffer> mpImageBuffer =
            IImageBufferAllocator::getInstance()->alloc_gb(LOG_TAG, imgParam,
                    extParam);
    if (mpImageBuffer == nullptr) {
        CAM_LOGE("acquireWorkingBuffer: create image buffer failed");
    } else if (!(mpImageBuffer->lockBuf(LOG_TAG, usage))) {
        CAM_LOGE("acquireWorkingBuffer: lock image buffer failed");
    }
    CAM_LOGD("allocBuffer: -");
    imageBuf = mpImageBuffer;
...
//        IImageBufferAllocator::ExtraParam extParam_null(usage);
//        IImageBufferAllocator *allocator = IImageBufferAllocator::getInstance();
//        sp<IImageBuffer> tmpImageBuffer = allocator->alloc_gb(LOG_TAG, blobParam, extParam_null);
...
//    } else
//    {
//      IImageBufferAllocator::ImgParam imgParam(format, MSize(w,h),
//              strideInBytes, bufBoundaryInBytes, planeCount);
//      MINT usage = eBUFFER_USAGE_SW_READ_OFTEN|eBUFFER_USAGE_SW_WRITE_OFTEN;
//      IImageBufferAllocator::ExtraParam extParam(usage);
//      pBuf = IImageBufferAllocator::getInstance()->alloc_gb(LOG_TAG,
//                      imgParam, extParam);
//    }
//
//    if (pBuf == NULL)
...
IImageBuffer* ImageBufferUtils::createBufferAlias(IImageBuffer* pOriginalBuf,
...
if (pBufHeap->getImgFormat() != eImgFmt_BLOB) {
    CAM_LOGE("heap buffer type must be BLOB=0x%x, this is 0x%x",
            eImgFmt_BLOB, pBufHeap->getImgFormat());
    ret = MFALSE;
    goto lbExit;
}
...
MBOOL ImageBufferUtils::removeBufferAlias(IImageBuffer *pOriginalBuf,
    IImageBuffer *pAliasBuf) {
MBOOL ret = MTRUE;
...
MBOOL ImageBufferUtils::createBuffer(sp<IImageBuffer> & output_buf,
    sp<IImageBuffer> & input_buf) {
