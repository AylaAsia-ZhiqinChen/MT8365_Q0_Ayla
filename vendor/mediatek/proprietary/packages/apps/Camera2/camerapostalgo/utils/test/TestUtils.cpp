...
)
{
    ...
    MY_CHECK(1==pHeap->getPlaneCount(), "PlaneCount:%zu", pHeap->getPlaneCount());
    MY_CHECK(8==pHeap->getImgBitsPerPixel(), "BitsPerPixel:%zu", pHeap->getImgBitsPerPixel());
    MY_CHECK(8==pHeap->getPlaneBitsPerPixel(0), "PlaneBitsPerPixel(0):%zu", pHeap->getPlaneBitsPerPixel(0));
    MY_CHECK(size==pHeap->getBufSizeInBytes(0), "BufSizeInBytes(0):%zu", pHeap->getBufSizeInBytes(0));
    MY_CHECK(size==pHeap->getBufStridesInBytes(0), "BufStridesInBytes(0):%zu", pHeap->getBufStridesInBytes(0));
    //
    ...
    MY_CHECK(pHeap->lockBuf("Test", usage), "lockBuf, usage:%#x", usage);
        MY_CHECK(0==pHeap->getBufPA(0), "BufPA(0)=%" PRIxPTR "", pHeap->getBufPA(0));
        MY_CHECK(0!=pHeap->getBufVA(0), "BufVA(0)=%" PRIxPTR "", pHeap->getBufVA(0));
        if  ( pHeap->getBufVA(0) )
        {
    ...
    usage = 0;
    MY_CHECK(pHeap->lockBuf("Test", usage), "lockBuf, usage:%#x", usage);
        MY_CHECK(0<=pHeap->getHeapIDCount(), "HeapIDCount:%zu", pHeap->getHeapIDCount());
...
)
{
    ...
    MY_CHECK(1==pImgBuf->getPlaneCount(), "PlaneCount:%zu", pImgBuf->getPlaneCount());
    MY_CHECK(8==pImgBuf->getImgBitsPerPixel(), "BitsPerPixel:%zu", pImgBuf->getImgBitsPerPixel());
    MY_CHECK(8==pImgBuf->getPlaneBitsPerPixel(0), "PlaneBitsPerPixel(0):%zu", pImgBuf->getPlaneBitsPerPixel(0));
    if ( !isSBS && !isROI )
    {
        MY_CHECK(size==pImgBuf->getBufSizeInBytes(0), "BufSizeInBytes(0):%zu", pImgBuf->getBufSizeInBytes(0));
        MY_CHECK(MSize(size, 1)==pImgBuf->getImgSize(), "Size:%dx%d", pImgBuf->getImgSize().w, pImgBuf->getImgSize().h);
        MY_CHECK(size==pImgBuf->getBufStridesInBytes(0), "BufStridesInBytes(0):%zu", pImgBuf->getBufStridesInBytes(0));
    }
    else if ( isSBS )
    {
        MY_CHECK(size>>1==pImgBuf->getBufSizeInBytes(0), "BufSizeInBytes(0):%zu", pImgBuf->getBufSizeInBytes(0));
        MY_CHECK(MSize(size>>1, 1)==pImgBuf->getImgSize(), "Size:%dx%d", pImgBuf->getImgSize().w, pImgBuf->getImgSize().h);
        MY_CHECK(size>>1==pImgBuf->getBufStridesInBytes(0), "BufStridesInBytes(0):%zu", pImgBuf->getBufStridesInBytes(0));
    }
    //
    ...
    MY_CHECK(pImgBuf->lockBuf("Test", usage), "lockBuf, usage:%#x", usage);
        MY_CHECK(0==pImgBuf->getBufPA(0), "BufPA(0)=%" PRIxPTR "", pImgBuf->getBufPA(0));
        MY_CHECK(0!=pImgBuf->getBufVA(0), "BufVA(0)=%" PRIxPTR "", pImgBuf->getBufVA(0));
...
)
{
    ...
    MY_CHECK(3==pHeap->getPlaneCount(), "PlaneCount:%zu", pHeap->getPlaneCount());
    MY_CHECK(12==pHeap->getImgBitsPerPixel(), "BitsPerPixel:%zu", pHeap->getImgBitsPerPixel());
    //
    MY_CHECK(8==pHeap->getPlaneBitsPerPixel(0), "PlaneBitsPerPixel(0):%zu", pHeap->getPlaneBitsPerPixel(0));
    MY_CHECK(8==pHeap->getPlaneBitsPerPixel(1), "PlaneBitsPerPixel(1):%zu", pHeap->getPlaneBitsPerPixel(1));
    MY_CHECK(8==pHeap->getPlaneBitsPerPixel(2), "PlaneBitsPerPixel(2):%zu", pHeap->getPlaneBitsPerPixel(2));
    //
    for (unsigned int i = 0; i < pHeap->getPlaneCount(); i++)
    {
        ...
        if ( !isGB )
        {
            MY_CHECK(bufSizeInBytes==pHeap->getBufSizeInBytes(i), "BufSizeInBytes(%d):%zu, %zux%d",
                i, pHeap->getBufSizeInBytes(i), imgParam.bufStridesInBytes[i], imgHeightInPixels);
            MY_CHECK(imgParam.bufStridesInBytes[i]==pHeap->getBufStridesInBytes(i), "BufStridesInBytes(%d):%zu", i, pHeap->getBufStridesInBytes(i));
        }
    }
    //
    MY_CHECK(pHeap->syncCache(eCACHECTRL_FLUSH), "syncCache()");
    ...
    MY_CHECK(pHeap->lockBuf("Test", usage), "lockBuf, usage:%#x", usage);
        for (unsigned int i = 0; i < pHeap->getPlaneCount(); i++)
        {
            MY_CHECK(0==pHeap->getBufPA(i), "BufPA(%d)=%" PRIxPTR "", i, pHeap->getBufPA(i));
            MY_CHECK(0!=pHeap->getBufVA(i), "BufVA(%d)=%" PRIxPTR "", i, pHeap->getBufVA(i));
            if  ( pHeap->getBufVA(i) )
            {
...
)
{
    ...
    MY_CHECK(3==pImgBuf->getPlaneCount(), "PlaneCount:%zu", pImgBuf->getPlaneCount());
    MY_CHECK(12==pImgBuf->getImgBitsPerPixel(), "BitsPerPixel:%zu", pImgBuf->getImgBitsPerPixel());
    //
    MY_CHECK(8==pImgBuf->getPlaneBitsPerPixel(0), "PlaneBitsPerPixel(0):%zu", pImgBuf->getPlaneBitsPerPixel(0));
    MY_CHECK(8==pImgBuf->getPlaneBitsPerPixel(1), "PlaneBitsPerPixel(1):%zu", pImgBuf->getPlaneBitsPerPixel(1));
    MY_CHECK(8==pImgBuf->getPlaneBitsPerPixel(2), "PlaneBitsPerPixel(2):%zu", pImgBuf->getPlaneBitsPerPixel(2));
    //
    CAM_LOGD("getImgSize(%dx%d), getTimestamp(%" PRId64 ")",
        pImgBuf->getImgSize().w, pImgBuf->getImgSize().h, pImgBuf->getTimestamp());

    for (unsigned int plane = 0; plane < pImgBuf->getPlaneCount(); ++plane)
    {
        CAM_LOGD("[plane %d] BitsPerPixel(%zu), BufOffsetInBytes(%zu), BufSizeInBytes(%zu), BufStridesInBytes(%zu)",
    ...
    MY_CHECK(pImgBuf->lockBuf("Test", usage), "lockBuf, usage:%#x", usage);
        ...
        {
            MY_CHECK(0==pImgBuf->getBufPA(i), "BufPA(%d)=%" PRIxPTR "", i, pImgBuf->getBufPA(i));
            MY_CHECK(0!=pImgBuf->getBufVA(i), "BufVA(%d)=%" PRIxPTR "", i, pImgBuf->getBufVA(i));
    ...
    CAM_LOGD("   Test: FDCount(%zu)", pImgBuf->getFDCount());
...
)
{
    ...
    MY_CHECK(8==pImgBuf->getPlaneBitsPerPixel(0), "PlaneBitsPerPixel(0):%zu", pImgBuf->getPlaneBitsPerPixel(0));
    //
    CAM_LOGD("getImgSize(%dx%d), getTimestamp(%" PRIu64 ")",
        pImgBuf->getImgSize().w, pImgBuf->getImgSize().h, pImgBuf->getTimestamp());
