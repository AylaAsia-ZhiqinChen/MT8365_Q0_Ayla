...
import com.mediatek.mms.appservice.pushparser.sl.SlMessage;
...
public class SlManager extends WapPushManager {
    ...
    public void handleIncoming(ParsedMessage message) {
        ...
        if (message == null) {
            Log.w(TAG, "SlManager handleIncoming: null message");
            return;
        }
    ...
    private boolean autoLanuching(String url) {
        ...
        if (isAutoLoading) {
            MmsConfig.updateWappushSlAutoLanuch(mContext, url);
            Uri uri = Uri.parse(checkAndModifyUrl(url));
            ...
            try {
                ...
                MmsConfig.showToast(mContext, MmsConfig.WAPPUSH_UNSUPPORTED_SCHEME);
