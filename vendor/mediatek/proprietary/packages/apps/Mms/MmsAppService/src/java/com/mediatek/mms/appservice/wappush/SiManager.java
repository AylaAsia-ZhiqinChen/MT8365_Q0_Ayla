...
import com.mediatek.mms.appservice.pushparser.si.SiMessage;
...
public class SiManager extends WapPushManager {
    ...
    public void handleIncoming(ParsedMessage message) {
        ...
        if (siMsg.expiration > 0 && siMsg.expiration < currentTime) {
            Log.d(TAG, "SiManager:Expired Message! " + siMsg.url);
            return;
        }

        //5,Discard SI has action = none
        if (siMsg.action == SiMessage.ACTION_NONE) {
            Log.d(TAG, "SiManager:Discard None Message! " + siMsg.url);
            return;
        }
                    ...
                    do {
                        ...
                        if (siid.equals(siMsg.siid)) {
                            ...
                            if (siMsg.create > 0 && siMsg.create < createdTime) {
                                Log.d(TAG, "SiManager:Out of order Message! " + siMsg.url);
                                return;
        ...
        if (siMsg.action == SiMessage.ACTION_DELETE) {
            ...
            MmsConfig.updateWappushNotification(mContext, "none");
            Log.d(TAG, "SiManager:Discard delete Message! " + siMsg.url);
            return;
        }
        ...
        if (!onlyUpdate) {
            ...
            Log.d(TAG, "update a si message.count:" + count);
        }

        //notification
        if (uri != null) {
            //called off of the UI thread so ok to block
            MmsConfig.updateWappushNotification(mContext, uri.toString());
        }
    }
