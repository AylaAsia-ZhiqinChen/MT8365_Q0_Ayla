...
public class CellBroadcastReceiver extends BroadcastReceiver {
    private static final String TAG = "Mms/CellBroadcastReceiver";
    Context mContext;
    int subId;
    ...
    public void onReceive(Context context, Intent intent) {
        if (!PermissionCheckUtil.checkRequiredPermissions(context)) {
            MmsLog.dpi(TAG,
                    "CellBroadcastReceiver onReceive no runtime permissions, intent: " + intent);
            return;
        }
        ...
        if (action != null
            ...
            MmsLog.d(TAG, "simstatus = " + simStatus);
            if (IccCardConstants.INTENT_VALUE_ICC_LOADED.equals(simStatus)) {
                ...
                if (subId == SubscriptionManager.INVALID_SUBSCRIPTION_ID
                        || subId < 0) {
                    MmsLog.d(TAG, "invalid subId");
                    return;
                }
                enableCellBroadcast();
            } else {
                MmsLog.d(TAG, "sim state not ready");
            }
        }
    }

    private void enableCellBroadcast() {
        MmsLog.d(TAG, "enableCellBroadcast" + subId);
        new Thread(new Runnable() {
                public void run() {
                        RecoverChannelSettings setting =
                                new RecoverChannelSettings(subId,
                                        mContext.getContentResolver());
                        if (!setting.updateChannelStatus()) {
                            MmsLog.d(TAG, "setCBState error case");
                        }
                        MmsLog.d(TAG, "setCBState end");
                }
            }).start();
    }
}
