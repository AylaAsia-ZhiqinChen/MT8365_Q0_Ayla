...
public class RecipientIdCache {
    ...
    private static final RecipientIdCache sInstance =
            new RecipientIdCache(MmsApp.getApplication());
        ...
        for (Contact contact : contacts) {
            ...
            synchronized (sInstance) {
                ...
                if (Log.isLoggable(LogTag.APP, Log.VERBOSE)) {
                    MmsLog.dpi(TAG, "[RecipientIdCache] updateNumbers: contact=" + contact +
    ...
    private void updateCanonicalAddressInDb(long id, String number) {
        if (LogTag.VERBOSE || Log.isLoggable(LogTag.APP, Log.VERBOSE)) {
            MmsLog.dpi(TAG, "[RecipientIdCache] updateCanonicalAddressInDb: id=" + id +
                    ", number=" + number);
        }
    ...
    public static void dump() {
        ...
        synchronized (sInstance) {
            ...
            for (Long id : sInstance.mCache.keySet()) {
                MmsLog.dpi(TAG, id + ": " + sInstance.mCache.get(id));
            }
        }
    }
    ...
    public static void canonicalTableDump() {
        ...
        try {
            while (c.moveToNext()) {
                ...
                long id = c.getLong(0);
                String number = c.getString(1);
                MmsLog.dpi(TAG, "id: " + id + " number: " + number);
            }
        } finally {
            c.close();
        }
    ...
    public static String getSingleAddressFromCanonicalAddressInDb(final Context context,
        ...
        Cursor c = SqliteWrapper.query(context, context.getContentResolver(),
                ...
                null, null, null, null);
        if (c == null) {
            MmsLog.wpi(TAG, "null Cursor looking up recipient: " + recipientId);
            return null;
        }
        try {
            if (c.moveToFirst()) {
    ...
    public static void insertCanonicalAddressInDb(final Context context, String number) {
        if (LogTag.VERBOSE || Log.isLoggable(LogTag.APP, Log.VERBOSE)) {
            MmsLog.dpi(TAG, "[RecipientIdCache] insertCanonicalAddressInDb: number=" + number);
        }
