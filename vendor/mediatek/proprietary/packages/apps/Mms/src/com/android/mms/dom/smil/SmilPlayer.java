...
public class SmilPlayer implements Runnable {
    ...
    private static ArrayList<TimelineEntry> getTimeline(ElementTime element,
        ...
        if (element instanceof ElementParallelTimeContainer) {
            ...
            for (int i = 0; i < endList.getLength(); ++i) {
                ...
                if (end.getResolved()) {
                    double endOffset = end.getResolvedOffset() + offset;
                    if ((float) endOffset <= (float) maxOffset) {
    ...
    /// M: Code analyze 002, fix bug ALPS00246974,
    /// new SmilPlayer for updating state when playing slideshow @{
    public static SmilPlayer getPlayer() {
        /** M: mtk81083 use the singlton pattern will come out
         * a bug when there is  animated gif file.
         * the onStop of Activity is invoked late when there is animated gif in the mms.
         * if open the mms again just after out of it, the state is in a mess,
         * the previous state is still keeping.
         */
        //if (sPlayer == null) {
    ...
    public synchronized void pause() {
        if (isPlayingState()) {
            ...
            /// M: Code analyze 003, new feature,
            /// must change PlayerState when change PlayerAction @{
    ...
    private static final class TimelineEntry {
        ...
        public String toString() {
            return "Type = " + mElement + " offset = "
                    + getOffsetTime() + " action = " + getAction();
        }
    }
