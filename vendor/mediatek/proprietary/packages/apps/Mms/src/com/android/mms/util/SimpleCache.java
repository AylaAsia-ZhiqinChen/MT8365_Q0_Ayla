...
public class SimpleCache<K, V> {
    ...
    public void checkAll() {
        Log.d("ThumbnailManager", "checkAll");
        if (mSoftReferences != null) {
            Log.d("ThumbnailManager", "checkAll1");
            //mSoftReferences.clear();
            //Map.Entry<K, SoftReference<V>> entry;
            for (Map.Entry<K, SoftReference<V>> entry : mSoftReferences.entrySet()) {
                Log.d("ThumbnailManager", "key = " + entry.getKey() + ", value = " + entry.getValue());
            }
        } else {
        Log.d("ThumbnailManager", "checkAll2");
            //mHardReferences.clear();
            //Map.Entry<K, V> entry;
            for (Map.Entry<K, V> entry : mHardReferences.entrySet()) {
                Log.d("ThumbnailManager", "key = " + entry.getKey() + ", value = " + entry.getValue());
            }
        }
    }
