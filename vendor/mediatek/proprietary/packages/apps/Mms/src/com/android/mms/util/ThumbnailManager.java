...
public class ThumbnailManager extends BackgroundLoaderManager {
    ...
    private ItemLoadedFuture getThumbnail(Uri uri, boolean isVideo,
        ...
        /// M: ALPS03641125, for non mms part uri, may cause permission denial when opening
        /// provider com.android.providers.media.MediaDocumentsProvider
        final boolean mmsPartUri = (sURLMatcher.match(getRealUri(uri)) == MMS_PART_ID);
        if (!mmsPartUri) {
            MmsLog.epi(TAG, Log.getStackTraceString(new Throwable()));
        }
        ...
        if (Log.isLoggable(TAG, Log.DEBUG)) {
            MmsLog.ipi(TAG, "getThumbnail mThumbnailCache.get for uri: " + uri + " thumbnail: " +
        ...
        if (callbackRequired && mmsPartUri) {
            addCallback(uri, callback);
        }

        if (newTaskRequired && mmsPartUri) {
    ...
    public class ThumbnailTask implements Runnable {
        ...
        @Override
        public void run() {
            MmsLog.dpi(TAG, "ThumbnailTask run, " + this + ", mUri:" + mUri);
            ...
            try {
                ...
                MmsLog.epi(TAG, "Couldn't load bitmap for " + mUri, e);
                ...
                public void run() {
                    ...
                    /* Mem opt*/
                        synchronized(this) {
                            ...
                            MmsLog.ipi(TAG, "in callback runnable: bitmap uri: " + mUri +
                    ...
                    if (Log.isLoggable(LogTag.THUMBNAIL_CACHE, Log.DEBUG)) {
                        MmsLog.dpi(TAG, "Image task for " + mUri + "exiting "
        ...
        private Bitmap getBitmap(boolean isVideo) {
            ...
            if (data != null) {
                ...
                /// @}
                if (bitmap == null) {
                    MmsLog.wpi(TAG, "decode cached failed " + path);
                }
                return bitmap;
            } else {
                Bitmap bitmap;
                ...
                if (bitmap == null) {
                    MmsLog.wpi(TAG, "decode orig failed " + path);
                    return null;
                } else {
                ...
                if (!isTempFile) {
                    ...
                    if (uriString != null &&
                            uriString.contains(FLAG_FNAME) &&
                            (uriString.contains(".png") || uriString.contains(".PNG"))) {
                        array = compressBitmap(bitmap, true);
                    } else {
                        array = compressBitmap(bitmap, false);
                    }
        ...
        private byte[] compressBitmap(Bitmap bitmap, boolean isPng) {
        ...
        private Bitmap requestDecode(final Uri uri, Options options, int targetSize) {
            ...
            try {
                ...
                MmsLog.epi(TAG, "Can't open uri: " + uri, e);
                return null;
            }
