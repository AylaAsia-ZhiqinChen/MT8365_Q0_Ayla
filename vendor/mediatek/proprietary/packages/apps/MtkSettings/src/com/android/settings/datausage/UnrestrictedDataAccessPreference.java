...
public class UnrestrictedDataAccessPreference extends AppSwitchPreference implements
    ...
    private static final String TAG = "UnrestrictedDataAccessPreference";
    ...
    public UnrestrictedDataAccessPreference(final Context context, AppEntry entry,
        ...
        if (mEntry.icon != null) {
            setIcon(mEntry.icon);
        /// M: Add log for app without icon. @{
        } else {
            Log.d(TAG, "No icon found for app="
                    + (mEntry.label == null ? "null" : mEntry.label));
        /// @}
        }
    }
    ...
    protected void onClick() {
        /// M: Add null checking for data usage state.
        if (mDataUsageState != null && mDataUsageState.isDataSaverBlacklisted) {
        ...
        if (mEntry.icon == null) {
            holder.itemView.post(new Runnable() {
                ...
                public void run() {
                    ...
                    /// M: Add log for app without icon. @{
                    if (mEntry.icon == null) {
                        Log.d(TAG, "Ensure icon but no icon found for app="
                                + (mEntry.label == null ? "null" : mEntry.label));
                    }
                    /// @}
