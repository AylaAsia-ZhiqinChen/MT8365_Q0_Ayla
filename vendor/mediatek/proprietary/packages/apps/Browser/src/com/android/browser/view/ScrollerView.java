...
public class ScrollerView extends FrameLayout {
    private static final String LOGTAG = "ScrollerView";
    ...
    public void setOrientation(int orientation) {
        ...
        Log.d(LOGTAG, "ScrollerView.setOrientation(): mHorizontal = " + mHorizontal);
        requestLayout();
    }
    ...
    public boolean onInterceptTouchEvent(MotionEvent ev) {
        ...
        case MotionEvent.ACTION_MOVE: {
            ...
            /// M: invalid pointer id error handling @ {
            if (pointerIndex == -1) {
                Log.e(LOGTAG, "Invalid active pointer index = " + activePointerId
                        + " at onInterceptTouchEvent ACTION_MOVE");
                break;
            }
            /// @ }
    ...
    public boolean onTouchEvent(MotionEvent ev) {
        ...
        switch (action & MotionEvent.ACTION_MASK) {
                ...
                if (mIsOrthoDragged) {
                    ...
                    /// M: invalid pointer id error handling @ {
                     if (activePointerIndex == -1) {
                        Log.e(LOGTAG, "Invalid active pointer index = " + mActivePointerId
                                + " at onTouchEvent ACTION_MOVE");
                        break;
                    }
                    /// @ }
                    ...
                    if (activePointerIndex == -1) {
                        Log.e(LOGTAG, "Invalid active pointer index = " + mActivePointerId
                                + " at onTouchEvent ACTION_MOVE begin dragged");
                        break;
                    }
                    /// @ }
            ...
            case MotionEvent.ACTION_POINTER_UP:
                ...
                if (pointerIndex == -1) {
                    Log.e(LOGTAG, "Invalid active pointer index = " + mActivePointerId
                            + " at onTouchEvent ACTION_POINTER_UP");
                    break;
                }
    ...
    private View findFocusableViewInMyBounds(final boolean topFocus,
        ...
        final int bottomWithoutFadingEdge
            = top + (mHorizontal ? getWidth() : getHeight()) - fadingEdgeLength;

        if ((preferredFocusable != null)
                && ((mHorizontal ? preferredFocusable.getLeft() : preferredFocusable.getTop())
                        < bottomWithoutFadingEdge)
                && ((mHorizontal ? preferredFocusable.getRight() : preferredFocusable.getBottom())
        ...
        for (int i = 0; i < count; i++) {
            ...
            if (top < viewBottom && viewTop < bottom) {
                ...
                if (focusCandidate == null) {
                    ...
                    final int ctop = mHorizontal ?
                                     focusCandidate.getLeft() : focusCandidate.getTop();
                    final int cbot = mHorizontal ?
                                     focusCandidate.getRight() : focusCandidate.getBottom();
