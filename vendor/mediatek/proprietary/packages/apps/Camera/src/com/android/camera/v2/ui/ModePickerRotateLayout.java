...
public class ModePickerRotateLayout extends ViewGroup implements Rotatable {
    private static final Tag TAG = new Tag(ModePickerRotateLayout.class.getSimpleName());
    ...
    public void setOrientation(int orientation, boolean animation) {
        LogHelper.d(TAG, "setOrientation(" + orientation + ", " + animation + ") mOrientation="
                + mOrientation);
        orientation = (orientation - ORIENTATION_OFFSET + 360) % 360;
        if (mOrientation != orientation) {
            mOrientation = orientation;
            CameraUtil.setOrientation(mChild, mOrientation, animation);
            requestLayout();
        }
    }
}
