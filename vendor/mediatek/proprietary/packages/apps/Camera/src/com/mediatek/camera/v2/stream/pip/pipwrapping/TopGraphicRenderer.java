...
public class TopGraphicRenderer extends Renderer {
    private static final Tag TAG = new Tag(TopGraphicRenderer.class.getSimpleName());
    // position
    ...
    public TopGraphicRenderer(Activity activity) {
        super(activity);
        LogHelper.d(TAG, "TopGraphicRenderer");
        initProgram();
    ...
    public void initTemplateTexture(int backResource, int frontResource) {
        LogHelper.d(TAG, "initTemplateTexture");
        if (mBackTempResId != backResource) {
            if (mBackTempTexId > 0) {
            ...
            try {
                ...
                LogHelper.e(TAG, "initBitmapTexture faile + " + e);
            }
            mBackTempResId = backResource;
        }
    ...
    public void setRendererSize(int width, int height, boolean needPortrait) {
        ...
        mTopTemplateRenderer.setRendererSize(selectedWidth, selectedHeight, false);
    }
    ...
    public void draw(int preTex, final float[] preTexMtx, final float[] texReverseRotateMtx,
        ...
        // copy AnimationRect
        AnimationRect animationRect = topRect.copy();
        ...
        if (needFlip && rotation % 180 != 0) {
            topGraphicCropValue = 1 - topGraphicCropValue;
        }
        // preview and video snapshot, preview is revered
        ...
        switch (rotation) {
        case 0:
            mTexCoordBuf = createFloatBuffer(mTexCoordBuf, GLUtil.createReverseStandTexCoord(
                    lowWidth, highWidth, lowHeight, highHeight, needFlip));
            break;
        case 90:
            mTexCoordBuf = createFloatBuffer(mTexCoordBuf,
                    GLUtil.createRightTexCoord(lowWidth, highWidth,
                            lowHeight, highHeight, needFlip));
            break;
        case 180:
            mTexCoordBuf = createFloatBuffer(mTexCoordBuf,
                    GLUtil.createStandTexCoord(lowWidth, highWidth,
                            lowHeight, highHeight, needFlip));
            break;
        case 270:
            mTexCoordBuf = createFloatBuffer(mTexCoordBuf,
                    GLUtil.createLeftTexCoord(lowWidth, highWidth,
                            lowHeight, highHeight, needFlip));
            break;
        }
        ...
        mTopTemplateRenderer.draw(0, 0, 0, getVtxFloatBuffer());
