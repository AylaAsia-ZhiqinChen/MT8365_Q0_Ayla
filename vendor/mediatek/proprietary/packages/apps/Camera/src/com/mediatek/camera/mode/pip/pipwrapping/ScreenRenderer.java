...
public class ScreenRenderer extends Renderer implements Runnable {
    ...
    protected void setSurface(Surface surface, boolean scaled, boolean rotated) {
        Log.d(TAG, "setSurface scaled = " + scaled + " rotated = " + rotated +
                ", mPreviewSurface = " + mPreviewSurface);
        if (skipUpdateSurface(surface)) {
            Log.d(TAG, "the same surface skip update mPreviewSurface = " + mPreviewSurface);
    ...
    private boolean isMatchingSurfaceSize(int width, int height) {
        Log.d(TAG, "isMatchingSurfaceSize mRenderTexWidth:" + mRenderTexWidth
    ...
    private void waitRendererThreadActive() {
        synchronized (mReadyFence) {
            if (mRunning) {
                Log.d(TAG, "screen renderer already running!");
                return;
            }
            mRunning = true;
            while (!mReady) {
                try {
                    Log.d(TAG, "wait for screen renderer thread ready, current mReady = " + mReady);
                    mReadyFence.wait();
    ...
    public void run() {
        Looper.prepare();
        synchronized (mReadyFence) {
            Log.d(TAG, "Screen renderer thread started!");
            mScreenHandler = new ScreenHandler();
            mReady = true;
            mReadyFence.notify();
        }
        Looper.loop();
        Log.d(TAG, "Screen renderer thread exiting!");
        synchronized (mReadyFence) {
    ...
    private void updateRendererSize(int width, int height) {
        Log.d(TAG, "updateRendererSize width = " + width + " height = " + height);
        resetMatrix();
    ...
    private class ScreenHandler extends Handler {
        ...
        private void doSetupEGLSurface() {
            Log.d(TAG, "handleSetupSurface  mEglCore = " + mEglCore +
        ...
        private void doUpdateEGLSurface() {
            Log.d(TAG, "updateEGLSurface mPreviewEGLSurface = " + mPreviewEGLSurface);
            if (mPreviewEGLSurface != null) {
