...
/**
 * An utils for GL operation.
 */
public class GLUtil {
    public static final Tag TAG = new Tag(GLUtil.class.getSimpleName());
     ...
     * 3->4->1
     */
    public static float[] createTopGraphicRect(int width, int height, float toTop) {
        int minValue = Math.min(width, height);
        float topGrapihcEdgeLength = (float) minValue *
        ...
        if (width >= height) {
            // landscape
            left = toTop;
            top = minValue * (1 - PIPCustomization.TOP_GRAPHIC_LEFT_TOP_RELATIVE_VALUE)
                    - topGrapihcEdgeLength;
        } else {
            // portrait
            left = minValue * PIPCustomization.TOP_GRAPHIC_LEFT_TOP_RELATIVE_VALUE;
            top = toTop;
        }
        float vertices[] = new float[] {
                left,                        top,                         0,
                left,                        topGrapihcEdgeLength + top,  0,
                left + topGrapihcEdgeLength, topGrapihcEdgeLength + top,  0,

                left + topGrapihcEdgeLength, topGrapihcEdgeLength + top,  0,
                left + topGrapihcEdgeLength, 0 + top,                     0,
                left,                        0 + top,                     0,
                };
        return vertices;
    }
    ...
    public static float[] createTopRightRect(int width, int height, float toTop) {
        ...
        float vertices[] = new float[] {
                (float) minValue * PIPCustomization.TOP_GRAPHIC_LEFT_TOP_RELATIVE_VALUE,
                    0 + toTop,              0,
                (float) minValue * PIPCustomization.TOP_GRAPHIC_LEFT_TOP_RELATIVE_VALUE,
                    topGrapihcEdge + toTop, 0,
                (float) minValue * PIPCustomization.TOP_GRAPHIC_LEFT_TOP_RELATIVE_VALUE
                    + topGrapihcEdge, topGrapihcEdge + toTop, 0,
    ...
    public static float[] convertAnimationRectToTextureArray(AnimationRect rect) {
    ...
    private static float[] createTexCoord(float lowWidth, float highWidth,
            float lowHeight, float highHeight) {
        float texCoor[] = new float[] {
                lowWidth, lowHeight,
                lowWidth, highHeight,
    ...
    public static float[] createStandTexCoord(float lowWidth, float highWidth, float lowHeight,
            float highHeight, boolean horizontalFlip) {
        float texCoor[];
        if (horizontalFlip) {
            texCoor = new float[] {
                    lowWidth, lowHeight,
    ...
    public static float[] createReverseStandTexCoord(float lowWidth, float highWidth,
            float lowHeight, float highHeight, boolean horizontalFlip) {
        float texCoor[];
        if (horizontalFlip) {
            texCoor = new float[] {
                    highWidth, highHeight,
    ...
    public static float[] createRightTexCoord(float lowWidth, float highWidth,
            float lowHeight, float highHeight, boolean horizontalFlip) {
        float texCoor[];
        if (horizontalFlip) {
            texCoor = new float[] {
    ...
    public static float[] createLeftTexCoord(float lowWidth, float highWidth,
            float lowHeight, float highHeight, boolean horizontalFlip) {
        float texCoor[];
        if (horizontalFlip) {
            texCoor = new float[] {
    ...
    public static int[] generateTextureIds(int num) {
        LogHelper.d(TAG, "GLUtil glGenTextures num = " + num);
        ...
        LogHelper.d(TAG, "GL_MAX_TEXTURE_SIZE sizes[0] = " + sizes[0] + " size[1] = " + sizes[1]);
        return textures;
    }

    public static void deleteTextures(int[] textureIds) {
        LogHelper.d(TAG, "GLUtil glDeleteTextures num = " + textureIds.length);
