            ...
            while ((xmlEventType = parser.next()) != XmlPullParser.END_DOCUMENT) {
                ...
                if (xmlEventType == XmlPullParser.START_TAG && "VoiceWakeupInfo".equals(name)) {
                    ...
                    if (packageName != null && (className != null || actionName != null)) {
                        String classOrAction = (className != null) ? className : actionName;
                        ComponentName component = new ComponentName(packageName, classOrAction);
                        ActivityInfo activityInfo = null;
                        try {
                            activityInfo = mContext.getPackageManager().getActivityInfo(component,
                            PackageManager.GET_SHARED_LIBRARY_FILES);
                         } catch (NameNotFoundException e) {
                             Log.e(TAG, "[readVoiceWakeupFromXml]NameNotFoundException:", e);
                         }
                        if (keyWord != null) {
                            String[] keyWordArray = keyWord.split(",");
                        ...
                        if (activityInfo != null) {
                            //Note: This will be null if no className and only actionName
                            Log.d(TAG, "[readVoiceWakeupFromXml] activityInfo != Null");
                            wakeupList.add(info);
                        } else {
                            Log.d(TAG, "[readVoiceWakeupFromXml] activityInfo == Null");
                            continue;
