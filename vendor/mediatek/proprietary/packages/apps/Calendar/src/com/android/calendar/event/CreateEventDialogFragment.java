...
public class CreateEventDialogFragment extends DialogFragment implements TextWatcher {
    ...
    public void onResume() {
        ...
        if (mButtonAddEvent == null) {
            ...
            mButtonAddEvent.setEnabled(mEventTitle.getText().toString().trim().length() > 0);
        }
    }
    ...
    private void createAllDayEvent() {
        ...
        } else if (mEditEventHelper.saveEvent(mModel, null, 0)) {
    ...
    public void onTextChanged(CharSequence s, int start, int before, int count) {
        if (mButtonAddEvent != null) {
            mButtonAddEvent.setEnabled(s.toString().trim().length() > 0);
        }
    }
