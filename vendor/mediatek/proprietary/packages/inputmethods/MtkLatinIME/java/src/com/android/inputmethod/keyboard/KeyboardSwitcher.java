...
public final class KeyboardSwitcher implements KeyboardState.SwitchActions {
    ...
    public void loadKeyboard(final EditorInfo editorInfo, final SettingsValues settingsValues,
        ...
        try {
            //can not rely on coming capsstate because capsstate may have been updated
            //after build new subtype. need to compute capstate of loaded subtype again
            mState.onLoadKeyboard(mLatinIME.getCurrentAutoCapsState(), currentRecapitalizeState);
    ...
    public boolean isCurrentKeyboardIndic() {
        int currsubtypescript = getCurrentKeyboardScriptId();

        if ((currsubtypescript == ScriptUtils.SCRIPT_BENGALI)   ||
                (currsubtypescript == ScriptUtils.SCRIPT_DEVANAGARI) ||
                (currsubtypescript == ScriptUtils.SCRIPT_KANNADA)   ||
                (currsubtypescript == ScriptUtils.SCRIPT_MALAYALAM) ||
                (currsubtypescript == ScriptUtils.SCRIPT_TAMIL)     ||
                (currsubtypescript == ScriptUtils.SCRIPT_TELUGU)    ||
                (currsubtypescript == ScriptUtils.SCRIPT_GURMUKHI)  ||
                (currsubtypescript == ScriptUtils.SCRIPT_ORIYA)     ||
                //supported script which follow indic layout
                (currsubtypescript == ScriptUtils.SCRIPT_GEORGIAN)  ||
                (currsubtypescript == ScriptUtils.SCRIPT_KHMER)     ||
                (currsubtypescript == ScriptUtils.SCRIPT_LAO)       ||
            (currsubtypescript == ScriptUtils.SCRIPT_MYANMAR)   ||
            (currsubtypescript == ScriptUtils.SCRIPT_SINHALA)   ||
                (currsubtypescript == ScriptUtils.SCRIPT_THAI)      ||
                (currsubtypescript == ScriptUtils.SCRIPT_GUJARATI)) {
            return true ;
        }
        return false ;
    }
