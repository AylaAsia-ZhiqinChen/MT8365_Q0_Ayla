 ...
 * register context.
 */
/*CASSERT(TSPD_C_RT_CTX_SIZE == sizeof(c_rt_regs_t), ==> modify for checkpatch*/
/*		assert_spd_c_rt_regs_size_mismatch);*/
CASSERT(sizeof(c_rt_regs_t) == TSPD_C_RT_CTX_SIZE,
		assert_spd_c_rt_regs_size_mismatch);
...
struct _teei_context {
	uint32_t state;
	uint64_t mpidr;
	uint64_t c_rt_ctx;
	cpu_context_t cpu_ctx;
};
#define teei_context struct _teei_context
#define KEY_LEN 32

enum device_type {
	MT_UNUSED = 0, MT_UART16550 = 1, MT_SEC_GPT, MT_SEC_WDT, MT_SEC_CRYPTO,
};
struct tee_dev_t {
	uint32_t dev_type;
	uint64_t base_addr;
...
#define tee_dev_t_ptr struct tee_dev_t *
...
#define tee_arg_t_ptr struct tee_arg_t *


struct  tee_keys_t {
	uint32_t magic; /* 0x434d4254*/
	uint32_t version;
	uint8_t rpmb_key[KEY_LEN];
	uint8_t hw_id[KEY_LEN];

};
#define tee_keys_t_ptr struct tee_keys_t *
 ...
 ******************************************************************************/
extern void teei_setup_entry(cpu_context_t *ns_context, uint32_t call_offset,
		uint32_t regfileNro);
