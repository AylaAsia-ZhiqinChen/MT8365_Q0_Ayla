			 ...
			 uint64_t flags)
{
	...
	if (is_caller_secure(flags)) {
		...
		NOTICE("%s(0x%x, 0x%lx, 0x%lx, 0x%lx, 0x%lx, 0x%lx, 0x%lx, 0x%lx) cpu %d, unknown smc\n",
		       __func__, smc_fid, x1, x2, x3, x4, (long unsigned int)cookie, (long unsigned int)handle, flags, cpu);
...
static void trusty_cpu_suspend(void)
{
	...
	if (ret.r0 != 0) {
		NOTICE("%s: cpu %d, SMC_FC_CPU_SUSPEND returned unexpected value, %lx\n",
...
static void trusty_cpu_resume(void)
{
	...
	if (ret.r0 != 0) {
		NOTICE("%s: cpu %d, SMC_FC_CPU_RESUME returned unexpected value, %lx\n",
		       __func__, linear_id, ret.r0);
	}
}
