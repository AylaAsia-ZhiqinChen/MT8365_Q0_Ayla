...
int fiq_smp_call_cpu(int cpu, inter_cpu_call_func_t func, void *info, int timeout)
{
	int lockval, tmp;
	int cnt = 0;
	...
	if (cpu < 0 || cpu >= PLATFORM_CORE_COUNT)
		return -1;
	...
	irq_raise_softirq(1 << cpu, FIQ_SMP_CALL_SGI);

	/* wait for timeout */
	while (cfd[cpu].lock != 0) {
		if (cnt > timeout) {
			/* INFO("Wait cpu%d too long, timeout = %d\n", cpu, timeout); */
			cfd[cpu].lock = 0;
			break;
		}
