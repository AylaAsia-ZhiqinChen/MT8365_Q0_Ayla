...
void go_to_sodi_before_wfi_no_resume(void)
{
	...
	if (pwrctrl->pcm_flags & SPM_FLAG_ENABLE_SODI3) {
		if (!pwrctrl->wdt_disable) {
			__spm_set_pcm_wdt(1);
			mmio_write_32(SODI3_FOOTPRINT, mmio_read_32(SODI3_FOOTPRINT) | SPM_WDT_ENABLE_FOOTPRINT);
		}
	}
...
static void go_to_sodi_after_wfi(void)
{
	...
	if (pwrctrl->pcm_flags & SPM_FLAG_ENABLE_SODI3) {
		if (!pwrctrl->wdt_disable) {
			__spm_set_pcm_wdt(0);
			mmio_write_32(SODI3_FOOTPRINT, mmio_read_32(SODI3_FOOTPRINT) | SPM_WDT_DISABLE_FOOTPRINT);
		}
	}

	__spm_get_wakeup_status(&spm_wakesta);
	__spm_clean_after_wakeup();
	spm_wake_reason = __spm_output_wake_reason(&spm_wakesta, pcmdesc, "sodi");
#if 0
