...
static void show_atf_wdt_footprint(void)
{
	...
	for (cpu = 0; cpu < PLATFORM_CORE_COUNT; cpu++) {
		...
		NOTICE("enter(%lu)", p_atf_log_ctrl->info.fiq_irq_enter_timestamp[cpu]);
...
static int mt_crash_log_dump(uint8_t *crash_log_addr, uint32_t crash_log_size,
	uint32_t start, uint32_t size, uint32_t aee_buf_size)
{
	uint64_t ret = 0;
	uint64_t read_count;
	uint64_t offset;
