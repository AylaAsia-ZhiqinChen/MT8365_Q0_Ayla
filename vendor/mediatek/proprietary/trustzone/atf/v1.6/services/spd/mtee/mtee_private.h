...
extern struct mtee_context mtee_sp_context[MTEE_CORE_COUNT];
...
extern struct mtee_context *secure_context;
...
extern int32_t mtee_init(void);

extern uint64_t mtee_enter_sp(uint64_t *c_rt_ctx);
extern void __dead2 mtee_exit_sp(uint64_t c_rt_ctx, uint64_t ret);
extern uint64_t mtee_synchronous_sp_entry(struct mtee_context *mtee_ctx);
extern void __dead2 mtee_synchronous_sp_exit(struct mtee_context *mtee_ctx, uint64_t ret);
