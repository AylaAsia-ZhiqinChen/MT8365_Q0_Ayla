...
int32_t pmic_read_interface(uint32_t RegNum, uint32_t *val, uint32_t MASK, uint32_t SHIFT)
{
	int32_t return_value = 0;
	uint32_t pmic_reg = 0, rdata;

	return_value = pwrap_read(RegNum, &rdata);
	pmic_reg = rdata;
	if (return_value != 0) {
		ERROR("[PMIC]Reg[0x%x]= pmic_wrap read data fail\n", RegNum);
		return return_value;
	}
...
int32_t pmic_config_interface(uint32_t RegNum, uint32_t val, uint32_t MASK, uint32_t SHIFT)
{
	int32_t return_value = 0;
	uint32_t pmic_reg = 0, rdata;
	...
	pmic_reg = rdata;
	if (return_value != 0) {
		ERROR("[PMIC]Reg[%x]= pmic_wrap read data fail\n", RegNum);
		return return_value;
	}
