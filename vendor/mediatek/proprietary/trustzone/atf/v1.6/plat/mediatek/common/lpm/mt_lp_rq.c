...
spinlock_t mt_lp_rq_lock;

/* Determine for spm sw resource user */
struct mt_lp_resource_user spm_res_user;
				  ...
				  unsigned int resource)
{
	...
	if (!this || !resource
		  || resource > MT_LP_RQ_ALL) {
		ERROR("invalid request(%x)\n", resource);
		return MT_LP_RQ_STA_BAD;
	}

	spin_lock(&mt_lp_rq_lock);
	for (i = 0, rs = (plat_mt_rqm.plat_rqm)->res;
	     i < plat_mt_rqm.resource_num; i++) {
		if (resource & rs[i]->res_id)
...
int mt_lp_resource_request_manager_register(
				struct mt_resource_req_manager *rqm)
{
	int i;
	struct mt_lp_res_req *const *rs;

	if (!rqm || !rqm->res
		 || plat_mt_rqm.plat_rqm
		 || (!mtk_lk_stage))
		return MT_LP_RQ_STA_BAD;

	for (i = 0, rs = rqm->res; *rs ; i++, rs++)
		;
	plat_mt_rqm.plat_rqm = rqm;
	plat_mt_rqm.resource_num = i;
	return MT_LP_RQ_STA_OK;
}
...
int mt_lp_resource_user_register(char *user, struct mt_lp_resource_user *ru)
{
	...
	if (!plat_mt_rqm.plat_rqm
	    || plat_mt_rqm.user_num >= MT_LP_RQ_USER_MAX
	    || !user) {
	...
	for (i = 0, uname = 0, s = 0;
	     i < len; i++, s += MT_LP_RQ_USER_CHAR_U)
		uname |= user[i] << s;
...
int mt_lp_rq_get_status(int type, void *p)
{
	...
	struct mt_lp_res_req *const *rs;
	struct resource_req_status *rq_sta =
		(struct resource_req_status *)p;

	if (plat_mt_rqm.flag) {
		spin_lock(&mt_lp_rq_lock);
		for (i = 0, update_sta = 0, rs = (plat_mt_rqm.plat_rqm)->res;
		     i < plat_mt_rqm.resource_num; i++) {
			update_sta |=
				((rs[i]->res_usage & plat_mt_rqm.user_valid)
				? rs[i]->res_rq : 0);
		}
...
int mt_lp_rq_update_status(int type, void *p)
{
	...
	if (type == PLAT_RQ_USER_VALID) {
		if (rq_sta->id < plat_mt_rqm.user_num) {
			user_mask = 1 << rq_sta->id;
			spin_lock(&mt_lp_rq_lock);
			plat_mt_rqm.user_valid = (!rq_sta->val) ?
				(plat_mt_rqm.user_valid & ~(user_mask)) :
				(plat_mt_rqm.user_valid | user_mask);
			plat_mt_rqm.flag = MT_LP_RQ_FLAG_NEED_UPDATE;
			spin_unlock(&mt_lp_rq_lock);
		}
	} else if (type == PLAT_RQ_USER_REQ) {
		if (spm_res_user.request
		    && (rq_sta->val < plat_mt_rqm.resource_num))
			spm_res_user.request(&spm_res_user, 1<<rq_sta->val);
	} else if (type == PLAT_RQ_USER_REL) {
		if (spm_res_user.release)
			spm_res_user.release(&spm_res_user);
	}
