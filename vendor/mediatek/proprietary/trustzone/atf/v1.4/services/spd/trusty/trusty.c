...
#include <mtk_plat_common.h> /* for struct atf_arg_t */
...
/* store info. from preloader for Trusty */
static struct tee_v8_arg_t mt_trusty_boot_cfg;
				   ...
				   void *cookie)
{
	...
	if ((id == WDT_IRQ_BIT_ID) || (id == FIQ_SMP_CALL_SGI)) {
		...
		/* route to S-EL1 */
		trusty_context_switch(NON_SECURE, SMC_YC_NOP, 0, 0, 0);
		SMC_RET0(handle);
	}
...
static int32_t trusty_init(void)
{
	...
	if (cpu == 0)
		INFO("Enter Trusty OS cold boot\n");
	trusty_context_switch_helper(&ctx->saved_sp, &zero_args);
	if (cpu == 0)
		INFO("Return from Trusty OS cold boot\n");
...
static void trusty_cpu_resume(void)
{
	...
	/*
	 * for 32-bit trusty, the virtual timer is required
	 * and thus should be reinitialized when CPU resume
	 */
	write_cntvoff_el2(0);
...
{
	struct trusty_cpu_ctx *ctx = get_trusty_ctx();
	struct atf_arg_t *atfarg = &gteearg;

	struct tee_v8_arg_t *teearg = (struct tee_v8_arg_t *)(uintptr_t)atfarg->tee_boot_arg_addr;
	...
	memcpy(mt_trusty_boot_cfg.rpmb_key, teearg->rpmb_key, RPMB_KEY_SIZE);
...
#if 0
	{
		...
		for (i = 0; i < RPMB_KEY_SIZE; i += 8) {
			INFO("mt_trusty_boot_cfg.rpmb_key[%d-%d] : 0x%x 0x%x 0x%x 0x%x 0x%x 0x%x 0x%x 0x%x\n",
					i, i+7,
					mt_trusty_boot_cfg.rpmb_key[i+0], mt_trusty_boot_cfg.rpmb_key[i+1],
					mt_trusty_boot_cfg.rpmb_key[i+2], mt_trusty_boot_cfg.rpmb_key[i+3],
					mt_trusty_boot_cfg.rpmb_key[i+4], mt_trusty_boot_cfg.rpmb_key[i+5],
					mt_trusty_boot_cfg.rpmb_key[i+6], mt_trusty_boot_cfg.rpmb_key[i+7]);
		}
	...
	ep_info->args.arg2 = sizeof(struct tee_v8_arg_t);
