...
static volatile int  cluster_flush_check[PLATFORM_CORE_COUNT];
/* only updated by the current core, could be not atomic */

static uint64_t fiq_step_addr;
static void (*fp_dcsw_op_level[PLATFORM_CACHE_LEVEL_ALL])(unsigned int) = {
				dcsw_op_level1, dcsw_op_level2, dcsw_op_level3};
...
void flush_cache(int start, int count)
{
	int i;

	for (i = 0; i < count; i++)
		fp_dcsw_op_level[i + start](DCCISW);
}

static void flush_core_cache(void *info)
{
	flush_cache(0, PLATFORM_CACHE_LEVEL_PER_CORE);
}

static void flush_cluster_cache(void *info)
{
	flush_cache(PLATFORM_CACHE_LEVEL_PER_CORE,
			PLATFORM_CACHE_LEVEL_PER_CLUSTER - PLATFORM_CACHE_LEVEL_PER_CORE);
}

static void flush_last_cache(void *info)
{
	flush_cache(PLATFORM_CACHE_LEVEL_PER_CLUSTER,
			PLATFORM_CACHE_LEVEL_ALL - PLATFORM_CACHE_LEVEL_PER_CLUSTER);
}

static void clear_cluster_flush_check(void)
{
	int i;

	for (i = 0; i < PLATFORM_CORE_COUNT; ++i)
		cluster_flush_check[i] = 0;
}
...
#define PLATFORM_CPUMASK	((1 << PLATFORM_CORE_COUNT)-1)
	...
	unsigned int cpus_should_flush_core_cache;
	unsigned int cpus_should_flush_cluster_cache;
	int i = 0;
	...
	/* get the cpu mask for those cpus need to flush cache but current one */
	cpus_should_flush_core_cache = ((~cpus_dont_flush_cache) & PLATFORM_CPUMASK)
					& (~(1 << cpu));

	set_fiq_cache_debug_step(0x4);

	/* ask other cores to flush per-core's cache, no wait!! */
	fiq_smp_call_function(cpus_should_flush_core_cache, flush_core_cache, 0, 1);

	set_fiq_cache_debug_step(0x5);

	/* flush per-core's cache by this core */
	flush_core_cache(NULL);
	...
	last_cid = 99;
	cpus_should_flush_cluster_cache = 0;
	for (i = 0; i < PLATFORM_CORE_COUNT; ++i) {
		if (!((1 << i) & cpus_should_flush_core_cache))
			continue;

		cid = get_cid(i);
		if (cid != last_cid) {
			last_cid = cid;
			cluster_flush_check[i] = 1;
			cpus_should_flush_cluster_cache |= (1<<i);
		}
	}

	fiq_smp_call_function(cpus_should_flush_cluster_cache, flush_cluster_cache, 0, 1);
	...
	if (this_cpu_need_flush_cluster_cache())
	...
	/* flush the last cache if needed */
	flush_last_cache(NULL);
