...
/**
 * storage_rpmb_read: Read RPMB raw blocks
 * @session: the storage_session_t returned from a call to storage_open_session
 * @blk_addr: the RPMB block address
 * @blk_num: the number of RPMB blocks to read
 * @buf: the buffer for in which to write the data read
 * @buflen: the size of buf
 */
int storage_rpmb_read(storage_session_t session, uint32_t blk_addr, uint32_t blk_num, void *buf, size_t buflen);
