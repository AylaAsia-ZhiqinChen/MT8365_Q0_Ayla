...
enum storage_cmd {
	...
	STORAGE_RPMB_RAW_READ  =  10 << STORAGE_REQ_SHIFT,
};
...
/**
 * struct storage_rpmb_raw_read_req - request format for STORAGE_RPMB_RAW_READ
 * @block_addr: the base block address
 * @block_num:  the number of blocks to read, each block contains 256 bytes
 */
struct storage_rpmb_raw_read_req {
	uint32_t blk_addr;
	uint32_t blk_num;
};
