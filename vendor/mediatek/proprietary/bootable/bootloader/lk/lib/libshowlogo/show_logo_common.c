...
/* begin: optimizate show logo time */
void fill_point_buffer_blue_argb8888(unsigned int *fill_addr, unsigned int src_color)
{
    *fill_addr = ARGB8888_TO_ABGR8888(src_color);
}

void fill_point_buffer_no_blue_argb8888(unsigned int *fill_addr, unsigned int src_color)
{
    *fill_addr = src_color;
}

void fill_point_buffer_blue_rgb565_argb8888(unsigned int *fill_addr, unsigned int src_color)
{
    *fill_addr = RGB565_TO_ARGB8888(src_color);
}

void fill_point_buffer_no_blue_rgb565_abgr8888(unsigned int *fill_addr, unsigned int src_color)
{
    *fill_addr = RGB565_TO_ABGR8888(src_color);
}
/* end: optimizate show logo time */
...
void fill_rect_with_content_by_32bit_argb8888(unsigned int *fill_addr, RECT_REGION_T rect, unsigned int *src_addr, LCM_SCREEN_T phical_screen, unsigned int bits)
{
    ...
    void (*fpb)(unsigned int *,unsigned int);

    if (32 == phical_screen.bits_per_pixel) {
        if (32 == bits) {
            if(16 == phical_screen.blue_offset) {
                fpb = fill_point_buffer_blue_argb8888;
            } else {
                fpb = fill_point_buffer_no_blue_argb8888;
            }
        }else {
            if(16 == phical_screen.blue_offset) {
                fpb = fill_point_buffer_blue_rgb565_argb8888;
            } else {
                fpb = fill_point_buffer_no_blue_rgb565_abgr8888;
            }
        }
    } else {
        LOG_ANIM("[show_logo_common %s %d]not support bits_per_pixel = %d \n",__FUNCTION__,__LINE__,(int)phical_screen.bits_per_pixel);
