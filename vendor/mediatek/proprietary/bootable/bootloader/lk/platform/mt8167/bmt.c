...
#define BLOCK_SIZE_BMT          ((devinfo.NAND_FLASH_TYPE == NAND_FLASH_TLC) ? (devinfo.blocksize * 1024) : \
    (nand_chip_bmt->sector_size == 512?nand_chip_bmt->erasesize : nand_chip_bmt->erasesize*2))
...
#define PAGE_ADDR(block)    ((block) * ((devinfo.NAND_FLASH_TYPE == NAND_FLASH_TLC) ? \
    PAGE_PER_SIZE_BMT : (BLOCK_SIZE_BMT / PAGE_SIZE_BMT)))
...
static int load_bmt_data(int start, int pool_size)
{
	...
	for (bmt_index = start + pool_size - 1; bmt_index >= start; bmt_index--) {
		...
		if (nand_block_bad_bmt((devinfo.NAND_FLASH_TYPE == NAND_FLASH_TLC) ?
		                       OFFSET(bmt_index):OFFSET_MLC(bmt_index))) {
...
static int find_available_block(bool start_from_end)
{
	...
	if ((!pool_erased)  &&
		...
		for (i = 0; i < bmt_block_count; i++) {
			...
			if (nand_block_bad_bmt((devinfo.NAND_FLASH_TYPE == NAND_FLASH_TLC)?
			                       OFFSET(block + i):OFFSET_MLC(block + i))) {
			...
			if (!nand_erase_bmt((devinfo.NAND_FLASH_TYPE == NAND_FLASH_TLC)?
			                    OFFSET(block + i):OFFSET_MLC(block + i))) {
				MSG(INIT, "Erase block 0x%x failed\n", block + i);
				mark_block_bad_bmt((devinfo.NAND_FLASH_TYPE == NAND_FLASH_TLC)?
				                   OFFSET(block + i):OFFSET_MLC(block + i));
			}
		}
...
static int migrate_from_bad(u64 offset, u8 * write_dat, u8 * write_oob)
{
	...
	if (devinfo.NAND_FLASH_TYPE != NAND_FLASH_TLC) {
		error_block = (u32)(offset / (devinfo.blocksize * 1024));
		error_page = (u32)(offset >> nand_chip_bmt->page_shift) % page_per_block;
	} else {
		error_block = (u32)(offset / BLOCK_SIZE_BMT);
		error_page = (u32)(offset / PAGE_SIZE_BMT) % page_per_block;
	}
	...
	if ((devinfo.NAND_FLASH_TYPE == NAND_FLASH_TLC)
		...
		if (!tlc_mode_block) {
			//mtk_slc_blk_addr((orig_block * (devinfo.blocksize * 1024)), &error_block, &error_page);
...
#endif
	{
		...
		if (!write_dat) {
			nand_read_page_bmt(PAGE_ADDR(error_block)
			                   + error_page, dat_buf, NULL);
			write_dat = dat_buf;
		}
		// memcpy(oob_buf, write_oob, MAX_OOB_SIZE);
...
static bool write_bmt_to_flash(u8 * dat, u8 * oob)
{
	...
	if (devinfo.NAND_FLASH_TYPE == NAND_FLASH_TLC) {
		...
		if (need_erase) {
			if (!nand_erase_bmt(OFFSET_MLC(bmt_block_index))) {
				MSG(INIT, "BMT block erase fail, mark bad: 0x%x\n", bmt_block_index);
...
bmt_struct *reconstruct_bmt(bmt_struct * bmt)
{
	...
	for (i = 0; i < bmt_block_count; i++, index++) {
		if (nand_block_bad_bmt(OFFSET_MLC(index))) {
		...
		if (!nand_block_bad_bmt(OFFSET_MLC(bad_index))) {
...
bmt_struct *init_bmt(struct nand_chip * chip, int size)
{
	...
	if (devinfo.NAND_FLASH_TYPE == NAND_FLASH_TLC) {
		...
		system_block_count = (u32)(chip->chipsize /
		                           (chip->sector_size == 512 ? chip->erasesize : chip->erasesize*2));
		page_per_block = (chip->sector_size == 512?chip->erasesize : chip->erasesize*2)
		                 / chip->page_size;
	}
...
bool update_bmt(u64 offset, update_reason_t reason, u8 * dat, u8 * oob)
{
	...
	if (devinfo.NAND_FLASH_TYPE == NAND_FLASH_TLC)
		bad_index = (u32)(offset / (devinfo.blocksize * 1024));
	else
		bad_index = (u32)(offset / BLOCK_SIZE_BMT);
