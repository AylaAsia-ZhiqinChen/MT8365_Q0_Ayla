...
void lk_msdc_irq_handler(unsigned int irq)
{
    ...
    MSDC_TRC_PRINT(MSDC_INIT,("[lk_msdc_irq_handler]irq %x enable:%x %x\n",irq,intrs,MSDC_READ32(MSDC_INT)));
    if((sts = MSDC_READ32(MSDC_INT))& intrs){
        g_int_status = sts;
        MSDC_TRC_PRINT(MSDC_INIT,("[lk_msdc_irq_handler]send event,%x\n",g_int_status));
...
static void msdc_irq_handler(void *data)
{
    ...
    if (intsts & MSDC_INT_CDSC) {
        /* card detection */
        in_iqr++;
        MSDC_TRC_PRINT(MSDC_IRQ,("in_irq(%d) intsts(0x%x)",in_iqr,intsts));
        MSDC_TRC_PRINT(MSDC_IRQ,("\n[SD%d] Card %s\n", id, msdc_card_avail(host) ? "Inserted" : "Removed"));
    }

    if (intsts & MSDC_INT_SDIOIRQ) {
        /* sdio bus interrupt */
        MSDC_TRC_PRINT(MSDC_IRQ,("[SD%d] SDIO interrupt <===\n", id));
    ...
    MSDC_TRC_PRINT(MSDC_IRQ,("[SD%d] IRQ_EVT(0x%x): MMCIRQ(%d) CDSC(%d), ACRDY(%d), ACTMO(%d), ACCRE(%d) AC19DN(%d)\n",
        id,
        intsts,
        int_reg->mmcirq,
        int_reg->cdsc,
    ...
    MSDC_TRC_PRINT(MSDC_IRQ,("[SD%d] IRQ_EVT(0x%x): SDIO(%d) CMDRDY(%d), CMDTMO(%d), RSPCRC(%d), CSTA(%d)\n",
        id,
        intsts,
        int_reg->sdioirq,
        int_reg->cmdrdy,
    ...
    MSDC_TRC_PRINT(MSDC_IRQ,("[SD%d] IRQ_EVT(0x%x): XFCMP(%d) DXDONE(%d), DATTMO(%d), DATCRC(%d), DMAEMP(%d)\n",
        id,
        intsts,
        int_reg->xfercomp,
        int_reg->dxferdone,
...
u32 msdc_lk_intr_wait(struct mmc_host *host, u32 intrs)
{
    ...
    MSDC_TRC_PRINT(MSDC_IRQ,("[msdc_lk_intr_wait]\n"));
    ...
    if(ret != 0) {
        MSDC_ERR_PRINT(MSDC_ERROR,("[%s]: failed to get event\n", __func__));
    } else {
        MSDC_TRC_PRINT(MSDC_IRQ,("[%s]: get event\n", __func__));
    }
    sts = g_int_status;
    g_int_status = 0;
    MSDC_TRC_PRINT(MSDC_IRQ,("[SD%d] INT(0x%x)\n", host->id, sts));
    if (~intrs & sts) {
        MSDC_TRC_PRINT(MSDC_WARN,("[SD%d]<CHECKME> Unexpected INT(0x%x)\n",
            host->id, ~intrs & sts));
    }
    return sts;
}
...
u32 msdc_intr_wait(struct mmc_host *host, u32 intrs)
{
...
#else
    {
        ...
        if (tmo == 0) {
            MSDC_ERR_PRINT(MSDC_ERROR,("[SD%d] ECO WARNNING ==> Wait INT timeout\n", host->id));
            MSDC_RESET();
        }
    }
    ...
    if (sts & MSDC_INT_SDIOIRQ) {
        ...
        MSDC_TRC_PRINT(MSDC_IRQ,"(%s)INT status:0x%x\n", __func__, sts));
#endif
