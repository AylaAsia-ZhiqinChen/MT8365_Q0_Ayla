...
static S32 _pwrap_init_dio( U32 dio_en )
{
	...
	if ( rdata != MT6392_DEFAULT_VALUE_READ_TEST ) {
...
static S32 _pwrap_init_sistrobe( void )
{
	...
	for ( ind=0; ind<24; ind++) { // 24 sampling clock edge
		...
		_pwrap_wacs2_nochk(0, MT6392_DEW_READ_TEST, 0, &rdata);
		if ( rdata == MT6392_DEFAULT_VALUE_READ_TEST ) {
			PWRAPLOG("_pwrap_init_sistrobe [Read Test of MT6392] pass,index=%d rdata=%x\n", ind,rdata);
			result[0] |= (0x1 << ind);
		} else {
			PWRAPLOG("_pwrap_init_sistrobe [Read Test of MT6392] tuning,index=%d rdata=%x\n", ind,rdata);
		}
	}
...
static S32 _pwrap_init_signature( U8 path )
{
	...
	if (path == 1) {
		...
		//###############################
		_pwrap_wacs2_nochk(1, MT6392_DEW_WRITE_TEST, 0x5678, &rdata);
		WRAP_WR32(PMIC_WRAP_SIG_ADR,MT6392_DEW_WRITE_TEST);
		...
		ret = pwrap_write_nochk(MT6392_DEW_CRC_EN, ENABLE);
		if ( ret != 0 ) {
			PWRAPERR("enable CRC fail,ret=%x\n", ret);
			return E_PWR_INIT_ENABLE_CRC;
		}
		WRAP_WR32(PMIC_WRAP_SIG_ADR, WRAP_RD32(PMIC_WRAP_SIG_ADR)|MT6392_DEW_CRC_VAL);
...
S32 pwrap_init ( void )
{
	...
	WRAP_WR32(MODULE_SW_CG_1_SET, cg_mask);
	...
	WRAP_WR32(MODULE_SW_CG_1_CLR, cg_mask);

	// Turn on module clock dcm (in global_con)
	WRAP_WR32(MODULE_SW_CG_3_SET, WRAP_RD32(MODULE_SW_CG_3_SET) | PMIC_B_DCM_EN | PMIC_SPI_DCM_EN);
...
//check Wtiet test default value
	sub_return = pwrap_write_nochk(MT6392_DEW_WRITE_TEST, MT6392_WRITE_TEST_VALUE);
	sub_return1 = pwrap_read_nochk(MT6392_DEW_WRITE_TEST, &rdata);
	if ( rdata != MT6392_WRITE_TEST_VALUE )  {
	...
	//###############################
	WRAP_WR32(PMIC_WRAP_ADC_CMD_ADDR    , MT6392_AUXADC_CON21);
	WRAP_WR32(PMIC_WRAP_PWRAP_ADC_CMD   , 0x8000);
	WRAP_WR32(PMIC_WRAP_ADC_RDY_ADDR    , MT6392_AUXADC_ADC12);
	WRAP_WR32(PMIC_WRAP_ADC_RDATA_ADDR1 , MT6392_AUXADC_ADC13);
	WRAP_WR32(PMIC_WRAP_ADC_RDATA_ADDR2 , MT6392_AUXADC_ADC14);
...
static S32 _pwrap_status_update_test_porting( void )
{
	...
	pwrap_write(MT6392_DEW_WRITE_TEST, MT6392_WRITE_TEST_VALUE);
	WRAP_WR32(PMIC_WRAP_SIG_ADR, MT6392_DEW_WRITE_TEST);
	...
	if ( (rdata&0xFFFF) != MT6392_WRITE_TEST_VALUE ) {
		PWRAPERR("MT6392 _pwrap_status_update_test error,error code=%x, rdata=%x\n", 1, (rdata&0xFFFF));
		//return 1;
	}

	WRAP_WR32(PMIC_WRAP_SIG_VALUE, MT6392_WRITE_TEST_VALUE);//tha same as write test
	...
	WRAP_WR32(PMIC_WRAP_SIG_ADR, MT6392_DEW_CRC_VAL);
