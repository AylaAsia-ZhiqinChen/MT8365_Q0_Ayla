...
#define WHEREDOESMSDCRUN           "MSDC@LK" 
...
#define MSDC_DBG_PRINT(cond,printf_exp)   \
   ((void)((cond) ? printf("%s:%s ",MSDC_FILTER_NAME(cond),WHEREDOESMSDCRUN),\
   				  ( printf printf_exp),1:0))
   				  
#define MSDC_DBGCHK(friendname,exp) \
   ((void)((exp)?1:(          \
       printf ( "%s: DBGCHK failed in file %s at line %d \r\n", \
...
#define MSDC_ASSERT(exp) \
   ((void)((exp)?1:(          \
       printf( "%s: MUSTBE: failed in file %s at line %d \r\n", \
...
#define MSDC_TRC_PRINT(cond,printf_exp)   \
   ((cond)? printf("%s:%s ",MSDC_FILTER_NAME(cond),WHEREDOESMSDCRUN),\
           (printf printf_exp),1:0)


#define MSDC_ERR_PRINT(cond,printf_exp)	 \
   ((cond)?( printf("%s:%s : ",MSDC_FILTER_NAME(cond),WHEREDOESMSDCRUN),\
              printf("***ERROR: %s line %d: ",__FILE__,__LINE__), printf printf_exp),1:0)
