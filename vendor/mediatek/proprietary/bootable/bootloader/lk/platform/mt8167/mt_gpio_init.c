...
void mt_gpio_set_power(void)
{
	...
	/* Set  set=0x5 rst=0xa*/
	GPIO_WR32(&pReg->bias_ctrl0.set, 0x5 << 0);
	GPIO_WR32(&pReg->bias_ctrl0.rst, 0xC << 0);

	/* Set  set=0x5 rst=0xa*/
	GPIO_WR32(&pReg->bias_ctrl0.set, 0x5 << 4);
	GPIO_WR32(&pReg->bias_ctrl0.rst, 0xC << 4);

	/* Set  set=0x5 rst=0xa*/
	GPIO_WR32(&pReg->bias_ctrl0.set, 0x5 << 8);
	GPIO_WR32(&pReg->bias_ctrl0.rst, 0xC << 8);

	/* Set  set=0x5 rst=0xa*/
	GPIO_WR32(&pReg->bias_ctrl0.set, 0x5 << 12);
	GPIO_WR32(&pReg->bias_ctrl0.rst, 0xC << 12);

	/* Set  set=0x5 rst=0xa*/
	GPIO_WR32(&pReg->bias_ctrl1.set, 0x5 << 0);
	GPIO_WR32(&pReg->bias_ctrl1.rst, 0xC << 0);

	/* Set  set=0x5 rst=0xa*/
	GPIO_WR32(&pReg->bias_ctrl1.set, 0x5 << 4);
	GPIO_WR32(&pReg->bias_ctrl1.rst, 0xC << 4);

	/* Set  set=0x5 rst=0xa*/
	GPIO_WR32(&pReg->bias_ctrl1.set, 0x5 << 8);
	GPIO_WR32(&pReg->bias_ctrl1.rst, 0xC << 8);

	/* Set  set=0x5 rst=0xa*/
	GPIO_WR32(&pReg->bias_ctrl1.set, 0x5 << 12);
	GPIO_WR32(&pReg->bias_ctrl1.rst, 0xC << 12);

	/* Set  set=0x5 rst=0xa*/
	GPIO_WR32(&pReg->bias_ctrl2.set, 0x5 << 0);
	GPIO_WR32(&pReg->bias_ctrl2.rst, 0xC << 0);

	/* Set  set=0x5 rst=0xa*/
	GPIO_WR32(&pReg->bias_ctrl2.set, 0x5 << 4);
	GPIO_WR32(&pReg->bias_ctrl2.rst, 0xC << 4);

	/* Set  set=0x5 rst=0xa*/
	GPIO_WR32(&pReg->bias_ctrl2.set, 0x5 << 8);
	GPIO_WR32(&pReg->bias_ctrl2.rst, 0xC << 8);

	/* Set  set=0x5 rst=0xa*/
	GPIO_WR32(&pReg->bias_ctrl2.set, 0x5 << 12);
	GPIO_WR32(&pReg->bias_ctrl2.rst, 0xC << 12);
}

void mt_gpio_set_rdsel(u8 dvdd_dpi,u8 dvdd_nfi, u8 dvdd_mds0, u8 dvdd_mds1, u8 dvdd_mds2)
{
	u32 reg = 0;
	GPIO_REGS *pReg = (GPIO_REGS*)(GPIO_BASE);

	if (dvdd_dpi == GPIO_VIO28) {
		reg = GPIO_RD32(&pReg->rdsel[0]);
		reg &= ~(0x3f);
		reg |= 0xc;
		GPIO_WR32(&pReg->rdsel[0].set, reg);
		...
		reg = GPIO_RD32(&pReg->rdsel[1]);
		reg &= ~(0x3f);
		reg |= (0xc);
		GPIO_WR32(&pReg->rdsel[1].set, reg);

		reg = GPIO_RD32(&pReg->rdsel[2]);
		reg &= ~(0x3f);
		reg |= 0xc;
		GPIO_WR32(&pReg->rdsel[2].set, reg);
	...
	if (dvdd_mds2 == GPIO_VIO28) {
		reg = GPIO_RD32(&pReg->rdsel[3]);
		reg &= ~(0x3f);
		reg |= 0xc;
		GPIO_WR32(&pReg->rdsel[3].set, reg);
		...
		reg = GPIO_RD32(&pReg->rdsel[4]);
		reg &= ~(0x3f);
		reg |= (0xc);
		GPIO_WR32(&pReg->rdsel[4].set, reg);
	} else {
		reg = GPIO_RD32(&pReg->rdsel[3]);
		reg &= ~(0x3f);
		GPIO_WR32(&pReg->rdsel[3].set, reg);
	...
	if (dvdd_mds1 == GPIO_VIO28) {
		reg = GPIO_RD32(&pReg->rdsel[4]);
		reg &= ~(0x3f << 6);
		reg |= (0xc << 6);
		GPIO_WR32(&pReg->rdsel[4].set, reg);

		reg = GPIO_RD32(&pReg->rdsel[5]);
		reg &= ~(0x3f);
		reg |= (0xc);
		GPIO_WR32(&pReg->rdsel[5].set, reg);
	...
	if (dvdd_mds0 == GPIO_VIO28) {
		reg = GPIO_RD32(&pReg->rdsel[6]);
		reg &= ~(0x3f);
		reg |= (0xc);
		GPIO_WR32(&pReg->rdsel[6].set, reg);
		...
		reg = GPIO_RD32(&pReg->rdsel[7]);
		reg &= ~(0x3f);
		reg |= (0xc);
		GPIO_WR32(&pReg->rdsel[7].set, reg);
...
void mt_gpio_set_default(void)
{
	...
	mt_gpio_set_power();
	mt_gpio_set_rdsel(GPIO_DVDD28_DPI,GPIO_DVDD28_NFI,GPIO_DVDD28_MSDC0,GPIO_DVDD28_MSDC1,GPIO_DVDD28_MSDC2);
	return;
}
/*----------------------------------------------------------------------------*/
