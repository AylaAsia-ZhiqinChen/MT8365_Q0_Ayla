	...
	if (err == MMC_ERR_NONE) {
		...
		if (part_config == 1) {
			...
			*bootpart = EMMC_PART_USER;
			...
			MSDC_ERR_PRINT(MSDC_ERROR,("[LK MTK] fail to get boot part\n"));
			return -1;
		}
	}
...
int mmc_set_boot_part(u32 bootpart)
{
	...
	if (((u32)cur_bootpart) == bootpart)
		goto out;
	MSDC_ERR_PRINT(MSDC_ERROR,("[SD0] Current boot part is %d\n", cur_bootpart));

	part_config = part_config & (~(0x7 << 3)) | (bootpart << 3);
	MSDC_ERR_PRINT(MSDC_ERROR,("[SD0] Set boot part as %d\n", bootpart));
