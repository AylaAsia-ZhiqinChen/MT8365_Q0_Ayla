...
int mmc_send_tuning(struct mmc_host *host, int *cmd_error)
{
	...
	err = msdc_dma_transfer(host, &cmd, &data);
	if (err)
		goto out;
	else {
...
#else
    printf("LK emmc ocr = 0x%x\n", g_boot_arg->emmc_ocr);
    printf("LK emmc cid: 0x%x, 0x%x, 0x%x, 0x%x\n",
		    g_boot_arg->emmc_cid[0],
		    g_boot_arg->emmc_cid[1],
		    g_boot_arg->emmc_cid[2],
		    g_boot_arg->emmc_cid[3]);
    printf("LK emmc csd: 0x%x, 0x%x, 0x%x, 0x%x\n",
		    g_boot_arg->emmc_raw_csd[0],
		    g_boot_arg->emmc_raw_csd[1],
		    g_boot_arg->emmc_raw_csd[2],
		    g_boot_arg->emmc_raw_csd[3]);
    card->ocr = g_boot_arg->emmc_ocr;
    card->state |= ((card->ocr >> 30) & 0x1) ? MMC_STATE_HIGHCAPS : 0;
    memcpy(card->raw_cid, g_boot_arg->emmc_cid, sizeof(card->raw_cid));
    mmc_decode_cid(card);
    card->rca = 0x1;
    memcpy(card->raw_csd, g_boot_arg->emmc_raw_csd, sizeof(card->raw_csd));
    mmc_decode_csd(card);
#endif
