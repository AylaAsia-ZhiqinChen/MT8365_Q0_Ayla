...
u64 OFFSET(u32 block) //return logical address
{
	...
	if (devinfo.NAND_FLASH_TYPE != NAND_FLASH_TLC)
		printf("not TLC for OFFSET function\n");
...
void mtk_slc_blk_addr(u64 addr, u32* blk_num, u32* page_in_block)
{
	...
	if (devinfo.NAND_FLASH_TYPE == NAND_FLASH_TLC) {
		if (raw_partition(idx)) { // raw partition just has SLC mode block
			*blk_num = (u32)((u32)(start_address / block_size) + (u32)((addr-start_address) / (block_size / slc_div)));
			*page_in_block = ((u32)((addr-start_address) / devinfo.pagesize) % ((block_size/devinfo.pagesize)/slc_div));
			*page_in_block *= slc_div;
		} else {
			if ((addr < lastest_part[idx + 1].offset) &&
			        (addr >= (lastest_part[idx + 1].offset - PMT_POOL_SIZE * block_size))) {
				//for pmt update
				*blk_num = (u32)(addr / block_size);
				*page_in_block = ((u32)(addr / devinfo.pagesize) % ((block_size/devinfo.pagesize)/slc_div));
				*page_in_block *= slc_div;
			} else {
				total_blk_num = (lastest_part[idx + 1].offset - lastest_part[idx].offset) / (devinfo.blocksize * 1024);
				if (!strcmp(lastest_part[idx].name,"ANDROID"))
				...
				if (slc_blk_num % slc_div)
					slc_blk_num += (slc_div - (slc_blk_num % slc_div)); // slc block number must be 3 aligned

				offset = start_address + (u64)(devinfo.blocksize * 1024) * (total_blk_num - slc_blk_num);

				if (offset <= addr) {
					*blk_num = (u32)((u32)(offset / block_size) + (u32)((addr-offset) / (block_size / slc_div)));
					*page_in_block = ((u32)((addr-offset) / devinfo.pagesize) % ((block_size/devinfo.pagesize)/slc_div));
					*page_in_block *= slc_div;
				} else {
