...
extern int mboot_common_load_part(char *part_name, unsigned long addr);	// !@#
...
int mt_load_vpu(void)
{
	...
	*pIramNum = 0;
	...
	for (i = 0 ; i < (sizeof(vpu_part_name)/sizeof(*vpu_part_name)) ; i++) {
		...
		if ((len = mboot_common_load_part(part_name, (unsigned long)part)) < 0) {	// !@#
	...
	nodeoffset = fdt_node_offset_by_compatible(g_fdt, -1, "mediatek,mt8168-vpu");  // get vpu node offset
