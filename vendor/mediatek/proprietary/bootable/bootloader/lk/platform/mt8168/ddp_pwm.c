...
#define CLK_CFG_UPATE	0x10000004
#endif

#ifndef CLK_CFG_5_SET
#define CLK_CFG_5_SET 0x10000094
#endif

#ifndef CLK_CFG_5_CLR
#define CLK_CFG_5_CLR 0x10000098
#endif
...
void disp_pwm_init(disp_pwm_id_t id)
{
	...
	if (cust_led_list) {
		...
		if ((strcmp(cust->name,"lcd-backlight") == 0) && (cust->mode == MT65XX_LED_MODE_CUST_BLS_PWM)) {
			...
			if (config_data->clock_source >= 0 && config_data->clock_source <= 3) {
				unsigned int reg_val = PWM_REG_GET(CLK_CFG_5);
				PWM_REG_SET(CLK_CFG_5_CLR, 0x30000);
				PWM_REG_SET(CLK_CFG_5_SET, (0x30000 & (config_data->clock_source << 16)));
				PWM_DBG("disp_pwm_init : CLK_CFG_5 0x%x => 0x%x", reg_val, PWM_REG_GET(CLK_CFG_5));
				PWM_REG_SET(CLK_CFG_UPATE, 1 << 22);
			}
	...
	PWM_REG_SET(reg_base + DISP_PWM_DEBUG, 3 << 0);
