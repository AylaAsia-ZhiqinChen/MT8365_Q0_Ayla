...
DISP_EXTERNAL_PATH_MODE external_display_mode = EXT_SINGLE_LAYER_MODE;// EXT_DECOUPLE_MODE;//EXT_DIRECT_LINK_MODE;
...
static int _build_path_direct_link(void)
{
	...
	pgc->dpmgr_handle = dpmgr_create_path(DDP_SCENARIO_SECONDARY_RDMA1_DISP, pgc->cmdq_handle_config);
	if (pgc->dpmgr_handle) {
...
static int _build_path_single_layer(void)
{
	...
	DISP_MODULE_ENUM dst_module = 0;
	DISPFUNC();
	pgc->mode = EXT_SINGLE_LAYER_MODE;

	dprintf(CRITICAL, "%s\n", __func__);
	pgc->dpmgr_handle = dpmgr_create_path_ext(DDP_SCENARIO_SECONDARY_RDMA1_DISP, pgc->cmdq_handle_config);
	if (pgc->dpmgr_handle) {
		DISPCHECK("dpmgr create path SUCCESS(0x%08x)\n", pgc->dpmgr_handle);
	} else {
		DISPCHECK("dpmgr create path FAIL\n");
	...
	dpmgr_path_set_dst_module(pgc->dpmgr_handle, dst_module);
	DISPCHECK("dpmgr set dst module FINISHED(%s)\n", ddp_get_module_name(dst_module));
	dprintf(CRITICAL, "%s dpmgr set dst module FINISHED(%s)\n", __func__, ddp_get_module_name(dst_module));
...
static int _convert_disp_input_to_rdma(RDMA_CONFIG_STRUCT *dst, disp_input_config* src)
{
	if (src && dst) {
		...
		dprintf(CRITICAL, "%s ext w=%d h=%d pitch=%d addr=0x%x\n", __func__, src->src_w, src->src_h, src->src_pitch, src->addr);
		return 0;
	} else {
...
int external_display_init(char *lcm_name)
{
	...
	if (pgc->plcm == NULL) {
		dprintf(CRITICAL, "%s disp_ext_lcm_probe returns null\n", __func__);
	...
	dprintf(CRITICAL, "%s external_display_mode=%d\n", __func__, external_display_mode);
	...
	dprintf(CRITICAL, "%s lcm_param->type=%d w=%d h=%d\n", __func__, lcm_param->type, data_config.dst_w, data_config.dst_h);
...
#endif
		DISPMSG("^^ EXT_DISP_GetVRamSize: %u bytes\n", vramSize);
	}
