...
#if (MTK_DUAL_DISPLAY_SUPPORT == 2)
void init_fb_ext_screen()
{
    dprintf(0, "[ext lk logo: %s %d]\n",__FUNCTION__,__LINE__);
    unsigned int fb1_size = mt_get_ext_fb_size();
    //logo_addr = mt_get_ext_logo_db_addr();
    logo_addr = mt_get_logo_db_addr();
    phical_screen.width = CFG_EXT_DISPLAY_WIDTH;
    phical_screen.height = CFG_EXT_DISPLAY_HEIGHT;
    dprintf(0, "[ext lk logo: %s %d %d %d]\n",__FUNCTION__,__LINE__, CFG_EXT_DISPLAY_WIDTH, CFG_EXT_DISPLAY_HEIGHT);
    phical_screen.fb_size = fb1_size;
    phical_screen.fill_dst_bits = CFG_EXT_DISPLAY_BPP;
    phical_screen.bits_per_pixel = CFG_EXT_DISPLAY_BPP;
    // in JB2.MP need to allign width and height to 32 ,but jb5.mp needn't
    phical_screen.needAllign = 1;
    phical_screen.allignWidth = ALIGN_TO(CFG_EXT_DISPLAY_WIDTH, MTK_FB_ALIGNMENT);
    /* In GB, no need to adjust 180 showing logo ,for fb driver dealing the change */
    /* but in JB, need adjust it for screen 180 roration           */
    phical_screen.need180Adjust = 1;   // need sync with chip driver
    phical_screen.rotation = 0;
    ...
    if (show_animationm_ver == 1)
    {
        ...
        unsigned int *pinfo = (unsigned int*)db_addr;
        logonum = pinfo[0];
        dprintf(0, "[ext lk logo: %s %d]pinfo[0]=0x%08x, pinfo[1]=0x%08x, pinfo[2]=%d\n", __FUNCTION__,__LINE__,
        pinfo[0], pinfo[1], pinfo[2]);
        dprintf(0, "[ext lk logo: %s %d]define ANIMATION_NEW:show new animation with capacity num\n",__FUNCTION__,__LINE__);
        dprintf(0, "[ext lk logo: %s %d]CAPACITY_LEFT =%d, CAPACITY_TOP =%d \n",__FUNCTION__,__LINE__,(CAPACITY_LEFT) ,(CAPACITY_TOP) );
        dprintf(0, "[ext lk logo: %s %d]LCM_HEIGHT=%d, LCM_WIDTH=%d \n",__FUNCTION__,__LINE__,(CAPACITY_RIGHT),(CAPACITY_BOTTOM));
        if(logonum < 6)
        {
            show_animationm_ver = 0 ;
        }
...
void mt_disp_show_boot_logo_by_index(int logo_index)
{
    dprintf(0, "[lk logo by index: %s %d] logo_index %d\n",__FUNCTION__,__LINE__,logo_index);
    ...
    else
    {
        ...
        init_fb_screen();
        phical_screen.rotation = 0;
        fill_animation_logo(logo_index, mt_get_fb_addr(), (void *)mt_get_tempfb_addr(), logo_addr, phical_screen);
        mt_disp_update(0, 0, CFG_DISPLAY_WIDTH, CFG_DISPLAY_HEIGHT);
    }
    return;
}

void mt_disp_show_boot_logo_ext(void)
{
    dprintf(0, "[lk logo ext: %s %d]\n",__FUNCTION__,__LINE__);
    mt_logo_get_custom_if();
    if(logo_cust_if->show_boot_logo)
    {
        logo_cust_if->show_boot_logo();
    ...
    else
    {
        ...
        init_fb_ext_screen();
        phical_screen.rotation = 0;
        fill_animation_logo(BOOT_LOGO_EXT_INDEX, mt_get_ext_fb_addr(), (void *)mt_get_ext_tempfb_addr(), logo_addr, phical_screen);
        mt_disp_update(0, 0, CFG_DISPLAY_WIDTH, CFG_DISPLAY_HEIGHT);
    }
    return;
}
...
#if 0
void mt_disp_show_low_battery_ext(void)
{
    dprintf(0, "[lk logo ext: %s %d]\n",__FUNCTION__,__LINE__);
    mt_logo_get_custom_if();
    if(logo_cust_if->show_boot_logo)
    ...
    else
    {
        init_fb1_screen();
        //show_logo(2);
        fill_animation_logo(LOW_BATTERY_INDEX, mt_get_ext_fb_addr(), (void *)mt_get_ext_tempfb_addr(), logo_addr, phical_screen);
        mt_disp_update(0, 0, CFG_DISPLAY_WIDTH, CFG_DISPLAY_HEIGHT);
    }
    return;
}
