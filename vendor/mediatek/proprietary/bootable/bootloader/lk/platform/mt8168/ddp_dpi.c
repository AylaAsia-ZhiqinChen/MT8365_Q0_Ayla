...
DPI_STATUS ddp_dpi_EnableColorBar(DISP_MODULE_ENUM module)
{
	...
	DPI_OUTREG32(0, DPI0_BASE + 0xF00, 0x41);
...
int ddp_dpi_init(DISP_MODULE_ENUM module, void *cmdq)
{
	...
	disp_register_module_irq_callback(DISP_MODULE_DPI, _DPI_InterruptHandler);
...
void LVDS_PLL_Init(DISP_MODULE_ENUM module, void *cmdq_handle, UINT32 PLL_CLK)
{
	...
	OUTREG32(APMIXED_BASE + 0x374, (1 << 4) | (1 << 0));	// LVDSPLL_CON0
	OUTREG32(APMIXED_BASE + 0x378, pcw | (postdiv << 24) | (1 << 31));	// LVDSPLL_CON1
	MASKREG32(MMSYS_CONFIG_BASE + 0xfdc, (7 << 12), (ck_div << 12));
}

void LVDS_ANA_Init(DISP_MODULE_ENUM module, cmdqRecHandle cmdq)
{
	DPI_OUTREG32(cmdq, MIPI_TX_CONFIG_BASE + 0xc, 0x100);
	DPI_OUTREG32(cmdq, MIPI_TX_CONFIG_BASE + 0x134, 0);
	DPI_OUTREG32(cmdq, MIPI_TX_CONFIG_BASE + 0x130, 0);
	DPI_OUTREG32(cmdq, MIPI_TX_CONFIG_BASE + 0x234, 0);
	DPI_OUTREG32(cmdq, MIPI_TX_CONFIG_BASE + 0x230, 0);
	DPI_OUTREG32(cmdq, MIPI_TX_CONFIG_BASE + 0x334, 0);
	DPI_OUTREG32(cmdq, MIPI_TX_CONFIG_BASE + 0x330, 0);
	DPI_OUTREG32(cmdq, MIPI_TX_CONFIG_BASE + 0x434, 0);
	DPI_OUTREG32(cmdq, MIPI_TX_CONFIG_BASE + 0x430, 0);
	DPI_OUTREG32(cmdq, MIPI_TX_CONFIG_BASE + 0x534, 0);
	DPI_OUTREG32(cmdq, MIPI_TX_CONFIG_BASE + 0x530, 0);

	DPI_OUTREG32(cmdq, LVDS_ANA_BASE + 0x14, 0x00008043);
	DPI_OUTREG32(cmdq, LVDS_ANA_BASE + 0x18, 5 << 28);
	DPI_OUTREG32(cmdq, LVDS_ANA_BASE + 0x0c, 1 << 21);
	DPI_OUTREG32(cmdq, LVDS_ANA_BASE + 0x10, 0x3c | (1 << 8));
	DPI_OUTREG32(cmdq, LVDS_ANA_BASE + 0x04,
		0x00020f87 | (1 << 22) | (1 << 23));

	DPI_OUTREG32(cmdq, LVDS_ANA_BASE + 0x1c, 0x48e2);
	DPI_OUTREG32(cmdq, LVDS_ANA_BASE + 0x08,
		0x07df | (1 << 23) | (1 << 25));

	DPI_OUTREG32(cmdq, LVDS_ANA_BASE + 0x14, 0x00008041 | (1 << 2));
	DPI_OUTREG32(cmdq, LVDS_ANA_BASE + 0x1c, 0x48e2 | (1 << 13));
	lcm_udelay(20);
	DPI_OUTREG32(cmdq, LVDS_ANA_BASE + 0x1c, 0x8e2 | (1 << 13));
	DPI_OUTREG32(cmdq, LVDS_ANA_BASE + 0x14, 0x00008043 | (1 << 2));
...
void LVDS_DIG_RST(DISP_MODULE_ENUM module, cmdqRecHandle cmdq)
{
	DPI_OUTREG32(cmdq_handle, LVDS_TX_BASE + 0x34, 0x00000000);
	DPI_OUTREG32(cmdq_handle, LVDS_TX_BASE + 0x34, 0x00000003);
}

void LVDS_DIG_Init(DISP_MODULE_ENUM module, cmdqRecHandle cmdq)
{
	DPI_OUTREG32(cmdq,LVDS_TX_BASE + 0x18, 0x00000001);
	DPI_OUTREG32(cmdq,LVDS_TX_BASE + 0x20, 0x00000007);
	LVDS_DIG_RST(module, cmdq);
...
void ddp_dpi_lvds_config(DISP_MODULE_ENUM module, LCM_DPI_FORMAT format, void *cmdq_handle)
{
	...
	if (format == LCM_DPI_FORMAT_RGB666)
		DPI_OUTREG32(cmdq_handle, LVDS_TX_BASE, 0x00000010);
...
int ddp_dpi_config(DISP_MODULE_ENUM module, disp_ddp_path_config *config, void *cmdq_handle)
{
	if (s_isDpiConfig == FALSE) {
		...
		DISP_LOG_PRINT(ANDROID_LOG_WARN, "DPI", "[LK]ddp_dpi_config DPI DPI_w=%d, DPI_h=%d,  DPI_clock=%d\n\n",
		               dpi_config->width,dpi_config->height,dpi_config->PLL_CLOCK);

		if (module == DISP_MODULE_DPI) {
			LVDS_PLL_Init(module, NULL, dpi_config->PLL_CLOCK);
			if (dpi_config->lvds_tx_en) {
				ddp_dpi_lvds_config(module, dpi_config->format, NULL);
				DPI_OUTREG32(cmdq_handle, DISPSYS_CONFIG_BASE + 0xfdc, 0x00030000);
			 } else {
		...
		if (module == DISP_MODULE_DPI) {
			...
			if (dpi_config->lvds_tx_en) {
				ddp_dpi_lvds_config(module, dpi_config->format, NULL);
				DPI_OUTREGBIT(NULL, DPI_REG_OUTPUT_SETTING, DPI_REG[DPI_IDX(module)]->OUTPUT_SETTING, DUAL_EDGE_SEL, 1);
				DPI_OUTREG32(NULL, DPI0_BASE + 0x50, 1);
				DPI_OUTREG32(cmdq_handle, DISPSYS_CONFIG_BASE + 0xfdc, 0x00030000);
			} else {
				ddp_dpi_RGB_set_DrivingCurrent(module, cmdq_handle, dpi_config->io_driving_current);
			}
		...
		//DPI_OUTREG32(cmdq_handle, DPI0_BASE + 0xf00, 0x00000041);
		//DPI_OUTREG32(cmdq_handle, DPI0_BASE, 0x00000001);

		s_isDpiConfig = TRUE;
		DISP_LOG_PRINT(ANDROID_LOG_WARN, "DPI", "ddp_dpi_config done\n");
	}
	DISPCHECK("ddp_dpi_config finished\n");
