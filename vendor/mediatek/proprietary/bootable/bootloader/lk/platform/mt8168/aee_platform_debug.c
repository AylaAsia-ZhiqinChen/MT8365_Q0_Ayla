...
int dfd_set_base_addr(void *fdt)
{
	...
	dfd_buffer = mblock_reserve_ext(&g_boot_arg->mblock_info, 0x00080000, 0x10000000, 0x100000000, 0,  "DFD dump");

	addr_msb = cpu_to_fdt32(dfd_buffer);
	ret = fdt_setprop(fdt, offset, "dfd,base_addr_msb", &addr_msb, sizeof(addr_msb));
	if (ret < 0)
		return ret;

	addr = cpu_to_fdt64(dfd_buffer);
	...
	if (!plat)
		return -1;
	plat->plat_sram_flag2 = (dfd_buffer & ~(0x1)) | ((dfd_buffer >> 32) & 0x1);
