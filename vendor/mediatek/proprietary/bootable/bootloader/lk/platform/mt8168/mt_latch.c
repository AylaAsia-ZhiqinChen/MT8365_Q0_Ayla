...
int plt_lastpc_dump(const struct plt_cfg_pc_latch * self, char * buf, int * wp)
{
	...
	for (i = 0; i < self->nr_max_core; i++) {
		lastpc_core = g_boot_arg->lastpc[i];
		pc_value = ((unsigned long long)lastpc_core[1] << 32) | lastpc_core[0];
		if (g_is_64bit_kernel) {
			fp_value = ((unsigned long long)lastpc_core[5] << 32) | lastpc_core[4];
			sp_value = ((unsigned long long)lastpc_core[7] << 32) | lastpc_core[6];
		} else {
			fp_value = lastpc_core[2];
			sp_value = lastpc_core[3];
		}
