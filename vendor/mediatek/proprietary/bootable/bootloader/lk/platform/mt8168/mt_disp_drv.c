...
void mt_ext_disp_init(void *lcdbase)
{
	...
	dprintf(CRITICAL,"%s ext_fb_va: 0x%08x, ext_fb_pa: 0x%08x, ext_fb_pa_k: 0x%llx\n", __func__,
	...
	memset((void*)lcdbase, 0xA0, EXT_DISP_GetVRamSize());
...
void mt_ext_scl_d(void *s_buf, int s_w, int s_h, void *d_buf, int d_w, int d_h)
{
	int *s_addr = NULL;
	int *d_addr = NULL;
	int idx_h, idx_w;
	...
	dprintf(CRITICAL, "%s f_w=%f, f_h=%f\n", __func__, f_w, f_h);
	for(idx_h = 0; idx_h < d_h; idx_h++) {
		f_s_h = idx_h * f_h + 0.5;
		for (idx_w = 0; idx_w < d_w; idx_w++) {
			f_s_w = idx_w * f_w + 0.5;
			d_addr = (int *)((unsigned long)d_buf + idx_h * d_w * 4 + idx_w * 4);
			s_addr = (int *)((unsigned long)s_buf + f_s_h * s_w * 4 + f_s_w * 4);
			*d_addr = *s_addr;
		}
	}
}

void mt_ext_disp_update(void)
{
	unsigned int va = ext_fb_addr;

	//mt_ext_scl_d(fb_addr, CFG_DISPLAY_WIDTH, CFG_DISPLAY_HEIGHT,
	//	ext_fb_addr, CFG_EXT_DISPLAY_WIDTH, CFG_EXT_DISPLAY_HEIGHT);
	arch_clean_cache_range((unsigned int)ext_fb_addr, EXT_DISP_GetFBRamSize());
	dprintf(CRITICAL, "%s external_display_trigger\n", __func__);
