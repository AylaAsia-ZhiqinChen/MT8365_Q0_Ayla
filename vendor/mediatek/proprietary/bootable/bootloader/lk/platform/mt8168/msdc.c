...
void msdc_dump_register(struct mmc_host *host)
{
	...
	for (i = 0; i < 0x230; i += 4) {
		dprintf(ALWAYS, "[0x%x]: 0x%08x\n", i, MSDC_READ32(base + i));
	}

	for (i = 0; i < 0x30; i += 4) {
		dprintf(ALWAYS, "TOP [0x%x]: 0x%08x\n", i, MSDC_READ32(host->top_base + i));
	}
}
...
{
	/*mt8168 eMMC don't support HS400 mode*/
	return 0;
}
...
void msdc_config_clksrc(struct mmc_host *host, u32 clksrc, u32 hclksrc)
{
	...
	/*
	 * in preloader stage, mmc_config_pll() has already set MSDC0/1 source
	 * PLL to 400Mhz/200Mhz
	 */
	if (host->id == 0)
		host->src_clk = 400 * 1000 * 1000;
	else if (host->id == 1)
		host->src_clk = 200 * 1000 * 1000;
...
void msdc_config_clock(struct mmc_host *host, int state, u32 hz)
{
    ...
    dprintf(ALWAYS,
            ...
            hz/1000, sclk/1000, mode, div, msdc_cap[host->id].data_edge,
            msdc_cap[host->id].cmd_edge,
            (u4buswidth == 0) ?
            "1-bit" : (u4buswidth == 1) ?
            "4-bits" : (u4buswidth == 2) ?
            "8-bits" : "undefined");
...
int msdc_switch_volt(struct mmc_host *host, int volt)
{
	...
	if ((MSDC_READ32(MSDC_PS) & ((1 << 24) | (0xF << 16))) == 0) {
		...
		if (bus_clk > 65000000) {
			/* config clock back to init clk freq. */
			msdc_config_clock(host, 0, bus_clk);
		}
	}
...
int msdc_tune_cmdrsp(struct mmc_host *host, struct mmc_command *cmd)
{
    ...
    MSDC_TRC_PRINT(MSDC_TUNING,("[SD%d] <TUNE_CMD%d><%s>@msdc_tune_cmdrsp\n",
...
int msdc_tune_bread(struct mmc_host *host, uchar *dst, ulong src, ulong nblks)
{
	...
	MSDC_TRC_PRINT(MSDC_TUNING,("[SD%d] <msdc_tune_bread<%s><cmd%d>@msdc_tune_bread\n",
		       host->id, (result == MMC_ERR_NONE && dcrc == 0) ?"PASS" : "FAIL", (nblks == 1 ? 17 : 18)));
...
int msdc_tune_bwrite(struct mmc_host *host, ulong dst, uchar *src, ulong nblks)
{
	...
	MSDC_TRC_PRINT(MSDC_TUNING,("[SD%d] <msdc_tune_bwrite<%s><cmd%d>@msdc_tune_bwrite\n",
			host->id, (result == MMC_ERR_NONE && dcrc == 0) ?"PASS" : "FAIL", (nblks == 1 ? 17 : 18)));
...
int msdc_emmc_boot_start(struct mmc_host *host, u32 hz, int ddr, int mode, int ackdis, u8 hostbuswidth, u64 size)
{
	...
	msdc_config_clock(host, (ddr ? MMC_STATE_DDR : 0), hz);
...
int msdc_init(int id, struct mmc_host *host, int clksrc, int mode)
{
	...
	u32 top_baddr[] = {MSDC0_TOP_BASE, MSDC1_TOP_BASE, MSDC2_TOP_BASE};
	u32 base = baddr[id];
...
#ifndef FPGA_PLATFORM
	/* now only consider MSDC0 */
	host->f_max  = hclks_msdc50[clksrc];
#else
	host->f_max  = MSDC_OP_SCLK;
#endif
	...
	MSDC_SET_FIELD(MSDC_PATCH_BIT1, MSDC_PATCH_BIT1_STOP_DLY, 3);
	MSDC_CLR_BIT32(SDC_FIFO_CFG, 0x1 << 24);
	MSDC_CLR_BIT32(SDC_FIFO_CFG, 0x1 << 25);
	MSDC_SET_BIT32(EMMC50_CFG0, 0x1 << 4);
