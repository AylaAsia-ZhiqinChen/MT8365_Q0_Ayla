...
int mntl_erase(const char *arg, void *data, unsigned int sz)
{
...
BOOL mntl_flash_img(const char *arg, void *data, unsigned int sz)
{
	unsigned int chunk;
	...
	dprintf(ALWAYS, "Image size span 0x%llx, partition size 0x%llx\n",
	        (unsigned long long)sparse_header->total_blks *
	        sparse_header->blk_sz, (EndLBA - StartLBA + 1) * 4096);

	if ((unsigned long long)sparse_header->total_blks *
	        sparse_header->blk_sz > (EndLBA - StartLBA + 1) * 4096) {
	...
	for (chunk = 0; chunk < sparse_header->total_chunks; chunk++) {
		...
		chunk_data_sz =
		    (unsigned long long)(sparse_header->blk_sz & 0xFFFFFFFF);
		...
		switch (chunk_header->chunk_type) {
			case CHUNK_TYPE_RAW:
				if (chunk_header->total_sz !=
					...
					dprintf(CRITICAL,
					        "mismatch header_sz %llu total sz %llu",
					        chunk_header->total_sz,
					        (sparse_header->chunk_hdr_sz +
					         chunk_data_sz));
					return FALSE;
				...
				if (ret < 0) {
					dprintf(CRITICAL, "mntl write fail");
					return FALSE;
				}
			...
			case CHUNK_TYPE_FILL:
				dprintf(ALWAYS,
				        "%s %d:TYPE_FILL=0x%x size=%d data_sz=%d\n",
		...
		dprintf(CRITICAL, "\rWrite Data",
		        total_blocks * sparse_header->blk_sz,
		        sparse_header->total_blks * sparse_header->blk_sz);
	}

	mntl_commit(mntl);
	dprintf(CRITICAL, "%s: mntl partition %s, sz %u, StartLBA %llu\n",
	        __func__, arg, sz, StartLBA);
