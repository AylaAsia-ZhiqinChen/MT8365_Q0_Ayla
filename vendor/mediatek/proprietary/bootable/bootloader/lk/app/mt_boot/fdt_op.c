...
bool dtb_dconfig_overlay(void *target_fdt)
{
	...
	if (target_fdt == NULL) {
		pal_log_info("%s: passing target fdt with null\n");
		return TRUE;
	}
	...
	struct fdt_header *fdth = (struct fdt_header *)target_fdt;
	/* Enlarge DT buffer */
	fdth->totalsize = cpu_to_fdt32(DTB_MAX_SIZE);

	ret = fdt_open_into(target_fdt, target_fdt, DTB_MAX_SIZE);
	if (ret) {
		pal_log_err("dconfig: fdt_open_into failed\n");
		free(dconfig_dtb);
		return FALSE;
	}
	...
	void *merged_fdt = ufdt_apply_overlay(blob, DTB_MAX_SIZE, dconfig_fdth,
					      fdt_totalsize(dconfig_fdth));
	if (!merged_fdt) {
	...
	pal_log_info("dconfig: fdt merged_size: %d\n", merged_size);
	...
	memcpy(target_fdt, merged_fdt, merged_size);
	((struct fdt_header *)target_fdt)->totalsize = cpu_to_fdt32(DTB_MAX_SIZE);
