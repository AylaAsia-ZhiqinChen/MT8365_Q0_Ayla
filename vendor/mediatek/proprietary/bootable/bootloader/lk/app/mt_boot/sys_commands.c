...
#define CMDLINE_TMP_CONCAT_SIZE 100     //only for string concat, 100 bytes is enough
	...
	usum = page_sz + kernel_actual;
	if (usum < page_sz || usum < kernel_actual) {
		fastboot_fail("unsigned parameter overflow");
		goto _cmd_boot_exit;
	}
	usum += ramdisk_actual;
	if (usum < ramdisk_actual) {
		fastboot_fail("unsigned parameter overflow");
		goto _cmd_boot_exit;
	}
	if (usum > sz) {
	...
	usum = boot_hdr->ramdisk_addr + boot_hdr->ramdisk_sz;
	if (usum < boot_hdr->ramdisk_addr || usum < boot_hdr->ramdisk_sz) {
		fastboot_fail("unsigned parameter overflow");
		goto _cmd_boot_exit;
	}
	usum = boot_hdr->kernel_addr + boot_hdr->kernel_sz;
	if (usum < boot_hdr->kernel_addr || usum < boot_hdr->kernel_sz) {
		fastboot_fail("unsigned parameter overflow");
		goto _cmd_boot_exit;
	}
	...
	usum = boot_hdr->kernel_sz + boot_hdr->ramdisk_sz;
	if (usum < boot_hdr->kernel_sz || usum < boot_hdr->ramdisk_sz) {
		fastboot_fail("unsigned parameter overflow");
		goto _cmd_boot_exit;
	}
	usum += SCRATCH_ADDR;
	if (usum < SCRATCH_ADDR) {
		fastboot_fail("unsigned parameter overflow");
		goto _cmd_boot_exit;
	}
