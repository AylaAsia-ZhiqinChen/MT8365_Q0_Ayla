...
static void mtk_wdt_reset(char mode)
{
    ...
    wdt_mode_val &= (~(MTK_WDT_MODE_DUAL_MODE | MTK_WDT_MODE_IRQ |MTK_WDT_MODE_ENABLE));
...
void mtk_wdt_sw_reset (void)
{
    ...
    mtk_wdt_reset(1);/* NOTE here, paramater 1 means  bypass power key */
...
int mtk_wdt_boot_check(void)
{
    ...
    if (wdt_sta & MTK_WDT_STATUS_HWWDT_RST) {
        return RE_BOOT_BY_WDT_HW;
    } else if (wdt_sta & MTK_WDT_STATUS_SPM_THERMAL_RST) {
        return RE_BOOT_BY_SPM_THERMAL;
    } else if (wdt_sta & MTK_WDT_STATUS_SPMWDT_RST) {
        return RE_BOOT_BY_SPM;
    } else if (wdt_sta & MTK_WDT_STATUS_THERMAL_CTL_RST) {
        return RE_BOOT_BY_THERMAL_DIRECT;
    } else if (wdt_sta & (MTK_WDT_STATUS_DEBUG_RST|MTK_WDT_STATUS_SWWDT_RST|MTK_WDT_STATUS_SECURITY_RST)) {
...
void mtk_wdt_init(void)
{
	...
	if(wdt_sta & MTK_WDT_STATUS_THERMAL_CTL_RST)
	{
...
void rgu_release_rg_dram_setting(void)
{
    ...
    for (i=0;i<10;i++);
    ucDram_Register_Write(0x007c, 0xB18E22E1);
    ucDram_Register_Write(0x0610, 0x8440003C);
    ucDram_Register_Write(0x0620, 0x82C00010);
    for (i=0;i<10;i++);
    read_data= *(volatile unsigned int *)(0x10004000+(0x77<<2));
    *(volatile unsigned int *)(0x10004000+(0x77<<2))= read_data | (0x02000000);
...
int rgu_is_reserve_ddr_mode_success(void)
{
    unsigned int wdt_dbg_ctrl;
    wdt_dbg_ctrl = READ_REG(MTK_WDT_DEBUG_CTL);
    if (wdt_dbg_ctrl & MTK_DDR_RESERVE_STA) {
...
void rgu_swsys_reset(WD_SYS_RST_TYPE reset_type)
{
    if (WD_MD_RST == reset_type) {
	   ...
	   wdt_dbg_ctrl |= MTK_WDT_SWSYS_RST_MD_RST;
