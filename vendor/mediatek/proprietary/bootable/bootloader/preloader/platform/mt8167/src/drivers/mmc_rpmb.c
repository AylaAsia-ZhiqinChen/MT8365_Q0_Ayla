...
static int mac_memcmp(unsigned char *cArray1,unsigned char *cArray2, unsigned int size)
{
    int ret = 0;
    int idx;
    for(idx=0;idx<size;idx++)
        ret |= cArray1[idx] ^ cArray2[idx];
...
/*        printf("%s, rpmb_cfg->wc = %x\n", __func__, *(rpmb_cfg->wc)); */
    ...
    if (rpmb_cfg->type == RPMB_GET_WRITE_COUNTER ||
        ...
        if(0!= mac_memcmp(mac, data_frame + RPMB_MAC_BEG, RPMB_SZ_MAC))
        {
...
static int mmc_rpmb_send_command(struct mmc_card *card, u8 *data_frame, u16 blks, u16 type, u8 req_type)
{
    ...
    */
	...
	/* Clear AutoCMD23, we issue it manually */
    msdc_set_autocmd(host, MSDC_AUTOCMD23, 0);
    msdc_set_blkcount(host, blks, is_rel_write);
	msdc_set_is_rpmb(host, true);
...
int mmc_rpmb_get_wc(u32 *wc)
{
    ...
    seclib_get_random_data(nonce, RPMB_SZ_NONCE);
