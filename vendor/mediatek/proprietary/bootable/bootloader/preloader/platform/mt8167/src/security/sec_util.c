...
U32 sec_util_read_image (U8* img_name, U8 *buf, U64 offset, U32 size)
{
	...
	#else
	src = (U64)(part->startblk) * bootdev->blksz + offset;
	#endif
...
static U32 sec_util_write_image (U8* img_name, U8 *buf, U64 offset, U32 size)
{
	...
	#ifdef MTK_GPT_SCHEME_SUPPORT
    dest = (U64)(part->start_sect) * bootdev->blksz + offset;
    #else
	dest = (U64)(part->startblk) * bootdev->blksz + offset;
	#endif
