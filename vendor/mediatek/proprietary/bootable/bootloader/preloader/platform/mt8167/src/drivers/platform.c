...
void cpu_freq_boost()
{
    pmic_config_interface(0x21E, 0x60, 0x7F, 0); // set Vproc to 1.3V for cpu freq 1.3/1.5GHz
    pmic_config_interface(0x220, 0x60, 0x7F, 0); // set Vproc to 1.3V for cpu freq 1.3/1.5GHz

    if ((seclib_get_devinfo_with_index(3) & 0x7) == 0x1) {
        print("Set cpu frequency to 1.5GHz\n");
        DRV_WriteReg32(ARMPLL_CON1, 0x800e8000);  // 1508 MHz
    } else {
        print("Set cpu frequency to 1.3GHz\n");
