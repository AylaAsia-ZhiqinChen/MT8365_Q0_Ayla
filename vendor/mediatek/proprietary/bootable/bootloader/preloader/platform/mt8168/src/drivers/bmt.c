...
static bool is_page_used(u8 *dat, u8 *oob)
{
	if (2048 == PAGE_SIZE_BMT) {
		...
		return ( (oob[OOB_INDEX_OFFSET] != 0xFF) \
			|| (oob[OOB_INDEX_OFFSET + 1] != 0xFF) );
	}
}
...
static int find_available_block(bool start_from_end)
{
	...
	if ((!pool_erased) && (devinfo.NAND_FLASH_TYPE != NAND_FLASH_TLC)) {
...
static bool write_bmt_to_flash(u8 *dat, u8 *oob)
{
	...
	MSG(INIT, "Try to write BMT bmt_block_index = 0x%x\n", bmt_block_index);
	...
	if (need_erase) {
		if (!nand_erase_bmt((devinfo.NAND_FLASH_TYPE == NAND_FLASH_TLC)?\
		    (((u64)bmt_block_index) * (devinfo.blocksize * 1024)):OFFSET(bmt_block_index))) {
...
bmt_struct *init_bmt(struct nand_chip *chip, int size)
{
	...
	if (devinfo.NAND_FLASH_TYPE == NAND_FLASH_TLC) {
		...
		system_block_count = (u32)(chip->chipsize / (nand_chip_bmt->sector_size == 512?\
			nand_chip_bmt->erasesize : nand_chip_bmt->erasesize*2));
		page_per_block = (nand_chip_bmt->sector_size == 512?\
			nand_chip_bmt->erasesize : nand_chip_bmt->erasesize*2) / chip->page_size;
	}
	...
	if ((bmt_block_index = load_bmt_data(system_block_count, size))) {
		...
		if (devinfo.NAND_FLASH_TYPE != NAND_FLASH_TLC)
			return NULL;
		#ifndef __UBOOT_NAND__
		/* BMT is not re-constructed in UBOOT. */
