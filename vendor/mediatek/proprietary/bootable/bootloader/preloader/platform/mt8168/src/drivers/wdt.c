...
void mtk_wdt_init(void)
{
	...
	wdt_ctrl |= MTK_RG_LATH_EN | MTK_RG_DVFSRC_LATCH_EN;
	wdt_ctrl |= MTK_RG_MCU_LATH_EN | MTK_RG_SPM_LATH_EN;
	wdt_ctrl &= ~(MTK_RG_MCU_LATCH_SELECT | MTK_RG_SPM_LATCH_SELECT | MTK_RG_MCUPM_LATCH_SEL);
	wdt_ctrl |= MTK_RG_DRAMC_LATH_EN | MTK_RG_MCUPM_LATCH_EN;
	...
	/* enable dvfsrc_en */
	rgu_update_reg(MTK_WDT_DEBUG_CTL2, RGU_REG_CLR, MTK_RGU_EMI_DCS_EN);
	rgu_update_reg(MTK_WDT_DEBUG_CTL2, RGU_REG_SET, MTK_RGU_DVFSRC_EN);
