...
#ifdef LDO_SUPPORT
static unsigned int vdram_volts[] = {
	1100000,
	1200000,
};

static unsigned int vdram_idxs[] = {
	1, 2,
};

static const unsigned int vfe28_volts[] = {
	2500000,
	2800000,
};

static const unsigned int vfe28_idxs[] = {
	7, 10,
};
...
static struct mt6357_ldo_trim_info ldo_trim_info[] = {
	...
	mt6357_ldo_trim_decl(vfe28, votrim_1_type),
};
...
static struct mt6357_ldo_info ldo_ext_info[] = {
	...
	mt6357_ldo_decl(vfe28_volts, vfe28_idxs),
};
#endif /*--LDO_SUPPORT--*/
...
static void update_vdram_vol(void)
{
	static bool bupdate = true;

	if (bupdate) {
		if (get_PMIC_chip_version() == PMIC_MT6390_SWCID) {
			vdram_volts[0] = 1200*1000;
			vdram_volts[1] = 1370*1000;
			vdram_idxs[0] = 6;
			vdram_idxs[1] = 7;
			mt6357_regu_info[MTK_REGULATOR_VDRAM].vmpos = VMPOS_TWO;
		}
		mt6357_regu_info[MTK_REGULATOR_VDRAM].min_uV = vdram_volts[0];
		mt6357_regu_info[MTK_REGULATOR_VDRAM].max_uV = vdram_volts[1];
		bupdate = false;
	}
}
...
static int mt6357_set_voltage(unsigned char id, unsigned int volt, unsigned int max_volt) {
	...
	if (id == MTK_REGULATOR_VDRAM) {
		/* add 70 mv for Vdram 1.37V, 40 mv for Vdram 1.24V in the mt6390 */
		if (selector == 7)
			pmic_config_interface(PMIC_RG_VDRAM_VOCAL_ADDR, 7, PMIC_RG_VDRAM_VOCAL_MASK, PMIC_RG_VDRAM_VOCAL_SHIFT);
		else
			pmic_config_interface(PMIC_RG_VDRAM_VOCAL_ADDR, 4, PMIC_RG_VDRAM_VOCAL_MASK, PMIC_RG_VDRAM_VOCAL_SHIFT);
...
static unsigned int mt6357_ldo_convert_data(unsigned char id, unsigned int cnvdata, volseltran transtype)
{
	...
	if (id == MTK_REGULATOR_VFE28) {
		if (get_PMIC_chip_version() == PMIC_MT6357_SWCID) {
			if (transtype == SELTOVOL)
				return 2800000;
			else if (transtype == VOLTOSEL)
				return 0;
		}
	}
