...
struct mt_ep mt_usb_ep[NUM_ENDPOINTS + 1];
u16 serialstate;
...
/*
    device descriptor for RTC emergency dl in pl dl mode
    just change the pid
*/
#if CFG_RTC_REG_EMERGENCY_DL_SUPPORT && CFG_RTC_REG_EMERGENCY_DL_USE_PL_DL_MD_SUPPORT
static struct device_descriptor device_descriptor_rtc_emg_dl_pl_dl_md = {
    sizeof (struct device_descriptor),
    USB_DESCRIPTOR_TYPE_DEVICE,
    USB_BCD_VERSION,
    USBDL_DEVICE_CLASS,
    ...
    EP0_MAX_PACKET_SIZE,
    USBD_VENDORID,
    USBD_PRODUCTID_RTC_DL_USE_PL_DL_MD,
    USBD_BCD_DEVICE,
    STR_MANUFACTURER,
    STR_PRODUCT,
    0,
...
*/
extern kal_bool emergency_dl_flag;
#endif /* CFG_RTC_REG_EMERGENCY_DL_SUPPORT && CFG_RTC_REG_EMERGENCY_DL_USE_PL_DL_MD_SUPPORT */
...
#if CFG_RTC_REG_EMERGENCY_DL_SUPPORT && CFG_RTC_REG_EMERGENCY_DL_USE_PL_DL_MD_SU
PPORT
    if (true == emergency_dl_flag)
    {
        return &device_descriptor_rtc_emg_dl_pl_dl_md;
    }
#endif
...
static void buf_to_ep (circbuf_t * buf)
{
    ...
    if (buf->size){
        ...
        while(is_tx_ep_busy(endpoint)){
            print(" wait usb TX  ep_busy: TX_ENDPOINT== %d,RX_ENDPOINT == %d\n", TX_ENDPOINT,RX_ENDPOINT);
        }
    }/* end if buf->size */
}
...
void usbdl_poll (void)
{
    ...
    while (is_tx_ep_busy(&mt_usb_ep[TX_ENDPOINT]))
    {
        if(get_timer(start_time) > 300){
            TTY_LOG ("usbdl_flush timeout");
            break;
        }
    }
...
{
    if (usb_input_buffer.size < 512){
		TTY_LOG(" mt_usbtty_query_data_size  >>>>>>>>>>>>>>>>>>.. stg00 \n");
        if (usbdl_configured ()){
			TTY_LOG(" mt_usbtty_query_data_size	>>>>>>>>>>>>>>>>>>.. stg01 \n");
            service_interrupts ();
            ep_to_buf (&usb_input_buffer);
        }
    }
