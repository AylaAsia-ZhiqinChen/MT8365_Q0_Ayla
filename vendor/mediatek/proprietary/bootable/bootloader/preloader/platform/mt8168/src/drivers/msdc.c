...
static u32 hclks_msdc50[] = { 26000000, 400000000, 312000000, 273000000, 249600000,
                             182000000, 100000000, 156000000, 39000000};
...
void msdc_config_clksrc(struct mmc_host *host)
{
    // modify the clock
    MSDC_ASSERT(host);

#ifdef FPGA_PLATFORM
    host->src_clk = 12 * 1000 * 1000;
#else
    /* we have already configure the MSDC PLL in mmc_config_pll() */
    if (host->id == 0)
...
#else
/* FPGA platform do not have top pad register, so only switch edge */
int msdc_tune_cmdrsp(struct mmc_host *host, struct mmc_command *cmd)
{
	u32 base = host->base;
	u32 rsmpl,cur_rsmpl, orig_rsmpl;
...
int msdc_tune_bwrite(struct mmc_host *host, ulong dst, uchar *src, ulong nblks)
{
	...
	MSDC_TRC_PRINT(MSDC_TUNING,("[SD%d] <msdc_tune_bwrte<%s><cmd%d>@msdc_tune_bwrite\n",
		       host->id, (result == MMC_ERR_NONE && dcrc == 0) ?"PASS" : "FAIL", (nblks == 1 ? 17 : 18)));
...
int msdc_init(int id, struct mmc_host *host, int clksrc, int mode)
{
	...
	host->f_max  = hclks_msdc30[clksrc];
	    ...
	    MSDC_ERR_PRINT(MSDC_ERROR, "incorret host id!\n");
#else
