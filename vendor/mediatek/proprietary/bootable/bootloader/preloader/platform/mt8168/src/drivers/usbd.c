 ...
 * GLOBAL VARIABLES
 */
extern struct mt_ep mt_usb_ep[NUM_ENDPOINTS + 1];
...
int is_tx_ep_busy(struct mt_ep *endpoint)
{
	...
	if (GET_EP_DIR(endpoint->endpoint_address) == USB_DIR_IN) {
		csr = USB_READCSR32(U3D_TX1CSR0, ep_num);
		USB_LOG("%s: ep%d is TX endpoint,csr == 0x%x", __func__, ep_num,csr);
	} else {
		USB_LOG ("%s: ep%d is RX endpoint", __func__, ep_num);
	}
...
int is_tx_ep_fifo_empty(struct mt_ep *endpoint)
{
	int ep_num = GET_EP_NUM(endpoint->endpoint_address);
	u32 csr = 0;
	...
	if (GET_EP_DIR(endpoint->endpoint_address) == USB_DIR_IN) {
		csr = USB_READCSR32(U3D_TX1CSR0, ep_num);
		USB_LOG("%s: ep%d is TX endpoint ,csr = 0x%x\n ", __func__, ep_num,csr);
	} else {
		USB_LOG ("%s: ep%d is RX endpoint", __func__, ep_num);
	}
...
bool get_linestate(void)
{
	...
	print(" USB PRB0 LineState: %x", tmpReg8);
	tmpReg8 = __raw_readb (DBG_PRB4);
	print(" USB PRB4 LineState: %x", tmpReg8);
	#endif
}
