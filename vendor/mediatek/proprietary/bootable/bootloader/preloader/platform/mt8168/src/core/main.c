...
#include "bootctrl.h"
extern int get_boot_mode(const char *) __attribute__((weak));
#endif
...
{
    print("\n%s - %s MTK_WDT_NONRST_REG2: %X\n", MOD, __func__,
                                 READ_REG(MTK_WDT_NONRST_REG2));

    if (READ_REG(MTK_WDT_NONRST_REG2) & MTK_WDT_NONRST2_RPMB_PROG_KEY)
        return !(false);
...
{
    u32 tmp;
    tmp = READ_REG(MTK_WDT_NONRST_REG2) & (~MTK_WDT_NONRST2_RPMB_PROG_KEY);
...
#ifdef MTK_EMMC_RPMB_SUPPORT
    /* for emmc rpmb support */
    do {
        int i;
        struct mmc_card *card;
        unsigned char cid_int32_be[16], *cid;
        card = mmc_get_card(0);

        cid = card->raw_cid;
        for (i = 0; i < 16; i++)
            cid_int32_be[i] = cid[i/4*4+(3-(i%4))];

        seclib_rpmb_mac_key_init(cid_int32_be, 16);
    } while (0);
    BOOTING_TIME_PROFILING_LOG("Rpmb Init");
#endif
...
#if CFG_FPGA_PLATFORM
    memmove(tee_addr - 0x240, tee_addr, *size);
#else
    /* header addr will be updated to entry point addr */
