...
static void usbdl_send_da(void)
{
    ...
    if (da_len > DA_RAM_LENGTH) {
        print("%s usbdl_send_da: DA (0x%x) relocate size (0x%x) fail\n", MOD, da_len, DA_RAM_LENGTH);
        status = ERR_DA_RELOCATE_SIZE_NOT_ENOUGH;
    }
    else if (sig_len >= da_len) {
        print("%s usbdl_send_da: DA (0x%x), SIG (0x%x), relocate size (0x%x) fail\n", MOD, da_len, sig_len, DA_RAM_LENGTH);
        status = ERR_DA_INVALID_LENGTH;
    }
    ...
    if (STATUS_OK != status) {
        print("%s usbdl_send_da: invalid da parameters\n", MOD);
        ASSERT(0);
    }
...
static void usbdl_jump_da(void)
{
    ...
    print("%s Before return status, the sync status = 0x%x", MOD, status);
