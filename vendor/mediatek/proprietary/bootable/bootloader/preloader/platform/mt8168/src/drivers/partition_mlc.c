...
static pt_info pi;
static u8 sig_buf[PT_SIG_SIZE + PT_TLCRATIO_SIZE];
...
int load_exist_part_tab(u8 *buf)
{
   ...
   find:
	...
	if (slc_mode_en)
		memcpy(buf,&page_readbuf[PT_SIG_SIZE + PT_TLCRATIO_SIZE],sizeof(lastest_part));
	else
      ...
      if(pi.pt_changed==1)
		...
		if (slc_mode_en) {
			*(u64 *)sig_buf = MPT_SIG | (slc_ratio << 32);
			...
			pi.sequencenumber+=1;
			memcpy(&page_buf[PAGE_SIZE+PT_SIG_SIZE + PT_TLCRATIO_SIZE],&pi,PT_SIG_SIZE + PT_TLCRATIO_SIZE);
		} else {
			*(u32 *)sig_buf = MPT_SIG;
