...
void get_part_tab_from_complier(void)
{
	...
	while(part->flags!= PART_FLAG_END) {
		...
		lastest_part[index].mask_flags = PART_FLAG_NONE;
...
void pmt_init(void)
{
	...
	/* get_part_tab_from_complier(); get from complier */
		...
		for (i = 0; i < PART_MAX_COUNT; i++,part++) {
			...
			if(!strcmp(lastest_part[i].name,"BMTPOOL") && (devinfo.NAND_FLASH_TYPE == NAND_FLASH_TLC)) {
				...
				if(devinfo.tlcControl.normaltlc)
					lastest_part[i].ext.type = REGION_SLC_MODE;
				else
					lastest_part[i].ext.type = REGION_TLC_MODE;
				}
				#endif
			}
			...
			part->part_attr = (unsigned long)lastest_part[i].ext.attr;
			...
			print("part %s size %llx %llx attr %x\n", lastest_part[i].name,
			    lastest_part[i].offset, lastest_part[i].size, lastest_part[i].ext.attr);
		}
	}
	init_pmt_done = TRUE;
}

u32 get_seccfg_offset()
{
...
u32 get_secro_offset()
{
	return part_secro_offset;
}

u32 get_secro_length()
{
...
part_t *pmt_get_part(part_t *part, int index)
{
	...
	tempart.part_attr = part->part_attr;
...
{
	int mpt_locate;
	u64 mpt_start_addr= total_size +block_size;
...
int load_exist_part_tab(u8 *buf)
{
	...
	MSG (INIT, "load_pt from 0x%llx page_size:%d block_size:0x%x total_size:0x%llx\n",
		pt_start_addr, page_size, block_size, total_size);
