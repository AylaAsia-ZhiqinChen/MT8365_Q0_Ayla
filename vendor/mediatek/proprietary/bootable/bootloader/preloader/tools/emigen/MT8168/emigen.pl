...
my $total_part_number_iter = 0;
    ...
    while ($iter < $TotalCustemChips)
    {
        ...
        {
            # if all discrete dram are LPDDR2, LPDDR3, it is allowed
...
#=cut                   
	    	...
	    	# DDR1,DDR3 and DDR4 only support one discrete DRAM in the Combo List
    	...
    	unless($ENV{PROJECT} eq "mt8168_evb"){
    ...
    if ($DIS_LPDDR2 > 0 && (($DIS_LPDDR3 > 0) || ($MCP_LPDDR3 > 0) || ($DIS_PCDDR3 > 0) || ($DIS_PCDDR4 > 0)))
    {
	    die "[Error] LPDDR2 and LPDDR3 are not allowed to be mixed in the Combo Discrete DRAM list.\n" ;
    }
    if ($DIS_LPDDR3 > 0 && (($DIS_LPDDR2 > 0) || ($MCP_LPDDR2 > 0) || ($DIS_PCDDR3 > 0) || ($DIS_PCDDR4 > 0)))
    {
	    die "[Error] LPDDR2 and LPDDR3 are not allowed to be mixed in the Combo Discrete DRAM list.\n" ;
    }
    if ($DIS_PCDDR3 > 1 || (($DIS_PCDDR3 == 1) && ($CustCS_CustemChips>1)))
    {
	    die "[Error] PCDDR3 not support Combo Discrete DRAM feature.\n" ;
    }
    if ($DIS_PCDDR4 > 1 || ($DIS_PCDDR4 == 1) && ($CustCS_CustemChips>1))
    {
	    die "[Error] PCDDR4 not support Combo Discrete DRAM feature.\n" ;
    }
...
if ($build_opt == 0)
{
    ...
    print CUSTOM_EMI_H &description_file_header(                      "custom_emi.c",
...
if ($build_opt == 0)
{
    ...
    print CUSTOM_EMI_C &description_file_header(                      "custom_emi.h",
          "This Module defines the EMI (external memory interface) related setting.",
                                                 "EMI auto generator". $EMIGEN_VERNO);
...
#define COMBO_PCDDR4 ($DIS_PCDDR4)
#define COMBO_LPDDR4 ($DIS_LPDDR4)
#define NUM_EMI_RECORD ($TotalCustemChips)

#endif /* __CUSTOM_EMI_H__ */
...
=cut
	...
	my $template = <<"__TEMPLATE";
#ifndef __CUSTOM_EMI_C__
#define __CUSTOM_EMI_C__
...
#
	if ($DEV_TYPE[$id] =~ /LPDDR2/)
	{
			$DDR1_2_3[$id] = 'LPDDR2';
            $DIS_TYPE = "0x0000";
            $DIS_CONA = "0x00002012";
            $DIS_CONH = "0x00000003";
            $DIS_AC_U00 = "0x09090320";
            $DIS_AC_U01 = "0x19491401";
            $DIS_AC_U02 = "0x0B090201";
            $DIS_AC_U03 = "0x550C0306";
            $DIS_AC_U04 = "0x23566560";
            $DIS_AC_U05 = "0x00000908";
            $DIS_AC_U06 = "0x14140000";
            $DIS_AC_U07 = "0x00000000";
            $DIS_CONF = "0x04210000";
            $DIS_CH0_CONA = "0x00000000";
            $DIS_CH1_CONA = "0x00000000";
            $DIS_CBT_MODE = "0x0";
	}
	...
	elsif ($DEV_MODE[$id] eq "CBT_BYTE_MODE2")
	{
            $MODE_VAL[$id] = '0x2'; 
	}
	else
...
#union
# DDR1
        if ($DDR1_2_3[$id] eq "DDR1")
        {
	          die "[Error][EMIgen] no support DDR1.";
        }
 # LPDDR2
        elsif ($DDR1_2_3[$id] eq "LPDDR2")
        {
...
# LPDDR3    
        ...
        elsif ($DDR1_2_3[$id] eq "PCDDR4")
        {
            $PCDDR4_MODE_REG5[$id]             = &xls_cell_value($Sheet, $_, $platform_scan_idx++) ;
        }
...
#        if (!($CHIP_ID[$id] eq $MTK_MACH_TYPE))
	...
	#rank size check END
        if ($DEV_TYPE[$id] eq "Discrete LPDDR3")
        {
            $DEV_TYPE1[$id] = "00" ;
                 $DEV_TYPE2[$id] = "02" ;
        ...
        elsif ($DEV_TYPE[$id] eq "Discrete PCDDR4")
        {
            $DEV_TYPE1[$id] = "00" ;
            $DEV_TYPE2[$id] = "01" ;
	        $DIS_PCDDR4 = $DIS_PCDDR4 + 1 ;
        ...
        elsif ($DEV_TYPE[$id] eq "MCP(eMMC+PCDDR4)")
        {
            $DEV_TYPE1[$id] = "02" ;
            $DEV_TYPE2[$id] = "01" ;
	        $MCP_PCDDR4 = $MCP_PCDDR4 + 1 ;
        }
        ...
        else
        {
	      ...
	      if (($DEV_TYPE2[$id] eq "02") && ($LPDDR3_MODE_REG5[$id] eq ''))
	      {
...
#1 LPDDR2
        ...
        {    
            ...
            $EMI_SETTINGS[$id] = $EMI_SETTINGS[$id] . $PCDDR4_MODE_REG5[$id] . ",\t\t/* LPDDR4_MODE_REG5 */\n\t\t" ;
...
#3 LPDDR3    
        ...
        {    
            $EMI_SETTINGS[$id] = $EMI_SETTINGS[$id] . $PCDDR4_MODE_REG5[$id] . ",\t\t/* PCDDR4_MODE_REG5 */\n\t\t" ;
        }  
...
sub write_tag()
{
    ...
    print FILE pack("a24", "MTK_BLOADER_INFO_v28"); #MT8168
...
#    print FILE pack("H8", bc000000);
    ...
    # 1.LPDDR2/LPDDR3discrete dram number >= 2
    #[DEL] 2.LPDDR2 discrete dram > 0, LPDDR2 MCP > 0
    #[DEL] 3.LPDDR3 discrete dram > 0, LPDDR3 MCP > 0
=head
    if (($Discrete_DDR >= 2))
    ...
    { 
	    ...
	    elsif ($DEV_TYPE2[0] eq "03")
	    {
	   	...
	   	elsif ($DEV_TYPE2[0] eq "04")
	    {
		    $filesize = $filesize + &write_tag_one_element_default_PCDDR3() ;
	    }
	    elsif ($DEV_TYPE2[0] eq "05")
	    {
		    $filesize = $filesize + &write_tag_one_element_default_PCDDR4() ;
	    }
    }
=cut
    for $iter (0..$TotalCustemChips-1)
...
sub write_tag_one_element_default_LPDDR2()
{
    ...
    print FILE pack ("L", hex (lc("0x00001012"))) ;     # EMI_CONA_VAL
    $fs = $fs + 4 ;

    print FILE pack ("L", hex (lc("0x00000003"))) ;     # EMI_CONH_VAL
    $fs = $fs + 4 ;
        
    print FILE pack ("L", hex (lc("0x09090320"))) ;     # AC Timing U 00
    $fs = $fs + 4 ;
    
    print FILE pack ("L", hex (lc("0x19491401"))) ;     # AC Timing U 01
    $fs = $fs + 4 ;
    
    print FILE pack ("L", hex (lc("0x0B090201"))) ;     # AC Timing U 02
    $fs = $fs + 4 ;
    
    print FILE pack ("L", hex (lc("0x550C0306"))) ;     # AC Timing U 03
    $fs = $fs + 4 ;
    
    print FILE pack ("L", hex (lc("0x23566560"))) ;     # AC Timing U 04
    $fs = $fs + 4 ;

    print FILE pack ("L", hex (lc("0x00000908"))) ;     # AC Timing U 05
    $fs = $fs + 4 ;

    print FILE pack ("L", hex (lc("0x14140000"))) ;     # AC Timing U 06
    $fs = $fs + 4 ;      

    print FILE pack ("L", hex (lc("0x00000000"))) ;     # AC Timing U 07
    $fs = $fs + 4 ;      

    print FILE pack ("L", hex ("0x20000000")) ;                                  #  DRAM_RANK_SIZE[4]
    ...
    print FILE pack ("L", hex (lc("0x00000000"))) ;     # CHN0_EMI_CONA_VAL
    $fs = $fs + 4 ;

    print FILE pack ("L", hex (lc("0x00000000"))) ;     # CHN1_EMI_CONA_VAL
    $fs = $fs + 4 ;
...
sub write_tag_one_element_default_PCDDR3()
{
    my $id = $_[0];
    my $fs = 0 ;

    print "in write_tag_one_element_default_PCDDR3";
    print FILE pack ("L", hex (lc("0x1"))) ;        # Sub_version checking for flash tool
    $fs = $fs + 4 ;

    print FILE pack("L", hex("0x0004"));                           #type
    $fs = $fs + 4 ;
...
sub write_tag_one_element_default_PCDDR4()
{
    my $id = $_[0];
    my $fs = 0 ;

    print "in write_tag_one_element_default_PCDDR4";
    print FILE pack ("L", hex (lc("0x1"))) ;        # Sub_version checking for flash tool
    $fs = $fs + 4 ;
...
sub write_tag_one_element()
{
    ...
    {
        #pcddr3
        print FILE pack ("L", hex (lc($PCDDR3_MODE_REG5[$id]))) ;        # PCDDR3_MODE_REG5
        $fs = $fs + 4 ;
    }
    elsif ($DEV_TYPE2[$id] == "05")
    {
        #pcddr4
        print FILE pack ("L", hex (lc($PCDDR4_MODE_REG5[$id]))) ;        # PCDDR4_MODE_REG5
        $fs = $fs + 4 ;
    }
    elsif ($DEV_TYPE2[$id] == "00")
    {
        #pcddr4
        print FILE pack ("L", hex (lc($PCDDR4_MODE_REG5[$id]))) ;        # PCDDR4_MODE_REG5
        $fs = $fs + 4 ;
