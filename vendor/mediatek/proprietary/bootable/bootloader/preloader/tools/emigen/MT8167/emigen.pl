...
#=cut                   
    	...
    	unless($ENV{PROJECT} eq "mt8167_evb"){
...
#define COMBO_PCDDR4 ($MCP_PCDDR4+$DIS_PCDDR4)
...
#        if (!($CHIP_ID[$id] eq $MTK_MACH_TYPE))
	...
	#rank size check END
	    ...
	    elsif ($DEV_TYPE[$id] eq "MCP(NAND+PCDDR3)")
        {
  	    ...
  	    elsif ($DEV_TYPE[$id] eq "MCP(NAND+PCDDR4)")
        {
            $DEV_TYPE1[$id] = "01" ;
            $DEV_TYPE2[$id] = "05" ;
...
sub write_tag()
{
    ...
    print FILE pack("a24", "MTK_BLOADER_INFO_v28"); #MT8167
