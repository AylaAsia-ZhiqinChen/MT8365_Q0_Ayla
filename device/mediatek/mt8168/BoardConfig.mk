# mt8168 platform boardconfig

# Use the common part
include device/mediatek/vendor/common/BoardConfig.mk

# Use the non-open-source part, if present
-include vendor/mediatek/mt8168/BoardConfigVendor.mk
...
PARTITION_XML := $(MTK_TARGET_PROJECT_FOLDER)/partition/partition.xml
endif
ifneq ($(wildcard $(PARTITION_XML)),)
define get-partition-size
$(shell device/mediatek/build/build/tools/partition/get-partition-size.py $(PARTITION_XML) $(1) $(2))
endef
else
$(error Cannot find $(MTK_TARGET_PROJECT_FOLDER)/partition/partition.xml)
endif

ifeq ($(strip $(MTK_AB_OTA_UPDATER)), yes)
ifeq ($(strip $(MTK_DYNAMIC_PARTITION_SUPPORT)), yes)
RESERVED_SIZE := 1048576
BOARD_MAIN_SIZE := $(shell expr \( $(call get-partition-size,super) - $(RESERVED_SIZE) \) / 2)
BOARD_SUPER_PARTITION_SIZE := $(call get-partition-size,super)
else
BOARD_MAIN_SIZE := $(shell expr $(call get-partition-size,system_a) + $(call get-partition-size,vendor_a))
endif
BOARD_SYSTEMIMAGE_PARTITION_SIZE := $(call get-partition-size,system_a)
...
BOARD_SEPOLICY_DIRS += \
        device/mediatek/mt8168/sepolicy/basic
endif
ifeq ($(strip $(MTK_BSP_PACKAGE)), yes)
BOARD_SEPOLICY_DIRS += \
        device/mediatek/mt8168/sepolicy/basic \
        device/mediatek/mt8168/sepolicy/bsp
endif
ifneq ($(strip $(MTK_BASIC_PACKAGE)), yes)
ifneq ($(strip $(MTK_BSP_PACKAGE)), yes)
BOARD_SEPOLICY_DIRS += \
        device/mediatek/mt8168/sepolicy/basic \
        device/mediatek/mt8168/sepolicy/bsp \
        device/mediatek/mt8168/sepolicy/full
endif
endif
...
PRODUCT_CUSTOM_IMAGE_MAKEFILES += device/mediatek/mt8168/odm/odm.mk
endif
endif

# config.fs
TARGET_FS_CONFIG_GEN += device/mediatek/mt8168/config.fs
