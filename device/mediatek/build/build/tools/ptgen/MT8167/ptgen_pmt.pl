...
my $COLUMN_UPDATE                   = $COLUMN_DL + 1;
my $COLUMN_FB_ERASE                 = $COLUMN_UPDATE + 1;  # fastboot support
...
my $SECRO_SIZE;
my $USERDATA_SIZE;
my $SYSTEM_SIZE = 0;	#for MNTL system
my $CACHE_SIZE = 0;	#for MNTL cache
...
my $CACHE_SIZE_KB = 0;	#for MNTL cache
my $vendor_partition_exists = "N";
my %mountPointMapList;

BEGIN
{
	print "PTGEN start here\n";
  $LOCAL_PATH = dirname($0);
}
END
{
	print "PTGEN end here\n";
}
my $CD_ALPS;
$CD_ALPS="$LOCAL_PATH/../../../../../../.."; 
...
my $PART_XML_LOCATION		= "$OUT_PATH/partition_nand.xml" ; # store the partition size for ext4 buil
...
if ($EMMC_SUPPORT eq "yes" || $MTK_NAND_UBIFS_SUPPORT eq "yes" || $MTK_NAND_MTK_FTL_SUPPORT eq "yes" || $MNTL_SUPPORT eq "yes"){
	&GenPartSizeFile ();
}
...
sub ReadExcelFile()
{
	...
	while($pt_name ne "END"){
		...
		}elsif($pt_name eq "ANDROID"){
			if($MNTL_SUPPORT eq "yes"){
				$SYSTEM_SIZE_KB=&check_boardconfig_partition_info("SYSTEM");
				if ($SYSTEM_SIZE_KB == 0) {
				...
				$GPT[$gpt_idx]{"END_SECTOR"} = $GPT[$gpt_idx]{"START_SECTOR"} + $SYSTEM_SIZE_KB * 1024 / $SECTOR_SIZE - 1;
				$gpt_idx++;
				print "Skip ANDROID for MNTL_SUPPORT and SYSTEM_SIZE_KB=$SYSTEM_SIZE_KB\n";
			}
		}elsif($pt_name eq "VENDOR"){
			if($MNTL_SUPPORT eq "yes"){
				$VENDOR_SIZE_KB=&check_boardconfig_partition_info("VENDOR");
				if ($VENDOR_SIZE_KB == 0) {
					$VENDOR_SIZE_KB=&xls_cell_value($sheet, $row, $COLUMN_SIZEKB,$SHEET_NAME) ;
				}
				...
				$GPT[$gpt_idx]{"END_SECTOR"} = $GPT[$gpt_idx]{"START_SECTOR"} + $VENDOR_SIZE_KB * 1024 / $SECTOR_SIZE - 1;
				$gpt_idx++;
				print "Skip VENDOR for MNTL_SUPPORT and VENDOR_SIZE_KB=$VENDOR_SIZE_KB\n";
			}
		}elsif($pt_name eq "CACHE"){
			if($MNTL_SUPPORT eq "yes"){
				$CACHE_SIZE_KB=&check_boardconfig_partition_info("CACHE");
				if ($CACHE_SIZE_KB == 0) {
				...
				$GPT[$gpt_idx]{"END_SECTOR"} = $GPT[$gpt_idx]{"START_SECTOR"} + $CACHE_SIZE_KB * 1024 / $SECTOR_SIZE - 1;
				$gpt_idx++;
		...
		}elsif($pt_name eq "VENDOR" && $vendor_partition_exists ne "Y")
		{
			...
			if($MNTL_SUPPORT eq "yes" && $pt_name eq "USRDATA"){
		...
		$UPDATE_FIELD[$row_t-1]    = &xls_cell_value($sheet, $row, $COLUMN_UPDATE,$SHEET_NAME) ;
	...
	for($row=1;$row < @PARTITION_FIELD;$row++){
		...
		#	$START_FIELD_Byte[$row] = &xls_cell_value($sheet, $row+1, $COLUMN_START,$SHEET_NAME);
			if($COMBO_NAND_SUPPORT eq "yes")
			{
				...
				if($MTK_NAND_UBIFS_FASTMAP_SUPPORT ne "yes")
				{
					$START_FIELD_Byte[$row] = "4294901760";	#value 0xffff0000

				}else
				{
					$START_FIELD_Byte[$row] = sprintf("%d", 4294901760 + $bmtpool);
					print "set START_FIELD_Byte bmtpool=$bmtpool\n";
				}
		...
		if($MLC_NAND_SUPPORT eq "yes")
		{
			...
			}else
			{
		...
		elsif($TLC_NAND_SUPPORT eq "yes")
                {
                        if($DL_TYPE[$row-1] eq "LOW_PAGE")
                        {
                                $START_FIELD_Byte[$row] = $START_FIELD_Byte[$row - 1] + $SIZE_FIELD_KB[$row - 1] * 3 * 1024;
                        }else
                        {
                                $START_FIELD_Byte[$row] = $START_FIELD_Byte[$row-1]+$SIZE_FIELD_KB[$row-1]*1024;
                        }
		...
		if($PARTITION_FIELD[$row] eq "BMTPOOL" || $PARTITION_FIELD[$row] eq "OTP"){
		  if($EMMC_SUPPORT eq "yes")
			...
			}else
			{
				...
				{
					$START_FIELD_Byte_HEX[$row] = sprintf("0xffff%04x", $bmtpool);
...
sub GenYAMLScatFile(){
	...
	my %fileHash=(
	    ...
	    {
		$fileHash{TEE1}="tee.img";
		$fileHash{TEE2}="tee.img";
	    }
            else
	    {
...
__TEMPLATE
	...
	{
		print SCAT_NAME "${SecondDashes}config_version${colon}V1.1.5\n";
	}
	...
	for ($iter=0; $iter<$total_rows; $iter++){
		...
		if($MLC_NAND_SUPPORT eq "yes" || $TLC_NAND_SUPPORT eq  "yes"){
			if ($PARTITION_FIELD[$iter] eq "USRDATA") {
				if($MNTL_SUPPORT eq "yes"){
					#Fixed slc ratio depends on nand type.
					$slc_percentage = 0;
				}else{
					$slc_percentage = $MTK_TLC_SLC_RATIO;
				}
                        }
                }
		...
		print SCAT_NAME "${FirstSpaceSymbol}slc_percentage${colon}${slc_percentage}\n";
		print SCAT_NAME "${FirstSpaceSymbol}is_upgradable${colon}$Scatter_Info{$PARTITION_FIELD[$iter]}{is_upgradable}\n";
...
my $Total_Size=512*1024*1024; #Hard Code 512MB for 4+2 project FIX ME!!!!!
	...
	if($MNTL_SUPPORT eq "yes"){
		...
		#$USERDATA_START_SECTOR= $CACHE_END_SECTOR + 1;
			...
			if($TYPE_FIELD[$index] eq "EXT4" || $TYPE_FIELD[$index] eq "FAT"){
				...
				if($MTK_NAND_MTK_FTL_SUPPORT eq "yes") {
					if(($PARTITION_FIELD[$index] eq "USRDATA") && ($temp == 0)){
						$temp = int((int($Total_Size/${PEB})-10)*$LEB/1024/1024);
					}else
					{
						$temp -= ${PEB}/1024/1024;
					}
    ...
    printf $part_size_fh ("MTK_BOARD_ROOT_EXTRA_SYMLINKS += /data/nvram:nvdata\n");
...
sub check_boardconfig_partition_info {
	my ($arg_name) = @_;
	my $board_config_path = load_boardconfig_partition_info($PROJECT);
	my $board_config = &open_for_read("$board_config_path");
	my $value = 0;
	print "board_config_path = $board_config_path, $PROJECT\n";
	if ($board_config)
	{
		my $line;
		while (defined($line = <$board_config>))
		{
			if ($line =~ /\A\s*BOARD_MTK_${arg_name}_SIZE_KB\s*:=\s*(\d+)/i || $line =~ /\A\s*BOARD_MTK_${arg_name}_SIZE_KB\s*:=\s*(NA)/i)
			{
				$value = $1;
				print "by project size $arg_name = $1 KB\n";
			}
