...
{
	$MTK_TLC_SLC_RATIO[1] = $MTK_TLC_SLC_RATIO*3;
}
else
{
...
$MTK_SLC_BUFFER_SUPPORT       = $ENV{MTK_SLC_BUFFER_SUPPORT};
...
if( ($MTK_DRM_KM_SUPPORT eq "yes") || ($MTK_IH_TEE_SUPPORT eq "yes") ){
	...
	{
		$SHEET_NAME = $SHEET_NAME . " ftl";
		if($MTK_SLC_BUFFER_SUPPORT eq "yes")
	        {
			$SHEET_NAME = $SHEET_NAME . " slc";
        	}
	}
	if( ($MTK_DRM_KM_SUPPORT eq "yes") || ($MTK_IH_TEE_SUPPORT eq "yes") ){
    $SHEET_NAME = $SHEET_NAME . " drm";
...
my $Total_Size=512*1024*1024; #Hard Code 512MB for 4+2 project FIX ME!!!!!
	...
	if($MLC_NAND_SUPPORT eq "yes" || $TLC_NAND_SUPPORT eq  "yes") {
		my $ocp_slc=10;
		for($i=0;$i<2;$i++){
			$INIFILE="$PRODUCT_OUT/tmp_ubi_info$i.ini";
			if($slc_blk[$i] == 0){
				next;
			}
	...
	for($index=0;$index < $total_rows;$index++){
		...
		#		my $ext_grp = part_grp($PARTITION_FIELD[$index]);

		#		$temp = $SIZE_FIELD_KB[$index]/1024;

		#		if($MTK_NAND_MTK_FTL_SUPPORT eq "yes") {
		#			if(($PARTITION_FIELD[$index] eq "USRDATA")){
		#				#$temp = int((int($temp/($PEB/1024/1024))-12)*$LEB/1024/1024); # Userdata Reserved 12
		#		#$temp = int(($SIZE_FIELD_KB[$index] / ($PEB/1024) - 23 - $slc_blk[$ext_grp]*2) * ($LEB/1024/1024)); #MB
		#			$temp = int(($SIZE_FIELD_KB[$index] / ($PEB/1024) - 23 - $slc_blk[$ext_grp]*2) * ($LEB/1024/1024)); #MB
		#		print "[Bean]user_size_ftl=$temp\n";
		#			}else{
		#				if ($PARTITION_FIELD[$index] eq "CACHE") {
		#					$temp -= 30*${PEB}/1024/1024; #Cache Reserved UBI more than 30 for FTL ext4
		#					print "[Bean] cache_size_ftl=$temp\n";
		#				}else{
		#					$temp = int(($SIZE_FIELD_KB[$index] / ($PEB/1024) - 20 - $slc_blk[$ext_grp]) * ($LEB/1024/1024)); #MB
		...
		if(($TYPE_FIELD[$index] eq "UBIFS/YAFFS2") || ($MTK_NAND_MTK_FTL_SUPPORT eq "yes" && $TYPE_FIELD[$index] eq "EXT4")){
			# UBI reserve 6 block
			my $ubi_used_blk=6;
			...
			if($MLC_NAND_SUPPORT eq "yes" || $TLC_NAND_SUPPORT eq  "yes"){
				$INIFILE="$PRODUCT_OUT/tmp_ubi_${part_name}.ini";
			} else {
				$INIFILE="$PRODUCT_OUT/ubi_${part_name}.ini";
			}
			...
			if($PARTITION_FIELD[$index] eq "USRDATA") {
				if($MTK_FAT_ON_NAND eq "yes" && (($MLC_NAND_SUPPORT eq "yes") || ($TLC_NAND_SUPPORT eq "yes"))){
...
#Disable autoresize when fastmap enabled
			...
			if($PARTITION_FIELD[$index] eq "USRDATA") {
				if(0 && $MTK_FAT_ON_NAND eq "yes" && (($MLC_NAND_SUPPORT eq "yes") || ($TLC_NAND_SUPPORT eq "yes"))){
#for fat volume
					print $ini_fd "[fat]\n";
					print $ini_fd "mode=ubi\n";
...
#for ipoh volume
			...
			if($MLC_NAND_SUPPORT eq "yes" || $TLC_NAND_SUPPORT eq "yes") {
        		...
        		close $ini_fd;
			if($MTK_NAND_MTK_FTL_SUPPORT eq "yes" && $TYPE_FIELD[$index] eq "EXT4") {
				...
				$temp = int($vol_size/1024/1024);
                                if($MTK_NAND_MTK_FTL_SUPPORT eq "yes") {
                                        if(($PARTITION_FIELD[$index] eq "USRDATA")){
                                                #$temp = int((int($temp/($PEB/1024/1024))-12)*$LEB/1024/1024); # Userdata Reserved 12
						if($TLC_NAND_SUPPORT eq "yes") {
			                                $temp = int(($SIZE_FIELD_KB[$index] / ($PEB/1024) - 150 - $slc_blk[$ext_grp]*2) * ($LEB/1024/1024)); #MB
						}else{
							$temp = int(($SIZE_FIELD_KB[$index] / ($PEB/1024) - 23 - $slc_blk[$ext_grp]*2) * ($LEB/1024/1024)); #MB
						}
						print "[Bean]user_size_ftl=$temp\n";
	...
	if($MLC_NAND_SUPPORT eq "yes" || $TLC_NAND_SUPPORT eq  "yes"){
		...
		if(scalar(@{$ini_files[0]}) > 0) {
			my $inifile="$PRODUCT_OUT/ubi_usrdata.ini";
			append_files($inifile, @{$ini_files[0]});
		}
		if(scalar(@{$ini_files[1]}) > 0) {
			my $inifile="$PRODUCT_OUT/ubi_android.ini";
			append_files($inifile, @{$ini_files[1]});
		} else {
			my $dst="$PRODUCT_OUT/ubi_usrdata.ini";
			my $src=("$PRODUCT_OUT/tmp_ubi_usrdata.ini");
