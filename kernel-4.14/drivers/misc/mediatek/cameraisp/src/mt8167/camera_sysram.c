...
static void SYSRAM_DumpLayout(void)
{
	...
	/*  */
	LOG_PR_DMP("[SYSRAM_DumpLayout]\n");
	LOG_PR_DMP("AllocatedTbl = 0x%08lX\n", Sysram.AllocatedTbl);
	LOG_PR_DMP("=========================================\n");
...
static SYSRAM_MEM_NODE_STRUCT *SYSRAM_AllocNode(SYSRAM_MEM_POOL_STRUCT * const pMemPoolInfo)
{
	...
	/* Shouldn't happen. */
	if (!pNode) {
		LOG_PR_ERR("returns NULL - pMemPoolInfo->IndexTbl(%lX)", pMemPoolInfo->IndexTbl);
	}
	return pNode;
}
...
EXIT:
	if (MaxSize < Size) {
		LOG_PR_ERR("[User:%s]requested size(0x%lX) > max size(0x%lX)",
...
static MBOOL SYSRAM_FreeUserPhy(SYSRAM_USER_ENUM const User, SYSRAM_MEM_BANK_ENUM const MemBankNo)
{
	...
	/*  */
	if (!pMemPoolInfo) {
		LOG_PR_ERR("pMemPoolInfo==NULL,User(%d),MemBankNo(%d)", User, MemBankNo);
		return MFALSE;
	}
	/*  */
	pCurrNode = &pMemPoolInfo->pMemNode[0];
...
static MUINT32 SYSRAM_AllocUser(SYSRAM_USER_ENUM const User, MUINT32 Size, MUINT32 const Alignment)
{
	...
	/*  */
	if (SYSRAM_IsBadOwner(User)) {
		LOG_PR_ERR("User(%d) out of range(%d)", User, SYSRAM_USER_AMOUNT);
		return 0;
	}
	/*  */
	...
	default:
		{
			if (SYSRAM_FreeUserPhy(User, MemBankNo)) {
				...
				LOG_PR_ERR("Cannot free User(%d)", User);
				SYSRAM_DumpLayout();
			}
			break;
		}
				   ...
				   MUINT32 const Size, MUINT32 const Alignment)
{
	...
	if (SYSRAM_UserIsLocked(User)) {
		SYSRAM_SpinUnlock();
		LOG_PR_ERR("[User:%s]has been already allocated!", SysramUserName[User]);
		return 0;
	}
	/*  */
				...
				MUINT32 const Size, MUINT32 Alignment, MUINT32 const TimeoutMS)
{
	...
	/*  */
	if (0 == Size) {
		LOG_PR_ERR("[User:%s]allocates 0 size!", SysramUserName[User]);
		return 0;
	}
	/*  */
	...
	/*  */
	if (0 == TimeOut && 0 == Addr) {
		LOG_PR_ERR("[User:%s]allocate timeout", SysramUserName[User]);
	}
	/*  */
EXIT:
	if (0 == Addr) {	/* Failure */
		LOG_PR_ERR("[User:%s]fails to allocate.Size(%lu),Alignment(%lu),TimeoutMS(%lu)",
	...
	*/
	if (pFile->private_data != NULL) {
		...
		if (pProc->Pid != 0 || pProc->Tgid != 0 || pProc->Table != 0) {
			/*  */
			LOG_PR_WRN("Proc:Name(%s),pid(%d),tgid(%d),Table(0x%08lX),Time(%ld.%06ld)",
				pProc->ProcName,
				pProc->Pid, pProc->Tgid, pProc->Table, pProc->TimeS, pProc->TimeUS);
			/*  */
			if (pProc->Table) {
				LOG_PR_WRN("Force to release");
				/*
				   LOG_PR_WRN("Proc:Name(%s),pid(%d),tgid(%d),Table(0x%08lX),Time(%ld.%06ld)",
				   pProc->ProcName,
				   pProc->Pid,
		...
		if (pProc->Pid != 0 || pProc->Tgid != 0 || pProc->Table != 0) {
			...
			if (pProc->Tgid == 0 && pProc->Table != 0) {
				...
				/*
				   ...
				   Sec,
				   USec);
				   LOG_PR_ERR("Proc:Name(%s),pid(%d),tgid(%d),Table(0x%08lX),Time(%ld.%06ld)",
				   pProc->ProcName,
				   pProc->Pid,
				   pProc->Tgid,
				   pProc->Table,
				...
				if (pProc->Table) {
					...
					/*  */
					pProc->Table = 0;
					LOG_PR_WRN("Force to release - (0x%X/0x%X)",
	...
	case SYSRAM_ALLOC:
		{
			if (copy_from_user(&Alloc, (void *)Param, sizeof(SYSRAM_ALLOC_STRUCT)) == 0) {
				if (SYSRAM_IsBadOwner(Alloc.User)) {
					LOG_PR_ERR("User(%d) out of range(%d)", Alloc.User,
						SYSRAM_USER_AMOUNT);
					Ret = -EFAULT;
					goto EXIT;
				}
				...
				if (Alloc.Addr != 0) {
					...
					if (pProc->Tgid == 0) {
						...
						SYSRAM_SpinUnlock();
					/**/	if (pProc->Tgid != current->tgid) {
							LOG_PR_ERR("Tgid is inconsistent");
							Ret = -EFAULT;
						}
					}
				} else {
				...
				if (copy_to_user
					...
					LOG_PR_ERR("copy to user failed");
					Ret = -EFAULT;
				}
			} else {
	...
	case SYSRAM_FREE:
		{
			if (copy_from_user(&User, (void *)Param, sizeof(SYSRAM_USER_ENUM)) == 0) {
				if (SYSRAM_IsBadOwner(User)) {
					LOG_PR_ERR("User(%d) out of range(%d)", User,
						SYSRAM_USER_AMOUNT);
					Ret = -EFAULT;
					goto EXIT;
				}
				...
				SYSRAM_SpinUnlock();
				LOG_PR_ERR("Freeing unallocated buffer user(%d/0x%x/0x%X)", User,
					(unsigned int)(pProc->Table), (unsigned int)(Keep_Table));
				SYSRAM_SpinLock();
				if (Flush_before || ((Keep_Table) & (1 << User))) {
					LOG_PR_WRN("WRN, flush before(%d), MASK(0x%X/0x%X)",
						Flush_before, (unsigned int)(pProc->Table),
						(unsigned int)(Keep_Table));
					Keep_Table &= ~(1 << User);
					LOG_PR_WRN("WRN_2, flush before(%d), MASK(0x%X/0x%X)",
						Flush_before, (unsigned int)(pProc->Table),
						(unsigned int)(Keep_Table));
					if ((int)Keep_Table == 0)
						Flush_before = false;
...
static int SYSRAM_Probe(struct platform_device *pDev)
{
	...
	if (SYSRAM_RegCharDrv()) {
		LOG_PR_ERR("register char failed");
		return -EAGAIN;
	}
	...
	if (IS_ERR(Sysram.pClass)) {
		...
		LOG_PR_ERR("Unable to create class, err(%ld)", Ret);
		return Ret;
	}
...
static int __init SYSRAM_Init(void)
{
	...
	if (pEntry) {
		...
		LOG_PR_ERR("add /proc/sysram entry fail.");
	}
	/*  */
	...
	if (pEntry) {
		...
		LOG_PR_ERR("add /proc/sysram_flag entry fail");
	}
#endif
	LOG_MSG("X");
	/*  */
