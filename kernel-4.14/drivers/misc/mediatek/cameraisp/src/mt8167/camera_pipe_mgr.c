					   ...
					   CAM_PIPE_MGR_SCEN_HW_ENUM ScenHw)
{
	...
	default:
		{
			LOG_PR_ERR("Unknown ScenHw(%d)", ScenHw);
			break;
		}
	}
	/*  */
	...
	default:
		{
			LOG_PR_ERR("Unknown ScenSw(%d)", ScenSw);
			break;
		}
	}
	/*  */
...
static int CamPipeMgr_Open(struct inode *pInode, struct file *pFile)
{
	...
	/*  */
	if (Ret == (-ENOMEM)) {
		LOG_PR_ERR("No enough memory");
		/*
		   LOG_PR_ERR("Cur:Name(%s),pid(%d),tgid(%d),Time(%ld.%06ld)",
		   current->comm,
		   current->pid,
		   current->tgid,
		   Sec,
	...
	*/
	if (pFile->private_data != NULL) {
		...
		if (pProc->Pid != 0 || pProc->Tgid != 0 || pProc->PipeMask != 0) {
			/*  */
			LOG_PR_WRN("Proc:Name(%s),Pid(%d),Tgid(%d),PipeMask(0x%lX),Time(%ld.%06ld)",
				pProc->ProcName,
				pProc->Pid,
				pProc->Tgid, pProc->PipeMask, pProc->TimeS, pProc->TimeUS);
			/*  */
			if (pProc->PipeMask) {
				LOG_PR_WRN("Force to unlock pipe");
				/*
				   LOG_PR_WRN("Proc:Name(%s),Pid(%d),Tgid(%d),PipeMask(0x%08lX),Time(%ld.%06ld)",
				   pProc->ProcName,
				   pProc->Pid,
				   pProc->Tgid,
				   pProc->PipeMask,
		...
		LOG_PR_WRN("private_data is NULL");
		/*
		   LOG_PR_WRN("Cur:Name(%s),pid(%d),tgid(%d),Time(%ld.%06ld)",
		   current->comm,
		   current->pid,
		   current->tgid,
		   Sec,
		...
		if (pProc->Pid != 0 || pProc->Tgid != 0 || pProc->PipeMask != 0) {
			...
			if (pProc->Tgid == 0 && pProc->PipeMask != 0) {
				...
				/*
				   ...
				   Sec,
				   USec);
				   LOG_PR_ERR("Proc:Name(%s),Pid(%d),Tgid(%d),PipeMask(0x%08lX),Time(%ld.%06ld)",
				   pProc->ProcName,
				   pProc->Pid,
				   pProc->Tgid,
				   pProc->PipeMask,
				...
				if (pProc->PipeMask) {
					...
					CamPipeMgr_UnlockPipe(&Unlock);
					pProc->PipeMask = 0;
					LOG_PR_WRN("Force to unlock pipe - (0x%x/0x%x/0x%x)",
...
static long CamPipeMgr_Ioctl(struct file *pFile, unsigned int Cmd, unsigned long Param)
{
	 ...
	 */
	if (pFile->private_data == NULL) {
		LOG_PR_ERR("private_data is NULL");
		Ret = -EFAULT;
		goto EXIT;
	}
	/*  */
	...
	case CAM_PIPE_MGR_LOCK:
		{
			if (copy_from_user(&Lock, (void *)Param, sizeof(CAM_PIPE_MGR_LOCK_STRUCT))
				...
				if ((Lock.PipeMask & CamPipeMgr.
					...
					LOG_PR_ERR("LOCK:Sw(%d),Hw(%d),LPM(0x%X),PLT(0x%lX) fail",
				...
				LOG_PR_ERR("LOCK:copy_from_user fail");
				Ret = -EFAULT;
			}
			break;
		}
	...
	case CAM_PIPE_MGR_UNLOCK:
		{
			if (copy_from_user
				...
				if (Flush_before || (pProc->PipeMask & Keep_PipeMask)) {
					LOG_PR_WRN("WRN, FLUSH before(%d), MASK(0x%X/0x%X/0x%X)",
						Flush_before, (unsigned int)(pProc->PipeMask),
						(unsigned int)(Unlock.PipeMask),
						(unsigned int)(Keep_PipeMask));
					Keep_PipeMask &= ~(pProc->PipeMask);
					Unlock.PipeMask &=  ~(pProc->PipeMask);
					LOG_PR_WRN("WRN_2, FLUSH before(%d), MASK(0x%X/0x%X/0x%X)",
						Flush_before, (unsigned int)(pProc->PipeMask),
						(unsigned int)(Unlock.PipeMask),
						(unsigned int)(Keep_PipeMask));
				...
				LOG_PR_ERR("UNLOCK:copy_from_user fail");
				Ret = -EFAULT;
			}
			break;
		}
	...
	case CAM_PIPE_MGR_SET_MODE:
		{
			if (copy_from_user(&Mode, (void *)Param, sizeof(CAM_PIPE_MGR_MODE_STRUCT))
				...
				LOG_PR_ERR("SET_MODE:copy_from_user fail");
				Ret = -EFAULT;
			}
			break;
		}
	...
	case CAM_PIPE_MGR_GET_MODE:
		{
			if ((CamPipeMgr.Mode.ScenHw > CAM_PIPE_MGR_SCEN_HW_VSS)
				...
				LOG_PR_ERR("ScenHw(%d) > max(%d)", CamPipeMgr.Mode.ScenHw,
					CAM_PIPE_MGR_SCEN_HW_VSS);
				Ret = -EFAULT;
				goto EXIT;
			}
			if ((CamPipeMgr.Mode.ScenSw > CAM_PIPE_MGR_SCEN_SW_N3D)
				...
				LOG_PR_ERR("ScenSw(%d) > max(%d)", CamPipeMgr.Mode.ScenSw,
					CAM_PIPE_MGR_SCEN_SW_N3D);
				Ret = -EFAULT;
				goto EXIT;
			}
			if ((CamPipeMgr.Mode.Dev > CAM_PIPE_MGR_DEV_VT)
				...
				LOG_PR_ERR("Dev(%d) > max(%d)", CamPipeMgr.Mode.Dev,
					CAM_PIPE_MGR_DEV_VT);
				Ret = -EFAULT;
				goto EXIT;
			}
			...
			if (copy_to_user
				...
				LOG_PR_ERR("GET_MODE:copy_to_user fail");
				Ret = -EFAULT;
			}
			break;
		}
			...
			if (copy_from_user
				...
				if ((Enable.
				     PipeMask & CamPipeMgr_GtePipeLockTable(CamPipeMgr.Mode.ScenSw,
									    ...
									    ScenHw)) !=
				    Enable.PipeMask) {
					LOG_PR_ERR("ENABLE_PIPE:Some pipe are not available");
					Ret = -EFAULT;
				} else {
					CamPipeMgr_SpinLock();
				...
				LOG_PR_ERR("ENABLE_PIPE:copy_from_user fail");
				Ret = -EFAULT;
			}
			break;
		}
			...
			if (copy_from_user
				...
				if ((Disable.
				     PipeMask & CamPipeMgr_GtePipeLockTable(CamPipeMgr.Mode.ScenSw,
									    ...
									    ScenHw)) !=
				    Disable.PipeMask) {
					LOG_PR_ERR("DISABLE_PIPE:Some pipe are not available");
					Ret = -EFAULT;
				} else {
					CamPipeMgr_SpinLock();
					CamPipeMgr.PipeLockTable[CamPipeMgr.Mode.ScenHw] &=
				...
				LOG_PR_ERR("DISABLE_PIPE:copy_from_user fail");
				Ret = -EFAULT;
			}
			break;
		}
	...
	case CAM_PIPE_MGR_VENCPLL_CTRL:
	#if 0 /* Not support mmdvfs */
		{
			if (copy_from_user
				...
				if (vencpll_ctrlEnum == CAM_PIPE_MGR_CMD_VECNPLL_CTRL_SET_HIGH) {
					...
					if (err) {
						LOG_PR_ERR("DISABLE_PIPE SET HIGH fail");
						Ret = -EFAULT;
					}
					...
					if (err) {
						LOG_PR_ERR("DISABLE_PIPE SET LOW fail");
						Ret = -EFAULT;
					}
				}
			} else {
				LOG_PR_ERR("VENCPLL_CTRL:copy_from_user fail");
				Ret = -EFAULT;
			}
			break;
		}
		...
		err = 0;
		Ret = -EFAULT;
		LOG_PR_WRN("MT8167 ISP only support HIGH Voltage: 312MHz");
		break;
	#endif
		/*  */
	default:
		{
			LOG_PR_ERR("Unknown cmd");
			Ret = -EFAULT;
			break;
		}
	}
...
static inline int CamPipeMgr_RegCharDev(void)
{
	...
	if (Ret < 0) {
		LOG_PR_ERR("alloc_chrdev_region fail:Ret(%ld)", Ret);
		return Ret;
	}
...
static int CamPipeMgr_Probe(struct platform_device *pDev)
{
	...
	if (Ret < 0) {
		LOG_PR_ERR("RegCharDev fail:Ret(%d)", Ret);
		return Ret;
	}
