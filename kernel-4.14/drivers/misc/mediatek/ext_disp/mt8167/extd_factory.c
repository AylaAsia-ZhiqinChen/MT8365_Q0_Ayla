...
int hdmi_factory_mode_init(void)
{
	...
	if (hdmi_tx_drv == NULL) {
		EXTD_FACTORY_ERR("[hdmi]%s can not get hdmi driver handle\n",
				__func__);
		return -1;
	}
...
void hdmi_factory_dpi_parameters(int arg, int io_driving)
{
	enum HDMI_POLARITY clk_pol =
		HDMI_POLARITY_RISING, de_pol = HDMI_POLARITY_RISING,
	...
	case HDMI_VIDEO_1280x720p_60Hz:
		{
			if (strncmp(CONFIG_CUSTOM_KERNEL_HDMI,
						"ANX7625", 7) == 0)
	...
	case STEP2_JUDGE_CALLBACK:
		{
			...
			if (copy_to_user(info, &hdmi_status,
						sizeof(hdmi_status))) {
				EXTD_FACTORY_ERR("[HDMI]failed:%d\n",
						__LINE__);
				ret = -1;
			}
			break;
	...
	case STEP3_START_DPI_AND_CONFIG:
		{
			...
			EXTD_FACTORY_LOG("res/test case:%d, driving:0x%x\n",
		...
		EXTD_FACTORY_LOG("[hdmi]Noneed video config for DPI HQA\n");
