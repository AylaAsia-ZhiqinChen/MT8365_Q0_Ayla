...
struct notify_dev hdmi_cec_notify_data;
//struct notify_dev hdmi_audio_notify_data;
...
int hdmi_uevent_dev_register(struct notify_dev *sdev)
{
	...
	if (!switch_class) {
		...
		if (ret == 0) {
			HDMI_LOG("[hdmi]  create_switch_class susesess\n");
		} else {
			HDMI_LOG("[hdmi]  create_switch_class fail\n");
			return ret;
		}
	}

	sdev->index = atomic_inc_return(&device_count);
	HDMI_LOG("[hdmi]  index:0x%x\n", sdev->index);

	sdev->dev = device_create(switch_class, NULL,
		 MKDEV(0, sdev->index), NULL, sdev->name);

	if (sdev->dev) {
		HDMI_LOG("[hdmi]  device create ok,index:0x%x\n",
			sdev->index);
		ret = 0;
	} else {
		HDMI_LOG("[hdmi]  device create fail,index:0x%x\n",
			sdev->index);
		ret = -1;
	}
...
int notify_uevent_user(struct notify_dev *sdev, int state)
{
	...
	HDMI_LOG("[hdmi]  notify state:0x%x\n", state);
	...
	envp[1] = state_buf;
	envp[2] = NULL;
	HDMI_LOG("[hdmi]  name:%s ,state:%s\n", envp[0], envp[1]);

	kobject_uevent_env(&sdev->dev->kobj, KOBJ_CHANGE, envp);

	HDMI_LOG("[hdmi]  kobj_name:%s\n", sdev->dev->kobj.name);
	HDMI_LOG("[hdmi]  init:0x%x\n", sdev->dev->kobj.state_initialized);
	HDMI_LOG("[hdmi]  sysfs:0x%x\n",
		sdev->dev->kobj.state_in_sysfs);
	HDMI_LOG("[hdmi]  uevent sent:0x%x\n",
		sdev->dev->kobj.state_add_uevent_sent);
	HDMI_LOG("[hdmi]  remove sent:0x%x\n",
		sdev->dev->kobj.state_remove_uevent_sent);
	HDMI_LOG("[hdmi]  uevent suppress:0x%x\n",
...
void hdmi_force_on(int from_uart_drv)
{
	...
	HDMI_LOG("%s %d force on %d, cnt %d\n", __func__, from_uart_drv,
	hdmi_force_to_on, mhl_forcr_on_cnt);
	mhl_forcr_on_cnt++;
	hdmi_force_to_on = 1;
...
int hdmi_is_force_awake(void *argp)
{
	...
	ret = copy_to_user(argp, &hdmi_params->is_force_awake,
				 sizeof(hdmi_params->is_force_awake)) ?
		-EFAULT : 0;
	return ret;
}
...
int hdmi_allocate_hdmi_buffer(void)
{
	...
	ret = m4u_alloc_mva(client, M4U_PORT_DISP_OVL1, hdmi_va, 0,
	hdmiBufferSize,
...
int hdmi_waitVsync(void)
{
	...
	if (wait_event_interruptible_timeout(
				hdmi_vsync_wq, hdmi_vsync_flag, HZ / 10) == 0)
...
static int hdmi_fence_release_kthread(void *data)
{
	...
	for (;;) {
		...
		if (ext_disp_path_source_is_RDMA(MHL_SESSION_ID)) {
			if (ext_disp_get_ovl_req_status(MHL_SESSION_ID) ==
					EXTD_OVL_REMOVED) {
				ext_disp_path_change(EXTD_OVL_NO_REQ,
						MHL_SESSION_ID);
				...
				if ((ovl_release == 1) && (
					...
					for (layid = 1; layid <
							...
							layid++) {
						fence_idx =
					    disp_sync_find_fence_idx_by_addr(
							    session_id,
							    layid,
							    ovl_config_address
							    [layid]);
			...
			for (layid = 0; layid < EXTD_OVERLAY_CNT; layid++) {
				if (ovl_config_address[layid] !=
						input_curr_addr[layid]) {
				...
				if (ext_disp_is_dim_layer(input_curr_addr[
							layid]) == 1)
					fence_idx =
					    disp_sync_find_fence_idx_by_addr(
							    session_id, layid,
				...
				mtkfb_release_fence(session_id, layid,
						fence_idx);

				if (hdmi_params->is_3d_support)
					layer_3d_format |=
						mtkfb_query_buf_info(session_id,
							layid,
							input_curr_addr[layid],
							0);
			}
...
/* LCD overlay to memory -> MDP resize and rotate to memory -> */
/* DPI read to HDMI */
...
static enum HDMI_STATUS hdmi_drv_init(void)
{
...
#ifndef MTK_EXTENSION_MODE_SUPPORT
	p->hdmi_width = hdmi_resolution_param_table[
		hdmi_params->init_config.vformat][0];
	p->hdmi_height = hdmi_resolution_param_table[
		hdmi_params->init_config.vformat][1];
#else
	...
	if (!hdmi_fence_release_task) {
		/* disp_register_module_irq_callback(DISP_MODULE_RDMA2, */
		/* _hdmi_rdma_irq_handler); K2 no RDMA2 */
		disp_register_module_irq_callback(DISP_MODULE_RDMA,
				_hdmi_rdma_irq_handler);
		hdmi_fence_release_task =
			kthread_create(hdmi_fence_release_kthread,
...
static void hdmi_state_reset(void)
{
	...
	if (hdmi_drv->get_state() == HDMI_STATE_ACTIVE) {
		...
		extcon_set_state_sync(hdmi_extcon,
			EXTCON_DISP_HDMI, HDMI_STATE_ACTIVE);
		notify_uevent_user(&hdmi_notify_data, HDMI_STATE_ACTIVE);
		hdmi_enable_dvfs(true);
		hdmi_reschange = HDMI_VIDEO_RESOLUTION_NUM;
	} else {
...
/*static*/ void hdmi_suspend(void)
{
	...
	mmprofile_log_ex(ddp_mmp_get_events()->Extd_State,
			MMPROFILE_FLAG_START, Plugout, 0);
...
static void hdmi_resolution_setting(int arg)
{
	...
	if (hdmi_drv && hdmi_drv->get_params) {
		...
		p->bg_height =
			((hdmi_params->height *
			  p->scaling_factor) / 100 >> 2) << 2;
		p->bg_width = ((hdmi_params->width *
					p->scaling_factor) / 100 >> 2) << 2;
		...
		extd_dpi_params.dispif_config.dpi.dsc_params.bit_per_pixel =
			128;
	...
	extd_dpi_params.dispif_config.dpi.dsc_params.increment_interval =
			0x11e;
	extd_dpi_params.dispif_config.dpi.dsc_params.decrement_interval =
			0x1a;
		extd_dpi_params.dispif_config.dpi.dsc_params.nfl_bpg_offset =
			0xdb7;	/*0x667 */
		extd_dpi_params.dispif_config.dpi.dsc_params.slice_bpg_offset =
			0x394;	/*0x1ca */
		extd_dpi_params.dispif_config.dpi.dsc_params.final_offset =
			0x10f0;
		extd_dpi_params.dispif_config.dpi.dsc_params.line_bpg_offset =
			0xc;
...
int hdmi_recompute_bg(int src_w, int src_h)
{
	...
	if ((extd_path_state == EXTD_DEINIT ||
	extd_path_state == EXTD_SUSPEND) || is_hdmi_active() == false) {
...
void hdmi_cec_state_callback(enum HDMI_CEC_STATE state)
{
	...
	notify_uevent_user(&hdmi_cec_notify_data, 0xff);
	switch (state) {
	case HDMI_CEC_STATE_PLUG_OUT:
		/*
		 *switch_set_state(&hdmi_cec_switch_data,
		 *		HDMI_CEC_STATE_PLUG_OUT);
		 */
		notify_uevent_user(&hdmi_cec_notify_data,
			HDMI_CEC_STATE_PLUG_OUT);
		break;
...
int hdmi_set_resolution(int res)
{
	...
	HDMI_LOG("video resolution config, res:%d, old res:%ld, video_on:%d\n",
	...
	mmprofile_log_ex(ddp_mmp_get_events()->Extd_State,
			MMPROFILE_FLAG_START, ResChange, res);
	...
	notify_uevent_user(&hdmires_notify_data, hdmi_reschange + 1);
	p->is_clock_on = true;
	mmprofile_log_ex(ddp_mmp_get_events()->Extd_State,
			MMPROFILE_FLAG_END, ResChange,
			 hdmi_reschange + 1);
#if 0
	hdmi_factory_mode_test(STEP3_START_DPI_AND_CONFIG, NULL);
	hdmi_drv->video_config(HDMI_VIDEO_1280x720p_60Hz,
...
int hdmi_get_dev_info(int is_sf, void *info)
{
	...
	if (is_sf == AP_GET_INFO) {
		...
		if (dispif_info->displayWidth *
				dispif_info->displayHeight <= 240 * 432)
...
int hdmi_init(void)
{
	...
	if (ret == 0)
		HDMI_LOG("[hdmi] hdmi_uevent_dev_register hdmi ok\n");
	else
		HDMI_LOG("[hdmi] hdmi_uevent_dev_register hdmi fail\n");
	...
	if (ret == 0)
		HDMI_LOG("[hdmi] hdmi_uevent_dev_register res_hdmi ok\n");
	else
		HDMI_LOG("[hdmi] hdmi_uevent_dev_register res_hdmi fail\n");


	hdmi_cec_notify_data.name = "cec_hdmi";
	hdmi_cec_notify_data.index = 0;
	hdmi_cec_notify_data.state = 0;
	ret = hdmi_uevent_dev_register(&hdmi_cec_notify_data);
	if (ret == 0)
		HDMI_LOG("[hdmi] hdmi_uevent_dev_register hdmi ok\n");
	else
		HDMI_LOG("[hdmi] hdmi_uevent_dev_register hdmi fail\n");
...
int hdmi_post_init(void)
{
	...
	if (hdmi_drv == NULL) {
		HDMI_ERR("[hdmi]%s,cannot get hdmi driver handle\n",
				__func__);
		return -1;
	}
