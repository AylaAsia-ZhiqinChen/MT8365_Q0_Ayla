...
struct ext_disp_path_context {
	...
	struct disp_internal_buffer_info *decouple_buffer_info[
...
static int _should_trigger_path(void)
{
	 ...
	 * we can't decide path trigger(ovl/rdma/dsi..)
	 * separately with mutex enable
	 * but it's lucky because path trigger and mutex enable
	 * is the same w/o cmdq, and it's correct w/ CMDQ(Y+N)
	 */
	if (ext_disp_is_video_mode())
		return dpmgr_path_is_idle(pgc->dpmgr_handle);
...
static int ext_init_decouple_buffers(void)
{
	...
	pgc->decouple_rdma_config.pitch =
	...
	pgc->decouple_wdma_config.dstPitch =
		ext_disp_get_width() * DP_COLOR_BITS_PER_PIXEL(eRGB888) / 8;
	...
	for (i = 0; i < DISP_INTERNAL_BUFFER_COUNT; i++) {
		if (pgc->decouple_buffer_info[i]) {
			EXT_DISP_LOG("extd free decouple buffer: 0x%x\n",
					pgc->dc_buf[i]);
			ion_free(pgc->decouple_buffer_info[i]->client,
					pgc->decouple_buffer_info[i]->handle);
...
static int _build_path_decouple(void)
{
	...
	dpmgr_map_event_to_irq(pgc->dpmgr_handle,
			DISP_PATH_EVENT_IF_VSYNC, DDP_IRQ_RDMA1_DONE);
	dpmgr_enable_event(pgc->dpmgr_handle, DISP_PATH_EVENT_IF_VSYNC);
	dpmgr_enable_event(pgc->dpmgr_handle, DISP_PATH_EVENT_FRAME_DONE);
...
static void _cmdq_build_trigger_loop(void)
{
	...
	if (ext_disp_is_video_mode()) {
		...
		dpmgr_path_trigger(pgc->dpmgr_handle,
				pgc->cmdq_handle_trigger, CMDQ_ENABLE);
		...
		struct disp_input_config *src)
{
	...
	EXT_DISP_LOG("%s: layer%d en%d w%d h%d pitch%d addr0x%lx sec%d\n",
			__func__,
...
static int ext_disp_rdma_update_kthread(void *data)
{
	...
	for (;;) {
		...
		data_config->rdma_config.is_top_filed =
			ddp_dpi_is_top_filed(DISP_MODULE_DPI1);
...
int ext_disp_init(char *lcm_name, unsigned int session)
{
	...
	if (ret) {
		EXT_DISP_LOG("%s,CMDQ_SCENARIO_MHL_DISP FAIL, ret=%d\n",
				__func__,
				ret);
		ret = EXT_DISP_STATUS_ERROR;
		goto done;
	...
	ret = cmdqRecCreate(CMDQ_SCENARIO_PRIMARY_MEMOUT,
			&(pgc->cmdq_handle_ovl2mem_config));
	if (ret != 0) {
		EXT_DISP_LOG("%d CMDQ_SCENARIO_PRIMARY_MEMOUT FAIL, ret=%d\n",
				__LINE__,
				ret);
		ret = EXT_DISP_STATUS_ERROR;
		goto done;
	...
	if (ext_disp_use_cmdq == CMDQ_ENABLE) {
		if (DISP_SESSION_DEV(session) != DEV_EINK + 1) {
			...
			EXT_DISP_LOG("ext_disp BUILD cmdq trigger loop done\n");
...
static int ext_disp_trigger_ovl_to_memory(disp_path_handle disp_handle,
...
static int _ovl_fence_release_callback(uint32_t block)
{
	...
	if (!block)
		_ext_disp_path_lock();
	if (!block && (pgc->state == EXTD_DEINIT ||
				pgc->state == EXTD_SUSPEND)) {
	...
	if (pgc->is_interlace) {
		...
		pgc->decouple_rdma_config.is_top_filed =
			ddp_dpi_is_top_filed(DISP_MODULE_DPI1);
	} else {
		pgc->decouple_rdma_config.is_interlace = false;
	}
	_config_rdma_input_data(&pgc->decouple_rdma_config, pgc->dpmgr_handle,
			pgc->cmdq_handle_config);
	_ext_disp_trigger(0, NULL, 0);
		...
		unsigned int session)
{
	...
	if (pgc->mode == EXTD_DECOUPLE_MODE) {
		ext_disp_trigger_ovl_to_memory(pgc->ovl2mem_path_handle,
				pgc->cmdq_handle_ovl2mem_config,
				(fence_release_callback
				 )_ovl_fence_release_callback, 0, 1);
		_ovl_fence_release_callback(1);
	} else {
		if (DISP_SESSION_TYPE(session) == DISP_SESSION_EXTERNAL &&
				DISP_SESSION_DEV(session) == DEV_MHL + 1)
			_ext_disp_trigger(blocking,
					_rdma_fence_release_callback, userdata);
...
static void ext_disp_path_blank(int enCmdq)
{
	...
	if (data_config) {
		...
		pgc->decouple_rdma_config.address =
			pgc->dc_buf[pgc->dc_buf_id];
		pgc->decouple_rdma_config.security = DISP_NORMAL_BUFFER;
		memcpy(&(data_config->rdma_config),
				&(pgc->decouple_rdma_config),
...
static void ext_disp_update_rdma(enum DISP_MODULE_ENUM module,
		unsigned int param)
{
	if (param & 0x2) {
...
static void ext_check_is_interlace_output(struct disp_ddp_path_config *config)
{
	...
	if ((config->dispif_config.dpi.dpi_clock ==
				HDMI_VIDEO_1920x1080i_60Hz) ||
		(config->dispif_config.dpi.dpi_clock ==
		 HDMI_VIDEO_1920x1080i_50Hz) ||
		(config->dispif_config.dpi.dpi_clock ==
		 HDMI_VIDEO_1920x1080i3d_60Hz) ||
		(config->dispif_config.dpi.dpi_clock ==
		 HDMI_VIDEO_1920x1080i3d_60Hz)) {
		if (!is_exist_callback) {
			disp_register_module_irq_callback(DISP_MODULE_RDMA1,
					ext_disp_update_rdma);
			is_exist_callback = true;
		}
		pgc->is_interlace = true;
	} else {
		if (is_exist_callback) {
			disp_unregister_module_irq_callback(DISP_MODULE_RDMA1,
					ext_disp_update_rdma);
		...
		int idx, unsigned int session)
{
	...
	if (pgc->state != EXTD_INIT && pgc->state != EXTD_RESUME &&
		...
		EXT_DISP_LOG("config ext disp is already slept, state:%d\n",
				pgc->state);
		mmprofile_log_ex(ddp_mmp_get_events()->Extd_ErrorInfo,
				MMPROFILE_FLAG_PULSE, Config, idx);
		mtkfb_release_session_fence(pgc->session);
		_ext_disp_path_unlock();
		return -2;
	}
	...
	if (_should_config_ovl_input()) {
		...
		for (i = 0; i < input->config_layer_num; i++) {
			...
			cmdqRecBackupUpdateSlot(cmdq_handle, pgc->ovl_address,
					i, input->config[i].next_buff_idx);
		}
		...
		if (layer_info)
			cmdqRecBackupUpdateSlot(cmdq_handle, pgc->rdma_info, 0,
	...
	if (pgc->mode == EXTD_DECOUPLE_MODE) {
		pgc->decouple_wdma_config.dstAddress =
			pgc->dc_buf[pgc->dc_buf_id];
		...
		for (i = 0; i < EXTD_OVERLAY_CNT; i++) {
			if (data_config->ovl_config[i].layer_en &&
			    (data_config->ovl_config[i].security ==
			     DISP_SECURE_BUFFER)) {
				pgc->decouple_wdma_config.dstAddress =
					pgc->dc_sec_buf[pgc->dc_buf_id];
				pgc->decouple_wdma_config.security =
					DISP_SECURE_BUFFER;
				break;
			}
		}

		cmdqRecBackupUpdateSlot(cmdq_handle, pgc->wdma_info, 0,
				pgc->decouple_wdma_config.security);
		cmdqRecBackupUpdateSlot(cmdq_handle, pgc->wdma_info, 1,
				pgc->decouple_wdma_config.dstAddress);
	...
	if (data_config->ovl_dirty) {
		...
		EXT_DISP_LOG("%s idx:%d -w:%d, h:%d, pitch:%d, mva:0x%lx\n",
				__func__,
				idx, data_config->rdma_config.width,
