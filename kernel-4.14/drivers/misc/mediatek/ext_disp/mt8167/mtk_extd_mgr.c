		...
		unsigned long arg)
{
	...
	case MTK_HDMI_AUDIO_VIDEO_ENABLE:
		{
			/* 0xXY
			 * the low 16 bits(Y) are for disable and enable,
			 * and the high 16 bits(X) are for device id
			 * the device id:
			 * X = 0 - mhl
			 * X = 1 - wifi display
			 */
	...
	default:
		{
			EXT_MGR_ERR("[EXTD]ioctl(%d) argument is not support\n",
					cmd & 0x0ff);
			r = -EFAULT;
			break;
		}
...
static int mtk_extd_mgr_probe(struct platform_device *pdev)
{
	...
	/* For character driver register to system, device number binded
	 * to file operations
	 */
	...
	/* mknod /dev/hdmitx */
	class_dev = (struct class_device *)device_create(extd_class, NULL,
			extd_devno, NULL, EXTD_DEVNAME);
