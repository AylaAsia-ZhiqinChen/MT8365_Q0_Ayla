...
static int extd_create_path(enum EXT_DISP_PATH_MODE mode, unsigned int session)
{
	...
	MULTI_COTRL_LOG("%s session:%08x, mode:%d\n", __func__,
			session, mode);
...
static int extd_get_device_type(unsigned int session)
{
	...
	if (device_id == DEV_MHL && extd_driver[DEV_MHL]->ioctl) {
		...
		/*for eink device*/
		ret = extd_driver[DEV_EINK]->ioctl(GET_DEV_TYPE_CMD, 0, 0,
				NULL);
	}
...
static void extd_set_layer_num(int layer_num, unsigned int session)
{
	...
	if (device_id == DEV_MHL && extd_driver[DEV_MHL]->ioctl) {
		/*for mhl device*/
		extd_driver[DEV_MHL]->ioctl(SET_LAYER_NUM_CMD, layer_num, 0,
				NULL);
	} else if (device_id == DEV_EINK && extd_driver[DEV_EINK]->ioctl) {
		/*for eink device*/
		extd_driver[DEV_EINK]->ioctl(SET_LAYER_NUM_CMD, layer_num, 0,
				NULL);
	}
}
	...
	if (DISP_SESSION_TYPE(session) == DISP_SESSION_MEMORY &&
		...
		|| path_info.old_session[DEV_WFD] != DISP_SESSION_MEMORY)) {
			...
			if (ext_disp_wait_ovl_available(0) > 0) {
				...
				MULTI_COTRL_ERR("OVL1can not be split out!\n");
				ret = -1;
			}
		...
		|| extd_type == DISP_IF_HDMI_SMARTBOOK)) {
			...
			if (ext_disp_wait_ovl_available(0) > 0) {
				...
				MULTI_COTRL_ERR("OVL1 can not be split out!\n");
...
static void destroy_external_display_path(unsigned int session, int mode)
{
	...
	DISP_SESSION_DIRECT_LINK_MIRROR_MODE)) {
		...
		MULTI_COTRL_LOG("noneed destroy path session:0x%08x, mode:%d\n",
...
static int disp_switch_mode_kthread(void *data)
{
	...
	MULTI_COTRL_LOG("switch mode, mode:%d, session:0x%x\n",
...
static int path_change_without_cascade(enum DISP_MODE mode,
		unsigned int session_id, unsigned int device_id)
{
	int ret = -1;
	...
	if (session_id == 0 &&
		...
		if (device_id == DEV_WFD) {
			...
			session = MAKE_DISP_SESSION(DISP_SESSION_MEMORY,
					device_id);
		} else {
	...
	if ((session_id > 0 && path_info.old_session[device_id] ==
				DISP_SESSION_PRIMARY)
...
int external_display_config_input(struct disp_session_input_config *input,
...
int external_display_get_info(void *info, unsigned int session)
{
	...
	} else if (device_id == DEV_EINK  &&
			extd_driver[DEV_EINK]->get_dev_info) {
		...
		unsigned int *session_created, unsigned int session)
{
	...
	for (i = 0; i < MAX_SESSION_COUNT; i++) {
		if (session_created[i] == MAKE_DISP_SESSION(
					DISP_SESSION_EXTERNAL, DEV_MHL+1)) {
			/*it has MHL session*/
			session_id[DEV_MHL] = session_created[i];
		}

		if (session_created[i] == MAKE_DISP_SESSION(
					DISP_SESSION_EXTERNAL, DEV_EINK+1)) {
			/*it has EINK session*/
			session_id[DEV_EINK] = session_created[i];
		}

		if (session_created[i] == MAKE_DISP_SESSION(
					DISP_SESSION_MEMORY, DEV_WFD)) {
			/*it has WFD session*/
	...
	path_info.switching = (switching == 0) ?
		DEV_MAX_NUM : path_info.switching;
