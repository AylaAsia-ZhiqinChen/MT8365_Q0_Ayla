...
/*global variable*/
/*============================================================*/
enum kal_bool chargin_hw_init_done = KAL_TRUE;
enum kal_bool charging_type_det_done = KAL_TRUE;
...
const unsigned int VBAT_CV_VTH[] = {
	BATTERY_VOLT_03_775000_V, BATTERY_VOLT_03_800000_V,
	BATTERY_VOLT_03_850000_V, BATTERY_VOLT_03_900000_V,
	BATTERY_VOLT_04_000000_V, BATTERY_VOLT_04_050000_V,
	BATTERY_VOLT_04_100000_V, BATTERY_VOLT_04_125000_V,
	BATTERY_VOLT_04_137500_V, BATTERY_VOLT_04_150000_V,
	BATTERY_VOLT_04_162500_V, BATTERY_VOLT_04_175000_V,
	BATTERY_VOLT_04_187500_V, BATTERY_VOLT_04_200000_V,
	BATTERY_VOLT_04_212500_V, BATTERY_VOLT_04_225000_V,
	BATTERY_VOLT_04_237500_V, BATTERY_VOLT_04_250000_V,
	BATTERY_VOLT_04_262500_V, BATTERY_VOLT_04_275000_V,
	BATTERY_VOLT_04_287500_V, BATTERY_VOLT_04_300000_V,
	BATTERY_VOLT_04_312500_V, BATTERY_VOLT_04_325000_V,
	BATTERY_VOLT_04_337500_V, BATTERY_VOLT_04_350000_V,
	BATTERY_VOLT_04_362500_V, BATTERY_VOLT_04_375000_V,
	BATTERY_VOLT_04_387500_V, BATTERY_VOLT_04_400000_V,
	BATTERY_VOLT_04_412500_V, BATTERY_VOLT_04_425000_V
};

const unsigned int CS_VTH[] = {
	CHARGE_CURRENT_1600_00_MA, CHARGE_CURRENT_1500_00_MA,
	CHARGE_CURRENT_1400_00_MA, CHARGE_CURRENT_1300_00_MA,
	CHARGE_CURRENT_1200_00_MA, CHARGE_CURRENT_1100_00_MA,
	CHARGE_CURRENT_1000_00_MA, CHARGE_CURRENT_900_00_MA,
	CHARGE_CURRENT_800_00_MA,  CHARGE_CURRENT_700_00_MA,
	CHARGE_CURRENT_650_00_MA,  CHARGE_CURRENT_550_00_MA,
	CHARGE_CURRENT_450_00_MA,  CHARGE_CURRENT_300_00_MA,
	CHARGE_CURRENT_200_00_MA,  CHARGE_CURRENT_70_00_MA};
...
const unsigned int VCDT_HV_VTH[] = {
	...
	BATTERY_VOLT_04_450000_V, BATTERY_VOLT_06_000000_V,
...
static enum kal_bool diso_thread_timeout;
...
static unsigned int charging_get_csdac_value(void)
{
	...
	pmic_read_interface(MT6392_CHR_CON10, &tempC, 0xF, 0x0);

	pmic_config_interface(MT6392_CHR_CON11, 0xA, 0xF, 0);
	/*bit 0 ~ 3 mapping bit 4 ~7 */
	pmic_read_interface(MT6392_CHR_CON10, &tempA, 0xF, 0x0);

	pmic_config_interface(MT6392_CHR_CON11, 0xB, 0xF, 0);
	/*bit 0~3 mapping bit 0~3 */
	pmic_read_interface(MT6392_CHR_CON10, &tempB, 0xF, 0x0);
...
static unsigned int charging_hw_init(void *data)
{
	...
	upmu_set_rg_chr_en(1);     /*MT6392_CHR_EN */
#endif
	return status;
}
...
static unsigned int charging_enable(void *data)
{
	...
	if (enable == KAL_TRUE) {
		...
		upmu_set_rg_chr_en(0);   /*MT6392_CHR_EN */
...
static unsigned int charging_set_cv_voltage(void *data)
{
	...
	register_value = charging_parameter_to_value(
		VBAT_CV_VTH, GETARRAYNUM(VBAT_CV_VTH), *(unsigned int *)(data));
...
static unsigned int charging_get_current(void *data)
{
	...
	*(unsigned int *)data =
		charging_value_to_parameter(CS_VTH, array_size, reg_value);
	return status;
}
...
static unsigned int charging_get_hv_status(void *data)
{
	...
	*(enum kal_bool *)(data) = upmu_get_rgs_vcdt_hv_det();
...
#else
	*(enum kal_bool *)(data) = upmu_get_rgs_chrdet();
#endif
...
enum kal_bool charging_type_detection_done(void)
{
...
#if defined(CONFIG_POWER_EXT) || defined(CONFIG_MTK_FPGA)
	*(enum kal_bool *)(data) = KAL_FALSE;
#else
	if (slp_get_wake_reason() == WR_PCM_TIMER)
		*(enum kal_bool *) (data) = KAL_TRUE;
	else
		*(enum kal_bool *) (data) = KAL_FALSE;
...
void set_vusb_auxadc_irq(bool enable, bool flag)
{
	...
	if (enable) {
		...
		threshold = (threshold * R_DISO_VBUS_PULL_DOWN) /
			    (R_DISO_VBUS_PULL_DOWN + R_DISO_VBUS_PULL_UP);
		mt_auxadc_enableBackgroundDection(
			DISO_IRQ.vusb_measure_channel.number, threshold,
			DISO_IRQ.vusb_measure_channel.period,
			DISO_IRQ.vusb_measure_channel.debounce, flag);
	} else {
		mt_auxadc_disableBackgroundDection(
			DISO_IRQ.vusb_measure_channel.number);
	}
#endif
...
void set_vdc_auxadc_irq(bool enable, bool flag)
{
	...
	if (enable) {
		...
		threshold = (threshold * R_DISO_DC_PULL_DOWN) /
			    (R_DISO_DC_PULL_DOWN + R_DISO_DC_PULL_UP);
		mt_auxadc_enableBackgroundDection(
			DISO_IRQ.vdc_measure_channel.number, threshold,
			DISO_IRQ.vdc_measure_channel.period,
			DISO_IRQ.vdc_measure_channel.debounce, flag);
	} else {
		mt_auxadc_disableBackgroundDection(
			DISO_IRQ.vdc_measure_channel.number);
	}
#endif
	pr_debug(" [%s] enable: %d, flag: %d!\n", __func__, enable, flag);
}
...
static unsigned int diso_get_current_voltage(int Channel)
{
	...
	while (i--) {
		...
		if (ret_value == 0) {
			...
			pr_debug(
				"[%s] ret_value=%d, times=%d\n",
				__func__, ret_value, times);
		}
	}
...
static void _get_diso_interrupt_state(void)
{
	...
	vol = (R_DISO_DC_PULL_UP + R_DISO_DC_PULL_DOWN) * 100 * vol /
	      (R_DISO_DC_PULL_DOWN) / 100;
	...
	vol = (R_DISO_VBUS_PULL_UP + R_DISO_VBUS_PULL_DOWN) * 100 * vol /
	      (R_DISO_VBUS_PULL_DOWN) / 100;
...
static void _get_polling_state(void)
{
	...
	vdc_vol = (R_DISO_DC_PULL_UP + R_DISO_DC_PULL_DOWN) * 100 * vdc_vol /
		  (R_DISO_DC_PULL_DOWN) / 100;

	vusb_vol = diso_get_current_voltage(AP_AUXADC_DISO_VUSB_CHANNEL);
	vusb_vol = (R_DISO_VBUS_PULL_UP + R_DISO_VBUS_PULL_DOWN) * 100 *
		   vusb_vol / (R_DISO_VBUS_PULL_DOWN) / 100;
	...
	if (DISO_Polling.reset_polling) {
		...
		vdc_vol_dir = _get_irq_direction(VDC_Polling->preVoltage,
						 VDC_Polling->curVoltage);
		vusb_vol_dir = _get_irq_direction(VUSB_Polling->preVoltage,
						  VUSB_Polling->curVoltage);
	}

	if (VDC_Polling->notify_irq_en &&
	    (vdc_vol_dir == VDC_Polling->notify_irq)) {
		schedule_delayed_work(&diso_polling_work,
				      10 * HZ / 1000); /*10ms */
...
int diso_thread_kthread(void *x)
{
	...
	while (1) {
		wait_event(diso_polling_thread_wq,
			   (diso_thread_timeout == KAL_TRUE));
...
static unsigned int charging_diso_init(void *data)
{
	...
	ret = request_threaded_irq(pDISO_data->irq_line_number,
				   diso_auxadc_irq_handler,
				   pDISO_data->irq_callback_func, IRQF_ONESHOT,
				   "DISO_ADC_IRQ", NULL);
 ...
 *        None
 */
signed int chr_control_interface(enum CHARGING_CTRL_CMD cmd, void *data)
{
	signed int status;
	...
	if (init == -1) {
		...
		charging_func[CHARGING_CMD_GET_POWER_SOURCE] =
		...
		charging_func[CHARGING_CMD_GET_DISO_STATE] =
