...
const unsigned int VBAT_CV_VTH[] = {
	...
	4272000, 4288000, 4304000, 4320000, 4336000, 4352000, 4368000};

const unsigned int CS_VTH[] = {51200,  57600,  64000,  70400,  76800,  83200,
...
const unsigned int INPUT_CS_VTH[] = {
	...
	CHARGE_CURRENT_2000_00_MA, CHARGE_CURRENT_MAX};
...
static unsigned int charging_get_current(void *data)
{
	...
	/*Get current level*/
	bq24296_read_interface(bq24296_CON2, &ret_val, CON2_ICHG_MASK,
			       CON2_ICHG_SHIFT);

	/*Get Force 20% option*/
	bq24296_read_interface(bq24296_CON2, &ret_force_20pct,
...
signed int chr_control_interface(enum CHARGING_CTRL_CMD cmd, void *data)
{
	...
	if (init == -1) {
		...
		charging_func[CHARGING_CMD_SET_VBUS_OVP_EN] =
			charging_set_vbus_ovp_en;
		charging_func[CHARGING_CMD_SET_VINDPM] = charging_set_vindpm;
		charging_func[CHARGING_CMD_SET_PWRSTAT_LED_EN] =
