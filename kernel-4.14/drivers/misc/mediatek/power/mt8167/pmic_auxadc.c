 ...
 *         mt6392 auxadc has no trimmed register, reserve it to align with
 * mt6397
 *
...
int PMIC_IMM_GetOneChannelValue(unsigned int dwChannel, int deCount, int trimd)
{
	...
	do {
		...
		if ((dwChannel == 5) || (dwChannel == 6)) {
			ret = pmic_config_interface(MT6392_TYPE_C_CTRL, 0x0,
						    0xffff, 0);
			if (ret < 0) {
			...
			ret = pmic_config_interface(
				MT6392_TYPE_C_CC_SW_FORCE_MODE_ENABLE_0, 0x5B8,
				0xffff, 0);
			if (ret < 0) {
				mutex_unlock(&mt6392_adc_mutex);
				return ret;
			...
			ret = pmic_config_interface(
				MT6392_TYPE_C_CC_SW_FORCE_MODE_VAL_0, 0x0100,
				0xffff, 0);
			if (ret < 0) {
				mutex_unlock(&mt6392_adc_mutex);
				return ret;
			...
			if (dwChannel == 5) {
				ret = pmic_config_interface(
					MT6392_TYPE_C_CC_SW_FORCE_MODE_VAL_0,
					0x1f40, 0xffff, 0);
				if (ret < 0) {
					mutex_unlock(&mt6392_adc_mutex);
					return ret;
			...
			ret = pmic_config_interface(
				MT6392_TYPE_C_CC_SW_FORCE_MODE_ENABLE_0, 0x05fc,
				0xffff, 0);
			if (ret < 0) {
				mutex_unlock(&mt6392_adc_mutex);
				return ret;
		...
		if ((dwChannel == 7) || (dwChannel == 8)) {
			if (dwChannel == 7) {
				/*DP channel*/
				ret = pmic_config_interface(MT6392_CHR_CON18,
							    0x0700, 0x0f00, 0);
				if (ret < 0) {
					mutex_unlock(&mt6392_adc_mutex);
					return ret;
		...
		/* AUXADC_RQST0 SET  */
		ret = pmic_config_interface(MT6392_AUXADC_RQST0_SET, 0x1,
					    0xffff, adc_channel);
		if (ret < 0) {
			mutex_unlock(&mt6392_adc_mutex);
			return ret;
		...
		do {
			ret = pmic_read_interface(
				mt6392_auxadc_regs[adc_channel], &reg_val,
				0xffff, 0);
			if (ret < 0) {
				mutex_unlock(&mt6392_adc_mutex);
				return ret;
		...
		if (adc_rdy != 1) {
			mutex_unlock(&mt6392_adc_mutex);
			pr_notice("%s adc get ready Fail\n", __func__);
			return -ETIMEDOUT;
		}
		count = 0;

		/* get the raw data and calculate the adc result of adc */
		ret = pmic_read_interface(mt6392_auxadc_regs[adc_channel],
					  &reg_val, 0xffff, 0);
		if (ret < 0) {
			mutex_unlock(&mt6392_adc_mutex);
			return ret;
