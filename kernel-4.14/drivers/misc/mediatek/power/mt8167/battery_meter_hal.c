...
void get_hw_chip_diff_trim_value(void)
{
	...
	bm_print(
		BM_LOG_FULL,
		"Reg[0x%x]=0x%x, Reg[0x%x]=0x%x, chip_diff_trim_value_4_0=%d\n",
	...
	case 0:
		...
		bm_print(BM_LOG_FULL,
			 "[Chip_Trim] chip_diff_trim_value = 1000\n");
		break;
	case 1:
...
int get_hw_ocv(void)
{
	...
	bm_print(
		BM_LOG_CRTI,
		"[oam] %s (swchr) : adc_result_reg=%d, adc_result=%d\n",
	...
	bm_print(BM_LOG_CRTI,
		 "[oam] %s (pchr) : adc_result_reg=%d, adc_result=%d\n",
		 __func__, adc_result_reg, adc_result);
#endif
...
static signed int read_adc_v_bat_sense(void *data)
{
	...
	*(signed int *)(data) = PMIC_IMM_GetOneChannelValue(
		VBAT_CHANNEL_NUMBER, *(signed int *)(data), 1);
#endif
...
#else
	*(signed int *)(data) = PMIC_IMM_GetOneChannelValue(
		ISENSE_CHANNEL_NUMBER, *(signed int *)(data), 1);
#endif
...
static signed int read_adc_v_bat_temp(void *data)
{
	...
	*(signed int *)(data) = PMIC_IMM_GetOneChannelValue(
		VBATTEMP_CHANNEL_NUMBER, *(signed int *)(data), 1);
#endif
#endif
...
static signed int read_adc_v_charger(void *data)
{
	...
	*(signed int *)(data) = 5001;
	...
	val = PMIC_IMM_GetOneChannelValue(VCHARGER_CHANNEL_NUMBER,
					  *(signed int *)(data), 1);
...
signed int bm_ctrl_cmd(int cmd, void *data)
{
	...
	if (init == -1) {
		...
		bm_func[BATTERY_METER_CMD_GET_HW_FG_CURRENT] =
			fgauge_read_current;
		bm_func[BATTERY_METER_CMD_GET_HW_FG_CURRENT_SIGN] =
