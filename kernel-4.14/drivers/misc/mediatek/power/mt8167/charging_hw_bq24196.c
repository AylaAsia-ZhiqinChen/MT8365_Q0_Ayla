...
static bool diso_thread_timeout;
...
static unsigned int charging_get_charger_det_status(void *data)
{
	...
	val = 1;
	pr_debug(
		"[%s] charger exist for bring up.\n", __func__);
#else
#if !defined(CONFIG_MTK_DUAL_INPUT_CHARGER_SUPPORT)
	val = upmu_get_rgs_chrdet();
#else
	if (((g_diso_state >> 1) & 0x3) != 0x0 ||
	    (upmu_get_rgs_chrdet() && !g_diso_otg))
		val = true;
	else
		val = false;
#endif
...
bool charging_type_detection_done(void)
{
...
void set_vusb_auxadc_irq(bool enable, bool flag)
{
	...
	DISO_Polling.reset_polling = true;
...
static void _get_polling_state(void)
{
	...
	if (DISO_Polling.reset_polling) {
		DISO_Polling.reset_polling = false;
...
static unsigned int charging_diso_init(void *data)
{
	...
	pDISO_data->chr_get_diso_state = false;
