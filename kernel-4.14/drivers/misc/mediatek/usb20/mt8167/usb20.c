...
#if (defined(CONFIG_POWER_EXT) && defined(CONFIG_MFD_MT6397))
static bool vcdt_status;
static bool vcdt_connect_status;
#endif
...
static inline u8 musb_rbyte(const void __iomem *addr,
	unsigned int offset)
{
	u8 rc = 0;
...
{
	DBG(0, "schedule work to do called\n");
...
void do_connection_work(struct work_struct *data)
{
	...
	if (!mtk_musb->is_ready) {
		/* re issue work */
		DBG_LIMIT(3,
			"!is_ready, after %d ms, is_host<%d>, power<%d>",
				CONN_WORK_DELAY,
				mtk_musb->is_host,
				mtk_musb->power);
		queue_delayed_work(mtk_musb->st_wq,
			&connection_work, msecs_to_jiffies(CONN_WORK_DELAY));
		return;
	}
...
#if (defined(CONFIG_POWER_EXT) && defined(CONFIG_MFD_MT6397))
/* PMIC MT6392 VCDT interrupt for vbus detection */
irqreturn_t ops_usbdet_int_handler(int irq, void *dev_id)
{
	vcdt_status = upmu_get_rgs_chrdet();
	DBG(0, "vcdt_status=%d\n", vcdt_status);
	if (vcdt_status && !vcdt_connect_status) {
		mt_usb_connect();
		vcdt_connect_status = true;
	} else if (!vcdt_status && vcdt_connect_status) {
		mt_usb_disconnect();
		vcdt_connect_status = false;
	}
...
static ssize_t mt_usb_show_tx(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	UINT8 var;
	UINT8 var2;
	...
	struct device_attribute *attr, const char *buf, size_t count)
{
	...
	/* } else if (1 == sscanf(buf, "%d", &val)) { */
		...
		DBG(0, "[MUSB]addr 0x6E TX After val: %x - %x\n", var, var2);
		sw_tx = var;
	}
	return count;
}
...
static ssize_t mt_usb_show_rx(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	UINT8 var;
	UINT8 var2;
			 ...
			 const struct i2c_device_id *id)
{
	...
	DBG(0, "[MUSB]usb i2c probe, start, base:%lx\n", base);
...
static int __init mt_usb_init(struct musb *musb)
{
	...
	musb_writel(musb->mregs,
		MUSB_HSDMA_INTR, 0xff | (0xff << DMA_INTR_UNMASK_SET_OFFSET));
...
static int mt_usb_probe(struct platform_device *pdev)
{
	...
	/* it should be put in musb_core probe */
	...
	usb_chrdet_number = platform_get_irq(pdev, 1);
	DBG(0, "POWER_EXT: usb_chrdet = %d\n", usb_chrdet_number);
	if (usb_chrdet_number <= 0) {
		DBG(0, "POWER_EXT: usb_chrdet get fail!\n");
		return 0;
	}
	ret = request_threaded_irq(usb_chrdet_number, NULL,
					ops_usbdet_int_handler,
					IRQF_TRIGGER_HIGH | IRQF_ONESHOT,
					"pmic_usb_chrdet", &pdev->dev);
	if (ret) {
		DBG(0, "POWER_EXT: request_threaded_irq err = %d\n", ret);
		return 0;
	}
	irq_set_irq_wake(usb_chrdet_number, 1);
#endif
...
{
	DBG(0, "%s, start to shut down\n", __func__);
	/* disable all level-1 interrupts in shutdown status */

	/* need prepare clock because  musb_generic_disable
	 * may call prepare clock in atomic context
	 */
#ifdef CONFIG_MTK_MUSB_PORT0_LOWPOWER_MODE
