...
void musb_otg_write_fifo(u16 len, u8 *buf)
{
	...
	DBG(0, "musb otg write fifo,len=%d\n", len);
	for (i = 0; i < len; i++)
...
void musb_otg_read_fifo(u16 len, u8 *buf)
{
	...
	DBG(0, "musb otg read fifo,len=%d\n", len);
	for (i = 0; i < len; i++)
...
unsigned int musb_polling_bus_interrupt(unsigned int intr)
{
	...
	do {
		...
		if (intr == ((MUSB_INTR_CONNECT | MUSB_INTR_RESUME))
				&& time_after(jiffies, timeout)) {
			DBG(0, "time out MUSB_INTR_CONNECT|MUSB_INTR_RESUME\n");
...
void musb_h_enumerate(void)
{
	...
	DBG(0, "get device descriptor OK!class=0x%x PID=0x%x VID=0x%x\n",
		device_descriptor.bDeviceClass, device_descriptor.idProduct,
		device_descriptor.idVendor);
	DBG(0, "get device descriptor OK!DescType=0x%x DevSubClass=0x%x\n",
	...
	otg_descriptor =
		(struct usb_otg_descriptor *)(descriptor +
			configuration_descriptor.wTotalLength - 3);
...
TD_4_6:
	...
	if (timeout_flag) {
		timeout_flag = false;
		musb_otg_reset_usb();
		DBG(0, "timeout, after reset usb, devctl=0x%x\n",
...
TD6_13:
		...
		if (!(intrusb & MUSB_INTR_CONNECT)) {
			...
			ret = musb_polling_bus_interrupt(
				MUSB_INTR_CONNECT | MUSB_INTR_RESUME);
			DBG(0, "done, ret=0x%x\n", ret);
			if (ret == TEST_IS_STOP)
				break;
			if (ret == MUSB_INTR_RESUME)
		...
		if (ret == TEST_IS_STOP)
			break;
			DBG(0, "A-OPT disconnect UUT will back to dev\n");
		if (!(ret & MUSB_INTR_RESET)) {
			musb_d_soft_connect(true);
			/*polling the reset single form the A-OPT*/
			DBG(0, "polling reset form A-OPT,begin\n");
...
ssize_t musb_otg_test_read(struct file *filp, char __user *buf,
	...
	size_t count, loff_t *ppos)
{
	...
	if (get_user(value, (unsigned char *)buf))
		...
		if (value == OTG_STOP_CMD) {
			...
			DBG(0, "the value is invalid, 0x%x\n", value);
			ret = -EFAULT;
		}
	}
	return ret;
				...
				unsigned int cmd, unsigned long arg)
{
	...
	DBG(0, "musb otg test ioctl:cmd=0x%x\n", cmd);
