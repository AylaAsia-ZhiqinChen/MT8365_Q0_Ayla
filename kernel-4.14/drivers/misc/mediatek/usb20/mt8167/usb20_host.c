...
void mt_usb_set_vbus(struct musb *musb, int is_on)
{
	...
	if (is_on && !vbus_on) {
		...
		if (drvvbus_pin >= 0 && !IS_ERR(pinctrl) &&
			!IS_ERR(pinctrl_drvvbus_high))
			pinctrl_select_state(pinctrl, pinctrl_drvvbus_high);
		else
			DBG(0, "Drive VBUS ON: drvvbus is not ready!\n");
#endif
#else
		...
		if (drvvbus_pin >= 0 && !IS_ERR(pinctrl) &&
			!IS_ERR(pinctrl_drvvbus_low))
			pinctrl_select_state(pinctrl, pinctrl_drvvbus_low);
		else
			DBG(0, "Drive VBUS OFF:drvvbus is not ready!\n");
#endif
#else
		mt_set_gpio_mode(GPIO_OTG_DRVVBUS_PIN,
			GPIO_OTG_DRVVBUS_PIN_M_GPIO);
...
#endif
	DBG(0, "switch int to device is done\n");
}
...
void switch_int_to_host(struct musb *musb)
{
	...
	musb_writel(musb->mregs, USB_L1INTM,
		IDDIG_INT_STATUS|musb_readl(musb->mregs, USB_L1INTM));
#endif
	DBG(0, "switch int to host is done\n");
}
...
void switch_int_to_host_and_mask(struct musb *musb)
{
	...
	musb_writel(musb->mregs, USB_L1INTP, IDDIG_INT_STATUS);
#endif
	DBG(0, "switch int to host and mask is done\n");
}
...
static void musb_id_pin_work(struct work_struct *data)
{
	...
	/* need prepare clock because
	 * musb_generic_disable may call
	 * prepare clock in atomic context
	 */
...
void mt_usb_iddig_int(struct musb *musb)
{
	...
	DBG(0, "id pin mt usb iddig int interrupt assert\n");
}
...
static void otg_int_init(void)
{
	...
	mt_eint_registration(IDDIG_EINT_PIN,
		EINTF_TRIGGER_LOW, mt_usb_ext_iddig_int, FALSE);
#else
	int ret = 0;
	...
	ret = request_irq(usb_iddig_number,
		mt_usb_ext_iddig_int, IRQF_TRIGGER_LOW, "USB_IDDIG", NULL);
	if (ret > 0)
	...
	if (node == NULL) {
		...
		if (iddig_pin < 0) {
			...
			ret = of_property_read_u32(node,
				"iddig_gpio_mode", &iddig_pin_mode);
			if (ret < 0)
				DBG(0, "read iddig_pin_mode failed\n");
			else
		...
		if (drvvbus_pin < 0) {
			...
			ret = of_property_read_u32(node,
				"drvvbus_gpio_mode", &drvvbus_pin_mode);
			if (ret < 0)
				DBG(0, "read drvvbus_gpio failed\n");
			else
