...
#define INS_REG(reg, offset, size) \
	...
	INS_REG(RP_RST, CTRL_BASE_OFFSET + 0x4C, 4),
	INS_REG(RP_CON, CTRL_BASE_OFFSET + 0x50, 4),
	INS_REG(RP_PRE_FUSE, CTRL_BASE_OFFSET + 0x54, 4),
	INS_REG(SLEEP_SRAM_CTL, CTRL_BASE_OFFSET + 0x58, 4),
	INS_REG(SRAM_CS_CTL, CTRL_BASE_OFFSET + 0x5C, 4),
	...
	INS_REG(SPARE4, CTRL_BASE_OFFSET + 0x80, 4),
	INS_REG(SPARE5, CTRL_BASE_OFFSET + 0x84, 4),
	INS_REG(SPARE6, CTRL_BASE_OFFSET + 0x88, 4),
	INS_REG(SPARE7, CTRL_BASE_OFFSET + 0x8C, 4),
...
int vpu_init_reg(struct vpu_core *vpu_core)
{
	...
	LOG_DBG("before cg_con(0x%x)\n",
		vpu_read_reg32(vpu_core->vpu_base, CTRL_BASE_OFFSET + 0x0));

	vpu_write_reg32(vpu_core->vpu_base, CTRL_BASE_OFFSET + 0x8,
				0xffffffff);
...
uint32_t vpu_read_field(uint64_t vpu_base, enum vpu_reg_field f)
{
...
void vpu_write_field(uint64_t vpu_base, enum vpu_reg_field f, uint32_t v)
{
	struct vpu_reg_desc *reg;
	struct vpu_reg_field_desc *field;
	uint8_t msb, lsb;
...
#else
	temp = vpu_read_reg32(vpu_base, reg->offset);
#endif
...
#else
	vpu_write_reg32(vpu_base, reg->offset, temp);
#endif
}
