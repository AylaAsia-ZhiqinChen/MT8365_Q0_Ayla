...
int mtk11_host_qmu_pipe_msk = (PIPE_ISOCHRONOUS + 1);
int mtk11_host_qmu_max_active_isoc_gpd;
int mtk11_host_qmu_number_pkts;
...
module_param(mtk11_host_qmu_number_pkts, int, 0644);
#endif
...
struct wakeup_source usb11_lock;
#endif
...
/*struct wakeup_source musbfsh_suspend_lock;*/
DEFINE_SPINLOCK(musbfs_io_lock);
/*-------------------------------------------------------------------------*/
#ifdef IC_USB
static ssize_t
show_start(struct device *dev, struct device_attribute *attr, char *buf)
{
			   ...
			   size_t size)
{
	...
	devctl = musbfsh_readb((unsigned char __iomem *)USB11_BASE,
		MUSBFSH_DEVCTL);
...
static
DEVICE_ATTR(start, 0644, show_start, store_start);
#endif
...
static irqreturn_t musbfsh_stage0_irq(struct musbfsh *musbfsh,
	u8 int_usb, u8 devctl, u8 power)
{
	...
	if (int_usb & MUSBFSH_INTR_VBUSERROR) {
		...
		if (musbfsh->vbuserr_retry) {
			...
			musbfsh->port1_status |= USB_PORT_STAT_OVERCURRENT |
...
static void musbfsh_shutdown(struct platform_device *pdev)
{
	...
	/* we prepare and enable usb clock first
	 * for avoiding schedule bug in stress tests
...
static int ep_config_from_table(struct musbfsh *musbfsh)
{
	...
	/* NOTE:  for RTL versions >= 1.400 EPINFO
	 * and RAMINFO would be better than static
	 * musbfsh->config->num_eps and DYN_FIFO_SIZE...
	 */
	...
	INFO("%s: %d/%d max ep, %d/%d memory\n",
	     musbfsh_driver_name, n + 1,
	     musbfsh->config->num_eps * 2 - 1,
	     offset, MAXFIFOSIZE);
...
void musbfsh_read_clear_generic_interrupt(struct musbfsh *musbfsh)
{
	...
	INFO("** musbfsh::IRQ! usb%04x tx%04x rx%04x dma%04x\n",
	     musbfsh->int_usb, musbfsh->int_tx,
	     musbfsh->int_rx, musbfsh->int_dma);
...
static irqreturn_t generic_interrupt(int irq, void *__hci)
{
	...
	INFO("musbfsh:generic interrupt++ line %d\r\n", __LINE__);
	spin_lock_irqsave(&musbfsh->lock, flags);
...
#ifdef CONFIG_MTK_MUSBFSH_QMU_SUPPORT
	if (musbfsh->int_usb || musbfsh->int_tx ||
		musbfsh->int_rx || musbfsh->int_queue)
...
irqreturn_t musbfsh_interrupt(struct musbfsh *musbfsh)
{
	 ...
	 */
	if (musbfsh->int_usb)
		retval |= musbfsh_stage0_irq(musbfsh,
					musbfsh->int_usb, devctl, power);
...
static void musbfsh_id_pin_work(struct work_struct *data)
{
	...
	if (!iddig_state) {
		...
		enable_irq(usb11_iddig_number);
		irq_set_irq_wake(usb11_iddig_number, 1);
		if (usb11_lock.active)
			__pm_relax(&usb11_lock);
	}
}
...
void musbfsh_iddig_int(void)
{
	...
	ret = request_irq(usb11_iddig_number, mt_usb11_iddig_int,
		IRQF_TRIGGER_LOW, "USB11_IDDIG", NULL);
	if (ret > 0)
...
static int
musbfsh_init_controller(struct device *dev, int nIrq,
...
#endif
{
...
#ifdef CONFIG_OF
	INFO("[Flow][USB11]%s:%d  base == 0x%lx ,phy_reg_base = 0x%lx\n",
	...
	if (request_irq(nIrq, musbfsh->isr,
			IRQF_TRIGGER_LOW, dev_name(dev), musbfsh)) {
...
static int musbfsh_probe(struct platform_device *pdev)
{
	...
	INFO("[USB11]probe reg:0x%lx phy:0x%lx pbase:0x%lx irq: %d\n",
	...
	wakeup_source_init(&usb11_lock, "USB11 suspend lock");
...
static void dump_op_state(const char *msg)
{
	...
	DBG(5, "\t0x%x = 0x%x\n", 0x620,
		musbfsh_readl(g_musbfsh->mregs, 0x620));

	for (i = 0; i < 4; i++)
		DBG(5, "\t0x%x = 0x%x\n", 0x640 + i * 4,
			musbfsh_readl(g_musbfsh->mregs, 0x640 + i * 4));
}
...
static void musbfsh_force_host_release_utmi(struct musbfsh *musbfsh)
{
	...
	/*Clear Test Mode */
	musbfsh_writeb(musbfsh->mregs, MUSBFSH_TESTMODE, 0x00);
...
static void musbfsh_save_context(struct musbfsh *musbfsh)
{
	...
	mtk11_qmu_ioc_setting =
		musbfsh_readl((musbfsh->mregs + MUSBFSH_QISAR), 0x30);
#endif
	...
	musbfsh->context.ep0_func_addr =
		musbfsh_readb(musbfsh_base, MUSBFSH_TXFUNCADDR);
...
static void musbfsh_restore_context(struct musbfsh *musbfsh)
{
	...
	musbfsh_writel((musbfsh->mregs + MUSBFSH_QISAR),
		0x30, mtk11_qmu_ioc_setting);
#endif

	musbfsh_writeb(musbfsh_base, MUSBFSH_TXFUNCADDR,
		musbfsh->context.ep0_func_addr);

	devctl = musbfsh_readb(musbfsh_base, MUSBFSH_DEVCTL);
	/* we don't restore DEVCTL:Bit0 Session */
	musbfsh_writeb(musbfsh_base, MUSBFSH_DEVCTL,
		((musbfsh->context.devctl) & 0xFE) | (devctl & 0x01));
	...
	musbfsh_writew(musbfsh_base, MUSBFSH_INTRTXE,
		musbfsh->context.intrtxe);
	musbfsh_writew(musbfsh_base, MUSBFSH_INTRRXE,
		musbfsh->context.intrrxe);
	musbfsh_writeb(musbfsh_base, MUSBFSH_INTRUSBE, MUSBFSH_INTR_CONNECT |
		MUSBFSH_INTR_BABBLE | MUSBFSH_INTR_SUSPEND |
		MUSBFSH_INTR_SESSREQ | MUSBFSH_INTR_VBUSERROR |
		MUSBFSH_INTR_DISCONNECT | MUSBFSH_INTR_RESUME);
	reg32 = musbfsh_readl(musbfsh_base, MUSBFSH_HSDMA_INTR);
...
static void musbfsh_restore_context_default(struct musbfsh *musbfsh)
{
	...
	musbfsh_writel((musbfsh->mregs + MUSBFSH_QISAR),
		0x30, mtk11_qmu_ioc_setting);
#endif
	musbfsh_writeb(musbfsh_base, MUSBFSH_POWER,
		musbfsh->context.power);
	musbfsh_writew(musbfsh_base, MUSBFSH_INTRTXE,
		musbfsh->context.intrtxe);
	musbfsh_writew(musbfsh_base, MUSBFSH_INTRRXE,
		musbfsh->context.intrrxe);
