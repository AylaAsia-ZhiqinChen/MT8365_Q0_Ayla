...
void
musbfshdebugfs_h_setup(struct usb_ctrlrequest *setup, struct musbfsh *mtk_musb)
{
	unsigned short csr0;
...
static unsigned int
my_strtoul(const char *nptr, char **endptr, unsigned int base)
{
	const char *s = nptr;
	unsigned long acc;
	int c;
	...
	const char __user *ubuf, size_t count, loff_t *ppos)
{
	...
	if (is_mac == 1) {
		if (offset >= 0x1000) {
			DBG(0, "Write Mac adddr failed! too big:0x%x\n",
				offset);
			return count;
		}
		...
		if (offset >= 0x100) {
			DBG(0, "Write Phy adddr failed! too big:0x%x\n",
				offset);
			return count;
		}
	...
	const char __user *ubuf, size_t count, loff_t *ppos)
{
	...
	if (is_mac == 1) {
		if (offset >= 0x1000) {
			DBG(0, "Read Mac adddr failed! too big:0x%x\n",
				offset);
			return count;
		}
		...
		if (offset >= 0x100) {
			DBG(0, "Read Phy adddr failed! too big:0x%x\n",
				offset);
			return count;
		}
...
int musbfsh_init_debugfs(struct musbfsh *musbfsh)
{
	...
	file = debugfs_create_file("regdump", 0444, root,
		musbfsh, &musbfsh_regdump_fops);
	if (!file) {
		ret = -ENOMEM;
		goto err1;
	}
	...
	file = debugfs_create_file("regw", 0644,
		root, musbfsh, &musbfsh_regw_fops);
	if (!file) {
		ret = -ENOMEM;
		goto err1;
	}
	...
	file = debugfs_create_file("regr", 0644,
		root, musbfsh, &musbfsh_regr_fops);
	if (!file) {
		ret = -ENOMEM;
		goto err1;
	}
