...
int jpeg_isr_enc_lisr(void)
{
	...
	IMG_REG_READ(tmp1, REG_ADDR_JPEG_ENC_INTERRUPT_STATUS);
...
unsigned int jpeg_drv_enc_set_src_image(unsigned int width,
		unsigned int height, unsigned int yuv_format,
...
unsigned int jpeg_drv_enc_set_src_buf(unsigned int yuv_format,
		unsigned int img_stride, unsigned int mem_stride,
		unsigned int srcAddr, unsigned int srcAddr_C)
{
	unsigned int ret = 1;
	...
	if (yuv_format == 0x00 || yuv_format == 0x01) {
		if ((mem_stride & 0x1f) || (img_stride & 0x1f)) {
			JPEG_MSG
			("JPEGENC: yuv422, but img stride is not align(32)\n");
			JPEG_MSG("img stride: 0x%x, mem stride: 0x%x\n",
...
void jpeg_drv_enc_start(void)
{
	...
	IMG_REG_READ(u4Value, REG_ADDR_JPEG_ENC_CTRL);
...
unsigned int jpeg_drv_enc_warm_reset(void)
{
	...
	IMG_REG_READ(value, REG_ADDR_JPEG_ENC_CTRL);
	value &= ~JPEG_ENC_CTRL_ENABLE_BIT;
	IMG_REG_WRITE(value, REG_ADDR_JPEG_ENC_CTRL);

	IMG_REG_READ(value, REG_ADDR_JPEG_ENC_CTRL);
	value |= JPEG_ENC_CTRL_ENABLE_BIT;
	IMG_REG_WRITE(value, REG_ADDR_JPEG_ENC_CTRL);

	IMG_REG_READ(value, REG_ADDR_JPEG_ENC_DEBUG_INFO0);
	while (0 == (value & JPEG_ENC_DEBUG_INFO0_GMC_IDLE_MASK)) {
		timeout--;
		if (timeout == 0) {
	...
	IMG_REG_READ(value, REG_ADDR_JPEG_ENC_RSTB);
	value &= ~JPEG_ENC_RST_BIT;
	IMG_REG_WRITE(value, REG_ADDR_JPEG_ENC_RSTB);
...
unsigned int jpeg_drv_enc_set_quality(unsigned int quality)
{
	...
	IMG_REG_READ(u4Value, REG_ADDR_JPEG_ENC_QUALITY);
	u4Value = (u4Value & 0xFFFF0000) | quality;
	IMG_REG_WRITE((u4Value), REG_ADDR_JPEG_ENC_QUALITY);
	return 1;
}

unsigned int jpeg_drv_enc_set_img_size(unsigned int width,
		unsigned int height)
{
	unsigned int u4Value;
...
unsigned int jpeg_drv_enc_set_luma_addr(unsigned int src_luma_addr)
{
	if (src_luma_addr & 0x0F)
		JPEG_MSG("JPEGENC: LUMA addr not align (%x)!\n", src_luma_addr);
...
unsigned int jpeg_drv_enc_set_chroma_addr(unsigned int src_chroma_addr)
{
	if (src_chroma_addr & 0x0F)
		JPEG_MSG("JPEGENC: CHROMA addr not align (%x)!\n",
			src_chroma_addr);
...
{
	if (mem_stride & 0x0F) {
		JPEG_MSG("JPEGENC: memory stride not align to 16(%x)!\n",
			 mem_stride);
		return 0;
	}
...
{
	if (img_stride & 0x0F) {
		JPEG_MSG("JPEGENC: image stride not align to 16 (%x)!!\n",
			 img_stride);
		return 0;
	}
...
unsigned int jpeg_drv_enc_set_dst_buff(unsigned int dst_addr,
		unsigned int stall_size, unsigned int init_offset,
		unsigned int offset_mask)
{
	if (stall_size < 624) {
		JPEG_MSG("JPEGENC:stall size %d is less than 624!\n",
			stall_size);
		return 0;
	}
...
unsigned int jpeg_drv_enc_get_cycle_count(void)
{
	...
	IMG_REG_READ(u4Value, REG_ADDR_JPEG_ENC_TOTAL_CYCLE);
...
unsigned int jpeg_drv_enc_get_file_size(void)
{
	...
	IMG_REG_READ(u4StartAddr, REG_ADDR_JPEG_ENC_DST_ADDR0);
	IMG_REG_READ(u4EndAddr, REG_ADDR_JPEG_ENC_DMA_ADDR0);

	u4Size = u4EndAddr - u4StartAddr;
	return u4Size;
}
