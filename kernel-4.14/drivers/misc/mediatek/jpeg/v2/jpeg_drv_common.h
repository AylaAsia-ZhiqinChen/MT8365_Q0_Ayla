...
struct _JpegDrvEncDstCfg {
	/* 0:JPG mode, 1:JFIF/EXIF mode */
	unsigned int exif_en;
...
struct _JpegDrvEncCtrlCfg {
	unsigned int quality;
	unsigned int restart_interval;
	/* HW not support */
	unsigned int gmc_disable;
};

/* JPEG Driver Decoder */
void jpeg_drv_dec_power_on(void);
void jpeg_drv_dec_power_off(void);
unsigned int jpeg_drv_dec_set_config_data
	(struct _JPEG_DEC_DRV_IN *config);
...
int jpeg_drv_dec_wait(struct _JPEG_DEC_DRV_IN *config);
...
unsigned int jpeg_drv_enc_src_cfg(struct _JpegDrvEncSrcCfg srcCfg);
unsigned int jpeg_drv_enc_dst_buff(struct _JpegDrvEncDstCfg dstCfg);
...
unsigned int jpeg_drv_enc_set_sample_format_related(unsigned int width,
	unsigned int height,
	unsigned int yuv_format);
