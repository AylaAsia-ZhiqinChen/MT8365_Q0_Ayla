...
#define CHECK_ALIGN(value, align, addr)  (\
	if (value & (align-1)) \
		JPEG_WRN("WriteREG: write REG 0x%x without align!!\n", addr); \
		JPEG_WRN("WriteREG: value: %d, align: %d\n", value, align); \
)
...
int jpeg_isr_dec_lisr(void)
{
	...
	IMG_REG_READ(tmp1, REG_ADDR_JPGDEC_INTERRUPT_STATUS);
	tmp = tmp1 & BIT_INQST_MASK_ALLIRQ;

	if (tmp) {
		_jpeg_dec_int_status = tmp;

		/* error handling, since irq can't be cleared by register.
		 * we need to mask the irq out
		 */
		if (((_jpeg_dec_int_status & BIT_INQST_MASK_ERROR_BS)
		...
		if (_jpeg_dec_mode == 1) {
			...
			/* always not clear, but mask jpeg interrupt temporarily
			 * when EOF irq have not rise yetthis is workaround for
			 * HW decoder ROW mode since clear pause irq will
			 * trigger HW to fire we need to disable jpeg irq
			 * before resume to avoid jpeg irq rise too frequenct
			 */
			if (((_jpeg_dec_int_status & BIT_INQST_MASK_EOF)
				...
				IMG_REG_WRITE(0x0, REG_ADDR_JPGDEC_IRQ_EN);
			/* clear jpeg decode interrupt if it is not the
			 * situiation that only pause irq was raised
			 */
			else
				IMG_REG_WRITE
				(tmp, REG_ADDR_JPGDEC_INTERRUPT_STATUS);
#else
...
void jpeg_drv_dec_start(void)
{
	...
	JPEG_MSG("JPGDC %s: JPEG_DEC_IRQ_ENHANCE!\n", __func__);
...
void jpeg_drv_dec_warm_reset(void)
{
	...
	IMG_REG_READ(int_status, REG_ADDR_JPGDEC_INTERRUPT_STATUS);
	...
	if (!(int_status & BIT_INQST_MASK_EOF)) {
		...
		/* dsiable GMC */
		IMG_REG_WRITE
		((value & 0xBFFFFFFF), REG_ADDR_JPGDEC_DU_CTRL);

		/* wait GMC idle */
		i = 5000;
		IMG_REG_READ(value, REG_ADDR_JPGDEC_STATUS);
		while (i > 0) {
		...
		if (i == 0) {
			...
			unsigned int u4DuCtrl;
			unsigned int u4Debug1;
			unsigned int u4SMIDebug0, u4SmiDebug1;
			unsigned int u4SMIDebug2, u4SmiDebug3;
			/* dump key register */
			JPEG_WRN("[JPEGDRV] jpeg_dec_reset timeout!!\n");
			IMG_REG_READ(u4Status, REG_ADDR_JPGDEC_STATUS);
			IMG_REG_READ(u4DuCtrl, REG_ADDR_JPGDEC_DU_CTRL);
			IMG_REG_READ(u4Debug1, REG_ADDR_JPGDEC_DEBUG1);
			JPEG_WRN
			("[JPEGDRV] 0x278 = 0x%x, 0x23c = 0x%x, 0x184 = 0x%x\n",
			u4Status, u4DuCtrl, u4Debug1);

			IMG_REG_READ(u4SMIDebug0, REG_ADDR_JPGDEC_SMI_DEBUG0);
			IMG_REG_READ(u4SmiDebug1, REG_ADDR_JPGDEC_SMI_DEBUG1);
			JPEG_WRN("[JPEGDRV] 0x304 = 0x%x, 0x308 = 0x%x\n",
				u4SMIDebug0, u4SmiDebug1);

			IMG_REG_READ(u4SMIDebug2, REG_ADDR_JPGDEC_SMI_DEBUG2);
			IMG_REG_READ(u4SmiDebug3, REG_ADDR_JPGDEC_SMI_DEBUG3);
			JPEG_WRN
			("[JPEGDRV]0x30c = 0x%x, 0x310 = 0x%x\n",
			u4SMIDebug2, u4SmiDebug3);
		}
	}
...
unsigned int  jpeg_drv_dec_set_brz_factor(unsigned char yHScale,
		unsigned char yVScale,
		unsigned char cbcrHScale,
...
unsigned int jpeg_drv_dec_set_dec_mode(int i4DecMode)
{
	...
	/* 0: full frame, 1: direct couple mode,
	 * 2: pause/resume mode, 3: Reserved
	 */
...
void jpeg_drv_dec_set_debug_mode(void)
{
	...
	IMG_REG_READ(u4Value, REG_ADDR_JPGDEC_DEBUG_MODE);
	u4Value |= 0x80000000;
...
unsigned int jpeg_drv_dec_set_bs_info(unsigned int bsBase,
		unsigned int bsSize)
{
...
{
#if 0
	/* u4Membership = u4Membership | 0x3FFFFF24 ;
	 * //111_111_111_111_111_111_111_100_100_100
	 * u4Membership = u4Membership | 0x24924924;
	 * 100_100_100_100_100_100_100_100_100_100
	 */
#endif
...
unsigned int jpeg_drv_dec_get_irqState(void)
{
	...
	IMG_REG_READ(u4Value, REG_ADDR_JPGDEC_INTERRUPT_STATUS);
...
unsigned int jpeg_drv_dec_get_decState(void)
{
	...
	IMG_REG_READ(u4Value, REG_ADDR_JPGDEC_STATUS);
...
unsigned int jpeg_drv_dec_get_decMCU(void)
{
	...
	IMG_REG_READ(u4Value, REG_ADDR_JPGDEC_MCU_CNT);
	JPEG_MSG("JPED_DEC_DRV: MCU_CNT %x!!\n", u4Value);
	return u4Value;
}

unsigned int jpeg_drv_dec_set_dma_group(unsigned int McuInGroup,
		unsigned int GroupNum, unsigned int LastMcuNum)
{
...
unsigned int jpeg_drv_dec_set_sampling_factor(unsigned int compNum,
		unsigned int u4Y_H, unsigned int u4Y_V,
		...
		unsigned int u4V_H, unsigned int u4V_V)
{
	...
	if (compNum > 3 || (u4Y_H > 4 || u4Y_V > 4 ||
		u4U_H > 4 || u4U_V > 4 || u4V_H > 4 ||
		u4V_V > 4) || (u4Y_H == 3 || u4Y_V == 3 ||
		u4U_H == 3 || u4U_V == 3 || u4V_H == 3 || u4V_V == 3)) {
		return 0;
	}
...
unsigned int jpeg_drv_dec_set_config_data(JPEG_DEC_DRV_IN *config)
{
	...
	/* set BRZ factor */
	ret &= jpeg_drv_dec_set_brz_factor(config->lumaHorDecimate,
			config->lumaVerDecimate,
			config->cbcrHorDecimate, config->cbcrVerDecimate);

	/* set group DMA */
	ret &= jpeg_drv_dec_set_dma_group(config->dma_McuInGroup,
			config->dma_GroupNum, config->dma_LastMcuNum);

	/* set componet ID */
	ret &= jpeg_drv_dec_set_comp_id(config->componentID[0],
			config->componentID[1], config->componentID[2]);


	/* set BLK membership */
	ret &= jpeg_drv_dec_set_du_membership(config->membershipList,
			config->gmcEn, (config->componentNum == 1) ? 1 : 0);

	/* set q table id */
	ret &= jpeg_drv_dec_set_q_table(config->qTableSelector[0],
			config->qTableSelector[1], config->qTableSelector[2]);

	/* set dst image stride  */
	ret &= jpeg_drv_dec_set_imgStride(config->compImgStride[0],
			config->compImgStride[1]);

	/* set dst Memory stride
	 * if( config->pauseRow_en ){
	 * jpeg_drv_dec_set_memStride(config->compTileBufStride[0],
	 * config->compTileBufStride[1]);
	 * }else{
	 * ret &= jpeg_drv_dec_set_memStride(config->compMemStride[0],
	...
	JPEG_MSG
	("[JPEGDRV] mode %d, Buf Base 0x%08x, Limit 0x%08x, Size 0x%08x!!\n",
		config->reg_OpMode, config->srcStreamAddrBase,
		config->srcStreamAddrWritePtr, config->srcStreamSize);
	ret &= jpeg_drv_dec_set_bs_info(config->srcStreamAddrBase,
	...
	/* output bank 0 */
	ret &= jpeg_drv_dec_set_dst_bank0(config->outputBuffer0[0],
		config->outputBuffer0[1], config->outputBuffer0[2]);

	/* output bank 1 */
	ret &= jpeg_drv_dec_set_dst_bank1(config->outputBuffer1[0],
		config->outputBuffer1[1], config->outputBuffer1[2]);
...
void jpeg_drv_dec_resume(unsigned int resume)
{
	...
	/* unmask jpeg interrupt since
	 * we've masked it out when pause irq rise
	 */
	IMG_REG_WRITE(0x37, REG_ADDR_JPGDEC_IRQ_EN);
#else
...
int jpeg_drv_dec_wait_one_row(JPEG_DEC_DRV_IN *config)
{
	...
	{
		IMG_REG_READ(irq_status, REG_ADDR_JPGDEC_INTERRUPT_STATUS);
		/* wait done */
		if (config->decodeMode == JPEG_DEC_MODE_MCU_ROW) {
			while ((irq_status & BIT_INQST_MASK_ALLIRQ) == 0) {
				timeout--;
				if (timeout == 0)
					break;
				IMG_REG_READ(irq_status,
		...
		JPEG_MSG("JPEG_DEC_WAIT_DONE\n");
		JPEG_MSG
		("tri_cnt %d, irq %x, MCUinRow %d, p_idx %d, %x %x %x!!\n",
...
int jpeg_drv_dec_break(void)
{
	...
	IMG_REG_READ(u4Status, REG_ADDR_JPGDEC_STATUS);
	while (((u4Status & BIT_DEC_ST_STATE_MASK)
