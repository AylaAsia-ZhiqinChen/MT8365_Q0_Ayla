...
#endif//#ifdef CONFIG_MTK_SMI_EXT
...
void jpeg_drv_enc_power_on(void)
{
	...
	if (clk_prepare_enable(gJpegClk.clk_venc_larb))
		JPEG_ERR("enable clk_venc_larb fail!");
#endif
#endif//#ifdef CONFIG_MTK_SMI_EXTs
#endif//#ifdef JPEG_PM_DOMAIN_ENABLE
...
static int jpeg_drv_dec_init(void)
{
	...
	if (dec_status != 0) {
		JPEG_WRN("JPEG DEC HW is busy! %s\n", __func__);
		retValue = -EBUSY;
	} else {
		dec_status = 1;
		dec_ready = 0;
...
static int jpeg_drv_enc_init(void)
{
	...
	if (enc_status != 0) {
		JPEG_WRN("JPEG ENC HW is busy! %s\n", __func__);
		retValue = -EBUSY;
	} else {
		enc_status = 1;
		enc_ready = 0;
			...
			struct file *file)
{
	...
	if (pStatus == NULL) {
		JPEG_MSG
			("[JPEGDRV]Decoder: Private data is null\n");
		return -EFAULT;
	}
	switch (cmd) {
	...
	case JPEG_DEC_IOCTL_CONFIG:
		if (*pStatus != JPEG_DEC_PROCESS) {
			JPEG_MSG
			("[JPEGDRV]This process can not access decoder\n");
			return -EFAULT;
		}
		if (dec_status == 0) {
		...
		if (copy_from_user(&dec_params, (void *)arg,
			sizeof(JPEG_DEC_DRV_IN))) {
			JPEG_MSG
			("[JPEGDRV]JPEG Decoder : Copy from user error\n");
			return -EFAULT;
		}
	...
	case JPEG_DEC_IOCTL_FLUSH_CMDQ:
		...
		if (copy_from_user(&cfg_cmdq_params, (void *)arg,
			sizeof(JPEG_DEC_CONFIG_CMDQ))) {
			JPEG_MSG("[JPEGDRV]Copy from user error\n");
			return -EFAULT;
		}
		...
		for (i = 0; i < cfg_cmdq_params.goNum; i++) {
			JPEG_MSG("[JPEGDRV]JPEG gen CMDQ %x %x %x %x  !!\n",
				 ...
				 cfg_cmdq_params.decRowBuf0[i],
				 cfg_cmdq_params.decRowBuf1[i],
				 cfg_cmdq_params.decRowBuf2[i]);
			cmdqRecWrite(jpegCMDQ_handle,
			0x18004170 /*REG_ADDR_JPGDEC_PAUSE_MCU_NUM */,
			cfg_cmdq_params.pauseMCUidx[i] - 1, 0xFFFFFFFF);
			/*REG_ADDR_JPGDEC_DEST_ADDR0_Y */
			cmdqRecWrite(jpegCMDQ_handle, 0x18004140,
				cfg_cmdq_params.decRowBuf0[i], 0xFFFFFFFF);
			/*REG_ADDR_JPGDEC_DEST_ADDR0_U */
			cmdqRecWrite(jpegCMDQ_handle, 0x18004144,
				cfg_cmdq_params.decRowBuf1[i], 0xFFFFFFFF);
			/*REG_ADDR_JPGDEC_DEST_ADDR0_V */
			cmdqRecWrite(jpegCMDQ_handle, 0x18004148,
	...
	case JPEG_DEC_IOCTL_RESUME:
		...
		if (copy_from_user(&dec_row_params, (void *)arg,
		...
		JPEG_MSG
		("[JPEGDRV][IOCTL] JPEG Decoder Resume, [%d] %x %x %x !!\n",
			 dec_row_params.pauseMCU - 1,
			 dec_row_params.decRowBuf[0],
		...
		if (!jpeg_drv_dec_set_pause_mcu_idx
			(dec_row_params.pauseMCU - 1))
			return -EFAULT;
	...
	case JPEG_DEC_IOCTL_START:	/* OT:OK */
		...
		if (dec_status == 0 || dec_ready == 0) {
			JPEG_WRN("Decoder status is not ready\n");
			*pStatus = 0;
			return -EFAULT;
		}
	...
	case JPEG_DEC_IOCTL_WAIT:
		...
		if (copy_from_user(&outParams, (void *)arg,
		...
		/* JPEG_MSG("[JPEGDRV][IOCTL] JPEG Decoder
		 * Wait Resume Time Jiffies : %ld\n", timeout_jiff);
		 */
#ifdef FPGA_VERSION
		JPEG_MSG("[JPEGDRV]Polling JPEG Status");

		do {
			IMG_REG_READ(_jpeg_dec_int_status,
				REG_ADDR_JPGDEC_INTERRUPT_STATUS);
		...
		if (jpeg_isr_dec_lisr() < 0) {
			...
			do {
				...
				(dec_wait_queue, _jpeg_dec_int_status,
							 timeout_jiff);
				if (ret == 0)
					JPEG_MSG
					("[JPEGDRV]JPEG Decoder timeout !!\n");
			} while (ret < 0);
		} else
			JPEG_MSG
			("[JPEGDRV][IOCTL] JPEG Decoder Wait IRQ Done!!\n");
#endif
		...
		if (copy_to_user(outParams.result, &decResult,
			sizeof(unsigned int))) {
			JPEG_WRN("Copy to user error (result)\n");
			return -EFAULT;
		}
	...
	case JPEG_DEC_IOCTL_DEINIT:
		...
		if (dec_status == 0) {
			JPEG_ERR("Dec status is not ready\n");
			*pStatus = 0;
			return -EFAULT;
		}
		jpeg_drv_dec_deinit();
		...
		struct file *file)
{
	...
	struct _JPEG_ENC_DRV_IN cfgEnc;

	struct _JPEG_ENC_DRV_OUT enc_result;

	 pStatus = (unsigned int *)file->private_data;

	if (pStatus == NULL) {
		JPEG_WRN("Private data is null in flush operation.\n");
		return -EFAULT;
	}
	...
	case JPEG_ENC_IOCTL_CONFIG:
		JPEG_MSG("[JPEGDRV][IOCTL] JPEG_ENC_IOCTL_CONFIG\n");
		if (*pStatus != JPEG_ENC_PROCESS) {
		...
		if (enc_status == 0) {
			JPEG_WRN("Encoder status is not ready.\n");
			*pStatus = 0;
			return -EFAULT;
		}
		...
		if (copy_from_user(&cfgEnc, (void *)arg,
			sizeof(struct _JPEG_ENC_DRV_IN))) {
			JPEG_MSG
			("[JPEGDRV]JPEG Encoder : Copy from user error\n");
			return -EFAULT;
		}
		...
		if (ret == 0) {
			JPEG_MSG
			("[JPEGDRV]JPEG Encoder set srouce image failed\n");
			return -EFAULT;
		}
		...
		if (ret == 0) {
			JPEG_MSG
			("[JPEGDRV]JPEG Encoder set srouce buffer failed\n");
			return -EFAULT;
		}
		...
		JPEG_MSG
		("[JPEGDRV]DST_BUF: addr:%x, size:%x, ofs:%x, mask:%x!!\n",
		...
		if (ret == 0) {
			JPEG_MSG
			("[JPEGDRV]JPEG Encoder set dst buffer failed\n");
			return -EFAULT;
		}
	...
	case JPEG_ENC_IOCTL_WAIT:
		...
		if (copy_from_user(&enc_result, (void *)arg,
			sizeof(struct _JPEG_ENC_DRV_OUT))) {
	...
	#ifdef FPGA_VERSION
		do {
			IMG_REG_READ(_jpeg_enc_int_status,
		...
		if (jpeg_isr_enc_lisr() < 0) {
			do {
				ret = wait_event_interruptible_timeout
					(enc_wait_queue, _jpeg_enc_int_status,
					timeout_jiff);
				if (ret > 0)
					JPEG_MSG
					("[JPEGDRV]JPEG Encoder done !!\n");
				else if (ret == 0)
					JPEG_MSG
					("[JPEGDRV]JPEG Encoder timeout !!\n");
			} while (ret < 0);
		} else
		...
		if (copy_to_user(enc_result.fileSize,
			...
			JPEG_MSG
			("[JPEGDRV]JPEG Encoder : Copy file size error\n");
			return -EFAULT;
		}
		if (copy_to_user(enc_result.result,
			&ret, sizeof(unsigned int))) {
			JPEG_MSG
			("[JPEGDRV]JPEG Encoder : Copy status error\n");
			return -EFAULT;
		}
		if (copy_to_user(enc_result.cycleCount,
			&cycle_count, sizeof(unsigned int))) {
			JPEG_MSG
			("[JPEGDRV]JPEG Encoder : Copy cycle error\n");
			return -EFAULT;
		}
		break;
...
static int compat_get_jpeg_dec_ioctl_wait_data
		(struct _compat_JPEG_DEC_DRV_OUT __user *data32,
		struct _JPEG_DEC_DRV_OUT __user *data)
{
...
static int compat_get_jpeg_dec_ioctl_chksum_data
		(struct _compat_JpegDrvDecResult __user *data32,
		struct _JpegDrvDecResult __user *data)
{
...
static int compat_get_jpeg_enc_ioctl_wait_data
		(struct _compat_JPEG_ENC_DRV_OUT __user *data32,
		struct _JPEG_ENC_DRV_OUT __user *data)
{
	compat_long_t timeout;
...
static long compat_jpeg_ioctl(struct file *filp,
	...
	case COMPAT_JPEG_DEC_IOCTL_WAIT:
		{
			...
			err = compat_get_jpeg_dec_ioctl_chksum_data(data32,
					data);
			if (err)
				return err;
			ret =
			    filp->f_op->unlocked_ioctl(filp,
					...
					(unsigned long)data);
			err = compat_put_jpeg_dec_ioctl_chksum_data(data32,
					data);
			return ret ? ret : err;
		}
 ...
 * unsigned int cmd, unsigned long arg)
 */
static long jpeg_unlocked_ioctl(struct file *file,
		unsigned int cmd, unsigned long arg)
{
	switch (cmd) {
#ifdef JPEG_DEC_DRIVER
...
static ssize_t jpeg_read(struct file *file,
...
static int jpeg_flush(struct file *a_pstFile, fl_owner_t a_id)
{
	...
	if (*pStatus == JPEG_ENC_PROCESS) {
		if (enc_status != 0) {
			JPEG_WRN
			("Error! Enable error handling for jpeg encoder");
			jpeg_drv_enc_deinit();
		}
	}
#ifdef JPEG_DEC_DRIVER
		...
		if (dec_status != 0) {
			JPEG_WRN
			("Error! Enable error handling for jpeg decoder");
			jpeg_drv_dec_deinit();
		}
	}
#endif
...

{
	...
	class_dev =
		(struct class_device *)device_create(jenc_class, NULL,
			jenc_devno, NULL, JPEG_DEVNAME);
#else
	...
	if (request_irq(gJpegqDev.decIrqId, jpeg_drv_dec_isr,
			IRQF_TRIGGER_FALLING, "jpeg_dec_driver", NULL))
...
static int jdec_probe(struct platform_device *pdev)
{
	...
	if (ret)
		JPEG_MSG("Error: Can't Get Majornumber for %s Device\n",
			JDEC_DEVNAME);
	else
		JPEG_MSG("Get %s Device Majornumber (%d)\n",
...
static int __init jpeg_init(void)
{
	...
	cmdqCoreRegisterCB(CMDQ_GROUP_JPEG,
			   cmdqJpegClockOn, cmdqJpegDumpInfo,
			   cmdqJpegResetEng, cmdqJpegClockOff);
#endif
	return 0;
}
