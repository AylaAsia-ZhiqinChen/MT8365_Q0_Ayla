...
/*Configurations*/
/*==============*/
/* 1: turn on RT kthread for
 * thermal protection in this sw module;
 * 0: turn off
 */
...
/* 1: thermal driver update temp to MET directly, use hrtimer;
 * 0: turn off
 */
...
#define thermal_readl(addr)\
	DRV_Reg32(addr)
#define thermal_writel(addr, val)\
...
static void set_static_gpu_power_limit(unsigned int limit)
{
	...
	final_limit = MIN(adaptive_gpu_power_limit,
...
/* TODO: We also need a pair of setting
 * functions for GPU power limit,
 * which is not supported on MT6582.
 */
#if 0
...
static void thermal_reset_and_initial(void)
{
	 ...
	 * 4*15.15ns* 256 = 15513.6 ms=15.5us
	 */
...
/* both filt and sen interval is 2016*15.5us = 31.25ms */
	THERMAL_WRAP_WR32(0x00000D79, TEMPAHBPOLL);
...
/* both filt and sen interval is 6.94ms */
	THERMAL_WRAP_WR32(0x000002FE, TEMPAHBPOLL);
/* poll is set to 6.94ms */
...
/* both filt and sen interval is 12.5ms */
	THERMAL_WRAP_WR32(0x00000563, TEMPAHBPOLL);
...
#if 0
static void set_tc_trigger_hw_protect_test(
	int temp_hot, int temp_normal, int temp_low, int loffset)
{
	int temp = 0;
	int offset = 0;
	...
	tscpu_dprintk
		("%s temp_hot=%d, temp_normal=%d, temp_low=%d, loffset=%d\n",
		 __func__, temp_hot, temp_normal, temp_low, loffset);
	...
	tscpu_dprintk(
		"%s 1 TEMPMONINT:temp=0x%x\n", __func__,
		temp);


	THERMAL_WRAP_WR32(raw_high, TEMPHTHRE);	/* set hot threshold */
	/* THERMAL_WRAP_WR32(raw_high_offset, TEMPOFFSETH);
	 * set high offset threshold
	 */
	THERMAL_WRAP_WR32(raw_hot_normal, TEMPH2NTHRE);
	/* set hot to normal threshold */
	/* THERMAL_WRAP_WR32(raw_low_offset,  TEMPOFFSETL);
	 * set low offset threshold
	 */
	...
	tscpu_dprintk(
	"%s 2 TEMPMONINT:temp=0x%x\n", __func__,
	temp);

}
#endif
#if 0				/* test int */
static void set_thermal_ctrl_trigger_SPM(
 ...
 *  for interrupting CPU. -275000 to disable it.
 */
...
static void set_tc_trigger_hw_protect(int temperature, int temperature2)
{
	...
	temp = DRV_Reg32(TEMPMONINT);
	/* tscpu_dprintk(
	 * "set_tc_trigger_hw_protect 1 TEMPMONINT:temp=0x%x\n",
	 * temp);
	 */
	/* THERMAL_WRAP_WR32(temp & 0x1FFFFFFF, TEMPMONINT);
	 * disable trigger SPM interrupt
	...
	/* tscpu_dprintk(
	 * "set_tc_trigger_hw_protect 2 TEMPMONINT:temp=0x%x\n",
	 * temp);
	 */
...
int mtk_cpufreq_register(struct mtk_cpu_power_info *freqs, int num)
{
	...
	for (i = 0; i < num; i++) {
		 ...
		 */
		int cl_id = (((freqs[i].cpufreq_power + gpu_power) + 99) /
						100) - 7;
...
static int init_cooler(void)
{
	...
	cl_dev_state = kzalloc((num) * sizeof(unsigned int),
					GFP_KERNEL);
	...
	/* using index=>0=200,1=800 ~ 33=3500 */
...
static void thermal_cal_prepare(void)
{
	...
	tscpu_dprintk(
		"[calibration] devinfo id 60=0x%x, id 61=0x%x, id 62=0x%x\n",
		temp1, temp0, temp2);
	/* mtktscpu_dprintk("thermal_cal_prepare\n"); */

	g_adc_ge_t = ((temp0 & 0xFFC00000) >> 22);
	/* ADC_GE_T    [9:0] *(0x10009184)[31:22] */

	g_adc_oe_t = ((temp0 & 0x003FF000) >> 12);
	/* ADC_OE_T    [9:0] *(0x10009184)[21:12] */

	g_o_vtsmcu1 = (temp1 & 0x03FE0000) >> 17;
	/* O_VTSMCU1    (9b) *(0x10009180)[25:17] */
	g_o_vtsmcu2 = (temp1 & 0x0001FF00) >> 8;
	/* O_VTSMCU2    (9b) *(0x10009180)[16:8] */
	g_o_vtsmcu3 = (temp2 & 0xFF800000) >> 23;
	/* O_VTSMCU3    (9b) *(0x10009188)[31:23] */
	g_o_vtsmcu4 = (temp2 & 0x007FC000) >> 14;
	/* O_VTSMCU4    (9b) *(0x10009188)[22:14] */
	g_o_vtsabb  = (temp0 & 0x00000FF8) >> 3;
	/* O_VTSABB     (9b) *(0x10009184)[11:3] */

	g_degc_cali = (temp1 & 0x0000007E) >> 1;
	/* DEGC_cali    (6b) *(0x10009180)[6:1] */
	g_adc_cali_en_t = (temp1 & 0x00000001);
	/* ADC_CALI_EN_T(1b) *(0x10009180)[0] */
	g_o_slope_sign = (temp1 & 0x00000080) >> 7;
	/* O_SLOPE_SIGN (1b) *(0x10009180)[7] */
	g_o_slope = (temp1 & 0xFC000000) >> 26;
	/* O_SLOPE      (6b) *(0x10009180)[31:26] */

	g_id = (temp0 & 0x00000004) >> 2;
	/* ID    (1b) *(0x10009184)[2] */
...
static s32 temperature_to_raw_room(u32 ret)
{
	/* Ycurr = [(Tcurr - DEGC_cali/2)*(165+O_slope)*
	 *			(18/15)*(1/10000)+
	...
	/* tscpu_dprintk(
	 * "[Temperature_to_raw_roomt] temperature=%d, raw[%d]=%d",
...
static s32 raw_to_temperature_roomt(
	u32 ret, enum thermal_sensor_name ts_name)
{
	s32 t_current = 0;
	s32 y_curr = ret;
	s32 format_1 = 0;
	...
	format_3 = (((((format_2) * 10000) >> 12) * 10000) / g_gain)
				- xtoomt;
	...
	/* tscpu_dprintk(
	 * "raw_to_temperature_room,t_current=%d\n",
	 * t_current);
	 */
	return t_current;
}
...
static int read_tc_raw_and_temp(
	u32 *tempmsr_name, enum thermal_sensor_name ts_name,
		int *ts_raw)
{
	int temp = 0, raw = 0;

	tscpu_dprintk(
		"read_tc_raw_temp,tempmsr_name=0x%x,ts_num=%d\n",
...
static int tscpu_get_temp(struct thermal_zone_device *thermal, int *t)
{
	...
	*t = (unsigned long)curr_temp;
	...
	if (isTimerCancelled) {
		...
		tscpu_printk("%s isTimerCancelled=%d!\n",
			__func__, isTimerCancelled);
...
/* 1. MINIMUM_BUDGET_CHANGE = 0 ==>
 * thermal equilibrium maybe at higher than TARGET_TJ_HIGH
 */
/* 2. Set MINIMUM_BUDGET_CHANGE > 0
 * if to keep Tj at TARGET_TJ
 */
...
static int P_adaptive(int total_power, unsigned int gpu_loading)
{
	...
	if (total_power <= MINIMUM_TOTAL_POWER) {
		...
		/* get GPU highest possible power and index
		 * and current power and index and next lower power
		 */
	...
	long prev_temp, long curr_temp, unsigned int gpu_loading)
{
	...
	if (cl_dev_adp_cpu_state_active == 1) {
		...
		/* even cooler not exit, when CTM is on
		 * and current Tj < current_ETJ, leave ATM
		 */
		if (triggered) {
			triggered = 0;
			tscpu_dprintk("%s Tp %ld, Tc %ld, Pt %d\n",
	...
	struct thermal_cooling_device *cdev, unsigned long state)
{
	...
	/* tscpu_dprintk("adp_cpu_set_cur_state[%d] =%d,
		...
		size_t count, loff_t *data)
{
	...
	if (kstrtoint(desc, 10, &lv_tempe_switch) == 0) {
		...
		tscpu_dprintk("%s temperature_switch=%d\n",
	...
	size_t count, loff_t *data)
{
	...
	if (sscanf(desc, "%d %d %d %d %d %d %d %d %d ",
				&atm_ver, &tmp_tt_ratio_high_rise,
				&tmp_tt_ratio_high_fall,
				&tmp_tt_ratio_low_rise,
				&tmp_tt_ratio_low_fall,
				&tmp_tp_ratio_high_rise,
				&tmp_tp_ratio_high_fall,
				&tmp_tp_ratio_low_rise,
				&tmp_tp_ratio_low_fall) == 9)
		/* if (5 <= sscanf(desc,
		 * "%d %d %d %d %d",
		 * &log_switch, &hot,
...
static ssize_t tscpu_write_dtm_setting(
		...
		loff_t *data)
{
	...
	int i_id = -1, i_first_step = -1, i_theta_r = -1,
		i_theta_f = -1, i_budget_change = -1,
		i_min_cpu_pwr = -1, i_max_cpu_pwr = -1,
	...
	if (sscanf(desc, "%d %d %d %d %d %d %d %d %d",
			&i_id, &i_first_step, &i_theta_r, &i_theta_f,
				&i_budget_change, &i_min_cpu_pwr,
				&i_max_cpu_pwr, &i_min_gpu_pwr,
				&i_max_gpu_pwr) >= 9) {
		tscpu_printk(
			"%s input %d %d %d %d %d %d %d %d %d\n",
			__func__, i_id, i_first_step, i_theta_r,
			i_theta_f, i_budget_change, i_min_cpu_pwr,
		...
		if (i_id >= 0 && i_id < MAX_CPT_ADAPTIVE_COOLERS) {
			...
			tscpu_printk("%s applied %d %d %d %d %d %d %d %d %d\n",
					__func__, i_id,
					FIRST_STEP_TOTAL_POWER_BUDGETS[i_id],
	...
	if (sscanf
		...
		tscpu_dprintk(
			"%s trip_0_temp=%d,trip_1_temp=%d,trip_2_temp=%d,",
				__func__, trip_temp[0],
				trip_temp[1], trip_temp[2]);
		tscpu_dprintk(
			"trip_3_temp=%d,trip_4_temp=%d,trip_5_temp=%d,",
				trip_temp[3], trip_temp[4], trip_temp[5]);
		tscpu_dprintk(
			"trip_6_temp=%d,trip_7_temp=%d,trip_8_temp=%d,",
				trip_temp[6],	trip_temp[7], trip_temp[8]);
		tscpu_dprintk(
			"trip_9_temp=%d,time_ms=%d, num_trip=%d\n",
		...
		struct platform_device *dev, pm_message_t state)
{
	...
	g_tc_resume = 1;
/* set "1", don't read temp during suspend */
...
/* freq/volt down or cpu down or backlight down or charging down... */
			...
			tscpu_printk("%s else temp=%d, trip=%d, ltd=%d\n",
					__func__, max_temp,
	...
	/* if(talking_flag==false) */
	{
		...
		while (cnt < 50) {
			...
			if (cnt > 10)
				...
				temp, cnt,	__LINE__);
			if (temp == 0x0) {
/* pause all periodoc temperature
 * sensing point 0~2
 */
				thermal_pause_all_periodoc_temp_sensing();
				/* TEMPMSRCTL1 */
...
/* AHB address for auxadc voltage output */
	...
	/* enable all interrupt except filter sense
	 * and immediate sense interrupt
	 */
	...
	while (temp != 0xDA1 && cunt < 20) {
		cunt++;
		tscpu_dprintk(
			"[Power/CPU_Thermal]0 temp=%d,cunt=%d\n",
			temp, cunt);
		temp = DRV_Reg32(TEMPMSR0) & 0x0fff;
	}
	...
	while (temp != 0xDA1 && cunt < 20) {
		cunt++;
		tscpu_dprintk(
			"[Power/CPU_Thermal]1 temp=%d,cunt=%d\n",
			temp, cunt);
		temp = DRV_Reg32(TEMPMSR1) & 0x0fff;
	}
	...
	while (temp != 0xDA1 && cunt < 20) {
		cunt++;
		tscpu_dprintk(
			"[Power/CPU_Thermal]2 temp=%d,cunt=%d\n",
			temp, cunt);
		temp = DRV_Reg32(TEMPMSR2) & 0x0fff;
	}
	...
	while (temp != 0xDA1 && cunt < 20) {
		cunt++;
		tscpu_dprintk(
			"[Power/CPU_Thermal]3 temp=%d,cunt=%d\n",
			temp, cunt);
		temp = DRV_Reg32(TEMPMSR3) & 0x0fff;
	}
...
static int get_io_reg_base(struct platform_device *pdev)
{
		...
		tscpu_printk(
			"[THERM_CTRL] pericfg_base=0x%lx\n",
			(unsigned long)pericfg_base);
#endif

		node = of_find_compatible_node(NULL, NULL,
				"mediatek,mt8167-auxadc");
...
/*static int __init tscpu_init(void)*/
{
...
#if MTK_TS_CPU_RT
	{
		...
		ktp_thread_handle = kthread_create(
					ktp_thread, (void *)NULL, "kthermp");
	...
	err =
		request_irq(thermal_irq_number,
				thermal_interrupt_handler, IRQF_TRIGGER_LOW,
				THERMAL_NAME, NULL);
	if (err)
	...
	if (!mtktscpu_dir) {
		...
		entry =
			proc_create("tzcpu_read_temperature", 0444,
					mtktscpu_dir,
		...
		if (entry)
			proc_set_user(entry, uid, gid);
		entry =
			proc_create("clatm_gpu_threshold", 0664,
					mtktscpu_dir,
...
#if MTKTSCPU_FAST_POLLING
		entry =
			proc_create("tzcpu_fastpoll", 0664,
					mtktscpu_dir,
		...
		entry =
			proc_create("tzcpu_Tj_out_via_HW_pin", 0644,
					mtktscpu_dir,
...
#if THERMAL_GPIO_OUT_TOGGLE
		entry =
			proc_create("tzcpu_GPIO_out_monitor", 0644,
					mtktscpu_dir,
