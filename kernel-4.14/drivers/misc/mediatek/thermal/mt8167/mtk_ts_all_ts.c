...
{
	tsallts_dprintk("E_WF: %s doesn't exist\n", __func__);
	return -127000;
}
...
static int tsallts_get_ts4_temp(void)
{
	...
	tsallts_dprintk("[%s] TS4(MCU2) =%d\n", __func__, t_ret);
	return t_ret;
}

static int tsallts_get_temp1(
	struct thermal_zone_device *thermal, int *t)
{
	...
	tsallts_dprintk("%s CPU TS1(MCU4)=%d\n", __func__, curr_temp);
	...
	if (curr_temp != 0)	{
		...
		if ((curr_temp > 150000) || (curr_temp < -20000))	{
			...
			if ((curr_temp - last_abb_read_temp > 20000) ||
				...
				tsallts_printk(
					"TS1(MCU4) temp float hugely temp=%d, lasttemp=%d\n",
	...
	if (curr_temp != 0)	{
		...
		if ((curr_temp > 150000) || (curr_temp < -20000))	{
			...
			if ((curr_temp - last_abb_read_temp > 20000) ||
				(last_abb_read_temp - curr_temp > 20000)) {
				tsallts_printk(
					"T5(MCU3) temp float hugely temp=%d, lasttemp=%d\n",
					       curr_temp, last_abb_read_temp);
				temp_temp = 50000;
				ret = -1;
			}
	...
	if (curr_temp != 0)	{
		...
		if ((curr_temp > 150000) || (curr_temp < -20000))	{
			...
			if ((curr_temp - last_abb_read_temp > 20000) ||
				(last_abb_read_temp - curr_temp > 20000)) {
				tsallts_printk(
					"TS3(MCU1) temp float hugely temp=%d, lasttemp=%d\n",
					       curr_temp, last_abb_read_temp);
				temp_temp = 50000;
				ret = -1;
			}
	...
	if (curr_temp != 0)	{
		...
		if ((curr_temp > 150000) ||
			...
			if ((curr_temp - last_abb_read_temp > 20000)
				|| (last_abb_read_temp - curr_temp > 20000)) {
				tsallts_printk(
					"TS4(MCU2) temp float hugely temp=%d, lasttemp=%d\n",
					       curr_temp, last_abb_read_temp);
				temp_temp = 50000;
				ret = -1;
			}
	...
	struct thermal_cooling_device *cdev)
{
	...
	if (mtk_thermal_zone_bind_cooling_device(thermal, table_val, cdev)) {
		tsallts_dprintk("[%s] error binding cooling dev\n", __func__);
		return -EINVAL;
	}
	tsallts_dprintk("[%s] binding OK, %d\n", __func__, table_val);
	...
	struct thermal_cooling_device *cdev)
{
	...
	if (thermal_zone_unbind_cooling_device(thermal, table_val, cdev)) {
		tsallts_dprintk("[%s] error unbinding cooling dev\n", __func__);
		return -EINVAL;
	}
	tsallts_dprintk("[%s] unbinding OK\n", __func__);
...
static int tsallts_get_trip_type(
...
static int tsallts_sysrst_get_max_state(
...
static int tsallts_sysrst_get_cur_state(
	struct thermal_cooling_device *cdev, unsigned long *state)
{
...
static int tsallts_sysrst_set_cur_state(
	struct thermal_cooling_device *cdev, unsigned long state)
{
	...
	size_t count, loff_t *data)
{
	...
	if (sscanf
	    (desc,
	     ...
	     &time_msec) == 32) {
		tsallts_dprintk("[%s] tsallts_unregister_thermal\n", __func__);
		...
		tsallts_dprintk("[%s] tsallts_register_thermal\n", __func__);
		tsallts_register_thermal();

		return count;
	}
	tsallts_dprintk("[%s] bad argument\n", __func__);

	return -EINVAL;
}

static ssize_t tsallts_write5(
	struct file *file, const char __user *buffer,
	size_t count, loff_t *data)
{
	int len = 0, time_msec = 0;
	...
	if (sscanf
		...
		tsallts_dprintk("[%s] g_THERMAL_TRIP_0=%d,g_THERMAL_TRIP_1=%d,",
			__func__, g_THERMAL_TRIP[0], g_THERMAL_TRIP[1]);
		tsallts_dprintk(
			"g_THERMAL_TRIP_2=%d,g_THERMAL_TRIP_3=%d,g_THERMAL_TRIP_4=%d,",
			g_THERMAL_TRIP[2], g_THERMAL_TRIP[3],
		...
		tsallts_dprintk("[%s] trip_0_temp=%d,trip_1_temp=%d,", __func__,
			trip_temp[0], trip_temp[1]);
	...
	size_t count, loff_t *data)
{
	...
	if (sscanf
		...
		tsallts_dprintk("[%s] g_THERMAL_TRIP_0=%d,",
...
int tsallts_register_thermal(void)
{
	...
	/* trips : trip 0~3 */
	thz_dev1 = mtk_thermal_zone_device_register("mtkts1",
			num_trip, NULL, &tsallts_dev_ops1,
			0, 0, 0, interval);
	/* trips : trip 0~3 */
	thz_dev5 = mtk_thermal_zone_device_register("mtkts5",
			num_trip, NULL, &tsallts_dev_ops5,
			0, 0, 0, interval);
	/* trips : trip 0~3 */
	thz_dev3 = mtk_thermal_zone_device_register("mtkts3",
			num_trip, NULL, &tsallts_dev_ops3,
			0, 0, 0, interval);
	/* trips : trip 0~3 */
	thz_dev4 = mtk_thermal_zone_device_register("mtkts4",
			num_trip, NULL, &tsallts_dev_ops4,
			0, 0, 0, interval);
