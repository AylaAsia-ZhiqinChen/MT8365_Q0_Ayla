...
static void mtk_cl_bcct_set_bcct_limit(void)
{
	...
	if (min_limit != cl_bcct_cur_limit) {
		...
		if (cl_bcct_cur_limit >= 65535) {
			...
			chrlmt_set_limit(&cl_bcct_chrlmt_handle,
				-1, cl_bcct_cur_limit);
	...
	struct thermal_cooling_device *cdev, unsigned long temp)
{
	...
	chrlmt_set_limit(&abcct_chrlmt_handle,
		abcct_cur_chr_input_curr_limit,
	...
	struct thermal_cooling_device *cdev, unsigned long temp)
{
	...
	delta = pterm/abcct_lcmoff_kp +
		abcct_lcmoff_iterm/abcct_lcmoff_ki +
...
/* Align limit to 50mA to avoid redundant calls to chrlmt. */
	...
	mtk_cooler_bcct_dprintk("%s %ld %ld %ld %ld %ld %d\n"
		, __func__, abcct_lcmoff_curr_temp,
		pterm, abcct_lcmoff_iterm, dterm, delta, limit);
...
static int mtk_cooler_abcct_register_ltf(void)
{
	...
	if (!cl_abcct_dev)
		...
		mtk_thermal_cooling_device_register_wrapper_extra(
			"abcct", (void *)NULL,
			&mtk_cl_abcct_ops, &mtk_cl_abcct_ops_ext);
	...
	size_t len, loff_t *data)
{
     ...
     * <mtk-cl-bcct00 limit> can only
     * be positive integer or -1 to denote no limit
     */
...
#else
#error "Change correspondent part when"\
"changing MAX_NUM_INSTANCE_MTK_COOLER_BCCT!"
#endif
	...
	size_t len, loff_t *data)
{
	...
	int _max_cur, _min_cur,
		_input_current_limit_on,
		_HW_thermal_sol, _min_input,
	...
	size_t len, loff_t *data)
{
	...
	long _abcct_lcmoff_target_temp,
		_abcct_lcmoff_kp,
		_abcct_lcmoff_ki,
		_abcct_lcmoff_kd;
	int _max_cur, _min_cur;
...
static int __init mtk_cooler_bcct_init(void)
{
	...
	/* create a proc file */
	{
		...
		entry =
		    proc_create("clbcct", 0664,
		    dir_entry, &_cl_bcct_fops);
		if (!entry)
			mtk_cooler_bcct_dprintk_always(
		...
		entry =
		    proc_create("clabcct_lcmoff", 0664, dir_entry,
		...
		entry = proc_create("battery_status",
					0444, NULL, &_cl_battery_status_fops);
	}
