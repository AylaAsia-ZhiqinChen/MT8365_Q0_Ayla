 ...
 *
 * We don't have to take care those two function lists
 * project by project, because each thermal zone
 * will register his functions to mtk_thermal_timer.c when booting up
 */

/* MAX_NUM is the maximum number of pair
 * of thermal zone functions we can hold
 */
#define MAX_NUM (20)
...
int mtkTTimer_register(
	const char *name, void (*start_timer)(void),
	void (*cancel_timer)(void))
{
	int index;
