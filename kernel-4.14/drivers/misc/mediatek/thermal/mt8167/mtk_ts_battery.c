...
static int mtktsbattery_get_hw_temp(void)
{
	...
	if (t_ret)
		...
		mtktsbattery_dprintk("[%s] counter=%d, first_time =%d\n",
...
static int mtktsbattery_get_crit_temp(
	struct thermal_zone_device *thermal,
				      int *temperature)
{
...
#if 0
static int dis_charge_get_max_state(
	struct thermal_cooling_device *cdev,
				 int *state)
{
		*state = 1;
...
static int dis_charge_get_cur_state(
	struct thermal_cooling_device *cdev,
				 int *state)
{
...
static int dis_charge_set_cur_state(
	struct thermal_cooling_device *cdev,
				 int state)
{
	cl_dev_dis_charge_state = state;
	if (cl_dev_dis_charge_state == 1) {
		mtktsbattery_dprintk("[%s] disable charging\n",
		__func__);
	}
	return 0;
}
...
#if 0
static int mtktsbattery_read(char *buf,
...
#if 0
static ssize_t mtktsbattery_write(struct file *file,
const char *buffer, int count, void *data)
#endif
{
	int len = 0, i;
...
static int mtktsbattery_register_thermal(void)
{
	...
	thz_dev = mtk_thermal_zone_device_register(
					"mtktsbattery", num_trip, NULL,
					&mtktsbattery_dev_ops, 0, 0, 0,
					interval * 1000);
