...
/* This function pointer is for GPU LKM to
 * register a function to get GPU loading.
 */
	...
	for_each_online_cpu(nCoreIndex) {
		...
		cpu_index_list[nCoreIndex].i[CPU_USAGE_FRAME_FIELD] =
		    TRIMz_ex(cpu_index_list[nCoreIndex].tz,
			     (cpu_index_list[nCoreIndex].i
			     [CPU_USAGE_CURRENT_FIELD] -
			      cpu_index_list[nCoreIndex].i
			      [CPU_USAGE_SAVE_FIELD]));
			       ...
			       char ***attr_units)
{
	...
	extra_attr_values[i] =
		get_sys_wifi_throughput(
		"/proc/driver/thermal/wifi_tx_thro", 3);
