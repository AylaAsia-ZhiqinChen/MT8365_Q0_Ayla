...
static int wmt_send_signal(int level)
{
	...
	if (ret == 0 && pg_task) {
		...
		ret = do_send_sig_info(SIGIO, &info, pg_task, false);
	}
	...
	if (cl_dev_adp_cpu_state_active == 1) {
		...
		wmt_tm_dprintk(
			"%s Wifi TJ %d Tp %ld, Tc %ld, wifi_tput_ratio %d tput %lu\n",
			       __func__, wifi_target_tj,
			       prev_temp, curr_temp, wifi_tput_ratio,
			       tx_throughput);
	...
	struct thermal_cooling_device *cool_dev, unsigned long v)
{
	...
	if (cl_dev_state == 1) {
		wmt_tm_printk("%s = 1\n", __func__);
		/* the temperature is over than the critical,
		 * system reboot.
		 */
		/*(unsigned int *)0x0 = 0xdead; */
		BUG();
		/* To trigger data abort to
		 * reset the system for thermal protection.
		 */
	}
...
/*At this moment, current throughput is none. Use the previous constraint. */
			else
				thro_constraint =
	...
	struct thermal_cooling_device *cool_dev, unsigned long v)
{
	...
	wmt_tm_dprintk(
		"[%s] wifi_throttle_version=%u,cl_pa1_dev_state=%u\n",
			__func__,
...
static int wmt_wifi_tx_thro_open(struct inode *inode, struct file *file)
{
	return single_open(file,
		wmt_wifi_tx_thro_read, PDE_DATA(inode));
}
...
static int wmt_wifi_tx_thro_limit_open(
...
/* wmt_wifi_in_soc_write ret 13 1 3 3 0
 * 70000 3000 1000 50 200 1000 50 50 10000
 */
	return len;
}
...
static int wmt_tm_read(struct seq_file *m, void *v)
{
	...
	seq_printf(m,
		   ...
		   __func__, check_str(g_bind0), g_trip_temp[0],
		   g_thermal_trip[0], check_str(g_bind1),
		   g_trip_temp[1], g_thermal_trip[1]);
	seq_printf(m,
		   "\n [2] %s\t%d\t\t%d\n [3] %s\t%d\t\t%d\n [4] %s\t%d\t\t%d\n [5] %s\t%d\t\t%d",
		   check_str(g_bind2), g_trip_temp[2],
		   g_thermal_trip[2], check_str(g_bind3),
		   g_trip_temp[3], g_thermal_trip[3],
		   check_str(g_bind4), g_trip_temp[4],
		   g_thermal_trip[4], check_str(g_bind5),
		   g_trip_temp[5], g_thermal_trip[5]);
	seq_printf(m,
		   "\n [6] %s\t%d\t\t%d\n [7] %s\t%d\t\t%d\n [8] %s\t%d\t\t%d\n [9] %s\t%d\t\t%d\ntime_ms=%d\n",
		   check_str(g_bind6), g_trip_temp[6],
		   g_thermal_trip[6], check_str(g_bind7),
		   g_trip_temp[7], g_thermal_trip[7],
		   check_str(g_bind8), g_trip_temp[8],
		   g_thermal_trip[8], check_str(g_bind9),
		   g_trip_temp[9], g_thermal_trip[9],
		   p_linux_if->interval);
	...
	struct file *filp, const char __user *buf, size_t count, loff_t *data)
{
	...
	if (sscanf
	    (ptr_tm_data->desc,
	     ...
	     &ptr_tm_data->thermal_trip[0],
	     ptr_tm_data->bind0,
	     &ptr_tm_data->trip_temp[1],
	     &ptr_tm_data->thermal_trip[1], ptr_tm_data->bind1,
	     &ptr_tm_data->trip_temp[2],
	     &ptr_tm_data->thermal_trip[2], ptr_tm_data->bind2,
	     &ptr_tm_data->trip_temp[3],
	     &ptr_tm_data->thermal_trip[3], ptr_tm_data->bind3,
	     &ptr_tm_data->trip_temp[4],
	     &ptr_tm_data->thermal_trip[4], ptr_tm_data->bind4,
	     &ptr_tm_data->trip_temp[5],
	     &ptr_tm_data->thermal_trip[5], ptr_tm_data->bind5,
	     &ptr_tm_data->trip_temp[6],
	     &ptr_tm_data->thermal_trip[6], ptr_tm_data->bind6,
	     &ptr_tm_data->trip_temp[7],
	     &ptr_tm_data->thermal_trip[7], ptr_tm_data->bind7,
	     &ptr_tm_data->trip_temp[8],
	     &ptr_tm_data->thermal_trip[8], ptr_tm_data->bind8,
	     &ptr_tm_data->trip_temp[9],
	     &ptr_tm_data->thermal_trip[9], ptr_tm_data->bind9,
		...
		g_bind0[0] = g_bind1[0] =
			g_bind2[0] = g_bind3[0] = g_bind4[0] = '\0';
		g_bind5[0] = g_bind6[0] =
		...
		wmt_tm_dprintk(
			"[%s] cooldev [5]=%s, [6]=%s, [7]=%s, [8]=%s, [9]=%s,\n",
			       __func__, g_bind5, g_bind6,
			       g_bind7, g_bind8, g_bind9);

		wmt_tm_dprintk(
			"[%s] trip_temp [0]=%d, [1]=%d, [2]=%d, [3]=%d, [4]=%d\n",
			       __func__, ptr_tm_data->trip_temp[0],
			       ptr_tm_data->trip_temp[1],
			       ptr_tm_data->trip_temp[2],
			       ptr_tm_data->trip_temp[3],
			       ptr_tm_data->trip_temp[4]);
		...
		wmt_tm_dprintk(
			"[%s] trip_temp [5]=%d, [6]=%d, [7]=%d, [8]=%d, [9]=%d\n",
			       __func__, ptr_tm_data->trip_temp[5],
			       ptr_tm_data->trip_temp[6],
			       ptr_tm_data->trip_temp[7],
			       ptr_tm_data->trip_temp[8],
			       ptr_tm_data->trip_temp[9]);
...
static int wmt_tm_proc_register(void)
{
	...
	if (!wmt_thro_proc_dir) {
		wmt_tm_printk("[%s]: mkdir /proc/wmt_tm failed\n", __func__);
	} else {
