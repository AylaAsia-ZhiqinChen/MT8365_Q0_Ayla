...
#if 0
	mtkts_bts_dprintk("%s : TAP_Value = %d\n", __func__, TAP_Value);
...
static int gadc_thermal_get_temp(void *data, int *temp)
{
	...
	if (IS_ERR(gti->channel)) {
		...
		mtkts_bts_dprintk(
			"%s: IIO channel not found: %d\n", __func__, ret);
		return ret;
	}
	ret = iio_read_channel_processed(gti->channel, &val);
	if (ret < 0) {
		mtkts_bts_dprintk("%s: IIO channel read failed %d\n",
			__func__, ret);
		return ret;
	}
	val = val * 1500 / 4096;
...
static int gadc_thermal_probe(struct platform_device *pdev)
{
	...
	if (!pdev->dev.of_node) {
		mtkts_bts_dprintk("%s: Only DT based supported\n", __func__);
		return -ENODEV;
	}
	...
	if (IS_ERR(gti->channel)) {
		ret = PTR_ERR(gti->channel);
		mtkts_bts_dprintk("%s: IIO channel not found: %d\n",
			__func__, ret);
		return ret;
	}
	mtkts_bts_dprintk("%s OK\n", __func__);
