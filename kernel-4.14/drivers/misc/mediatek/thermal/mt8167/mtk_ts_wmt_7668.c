...
set_adaptive_wifi_tput_limit(unsigned int cur_tput, unsigned int limit_ratio)
{
	...
	if (limit_ratio == -1) {
		...
		if (limit_upper_bound >= min_wifi_tput)
			limit_tput = clamp(limit_tput,
				min_wifi_tput, limit_upper_bound);
		else
			limit_tput = -1;
...
{
	wmt_tm_dprintk(
		"%s: wifi_throttle_version=%d, wifi temp=%d\n"
...
static int wmt_tm_read(struct seq_file *m, void *v)
{
	...
	seq_printf(m,
		   ...
		   __func__, check_str(g_bind0),
		   g_trip_temp[0], g_thermal_trip[0],
		   check_str(g_bind1),
