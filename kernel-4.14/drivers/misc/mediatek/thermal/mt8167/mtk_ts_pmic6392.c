...
static void pmic_cali_prepare(void)
{
	...
	pr_info(
		"Power/PMIC_Thermal: Reg(0x666)=0x%x, Reg(0x668)=0x%x\n",
		temp0, temp1);
	...
	if (g_adc_cali_en != 1) {
		...
		g_o_slope = 0;
		g_o_slope_sign = 0;
		pr_err("[%s] No thermal calibration data !!!", __func__);
	}
}
...
static s32 thermal_cal_exec(u32 ret)
{
	...
	mtktspmic_dprintk("[%s] ret=%d, y_curr=%d\n",
				__func__, ret, y_curr);
	mtktspmic_dprintk("[%s] data =%d, slope_data=%d, VBE_T=%d\n",
				__func__, (g_degc_cali / 2), slope_data, VBE_T);
	mtktspmic_dprintk("[%s] T_PMIC=%d\n",
				__func__,  t_current);
...
static int mtktspmic_get_hw_temp(void)
{
	...
	mtktspmic_dprintk(
		"[%s] PMIC_IMM_GetOneChannel 4=%d, T=%d\n",
		__func__, temp, temp1);
...
static int mtktspmic_probe(struct platform_device *pdev)
{
	...
	if (!mtktspmic_dir)
		mtktspmic_dprintk(
			...
			__func__);
	entry = proc_create("tzpmic", 0664,
				mtktspmic_dir, &mtktspmic_fops);
	if (entry)
