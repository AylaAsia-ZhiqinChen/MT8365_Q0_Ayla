...
thermal_bank_name ts_bank)
{
	...
	if (g_o_slope_sign == 0)
		temp1 = (temp0 * 10) / (1640 + g_o_slope * 10);
	else
		temp1 = (temp0 * 10) / (1640 - g_o_slope * 10);
	...
	if (g_o_slope_sign == 0)
		temp2 = temp1 * 100 / (1640 + g_o_slope * 10);
	else
		temp2 = temp1 * 100 / (1640 - g_o_slope * 10);
...
void tscpu_thermal_cal_prepare(void)
{
	...
	calefuse1 = get_devinfo_with_index(ADDRESS_INDEX_1);  // 0x11C50180
	calefuse2 = get_devinfo_with_index(ADDRESS_INDEX_0);  // 0x11C50184
	calefuse3 = get_devinfo_with_index(ADDRESS_INDEX_2);  // 0x11C50188

	temp0 = calefuse2;  // 0x11C50184
	temp1 = calefuse1;  // 0x11C50180
	temp2 = calefuse3;  // 0x11C50188
...
static __s32 temperature_to_raw_room(__u32 ret, enum thermal_sensor ts_name)
{
	...
	if (g_o_slope_sign == 0) {	/* O_SLOPE is Positive. */
		...
		format_2 = format_1 * (1640 + g_o_slope * 10) / 10 * 18 / 15;
...
static __s32 raw_to_temperature_roomt(__u32 ret, enum thermal_sensor ts_name)
{
	...
	if (g_o_slope_sign == 0) /* uint = 0.1 deg */
		format_4 = ((format_3 * 1000) / (1640 + g_o_slope * 10));
	else			/* uint = 0.1 deg */
		format_4 = ((format_3 * 1000) / (1640 - g_o_slope * 10));
...
void tscpu_thermal_initial_all_tc(void)
{
	...
	/* AuxADC Initialization,ref
	 * MT8168_Temperature Sensor SW Implement Guide.doc
	 */
	temp = readl(AUXADC_CON0_V);	/* Auto set enable for CH10 */
	temp &= 0xFFFFFBFF;	/* 0: Not AUTOSET mode */

	/* disable auxadc channel 10 synchronous mode */
	mt_reg_sync_writel(temp, AUXADC_CON0_V);

	/* disable auxadc channel 10 immediate mode */
	mt_reg_sync_writel(0x400, AUXADC_CON1_CLR_V);
...
int get_io_reg_base(void)
{
	...
	if (node) {
		...
		tscpu_printk("[THERM_CTRL] config error thermal_base\n");
		return 0;
	}
	...
	node = of_find_compatible_node(NULL, NULL, "mediatek,mt8168-auxadc");
	if (node) {
		/* Setup IO addresses */
		...
		tscpu_printk("[THERM_CTRL] config error auxadc_ts_base\n");
		return 0;
	}
	...
	node = of_find_compatible_node(NULL, NULL, "mediatek,mt8168-infracfg");
	if (node) {
		/* Setup IO addresses */
		...
		tscpu_printk("[THERM_CTRL] config error infracfg_ao_base\n");
		return 0;
	}
	...
	if (node) {
		...
		tscpu_printk("[THERM_CTRL] config error apmixed_base\n");
		return 0;
	}
