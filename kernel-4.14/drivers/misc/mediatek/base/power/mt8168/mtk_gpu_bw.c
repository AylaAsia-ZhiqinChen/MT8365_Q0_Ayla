...
#include <spm_v4/mtk_spm_vcore_dvfs_ipi_mt8168.h>
...
static int mt_gpu_bw_ap2sspm(unsigned int eCMD, int type)
{
	...
	apDebug =
	    sspm_ipi_send_sync(IPI_ID_QOS, IPI_OPT_POLLING, &qos_d, 1, &md32Ret,
			       1);
...
{
	pm_qos_add_request(&gpu_qos_request, PM_QOS_GPU_MEMORY_BANDWIDTH,
