...
#define WAKE_SRC_FOR_SODI                                                      \
	(WAKE_SRC_KP | WAKE_SRC_GPT | WAKE_SRC_EINT | WAKE_SRC_WDT |           \
	 ...
	 WAKE_SRC_AFE | WAKE_SRC_CIRQ | WAKE_SRC_SYSPWREQ | WAKE_SRC_CONN_WDT)
...
struct spm_lp_scen __spm_sodi = {
	.pcmdesc = &suspend_pcm, .pwrctrl = &sodi_ctrl,
};
...
static void spm_sodi_pre_process(void)
{
	...
	RCU_NONIDLE(__spm_set_pmic_phase(PMIC_WRAP_PHASE_SODI));
}
...
static void spm_sodi_post_process(void)
{
	...
	RCU_NONIDLE(__spm_set_pmic_phase(PMIC_WRAP_PHASE_NORMAL));
}
#endif
...
void spm_go_to_sodi(u32 spm_flags, u32 spm_data)
{
	...
	spm_flags |= (SPM_BUS26M_DIS | SPM_MPLLOFF_DIS | SPM_FHC_SLEEP_DIS |
		      SPM_26M_DIS);
	...
	/* Display set SPM_PCM_SRC_REQ[0]=1'b1 to force DRAM not enter
	 * self-refresh mode
	 */
	...
	if (wr == WR_PCM_ASSERT) {
		...
		sodi_debug(
			"Warning: not wakeup by SPM, r13 = 0x%x, debug_flag = 0x%x\n",
	...
	if (wakesta.assert_pc != 0) {
		...
		if (by_conn2ap_count >= SODI_CONN2AP_CNT_CRITERA)
			...
			by_conn2ap_count++;
	} else if ((sodi_logout_curr_time - sodi_logout_prev_time) >
	...
	if (need_log_out) {
		...
		if (need_log_out == 1 || need_log_out == 2) {
			sodi_debug(
				"Warning: %s, self-refrsh = %d, pcm_flag = 0x%x\n",
				(wakesta.assert_pc != 0)
					? "wakeup by SPM assert"
					: "not wakeup by SPM",
				wakesta.apsrc_cnt, spm_read(SPM_PCM_FLAGS));
			sodi_debug(
				"sodi_cnt =%d, self-refresh_cnt = %d, spm_pc = 0x%0x, r13 = 0x%x, debug_flag = 0x%x\n",
				sodi_cnt, refresh_cnt, wakesta.assert_pc,
				wakesta.r13, wakesta.debug_flag);
			sodi_debug(
				"r12 = 0x%x, raw_sta = 0x%x, idle_sta = 0x%x, event_reg = 0x%x, isr = 0x%x, %s\n",
			...
			for (i = 1; i < 32; i++) {
				if (wakesta.r12 & (1U << i)) {
					left_size =
						sizeof(buf) - strlen(buf) - 1;
					strncat(buf, sodi_wakesrc_str[i],
						left_size);
			...
			sodi_debug(
				"%d, wakeup by %s, self-refresh=%d, pcm_flag=0x%x, debug_flag = 0x%x, timer_out=%u\n",
				need_log_out, buf, wakesta.apsrc_cnt,
				spm_read(SPM_PCM_FLAGS), wakesta.debug_flag,
				wakesta.timer_out);

			sodi_debug(
				"sodi_cnt=%u, not_refresh_cnt=%u, refresh_cnt=%u, avg_refresh_cnt=%u, timeout_cnt=%u, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x\n",
				sodi_cnt, not_refresh_cnt, refresh_cnt,
				(refresh_cnt /
				 ((sodi_cnt - not_refresh_cnt)
					  ? (sodi_cnt - not_refresh_cnt)
					  : 1)),
				timeout_cnt, wakesta.r12, wakesta.r13,
...
{
	spm_debug("sig_high = %u%%  %u%%  %u%%  %u%%, r13 = 0x%x\n",
