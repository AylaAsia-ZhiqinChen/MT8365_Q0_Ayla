...
static const char *idle_name[NR_TYPES] = {
	"dpidle", "soidle", "mcidle", "slidle", "rgidle",
};

static const char *reason_name[NR_REASONS] = {
	"by_cpu", "by_clk", "by_tmr", "by_oth", "by_vtg",
};
...
request_gpt(unsigned int id, unsigned int mode, unsigned int clksrc,
	    unsigned int clkdiv, unsigned int cmp, void (*func)(unsigned long),
	    unsigned int flags)
{
	return 0;
}
...
int __attribute__((weak))
cpu_xgpt_register_timer(unsigned int id,
...
{
	u32 pwr_sta_mask[] = {
		PWR_STA_CONN, PWR_STA_DIS,    PWR_STA_MFG_3D,    PWR_STA_ISP,
		PWR_STA_VDE,  PWR_STA_MFG_2D, PWR_STA_MFG_ASYNC,
	};
...
static const char *grp_get_name(int id)
{
	static const char *const grp_name[] = {
		"CG_CTRL0",  "CG_CTRL1",  "CG_CTRL2",  "CG_CTRL8",
		"CG_MMSYS0", "CG_MMSYS1", "CG_IMGSYS", "CG_MFGSYS",
		"CG_AUDIO",  "CG_VDEC0",  "CG_VDEC1",
	};
...
#if IDLE_HAVE_STD_TIMER
	t = ktime_to_us(tick_nohz_get_sleep_length(&delta_next));
	if (t < soidle_time_critera) {
		reason = BY_TMR;
		goto out;
	}
#else /* !IDLE_HAVE_STD_TIMER */
...
static bool go_to_mcidle(int cpu)
{
	...
	if (g_SPM_MCDI_Abnormal_WakeUp != g_pre_SPM_MCDI_Abnormal_WakeUp) {
		idle_pr_err("SPM-MCDI Abnormal %x\n",
...
static void mcidle_idle_pre_handler(int cpu)
{
	if (cpu == 0) {
#if IDLE_HAVE_MCDI
		if (only_one_cpu_online() || (idle_switch[IDLE_TYPE_MC] == 0))
...
static void backup_audintbus_26MHz(void)
{
	...
	/* select audiosys internal bus clock to 26MHz, CLK_MUX_SEL0[29:27] =
	 * 3'b001
	 * select audio hopping clock to 26MHz, CLK_MUX_SEL0[26] = 0
	 */
	idle_writel(CLK_MUX_SEL0,
		    ((clk_mux_sel0 & ~GENMASK(29, 26)) | (1 << 27)));
}
...
static u32 slp_spm_deepidle_flags = {SPM_INFRA_PDN_DIS | SPM_26M_OFF_DIS};

void set_slp_spm_deepidle_flags(bool en)
{
	idle_dbg("unused function: %s()\n", __func__);
}
...
static int (*idle_handlers[NR_TYPES])(int) = {
	dpidle_handler, soidle_handler, mcidle_handler,
	slidle_handler, rgidle_handler,
};
...
static int (*idle_select_handlers[NR_TYPES])(int) = {
	dpidle_select_handler, soidle_select_handler, mcidle_select_handler,
	slidle_select_handler, rgidle_select_handler,
};
...
static int get_base_from_node(const char *cmp, void __iomem **pbase, int idx)
{
			       ...
			       size_t count, loff_t *f_pos)
{
	...
	for (i = 0; i < nr_cpu_ids; i++) {
		p += sprintf(
			...
			i, soidle_cnt[i], i, dpidle_cnt[i], i, mcidle_cnt[i], i,
			slidle_cnt[i], i, rgidle_cnt[i]);
	}
...
static int mt_cpuidle_debugfs_init(void)
{
	...
	if (!root_entry) {
		idle_pr_err("Can not create debugfs 'cpuidle'\n");
		return 1;
	}
...
void mtk_cpuidle_framework_init(void)
{
	...
	if (err)
		idle_pr_err("[%s]fail to request GPT%d\n", __func__,
				idle_gpt + 1);
	...
	if (err)
		idle_pr_err("[%s]fail to request cpuxgpt\n", __func__);
#endif /* !IDLE_HAVE_STD_TIMER */
