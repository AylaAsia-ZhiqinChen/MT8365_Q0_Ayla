...
static struct attribute *spm_attrs[] = {
	...
	__ATTR_OF(suspend_pcm), __ATTR_OF(dpidle_pcm), __ATTR_OF(sodi_pcm),
	...
	__ATTR_OF(suspend_ctrl), __ATTR_OF(dpidle_ctrl), __ATTR_OF(sodi_ctrl),
...
int spm_fs_init(void)
{
	...
	if (r)
		spm_pr_err("FAILED TO CREATE /sys/power/spm (%d)\n", r);
