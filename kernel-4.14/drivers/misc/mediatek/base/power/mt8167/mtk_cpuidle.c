...
#define dormant_pr_err(fmt, args...) pr_err(TAG fmt, ##args)
...
#endif
unsigned long long mt_cpuidle_timestamp[CONFIG_NR_CPUS]
				       [MT_CPUIDLE_TIMESTAMP_COUNT];
...
#define MT_CPUIDLE_FOOTPRINT_LOG(cid, idx)                                     \
	(sleep_aee_rec_cpu_dormant_va[cid] |= (1 << idx))
#define MT_CPUIDLE_FOOTPRINT_LOG_CLEAR(cid)                                    \
	(sleep_aee_rec_cpu_dormant_va[cid] = 0)
#else
#define MT_CPUIDLE_FOOTPRINT_LOG(cid, idx)
#define MT_CPUIDLE_FOOTPRINT_LOG_CLEAR(cid)
#endif
...
#define MT_CPUIDLE_TIMESTAMP_LOG(cpuid, idx)                                   \
	(mt_cpuidle_timestamp[cpuid][idx] = arch_counter_get_cntvct())
#else
#define MT_CPUIDLE_TIMESTAMP_LOG(cpuid, idx)
#endif

#define read_cpu_idx() ({ ((read_cluster_id() >> 6) | read_cpu_id()); })
...
static void restore_gic_spm_irq(struct interrupt_distributor *id, int wake_src,
				int *irq_bit)
{
	int i, j;
...
static void restore_edge_gic_spm_irq(unsigned long gic_distributor_address)
{
	...
	restore_gic_spm_irq(id, CPUIDLE_WAKE_SRC_R12_CONN_WDT_IRQ_B,
			    &conn_wdt_irq_bit);
	restore_gic_spm_irq(id, CPUIDLE_WAKE_SRC_R12_LOWBATTERY_IRQ_B,
			    &lowbattery_irq_bit);
...
void mt_cpu_save(void)
{
	...
	/* TODO: check this part */
	sleep_sta = (spm_read(CPUIDLE_CPU_IDLE_STA) >>
		     CPUIDLE_CPU_IDLE_STA_OFFSET) &
		    0xff;
...
void kernel_neon_begin_64(void)
{
	if (WARN_ON(!system_supports_fpsimd()))
		return;

	WARN_ON(!(!in_irq() && !in_nmi() && !this_cpu_read(kernel_neon_busy)));
...
int mt_cpu_dormant(unsigned long flags)
{
	...
	for (i = 0; i < MT_CPUIDLE_TIMESTAMP_COUNT; i++)
		dormant_debug("CPU%d,Timestamp%d,%llu\n", cpu_idx, i,
			    mt_cpuidle_timestamp[cpu_idx][i]);
...
static unsigned long get_dts_node_address(char *node_compatible, int index)
{
	...
	if (!node)
		dormant_pr_err("error: cannot find node [%s]\n",
				node_compatible);

	node_address = (unsigned long)of_iomap(node, index);
	if (!node_address)
		dormant_pr_err("error: cannot iomap [%s]\n", node_compatible);
...
static u32 get_dts_node_irq_bit(char *node_compatible, const int int_size,
				int int_offset)
{
	struct device_node *node;
	...
	if (of_property_read_u32_array(node, "interrupts", node_interrupt,
				       int_size))
		dormant_pr_err("error: cannot property_read [%s]\n",
			    node_compatible);
	/* irq[0] = 0 => spi */
	irq_bit = ((1 - node_interrupt[int_offset]) << 5) +
		  node_interrupt[int_offset + 1];
	of_node_put(node);
...
static void get_dts_nodes_irq_bit(void)
{
	...
	lowbattery_irq_bit =
		get_dts_node_irq_bit("mediatek,mt8167-auxadc", 3, 0);
}
