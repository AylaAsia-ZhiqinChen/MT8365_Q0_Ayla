...
struct spm_lp_scen __spm_dpidle = {
	.pcmdesc = &suspend_pcm, .pwrctrl = &dpidle_ctrl,
};
...
mt_cpufreq_set_pmic_phase(enum pmic_wrap_phase_id phase)
{
}
#endif
...
static void spm_trigger_wfi_for_dpidle(struct pwr_ctrl *pwrctrl)
{
	if (is_cpu_pdn(pwrctrl->pcm_flags)) {
		...
		/*
		 * MT6735/MT6735M: Mp0_axi_config[4] is one by default. No need
		 * to program it before entering deepidle.
	...
	if (dump_log == DEEPIDLE_LOG_FULL) {
		...
		if (log_print == true) {
			dpidle_dbg(
				...
				dpidle_log_discard_cnt, timer_out_too_short);
			wr = __spm_output_wake_reason(wakesta, pcmdesc, false);
...
static void spm_dpidle_pre_process(void)
{
	...
	RCU_NONIDLE(__spm_set_pmic_phase(PMIC_WRAP_PHASE_DEEPIDLE));
}
...
unsigned int spm_go_to_dpidle(u32 spm_flags, u32 spm_data, u32 dump_log)
{
	...
	aee_rr_rec_deepidle_val(aee_rr_curr_deepidle_val() |
				(1 << SPM_DEEPIDLE_ENTER_UART_SLEEP));
#endif
...
{
	spm_pr_crit("sig_high = %u%%  %u%%  %u%%  %u%%, r13 = 0x%x\n",
