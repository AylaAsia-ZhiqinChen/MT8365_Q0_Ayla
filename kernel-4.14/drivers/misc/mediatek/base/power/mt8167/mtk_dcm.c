...
int dcm_armcore(enum ENUM_ARMCORE_DCM mode)
{
	if (mode == ARMCORE_DCM_OFF) {
		...
		/* enable mode 1 */
		reg_write(INFRA_TOPCKGEN_DCMDBC,
			  aor(reg_read(INFRA_TOPCKGEN_DCMDBC), ~(1 << 0),
			      (1 << 0)));
	} else {
		dcm_pr_err("%s(): unkwon value\n", __func__);
		return -EINVAL;
	}
...
int dcm_mcusys(enum ENUM_MCUSYS_DCM on)
{
	if (on == MCUSYS_DCM_OFF) {
		reg_write(CCI_ADB400_DCM_CONFIG,
			  and(reg_read(CCI_ADB400_DCM_CONFIG),
			      ~CCI_ADB400_DCM_CONFIG_SETTING));
		reg_write(L2C_SRAM_CTRL,
			  and(reg_read(L2C_SRAM_CTRL), ~L2C_SRAM_DCM_EN));
		reg_write(CCI_STATUS,
			  and(reg_read(CCI_STATUS), ~MCU_BUS_DCM_EN));
		...
		reg_write(CCI_ADB400_DCM_CONFIG,
			  aor(reg_read(CCI_ADB400_DCM_CONFIG),
			      ~CCI_ADB400_DCM_CONFIG_SETTING,
			      CCI_ADB400_DCM_CONFIG_SETTING));
		reg_write(L2C_SRAM_CTRL,
			  aor(reg_read(L2C_SRAM_CTRL), ~L2C_SRAM_DCM_EN,
			      L2C_SRAM_DCM_EN));
		reg_write(CCI_STATUS, aor(reg_read(CCI_STATUS), ~MCU_BUS_DCM_EN,
					  MCU_BUS_DCM_EN));
		reg_write(MCUSYS_BUS_FABRIC_DCM_CTRL,
		...
		reg_write(MCU_MISC_DCM_CTRL,
			  aor(reg_read(MCU_MISC_DCM_CTRL),
 ...
 * this field actually is to activate clock ratio between
 *infra/fast_peri/slow_peri.
...
int dcm_infra(enum ENUM_INFRA_DCM on)
{
	...
	if (on) {
		...
		reg_write(TOPBUS_DCMCTL,
			  aor(reg_read(TOPBUS_DCMCTL), ~TOPBUS_DCMCTL_SETTING,
			      TOPBUS_DCMCTL_SETTING));
		reg_write(TOPBUS_DCMCTL,
			  and(reg_read(TOPBUS_DCMCTL), ~BUSDCM_APB_TOG));
		...
		reg_write(INFRABUS_DCMCTL0,
			  aor(reg_read(INFRABUS_DCMCTL0), ~INFRA_DCM_EN_MASK,
			      INFRA_DCM_EN_OFF));
		reg_write(TOPBUS_DCMCTL,
			  and(reg_read(TOPBUS_DCMCTL), ~TOPBUS_DCMCTL_SETTING));
		reg_write(TOPBUS_DCMCTL,
			  and(reg_read(TOPBUS_DCMCTL), ~BUSDCM_APB_TOG));
		reg_write(TOPBUS_DCMCTL, aor(reg_read(TOPBUS_DCMCTL),
					     ~BUSDCM_APB_TOG, BUSDCM_APB_TOG));
...
int dcm_peri_sfsel(int cnt)
{
	...
	reg_write(INFRABUS_DCMCTL0,
		  aor(reg_read(INFRABUS_DCMCTL0), ~PERI_DCM_SFSEL_MASK, value));
...
#endif
		reg_write(INFRABUS_DCMCTL0,
			  aor(reg_read(INFRABUS_DCMCTL0), ~PERI_DCM_EN_MASK,
			      PERI_DCM_EN_ON));
	} else {
#if 1 /* override the dbc and fsel setting !! */
...
#endif
		reg_write(INFRABUS_DCMCTL0,
			  aor(reg_read(INFRABUS_DCMCTL0), ~PERI_DCM_EN_ON,
			      PERI_DCM_EN_OFF));
	}
...
int dcm_usb(enum ENUM_USB_DCM on)
{
	if (on) {
		reg_write(INFRABUS_DCMCTL1,
			  aor(reg_read(INFRABUS_DCMCTL1), ~USBDCM_EN_MASK,
			      USB_DCM_EN_ON));
	} else {
		reg_write(INFRABUS_DCMCTL1,
			  aor(reg_read(INFRABUS_DCMCTL1), ~USBDCM_EN_MASK,
...
/* 0x10205038	EMI_CONH
 * 1 0 DCM RATION (00: non dcm, 01: 1/8 slow down, 10: 1/16 slow down, 11: 1/32
 *slow down)
 */
...
int dcm_emi(enum ENUM_EMI_DCM on)
{
	if (on) {
		...
		reg_write(TOPEMI_DCMCTL,
			  aor(reg_read(TOPEMI_DCMCTL), ~EMIDCM_EN_MASK,
			      EMIDCM_EN_OFF | EMIDCM_APB_SEL_DCM));
	} else {
		dcm_emi_dbc(0, 0);
		dcm_emi_slow_down_ratio(EMI_DCM_SLOW_DOWN_RATIO_DIV_32);
		reg_write(EMI_CONM,
			  aor(reg_read(EMI_CONM), ~EMI_DCM_DIS, EMI_DCM_DIS));
		reg_write(TOPEMI_DCMCTL,
			  aor(reg_read(TOPEMI_DCMCTL), ~EMIDCM_EN_MASK,
			      EMIDCM_EN_OFF | EMIDCM_APB_SEL_DCM));
	}
...
void dcm_set_default(unsigned int type)
{
	...
	if (dcm_initiated == 0) {
		dcm_pr_err("DCM driver is not initialized\n");
		return;
	}
...
int mt_dcm_init(void)
{
	...
	ret = mt_dcm_dts_map();
	if (ret != 0) {
		dcm_pr_err("Fail to initialized DCM\n");
		return 0;
	}
