...
#define PCM_SW_INT_ALL                                                         \
	(PCM_SW_INT7 | PCM_SW_INT6 | PCM_SW_INT5 | PCM_SW_INT4 | PCM_SW_INT3 | \
...
spm_check_talking_get_lpscen(struct spm_lp_scen *lpscen, u32 *spm_flags);

/**************************************
 * Macro and Inline
 **************************************/
#define EVENT_VEC(event, resume, imme, pc)                          \
	(((pc) << 16) | (!!(imme) << 7) | (!!(resume) << 6) | ((event)&0x3f))

#define spm_pr_crit(fmt, args...) pr_crit("[SPM] " fmt, ##args)
#define spm_pr_err(fmt, args...) pr_err("[SPM] " fmt, ##args)
#define spm_pr_warn(fmt, args...) pr_warn("[SPM] " fmt, ##args)
...
{
	phys_addr_t pa = virt_to_phys(lm_alias(base));
