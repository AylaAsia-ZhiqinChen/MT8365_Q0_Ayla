...
pmic_read_interface_nolock(unsigned int RegNum, unsigned int *val,
...
unsigned int __attribute__((weak))
pmic_config_interface_nolock(unsigned int RegNum, unsigned int val,
			     unsigned int MASK, unsigned int SHIFT)
{
	return 0;
}
...
void __spm_reset_and_init_pcm(const struct pcm_desc *pcmdesc)
{
	...
	WARN_ON((spm_read(SPM_PCM_FSM_STA) & 0x3fffff) !=
		PCM_FSM_STA_DEF); /* PCM reset failed */
	...
	spm_write(SPM_PCM_CON1,
		  con1 | CON1_CFG_KEY | CON1_EVENT_LOCK_EN |
...
void __spm_kick_pcm_to_run(const struct pwr_ctrl *pwrctrl)
{
	...
	spm_write(SPM_CLK_CON,
		  (spm_read(SPM_CLK_CON) & ~CC_LOCK_INFRA_DCM) |
				      ...
				      bool suspend)
{
	...
	spm_debug(
		"wake up by%s, timer_out = %u, r13 = 0x%x, debug_flag = 0x%x, apsrc_cnt = %d\n",
...
{
	spm_write(SPM_PCM_RESERVE,
		  (spm_read(SPM_PCM_RESERVE) & ~PCM_CMD_MASK) | cmd);
}
#endif

/* Pwrap API */
enum { IDX_DI_VPROC_CA7_NORMAL,
       /* 0 */			       /* PMIC_WRAP_PHASE_DEEPIDLE */
...
enum { IDX_SO_VSRAM_CA15L_NORMAL,
       /* 0 */			       /* PMIC_WRAP_PHASE_SODI */
...
static int __init spm_get_pwrap_base(void)
{
	...
	if (!node)
		spm_pr_warn("find pwrap node failed\n");
	pwrap_base = of_iomap(node, 0);
	if (!pwrap_base)
		spm_pr_warn("get pwrap_base failed\n");
	...
	if (pwrap_node) {
		...
		if (IS_ERR(pmic_regmap))
			spm_pr_err("cannot get pmic_regmap\n");
	} else {
		spm_pr_err("pwrap node has not register regmap\n");
	}
...
void __spm_set_pmic_phase(enum pmic_wrap_phase_id phase)
{
	...
	if (pw.addr[0].cmd_addr == 0) {
		spm_pr_warn("pmic table not initialized\n");
		spm_pmic_table_init();
	}
