...
static void spm_trigger_wfi_for_sleep(struct pwr_ctrl *pwrctrl)
{
	if (is_cpu_pdn(pwrctrl->pcm_flags)) {
		spm_dormant_sta = mt_cpu_dormant(
			CPU_SHUTDOWN_MODE /* | DORMANT_SKIP_WFI */);
...
/* disable PCM WDT to stop count if needed */
	...
	spm_write(SPM_PCM_CON1,
		  CON1_CFG_KEY | (spm_read(SPM_PCM_CON1) & ~CON1_PCM_WDT_EN));
#endif
					   ...
					   struct pcm_desc *pcmdesc)
{
	...
	spm_debug("suspend dormant state = %d\n", spm_dormant_sta);
	spm_debug("log_wakesta_index = %d\n", log_wakesta_index);
...
static u32 spm_get_wake_period(int pwake_time, unsigned int last_wr)
{
	...
	if (pwake_time < 0) {
		...
		period = get_dynamic_period(last_wr != WR_PCM_TIMER ? 1 : 0,
					    SPM_WAKE_PERIOD, 1);
		if (period <= 0) {
			spm_pr_warn("CANNOT GET PERIOD FROM FUEL GAUGE\n");
...
{
	spm_notice(
		"PWAKE_EN:%d, PCMWDT_EN:%d, BYPASS_SYSPWREQ:%d, I2C_CHANNEL:%d\n",
