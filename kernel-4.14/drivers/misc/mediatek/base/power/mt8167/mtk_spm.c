...
void __weak aee_kernel_warning_api(const char *file, const int line,
...
static irqreturn_t spm_irq0_handler(int irq, void *dev_id)
{
	...
	if (isr & (ISRS_SW_INT0 | ISRS_PCM_RETURN))
		spm_debug("IRQ0 HANDLER SHOULD NOT BE EXECUTED (0x%x)\n", isr);
...
static irqreturn_t spm_irq_aux_handler(u32 irq_id)
{
	...
	spm_debug("IRQ%u HANDLER SHOULD NOT BE EXECUTED (0x%x)\n",
			irq_id, isr);
...
static int spm_irq_register(void)
{
	...
	for (i = 0; i < ARRAY_SIZE(irqdesc); i++) {
		err = request_irq(irqdesc[i].irq, irqdesc[i].handler,
				  ...
				  NULL);
		if (err) {
			spm_pr_err("FAILED TO REQUEST IRQ%d (%d)\n", i, err);
			r = -EPERM;
		}
	}
#if 0
...
static void spm_register_init(void)
{
	...
	if (!node)
		spm_pr_err("find mt8167-scpsys node failed\n");
	spm_base = of_iomap(node, 0);
	if (!spm_base)
		spm_pr_err("base spm_base failed\n");

	spm_irq_0 = irq_of_parse_and_map(node, 0);
	if (!spm_irq_0)
		spm_pr_err("get spm_irq_0 failed\n");
	spm_irq_1 = irq_of_parse_and_map(node, 1);
	if (!spm_irq_1)
		spm_pr_err("get spm_irq_1 failed\n");
	spm_irq_2 = irq_of_parse_and_map(node, 2);
	if (!spm_irq_2)
		spm_pr_err("get spm_irq_2 failed\n");
	spm_irq_3 = irq_of_parse_and_map(node, 3);
	if (!spm_irq_3)
		spm_pr_err("get spm_irq_3 failed\n");

	spm_debug("spm_base = %p\n", spm_base);
	spm_debug("spm_irq_0 = %d, spm_irq_1 = %d, spm_irq_2 = %d, spm_irq_3 = %d\n",
	...
	/* CC_CLKSQ0_SEL is DONT-CARE in Suspend since PCM_PWR_IO_EN[0]=1 in
	 * Suspend
	 */
	...
	/* unmask gce_busy_mask (set to 1b1); otherwise, gce (cmd-q) can not
	 * notify SPM to exit EMI self-refresh
	 */
...
static int __init spm_module_init(void)
{
	int r = 0;
/*
 * This following setting is moved to LK by WDT init, because of DTS init level
 * issue
 */
#if 0
	...
	if (wd_api->wd_spmwdt_mode_config) {
		...
		spm_pr_err("FAILED TO GET WD API\n");
		r = -ENODEV;
	}
#endif
			     ...
			     unsigned int window_len)
{
	...
	spm_write(SPM_SLEEP_TWAM_CON,
		  ((sig3 << 27) | (sig2 << 22) | (sig1 << 17) | (sig0 << 12) |
		   (TWAM_MON_TYPE_HIGH << 4) | (TWAM_MON_TYPE_HIGH << 6) |
		   (TWAM_MON_TYPE_HIGH << 8) | (TWAM_MON_TYPE_HIGH << 10) |
		   (speed_mode ? TWAM_CON_SPEED_EN : 0) | TWAM_CON_EN));
	spm_write(SPM_SLEEP_TWAM_WINDOW_LEN, window_len);
	spin_unlock_irqrestore(&__spm_lock, flags);

	spm_pr_crit("enable TWAM for signal %u, %u, %u, %u (%u)\n", sig0, sig1,
		 sig2, sig3, speed_mode);
}
...
static struct ddrphy_golden_cfg ddrphy_setting[] = {
	{0x5c0, 0x063c0000},
	{0x5c4, 0x00000000},
	/* temp remove mempll2/3 control for golden setting refine */
	{0x5c8, 0x0000fC10},
	{0x5cc, 0x40101000},
};
...
int spm_golden_setting_cmp(bool en)
{
	...
	for (i = 0; i < ddrphy_num; i++) {
		if (spm_read(spm_ddrphy_base + ddrphy_setting[i].addr) !=
				ddrphy_setting[i].value) {
			spm_debug("dramc setting mismatch addr: %p, val: 0x%x\n",
				spm_ddrphy_base + ddrphy_setting[i].addr,
				spm_read(spm_ddrphy_base +
...
unsigned int spm_get_cpu_pwr_status(void)
{
	...
	for (i = 0; i < num_possible_cpus(); i++) {
		val = spm_read(SPM_SPMC_MP0_CPU0_PWR_CON + 0x4 * i) &
		      SPMC_CPU_PWR_STA_MASK;
		if (val != 0x0)
