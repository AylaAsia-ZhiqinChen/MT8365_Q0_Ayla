...
static const struct freqhopping_ssc ssc_mempll_setting[] = {
	...
	/* 0 ~ -2% 0812 Requested by de-sense SA Leonard.Liang */
	{0, 0, 0, 0, 0, 0}
};
...
static void mt_fh_hal_default_conf(void)
{
	...
	/* /< Set FHCTL_DSSC_CFG(0x1000CF14), Bit3 is RF3(LTE) */
	/* SSC control. Clear to 0 is enable. */
	fh_set_field(REG_FHCTL_DSSC_CFG, (0x1U<<3), 0);
	/* /< RF(3) LTE BAN control */
...
static void fh_sync_ncpo_to_fhctl_dds(enum FH_PLL_ID pll_id)
{
	...
	if (pll_id == FH_MEM_PLLID)
		fh_write32(reg_dst,
			((fh_read32(reg_src) >> 10) & MASK21b)|BIT32);
	else
...
int mt_dfs_mmpll(unsigned int target_dds)  /* mmpll dfs mode */
{
	unsigned long	flags = 0;
...
static int fh_dumpregs_proc_read(struct seq_file *m, void *v)
{
	...
	seq_printf(m, "ARMCA7:0x%08x MAIN:0x%08x MEM:0x%08x ",
		...
		fh_read32(REG_MEMPLL_CON0));
	seq_printf(m, "MM:0x%08x TVD:0x%08x LVDS:0x%08x\r\n",
		fh_read32(REG_MMPLL_CON0), fh_read32(REG_TVDPLL_CON0),
		fh_read32(REG_LVDSPLL_CON0));

	seq_puts(m, "\r\nPLL_CON1 :\r\n");
	seq_printf(m, "ARMCA7:0x%08x MAIN:0x%08x MEM:0x%08x ",
		fh_read32(REG_ARMCA7PLL_CON1), fh_read32(REG_MAINPLL_CON1),
		fh_read32(REG_MEMPLL_CON1));
	seq_printf(m, "MM:0x%08x TVD:0x%08x LVDS:0x%08x\r\n",
		fh_read32(REG_MMPLL_CON1), fh_read32(REG_TVDPLL_CON1),
		fh_read32(REG_LVDSPLL_CON1));
...
static void __reg_tbl_init(void)
{
	...
	/* g_reg_cfg[i], g_reg_updnlmt[i], g_reg_mon[i], g_reg_dvfs[i], */
	/* g_reg_pll_con1[i]); */
#else
