...
unsigned int hdmi_sys_read(unsigned short u2Reg)
{
	...
	internal_hdmi_read(hdmi_ref_reg[MMSYS_CONFIG]
		+ u2Reg, &u4Data);
	HDMI_DRV_LOG("[R]addr = 0x%04x, data = 0x%08x\n",
		u2Reg, u4Data);
	return u4Data;
...
void hdmi_sys_write(unsigned short u2Reg, unsigned int u4Data)
{
	HDMI_DRV_LOG("[W]addr= 0x%04x, data = 0x%08x\n",
		u2Reg, u4Data);
	internal_hdmi_write(hdmi_ref_reg[MMSYS_CONFIG]
		+ u2Reg, u4Data);
}
...
unsigned int hdmi_hdmitopck_read(unsigned short u2Reg)
{
	...
	internal_hdmi_read(hdmi_ref_reg[TOPCK_GEN]
		+ u2Reg, &u4Data);
	HDMI_DRV_LOG("[R]addr = 0x%04x, data = 0x%08x\n",
		u2Reg, u4Data);
	return u4Data;
...
void hdmi_hdmitopck_write(unsigned short u2Reg, unsigned int u4Data)
{
	HDMI_DRV_LOG("[W]addr= 0x%04x, data = 0x%08x\n",
		u2Reg, u4Data);
	internal_hdmi_write(hdmi_ref_reg[TOPCK_GEN]
		+ u2Reg, u4Data);
...
unsigned int hdmi_infrasys_read(unsigned short u2Reg)
{
	...
	internal_hdmi_read(hdmi_ref_reg[INFRA_SYS]
		+ u2Reg, &u4Data);
	HDMI_DRV_LOG("[R]addr = 0x%04x, data = 0x%08x\n",
		u2Reg, u4Data);
	return u4Data;
...
void hdmi_infrasys_write(unsigned short u2Reg, unsigned int u4Data)
{
	HDMI_DRV_LOG("[W]addr= 0x%04x, data = 0x%08x\n",
		u2Reg, u4Data);
	internal_hdmi_write(hdmi_ref_reg[INFRA_SYS]
		+ u2Reg, u4Data);
...
unsigned int hdmi_pll_read(unsigned short u2Reg)
{
	...
	internal_hdmi_read(hdmi_ref_reg[AP_CCIF0]
		+ u2Reg, &u4Data);
...
void hdmi_pll_write(unsigned short u2Reg, unsigned int u4Data)
{
	HDMI_PLL_LOG("[W]addr= 0x%04x, data = 0x%08x\n",
		u2Reg, u4Data);
	internal_hdmi_write(hdmi_ref_reg[AP_CCIF0]
		+ u2Reg, u4Data);
}
...
unsigned int hdmi_pad_read(unsigned short u2Reg)
{
	...
	internal_hdmi_read(hdmi_ref_reg[GPIO_REG]
		+ u2Reg, &u4Data);
	HDMI_DRV_LOG("[R]addr = 0x%04x, data = 0x%08x\n",
		u2Reg, u4Data);
	return u4Data;
...
void hdmi_pad_write(unsigned short u2Reg, unsigned int u4Data)
{
	HDMI_DRV_LOG("[W]addr = 0x%04x, data = 0x%08x\n",
		u2Reg, u4Data);
	internal_hdmi_write(hdmi_ref_reg[GPIO_REG]
		+ u2Reg, u4Data);
}
...
void vHotPlugPinInit(struct platform_device *pdev)
{
	...
	if (IS_ERR(pinctrl)) {
		ret = PTR_ERR(pinctrl);
		pr_debug("hdmi HPD pin, failure of setting\n");
	}

	pins_default = pinctrl_lookup_state(pinctrl, "hdmi_hpd");
	if (IS_ERR(pins_default)) {
		ret = PTR_ERR(pins_default);
		pr_debug("cannot find hdmi HPD pinctrl default");
	} else {
...
unsigned char bCheckStatus(unsigned char bMode)
{
	...
	if ((STATUS_HTPLG & bMode) == STATUS_HTPLG)
		bStatus = IS_HDMI_HTPLG() ?
		(bStatus | STATUS_HTPLG) : (bStatus & (~STATUS_HTPLG));
...
{
	if (bOn) {
		vWriteIoPllMsk(HDMI_CON7,
			RG_HTPLL_AUTOK_EN, RG_HTPLL_AUTOK_EN);
		vWriteIoPllMsk(HDMI_CON6,
			0, RG_HTPLL_RLH_EN);
		vWriteIoPllMsk(HDMI_CON6,
			(0x3 << RG_HTPLL_POSDIV), RG_HTPLL_POSDIV_MASK);
		vWriteIoPllMsk(HDMI_CON2,
			RG_HDMITX_EN_MBIAS, RG_HDMITX_EN_MBIAS);
		udelay(10);
		vWriteIoPllMsk(HDMI_CON6,
			RG_HTPLL_EN, RG_HTPLL_EN);
		vWriteIoPllMsk(HDMI_CON2,
			RG_HDMITX_EN_TX_CKLDO, RG_HDMITX_EN_TX_CKLDO);
		vWriteIoPllMsk(HDMI_CON0,
			(0xf << RG_HDMITX_EN_SLDO), RG_HDMITX_EN_SLDO_MASK);
		udelay(200);
		vWriteIoPllMsk(HDMI_CON2,
			RG_HDMITX_MBIAS_LPF_EN, RG_HDMITX_MBIAS_LPF_EN);
		vWriteIoPllMsk(HDMI_CON2,
			RG_HDMITX_EN_TX_POSDIV, RG_HDMITX_EN_TX_POSDIV);
		vWriteIoPllMsk(HDMI_CON0,
			(0xf << RG_HDMITX_EN_SER), RG_HDMITX_EN_SER_MASK);
		vWriteIoPllMsk(HDMI_CON0,
			(0xf << RG_HDMITX_EN_PRED), RG_HDMITX_EN_PRED_MASK);
		vWriteIoPllMsk(HDMI_CON0,
			(0xf << RG_HDMITX_EN_DRV), RG_HDMITX_EN_DRV_MASK);
		udelay(3);
	} else {
...
#else
	vWriteHdmiSYSMsk(DISP_HDMI_SYS_CFG_01,
		HDMI_PSECURE_EN, HDMI_PSECURE_EN_MASK);
	udelay(10);
...
void vSetHDMITxPLL(unsigned char bResIndex, unsigned char bdeepmode)
{
	...
	vWriteIoPllMsk(HDMI_CON6, (pre_div << RG_HTPLL_PREDIV),
		RG_HTPLL_PREDIV_MASK);
	vWriteIoPllMsk(HDMI_CON6, (post_div << RG_HTPLL_POSDIV),
	...
	vWriteIoPllMsk(HDMI_CON2, ((POSDIV[u4Feq][bdeepmode - 1])
		<< RG_HDMITX_TX_POSDIV), RG_HDMITX_TX_POSDIV_MASK);
	vWriteIoPllMsk(HDMI_CON6, ((FBKSEL[u4Feq][bdeepmode - 1])
		<< RG_HTPLL_FBKSEL), RG_HTPLL_FBKSEL_MASK);
	vWriteIoPllMsk(HDMI_CON6, ((FBKDIV[u4Feq][bdeepmode - 1])
		<< RG_HTPLL_FBKDIV), RG_HTPLL_FBKDIV_MASK);
	vWriteIoPllMsk(HDMI_CON7, ((DIVEN[u4Feq][bdeepmode - 1])
		<< RG_HTPLL_DIVEN), RG_HTPLL_DIVEN_MASK);
	vWriteIoPllMsk(HDMI_CON6, ((HTPLLBP[u4Feq][bdeepmode - 1])
		<< RG_HTPLL_BP), RG_HTPLL_BP_MASK);
	vWriteIoPllMsk(HDMI_CON6, ((HTPLLBC[u4Feq][bdeepmode - 1])
		<< RG_HTPLL_BC), RG_HTPLL_BC_MASK);
	vWriteIoPllMsk(HDMI_CON6, ((HTPLLBR[u4Feq][bdeepmode - 1])
		<< RG_HTPLL_BR), RG_HTPLL_BR_MASK);
	vWriteIoPllMsk(HDMI_CON1, pre_imp_en <<
		RG_HDMITX_PRED_IMP_SHIFT, RG_HDMITX_PRED_IMP);
	vWriteIoPllMsk(HDMI_CON1, (pre_ibias <<
		RG_HDMITX_PRED_IBIAS), RG_HDMITX_PRED_IBIAS_MASK);
	vWriteIoPllMsk(HDMI_CON0, (imp_en <<
		RG_HDMITX_EN_IMP), RG_HDMITX_EN_IMP_MASK);
	vWriteIoPllMsk(HDMI_CON1, (imp_clk <<
		RG_HDMITX_DRV_IMP), RG_HDMITX_DRV_IMP_MASK);
	vWriteIoPllMsk(HDMI_CON4, (imp_d0 <<
		RG_HDMITX_D0_IMP_SHIFT) | (imp_d1 <<
		RG_HDMITX_D1_IMP_SHIFT) | (imp_d2 <<
		RG_HDMITX_D2_IMP_SHIFT), RG_HDMITX_D0_IMP_MASK
		| RG_HDMITX_D1_IMP_MASK | RG_HDMITX_D2_IMP_MASK);
	vWriteIoPllMsk(HDMI_CON0, (drv_ibias <<
		RG_HDMITX_DRV_IBIAS), RG_HDMITX_DRV_IBIAS_MASK);
}
...
void vEnableDeepColor(unsigned char ui1Mode)
{
	...
	if (u4Data == COLOR_8BIT_MODE)
		vWriteHdmiSYSMsk(DISP_HDMI_SYS_CFG_01,
			u4Data, DEEP_COLOR_MODE_MASK | DEEP_COLOR_EN);
	else
		vWriteHdmiSYSMsk(DISP_HDMI_SYS_CFG_01,
			u4Data | DEEP_COLOR_EN, DEEP_COLOR_MODE_MASK
			| DEEP_COLOR_EN);
...
void vConfigHdmiSYS(unsigned char bResIndex)
{
	...
	vWriteHdmiTOPCKMsk(CLK_MUX_SEL8, 0,
		RG_FDPI1_MUX_SEL_MASK);
	if (u4Feq == 2)
		vWriteHdmiTOPCKMsk(CLK_MUX_SEL8,
			DPI1_148M_CLK_SEL, RG_FDPI1_MUX_SEL_MASK);
	else if (u4Feq == 1)
		vWriteHdmiTOPCKMsk(CLK_MUX_SEL8,
			DPI1_78M_CLK_SEL, RG_FDPI1_MUX_SEL_MASK);
	else if (u4Feq == 0)
		vWriteHdmiTOPCKMsk(CLK_MUX_SEL8,
			DPI1_37M_CLK_SEL, RG_FDPI1_MUX_SEL_MASK);
	else
		pr_debug("Invalid resolution\n");
	vWriteHdmiTOPCKMsk(CLK_GATING_CTRL8,
		RG_FDPI1_SW_CG_CLR, RG_FDPI1_SW_CG);

	vWriteHdmiSYSMsk(MMSYS_CG_CON1,
		DPI1_PIXEL_CLK_CG_CLR | DPI1_ENGINE_CLK_CG_CLR,
		DPI1_PIXEL_CLK_CG | DPI1_ENGINE_CLK_CG);
	vWriteHdmiSYSMsk(MMSYS_CG_CON1,
		HDMI_PIXEL_CLK_CG_CLR, HDMI_PIXEL_CLK_CG);

	if (_stAvdAVInfo.e_hdmi_aud_in == SV_SPDIF) {
		vWriteHdmiSYSMsk(MMSYS_CG_CON1,
			HDMI_SPDIF_CLK_CG_CLR, HDMI_SPDIF_CLK_CG_CLR);
		vWriteHdmiSYSMsk(MMSYS_CG_CON1,
			HDMI_ADSP_BCK_CG, HDMI_ADSP_BCK_CG);
	} else {
		vWriteHdmiSYSMsk(MMSYS_CG_CON1,
			HDMI_ADSP_BCK_CG_CLR, HDMI_ADSP_BCK_CG);
		vWriteHdmiGRLMsk(HDCP_STATUS_RESET,
	...
	vWriteHdmiSYSMsk(DISP_HDMI_SYS_CFG_01,
		HDMI_OUT_FIFO_EN, HDMI_OUT_FIFO_EN | MHL_MODE_ON);
	vWriteHdmiSYSMsk(DISP_HDMI_SYS_CFG_01,
		HDMI_PP_MODE, HDMI_PP_MODE);
	vWriteHdmiSYSMsk(MMSYS_CG_CON1,
		HDMI_PLL_CK_CG_CLR, HDMI_PLL_CK_CG);
}
...
void vEnableNotice(unsigned char bOn)
{
	...
	if (bOn == TRUE) {
		...
		bData |= 0x40;
/* temp. solve 720p issue. to avoid audio packet jitter problem */
...
void vEnableNCTSAutoWrite(void)
{
	...
	bData |= NCTS_WRI_ANYTIME;
	/* enabel NCTS can be written in any time */
...
void vSetHdmiI2SChNum(unsigned char bChNum,
...
/* L, R, RC, LFE -> expand S to RR,RL to be  L, R, RR, RL, LFE */
		_stAvdAVInfo.ui2_aud_out_ch.bit.FR = 1;
		_stAvdAVInfo.ui2_aud_out_ch.bit.FL = 1;
...
/* L, R, FC, RC -> expand S to RR,RL to be  L, R, FC, RR, RL */
		_stAvdAVInfo.ui2_aud_out_ch.bit.FR = 1;
		_stAvdAVInfo.ui2_aud_out_ch.bit.FL = 1;
		_stAvdAVInfo.ui2_aud_out_ch.bit.FC = 1;
		_stAvdAVInfo.ui2_aud_out_ch.bit.LFE = 0;
...
/* L, R, FC, RC, LFE -> expand S to RR,RL to be  L, R, FC, RR, RL, LFE */
		_stAvdAVInfo.ui2_aud_out_ch.bit.FR = 1;
		_stAvdAVInfo.ui2_aud_out_ch.bit.FL = 1;
		_stAvdAVInfo.ui2_aud_out_ch.bit.FC = 1;
		_stAvdAVInfo.ui2_aud_out_ch.bit.LFE = 1;
...
void vSetHDMIAudioIn(void)
{
	...
	if ((_stAvdAVInfo.e_hdmi_aud_in == SV_SPDIF) &&
		(_stAvdAVInfo.e_aud_code == AVD_DST)) {
	...
	if (_stAvdAVInfo.e_hdmi_aud_in == SV_SPDIF) {
		...
		vWriteHdmiGRLMsk(HDCP_STATUS_RESET,
			RG_SPD_IIS_SEL | RG_N_DIV2,
			RG_SPD_IIS_SEL | RG_N_DIV2);
		vSetHdmiI2SChNum(_stAvdAVInfo.ui1_aud_out_ch_number,
			bChMapping);
...
void vHalHDMI_NCTS(unsigned char bAudioFreq,
	unsigned char bPix, unsigned char bDeepMode)
{
	...
	if (bDeepMode == HDMI_NO_DEEP_COLOR) {
		for (bTemp = 0; bTemp < NCTS_BYTES; bTemp++) {
			if ((bAudioFreq < 7) && (bPix < 9))
				bData1[bTemp] =
				HDMI_NCTS[bAudioFreq][bPix][bTemp];
		}

		u4NTemp = (bData1[4] << 16) |
			(bData1[5] << 8) | (bData1[6]);	/* N */
		u4Temp = (bData1[0] << 24) | (bData1[1] << 16)
			| (bData1[2] << 8) | (bData1[3]);/* CTSvalue */
		...
		u4NTemp = (bData1[4] << 16) | (bData1[5] << 8)
			| (bData1[6]);	/* N */
		u4Temp = (bData1[0] << 24) | (bData1[1] << 16)
			| (bData1[2] << 8) | (bData1[3]);
...
void vHDMI_NCTS(unsigned char bHDMIFsFreq,
	unsigned char bResolution, unsigned char bdeepmode)
{
	unsigned char bPix;

	HDMI_AUDIO_FUNC();

	vWriteHdmiGRLMsk(DUMMY_304, AUDIO_I2S_NCTS_SEL_64,
		AUDIO_I2S_NCTS_SEL);
...
void vSetHDMIFS(unsigned char bFs, unsigned char fgAclInv)
{
	...
	if (fgAclInv) {
		vHdmiAclkInv(TRUE);
		/*fix 192kHz, SPDIF downsample noise issue, ACL iNV */
	} else {
		vHdmiAclkInv(FALSE);
		/* fix 192kHz, SPDIF downsample noise issue */
	}
}
...
void vHDMIAudioSRC(unsigned char ui1hdmifs,
	unsigned char ui1resindex, unsigned char bdeepmode)
{
...
void vHwSet_Hdmi_I2S_C_Status(unsigned char *prLChData,
	unsigned char *prRChData)
{
	unsigned char bData;
...
void vHDMI_I2S_C_Status(void)
{
	...
	vHwSet_Hdmi_I2S_C_Status(&bhdmi_LCh_status[0],
		&bhdmi_RCh_status[0]);

}

void vHalSendAudioInfoFrame(unsigned char bData1,
	unsigned char bData2, unsigned char bData4,
...
void vSendAudioInfoFrame(void)
{
	...
	vHalSendAudioInfoFrame(_bAudInfoFm[0],
		_bAudInfoFm[1], _bAudInfoFm[3], _bAudInfoFm[4]);

}

void vChgHDMIAudioOutput(unsigned char ui1hdmifs,
	unsigned char ui1resindex, unsigned char bdeepmode)
{
...
void vHalSendAVIInfoFrame(unsigned char *pr_bData)
{
	...
	unsigned char bData1 = 0, bData2 = 0;
	unsigned char bData3 = 0, bData4 = 0, bData5 = 0;
	unsigned char bData;
...
void vSendAVIInfoFrame(unsigned char ui1resindex,
	unsigned char ui1colorspace)
{
	HDMI_VIDEO_FUNC();
	if (i4SharedInfo(SI_EDID_VSDB_EXIST) == FALSE)
		return;
				     ...
				     unsigned char b3dstruct)
{
	...
	bVS_CHSUM = VS_VERS + VS_TYPE + VS_LEN +
		bPB1 + bPB2 + bPB3 + bPB4 + bPB5;
	bVS_CHSUM = 0x100 - bVS_CHSUM;

	vWriteByteHdmiGRL(GRL_IFM_PORT, bVS_CHSUM);
	/* check sum */
	vWriteByteHdmiGRL(GRL_IFM_PORT, bPB1);
	/* 24bit IEEE Registration Identifier */
	vWriteByteHdmiGRL(GRL_IFM_PORT, bPB2);
	/* 24bit IEEE Registration Identifier */
	vWriteByteHdmiGRL(GRL_IFM_PORT, bPB3);
	/* 24bit IEEE Registration Identifier */
	vWriteByteHdmiGRL(GRL_IFM_PORT, bPB4);
	/* HDMI_Video_Format */
	vWriteByteHdmiGRL(GRL_IFM_PORT, bPB5);
	/* 3D_Structure/4k2k */
...
void vSendVendorSpecificInfoFrame(unsigned char ui1resindex)
{
	...
	if (fg3DRes == TRUE)
		vHalSendVendorSpecificInfoFrame(fg3DRes,
			bResTableIndex, b3DStruct);

}

void vChgHDMIVideoResolution(unsigned char ui1resindex,
	unsigned char ui1colorspace, unsigned char ui1hdmifs,
...
void vChgtoSoftNCTS(unsigned char ui1resindex,
	unsigned char ui1audiosoft, unsigned char ui1hdmifs,
	unsigned char bdeepmode)
{
	HDMI_AUDIO_FUNC();

	vHwNCTSOnOff(ui1audiosoft);
	/* change to software NCTS; */
	vHDMI_NCTS(ui1hdmifs, ui1resindex, bdeepmode);
...
void vShowHpdRsenStatus(void)
{
	...
	if (i4SharedInfo(SI_HDMI_RECEIVER_STATUS)
		== HDMI_PLUG_IN_ONLY)
		pr_debug(
		"[HDMI]SI_HDMI_RECEIVER_STATUS = HDMI_PLUG_IN_ONLY\n");
		...
		== HDMI_PLUG_OUT)
		pr_debug(
		"[HDMI]SI_HDMI_RECEIVER_STATUS = HDMI_PLUG_OUT\n");
	else
...
{
	pr_debug("=======Audio inforFrame Start =========\n");
	...
	pr_debug("Data Byte (1~8)  = 0x%x  0x%x  0x%x  0x%x  0x%x  0x%x  0x%x  0x%x\n",
		_bSpdInf[0], _bSpdInf[1], _bSpdInf[2], _bSpdInf[3],
		_bSpdInf[4], _bSpdInf[5], _bSpdInf[6], _bSpdInf[7]);
	pr_debug("Data Byte (9~16) = 0x%x  0x%x  0x%x  0x%x  0x%x  0x%x  0x%x  0x%x\n",
		_bSpdInf[8], _bSpdInf[9], _bSpdInf[10], _bSpdInf[11],
		_bSpdInf[12], _bSpdInf[13], _bSpdInf[14], _bSpdInf[15]);
	pr_debug("Data Byte (17~24)= 0x%x  0x%x  0x%x  0x%x  0x%x  0x%x  0x%x  0x%x\n",
		_bSpdInf[16], _bSpdInf[17], _bSpdInf[18], _bSpdInf[19],
		_bSpdInf[20], _bSpdInf[21], _bSpdInf[22], _bSpdInf[23]);
	...
	if (fgIsAcpEnable()) {
		...
		if (_bAcpType == 0) {
			...
			pr_debug("Data Byte (1~8)= 0x%x  0x%x  0x%x  0x%x  0x%x  0x%x  0x%x  0x%x\n",
				_bAcpData[0], _bAcpData[1],
				_bAcpData[2], _bAcpData[3],
				_bAcpData[4], _bAcpData[5],
				_bAcpData[6], _bAcpData[7]);
			pr_debug("Data Byte (9~16)= 0x%x  0x%x  0x%x  0x%x  0x%x  0x%x  0x%x  0x%x\n",
				_bAcpData[8], _bAcpData[9],
				_bAcpData[10], _bAcpData[11],
				_bAcpData[12], _bAcpData[13],
				_bAcpData[14], _bAcpData[15]);
		} else if (_bAcpType == 1) {
			...
			pr_debug("CCI_1 Byte (1~8)= 0x%x  0x%x  0x%x  0x%x  0x%x  0x%x  0x%x  0x%x\n",
				_bAcpData[0], _bAcpData[1],
				_bAcpData[2], _bAcpData[3],
				_bAcpData[4], _bAcpData[5],
				_bAcpData[6], _bAcpData[7]);
	...
	if (fgIsISRC1Enable()) {
		...
		pr_debug("Data Byte (1~8)= 0x%x  0x%x  0x%x  0x%x  0x%x  0x%x  0x%x  0x%x\n",
			_bIsrc1Data[0], _bIsrc1Data[1],
			_bIsrc1Data[2], _bIsrc1Data[3],
			_bIsrc1Data[4], _bIsrc1Data[5],
			_bIsrc1Data[6], _bIsrc1Data[7]);
		pr_debug
	...
	if (fgIsISRC2Enable()) {
		...
		pr_debug("Data Byte (1~8)= 0x%x  0x%x  0x%x  0x%x  0x%x  0x%x  0x%x  0x%x\n",
			_bIsrc1Data[8], _bIsrc1Data[9],
			_bIsrc1Data[10], _bIsrc1Data[11],
			_bIsrc1Data[12], _bIsrc1Data[13],
			_bIsrc1Data[14], _bIsrc1Data[15]);
		pr_debug
...
void vShowHdmiAudioStatus(void)
{
	...
	pr_debug("[HDMI]HDMI output ACR N= %d, CTSvalue = %d\n",
		u4ReadNValue(), u4ReadCtsValue());
}

void vShowHdmiDrmHdcpStatus(void)
{
	pr_debug("[HDMI]DrmHdcp _bflagvideomute =%d\n", _bflagvideomute);
	pr_debug("[HDMI]DrmHdcp _bflagaudiomute =%d\n", _bflagaudiomute);
	pr_debug("[HDMI]DrmHdcp _bsvpvideomute =%d\n", _bsvpvideomute);
	pr_debug("[HDMI]DrmHdcp _bsvpaudiomute =%d\n", _bsvpaudiomute);
	pr_debug("[HDMI]DrmHdcp _bNeedSwHdcp =%d\n", _bHdcpOff);
	pr_debug("[HDMI]DrmHdcp _bHdcpOff =%d\n", _bHdcpOff);
}
...
void vCheckHDMICRC(void)
{
	...
	for (i = 0; i < 10; i++) {
		vWriteHdmiGRLMsk(CRC_CTRL, clr_crc_result,
			clr_crc_result | init_crc);
		vWriteHdmiGRLMsk(CRC_CTRL, init_crc,
			clr_crc_result | init_crc);
...
void vCheckHDMICLKPIN(void)
{
	...
	for (i = 0; i < 5; i++) {
		vWriteHdmiSYSMsk(HDMI_SYS_FMETER, TRI_CAL | CALSEL,
			TRI_CAL | CALSEL);
		...
		u4Data = ((dReadHdmiSYS(HDMI_SYS_FMETER) &
			(0xffff0000)) >> 16) * 26000 / 1024;
