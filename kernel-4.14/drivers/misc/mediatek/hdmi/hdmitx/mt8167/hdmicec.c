...
/* HDMI interrupt use flag, when TRUE,it means use the hdmi shell,
 *or it use the CEC interrupt
 */
...
#define	RegReadFldAlign(reg16, fld) ((hdmi_cec_read(reg16)>> \
	(Fld_shft(fld)))&(hdmi_cec_2n(Fld_wid(fld))))
#define	vRegWriteFldAlign(reg16, val, fld) \
	(hdmi_cec_write(reg16, \
	(((hdmi_cec_read(reg16))&(hdmi_cec_maskvalue(Fld_wid(fld), \
	Fld_shft(fld))))|(val<<(Fld_shft(fld))))))
...
#define ENABLE_INT_DATA_RDY(onoff) vRegWriteFldAlign( \
	RX_EVENT, onoff, I_EN_DATA)
#define ENABLE_INT_HEADER_RDY(onoff) vRegWriteFldAlign( \
	RX_EVENT, onoff, I_EN_HEADER)
#define ENABLE_INT_MODE_RDY(onoff) vRegWriteFldAlign( \
	RX_EVENT, onoff, I_EN_MODE)
#define ENABLE_INT_OV(onoff) vRegWriteFldAlign( \
	RX_EVENT, onoff, I_EN_OV)
#define ENABLE_INT_PULSE(onoff) vRegWriteFldAlign( \
	RX_EVENT, onoff, I_EN_PULSE)
#define ENABLE_INT_BR_SB_RDY(onoff) vRegWriteFldAlign( \
	RX_EVENT, onoff, I_EN_BR_SB)
#define ENABLE_INT_SB_RDY(onoff) vRegWriteFldAlign( \
	RX_EVENT, onoff, I_EN_SB)
#define ENABLE_INT_BR_RDY(onoff) vRegWriteFldAlign( \
	RX_EVENT, onoff, I_EN_BR)
...
#define ENABLE_INT_UN(onoff) vRegWriteFldAlign( \
	TX_EVENT, onoff, I_EN_UN)
#define ENABLE_INT_FAIL(onoff) vRegWriteFldAlign( \
	TX_EVENT, onoff, I_EN_FAIL)
#define ENABLE_INT_LOW(onoff) vRegWriteFldAlign( \
	TX_EVENT, onoff, I_EN_LOW)
#define ENABLE_INT_BS(onoff) vRegWriteFldAlign( \
	TX_EVENT, onoff, I_EN_BS)
#define ENABLE_INT_RB(onoff) vRegWriteFldAlign( \
	TX_EVENT, onoff, I_EN_RB)
...
void vRegWrite4BMsk(unsigned short reg16,
...
void hdmi_cec_init(void)
{
	...
	vRegWrite4B(CEC_CKGEN, 0x000a0082);
	/* 3MHz //different from BD /100k */
	vRegWrite4B(TR_TEST, 0x40004019);
	/* Bpad enable¡BTx compared timing 0x19 */
...
static unsigned char CEC_rx_enqueue(struct CEC_FRAME_DESCRIPTION_IO *frame)
{
	...
	memcpy(&(CEC_rx_msg_queue[CEC_rxQ_write_idx]),
		frame, sizeof(struct CEC_FRAME_DESCRIPTION_IO));
...
static void _CEC_Receiving(void)
{
	...
	if (!IS_INT_BR_RDY()) {
		/* ASSERT(0); */
		pr_debug("[HDMI_CEC] No data available\n");
		return;
	}
...
static void _CEC_Check_Active_Tx_Result(void)
{
	...
	if (IsCECStatus(STATE_TXING_FRAME)) {
		...
		} else if ((ActiveTXFrame->sendidx + 1) ==
			(ActiveTXFrame->size)) {
...
static void _CEC_TX_Queue_Loop(void)
{
	...
	if (IsCECStatus(STATE_TXING_FRAME)) {
		if (IsCECErrorFlag(ERR_TX_MISALARM) && (TX_FAIL_RECORD() != 0)
			...
			ClrCECStatus(STATE_TXING_FRAME);
			RESET_HW_TX();
			pr_debug("[hdmi_cec]ERR_TX_MISALARM\n");
		}
		...
		if (IsCECStatus(STATE_TX_FRAME_SUCCESS)) {
			...
			if (frame == NULL) {
				pr_debug("[hdmi_cec] ack msg frame null\n");
				/* ASSERT(0); */
				return;
			}
		...
		if (IsCECStatus(STATE_TX_NOACK)) {
			frame = _CEC_Get_Cur_TX_Q_Msg();
			if (frame == NULL) {
				pr_debug("[hdmi_cec] noack msg frame null\n");
				/* ASSERT(0); */
				return;
			}
		...
		/* if the tx is not active, send the next message */
	{
		frame = _CEC_Get_Cur_TX_Q_Msg();
		if (frame == NULL) {
			pr_debug("[hdmi_cec] next msg frame null\n");
			/* ASSERT(0); */
			return;
		}
...
static unsigned char CEC_frame_validation(
	struct CEC_FRAME_DESCRIPTION_IO *frame)
{
...
static unsigned char check_and_init_tx_frame(
	struct CEC_FRAME_DESCRIPTION_IO *frame)
{
...
unsigned char _CEC_TX_Enqueue(struct CEC_FRAME_DESCRIPTION_IO *frame)
{
	...
	memcpy(&(CEC_tx_msg_queue[CEC_txQ_write_idx]),
		frame, sizeof(struct CEC_FRAME_DESCRIPTION_IO));
	/* CYJ.NOTE: no critical section */
...
void CTS_RXProcess(struct CEC_FRAME_DESCRIPTION_IO *frame)
{
	...
	if (frame->blocks.opcode == OPCODE_ABORT) {
		...
		CTSTestFrame.reTXcnt = 0;
		CTSTestFrame.txtag = NULL;
		CTSTestFrame.blocks.header.destination =
			frame->blocks.header.initiator;
...
/* /////////////////////Test//////////////////////////////////// */
		...
		_CEC_TX_Enqueue(&CTSTestFrame);
		HDMI_CEC_LOG("Send: OPCODE_FEATURE_ABORT\n");
	} else if (frame->blocks.opcode ==
		...
		_CEC_TX_Enqueue(&CTSTestFrame);
		HDMI_CEC_LOG("Send: OPCODE_REPORT_PHYSICAL_ADDRESS\n");
	}
}
...
static void CEC_rx_msg_notify(unsigned char u1rxmode)
{
	...
	while (1) {
		...
		if (CEC_frame_validation(frame)) {
			...
			PrintFrameDescription(frame);
			HDMI_CEC_COMMAND_LOG(
				"[cec_command]rxing opcode-->success: 0x%x\n",
				frame->blocks.opcode);
...
unsigned char hdmi_cec_isrprocess(unsigned char u1rxmode)
{
	...
	if (IsCECStatus(STATE_TXING_FRAME)) {
		...
		if (IS_INT_LOW()) {
			...
			if (!IS_INT_RB_RDY()) {
				pr_debug(
				"[hdmi_cec]FW is slow to trigger the following blocks\n");
...
void CECMWSetLA(struct CEC_LA_ADDRESS *prLA)
{
	...
	HDMI_CEC_LOG("LA num = 0x%x , LA = 0x%x 0x%x 0x%x\n",
		_rCECLaAddr.ui1_num, _rCECLaAddr.aui1_la[0],
		_rCECLaAddr.aui1_la[1], _rCECLaAddr.aui1_la[2]);
	HDMI_CEC_LOG("PA = %04x\n", _rCECLaAddr.ui2_pa);
}
...
void hdmi_CECMWGet(struct CEC_FRAME_DESCRIPTION_IO *frame)
{
	...
	if (cec_msg_report_pending == 0) {
		pr_debug("[hdim_cec]get cec msg fail\n");
		return;
	}
...
void hdmi_CECMWSend(struct CEC_SEND_MSG_T *msg)
{
	...
	for (i = 0; i < cecMwTxMsg.size - 2; i++)
		cecMwTxMsg.blocks.operand[i] =
			msg->t_frame_info.aui1_operand[i];
