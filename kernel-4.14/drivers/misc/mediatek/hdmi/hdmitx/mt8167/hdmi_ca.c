...
bool fgCaHDMICreate(void)
{
	...
	if (get_boot_mode() != FACTORY_BOOT) {
		...
		/* Should provide strerror style error string in UREE. */
			pr_debug("Create ca_hdmi_handle Error: %d\n", tz_ret);
			return FALSE;
		}
		pr_debug("[HDMI]Create ca_hdmi_handle ok: %d\n", tz_ret);
	}
...
bool fgCaHDMIClose(void)
{
	...
	if (tz_ret != TZ_RESULT_SUCCESS) {
		/* Should provide strerror style error string in UREE. */
		pr_debug("Close ca_hdmi_handle Error: %d\n", tz_ret);
		return FALSE;
	}
	pr_debug("[HDMI]Close ca_hdmi_handle ok: %d\n", tz_ret);
	return TRUE;
}
...
void vCaHDMIWriteReg(unsigned int u4addr, unsigned int u4data)
{
	...
	if (get_boot_mode() != FACTORY_BOOT) {
		if (ca_hdmi_handle == 0) {
			pr_debug("[HDMI] TEE ca_hdmi_handle=0\n");
			return;
		}
	}
	...
	if (get_boot_mode() != FACTORY_BOOT) {
		tz_ret = KREE_TeeServiceCall(ca_hdmi_handle,
			HDMI_TA_WRITE_REG, TZ_ParamTypes2(
			TZPT_VALUE_INPUT, TZPT_VALUE_INPUT), param);
	} else {
	...
	if (tz_ret != TZ_RESULT_SUCCESS)
		pr_debug("[HDMI]CA HDMI_TA_WRITE_REG err:%X\n", tz_ret);
...
void vCaDPI1WriteReg(unsigned int u4addr, unsigned int u4data)
{
	...
	pr_debug("[HDMI_DPI1]W:%X=%X\n", u4addr, u4data);

	if (ca_hdmi_handle == 0) {
		pr_debug("[HDMI_DPI1] TEE ca_dpi1_handle=0\n");
		return;
	}
...
bool fgCaHDMIGetAKsv(unsigned char *pdata)
{
	...
	pr_debug("[HDMI]hdcp aksv : %x %x %x %x %x\n",
...
bool fgCaHDMILoadHDCPKey(void)
{
	...
	if (tz_ret != TZ_RESULT_SUCCESS) {
		pr_debug("[HDMI]CA HDMI_TA_LOAD_HDCP_KEY err:%X\n", tz_ret);
		return FALSE;
	}
	return TRUE;
}
...
bool fgCaHDMIHDCPReset(bool fgen)
{
	...
	if (tz_ret != TZ_RESULT_SUCCESS) {
		pr_debug("[HDMI]CA HDMI_TA_HDCP_RST err:%X\n", tz_ret);
		return FALSE;
	}
	return TRUE;
}
...
bool fgCaHDMIHDCPEncEn(bool fgen)
{
	...
	if (tz_ret != TZ_RESULT_SUCCESS) {
		pr_debug("[HDMI]CA HDMI_TA_HDCP_ENC_EN err:%X\n", tz_ret);
		return FALSE;
	}
	return TRUE;
}
...
bool fgCaHDMIVideoUnMute(bool fgen)
{
	...
	if (tz_ret != TZ_RESULT_SUCCESS) {
		pr_debug("[HDMI]CA HDMI_TA_VID_UNMUTE err:%X\n", tz_ret);
		return FALSE;
	}
	return TRUE;
}
...
bool fgCaHDMIAudioUnMute(bool fgen)
{
	...
	if (tz_ret != TZ_RESULT_SUCCESS) {
		pr_debug("[HDMI]CA HDMI_TA_AUD_UNMUTE err:%X\n", tz_ret);
		return FALSE;
	}
	return TRUE;
}
...
void vCaHDCPFailState(unsigned int u4addr, unsigned int u4data)
{
	...
	param[1].value.a = u4data;
	param[1].value.b = 0;
	pr_debug("hdcp fail para1=%d, para2=%d\n", u4addr, u4data);
	tz_ret = KREE_TeeServiceCall(ca_hdmi_handle, HDMI_TA_HDCP_FAIL,
		TZ_ParamTypes2(TZPT_VALUE_INPUT, TZPT_VALUE_INPUT), param);


	if (tz_ret != TZ_RESULT_SUCCESS)
		pr_debug("[HDMI_HDCP]CA %s err:%X\n", __func__, tz_ret);
...
void vCaHDCPOffState(unsigned int u4addr, unsigned int u4data)
{
	...
	param[1].value.a = u4data;
	param[1].value.b = 0;
	pr_debug("hdcp off para1=%d, para2=%d\n", u4addr, u4data);
