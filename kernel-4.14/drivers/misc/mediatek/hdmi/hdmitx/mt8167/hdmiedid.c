...
void vClearEdidInfo(void)
{
	...
	_HdmiSinkAvCap.ui2_edid_chksum_and_audio_sup =
		...
		| SINK_BASE_BLK_CHKSUM_ERR | SINK_EXT_BLK_CHKSUM_ERR);
...
unsigned char hdmi_fgreadedid(unsigned char i1noedid)
{
	...
	/*block 0 : standard EDID block,
	 *address 0x00 ~ 0x7F w/ device ID=0xA0
	 */
	bExtBlockNo = 0xff;

	for (bBlockIdx = 0; bBlockIdx <= bExtBlockNo; bBlockIdx++) {
		for (bIdx = 0; bIdx < 5; bIdx++) {
			if ((bBlockIdx * EDID_BLOCK_LEN) < EDID_SIZE) {
				/* for Buffer overflow error */
				if (fgDDCDataRead
				    (EDID_ID + (bBlockIdx >> 1),
				     0x00 + (bBlockIdx & 0x01) *
				     EDID_BLOCK_LEN, EDID_BLOCK_LEN,
				     &_bEdidData[bBlockIdx *
				     EDID_BLOCK_LEN]) != TRUE) {
					if (bIdx == 4)
						return 0;
				} else
...
void vAnalyzeDTD(unsigned short ui2Active,
	unsigned short ui2HBlanking, unsigned char bFormat,
	unsigned char fgFirstDTD)
{
	...
	case 0x5a0:		/* 480i */
		...
		break;
	case 0x2d0:		/* 480p */
		if ((ui2HBlanking == 0x8a) && (bFormat == 0)) {
			/* NTSC, p-scan */
			ui4NTSC |= SINK_480P;
			if (fgFirstDTD)
				ui41stNTSC |= SINK_480P;

		} else if ((ui2HBlanking == 0x90) && (bFormat == 0)) {
			/* PAL, p-scan */
			ui4PAL |= SINK_576P;
			if (fgFirstDTD)
				ui41stPAL |= SINK_576P;
		...
		break;
	case 0x500:		/* 720p */
		if ((ui2HBlanking == 0x172) && (bFormat == 0)) {
			/* NTSC, p-scan */
			ui4NTSC |= SINK_720P60;
			if (fgFirstDTD)
				ui41stNTSC |= SINK_720P60;

		} else if ((ui2HBlanking == 0x2bc) && (bFormat == 0)) {
			/* PAL, p-scan */
			ui4PAL |= SINK_720P50;
			if (fgFirstDTD)
...
void vSetEdidChkError(void)
{
	...
	for (bInx = 0; bInx < 7; bInx++) {
		if (bInx == 0)
			_HdmiSinkAvCap.ui1_sink_pcm_bit_size[bInx]
				= 0x07;/* 2ch 24 bits */
		else
			_HdmiSinkAvCap.ui1_sink_pcm_bit_size[bInx] = 0;
	}
...
{
	/* Video data block */
	unsigned int ui4CEA_NTSC = 0, ui4CEA_PAL = 0, ui4OrgCEA_NTSC = 0,
		ui4OrgCEA_PAL = 0, ui4NativeCEA_NTSC = 0, ui4NativeCEA_PAL = 0;
...
static void  vParser_Audio_Data_Block(unsigned char *prData, unsigned char Len)
{
	...
	for (bIdx = 0; bIdx < (bNo / 3); bIdx++) {
		bLengthSum = bIdx * 3;
		bAudCode = (*(prData + bLengthSum + 1) & 0x78) >> 3;
		/* get audio code */
...
/* PCM:1 HDMI_SINK_AUDIO_DEC_LPCM AC3:2 HDMI_SINK_AUDIO_DEC_AC3 */
		...
		if (bAudCode == AVD_LPCM) { /* LPCM */
			...
			if (bPcmChNum >= 2) {
				_HdmiSinkAvCap.ui1_sink_pcm_ch_sampling[
					bPcmChNum - 2] =
					(*(prData + bLengthSum + 2) & 0x7f);
				_HdmiSinkAvCap.ui1_sink_pcm_bit_size[
					bPcmChNum - 2] =
		...
		if (bAudCode == AVD_DST) {	/* DST */
			bPcmChNum = (*(prData + bLengthSum + 1) & 0x07) + 1;
			if (bPcmChNum >= 2) {
				_HdmiSinkAvCap.ui1_sink_dst_ch_sampling[
					bPcmChNum -	2] =
					(*(prData + bLengthSum + 2) & 0x7f);
			}
		...
		if (bAudCode == AVD_DSD) {	/* DSD */
			bPcmChNum = (*(prData + bLengthSum + 1) & 0x07) + 1;
			if (bPcmChNum >= 2) {
				_HdmiSinkAvCap.ui1_sink_dsd_ch_sampling[
					bPcmChNum - 2] =
					(*(prData + bLengthSum + 2) & 0x7f);
			}
		}
	...
	unsigned char *prData, unsigned char Len)
{
	...
	unsigned char b3D_Multi_present = 0, b3D_Structure_7_0 = 1,
		b3D_MASK_15_8 = 1, b3D_MASK_7_0 = 1, b2D_VIC_order_Index = 0;
	...
	if (bTemp == EDID_VSDB_LEN) {
		...
		if (bNo >= 8) {
			...
			if (bTemp & 0x80) {
				/* Latency Present */
				/* kenny add 2010/4/25 */
				_HdmiSinkAvCap.ui1_sink_p_latency_present
					= TRUE;
		...
		if ((bNo > (14 - bLatency_offset + ((bTemp14 & 0xE0) >> 5)))
			...
			if (((15 - bLatency_offset + ((bTemp14 & 0xE0) >> 5))
				+ (bTemp14 & 0x1F)) >= (15 - bLatency_offset
				+ ((bTemp14 & 0xE0) >> 5) + 2))
				b3D_Structure_7_0 = *(prData
					+ 15 - bLatency_offset
					+ ((bTemp14 & 0xE0) >> 5) + 1);
			...
			while (((15 - bLatency_offset + ((bTemp14 & 0xE0)
				>> 5)) + (bTemp14 & 0x1F)) > ((15 -
				bLatency_offset + ((bTemp14 & 0xE0) >> 5))
				+ 2 + b2D_VIC_order_Index)) {
				/* 2 is 3D_structure */
				bDataTemp = *(prData + 15 - bLatency_offset +
					((bTemp14 & 0xE0) >> 5) + 2
					+ b2D_VIC_order_Index);
				if ((bDataTemp & 0x0F) == 0x00)
					_u4i_3D_VIC |= _ui4First_16_VIC[(
					(bDataTemp & 0xF0) >> 4)];
...
/* 3D_Structure=0,support frame packet */
		...
		if ((bNo > (14 - bLatency_offset + ((bTemp14 & 0xE0) >> 5)))
			...
			if (((15 - bLatency_offset + ((bTemp14 & 0xE0) >> 5))
				+ (bTemp14 & 0x1F)) >= ((15 - bLatency_offset
				+ ((bTemp14 & 0xE0) >> 5)) + 4)) {
				b3D_Structure_7_0 = *(prData + 15
					- bLatency_offset +
					((bTemp14 & 0xE0) >> 5) + 1);
				b3D_MASK_15_8 = *(prData + 15 - bLatency_offset
					+ ((bTemp14 & 0xE0) >> 5) + 2);
				b3D_MASK_7_0 = *(prData + 15 - bLatency_offset
					+ ((bTemp14 & 0xE0) >> 5) + 3);
			}
			...
			if ((((15 - bLatency_offset + ((bTemp14 & 0xE0) >> 5))
				+ (bTemp14 & 0x1F)) >= ((15 - bLatency_offset +
				((bTemp14 & 0xE0) >> 5)) + 4)) &&
				((b3D_Structure_7_0 & 0x01) == 0x01)) {
				/* support frame packet */
				u23D_MASK_ALL =	(((unsigned short)
					(b3D_MASK_15_8)) << 8)|
			...
			while (((15 - bLatency_offset + ((bTemp14 & 0xE0) >> 5))
				+ (bTemp14 & 0x1F)) > (15 - bLatency_offset +
				((bTemp14 & 0xE0) >> 5) + 4
				+ b2D_VIC_order_Index)) {
				bDataTemp = *(prData + 15 - bLatency_offset +
...
unsigned char fgParserExtEDID(unsigned char *prData)
{
	...
	/* Step 1: get the offset value of 1st detail timing
	 *description within extension block
	 */
	...
	/* Max'0528'04, move to here,
	 *after read 0x80 ~ 0xFF because it is 0x83...
	 */
	...
	for (bIdx = 0; bIdx < 6; bIdx++) {
		...
		ui2HActive = (unsigned short)(*(prData + bOfst +
			18 * bIdx + OFST_H_ACT_BLA_HI) & 0xf0) << 4;
		ui2HActive |= *(prData + bOfst +
			18 * bIdx + OFST_H_ACTIVE_LO);
		ui2HBlanking = (unsigned short)(*(prData +
			bOfst + 18 * bIdx + OFST_H_ACT_BLA_HI) & 0x0f) << 8;
		ui2HBlanking |= *(prData + bOfst +
			18 * bIdx + OFST_H_BLANKING_LO);
		ui2VBlanking = (unsigned short)(*(prData +
			bOfst + 18 * bIdx + OFST_V_ACTIVE_HI) & 0x0f) << 8;
		ui2VBlanking |= *(prData + bOfst +
...
void vParserExtEDIDState(unsigned char *prEdid)
{
	...
	if (i4SharedInfo(SI_EDID_PARSING_RESULT) == TRUE) {
		...
		for (bTemp = 0;
			bTemp < i4SharedInfo(SI_EDID_EXT_BLOCK_NO);
			bTemp++) {
			if ((EDID_BLOCK_LEN + bTemp *
				EDID_BLOCK_LEN) < EDID_SIZE) {
				/* for Buffer Overflow error */
				if (*(prEdid + EDID_BLOCK_LEN +
					bTemp * EDID_BLOCK_LEN) == 0x02) {
					prData = (prEdid + EDID_BLOCK_LEN +
						bTemp * EDID_BLOCK_LEN);
					fgParserExtEDID(prData);
				} else if (*(prEdid + EDID_BLOCK_LEN +
					bTemp * EDID_BLOCK_LEN) == 0xF0) {
					;
				}
			}
		}
...
void hdmi_checkedid(unsigned char i1noedid)
{
	...
	vSetSharedInfo(SI_EDID_EXT_BLOCK_NO,
		_bEdidData[EDID_ADDR_EXT_BLOCK_FLAG]);
	...
	if (mhl_bridge_drv->getppmodesupport() == 0) {
		pr_debug("[it668x]can not support pp mode\n");
		...
		pr_debug("[it668x]support pp mode\n");
	}
...
void hdmi_checkedid(unsigned char i1noedid)
{
	...
	if ((i4SharedInfo(SI_EDID_EXT_BLOCK_NO)
		* EDID_BLOCK_LEN) < EDID_SIZE)/* for Buffer Overflow error */
		vParserExtEDIDState(&_bEdidData[0]);
...
int hdmi_drv_get_external_device_capablity(void)
{
	...
	unsigned char Cap_SampleRate = 0,
		Cap_SampleChannel = 0, Cap_SampleBit = 0;
	unsigned char bTemp = 0;
	...
	for (bInx = 0; bInx < 7; bInx++) {
		if ((_HdmiSinkAvCap.ui1_sink_pcm_ch_sampling[bTemp]
			>> bInx) & 0x01)
			Cap_SampleRate = Audio_SampleRate[bInx];
		}
	capablity = Cap_SampleChannel << 3 | Cap_SampleRate << 7
		| Cap_SampleBit << 10;
...
unsigned char hdmi_check_edid_header(void)
{
	...
	for (bIdx = EDID_ADDR_HEADER;
		bIdx < (EDID_ADDR_HEADER + EDID_HEADER_LEN);
		bIdx++) {
