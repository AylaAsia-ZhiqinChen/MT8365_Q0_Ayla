...
void vShowHdcpRawData(void)
{
	...
	while (bTemp < 3) {
		...
		for (k = 0; k < 16; k++) {
			if (k == 15) {
				if ((j + (i * 16 + k)) < 287) {
					if (bHdcpKeyExternalBuff[j +
						(i * 16 + k)] > 0x0f)
						pr_debug("%2x\n",
						bHdcpKeyExternalBuff[
							j + (i * 16 + k)]);
					else
						pr_debug("0%x\n",
...
#else
	pr_debug("[HDMI]can not get hdcp key by this case\n");
#endif
#else
	unsigned short i;
...
void vInitHdcpKeyGetMethod(unsigned char bMethod)
{
	...
	if (bMethod == NON_HOST_ACCESS_FROM_EEPROM) {
		vWriteHdmiSYSMsk(DISP_HDMI_SYS_CFG_00, (I2CM_ON |
			EXT_E2PROM_ON), (I2CM_ON | EXT_E2PROM_ON));
	} else if (bMethod == NON_HOST_ACCESS_FROM_MCM) {
		vWriteHdmiSYSMsk(DISP_HDMI_SYS_CFG_00, (I2CM_ON |
			MCM_E2PROM_ON), (I2CM_ON | MCM_E2PROM_ON));
	} else if (bMethod == NON_HOST_ACCESS_FROM_GCPU) {
		vWriteHdmiSYSMsk(DISP_HDMI_SYS_CFG_00, AES_EFUSE_ENABLE,
			(AES_EFUSE_ENABLE | I2CM_ON |
			EXT_E2PROM_ON | MCM_E2PROM_ON));
	}
}
...
void bReadDataHdmiGRL(unsigned char bAddr,
	unsigned char bCount, unsigned char *bVal)
{
	unsigned char i;
...
void vWriteDataHdmiGRL(unsigned char bAddr,
	unsigned char bCount, unsigned char *bVal)
{
	unsigned char i;
...
void vExchangeKSVs(void)
{
	...
	if (fgHostKey()) {
		fgDDCDataWrite(RX_ID, RX_REG_HDCP_AKSV,
			HDCP_AKSV_COUNT, HDMI_AKSV);
	} else {
		/* fgI2CDataRead(HDMI_DEV_GRL, GRL_RD_AKSV0,
		 *HDCP_AKSV_COUNT, bHDCPBuf);
		 */
		bReadDataHdmiGRL(GRL_RD_AKSV0,
			HDCP_AKSV_COUNT, bHDCPBuf);
		fgDDCDataWrite(RX_ID, RX_REG_HDCP_AKSV,
			HDCP_AKSV_COUNT, bHDCPBuf);
	}
	/* Step 4: read Bksv from receiver, and send to transmitter */
	fgDDCDataRead(RX_ID, RX_REG_HDCP_BKSV,
		HDCP_BKSV_COUNT, bHDCPBuf);
	/* fgI2CDataWrite(HDMI_DEV_GRL, GRL_WR_BKSV0,
	 *HDCP_BKSV_COUNT, bHDCPBuf);
	 */
	vWriteDataHdmiGRL(GRL_WR_BKSV0,
		HDCP_BKSV_COUNT, bHDCPBuf);
...
void vSendAKey(unsigned char *prAKey)
{
	...
	for (ui2Index = 0; ui2Index < 280; ui2Index++) {
		/* get key from flash */
		if ((ui2Index == 5) && (bCheckPordHotPlug(PORD_MODE
			| HOTPLUG_MODE) == FALSE)) {
			vSetHDCPState(HDCP_RECEIVER_NOT_READY);
			return;
		}
...
unsigned char fgCompareRi(void)
{
	...
	/* fgI2CDataRead(HDMI_DEV_GRL, GRL_RI_0,
	 *HDCP_RI_COUNT, bHDCPBuf+HDCP_RI_COUNT);
	 */
	...
	HDMI_HDCP_LOG(
		"bHDCPBuf[0]=0x%x,bHDCPBuf[1]=0x%x,bHDCPBuf[2]=0x%x,bHDCPBuf[3]=0x%x\n",
...
void vHalWriteKsvListPort(unsigned char *prKsvData,
	unsigned char bDevice_Count, unsigned char *prBstatus)
{
...
void vHalWriteHashPort(unsigned char *prHashVBuff)
{
	...
	for (bIndex = 0; bIndex < 20; bIndex++)
		vWriteByteHdmiGRL(GRL_REPEATER_HASH + bIndex * 4,
			*(prHashVBuff + bIndex));
}
...
void vReadKSVFIFO(void)
{
	...
	if (bDevice_Count > 32) {
		for (bTemp = 0; bTemp < 2; bTemp++) {
			/* retry 1 times */
			fgDDCDataRead(RX_ID, RX_REG_BSTATUS1,
				1, &bDevice_Count);
			bDevice_Count &= DEVICE_COUNT_MASK;
			if (bDevice_Count <= 32)
				break;
	...
	if (bDevice_Count == 0) {
		...
		fgDDCDataRead(RX_ID, RX_REG_KSV_FIFO,
			bDevice_Count * 5, bKsv_buff);
	}
...
void VHdmiMuteVideoAudio(unsigned char u1flagvideomute,
...
void vDrm_mutehdmi(unsigned char u1flagvideomute,
	unsigned char u1flagaudiomute)
{
...
void vSvp_mutehdmi(unsigned char u1svpvideomute,
...
void HdcpService(enum HDCP_CTRL_STATE_T e_hdcp_state)
{
	...
	&& defined(CONFIG_MTK_HDMI_HDCP_SUPPORT))
		...
		if ((bhdcpkey == INTERNAL_ENCRYPT_KEY)
			|| (bhdcpkey == EXTERNAL_KEY))
	...
	case HDCP_CHECK_LINK_INTEGRITY:
		...
		if (fgCompareRi() == TRUE) {
			...
			if (fgIsRepeater()) {
				/* step 1, 2, 3. */
				if (i4SharedInfo(SI_HDMI_HDCP_RESULT) == 0x07)
					vSetSharedInfo(SI_HDMI_HDCP_RESULT,
						(i4SharedInfo(
						SI_HDMI_HDCP_RESULT)
						| 0x08));
