...
struct HDMI_EDID_T {
	 ...
	 *Zero means none native NTSC resolution is available
	 */
	unsigned int ui4_sink_native_ntsc_resolution;
	/* use EDID_VIDEO_RES_T, only one resolution,
	 *Zero means none native PAL resolution is available
	 */
...
/* ACK condition */
enum APK_CEC_ACK_COND {
	APK_CEC_ACK_COND_OK = 0,
	APK_CEC_ACK_COND_NO_RESPONSE,
};

/* ACK info */
struct APK_CEC_ACK_INFO {
	void *pv_tag;
	enum APK_CEC_ACK_COND e_ack_cond;
};
...
/* coding sytle */
extern void hdmi_switch_resolution(int res);
/* coding sytle */
...
#define	COMPAT_MTK_HDMI_AUDIO_SETTING \
	HDMI_IOWR(21, struct COMPAT_HDMITX_AUDIO_PARA)
