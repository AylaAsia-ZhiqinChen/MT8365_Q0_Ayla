...
do { \
	if (hdmidrv_log_on&hdmiplllog) { \
		pr_debug("[hdmi_pll]%s,%d ", __func__, __LINE__); \
		pr_debug(fmt, ##arg); } \
} while (0)
...
do { \
	if (hdmidrv_log_on&hdmitxvideolog) { \
		pr_debug("[hdmi_video]%s,%d ", __func__, __LINE__); \
		pr_debug(fmt, ##arg); } \
} while (0)
...
do { \
	if (hdmidrv_log_on&hdmitxaudiolog) { \
		pr_debug("[hdmi_audio]%s,%d ", \
		__func__, __LINE__); pr_debug(fmt, ##arg); } \
} while (0)
...
do { \
	if (hdmidrv_log_on&hdmihdcplog) { \
		pr_debug("[hdmi_hdcp]%s,%d ", __func__, __LINE__); \
		pr_debug(fmt, ##arg); } \
} while (0)
...
do { \
	if (hdmidrv_log_on&hdmiceclog) {\
		pr_debug("[hdmi_cec]%s,%d ", __func__, __LINE__); \
		pr_debug(fmt, ##arg); } \
} while (0)
...
do { \
	if (hdmidrv_log_on&hdmiedidlog) { \
		pr_debug("[hdmi_edid]%s,%d ", __func__, __LINE__); \
		pr_debug(fmt, ##arg); } \
} while (0)
...
do { \
	if (hdmidrv_log_on&hdmidrvlog) { \
		pr_debug("[hdmi_drv]%s,%d ", __func__, __LINE__); \
		pr_debug(fmt, ##arg); } \
} while (0)
...
/* #define RETNULL(cond)       \
 *{if ((cond)) {HDMI_DRV_LOG("return in %d\n", __LINE__); return; } }
 */
/* #define RETINT(cond, rslt)  \
