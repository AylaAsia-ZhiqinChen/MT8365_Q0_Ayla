...
/* including sampling frequency information. */
...
#define AUD_OUTSYNC_EN     (1 << 24)
#define AUD_OUTSYNC_PRE_EN (1 << 25)
#define I2CM_ON            (1 << 26)
#define E2PROM_TYPE_8BIT   (1 << 27)
#define MCM_E2PROM_ON      (1 << 28)
#define EXT_E2PROM_ON      (1 << 29)
#define HTPLG_PIN_SEL_OFF  (1 << 30)
#define AES_EFUSE_ENABLE   (1 << 31)
...
#define TVDPLL_POSDIV			24
/* default value 010, div = /4 */
...
extern void hdmi_hdmitopck_write(unsigned short u2Reg,
	unsigned int u4Data);

extern void hdmi_infrasys_write(unsigned short u2Reg,
	unsigned int u4Data);
extern unsigned int hdmi_infrasys_read(unsigned short u2Reg);
extern void hdmi_perisys_write(unsigned short u2Reg,
	unsigned int u4Data);
...
(vWriteByteHdmiGRL((dAddr), (bReadByteHdmiGRL(dAddr) \
& (~(dMsk))) | ((dVal) & (dMsk))))
...
(vWriteHdmiSYS((dAddr), (dReadHdmiSYS(dAddr) & \
(~(dMsk))) | ((dVal) & (dMsk))))
...
(vWriteHdmiTOPCK((dAddr), (dReadHdmiTOPCK(dAddr) \
& (~(dMsk))) | ((dVal) & (dMsk))))
#define vWriteHdmiTOPCKUnMsk(dAddr, dMsk)(vWriteHdmiTOPCK((dAddr), \
	(dReadHdmiTOPCK(dAddr) & (~(dMsk)))))

#define vWriteIoPll(dAddr, dVal)  (hdmi_pll_write(dAddr, dVal))
#define dReadIoPll(dAddr)         (hdmi_pll_read(dAddr))
#define vWriteIoPllMsk(dAddr, dVal, dMsk) vWriteIoPll((dAddr), \
	(dReadIoPll(dAddr) & (~(dMsk))) | ((dVal) & (dMsk)))
...
(vWriteINFRASYS((dAddr), (dReadINFRASYS(dAddr) & \
(~(dMsk))) | ((dVal) & (dMsk))))
#define vWriteINFRASYSUnMsk(dAddr, dMsk)(vWriteINFRASYS((dAddr), \
	(dReadINFRASYS(dAddr) & (~(dMsk)))))
...
extern void vChgHDMIVideoResolution(
	unsigned char ui1resindex, unsigned char ui1colorspace,
	unsigned char ui1hdmifs, unsigned char bdeepmode);
extern void vChgHDMIAudioOutput(unsigned char ui1hdmifs,
	unsigned char ui1resindex, unsigned char bdeepmode);
extern void vChgtoSoftNCTS(unsigned char ui1resindex,
	unsigned char ui1audiosoft, unsigned char ui1hdmifs,
	unsigned char bdeepmode);
extern void vSendAVIInfoFrame(unsigned char ui1resindex,
