...
#define SIF_SET_BIT(u4Addr, u4Val)  SIF_WRITE32((u4Addr), \
	(SIF_READ32(u4Addr) | (u4Val)))
#define SIF_CLR_BIT(u4Addr, u4Val)  SIF_WRITE32((u4Addr), \
	(SIF_READ32(u4Addr) & (~(u4Val))))
...
#define SIF_READ_MASK(u4Addr, u4Mask, u4Offset)  ((SIF_READ32(u4Addr) \
	& (u4Mask)) >> (u4Offset))
...
#define DDCM_DATA0_WRITE(u4Val)   SIF_WRITE_MASK(DDC_DDCMD0, \
	DDCM_DATA0, 0,  u4Val)
#define DDCM_DATA1_WRITE(u4Val)   SIF_WRITE_MASK(DDC_DDCMD0, \
	DDCM_DATA1, 8,  u4Val)
#define DDCM_DATA2_WRITE(u4Val)   SIF_WRITE_MASK(DDC_DDCMD0, \
	DDCM_DATA2, 16, u4Val)
#define DDCM_DATA3_WRITE(u4Val)   SIF_WRITE_MASK(DDC_DDCMD0, \
	DDCM_DATA3, 24, u4Val)
#define DDCM_DATA4_WRITE(u4Val)   SIF_WRITE_MASK(DDC_DDCMD1, \
	DDCM_DATA4, 0,  u4Val)
#define DDCM_DATA5_WRITE(u4Val)   SIF_WRITE_MASK(DDC_DDCMD1, \
	DDCM_DATA5, 8,  u4Val)
#define DDCM_DATA6_WRITE(u4Val)   SIF_WRITE_MASK(DDC_DDCMD1, \
	DDCM_DATA6, 16, u4Val)
#define DDCM_DATA7_WRITE(u4Val)   SIF_WRITE_MASK(DDC_DDCMD1, \
	DDCM_DATA7, 24, u4Val)

#define DDCM_CLK_DIV_READ()        SIF_READ_MASK(DDC_DDCMCTL0, \
	DDCM_CLK_DIV_MASK, DDCM_CLK_DIV_OFFSET)
#define DDCM_CLK_DIV_WRITE(u4Val)  SIF_WRITE_MASK(DDC_DDCMCTL0, \
	DDCM_CLK_DIV_MASK, DDCM_CLK_DIV_OFFSET, u4Val)

#define DDCM_ACK_READ()            SIF_READ_MASK(DDC_DDCMCTL1, \
	DDCM_ACK_MASK, DDCM_ACK_OFFSET)

#define DDCM_PGLEN_READ()          SIF_READ_MASK(DDC_DDCMCTL1, \
	DDCM_PGLEN_MASK, DDCM_PGLEN_OFFSET)
#define DDCM_PGLEN_WRITE(u4Val)    SIF_WRITE_MASK(DDC_DDCMCTL1, \
	DDCM_PGLEN_MASK, DDCM_PGLEN_OFFSET, u4Val)

#define DDCM_SIF_MODE_READ()          SIF_READ_MASK(DDC_DDCMCTL1, \
	DDCM_SIF_MODE_MASK, DDCM_SIF_MODE_OFFSET)
#define DDCM_SIF_MODE_WRITE(u4Val)    SIF_WRITE_MASK(DDC_DDCMCTL1, \
	DDCM_SIF_MODE_MASK, DDCM_SIF_MODE_OFFSET, u4Val)
...
static unsigned char _DDCMRead(unsigned char ucCurAddrMode,
	...
	unsigned char *pucValue, unsigned int u4Count)
{
	...
	while (u4Count > 0) {
		...
		DDCM_TrigMode((ucAckFinal == 1) ?
			DDCM_READ_DATA_NO_ACK : DDCM_READ_DATA_ACK);

		/* ack, killua */
		u4Ack = DDCM_ACK_READ();
		for (ucTmpCount = 0; ((u4Ack & (1 << ucTmpCount))
			!= 0) && (ucTmpCount < 8); ucTmpCount++)
			;
		if (((ucAckFinal == 1) && (ucTmpCount != (ucReadCount - 1)))
		    || ((ucAckFinal == 0) && (ucTmpCount != ucReadCount))) {
			HDMI_DDC_LOG(
				"[DDC] Device(0x%x)/Word(0x%x) Address NACK! ACK(0x%x)\n",
				ucDev, u4Addr, u4Ack);
			break;
		}
...
static unsigned char _DDCMWrite(unsigned char ucCurAddrMode,
	unsigned int u4ClkDiv, unsigned char ucDev,
	unsigned int u4Addr, enum SIF_BIT_T ucAddrType,
	const unsigned char *pucValue, unsigned int u4Count)
{
	...
	while (u4Count > 0) {
		...
		/* ack, killua */
		u4Ack = DDCM_ACK_READ();
		for (ucTmpCount = 0; ((u4Ack & (1 << ucTmpCount)) != 0)
...
unsigned int DDCM_RanAddr_Write(unsigned int u4ClkDiv,
	...
	unsigned int u4Count)
{
	...
	u4WriteCount1 = (u4WriteCount1 > u4Count) ?
		u4Count : u4WriteCount1;
	ucReturnVaule = _DDCMWrite(0, u4ClkDiv, ucDev,
		u4Addr, ucAddrType, pucValue, u4WriteCount1);

	return (unsigned int)ucReturnVaule;
}

unsigned int DDCM_CurAddr_Read(unsigned int u4ClkDiv,
	unsigned char ucDev, unsigned char *pucValue,
	unsigned int u4Count)
{
	unsigned char ucReturnVaule;
	...
	ucReturnVaule = _DDCMRead(1, u4ClkDiv, ucDev,
		0, SIF_8_BIT, pucValue, u4Count);

	return (__u32) ucReturnVaule;
}

unsigned char DDCM_RanAddr_Read(unsigned int u4ClkDiv,
	unsigned char ucDev, unsigned int u4Addr,
	...
	unsigned int u4Count)
{
	...
	ucReturnVaule = _DDCMRead(0, u4ClkDiv, ucDev, u4Addr,
...
unsigned char fgDDCDataRead(unsigned char bDevice,
	unsigned char bData_Addr, unsigned char bDataCount,
	unsigned char *prData)
{
	HDMI_DDC_FUNC();
	if (DDCM_RanAddr_Read(SIF1_CLOK,
		(unsigned char)bDevice, (unsigned int)bData_Addr,
		SIF_8_BIT, (unsigned char *)prData,
...
unsigned char fgDDCDataWrite(unsigned char bDevice,
	unsigned char bData_Addr, unsigned char bDataCount,
	unsigned char *prData)
{
	HDMI_DDC_FUNC();
	if ((DDCM_RanAddr_Write(SIF1_CLOK, (unsigned char)bDevice,
		(unsigned int)bData_Addr, SIF_8_BIT, (unsigned char *)prData,
		(unsigned int)bDataCount)) == bDataCount)
		return TRUE;
	else
		return FALSE;
