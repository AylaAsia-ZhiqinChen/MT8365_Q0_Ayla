...
static void config_tvdpll(unsigned int resolutionmode)
{
	...
	pr_debug("pcw_value = %d\n", pcw_value);
	udelay(10);
	vWriteIoPllMsk(TVDPLL_PWR_CON0, 0x3, TVDPLL_PWR_ON |
		TVDPLL_SDM_ISO_EN);
	udelay(10);
	vWriteIoPllMsk(TVDPLL_PWR_CON0, 0x0, TVDPLL_SDM_ISO_EN);
	udelay(10);
	vWriteIoPllMsk(TVDPLL_CON1, (pcw_value << RG_TVDPLL_SDM_PCW),
		RG_TVDPLL_SDM_PCW_MASK);
	udelay(10);
	vWriteIoPllMsk(TVDPLL_CON1, TVDPLL_SDM_PCW_CHG,
		TVDPLL_SDM_PCW_CHG);
	udelay(10);
...
static int hdmi_internal_audio_config(
	enum HDMI_AUDIO_FORMAT aformat, int sampleBit)
{
	HDMI_DRV_FUNC();
	...
	for (i = 0; i < MAX_HDMI_TMR_NUMBER; i++) {
		if (hdmi_TmrValue[i] >= AVD_TMR_ISR_TICKS) {
			...
			if ((i == HDMI_PLUG_DETECT_CMD) &&
				(hdmi_TmrValue[HDMI_PLUG_DETECT_CMD]
				== 0))
				vSendHdmiCmd(HDMI_PLUG_DETECT_CMD);
			else if ((i == HDMI_HDCP_PROTOCAL_CMD) &&
				(hdmi_TmrValue[HDMI_HDCP_PROTOCAL_CMD]
				== 0))
...
int hdmi_internal_power_on(void)
{
	...
	if (hdmi_power_control_pin > 0) {
		pr_debug("[hdmi]hdmi control pin number is %d\n",
	...
	pm_runtime_get_sync(smi_larb);
	vWriteHdmiSYSMsk(DISP_HDMI_SYS_CFG_01,
		HDMI_PCLK_FREE_RUN, HDMI_PCLK_FREE_RUN);
	vWriteHdmiSYSMsk(DISP_HDMI_SYS_CFG_01,
		HDMI_OUT_FIFO_EN, HDMI_OUT_FIFO_EN);
	vWriteHdmiSYSMsk(DISP_HDMI_SYS_CFG_00,
		ANLG_ON | HDMI_SPIDIF_ON, ANLG_ON | HDMI_SPIDIF_ON);
	vWriteHdmiSYSMsk(DISP_HDMI_SYS_CFG_01,
...
void hdmi_read(unsigned long u2Reg, unsigned int *p4Data)
{
	...
	case 0x1401b000:
		internal_hdmi_read(hdmi_reg[HDMI_SHELL] +
			u2Reg - 0x1401b000, p4Data);
		break;

	case 0x11011000:
		internal_hdmi_read(hdmi_reg[HDMI_DDC] +
			u2Reg - 0x11011000, p4Data);
		break;

	case 0x1001a000:
		internal_hdmi_read(hdmi_reg[HDMI_CEC] +
			u2Reg - 0x1001a000, p4Data);
		break;

	case 0x10018000:
		internal_hdmi_read(hdmi_ref_reg[AP_CCIF0] +
			u2Reg - 0x10018000, p4Data);
		break;

	case 0x10000000:
		internal_hdmi_read(hdmi_ref_reg[TOPCK_GEN] +
			u2Reg - 0x10000000, p4Data);
		break;

	case 0x10001000:
		internal_hdmi_read(hdmi_ref_reg[INFRA_SYS] +
			u2Reg - 0x10001000, p4Data);
		break;

	case 0x14000000:
		internal_hdmi_read(hdmi_ref_reg[MMSYS_CONFIG] +
			u2Reg - 0x14000000, p4Data);
		break;

	case 0x10005000:
		internal_hdmi_read(hdmi_ref_reg[GPIO_REG] +
			u2Reg - 0x10005000, p4Data);
		break;

	case 0x14019000:
		internal_hdmi_read(hdmi_ref_reg[DPI1_REG] +
			u2Reg - 0x14019000, p4Data);
		break;
...
#else
		internal_hdmi_write(hdmi_reg[HDMI_SHELL] +
		u2Reg - 0x1401b000, u4Data);
#endif
		break;

	case 0x11001000:
		internal_hdmi_write(hdmi_reg[HDMI_DDC] +
			u2Reg - 0x11001000, u4Data);
		break;

	case 0x1001a000:
		internal_hdmi_write(hdmi_reg[HDMI_CEC] +
			u2Reg - 0x1001a000, u4Data);
		break;

	case 0x10018000:
		internal_hdmi_write(hdmi_ref_reg[AP_CCIF0] +
			u2Reg - 0x10018000, u4Data);
		break;

	case 0x10000000:
		internal_hdmi_write(hdmi_ref_reg[TOPCK_GEN] +
			u2Reg - 0x10000000, u4Data);
		break;

	case 0x10010000:
		internal_hdmi_write(hdmi_ref_reg[INFRA_SYS] +
			u2Reg - 0x10010000, u4Data);
		break;

	case 0x14000000:
		internal_hdmi_write(hdmi_ref_reg[MMSYS_CONFIG] +
			u2Reg - 0x14000000, u4Data);
		break;

	case 0x10005000:
		internal_hdmi_write(hdmi_ref_reg[GPIO_REG] +
			u2Reg - 0x10005000, u4Data);
		break;

	case 0x14019000:
		internal_hdmi_write(hdmi_ref_reg[DPI1_REG] +
			u2Reg - 0x14019000, u4Data);
		break;

	default:
		break;
	}
	pr_debug("Reg write= 0x%08lx, data = 0x%08x\n", u2Reg, u4Data);
}
...
int hdmi_internal_probe(struct platform_device *pdev)
{
	...
	HDMI_DRV_LOG("[%s] probe start\n", __func__);


	if (pdev->dev.of_node == NULL) {
		pr_debug("[%s] Device Node Error\n", __func__);
		return -1;
	}
	...
	for (i = 0; i < HDMI_REG_NUM; i++) {
		...
		if (!hdmi_reg[i]) {
			pr_debug("HDMI of_iomap fail, i=%d\n", i);
			return -ENOMEM;
		}
	...
	if (hdmi_cec_on == 1)
		...
		hdmi_irq = irq_of_parse_and_map(pdev->dev.of_node, 1);

	HDMI_DRV_LOG("hdmi_irq = %d\n", hdmi_irq);
	if (hdmi_irq > 0) {
		if (request_irq(hdmi_irq, hdmi_irq_handler,
			IRQF_TRIGGER_LOW, "mtkhdmi", NULL) < 0)
			pr_debug("request interrupt failed.\n");
	}
	...
	if (IS_ERR(pinctrl)) {
		ret = PTR_ERR(pinctrl);
		pr_debug("hdmi power control pin, failure of setting\n");
	} else {
		pins_default = pinctrl_lookup_state(pinctrl, "default");
		if (IS_ERR(pins_default)) {
			ret = PTR_ERR(pins_default);
			pr_debug("cannot find hdmi pinctrl default");
		} else {
			pinctrl_select_state(pinctrl, pins_default);
		}
	}
	np = of_find_compatible_node(NULL, NULL,
		"mediatek,mt8167-hdmitx");
	/* Get 5V DDC Power Control Pin */
	hdmi_power_control_pin = of_get_named_gpio(np,
		"hdmi_power_control", 0);
	if (hdmi_power_control_pin > 0) {
		ret = gpio_request(hdmi_power_control_pin,
			"hdmi power control pin");
		if (ret)
			pr_debug("hdmi power control pin, failure of setting\n");
	}
	/* Get PLL/TOPCK/INFRA_SYS/MMSSYS /GPIO/EFUSE
	 *phy base address and iomap virtual address
	 */
	for (i = 0; i < HDMI_REF_REG_NUM; i++) {
		np = of_find_compatible_node(NULL, NULL,
			hdmi_use_module_name_spy(i));
		if (np == NULL)
			continue;
	...
	if (!larb_node) {
		pr_debug("missing mediatek,larb phandle in hdmi node\n");
		return -EINVAL;
	}
	larb_pdev = of_find_device_by_node(larb_node);
	of_node_put(larb_node);
	if ((!larb_pdev) || (!larb_pdev->dev.driver)) {
		pr_debug("hdmi_probe is earlier than SMI\n");
		return -EPROBE_DEFER;
	}
	smi_larb = &larb_pdev->dev;
...
static void vPlugDetectService(enum HDMI_CTRL_STATE_T e_state)
{
	...
	case HDMI_STATE_HOT_PLUG_OUT:
		vClearEdidInfo();
/* AV mute and turn off tmds before reset HDCP
 * to avoid TV generate blank area on screen
 */
		vHDMIAVMute();
		mdelay(1);
...
/***********************/
	...
	case HDMI_STATE_HOT_PLUGIN_AND_POWER_ON:
		if (hdmi_clockenable == 0) {
			HDMI_PLUG_LOG("hdmi plug in and power on\n");
			hdmi_clockenable = 1;
...
void hdmi_module_init(void)
{
	...
	if (hdmi_cec_on == 1) {
		/* must set this bit, otherwise hpd/port and
		 *cec module can not read/write
		 */
...
static int cec_timer_kthread(void *data)
{
	...
	for (;;) {
		wait_event_interruptible(cec_timer_wq,
			atomic_read(&cec_timer_event));
...
static int hdmi_irq_kthread(void *data)
{
	...
	for (;;) {
		wait_event_interruptible(hdmi_irq_wq,
			atomic_read(&hdmi_irq_event));
...
{
	if (r_cec_timer.function)
		mod_timer(&r_cec_timer, jiffies +
			gCEC_CHK_INTERVAL / (1000 / HZ));
}
...
const struct HDMI_DRIVER *HDMI_GetDriver(void)
{
	static const struct HDMI_DRIVER HDMI_DRV = {
		...
		.get_external_device_capablity =
			hdmi_drv_get_external_device_capablity,
...
static int __init mtk_hdmitx_init(void)
{
	...
	hdmi_timer_task = kthread_create(hdmi_timer_kthread,
		NULL, "hdmi_timer_kthread");
	wake_up_process(hdmi_timer_task);

	init_waitqueue_head(&cec_timer_wq);
	cec_timer_task = kthread_create(cec_timer_kthread,
		NULL, "cec_timer_kthread");
	wake_up_process(cec_timer_task);

	init_waitqueue_head(&hdmi_irq_wq);
	hdmi_irq_task = kthread_create(hdmi_irq_kthread,
		NULL, "hdmi_irq_kthread");
	wake_up_process(hdmi_irq_task);

	if (platform_driver_register(&hdmi_of_driver)) {
		pr_debug("failed to register disp driver\n");
		ret = -1;
	}
	return ret;
