...
#define ACCDET_ERROR(format, args...) pr_notice(format, ##args)
...
static inline void enable_accdet(u32 state_swctrl)
{
	...
	accdet_write(ACCDET_STATE_SWCTRL,
		     accdet_read(ACCDET_STATE_SWCTRL) | state_swctrl);
...
static inline void disable_accdet(void)
{
	...
	/* sync with accdet_irq_handler set clear
	 * accdet irq bit to avoid  set clear accdet irq bit
	...
	while (accdet_read(ACCDET_IRQ_STS) & IRQ_STATUS_BIT) {
		ACCDET_DEBUG
		    ("[Accdet]Clear interrupt on-going....\n");
		msleep(20);
	}
	...
	accdet_write(ACCDET_CTRL,
		     accdet_read(ACCDET_CTRL) & (~(ACCDET_ENABLE)));
...
static void disable_micbias_callback(struct work_struct *work)
{
	if (cable_type == HEADSET_NO_MIC) {
		/*setting pwm idle; */
		accdet_write(ACCDET_STATE_SWCTRL,
			     accdet_read(ACCDET_STATE_SWCTRL) &
...
/*Disable ACCDET*/
		ACCDET_DEBUG
		    ("[Accdet]ACC EINT func :plug-out, cur_eint_state = %d\n",
		     cur_eint_state);
		mutex_lock(&accdet_eint_irq_sync_mutex);
...
static irqreturn_t accdet_eint_func(int irq, void *data)
{
	...
	ACCDET_DEBUG("[Accdet]%s after cur_eint_state=%d\n",
...
static inline int accdet_setup_eint(struct platform_device *accdet_device)
{
	...
	if (node) {
		...
		ret =
		    request_irq(accdet_irq_gic, accdet_gic_handler,
				IRQF_TRIGGER_NONE, "ACCDET-GIC", NULL);
		if (ret > 0)
		...
		ret =
		    request_irq(accdet_irq, accdet_eint_func, IRQ_TYPE_NONE,
				"ACCDET-eint", NULL);
		if (ret > 0)
			ACCDET_ERROR("[Accdet]EINT IRQ LINE NOT AVAILABLE\n");
		else
			ACCDET_ERROR
			    ("[Accdet]accdet_irq=%d,headsetdebounce=%d\n",
...
static struct iio_channel *g_accdet_adc_channel;
...
static void multi_key_detection(int current_status)
{
	...
	if (current_status == 0) {
		ret =
		    iio_read_channel_processed(g_accdet_adc_channel,
					       &cali_voltage);
		if (ret < 0)
			ACCDET_ERROR
			    ("[Accdet]get auxadc value fail, ret = %d\n", ret);

		cali_voltage = cali_voltage * 1500 / 4096;	/* uV->mV */
	...
	mdelay(30);
	if (((accdet_read(ACCDET_IRQ_STS) & IRQ_STATUS_BIT) != IRQ_STATUS_BIT)
	    || eint_accdet_sync_flag) {
		send_key_event(cur_key, !current_status);
	} else {
		ACCDET_DEBUG
		    ("[Accdet]plug out side effect ,do not report key = %d\n",
		    cur_key);
		cur_key = NO_KEY;
	}
	if (current_status)
...
static inline void clear_accdet_interrupt(void)
{
	...
	accdet_write(ACCDET_IRQ_STS,
		     ((accdet_read(ACCDET_IRQ_STS)) & 0x8000) | (IRQ_CLR_BIT));
	ACCDET_DEBUG("[Accdet]%s: ACCDET_IRQ_STS = 0x%x\n",
		     __func__, accdet_read(ACCDET_IRQ_STS));
}
...
/* next, previous, volumeup, volumedown send key in multi_key_detection() */
		...
		/* __pm_relax(&accdet_key_lock); //unlock wake lock */
	}
	return 0;
}
...
static inline void check_cable_type(void)
{
	...
	case PLUG_OUT:
		if (current_status == 0) {
			...
			/*add for suspend disable accdet more than 5S */
			/* __pm_relax(&accdet_timer_lock); */
			/* #endif */
		} else if (current_status == 3) {
			ACCDET_DEBUG("[Accdet]PLUG_OUT state not change!\n");
#ifdef ACCDET_MULTI_KEY_FEATURE
			ACCDET_DEBUG
			("[Accdet] donot send plug out event in plug out\n");
#else
			mutex_lock(&accdet_eint_irq_sync_mutex);
			if (eint_accdet_sync_flag == 1) {
				accdet_status = PLUG_OUT;
	...
	case MIC_BIAS:
		/* ALPS00038030:reduce the time of remote button */
		/* pressed during incoming call */
		/* solution: resume hook switch debounce time */
		accdet_write(ACCDET_DEBOUNCE0,
			     cust_headset_settings->debounce0);
		if (current_status == 0) {
			mutex_lock(&accdet_eint_irq_sync_mutex);
			if (eint_accdet_sync_flag == 1) {
				while ((accdet_read(ACCDET_IRQ_STS) &
					IRQ_STATUS_BIT)
				       && (wait_clear_irq_times < 3)) {
					ACCDET_DEBUG
					    ("[Accdet]MIC BIAS clear IRQ\n");
	...
	case HOOK_SWITCH:
		if (current_status == 0) {
			...
			/* ALPS00038030:reduce the time of remote button */
			/* pressed during incoming call */
			/* solution: reduce hook switch debounce time */
			/* to 0x400 */
			accdet_write(ACCDET_DEBOUNCE0, button_press_debounce);
			/* #ifdef ACCDET_LOW_POWER */
			/* __pm_relax(&accdet_timer_lock); */
...
#ifdef ACCDET_MULTI_KEY_FEATURE
			ACCDET_DEBUG
			    ("[Accdet] do not send event in hook switch\n");
			mutex_lock(&accdet_eint_irq_sync_mutex);
			if (eint_accdet_sync_flag == 1)
				accdet_status = PLUG_OUT;
			else
			...
			ACCDET_DEBUG
			("[Accdet]HOOK_SWITCH can't to this state!\n");
		}
		break;
	...
	if (!IRQ_CLR_FLAG) {
		...
		IRQ_CLR_FLAG = true;
		ACCDET_DEBUG
		    ("[Accdet]%s:Clear interrupt:Done[0x%x]!\n",
		     __func__, accdet_read(ACCDET_IRQ_STS));
	} else {
		IRQ_CLR_FLAG = false;
	}
...
void accdet_get_dts_data(void)
{
	...
	ACCDET_INFO("[Accdet] accdet_dts_data.mic_mode_vol = %d\n",
		    accdet_dts_data.mic_mode_vol);
	ret =
	    regmap_update_bits(apmix_regmap, AUDIO_CODEC_CON01, 0x7 << 22,
			       (accdet_dts_data.mic_mode_vol) << 22);
	if (ret) {
		ACCDET_ERROR
		    ("Failed to update register AUDIO_CODEC_CON01: %d\n", ret);
		return;
	}
	ret = regmap_read(apmix_regmap, AUDIO_CODEC_CON01, &val);
	if (ret) {
		ACCDET_ERROR
		    ("[Accdet] Failed to read AUDIO_CODEC_CON01 value: %d\n",
		     ret);
		return;
	}
...
void accdet_set_four_key_threshold(void)
{
	...
	int mid_key_four_code, voice_key_four_code, up_key_four_code,
	    down_key_four_code;
	...
	case 0:
		...
		mid_key_four_code =
		    23 * (2048 + GE) * ratio_tmp / (152 * ratio_tmp2) + (OE -
									 512);
		voice_key_four_code =
		    48 * (2048 + GE) * ratio_tmp / (152 * ratio_tmp2) + (OE -
									 512);
		up_key_four_code =
		    55 * (2048 + GE) * ratio_tmp / (114 * ratio_tmp2) + (OE -
									 512);
		down_key_four_code =
		    125 * (2048 + GE) * ratio_tmp / (114 * ratio_tmp2) + (OE -
									  512);
		break;
	case 1:
		ACCDET_INFO("[ACCDET] MICBIAS1 Voltage is 2.2V!\n");
		mid_key_four_code =
		    253 * (2048 + GE) * ratio_tmp / (1900 * ratio_tmp2) + (OE -
									   512);
		voice_key_four_code =
		    132 * (2048 + GE) * ratio_tmp / (475 * ratio_tmp2) + (OE -
									  512);
		up_key_four_code =
		    121 * (2048 + GE) * ratio_tmp / (285 * ratio_tmp2) + (OE -
									  512);
		down_key_four_code =
		    55 * (2048 + GE) * ratio_tmp / (57 * ratio_tmp2) + (OE -
									512);
		break;
	case 2:
		ACCDET_INFO("[ACCDET] MICBIAS1 Voltage is 2.1V!\n");
		mid_key_four_code =
		    7 * 69 * (2048 + GE) * ratio_tmp / (3800 * ratio_tmp2) +
		    (OE - 512);
		voice_key_four_code =
		    126 * (2048 + GE) * ratio_tmp / (475 * ratio_tmp2) + (OE -
									  512);
		up_key_four_code =
		    154 * (2048 + GE) * ratio_tmp / (380 * ratio_tmp2) + (OE -
									  512);
		down_key_four_code =
		    105 * (2048 + GE) * ratio_tmp / (114 * ratio_tmp2) + (OE -
									  512);
		break;
	case 3:
		ACCDET_INFO("[ACCDET] MICBIAS1 Voltage is 2.0V!\n");
		mid_key_four_code =
		    23 * (2048 + GE) * ratio_tmp / (190 * ratio_tmp2) + (OE -
									 512);
		voice_key_four_code =
		    24 * (2048 + GE) * ratio_tmp / (95 * ratio_tmp2) + (OE -
									512);
		up_key_four_code =
		    22 * (2048 + GE) * ratio_tmp / (57 * ratio_tmp2) + (OE -
									512);
		down_key_four_code =
		    50 * (2048 + GE) * ratio_tmp / (57 * ratio_tmp2) + (OE -
									512);
		break;
	case 4:
		ACCDET_INFO("[ACCDET] MICBIAS1 Voltage is 1.9V!\n");
		mid_key_four_code =
		    23 * (2048 + GE) * ratio_tmp / (200 * ratio_tmp2) + (OE -
									 512);
		voice_key_four_code =
		    24 * (2048 + GE) * ratio_tmp / (100 * ratio_tmp2) + (OE -
									 512);
		up_key_four_code =
		    11 * (2048 + GE) * ratio_tmp / (30 * ratio_tmp2) + (OE -
									512);
		down_key_four_code =
		    5 * (2048 + GE) * ratio_tmp / (6 * ratio_tmp2) + (OE - 512);
		break;
	default:
...
static inline void accdet_init(void)
{
	...
	accdet_write(ACCDET_IRQ_STS,
		     accdet_read(ACCDET_IRQ_STS) & (~IRQ_CLR_BIT));
...
static ssize_t accdet_call_state_store(struct device_driver *ddri,
...
static ssize_t pin_recognition_show(struct device_driver *ddri, char *buf)
{
...
static ssize_t start_debug_store(struct device_driver *ddri, const char *buf,
				 size_t count)
{
	int start_flag;
	int error;
...
static ssize_t dump_reg_store(struct device_driver *ddri, const char *buf,
			      size_t count)
{
	int value;
	int ret;
...
int mt_accdet_probe(struct platform_device *dev)
{
	...
	apmix_regmap =
	    syscon_regmap_lookup_by_phandle(dev->dev.of_node,
	...
	g_accdet_adc_channel = iio_channel_get(&dev->dev, "accdet-channel");
	if (IS_ERR(g_accdet_adc_channel)) {
		ret = IS_ERR(g_accdet_adc_channel);
		ACCDET_ERROR("[Accdet] failed to get adc channel!\n");
		return ret;
	}
	...
	accdet_nor_device =
	    device_create(accdet_class, NULL, accdet_devno, NULL,
			  ACCDET_DEVNAME);
...
#else
	ACCDET_DEBUG
	    ("[Accdet]ACCDET_CTRL=[0x%x],STATE=[0x%x]->[0x%x]\n",
	     accdet_read(ACCDET_CTRL), pre_state_swctrl,
	     accdet_read(ACCDET_STATE_SWCTRL));
#endif
}
...
void mt_accdet_pm_restore_noirq(void)
{
	...
	eint_accdet_sync_flag = 1;
	current_status_restore = ((accdet_read(ACCDET_STATE_RG) & 0xc0) >> 6);
	...
	default:
		ACCDET_DEBUG
		    ("[Accdet]accdet current status error!\n");
		break;
	}
#if JUST_INPUT_NO_SWITCH
	switch_set_state((struct switch_dev *)&accdet_data, cable_type);
