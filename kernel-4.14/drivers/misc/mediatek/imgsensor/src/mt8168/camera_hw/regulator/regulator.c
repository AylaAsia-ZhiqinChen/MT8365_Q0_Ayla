...
static enum IMGSENSOR_RETURN regulator_init(void *pinstance)
{
	...
	if (pdevice->of_node == NULL) {
		cam_pr_debug("regulator get cust camera node failed!\n");
	...
	for (i = 0; i < REGULATOR_TYPE_MAX_NUM; i++, pregulator_ctrl++) {
		...
		if (preg->pregulator[i] == NULL)
			cam_pr_debug("regulator[%d]  %s fail!\n",
	...
	if (pregulator) {
		if (pin_state != IMGSENSOR_HW_PIN_STATE_LEVEL_0) {
			...
			if (regulator_set_voltage(
				...
				cam_pr_debug("[regulator]fail to regulator_set_voltage, powertype:%d powerId:%d\n",
					pin,
					regulator_voltage[
					pin_state -
			...
			if (regulator_enable(pregulator)) {
				cam_pr_debug("[regulator]fail to regulator_enable, powertype:%d powerId:%d\n",
					pin,
					regulator_voltage[
					pin_state -
					IMGSENSOR_HW_PIN_STATE_LEVEL_0]);
		...
		cam_pr_debug("regulator == NULL %d %d %d\n",
		    reg_type_offset,
		    pin,
