...
static void sensor_init(void)
{
	...
	write_cmos_sensor(0x0101, 0x03);//HV_MIRROR
...
{
	cam_pr_debug("preview_setting_mHDR\n");
	write_cmos_sensor(0x0100, 0x00);
	write_cmos_sensor(0x0114, 0x03);
	...
	write_cmos_sensor(0x0138, 0x01);
	write_cmos_sensor(0x0100, 0x01);
	cam_pr_debug("preview_setting_mHDR mode 0x0220(0x21), Ratio 0x0222(0x08)\n");
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id)
{
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		spin_unlock(&imgsensor_drv_lock);
		do {
			*sensor_id = ((read_cmos_sensor(0x0016) << 8) |
				read_cmos_sensor(0x0017));
			if (*sensor_id == imgsensor_info.sensor_id) {
				cam_pr_debug("IMX214 i2c write id: 0x%x, sensor id: 0x%x\n",
					imgsensor.i2c_write_id, *sensor_id);
				return ERROR_NONE;
			}
			cam_pr_debug("IMX214 Read sensor id fail, write id:0x%x id: 0x%x\n",
				imgsensor.i2c_write_id, *sensor_id);
			retry--;
		} while (retry > 0);
		i++;
...
static kal_uint32 open(void)
{
	...
	cam_pr_debug("PLATFORM:MT6595,MIPI 2LANE\n");
	//sensor have two i2c address 0x6c 0x6d & 0x21 0x20,
	//we should detect the module used i2c address
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		spin_lock(&imgsensor_drv_lock);
