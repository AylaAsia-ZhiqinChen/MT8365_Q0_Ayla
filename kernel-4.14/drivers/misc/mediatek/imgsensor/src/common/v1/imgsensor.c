	...
	MSDK_SENSOR_CONFIG_STRUCT *pSensorConfigData)
{
	...
	if (psensor_func &&
		...
		if (ret != ERROR_NONE)
			cam_pr_info("[%s] SensorGetInfo failed\n", __func__);
...
static inline int imgsensor_check_is_alive(struct IMGSENSOR_SENSOR *psensor)
{
	...
	if (sensorID == 0 || sensorID == 0xFFFFFFFF) {
		cam_pr_info("Fail to get sensor ID %x\n", sensorID);
		err = ERROR_SENSOR_CONNECT_FAIL;
	} else {
		cam_pr_info(" Sensor found ID = 0x%x\n", sensorID);
		err = ERROR_NONE;
	}

	if (err != ERROR_NONE)
		cam_pr_info("ERROR: No imgsensor alive\n");
...
#define STRINGIZE(stringizedName) TOSTRING(stringizedName)
	...
	/*cam_pr_debug("get_search_list %d,\n %d %d %d %d\n %d %d %d %d\n",
	 *   get_search_list,
	 *   orderedSearchList[0],
	 *   orderedSearchList[1],
	 *   orderedSearchList[2],
	...
	for (i = 0; i < MAX_NUM_OF_SUPPORT_SENSOR; i++) {
		/*cam_pr_debug("orderedSearchList[%d]=%d\n",
		...
		if (pSensorList[drv_idx].init) {
			...
			cam_pr_info("ERROR:NULL sensor list[%d]\n", drv_idx);
		}
...
static inline int adopt_CAMERA_HW_GetInfo(void *pBuf)
{
	...
	if (psensor == NULL) {
		cam_pr_debug("[CAMERA_HW] NULL psensor.\n");
		return -EFAULT;
	}
	...
	if (copy_to_user((void __user *)(pSensorGetInfo->pInfo),
		...
		cam_pr_info("[CAMERA_HW][info] ioctl copy to user failed\n");
		if (pInfo != NULL)
			kfree(pInfo);
		if (pConfig != NULL)
			kfree(pConfig);
	...
	struct work_struct *data)
{
	...
	if (ret != ERROR_NONE)
		cam_pr_info("Get Main cam temperature error(%d)!\n", ret);
	...
	if (ret != ERROR_NONE)
		cam_pr_info("Get Sub cam temperature error(%d)!\n", ret);
	...
	if (ret != ERROR_NONE)
		cam_pr_info("Get Main2 cam temperature error(%d)!\n", ret);
	...
	if (ret != ERROR_NONE)
		cam_pr_info("Get Sub2 cam temperature error(%d)!\n", ret);
...
static inline int adopt_CAMERA_HW_GetInfo2(void *pBuf)
{
	...
	if (pSensorGetInfo == NULL ||
		...
		cam_pr_info("[%s] NULL arg.\n", __func__);
		return -EFAULT;
	}

	psensor = imgsensor_sensor_get_inst(pSensorGetInfo->SensorId);
	if (psensor == NULL) {
		cam_pr_info("[%s] NULL psensor.\n", __func__);
		return -EFAULT;
	}
	...
	if (copy_to_user(
		...
		cam_pr_debug("[CAMERA_HW][Resolution] ioctl copy to user failed\n");
		ret = -EFAULT;
		goto IMGSENSOR_GET_INFO_RETURN;
	}
...
static inline int adopt_CAMERA_HW_FeatureControl(void *pBuf)
{
	...
	case SENSOR_FEATURE_SET_PDAF_REG_SETTING:
	{
		...
		if (u4RegLen > PDAF_DATA_SIZE) {
			kfree(pFeaturePara);
			cam_pr_debug("check: u4RegLen > PDAF_DATA_SIZE\n");
			return -EINVAL;
		}
	...
	case SENSOR_FEATURE_GET_4CELL_DATA:
	{
		...
		if (buf_size > PDAF_DATA_SIZE) {
			kfree(pFeaturePara);
			cam_pr_debug("check: buf_size > PDAF_DATA_SIZE\n");
			return -EINVAL;
		}
		pPdaf_data = kmalloc(
...
imgsensor_compat_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	case COMPAT_KDIMGSENSORIOC_X_GETINFO:
	{
		...
		struct IMGSENSOR_GET_CONFIG_INFO_STRUCT __user *data;
		int err;
		/*cam_pr_debug("CAOMPAT_KDIMGSENSORIOC_X_GETINFO E\n"); */
	...
	case COMPAT_KDIMGSENSORIOC_X_CONTROL:
	{
		...
		cam_pr_debug("[CAMERA SENSOR] CAOMPAT_KDIMGSENSORIOC_X_CONTROL\n");
	...
	unsigned long a_u4Param)
{
	...
	if (_IOC_DIR(a_u4Command) != _IOC_NONE) {
		...
		if (pBuff == NULL) {
			/*
			 * cam_pr_debug("[CAMERA SENSOR] ioctl allocate
			 * mem failed\n");
			 */
			i4RetValue = -ENOMEM;
...
#define ISP_CLK_HIGH   490
	...
	case KDIMGSENSORIOC_X_SET_CURRENT_SENSOR:
		...
		break;
	default:
		cam_pr_debug("No such command %d\n", a_u4Command);
		i4RetValue = -EPERM;
		break;
	}
...
static inline int imgsensor_driver_register(void)
{
	...
	if (cdev_add(gpimgsensor_cdev, dev_no, 1)) {
		cam_pr_debug("Attatch file operation failed\n");
		unregister_chrdev_region(dev_no, 1);
		return -EAGAIN;
	}
...
{
	/*cam_pr_debug("mmsys_clk_change_cb
	 * ori:%d, new:%d, current_mmsys_clk %d\n",
	 *		ori_clk_mode,
	 *		new_clk_mode,
	 *		current_mmsys_clk);
	 */
...
{
	cam_pr_info("[camerahw_probe] start\n");
