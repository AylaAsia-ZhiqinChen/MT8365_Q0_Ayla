...
static struct SENSOR_WINSIZE_INFO_STRUCT imgsensor_winsize_info[5] = {
	...
	{2592, 1944, 0, 0, 2592, 1944, 2592, 1944,
	    0000, 0000, 2592, 1944, 0, 0, 2592, 1944},/* hs video */
	{2592, 1944, 0, 0, 2592, 1944, 2592, 1944,
...
static void gc5025_gcore_read_otp_info(void)
{
	...
	case 0x01:
		...
		if (total_number <= 31)
			gc5025_read_otp_group(0x03,
			&ddtempbuff[0], total_number * 4);
		else {
			gc5025_read_otp_group(0x03, &ddtempbuff[0], 31 * 4);
			gc5025_select_page_otp(otp_page1);
			gc5025_read_otp_group(0x29,
				&ddtempbuff[31 * 4], (total_number - 31) * 4);
		}

		for (i = 0; i < total_number; i++) {
			LOG_INF("index=%d,d[0]=%x,d[1]=%x,d[2]=%x,d[3]=%x\n",
	...
	case 0x01:
		...
		i = 0;
		do {
			gc5025_otp_info.reg_addr[i] =
				gc5025_read_otp(REG_ROM_START + i*2);
			gc5025_otp_info.reg_value[i] =
				gc5025_read_otp(REG_ROM_START + i * 2 + 1);
			LOG_INF("reg_addr[%d] = 0x%x, reg_value[%d] = 0x%x\n",
				i, gc5025_otp_info.reg_addr[i],
				i, gc5025_otp_info.reg_value[i]);
			i++;
		...
		case 0x01:
			...
			if ((check % 255 + 1) == wb[WB_WIDTH - 1]) {
				gc5025_otp_info.rg_gain =
					(((wb[0] << 8) & 0xff00) | wb[1]) > 0 ?
					(((wb[0] << 8) & 0xff00) | wb[1]) :
					0x400;
				gc5025_otp_info.bg_gain =
					(((wb[2] << 8) & 0xff00) | wb[3]) > 0 ?
					(((wb[2] << 8) & 0xff00) | wb[3]) :
					0x400;
				gc5025_otp_info.wb_flag =
...
static void gc5025_gcore_update_dd(void)
{
	...
	if (gc5025_otp_info.dd_flag == 0x01) {
		...
		for (i = 0; i < gc5025_otp_info.dd_cnt; i++) {
			if (gc5025_otp_info.dd_param_type[i] == 0) {
				gc5025_otp_info.dd_param_x[i] =
					WINDOW_WIDTH -
					gc5025_otp_info.dd_param_x[i] + 1;
				gc5025_otp_info.dd_param_y[i] =
					WINDOW_HEIGHT -
					gc5025_otp_info.dd_param_y[i] + 1;
			...
			for (j = i + 1; j < gc5025_otp_info.dd_cnt; j++)
				if (gc5025_otp_info.dd_param_y[i] * WINDOW_WIDTH
					...
					* WINDOW_WIDTH
					...
					temp_type =
					 gc5025_otp_info.dd_param_type[i];
					gc5025_otp_info.dd_param_type[i] =
					 gc5025_otp_info.dd_param_type[j];
					gc5025_otp_info.dd_param_type[j] =
						temp_type;
				}
		...
		for (i = 0; i < gc5025_otp_info.dd_cnt; i++) {
			...
			temp_val1 =
			  (((gc5025_otp_info.dd_param_y[i]) << 4) & 0x00f0)
...
static void gc5025_gcore_update_wb(void)
{
	...
	if (0x01 == (gc5025_otp_info.wb_flag & 0x01)) {
		...
		LOG_INF("rgcurrent = 0x%x, bgcurrent = 0x%x, bgain = 0x%x\n",
		...
		write_cmos_sensor(0xc4,
			((g_gain & 0x07) << 4) + (r_gain & 0x07));
		write_cmos_sensor(0xc5,
...
static void gc5025_gcore_update_chipversion(void)
{
	...
	for (i = 0; i < gc5025_otp_info.reg_num; i++) {
		write_cmos_sensor(gc5025_otp_info.reg_addr[i],
			gc5025_otp_info.reg_value[i]);
		LOG_INF("{0x%x, 0x%x}\n",
			gc5025_otp_info.reg_addr[i],
			gc5025_otp_info.reg_value[i]);
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			*sensor_id = return_sensor_id();
			...
			LOG_INF("Read sensor fail,write id:0x%x,id:0x%x\n",
				imgsensor.i2c_write_id, *sensor_id);
			retry--;
		} while (retry > 0);
		i++;
	...
	MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	...
	if (imgsensor.current_fps == imgsensor_info.cap1.max_framerate) {
		...
		if (imgsensor.current_fps != imgsensor_info.cap.max_framerate)
			LOG_INF("Warning:cfps %d is not support,use: %d!\n",
	...
	case MSDK_SCENARIO_ID_CAMERA_CAPTURE_JPEG:
		if (imgsensor.current_fps ==
			...
			if (imgsensor.current_fps !=
				...
				LOG_INF("Warning:%d not support,use: %d!\n",
					framerate,
					imgsensor_info.cap.max_framerate / 10);
