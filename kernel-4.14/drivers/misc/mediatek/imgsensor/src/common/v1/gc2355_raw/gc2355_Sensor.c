...
#define LOG_1 cam_pr_debug("GC2355, DVP\n")
/***************************   Modify end    ********************************/
...
#define ANALOG_GAIN_7 470	/* 7.15x */
	...
	if ((iReg >= ANALOG_GAIN_1) && (iReg < ANALOG_GAIN_2)) {
		...
		cam_pr_debug("GC2355 analogic gain 1x , GC2355 add pregain = %d\n",
			temp);
		...
		write_cmos_sensor(0xb1, temp >> 6);
		write_cmos_sensor(0xb2, (temp << 2) & 0xfc);
		cam_pr_debug("GC2355 analogic gain 1.45x , GC2355 add pregain = %d\n",
			temp);
		...
		write_cmos_sensor(0xb1, temp >> 6);
		write_cmos_sensor(0xb2, (temp << 2) & 0xfc);
		cam_pr_debug("GC2355 analogic gain 2.02x , GC2355 add pregain = %d\n",
			temp);
	}
	/* else if(ANALOG_GAIN_4<= iReg) */
		...
		write_cmos_sensor(0xb1, temp >> 6);
		write_cmos_sensor(0xb2, (temp << 2) & 0xfc);
		cam_pr_debug("GC2355 analogic gain 2.8x, GC2355 add pregain = %d\n",
			temp);
	}
	return gain;
	...
	case SENSOR_FEATURE_GET_CROP_INFO:
		...
		case MSDK_SCENARIO_ID_CAMERA_CAPTURE_JPEG:
			if (copy_to_user((void *)wininfo,
				...
				cam_pr_debug("copy to user failed\n");
				return -EFAULT;
			}
			break;
		case MSDK_SCENARIO_ID_VIDEO_PREVIEW:
