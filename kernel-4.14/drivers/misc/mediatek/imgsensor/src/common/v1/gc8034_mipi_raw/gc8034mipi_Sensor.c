...
#define LOG_1 cam_pr_debug("GC8034,MIPI 4LANE\n")
/******************   Modify end  ******************************************/
#undef GC8034_DEBUG
...
static struct SENSOR_WINSIZE_INFO_STRUCT imgsensor_winsize_info[5] = {
	...
	{ 3264, 2448, 252, 504, 2560, 1440, 1280,  720,
		0, 0, 1280,  720, 0, 0, 1280,  720}  /* slim video */
};
...
static void gc8034_read_otp_group(kal_uint8 page,
		kal_uint8 addr, kal_uint8 *buff, int size)
{
	kal_uint8 i;
	kal_uint8 regf4 = read_cmos_sensor(0xf4);
...
static void gc8034_gcore_read_otp_info(void)
{
...
#if defined(GC8034OTP_FOR_CUSTOMER)
	kal_uint32 check = 0;
	kal_uint8  flag_wb = 0, index = 0,
			   flag_module = 0, flag_lsc = 0, flag_af = 0;
	kal_uint8  info[8] = { 0 };
	kal_uint8  wb[4] = { 0 };
	...
	cam_pr_debug("GC8034_OTP_DD : flag_dd = 0x%x\n", flagdd);

	switch (flagdd & 0x03) {
	case 0x00:
		cam_pr_debug("GC8034_OTP_DD is Empty !!\n");
		gc8034_otp_info.dd_flag = 0x00;
		break;
	case 0x01:
		cam_pr_debug("GC8034_OTP_DD is Valid!!\n");
		total_number = gc8034_read_otp(0, 0x0c) +
			gc8034_read_otp(0, 0x0d);
		gc8034_otp_info.dd_flag = 0x01;
		gc8034_read_otp_kgroup(0, 0x0e,
			&ddtempbuff[0], 4 * total_number);
		for (i = 0; i < total_number; i++) {
			if ((ddtempbuff[4 * i + 3] & 0x80) == 0x80) {
				if ((ddtempbuff[4 * i + 3] & 0x03) == 0x03) {
					gc8034_otp_info.dd_param[cnt].x =
						(((kal_uint16)ddtempbuff[4
						  * i + 1]
						  & 0x0f) << 8)
						+ ddtempbuff[4 * i];
		...
		cam_pr_debug("GC8034_OTP : total_number = %d\n",
				gc8034_otp_info.dd_cnt);
		break;
	case 0x02:
	case 0x03:
		cam_pr_debug("GC8034_OTP_DD is Invalid !!\n");
	...
	cam_pr_debug("GC8034_OTP : flag_module = 0x%x,
			flag_wb = 0x%x, flag_af = 0x%x\n",
	...
	for (index = 0; index < 2; index++) {
		...
		case 0x00:
			cam_pr_debug("GC8034_OTP_INFO group %d is Empty !!\n",
					index + 1);
			break;
		case 0x04:
			cam_pr_debug("GC8034_OTP_INFO group %d is Valid !!\n",
					index + 1);
			check = 0;
			gc8034_read_otp_group(9, INFO_ROM_START + index
					* INFO_WIDTH, &info[0], INFO_WIDTH);
			...
			if ((check % 255 + 1) == info[INFO_WIDTH - 1]) {
				...
				cam_pr_debug("GC8034_OTP_INFO Check sum %d Error !!\n",
					index + 1);
			break;
		case 0x08:
		case 0x0c:
			cam_pr_debug("GC8034_OTP_INFO group %d
					is Invalid !!\n", index + 1);
			break;
		default:
			break;
		...
		case 0x00:
			cam_pr_debug("GC8034_OTP_WB group %d is Empty !!\n",
					index + 1);
			gc8034_otp_info.wb_flag =
				gc8034_otp_info.wb_flag | 0x00;
			break;
		case 0x04:
			cam_pr_debug("GC8034_OTP_WB group %d is Valid !!\n",
					index + 1);
			check = 0;
			gc8034_read_otp_group(9, WB_ROM_START + index
					* WB_WIDTH, &wb[0], WB_WIDTH);
			for (i = 0; i < WB_WIDTH - 1; i++)
				check += wb[i];

			if ((check % 255 + 1) == wb[WB_WIDTH - 1]) {
				gc8034_otp_info.rg_gain = (wb[0] |
						((wb[1] & 0xf0) << 4)) > 0 ?
					(wb[0] | ((wb[1] & 0xf0) << 4)) : 0x400;
				gc8034_otp_info.bg_gain = (((wb[1]
						& 0x0f) << 8) | wb[2]) > 0 ?
					(((wb[1] & 0x0f) << 8) | wb[2])
					: 0x400;
				gc8034_otp_info.wb_flag =
					gc8034_otp_info.wb_flag | 0x01;
			} else
				cam_pr_debug("GC8034_OTP_WB Check sum %d Error !!\n",
						index + 1);
			break;
		case 0x08:
		case 0x0c:
			cam_pr_debug("GC8034_OTP_WB group %d is Invalid !!\n",
					index + 1);
			gc8034_otp_info.wb_flag =
				gc8034_otp_info.wb_flag | 0x02;
			break;
		default:
			break;
		}
		...
		case 0x00:
			cam_pr_debug("GC8034_OTP_GOLDEN group %d is Empty !!\n",
					index + 1);
			gc8034_otp_info.golden_flag =
				gc8034_otp_info.golden_flag | 0x00;
			break;
		case 0x40:
			cam_pr_debug("GC8034_OTP_GOLDEN group %d is Valid !!\n",
					index + 1);
			check = 0;
			gc8034_read_otp_group(9, GOLDEN_ROM_START + index
				* GOLDEN_WIDTH, &golden[0], GOLDEN_WIDTH);
			for (i = 0; i < GOLDEN_WIDTH - 1; i++)
				check += golden[i];

			if ((check % 255 + 1) == golden[GOLDEN_WIDTH - 1]) {
				gc8034_otp_info.golden_rg = (golden[0]
					| ((golden[1] & 0xf0) << 4)) > 0 ?
				...
				gc8034_otp_info.golden_bg =
					...
					: BG_TYPICAL;
				gc8034_otp_info.golden_flag =
					gc8034_otp_info.golden_flag | 0x01;
			} else
				cam_pr_debug("GC8034_OTP_GOLDEN Check sum
						%d Error !!\n", index + 1);

			break;
		case 0x80:
		case 0xc0:
			cam_pr_debug("GC8034_OTP_GOLDEN group %d is Invalid !!\n",
					index + 1);
			gc8034_otp_info.golden_flag =
				gc8034_otp_info.golden_flag | 0x02;
			break;
		default:
			break;
		}
		...
		case 0x00:
			cam_pr_debug("GC8034_OTP_AF group %d is Empty !!\n",
					index + 1);
			gc8034_otp_info.af_flag =
				gc8034_otp_info.af_flag | 0x00;
			break;
		case 0x04:
			cam_pr_debug("GC8034_OTP_AF group %d is Valid !!\n",
					index + 1);
			check = 0;
			gc8034_read_otp_group(3, AF_ROM_START +
					index * AF_WIDTH, &af[0], AF_WIDTH);
			...
			if ((check % 255 + 1) == af[AF_WIDTH - 1]) {
				...
				gc8034_otp_info.af_macro =
					((af[0] << 8) & 0x0f00) + af[2];
				gc8034_otp_info.af_flag =
					gc8034_otp_info.af_flag | 0x01;
			} else
				cam_pr_debug("GC8034_OTP_AF Check sum %d Error !!\n",
						index + 1);
			break;
		case 0x08:
		case 0x0c:
			cam_pr_debug("GC8034_OTP_AF group %d is Invalid !!\n",
					index + 1);
			gc8034_otp_info.af_flag =
				gc8034_otp_info.af_flag | 0x02;
			break;
		default:
			break;
		}
	...
	cam_pr_debug("GC8034_OTP_LSC : flag_lsc = 0x%x\n", flag_lsc);
	gc8034_otp_info.lsc_flag = 0;
	for (index = 0; index < 2; index++) {
		switch ((flag_lsc << (2 * index)) & 0x0c) {
		case 0x00:
			cam_pr_debug("GC8034_OTP_LSC group %d is Empty !\n",
					index + 1);
			gc8034_otp_info.lsc_flag =
				gc8034_otp_info.lsc_flag | 0x00;
			break;
		case 0x04:
			cam_pr_debug("GC8034_OTP_LSC group %d is Valid !\n",
					index + 1);
			check = 0;
			if (index == 0) {
				gc8034_read_otp_kgroup(3, 0x44,
					&gc8034_otp_info.lsc_param[0], 396);
				lsc_checksum = gc8034_read_otp(6, 0x50);
			} else {
				gc8034_read_otp_kgroup(6, 0x51,
					&gc8034_otp_info.lsc_param[0], 396);
			...
			if ((check % 255 + 1) == lsc_checksum)
				...
				cam_pr_debug("GC8034_OTP_LSC check sum error, check sum read : 0x%x, calculate:0x%x!!\n",
					lsc_checksum, check % 255 + 1);
			}
			break;
		case 0x08:
		case 0x0c:
			cam_pr_debug("GC8034_OTP_LSC group %d is Invalid !!\n",
					index + 1);
			gc8034_otp_info.lsc_flag =
				gc8034_otp_info.lsc_flag | 0x02;
			break;
	...
	cam_pr_debug("GC8034_OTP_INFO:module_id=0x%x\n",
		gc8034_otp_info.module_id);
	cam_pr_debug("GC8034_OTP_INFO:lens_id=0x%x\n",
		gc8034_otp_info.lens_id);
	cam_pr_debug("GC8034_OTP_INFO:vcm_id=0x%x\n",
		gc8034_otp_info.vcm_id);
	cam_pr_debug("GC8034_OTP_INFO:vcm_driver_id=0x%x\n",
		gc8034_otp_info.vcm_driver_id);
	cam_pr_debug("GC8034_OTP_INFO:data=%d-%d-%d\n",
		gc8034_otp_info.year,
		gc8034_otp_info.month, gc8034_otp_info.day);
	cam_pr_debug("GC8034_OTP_WB:r/g=0x%x\n",
		gc8034_otp_info.rg_gain);
	cam_pr_debug("GC8034_OTP_WB:b/g=0x%x\n",
		gc8034_otp_info.bg_gain);
	cam_pr_debug("GC8034_OTP_GOLDEN:golden_rg=0x%x\n",
		gc8034_otp_info.golden_rg);
	cam_pr_debug("GC8034_OTP_GOLDEN:golden_bg=0x%x\n",
		gc8034_otp_info.golden_bg);
	cam_pr_debug("GC8034_OTP_AF:infitiy=0x%x\n",
		gc8034_otp_info.af_infinity);
	cam_pr_debug("GC8034_OTP_AF:macro=0x%x\n",
		gc8034_otp_info.af_macro);
#endif
		...
		for (i = 0; i < 5; i++) {
			...
			for (j = 0; j < 2; j++)
				if (((temp >> (4 * j + 3)) & 0x01) == 0x01) {
					gc8034_otp_info.reg_page
						[gc8034_otp_info.reg_num] =
						(temp >> (4 * j)) & 0x03;
					gc8034_otp_info.reg_addr
						[gc8034_otp_info.reg_num] =
						gc8034_read_otp(2,
						0x50 + 5 * i + 2 * j);
					gc8034_otp_info.reg_value
						[gc8034_otp_info.reg_num] =
						gc8034_read_otp(2, 0x50
						+ 5 * i + 2 * j + 1);
					gc8034_otp_info.reg_num++;
				}
		}
	...
	for (i = 0; i < 10; i++) {
		...
		for (j = 0; j < 128; j++)
			cam_pr_debug("GC8034_OTP_DEBUG: Page%d:addr = 0x%x, value = 0x%x;\n",
				i, j, debug[j]);
	}
#endif
}
...
static void gc8034_gcore_update_dd(void)
{
	...
	if (gc8034_otp_info.dd_flag == 0x01) {
		cam_pr_debug("GC8034_OTP_AUTO_DD start !\n");
		...
		for (i = 0; i < gc8034_otp_info.dd_cnt - 1; i++) {
			for (j = i + 1; j < gc8034_otp_info.dd_cnt; j++) {
				if (gc8034_otp_info.dd_param[i].y *
					DD_WIDTH + gc8034_otp_info.dd_param[i].x
				 > gc8034_otp_info.dd_param[j].y *
				 DD_WIDTH + gc8034_otp_info.dd_param[j].x) {
					dd_temp.x =
						gc8034_otp_info.dd_param[i].x;
					dd_temp.y =
						gc8034_otp_info.dd_param[i].y;
		...
		for (i = 0; i < gc8034_otp_info.dd_cnt; i++) {
			...
			temp_val1 = ((gc8034_otp_info.dd_param[i].y
				& 0x000f) << 4) +
				((gc8034_otp_info.dd_param[i].x & 0x0f00)>>8);
			temp_val2 =
				(gc8034_otp_info.dd_param[i].y & 0x0ff0) >> 4;
			...
			cam_pr_debug("GC8034_OTP_GC val0 = 0x%x, val1 = 0x%x, val2 = 0x%x\n",
				temp_val0, temp_val1, temp_val2);
			cam_pr_debug("GC8034_OTP_GC x = %d, y = %d\n",
...
static void gc8034_gcore_update_wb(void)
{
	kal_uint16 r_gain_current = 0,
		g_gain_current = 0, b_gain_current = 0, base_gain = 0;
	...
	if (0x01 == (gc8034_otp_info.golden_flag & 0x01)) {
		...
		cam_pr_debug("GC8034_OTP_UPDATE_AWB:rg_typical = 0x%x, bg_typical = 0x%x\n",
	...
	if (0x01 == (gc8034_otp_info.wb_flag & 0x01)) {
		...
		cam_pr_debug("GC8034_OTP_UPDATE_AWB:r = 0x%x, g = 0x%x, b = 0x%x\n",
	...
	if ((gc8034_otp_info.lsc_flag & 0x01) == 0x01) {
		...
		for (i = 0; i < 9; i++) {
			...
			for (j = 0; j < 11; j++) {
				...
				base = 4 * (11 * (8 - i) + 10 - j);
#endif
				write_cmos_sensor(0xcc,
					gc8034_otp_info.lsc_param[base + 0]);
				write_cmos_sensor(0xcc,
					gc8034_otp_info.lsc_param[base + 1]);
				write_cmos_sensor(0xcc,
					gc8034_otp_info.lsc_param[base + 2]);
				write_cmos_sensor(0xcc,
					gc8034_otp_info.lsc_param[base + 3]);
		}
		}
...
static void gc8034_gcore_update_chipversion(void)
{
	...
	cam_pr_debug("GC8034_OTP_UPDATE_CHIPVERSION:reg_num = %d\n",
			gc8034_otp_info.reg_num);
	...
	if (gc8034_otp_info.reg_flag)
		for (i = 0; i < gc8034_otp_info.reg_num; i++) {
			...
			cam_pr_debug("GC8034_OTP_UPDATE_CHIP_VERSION: P%d:0x%x -> 0x%x\n",
			gc8034_otp_info.reg_page[i],
			gc8034_otp_info.reg_addr[i],
			gc8034_otp_info.reg_value[i]);
		}
}
...
static void gc8034_gcore_enable_otp(kal_uint8 state)
{
	...
	if (state) {
		...
		cam_pr_debug("GC8034_OTP: Enable OTP!\n");
	} else {
		otp_en = otp_en & 0xf7;
		otp_clk = otp_clk & 0xfe;
		write_cmos_sensor(0xf4, otp_en);
		write_cmos_sensor(0xf2, otp_clk);
		cam_pr_debug("GC8034_OTP: Disable OTP!\n");
	}
}
...
static void set_shutter(kal_uint16 shutter)
{
	...
	cam_pr_debug("Enter set shutter!\n");
...
static void gain2reg(kal_uint16 gain)
{
	...
	kal_uint8 agc_register[2 * MAX_AG_INDEX][AGC_REG_NUM] = {
	  /*{ 0xfe, 0x20, 0x33, 0xfe, 0xdf, 0xe7, 0xe8, 0xe9,
	   * 0xea, 0xeb, 0xec, 0xed, 0xee, 0xfe },
	   */
		/* binning */
		{ 0x00, 0x55, 0x83, 0x01, 0x06, 0x18, 0x20, 0x16,
			0x17, 0x50, 0x6c, 0x9b, 0xd8, 0x00 },
		{ 0x00, 0x55, 0x83, 0x01, 0x06, 0x18, 0x20, 0x16,
			0x17, 0x50, 0x6c, 0x9b, 0xd8, 0x00 },
		{ 0x00, 0x4e, 0x84, 0x01, 0x0c, 0x2e, 0x2d, 0x15,
			0x19, 0x47, 0x70, 0x9f, 0xd8, 0x00 },
		{ 0x00, 0x51, 0x80, 0x01, 0x07, 0x28, 0x32, 0x22,
			0x20, 0x49, 0x70, 0x91, 0xd9, 0x00 },
		{ 0x00, 0x4d, 0x83, 0x01, 0x0f, 0x3b, 0x3b, 0x1c,
			0x1f, 0x47, 0x6f, 0x9b, 0xd3, 0x00 },
		{ 0x00, 0x50, 0x83, 0x01, 0x08, 0x35, 0x46, 0x1e,
			0x22, 0x4c, 0x70, 0x9a, 0xd2, 0x00 },
		{ 0x00, 0x52, 0x80, 0x01, 0x0c, 0x35, 0x3a, 0x2b,
			0x2d, 0x4c, 0x67, 0x8d, 0xc0, 0x00 },
		{ 0x00, 0x52, 0x80, 0x01, 0x0c, 0x35, 0x3a, 0x2b,
			0x2d, 0x4c, 0x67, 0x8d, 0xc0, 0x00 },
		{ 0x00, 0x52, 0x80, 0x01, 0x0c, 0x35, 0x3a, 0x2b,
			0x2d, 0x4c, 0x67, 0x8d, 0xc0, 0x00 },
	...
	for (gain_index = MEAG_INDEX - 1; gain_index >= 0; gain_index--)
		if (gain >= gain_level[gain_index]) {
			...
			write_cmos_sensor(0xfe,
				agc_register[BorF *
				MAX_AG_INDEX + gain_index][0]);
			write_cmos_sensor(0x20,
				agc_register[BorF *
				MAX_AG_INDEX + gain_index][1]);
			write_cmos_sensor(0x33,
				agc_register[BorF *
				MAX_AG_INDEX + gain_index][2]);
			write_cmos_sensor(0xfe,
				agc_register[BorF *
				MAX_AG_INDEX + gain_index][3]);
			write_cmos_sensor(0xdf,
				agc_register[BorF *
				MAX_AG_INDEX + gain_index][4]);
			write_cmos_sensor(0xe7,
				agc_register[BorF *
				MAX_AG_INDEX + gain_index][5]);
			write_cmos_sensor(0xe8,
				agc_register[BorF *
				MAX_AG_INDEX + gain_index][6]);
			write_cmos_sensor(0xe9,
				agc_register[BorF *
				MAX_AG_INDEX + gain_index][7]);
			write_cmos_sensor(0xea,
				agc_register[BorF *
				MAX_AG_INDEX + gain_index][8]);
			write_cmos_sensor(0xeb,
				agc_register[BorF *
				MAX_AG_INDEX + gain_index][9]);
			write_cmos_sensor(0xec,
				agc_register[BorF *
				MAX_AG_INDEX + gain_index][10]);
			write_cmos_sensor(0xed,
				agc_register[BorF *
				MAX_AG_INDEX + gain_index][11]);
			write_cmos_sensor(0xee,
				agc_register[BorF *
				MAX_AG_INDEX + gain_index][12]);
			write_cmos_sensor(0xfe,
				agc_register[BorF *
				MAX_AG_INDEX + gain_index][13]);
			break;
		}
}
...
{
	cam_pr_debug("Enter preview setting!\n");
...
{
	cam_pr_debug("Enter capture setting!\n");
	fullsize_setting();
}

static void normal_video_setting(void)
{
	cam_pr_debug("Enter normal video setting!\n");
	fullsize_setting();
}
...
{
	cam_pr_debug("E\n");
	cam_pr_debug("Enter normal_video_setting!\n");
	spin_lock(&imgsensor_drv_lock);
		...
		enum MSDK_SCENARIO_ID_ENUM scenario_id, MUINT32 framerate)
{
	...
	case MSDK_SCENARIO_ID_CAMERA_CAPTURE_JPEG:
		if (imgsensor.current_fps ==
			...
			frame_length =
				imgsensor_info.cap1.pclk / framerate
	...
	UINT8 *feature_para, UINT32 *feature_para_len)
{
	...
	case SENSOR_FEATURE_GET_REGISTER:
		...
		cam_pr_debug("adb_i2c_read 0x%x = 0x%x\n",
