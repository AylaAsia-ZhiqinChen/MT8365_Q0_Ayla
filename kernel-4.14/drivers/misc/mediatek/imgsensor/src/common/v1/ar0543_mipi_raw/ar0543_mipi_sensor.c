...
//#define LOG_1 cam_pr_debug("AR0543, MIPI 2LANE\n")
//#define LOG_2 cam_pr_debug("preview 1280*960@30fps;
//video 1280*960@30fps; capture 5M@15fps\n")
/*****Modify end******/
...
static struct imgsensor_info_struct imgsensor_info = {
	...
	.checksum_value = 0xc9e24698,

	.pre = {
		//record different mode's pclk
		.pclk = 111800000,
		//3694, //record different mode's linelength
		.linelength = 3694,
		.framelength = 2021,//record different mode's framelength
...
static struct imgsensor_struct imgsensor = {
	...
	//300 //full size current fps : 24fps for PIP, 30fps for Normal or ZSD
	.current_fps = 0,
...
static struct SENSOR_WINSIZE_INFO_STRUCT imgsensor_winsize_info[5] = {
	{ 2608, 1960,    8,  8, 2592, 1944, 2592,
		1944,    0,   0, 2592, 1944,  0,  1, 2560,  1920}, // capture
	{ 2608, 1960,    8,  8, 2592, 1944, 2592,
		1944,    0,   0, 2592, 1944,  0,  1, 2560,  1920}, // capture
	{ 2608, 1960,    8,  8, 2592, 1944, 2592,
		1944,    0,   0, 2592, 1944,  0,  1, 2560,  1920}, // capture
...
static kal_uint16 read_cmos_sensor(kal_uint32 addr)
{
	kal_uint16 get_byte = 0;
		 char pu_send_cmd[2] = {(char)(addr >> 8),
			 (char)(addr & 0xFF) };
...
static void write_shutter(kal_uint16 shutter)
{
	...
	cam_pr_debug("[AR0543] >> write shutter , shutter = %d\n", shutter);
	...
	cam_pr_debug("[AR0543] << write shutter, shutter =%d, framelength =%d\n",
...
static kal_uint16 set_gain(kal_uint16 gain)
{
	...
	cam_pr_debug("[AR0543] >> set gain , gain = %d\n", gain);
	reg_gain = gain>>1;
...
static void set_mirror_flip(kal_uint8 image_mirror)
{
	...
	cam_pr_debug("[AR0543] >> set mirror flip , image_mirror = %d\n",
			image_mirror);
	switch (image_mirror) {
	case IMAGE_NORMAL:
...
{
	cam_pr_debug("[AR0543] >> sensor init\n");
	...
	//RESET_REGISTER enable mipi interface  bit[9] mask bad frame
	...
	// disable AF  AR0543 have not internal AF IC
	write_cmos_sensor(0x30F0, 0x0000);
...
static void preview_setting(void)
{
	 ...
	 *
	 ********************************************************/
	//	cam_pr_debug("[AR0543] >>  preview_setting\n");
	...
	temp = temp & 0xF000;
	temp = temp | 0x04C3;
	// READ_MODE  10 011 000011 xy binning enable xodd=3, yodd=3
	write_cmos_sensor(0x3040, temp);
	...
	// coarse_integration_time, nick-differnce
	//write_cmos_sensor(0x3012, 0x0414);
	...
	temp = temp & 0xF000;
	temp = temp | 0x0041;
	//Read Mode = 0x41   1 000001 binning disable
	write_cmos_sensor(0x3040, temp);
	...
	mDELAY(100);
	#endif
	cam_pr_debug("[AR0543] <<  preview setting\n");
...
//	cam_pr_debug("E! currefps:%d\n", currefps);
	...
	cam_pr_debug("[AR0543] >>  capture setting\n");
...
//	cam_pr_debug("E! currefps:%d\n", currefps);
	...
	cam_pr_debug("[AR0543] <<  normal video setting\n");
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id)
{
	...
	cam_pr_debug("[AR0543] >> get imgsensor id\n");
		...
		do {
			*sensor_id = return_sensor_id();
			if (*sensor_id == imgsensor_info.sensor_id) {
				cam_pr_debug("AR0543 i2c write id: 0x%x, sensor id: 0x%x\n",
					imgsensor.i2c_write_id, *sensor_id);
				return ERROR_NONE;
			}
			cam_pr_debug("AR0543 Read sensor id fail, id: 0x%x\n",
					*sensor_id);
			retry--;
		} while (retry > 0);
		i++;
...
//	cam_pr_debug("E\n");
	...
	cam_pr_debug("[AR0543] >>  normal video\n");
...
//   cam_pr_debug("E\n");
	cam_pr_debug("[AR0543] >>  slim video\n");
...
//	cam_pr_debug("E\n");
	...
	cam_pr_debug("[AR0543] >>  get resolution\n");
			...
			MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	...
	cam_pr_debug("[AR0543] >>  get info , scenario_id = %d\n", scenario_id);
		...
		MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	...
	cam_pr_debug("[AR0543] >> _control  , scenario_id = %d\n", scenario_id);
...
static kal_uint32 set_video_mode(UINT16 framerate)
{
	...
	cam_pr_debug("[AR0543] >>  set video mode  framerate = %d\n",
		framerate);
	...
	cam_pr_debug("[AR0543] <<  set video mode\n");
...
static kal_uint32 set_auto_flicker_mode(kal_bool enable, UINT16 framerate)
{
	...
	cam_pr_debug("[AR0543] >>, enable = %d, framerate = %d\n",
		enable, framerate);
	...
	// register, and Test Pattern do not need
	// auto flicker function, so return
...
static kal_uint32 set_max_framerate_by_scenario(enum
...
//cam_pr_debug("scenario_id = %d, framerate = %d\n", scenario_id, framerate);
	...
	cam_pr_debug("[AR0543] >>, scenario_id = %d, framerate = %d\n",
		scenario_id, framerate);
...
//	cam_pr_debug("scenario_id = %d\n", scenario_id);
	...
	cam_pr_debug("[AR0543] >> , scenario_id = %d\n", scenario_id);
...
//   cam_pr_debug("enable: %d\n", enable);
	...
	if (enable) {
		...
		// FINE_DIG_CORRECTION_CONTROL
		//write_cmos_sensor(0x3180, 0xB0FF);
		...
		cam_pr_debug("[AR0543] <<  set test pattern mode\n");
			...
			UINT8 *feature_para, UINT32 *feature_para_len)
{
	...
	cam_pr_debug("[AR0543] >> feature control,feature_id = %d\n",
		feature_id);
...
//		   cam_pr_debug("ihdr enable :%d\n", *feature_data_16);
	...
	cam_pr_debug("[AR0543] << feature control\n");
