...
/* Move these defines to kd_camera_hw.h,
 * so they can be project-dependent //Jessy @2014/06/04
 */
...
#define cam_pr_debug_NONE(fmt, arg...)     do {} while (0)
#define cam_pr_debug_FUNC(fmt, arg...) \
...
#define cam_pr_debug cam_pr_debug_FUNC
#define cam_pr_info(fmt, args...)		pr_info(PFX  fmt, ##args)
#define PK_XLOG_INFO(fmt, arg...) \
	pr_debug(fmt, ##arg)

#ifdef KD_FUNC_ENTRY_EXIT_DEBUG
#define KD_MULTI_FUNCTION_ENTRY()    cam_pr_info("[%s]: E\n", __func__)
#define KD_MULTI_FUNCTION_EXIT()     cam_pr_info("[%s]: X\n", __func__)
#else
...
#else
#define cam_pr_debug(a, ...)
#define cam_pr_info(fmt, args...)
#define PK_XLOG_INFO(fmt, arg...)
...
#define cam_pr_info_IF(cond, fmt, arg...) \
	do { \
		if (cond) \
...
static struct MULTI_SENSOR_FUNCTION_STRUCT2 *g_pSensorFunc =
	&kd_MultiSensorFunc;
BOOL g_bEnableDriver[KDIMGSENSOR_MAX_INVOKE_DRIVERS] = {
	FALSE, FALSE };
...
/*Not used now */
MSDK_SENSOR_RESOLUTION_INFO_STRUCT gSensorResolution[2];

MSDK_SENSOR_INFO_STRUCT ginfo_modified[2];

/* if not 0, then it will assign to sensor id value */
MUINT32 gSensorID;
 ...
 */
UINT32 kdGetSensorInitFuncList(struct ACDK_KD_SENSOR_INIT_FUNCTION_STRUCT
	**ppSensorList)
{
	if (ppSensorList == NULL) {
		cam_pr_info("[%s]ERROR: NULL ppSensorList\n", __func__);
		return 1;
	}
...
int iMultiReadReg(u16 a_u2Addr, u8 *a_puBuff, u16 i2cId, u8 number)
{
	...
	if (gI2CBusNum == SUPPORT_I2C_BUS_NUM1) {
		...
		if (i4RetValue != 2) {
			cam_pr_info("[CAMERA SENSOR] I2C send failed addr = 0x%x, data = 0x%x !!\n",
			       a_u2Addr, *a_puBuff);
			return -1;
		}
		/*  */
		...
		if (i4RetValue != 1) {
			cam_pr_info("[CAMERA SENSOR] I2C read failed!!\n");
			return -1;
		}
	} else {
		spin_lock(&kdsensor_drv_lock);
	...
	u8 *a_pRecvData, u16 a_sizeRecvData, u16 i2cId)
{
	...
	if (gI2CBusNum == SUPPORT_I2C_BUS_NUM1) {
		...
		if (i4RetValue != a_sizeSendData) {
			cam_pr_info("[CAMERA SENSOR] I2C send failed!!, Addr = 0x%x\n",
				a_pSendData[0]);
			return -1;
		}
...
/* cam_pr_debug("Addr : 0x%x,Val : 0x%x\n",a_u2Addr,a_u4Data); */
	...
	if (a_u4Bytes > 2) {
		cam_pr_info("[CAMERA SENSOR] exceed 2 bytes\n");
		return -1;
	}

	if (a_u4Data >> (a_u4Bytes << 3))
		cam_pr_debug("[CAMERA SENSOR] warning!! some data is not sent!!\n");
...
int kdSetI2CBusNum(u32 i2cBusNum)
{
	...
	if ((i2cBusNum != SUPPORT_I2C_BUS_NUM2) &&
		...
		cam_pr_info("[%s] i2c bus number is not correct(%d)\n",
			__func__, i2cBusNum);
		return -1;
	}
	spin_lock(&kdsensor_drv_lock);
...
int kdReleaseI2CTriggerLock(void)
{
	...
	/* if (ret < 0 ) { */
	/* cam_pr_debug("[error]wait i2c fail\n"); */
	/* } */
	...
	u16 i2cId, u16 transfer_length)
{
	...
	if (gI2CBusNum == SUPPORT_I2C_BUS_NUM1) {
		if (bytes > MAX_CMD_LEN) {
			cam_pr_debug("[%s] exceed the max write length\n",
				__func__);
			return 1;
		}
		...
		if (buf == NULL) {
			cam_pr_debug("[iBurstWriteReg] Not enough memory\n");
			return -1;
		}
		memset(buf, 0, bytes);
		...
		do {
			...
			if ((ret & 0xffff) != transfer_length)
				cam_pr_info("Error sent I2C ret = %d\n", ret);

		} while (((ret & 0xffff) != transfer_length) && (retry > 0));

		dma_free_coherent(&(camerahw_platform_device.dev),
			bytes, buf, phyAddr);
		spin_lock(&kdsensor_drv_lock);
...
int iMultiWriteReg(u8 *pData, u16 lens, u16 i2cId)
{
	...
	if (ret != lens)
		cam_pr_debug("Error sent I2C ret = %d\n", ret);
...
int iWriteRegI2C(u8 *a_pSendData, u16 a_sizeSendData, u16 i2cId)
{
	...
	do {
		...
		if (i4RetValue != a_sizeSendData) {
			cam_pr_debug("[CAMERA SENSOR] I2C send failed!!, Addr = 0x%x, Data = 0x%x\n",
...
static void debug_imgsensor(UINT8 sensor_index,
	MUINT8 *feature_para, void *extra)
{
	MSDK_SENSOR_DBG_IMGSENSOR_INFO_STRUCT *debug_info =
	    (MSDK_SENSOR_DBG_IMGSENSOR_INFO_STRUCT *) feature_para;
	struct ACDK_SENSOR_INFO_STRUCT *sensor_info = &ginfo[0];
	...
	if (strcmp1(debug_info->debugStruct,
		(MUINT8 *) "sensor_output_dataformat") == 0) {
		cam_pr_info("enter sensor_output_dataformat\n");
	...
	} else if (strcmp1(debug_info->debugStruct,
		(MUINT8 *) "sensor_id") == 0) {
		if (debug_info->isGet == 1)
		...
		if (strcmp1(debug_info->debugSubstruct,
			...
			if (debug_info->isGet == 1)
				debug_info->value =
					ginfo[sensor_index].SensorGrabStartY;
			else
				ginfo[sensor_index].SensorGrabStartY =
					debug_info->value;

		} else if (strcmp1(debug_info->debugSubstruct,
			(MUINT8 *) "grabwindow_width") == 0) {
			if (debug_info->isGet == 1)
				debug_info->value =
				ginfo[sensor_index].SensorPreviewResolutionX;
			else {
				ginfo[sensor_index].SensorPreviewResolutionX =
					debug_info->value;
			...
			(MUINT8 *) "grabwindow_height") == 0) {
			if (debug_info->isGet == 1)
				debug_info->value =
				ginfo[sensor_index].SensorPreviewResolutionY;
			else {
				ginfo[sensor_index].SensorPreviewResolutionY =
					debug_info->value;
			...
			(MUINT8 *) "max_framerate") == 0) {
		}
	} else {
		cam_pr_info("unknown debug\n");
	}
}
		...
		if (g_bEnableDriver[i] && g_pInvokeSensorFunc[i]) {
			if (0 != (g_CurrentSensorIdx & g_invokeSocketIdx[i])) {
				...
				if (ret != ERROR_NONE) {
					...
					(enum CAMERA_DUAL_CAMERA_SENSOR_ENUM)
					g_invokeSocketIdx[i],
					(char *)g_invokeSensorNameStr[i],
					false,
					CAMERA_HW_DRVNAME1);
#endif
					cam_pr_info("SensorOpen");
					return ret;
				}
				/* set i2c slave ID */
	...
	for (i = KDIMGSENSOR_INVOKE_DRIVER_0;
		...
		if (g_bEnableDriver[i] && g_pInvokeSensorFunc[i]) {
			if (g_invokeSocketIdx[i] == DUAL_CAMERA_MAIN_SENSOR) {
				ret = g_pInvokeSensorFunc[i]->SensorGetInfo(
...
MUINT32 kd_MultiSensorGetResolution(MSDK_SENSOR_RESOLUTION_INFO_STRUCT
	*pSensorResolution[2])
{
	MUINT32 ret = ERROR_NONE;
	u32 i = 0;
	...
	for (i = KDIMGSENSOR_INVOKE_DRIVER_0;
		...
		if (g_bEnableDriver[i] && g_pInvokeSensorFunc[i]) {
			if (g_invokeSocketIdx[i] == DUAL_CAMERA_MAIN_SENSOR) {
				ret =
				    g_pInvokeSensorFunc[i]->SensorGetResolution
				    (pSensorResolution[0]);
			} else if ((g_invokeSocketIdx[i] ==
					DUAL_CAMERA_MAIN_2_SENSOR)
				   || (g_invokeSocketIdx[i] ==
	...
	for (i = KDIMGSENSOR_INVOKE_DRIVER_0;
		...
		if (g_bEnableDriver[i] && g_pInvokeSensorFunc[i]) {
			...
			if (InvokeCamera == g_invokeSocketIdx[i]) {
				...
				/*  */
				ret =
				    g_pInvokeSensorFunc[i]->SensorFeatureControl
						(FeatureId,
						pFeaturePara,
						pFeatureParaLen);
				if (ret != ERROR_NONE) {
		      ...
		      MSDK_SENSOR_CONFIG_STRUCT *pSensorConfigData)
{
	...
	for (i = KDIMGSENSOR_INVOKE_DRIVER_0;
		...
		if (g_bEnableDriver[i] && g_pInvokeSensorFunc[i]) {
			if (InvokeCamera == g_invokeSocketIdx[i]) {
				...
				ret =
				    g_pInvokeSensorFunc[i]->SensorControl
						(ScenarioId, pImageWindow,
						pSensorConfigData);
				if (ret != ERROR_NONE) {
					cam_pr_info("ERR:SensorControl(), i =%d\n",
						i);
					return ret;
				}
			}
	...
	/* } */
	/* else { */
	/* drive 2 or more sensor simultaneously */
	if (g_bEnableDriver[1]) {
		MUINT8 frameSync = 0;
		MUINT32 frameSyncSize = 0;

		kd_MultiSensorFeatureControl
			(g_invokeSocketIdx[1], SENSOR_FEATURE_SUSPEND,
			&frameSync, &frameSyncSize);
		mDELAY(10);
		kd_MultiSensorFeatureControl
			(g_invokeSocketIdx[1], SENSOR_FEATURE_RESUME,
			&frameSync, &frameSyncSize);
	}
	/* } */
...
#ifndef CONFIG_FPGA_EARLY_PORTING
				/* Change the close power flow to close power
				 * in this function & directly call
				 * kdCISModulePowerOn
				 * to close the specific sensor
				 *
				 * The original flow will close all opened
				 * sensors at once
				 */
				kdCISModulePowerOn(
				(enum CAMERA_DUAL_CAMERA_SENSOR_ENUM)
...
int kdSetDriver(unsigned int *pDrvIndex)
{
	...
	if (kdGetSensorInitFuncList(&pSensorList) != 0) {
		cam_pr_info("ERROR:kdGetSensorInitFuncList()\n");
		return -EIO;
	}
	...
	for (i = KDIMGSENSOR_INVOKE_DRIVER_0;
		...
		cam_pr_info("[%s]g_invokeSocketIdx[%d] = %d\n",
			__func__, i, g_invokeSocketIdx[i]);
		cam_pr_info("[%s]drvIdx[%d] = %d\n",
			__func__, i, drvIdx[i]);
		/*  */
		if (drvIdx[i] < MAX_NUM_OF_SUPPORT_SENSOR) {
			...
			if (g_pInvokeSensorFunc[i] == NULL) {
				cam_pr_info("ERROR:NULL g_pSensorFunc[%d]\n",
					i);
				return -EIO;
			}
			/*  */
...
int kdSensorSyncFunctionPtr(void)
{
	...
	if (g_pSensorFunc == NULL) {
		cam_pr_info("ERROR:NULL g_pSensorFunc\n");
	...
	*(pRAWGain + 3) = 0x00;
	...
	if (g_NewSensorExpGain.uISPGainDelayFrame == 0) {
		...
		*(pRAWGain + 3) = g_NewSensorExpGain.u2ISPNewBGain;
		/* cam_pr_debug("[Sensor] ISP Gain:%d",
...
int kdSetExpGain(enum CAMERA_DUAL_CAMERA_SENSOR_ENUM InvokeCamera)
{
	...
	cam_pr_debug("[kd_sensorlist]enter %s\n", __func__);
	if (g_pSensorFunc == NULL) {
		cam_pr_info("ERROR:NULL g_pSensorFunc\n");
	...
	cam_pr_debug("[kd_sensorlist]before wake_up_interruptible\n");
	wake_up_interruptible(&kd_sensor_wait_queue);
	cam_pr_debug("[kd_sensorlist]after wake_up_interruptible\n");
...
int kdSensorSetExpGainWaitDone(int *ptime)
{
	...
	cam_pr_debug("[kd_sensorlist]enter %s: time: %d\n",
		__func__, *ptime);
	timeout = wait_event_interruptible_timeout(kd_sensor_wait_queue,
		(setExpGainDoneFlag & 1), ms_to_jiffies(*ptime));

	cam_pr_debug("[kd_sensorlist]after wait_event_interruptible_timeout\n");
	if (timeout == 0) {
		cam_pr_info("[kd_sensorlist] %s: timeout=%d\n",
		__func__, *ptime);
...
static inline int adopt_CAMERA_HW_Open(void)
{
	...
	if (g_pSensorFunc) {
		...
		if (err != ERROR_NONE) {
			...
			cam_pr_info("ERROR:SensorOpen(), turn off power\n");
		}
	} else {
		cam_pr_debug(" ERROR:NULL g_pSensorFunc\n");
	}
...
static inline int adopt_CAMERA_HW_CheckIsAlive(void)
{
	...
	/* power on sensor */
	kdModulePowerOn(
		(enum CAMERA_DUAL_CAMERA_SENSOR_ENUM *) g_invokeSocketIdx,
		g_invokeSensorNameStr,
		true, CAMERA_HW_DRVNAME1);
	/* wait for power stable */
	mDELAY(10);
	cam_pr_info("kdModulePowerOn\n");
		...
		for (i = KDIMGSENSOR_INVOKE_DRIVER_0;
			...
			if (g_invokeSocketIdx[i] != DUAL_CAMERA_NONE_SENSOR) {
				...
				if (sensorID == 0) {
					...
					cam_pr_info(" Not implement!!, use old open func to check\n");
					err = ERROR_SENSOR_CONNECT_FAIL;
				/* fail to open the sensor */
				} else if (sensorID == 0xFFFFFFFF) {
					cam_pr_info("CheckIsAlive No Sensor Found\n");
					err = ERROR_SENSOR_CONNECT_FAIL;
				} else {
					cam_pr_info("CheckIsAlive Sensor found ID = 0x%x\n",
						sensorID);
...
static inline int adopt_CAMERA_HW_GetResolution(void *pBuf)
{
	...
	struct ACDK_SENSOR_RESOLUTION_INFO_STRUCT *pRes[2] = {
		NULL, NULL };
	KD_MULTI_FUNCTION_ENTRY();

	PK_XLOG_INFO("[CAMERA_HW] %s, pBuf: %p\n", __func__, pBuf);

	pRes[0] = (struct ACDK_SENSOR_RESOLUTION_INFO_STRUCT *)
		kmalloc(sizeof(MSDK_SENSOR_RESOLUTION_INFO_STRUCT), GFP_KERNEL);
	if (pRes[0] == NULL) {
		cam_pr_info(" ioctl allocate mem failed\n");
		return -ENOMEM;
	}
	...
	if (g_pSensorFunc) {
		...
		if (copy_to_user(
			(void __user *)(pBufResolution->pResolution[0]),
			(void *)pRes[0],
			sizeof(MSDK_SENSOR_RESOLUTION_INFO_STRUCT))) {
			cam_pr_info("copy to user failed\n");
		}
		if (copy_to_user(
			(void __user *)(pBufResolution->pResolution[1]),
			(void *)pRes[1],
			sizeof(MSDK_SENSOR_RESOLUTION_INFO_STRUCT))) {
			cam_pr_info("copy to user failed\n");
		}
	} else {
		cam_pr_debug("[CAMERA_HW]ERROR:NULL g_pSensorFunc\n");
	}
	if (pRes[0] != NULL)
		kfree(pRes[0]);
	if (pRes[1] != NULL)
...
static inline int adopt_CAMERA_HW_GetInfo(void *pBuf)
{
	...
	if (pSensorGetInfo == NULL) {
		cam_pr_debug("[CAMERA_HW] NULL arg.\n");
		return -EFAULT;
	}
	...
	for (i = 0; i < 2; i++) {
		...
		if (copy_to_user
			...
			(void *)pInfo[i],
			sizeof(MSDK_SENSOR_INFO_STRUCT))) {
			cam_pr_debug("[CAMERA_HW][info] ioctl copy to user failed\n");
			return -EFAULT;
		}
		...
		if (copy_to_user
			...
			(void *)pConfig[i],
			sizeof(MSDK_SENSOR_CONFIG_STRUCT))) {
			cam_pr_debug("[CAMERA_HW][config] ioctl copy to user failed\n");
			return -EFAULT;
		}
	}
...
static inline int adopt_CAMERA_HW_GetInfo2(void *pBuf)
{
	...
	cam_pr_debug("[CAMERA_HW][Resolution] 0x%p\n",
		pSensorGetInfo->pSensorResolution);
	cam_pr_info("Prev: SensorClockFreq: %d,IDNum: %d, SensorFormat:%d, is_get_info_from_sensor_called:%d",
	    SensorInfo.SensorClockFreq, IDNum,
	...
	SensorInfo.SensorMIPILaneNumber =
		pInfo[IDNum]->SensorMIPILaneNumber;
	SensorInfo.CaptureDelayFrame =
		pInfo[IDNum]->CaptureDelayFrame;
	SensorInfo.PreviewDelayFrame =
		pInfo[IDNum]->PreviewDelayFrame;
	SensorInfo.VideoDelayFrame =
		pInfo[IDNum]->VideoDelayFrame;
	SensorInfo.HighSpeedVideoDelayFrame =
		pInfo[IDNum]->HighSpeedVideoDelayFrame;
	SensorInfo.SlimVideoDelayFrame =
		pInfo[IDNum]->SlimVideoDelayFrame;
	SensorInfo.Custom1DelayFrame =
		pInfo[IDNum]->Custom1DelayFrame;
	SensorInfo.Custom2DelayFrame =
		pInfo[IDNum]->Custom2DelayFrame;
	SensorInfo.Custom3DelayFrame =
		pInfo[IDNum]->Custom3DelayFrame;
	SensorInfo.Custom4DelayFrame =
		pInfo[IDNum]->Custom4DelayFrame;
	SensorInfo.Custom5DelayFrame =
		pInfo[IDNum]->Custom5DelayFrame;
	SensorInfo.YUVAwbDelayFrame =
		pInfo[IDNum]->YUVAwbDelayFrame;
	SensorInfo.YUVEffectDelayFrame =
		pInfo[IDNum]->YUVEffectDelayFrame;
	SensorInfo.SensorGrabStartX_PRV =
		pInfo[IDNum]->SensorGrabStartX;
	SensorInfo.SensorGrabStartY_PRV =
		pInfo[IDNum]->SensorGrabStartY;
	SensorInfo.SensorGrabStartX_CAP =
		pInfo1[IDNum]->SensorGrabStartX;
	SensorInfo.SensorGrabStartY_CAP =
		pInfo1[IDNum]->SensorGrabStartY;
	SensorInfo.SensorGrabStartX_VD =
		pInfo2[IDNum]->SensorGrabStartX;
	SensorInfo.SensorGrabStartY_VD =
		pInfo2[IDNum]->SensorGrabStartY;
	SensorInfo.SensorGrabStartX_VD1 =
		pInfo3[IDNum]->SensorGrabStartX;
	SensorInfo.SensorGrabStartY_VD1 =
		pInfo3[IDNum]->SensorGrabStartY;
	SensorInfo.SensorGrabStartX_VD2 =
		pInfo4[IDNum]->SensorGrabStartX;
	SensorInfo.SensorGrabStartY_VD2 =
		pInfo4[IDNum]->SensorGrabStartY;
	SensorInfo.SensorDrivingCurrent =
		pInfo[IDNum]->SensorDrivingCurrent;
	SensorInfo.SensorMasterClockSwitch =
		pInfo[IDNum]->SensorMasterClockSwitch;
	SensorInfo.AEShutDelayFrame =
		pInfo[IDNum]->AEShutDelayFrame;
	SensorInfo.AESensorGainDelayFrame =
		pInfo[IDNum]->AESensorGainDelayFrame;
	SensorInfo.AEISPGainDelayFrame =
		pInfo[IDNum]->AEISPGainDelayFrame;
	...
	SensorInfo.SensorWidthSampling =
		pInfo[IDNum]->SensorWidthSampling;
	SensorInfo.SensorHightSampling =
		pInfo[IDNum]->SensorHightSampling;
	SensorInfo.SensorPacketECCOrder =
		pInfo[IDNum]->SensorPacketECCOrder;
	SensorInfo.MIPIsensorType =
		pInfo[IDNum]->MIPIsensorType;
	SensorInfo.IHDR_LE_FirstLine =
		pInfo[IDNum]->IHDR_LE_FirstLine;
	SensorInfo.IHDR_Support =
		pInfo[IDNum]->IHDR_Support;
	SensorInfo.SensorModeNum =
		pInfo[IDNum]->SensorModeNum;
	SensorInfo.SettleDelayMode =
		pInfo[IDNum]->SettleDelayMode;
	SensorInfo.PDAF_Support =
		pInfo[IDNum]->PDAF_Support;
	SensorInfo.IMGSENSOR_DPCM_TYPE_PRE =
		pInfo[IDNum]->DPCM_INFO;
	...
	SensorInfo.PerFrameCTL_Support =
		pInfo[IDNum]->PerFrameCTL_Support;
	/*SCAM number */
	SensorInfo.SCAM_DataNumber =
		pInfo[IDNum]->SCAM_DataNumber;
	SensorInfo.SCAM_DDR_En =
		pInfo[IDNum]->SCAM_DDR_En;
	SensorInfo.SCAM_CLK_INV =
		pInfo[IDNum]->SCAM_CLK_INV;

	cam_pr_info("SensorClockFreq: %d,IDNum: %d, ",
		SensorInfo.SensorClockFreq, IDNum);
	cam_pr_info("SensorFormat:%d,is_get_info_from_sensor_called:%d",
		SensorInfo.SensorOutputDataFormat,
		is_get_info_from_sensor_called);
	...
	if (pSensorGetInfo->SensorId == DUAL_CAMERA_MAIN_SENSOR) {
		/* Resolution */
		cam_pr_debug("[%s] Resolution\n", __func__);
		if (copy_to_user
		    ((void __user *)(pSensorGetInfo->pSensorResolution),
		     (void *)psensorResolution[0],
		     sizeof(MSDK_SENSOR_RESOLUTION_INFO_STRUCT))) {
			cam_pr_info("[CAMERA_HW][Resolution] ioctl copy to user failed, addr:%p\n",
			       pSensorGetInfo->pSensorResolution);
			return -EFAULT;
		}
	} else {
		...
		if (copy_to_user
		    ((void __user *)(pSensorGetInfo->pSensorResolution),
		     ...
		     sizeof(MSDK_SENSOR_RESOLUTION_INFO_STRUCT))) {
			cam_pr_info("[CAMERA_HW][Resolution][not RFC] ioctl copy to user failed, addr:%p\n",
			       pSensorGetInfo->pSensorResolution);
			return -EFAULT;
		}
	}
...
static inline int adopt_CAMERA_HW_Control(void *pBuf)
{
	...
	memset(&sensorConfigData, 0,
		sizeof(struct ACDK_SENSOR_CONFIG_STRUCT));

	if (pSensorCtrl == NULL) {
		cam_pr_debug("[CAMERA_HW] pSensorCtrl NULL arg.\n");
		return -EFAULT;
	}

	if (pSensorCtrl->pImageWindow == NULL ||
		pSensorCtrl->pSensorConfigData == NULL) {
		cam_pr_debug("[CAMERA_HW] pImageWindow or pSensorConfigData NULL arg.\n");
		return -EFAULT;
	}
	...
	if (copy_from_user
	    ((void *)&imageWindow, (void *)pSensorCtrl->pImageWindow,
	     sizeof(struct ACDK_SENSOR_EXPOSURE_WINDOW_STRUCT))) {
		cam_pr_debug("[CAMERA_HW][pFeatureData32] ioctl copy from user failed\n");
		return -EFAULT;
	}
	...
	/*  */
	if (copy_to_user
	    ((void __user *)pSensorCtrl->pImageWindow,
	    (void *)&imageWindow,
		sizeof(MSDK_SENSOR_EXPOSURE_WINDOW_STRUCT))) {
		cam_pr_debug("[CAMERA_HW][imageWindow] ioctl copy to user failed\n");
		return -EFAULT;
	}
...
static inline int adopt_CAMERA_HW_FeatureControl(void *pBuf)
{
	...
	cam_pr_info("feature id:%d\n", pFeatureCtrl->FeatureId);
	if (pFeatureCtrl == NULL) {
		cam_pr_info(" NULL arg.\n");
		return -EFAULT;
	}
	...
	if (copy_from_user
	    ...
	    (void *)pFeatureCtrl->pFeatureParaLen,
	     sizeof(unsigned int))) {
		cam_pr_info(" ioctl copy from user failed\n");
		return -EFAULT;
	}
	...
	case SENSOR_FEATURE_GET_SENSOR_PDAF_CAPACITY:
		...
		if (copy_from_user
		    ...
		    FeatureParaLen)) {
			kfree(pFeaturePara);
			cam_pr_debug("[CAMERA_HW][pFeaturePara] ioctl copy from user failed\n");
			return -EFAULT;
		}
		break;
	/* Update new sensor exposure time and gain to keep */
...
/*in case that some structure are passed from user sapce by ptr */
	...
	case SENSOR_FEATURE_GET_EV_AWB_REF:
		{
			...
			pAeAwbRef =
				kmalloc(sizeof(struct SENSOR_AE_AWB_REF_STRUCT),
				GFP_KERNEL);
			if (pAeAwbRef == NULL) {
				cam_pr_info(" ioctl allocate mem failed\n");
				return -ENOMEM;
			}
			memset(pAeAwbRef, 0x0,
			...
			*(pFeaturePara_64) = (uintptr_t)pAeAwbRef;
			...
			if (copy_to_user
			    ((void __user *)usr_ptr, (void *)pAeAwbRef,
			     sizeof(struct SENSOR_AE_AWB_REF_STRUCT))) {
				cam_pr_debug("[CAMERA_HW]ERROR: copy_to_user fail\n");
			}
			kfree(pAeAwbRef);
	...
	case SENSOR_FEATURE_SET_AE_WINDOW:
		{
			...
			*(pFeaturePara_64) = (uintptr_t)pApWindows;
			...
			if (copy_from_user((void *)pApWindows,
				...
				cam_pr_info("[CAMERA_HW]ERROR: copy from user fail\n");
			if (g_pSensorFunc) {
				ret =
				    g_pSensorFunc->SensorFeatureControl
						(pFeatureCtrl->InvokeCamera,
				...
				cam_pr_info("[CAMERA_HW]ERROR:NULL g_pSensorFunc\n");
			}
			kfree(pApWindows);
			*(pFeaturePara_64) = (uintptr_t)usr_ptr;
		}
	...
	case SENSOR_FEATURE_GET_SENSOR_PDAF_CAPACITY:
		...
		if (copy_to_user
		    ...
		    (void *)pFeaturePara,
		     FeatureParaLen)) {
			kfree(pFeaturePara);
			cam_pr_debug("[CAMERA_HW][pSensorRegData] ioctl copy to user failed\n");
			return -EFAULT;
		}
		break;
	default:
	...
	if (copy_to_user
	    ...
	    (void *)&FeatureParaLen,
	     sizeof(unsigned int))) {
		cam_pr_debug("[CAMERA_HW][pFeatureParaLen] ioctl copy to user failed\n");
		return -EFAULT;
	}
	return ret;
...
static inline int Check_ccf_clk(void)
{
	...
	if (IS_ERR(g_camclk_camtg_sel)) {
		cam_pr_info("g_camclk_camtg_sel invalid...\n");
		return 0;
	}

	if (IS_ERR(g_camclk_mipi_26m)) {
		cam_pr_info("g_camclk_mipi_26m invalid...\n");
		return 0;
	}

	if (IS_ERR(g_camclk_camtg)) {
		cam_pr_info("g_camclk_camtg invalid...\n");
		return 0;
	}

	if (IS_ERR(g_camclk_48m)) {
		cam_pr_info("g_camclk_48m invalid...\n");
		return 0;
	}

	if (IS_ERR(g_camclk_208m)) {
		cam_pr_info("g_camclk_208m invalid...\n");
		return 0;
	}
...
/* #ifndef CONFIG_ARM64 */
	...
	cam_pr_info("[CAMERA SENSOR] %s on=%d, freq= %d\n",
		__func__,
		pSensorCtrl->on,
		pSensorCtrl->freq);

	cam_pr_debug("========= MTKCAM_USING_CCF =======\n");
	if (!Check_ccf_clk()) {
		cam_pr_info("Check Camera CCF error!\n");
		return 0;
	}
	if (pSensorCtrl->on == 1) {
		is_mclk_enable = 1;
...
#define GPIO_CMPCLK_M_GPIO      (GPIO_MODE_00)
	...
	pr_debug("[CAMERA SENSOR] %s enable=%d, type=%d\n",
		__func__, pSensorgpio->GpioEnable,
		pSensorgpio->SensroInterfaceType);
...
{
	cam_pr_debug(" == MTKCAM_USING_PWRREG1 : %s (%d) ==",
		__func__, PinIdx);

#if defined(CONFIG_FPGA_EARLY_PORTING) || WORKAROUND_FOR_PMIC
	cam_pr_debug("Just bypass power\n");
	return 1;
#else
...
bool CAMERA_Regulator_PowerOnOFF(struct regulator *pwrreg, BOOL IsOn, int vol)
{
	...
	cam_pr_debug("IsOn:%d , vol:%d\n", IsOn, vol);

	if (IS_ERR(pwrreg)) {
		cam_pr_info("camera power regulator is null, ret:%ld",
			PTR_ERR(pwrreg));
		return FALSE;
	}
	...
	if (IsOn) {		/* Power ON */
		/*
		 *  Need to reset Highest Voltage,
		 * if the power is used by multi-user,
		 *  otherwise, it would be bounding
		 * at last time settings/Highest vol
		 */
		voltage_count = regulator_count_voltages(pwrreg);
		if (voltage_count <= 0) {
			cam_pr_info("Fails to count, voltage_count = %d",
				voltage_count);
			return FALSE;
		}
		...
		high_bound_voltage = regulator_list_voltage(pwrreg,
			voltage_count - 1);
		cam_pr_debug("high_bound_voltage = %d", high_bound_voltage);

		if (high_bound_voltage <= 0) {
			cam_pr_info("Fails to list, high_bound_voltage = %d",
				high_bound_voltage);
			return FALSE;
		}
		...
		if (ret != 0) {
			cam_pr_info("Fails to set vol = %d , ret = 0x%x",
				vol, ret);
			return FALSE;
		}
		volt = regulator_get_voltage(pwrreg);
		if (volt != vol)
			cam_pr_info("Regulator Need to Check: volt(%u) != vol(%d)",
				volt, vol);
		else
			cam_pr_info("Set vol:%d, get the same:%u\n", vol, volt);

		ret = regulator_enable(pwrreg);
		if (ret != 0) {
			cam_pr_info("Fails to enabled, ret = 0x%x", ret);
			return FALSE;
		}
	} else {		/* Power OFF */
		...
		if (ret != 0) {
			cam_pr_info("Fails to disable, ret = 0x%x", ret);
			return FALSE;
		}
		...
		/*
		 *  Need to reset Lowest/Highest Voltage,
		 *  if the power is used by multi-user,
		...
		cam_pr_debug("low_bound_voltage = %d\n", low_bound_voltage);

		if (low_bound_voltage <= 0) {
			cam_pr_info("Fails to list, low_bound_voltage = %d",
				low_bound_voltage);
			return FALSE;
		}
		ret = regulator_set_voltage(pwrreg,
			low_bound_voltage, high_bound_voltage);
		if (ret != 0) {
			cam_pr_info("Fails to set High(%d) ~ Low(%d) voltage",
...
bool _hwPowerOn(int PinIdx, enum KD_REGULATOR_TYPE_T PwrType, int Voltage)
{
	...
	cam_pr_debug("PinIndex=%d , PowerType = %d , Voltage = %d",
		PinIdx, PwrType, Voltage);

#if defined(CONFIG_FPGA_EARLY_PORTING) || WORKAROUND_FOR_PMIC
	return TRUE;
#endif
	if ((PinIdx != 0) && (PinIdx != 1)) {
		cam_pr_info("PinIndex=%d is invalid\n", PinIdx);
		return FALSE;
	}
	...
	case VCAMAF:
		...
		break;
	default:
		cam_pr_info("PwrType=%d is invalid\n", PwrType);
		pwr = NULL;
		break;
	}
	if (pwr != NULL)
...
bool _hwPowerDown(int PinIdx, enum KD_REGULATOR_TYPE_T PwrType)
{
	...
	cam_pr_debug("PinIndex=%d , PowerType = %d", PinIdx, PwrType);
	...
	case COMPAT_KDIMGSENSORIOC_X_GETINFO:
		{
			...
			cam_pr_debug("[CAMERA SENSOR] CAOMPAT_KDIMGSENSORIOC_X_GETINFO E\n");
			...
			if (err != 0)
				cam_pr_debug("[CAMERA SENSOR] compat_put_acdk_sensor_getinfo_struct failed\n");
	...
	case COMPAT_KDIMGSENSORIOC_X_FEATURECONCTROL:
		{
			...
			cam_pr_debug("[CAMERA SENSOR] CAOMPAT_KDIMGSENSORIOC_X_FEATURECONCTROL\n");
			data32 = compat_ptr(arg);
			data = compat_alloc_user_space(sizeof(*data));
			if (data == NULL)
				return -EFAULT;
			...
			if (err != 0)
				cam_pr_info("[CAMERA SENSOR] compat_put_acdk_sensor_getinfo_struct failed\n");
	...
	case COMPAT_KDIMGSENSORIOC_X_CONTROL:
		{
			...
			cam_pr_debug("[CAMERA SENSOR] COMPAT_KDIMGSENSORIOC_X_CONTROL\n");
			data32 = compat_ptr(arg);
			data = compat_alloc_user_space(sizeof(*data));
			if (data == NULL)
				return -EFAULT;
	...
	case COMPAT_KDIMGSENSORIOC_X_GETINFO2:
		{
			...
			cam_pr_debug("[CAMERA SENSOR] CAOMPAT_KDIMGSENSORIOC_X_GETINFO2\n");
	...
	case COMPAT_KDIMGSENSORIOC_X_GETRESOLUTION2:
		{
			...
			struct COMPAT_ACDK_SENSOR_PRESOLUTION_STRUCT __user
				*data32;
			struct ACDK_SENSOR_PRESOLUTION_STRUCT __user *data;
			int err;

			cam_pr_debug("[CAMERA SENSOR] KDIMGSENSORIOC_X_GETRESOLUTION\n");

			data32 = compat_ptr(arg);
			data = compat_alloc_user_space(sizeof(data));
			if (data == NULL)
				return -EFAULT;
			cam_pr_debug("[CAMERA SENSOR] compat_get_acdk_sensor_resolution_info_struct\n");

			err = compat_get_acdk_sensor_resolution_info_struct
				(data32, data);
			if (err)
				return err;
			cam_pr_debug("[CAMERA SENSOR] unlocked_ioctl\n");
			ret =
			    filp->f_op->unlocked_ioctl(filp,
					KDIMGSENSORIOC_X_GETRESOLUTION2,
					(unsigned long)data);

			err = compat_put_acdk_sensor_resolution_info_struct
				(data32, data);
			if (err != 0)
				cam_pr_info("[CAMERA SENSOR] compat_get_Acdk_sensor_resolution_info_struct failed\n");

			return ret;
		}
		/* Data in the following commands is not required
		 * to be converted to kernel 64-bit & user 32-bit
		 */
			    ...
			    unsigned int a_u4Command, unsigned long a_u4Param)
{
	...
	if (_IOC_DIR(a_u4Command) == _IOC_NONE) {
		...
		if (_IOC_WRITE & _IOC_DIR(a_u4Command)) {
			if (copy_from_user(pBuff, (void *)a_u4Param,
				...
				kfree(pBuff);
				cam_pr_debug("[CAMERA SENSOR] ioctl copy from user failed\n");
				i4RetValue = -EFAULT;
				goto CAMERA_HW_Ioctl_EXIT;
			}
		}
	...
	case KDIMGSENSORIOC_X_GET_ISP_CLK:
		/* cam_pr_debug("get_isp_clk=%d\n",get_isp_clk()); */
		/* *(unsigned int*)pBuff = get_isp_clk(); */
		break;

	default:
		cam_pr_debug("No such command\n");
		i4RetValue = -EPERM;
		break;
	...
	if (_IOC_READ & _IOC_DIR(a_u4Command)) {
		if (copy_to_user((void __user *)a_u4Param, pBuff,
			_IOC_SIZE(a_u4Command))) {
			kfree(pBuff);
			cam_pr_debug("[CAMERA SENSOR] ioctl copy to user failed\n");
			i4RetValue = -EFAULT;
			goto CAMERA_HW_Ioctl_EXIT;
		}
	}
 ...
 */
static int CAMERA_HW_Release(struct inode *a_pstInode,
	struct file *a_pstFile)
{
	atomic_dec(&g_CamDrvOpenCnt);
/* cam_pr_debug("[CAMERA_HW_Release] g_CamDrvOpenCnt %d\n",g_CamDrvOpenCnt); */
...
static inline int RegisterCAMERA_HWCharDrv(void)
{
	...
	if (alloc_chrdev_region(&g_CAMERA_HWdevno,
		0, 1, CAMERA_HW_DRVNAME1)) {
		cam_pr_debug("[CAMERA SENSOR] Allocate device no failed\n");

		return -EAGAIN;
	}
#else
	if (register_chrdev_region(g_CAMERA_HWdevno,
		1, CAMERA_HW_DRVNAME1)) {
		cam_pr_debug("[CAMERA SENSOR] Register device no failed\n");
	...
	if (g_pCAMERA_HW_CharDrv == NULL) {
		...
		cam_pr_debug("[CAMERA SENSOR] Allocate mem for kobject failed\n");
	...
	if (cdev_add(g_pCAMERA_HW_CharDrv, g_CAMERA_HWdevno, 1)) {
		cam_pr_debug("[mt6516_IDP] Attatch file operation failed\n");
	...
	if (IS_ERR(sensor_class)) {
		...
		cam_pr_debug("Unable to create class, err = %d\n", ret);
		return ret;
	}
	sensor_device =
	    device_create(sensor_class, NULL,
	    g_CAMERA_HWdevno, NULL, CAMERA_HW_DRVNAME1);
	...
	const struct i2c_device_id *id)
{
	...
	cam_pr_debug("[CAMERA_HW] Attach I2C\n");
	...
	if (i4RetValue) {
		cam_pr_info("[CAMERA_HW] register char device failed!\n");
		return i4RetValue;
	}
	...
	cam_pr_debug("[CAMERA_HW] Attached!!\n");
	return 0;
}
 ...
 * Q1 : Try release multiple times.
 */
static int CAMERA_HW_Release2(struct inode *a_pstInode,
	struct file *a_pstFile)
{
...
static inline int RegisterCAMERA_HWCharDrv2(void)
{
...
#if CAMERA_HW_DYNAMIC_ALLOCATE_DEVNO
	if (alloc_chrdev_region(&g_CAMERA_HWdevno2,
			0, 1, CAMERA_HW_DRVNAME2)) {
		cam_pr_debug("[CAMERA SENSOR] Allocate device no failed\n");

		return -EAGAIN;
	}
#else
	if (register_chrdev_region(g_CAMERA_HWdevno2,
		1, CAMERA_HW_DRVNAME2)) {
		cam_pr_debug("[CAMERA SENSOR] Register device no failed\n");
	...
	sensor_device =
	    device_create(sensor2_class, NULL,
			g_CAMERA_HWdevno2, NULL, CAMERA_HW_DRVNAME2);
	...
	const struct i2c_device_id *id)
{
	...
	cam_pr_debug("[CAMERA_HW] Attach I2C0\n");
...
static int CAMERA_HW_probe(struct platform_device *pdev)
{
	...
	if (ret < 0) {
#if defined(CONFIG_FPGA_EARLY_PORTING) || WORKAROUND_FOR_PMIC
		cam_pr_info("Workaround for Camera Probe because of PMIC error, just continue...\n");
		if (ret == (-EPROBE_DEFER))
			cam_pr_info("Reason: Camera HW driver is probed earlier than PMIC, it may probe continuous...\n");
#else
		if (ret == (-EPROBE_DEFER))
			cam_pr_info("Camera HW driver is probed earlier than PMIC, let's deferring probe\n");
		return ret;
#endif
	}
#endif
	...
	cam_pr_info("cam0_rst:%d, cam1_rst:%d\n", GPIO_CAM0_RST, GPIO_CAM1_RST);
	if (GPIO_CAM0_RST == GPIO_CAM1_RST) {
		cam_pr_info("cam0 rst == cam1 rst");
		CamUseSameRst = 1;
	}
	...
	if (!gpio_is_valid(GPIO_CAM0_RST)) {
		cam_pr_info("[Camera][ERROR] unvalid GPIO_CAM0_RST\n");
	} else {
		ret = gpio_request(GPIO_CAM0_RST, "cam0_rst");
		if (ret < 0)
			cam_pr_info("[Camera][ERROR] Unable to request GPIO_CAM0_RST\n");
	}

	if (!gpio_is_valid(GPIO_CAM1_RST)) {
		cam_pr_info("[Camera][ERROR] unvalid GPIO_CAM1_RST\n");
	} else {
		ret = gpio_request(GPIO_CAM1_RST, "cam1_rst");
		if (ret < 0) {
			cam_pr_info("[Camera][ERROR] Unable to request GPIO_CAM1_RST\n");
			if (CamUseSameRst == 1)
	...
	if (!gpio_is_valid(GPIO_CAM0_PDN)) {
		cam_pr_info("[Camera][ERROR] unvalid GPIO_CAM0_PDN\n");
	} else {
		ret = gpio_request(GPIO_CAM0_PDN, "cam0_pdn");
		if (ret < 0)
			cam_pr_info("[Camera][ERROR] Unable to request GPIO_CAM0_PDN\n");
	}

	if (!gpio_is_valid(GPIO_CAM1_PDN)) {
		cam_pr_info("[Camera][ERROR] unvalid GPIO_CAM1_PDN\n");
	} else {
		ret = gpio_request(GPIO_CAM1_PDN, "cam1_pdn");
		if (ret < 0)
			cam_pr_info("[Camera][ERROR] Unable to request GPIO_CAM1_PDN\n");
	}
#endif
...
static int CAMERA_HW_probe2(struct platform_device *pdev)
{
	...
	if (ret < 0) {
		...
		cam_pr_info("Workaround for Camera Probe because of PMIC error, just continue...\n");
		if (ret == (-EPROBE_DEFER))
			cam_pr_info("Reason: Camera HW driver is probed earlier than PMIC\n");
#else
		if (ret == (-EPROBE_DEFER))
			cam_pr_info("Camera HW driver is probed earlier than PMIC, let's deferring probe\n");
		return ret;
	...
	if (sscanf(regBuf, "%x %x",
		...
		if (g_pSensorFunc != NULL) {
			...
			g_pSensorFunc->SensorFeatureControl
				...
				(MUINT8 *) &sensorReg, (MUINT32 *)
				sizeof(MSDK_SENSOR_REG_INFO_STRUCT));
			cam_pr_debug("write addr = 0x%08x, data = 0x%08x\n",
		...
		if (g_pSensorFunc != NULL) {
			g_pSensorFunc->SensorFeatureControl
				...
				(MUINT8 *) &sensorReg, (MUINT32 *)
				sizeof(MSDK_SENSOR_REG_INFO_STRUCT));
			cam_pr_debug("read addr = 0x%08x, data = 0x%08x\n",
				sensorReg.RegAddr, sensorReg.RegData);
		}
	}

	if (sscanf(regBuf, "%s %s %d %x",
		debugSensor.debugStruct, debugSensor.debugSubstruct,
		...
		if (g_pSensorFunc != NULL) {
			...
			cam_pr_debug("debug imgsensor = 0x%x, data = 0x%x\n",
				debugSensor.isGet, debugSensor.value);
		}
	} else if (sscanf(regBuf, "%x %31s %31s %x %x",
			&debugSensor.sensor_index, debugSensor.debugStruct,
			debugSensor.debugSubstruct, &(debugSensor.isGet),
			&(debugSensor.value)) == 5) {
		/*
		 *   if (g_pSensorFunc != NULL) {
		 *   g_pSensorFunc->SensorFeatureControl
		 ...
		 */
		debug_imgsensor(debugSensor.sensor_index,
			(MUINT8 *) &debugSensor, NULL);
		cam_pr_info("debug Main Cam: sensor_index:%d, debug1:%s, debug2:%s,  isGet = 0x%x, data = 0x%x\n",
		     debugSensor.sensor_index, debugSensor.debugStruct,
		     debugSensor.debugSubstruct,
		     debugSensor.isGet, debugSensor.value);
	} else {
		cam_pr_info("Unknown debug!Please Check!");
	}
...
static ssize_t CAMERA_HW_Reg_Debug2(struct file *file,
	const char *buffer, size_t count, loff_t *data)
{
	char regBuf[64] = { '\0' };
	u32 u4CopyBufSize =
	...
	if (sscanf(regBuf, "%x %x", &sensorReg.RegAddr,
		&sensorReg.RegData) == 2) {
		if (g_pSensorFunc != NULL) {
			g_pSensorFunc->SensorFeatureControl
				(DUAL_CAMERA_MAIN_2_SENSOR,
	...
	if (sscanf(regBuf, "%x %31s %31s %x %x",
		&debugSensor.sensor_index, debugSensor.debugStruct,
		debugSensor.debugSubstruct, &(debugSensor.isGet),
		&(debugSensor.value)) == 5) {
		/*
		...
		debug_imgsensor(debugSensor.sensor_index,
			(MUINT8 *) &debugSensor, NULL);
		cam_pr_info("debug Main2 Cam: sensor_index:%d, debug1:%s, debug2:%s,  isGet = 0x%x, data = 0x%x\n",
			debugSensor.sensor_index, debugSensor.debugStruct,
			debugSensor.debugSubstruct,
			debugSensor.isGet, debugSensor.value);
	} else {
...
static ssize_t CAMERA_HW_Reg_Debug3(struct file *file,
	const char *buffer, size_t count, loff_t *data)
{
	char regBuf[64] = { '\0' };
	u32 u4CopyBufSize =
	...
	if (sscanf(regBuf, "%x %31s %31s %x %x",
		...
		debug_imgsensor(debugSensor.sensor_index,
			(MUINT8 *) &debugSensor, NULL);
		cam_pr_info("debug Sub Cam: sensor_index:%d, debug1:%s, debug2:%s,  isGet = 0x%x, data = 0x%x\n",
			debugSensor.sensor_index, debugSensor.debugStruct,
			debugSensor.debugSubstruct,
			debugSensor.isGet, debugSensor.value);
	} else {
...
#endif
	cam_pr_debug("[%s] start\n", __func__);

	if (platform_driver_register(&g_stCAMERA_HW_Driver)) {
		cam_pr_info("failed to register CAMERA_HW driver\n");
		return -ENODEV;
	}
...
/* FIX-ME: linux-3.10 procfs API changed */
	...
	if (prEntry) {
		...
		cam_pr_info("add /proc/driver/camsensor entry fail\n");
	}
	...
	if (prEntry) {
		...
		cam_pr_info("add /proc/driver/camsensor2 entry fail\n");
	}
	...
	if (prEntry) {
		...
		cam_pr_info("add /proc/driver/maincam_status entry fail\n");
	}
	...
	if (prEntry) {
		...
		cam_pr_info("add /proc/driver/subcam_status entry fail\n");
	}
	...
	if (prEntry) {
		...
		cam_pr_info("add /proc/driver/3dcam_status entry fail\n");
	}
