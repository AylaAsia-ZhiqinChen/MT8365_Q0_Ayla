...
static struct SENSOR_WINSIZE_INFO_STRUCT imgsensor_winsize_info[5] = {
	...
	{1600, 1200, 0, 0, 1600, 1200, 1600, 1200,
		0000, 0000, 1600, 1200, 0, 0, 1600, 1200},
	/* capture */
	{1600, 1200, 0, 0, 1600, 1200, 1600, 1200,
		0000, 0000, 1600, 1200, 0, 0, 1600, 1200},
	{1600, 1200, 0, 0, 1600, 1200, 1600, 1200,
...
static kal_uint16 read_cmos_sensor(kal_uint32 addr)
{
	...
	iReadRegI2C(pu_send_cmd, 1, (u8 *) &get_byte,
...
static void set_max_framerate(UINT16 framerate, kal_bool min_framelength_en)
{
	...
	/* unsigned long flags; */
	kal_uint32 min_frame_length = imgsensor.min_frame_length;
	...
	imgsensor.frame_length =
	    (frame_length > min_frame_length) ? frame_length : min_frame_length;
	imgsensor.dummy_line = imgsensor.frame_length - min_frame_length;
	...
	if (imgsensor.frame_length > imgsensor_info.max_frame_length) {
		...
		imgsensor.dummy_line =
			imgsensor.frame_length - min_frame_length;
	}
...
static void set_shutter(kal_uint16 shutter)
{
	...
	if (imgsensor.frame_length > imgsensor_info.max_frame_length)
		imgsensor.frame_length = imgsensor_info.max_frame_length;
	spin_unlock(&imgsensor_drv_lock);
	shutter = (shutter < min_shutter) ? min_shutter : shutter;
	shutter =
	    (shutter >
	...
	case MSDK_SCENARIO_ID_CAMERA_CAPTURE_JPEG:
		if (imgsensor.current_fps ==
			...
			if (imgsensor.current_fps !=
				...
				LOG_INF("Warn: current_fps %d is not support,",
				     framerate);
				LOG_INF(" so use cap's setting: %d fps!\n",
				     imgsensor_info.cap.max_framerate / 10);
...
UINT32 GC2355_RAW_SensorInit(struct SENSOR_FUNCTION_STRUCT **pfFunc)
{
