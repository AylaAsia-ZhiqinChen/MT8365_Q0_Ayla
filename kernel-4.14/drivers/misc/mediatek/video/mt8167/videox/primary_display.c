...
static struct disp_internal_buffer_info *decouple_buffer_info[
DISP_INTERNAL_BUFFER_COUNT];
...
void enqueue_buffer(struct display_primary_path_context *ctx,
		struct list_head *head, struct disp_internal_buffer_info *buf)
{
	if (ctx && head && buf)
...
struct disp_internal_buffer_info *dequeue_buffer(
	...
	if (ctx && head) {
		if (!list_empty(head)) {
			temp = list_entry(head->prev,
					struct disp_internal_buffer_info, list);
...
struct disp_internal_buffer_info *find_buffer_by_mva(
		struct display_primary_path_context *ctx,
...
{
	if (mode == DISP_SESSION_DIRECT_LINK_MODE ||
...
struct display_primary_path_context *primary_display_path_lock(
		const char *caller)
{
...
static void primary_suspend_release_present_fence(void)
{
	...
	if (pgc->session_id == 0) {
		DISPDBG("_get_sync_info err in present_fence_release thread\n");
		return;
	}
	...
	if (layer_info == NULL) {
		DISPERR("_get_sync_info err in present_fence_release thread\n");
		return;
	}
	_primary_path_lock(__func__);
...
int _disp_primary_path_set_vfp(int enter)
{
	...
	if (primary_display_is_video_mode()) {
		...
		ret = cmdqRecCreate(CMDQ_SCENARIO_PRIMARY_DISP,
				&cmdq_handle_vfp);
		if (ret != 0) {
			DISPCHECK("err2create primary cmdq handle2set vfp\n");
			return -1;
		}
		...
		unsigned int need_primary_lock)
{
	...
	if (disp_low_power_disable_ddp_clock == 1) {/* open */
		if (primary_display_is_video_mode() == 0) {
			static unsigned int low_power_disable_ddp_clock_cnt;

			DISPDBG("MM clock, lp_disable_ddp_clock enter %d.\n",
				low_power_disable_ddp_clock_cnt++);
			if (enter == 1)	{ /* only for command mode */
				if (isIdlePowerOff == 0) {
					...
					spin_lock_irqsave(&gLockTopClockOff,
							flags);
					_disp_primary_path_idle_clock_off(0);
					/* parameter represent level */
					isIdlePowerOff = 1;
					...
					DISPMSG("dump regs!clk_stat_check\n");
#ifdef CONFIG_MTK_CLKMGR
	...
	/* no need idle lock,
	 * cause primary lock will be used inside switch_mode
	 */
	...
	if (is_mmdvfs_supported() &&
			mmdvfs_get_mmdvfs_profile() == MMDVFS_PROFILE_D1_PLUS) {
		if (_primary_path_IsForcedHPM(0) == false) {
			DISPMSG("MMDVFS enter:%d\n", enter);
			if (enter)
				mmdvfs_set_step(MMDVFS_SCEN_DISP,
						MMDVFS_VOLTAGE_LOW);
...
/* used by aal/pwm to exit idle before config register
 * (for "screen idle top clock off" feature)
 */
...
static int _disp_primary_path_idle_detect_thread(void *data)
{
	...
	while (1) {
		...
		if (pgc->session_mode != DISP_SESSION_DIRECT_LINK_MODE) {
			DISPCHECK("[ddp_idle]primary is DC.skip ddp_idle\n");
			_primary_path_unlock(__func__);
			continue;
		}
		...
		if (((sched_clock() - last_primary_trigger_time) /
					1000) > idle_time * 1000) {
#if defined(CONFIG_MTK_GMO_RAM_OPTIMIZE)
			...
			if (primary_display_is_video_mode()) {
				...
				if (ret < 0) {
					DISPCHECK("ddp_idle]alloc dcbuf err\n");
...
static int _switch_mmsys_clk(int mmsys_clk_old, int mmsys_clk_new)
{
	...
	if (pgc->state == DISP_SLEPT) {
		DISP_REG_MASK(NULL, DISP_REG_CONFIG_C13, 0x07000000,
				0x07000000);	/* clear */
		DISP_REG_MASK(NULL, DISP_REG_CONFIG_C12, 0x06000000,
				0x07000000);	/* set syspll2_d2 */
		...
		case MMSYS_CLK_LOW:
			DISP_REG_MASK(NULL, DISP_REG_VENCPLL_CON1, 0x830E0000,
					0xFFFFFFFF);	/* update */
			break;
		...
		case MMSYS_CLK_HIGH:
			if (mmdvfs_get_mmdvfs_profile() ==
					MMDVFS_PROFILE_D2_P_PLUS)
				DISP_REG_MASK(NULL, DISP_REG_VENCPLL_CON1,
						0x82110000, 0xFFFFFFFF);
			else if (mmdvfs_get_mmdvfs_profile() ==
					MMDVFS_PROFILE_D2_M_PLUS)
				DISP_REG_MASK(NULL, DISP_REG_VENCPLL_CON1,
						0x820F0000, 0xFFFFFFFF);
			break;
		...
		DISP_REG_MASK(NULL, DISP_REG_CONFIG_C13, 0x07000000,
				0x07000000);	/* clear */
		DISP_REG_MASK(NULL, DISP_REG_CONFIG_C12, 0x01000000,
				0x07000000);	/* set vencpll_ck */
...
cpu_d:
	...
	cmdqRecWrite(handle, 0x10210048, 0x07000000,
			0x07000000);	/* clear */
	cmdqRecWrite(handle, 0x10210044, 0x06000000,
			0x07000000);	/* set syspll2_d2 */
	...
	switch (mmsys_clk_new) {
	case MMSYS_CLK_LOW:
		cmdqRecWrite(handle, 0x10209254, 0x830E0000,
				0xFFFFFFFF);	/* update */
		break;
	...
	case MMSYS_CLK_HIGH:
		if (mmdvfs_get_mmdvfs_profile() == MMDVFS_PROFILE_D2_P_PLUS)
			cmdqRecWrite(handle, 0x10209254, 0x82110000,
					0xFFFFFFFF);	/* update */
		else if (mmdvfs_get_mmdvfs_profile() ==
				MMDVFS_PROFILE_D2_M_PLUS) {
			cmdqRecWrite(handle, 0x10209254, 0x820F0000,
					0xFFFFFFFF);	/* update */
		break;
	...
	/* wait rdma0_sof: only used for video mode &
	 * trigger loop need wait and clear rdma0 sof
	 */
	cmdqRecWaitNoClear(handle, CMDQ_EVENT_DISP_RDMA0_SOF);

	cmdqRecWrite(handle, 0x10210048, 0x07000000,
			0x07000000);	/* clear */
	cmdqRecWrite(handle, 0x10210044, 0x01000000,
			0x07000000);	/* set vencpll_ck */
...
int _config_wdma_output(struct WDMA_CONFIG_STRUCT *wdma_config,
...
int _config_rdma_input_data(struct RDMA_CONFIG_STRUCT *rdma_config,
		disp_path_handle disp_handle, struct cmdqRecStruct *cmdq_handle)
{
	struct disp_ddp_path_config *pconfig =
		dpmgr_path_get_last_config(disp_handle);
...
static void _idlemgr_wait_for_wakeup(void)
{
	...
	ret = wait_event_interruptible(idlemgr_wq,
			atomic_read(&idlemgr_wakeup));
	if (ret < 0)
...
{
	mmprofile_log_ex(ddp_mmp_get_events()->idlemgr_enter_idle,
...
{
	mmprofile_log_ex(ddp_mmp_get_events()->idlemgr_leave_idle,
			MMPROFILE_FLAG_PULSE, 0, 0);

	if (!primary_display_is_video_mode()) {
		spm_enable_sodi(0);
		mmprofile_log_ex(ddp_mmp_get_events()->sodi_disable,
				MMPROFILE_FLAG_PULSE, 0, 0);
	}
}
...
static int _primary_path_idlemgr_monitor_thread(void *data)
{
	...
	while (1) {
		...
		if (((sched_clock() - idlemgr_last_kick_time) /
					1000) > 500000)
			primary_display_idlemgr_enter_idle();
...
static int _disp_primary_path_decouple_worker_thread(void *data)
{
	...
	while (1) {
		/* shuold wait FRAME_START here, but MUTEX1's sof always
		 * issued with MUTEX0's sof,
		...
		memset((void *)&input, 0,
				sizeof(struct primary_disp_input_config));
		...
		ret = _trigger_display_interface(FALSE,
...
static enum DISP_MODULE_ENUM _get_dst_module_by_lcm(
...
{
	/* this is not a perfect design, we can't decide path trigger
	 * (ovl/rdma/dsi..) separately with mutex enable
	 */
	/* but it's lucky because path trigger and mutex enable is the same
	 * w/o cmdq, and it's correct w/ CMDQ(Y+N).
	 */
	if (primary_display_cmdq_enabled()) {
		if (primary_display_is_video_mode())
			return 0;
...
int _init_vsync_fake_monitor(int fps)
{
	...
	hrtimer_start(&cmd_mode_update_timer,
			ns_to_ktime(16666666), HRTIMER_MODE_REL);
...
void disp_set_sodi(unsigned int enable, void *cmdq_handle)
{
	...
	if (gEnableSODIControl == 1) {
		if (cmdq_handle != NULL) {
			if (enable == 1)
				cmdqRecWrite(cmdq_handle, DISP_REG_SODI_PA,
						0, 1);
			else
				cmdqRecWrite(cmdq_handle, DISP_REG_SODI_PA,
						1, 1);
...
static void _cmdq_build_trigger_loop(void)
{
	...
	if (primary_display_is_video_mode()) {
		 ...
		 */
		/* todo: should let dpmanager
		 * to decide wait which mutex's eof.
		 */
		...
		/* for some module(like COLOR) to read hw register to
		 * GPR after frame done
		 */
		 ...
		 * so mutex enable/dsi start/CPU wait
		 ...
		 */
		/* do not polling dsi idle directly which will decrease
		 * CMDQ performance
		 */
		dpmgr_path_build_cmdq(pgc->dpmgr_handle,
				pgc->cmdq_handle_trigger,
		...
		/* ret = cmdqRecPoll(pgc->cmdq_handle_trigger, 0x140060A0,
		 * 1, 0x1);
		 */
	 ...
	 * dpmgr_path_build_cmdq works correctly
	 */
...
static void _cmdq_start_trigger_loop(void)
{
	...
	/* this should be called only once because trigger loop
	 * will nevet stop
	 */
...
{
	int ret = 0;
	int secure_path_on = primary_display_is_secure_path(
...
static KREE_SECUREMEM_HANDLE allocate_decouple_sec_buffer(
...
static KREE_SECUREMEM_HANDLE free_decouple_sec_buffer(
		KREE_SECUREMEM_HANDLE mem_handle)
{
	int ret;
...
static int _DL_switch_to_DC_fast(void)
{
	int ret = 0;
	int secure_path_on =
		primary_display_is_secure_path(DISP_SESSION_PRIMARY);
	...
	_update_ovl_from_wdma_info(last_primary_config.ovl_config,
			&wdma_config);
	...
	rdma_config.pitch = primary_display_get_width() *
		DP_COLOR_BITS_PER_PIXEL(rdma_config.inputFormat) >> 3;
	if (secure_path_on) {
	...
	if (DISP_INTERNAL_BUFFER_COUNT == 1) {
		...
		for (i = 0; i < DISP_INTERNAL_BUFFER_COUNT; i++) {
			...
			if (decouple_buffer_info[i] != NULL) {
				...
				dc_vAddr[i] =
					(unsigned long)decouple_buffer_info[
					i]->va;
				DISPMSG("primary alloc decouple buffer: 0x%x\n",
						pgc->dc_buf[i]);
			}
...
static void copy_lklogo_to_dc_buf(void)
{
	...
	for (line = 0; line < primary_display_get_height(); line++) {
		d = (char *)(dc_vAddr[0] + line * primary_display_get_width() *
				DP_COLOR_BITS_PER_PIXEL(eRGB888) / 8);
		s = (int *)(pgc->framebuffer_va + line *
				ALIGN_TO(primary_display_get_width(),
					MTK_FB_ALIGNMENT) *
				primary_display_get_dc_bpp() / 8);
...
static int _build_path_rdma_to_dsi(void)
{
...
#ifdef CONFIG_FOR_SOURCE_PQ
	pgc->dpmgr_handle =
	    dpmgr_create_path(DDP_SCENARIO_PRIMARY_RDMA0_DISP,
			    pgc->cmdq_handle_config);
#else
	pgc->dpmgr_handle =
	...
	decouple_rdma_config.idx = 0;
	decouple_rdma_config.inputFormat = eRGB888;
	decouple_rdma_config.pitch = primary_display_get_width() *
		DP_COLOR_BITS_PER_PIXEL(eRGB888) / 8;
...
static int _build_path_ovl_to_wdma(void)
{
	...
	/*
	 * FRAME_START will be used in decouple-mirror mode,
	 * for post-path fence release(rdma->dsi)
	 */
	...
	decouple_wdma_config.useSpecifiedAlpha = 1;
	decouple_wdma_config.alpha = 0xFF;
	decouple_wdma_config.dstPitch = primary_display_get_width() *
		DP_COLOR_BITS_PER_PIXEL(eRGB888) / 8;
		...
		unsigned int userdata)
{
	...
	/* TODO: _is_decouple_mode() shuold be protected by mutex!!!!!!!!
	 * when dynamic switch decouple/directlink
	 */
...
int _trigger_ovl_to_memory(disp_path_handle disp_handle,
		...
		unsigned int data, int blocking)
{
	...
	if (blocking)
		...
		cmdqRecFlushAsyncCallback(cmdq_handle,
				(CmdqAsyncFlushCB) callback, data);
		...
		fence_release_callback callback, unsigned int data)
{
	...
	/*rdma pitch only use bit[15..0], we use bit[31:30]
	 * to store secure information
	 */
	rdma_pitch_sec = mem_config.pitch | (mem_config.security << 30);
	cmdqRecBackupUpdateSlot(cmdq_handle, pgc->rdma_buff_info, 1,
			rdma_pitch_sec);
...
static atomic_t esd_ext_te_event = ATOMIC_INIT(0); /* Vdo Mode EXT TE Check */
...
int _esd_check_config_handle_vdo(void)
{
	...
	cmdqRecBackupRegisterToSlot(pgc->cmdq_handle_config_esd,
			pgc->event_status, 1, DISP_REG_CMDQ_TOKEN_VALUE);
#endif
...
#ifdef DISP_DUMP_EVENT_STATUS
	{
		...
		for (i = 0; i < 6; i++)
			cmdqBackupReadSlot(pgc->event_status, i,
					&event_status[i]);
...
int primary_display_esd_check(void)
{
	...
	if (pgc->state == DISP_SLEPT) {
		mmprofile_log_ex(ddp_mmp_get_events()->esd_check_t,
				MMPROFILE_FLAG_PULSE, 1, 0);
		DISPCHECK("[ESD]primary path is slept?skip esd check\n");
		_primary_path_unlock(__func__);
		/* goto done; */
	...
	if (primary_display_cmdq_enabled()) {
		...
		if (ret == 1) {/* cmdq fail */
			/* Need set esd check eof synctoken
			 * to let trigger loop go.
			 */
			if (_need_wait_esd_eof())
				cmdqCoreSetEvent(CMDQ_SYNC_TOKEN_ESD_EOF);
...
static int primary_display_esd_check_worker_kthread(void *data)
{
	...
	while (1) {
		...
		ret = wait_event_interruptible(esd_check_task_wq,
		...
		if (ret == 1) {
			DISPCHECK("[ESD]esd check err,will do esd recovery%d\n",
					ret);
			i = esd_try_cnt;
			while (i--) {
				...
				ret = primary_display_esd_check();
				if (ret == 0) {
					DISPCHECK("[ESD]esd recovery ok\n");
					break;
				}
				DISPCHECK(
				"[ESD]after esd recovery,esd check also err\n");
				if (i == 0) {
					DISPCHECK(
					"[ESD]after esd recovery %d times, esd check still fail,\n"
						  , esd_try_cnt);
...
int primary_display_esd_recovery(void)
{
	...
	if (pgc->state == DISP_SLEPT) {
		DISPCHECK("[ESD]esd recovery but primary path is slept??\n");
		goto done;
	}
	...
	DISPCHECK("[ESD]start dpmgr path[end]\n");
	if (dpmgr_path_is_busy(pgc->dpmgr_handle)) {
		DISPERR("[ESD]Fatal err,display path no trigger but busy\n");
		ret = -1;
		/* goto done; */
	}
...
void primary_display_esd_check_enable(int enable)
{
	...
	if (_is_enable_esd_check() != true) {
		DISPCHECK("[ESD]ESD en but ESD check mechanism is not en.\n");
		return;
	}
...
static int primary_display_vdo_pullclk_worker_kthread(void *data)
{
	...
	while (1) {
		...
		_primary_path_unlock(__func__);
#ifdef DISP_DUMP_EVENT_STATUS
		DISP_REG_SET_PA(pgc->cmdq_handle_config_esd,
				DISP_REG_CMDQ_TOKEN_ID,
				DISP_MUTEX0_STREAM_EOF_ID);
		cmdqRecBackupRegisterToSlot(pgc->cmdq_handle_config_esd,
				pgc->event_status, 1,
		...
		dpmgr_path_trigger(pgc->dpmgr_handle,
				pgc->cmdq_handle_config_esd, CMDQ_ENABLE);
		if (gDumpESDCMD == 1) {
			DISPMSG("esd dump before flush:\n");
			cmdqRecDumpCommand(pgc->cmdq_handle_config_esd);
		}
...
{
	DISPERR("%s: engineFlag=0x%016llx\n", __func__, engineFlag);
...
{
	if (pgc->plcm) {
		DISPMSG("LCM Res will be changed,original: %dx%d, now: %dx%d\n",
			pgc->plcm->params->width, pgc->plcm->params->height,
			width, height);
		/* align with 4 is the minimal check, to ensure
		 * we can boot up into kernel,
		...
		if (primary_display_is_video_mode()) {
			DISPERR("Video Mode can't support multiple res!\n");
			return -1;
		}
		...
		enum DISP_MODULE_ENUM module, unsigned int param)
{
	...
	/* In TEE, we have to protect WDMA registers,
	 * so we can't enable WDMA interrupt
	 */
...
static int _ovl2mem_fence_release_worker_thread(void *data)
{
	...
	while (1) {
		 ...
		 * because when CPU read wdma addr,
...
static int _ovl_ext_fence_release_callback(uint32_t userdata)
{
	...
	for (i = 0; i < PRIMARY_DISPLAY_SESSION_LAYER_COUNT; i++) {
		...
		if (i == primary_display_get_option("ASSERT_LAYER") &&
			...
			cmdqBackupReadSlot(pgc->cur_mem_config_fence, i,
					&fence_idx);
			cmdqBackupReadSlot(pgc->mem_subtractor_when_free, i,
					&subtractor);
			mtkfb_release_fence(ext_session_id, i, fence_idx);
		}
		mmprofile_log_ex(ddp_mmp_get_events()->
				primary_ovl_fence_release,
				MMPROFILE_FLAG_PULSE, i, fence_idx);
	}
#endif
	...
	if (userdata == DISP_SESSION_MEMORY) {
#ifndef MTK_FB_CMDQ_DISABLE
		layer = disp_sync_get_output_timeline_id();
		cmdqBackupReadSlot(pgc->cur_mem_config_fence, layer,
				&fence_idx);
		...
		if (data_config) {
			...
			wdma_layer.dstAddress =
				mtkfb_query_buf_va(ext_session_id, 4,
						fence_idx);
...
static int _ovl_fence_release_callback(uint32_t userdata)
{
	...
	mmprofile_log_ex(ddp_mmp_get_events()->session_release,
			MMPROFILE_FLAG_START, 0, userdata);
	...
	for (i = 0; i < PRIMARY_DISPLAY_SESSION_LAYER_COUNT; i++) {
		...
		mmprofile_log_ex(ddp_mmp_get_events()->
				primary_ovl_fence_release, MMPROFILE_FLAG_PULSE,
		...
		for (i = 0; i < 10; i++) {
			...
			if ((dsi_state[i] & 0x2) == 0x2) {
				...
				cmdqBackupReadSlot(pgc->rdma_state_info,
						...
						&rdma_state[i * 5 + 4]);
				DISPERR(
					"disp RDMA_STATE:%d, irq=0x%x, in/out=(%d,%d,%d,%d)\n"
						, i, rdma_state[i * 5 + 0],
						rdma_state[i * 5 + 1],
						rdma_state[i * 5 + 2],
						rdma_state[i * 5 + 3],
	...
	if (_is_decouple_mode(pgc->session_mode)
		...
		if (ret == 0) {
			...
			if (data_config) {
				struct WDMA_CONFIG_STRUCT wdma_layer =
					decouple_wdma_config;

				wdma_layer.dstAddress =
					get_decouple_va_addr(addr);
				dprec_mmp_dump_wdma_layer(&wdma_layer, 0);
			}
			/* cmdqRecDestroy(cmdq_handle); */
		} else {
	...
	for (;;) {
		...
		if (pgc->session_mode == DISP_SESSION_DECOUPLE_MIRROR_MODE
			...
			if (ret == 0) {
				...
				/*rdma pitch only use bit[15..0],
				 * we use bit[31:30] to store secure
				 * Information
				 */
				cmdqBackupReadSlot(pgc->rdma_buff_info, 1,
						&(rdma_pitch_sec));
...
static int _present_fence_release_worker_thread(void *data)
{
	...
	while (1) {
		...
		/* release present fence in vsync callback */
		{
			...
			_primary_path_lock(__func__);
			fence_increment = gPresentFenceIndex -
...
int primary_display_capture_framebuffer_wdma(void *data)
{
	...
	unsigned int pixel_byte = primary_display_get_dc_bpp() / 8;
...
int primary_display_init(char *lcm_name, unsigned int lcm_fps)
{
	...
	init_cmdq_slots(&(pgc->subtractor_when_free),
			DISP_SESSION_TIMELINE_COUNT, 0);
	init_cmdq_slots(&(pgc->cur_mem_config_fence),
			DISP_SESSION_TIMELINE_COUNT, 0);
	init_cmdq_slots(&(pgc->mem_subtractor_when_free),
			DISP_SESSION_TIMELINE_COUNT, 0);
...
/* #endif */
	...
	if ((dfo_query("LCM_FAKE_WIDTH", &lcm_fake_width) == 0)
		...
		if (lcm_fake_width && lcm_fake_height) {
			if (primary_display_change_lcm_resolution(
						lcm_fake_width,
						lcm_fake_height) !=
					DISP_STATUS_OK)
				DISPMSG(
	...
	/* use fake timer to generate vsync signal for cmd mode w/o LCM
	 * (originally using LCM TE Signal as VSYNC)
	 */
	...
	rdma_set_target_line(DISP_MODULE_RDMA0, primary_display_get_height() *
			1 / 2,
			     pgc->cmdq_handle_config);
	rdma_set_target_line(DISP_MODULE_RDMA0, primary_display_get_height() *
			1 / 2, NULL);
...
#ifdef MTK_FB_PULLCLK_ENABLE
	primary_display_pullclk_task =
		kthread_create(primary_display_vdo_pullclk_worker_kthread,
	...
	/* disable OVL TF in video mode, cause cmdq/sodi log is not ready,
	 * avoid too much TF issue
	 */
#ifdef CONFIG_MTK_M4U
	{
	...
	if (pgc->framebuffer_mva) {
		...
		if (PageHighMem(pfn_to_page(pa_start >> PAGE_SHIFT))) {
			...
			for (i = 0; i < npages; i++) {
				...
				/* free_highmem_page only for arm
				 * use free_reserved_area will cause nonzero
				 * mapcount page dumped
				 */
...
int primary_display_release_fence_fake(void)
{
	...
	for (i = 0; i < PRIMARY_DISPLAY_SESSION_LAYER_COUNT; i++) {
		if (i == primary_display_get_option("ASSERT_LAYER") &&
			...
			disp_sync_get_cached_layer_info(session_id, i,
					&layer_en,
					(unsigned long *)&addr,
					&fence_idx);
			if (fence_idx < 0) {
				if (fence_idx == -1) {
					DISPPR_DEBUG(
					"find fence idx for layer %d,addr 0x%08x fail,\n"
					, i, 0);
					DISPPR_DEBUG("unregistered addr%d\n",
							fence_idx);
				} else if (fence_idx == -2) {
					;
				} else {
...
int primary_display_wait_for_vsync(void *config)
{
	...
	/* kick idle manager here to ensure sodi is disabled when screen
	 * update begin(not 100% ensure)
	 */
...
int primary_suspend_release_ovl_fence(
		struct disp_session_input_config *session_input)
{
...
int primary_display_suspend(void)
{
	...
	if (dpmgr_path_is_busy(pgc->dpmgr_handle)) {
		...
		if (event_ret <= 0) {
			...
			mmprofile_log_ex(ddp_mmp_get_events(
				)->primary_suspend, MMPROFILE_FLAG_PULSE,
					3, 2);
			primary_display_diagnose();
			ret = -1;
		}
	...
	/*
	 *
	 *   Keep CMDQ engine on by trgger loop in WFD extension
	 *   case while suspend.
...
#if 0
	else if (is_mmdvfs_supported() && mmdvfs_get_mmdvfs_profile() ==
			MMDVFS_PROFILE_D1_PLUS) {
...
static void primary_display_check_clock(void)
{
	...
	if ((cg1 & 0xc) != 0x0 && (cg1 & 0x30) != 0x0)
		DISPERR("dpi/dsi clk is not enable,display will be abnormal\n");
}
...
int primary_display_resume(void)
{
	 ...
	 * powerkey to enable thsi feature.
	 */
...
#if 0
	if (is_mmdvfs_supported() && mmdvfs_get_mmdvfs_profile() ==
			MMDVFS_PROFILE_D1_PLUS &&
...
static int primary_display_remove_output(void *callback, unsigned int userdata)
{
	...
	if (ret == 0) {
		...
		ret = cmdqRecCreate(CMDQ_SCENARIO_DISP_MIRROR_MODE,
				&cmdq_wait_handle);
		if (ret == 0) {
			cmdqRecReset(cmdq_wait_handle);
...
int primary_display_merge_session_cmd(struct disp_session_config *config)
{
	...
	cached_session_input[config->type - 1] =
		captured_session_input[config->type - 1];
	cached_session_input[config->type - 1].session_id = config->session_id;

	if (output_type == DISP_OUTPUT_MEMORY || is_multipass_trigger)
		cached_session_output[config->type - 1] =
			captured_session_output[config->type - 1];
	...
	if (isAEEEnabled) {
		...
		/*
		 *
		 *  For multipass mode, the AEE layer can only be enabled
		 *  at final pass when AEE enabled.
		...
		if (config->type == DISP_SESSION_PRIMARY) {
			if (output_type == DISP_OUTPUT_MEMORY) {
				session_input->config[
					aee_layer].layer_enable = 0;
				DISPMSG("set the aee layer:%d en to 0\n",
					session_input->config[
					aee_layer].layer_id);
			} else {
				session_input->config[
					aee_layer].layer_enable = 1;
			}
		}
	}
...
static int primary_display_fps_update(void)
{
	...
	unsigned long long cur_trigger = sched_clock();
	...
	if (time > 1000000000) {
		pgc->avg_fps = trigger_cnt * 1000ULL;
		do_div(time, 1000000);
...
static int config_wdma_output(disp_path_handle disp_handle,
		struct cmdqRecStruct *cmdq_handle,
		struct disp_mem_output_config *output,
		int is_multipass);
...
int primary_display_trigger(int blocking, void *callback, unsigned int userdata)
{
	...
	_disp_primary_path_exit_idle(__func__, 0);
#endif
	dprec_logger_start(DPREC_LOGGER_PRIMARY_TRIGGER, pgc->session_mode,
			pgc->dc_type);
	...
	if (pgc->session_mode == DISP_SESSION_DIRECT_LINK_MODE) {
		...
		if (pgc->need_trigger_dcMirror_out == 0) {
			if (cached_session_output[
				DISP_SESSION_PRIMARY - 1].security ==
				DISP_SECURE_BUFFER)
				config_wdma_output(pgc->ovl2mem_path_handle,
						pgc->cmdq_handle_ovl1to2_config,
					&cached_session_output[
					DISP_SESSION_PRIMARY - 1], 0);
			DISPPR_DEBUG(
		...
		if (pgc->dc_type == DISP_OUTPUT_DECOUPLE) {
			...
			if (primary_display_is_secure_path(
						DISP_SESSION_PRIMARY)) {
				pgc->session_buf_id++;
				pgc->session_buf_id %=
					DISP_INTERNAL_BUFFER_COUNT;
				writing_mva = pgc->session_buf[
					pgc->session_buf_id];
				mem_config.security = DISP_SECURE_BUFFER;
				decouple_wdma_config.security =
					DISP_SECURE_BUFFER;
			} else {
				pgc->dc_buf_id++;
				pgc->dc_buf_id %= DISP_INTERNAL_BUFFER_COUNT;
			...
			mmprofile_log_ex(ddp_mmp_get_events(
					...
					writing_mva);
			/*
			 * At the last round of multipass, the session MODE
			 * would be configured as DECOUPLE MODE.
			 * Need to set data as
			 * DISP_SESSION_DECOUPLE_MULTIPASS_MODE
			 * to _ovl_fence_release_callback.
			 * Then the multipass output buffer fence would be
			 * release in _ovl_fence_release_callback.
			 */
...
int primary_display_memory_trigger(int blocking, void *callback,
		unsigned int userdata)
{
	int ret = 0;
	...
	if (pgc->session_mode == DISP_SESSION_DECOUPLE_MODE ||
		...
		_trigger_ovl_to_memory(pgc->ovl2mem_path_handle,
				pgc->cmdq_handle_ovl1to2_config,
				(fence_release_callback
				 )_ovl_ext_fence_release_callback,
...
static int primary_display_ovl2mem_callback(unsigned int userdata)
{
	...
	if (data_config) {
		...
		wdma_layer.dstAddress = mtkfb_query_buf_mva(session_id, 4,
				fence_idx);
		...
		unsigned int userdata)
{
	...
	if (_should_flush_cmdq_config_handle())
		cmdqRecFlushAsyncCallback(pgc->cmdq_handle_ovl1to2_config,
					  (CmdqAsyncFlushCB
					   )primary_display_ovl2mem_callback,
					  userdata);
		...
		unsigned int session_id)
{
	...
	if (DISP_SESSION_TYPE(session_id) == DISP_SESSION_PRIMARY) {
		...
		layer = disp_sync_get_output_timeline_id();
		cmdqRecBackupUpdateSlot(cmdq_handle, pgc->cur_mem_config_fence,
				layer, output->buff_idx);
		layer = disp_sync_get_output_interface_timeline_id();
		cmdqRecBackupUpdateSlot(cmdq_handle, pgc->cur_mem_config_fence,
				layer, output->interface_idx);
	}
...
static void update_debug_fps_meter(struct disp_ddp_path_config *data_config)
{
	...
	_debug_fps_meter(data_config->ovl_config[dst_id].addr,
			 ...
			 data_config->ovl_config[dst_id].src_pitch
			 , 0x00000000, dst_id,
			     ...
			     struct cmdqRecStruct *cmdq_handle)
{
	...
	if (DISP_SESSION_TYPE(session_input->session_id) ==
			DISP_SESSION_MEMORY) {
	...
	for (i = 0; i < session_input->config_layer_num; i++) {
		...
		if ((ovl_cfg->layer == 0) &&
			...
			update_frm_seq_info(ovl_cfg->addr,
					ovl_cfg->src_x * 4 + ovl_cfg->src_y *
					ovl_cfg->src_pitch,
					mtkfb_query_frm_seq_by_addr(
					pgc->session_id, 0, 0), FRM_CONFIG);
		...
		if (gCapturePriLayerEnable) {
			...
			cfg.addr = mtkfb_query_buf_va(primary_session_id,
					layer, cfg.buff_idx);
	...
	for (i = 0; i < session_input->config_layer_num; i++) {
		...
		if (cur_fence != -1 && cur_fence > last_fence)
			cmdqRecBackupUpdateSlot(cmdq_handle,
					*p_cur_config_fence, layer, cur_fence);
		...
		if (input_cfg->buffer_source == DISP_BUFFER_ALPHA ||
				...
				cur_fence == -1 ||
				DISP_SESSION_TYPE(session_input->session_id) ==
				DISP_SESSION_MEMORY)
			cmdqRecBackupUpdateSlot(cmdq_handle,
					*p_subtractor_when_free, layer, 0);
		else
			cmdqRecBackupUpdateSlot(cmdq_handle,
					*p_subtractor_when_free, layer, 1);
	}

	if (force_disable_ovl1) {
		for (layer = OVL_LAYER_NUM_PER_OVL; layer <
				OVL_LAYER_NUM; layer++)
...
static int _config_ovl_output(struct disp_mem_output_config *output)
{
	...
	ret = dpmgr_path_config(pgc->ovl2mem_path_handle, data_config,
			pgc->cmdq_handle_config);
		...
		struct disp_session_input_config *session_input)
{
	...
	memcpy(&m_ccorr_config, &(session_input->ccorr_config),
			sizeof(m_ccorr_config));
		...
		struct primary_disp_input_config *input)
{
	...
	/* this is used for decouple mode, to indicate
	 * whether we need to trigger ovl
	 */
...
#ifdef CONFIG_FOR_SOURCE_PQ
	if (primary_display_is_decouple_mode())
		ret = dpmgr_path_user_cmd(pgc->ovl2mem_path_handle, cmd, arg,
				handle);
	else
		...
		if (cmdqRecGetInstructionCount(handle) > cmdqsize) {
			...
			if (pgc->state == DISP_ALIVE) {
				...
				if (primary_display_is_decouple_mode()) {
					...
					_cmdq_set_config_handle_dirty_mira(
							handle);
					/* use non-blocking flush here to
					 * avoid primary path is locked for
					 * too long
					 */
...
int primary_display_switch_mode(int sess_mode, unsigned int session, int force)
{
	...
	DISPDBG("%s sess_mode %d, session 0x%x\n",
			__func__, sess_mode, session);

	/* to destroy undestroyed memory session at suspend state */
	if (pgc->later_destroy_memory_session && !primary_display_is_sleepd()) {
		memset((void *)&memory_session_config, 0,
				sizeof(struct disp_session_config));
		memory_session_config.session_id =
			pgc->later_destroy_memory_session_id;
		if (disp_destroy_session(&memory_session_config) != 0) {
			...
			primary_display_set_secondary_display(0,
					DISP_SESSION_MEMORY);
	...
	if (pgc->state == DISP_SLEPT) {
		...
		/*
		 *
		 *Allow system to switch mode between Deouple and Decouple
		 *Mirror when time-sharing support.
		 *Since system may use extension path to output data even
		 *when the primay path has suspended.
		 */
		if (pgc->session_mode == DISP_SESSION_DECOUPLE_MODE
...
int primary_display_get_info(void *info)
{
	...
	case LCM_TYPE_DSI:
	{
		...
		if (lcm_param->dsi.mode == CMD_MODE) {
			dispif_info->displayMode = DISP_IF_MODE_COMMAND;
			dispif_info->isHwVsyncAvailable = 1;
			/* DISPMSG("DISP Info: DSI, CMD Mode,
			 * HW Vsync enable\n");
			 */
		} else {
			dispif_info->displayMode = DISP_IF_MODE_VIDEO;
			dispif_info->isHwVsyncAvailable = 1;
			/* DISPMSG("DISP Info: DSI, VDO Mode,
			 * HW Vsync enable\n");
			 */
		}
...
{
	/* TODO: we should store the video/cmd mode in runtime,
	 * because we will support cmd/vdo dynamic switch
	 */
...
{
	if (handle && (handle == (struct cmdqRecStruct *
					)pgc->cmdq_handle_ovl1to2_config))
		return 1;
	else
		return 0;
...
int primary_display_insert_session_buf(
...
int primary_display_setbacklight(unsigned int level)
{
...
#ifdef MTK_DISP_IDLE_LP
				/* CMDmode need to exit top clk off idle mode */
				_disp_primary_path_exit_idle(
						"primary_display_setbacklight",
						0);
#endif
	...
	{
		/*
		 * extern uint32_t DSI_dcs_read_lcm_reg_v2(
		 * enum DISP_MODULE_ENUM module, struct cmdqRecStruct *cmdq,
		...
		DSI_dcs_read_lcm_reg_v2(DISP_MODULE_DSI0, NULL, 0x51, buffer,
				1);
...
int primary_display_set_cmd(int *lcm_cmd, unsigned int cmd_num)
{
	...
	_primary_path_lock(__func__);
	if (pgc->state == DISP_SLEPT) {
		DISPCHECK("Sleep State set cmd Invalid\n");
	} else {
		if (primary_display_cmdq_enabled()) {	/* cmdq */
			...
			if (primary_display_is_video_mode()) {
				...
				_cmdq_insert_wait_frame_done_token_mira(
						cmdq_handle_cmd);
				cmdqRecClearEventToken(cmdq_handle_cmd,
						CMDQ_SYNC_TOKEN_CABC_EOF);
				disp_lcm_set_cmd(pgc->plcm,
						(void *)cmdq_handle_cmd,
						lcm_cmd,
		...
		unsigned int format)
{
	...
	ret = m4u_cache_sync(m4uClient, M4U_PORT_DISP_WDMA0, pbuf,
			buffer_size, mva,
			M4U_CACHE_FLUSH_BY_RANGE);
	if (ret != 0) {
...
#if 0		/* do not have to call enable_cascade here,
		 * dpmgr_path_add_memout() will add ovl1 if necessary
		 */
		if (ovl_get_status() == DDP_OVL1_STATUS_IDLE ||
				ovl_get_status() == DDP_OVL1_STATUS_PRIMARY) {
...
int primary_display_capture_framebuffer(unsigned long pbuf)
{
	...
	unsigned long fbaddress = dpmgr_path_get_last_config(
			pgc->dpmgr_handle)->ovl_config[fb_layer_id].addr;
	unsigned long fbv = 0;
	unsigned int i = 0;
	unsigned long ttt = 0;
	...
	for (i = 0; i < h_yres; i++) {
		...
		memcpy((void *)(pbuf + i * w_xres * pixel_bpp),
		       (void *)(fbv + i * w_fb * pixel_bpp), w_xres *
		       pixel_bpp);
	}
...
#endif
	/* not necessary */
	/* DISPCHECK("[DT]display vram size =
	 * 0x%08x|%d\n", vramsize, vramsize);
	 */
	return vramsize;
#else
		...
		unsigned long *va, unsigned long *mva)
{
	...
	/* ioremap is only for non mapping reserved memory */
	...
	if (fb_va == NULL) {
		...
		if (PageHighMem(pfn_to_page(pa_start >> PAGE_SHIFT))) {
			...
			pr_info("vmap framebuffer at high zone\n");
			for (i = 0; i < npages; i++) {
				addr = pa_start + (i * PAGE_SIZE);
				pages[i] = pfn_to_page(addr >> PAGE_SHIFT);
			}
...
#endif
		{
			pr_info("__va framebuffer at low zone\n");
			fb_va = __va(pa_start);
		}
	}
	*va = (unsigned long)fb_va;
	pr_info("disphal_allocate_fb, pa=%lx, va=0x%lx\n",
			(unsigned long)pa_start, *va);
...
int primary_display_lcm_ATA(void)
{
	...
	_primary_path_lock(__func__);
	if (pgc->state == 0) {
		DISPCHECK("ATA_LCM, primary display path is sleep, skip\n");
		goto done;
	}
		...
		uint32_t num)
{
	...
	/* extern int fbconfig_get_esd_check(DSI_INDEX dsi_id, uint32_t cmd,
	 * uint8_t *buffer, uint32_t num);
	 */

	dst_module = _get_dst_module_by_lcm(pgc->plcm);
	if (dst_module == DISP_MODULE_UNKNOWN || dst_module == DISP_MODULE_DPI0
			|| dst_module == DISP_MODULE_DPI1) {
		pr_info("[ESD]no need to do esd check on dpi panel");
		goto done;
	}
...
int Panel_Master_dsi_config_entry(const char *name, void *config_value)
{
	...
	lcm_param = disp_lcm_get_params(pgc->plcm);
	if (pgc->state == DISP_SLEPT) {
		DISPERR("[Pmaster]Panel_Master: primary path is slept??\n");
		goto done;
	}
	/* / Esd Check : Read from lcm */
...
int primary_display_switch_dst_mode(int mode)
{
	...
	if (pgc->plcm->params->type != LCM_TYPE_DSI) {
		pr_debug("[%s] Error, only support DSI IF\n", __func__);
		goto done;
	}
	if (pgc->state == DISP_SLEPT) {
		DISPCHECK
	...
	/* DISPCHECK("[%s]need switch, cur_mode:%d, switch_mode:%d\n",*/
	/*	     __func__, primary_display_cur_dst_mode, mode);*/
	lcm_cmd = disp_lcm_switch_mode(pgc->plcm, mode);
	if (lcm_cmd == NULL) {
		DISPCHECK("[%s]get lcm cmd fail %d, %d\n", __func__,
	...
	_primary_path_switch_dst_unlock();
#else
	pr_debug("[ERROR: %s]this function not enable in disp driver\n",
			__func__);
#endif
	return ret;
}
...
void primary_display_reset_ovl_by_cmdq(unsigned int force)
{
	...
	if (force == 0) { /* warm reset ovl */
		...
		cmdqRecPoll(pgc->cmdq_handle_config,
				disp_addr_convert(DISP_REG_OVL_STA), 0, 0x1);

		/* reset ovl1 */
		if (ovl_get_status() == DDP_OVL1_STATUS_PRIMARY) {
			DISP_REG_SET(pgc->cmdq_handle_config,
				     DISP_REG_OVL_RST + DISP_OVL_INDEX_OFFSET,
				     0x1);
			DISP_REG_SET(pgc->cmdq_handle_config,
				     DISP_REG_OVL_RST + DISP_OVL_INDEX_OFFSET,
				     0x0);
			cmdqRecPoll(pgc->cmdq_handle_config,
				    disp_addr_convert(DISP_REG_OVL_STA +
					    DISP_OVL_INDEX_OFFSET), 0, 0x1);
		}
	} else if (force == 1) { /* force reset ovl */
		DISPMSG("force reset ovl\n");
		/* reset ovl0 */
		DISP_REG_SET_FIELD(pgc->cmdq_handle_config, RST_FLD_FORCE_RST,
				DISP_REG_OVL_RST, 0x1);
		DISP_REG_SET_FIELD(pgc->cmdq_handle_config, RST_FLD_FORCE_RST,
				DISP_REG_OVL_RST, 0x0);
		cmdqRecPoll(pgc->cmdq_handle_config,
				disp_addr_convert(DISP_REG_OVL_STA), 0, 0x1);

		/* reset ovl1 */
		if (ovl_get_status() == DDP_OVL1_STATUS_PRIMARY) {
			DISP_REG_SET_FIELD(pgc->cmdq_handle_config,
					RST_FLD_FORCE_RST,
					DISP_REG_OVL_RST +
					DISP_OVL_INDEX_OFFSET, 0x1);
			DISP_REG_SET_FIELD(pgc->cmdq_handle_config,
					RST_FLD_FORCE_RST,
					DISP_REG_OVL_RST +
					DISP_OVL_INDEX_OFFSET, 0x0);
			cmdqRecPoll(pgc->cmdq_handle_config,
					disp_addr_convert(DISP_REG_OVL_STA +
...
int primary_display_fps_test(void)
{
	...
	lcm_param2 = NULL;
	memcpy((void *)&data_config2, (void *)dpmgr_path_get_last_config(
				pgc->dpmgr_handle),
...
int primary_display_check_test(void)
{
	...
	if (pgc->state == DISP_SLEPT) {
		...
		DISPCHECK("[display_test_result]=>Fail:primary path slept\n");
		DISPCHECK("[display_test_result]===========================\n");
		goto done;
	}
	/* stop trigger loop */
