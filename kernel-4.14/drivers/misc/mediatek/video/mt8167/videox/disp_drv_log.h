...
do {									\
	...
	if (0)								\
		dprec_error_log_len += \
	scnprintf(dprec_error_log_buffer + dprec_error_log_len,	\
			dprec_error_log_buflen - dprec_error_log_len,	\
			"[%d][%s #%d]"string, dprec_error_log_id++,	\
			__func__, __LINE__, ##args);			\
} while (0)
#endif
#define DISPPRINT(string, args...) pr_info("[DISP]"string, ##args)
...
#define DISPPR_DEBUG(string, args...)					\
	do {								\
...
#define DISPPR(string, args...)						\
	do {								\
		dprec_error_log_len = \
		scnprintf(dprec_error_log_buffer, dprec_error_log_buflen, \
				string, ##args); \
		dprec_logger_pr(DPREC_LOGGER_HWOP, dprec_error_log_buffer); \
	} while (0)
