...
static int ovl2mem_callback(unsigned int userdata)
{
	...
	DISPMSG("%s(%x), current tick=%d, release tick: %d\n", __func__,
...
int ovl2mem_trigger(int blocking, void *callback, unsigned int userdata)
{
	...
	if (pgc->need_trigger_path == 0) {
		...
		if ((atomic_read(&g_trigger_ticket) - atomic_read(
						&g_release_ticket)) == 1) {
			DISPMSG("%s(%x), cfg input, but not config output!!\n",
					__func__, pgc->session);
			for (layid = 0; layid < (HW_OVERLAY_COUNT + 1
						); layid++) {
				fence_idx =
					mtkfb_query_idx_by_ticket(pgc->session,
							layid, atomic_read(
							&g_trigger_ticket));
				if (fence_idx >= 0)
...
void ovl2mem_wait_done(void)
{
	...
	while ((atomic_read(&g_trigger_ticket) - atomic_read(
					&g_release_ticket)) > 1) {
		dpmgr_wait_event_timeout(pgc->dpmgr_handle,
	...
	DISPMSG("%s loop %d, trigger tick:%d, release tick:%d\n", __func__,
			loop_cnt,
