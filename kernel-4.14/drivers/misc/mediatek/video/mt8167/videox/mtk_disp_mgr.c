...
int disp_create_session(struct disp_session_config *config)
{
	...
	for (i = 0; i < MAX_SESSION_COUNT; i++) {
		if (session_config[i] == session) {
			...
			idx = i;
			session_cnt[i]++;
			DISPMSG("%s, session_id:%d, session_cnt:%d\n",
				__func__, session, session_cnt[i]);
			break;
		}
	}
	...
	if (idx != -1) {
		...
		if ((path_info.switching == DEV_MAX_NUM) &&
			...
			pr_info("create external session, but path have not been destroyed ,need destroy first\n");
			external_display_path_change_without_cascade(
			    ...
			    unsigned long layer_phy_addr)
{
	...
	if (layerid < release_layer_num) {
		...
		if (buff_idx > 0)
			DISPMSG("%s layerid %x addr 0x%lx idx %d\n", __func__,
		...
		DISPMSG("%s(type %d) release layer %d\n", __func__,
...
int _ioctl_create_session(unsigned long arg)
{
	...
	if (disp_get_session_number() > 1
		...
		for (i = DISP_SESSION_EXTERNAL; i < DISP_SESSION_MEMORY; i++) {
			captured_session_input[i].config_layer_num =
				DISP_HW_MAX_LAYER;
			for (j = 0; j < DISP_HW_MAX_LAYER; j++) {
				captured_session_input[i].config[
					j].layer_enable = 0;
				captured_session_input[i].config[
					j].layer_id = j;
			}
	...
	if (config.type == DISP_SESSION_MEMORY) {
		...
		if (primary_display_is_sleepd()) {
			DISPMSG("destroy memory session 0x%x after resume\n",
					config.session_id);
			primary_display_set_later_destroy_memory_session(
					config.session_id);
			return 0;
		}
	}
	...
	if (DISP_SESSION_TYPE(session_id) == DISP_SESSION_PRIMARY) {
		...
		if (config.present_fence_idx != -1) {
			...
			mmprofile_log_ex(ddp_mmp_get_events()->
					present_fence_set, MMPROFILE_FLAG_PULSE,
		...
		mutex_lock(&disp_session_lock);
		ext_disp_config_input_multiple(&captured_session_input
				[DISP_SESSION_EXTERNAL - 1],
				captured_session_input[
				DISP_SESSION_EXTERNAL - 1].config[
				0].next_buff_idx,
				session_id);
...
int _ioctl_prepare_present_fence(unsigned long arg)
{
	...
	if (ret != 0) {
		DISPPR_DEBUG("%s%d,layer%d create Fence Object failed!\n",
			     disp_session_mode_spy(session_id),
			     DISP_SESSION_DEV(session_id),
			     timeline_id);
		ret = -EFAULT;
...
int _ioctl_prepare_present_fence(unsigned long arg)
{
	...
	if ((DISP_SESSION_TYPE(preset_fence_struct.session_id) !=
		...
		DISPCHECK("non-primary or sleep request pf! session=0x%x\n",
		...
		if (ret != 0) {
			DISPPR_DEBUG("%s%d,layer%d create Fence Obj failed!\n",
...
int _ioctl_prepare_buffer(unsigned long arg, enum PREPARE_FENCE_TYPE type)
{
	...
	if (info.layer_en) {
		...
		if (type == PREPARE_OUTPUT_FENCE &&
			...
			/*create second fence for wdma when
			...
			if (buf2 != NULL) {
				...
				DISPPR_DEBUG(
				"preFenceFailed,0x%08x/l%d/e%d/ion%d/cache%d\n",
				info.session_id,
				info.layer_id,
				info.layer_en,
				info.ion_fd,
		...
		DISPPR_DEBUG("err prepare param,0x%08x/l%d/e%d/ion%d/cache%d\n",
...
static int _sync_convert_fb_layer_to_ovl_struct(unsigned int session_id,
		struct disp_input_config *src, struct OVL_CONFIG_STRUCT *dst,
	...
	for (i = 0; i < input->config_layer_num; i++) {
		...
		if (input->config[i].layer_enable) {
			...
			if (input->config[i].src_phy_addr)
				dst_mva = (unsigned long)input->config[
					i].src_phy_addr;
			else
				disp_sync_query_buf_info(session_id, layer_id,
						 (unsigned int)input->config[
						 i].next_buff_idx,
						 &dst_mva, &dst_size);
			...
			DISPPR_FENCE("S+/ML%d/e%d/id%d/%dx%d(%d,%d)(%d,%d)",
				      input->config[i].layer_id,
			...
			DISPPR_FENCE("/%s/%d/0x%p/mva0x%08lx/t%d/sec%d\n",
				      _disp_format_spy(input->config[
					      i].src_fmt),
				      input->config[i].src_pitch,
...
#if defined(OVL_TIME_SHARING)
		captured_session_input[DISP_SESSION_MEMORY - 1].config[
			layer_id] = input->config[i];
#endif
...
#else
	captured_session_input[DISP_SESSION_MEMORY - 1].session_id =
		input->session_id;
#endif
...
static int set_external_buffer(struct disp_session_input_config *input)
{
	...
	for (i = 0; i < input->config_layer_num; ++i) {
		...
		if (input->config[i].layer_enable) {
			...
			if (input->config[i].src_phy_addr) {
				dst_mva = (unsigned long int)input->config[
					i].src_phy_addr;
			} else {
				disp_sync_query_buf_info(session_id, layer_id,
						(unsigned int)input->config[
			...
			DISPPR_FENCE
				("S+/EL%d/e%d/id%d/%dx%d(%d,%d)(%d,%d)",
				 ...
				 input->config[i].tgt_offset_x,
				 input->config[i].tgt_offset_y);
			DISPPR_FENCE("/%s/%d/0x%p/mva0x/%08lx\n",
		...
		captured_session_input[DISP_SESSION_EXTERNAL - 1].session_id =
			input->session_id;
		captured_session_input[
			DISP_SESSION_EXTERNAL - 1].config[layer_id] =
			input->config[i];
#endif
	}
...
#if defined(CONFIG_MTK_HDMI_SUPPORT) || defined(CONFIG_MTK_EPD_SUPPORT)
	ret = ext_disp_config_input_multiple(input,
			input->config[0].next_buff_idx, session_id);
#endif
#endif
...
static int set_primary_buffer(struct disp_session_input_config *input)
{
	...
	if (input->config_layer_num == 0 ||
		...
		DISPERR("%s, config_layer_num invalid = %d!\n", __func__,
			input->config_layer_num);
		return 0;
	}
	...
	for (i = 0; i < input->config_layer_num; i++) {
		...
		if (input->config[i].layer_enable) {
			...
			if (input->config[i].buffer_source ==
					DISP_BUFFER_ALPHA) {
				fence_msg_len +=
					sprintf(fence_msg_buf + fence_msg_len,
			...
			if (input->config[i].src_phy_addr)
				dst_mva = (unsigned long
						)input->config[i].src_phy_addr;
			else
				disp_sync_query_buf_info(session_id,
						(unsigned int)layer_id,
						(unsigned int)input->config[
						i].next_buff_idx, &dst_mva,
						&dst_size);

			input->config[i].src_phy_addr = (void *)dst_mva;

			if (dst_mva == 0) {
				DISPPR_DEBUG("dislayer%d for no valid mva\n",
					     input->config[i].layer_id);
				input->config[i].layer_enable = 0;
			}
		...
		if (isAEEEnabled == 1) {
			if (layer_id == primary_display_get_option(
						"ASSERT_LAYER"))
				DISPMSG("AEElayer has en, skip aeelayer cfg\n");
			else
				captured_session_input[
					DISP_SESSION_PRIMARY - 1].
					config[layer_id] = input->config[i];

		} else {
			captured_session_input[DISP_SESSION_PRIMARY - 1].config[
				layer_id] = input->config[i];
			memcpy(&(captured_session_input[
						DISP_SESSION_PRIMARY - 1].
						ccorr_config),
					&(input->ccorr_config),
					sizeof(input->ccorr_config));
		}
		up(&dal_sem);
#endif
		if (session_info)
...
#ifdef CONFIG_ALL_IN_TRIGGER_STAGE
	captured_session_input[DISP_SESSION_PRIMARY - 1].session_id =
		input->session_id;
#endif
	DISPPR_FENCE("%s\n", fence_msg_buf);
	mutex_unlock(&session_config_mutex);
	...
	if (force_sec) {
		for (i = 0; i < session_input->config_layer_num; i++) {
			if (session_input->config[i].layer_id == 0) {
				session_input->config[i].security =
					DISP_SECURE_BUFFER;
				if (secure_handle == 0)
					secure_handle =
						alloc_sec_buffer(1920*1200*4);
				session_input->config[i].src_phy_addr =
					(void *)secure_handle;
			}
		}
	} else {
...
int _ioctl_set_output_buffer(unsigned long arg)
{
	...
	DISPMSG("%s session = 0x%x, idx %x, seq %x\n", __func__,
	...
	if (DISP_SESSION_TYPE(session_id) == DISP_SESSION_PRIMARY) {
		...
		if (primary_display_is_decouple_mode()) {
			/* xuecheng, UGLY WORKAROUND!!! should use
			 * a common struct for input/output cache info
			 */
			struct disp_input_config src;
		...
		DISPPR_FENCE("S+O/%s%d/L%d(id%d)/L%d(id%d)/%dx%d(%d,%d)",
				disp_session_mode_spy(session_id),
				DISP_SESSION_DEV(session_id),
				disp_sync_get_output_timeline_id(),
				session_output.config.buff_idx,
		...
		DISPPR_FENCE("/%s/%d/0x%08x/mva0x%08lx/sec%d\n",
...
#ifdef CONFIG_ALL_IN_TRIGGER_STAGE
		mutex_lock(&session_config_mutex);
		captured_session_output[DISP_SESSION_PRIMARY - 1] =
			primary_output;
		mutex_unlock(&session_config_mutex);
#else
		...
		DISPMSG("%s done idx 0x%x,mva %lx,fmt %x,w%x,h%x(%x %x),p %x\n",
			__func__,
			session_output.config.buff_idx, dst_mva,
			session_output.config.fmt,
			session_output.config.width,
			session_output.config.height,
		...
		DISPPR_FENCE("S+O/%s%d/L%d/id%d/%dx%d(%d,%d)/%s/%d/%d/0x%p/",
				disp_session_mode_spy(session_id),
		...
		DISPPR_FENCE("mva0x%08lx/t%d/sec%d(%d)\n",
				dst_mva,
				get_ovl2mem_ticket(),
				session_output.config.security,
...
#if defined(OVL_TIME_SHARING)
		mutex_lock(&session_config_mutex);
		captured_session_output[DISP_SESSION_MEMORY - 1] =
			primary_output;
		mutex_unlock(&session_config_mutex);
#else
		if (dst_mva)
			ovl2mem_output_config((struct disp_mem_output_config *
						)&primary_output);
		else
		...
		DISPMSG("%s done idx 0x%x, mva %lx, fmt %x, w %x, h %x, p %x\n",
			__func__,
			session_output.config.buff_idx, dst_mva,
			session_output.config.fmt,
			session_output.config.width,
...
int _ioctl_wait_vsync(unsigned long arg)
{
	...
	if (primary_display_is_sleepd() && is_hdmi_active()) {
		...
		if (ret <= 0)
			DISPERR("%s, ext_disp fail, ret = %d.\n", __func__,
					ret);
	} else {
		ret = primary_display_wait_for_vsync(&vsync_config);
		if (ret != 0)
			DISPERR("%s ,primary_display fail, ret = %d.\n",
					__func__, ret);
	}
#else
	if (DISP_SESSION_TYPE(vsync_config.session_id) ==
			DISP_SESSION_PRIMARY) {
		ret = primary_display_wait_for_vsync(&vsync_config);
		if (ret != 0)
			DISPERR("primary_display_%s fail, ret = %d.\n",
					__func__, ret);
#if defined(CONFIG_MTK_HDMI_SUPPORT) || defined(CONFIG_MTK_EPD_SUPPORT)
	} else if (DISP_SESSION_TYPE(vsync_config.session_id) ==
	...
	if (copy_to_user(argp, &vsync_config, sizeof(vsync_config))) {
		DISPPR_DEBUG("[FB]: copy_to_user failed! line:%d\n", __LINE__);
		return -EFAULT;
	}
...
int _ioctl_insert_session_buffers(unsigned long arg)
{
	...
	if (copy_from_user(&session_buf_info, argp,
				sizeof(struct disp_session_buf_info))) {
		DISPMSG("[FB]: copy_from_user failed! line:%d\n", __LINE__);
		return -EFAULT;
	}
...
static enum DISP_MODE select_session_mode(
		struct disp_session_config *session_info)
{
	...
	if (session_info->user == SESSION_USER_GUIEXT) {
		/*miravison */
		if (primary_display_is_sleepd()) {
			DISPERR("%s(%d) Ignored because of suspended!\n",
				__func__, session_info->mode);
			return final_mode;
		}
		...
		if (session_info->mode == DISP_SESSION_DIRECT_LINK_MODE) {
			...
			if (output_mode == DISP_OUTPUT_CAP_DECOUPLE &&
				...
				final_mode = DISP_SESSION_DECOUPLE_MODE;
				DISPDBG(
				"%s: use decouple for disp mode capacility",
						__func__);
			}
		} else if (session_info->mode ==
				DISP_SESSION_DECOUPLE_MIRROR_MODE) {
			/* DC-->DCm
			 * (SingleDisplay: enter ExternalDisplay while VP)
			 */
			/* ExternalDisplay: exit VP */
			/* DL-->DCm
			 * (SingleDisplay: just enter ExternalDisplay)
			 */
			if (final_mode != DISP_SESSION_DECOUPLE_MIRROR_MODE)
...
int set_session_mode(struct disp_session_config *config_info, int force)
{
	...
	DISPDBG("%s: 0x%x -- %d\n", __func__, config_info->session_id,
			config_info->mode);
...
static int mtk_disp_mgr_probe(struct platform_device *pdev)
{
	...
	class_dev =
		(struct class_device *)device_create(mtk_disp_mgr_class, NULL,
				mtk_disp_mgr_devno, NULL, DISP_SESSION_DEVICE);
	disp_sync_init();
