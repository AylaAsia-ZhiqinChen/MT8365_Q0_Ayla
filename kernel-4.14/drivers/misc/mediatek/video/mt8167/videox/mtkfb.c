		...
		struct disp_input_config *dst)
{
...
#if 1
	DISPDBG("%s():id=%u, en=%u, next_idx=%u, vaddr=%p, paddr=%p,\n",
	...
	DISPDBG("%s:tgt xoff=%u,tgt yoff=%u,tgt w=%u, tgt h=%u, aen=%u\n",
		...
		struct fb_info *info)
{
	...
	if (no_update) {
		DISPMSG("FB_ACTIVATE_NO_UPDATE flag found, ignore %s\n",
				__func__);
		no_update = false;
		return ret;
	}
	...
	case 24:
		...
		break;
	case 32:
		input->src_fmt = (var->blue.offset == 0) ?
...
static void set_fb_fix(struct mtkfb_device *fbdev)
{
	...
	fix->line_length = ALIGN_TO(var->xres_virtual,
...
static int mtkfb_check_var(struct fb_var_screeninfo *var, struct fb_info *fbi)
{
	...
	DISPDBG("%s, xres=%u, yres=%u, xres_virtual=%u, yres_virtual=%u,\n",
...
static int mtkfb_set_par(struct fb_info *fbi)
{
	...
	DISPDBG("fb_pa=0x%08lx,yoffset=0x%08x,fbi->fix.line_length=0x%08x\n",
	...
	/* fb_layer.src_width = fb_layer.tgt_width = fb_layer.src_pitch =
	 * var->xres;
	 */
...
#if 0
	MTKFB_LOG("[FB Driver] %s():id=%u, layer en=%u, next_en=%u,\n",
		  __func__, layerInfo->layer_id, layerInfo->layer_enabled,
		  layerInfo->next_en);
		...
		unsigned long arg)
{
	...
	mmprofile_log_ex(MTKFB_MMP_Events.IOCtrl, MMPROFILE_FLAG_PULSE,
			_IOC_NR(cmd), arg);
	MTKFB_LOG_DBG("%s, info=%p, cmd nr=0x%08x, cmd size=0x%08x\n",
			__func__, info,
	...
	case MTKFB_GET_DISPLAY_IF_INFORMATION:
	{
		...
		if (displayid == 0) {
			...
			DISPERR("info for displayid: %d is not available now\n",
				displayid);
			return -EFAULT;
		}
	...
	case MTKFB_CAPTURE_FRAMEBUFFER:
	{
		...
		if (copy_from_user(&dst_pbuf, (void __user *)arg,
			...
			if (!src_pbuf) {
				MTKFB_LOG("[FB]:vmalloc cap src_pbuf err:%d\n",
						__LINE__);
				r = -EFAULT;
			} else {
				...
				if (copy_to_user((unsigned long *)dst_pbuf,
							src_pbuf, fbsize)) {
					MTKFB_LOG("[FB]: copy_to_user err:%d\n",
							__LINE__);
					r = -EFAULT;
				}
				vfree(src_pbuf);
	...
	#else
		pr_info("[FB]: do not support capture fb with iommu, line:%d\n",
				__LINE__);
		r = -EFAULT;
	#endif
		return r;
	...
	case MTKFB_SLT_AUTO_CAPTURE:
	{
		...
		if (copy_from_user(&capConfig, (void __user *)arg,
			...
			fb_size = DISP_GetScreenWidth() *
				DISP_GetScreenHeight() * 4;
			if (!capConfig.outputBuffer) {
				MTKFB_LOG("[FB]:vmalloc cap outBuf failed:%d\n",
						__LINE__);
				r = -EFAULT;
			} else {
				...
				primary_display_capture_framebuffer_ovl(
						...
						format);
				if (copy_to_user(dst_buffer,
						(char *)capConfig.outputBuffer,
						fb_size)) {
					MTKFB_LOG("[FB]:copy_to_user err:%d\n",
							__LINE__);
					r = -EFAULT;
	...
	#else
		pr_info("[FB]: do not support capture framebuffer with iommu, line:%d\n",
				__LINE__);
		r = -EFAULT;
	#endif
		...
		if (copy_from_user(&layerInfo, (void __user *)arg,
			...
			for (i = 0; i < VIDEO_LAYER_COUNT; ++i) {
				...
				input =
				    ...
				    session_input.config_layer_num++];
				_convert_fb_layer_to_disp_input(&layerInfo[i],
						input);
			}
			primary_display_config_input_multiple(&session_input);
			primary_display_trigger(1, NULL, 0);
	...
	{
		MTKFB_LOG(" %s():MTKFB_TRIG_OVERLAY_OUT\n", __func__);
	...
	default:
		pr_debug("%s Not support, info=%p, cmd=0x%08x, arg=0x%lx\n",
		...
		unsigned long arg)
{
	...
	pr_debug("[FB Driver] %s, cmd=0x%08x, cmd nr=0x%08x, cmd size=0x%08x\n",
	...
	case COMPAT_MTKFB_CAPTURE_FRAMEBUFFER:
	{
		...
		if (!pbuf) {
			pr_info("[FB]: vmalloc capture src_pbuf failed! line:%d\n",
					__LINE__);
			ret  = -EFAULT;
		} else {
			dprec_logger_start(DPREC_LOGGER_WDMA_DUMP, 0, 0);
			primary_display_capture_framebuffer_ovl(
					(unsigned long)pbuf, eBGRA8888);
			dprec_logger_done(DPREC_LOGGER_WDMA_DUMP, 0, 0);
			ret = get_user(dest, data32);
			if (copy_in_user((unsigned long *)dest, pbuf,
						fbsize/2)) {
...
static int mtkfb_fbinfo_init(struct fb_info *info)
{
	...
	DISPMSG("FB_XRES=%d, FB_YRES=%d, FB_XRES_V=%d, FB_YRES_V=%d,",
		MTK_FB_XRES, MTK_FB_YRES, MTK_FB_XRESV, MTK_FB_YRESV);
	DISPMSG(" BPP=%d, FB_PAGES=%d, FB_LINE=%d, FB_SIZEV=%d\n",
		MTK_FB_BPP, MTK_FB_PAGES, MTK_FB_LINE, MTK_FB_SIZEV);
		...
		unsigned int color)
{
	...
	DISPMSG("@(%d,%d)addr=0x%lx, MTK_FB_XRESV=%d, addr=0x%lx, w=%d, h=%d\n",
		  x, y, addr, MTK_FB_XRESV, start_addr, w, h);

	for (j = 0; j < h; j++) {
		for (i = 0; i < w; i++)
			*(unsigned int *)(start_addr + i * 4 + j *
					MTK_FB_XRESV * 4) = color;
...
static int mtkfb_fbinfo_modify(struct fb_info *info, unsigned int fmt)
{
	...
	if (fmt == 0) {
		/* default as BGRA, GPU(default data is BGRA too)
		 * will not change this value at home screen_base
		 */
		var.red.offset = 16;
		var.red.length = 8;
		var.green.offset = 8;
...
char *mtkfb_find_lcm_driver(void)
{
...
#else
	{
		 ...
		 * the uboot should be old version
		 */
		if (p == NULL)
			return NULL;
...
int _parse_tag_videolfb(void)
{
	...
	if (chosen_node) {
		videolfb_tag = (struct tag_videolfb *)of_get_property(
				chosen_node, "atag,videolfb", NULL);
		if (videolfb_tag) {
			...
			DISPPRINT(
	"[DT][videolfb]lcmfound=%d,fps=%d,fb_base=%p,vram=%d, lcmname=%s\n",
				islcmconnected, lcd_fps, (void *)fb_base,
				vramsize, mtkfb_lcm_name);
			return 0;
		}
...
static int mtkfb_probe(struct platform_device *pdev)
{
	...
	if ((!larb_pdev) || (!larb_pdev->dev.driver)) {
		pr_info("disp_probe is earlier than SMI\n");
		return -EPROBE_DEFER;
	}
	...
	if (get_boot_mode() == NORMAL_BOOT) {
		free_fb_buf = 1;
		pr_info("%s free fb at NORMAL_BOOT\n", __func__);
	}
#endif
	...
	if (dts_gpio_state != 0)
		dev_info(&pdev->dev, "retrieve GPIO DTS failed.");
#endif
...

	{
...
#else
		{
			...
			fb_mem_addr_va =
				(unsigned long)of_iomap(dev->of_node, 0);

			pr_debug("%s: fb_pa = 0x%lx, fb_va = 0x%lx\n",
				__func__, fb_mem_addr_pa, fb_mem_addr_va);
	 ...
	 */
	DISPPRINT("ion map fb base=0x%p,fb total size=%d DAL size=%d\n",
		(void *)mtkfb_get_fb_base(),
		(unsigned int)mtkfb_get_fb_size(),
		(unsigned int)DAL_GetLayerSize());
	...
	_mtkfb_internal_test((unsigned long)fbdev->fb_va_base, MTK_FB_XRES,
			MTK_FB_YRES, 0);
...
int mtkfb_ipoh_restore(struct notifier_block *nb, unsigned long val, void *ign)
{
	...
	case PM_RESTORE_PREPARE:
		...
		pr_debug("[FB Driver] %s PM_RESTORE_PREPARE\n", __func__);
		return NOTIFY_DONE;
...
#ifdef CONFIG_MTK_LEDS
	mt65xx_leds_brightness_set(TYPE_LCD, LED_OFF);
#endif
	if (!lcd_fps)
		msleep(30);
	else
