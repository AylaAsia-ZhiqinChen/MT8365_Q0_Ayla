...
void _dump_lcm_info(struct disp_lcm_handle *plcm)
{
	...
	if (l && p) {
		buf_pos += scnprintf(str_buf + buf_pos, 128 - buf_pos,
				"resolution: %d x %d",
				     p->width, p->height);
		...
		case LCM_INTERFACE_DSI0:
			buf_pos +=
			    scnprintf(str_buf + buf_pos, buf_len - buf_pos,
					    ", interface: DSI0");
			break;
		...
		switch (p->type) {
		case LCM_TYPE_DBI:
			buf_pos += scnprintf(str_buf + buf_pos,
					buf_len - buf_pos, ", Type : DBI");
			break;
		case LCM_TYPE_DSI:
			buf_pos += scnprintf(str_buf + buf_pos,
					buf_len - buf_pos, ", Type : DSI");
			break;
		case LCM_TYPE_DPI:
			buf_pos += scnprintf(str_buf + buf_pos,
					buf_len - buf_pos, ", Type : DPI");
			break;
		default:
			buf_pos +=
			    scnprintf(str_buf + buf_pos, buf_len - buf_pos,
...
#error
			...
			DISPCHECK("vact_line: %u, hact: %u, hbp: %u, hfp: %u,",
			...
			DISPCHECK("[LCM] pll_select: %d, pll_div1: %d,",
				  p->dsi.pll_select, p->dsi.pll_div1);
			DISPCHECK(" pll_div2: %d, fbk_div: %d,\n",
				  p->dsi.pll_div2, p->dsi.fbk_div);
			DISPCHECK("fbk_sel: %d, rg_bir: %d\n",
					p->dsi.fbk_sel, p->dsi.rg_bir);
			DISPCHECK("[LCM] rg_bic: %d, rg_bp: %d,",
				  p->dsi.rg_bic, p->dsi.rg_bp);
			DISPCHECK(" PLL_CLOCK: %d, dsi_clock: %d,\n",
			...
			DISPCHECK("compatibility_for_nvk: %d, cont_clock: %d\n",
				  ...
				  p->dsi.cont_clock);
			DISPCHECK("[LCM]lcm_ext_te_enable:%d,noncont_clk: %d,",
				  p->dsi.lcm_ext_te_enable,
				  p->dsi.noncont_clock);
			DISPCHECK(" noncont_clock_period: %d\n",
...
#define MAX_SIZE \
	(MAX(MAX(MAX(INIT_SIZE, COMPARE_ID_SIZE), SUSPEND_SIZE), \
	     BACKLIGHT_SIZE))
		...
		unsigned char *dts)
{
	...
	if (len <= 0) {
		...
		for (i = 0; i < 32; i++) {
			...
			default:
				pr_info("%s:%d: %d\n", __FILE__, __LINE__,
				       (unsigned int)lcm_dts->compare_id[
				       i].func);
				return;
			}
	...
	if (len <= 0) {
		...
		for (i = 0; i < 32; i++) {
			...
			default:
				pr_info("%s:%d: %d\n", __FILE__, __LINE__,
				       (unsigned int)lcm_dts->backlight[
				       i].func);
				return;
			}
		...
		enum LCM_INTERFACE_ID lcm_id)
{
	...
	if (_lcm_count() == 0) {
		...
		if (plcm_name == NULL) {
			...
			if (strcmp(lcm_drv->name, plcm_name)) {
				DISPERR
				    ("FATAL ERROR!!!LCM Driver defined in");
				DISPERR(" kernel is different with LK\n");
				return NULL;
			}
		...
		if (plcm_name == NULL) {
			...
			DISPERR("FATAL ERROR: can't found lcm driver:%s",
					plcm_name);
			DISPERR(" in linux kernel driver\n");
		}
		/* TODO: */
	}
...

	{
		...
		DISPERR("the LCM Interface [%d] didn't define any lcm driver\n",
				lcm_id);
		goto FAIL;
	}
	...
	if (_is_lcm_inited(plcm)) {
		...
		if (lcm_drv->update) {
			...
			if (disp_lcm_is_video_mode(plcm))
				...
				DISPERR("FATAL ERROR, lcm is cmd mode");
				DISPERR(" lcm_drv->update is null\n");
			}
...
void *disp_lcm_switch_mode(struct disp_lcm_handle *plcm, int mode)
{
	...
	if (_is_lcm_inited(plcm)) {
		if (plcm->params->dsi.switch_mode_enable == 0) {
			DISPERR("ERR,Not en switch in lcm_get_params func\n");
			return NULL;
		}
		lcm_drv = plcm->drv;
