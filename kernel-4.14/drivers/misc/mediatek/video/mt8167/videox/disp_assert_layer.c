...
enum DAL_STATUS DAL_Init(unsigned long layerVA, unsigned long layerPA)
{
	...
	ret = MFC_Open(&mfc_handle, dal_fb_addr, DAL_WIDTH, DAL_HEIGHT,
			DAL_BPP, DAL_FG_COLOR, DAL_BG_COLOR);
	if (ret != MFC_STATUS_OK) {
		pr_debug(
			"DISP/DAL:Warning:MFC_XXX failed %s,line:%d,ret: %x\n",
...
enum DAL_STATUS DAL_SetColor(unsigned int fgColor, unsigned int bgColor)
{
	...
	if (ret != MFC_STATUS_OK) {
		pr_debug("DISP/DAL:Warning:MFC_XXX failed %s,line:%d,ret:%x\n",
			__func__, __LINE__, ret);
		return DAL_STATUS_FATAL_ERROR;
	}
...
enum DAL_STATUS DAL_Dynamic_Change_FB_Layer(unsigned int isAEEEnabled)
{
	...
	pr_debug("[DDP] %s, isAEEEnabled=%d\n", __func__,
			isAEEEnabled);

	if (DISP_DEFAULT_UI_LAYER_ID == DISP_CHANGED_UI_LAYER_ID) {
		pr_debug("%s,no dynamic switch\n", __func__);
		return DAL_STATUS_OK;
	}
	...
	if (isAEEEnabled == 1) {
		...
		/* change global variable value, else error-check will find
		 * layer 2, 3 enable tdshp together
		 */
		...
		memset((void *)(&cached_layer_config[DISP_CHANGED_UI_LAYER_ID]),
				0, sizeof(struct OVL_CONFIG_STRUCT));
	}
...
enum DAL_STATUS DAL_Clean(void)
{
	...
	/* TODO: if dal_shown=false, and 3D enabled,
	 * mtkfb may disable UI layer, please modify 3D driver
	 */
	if (isAEEEnabled == 1) {
		...
		session_input =
			&captured_session_input[DISP_SESSION_PRIMARY - 1];
		...
		captured_session_input[
			DISP_SESSION_PRIMARY - 1].config[input.layer_id]
			= input;
...
enum DAL_STATUS DAL_Printf(const char *fmt, ...)
{
	...
	if (isAEEEnabled == 0) {
		...
		pr_info("[DDP] isAEEEnabled from 0 to 1, ASSERT_LAYER=%d, dal_fb_pa %lx\n",
