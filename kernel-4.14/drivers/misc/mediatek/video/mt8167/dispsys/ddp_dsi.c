	...
	do {\
		...
		if (cmdq) {\
			...
			*(unsigned int *)(&v) = (0x00000000);\
			v.bit = value;\
			DISP_REG_MASK(cmdq, &REG, AS_UINT32( \
						&v), AS_UINT32(&r)); \
		} else {	\
...
struct LCM_DSI_PARAMS dsi_lcm_params;
...
void DSI_sw_clk_trail(int module_idx)
{
	...
	DISP_REG_SET(NULL, &DSI_PHY_REG[module_idx]->MIPITX_DSI_SW_CTRL_CON0,
			0x00000031);
	DISP_REG_SET(NULL, &DSI_PHY_REG[module_idx]->MIPITX_DSI_SW_CTRL_CON1,
			0x0F0F0F0F);
	DISP_REG_SET(NULL, &DSI_PHY_REG[module_idx]->MIPITX_DSI_SW_CTRL,
			0x00000001);
	/* control DSI clk trail duration */
	SW_CTRL_CON0_addr = (unsigned long *)(
			&DSI_PHY_REG[module_idx]->MIPITX_DSI_SW_CTRL_CON0);
	spin_lock_irqsave(&s_lock, flags);
	/* force clk lane keep low */
	mt_reg_sync_writel(0x00000071,
			(unsigned long *)SW_CTRL_CON0_addr);
	/* pull clk lane to LP state */
	mt_reg_sync_writel(0x0000000F,
			(unsigned long *)SW_CTRL_CON0_addr);
...
void DSI_sw_clk_trail_cmdq(int module_idx, struct cmdqRecStruct *cmdq)
{
	...
	DISP_REG_SET(cmdq, &DSI_PHY_REG[module_idx]->MIPITX_DSI_SW_CTRL_CON0,
			0x31);
	DISP_REG_SET(cmdq, &DSI_PHY_REG[module_idx]->MIPITX_DSI_SW_CTRL_CON1,
			0x0F0F0F0F);
	...
	DISP_REG_CMDQ_POLLING(cmdq, &DSI_REG[module_idx]->DSI_STATE_DBG0,
			0x10000, 0x10000);
	DISP_REG_SET(cmdq, &DSI_PHY_REG[module_idx]->MIPITX_DSI_SW_CTRL,
			0x00000000);
}
...
void DSI_clk_ULP_mode(enum DISP_MODULE_ENUM module,
		struct cmdqRecStruct *cmdq, bool enter)
{
	int i = 0;
...
void DSI_clk_HSLP_mode(enum DISP_MODULE_ENUM module,
		struct cmdqRecStruct *cmdq)
{
	int i = 0;
...
{
	DSI_OUTREGBIT(cmdq, struct DSI_PHY_LCCON_REG,
			DSI_REG[0]->DSI_PHY_LCCON, LC_HS_TX_EN, 1);
}
...
enum DSI_STATUS DSI_DumpRegisters(enum DISP_MODULE_ENUM module, int level)
{
	...
	DDPMSG("DSI_STATE_DBG6=0x%08x, count=%d, means: [%s]\n",
		       DSI_DBG6_Status, count,
		       DSI_DBG_STATUS_DESCRIPTION[count]);

	DDPMSG("---------- Start dump DSI registers ----------\n");

	for (i = 0; i < sizeof(struct DSI_REGS); i += 16) {
		DDPMSG("DSI+%04x : 0x%08x  0x%08x  0x%08x  0x%08x\n", i,
			       INREG32(dsi_base_addr + i),
			       INREG32(dsi_base_addr + i + 0x4),
...
int DSI_WaitVMDone(enum DISP_MODULE_ENUM module)
{
	...
	if (_dsi_is_video_mode(module)) {
		DDPERR("%s error: should set DSI to CMD mode firstly\n",
				__func__);
		return -1;
	}
...
{
	DSI_OUTREGBIT(cmdq, struct DSI_MODE_CTRL_REG,
			DSI_REG[0]->DSI_MODE_CTRL, SLEEP_MODE, 1);
	DSI_OUTREGBIT(cmdq, struct DSI_PHY_TIMCON4_REG,
		DSI_REG[0]->DSI_PHY_TIMECON4, ULPS_WAKEUP, 0x22E09);
		...
		struct cmdqRecStruct *cmdq)
{
	...
	DSI_OUTREGBIT(cmdq, struct DSI_START_REG, DSI_REG[0]->DSI_START,
			SLEEPOUT_START, 0);
	DSI_OUTREGBIT(cmdq, struct DSI_MODE_CTRL_REG,
			DSI_REG[0]->DSI_MODE_CTRL, SLEEP_MODE, 0);
...
enum DSI_STATUS DSI_RestoreRegisters(enum DISP_MODULE_ENUM module, void *cmdq)
{
	...
	DSI_OUTREG32(cmdq, &DSI_REG[0]->DSI_INTEN,
			AS_UINT32(&regs->DSI_INTEN));
	DSI_OUTREG32(cmdq, &DSI_REG[0]->DSI_MODE_CTRL,
			AS_UINT32(&regs->DSI_MODE_CTRL));
	DSI_OUTREG32(cmdq, &DSI_REG[0]->DSI_TXRX_CTRL,
			AS_UINT32(&regs->DSI_TXRX_CTRL));
	DSI_OUTREG32(cmdq, &DSI_REG[0]->DSI_PSCTRL,
			AS_UINT32(&regs->DSI_PSCTRL));

	DSI_OUTREG32(cmdq, &DSI_REG[0]->DSI_VSA_NL,
			AS_UINT32(&regs->DSI_VSA_NL));
	DSI_OUTREG32(cmdq, &DSI_REG[0]->DSI_VBP_NL,
			AS_UINT32(&regs->DSI_VBP_NL));
	DSI_OUTREG32(cmdq, &DSI_REG[0]->DSI_VFP_NL,
			AS_UINT32(&regs->DSI_VFP_NL));
	DSI_OUTREG32(cmdq, &DSI_REG[0]->DSI_VACT_NL,
			AS_UINT32(&regs->DSI_VACT_NL));

	DSI_OUTREG32(cmdq, &DSI_REG[0]->DSI_HSA_WC,
			AS_UINT32(&regs->DSI_HSA_WC));
	DSI_OUTREG32(cmdq, &DSI_REG[0]->DSI_HBP_WC,
			AS_UINT32(&regs->DSI_HBP_WC));
	DSI_OUTREG32(cmdq, &DSI_REG[0]->DSI_HFP_WC,
			AS_UINT32(&regs->DSI_HFP_WC));
	DSI_OUTREG32(cmdq, &DSI_REG[0]->DSI_BLLP_WC,
			AS_UINT32(&regs->DSI_BLLP_WC));

	DSI_OUTREG32(cmdq, &DSI_REG[0]->DSI_HSTX_CKL_WC,
			AS_UINT32(&regs->DSI_HSTX_CKL_WC));
	DSI_OUTREG32(cmdq, &DSI_REG[0]->DSI_MEM_CONTI,
			AS_UINT32(&regs->DSI_MEM_CONTI));

	DSI_OUTREG32(cmdq, &DSI_REG[0]->DSI_PHY_TIMECON0,
			AS_UINT32(&regs->DSI_PHY_TIMECON0));
	DSI_OUTREG32(cmdq, &DSI_REG[0]->DSI_PHY_TIMECON1,
			AS_UINT32(&regs->DSI_PHY_TIMECON1));
	DSI_OUTREG32(cmdq, &DSI_REG[0]->DSI_PHY_TIMECON2,
			AS_UINT32(&regs->DSI_PHY_TIMECON2));
	DSI_OUTREG32(cmdq, &DSI_REG[0]->DSI_PHY_TIMECON3,
			AS_UINT32(&regs->DSI_PHY_TIMECON3));
	DSI_OUTREG32(cmdq, &DSI_REG[0]->DSI_VM_CMD_CON,
		...
		struct cmdqRecStruct *cmdq, bool enable, unsigned int color)
{
	...
	for (i = DSI_MODULE_BEGIN(module); i <= DSI_MODULE_END(module); i++) {
		if (enable) {
			 ...
			 * can't just start dsi
			 */
			/* so we just disable pattern bit,
			 * do not start dsi here
			 */
...
void DSI_Config_VDO_Timing(enum DISP_MODULE_ENUM module, void *cmdq,
		struct LCM_DSI_PARAMS *dsi_params)
{
	int i = 0;
		...
		struct cmdqRecStruct *cmdq, struct LCM_DSI_PARAMS *dsi_params)
{
	...
	for (i = DSI_MODULE_BEGIN(module); i <= DSI_MODULE_END(module); i++) {
		...
		DSI_OUTREGBIT(cmdq, struct DSI_TXRX_CTRL_REG,
				...
				dis_eotp_en);
		DSI_OUTREGBIT(cmdq, struct DSI_TXRX_CTRL_REG,
				DSI_REG[i]->DSI_TXRX_CTRL, NULL_EN,
				null_packet_en);
		DSI_OUTREGBIT(cmdq, struct DSI_TXRX_CTRL_REG,
...
int MIPITX_IsEnabled(enum DISP_MODULE_ENUM module, struct cmdqRecStruct *cmdq)
{
	...
	DISPMSG("MIPITX for %s is %s\n", ddp_get_module_name(module), ret ?
			"on" : "off");
	return ret;
}
...
unsigned int dsi_phy_get_clk(enum DISP_MODULE_ENUM module)
{
	...
	unsigned int prediv = (1 << (DSI_PHY_REG[
				i]->MIPITX_DSI_PLL_CON0.RG_DSI0_MPPLL_PREDIV));
	unsigned int posdiv = (1 << (DSI_PHY_REG[
				i]->MIPITX_DSI_PLL_CON0.RG_DSI0_MPPLL_POSDIV));
	unsigned int txdiv0 = (1 << (DSI_PHY_REG[
				i]->MIPITX_DSI_PLL_CON0.RG_DSI0_MPPLL_TXDIV0));
	unsigned int txdiv1 = (1 << (DSI_PHY_REG[
				i]->MIPITX_DSI_PLL_CON0.RG_DSI0_MPPLL_TXDIV1));
		...
		struct cmdqRecStruct *cmdq, struct LCM_DSI_PARAMS *dsi_params)
{
	...
	for (i = DSI_MODULE_BEGIN(module); i <= DSI_MODULE_END(module); i++) {
		...
		if (data_Rate != 0) {
			if (data_Rate > 1250) {
				DISPCHECK("mipitx DataRate exceed limit(%d)\n",
						data_Rate);
				ASSERT(0);
			...
			MIPITX_OUTREGBIT(struct MIPITX_DSI_PLL_CON2_REG,
					...
					((256 * (data_Rate * txdiv % 13) /
					  13) & 0xFF));
			MIPITX_OUTREGBIT(struct MIPITX_DSI_PLL_CON2_REG,
					DSI_PHY_REG[i]->MIPITX_DSI_PLL_CON2,
					RG_DSI0_MPPLL_SDM_PCW_8_15,
					((256 * (256 * (data_Rate * txdiv %
							13) % 13) /
					  13) & 0xFF));
			MIPITX_OUTREGBIT(struct MIPITX_DSI_PLL_CON2_REG,
					DSI_PHY_REG[i]->MIPITX_DSI_PLL_CON2,
			...
			if (dsi_params->ssc_disable != 1) {
				...
				ASSERT(delta1 <= 8);
				pdelta1 = (delta1 * data_Rate * txdiv *
				...
				MIPITX_OUTREGBIT(struct MIPITX_DSI_PLL_CON3_REG,
					 ...
					 pdelta1);
				/* DSI_OUTREGBIT(struct MIPITX_DSI_PLL_CON1_REG,
				 * DSI_PHY_REG->MIPITX_DSI_PLL_CON1,
				 */
				/*	RG_DSI0_MPPLL_SDM_FRA_EN,1); */
				DISPMSG("[dsi_drv.c] PLL config:data_rate=%d",
						data_Rate);
				DISPMSG
				("txdiv=%d,pcw=%d,delta1=%d,pdelta1=0x%x\n",
				     txdiv, DSI_INREG32(
		...
		MIPITX_OUTREGBIT(struct MIPITX_DSI_CLOCK_LANE_REG,
				DSI_PHY_REG[i]->MIPITX_DSI0_CLOCK_LANE,
...
void DSI_PHY_TIMCONFIG(struct LCM_DSI_PARAMS *lcm_params)
{
	...
	if (lcm_params->PLL_CLOCK != 0) {
		...
		DDPMSG("DSI_PHY, Cycle Time: %d(ns), Unit Interval: %d(ns). ,",
			       cycle_time, ui);
		DDPMSG(" lane#: %d\n", lane_no);
	} else {
...
#define NS_TO_CYCLE(n, c)   ((n) / (c))
	...
	hs_trail_m = 1;
	hs_trail_n =
		(lcm_params->HS_TRAIL == 0) ?
		NS_TO_CYCLE(((hs_trail_m * 0x4) + 0x60),
				cycle_time) : lcm_params->HS_TRAIL;
	/* +3 is recommended from designer becauase of HW latency */
	timcon0.HS_TRAIL = ((hs_trail_m > hs_trail_n) ?
			hs_trail_m : hs_trail_n) + 0x0a;

	timcon0.HS_PRPR =
		(lcm_params->HS_PRPR == 0) ?
		NS_TO_CYCLE((0x40 + 0x5 * ui),
	...
	timcon0.LPX =
		(lcm_params->LPX == 0) ?
		NS_TO_CYCLE(0x50, cycle_time) : lcm_params->LPX;
	if (timcon0.LPX == 0)
		timcon0.LPX = 1;

	/* timcon1.TA_SACK     =
	 * (lcm_params->TA_SACK == 0) ? 1 : lcm_params->TA_SACK;
	 */
	timcon1.TA_GET =
		(lcm_params->TA_GET == 0) ?
		(0x5 * timcon0.LPX) : lcm_params->TA_GET;
	timcon1.TA_SURE =
		(lcm_params->TA_SURE == 0) ?
		(0x3 * timcon0.LPX / 0x2) : lcm_params->TA_SURE;
	timcon1.TA_GO = (lcm_params->TA_GO == 0) ?
		(0x4 * timcon0.LPX) : lcm_params->TA_GO;
	/* -------------------------------------------------------------- */
	...
	/* -------------------------------------------------------------- */
	timcon1.DA_HS_EXIT =
		(lcm_params->DA_HS_EXIT == 0) ?
		NS_TO_CYCLE((0x3c + 0x80 * ui),
				cycle_time) : lcm_params->DA_HS_EXIT;
	timcon2.CLK_TRAIL =
		((lcm_params->CLK_TRAIL == 0) ?
		 NS_TO_CYCLE(0x64,
	...
	/* timcon2.LPX_WAIT    =
	 * (lcm_params->LPX_WAIT == 0) ? 1 : lcm_params->LPX_WAIT;
	 */
	timcon2.CONT_DET = lcm_params->CONT_DET;
	timcon2.CLK_ZERO =
		(lcm_params->CLK_ZERO == 0) ?
		NS_TO_CYCLE(0x190,
				cycle_time) : lcm_params->CLK_ZERO;
	timcon3.CLK_HS_PRPR =
		(lcm_params->CLK_HS_PRPR == 0) ?
		NS_TO_CYCLE(0x40,
				cycle_time) : lcm_params->CLK_HS_PRPR;
	if (timcon3.CLK_HS_PRPR == 0)
		timcon3.CLK_HS_PRPR = 1;
	timcon3.CLK_HS_EXIT =
		(lcm_params->CLK_HS_EXIT == 0) ?
		(2 * timcon0.LPX) : lcm_params->CLK_HS_EXIT;
	timcon3.CLK_HS_POST =
		(lcm_params->CLK_HS_POST == 0) ?
		NS_TO_CYCLE((0x3c + 0x80 * ui),
				cycle_time) : lcm_params->CLK_HS_POST;
	DDPMSG("DSI_PHY, HS_TRAIL:%d, HS_ZERO:%d, HS_PRPR:%d, LPX:%d,",
			timcon0.HS_TRAIL, timcon0.HS_ZERO, timcon0.HS_PRPR,
			timcon0.LPX);
	DDPMSG("TA_GET:%d, TA_SURE:%d\n",
	...
	DSI_OUTREGBIT(NULL, struct DSI_PHY_TIMCON0_REG,
			DSI_REG[0]->DSI_PHY_TIMECON0, LPX, timcon0.LPX);
	DSI_OUTREGBIT(NULL, struct DSI_PHY_TIMCON0_REG,
			DSI_REG[0]->DSI_PHY_TIMECON0, HS_PRPR, timcon0.HS_PRPR);
	DSI_OUTREGBIT(NULL, struct DSI_PHY_TIMCON0_REG,
			DSI_REG[0]->DSI_PHY_TIMECON0, HS_ZERO, timcon0.HS_ZERO);
	DSI_OUTREGBIT(NULL, struct DSI_PHY_TIMCON0_REG,
			DSI_REG[0]->DSI_PHY_TIMECON0, HS_TRAIL,
			timcon0.HS_TRAIL);

	DSI_OUTREGBIT(NULL, struct DSI_PHY_TIMCON1_REG,
			DSI_REG[0]->DSI_PHY_TIMECON1, TA_GO, timcon1.TA_GO);
	DSI_OUTREGBIT(NULL, struct DSI_PHY_TIMCON1_REG,
			DSI_REG[0]->DSI_PHY_TIMECON1, TA_SURE, timcon1.TA_SURE);
	DSI_OUTREGBIT(NULL, struct DSI_PHY_TIMCON1_REG,
			DSI_REG[0]->DSI_PHY_TIMECON1, TA_GET, timcon1.TA_GET);
	DSI_OUTREGBIT(NULL, struct DSI_PHY_TIMCON1_REG,
	...
	/*		DISP_REG_GET(DSI_REG[0] + 0x110),
	 *		DISP_REG_GET(DSI_REG[0] + 0x114),
	 */
	/*		DISP_REG_GET(DSI_REG[0] + 0x118),
	 *		DISP_REG_GET(DSI_REG[0] + 0x11c));
	 */
}
...
void DSI_PHY_clk_switch(enum DISP_MODULE_ENUM module, void *cmdq, int on)
{
	...
	if (on) {
		...
		for (i = DSI_MODULE_BEGIN(module); i <=
			...
			/* pre_oe/oe = 1 */
			MIPITX_OUTREGBIT(struct MIPITX_DSI_SW_CTRL_CON0_REG,
					DSI_PHY_REG[i]->MIPITX_DSI_SW_CTRL_CON0,
					SW_LNTC_LPTX_PRE_OE, 1);
			MIPITX_OUTREGBIT(struct MIPITX_DSI_SW_CTRL_CON0_REG,
					DSI_PHY_REG[i]->MIPITX_DSI_SW_CTRL_CON0,
					SW_LNTC_LPTX_OE, 1);
			MIPITX_OUTREGBIT(struct MIPITX_DSI_SW_CTRL_CON1_REG,
					DSI_PHY_REG[i]->MIPITX_DSI_SW_CTRL_CON1,
			...
			MIPITX_OUTREGBIT(struct MIPITX_DSI_SW_CTRL_REG,
					DSI_PHY_REG[i]->MIPITX_DSI_SW_CTRL,
					SW_CTRL_EN, 1);
...
uint32_t DSI_dcs_read_lcm_reg_v2(enum DISP_MODULE_ENUM module, void *cmdq,
		uint8_t cmd, uint8_t *buffer, uint8_t buffer_size)
{
	...
	do {
		...
		if (ret > 0) {
			do {
				...
				DSI_OUTREGBIT(cmdq, struct DSI_RACK_REG,
						DSI_REG[i]->DSI_RACK, DSI_RACK,
						1);
				} while (DSI_REG[i]->DSI_INTSTA.BUSY &&
						(timeout < 1000));
			if (timeout == 1000) {
				/* wait cmd done timeout */
				DISPERR("DSIReadFail:wait cmd done timeout\n");
		...
		DSI_OUTREG32(cmdq, &read_data0,
				AS_UINT32(&DSI_REG[i]->DSI_RX_DATA0));
		DSI_OUTREG32(cmdq, &read_data1,
				AS_UINT32(&DSI_REG[i]->DSI_RX_DATA1));
		DSI_OUTREG32(cmdq, &read_data2,
				AS_UINT32(&DSI_REG[i]->DSI_RX_DATA2));
		DSI_OUTREG32(cmdq, &read_data3,
				AS_UINT32(&DSI_REG[i]->DSI_RX_DATA3));
#ifdef DDI_DRV_DEBUG_LOG_ENABLE
...
#ifdef DDI_DRV_DEBUG_LOG_ENABLE
				if (dsi_log_on)
					DDPMSG("DSI r ldata exceed 4 bytes\n");
#endif
				recv_data_cnt = 10;
			}
...
#ifdef DDI_DRV_DEBUG_LOG_ENABLE
				if (dsi_log_on)
					DDPMSG("DSI r ldata exceed buf: %d\n",
						       buffer_size);
#endif
...
#ifdef DDI_DRV_DEBUG_LOG_ENABLE
			if (dsi_log_on)
				DDPMSG(" DSI r sdata exceeds buffer size: %d\n",
					       buffer_size);
#endif
			recv_data_cnt = buffer_size;
		...
		if (DSI_REG[d]->DSI_MODE_CTRL.MODE != 0) {
			...
			if (cmd < 0xB0) {
				...
				if (count > 1) {
					...
					vm_cmdq.CM_DATA_0 = count + 1;
					DSI_OUTREG32(cmdq, &DSI_REG[
							d]->DSI_VM_CMD_CON,
							AS_UINT32(&vm_cmdq));

					goto_addr =
						(unsigned long)(&DSI_VM_CMD_REG[
								d]->data[
								0].byte0);
					mask_para = (0xFF <<
							((goto_addr & 0x3) *
			...
			if (cmd < 0xB0) {
				if (count > 1) {
					...
					DSI_OUTREG32(cmdq, &DSI_CMDQ_REG[
							d]->data[0],
							AS_UINT32(&t2));
	...
	unsigned char force_update)
{
	...
	do {
		...
		if (count > 1) {
			...
			goto_addr = (unsigned long) (&DSI_CMDQ_REG[0]->data[
					1].byte0);
		...
		for (i = 0; i < AS_UINT32(&DSI_REG[0]->DSI_CMDQ_SIZE); i++)
			DDPMSG("%s[%d]. DSI_CMDQ+%04x : 0x%08x\n", __func__,
					index, i * 4,
					DISP_REG_GET(DSI_PHY_REG[0] + 0x180 +
						i * 4));
	...
	unsigned char force_update)
{
	...
	for (i = 0; i < queue_size; i++)
		DSI_OUTREG32(cmdq, &DSI_CMDQ_REG[0]->data[i],
				AS_UINT32((pdata+i)));
...
#else
	/* TODO: attach replacements of these functions
	 * if using kernel standardization...
	 */
...
{
	struct LCM_DSI_PARAMS lcm_params_for_clk_setting;

	if (clk > 1000)
		return DSI_STATUS_ERROR;
	memcpy((void *)&lcm_params_for_clk_setting, (void *)&dsi_lcm_params,
			sizeof(struct LCM_DSI_PARAMS));
	...
	if (lcm_params_for_clk_setting.pll_div2 == 0) {
		for (lcm_params_for_clk_setting.pll_div1 = 0;
			 lcm_params_for_clk_setting.pll_div1 < 39;
			 lcm_params_for_clk_setting.pll_div1++) {
			if ((26 * (lcm_params_for_clk_setting.pll_div1 + 1)) >=
					clk)
				goto end;
		}
	}
...
void _dump_dsi_params(struct LCM_DSI_PARAMS *dsi_config)
{
	if (dsi_config) {
		...
		DISPCHECK
			("[DDPDSI] vact: %d, vbp: %d, vfp: %d, vact_line: %d,",
		...
		DISPCHECK
			("[DDPDSI] pll_select: %d, pll_div1: %d, pll_div2: %d,",
			 dsi_config->pll_select, dsi_config->pll_div1,
			 dsi_config->pll_div2);
		DISPCHECK
			("fbk_div: %d,fbk_sel: %d, rg_bir: %d\n",
			 dsi_config->fbk_div, dsi_config->fbk_sel,
			 dsi_config->rg_bir);
		DISPCHECK(
			"[DDPDSI] rg_bic: %d, rg_bp: %d, PLL_CLOCK: %d,",
			dsi_config->rg_bic, dsi_config->rg_bp,
			dsi_config->PLL_CLOCK);
		DISPCHECK(
		...
		DISPCHECK
			("[DDPDSI] lcm_ext_te_enable: %d, noncont_clock: %d,",
		     dsi_config->lcm_ext_te_enable, dsi_config->noncont_clock);
		DISPCHECK(
			 " noncont_clock_period: %d\n",
		...
		struct cmdqRecStruct *cmdq, struct LCM_DSI_PARAMS *dsi_params)
{
	...
	for (i = DSI_MODULE_BEGIN(module); i <= DSI_MODULE_END(module); i++) {
		...
		/* CLK init WC(B) =
		 * (CLK_HS_EXIT + LPX + CLK_HS_PREP + CLK_HS_ZERO)*lane_num
		 */
		...
		if (dsi_mode == SYNC_PULSE_VDO_MODE) {
			v_a = hsa.HSA_WC + hbp.HBP_WC + hfp.HFP_WC +
				ps.DSI_PS_WC + 32;
			v_b =
			    (timcon3.CLK_HS_EXIT + timcon0.LPX +
			     timcon3.CLK_HS_PRPR +
			 ...
			 * HFP_WC=0x%x\n",(v_b+v_c),hfp);
			 */
			DISPDBG("Reconfig to fulfill LP clock lane per line\n");
		...
		/* Limitation: B + C < HFP_WC */
			...
			DISPDBG("===>v_a-v_b=0x%x,HSTX_CKLP_WC=0x%x\n",
					(v_a - v_b), hstx_ckl_wc);
			DISPDBG("Reconfig fulfill LP clock lane per line\n");
...
int ddp_dsi_stop(enum DISP_MODULE_ENUM module, void *cmdq_handle)
{
	...
	if (_dsi_is_video_mode(module)) {
		...
		/*DSI clk lane enter LP at this time when Clktrail not
		 * deploy at trigger loop
		 */
...
int ddp_dsi_switch_lcm_mode(enum DISP_MODULE_ENUM module, void *params)
{
	...
	if (dsi_currect_mode == mode) {
		DDPMSG
		    ("%s noneed switch mode, current mode = %d, switch to %d\n",
		...
		void *params)
{
	...
	if (mode == 0) {	/* V2C */
		...
		DSI_OUTREG32(cmdq_handle,
				...
				0x00000020);
		DSI_OUTREG32(cmdq_handle,
				(unsigned long)(DSI_REG[i]) + 0x60, 2);
		  ...
		  unsigned long *params)
{
	...
	case DDP_STOP_VIDEO_MODE:
		{
			/* ths caller should call wait_event_or_idle
			 * for frame stop event then.
			 */
			DSI_SetMode(module, cmdq_handle, CMD_MODE);
			/* TODO: modify this with wait event */
...
#endif
				if (ret > 0)
					DDPERR("DISP/DSI,DSI PM return err\n");
			}
			s_isDsiPowerOn = true;
...
int ddp_dsi_power_off(enum DISP_MODULE_ENUM module, void *cmdq_handle)
{
	...
	if (s_isDsiPowerOn) {
		...
		DSI_OUTREGBIT(NULL, struct DSI_TXRX_CTRL_REG,
				DSI_REG[0]->DSI_TXRX_CTRL, LANE_NUM, 0);
...
void dsi_analysis(enum DISP_MODULE_ENUM module)
{
	...
	for (i = DSI_MODULE_BEGIN(module); i <= DSI_MODULE_END(module); i++) {
		...
		DDPMSG
		("DSI%d Start:%x, Busy:%d, DSI_DUAL_EN:%d, MODE:%s,",
		...
		DDPMSG(
			" High Speed:%d, FSM State:%s\n",
			DSI_REG[i]->DSI_PHY_LCCON.LC_HS_TX_EN,
			_dsi_cmd_mode_parse_state(DSI_REG[
				i]->DSI_STATE_DBG6.CMTRL_STATE));

		DDPMSG(
			"DSI%d IRQ,RD_RDY:%d, CMD_DONE:%d, SLEEPOUT_DONE:%d,",
		...
		DDPMSG(
			"DSI%d Lane Num:%d, Ext_TE_EN:%d, Ext_TE_Edge:%d,",
			i,
			DSI_REG[i]->DSI_TXRX_CTRL.LANE_NUM,
			DSI_REG[i]->DSI_TXRX_CTRL.EXT_TE_EN,
		...
		enum CMDQ_STATE state)
{
	...
	if (state == CMDQ_BEFORE_STREAM_SOF) {
		...
		/* enable dsi interrupt: RD_RDY/CMD_DONE
		 * (need do this here?)
		 */
		DSI_OUTREGBIT(cmdq_trigger_handle, struct DSI_INT_ENABLE_REG,
				DSI_REG[dsi_i]->DSI_INTEN,
				RD_RDY, 1);
		DSI_OUTREGBIT(cmdq_trigger_handle, struct DSI_INT_ENABLE_REG,
				DSI_REG[dsi_i]->DSI_INTEN,
				CMD_DONE, 1);
		...
		for (i = 0; i < 3; i++) {
			...
			/* / 0xB0 is used to distinguish DCS cmd or
			 * Gerneric cmd, is that Right???
			 */
			t0.Data_ID =
				(t0.Data0 <
		...
		for (i = 0; i < 3; i++) {
			...
			if (hSlot) {
				/* read from slot */
				cmdqBackupReadSlot(hSlot, i * 4 + 0,
						((uint32_t *)&read_data0));
				cmdqBackupReadSlot(hSlot, i * 4 + 1,
						((uint32_t *)&read_data1));
				cmdqBackupReadSlot(hSlot, i * 4 + 2,
						((uint32_t *)&read_data2));
				cmdqBackupReadSlot(hSlot, i * 4 + 3,
						((uint32_t *)&read_data3));
			} else {
			...
			mmprofile_log_ex(ddp_mmp_get_events()->esd_rdlcm,
				...
				AS_UINT32(&(dsi_params->lcm_esd_check_table[
						i])));

			DISPDBG("[DSI]enter cmp read_data0 byte0=0x%x",
				read_data0.byte0);
			DISPDBG(
				...
				read_data0.byte3);
			DISPDBG
			    ("[DSI]enter cmp check_table cmd=0x%x,count=0x%x,",
			     dsi_params->lcm_esd_check_table[i].cmd,
			     dsi_params->lcm_esd_check_table[i].count);
			DISPDBG(
			     "para_list[0]=0x%x,para_list[1]=0x%x\n",
			...
			if (packet_type == 0x1A || packet_type == 0x1C) {
				recv_data_cnt = read_data0.byte1 +
					read_data0.byte2 * 16;
				DISPCHECK("Set receive data count's %d bytes\n",
						recv_data_cnt);
				if (recv_data_cnt > RT_MAX_NUM) {
					DISPCHECK("DSIread ldata exceed10 b\n");
					recv_data_cnt = RT_MAX_NUM;
				}

				if (recv_data_cnt >
					dsi_params->lcm_esd_check_table[
					i].count) {
					DISPCHECK(
					"DSIrldata exceed buf return size %d\n",
					 dsi_params->lcm_esd_check_table[
					 i].count);
					recv_data_cnt =
						dsi_params->lcm_esd_check_table[
				...
				for (j = 0; j < recv_data_cnt; j++) {
					...
					if (buffer[j] !=
						...
						i].para_list[j]) {
						result = 1;
						DISPCHECK("[ESD]CMP i%d return",
								i);
						DISPCHECK(
						"val 0x%x,para_list[%d]=0x%x\n",
						buffer[j], j,
						dsi_params->lcm_esd_check_table[
						i].para_list[j]);
						break;
				...
				if (recv_data_cnt >
					...
					i].count) {
					DISPCHECK
					("DSI r sdata exceed buf ret size %d\n",
					dsi_params->lcm_esd_check_table[
					i].count);
					recv_data_cnt =
						dsi_params->lcm_esd_check_table[
				...
				DISPCHECK("type is non-recognite,type = 0x%x\n",
						packet_type);
				result = 1;
			}
...
struct DDP_MODULE_DRIVER ddp_driver_dsi0 = {
	...
	.config = (int (*)(enum DISP_MODULE_ENUM, struct disp_ddp_path_config*,
				void*)) ddp_dsi_config,
	.build_cmdq = (int (*)(enum DISP_MODULE_ENUM, void*,
				enum CMDQ_STATE)) ddp_dsi_build_cmdq,
	...
	.set_lcm_utils = (int (*)(enum DISP_MODULE_ENUM,
				struct LCM_DRIVER*)) ddp_dsi_set_lcm_utils,
	.ioctl = (int (*)(enum DISP_MODULE_ENUM, void*, unsigned int,
				unsigned long*)) ddp_dsi_ioctl
};
...
int DSI_check_roi(void)
{
	...
	DDPMSG("x0=%d count=%d,read_buf[0]=%d,read_buf[1]=%d,read_buf[2]=%d,",
		x0,
		count, read_buf[0], read_buf[1], read_buf[2]);
	DDPMSG("read_buf[3]=%d\n", read_buf[3]);
	if ((count == 0) || (x0 != 0)) {
		DDPMSG
		("[DSI]x count %d read_buf[0]=%d,read_buf[1]=%d,",
		 count, read_buf[0], read_buf[1]);
		DDPMSG(
		 "read_buf[2]=%d,read_buf[3]=%d\n", read_buf[2], read_buf[3]);
		return -1;
	}
	msleep(20);
	...
	DDPMSG("y0=%d count %d,read_buf[0]=%d,read_buf[1]=%d,read_buf[2]=%d,",
		y0,
		count, read_buf[0], read_buf[1], read_buf[2]);
	DDPMSG("read_buf[3]=%d\n", read_buf[3]);
	if ((count == 0) || (y0 != 0)) {
		DDPMSG
		("[DSI]y cnt %d read_buf[0]=%d,read_buf[1]=%d,read_buf[2]=%d,",
		 count, read_buf[0], read_buf[1], read_buf[2]);
		DDPMSG("read_buf[3]=%d\n", read_buf[3]);
		return -1;
	}
