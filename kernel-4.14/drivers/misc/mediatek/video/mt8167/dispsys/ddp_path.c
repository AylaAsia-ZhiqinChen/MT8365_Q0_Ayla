...
struct mout_t mout_map[DDP_MOUT_NUM] = {
	/* [0]OVL_MOUT */
	{DISP_MODULE_OVL0, {{DISP_MODULE_COLOR0, 1 << 0},
				   {DISP_MODULE_WDMA0, 1 << 1}, {-1, 0} },
	0, 0},
	/* [1]DITHER0_MOUT */
	{DISP_MODULE_DITHER,
		{{DISP_MODULE_RDMA0, 1 << 0}, {DISP_MODULE_UFOE, 1 << 1},
			{DISP_MODULE_WDMA0, 1 << 2}, {-1, 0} },
		0, 0},
	/* [2]UFOE_MOUT */
	{DISP_MODULE_UFOE,
		{{DISP_MODULE_DSI0, 1 << 0}, {DISP_MODULE_DPI0, 1 << 1},
			{DISP_MODULE_DPI1, 1 << 2},
...
struct sel_t sel_out_map[DDP_SEL_OUT_NUM] = {
	...
	{DISP_MODULE_RDMA0,
		...
		0, 0},
	/* [1]RDMA1_SOUT */
	{DISP_MODULE_RDMA1, {DISP_MODULE_DSI0, DISP_MODULE_DPI0,
				    DISP_MODULE_DPI1, -1},
	0, 0},
};
...
struct sel_t sel_in_map[DDP_SEL_IN_NUM] = {
	...
	{DISP_MODULE_UFOE, {DISP_MODULE_RDMA0, DISP_MODULE_DITHER, -1},
		0, 0},
	/* [2]DSI0_SEL */
	{DISP_MODULE_DSI0, {DISP_MODULE_UFOE, DISP_MODULE_RDMA0,
				   DISP_MODULE_RDMA1, -1},
	0, 0},
	/* [3]DPI0_SEL */
	{DISP_MODULE_DPI0, {DISP_MODULE_UFOE, DISP_MODULE_RDMA0,
				   DISP_MODULE_RDMA1, -1},
	0, 0},
	/* [4]DPI1_SEL */
	{DISP_MODULE_DPI1, {DISP_MODULE_UFOE, DISP_MODULE_RDMA0,
				   DISP_MODULE_RDMA1, -1},
	0, 0},
	/* [5]WDMA_SEL */
	{DISP_MODULE_WDMA0, {DISP_MODULE_OVL0, DISP_MODULE_DITHER,
				    DISP_MODULE_UFOE, -1},
	0, 0},
};
...
int ddp_path_init(void)
{
	...
	mout_map[index++].reg =
		(unsigned long *)DISP_REG_CONFIG_DISP_OVL1_MOUT_EN;
#endif
	mout_map[index++].reg =
		(unsigned long *)DISP_REG_CONFIG_DISP_DITHER_MOUT_EN;
	mout_map[index++].reg =
		(unsigned long *)DISP_REG_CONFIG_DISP_UFOE_MOUT_EN;
	/* sel_out */
	sel_out_map[0].reg =
	...
	/* COLOR_SEL */
	sel_in_map[1].reg =
		(unsigned long *)DISP_REG_CONFIG_DISP_UFOE_SEL_IN;
	/* UFOE_SEL */
	sel_in_map[2].reg =
	...
	/* DPI0_SEL */
	sel_in_map[4].reg =
		(unsigned long *)DISP_REG_CONFIG_DPI1_SEL_IN;
	/* DPI1_SEL */
	sel_in_map[5].reg =
			...
			if (module_list[i] == mout_map[j].id) {
				...
				for (k = 0; k < 5; k++) {
					...
					if (mout_map[j].out_id_bit_map[k].m ==
							module_list[step]) {
						mout |= mout_map[
							j].out_id_bit_map[k].v;
						reg_mout |= mout;
						mout_idx = j;
						DDPDBG("mout %s2%s val 0x%x\n",
						       ddp_get_module_name(
							       module_list[i]),
						       ddp_get_module_name(
							       module_list[
	...
	for (i = 1; i < module_num; i++) {
		for (j = 0; j < DDP_SEL_IN_NUM; j++) {
			if (module_list[i] == sel_in_map[j].id) {
				step = i - 1;
				/* find next module which can be connected */
				while (module_can_connect[module_list[
						step]].bit == 0 && step > 0)
					step--;
	...
	for (i = 0; i < module_num - 1; i++) {
		for (j = 0; j < DDP_MOUT_NUM; j++) {
			if (module_list[i] == mout_map[j].id) {
				...
				if (valid) {
					valid = 0;
					if ((DISP_REG_GET(mout_map[
							j].reg) & mout) == 0) {
						path_error += 1;
						DDPDUMP(
						"error:%s mout,e=0x%x,r=0x%x\n",
							ddp_get_module_name(
								module_list[i]),
							mout, DISP_REG_GET(
								mout_map[
						...
						mout_map[j].reg) != mout) {
						DDPDUMP(
						"warn:%s mout e=0x%x,r=0x%x\n",
						     ddp_get_module_name(
							     module_list[i]),
						     mout,
						     DISP_REG_GET(mout_map[
							     j].reg));
					}
				}
				break;
	...
	for (i = 0; i < module_num - 1; i++) {
		for (j = 0; j < DDP_SEL_OUT_NUM; j++) {
			...
			for (k = 0; k < 4; k++) {
				if (sel_out_map[j].id_bit_map[k] == -1)
					break;
				if (sel_out_map[j].id_bit_map[k] == module_list[
						step]) {
					if (DISP_REG_GET(sel_out_map[j].reg) !=
							k) {
						path_error += 1;
						DDPDUMP(
						"err:out_s %s not connect2 %s,",
						     ddp_get_module_name(
							     module_list[i]),
						     ddp_get_module_name(
							     module_list[
							     step]));
						DDPDUMP("e=0x%x,r=0x%x\n",
						     k,
						     DISP_REG_GET(sel_out_map[
							     j].reg));
					}
					break;
				}
...
static void ddp_disconnect_path_l(int *module_list, void *handle)
{
	...
	for (i = 0; i < module_num - 1; i++) {
		...
		if (reg_mout) {
			DISP_REG_SET(handle, mout_map[mout_idx].reg, mout_map[
					mout_idx].reg_val);
			reg_mout = 0;
			mout_idx = 0;
		}
		...
		enum DDP_MODE ddp_mode)
{
	...
	case DISP_MODULE_DPI0:
		{
			mode = SOF_DPI0;
			/* SOF_DPI0;   has one DSI,
			 * so the DPI should use 1 for MUTEX_SOF
			 */
			break;
		}
	...
	case DISP_MODULE_WDMA1:
		...
		break;
	default:
		DDPERR("%s, invalid param dst module = %s(%d), dis mode %s\n",
				__func__,
		...
		enum DDP_MODE ddp_mode, void *handle)
{
	...
	enum MUTEX_SOF mode = ddp_get_mutex_sof(module_list[module_num - 1],
			ddp_mode);
	...
	if (gEnableSWTrigger == 1 && primary_display_is_video_mode() == 1 &&
		...
		DISP_REG_SET(handle, DISP_REG_CONFIG_MUTEX_MOD(
					DISP_OVL_SEPARATE_MUTEX_ID),
			     value & ((1 << module_mutex_map[
					     DISP_MODULE_OVL0].bit) |
				      (1 << module_mutex_map[
				       DISP_MODULE_OVL1].bit)));
		DISP_REG_SET(handle, DISP_REG_CONFIG_MUTEX_SOF(
					DISP_OVL_SEPARATE_MUTEX_ID),
			     SOF_SINGLE);
		...
		DISP_REG_SET(handle, DISP_REG_CONFIG_MUTEX_MOD(mutex_id),
			     value &
			     (~
			      ((1 << module_mutex_map[DISP_MODULE_OVL0].bit) |
			       (1 << module_mutex_map[
					DISP_MODULE_OVL1].bit))));
	}
...
{
	if (gMutexFreeRun == 1 && primary_display_is_video_mode() == 0 &&
			mutex_idx == 0)
		      ...
		      enum DISP_MODULE_ENUM module)
{
	...
	for (i = 0; i < DDP_ENING_NUM; i++) {
		if (module_list_scenario[ddp_scenario][i] == module) {
			DDPERR("err:%s, module=%s is already in scenario %s!\n",
					__func__,
					ddp_get_module_name(module),
					ddp_get_scenario_name(ddp_scenario));
			return -1;
...
void ddp_connect_path(enum DDP_SCENARIO_ENUM scenario, void *handle)
{
	DDPDBG("path connect on scenario %s\n",
			ddp_get_scenario_name(scenario));
	if (scenario == DDP_SCENARIO_PRIMARY_ALL) {
		ddp_connect_path_l(module_list_scenario[
				DDP_SCENARIO_PRIMARY_DISP], handle);
		ddp_connect_path_l(module_list_scenario[
				DDP_SCENARIO_PRIMARY_OVL_MEMOUT], handle);
	} else if (scenario == DDP_SCENARIO_SUB_ALL) {
		ddp_connect_path_l(module_list_scenario[
				DDP_SCENARIO_SUB_DISP], handle);
		ddp_connect_path_l(module_list_scenario[
				DDP_SCENARIO_SUB_OVL_MEMOUT], handle);
	} else {
...
void ddp_disconnect_path(enum DDP_SCENARIO_ENUM scenario, void *handle)
{
	...
	if (scenario == DDP_SCENARIO_PRIMARY_ALL) {
		ddp_disconnect_path_l(module_list_scenario[
				DDP_SCENARIO_PRIMARY_DISP], handle);
		ddp_disconnect_path_l(module_list_scenario[
...
void ddp_check_path(enum DDP_SCENARIO_ENUM scenario)
{
	...
	if (scenario == DDP_SCENARIO_PRIMARY_ALL) {
		ddp_check_path_l(module_list_scenario[
		...
		enum DDP_MODE mode, void *handle)
{
	if (scenario < DDP_SCENARIO_MAX)
		return ddp_mutex_set_l(mutex_id,
				module_list_scenario[scenario], mode, handle);
...
{
	/* check engines' clock bit &
	 * enable bit & status bit before unlock mutex
	 */
...
#if 0
	pm_runtime_enable(disp_get_device());
	pm_runtime_get_sync(disp_get_device());
#endif
	dispsysdev =
		(struct dispsys_device *) dev_get_drvdata(disp_get_device());

	pm_runtime_get_sync(&dispsysdev->larb_pdev[0]->dev);
#endif
...
int ddp_path_top_clock_off(void)
{
	...
	pm_runtime_put_sync(&dispsysdev->larb_pdev[0]->dev);
#endif

#if 0
	pm_runtime_put_sync(disp_get_device());
#endif
