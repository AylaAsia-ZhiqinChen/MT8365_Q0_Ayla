...
void init_ddp_mmp_events(void)
{
	if (DDP_MMP_Events.DDP == 0) {
		DDP_MMP_Events.DDP = mmprofile_register_event(MMP_ROOT_EVENT,
				"Display");
	...
	if (session == 1) {
		if (ovl_get_status() == DDP_OVL1_STATUS_PRIMARY
			...
			if (pLayer->layer < 4)
				...
				mmprofile_log_meta(DDP_MMP_Events.layer_dump[
						pLayer->layer % 4],
						MMPROFILE_FLAG_PULSE, meta);
...
get_pages_done:
	if (k < page_num) {
		 ...
		 */
		DDPERR("%s:only get %d pages:mva=0x%lx, size=0x%x, pg_num=%d\n",
				__func__,
				k,
				mva, size, page_num);
		ret = -1;
...
void *ddp_map_mva_to_va(unsigned long mva, unsigned int size)
{
	...
	if (dev) {
		...
		pages = kmalloc((sizeof(struct page *) * page_count),
				GFP_KERNEL);
		DDPMSG("page count: %d\n", page_count);
		...
		DDPMSG("%s: mva=0x%lx, pa=0x%lx, va=0x%p, size=%d\n",
				__func__,
		...
		unsigned int session /*1:primary, 2:external, 3:memory */)
{
	...
	if (pLayer->layer_en) {
		...
		case eABGR8888:
			...
			break;
		default:
			DDPERR("%s, unknown fmt=0x%x, %s, dump raw\n",
					__func__,
					pLayer->fmt,
		...
		if (!raw) {
			...
			Bitmap.p_data = ddp_map_mva_to_va(pLayer->addr,
					Bitmap.data_size);
			if (Bitmap.p_data) {
				_ddp_mmp_ovl_not_raw_under_session(session,
						pLayer, &Bitmap);
			...
			meta.p_data = ddp_map_mva_to_va(pLayer->addr,
					meta.size);
			if (meta.p_data) {
				_ddp_mmp_ovl_raw_under_session(session,
						pLayer, &meta);
		...
		unsigned int down_sample_x, unsigned int down_sample_y)
{
	...
	if (!raw) {
		...
		Bitmap.down_sample_x = down_sample_x;
		Bitmap.down_sample_y = down_sample_y;
		Bitmap.p_data = ddp_map_mva_to_va(wdma_layer->dstAddress,
				Bitmap.data_size);
		if (Bitmap.p_data) {
			mmprofile_log_meta_bitmap(DDP_MMP_Events.wdma_dump[
					wdma_num],
					MMPROFILE_FLAG_PULSE, &Bitmap);
			ddp_umap_va(Bitmap.p_data);
		} else {
			DDPERR("dprec_mmp_dump_wdma_layer,err2dump rgb(0x%x)\n",
					wdma_layer->outputFormat);
		}
	} else {
		meta.data_type = MMPROFILE_META_RAW;
		meta.size = wdma_layer->dstPitch * wdma_layer->srcHeight;
		meta.p_data = ddp_map_mva_to_va(wdma_layer->dstAddress,
				meta.size);
		if (meta.p_data) {
			...
			DDPERR("dprec_mmp_dump_wdma_layer,err2dump raw(0x%x)\n",
			       wdma_layer->outputFormat);
	}
}
		...
		unsigned int down_sample_x, unsigned int down_sample_y)
{
	...
	if (!raw) {
		...
		Bitmap.down_sample_x = down_sample_x;
		Bitmap.down_sample_y = down_sample_y;
		Bitmap.p_data = ddp_map_mva_to_va(rdma_layer->address,
				Bitmap.data_size);
		if (Bitmap.p_data) {
			mmprofile_log_meta_bitmap(DDP_MMP_Events.rdma_dump[
					rdma_num],
					MMPROFILE_FLAG_PULSE, &Bitmap);
			ddp_umap_va(Bitmap.p_data);
		} else {
		...
		if (meta.p_data) {
			...
			DDPERR("dprec_mmp_dump_rdma_layer,err2dump raw(0x%x)\n",
			       rdma_layer->inputFormat);
		}
	}
}
