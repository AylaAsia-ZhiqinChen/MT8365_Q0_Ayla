...
#define PDSI_PHY_LCPAT_REG (DSI_PHY_LCPAT_REG *)
...
struct DSI_PHY_REGS {
	...
	struct MIPITX_DSI_CLOCK_LANE_REG MIPITX_DSI0_CLOCK_LANE;
	/* 0004 */
	struct MIPITX_DSI_DATA_LANE0_REG MIPITX_DSI0_DATA_LANE0;
	/* 0008 */
	struct MIPITX_DSI_DATA_LANE1_REG MIPITX_DSI0_DATA_LANE1;
	/* 000C */
	struct MIPITX_DSI_DATA_LANE2_REG MIPITX_DSI0_DATA_LANE2;
	/* 0010 */
	struct MIPITX_DSI_DATA_LANE3_REG MIPITX_DSI0_DATA_LANE3;
	/* 0014 */
	...
	struct MIPITX_DSI_SW_CTRL_CON0_REG MIPITX_DSI_SW_CTRL_CON0;
	/* 0084 */
	struct MIPITX_DSI_SW_CTRL_CON1_REG MIPITX_DSI_SW_CTRL_CON1;
	/* 0088 */
...
#define READ_REGISTER_UINT32(reg) \
	(*(unsigned long * const)(reg))

#define WRITE_REGISTER_UINT32(reg, val) \
	((*(unsigned long * const)(reg)) = (val))

#define READ_REGISTER_UINT16(reg) \
	(*(uint16_t * const)(reg))

#define WRITE_REGISTER_UINT16(reg, val) \
	((*(uint16_t * const)(reg)) = (val))

#define READ_REGISTER_UINT8(reg) \
	(*(uint8_t * const)(reg))

#define WRITE_REGISTER_UINT8(reg, val) \
	((*(uint8_t * const)(reg)) = (val))
#endif
...
#define OUTREG16(x, y) \
	WRITE_REGISTER_UINT16((uint16_t *)(x), (uint16_t)(y))
...
#define DISP_REG_GET_FIELD(field, reg32) \
	((__raw_readl((void *)(reg32)) & REG_FLD_MASK(field)) \
	 >> REG_FLD_SHIFT(field))
...
#define DISP_CPU_REG_SET_FIELD(field, reg32, val)  \
	do {                                \
		if (0) \
			...
			val<<REG_FLD_SHIFT(field), REG_FLD_MASK(field));\
		mt_reg_sync_writel(((unsigned int)(*(unsigned long *) \
					(reg32)) & ~REG_FLD_MASK(field))\
		| REG_FLD_VAL((field) \
			, (val)), (unsigned long *)reg32);  \
	} while (0)
...
#define DISP_REG_MASK(handle, reg32, val, mask)	\
	do { \
		...
		if (handle == NULL) \
			mt_reg_sync_writel((unsigned int) \
				(INREG32(reg32)&~(mask))|(val), \
				(unsigned long *)(reg32));\
		else \
			cmdqRecWrite(handle \
			, disp_addr_convert((unsigned long)reg32), val, mask); \
	} while (0)
...
#define DISP_REG_SET(handle, reg32, val) \
	do { \
		...
		if (handle == NULL) {\
			mt_reg_sync_writel(val, (unsigned long *) \
					(reg32));\
		} \
		else {\
...
#define DISP_REG_SET_FIELD(handle, field, reg32, val)  \
	do {  \
		...
		if (handle == NULL) { \
			mt_reg_sync_writel((__raw_readl(reg32) \
						& ~REG_FLD_MASK(field)) \
			...
			dprec_reg_op((void *)handle, (unsigned long)(reg32) \
					, val << REG_FLD_SHIFT(field),\
				     REG_FLD_MASK(field));\
			cmdqRecWrite(handle, disp_addr_convert((unsigned long) \
						reg32), \
				val<<REG_FLD_SHIFT(field) \
					, REG_FLD_MASK(field)); \
		} \
...
#define DISP_REG_SET(handle, reg32, val) \
	do { \
		if (handle == NULL) {\
			...
			dprec_reg_op((void *)handle, (unsigned long) \
					reg32, val, 0x00000000);\
			cmdqRecWrite(handle, disp_addr_convert((unsigned long) \
						reg32), val, ~0); \
		}  \
	} while (0)
...
#define DISP_REG_SET_FIELD(handle, field, reg32, val)  \
	do {  \
		if (handle == NULL) { \
			mt_reg_sync_writel((__raw_readl((void *)(reg32) \
						) & ~REG_FLD_MASK(field)) \
					|REG_FLD_VAL((field), \
				(val)), (reg32)); \
		} \
...
#define DISP_REG_CMDQ_POLLING(handle, reg32, val, mask) \
	do { \
		if (handle == NULL) {\
			...
			dprec_reg_op((void *)handle, (unsigned long) \
					reg32, val, 0x00000000);\
			cmdqRecPoll(handle, disp_addr_convert((unsigned long) \
						reg32), val, mask); \
		}  \
	} while (0)
...
#define DISP_REG_SET_DIRTY(handle, reg32, val) \
	do { \
		if (gEnableReduceRegWrite == 0 || DISP_REG_GET(reg32) \
				!= (val)) { \
			if (handle == NULL) { \
...
#define DISP_AAL_MAX_HIST_CONFIG_00 \
	(DISPSYS_AAL_BASE + 0x204)
...
#define CFG_MAIN_FLD_ALL				REG_FLD(1, 7)
...
#define DISP_REG_CONFIG_PWM_APB_ERR_ADDR \
	(DISPSYS_CONFIG_BASE + 0xc)
#define DISP_REG_CONFIG_ISP_MOUT_EN \
	(DISPSYS_CONFIG_BASE + 0x01c)
#define DISP_REG_CONFIG_MDP_RDMA0_MOUT_EN \
	(DISPSYS_CONFIG_BASE + 0x020)
#define DISP_REG_CONFIG_MDP_PRZ0_MOUT_EN \
	(DISPSYS_CONFIG_BASE + 0x024)
#define DISP_REG_CONFIG_MDP_PRZ1_MOUT_EN \
	(DISPSYS_CONFIG_BASE + 0x028)
#define DISP_REG_CONFIG_MDP_TDSHP0_MOUT_EN \
	(DISPSYS_CONFIG_BASE + 0x02C)
#define DISP_REG_CONFIG_DISP_OVL0_MOUT_EN \
	(DISPSYS_CONFIG_BASE + 0x030)
#define DISP_REG_CONFIG_DISP_DITHER_MOUT_EN      \
	(DISPSYS_CONFIG_BASE + 0x038)
#define DISP_REG_CONFIG_DISP_UFOE_MOUT_EN	 \
	(DISPSYS_CONFIG_BASE + 0x03C)
#define DISP_REG_CONFIG_MMSYS_MOUT_RST		 \
	(DISPSYS_CONFIG_BASE + 0x040)
#define DISP_REG_CONFIG_MDP_PRZ0_SEL_IN		 \
	(DISPSYS_CONFIG_BASE + 0x044)
#define DISP_REG_CONFIG_MDP_PRZ1_SEL_IN		 \
	(DISPSYS_CONFIG_BASE + 0x048)
#define DISP_REG_CONFIG_MDP_TDSHP0_SEL_IN		 \
	(DISPSYS_CONFIG_BASE + 0x04C)
#define DISP_REG_CONFIG_MDP_WDMA0_SEL_IN		 \
	(DISPSYS_CONFIG_BASE + 0x050)
#define DISP_REG_CONFIG_MDP_WROT0_SEL_IN		 \
	(DISPSYS_CONFIG_BASE + 0x054)
#define DISP_REG_CONFIG_DISP_COLOR0_SEL_IN		 \
	(DISPSYS_CONFIG_BASE + 0x058)
#define DISP_REG_CONFIG_DISP_WDMA0_SEL_IN		 \
	(DISPSYS_CONFIG_BASE + 0x05C)
#define DISP_REG_CONFIG_DISP_UFOE_SEL_IN		 \
	(DISPSYS_CONFIG_BASE + 0x060)
#define DISP_REG_CONFIG_DSI0_SEL_IN			 \
	(DISPSYS_CONFIG_BASE + 0x064)
#define DISP_REG_CONFIG_DPI0_SEL_IN			 \
	(DISPSYS_CONFIG_BASE + 0x068)
#define DISP_REG_CONFIG_DISP_RDMA0_SOUT_SEL_IN       \
	(DISPSYS_CONFIG_BASE + 0x06C)
#define DISP_REG_CONFIG_DISP_RDMA1_SOUT_SEL_IN	 \
	(DISPSYS_CONFIG_BASE + 0x070)
#define DISP_REG_CONFIG_DPI1_SEL_IN			 \
	(DISPSYS_CONFIG_BASE + 0x074)
...
#define DISP_REG_CONFIG_MMSYS_HW_DCM_DIS0		 \
	(DISPSYS_CONFIG_BASE + 0x120)
#define DISP_REG_CONFIG_MMSYS_HW_DCM_DIS_SET0               \
	(DISPSYS_CONFIG_BASE + 0x124)
#define DISP_REG_CONFIG_MMSYS_HW_DCM_DIS_CLR0              \
...
#define DISP_REG_CONFIG_DISP_FAKE_ENG_EN		 \
	(DISPSYS_CONFIG_BASE + 0x200)
#define DISP_REG_CONFIG_DISP_FAKE_ENG_RST		 \
	(DISPSYS_CONFIG_BASE + 0x204)
#define DISP_REG_CONFIG_DISP_FAKE_ENG_CON0		 \
	(DISPSYS_CONFIG_BASE + 0x208)
#define DISP_REG_CONFIG_DISP_FAKE_ENG_CON1		 \
	(DISPSYS_CONFIG_BASE + 0x20c)
#define DISP_REG_CONFIG_DISP_FAKE_ENG_RD_ADDR		   \
	(DISPSYS_CONFIG_BASE + 0x210)
#define DISP_REG_CONFIG_DISP_FAKE_ENG_WR_ADDR		  \
	(DISPSYS_CONFIG_BASE + 0x214)
#define DISP_REG_CONFIG_DISP_FAKE_ENG_STATE		 \
	(DISPSYS_CONFIG_BASE + 0x218)
#define DISP_REG_CONFIG_MMSYS_MBIST_CON		 \
	(DISPSYS_CONFIG_BASE + 0x800)
#define DISP_REG_CONFIG_MMSYS_MBIST_DONE		 \
	(DISPSYS_CONFIG_BASE + 0x804)
#define DISP_REG_CONFIG_MMSYS_MBIST_HOLDB		 \
	(DISPSYS_CONFIG_BASE + 0x808)
#define DISP_REG_CONFIG_MMSYS_MBIST_MODE		 \
	(DISPSYS_CONFIG_BASE + 0x80c)
#define DISP_REG_CONFIG_MMSYS_MBIST_FAIL0		 \
	(DISPSYS_CONFIG_BASE + 0x810)
#define DISP_REG_CONFIG_MMSYS_MBIST_FAIL1		 \
	(DISPSYS_CONFIG_BASE + 0x814)
#define DISP_REG_CONFIG_MMSYS_MBIST_BSEL0			 \
	(DISPSYS_CONFIG_BASE + 0x820)
#define DISP_REG_CONFIG_MMSYS_MBIST_BSEL1			 \
	(DISPSYS_CONFIG_BASE + 0x824)
#define DISP_REG_CONFIG_MMSYS_MEM_DELSEL0		 \
	(DISPSYS_CONFIG_BASE + 0x830)
#define DISP_REG_CONFIG_MMSYS_MEM_DELSEL1		 \
	(DISPSYS_CONFIG_BASE + 0x834)
#define DISP_REG_CONFIG_MMSYS_MEM_DELSEL2		 \
	(DISPSYS_CONFIG_BASE + 0x838)
#define DISP_REG_CONFIG_MMSYS_MEM_DELSEL3		 \
	(DISPSYS_CONFIG_BASE + 0x83c)
#define DISP_REG_CONFIG_MMSYS_DEBUG_OUT_SEL		 \
	(DISPSYS_CONFIG_BASE + 0x880)
#define DISP_REG_CONFIG_MMSYS_DUMMY			 \
	(DISPSYS_CONFIG_BASE + 0x890)
#define DISP_REG_CONFIG_DISP_DL_VALID_0			 \
	(DISPSYS_CONFIG_BASE + 0x8a0)
#define DISP_REG_CONFIG_DISP_DL_READY_0			 \
	(DISPSYS_CONFIG_BASE + 0x8a4)
#define DISP_REG_CONFIG_MDP_DL_VALID_0			 \
	(DISPSYS_CONFIG_BASE + 0x8a8)
#define DISP_REG_CONFIG_MDP_DL_READY_0			 \
	(DISPSYS_CONFIG_BASE + 0x8ac)
#define DISP_REG_CONFIG_SMI_LARB0_GREQ			 \
	(DISPSYS_CONFIG_BASE + 0x8d0)
#if 0	/* for debug*/
#define DISP_REG_CONFIG_C08                      \
	(DISPSYS_CONFIG_BASE + 0xc08)
#define DISP_REG_CONFIG_C09                       \
	(DISPSYS_CONFIG2_BASE + 0x40)	/* 0x10206040 */
#define DISP_REG_CONFIG_C10                      \
	(DISPSYS_CONFIG2_BASE + 0x44)	/* 0x10206044 */
#define DISP_REG_CONFIG_C11                     \
	(DISPSYS_CONFIG3_BASE + 0x40)	/* 0x10210040 */
#define DISP_REG_CONFIG_C12                     \
	(DISPSYS_CONFIG3_BASE + 0x44)	/* 0x10210044 */
#define DISP_REG_CONFIG_C13                      \
	(DISPSYS_CONFIG3_BASE + 0x48)	/* 0x10210048 */
#endif
#define DISP_REG_CLK_CFG_0_MM_CLK                 \
	(DISPSYS_CONFIG_BASE+0x40)	/* BIT31 0 */
...
#define MMSYS_MISC_FLD_SMI_LARB0_TEST_MODE				 \
	REG_FLD(1, 0)
...
#define ELA2GMC_BASE_ADDR_END_FLD_ELA2GMC_BASE_ADDR_END		 \
	REG_FLD(32, 0)
#define ELA2GMC_FINAL_ADDR_FLD_ELA2GMC_FINAL_ADDR		REG_FLD(32, 0)
#define ELA2GMC_STATUS_FLD_ELA2GMC_STATUS			 \
	REG_FLD(1, 0)
...
#define DISP_FAKE_ENG_CON0_FLD_DFE_LOOP_MODE		 \
	REG_FLD(1, 22)
...
#define DISP_FAKE_ENG_RD_ADDR_FLD_DISP_FAKE_ENG_RD_ADDR		 \
	REG_FLD(32, 0)
...
#define MMSYS_MBIST_CON_FLD_MMSYS_MBIST_BACKGROUND		 \
	REG_FLD(3, 16)
#define MMSYS_MBIST_CON_FLD_MMSYS_MBIST_RSTB			REG_FLD(1, 15)
#define MMSYS_MBIST_CON_FLD_MMSYS_MBIST_SCANOUT_SEL		 \
	REG_FLD(4, 8)
#define MMSYS_MBIST_CON_FLD_MMSYS_MBIST_DEBUG			REG_FLD(1, 7)
#define MMSYS_MBIST_CON_FLD_MMSYS_MBIST_FAILOUT_SEL		 \
	REG_FLD(6, 0)
...
#define MMSYS_MBIST_BSEL0_FLD_MDP_TDSHP_MBIST_BSEL		 \
	REG_FLD(16, 0)
...
#define MMSYS_MBIST_BSEL1_FLD_MDP_RSZ0_MBIST_BSEL		 \
	REG_FLD(12, 0)
#define MMSYS_MBIST_BSEL1_FLD_MDP_RSZ1_MBIST_BSEL		 \
	REG_FLD(12, 12)
...
#define MMSYS_MEM_DELSEL1_FLD_MDP_WROT_MEM_DELSEL		 \
	REG_FLD(22, 0)
...
/* ------------------------------------------------------------- */
/* SMI_LARB */
#define DISP_REG_SMI_LARB_MMU_EN				 \
	(DISPSYS_SMI_LARB0_BASE+0xfc0)

/* ------------------------------------------------------------- */
/* DSI */
#define DISP_REG_DSI_STATE					 \
	(DISPSYS_DSI0_BASE + 0x004)

/* ------------------------------------------------------------- */
/* DPI */
#if 0
#define DISP_REG_DPI_EN						 \
	(DISPSYS_DPI_BASE + 0x000)
#define DISP_REG_DPI_RST				 \
	(DISPSYS_DPI_BASE + 0x004)
#define DISP_REG_DPI_INTEN				 \
	(DISPSYS_DPI_BASE + 0x008)
#define DISP_REG_DPI_INSTA				 \
	(DISPSYS_DPI_BASE + 0x00C)
#define DISP_REG_DPI_CON				 \
	(DISPSYS_DPI_BASE + 0x010)
#define DISP_REG_DPI_OUTPUT_SETTING				 \
	(DISPSYS_DPI_BASE + 0x014)
#define DISP_REG_DPI_SIZE				 \
	(DISPSYS_DPI_BASE + 0x018)
#define DISP_REG_DPI_DDR_SETTING			 \
	(DISPSYS_DPI_BASE + 0x01c)
#define DISP_REG_DPI_TGEN_HWIDTH			 \
	(DISPSYS_DPI_BASE + 0x020)
#define DISP_REG_DPI_TGEN_HPORCH			 \
	(DISPSYS_DPI_BASE + 0x024)
#define DISP_REG_DPI_TGEN_VWIDTH			 \
	(DISPSYS_DPI_BASE + 0x028)
#define DISP_REG_DPI_TGEN_VPORCH			 \
	(DISPSYS_DPI_BASE + 0x02C)
#define DISP_REG_DPI_BG_HCNTL				 \
	(DISPSYS_DPI_BASE + 0x030)
#define DISP_REG_DPI_BG_VCNTL				 \
	(DISPSYS_DPI_BASE + 0x034)
#define DISP_REG_DPI_BG_COLOR				 \
	(DISPSYS_DPI_BASE + 0x038)
#define DISP_REG_DPI_FIFO_CTL					 \
	(DISPSYS_DPI_BASE + 0x03C)
#define DISP_REG_DPI_STATUS						 \
	(DISPSYS_DPI_BASE + 0x040)
#define DISP_REG_DPI_TMODE					 \
	(DISPSYS_DPI_BASE + 0x044)
#define DISP_REG_DPI_CHKSUM					 \
	(DISPSYS_DPI_BASE + 0x048)
#define DISP_REG_DPI_DUMMY					 \
	(DISPSYS_DPI_BASE + 0x050)
#define DISP_REG_DPI_TGEN_VWIDTH_LEVEN			 \
	(DISPSYS_DPI_BASE + 0x068)
#define DISP_REG_DPI_TGEN_VPORCH_LEVEN			 \
	(DISPSYS_DPI_BASE + 0x06c)
#define DISP_REG_DPI_TGEN_VWIDTH_RODD			 \
	(DISPSYS_DPI_BASE + 0x070)
#define DISP_REG_DPI_TGEN_VPORCH_RODD			 \
	(DISPSYS_DPI_BASE + 0x074)
#define DISP_REG_DPI_TGEN_VWIDTH_REVEN			 \
	(DISPSYS_DPI_BASE + 0x078)
#define DISP_REG_DPI_TGEN_VPORCH_REVEN			 \
	(DISPSYS_DPI_BASE + 0x07c)
#define DISP_REG_DPI_ESAV_VTIM_LODD			 \
	(DISPSYS_DPI_BASE + 0x080)
#define DISP_REG_DPI_ESAV_VTIM_LEVEN			 \
	(DISPSYS_DPI_BASE + 0x084)
#define DISP_REG_DPI_ESAV_VTIM_RODD			 \
	(DISPSYS_DPI_BASE + 0x088)
#define DISP_REG_DPI_ESAV_VTIM_REVEN			 \
	(DISPSYS_DPI_BASE + 0x08C)
#define DISP_REG_DPI_ESAV_FTIM				 \
	(DISPSYS_DPI_BASE + 0x090)
#define DISP_REG_DPI_CLPF_SETTING			 \
	(DISPSYS_DPI_BASE + 0x094)
#define DISP_REG_DPI_Y_LIMIT				 \
	(DISPSYS_DPI_BASE + 0x098)
#define DISP_REG_DPI_C_LIMIT				 \
	(DISPSYS_DPI_BASE + 0x09C)
#define DISP_REG_DPI_YUV422_SETTING			 \
	(DISPSYS_DPI_BASE + 0x0A0)
#define DISP_REG_DPI_EMBSYNC_SETTING			 \
	(DISPSYS_DPI_BASE + 0x0A4)
#define DISP_REG_DPI_ESAV_CODE_SET0			 \
	(DISPSYS_DPI_BASE + 0x0A8)
#define DISP_REG_DPI_ESAV_CODE_SET1			 \
	(DISPSYS_DPI_BASE + 0x0AC)
...
#define INSTA_FLD_INTSTA_VSYNC_EN				 \
	REG_FLD(1, 0)
#define CON_FLD_IN_RB_SWAP					 \
	REG_FLD(1, 1)
#define CON_FLD_BG_ENABLE				 \
	REG_FLD(1, 0)
#define OUTPUT_SETTING_FLD_EDGE_SEL				 \
	REG_FLD(1, 17)
#define OUTPUT_SETTING_FLD_OEN_OFF				REG_FLD(1, 16)
#define OUTPUT_SETTING_FLD_CK_POL			 \
	REG_FLD(1, 15)
#define OUTPUT_SETTING_FLD_VSYNC_POL				REG_FLD(1, 14)
#define OUTPUT_SETTING_FLD_HSYNC_POL				REG_FLD(1, 13)
#define OUTPUT_SETTING_FLD_DE_POL			 \
	REG_FLD(1, 12)
#define OUTPUT_SETTING_FLD_VS_MASK			 \
	REG_FLD(1, 10)
#define OUTPUT_SETTING_FLD_HS_MASK				REG_FLD(1, 9)
#define OUTPUT_SETTING_FLD_DE_MASK				REG_FLD(1, 8)
#define OUTPUT_SETTING_FLD_R_MASK			 \
	REG_FLD(1, 6)
...
#define OUTPUT_SETTING_FLD_CH_SWAP				 \
	REG_FLD(3, 0)
#define DPI_SIZE_FLD_HSIZE					 \
	REG_FLD(11, 16)
#define DPI_SIZE_FLD_VSIZE					 \
	REG_FLD(11, 0)
#define TGEN_HWIDTH_FLD_HPW						 \
	REG_FLD(12, 0)
#define TGEN_HPORCH_FLD_HFP					 \
	REG_FLD(12, 16)
#define TGEN_HPORCH_FLD_HBP					 \
	REG_FLD(12, 0)
#define TGEN_VWIDTH_FLD_VPW					 \
	REG_FLD(8, 0)
#define TGEN_VPORCH_FLD_VFP						 \
	REG_FLD(8, 16)
#define TGEN_VPORCH_FLD_VBP					 \
	REG_FLD(8, 0)
#define BG_HCNTL_FLD_BG_LEFT						 \
	REG_FLD(11, 16)
#define BG_HCNTL_FLD_BG_RIGHT					 \
	REG_FLD(11, 0)
#define BG_VCNTL_FLD_BG_TOP					 \
	REG_FLD(11, 16)
#define BG_VCNTL_FLD_BG_BOT					 \
	REG_FLD(11, 0)
#define BG_COLOR_FLD_BG_R				 \
	REG_FLD(8, 16)
#define BG_COLOR_FLD_BG_G					 \
	REG_FLD(8, 8)
#define BG_COLOR_FLD_BG_B				 \
	REG_FLD(8, 0)
#define FIFO_CTL_FLD_FIFO_RST_SEL				 \
	REG_FLD(1, 8)
#define FIFO_CTL_FLD_FIFO_VALID_SET			 \
	REG_FLD(5, 0)
#define STATUS_FLD_OUTEN				 \
	REG_FLD(1, 17)
#define STATUS_FLD_DPI_BUSY			 \
	REG_FLD(1, 16)
#define STATUS_FLD_V_COUNTER			 \
	REG_FLD(13, 0)
#define TMODE_FLD_DPI_OEN_ON			 \
	REG_FLD(1, 0)
#define CHKSUM_FLD_DPI_CHKSUM_EN		 \
	REG_FLD(1, 31)
#define CHKSUM_FLD_DPI_CHKSUM_READY		 \
	REG_FLD(1, 30)
#define CHKSUM_FLD_DPI_CHKSUM			 \
	REG_FLD(24, 0)
#define PATTERN_FLD_PAT_R_MAN			 \
	REG_FLD(8, 24)
#define PATTERN_FLD_PAT_G_MAN			 \
	REG_FLD(8, 16)
#define PATTERN_FLD_PAT_B_MAN			 \
	REG_FLD(8, 8)
#define PATTERN_FLD_PAT_SEL			 \
	REG_FLD(3, 4)
#define PATTERN_FLD_PAT_EN			 \
	REG_FLD(1, 0)
#endif
/* CCORR */
#define DISP_REG_CCORR_EN                             \
	(DISPSYS_CCORR_BASE + 0x000)
#define DISP_REG_CCORR_RESET                              \
	(DISPSYS_CCORR_BASE + 0x004)
#define DISP_REG_CCORR_INTEN                             \
	(DISPSYS_CCORR_BASE + 0x008)
#define DISP_REG_CCORR_INTSTA                           \
	(DISPSYS_CCORR_BASE + 0x00c)
#define DISP_REG_CCORR_STATUS                           \
	(DISPSYS_CCORR_BASE + 0x010)
#define DISP_REG_CCORR_CFG                              \
	(DISPSYS_CCORR_BASE + 0x020)
#define DISP_REG_CCORR_IN_CNT                            \
	(DISPSYS_CCORR_BASE + 0x024)
#define DISP_REG_CCORR_OUT_CNT                        \
	(DISPSYS_CCORR_BASE + 0x028)
#define DISP_REG_CCORR_CHKSUM                         \
	(DISPSYS_CCORR_BASE + 0x02c)
#define DISP_REG_CCORR_SIZE                           \
	(DISPSYS_CCORR_BASE + 0x030)
#define DISP_REG_CCORR_COEF_0                          \
	(DISPSYS_CCORR_BASE + 0x080)
#define DISP_REG_CCORR_DUMMY_REG                       \
	(DISPSYS_CCORR_BASE + 0x0c0)
...
/* ------------------------------------------------------------- */
/* GAMMA */
#define DISP_REG_GAMMA_EN				 \
	(DISPSYS_GAMMA_BASE + 0x000)
#define DISP_REG_GAMMA_RESET				 \
	(DISPSYS_GAMMA_BASE + 0x004)
#define DISP_REG_GAMMA_INTEN				 \
	(DISPSYS_GAMMA_BASE + 0x008)
#define DISP_REG_GAMMA_INTSTA					 \
	(DISPSYS_GAMMA_BASE + 0x00c)
#define DISP_REG_GAMMA_STATUS				 \
	(DISPSYS_GAMMA_BASE + 0x010)
#define DISP_REG_GAMMA_CFG				 \
	(DISPSYS_GAMMA_BASE + 0x020)
#define DISP_REG_GAMMA_INPUT_COUNT			 \
	(DISPSYS_GAMMA_BASE + 0x024)
#define DISP_REG_GAMMA_OUTPUT_COUNT				 \
	(DISPSYS_GAMMA_BASE + 0x028)
#define DISP_REG_GAMMA_CHKSUM				 \
	(DISPSYS_GAMMA_BASE + 0x02c)
#define DISP_REG_GAMMA_SIZE					 \
	(DISPSYS_GAMMA_BASE + 0x030)
#define DISP_REG_GAMMA_DUMMY_REG			 \
	(DISPSYS_GAMMA_BASE + 0x0c0)
#define DISP_REG_GAMMA_LUT					 \
	(DISPSYS_GAMMA_BASE + 0x700)
...
#define CFG_FLD_CHKSUM_EN					 \
	REG_FLD(1, 28)
#define CFG_FLD_CCORR_GAMMA_OFF			REG_FLD(1, 5)
#define CFG_FLD_CCORR_EN					 \
	REG_FLD(1, 4)
#define CFG_FLD_DITHER_EN					 \
	REG_FLD(1, 2)
...
#define SIZE_FLD_HSIZE						 \
	REG_FLD(13, 16)
#define SIZE_FLD_VSIZE					 \
	REG_FLD(13, 0)
...
/* ------------------------------------------------------------- */
/* Dither */
#define DISP_REG_DITHER_EN                                      \
	(DISPSYS_DITHER_BASE + 0x000)
#define DISP_REG_DITHER_RESET                                 \
	(DISPSYS_DITHER_BASE + 0x004)
#define DISP_REG_DITHER_INTEN                                \
	(DISPSYS_DITHER_BASE + 0x008)
#define DISP_REG_DITHER_INTSTA                               \
	(DISPSYS_DITHER_BASE + 0x00c)
#define DISP_REG_DITHER_STATUS                                 \
	(DISPSYS_DITHER_BASE + 0x010)
#define DISP_REG_DITHER_CFG                                   \
	(DISPSYS_DITHER_BASE + 0x020)
#define DISP_REG_DITHER_IN_CNT                               \
	(DISPSYS_DITHER_BASE + 0x024)
#define DISP_REG_DITHER_OUT_CNT                              \
	(DISPSYS_DITHER_BASE + 0x028)
#define DISP_REG_DITHER_CHKSUM                                \
	(DISPSYS_DITHER_BASE + 0x02c)
#define DISP_REG_DITHER_SIZE                               \
	(DISPSYS_DITHER_BASE + 0x030)
#define DISP_REG_DITHER_DUMMY_REG                            \
	(DISPSYS_DITHER_BASE + 0x0c0)
#define DISP_REG_DITHER_0                                  \
	(DISPSYS_DITHER_BASE + 0x100)
#define DISP_REG_DITHER_5                                    \
	(DISPSYS_DITHER_BASE + 0x114)
#define DISP_REG_DITHER_6                                 \
	(DISPSYS_DITHER_BASE + 0x118)
#define DISP_REG_DITHER_7                                        \
	(DISPSYS_DITHER_BASE + 0x11c)
#define DISP_REG_DITHER_8                                       \
	(DISPSYS_DITHER_BASE + 0x120)
#define DISP_REG_DITHER_9                               \
	(DISPSYS_DITHER_BASE + 0x124)
#define DISP_REG_DITHER_10                                  \
	(DISPSYS_DITHER_BASE + 0x128)
#define DISP_REG_DITHER_11                                 \
	(DISPSYS_DITHER_BASE + 0x12c)
#define DISP_REG_DITHER_12                                \
	(DISPSYS_DITHER_BASE + 0x130)
#define DISP_REG_DITHER_13                                   \
	(DISPSYS_DITHER_BASE + 0x134)
#define DISP_REG_DITHER_14                               \
	(DISPSYS_DITHER_BASE + 0x138)
#define DISP_REG_DITHER_15                                 \
	(DISPSYS_DITHER_BASE + 0x13c)
#define DISP_REG_DITHER_16                                \
	(DISPSYS_DITHER_BASE + 0x140)
#define DISP_REG_DITHER_17                               \
	(DISPSYS_DITHER_BASE + 0x144)
...
/* ------------------------------------------------------------- */
/* MERGE */
#define DISP_REG_MERGE_ENABLE				 \
	(DISPSYS_MERGE_BASE + 0x000)
#define DISP_REG_MERGE_SW_RESET			 \
	(DISPSYS_MERGE_BASE + 0x004)
#define DISP_REG_MERGE_DEBUG			 \
	(DISPSYS_MERGE_BASE + 0x008)

#define ENABLE_FLD_MERGE_EN			 \
	REG_FLD(1, 0)
#define SW_RESET_FLD_MERGE_SW_RST				REG_FLD(1, 0)
#define DEBUG_FLD_MERGE_FSM					 \
	REG_FLD(3, 29)
#define DEBUG_FLD_OUT_PIXEL_CNT				 \
	REG_FLD(24, 0)
...
#define DISP_REG_CONFIG_MUTEX_INTEN			 \
	(DISPSYS_MUTEX_BASE + 0x000)
#define DISP_REG_CONFIG_MUTEX_INTSTA                      \
	(DISPSYS_MUTEX_BASE + 0x004)
#define DISP_REG_CONFIG_MUTEX_HW_DCM                       \
	(DISPSYS_MUTEX_BASE + 0x008)
#define DISP_REG_CONFIG_MUTEX0_EN		 \
	(DISPSYS_MUTEX_BASE + 0x020)
#define DISP_REG_CONFIG_MUTEX0_RST			 \
	(DISPSYS_MUTEX_BASE + 0x028)
#define DISP_REG_CONFIG_MUTEX0_MOD			 \
	(DISPSYS_MUTEX_BASE + 0x02C)
#define DISP_REG_CONFIG_MUTEX0_SOF			 \
	(DISPSYS_MUTEX_BASE + 0x030)
#define DISP_REG_CONFIG_MUTEX1_EN			 \
	(DISPSYS_MUTEX_BASE + 0x040)
#define DISP_REG_CONFIG_MUTEX1_RST			 \
	(DISPSYS_MUTEX_BASE + 0x048)
#define DISP_REG_CONFIG_MUTEX1_MOD			 \
	(DISPSYS_MUTEX_BASE + 0x04C)
#define DISP_REG_CONFIG_MUTEX1_SOF			 \
	(DISPSYS_MUTEX_BASE + 0x050)
#define DISP_REG_CONFIG_MUTEX2_EN			 \
	(DISPSYS_MUTEX_BASE + 0x060)
#define DISP_REG_CONFIG_MUTEX2_RST			 \
	(DISPSYS_MUTEX_BASE + 0x068)
#define DISP_REG_CONFIG_MUTEX2_MOD			 \
	(DISPSYS_MUTEX_BASE + 0x06C)
#define DISP_REG_CONFIG_MUTEX2_SOF		 \
	(DISPSYS_MUTEX_BASE + 0x070)
#define DISP_REG_CONFIG_MUTEX3_EN			 \
	(DISPSYS_MUTEX_BASE + 0x080)
#define DISP_REG_CONFIG_MUTEX3_RST			 \
	(DISPSYS_MUTEX_BASE + 0x088)
#define DISP_REG_CONFIG_MUTEX3_MOD			 \
	(DISPSYS_MUTEX_BASE + 0x08C)
#define DISP_REG_CONFIG_MUTEX3_SOF			 \
	(DISPSYS_MUTEX_BASE + 0x090)
#define DISP_REG_CONFIG_MUTEX4_EN			 \
	(DISPSYS_MUTEX_BASE + 0x0A0)
#define DISP_REG_CONFIG_MUTEX4_RST		 \
	(DISPSYS_MUTEX_BASE + 0x0A8)
#define DISP_REG_CONFIG_MUTEX4_MOD			 \
	(DISPSYS_MUTEX_BASE + 0x0AC)
#define DISP_REG_CONFIG_MUTEX4_SOF			 \
	(DISPSYS_MUTEX_BASE + 0x0B0)
#define DISP_REG_CONFIG_MUTEX5_EN			 \
	(DISPSYS_MUTEX_BASE + 0x0C0)
#define DISP_REG_CONFIG_MUTEX5_RST			 \
	(DISPSYS_MUTEX_BASE + 0x0C8)
#define DISP_REG_CONFIG_MUTEX5_MOD			 \
	(DISPSYS_MUTEX_BASE + 0x0CC)
#define DISP_REG_CONFIG_MUTEX5_SOF			 \
	(DISPSYS_MUTEX_BASE + 0x0D0)
#define DISP_REG_CONFIG_DEBUG_OUT_SEL			 \
	(DISPSYS_MUTEX_BASE + 0x200)
...
#define DISP_REG_CONFIG_MUTEX_MOD(n)         \
	(DISP_REG_CONFIG_MUTEX0_MOD + (0x20 * n))
#define DISP_REG_CONFIG_MUTEX_SOF(n)         \
	(DISP_REG_CONFIG_MUTEX0_SOF + (0x20 * n))

#define INTEN_FLD_MUTEX_INTEN					 \
	REG_FLD(20, 0)
#define INTSTA_FLD_MUTEX_INTSTA					 \
	REG_FLD(20, 0)
#define EN_FLD_MUTEX0_EN				 \
	REG_FLD(1, 0)
#define RST_FLD_MUTEX0_RST				 \
	REG_FLD(1, 0)
#define MOD_FLD_MUTEX0_MOD				 \
	REG_FLD(18, 0)
#define SOF_FLD_MUTEX0_SOF_TIMING			 \
	REG_FLD(1, 3)
#define SOF_FLD_MUTEX0_SOF				 \
	REG_FLD(3, 0)
#define EN_FLD_MUTEX1_EN					 \
	REG_FLD(1, 0)
#define RST_FLD_MUTEX1_RST					 \
	REG_FLD(1, 0)
#define MOD_FLD_MUTEX0_MOD					 \
	REG_FLD(18, 0)
#define SOF_FLD_MUTEX1_SOF_TIMING			 \
	REG_FLD(1, 3)
#define SOF_FLD_MUTEX1_SOF				 \
	REG_FLD(3, 0)
#define EN_FLD_MUTEX2_EN						 \
	REG_FLD(1, 0)
#define RST_FLD_MUTEX2_RST						 \
	REG_FLD(1, 0)
#define MOD_FLD_MUTEX0_MOD					 \
	REG_FLD(18, 0)
#define SOF_FLD_MUTEX2_SOF_TIMING				 \
	REG_FLD(1, 3)
#define SOF_FLD_MUTEX2_SOF					 \
	REG_FLD(3, 0)
#define EN_FLD_MUTEX3_EN					 \
	REG_FLD(1, 0)
#define RST_FLD_MUTEX3_RST					 \
	REG_FLD(1, 0)
#define MOD_FLD_MUTEX0_MOD					 \
	REG_FLD(18, 0)
#define SOF_FLD_MUTEX3_SOF_TIMING				 \
	REG_FLD(1, 3)
#define SOF_FLD_MUTEX3_SOF					 \
	REG_FLD(3, 0)
#define EN_FLD_MUTEX4_EN					 \
	REG_FLD(1, 0)
#define RST_FLD_MUTEX4_RST					 \
	REG_FLD(1, 0)
#define MOD_FLD_MUTEX0_MOD						 \
	REG_FLD(18, 0)
#define SOF_FLD_MUTEX4_SOF_TIMING				 \
	REG_FLD(1, 3)
#define SOF_FLD_MUTEX4_SOF					 \
	REG_FLD(3, 0)
#define EN_FLD_MUTEX5_EN						 \
	REG_FLD(1, 0)
#define RST_FLD_MUTEX5_RST					 \
	REG_FLD(1, 0)
#define MOD_FLD_MUTEX0_MOD					 \
	REG_FLD(18, 0)
#define SOF_FLD_MUTEX5_SOF_TIMING				 \
	REG_FLD(1, 3)
#define SOF_FLD_MUTEX5_SOF						 \
	REG_FLD(3, 0)
...
#define DISP_REG_OVL_STA				 \
	(DISPSYS_OVL0_BASE + 0x000)
#define DISP_REG_OVL_INTEN			 \
	(DISPSYS_OVL0_BASE + 0x004)
#define DISP_REG_OVL_INTSTA			 \
	(DISPSYS_OVL0_BASE + 0x008)
#define DISP_REG_OVL_EN						 \
	(DISPSYS_OVL0_BASE + 0x00C)
#define DISP_REG_OVL_TRIG					 \
	(DISPSYS_OVL0_BASE + 0x010)
#define DISP_REG_OVL_RST					 \
	(DISPSYS_OVL0_BASE + 0x014)
#define DISP_REG_OVL_ROI_SIZE					 \
	(DISPSYS_OVL0_BASE + 0x020)
#define DISP_REG_OVL_DATAPATH_CON			 \
	(DISPSYS_OVL0_BASE + 0x024)
#define DISP_REG_OVL_ROI_BGCLR				 \
	(DISPSYS_OVL0_BASE + 0x028)
#define DISP_REG_OVL_SRC_CON				 \
	(DISPSYS_OVL0_BASE + 0x02C)
#define DISP_REG_OVL_L0_CON					 \
	(DISPSYS_OVL0_BASE + 0x030)
#define DISP_REG_OVL_L0_SRCKEY				 \
	(DISPSYS_OVL0_BASE + 0x034)
#define DISP_REG_OVL_L0_SRC_SIZE				 \
	(DISPSYS_OVL0_BASE + 0x038)
#define DISP_REG_OVL_L0_OFFSET				 \
	(DISPSYS_OVL0_BASE + 0x03C)
#define DISP_REG_OVL_L0_ADDR					 \
	(DISPSYS_OVL0_BASE + 0xf40)
#define DISP_REG_OVL_L0_PITCH				 \
	(DISPSYS_OVL0_BASE + 0x044)
#define DISP_REG_OVL_L0_TILE					 \
	(DISPSYS_OVL0_BASE + 0x048)
...
#define DISP_REG_OVL_L3_CLIP (DISPSYS_OVL0_BASE + 0x0AC)
	#define OVL_L_CLIP_FLD_LEFT                     REG_FLD(7, 0)
	#define OVL_L_CLIP_FLD_RIGHT                    REG_FLD(15, 8)
	#define OVL_L_CLIP_FLD_TOP                      REG_FLD(23, 16)
	#define OVL_L_CLIP_FLD_BOTTOM                   REG_FLD(31, 24)

#define DISP_REG_OVL_L1_CON					 \
	(DISPSYS_OVL0_BASE + 0x050)
#define DISP_REG_OVL_L1_SRCKEY				 \
	(DISPSYS_OVL0_BASE + 0x054)
#define DISP_REG_OVL_L1_SRC_SIZE				 \
	(DISPSYS_OVL0_BASE + 0x058)
#define DISP_REG_OVL_L1_OFFSET				 \
	(DISPSYS_OVL0_BASE + 0x05C)
#define DISP_REG_OVL_L1_ADDR					 \
	(DISPSYS_OVL0_BASE + 0xf60)
#define DISP_REG_OVL_L1_PITCH					 \
	(DISPSYS_OVL0_BASE + 0x064)
#define DISP_REG_OVL_L1_TILE					 \
	(DISPSYS_OVL0_BASE + 0x068)
#define DISP_REG_OVL_L2_CON					 \
	(DISPSYS_OVL0_BASE + 0x070)
#define DISP_REG_OVL_L2_SRCKEY				 \
	(DISPSYS_OVL0_BASE + 0x074)
#define DISP_REG_OVL_L2_SRC_SIZE				 \
	(DISPSYS_OVL0_BASE + 0x078)
#define DISP_REG_OVL_L2_OFFSET				 \
	(DISPSYS_OVL0_BASE + 0x07C)
#define DISP_REG_OVL_L2_ADDR					 \
	(DISPSYS_OVL0_BASE + 0xf80)
#define DISP_REG_OVL_L2_PITCH				 \
	(DISPSYS_OVL0_BASE + 0x084)
#define DISP_REG_OVL_L2_TILE					 \
	(DISPSYS_OVL0_BASE + 0x088)
#define DISP_REG_OVL_L3_CON					 \
	(DISPSYS_OVL0_BASE + 0x090)
#define DISP_REG_OVL_L3_SRCKEY				 \
	(DISPSYS_OVL0_BASE + 0x094)
#define DISP_REG_OVL_L3_SRC_SIZE				 \
	(DISPSYS_OVL0_BASE + 0x098)
#define DISP_REG_OVL_L3_OFFSET					 \
	(DISPSYS_OVL0_BASE + 0x09C)
#define DISP_REG_OVL_L3_ADDR					 \
	(DISPSYS_OVL0_BASE + 0xfA0)
#define DISP_REG_OVL_L3_PITCH				 \
	(DISPSYS_OVL0_BASE + 0x0A4)
#define DISP_REG_OVL_L3_TILE				 \
	(DISPSYS_OVL0_BASE + 0x0A8)
#define DISP_REG_OVL_RDMA0_CTRL				 \
	(DISPSYS_OVL0_BASE + 0x0C0)
#define DISP_REG_OVL_RDMA0_MEM_GMC_SETTING	 \
	(DISPSYS_OVL0_BASE + 0x0C8)
#define DISP_REG_OVL_RDMA0_MEM_SLOW_CON	 \
	(DISPSYS_OVL0_BASE + 0x0CC)
#define DISP_REG_OVL_RDMA0_FIFO_CTRL			 \
	(DISPSYS_OVL0_BASE + 0x0D0)
#define DISP_REG_OVL_RDMA1_CTRL				 \
	(DISPSYS_OVL0_BASE + 0x0E0)
#define DISP_REG_OVL_RDMA1_MEM_GMC_SETTING	 \
	(DISPSYS_OVL0_BASE + 0x0E8)
#define DISP_REG_OVL_RDMA1_MEM_SLOW_CON		 \
	(DISPSYS_OVL0_BASE + 0x0EC)
#define DISP_REG_OVL_RDMA1_FIFO_CTRL			 \
	(DISPSYS_OVL0_BASE + 0x0F0)
#define DISP_REG_OVL_RDMA2_CTRL			 \
	(DISPSYS_OVL0_BASE + 0x100)
#define DISP_REG_OVL_RDMA2_MEM_GMC_SETTING	 \
	(DISPSYS_OVL0_BASE + 0x108)
#define DISP_REG_OVL_RDMA2_MEM_SLOW_CON		 \
	(DISPSYS_OVL0_BASE + 0x10C)
#define DISP_REG_OVL_RDMA2_FIFO_CTRL		 \
	(DISPSYS_OVL0_BASE + 0x110)
#define DISP_REG_OVL_RDMA3_CTRL			 \
	(DISPSYS_OVL0_BASE + 0x120)
#define DISP_REG_OVL_RDMA3_MEM_GMC_SETTING	 \
	(DISPSYS_OVL0_BASE + 0x128)
#define DISP_REG_OVL_RDMA3_MEM_SLOW_CON		 \
	(DISPSYS_OVL0_BASE + 0x12C)
#define DISP_REG_OVL_RDMA3_FIFO_CTRL		\
	(DISPSYS_OVL0_BASE + 0x130)
#define DISP_REG_OVL_L0_Y2R_PARA_R0	\
	(DISPSYS_OVL0_BASE + 0x134)
#define DISP_REG_OVL_L0_Y2R_PARA_R1			\
	(DISPSYS_OVL0_BASE + 0x138)
#define DISP_REG_OVL_L0_Y2R_PARA_G0		\
	(DISPSYS_OVL0_BASE + 0x13C)
#define DISP_REG_OVL_L0_Y2R_PARA_G1			\
	(DISPSYS_OVL0_BASE + 0x140)
#define DISP_REG_OVL_L0_Y2R_PARA_B0			\
	(DISPSYS_OVL0_BASE + 0x144)
#define DISP_REG_OVL_L0_Y2R_PARA_B1			\
	(DISPSYS_OVL0_BASE + 0x148)
#define DISP_REG_OVL_L0_Y2R_PARA_YUV_A_0	\
	(DISPSYS_OVL0_BASE + 0x14C)
#define DISP_REG_OVL_L0_Y2R_PARA_YUV_A_1		\
	(DISPSYS_OVL0_BASE + 0x150)
#define DISP_REG_OVL_L0_Y2R_PARA_RGB_A_0		\
	(DISPSYS_OVL0_BASE + 0x154)
#define DISP_REG_OVL_L0_Y2R_PARA_RGB_A_1		\
	(DISPSYS_OVL0_BASE + 0x158)
#define DISP_REG_OVL_L1_Y2R_PARA_R0			\
	(DISPSYS_OVL0_BASE + 0x15C)
#define DISP_REG_OVL_L1_Y2R_PARA_R1			\
	(DISPSYS_OVL0_BASE + 0x160)
#define DISP_REG_OVL_L1_Y2R_PARA_G0			\
	(DISPSYS_OVL0_BASE + 0x164)
#define DISP_REG_OVL_L1_Y2R_PARA_G1			\
	(DISPSYS_OVL0_BASE + 0x168)
#define DISP_REG_OVL_L1_Y2R_PARA_B0			\
	(DISPSYS_OVL0_BASE + 0x16C)
#define DISP_REG_OVL_L1_Y2R_PARA_B1			\
	(DISPSYS_OVL0_BASE + 0x170)
#define DISP_REG_OVL_L1_Y2R_PARA_YUV_A_0		\
	(DISPSYS_OVL0_BASE + 0x174)
#define DISP_REG_OVL_L1_Y2R_PARA_YUV_A_1		\
	(DISPSYS_OVL0_BASE + 0x178)
#define DISP_REG_OVL_L1_Y2R_PARA_RGB_A_0		\
	(DISPSYS_OVL0_BASE + 0x17C)
#define DISP_REG_OVL_L1_Y2R_PARA_RGB_A_1		\
	(DISPSYS_OVL0_BASE + 0x180)
#define DISP_REG_OVL_L2_Y2R_PARA_R0			\
	(DISPSYS_OVL0_BASE + 0x184)
#define DISP_REG_OVL_L2_Y2R_PARA_R1			\
	(DISPSYS_OVL0_BASE + 0x188)
#define DISP_REG_OVL_L2_Y2R_PARA_G0			\
	(DISPSYS_OVL0_BASE + 0x18C)
#define DISP_REG_OVL_L2_Y2R_PARA_G1			\
	(DISPSYS_OVL0_BASE + 0x190)
#define DISP_REG_OVL_L2_Y2R_PARA_B0			\
	(DISPSYS_OVL0_BASE + 0x194)
#define DISP_REG_OVL_L2_Y2R_PARA_B1			\
	(DISPSYS_OVL0_BASE + 0x198)
#define DISP_REG_OVL_L2_Y2R_PARA_YUV_A_0		\
	(DISPSYS_OVL0_BASE + 0x19C)
#define DISP_REG_OVL_L2_Y2R_PARA_YUV_A_1		\
	(DISPSYS_OVL0_BASE + 0x1A0)
#define DISP_REG_OVL_L2_Y2R_PARA_RGB_A_0		\
	(DISPSYS_OVL0_BASE + 0x1A4)
#define DISP_REG_OVL_L2_Y2R_PARA_RGB_A_1		\
	(DISPSYS_OVL0_BASE + 0x1A8)
#define DISP_REG_OVL_L3_Y2R_PARA_R0		\
	(DISPSYS_OVL0_BASE + 0x1AC)
#define DISP_REG_OVL_L3_Y2R_PARA_R1			\
	(DISPSYS_OVL0_BASE + 0x1B0)
#define DISP_REG_OVL_L3_Y2R_PARA_G0			\
	(DISPSYS_OVL0_BASE + 0x1B4)
#define DISP_REG_OVL_L3_Y2R_PARA_G1			\
	(DISPSYS_OVL0_BASE + 0x1B8)
#define DISP_REG_OVL_L3_Y2R_PARA_B0			\
	(DISPSYS_OVL0_BASE + 0x1BC)
#define DISP_REG_OVL_L3_Y2R_PARA_B1			\
	(DISPSYS_OVL0_BASE + 0x1C0)
#define DISP_REG_OVL_L3_Y2R_PARA_YUV_A_0		\
	(DISPSYS_OVL0_BASE + 0x1C4)
#define DISP_REG_OVL_L3_Y2R_PARA_YUV_A_1		\
	(DISPSYS_OVL0_BASE + 0x1C8)
#define DISP_REG_OVL_L3_Y2R_PARA_RGB_A_0		\
	(DISPSYS_OVL0_BASE + 0x1CC)
#define DISP_REG_OVL_L3_Y2R_PARA_RGB_A_1		\
	(DISPSYS_OVL0_BASE + 0x1D0)
#define DISP_REG_OVL_DEBUG_MON_SEL			\
	(DISPSYS_OVL0_BASE + 0x1D4)
...
#define DISP_REG_OVL_DUMMY_REG				\
	(DISPSYS_OVL0_BASE + 0x200)
#define DISP_REG_OVL_SMI_DBG                            \
	(DISPSYS_OVL0_BASE + 0x230)
#define DISP_REG_OVL_GREQ_LAYER_CNT                    \
	(DISPSYS_OVL0_BASE + 0x234)
#define DISP_REG_OVL_FLOW_CTRL_DBG		\
	(DISPSYS_OVL0_BASE + 0x240)
#define DISP_REG_OVL_ADDCON_DBG				\
	(DISPSYS_OVL0_BASE + 0x244)
#define DISP_REG_OVL_RDMA0_DBG				\
	(DISPSYS_OVL0_BASE + 0x24C)
#define DISP_REG_OVL_RDMA1_DBG					\
	(DISPSYS_OVL0_BASE + 0x250)
#define DISP_REG_OVL_RDMA2_DBG				\
	(DISPSYS_OVL0_BASE + 0x254)
#define DISP_REG_OVL_RDMA3_DBG				\
	(DISPSYS_OVL0_BASE + 0x258)
#define DISP_REG_OVL_L0_CLR				\
	(DISPSYS_OVL0_BASE + 0x25c)
#define DISP_REG_OVL_L1_CLR				\
	(DISPSYS_OVL0_BASE + 0x260)
#define DISP_REG_OVL_L2_CLR				\
	(DISPSYS_OVL0_BASE + 0x264)
#define DISP_REG_OVL_L3_CLR				\
	(DISPSYS_OVL0_BASE + 0x268)

#define STA_FLD_RDMA3_IDLE					\
	REG_FLD(1, 4)
#define STA_FLD_RDMA2_IDLE					\
	REG_FLD(1, 3)
#define STA_FLD_RDMA1_IDLE					\
	REG_FLD(1, 2)
#define STA_FLD_RDMA0_IDLE					\
	REG_FLD(1, 1)
#define STA_FLD_RUN						\
	REG_FLD(1, 0)
...
#define INTEN_FLD_FME_UND_INTEN					\
	REG_FLD(1, 2)
#define INTEN_FLD_FME_CPL_INTEN					\
	REG_FLD(1, 1)
#define INTEN_FLD_REG_CMT_INTEN					\
	REG_FLD(1, 0)
...
#define EN_FLD_EN							\
	REG_FLD(1, 0)
#define TRIG_FLD_SW_TRIG					\
	REG_FLD(1, 0)
#define RST_FLD_RST							\
	REG_FLD(1, 0)
#define RST_FLD_FORCE_RST						\
	REG_FLD(1, 31)
#define ROI_SIZE_FLD_ROI_H						\
	REG_FLD(12, 16)
#define ROI_SIZE_FLD_ROI_W						\
	REG_FLD(12, 0)
...
#define DATAPATH_CON_FLD_BGCLR_IN_SEL                                \
	REG_FLD(1, 2)
#define DATAPATH_CON_FLD_RANDOM_BGCLR_EN		REG_FLD(1, 1)
#define DATAPATH_CON_FLD_LAYER_SMI_ID_EN			REG_FLD(1, 0)
#define ROI_BGCLR_FLD_ALPHA						\
	REG_FLD(8, 24)
#define ROI_BGCLR_FLD_RED						\
	REG_FLD(8, 16)
#define ROI_BGCLR_FLD_GREEN						\
	REG_FLD(8, 8)
#define ROI_BGCLR_FLD_BLUE						\
	REG_FLD(8, 0)
...
/* ------------------------------------------------------------- */
/* RDMA */
#define DISP_REG_RDMA_INT_ENABLE			\
	(DISPSYS_RDMA0_BASE+0x000)
#define DISP_REG_RDMA_INT_STATUS			\
	(DISPSYS_RDMA0_BASE+0x004)
#define DISP_REG_RDMA_GLOBAL_CON			\
	(DISPSYS_RDMA0_BASE+0x010)
#define DISP_REG_RDMA_SIZE_CON_0			\
	(DISPSYS_RDMA0_BASE+0x014)
#define DISP_REG_RDMA_SIZE_CON_1			\
	(DISPSYS_RDMA0_BASE+0x018)
#define DISP_REG_RDMA_TARGET_LINE			\
	(DISPSYS_RDMA0_BASE+0x01C)
#define DISP_REG_RDMA_MEM_CON				\
	(DISPSYS_RDMA0_BASE+0x024)
#define DISP_REG_RDMA_MEM_SRC_PITCH			\
	(DISPSYS_RDMA0_BASE+0x02C)
#define DISP_REG_RDMA_MEM_GMC_SETTING_0		\
	(DISPSYS_RDMA0_BASE+0x030)
#define DISP_REG_RDMA_MEM_SLOW_CON			\
	(DISPSYS_RDMA0_BASE+0x034)
#define DISP_REG_RDMA_MEM_GMC_SETTING_1		\
	(DISPSYS_RDMA0_BASE+0x038)
#define DISP_REG_RDMA_FIFO_CON				\
	(DISPSYS_RDMA0_BASE+0x040)
#define DISP_REG_RDMA_FIFO_LOG				\
	(DISPSYS_RDMA0_BASE+0x044)
#define DISP_REG_RDMA_C00					\
	(DISPSYS_RDMA0_BASE+0x054)
#define DISP_REG_RDMA_C01					\
	(DISPSYS_RDMA0_BASE+0x058)
#define DISP_REG_RDMA_C02					\
	(DISPSYS_RDMA0_BASE+0x05C)
#define DISP_REG_RDMA_C10					\
	(DISPSYS_RDMA0_BASE+0x060)
#define DISP_REG_RDMA_C11					\
	(DISPSYS_RDMA0_BASE+0x064)
#define DISP_REG_RDMA_C12					\
	(DISPSYS_RDMA0_BASE+0x068)
#define DISP_REG_RDMA_C20					\
	(DISPSYS_RDMA0_BASE+0x06C)
#define DISP_REG_RDMA_C21					\
	(DISPSYS_RDMA0_BASE+0x070)
#define DISP_REG_RDMA_C22					\
	(DISPSYS_RDMA0_BASE+0x074)
#define DISP_REG_RDMA_PRE_ADD_0				\
	(DISPSYS_RDMA0_BASE+0x078)
#define DISP_REG_RDMA_PRE_ADD_1				\
	(DISPSYS_RDMA0_BASE+0x07C)
#define DISP_REG_RDMA_PRE_ADD_2				\
	(DISPSYS_RDMA0_BASE+0x080)
#define DISP_REG_RDMA_POST_ADD_0			\
	(DISPSYS_RDMA0_BASE+0x084)
#define DISP_REG_RDMA_POST_ADD_1			\
	(DISPSYS_RDMA0_BASE+0x088)
#define DISP_REG_RDMA_POST_ADD_2			\
	(DISPSYS_RDMA0_BASE+0x08C)
#define DISP_REG_RDMA_DUMMY				\
	(DISPSYS_RDMA0_BASE+0x090)
#define DISP_REG_RDMA_DEBUG_OUT_SEL			\
	(DISPSYS_RDMA0_BASE+0x094)
#define DISP_REG_RDMA_BG_CON_0				\
	(DISPSYS_RDMA0_BASE+0x0a0)
#define DISP_REG_RDMA_BG_CON_1				\
	(DISPSYS_RDMA0_BASE+0x0a4)
#define DISP_REG_RDMA_THRESHOLD_FOR_SODI	\
	(DISPSYS_RDMA0_BASE+0x0a8)
#define DISP_REG_RDMA_IN_P_CNT				\
	(DISPSYS_RDMA0_BASE+0x0f0)
#define DISP_REG_RDMA_IN_LINE_CNT			\
	(DISPSYS_RDMA0_BASE+0x0f4)
#define DISP_REG_RDMA_OUT_P_CNT				\
	(DISPSYS_RDMA0_BASE+0x0f8)
#define DISP_REG_RDMA_OUT_LINE_CNT			\
	(DISPSYS_RDMA0_BASE+0x0fc)
#define DISP_REG_RDMA_MEM_START_ADDR		\
	(DISPSYS_RDMA0_BASE+0xf00)
...
#define MEM_GMC_SETTING_0_FLD_ULTRA_THRESHOLD_LOW		\
	REG_FLD(8, 0)
#define MEM_SLOW_CON_FLD_MEM_MODE_SLOW_COUNT			\
	REG_FLD(16, 16)
#define MEM_SLOW_CON_FLD_MEM_MODE_SLOW_EN				\
	REG_FLD(1, 0)
#define MEM_GMC_SETTING_1_FLD_ISSUE_REQ_THRESHOLD		\
	REG_FLD(8, 0)
#define FIFO_CON_FLD_FIFO_UNDERFLOW_EN					\
	REG_FLD(1, 31)
#define FIFO_CON_FLD_FIFO_PSEUDO_SIZE					\
	REG_FLD(10, 16)
#define FIFO_CON_FLD_OUTPUT_VALID_FIFO_THRESHOLD		\
	REG_FLD(10, 0)
#define FIFO_LOG_FLD_RDMA_FIFO_LOG				\
	REG_FLD(10, 0)
#define C00_FLD_DISP_RDMA_C00						\
	REG_FLD(13, 0)
#define C01_FLD_DISP_RDMA_C01						\
	REG_FLD(13, 0)
#define C02_FLD_DISP_RDMA_C02						\
	REG_FLD(13, 0)
#define C10_FLD_DISP_RDMA_C10						\
	REG_FLD(13, 0)
#define C11_FLD_DISP_RDMA_C11						\
	REG_FLD(13, 0)
#define C12_FLD_DISP_RDMA_C12						\
	REG_FLD(13, 0)
#define C20_FLD_DISP_RDMA_C20						\
	REG_FLD(13, 0)
#define C21_FLD_DISP_RDMA_C21						\
	REG_FLD(13, 0)
#define C22_FLD_DISP_RDMA_C22					\
	REG_FLD(13, 0)
#define PRE_ADD_0_FLD_DISP_RDMA_PRE_ADD_0			\
	REG_FLD(9, 0)
#define PRE_ADD_1_FLD_DISP_RDMA_PRE_ADD_1		\
	REG_FLD(9, 0)
#define PRE_ADD_2_FLD_DISP_RDMA_PRE_ADD_2		\
	REG_FLD(9, 0)
...
#define DUMMY_FLD_DISP_RDMA_DUMMY				\
	REG_FLD(32, 0)
