...
unsigned long int gRDMAUltraSetting;
/* so we can modify RDMA ultra at run-time */
...
static void process_dbg_opt(const char *opt)
{
	...
	if (strncmp(opt, "regr:", 5) == 0) {
		...
		if (is_reg_addr_valid(1, addr) == 1) {
			/* (addr >= 0xf0000000U && addr <= 0xff000000U) */
		...
		if (reg_pa < 0x10000000 || reg_pa > 0x20000000) {
			...
			reg_va = (unsigned long)ioremap_nocache(reg_pa,
		...
		if (reg_pa < 0x10000000 || reg_pa > 0x20000000) {
			...
			pr_debug
			    ("pa=%lx,va=0x%lx,v=0x%x,before=0x%x,after=0x%x\n",
			     reg_pa, reg_va, (unsigned int)val,
			     reg_va_before, reg_va_after);
			sprintf(buf,
				"pa=%lx,v=0x%x, before=0x%x, after=0x%x\n",
				reg_pa, (unsigned int)val, reg_va_before,
				reg_va_after);
		...
		DDPMSG("%s, ovl_en=0x%x\n", __func__, (unsigned int)ovl_en);
		force_ovl_en = ovl_en;
...
void ddp_debug_init(void)
{
	if (!debug_init) {
		debug_init = 1;
		debugfs =
			debugfs_create_file("dispsys", S_IFREG | 0444, NULL,
					(void *)0, &debug_fops);
...
unsigned int ddp_dump_reg_to_buf(unsigned int start_module,
		unsigned long *addr)
{
	unsigned int cnt = 0;
...
unsigned int ddp_dump_lcm_param_to_buf(unsigned int start_module,
		unsigned long *addr)
{
	unsigned int cnt = 0;
