...
/* --------------------------------------// */
static ssize_t disp_kobj_show(struct kobject *kobj, struct attribute *attr,
		char *buffer)
{
	int size = 0x0;
	...
	/* DISP_MODULE_WDMA0 */
		...
		size = ddp_dump_lcm_param_to_buf(0, (unsigned long *)buffer);
	/* DISP_MODULE_LCM */
...
int ddp_clk_prepare(enum eDDP_CLK_ID id)
{
	...
	if (ret)
		pr_info("DISPSYS CLK prepare failed: errno %d\n", ret);
...
int ddp_clk_enable(enum eDDP_CLK_ID id)
{
	...
	if (ret)
		pr_info("DISPSYS CLK enable failed: errno %d\n", ret);
...
#ifndef CONFIG_MTK_CLKMGR
#ifndef CONFIG_FPGA_EARLY_PORTING	/* CCF not ready */
	return clk_set_parent(dispsys_dev->disp_clk[id],
			dispsys_dev->disp_clk[parent]);
#endif
#endif
	return 0;
}
...
static int disp_probe(struct platform_device *pdev)
{
	...
	if ((!larb_pdev[0]) || (!larb_pdev[0]->dev.driver)) {
		DDPERR("%s is earlier than SMI\n", __func__);
		return -EPROBE_DEFER;
	}

	/* add for mmp dump mva->pa */
	np = of_find_compatible_node(NULL, NULL,
			"mediatek,mt8167-pseudo-port-m4u");
	if (np == NULL) {
	...
	for (i = 0; i < MAX_DISP_CLK_CNT; i++) {
		...
		dispsys_dev->disp_clk[i] = devm_clk_get(&pdev->dev,
				disp_clk_name[i]);
	...
	for (i = 0; i < DISP_REG_NUM; i++) {
		...
		if (!dispsys_dev->regs[i]) {
			DDPERR("Unable2ioremap registers,of_iomap fail,i=%d\n",
					i);
			return -ENOMEM;
		}
		...
		DDPMSG("DT,i=%d,mod=%s,m_addr=%p,m_irq=%d,r_pa=0x%x,irq=%d\n",
	...
	DDPMSG("sysfs disp +");
	/* add kobject */
	if (kobject_init_and_add(&kdispobj, &disp_kobj_ktype, NULL,
				"disp") < 0) {
	...
	/* when MMSYS clock off but GPU/MJC/PWM clock on,
	 * avoid display hang and trigger error intr
	 */
	{
		DISP_REG_SET_FIELD(NULL, MMSYS_TO_MFG_TX_ERROR,
				DISP_REG_CONFIG_MMSYS_INTEN, 1);
		DISP_REG_SET_FIELD(NULL, MMSYS_TO_MJC_TX_ERROR,
				DISP_REG_CONFIG_MMSYS_INTEN, 1);
		DISP_REG_SET_FIELD(NULL, PWM0_APB_TX_ERROR,
				DISP_REG_CONFIG_MMSYS_INTEN, 1);
	}
