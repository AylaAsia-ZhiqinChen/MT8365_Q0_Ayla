...
static void mutex_dump_analysis(void)
{
	...
	for (i = 0; i < 5; i++) {
		...
		if (DISP_REG_GET(DISP_REG_CONFIG_MUTEX_MOD(i)) != 0 &&
			...
			p += len;
			for (j = 6; j <= 18; j++) {
				if ((DISP_REG_GET(DISP_REG_CONFIG_MUTEX_MOD(i)
								) >> j) & 0x1) {
					len = sprintf(p, "%s,",
 ...
 *--------valid & ready
 *example:
 *ovl0 -> ovl0_mout_ready=1 means engines after ovl_mout are ready
 *for receiving data
...
static void mmsys_config_dump_analysis(void)
{
	...
	for (i = 0; i <= 1; i++) {
		if ((reg & (1 << i)) == 0)
			strncat(clock_on, ddp_clock_0(i),
	...
	for (i = 0; i <= 5; i++) {
		if ((reg & (1 << i)) == 0)
			strncat(clock_on, ddp_clock_1(i),
				sizeof(clock_on) - strlen(clock_on) - 1);
	}
	...
	if ((DISP_REG_GET(DISP_REG_CONFIG_C11) & 0xff000000) != 0xff000000) {
		DDPDUMP("err,MMclock bit24~bit31 should be 1, but value=0x%x",
			DISP_REG_GET(DISP_REG_CONFIG_C11));
	}
#endif
	...
	if (/*((DISP_REG_GET(DISP_REG_CONFIG_C10) >> 7) & 0x7) == 5 &&*/
	    ((DISP_REG_GET(DISP_REG_OVL_ROI_SIZE + DISP_OVL_INDEX_OFFSET *
			   1) & 0xfff) > 1200 ||
	     ((DISP_REG_GET(DISP_REG_OVL_ROI_SIZE + DISP_OVL_INDEX_OFFSET *
			    1) >> 16) & 0xfff) > 1920)) {
		DDPDUMP("check OVL ROI setting:(120,192),(%d, %d)\n",
		       DISP_REG_GET(DISP_REG_OVL_ROI_SIZE +
			       DISP_OVL_INDEX_OFFSET * 1) & 0xfff,
		       (DISP_REG_GET(DISP_REG_OVL_ROI_SIZE +
				     DISP_OVL_INDEX_OFFSET * 1) >> 16) &
		       0xfff);
	} else if (/*((DISP_REG_GET(DISP_REG_CONFIG_C10) >> 8) & 0x3) == 3 &&*/
		   ((DISP_REG_GET(DISP_REG_OVL_ROI_SIZE +
				  DISP_OVL_INDEX_OFFSET * 1) & 0xfff) > 800 ||
		   ((DISP_REG_GET(DISP_REG_OVL_ROI_SIZE +
				  DISP_OVL_INDEX_OFFSET * 1) >> 16
		     ) & 0xfff) > 1280)) {
		DDPDUMP("check OVL ROI setting:(80,128),(%d, %d)\n",
		       DISP_REG_GET(DISP_REG_OVL_ROI_SIZE +
			       DISP_OVL_INDEX_OFFSET * 1) & 0xfff,
		       (DISP_REG_GET(DISP_REG_OVL_ROI_SIZE +
				     DISP_OVL_INDEX_OFFSET * 1) >> 16) & 0xfff);
	}
	...
	/* AEE status dump */
	{
		if (isAEEEnabled == 0) {
			...
			DDPDUMP("isAEEEnabled=%d, O0L3=0x%x, AEE=0x%lx\n",
					...
					get_Assert_Layer_PA());
			if (DISP_REG_GET(DISP_REG_OVL_L3_ADDR) !=
					get_Assert_Layer_PA())
				DDPDUMP("err: AEE en but O-L3 Addr wrong!\n");
		}
	}
}
...
static void gamma_dump_analysis(void)
{
	...
	DDPDUMP(" out_p_cnt=%d, out_l_cnt=%d\n",
...
static void ccorr_dump_analyze(void)
{
	...
	DDPDUMP(" in_l_cnt=%d, out_p_cnt=%d, out_l_cnt=%d\n",
