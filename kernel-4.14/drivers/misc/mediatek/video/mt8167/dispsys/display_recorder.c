...
static const char *_get_event_string(enum DPREC_EVENT event)
{
	...
	for (i = 0; i < sizeof(event_map) / sizeof(
				struct event_string_map); i++) {
...
static unsigned char dprec_string_buffer_analysize[
dprec_dump_max_length] = { 0 };
...
int dprec_logger_pr(unsigned int type, char *fmt, ...)
{
	...
	if (dprec_logger_buffer[type].len < 128) {
		...
		dprec_logger_buffer[type].id =
			dprec_logger_buffer[type].id % dprec_logger_buffer[
			type].count;
		dprec_logger_buffer[type].len = dprec_logger_buffer[type].size;
	}
	buf_arr =
		(char (*)[LOGGER_BUFFER_SIZE])dprec_logger_buffer[
		type].buffer_ptr;
		...
		int len)
{
	...
	if (type >= DPREC_LOGGER_PR_NUM || len < 0)
		return 0;
	buf_arr = (char (*)[LOGGER_BUFFER_SIZE])dprec_logger_buffer[
		type].buffer_ptr;
