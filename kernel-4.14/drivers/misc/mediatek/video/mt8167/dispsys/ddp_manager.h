 ...
 * encmdq: 1 use command queue, 0 not.
 */
int dpmgr_path_dsi_power_on(disp_path_handle dp_handle,
		enum CMDQ_SWITCH encmdq);

/* DSI power 0ff,  turn off each modules clk, if all hande are closed.
 * top clock will be off.
 * return 0.
 * dp_handle: disp path handle.
 * encmdq: 1 use command queue, 0 not.
...
/* dpmgr_ioctl , it will call ioctl of ddp modules ioctl()
 * to do some special config or setting.
 * return 0.
 * dp_handle: disp path handle.
...
extern int dpmgr_path_get_handle(unsigned int *dp_handle,
