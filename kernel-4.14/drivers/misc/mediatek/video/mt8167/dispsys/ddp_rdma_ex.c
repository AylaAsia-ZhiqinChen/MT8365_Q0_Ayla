...
static enum RDMA_INPUT_FORMAT rdma_input_format_convert(enum DP_COLOR_ENUM fmt)
{
	...
	case eYUY2:
		...
		break;
	default:
		DDPERR("%s fmt=%d, rdma_fmt=%d\n", __func__,
				fmt, rdma_fmt);
	}
	return rdma_fmt;
}

static unsigned int rdma_input_format_byte_swap(
...
static unsigned int rdma_input_format_color_space(
		enum RDMA_INPUT_FORMAT inputFormat)
{
	int space = 0;
...
static unsigned int rdma_input_format_reg_value(
		enum RDMA_INPUT_FORMAT inputFormat)
{
	int reg_value = 0;
...
int rdma_reset(enum DISP_MODULE_ENUM module, void *handle)
{
	...
	while ((DISP_REG_GET(idx * DISP_RDMA_INDEX_OFFSET +
		...
		if (delay_cnt > 10000) {
			ret = -1;
			DDPERR("rdma%d_reset timeout, stage 1!", idx);
			DDPERR(" DISP_REG_RDMA_GLOBAL_CON=0x%x\n",
	...
	while ((DISP_REG_GET(idx * DISP_RDMA_INDEX_OFFSET +
		...
		if (delay_cnt > 10000) {
			ret = -1;
			DDPERR("rdma%d_reset timeout, stage 2!", idx);
			DDPERR(" DISP_REG_RDMA_GLOBAL_CON=0x%x\n",
			       DISP_REG_GET(idx * DISP_RDMA_INDEX_OFFSET +
				       DISP_REG_RDMA_GLOBAL_CON));
			break;
...
void rdma_set_ultra(unsigned int idx, unsigned int width, unsigned int height,
		...
		enum RDMA_MODE mode, bool isidle)
{
	/* constant */
	static const unsigned int blank_overhead = 115;
	/* it is 1.15, need to divide 100 later */
	...
	fifo_valid_size = fifo_valid_size > fifo_min_size ?
		fifo_valid_size : fifo_min_size;
	...
	pre_ultra_high_level =
	    (unsigned int)(pre_ultra_high_time *
			    consume_levels_per_sec / 1000000);
	...
	if (idx == 0) {
		...
		if (((width >= 1280) && (height >= 800)) || ((width >= 800) &&
					(height >= 1280))) {
			ultra_low_level = 56;
			pre_ultra_low_level = 83;
			pre_ultra_high_level = 97;
			pre_ultra_low_ofs =
				pre_ultra_low_level - ultra_low_level;
			ultra_high_ofs = 1;
			pre_ultra_high_ofs =
				pre_ultra_high_level - pre_ultra_low_level;
			...
			/* Max ultra */
			/* DISP_REG_SET(handle,idx*DISP_RDMA_INDEX_OFFSET+
			 * DISP_REG_RDMA_MEM_GMC_SETTING_0, 0x0x);
			 */
			if (mode == RDMA_MODE_DIRECT_LINK) {
				...
				/* home screen idle senario (300MHz),
				 * in future we should setting
				 * by different clock
				 */
				if (isidle) {
					/* low:101 high:112 (decouple in HD) */
			...
			if (mode == RDMA_MODE_DIRECT_LINK) {
				 ...
				 */
				if (isidle) {
					/* low:21 high:153 (decouple in qHD
					 * when home screen idle senario)
					 */
					sodi_threshold = 42 | (49 << 10);
					fifo_valid_size = 32;
					DISP_REG_SET(handle,
			...
			if (mode == RDMA_MODE_DIRECT_LINK) {
				 ...
				 */
				if (isidle) {
					/* low:17 high:152 (decouple in WVGA
					 * when home screen idle senario)
					 */
	...
	DISP_REG_SET_FIELD(handle, FIFO_CON_FLD_OUTPUT_VALID_FIFO_THRESHOLD,
			   idx * DISP_RDMA_INDEX_OFFSET +
			   DISP_REG_RDMA_FIFO_CON, fifo_valid_size);
		       ...
		       bool is_interlace, bool is_top_filed)
{
	...
	DDPMSG("RDMAConfig idx %d, mode %d, address 0x%lx, inputformat %s,",
		idx, mode, address,
		rdma_intput_format_name(inputFormat, input_swap));
	DDPMSG(" pitch %u, width %u, height %u,sec%d\n",
		pitch, width, height, sec);
	...
	if (input_is_yuv == 1 && output_is_yuv == 0) {
		DISP_REG_SET_FIELD(NULL, SIZE_CON_0_FLD_MATRIX_ENABLE,
				&size_con_reg, 1);
	...
	if (sec != DISP_SECURE_BUFFER) {
		...
		/* cmdq sec driver will help
		 * to convert handle to correct address
		 */
		cmdqRecWriteSecure(handle,
				   disp_addr_convert(idx *
					   DISP_RDMA_INDEX_OFFSET +
					   DISP_REG_RDMA_MEM_START_ADDR),
				   CMDQ_SAM_H_2_MVA, address, offset, size
				   , m4u_port);
		/* DISP_REG_SET(handle,idx*DISP_RDMA_INDEX_OFFSET+
		 * DISP_REG_RDMA_MEM_START_ADDR,
		 */
	...
	DISP_REG_SET_FIELD(NULL, SIZE_CON_0_FLD_OUTPUT_FRAME_WIDTH,
			&size_con_reg,
			   width);
	DISP_REG_SET(handle, idx * DISP_RDMA_INDEX_OFFSET +
			DISP_REG_RDMA_SIZE_CON_0, size_con_reg);
	DISP_REG_SET(handle, idx * DISP_RDMA_INDEX_OFFSET +
...
void rdma_dump_analysis(enum DISP_MODULE_ENUM module)
{
	...
	DDPDUMP("rdma%d: en=%d, memory mode=%d, w=%d, h=%d, pitch=%d,",
			idx, DISP_REG_GET(DISP_REG_RDMA_GLOBAL_CON +
				DISP_RDMA_INDEX_OFFSET * idx) & 0x1,
			(DISP_REG_GET(DISP_REG_RDMA_GLOBAL_CON +
				      DISP_RDMA_INDEX_OFFSET * idx) & 0x2) ?
			1 : 0,
			DISP_REG_GET(DISP_REG_RDMA_SIZE_CON_0 +
				DISP_RDMA_INDEX_OFFSET * idx) & 0xfff,
			DISP_REG_GET(DISP_REG_RDMA_SIZE_CON_1 +
				DISP_RDMA_INDEX_OFFSET * idx) & 0xfffff,
			DISP_REG_GET(DISP_REG_RDMA_MEM_SRC_PITCH +
				DISP_RDMA_INDEX_OFFSET * idx));
	DDPDUMP(" addr=0x%x, fmt=%s, fifo_min=%d,\n",
			DISP_REG_GET(DISP_REG_RDMA_MEM_START_ADDR +
				DISP_RDMA_INDEX_OFFSET * idx),
			...
			DISP_REG_GET(DISP_REG_RDMA_FIFO_LOG +
				DISP_RDMA_INDEX_OFFSET * idx));
	DDPDUMP("in_p_cnt=%d, in_l_cnt=%d, out_p_cnt=%d, out_l_cnt=%d,",
			DISP_REG_GET(DISP_REG_RDMA_IN_P_CNT +
				DISP_RDMA_INDEX_OFFSET * idx),
			DISP_REG_GET(DISP_REG_RDMA_IN_LINE_CNT +
				DISP_RDMA_INDEX_OFFSET * idx),
...
void rdma_get_info(int idx, struct RDMA_BASIC_STRUCT *info)
{
	...
	p->src_h = DISP_REG_GET(DISP_REG_RDMA_SIZE_CON_1 +
			DISP_RDMA_INDEX_OFFSET * idx) & 0xfffff;
	p->bpp = rdma_input_format_bpp((DISP_REG_GET(DISP_REG_RDMA_MEM_CON +
					DISP_RDMA_INDEX_OFFSET * idx) >> 4) &
				       0xf);
}
		...
		struct disp_ddp_path_config *pConfig, void *handle)
{
	...
	unsigned int width = pConfig->dst_dirty ?
		pConfig->dst_w : r_config->width;
	unsigned int height = pConfig->dst_dirty ?
		pConfig->dst_h : r_config->height;
	...
	rdma_config(module, mode, (mode == RDMA_MODE_DIRECT_LINK) ?
			0 : r_config->address, /* address */
		    (mode == RDMA_MODE_DIRECT_LINK) ?
		    eRGB888 : r_config->inputFormat, /* inputFormat */
		    (mode == RDMA_MODE_DIRECT_LINK) ?
		    0 : r_config->pitch, /* pitch */
		    width, height, lcm_param->dsi.ufoe_enable,
		    r_config->security, handle,
	...
	if (mode == RDMA_MODE_MEMORY) {
		if (security == DISP_SECURE_BUFFER) {
			...
			if (rdma_is_sec[rdma_idx] == 0) {
				pr_info("[SVP] switch rdma%d to sec\n",
						rdma_idx);
			...
			if (rdma_is_sec[rdma_idx]) {
				/* rdma is in sec stat,
				 * we need to switch it to nonsec
				 */
				...
				CMDQ_SCENARIO_DISP_SUB_DISABLE_SECURE_PATH,
						  &(nonsec_switch_handle));
				if (ret)
					DDPAEE("[SVP]err2create hnd%s ret=%d\n",
					       __func__, ret);

				cmdqRecReset(nonsec_switch_handle);
				cmdqRecWait(nonsec_switch_handle,
						pConfig->hw_mutex_id +
				 ...
				 */
				/* To avoid translation fault like ovl
				 * (see notes in ovl.c)
				 */
				do_rdma_config_l(module, pConfig,
						nonsec_switch_handle);

				/*in fact, dapc/port_sec will be
				 * disabled by cmdq
				 */
				...
				pr_info("[SVP] switch rdma%d to nonsec done\n",
						rdma_idx);
