...
enum DPI_STATUS ddp_dpi_ConfigPclk(enum DISP_MODULE_ENUM module,
		struct cmdqRecStruct *cmdq, unsigned int clk_req,
			      enum DPI_POLARITY polarity)
{
	...
	default:
		{
			pr_info("DISP/DPI,unknown clock frequency: %d\n",
					clk_req);
			break;
		}
	}
...
enum DPI_STATUS ddp_dpi_ConfigCLK(enum DISP_MODULE_ENUM module,
		struct cmdqRecStruct *cmdq, enum DPI_POLARITY polarity,
		bool LVDSEN)
{
	...
	DPI_OUTREGBIT(cmdq, struct DPI_REG_DDR_SETTING,
			DPI_REG[DPI_IDX(module)]->DDR_SETTING, DDR_4PHASE,
			0);
...
enum DPI_STATUS ddp_dpi_ConfigHsync(enum DISP_MODULE_ENUM module,
		...
		enum DPI_POLARITY polarity, uint32_t pulseWidth,
		uint32_t backPorch, uint32_t frontPorch)
{
...
enum DPI_STATUS ddp_dpi_ConfigDualEdge(enum DISP_MODULE_ENUM module,
		struct cmdqRecStruct *cmdq, bool enable, uint32_t mode)
{
...
enum DPI_STATUS ddp_dpi_ConfigBG(enum DISP_MODULE_ENUM module,
		struct cmdqRecStruct *cmdq, bool enable, int BG_W,
		int BG_H)
{
	if (enable == false) {
...
enum DPI_STATUS ddp_dpi_ConfigSize(enum DISP_MODULE_ENUM module,
		struct cmdqRecStruct *cmdq, uint32_t width, uint32_t height)
{
...
enum DPI_STATUS ddp_dpi_yuv422_setting(enum DISP_MODULE_ENUM module,
		struct cmdqRecStruct *cmdq, uint32_t uvsw)
{
...
enum DPI_STATUS ddp_dpi_CLPFSetting(enum DISP_MODULE_ENUM module,
		struct cmdqRecStruct *cmdq, uint8_t clpfType,
		bool roundingEnable)
{
...
enum DPI_STATUS ddp_dpi_ConfigHDMI(enum DISP_MODULE_ENUM module,
		struct cmdqRecStruct *cmdq, uint32_t yuv422en,
...
enum DPI_STATUS ddp_dpi_ConfigVsync_LEVEN(enum DISP_MODULE_ENUM module,
		struct cmdqRecStruct *cmdq, uint32_t pulseWidth,
		uint32_t backPorch, uint32_t frontPorch, bool fgInterlace)
{
		...
		unsigned int param)
{
	...
	if (status.VSYNC) {
		...
		if (counter) {
			pr_info("DISP/DPI error DPI FIFO is empty,received %d times interrupt !!!\n",
					counter);
			counter = 0;
		}
	}
		...
		struct LCM_DRIVER *lcm_drv)
{
	...
	if (lcm_drv == NULL) {
		pr_info("DISP/DPI,lcm_drv is null!\n");
		return -1;
	}
...
void LVDS_DIG_Init(enum DISP_MODULE_ENUM module, void *cmdq_handle)
{
	...
	/*ADD for suspend&resume blurred screen issue */
		...
		struct disp_ddp_path_config *config, void *cmdq_handle)
{
	if (((s_isDpiConfig == false) && (module == DISP_MODULE_DPI0)) ||
		...
		DDPDBG("DISP/DPI,%s DPI status:%x, cmdq:%p\n", __func__,
				INREG32(&DPI_REG[DPI_IDX(module)]->STATUS),
				cmdq_handle);
		ddp_dpi_ConfigCLK(module, cmdq_handle,
				(enum DPI_POLARITY)dpi_config->clk_pol,
		...
		ddp_dpi_ConfigDE(module, cmdq_handle,
				(enum DPI_POLARITY)dpi_config->de_pol);
		ddp_dpi_ConfigVsync(module, cmdq_handle,
				(enum DPI_POLARITY)dpi_config->vsync_pol,
				dpi_config->vsync_pulse_width,
				dpi_config->vsync_back_porch,
				dpi_config->vsync_front_porch);
		ddp_dpi_ConfigHsync(module, cmdq_handle,
				(enum DPI_POLARITY)dpi_config->hsync_pol,
		...
		if (module == DISP_MODULE_DPI1) {
			ddp_dpi_ConfigPclk(module, cmdq_handle,
					...
					(enum DPI_POLARITY)dpi_config->clk_pol);
			ddp_dpi_ConfigSize(module, cmdq_handle,
					dpi_config->width,
...
int ddp_dpi_power_on(enum DISP_MODULE_ENUM module, void *cmdq_handle)
{
	...
	if ((!s_isDpiPowerOn) && (module == DISP_MODULE_DPI0)) {
		...
		if (ret > 0)
			pr_info("DPI0 power manager API return FALSE 1\n");
#endif
		s_isDpiPowerOn = true;
	}
	if ((!s_isDpi1PowerOn) && (module == DISP_MODULE_DPI1)) {
		...
		if (ret > 0)
			pr_info("DPI1 power manager on return FALSE 1\n");
#endif
...
int ddp_dpi_power_off(enum DISP_MODULE_ENUM module, void *cmdq_handle)
{
	...
	if ((s_isDpiPowerOn) && (module == DISP_MODULE_DPI0)) {
		...
		if (ret > 0)
			pr_info("DPI0 power manager off return FALSE 1\n");
#endif
	...
	if ((s_isDpi1PowerOn) && (module == DISP_MODULE_DPI1)) {
		...
		if (ret > 0)
			pr_info("DPI1 power manager off return FALSE 1\n");
#endif
