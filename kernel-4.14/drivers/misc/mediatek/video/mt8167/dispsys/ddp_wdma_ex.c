...
int wdma_start(enum DISP_MODULE_ENUM module, void *handle)
{
	...
	DISP_REG_SET(handle, idx * DISP_WDMA_INDEX_OFFSET +
			DISP_REG_WDMA_INTEN, 0x03);
	DISP_REG_SET(handle, idx * DISP_WDMA_INDEX_OFFSET +
			DISP_REG_WDMA_EN, 0x01);
			      ...
			      enum DISP_BUFFER_TYPE sec, void *handle)
{
	...
	if (sec != DISP_SECURE_BUFFER) {
		...
		cmdqRecWriteSecure(handle,
				disp_addr_convert(
					idx_offst + DISP_REG_WDMA_DST_ADDR1),
				CMDQ_SAM_H_2_MVA, dstAddress,
				u_off, u_size, m4u_port);
		if (has_v)
			cmdqRecWriteSecure(handle,
		...
		unsigned int frame_rate, void *handle)
{
	...
	unsigned int pre_ultra_low_time = 2;
	/* in unit of 0.5 us */
	...
	consume_levels_per_sec =
	    (width * height * frame_rate * bpp / 2 / rdma_fifo_width / 100) *
	    blank_overhead;
	 ...
	 * pre_ultra_low_ofs=pre_ultra_low_level into register
	 * DISP_WDMA_BUF_CON2
	 */
	DISP_REG_SET(handle, idx*DISP_WDMA_INDEX_OFFSET +
	 ...
	 */
	/* DISP_REG_SET(handle,
	 * idx*DISP_WDMA_INDEX_OFFSET+DISP_REG_WDMA_BUF_CON2, 0x35011b44);
	 */
		       ...
		       enum DISP_BUFFER_TYPE sec, void *handle)
{
	...
	DDPMSG("module %s, src(w=%d,h=%d), clip(x=%d,y=%d,w=%d,h=%d),",
		ddp_get_module_name(module), srcWidth, srcHeight, clipX, clipY,
		clipWidth, clipHeight);
	DDPMSG("out_fmt=%s,dst_address=0x%lx,dst_p=%d,\n",
	...
	if (sec != DISP_SECURE_BUFFER) {
		...
		cmdqRecWriteSecure(handle, disp_addr_convert(idx_offst +
					DISP_REG_WDMA_DST_ADDR0),
				   CMDQ_SAM_H_2_MVA, dstAddress, 0,
				   size, m4u_port);
	}
...
void wdma_dump_analysis(enum DISP_MODULE_ENUM module)
{
	...
	DDPDUMP
	    ("wdma%d:en=%d,w=%d,h=%d,clip=(%d,%d,%d,%d),pitch=(W=%d,UV=%d),",
	...
	DDPDUMP("addr=(0x%x,0x%x,0x%x),fmt=%s\n",
	...
	DDPDUMP("wdma%d:status=%s,in_req=%d,in_ack=%d, exec=%d,",
		index,
		...
		struct disp_ddp_path_config *pConfig, void *handle)
{
	...
	if (config->security == DISP_SECURE_BUFFER) {
		...
		if (wdma_is_sec[wdma_idx] == 0) {
			pr_info("[SVP] switch wdma%d to sec\n", wdma_idx);
		...
		if (wdma_is_sec[wdma_idx] == 1) {
			...
			if (primary_display_is_decouple_mode())
				...
				CMDQ_SCENARIO_DISP_PRIMARY_DISABLE_SECURE_PATH,
						&(nonsec_switch_handle));
			if (ret)
				DDPAEE("[SVP]err2create disable hnd%s ret=%d\n",
				       __func__, ret);
			...
			pr_info("[SVP] switch wdma%d to nonsec\n", wdma_idx);
