...
static struct DDP_IRQ_EVENT_MAPPING ddp_irq_event_list[
DEFAULT_IRQ_EVENT_SCENARIO][DISP_PATH_EVENT_NUM] = {
	{			/* ovl0 path */
...
int dpmgr_path_add_memout(disp_path_handle dp_handle, enum ENGINE_DUMP engine,
		void *cmdq_handle)
{
	struct ddp_path_handle *handle;
	...
	wdma = ddp_is_scenario_on_primary(handle->scenario) ?
...
int dpmgr_insert_ovl1_sub(disp_path_handle dp_handle, void *cmdq_handle)
{
	...
	if ((handle->scenario != DDP_SCENARIO_SUB_RDMA1_DISP) &&
			(handle->scenario != DDP_SCENARIO_SUB_DISP)) {
		DDPERR("%s error, handle->scenario=%s\n", __func__,
		       ddp_get_scenario_name(handle->scenario));
		return -1;
	}
...
int dpmgr_path_enable_cascade(disp_path_handle dp_handle, void *cmdq_handle)
{
	...
	if (ddp_is_module_in_scenario(handle->scenario,
				DISP_MODULE_OVL1) == 1) {
	...
	/* from primary-with-OVL1 to primary_all-with-OVL1
	 * should not rebuild trigger loop cmdq_handle
	 */
...
int dpmgr_path_disable_cascade(disp_path_handle dp_handle, void *cmdq_handle)
{
	...
	if (ddp_modules_driver[DISP_MODULE_OVL1] != 0) {
		if (ddp_modules_driver[DISP_MODULE_OVL1]->cmd != 0)
			ddp_modules_driver[DISP_MODULE_OVL1]->cmd(
					DISP_MODULE_OVL1,
					DISP_IOCTL_OVL_DISABLE_CASCADE,
					0,
					cmdq_handle);
...
int dpmgr_path_release_8_layer_fence(void)
{
	...
	for (i = OVL_LAYER_NUM_PER_OVL; i < OVL_LAYER_NUM; i++)
		mtkfb_release_layer_fence(MAKE_DISP_SESSION(
					DISP_SESSION_PRIMARY, 0), i);
	...
	for (i = module_num - 1; i >= 0; i--) {
		...
		if (ddp_modules_driver[module_name] != 0) {
			if (ddp_modules_driver[module_name]->enable_irq != 0) {
				DISP_LOG_I("scen%s,module%s enirq level%d\n",
				       ddp_get_scenario_name(handle->scenario),
				       ddp_get_module_name(module_name),
				       irq_level);
				ret +=
				    ddp_modules_driver[module_name]->enable_irq(
						    module_name,
						    cmdq_handle,
						    irq_level);
			}
			...
			if (ddp_modules_driver[module_name]->build_cmdq != 0) {
				DISP_LOG_D("%s build cmdq, state=%d\n",
						ddp_get_module_name(
							module_name), state);
				/* now just 0; */
				ret = ddp_modules_driver[
					module_name]->build_cmdq(module_name,
							trigger_loop_handle,
							state);
			}
		}
...
int dpmgr_path_flush(disp_path_handle dp_handle, int encmdq)
{
	...
	DISP_LOG_V("%s: Reset OVL and enable mutex %d\n", __func__,
			handle->hwmutexid);
...
int dpmgr_path_dsi_power_off(disp_path_handle dp_handle,
...
int dpmgr_path_dsi_off(disp_path_handle dp_handle, void *cmdq_handle,
		unsigned int level)
{
	int module_name;
	struct ddp_path_handle *handle;
	...
	if (module_name >= 0 && module_name < DISP_MODULE_UNKNOWN)
		...
		DISP_LOG_E("%s unknown module %d", __func__, module_name);

	ddp_path_lp_top_clock_off();
	return 0;
}

int dpmgr_path_dsi_on(disp_path_handle dp_handle, void *cmdq_handle,
		unsigned int level)
{
	int module_name;
	struct ddp_path_handle *handle;
...
int dpmgr_path_idle_off(disp_path_handle dp_handle, void *cmdq_handle,
		unsigned int level)
{
	int i = 0;
	int module_name;
	...
	/* For DSI CMD mode, do not clock pwm_26M, used to make sure
	 * MM clock off but MTCMOS on
	 */
...
int dpmgr_path_idle_on(disp_path_handle dp_handle, void *cmdq_handle,
		unsigned int level)
{
	int i = 0;
	int module_name;
	...
	/* clock on PEM_26M one more time when idle off,
	 * so need clock off it here
	 */
	if (primary_display_is_video_mode() == 0) {
#ifdef CONFIG_MTK_CLKMGR
	...
	case DISP_IOCTL_OD_CTL:
	{
		if (is_module_in_path(DISP_MODULE_OD, handle)) {
			if (ddp_modules_driver[DISP_MODULE_OD]->cmd != NULL)
				ret = ddp_modules_driver[DISP_MODULE_OD]->cmd(
						DISP_MODULE_OD, msg, arg,
						cmdqhandle);
		}
		break;
	}
...
void dpmgr_debug_path_status(int mutex_id)
{
	...
	if (mutex_id >= DISP_MUTEX_DDP_FIRST
		...
		for (i = DISP_MUTEX_DDP_FIRST; i < (DISP_MUTEX_DDP_FIRST +
					DISP_MUTEX_DDP_COUNT); i++) {
		...
		enum DISP_PATH_EVENT event, int timeout)
{
	...
	if (wq_handle->init) {
		...
		ret =
			wait_event_interruptible_timeout(wq_handle->wq,
		...
		if (ret == 0) {
			...
			/* uncomment to show more timeout info */
			/* dpmgr_path_reset(dp_handle, 0); */
			/* gDDPError = 1; */
		} else if (ret < 0) {
			DISP_LOG_E("wait %s irq by timeleft %d on scen %s\n",
				   path_event_name(event), ret,
				   ddp_get_scenario_name(handle->scenario));
		} else {
			DISP_LOG_V("received event %s timeleft %d on scen %s\n",
				   path_event_name(event), ret,
				   ddp_get_scenario_name(handle->scenario));
		}
		return ret;
		...
		if (regvalue & (0x1 << i)) {
			...
			for (j = 0; j < DISP_PATH_EVENT_NUM; j++) {
				if (handle->wq_list[j].init &&
					...
					DDPIRQ("irq signal event %s ",
					       path_event_name(j));
					DDPIRQ(
					"on cycle %llu on scenario %s\n",
					handle->wq_list[j].data,
					ddp_get_scenario_name(
						handle->scenario));
...
int dpmgr_factory_mode_test(int module_name, void *cmdqhandle, void *config)
{
	if (ddp_modules_driver[module_name] != 0) {
		if (ddp_modules_driver[module_name]->ioctl != 0) {
			...
			ddp_modules_driver[DISP_MODULE_DPI0]->ioctl(
					module_name, cmdqhandle,
...
int dpmgr_wait_ovl_available(int ovl_num)
{
	...
	if (ovl_state == DDP_OVL1_STATUS_PRIMARY ||
			ovl_state == DDP_OVL1_STATUS_SUB_REQUESTING) {
		struct ddp_path_handle *handle =
			(struct ddp_path_handle *)g_dp_handle;
	...
	while ((ovl_get_status() != DDP_OVL1_STATUS_SUB) &&
		...
		(ovl_get_status() == DDP_OVL1_STATUS_SUB ||
		 ovl_get_status() == DDP_OVL1_STATUS_IDLE), HZ/2);
		if (ret < 0)
