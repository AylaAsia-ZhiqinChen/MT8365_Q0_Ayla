	...
	struct disp_session_info __user *data)
{
	...
	err |= get_user(i, &(data->const_layer_num));
	err |= put_user(i, &(data32->const_layer_num));
	...
	struct disp_frame_cfg_t __user *data)
{
	...
	err |= get_user(p, &(data32->prev_present_fence_struct));
	err |= put_user(compat_ptr(p), &(data->prev_present_fence_struct));
