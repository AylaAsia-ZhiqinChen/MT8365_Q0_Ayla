...
#define MMC3630X_RESET_INTV		150
...
static DRIVER_ATTR(powerstatus,        0444, show_power_status, NULL);
...
static int mmc3630x_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	struct mmc3630x_i2c_data *data = NULL;
	int err = 0;
	struct mag_control_path ctl={0};
	struct mag_data_path mag_data={0};
	const char *name = "mediatek,cust_mag_mmc3630x";
    ...
    //~ MEMSIC_INFO("direction =%d\n",hw->direction);
        ...
        if (node == NULL){
                printk("mmc3630x get node fail \n");
                goto exit;
        }  

	err =   get_mag_dts_func(node, hw);
	if (err<0){
		MEMSIC_ERR("mmc3630x get dts info fail\n");
		err = -EFAULT;
		goto exit;
	}
	...
	//hw = get_mag_dts_func("mediatek,cust_mag_mmc3630x", hw);
	data->hw = hw;
	...
	ctl.libinfo.deviceid = MMC3630x_DEVICE_ID; //akm_device;	
	#else
