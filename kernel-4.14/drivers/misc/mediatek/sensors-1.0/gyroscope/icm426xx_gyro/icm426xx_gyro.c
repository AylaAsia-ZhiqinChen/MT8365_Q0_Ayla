...
#include "gyroscope.h"
#include "../../accelerometer/icm426xx_acc/icm426xx_register.h"
#include "../../accelerometer/icm426xx_acc/icm426xx_share.h"
...
struct gyro_hw gyro_icm426_cust;
static struct gyro_hw *hw = &gyro_icm426_cust;
    ...
    const struct i2c_device_id *id)
{
    ...
    int res = 0;
    const char *name = "mediatek,cust_gyro_icm426";
    struct device_node *node = NULL;	

        node = of_find_compatible_node(NULL, NULL, name);
        if (node == NULL){
                printk("icm426_gyro get node fail \n");
                return -1;
        }
	res =   get_gyro_dts_func(node, hw);
	if (res<0){
		GYRO_PR_ERR("icm426_gyro get dts info fail\n");
		res = -EFAULT;
		goto exit;
	}
    ...
    /*hw =get_gyro_dts_func("mediatek,cust_gyro_icm426", hw);
    if (hw == NULL) {
