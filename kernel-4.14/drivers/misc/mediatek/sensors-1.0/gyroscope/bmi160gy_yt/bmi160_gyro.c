/* BOSCH BMI160 Gyroscope Sensor Driver
 ...
 *          V2.1  add I2C retry in initialization
 *          V2.2  change the report value with high accuracy
 *          V3.0  update to the android O mtk6757
 */
...
#if 0
/*this address can change base on the customer side,
bmi160 include acc +gyro, only have one i2c address and only
setting in the bmi160_acc.c,0x69 is the fake address*/
static struct i2c_board_info __initdata i2c_bmi160_gyro = {
	I2C_BOARD_INFO(BMG_DEV_NAME, 0x69)};
#endif
...
static int bmg_get_chip_type(struct i2c_client *client)
{
	...
	while ((count < 3) && (err < 0)) {
		...
		client, BMI160_USER_CHIP_ID__REG, &chip_id, 1);
		udelay(1000);
		count++;
	}
	if (err != 0) {
		...
		enum BMG_POWERMODE_ENUM power_mode)
{
	...
	if (obj->sensor_type == BMI160_GYRO_TYPE) {
		...
		bmi160_gyro_delay(90);
		err += bmg_i2c_read_block(
		client, BMI160_USER_PMU_STATUS_ADDR, &data, 1);
		printk("get power status = 0x%x \n", data);
	}
	if (err < 0)
...
static int bmg_set_range(struct i2c_client *client, enum BMG_RANGE_ENUM range)
{
	...
	mutex_unlock(&obj->lock);
	printk("set range ok.range=%d sens=%d\n",
		...
		char *buf, int bufsize)
{
	...
	if (err) {
		...
		obj->sensitivity;
        #endif
		printk("gyro final xyz data: %d,%d,%d, sens:%d\n",
...
static ssize_t show_power_mode_value(struct device_driver *ddri, char *buf)
{
	...
	if (err < 0) {
		GYRO_PR_ERR("get gyro op mode failed.\n");
		return err;
	}
...
static int bmi160gyro_factory_enable_sensor(bool enabledisable, int64_t sample_periods_ms)
{
	int err = 0;
	err = bmi160_gyro_enable_nodata(enabledisable);
	if (err) {
...
static int bmi160gyro_factory_get_data(int32_t data[3], int *status)
{
	return bmi160_gyro_get_data(&data[0], &data[1], &data[2], status);
}

static int bmi160gyro_factory_get_raw_data(int32_t data[3])
{
	int err = 0;
	s16 strbuf[BMG_BUFSIZE] = {0};
	struct bmg_i2c_data *obj = obj_i2c_data;
	err = bmg_read_raw_data(obj->client, strbuf);
	if (err) {
		GYRO_INFO("bmi160_get_raw data failed!\n");
		return -1;
	}
	data[0] = (int32_t) (data[0]);
	data[1] = (int32_t) (data[1]);
	data[2] = (int32_t) (data[2]);
	return 0;
}

static int bmi160gyro_factory_enable_calibration(void)
{
	return 0;
}

static int bmi160gyro_factory_clear_cali(void)
{
	int err = 0;
	struct bmg_i2c_data *obj = obj_i2c_data;
	err = bmg_reset_calibration(obj->client);
...
static int bmi160gyro_factory_set_cali(int32_t data[3])
{
	int err = 0;
	int cali[3] = { 0 };
	struct bmg_i2c_data *obj = obj_i2c_data;
	...
	err = bmg_write_calibration(obj->client, cali);
	if (err) {
		GYRO_INFO("bmg_WriteCalibration failed!\n");
		return -1;
	}
	printk("gyro set cali:[%5d %5d %5d]\n", cali[BMG_AXIS_X],
		 cali[BMG_AXIS_Y], cali[BMG_AXIS_Z]);

	return 0;
}

static int bmi160gyro_factory_get_cali(int32_t data[3])
{
	int err = 0;
	int cali[3] = { 0 };
	int raw_offset[BMG_BUFSIZE] = { 0 };
...
static int bmi160gyro_factory_do_self_test(void)
{
	return 0;
}

static struct gyro_factory_fops bmi160gyro_factory_fops = {
	.enable_sensor = bmi160gyro_factory_enable_sensor,
	.get_data = bmi160gyro_factory_get_data,
	.get_raw_data = bmi160gyro_factory_get_raw_data,
	.enable_calibration = bmi160gyro_factory_enable_calibration,
	.clear_cali = bmi160gyro_factory_clear_cali,
	.set_cali = bmi160gyro_factory_set_cali,
	.get_cali = bmi160gyro_factory_get_cali,
	.do_self_test = bmi160gyro_factory_do_self_test,
};

static struct gyro_factory_public bmi160gyro_factory_device = {
	.gain = 1,
	.sensitivity = 1,
	.fops = &bmi160gyro_factory_fops,
};
		...
		const struct i2c_device_id *id)
{
	...
	const char *name = "mediatek,cust_gyro_bmi160";
        struct device_node *node = NULL;	


        node = of_find_compatible_node(NULL, NULL, name);
        if (node == NULL){
                printk("bmi160_gyro get node fail \n");
                return -1;
        }
	err =   get_gyro_dts_func(node, hw);
	if (err<0){
		GYRO_PR_ERR("bmi160_gyro get dts info fail\n");
		err = -EFAULT;
		goto exit;
	}
	...
	/*GYRO_FUN();*/
	 ...
	 //hw =get_gyro_dts_func("mediatek,cust_gyro_bmi160", hw);
	...
	err = gyro_factory_device_register(&bmi160gyro_factory_device);
	if (err) {
		GYRO_PR_ERR("bmi160_gyro_device misc register failed!\n");
...
static int bmi160_gyro_i2c_remove(struct i2c_client *client)
{
	...
	//misc_deregister(&bmg_device);
	err = gyro_factory_device_deregister(&bmi160gyro_factory_device);
	if (err)
...
static const struct dev_pm_ops bmi160gyro_pm_ops = {
...
{
	printk("%s: bosch gyroscope driver version: %s\n",
	__func__, BMG_DRIVER_VERSION);
	#if 0
	i2c_register_board_info(1, &i2c_bmi160_gyro, 1);
	#endif
