/* BOSCH BMI160 ACC sensor driver
 ...
 *V2.1add the I2C retry in initialization
 *V2.2clear the interrupt status after report the Sigmotion
 *V2.3use i2c_register_board_info register i2c device
 *V3.0update the code to the android O mtk6757
 */
...
//#define GSE_DEBUG //added by linhui 2019.06.10
...
//static struct i2c_board_info __initdata i2c_bmi160_acc = {
//	I2C_BOARD_INFO(BMI160_DEV_NAME, 0x68)};
	...
	const char *buf, size_t count)
{
	...
	u8 acc_odr=0, range=0, acc_selftest_amp, acc_selftest_sign;
	struct bmi160_acc_i2c_data *client_data = obj_i2c_data;
	...
	struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	case GSENSOR_IOCTL_READ_GAIN:
		...
		if (copy_to_user(data,
			&gsensor_gain, sizeof(gsensor_gain))) {
			err = -EFAULT;
		}
		break;
...
{
	int err;
	err = bmi160_acc_enable_nodata(enabledisable);
	if (err) {
...
static struct accel_factory_fops bmi160acc_factory_fops = {
...
static struct accel_factory_public bmi160acc_factory_device = {
	.gain = 1,
	.sensitivity = 1,
	.fops = &bmi160acc_factory_fops,
};
...
static const struct dev_pm_ops bmi160acc_pm_ops = {
...
static int bmi160_request_irq(struct bmi160_acc_i2c_data *client_data)
{
	...
	node = of_find_compatible_node(NULL, NULL, "mediatek,gyro_bmi160");
	...
	if (node) {
		...
		err = request_irq(client_data->IRQ, bmi_irq_handler,
			...
			if (err > 0)
				printk("bmi160 request_irq failed");
	} else
		printk("[%s] can not find!", __func__);

	printk("irq_num= %d IRQ_num=%d",
	...
	struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	const char *name = "mediatek,cust_acc_bmi160";
        struct device_node *node = NULL;	
	printk("%s: is begin.\n", __func__);

        node = of_find_compatible_node(NULL, NULL, name);
        if (node == NULL){
                printk("bmi160_acc get node fail \n");
                return -1;
        }
	err =   get_accel_dts_func(node, hw);
	if (err) {
		GSE_ERR("bmi160_acc get dts info fail\n");
		err = -EFAULT;
		goto exit;
	}
	
    //hw =get_accel_dts_func("mediatek,cust_acc_bmi160", hw);
...
/*	GSE_LOG("%s: is begin.\n", __func__);
	...
	/*this need care in the DTS side*/
	obj->hw = hw;
	...
	#endif
	err = accel_factory_device_register(&bmi160acc_factory_device);
	if (err) {
...
static int bmi160_acc_i2c_remove(struct i2c_client *client)
{
	...
	accel_factory_device_deregister(&bmi160acc_factory_device);
	if (err)
...
{
	GSE_FUN();
	//i2c_register_board_info(1, &i2c_bmi160_acc, 1);
