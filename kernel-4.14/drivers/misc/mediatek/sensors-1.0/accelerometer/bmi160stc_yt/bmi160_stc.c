 ...
 *
*/
static BMI160_RETURN_FUNCTION_TYPE bmi160_set_intr_edge_ctrl(
 ...
 *
*/
static BMI160_RETURN_FUNCTION_TYPE bmi160_set_intr_level(
 ...
 *
*/
static BMI160_RETURN_FUNCTION_TYPE bmi160_set_output_enable(
...
static int step_c_get_chip_type(struct i2c_client *client)
{
	...
	if (obj->sensor_type == INVALID_TYPE) {
		printk("unknown sensor.\n");
		return ERROR;
	}
	printk("read sensor chip id = %x ok.\n", (int)chip_id);
	return SUCCESS;
}
...
{
	int err = 0;
	printk("bmi160_step_c_enable_nodata\n");

	if (bmi160_set_step_counter_enable(en) < 0) {
		printk("set bmi160_STEP_COUNTER error");
		return -EINVAL;
	}
	bmi160_stc_delay(BMI160_GEN_READ_WRITE_DELAY);
	if (en == 1)
...
static int bmi160_step_c_set_delay(u64 ns)
{
	...
	printk("bmi160_step_c_set_delay\n");
	if (value <= 5)
		...
		sample_delay = BMI160_ACCEL_ODR_100HZ;
	printk("sensor delay value = %d, sample delay = %d\n",
			value, sample_delay);
	err = step_c_set_datarate(sample_delay);
	if (err < 0)
		printk("set data rate error.\n");
	return err;
}

static int bmi160_floor_set_delay(u64 ns)
{
	return 0;
}
...
static int bmi160_step_c_enable_significant(int en)
{
	...
	printk("bmi160_step_c_enable_significant, en = %d\n", en);
	if (bmi160_set_intr_significant_motion_select(en) < 0) {
		printk("set bmi160_significant error");
		return -EINVAL;
	}
	if (en == 1) {
		err = step_c_set_powermode(STC_NORMAL_MODE);
	...
	if (err)
		printk("set acc_config failed");
	obj_i2c_data->sigmotion_enable = en;
	return err;
}

static int bmi160_floor_enable(int en)
{
	return 0;
}
...
static int bmi160_step_c_enable_step_detect(int en)
{
	...
	printk("bmi160_step_c_enable_step_detect, en = %d\n", en);
	...
	if (bmi160_set_intr_low_g(BMI160_MAP_INTR1, en) < 0) {
		printk("set bmi160_STEP_COUNTER_lowg error");
		return -EINVAL;
	}
	#endif
	bmi160_stc_delay(BMI160_GEN_READ_WRITE_DELAY);
...
static int bmi160_step_c_get_data(u32 *value, int *status)
{
	u16 data;
	int err;
	...
	if (0 == data) {
		...
		if (err < 0)
			printk("bmi160_acc_get_mode failed.\n");
		printk("step_c_get_data acc mode = %d.\n", acc_mode);
		err = get_step_counter_enable(obj->client, &stc_enable);
		if (err < 0)
			printk("get_step_counter_enable failed.\n");
		printk("step_c_get_data stc enable = %d.\n", stc_enable);
		if (acc_mode != 0 && stc_enable == 1)
			data = last_stc_value;
	}
	bmi160_stc_delay(30);
	if (err < 0) {
		printk("read step count fail.\n");
		return err;
	}
...
{
	printk("bmi160_stc_get_data_significant\n");
	return 0;
}

static int bmi160_stc_get_data_step_d(uint32_t *value, int *status)
{
	printk("bmi160_stc_get_data_step_d\n");

	return 0;
}

static int bmi160_get_data_floor(uint32_t *value, int *status)
{
	printk("bmi160_stc_get_data_step_d\n");
...
static int step_c_i2c_probe(void)
{
	...
	printk("step_c_i2c_probe\n");
        if(bmi160_acc_i2c_client == NULL){
            printk("%s: bmi160 client fail \n", __func__);
            err = -EFAULT;
	    goto exit;
        }
	...
	ctl.enable_floor_c = bmi160_floor_enable;
	ctl.floor_c_set_delay = bmi160_floor_set_delay;
	ctl.floor_c_batch = NULL;
	ctl.floor_c_flush = NULL;
	...
	data.get_data_floor_c = bmi160_get_data_floor;
	...
	if (err)
		printk("bmi160_map_interrupt fail = %d\n", err);
	step_c_init_flag = 0;
	printk("%s: is ok.\n", __func__);
	return 0;
...
{
	printk("bmi160_stc_local_init.\n");
	if (step_c_i2c_probe()) {
		printk("failed to register bmi160 step_c driver\n");
		return -ENODEV;
	}
	return 0;
}
