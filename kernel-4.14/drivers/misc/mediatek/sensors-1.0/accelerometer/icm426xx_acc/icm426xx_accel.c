...
struct acc_hw accel_icm426_cust;
static struct acc_hw *hw = &accel_icm426_cust;
struct i2c_client *icm426xx_accel_i2c_client = NULL;
    ...
    const struct i2c_device_id *id)
{
    ...
    int res = 0;
    	...
    	const char *name = "mediatek,cust_acc_icm426";
	...
	printk("%s: is begin.\n", __func__);
        ...
        if (node == NULL){
                printk("icm426_acc get node fail \n");
                return -1;
        }
	res =   get_accel_dts_func(node, hw);
	if (res) {
		ACC_PR_ERR("icm426_acc get dts info fail\n");
		res = -EFAULT;
		goto exit;
	}   
    ...
    if (!obj) {
        ...
        /*hw =get_accel_dts_func("mediatek,cust_acc_icm426", hw);
        if (hw == NULL) {
    ...
    //    client->addr = i;
    res = icm426xx_share_ChipSoftReset();
    //if (res == ICM426XX_SUCCESS) break;
    //}
    printk("icm426 addr = 0x%x  \n",client->addr);
    if (res != ICM426XX_SUCCESS)
...
static int icm426xx_accel_local_init(void)
{
    ...
    initDebugData();
    printk("icm426xx_accel_local_init\n");
    if (i2c_add_driver(&icm426xx_accel_i2c_driver)) {
        ACC_PR_ERR("add driver error\n");
        return -1;
    }
    if (-1 == icm426xx_accel_init_flag){
        printk("icm426xx_accel_local_init fail \n");
        return -1;
    }
    printk("icm426xx_accel_local_init success \n");
    return 0;
}
