				    ...
				    unsigned int a_u4ParamSize)
{
	...
	pFlag = kmalloc(sizeof(unsigned short), GFP_ATOMIC);
	...
	if (pFlag != VAL_NULL) {
		...
		*((unsigned short *)a_prParam->pvReserved) = VAL_FALSE;
	} else {
				   ...
				   unsigned int a_u4ParamSize)
{
	...
	pFlag = (unsigned short *)a_prParam->pvReserved;
	kfree(pWaitQueue);
	kfree(pFlag);
				  ...
				  unsigned int a_u4ParamSize)
{
	...
	i4Ret = wait_event_interruptible_timeout(
		*pWaitQueue,
		*((unsigned short *)a_prParam
			  ->pvReserved) /*g_mflexvideo_interrupt_handler */,
		timeout_jiff);
	if (i4Ret == 0) {
		pr_debug(
			"[VCODEC] %s wake up by VAL_RESULT_INVALID_ISR",
			__func__);
				 ...
				 unsigned int a_u4ParamSize)
{
	...
	/* Add one line comment for avoid kernel coding style*/
		*((unsigned short *)a_prParam->pvReserved) = VAL_TRUE;
	} else {
		pr_debug(
				    ...
				    unsigned int a_u4ParamSize)
{
	...
	/* Add one line comment for avoid kernel coding style */
		...
		pr_debug(
		"[VCODEC][ERROR]Enable to create mutex!\n");
	}
	/* init_MUTEX(pLock); */
	sema_init(pLock, 1);
	return VAL_RESULT_NO_ERROR;
