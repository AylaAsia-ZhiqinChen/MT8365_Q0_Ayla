...
#ifdef CONFIG_ARM64
static int systracker_timeout_handler(unsigned long addr,
	unsigned int fsr, struct pt_regs *regs)
{
	...
	if (readl(IOMEM(BUS_DBG_CON)) &
		...
		pr_notice("%s:%d: write timeout BUS_DBG_CON:0x%x\n",
			__func__, __LINE__, readl(IOMEM(BUS_DBG_CON)));
	...
	if (readl(IOMEM(BUS_DBG_CON)) &
		...
		pr_notice("%s:%d: read timeout BUS_DBG_CON:0x%x\n",
			__func__, __LINE__, readl(IOMEM(BUS_DBG_CON)));
		//aee_dump_backtrace(regs, NULL);
		for (i = 0; i < BUS_DBG_NUM_TRACKER; i++) {
