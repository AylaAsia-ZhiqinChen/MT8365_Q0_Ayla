...
int Binning_DRAM_complex_mem_test(void)
{
	...
	/* === Read Check then Fill Memory
	 *with a5a5a5a5 Pattern ===
	 */
	for (i = 0; i < size; i++) {
	...
	/* === Read Check then Fill Memory
	 *with 00 Byte Pattern at offset 0h ===
	 */
	for (i = 0; i < size; i++) {
	...
	/* === Read Check then Fill Memory
	 *with 00 Byte Pattern at offset 2h ===
	 */
	for (i = 0; i < size; i++) {
	...
	/* === Read Check then Fill Memory
	 *with 00 Byte Pattern at offset 1h ===
	 */
	for (i = 0; i < size; i++) {
	...
	/* === Read Check then Fill Memory
	 *with 00 Byte Pattern at offset 3h ===
	 */
	for (i = 0; i < size; i++) {
	...
	/* === Read Check then Fill Memory
	 *with ffff Word Pattern at offset 1h ==
	 */
	for (i = 0; i < size; i++) {
	...
	/* === Read Check then Fill Memory
	 *with ffff Word Pattern at offset 0h ==
	 */
	for (i = 0; i < size; i++) {
...
int __init dram_test_init(void)
{
	...
	if (node) {
		...
		pr_debug("can't find DRAMC0 compatible node\n");
		return -1;
	}
	...
	if (node) {
		...
		pr_debug("can't find DDRPHY compatible node\n");
		return -1;
	}
#if 0
	...
	if (node) {
		...
		pr_debug("can't find DDRPHY1 compatible node\n");
		return -1;
	}
#endif
	...
	if (node) {
		...
		pr_debug("can't find DRAMCNAO compatible node\n");
		return -1;
	}

	ret = platform_driver_register(&dram_test_drv);
	if (ret) {
		pr_debug("fail to create the dram_test driver\n");
		return ret;
	}
