...
m4u_callback_ret_t cmdq_M4U_TranslationFault_callback(int port,
...
void cmdq_mdp_init_module_base_VA(void)
{
	...
	gCmdqMdpModuleBaseVA.MDP_RDMA = cmdq_dev_alloc_module_base_VA_by_name(
		"mediatek,mt8167-mdp_rdma");
	gCmdqMdpModuleBaseVA.MDP_RSZ0 = cmdq_dev_alloc_module_base_VA_by_name(
		"mediatek,mt8167-mdp_rsz0");
	gCmdqMdpModuleBaseVA.MDP_RSZ1 = cmdq_dev_alloc_module_base_VA_by_name(
		"mediatek,mt8167-mdp_rsz1");
	gCmdqMdpModuleBaseVA.MDP_WDMA = cmdq_dev_alloc_module_base_VA_by_name(
		"mediatek,mt8167-mdp_wdma");
	gCmdqMdpModuleBaseVA.MDP_WROT = cmdq_dev_alloc_module_base_VA_by_name(
		"mediatek,mt8167-mdp_wrot");
	gCmdqMdpModuleBaseVA.MDP_TDSHP = cmdq_dev_alloc_module_base_VA_by_name(
		"mediatek,mdp_tdshp");
	gCmdqMdpModuleBaseVA.VENC = cmdq_dev_alloc_module_base_VA_by_name(
		"mediatek,mt8167-VENC");
#endif
}
...
void cmdq_mdp_init_module_clk(void)
{
	...
	cmdq_dev_get_module_clock_by_name("mediatek,mt8167-gce",
		"MT_CG_DISP0_CAM_MDP",
...
static struct platform_device *cmdq_mdp_get_smi_device(void)
{
	static struct platform_device *smi_device;
	struct device_node *smi_node = NULL;

	if (smi_device)
		return smi_device;

	smi_node = of_parse_phandle(cmdq_dev_get()->of_node,
		"mediatek,larb", 0);
	if (!smi_node) {
		CMDQ_ERR("cmdq driver get larb fail\n");
		return NULL;
	}

	smi_device = of_find_device_by_node(smi_node);
	of_node_put(smi_node);
	if ((!smi_device) || (!smi_device->dev.driver)) {
		if (!smi_device)
			CMDQ_ERR("earlier than SMI, larb_pdev null\n");
		else
			CMDQ_ERR("earlier than SMI, larb drv null\n");
		return NULL;
	}
...
#else
	struct platform_device *smi_device = cmdq_mdp_get_smi_device();

	if (smi_device == NULL) {
		CMDQ_ERR("get iommu fail\n");
		return;
	}

	if (enable) {
		/* smi power & clock */
		pm_runtime_get_sync(&smi_device->dev);
	} else {
		/* smi power & clock */
		pm_runtime_put_sync(&smi_device->dev);
	}
#endif
}
...
enum MOUT_BITS {
	...
	/* bit  4: MDP_TDSHP0 multiple outupt reset */
...
int32_t cmdqMdpResetEng(uint64_t engineFlag)
{
	...
	if (engineToResetAgain != 0) {
		...
		CMDQ_ERR("Reset failed MDP engines(0x%llx), reset again\n",
			 engineToResetAgain);

		/* reset again with MMSYS_SW0_RST_B */
...
int32_t cmdqMdpClockOff(uint64_t engineFlag)
{
	...
	if (engineFlag & (1LL << CMDQ_ENG_MDP_WROT0)) {
		cmdq_mdp_loop_off(CMDQ_ENG_MDP_WROT0,
				  MDP_WROT_BASE + 0X010, MDP_WROT_BASE + 0X014,
				  0x1, 0x1, true);
	}
...
void cmdqMdpInitialSetting(void)
{
	...
	m4u_register_fault_callback(M4U_PORT_MDP_RDMA,
		cmdq_M4U_TranslationFault_callback, (void *)data);
	m4u_register_fault_callback(M4U_PORT_MDP_WDMA,
		cmdq_M4U_TranslationFault_callback, (void *)data);
	m4u_register_fault_callback(M4U_PORT_MDP_WROT,
		cmdq_M4U_TranslationFault_callback, (void *)data);
#endif
#endif
}
...
const char *cmdq_mdp_dispatch(uint64_t engineFlag)
{
	...
	if (engineFlag & (1LL << CMDQ_ENG_MDP_RDMA0)) {
		...
		/* 1-in 2-out */
		if ((engineFlag & (1LL << CMDQ_ENG_MDP_RSZ0))
			&& (engineFlag & (1LL << CMDQ_ENG_MDP_RSZ1))) {
			...
			if ((state[0] == 0xa) && (state[1] == 0xa))
				module = "ISP (caused mdp upstream hang)";
		} else {/* 1-in 1-out */
			if (engineFlag & (1LL << CMDQ_ENG_MDP_RSZ0)) {
				CMDQ_REG_SET32(MDP_RSZ0_BASE + 0x040,
					0x00000002);
				state[0] = CMDQ_REG_GET32(
						MDP_RSZ0_BASE + 0x044) & 0xF;
			}
			if (engineFlag & (1LL << CMDQ_ENG_MDP_RSZ1)) {
				CMDQ_REG_SET32(MDP_RSZ1_BASE + 0x040,
					0x00000002);
				state[1] = CMDQ_REG_GET32(
					MDP_RSZ1_BASE + 0x044) & 0xF;
			}
			/* 1,0,1,0 */
			if ((state[0] == 0xa) || (state[1] == 0xa))
				module = "ISP (caused mdp upstream hang)";
