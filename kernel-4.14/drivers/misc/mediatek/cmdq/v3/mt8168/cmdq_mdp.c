...
int32_t cmdq_mdp_reset_with_mmsys(const uint64_t engineToResetAgain)
{
	...
	int engineResetBit[32] = {
		...
		CMDQ_ENG_MDP_WROT1,	/* bit  6 : MDP_WROT1 */
		-1,
		-1,
		-1,
		-1,
		...
		[24 ... 29] = -1,
		CMDQ_ENG_MDP_RDMA1,	/* bit  30 : MDP_RDMA1 */
		-1
	};
	...
	int port, unsigned int mva, void *data)
{
	...
	case M4U_PORT_MDP_RDMA0:
		cmdq_mdp_dump_rdma(MDP_RDMA0_BASE, "RDMA0");
		break;
//	case M4U_PORT_MDP_RDMA1: //TODO
//		cmdq_mdp_dump_rdma(MDP_RDMA1_BASE, "RDMA1");
//		break;
	case M4U_PORT_MDP_WROT0:
		cmdq_mdp_dump_rot(MDP_WROT0_BASE, "WROT0");
		break;
//	case M4U_PORT_MDP_WROT1://TODO
//		cmdq_mdp_dump_rot(MDP_WROT1_BASE, "WROT1");
//		break;
	default:
		CMDQ_ERR("[MDP M4U]fault callback function");
...
void cmdq_mdp_enable_clock(bool enable, enum CMDQ_ENG_ENUM engine)
{
	...
	/* clk module may probe later than cmdq, should check mdp_wrot0 */
	if (enable && IS_ERR(gCmdqMdpModuleClock.clk_MDP_WROT0)) {
		CMDQ_LOG("try to init all clk again\n");
...
enum MOUT_BITS {//MOUT_RST
	...
	MOUT_BITS_MDP_RDMA1 = 10,/* bit 10: MDP_RDMA1 multiple outupt reset */
};
...
static void cmdq_mdp_enable_common_clock(bool enable)
{
	...
	if (enable) {
		/* NOTE: first enable mdp power, then smi power & clock. */
		/* mdp power: mt_disp (display + MDP) */
