...
/*example. value "5" mean that 5 times check most have 5 exceptions,*/
/* else result is OK .  */
...
static void gsl_monitor_worker(struct work_struct *work)
{
	...
	if (i2c_lock_flag == 0) {
		...
		if (!result)
			check_result_flag++;
		GSL_LOGD(
			"--result num is[%d] check_result_flag[%d] check_times[%d] check_result_flag[%d]\n",
			result, check_result_flag, check_times,
			check_result_flag);
...
static void green_mode(struct i2c_client *client, int mode)
{
	...
	for (i = 0; i < 5; i++) {
		...
		buf[1] = 0x00;
		if (mode == MODE_ON) {
			GSL_LOGD("%s is on.", __func__);
			buf[2] = 0x01;
		} else if (mode == MODE_OFF) {
			GSL_LOGD("%s is off.", __func__);
			buf[2] = 0x00;
		}
		buf[3] = 0x5a;
			 ...
			 const struct i2c_device_id *id)
{
	...
	if (get_boot_mode() == RECOVERY_BOOT) {
		GSL_LOGE(TPD_DEVICE
			 " recovery mode do not need touch panel!\n");
		return 0;
	}

	probe_thread =
		kthread_run(tpd_registration, (void *)client, "tpd_i2c_probe");
	...
	GSL_LOGD("%s start.wait_event_interruptible", __func__);
	wait_event_interruptible_timeout(init_waiter, check_flag == true,
					 5 * HZ);
	GSL_LOGD("%s end.wait_event_interruptible", __func__);
