...
//#define CONFIG_GTP_USE_PINCTRL //Add by linhui
...
static int tpd_irq_registration(void)
{
	...
	node = of_find_compatible_node(NULL, NULL, "mediatek,cap_touch_gt");//0704
		...
		if (touch_num) {
			for (i = 0; i < touch_num; i++) {
				...
				if (strncmp(CONFIG_MTK_LCM_PHYSICAL_ROTATION, "90", 2) == 0) {
					tpd_rotate_90(&input_x, &input_y);
				}
#endif
#if 1
				tpd_down(input_x, (1080-input_y), input_w, id);
#else
