...
static void mtk_uart_apdma_start_tx(struct mtk_chan *c)
{
	...
	unsigned int wpt, tmp;

	tmp = c->cfg.dst_port_window_size;
	...
	mtk_uart_apdma_write(c, VFF_INT_EN, VFF_TX_INT_EN_B);
	if (mtk_uart_apdma_read(c, VFF_FLUSH) == 0)
...
static void mtk_uart_apdma_start_rx(struct mtk_chan *c)
{
	...
	tmp = c->cfg.src_port_window_size;
	if (mtk_uart_apdma_read(c, VFF_LEN) == 0) {
		mtk_uart_apdma_write(c, VFF_ADDR, d->addr);
		mtk_uart_apdma_write(c, VFF_LEN, tmp);
...
static void mtk_uart_apdma_rx_handler(struct mtk_chan *c)
{
	...
	if (mtk_uart_apdma_read(c, VFF_VALID_SIZE) == 0)
		return;
...
static int mtk_uart_apdma_alloc_chan_resources(struct dma_chan *chan)
{
	...
	error = pm_runtime_get_sync(mtkd->ddev.dev);
	if (error < 0) {
		dev_err(chan->device->dev, "%s pm runtime get: %i\n",
			__func__, error);
...
static int mtk_uart_apdma_terminate_all(struct dma_chan *chan)
{
	...
	ret = readx_poll_timeout(readl, c->base + VFF_EN, tmp, !tmp, 0, 1000);
	if (ret)
