...
static int scpsys_power_on(struct generic_pm_domain *genpd)
{
	...
	for (i = 0; i < MAX_CLKS && scpd->clk[i]; i++) {
		...
		if (ret) {
			...
			dev_notice(scp->dev, "%s: clk failed\n", genpd->name);
			goto err_clk;
		}
	}
	...
	while (1) {
		...
		if (expired) {
			ret = -ETIMEDOUT;
			dev_notice(scp->dev, "%s: PWR_ACK timeout\n",
						genpd->name);
			goto err_pwr_ack;
		}
	...
	while (sram_pdn_ack && (readl(ctl_addr) & sram_pdn_ack)) {
		...
		if (expired) {
			ret = -ETIMEDOUT;
			dev_notice(scp->dev, "%s: SRAM_PDN_ACK timeout\n",
					genpd->name);
			goto err_pwr_ack;
		}
	...
	if (scpd->data->bus_prot_mask) {
		ret = mtk_infracfg_clear_bus_protection(scp->infracfg,
				scpd->data->bus_prot_mask, true);
		if (ret) {
			dev_notice(scp->dev, "%s: bus protection failed\n",
					genpd->name);
			goto err_pwr_ack;
		}
	}
			...
			const struct scp_domain_data *scp_domain_data, int num)
{
	...
	if (IS_ERR(scp->infracfg)) {
		dev_notice(&pdev->dev, "Cannot find infracfg controller: %ld\n",
	...
	for (i = 0; i < num; i++) {
		...
		for (j = 0; j < MAX_CLKS && data->clk_id[j]; j++) {
			...
			if (IS_ERR(c)) {
				dev_notice(&pdev->dev, "%s: clk unavailable\n",
					data->name);
				return ERR_CAST(c);
			}
				...
				struct scp *scp, int num)
{
	...
	if (ret)
		dev_notice(&pdev->dev, "Failed to add OF provider: %d\n", ret);
}
...
static int __init scpsys_probe_mt8167(struct platform_device *pdev)
{
	...
	ret = pm_genpd_add_subdomain(
		pd_data->domains[MT8167_POWER_DOMAIN_MFG_ASYNC],
		pd_data->domains[MT8167_POWER_DOMAIN_MFG_2D]);
	if (ret && IS_ENABLED(CONFIG_PM))
		dev_info(&pdev->dev, "Failed to add subdomain: %d\n", ret);

	ret = pm_genpd_add_subdomain(
		pd_data->domains[MT8167_POWER_DOMAIN_MFG_2D],
		pd_data->domains[MT8167_POWER_DOMAIN_MFG]);
	if (ret && IS_ENABLED(CONFIG_PM))
		dev_info(&pdev->dev, "Failed to add subdomain: %d\n", ret);
