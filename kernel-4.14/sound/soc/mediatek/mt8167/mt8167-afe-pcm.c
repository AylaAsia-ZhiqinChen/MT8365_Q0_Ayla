			 ...
			 (struct snd_pcm_substream *substream)
{
	...
	if (ret || hw_ptr == 0) {
		dev_dbg(afe->dev, "%s hw_ptr err ret = %d\n", __func__, ret);
...
static int mt8167_afe_set_adda_in(struct mtk_afe *afe, unsigned int rate)
{
	...
	regmap_update_bits(afe->regmap,
		AFE_ADDA_UL_SRC_CON0, 0x001e0000, val);

	regmap_update_bits(afe->regmap,
		AFE_ADDA_NEWIF_CFG1, 0xc00, val2);

	regmap_update_bits(afe->regmap,
		AFE_ADDA_TOP_CON0, 0x1, 0x0);
...
{
	regmap_update_bits(afe->regmap,
		AFE_ADDA_DL_SRC2_CON0, 0x1, enable);
...
{
	regmap_update_bits(afe->regmap,
		AFE_ADDA_UL_SRC_CON0, 0x1, enable);
...
void mt8167_afe_set_hw_digital_gain_mode(struct mtk_afe *afe,
					 uint32_t gain_type,
					 uint32_t sample_rate,
					 uint32_t sample_per_step)
{
	uint32_t value = sample_per_step << 8 |
				 (mt8167_afe_rate_to_idx(sample_rate) << 4);
...
void mt8167_afe_set_hw_digital_gain_state(struct mtk_afe *afe, int gain_type,
					 bool enable)
{
	switch (gain_type) {
	case MT8167_AFE_HW_DIGITAL_GAIN1:
	...
	case MT8167_AFE_HW_DIGITAL_GAIN2:
		if (enable) {
			/* Let current gain be 0 to ramp up */
			regmap_update_bits(afe->regmap, AFE_GAIN2_CUR,
						 0xFFFFF, 0);
		}
...
static int mt8167_afe_set_mrg_i2s(struct mtk_afe *afe, unsigned int rate)
{
	...
	regmap_update_bits(afe->regmap, AFE_MRGIF_CON, 0xf00000,
				 rate_idex << 20);
	return 0;
}
...
static int mt8167_afe_set_mrg(struct mtk_afe *afe, unsigned int rate)
{
	...
	regmap_update_bits(afe->regmap,
		AFE_MRGIF_CON, 0xf00000, 9 << 20);
	regmap_update_bits(afe->regmap,
		AFE_DAIBT_CON0, 0x1208, val);
...
static void mt8167_afe_enable_mrg(struct mtk_afe *afe)
{
	...
	regmap_update_bits(afe->regmap,
		AFE_MRGIF_CON, 1 << 16, 1 << 16);
	regmap_update_bits(afe->regmap,
		AFE_MRGIF_CON, 0x1, 0x1);

	udelay(100);

	regmap_update_bits(afe->regmap,
		AFE_DAIBT_CON0, 0x3, 0x3);
}
...
static void mt8167_afe_disable_mrg(struct mtk_afe *afe)
{
	...
	regmap_update_bits(afe->regmap,
		AFE_DAIBT_CON0, 0x3, 0x0);

	udelay(100);

	regmap_update_bits(afe->regmap,
		AFE_MRGIF_CON, 1 << 16, 0x0);
	regmap_update_bits(afe->regmap,
		AFE_MRGIF_CON, 0x1, 0x0);
}
...
static int mt8167_afe_enable_irq(struct mtk_afe *afe,
	struct mt8167_afe_memif *memif)
{
	...
	case MT8167_AFE_IRQ_1:
		regmap_update_bits(afe->regmap,
			AFE_IRQ_MCU_CON, 1 << 0, 1 << 0);
		break;
	case MT8167_AFE_IRQ_2:
		regmap_update_bits(afe->regmap,
			AFE_IRQ_MCU_CON, 1 << 1, 1 << 1);
		break;
	case MT8167_AFE_IRQ_5:
		regmap_update_bits(afe->regmap,
			AFE_IRQ_MCU_CON2, 1 << 3, 1 << 3);
		break;
	case MT8167_AFE_IRQ_7:
		regmap_update_bits(afe->regmap,
			AFE_IRQ_MCU_CON, 1 << 14, 1 << 14);
		break;
	case MT8167_AFE_IRQ_10:
		regmap_update_bits(afe->regmap,
			AFE_IRQ_MCU_CON2, 1 << 4, 1 << 4);
		break;
	default:
		break;
	}
...
static int mt8167_afe_disable_irq(struct mtk_afe *afe,
	struct mt8167_afe_memif *memif)
{
	int irq_mode = memif->data->irq_mode;
	unsigned long flags;
				    ...
				    struct clk *m_ck, struct clk *b_ck)
{
	...
	if (m_ck) {
		...
		if (ret) {
			dev_dbg(afe->dev, "Failed to enable m_ck\n");
			return ret;
		}
	}
	...
	if (b_ck) {
		...
		if (ret) {
			dev_dbg(afe->dev, "Failed to enable b_ck\n");
			return ret;
		}
	}
#endif
				 ...
				 struct clk *b_ck, unsigned int bck_rate)
{
	...
	if (m_ck) {
		...
		if (ret) {
			dev_dbg(afe->dev, "Failed to set m_ck rate\n");
			return ret;
		}
	}
	...
	if (b_ck) {
		...
		if (ret) {
			dev_dbg(afe->dev, "Failed to set b_ck rate\n");
			return ret;
		}
	}
#endif
			       ...
			       struct snd_soc_dai *dai)
{
	...
	if (substream->stream == SNDRV_PCM_STREAM_PLAYBACK ||
		...
		mt8167_afe_dais_enable_clks(afe,
		afe->clocks[MT8167_CLK_APLL12_DIV1], NULL);

	if (substream->stream == SNDRV_PCM_STREAM_CAPTURE ||
	    clk_mode == MT8167_AFE_I2S_SHARED_CLOCK)
		mt8167_afe_dais_enable_clks(afe,
		afe->clocks[MT8167_CLK_APLL12_DIV2], NULL);
				 ...
				 struct snd_soc_dai *dai)
{
	struct snd_soc_pcm_runtime *rtd = substream->private_data;
	struct mtk_afe *afe = snd_soc_platform_get_drvdata(rtd->platform);
	struct mt8167_afe_be_dai_data *be =
		&afe->be_data[dai->id - MT8167_AFE_BACKEND_BASE];
	...
	if (be->prepared[stream]) {
		...
		if (rate % 8000)
			mt8167_afe_disable_apll_associated_cfg(afe,
			MT8167_AFE_APLL1);
		else
			mt8167_afe_disable_apll_associated_cfg(afe,
			MT8167_AFE_APLL2);
	...
	if (reset_i2s_out_change)
		mt8167_afe_dais_disable_clks(afe,
		afe->clocks[MT8167_CLK_APLL12_DIV1], NULL);
...
afe->clocks[MT8167_CLK_AUD1] : afe->clocks[MT8167_CLK_AUD2]);
	...
	if (apply_i2s_in_change) {
		clk_set_parent(afe->clocks[MT8167_CLK_I2S2_M_SEL],
			(rate % 8000) ?
afe->clocks[MT8167_CLK_AUD1] : afe->clocks[MT8167_CLK_AUD2]);

		mt8167_afe_dais_set_clks(afe,
			afe->clocks[MT8167_CLK_APLL12_DIV2],
			       ...
			       struct snd_soc_dai *dai)
{
	...
	if (stream == SNDRV_PCM_STREAM_PLAYBACK ||
	    clk_mode == MT8167_AFE_I2S_SHARED_CLOCK)
		mt8167_afe_dais_enable_clks(afe,
		afe->clocks[MT8167_CLK_APLL12_DIV3], NULL);

	if (stream == SNDRV_PCM_STREAM_CAPTURE ||
	    clk_mode == MT8167_AFE_I2S_SHARED_CLOCK)
		mt8167_afe_dais_enable_clks(afe,
		afe->clocks[MT8167_CLK_APLL12_DIV0], NULL);
			       ...
			       struct snd_soc_dai *dai)
{
	...
	if (rate % 8000)
		mt8167_afe_enable_apll_associated_cfg(afe,
		MT8167_AFE_APLL1);
	else
		mt8167_afe_enable_apll_associated_cfg(afe, MT8167_AFE_APLL2);
	...
	if (apply_i2s_out_change) {
		clk_set_parent(afe->clocks[MT8167_CLK_I2S3_M_SEL],
			(rate % 8000) ?
afe->clocks[MT8167_CLK_AUD1] : afe->clocks[MT8167_CLK_AUD2]);

		mt8167_afe_dais_set_clks(afe,
			afe->clocks[MT8167_CLK_APLL12_DIV3],
	...
	if (apply_i2s_in_change) {
		clk_set_parent(afe->clocks[MT8167_CLK_I2S0_M_SEL],
			(rate % 8000) ?
afe->clocks[MT8167_CLK_AUD1] : afe->clocks[MT8167_CLK_AUD2]);

		mt8167_afe_dais_set_clks(afe,
			afe->clocks[MT8167_CLK_APLL12_DIV0],
...
static int mt8167_afe_mrg_bt_trigger(struct snd_pcm_substream *substream,
	int cmd,
	struct snd_soc_dai *dai)
{
	struct snd_soc_pcm_runtime *rtd = substream->private_data;
			  ...
			  struct snd_soc_dai *dai)
{
	...
	/*set hw gain*/
	mt8167_afe_set_hw_digital_gain_mode(afe, MT8167_AFE_HW_DIGITAL_GAIN1,
						 runtime->rate, 0x80);
	mt8167_afe_set_hw_digital_gain_state(afe, MT8167_AFE_HW_DIGITAL_GAIN1,
						 true);
	mt8167_afe_set_hw_digital_gain(afe, afe->mrgrx_priv->mrgrx_volume,
				...
				struct snd_soc_dai *dai)
{
	...
	const unsigned int out_channels_per_sdata =
		mt8167_afe_tdm_out_ch_per_sdata(tdm_out_mode,
		runtime->channels);
	const int phy_width = snd_pcm_format_physical_width(runtime->format);
	const int out_bit_width =
		mt8167_afe_tdm_out_bitwidth_fixup(tdm_out_mode, phy_width);
	const unsigned int stream = substream->stream;
	unsigned int val;
	...
	if (rate % 8000) {
		mt8167_afe_enable_apll_associated_cfg(afe, MT8167_AFE_APLL1);
		clk_set_parent(afe->clocks[MT8167_CLK_I2S4_M_SEL],
			afe->clocks[MT8167_CLK_AUD1]);
	} else {
		mt8167_afe_enable_apll_associated_cfg(afe, MT8167_AFE_APLL2);
		clk_set_parent(afe->clocks[MT8167_CLK_I2S4_M_SEL],
			afe->clocks[MT8167_CLK_AUD2]);
	}
	...
	if (tdm_out_mode != MT8167_AFE_TDM_OUT_HDMI)
		regmap_update_bits(afe->regmap, AFE_I2S_CON1,
			   AFE_I2S_CON1_TDMOUT_MUX_MASK,
			   AFE_I2S_CON1_TDMOUT_TO_PAD);
...
static int mt8167_afe_hdmi_trigger(struct snd_pcm_substream *substream,
	int cmd,
	struct snd_soc_dai *dai)
{
	struct snd_soc_pcm_runtime *rtd = substream->private_data;
	...
	case SNDRV_PCM_TRIGGER_RESUME:
		...
		/* enable Out control */
		regmap_update_bits(afe->regmap,
		AFE_HDMI_OUT_CON0, 0x1, 0x1);
		return 0;
	case SNDRV_PCM_TRIGGER_STOP:
	case SNDRV_PCM_TRIGGER_SUSPEND:
		/* disable Out control */
		regmap_update_bits(afe->regmap,
		AFE_HDMI_OUT_CON0, 0x1, 0);
		return 0;
	default:
		return -EINVAL;
	}
				...
				unsigned int fmt)
{
	...
	case SND_SOC_DAIFMT_LEFT_J:
		...
		break;
	default:
		dev_dbg(afe->dev, "invalid dai format %u\n", fmt);
		return -EINVAL;
	}
	return 0;
}
				...
				struct snd_soc_dai *dai)
{
	...
	mt8167_afe_dais_enable_clks(afe,
		afe->clocks[MT8167_CLK_APLL12_DIV5],
				...
				struct snd_soc_dai *dai)
{
	...
	if (rate % 8000) {
		mt8167_afe_enable_apll_associated_cfg(afe,
			MT8167_AFE_APLL1);
		clk_set_parent(afe->clocks[MT8167_CLK_I2S5_M_SEL],
			afe->clocks[MT8167_CLK_AUD1]);
	} else {
		mt8167_afe_enable_apll_associated_cfg(afe,
			MT8167_AFE_APLL2);
...
static int mt8167_afe_tdm_in_trigger(struct snd_pcm_substream *substream,
	int cmd,
				struct snd_soc_dai *dai)
{
	struct snd_soc_pcm_runtime *rtd = substream->private_data;
	...
	case SNDRV_PCM_TRIGGER_RESUME:
		regmap_update_bits(afe->regmap, AFE_CONN_TDMIN_CON,
				   ...
				   AFE_CONN_TDMIN_O40_I40 |
				   AFE_CONN_TDMIN_O41_I41);

		regmap_update_bits(afe->regmap,
			AFE_HDMI_IN_2CH_CON0, 0x1, 0x1);

		/* enable tdm in */
		regmap_update_bits(afe->regmap,
		AFE_TDM_IN_CON1, 0x1, 0x1);
		return 0;
	case SNDRV_PCM_TRIGGER_STOP:
	case SNDRV_PCM_TRIGGER_SUSPEND:
		/* disable tdm in */
		regmap_update_bits(afe->regmap,
		AFE_TDM_IN_CON1, 0x1, 0);

		regmap_update_bits(afe->regmap,
			AFE_HDMI_IN_2CH_CON0, 0x1, 0);
		return 0;
	default:
		return -EINVAL;
	}
				...
				struct snd_soc_dai *dai)
{
	...
	ret = snd_pcm_hw_constraint_integer(runtime,
					    SNDRV_PCM_HW_PARAM_PERIODS);
	if (ret < 0) {
		dev_dbg(afe->dev, "snd_pcm_hw_constraint_integer failed\n");
		return ret;
	}
				  ...
				  struct snd_soc_dai *dai)
{
	...
	if (request_size > data->max_sram_size) {
		...
		if (afe->use_sramrom && data->max_sram_size == 55 * 1024) {
			dma_buf->area = ((unsigned char *)afe->sram_address1) +
				   data->sram_offset;
			dma_buf->addr = afe->sram_phy_address1 +
					 data->sram_offset;
	...
	if (data->id == MT8167_AFE_MEMIF_DAI ||
		...
		case 32000:
			...
			break;
		default:
			dev_dbg(afe->dev, "%s %s rate %u not supported\n",
...
static int mt8167_afe_dais_trigger(struct snd_pcm_substream *substream,
	int cmd,
	struct snd_soc_dai *dai)
{
	struct snd_soc_pcm_runtime *rtd = substream->private_data;
				  ...
				  struct snd_soc_dai *dai)
{
	struct snd_soc_pcm_runtime *rtd = substream->private_data;
	struct mtk_afe *afe = snd_soc_platform_get_drvdata(rtd->platform);
	struct mt8167_afe_be_dai_data *be = &afe->be_data[dai->id -
						 MT8167_AFE_BACKEND_BASE];
	const unsigned int stream = substream->stream;
...
static const struct snd_kcontrol_new daibt_mux =
	SOC_DAPM_ENUM("DAIBT Source", daibt_mux_enum);
...
static irqreturn_t mt8167_afe_irq_handler(int irq, void *dev_id)
{
	...
	if (ret) {
		dev_dbg(afe->dev, "%s memif status err\n", __func__);
		reg_value = AFE_IRQ_STATUS_BITS;
		goto err_irq;
	}
...
static int mt8167_afe_pcm_dev_probe(struct platform_device *pdev)
{
	...
	afe->backup_regs =
		kcalloc(ARRAY_SIZE(mt8167_afe_backup_list),
	...
	if (!irq_id) {
		dev_dbg(afe->dev, "np %s no irq\n", np->name);
		return -ENXIO;
	}

	ret = devm_request_irq(afe->dev, irq_id,
		mt8167_afe_irq_handler, 0, "Afe_ISR_Handle", (void *)afe);
	if (ret) {
		dev_dbg(afe->dev, "could not request_irq\n");
		return ret;
	}
	...
	if (!IS_ERR(afe->sram_address)) {
		...
		/*dev_info(&pdev->dev, "SRAM initialized, addr(0x%x),*/
	...
	if (!IS_ERR(afe->sram_address1)) {
		...
		/*dev_info(&pdev->dev, "SRAMROM initialized, addr(0x%x),*/
		/*size(0x%x)\n", afe->sram_phy_address1, afe->sram_size1);*/
	}
#endif

	afe->mrgrx_priv = devm_kzalloc(&pdev->dev, sizeof(struct
				 mt8167_pcm_mrgrx_priv), GFP_KERNEL);
	if (!(afe->mrgrx_priv))
		return -ENOMEM;
	...
	if (!IS_ERR(afe->sram_address1)) {
		...
		afe->sram_size1 = resource_size(res);
		afe->use_sramrom = true;
		/*dev_dbg(&pdev->dev, "SRAMROM initialized, addr(0x%x), */
		/*size(0x%x)\n", afe->sram_phy_address1, afe->sram_size1); */
	}
#endif
	...
	if	(ret) {
		dev_dbg(afe->dev,
		"%s mt8167_afe_init_audio_clk fail\n",
		__func__);
		return ret;
	}
	...
	if (of_property_read_u32(np, "mediatek,tdm-out-mode",
		&afe->tdm_out_mode))
	...
	if	(of_property_read_u32(np, "mediatek,awb-irq-mode",
		&afe->awb_irq_mode))
		afe->awb_irq_mode = MT8167_AFE_IRQ_2;

	if	(afe->awb_irq_mode != memif_data[MT8167_AFE_MEMIF_AWB].irq_mode)
		mt8167_afe_set_memif_irq_by_mode(
			&memif_data[MT8167_AFE_MEMIF_AWB],
			afe->awb_irq_mode);

	if	(of_property_read_u32(np, "mediatek,dai-irq-mode",
		&afe->dai_irq_mode))
		afe->dai_irq_mode = MT8167_AFE_IRQ_2;

	if	(afe->dai_irq_mode != memif_data[MT8167_AFE_MEMIF_DAI].irq_mode)
		mt8167_afe_set_memif_irq_by_mode(
			&memif_data[MT8167_AFE_MEMIF_DAI],
			afe->dai_irq_mode);
