	...
	size_t count, loff_t *pos)
{
	...
	for (i = 0; i < ARRAY_SIZE(afe_dump_regs); i++) {
		if (regmap_read(afe->regmap,
			afe_dump_regs[i].offset,
			&reg_value))
...
static ssize_t mt8167_afe_write_file(struct file *file,
...
static ssize_t mt8167_afe_hdmi_read_file(struct file *file,
...
static ssize_t mt8167_afe_tdm_in_read_file(struct file *file,
	char __user *user_buf,
	size_t count, loff_t *pos)
{
	struct mtk_afe *afe = file->private_data;
...
void mt8167_afe_init_debugfs(struct mtk_afe *afe)
{
	...
	for (i = 0; i < ARRAY_SIZE(afe_debug_fs); i++) {
		...
		if (!afe->debugfs_dentry[i])
			dev_dbg(afe->dev, "%s failed to create %s debugfs file\n",
