...
#define AFE_PREFETCH_BUFFER_SIZE 8     /* 8 bytes */
	...
	struct snd_soc_dai *dai)
{
	...
	case SNDRV_PCM_TRIGGER_RESUME:
		...
		ret = mtk_memif_set_enable(afe, dai_id);
		if (ret) {
		...
		/* for capture, wait for delaytime to */
		/* make prefetch buffer full */
		if (substream->stream == SNDRV_PCM_STREAM_CAPTURE) {
			delayframes = AFE_PREFETCH_BUFFER_SIZE /
			  (runtime->channels * (runtime->sample_bits / 8));
			delaytime = (1000000000 / runtime->rate) * delayframes;
	...
	case SNDRV_PCM_TRIGGER_SUSPEND:
		...
		ret = mtk_memif_set_disable(afe, dai_id);
		if (ret) {
			dev_info(afe->dev, "%s(), id %d, memif enable, ret %d\n",
				__func__, dai_id, ret);
		}
