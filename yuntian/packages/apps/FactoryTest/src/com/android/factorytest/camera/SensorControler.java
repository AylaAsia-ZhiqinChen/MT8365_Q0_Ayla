...
public class SensorControler implements IActivityLifiCycle, SensorEventListener {
    ...
    private SensorControler(Context mContext) {
        mSensorManager = (SensorManager) mContext.getSystemService(Activity.SENSOR_SERVICE);
        mSensor = mSensorManager.getDefaultSensor(Sensor.TYPE_ACCELEROMETER);// TYPE_GRAVITY
    }

    public static SensorControler getInstance(Context mContext) {
        if (mInstance == null) {
            mInstance = new SensorControler(mContext);
        }
        return mInstance;
    }
    ...
    public void onSensorChanged(SensorEvent event) {
        ...
        if (event.sensor.getType() == Sensor.TYPE_ACCELEROMETER) {
            ...
            if (STATUE != STATUS_NONE) {
                ...
                Log.i(TAG, "pX:" + px + "  pY:" + py + "  pZ:" + pz + "    stamp:"
                        + stamp + "  second:" + second);
